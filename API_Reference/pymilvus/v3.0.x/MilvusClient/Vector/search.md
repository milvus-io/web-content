...
- **group_by** (*[GroupBy](GroupBy.md) | None*) -
...
- **order_by_fields** (*list[dict] | None*) -
    ...
    Milvus applies multiple order-by fields in the order that you specify. For entities with the same values in all specified order-by fields, Milvus keeps the original similarity-score order.

    In a grouping search, Milvus orders groups by the specified scalar field value of each group's top entity. The `limit` parameter still controls the number of groups, and `group_size` controls the number of entities per group.
