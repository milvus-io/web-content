...
This operation grants a privilege to a role on an arbitrary object. The object is identified by its type and name, for example a `"Collection"` named `"my_collection"`, a `"Database"` named `"my_db"`, or the `"Global"` scope. Read this page for more about [users and roles](https://milvus.io/docs/users_and_roles.md).
...
- `WithPrivilege(const std::string& privilege)`
    ...
    Sets the name of the privilege to assign to the role.
