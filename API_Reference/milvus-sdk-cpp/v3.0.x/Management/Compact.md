...
- `WithTargetSize(int64_t target_size)`
    ...
    Sets the target segment size, expressed in the unit returned by `TargetSizeUnit()`. Use values greater than 0 to guide output segment sizing.

- `WithTargetSizeUnit(const std::string& unit)`

    Sets the unit of the target segment size. Supported values: `"b"`, `"kb"`, `"mb"`, `"gb"`.
