...
- `targetSize(Long targetSize)`
    ...
    The target segment size, expressed in the unit configured by `targetSizeUnit`. `null` means the server default.

- `targetSizeUnit(String targetSizeUnit)`

    The unit of the `targetSize` value, e.g. `"mb"`. Defaults to `"mb"`.
