...
title: Installer Milvus Cluster avec Helm
---
...
<li><p>Avant d’installer Milvus, il est recommandé d’utiliser <a href="https://milvus.io/tools/sizing">l’outil de dimensionnement Milvus (Milvus Sizing Tool</a> ) pour estimer la configuration matérielle requise en fonction de la taille de vos données. Cela permet de garantir des performances optimales et une allocation optimale des ressources pour votre installation Milvus.</p></li>
</ul>
...
<button class="copy-code-btn"></button></code></pre>
<p>Vous pouvez à tout moment exécuter cette commande pour récupérer les derniers graphiques Helm de Milvus.</p>
...
<p>La commande suivante déploie un cluster Milvus avec des paramètres optimisés pour la version v3.0-beta, en utilisant Woodpecker comme file d’attente de messages recommandée :</p>
<pre><code translate="no" class="language-bash">helm install my-release zilliztech/milvus \
  --<span class="hljs-built_in">set</span> image.all.tag=v3.0-beta \
  --<span class="hljs-built_in">set</span> woodpecker.enabled=<span class="hljs-literal">true</span> \
...
<li><strong>Composants fusionnés</strong>: <strong>le nœud d'index</strong> et <strong>le nœud de données</strong> sont regroupés en un seul <strong>nœud de données</strong></li>
</ul>
...
<li>Consultez <a href="https://milvus.io/docs/system_configuration.md">la liste de contrôle des configurations système de Milvus</a> pour les options de configuration avancées</li>
</ul>
<div class="alert note">
...
<p>Pour plus d’informations, consultez <a href="https://artifacthub.io/packages/helm/milvus/milvus">le Helm Chart Milvus</a> et <a href="https://helm.sh/docs/">la documentation Helm</a>.</p>
</div>
<h3 id="2-Check-Milvus-cluster-status" class="common-anchor-header">2. Vérifiez l'état du cluster Milvus<button data-href="#2-Check-Milvus-cluster-status" class="anchor-icon" translate="no">
    ...
    </button></h3><p>Vérifiez que votre déploiement a bien abouti en contrôlant l'état des pods :</p>
<pre><code translate="no" class="language-bash">kubectl get pods
<button class="copy-code-btn"></button></code></pre>
<p><strong>Attendez que tous les pods affichent le statut « Running ».</strong> Avec la configuration v3.0-beta, vous devriez voir des pods similaires à ceux-ci :</p>
...
<h3 id="3-Connect-to-Milvus" class="common-anchor-header">3. Se connecter à Milvus<button data-href="#3-Connect-to-Milvus" class="anchor-icon" translate="no">
    ...
    </button></h3><p>Pour vous connecter à votre cluster Milvus depuis l’extérieur de Kubernetes, vous devez configurer la redirection de ports.</p>
...
<ul>
<li><strong>Attribuer automatiquement un port local</strong>: utilisez <code translate="no">:19530</code> au lieu de <code translate="no">27017:19530</code> pour laisser kubectl choisir un port disponible</li>
...
<h2 id="Optional-Update-Milvus-configurations" class="common-anchor-header">(Facultatif) Mettre à jour les configurations de Milvus<button data-href="#Optional-Update-Milvus-configurations" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<p>La procédure suivante part du principe que vous souhaitez activer l’ <code translate="no">proxy.http</code>.</p>
...
<li><p>Appliquez le fichier d'<code translate="no">values.yaml</code>.</p></li>
</ol>
...
<button class="copy-code-btn"></button></code></pre>
<p>Vous pouvez désormais accéder à l’interface Web Milvus à l’adresse <code translate="no">http://localhost:27018</code>.</p>
...
<h3 id="2-Download-image-pulling-script" class="common-anchor-header">2. Télécharger le script de récupération d’images<button data-href="#2-Download-image-pulling-script" class="anchor-icon" translate="no">
    ...
    </button></h3><p>Le script de récupération d’images est écrit en Python. Vous devez télécharger le script ainsi que ses dépendances dans le fichier <code translate="no">requirement.txt</code>.</p>
...
<h2 id="Upgrade-running-Milvus-cluster" class="common-anchor-header">Mise à niveau d'un cluster Milvus en cours d'exécution<button data-href="#Upgrade-running-Milvus-cluster" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<div class="alert note">
<p>Storage V3 est désactivé par défaut. Activez-le avant d’utiliser les fonctionnalités qui en dépendent. Pour connaître les prérequis et les considérations de compatibilité, consultez <a href="/docs/fr/storage-v3.md">Storage V3</a>.</p>
</div>
...
<ul>
<li><p>Consulter <a href="/docs/fr/quickstart.md">Hello Milvus</a> pour découvrir les capacités de Milvus.</p></li>
