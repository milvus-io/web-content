...
<h2 id="Overview" class="common-anchor-header">Présentation<button data-href="#Overview" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Helm utilise un format de packaging appelé « charts ». Un chart est un ensemble de fichiers décrivant un ensemble de ressources Kubernetes associées. Milvus fournit un ensemble de charts pour vous aider à déployer ses dépendances et ses composants.</p>
...
<li><p><a href="/docs/fr/prerequisite-helm.md#How-can-I-start-a-K8s-cluster-locally-for-test-purposes">Créez un cluster K8s</a>.</p></li>
...
<h2 id="Install-Milvus-Helm-Chart" class="common-anchor-header">Installer le Helm Chart Milvus<button data-href="#Install-Milvus-Helm-Chart" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>Avant d’installer les Helm Charts Milvus, vous devez ajouter le référentiel Helm de Milvus.</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>Vous pouvez à tout moment exécuter cette commande pour récupérer les derniers charts Helm de Milvus.</p>
<h2 id="Online-install" class="common-anchor-header">Installation en ligne<button data-href="#Online-install" class="anchor-icon" translate="no">
    ...
    </button></h3><p>Une fois le chart Helm installé, vous pouvez démarrer Milvus sur Kubernetes. Cette section vous guide tout au long du déploiement d’un cluster Milvus.</p>
...
<button class="copy-code-btn"></button></code></pre>
<p><strong>Remarque</strong>: le mode autonome utilise Woodpecker comme file d’attente de messages par défaut et active le composant Streaming Node. Les déploiements autonomes exécutent Woodpecker <strong>intégré</strong> au pod Milvus ; le <strong>service</strong> Woodpecker dédié (pods séparés) est utilisé uniquement pour les déploiements <strong>distribués/en cluster</strong>. Pour plus de détails, consultez la <a href="/docs/fr/architecture_overview.md">Présentation de l’architecture</a> et la section <a href="/docs/fr/woodpecker.md">Woodpecker</a>.</p>
</div>
<p><strong>Déployer un cluster Milvus :</strong></p>
<div class="alert note">
<p>Pour le mode de service Woodpecker, nous vous recommandons d’utiliser la prochaine version Milvus 3.0.1 ou une version ultérieure avec Woodpecker v0.1.37 ou une version plus récente afin de bénéficier des optimisations en matière de nettoyage par compaction et de validation groupée.</p>
</div>
<p>La commande suivante déploie un cluster Milvus avec des paramètres optimisés pour la version 3.0.1, en utilisant Woodpecker comme file d’attente de messages recommandée :</p>
<pre><code translate="no" class="language-bash">helm install my-release zilliztech/milvus \
  --<span class="hljs-built_in">set</span> image.all.tag=v3.0.1 \
  --<span class="hljs-built_in">set</span> woodpecker.enabled=<span class="hljs-literal">true</span> \
...
<button class="copy-code-btn"></button></code></pre>
<p><strong>Fonctionnalité de cette commande :</strong></p>
<ul>
<li>Utilise <strong>Woodpecker</strong> comme file d’attente de messages (recommandé pour réduire la maintenance)</li>
<li>Exécute <strong>Woodpecker en tant que service dédié</strong> (un StatefulSet distinct), et non intégré au nœud de streaming</li>
...
<div class="alert note">
<p><strong>Changements d'architecture dans Milvus 2.6.x :</strong></p>
<ul>
<li><strong>File d’attente de messages</strong>: <strong>Woodpecker</strong> est désormais recommandé (réduit la maintenance de l’infrastructure par rapport à Pulsar)</li>
<li><strong>Nouveau composant</strong>: <strong>le nœud de streaming</strong> est introduit et activé par défaut</li>
<li><strong>Composants fusionnés</strong>: <strong>le nœud d'index</strong> et <strong>le nœud de données</strong> sont combinés en un seul <strong>nœud de données</strong></li>
</ul>
<p>Pour plus de détails sur l'architecture, consultez la <a href="/docs/fr/architecture_overview.md">présentation</a> de <a href="/docs/fr/architecture_overview.md">l'architecture</a>.</p>
</div>
<p><strong>Files d’attente de messages alternatives :</strong> pour effectuer un déploiement avec Pulsar, Kafka ou RocksMQ à la place de Woodpecker, consultez la section « <a href="#Optional-dependencies">Dépendances facultatives</a> ».</p>
<p><strong>Étapes suivantes :</strong>
La commande ci-dessus déploie Milvus avec les configurations recommandées. Pour une utilisation en production :</p>
<ul>
<li>Utilisez <a href="https://milvus.io/tools/sizing">l’outil de dimensionnement Milvus</a> pour optimiser les paramètres en fonction du volume de vos données</li>
<li>Consultez <a href="https://milvus.io/docs/system_configuration.md">la liste de contrôle des configurations système de Milvus</a> pour connaître les options de configuration avancées</li>
</ul>
<div class="alert note">
<p><strong>Remarques importantes :</strong></p>
<ul>
<li><strong>Nommage des versions</strong>: utilisez uniquement des lettres, des chiffres et des tirets (les points ne sont pas autorisés)</li>
</ul>
<p>Pour plus d’informations, consultez <a href="https://artifacthub.io/packages/helm/milvus/milvus">le Helm Chart de Milvus</a> et <a href="https://helm.sh/docs/">la documentation Helm</a>.</p>
</div>
<h3 id="2-Check-Milvus-cluster-status" class="common-anchor-header">2. Vérifiez l'état du cluster Milvus<button data-href="#2-Check-Milvus-cluster-status" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Vérifiez que votre déploiement a bien abouti en vérifiant l’état des pods :</p>
<pre><code translate="no" class="language-bash">kubectl get pods
<button class="copy-code-btn"></button></code></pre>
<p><strong>Attendez que tous les pods affichent le statut « Running ».</strong> Avec la configuration v3.0.1, vous devriez voir des pods similaires à ceux-ci :</p>
...
<button class="copy-code-btn"></button></code></pre>
<p><strong>Éléments clés à vérifier :</strong></p>
<ul>
<li><strong>Composants Milvus</strong>: <code translate="no">mixcoord</code>, <code translate="no">datanode</code>, <code translate="no">querynode</code>, <code translate="no">proxy</code>, <code translate="no">streamingnode</code></li>
<li><strong>Dépendances</strong>: <code translate="no">etcd</code> (métadonnées), <code translate="no">minio</code> (stockage d’objets), <code translate="no">woodpecker</code> (file d’attente de messages)</li>
</ul>
<div class="alert note">
<p>Avec <code translate="no">streaming.woodpecker.embedded=false</code>, Woodpecker s’exécute en tant que <strong>StatefulSet dédié</strong> (<code translate="no">my-release-milvus-woodpecker</code>, 4 répliques par défaut — un quorum de 3 nœuds plus un nœud de secours pour la tolérance aux pannes ; ne définissez pas <code translate="no">woodpecker.replicaCount</code> sur une valeur inférieure à 3) avec en avant un service sans interface graphique, utilisant MinIO comme backend de stockage — le cluster dispose donc d’un ensemble de pods <code translate="no">woodpecker</code> distinct, séparé du nœud de streaming.</p>
</div>
<p>Vous pouvez également accéder à <strong>l’interface Web de Milvus</strong> à l’adresse <code translate="no">http://127.0.0.1:9091/webui/</code> une fois la redirection de port configurée (voir l’étape suivante). Pour plus de détails, consultez la documentation relative à <a href="/docs/fr/milvus-webui.md">l’interface Web de Milvus</a>.</p>
<h3 id="3-Connect-to-Milvus" class="common-anchor-header">3. Se connecter à Milvus<button data-href="#3-Connect-to-Milvus" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Pour vous connecter à votre cluster Milvus depuis l’extérieur de Kubernetes, vous devez configurer la redirection de port.</p>
<p><strong>Configurer la redirection de port :</strong></p>
<pre><code translate="no" class="language-bash">kubectl port-forward service/my-release-milvus 27017:19530
<button class="copy-code-btn"></button></code></pre>
<p>Cette commande redirige votre port local <code translate="no">27017</code> vers le port Milvus <code translate="no">19530</code>. Vous devriez voir s'afficher :</p>
...
<ul>
<li><strong>Connexion locale</strong>: <code translate="no">localhost:27017</code></li>
<li><strong>Port par défaut de Milvus</strong>: <code translate="no">19530</code></li>
</ul>
<div class="alert note">
<p><strong>Options de redirection de port :</strong></p>
<ul>
<li><strong>Attribution automatique du port local</strong>: utilisez <code translate="no">:19530</code> au lieu de <code translate="no">27017:19530</code> pour laisser kubectl choisir un port disponible</li>
<li><strong>Écouter sur toutes les interfaces</strong>: Ajoutez <code translate="no">--address 0.0.0.0</code> pour autoriser les connexions depuis d'autres machines :
...
<p><strong>Laissez ce terminal ouvert</strong> pendant que vous utilisez Milvus. Vous pouvez désormais vous connecter à Milvus à l’aide de n’importe quel SDK Milvus à l’adresse <code translate="no">localhost:27017</code>.</p>
...
<ol>
<li><p>Créez un fichier <code translate="no">values.yaml</code> contenant les configurations souhaitées.</p>
<p>La procédure suivante part du principe que vous souhaitez activer la fonctionnalité « <code translate="no">proxy.http</code> ».</p>
...
<p>Pour connaître les éléments de configuration concernés, reportez-vous à la section « <a href="/docs/fr/system_configuration.md">Configuration du système</a> ».</p></li>
<li><p>Appliquez le fichier ` <code translate="no">values.yaml</code> `.</p></li>
</ol>
...
<button class="copy-code-btn"></button></code></pre>
<ol>
<li><p>Vérifiez les configurations mises à jour.</p>
<pre><code translate="no" class="language-shell">helm get values my-release
<button class="copy-code-btn"></button></code></pre>
<p>La sortie devrait afficher les configurations mises à jour.</p></li>
</ol>
<h2 id="Access-Milvus-WebUI" class="common-anchor-header">Accéder à l'interface Web de Milvus<button data-href="#Access-Milvus-WebUI" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Milvus est fourni avec un outil graphique intégré appelé Milvus WebUI, accessible via votre navigateur. Milvus WebUI améliore la visibilité du système grâce à une interface simple et intuitive. Vous pouvez utiliser Milvus WebUI pour consulter les statistiques et les métriques des composants et des dépendances de Milvus, vérifier les détails de la base de données et des collections, et afficher la liste détaillée des configurations de Milvus. Pour plus d'informations sur l'interface Web de Milvus, consultez la page <a href="/docs/fr/milvus-webui.md">Milvus WebUI</a></p>
...
<h3 id="1-Get-Milvus-manifest" class="common-anchor-header">1. Récupérer le manifeste Milvus<button data-href="#1-Get-Milvus-manifest" class="anchor-icon" translate="no">
    ...
    </button></h3><p>Exécutez la commande suivante pour récupérer le manifeste Milvus.</p>
...
<h3 id="2-Download-image-pulling-script" class="common-anchor-header">2. Télécharger le script de récupération d’images<button data-href="#2-Download-image-pulling-script" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Le script de récupération des images est développé en Python. Vous devez télécharger le script ainsi que ses dépendances dans le fichier <code translate="no">requirement.txt</code>.</p>
...
<button class="copy-code-btn"></button></code></pre>
<h3 id="3-Pull-and-save-images" class="common-anchor-header">3. Récupérer et enregistrer les images<button data-href="#3-Pull-and-save-images" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Exécutez la commande suivante pour récupérer et enregistrer les images requises.</p>
...
<h3 id="4-Load-images" class="common-anchor-header">4. Charger les images<button data-href="#4-Load-images" class="anchor-icon" translate="no">
    ...
    </button></h3><p>Vous pouvez désormais charger les images sur les hôtes de l’environnement à accès restreint comme suit :</p>
...
<button class="copy-code-btn"></button></code></pre>
<h3 id="5-Deploy-Milvus" class="common-anchor-header">5. Déployer Milvus<button data-href="#5-Deploy-Milvus" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<button class="copy-code-btn"></button></code></pre>
<p>Jusqu’à présent, vous pouvez suivre les étapes <a href="#2-Check-Milvus-cluster-status">2</a> et <a href="#3-Connect-to-Milvus">3</a> de l’installation en ligne pour vérifier l’état du cluster et rediriger un port local vers Milvus.</p>
<h2 id="Upgrade-running-Milvus-cluster" class="common-anchor-header">Mise à jour d’un cluster Milvus en cours d’exécution<button data-href="#Upgrade-running-Milvus-cluster" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<h2 id="Optional-dependencies" class="common-anchor-header">Dépendances facultatives<button data-href="#Optional-dependencies" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Ce déploiement utilise <strong>Woodpecker</strong> comme file d’attente de messages, <strong>etcd</strong> pour les métadonnées et <strong>MinIO</strong> pour le stockage d’objets. Pour utiliser une autre file d’attente de messages ou connecter un stockage d’objets / des métadonnées externes, consultez :</p>
<ul>
...
<div class="alert note">
<p>Storage V3 est désactivé par défaut. Activez-le avant d’utiliser les fonctionnalités qui en dépendent. Pour connaître les exigences et les considérations de compatibilité, consultez la page <a href="/docs/fr/storage-v3.md">Storage V3</a>.</p>
</div>
...
<ul>
<li><p>Consulter la page « <a href="/docs/fr/quickstart.md">Hello Milvus</a> » pour découvrir les capacités de Milvus.</p></li>
...
<li><p>Découvrez <a href="/docs/fr/birdwatcher_overview.md">Birdwatcher</a>, un outil open source dédié au débogage de Milvus et aux mises à jour dynamiques de configuration.</p></li>
