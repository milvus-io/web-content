...
<p>Lors de l’insertion d’une entité dans une collection, celle-ci ne peut être ajoutée avec succès que si elle contient tous les champs définis dans le schéma. L’entité insérée sera placée dans une partition nommée <strong>_default</strong>, dans l’ordre d’insertion. À condition qu’une partition spécifique existe, vous pouvez également insérer des entités dans cette partition en spécifiant son nom dans la requête d’insertion.</p>
