...
<h3 id="Basic-Operators-in-JSON-Fields" class="common-anchor-header">Opérateurs de base dans les champs JSON<button data-href="#Basic-Operators-in-JSON-Fields" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Le tableau suivant suppose que la valeur d'un champ JSON nommé <code translate="no">json_key</code> a une clé nommée <code translate="no">A</code>. Utilisez-le comme référence lorsque vous construisez des expressions booléennes à l'aide de clés de champ JSON.</p>
<table>
<thead>
...
<tr><td><strong>ou (||)</strong></td><td><code translate="no">‘json_field[“A”] > 1 || json_field[“A”] < 3’</code></td><td>Cette expression est considérée comme vraie si la valeur de <code translate="no">json_field[&quot;A&quot;]</code> est supérieure à <code translate="no">1</code> ou inférieure à <code translate="no">3</code>.</td></tr>
...
<h3 id="Advanced-Operators" class="common-anchor-header">Opérateurs avancés<button data-href="#Advanced-Operators" class="anchor-icon" translate="no">
    ...
    </button></h3><p>Les opérateurs suivants sont spécifiques aux champs JSON :</p>
<ul>
