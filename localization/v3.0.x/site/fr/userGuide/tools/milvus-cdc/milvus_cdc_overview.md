...
<h2 id="Planned-Switchover-vs-Failover" class="common-anchor-header">Basculement planifié vs basculement de secours<button data-href="#Planned-Switchover-vs-Failover" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Milvus CDC propose deux méthodes pour transférer le trafic du service du serveur principal actuel vers un cluster de secours.</p>
<table>
<thead>
...
<h2 id="Bulk-Import-in-CDC-Replication" class="common-anchor-header">Importation en masse dans la réplication CDC<button data-href="#Bulk-Import-in-CDC-Replication" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Dans une topologie de réplication CDC, l’importation en masse doit utiliser le mode de validation en deux phases (2PC) avec l’option « <code translate="no">auto_commit=false</code> ». Exécutez l’importation et la validation uniquement sur le cluster principal, et assurez-vous que les fichiers d’importation sont disponibles à la fois sur le cluster principal et sur le cluster de secours. Pour plus de détails, reportez-vous à <a href="/docs/fr/bulk-import-in-cdc-replication.md">la section Importation en masse dans la réplication CDC</a>.</p>
