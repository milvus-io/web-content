...
<h2 id="Build-index" class="common-anchor-header">Créer un index<button data-href="#Build-index" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Pour créer un index « <code translate="no">SPARSE_INVERTED_INDEX</code> » sur un champ de vecteurs creux dans Milvus, utilisez la méthode « <code translate="no">add_index()</code> » et spécifiez les paramètres « <code translate="no">index_type</code> », « <code translate="no">metric_type</code> » et « index ».</p>
...
<table>
   ...
   <tr>
     ...
     <td><p>Réglez ce paramètre en fonction de la densité et du niveau de bruit de vos vecteurs de requête.</p><p>Ce paramètre contrôle la proportion de valeurs de faible amplitude écartées lors de la recherche. Augmenter cette valeur (par exemple, à <code translate="no">0.2</code>) peut réduire le bruit et concentrer la recherche sur des composantes plus significatives, ce qui peut améliorer la précision et l'efficacité. Cependant, écarter davantage de valeurs peut également réduire le rappel en excluant des signaux potentiellement pertinents. Choisissez une valeur qui offre un bon équilibre entre rappel et précision pour votre charge de travail.</p></td>
   </tr>
</table>
