...
title: Présentation de la fonction d'intégrationCompatible with Milvus 2.6.x
summary: >-
  Le module « Function » de Milvus vous permet de transformer des données
...
beta: Milvus 2.6.x
---
<h1 id="Embedding-Function-Overview" class="common-anchor-header">Présentation de la fonction d'intégration<span class="beta-tag" style="background-color:rgb(0, 179, 255);color:white" translate="no">Compatible with Milvus 2.6.x</span><button data-href="#Embedding-Function-Overview" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<li><p>Le module Function traite uniquement les champs explicitement définis dans le schéma de la collection ; il ne génère pas d’embeddings pour les champs dynamiques.</p></li>
<li><p>Les champs d’entrée à encoder doivent être de type « <code translate="no">VARCHAR</code> ».</p></li>
<li><p>Le module « Function » peut intégrer un champ d’entrée pour :</p>
<ul>
<li><p><code translate="no">FLOAT_VECTOR</code></p></li>
<li><p><code translate="no">INT8_VECTOR</code></p></li>
</ul>
<p>Les conversions vers les types « <code translate="no">BINARY_VECTOR</code> », « <code translate="no">FLOAT16_VECTOR</code> » ou « <code translate="no">BFLOAT16_VECTOR</code> » ne sont pas prises en charge.</p></li>
</ul>
<h2 id="Supported-embedding-service-providers" class="common-anchor-header">Fournisseurs de services d’intégration pris en charge<button data-href="#Supported-embedding-service-providers" class="anchor-icon" translate="no">
   ...
   <tr>
     ...
     <td><p>Basée sur le déploiement</p></td>
     <td><p><code translate="no">FLOAT_VECTOR</code></p></td>
     <td><p>Clé API</p></td>
   </tr>
   <tr>
   ...
   <tr>
     ...
     <td><p><code translate="no">FLOAT_VECTOR</code></p></td>
     <td><p>Compte de service GCP – Identifiants JSON</p></td>
   </tr>
   <tr>
   ...
   <tr>
     <td><p><a href="/docs/fr/hugging-face-tei.md">Hugging Face TEI</a></p></td>
     <td><p>Tout modèle pris en charge par TEI</p></td>
     <td><p><code translate="no">FLOAT_VECTOR</code></p></td>
     <td><p>Clé API facultative</p></td>
   </tr>
   <tr>
     <td><p><a href="/docs/fr/hugging-face.md">Hugging Face</a></p></td>
     <td><p>Modèles hébergés sur <code translate="no">hf-inference</code> pour l'extraction de caractéristiques</p></td>
     <td><p><code translate="no">FLOAT_VECTOR</code></p></td>
     <td><p>Clé API</p></td>
   </tr>
</table>
<h2 id="How-it-works" class="common-anchor-header">Fonctionnement<button data-href="#How-it-works" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Le diagramme suivant illustre le fonctionnement de la fonction dans Milvus.</p>
<ol>
...
<li><p><strong>Génération d’embeddings</strong>: le module « Function » de Milvus appelle automatiquement le fournisseur de modèles configuré pour convertir les données brutes en embeddings vectoriels.</p></li>
<li><p><strong>Stockage des représentations</strong>: les représentations obtenues sont stockées dans des champs vectoriels explicitement définis au sein des collections Milvus.</p></li>
<li><p><strong>Texte de requête</strong>: les utilisateurs soumettent des requêtes textuelles à Milvus.</p></li>
<li><p><strong>Recherche sémantique</strong>: Milvus convertit en interne les requêtes en représentations vectorielles, effectue des recherches de similarité par rapport aux représentations stockées et récupère les résultats pertinents.</p></li>
<li><p><strong>Retour des résultats</strong>: Milvus renvoie les résultats les plus pertinents à l’application.</p></li>
</ol>
<p><span class="img-wrapper">
   ...
   <span>Présentation de la fonction d’embedding</span>
...
<p>Pour plus de détails sur la configuration des identifiants via des variables d’environnement, consultez la documentation du fournisseur de service d’intégration (par exemple, <a href="/docs/fr/openai.md">OpenAI</a> ou <a href="/docs/fr/azure-openai.md">Azure OpenAI</a>).</p></li>
</ul>
<p>Le schéma suivant illustre le processus de configuration des identifiants via le fichier de configuration Milvus (<code translate="no">milvus.yaml</code>), puis l'appel de la fonction au sein de Milvus.</p>
<p><span class="img-wrapper">
   ...
   <span>Configuration des identifiants dans Overflow</span>
  
 </span></p>
<h3 id="Step-1-Add-credentials-to-Milvus-configuration-file" class="common-anchor-header">Étape 1 : Ajouter les informations d’authentification au fichier de configuration de Milvus<button data-href="#Step-1-Add-credentials-to-Milvus-configuration-file" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Dans votre fichier <code translate="no">milvus.yaml</code>, modifiez le bloc <code translate="no">credential</code> en y ajoutant les entrées correspondant à chaque fournisseur auquel vous devez accéder :</p>
...
<h3 id="Step-2-Configure-provider-settings" class="common-anchor-header">Étape 2 : Configurer les paramètres du fournisseur<button data-href="#Step-2-Configure-provider-settings" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Dans le même fichier de configuration (<code translate="no">milvus.yaml</code>), modifiez le bloc <code translate="no">function</code> pour indiquer à Milvus quelle clé utiliser pour l’intégration des appels de service :</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>Pour plus d’informations sur la manière d’appliquer la configuration de Milvus, consultez la section « <a href="/docs/fr/dynamic_config.md">Configurer Milvus à la volée</a> ».</p>
<h2 id="Use-embedding-function" class="common-anchor-header">Utiliser la fonction d’intégration<button data-href="#Use-embedding-function" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Une fois les identifiants configurés dans votre fichier de configuration Milvus, suivez ces étapes pour définir et utiliser les fonctions d’intégration.</p>
<h3 id="Step-1-Define-schema-fields" class="common-anchor-header">Étape 1 : Définir les champs du schéma<button data-href="#Step-1-Define-schema-fields" class="anchor-icon" translate="no">
    ...
    </button></h3><p>Pour utiliser une fonction d’intégration, créez une collection avec un schéma spécifique. Ce schéma doit inclure au moins trois champs obligatoires :</p>
<ul>
<li><p><strong>Le champ principal</strong> qui identifie de manière unique chaque entité d’une collection.</p></li>
<li><p><strong>Un champ scalaire</strong> qui stocke les données brutes à intégrer.</p></li>
<li><p>Un <strong>champ vectoriel</strong> réservé au stockage des représentations vectorielles que la fonction générera pour le champ scalaire.</p></li>
</ul>
<p>L'exemple suivant définit un schéma comportant un champ scalaire <code translate="no">&quot;document&quot;</code> destiné à stocker des données textuelles et un champ vectoriel <code translate="no">&quot;dense&quot;</code> destiné à stocker les représentations qui seront générées par le module Function. N'oubliez pas de définir la dimension vectorielle (<code translate="no">dim</code>) afin qu'elle corresponde à la sortie du modèle de représentation que vous avez choisi.</p>
...
<button class="copy-code-btn"></button></code></pre>
<h3 id="Step-2-Add-embedding-function-to-schema" class="common-anchor-header">Étape 2 : Ajouter la fonction d’embedding au schéma<button data-href="#Step-2-Add-embedding-function-to-schema" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<table>
   ...
   <tr>
     ...
     <td><p>Identifiant unique de la fonction d’embedding au sein de Milvus.</p></td>
   ...
   <tr>
     ...
     <td><p>Champ scalaire contenant les données brutes à vectoriser. Actuellement, ce paramètre n’accepte qu’un seul nom de champ.</p></td>
     <td><p><code translate="no">["document"]</code></p></td>
   </tr>
   <tr>
     <td><p><code translate="no">output_field_names</code></p></td>
     <td><p>Champ vectoriel destiné au stockage des représentations générées. Actuellement, ce paramètre n’accepte qu’un seul nom de champ.</p></td>
   ...
   <tr>
     ...
     <td><p>Le fournisseur de modèle d’intégration.</p></td>
   ...
   <tr>
     ...
     <td><p>L'étiquette d'un identifiant défini dans la section de niveau supérieur « <code translate="no">credential:</code> » de <code translate="no">milvus.yaml</code>. </p><ul><li><p>Lorsqu’il est fourni, Milvus récupère la paire de clés ou le jeton API correspondant et signe la requête côté serveur.</p></li><li><p>Si ce paramètre est omis (<code translate="no">None</code>), Milvus utilise par défaut les informations d’authentification explicitement configurées pour le fournisseur de modèle cible dans <code translate="no">milvus.yaml</code>.</p></li><li><p>Si l'étiquette est inconnue ou si la clé référencée est manquante, l'appel échoue.</p></li></ul></td>
   ...
   <tr>
     ...
     <td><p>Identifiant au niveau de l’utilisateur permettant de suivre l’utilisation de l’API.</p></td>
     <td><p><code translate="no">"user123"</code></p></td>
   </tr>
</table>
<div class="alert note">
<p>Pour les collections comportant plusieurs champs scalaires nécessitant une conversion texte-vecteur, ajoutez des fonctions distinctes au schéma de la collection, en veillant à ce que chaque fonction possède un nom et une valeur « <code translate="no">output_field_names</code> » uniques.</p>
</div>
<h3 id="Step-3-Configure-index" class="common-anchor-header">Étape 3 : Configurer l’index<button data-href="#Step-3-Configure-index" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<pre><code translate="no" class="language-bash"><span class="hljs-comment"># restful</span>
<button class="copy-code-btn"></button></code></pre>
<h3 id="Step-4-Create-collection" class="common-anchor-header">Étape 4 : Créer une collection<button data-href="#Step-4-Create-collection" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<pre><code translate="no" class="language-bash"><span class="hljs-comment"># restful</span>
<button class="copy-code-btn"></button></code></pre>
<h3 id="Step-5-Insert-data" class="common-anchor-header">Étape 5 : Insérer les données<button data-href="#Step-5-Insert-data" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Une fois votre collection et votre index configurés, vous êtes prêt à insérer vos données brutes. Au cours de ce processus, il vous suffit de fournir le texte brut. Le module Function que nous avons défini précédemment génère automatiquement le vecteur clairsemé correspondant à chaque entrée de texte.</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#javascript">   NodeJS</a>
...
<pre><code translate="no" class="language-bash"><span class="hljs-comment"># restful</span>
<button class="copy-code-btn"></button></code></pre>
<p>Pour plus d’informations sur les opérations de recherche et de requête, consultez la section « <a href="/docs/fr/single-vector-search.md">Recherche vectorielle</a> et <a href="/docs/fr/get-and-scalar-query.md">requêtes</a> <a href="/docs/fr/single-vector-search.md">de base</a> ».</p>
<h2 id="FAQ" class="common-anchor-header">FAQ<button data-href="#FAQ" class="anchor-icon" translate="no">
    ...
    </button></h2><h3 id="Whats-the-difference-between-configuring-credentials-in-milvusyaml-vs-environment-variables" class="common-anchor-header">Quelle est la différence entre la configuration des identifiants dans le fichier milvus.yaml et celle via des variables d’environnement ?<button data-href="#Whats-the-difference-between-configuring-credentials-in-milvusyaml-vs-environment-variables" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Les deux méthodes fonctionnent, mais l’utilisation d’ <code translate="no">milvus.yaml</code> est l’approche recommandée, car elle permet une gestion centralisée des identifiants et une nomenclature cohérente pour tous les fournisseurs. Lorsque vous utilisez des variables d’environnement, les noms de variables varient en fonction du fournisseur de service d’encodage ; consultez donc la page dédiée à chaque fournisseur pour connaître les noms de variables d’environnement spécifiques requis (par exemple, <a href="/docs/fr/openai.md">OpenAI</a> ou <a href="/docs/fr/azure-openai.md">Azure OpenAI</a>).</p>
<h3 id="What-happens-if-I-dont-specify-a-credential-parameter-in-the-function-definition" class="common-anchor-header">Que se passe-t-il si je ne spécifie pas de paramètre d’identifiants dans la définition de la fonction ?<button data-href="#What-happens-if-I-dont-specify-a-credential-parameter-in-the-function-definition" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Milvus suit l’ordre de résolution des identifiants suivant :</p>
<ol>
<li>Tout d’abord, il recherche les identifiants par défaut configurés pour ce fournisseur dans le fichier ` <code translate="no">milvus.yaml</code> `</li>
<li>Si aucune information d’authentification par défaut n’existe dans milvus.yaml, il se rabat sur les variables d’environnement (si elles sont configurées)</li>
<li>Si ni les identifiants du fichier ` <code translate="no">milvus.yaml</code> ` ni les variables d’environnement ne sont configurés, Milvus génère une erreur</li>
</ol>
<h3 id="How-can-I-verify-that-embeddings-are-being-generated-correctly" class="common-anchor-header">Comment puis-je vérifier que les représentations sont générées correctement ?<button data-href="#How-can-I-verify-that-embeddings-are-being-generated-correctly" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Vous pouvez vérifier cela en :</p>
<ol>
<li>Interroger votre collection après l’insertion pour voir si le champ vectoriel contient des données</li>
