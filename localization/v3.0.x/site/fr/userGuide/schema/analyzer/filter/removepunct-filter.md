...
summary: >-
  Le filtre « removepunct » supprime les tokens contenant des signes de
  ponctuation ou des espaces. Avec les tokeniseurs qui conservent la ponctuation
  à l'intérieur des mots, il supprime l'intégralité du token concerné.
beta: Milvus 2.5.11+
---
<h1 id="Remove-Punct" class="common-anchor-header">Supprimer la ponctuation<span class="beta-tag" style="background-color:rgb(0, 179, 255);color:white" translate="no">Compatible with Milvus 2.5.11+</span><button data-href="#Remove-Punct" class="anchor-icon" translate="no">
    ...
    </button></h1><p>Le filtre « <code translate="no">removepunct</code> » supprime les tokens contenant des signes de ponctuation ou des espaces du flux de tokens. Utilisez-le lorsque vous souhaitez un traitement de texte plus propre, axé sur les mots de contenu significatifs plutôt que sur les signes de ponctuation.</p>
<div class="alert note">
<p>Ce filtre est particulièrement efficace avec les tokenizers « <code translate="no">jieba</code> », « <code translate="no">lindera</code> » et « <code translate="no">icu</code> », qui conservent la ponctuation sous forme de tokens distincts (par exemple, « <code translate="no">&quot;Hello!&quot;</code> » → « <code translate="no">[&quot;Hello&quot;, &quot;!&quot;]</code> »). Le tokenizer <code translate="no">standard</code> supprime la ponctuation lors de la tokenisation. Le tokenizer <code translate="no">whitespace</code> conserve la ponctuation, y compris celle contenue dans un token. Lorsqu’il est associé à <code translate="no">whitespace</code>, <code translate="no">removepunct</code> supprime le token entier s’il contient de la ponctuation ou des espaces ; il ne supprime pas les caractères individuels du token.</p>
</div>
<h2 id="Configuration" class="common-anchor-header">Configuration<button data-href="#Configuration" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Le filtre « <code translate="no">removepunct</code> » est intégré à Milvus. Pour l’utiliser, il suffit d’indiquer son nom dans la section « <code translate="no">filter</code> » du fichier « <code translate="no">analyzer_params</code> ».</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>Le filtre « <code translate="no">removepunct</code> » agit sur les termes générés par le tokenizer ; il doit donc être utilisé en combinaison avec un tokenizer.</p>
