---
id: regex-filter.md
title: Filtre « Regex Analyzer »Compatible with Milvus 2.5.11+
summary: >-
  Le filtre d'analyse d'expressions régulières conserve les tokens qui
  correspondent à une expression régulière et élimine les autres.
beta: Milvus 2.5.11+
---
<h1 id="Regex-Analyzer-Filter" class="common-anchor-header">Filtre « Regex Analyzer »<span class="beta-tag" style="background-color:rgb(0, 179, 255);color:white" translate="no">Compatible with Milvus 2.5.11+</span><button data-href="#Regex-Analyzer-Filter" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h1><p>Le filtre « <code translate="no">regex</code> » est un filtre d’expressions régulières : tout token produit par le tokeniseur n’est conservé que s’il correspond à l’expression que vous fournissez ; tout le reste est écarté.</p>
<div class="alert note">
<p>Cette page décrit le filtre « <code translate="no">regex</code> » dans le pipeline d’analyse. Il filtre les tokens produits par un tokeniseur et influe sur les termes générés lors de l’analyse de texte. Pour filtrer des entités à l’aide d’expressions scalaires telles que « <code translate="no">field =~ &quot;pattern&quot;</code> » ou « <code translate="no">field !~ &quot;pattern&quot;</code> » dans les recherches « <code translate="no">query</code> », « <code translate="no">search</code> » ou hybrides, reportez-vous à <a href="/docs/fr/pattern-matching.md">la section «Correspondance de motifs</a>».</p>
</div>
<h2 id="Configuration" class="common-anchor-header">Configuration<button data-href="#Configuration" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Le filtre « <code translate="no">regex</code> » est un filtre personnalisé de Milvus. Pour l’utiliser, spécifiez « <code translate="no">&quot;type&quot;: &quot;regex&quot;</code> » dans la configuration du filtre, ainsi qu’un paramètre « <code translate="no">expr</code> » permettant de définir les expressions régulières souhaitées.</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>Le filtre « <code translate="no">regex</code> » accepte les paramètres configurables suivants.</p>
<table>
   <tr>
     <th><p>Paramètre</p></th>
   ...
   <tr>
     ...
     <td><p>Modèle d’expression régulière appliqué à chaque token. Les tokens qui correspondent sont conservés ; ceux qui ne correspondent pas sont ignorés.
 Pour plus de détails sur la syntaxe des expressions régulières, reportez-vous à <a href="https://docs.rs/regex/latest/regex/#syntax">la section Syntaxe</a>.</p></td>
   </tr>
</table>
