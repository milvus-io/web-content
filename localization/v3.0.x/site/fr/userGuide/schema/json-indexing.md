...
summary: >-
  ...
  permet d'accélérer l'exécution des requêtes d'égalité, de plage et d'autres
  requêtes de filtrage portant sur ces chemins.
---
<h1 id="JSON-Indexing" class="common-anchor-header">Indexation JSON<button data-href="#JSON-Indexing" class="anchor-icon" translate="no">
    ...
    </button></h1><p>Les champs JSON offrent un moyen flexible de stocker des métadonnées structurées dans Milvus. Sans indexation, les requêtes sur les champs JSON nécessitent un balayage complet de la collection, ce qui ralentit le système à mesure que votre ensemble de données s'étoffe. L'indexation JSON crée un index sur un chemin spécifique au sein de vos données JSON, ce qui permet d'accélérer l'exécution des requêtes d'égalité, de plage et d'autres requêtes de filtrage sur ce chemin.</p>
...
<li><p>Les scénarios dans lesquels vous avez besoin d’un contrôle précis sur les clés indexées</p></li>
</ul>
...
<h3 id="Understand-cast-types" class="common-anchor-header">Comprendre les types de conversion<button data-href="#Understand-cast-types" class="anchor-icon" translate="no">
    ...
    </button></h3><p><code translate="no">json_cast_type</code> Il s'agit du type de données utilisé pour interpréter et indexer la valeur située à l'adresse <code translate="no">json_path</code>. Il diffère du type de schéma du champ : le champ reste un champ « <code translate="no">JSON</code> », mais chaque chemin indexé est traité comme un type spécifique (scalaire, tableau ou objet JSON).</p>
...
<li><p>Vous souhaitez forcer une disposition en bitmap au lieu de laisser <code translate="no">AUTOINDEX</code> choisir.</p></li>
</ul>
...
<p>Pour les cellules marquées « <code translate="no">No</code> », Milvus rejette la requête au moment de la création de l’index. Pour les types de conversion de tableaux, utilisez explicitement « <code translate="no">INVERTED</code> » (la méthode «<code translate="no">AUTOINDEX</code> » ne prend pas en charge les tableaux).</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>Chaque exemple ci-dessous présente un appel ` <code translate="no">index_params.add_index(...)</code> `. Choisissez ceux qui correspondent à vos données et appelez-les sur le même objet ` <code translate="no">index_params</code> `. Appliquez ensuite le tout en un seul appel ` <code translate="no">client.create_index(...)</code> ` à la fin. Pour plus de détails, consultez la section « <a href="/docs/fr/json-indexing.md#apply-the-index">Appliquer l’index</a> ».</p>
...
<h3 id="Example-2-Index-a-nested-key" class="common-anchor-header">Exemple 2 : indexer une clé imbriquée<button data-href="#Example-2-Index-a-nested-key" class="anchor-icon" translate="no">
    ...
    </button></h3><p>Indexez le champ « <code translate="no">email</code> », fortement imbriqué, pour la recherche de contacts chez les fournisseurs. Le paramètre « <code translate="no">json_path</code> » accepte toute profondeur de notation entre crochets.</p>
<pre><code translate="no" class="language-python">index_params.add_index(
    field_name=<span class="hljs-string">&quot;metadata&quot;</span>,
<span class="highlighted-wrapper-line">    index_type=<span class="hljs-string">&quot;AUTOINDEX&quot;</span>,</span>
...
<li><p>Vous gérez un index JSON d’objet entier existant. Les options <code translate="no">INVERTED</code> et <code translate="no">AUTOINDEX</code> restent prises en charge à des fins de compatibilité, mais l’indexation JSON d’objet entier est dépréciée à partir de Milvus 3.0.0.</p></li>
</ul>
