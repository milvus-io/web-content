...
<h2 id="Overview​" class="common-anchor-header">Présentation<button data-href="#Overview​" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Dans Milvus, le schéma d’une collection correspond à une table dans une base de données relationnelle ; il définit la manière dont Milvus organise les données au sein de la collection. ​</p>
<p>Un schéma bien conçu est essentiel, car il permet d’abstraire le modèle de données et détermine si vous pouvez atteindre vos objectifs métier grâce à la recherche. De plus, comme chaque ligne de données insérée dans la collection doit respecter le schéma, cela contribue à maintenir la cohérence et la qualité des données à long terme. D’un point de vue technique, un schéma bien défini permet un stockage bien organisé des données par colonnes et une structure d’index plus épurée, ce qui améliore les performances de recherche.​</p>
...
<p>La conception du modèle de données d’un système de recherche implique d’analyser les besoins métier et d’abstraire les informations en un modèle de données exprimé sous forme de schéma. Par exemple, la recherche d’un passage de texte doit être « indexée » en convertissant la chaîne de caractères littérale en vecteur via un « embedding » et en activant la recherche vectorielle. Au-delà de cette exigence essentielle, il peut s’avérer nécessaire de stocker d’autres propriétés telles que l’horodatage de publication et l’auteur. Ces métadonnées permettent d’affiner les recherches sémantiques par filtrage, en ne renvoyant que les textes publiés après une date spécifique ou par un auteur particulier. Vous pouvez également récupérer ces valeurs scalaires avec le texte principal pour afficher le résultat de la recherche dans l’application. Chacune doit se voir attribuer un identifiant unique, sous forme d’entier ou de chaîne de caractères, afin d’organiser ces fragments de texte. Ces éléments sont essentiels pour mettre en place une logique de recherche sophistiquée.​</p>
...
<h3 id="Add-TEXT-Fields" class="common-anchor-header">Ajouter des champs TEXT<button data-href="#Add-TEXT-Fields" class="anchor-icon" translate="no">
    ...
    </button></h3><p>Dans Milvus 3.0 et versions ultérieures, vous pouvez utiliser des champs « <code translate="no">TEXT</code> » pour stocker du texte de document, des passages, des journaux et d’autres contenus textuels longs. Contrairement au champ « <code translate="no">VARCHAR</code> », un champ « <code translate="no">TEXT</code> » ne nécessite pas de « <code translate="no">max_length</code> ». Pour en savoir plus sur le champ « <code translate="no">TEXT</code> », consultez la section « <a href="/docs/fr/text.md">Champ Text</a> ».</p>
<pre><code translate="no" class="language-python">schema.add_field(
