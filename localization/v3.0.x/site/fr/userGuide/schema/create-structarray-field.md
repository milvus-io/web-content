...
<tr><td><code translate="no">title_vector</code></td><td><code translate="no">FLOAT_VECTOR</code></td><td>Champ vectoriel au niveau de l’article, utilisé par la suite dans des exemples de recherche hybride.</td></tr>
...
<tr><td><code translate="no">Array</code></td><td>Non pris en charge</td><td>Les sous-champs « timestamptz » et les expressions liées à l'heure ne sont pas pris en charge dans les champs StructArray.</td></tr>
...
<h2 id="Add-a-StructArray-field-to-an-existing-collection" class="common-anchor-header">Ajouter un champ StructArray à une collection existante<button data-href="#Add-a-StructArray-field-to-an-existing-collection" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Milvus v3.0.x prend en charge l’ajout d’un champ StructArray à une collection existante. Le champ StructArray ajouté doit être nullable, car les entités qui existent déjà dans la collection ne disposent pas de valeurs pour ce nouveau champ.</p>
...
<tr><td>Les fonctions ne sont pas prises en charge à l’intérieur de Struct.</td><td>Ne définissez pas de fonctions pour les champs ou sous-champs à l'intérieur d'un champ StructArray.</td></tr>
...
<li><p>Utilisation d’un seul sous-champ vectoriel à la fois pour la recherche dans l’EmbeddingList et pour la recherche au niveau des éléments.</p></li>
...
<li><p>Pour connaître les types de données pris en charge, le comportement des valeurs nulles et les limitations spécifiques à chaque version, consultez la section « <a href="/docs/fr/structarray-limits.md">Limites de StructArray</a> ».</p></li>
</ol>
