...
<h2 id="What-is-a-TIMESTAMPTZ-field" class="common-anchor-header">Qu'est-ce qu'un champ TIMESTAMPTZ ?<button data-href="#What-is-a-TIMESTAMPTZ-field" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Un champ <code translate="no">TIMESTAMPTZ</code> est un type de données défini par le schéma (<code translate="no">DataType.TIMESTAMPTZ</code>) dans Milvus qui traite les entrées tenant compte du fuseau horaire et stocke en interne tous les instants sous forme d’heure absolue UTC :</p>
<ul>
...
<li><p><code translate="no">!=</code> compare la valeur du champ à ce littéral. Parmi les autres opérateurs pris en charge, on trouve : <code translate="no">==</code>, <code translate="no">&lt;</code>, <code translate="no">&lt;=</code>, <code translate="no">&gt;</code> et <code translate="no">&gt;=</code>.</p></li>
</ul>
