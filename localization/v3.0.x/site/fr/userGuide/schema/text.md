...
<li><strong>Stockage LOB</strong>: si la valeur d’un champ « <code translate="no">TEXT</code> » est supérieure ou égale à <code translate="no">dataNode.text.inlineThreshold</code>, Milvus traite cette valeur comme un objet volumineux et stocke le texte d’origine séparément dans un système de stockage d’objets, tel que MinIO. Les données du champ « <code translate="no">TEXT</code> » stockent une référence interne vers le texte stocké séparément. Lorsqu’une requête porte sur le champ « <code translate="no">TEXT</code> » dans les résultats d’une requête ou d’une recherche, Milvus utilise cette référence pour récupérer et renvoyer le texte d’origine.</li>
</ul>
