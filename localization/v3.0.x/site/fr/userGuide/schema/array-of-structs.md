...
<h2 id="What-is-StructArray" class="common-anchor-header">Qu'est-ce qu'un StructArray ?<button data-href="#What-is-StructArray" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Un <strong>StructArray</strong>, également appelé « tableau de structures », stocke un ensemble ordonné d’éléments Struct dans chaque entité. Chaque élément Struct du tableau respecte le même schéma. Un élément Struct peut contenir des sous-champs scalaires, des sous-champs vectoriels, ou les deux.</p>
...
<tr><td>Récupération par interaction tardive</td><td>Stockez un document ou une page sous forme de liste d'embeddings et évaluez-le à l'aide de <code translate="no">MAX_SIM*</code>.</td><td><code translate="no">chunks[emb_list_vector]</code> ou <code translate="no">patches[emb]</code></td></tr>
<tr><td>Récupération au niveau des éléments</td><td>Renvoyer le fragment, l'extrait, le patch ou l'observation le plus pertinent, y compris son indice de tableau.</td><td><code translate="no">chunks[emb]</code></td></tr>
<tr><td>Filtrage structuré</td><td>Filtrer par sous-champs scalaires à l'intérieur des éléments Struct, tels que section, score, page ou flags.</td><td><code translate="no">chunks[section]</code>, <code translate="no">chunks[quality_score]</code></td></tr>
...
<tbody>
<tr><td>Modélisez un objet parent comportant de nombreux enfants structurés.</td><td>Créez un champ StructArray.</td><td>L'entité contient des éléments Struct ordonnés.</td><td><a href="/docs/fr/create-structarray-field.md">Créer un champ StructArray</a></td></tr>
...
<tr><td>Exécuter ColBERT, ColPali ou une recherche par interaction tardive au niveau du document.</td><td>Utiliser la recherche EmbeddingList avec un index « <code translate="no">MAX_SIM*</code> ».</td><td>Au niveau de l'entité.</td><td><a href="/docs/fr/search-with-embedding-lists.md">Recherche avec des listes d'embeddings</a></td></tr>
<tr><td>Recherchez des segments, des extraits ou des fragments individuels.</td><td>Utilisez la recherche au niveau des éléments avec une métrique vectorielle standard.</td><td>Niveau des éléments de structure, avec décalage lorsque celui-ci est disponible.</td><td><a href="/docs/fr/basic-vector-search-with-structarray.md">Recherche vectorielle de base avec StructArray</a></td></tr>
<tr><td>Limitez la recherche vectorielle au niveau des éléments à ceux qui répondent à des conditions scalaires.</td><td>Utilisez ` <code translate="no">element_filter</code>`.</td><td>Filtrage au niveau des éléments ; la forme du résultat dépend du type de recherche.</td><td><a href="/docs/fr/filtered-search-with-structarray.md">Recherche filtrée avec StructArray</a></td></tr>
...
<tr><td>Utilisez des limites de score ou de distance sur les sous-champs vectoriels de StructArray.</td><td>Utilisez la recherche par plage au niveau des éléments.</td><td>Niveau des éléments de Struct.</td><td><a href="/docs/fr/range-search-with-structarray.md">Recherche par plage avec StructArray</a></td></tr>
<tr><td>Renvoyer au maximum un résultat par entité parente après une recherche au niveau des éléments.</td><td>Utiliser la recherche par regroupement selon la clé primaire.</td><td>Niveau de l’entité après regroupement.</td><td><a href="/docs/fr/grouping-search-with-structarray.md">Recherche groupée avec StructArray</a></td></tr>
<tr><td>Combiner la recherche d'éléments StructArray avec un autre champ vectoriel.</td><td>Utilisez une recherche hybride avec une requête AnnSearchRequest ciblant un sous-champ vectoriel de StructArray.</td><td>Sous-recherche au niveau des éléments, reclassement au niveau des entités.</td><td><a href="/docs/fr/hybrid-search-with-structarray.md">Recherche hybride avec StructArray</a></td></tr>
</tbody>
</table>
<h2 id="Understand-the-two-search-models" class="common-anchor-header">Comprendre les deux modèles de recherche<button data-href="#Understand-the-two-search-models" class="anchor-icon" translate="no">
  ...
  <thead>
    <tr>
      <th scope="col"><h3>Recherche EmbeddingList</h3></th>
      <th scope="col"><h3>Recherche au niveau des éléments</h3></th>
    </tr>
  </thead>
  <tbody>
    <tr>
      <td>
        <p>La recherche EmbeddingList traite les vecteurs contenus dans un sous-champ vectoriel de StructArray comme une seule liste d’embeddings pour l’entité parente. La requête est également une liste d’embeddings. Milvus compare la liste d’embeddings de la requête à la liste d’embeddings stockée à l’aide d’une métrique de type « <code translate="no">MAX_SIM*</code> » et renvoie les entités correspondantes.</p>
        <ul>
          <li>Données de requête : liste d’encodage.</li>
          <li>Famille de métriques : « <code translate="no">MAX_SIM*</code> ».</li>
          <li>Granularité des résultats : niveau de l’entité.</li>
          <li>Idéal pour : la recherche en phase avancée au niveau du document ou de la page.</li>
        </ul>
      </td>
      <td>
        <p>La recherche au niveau des éléments traite chaque élément de Struct comme un candidat indépendant à la recherche vectorielle. Chaque résultat correspond à un élément apparié dans le champ StructArray, et les résultats non regroupés peuvent révéler le décalage de l’élément.</p>
        <ul>
          <li>Données de requête : vecteur standard.</li>
          <li>Famille de métriques : métriques vectorielles classiques.</li>
          <li>Granularité des résultats : niveau des éléments Struct.</li>
          <li>Idéal pour : la recherche au niveau des blocs, des extraits ou des segments.</li>
        </ul>
      </td>
    </tr>
  </tbody>
...
<tr><td>Référence</td><td><a href="/docs/fr/structarray-limits.md">Limites de StructArray</a></td><td>Vérifier les limites de schéma, de type de données, d’index, de recherche, de filtrage et de version.</td></tr>
<tr><td>Recherche</td><td><a href="/docs/fr/basic-vector-search-with-structarray.md">Recherche vectorielle de base avec StructArray</a></td><td>Comparez la recherche EmbeddingList et la recherche vectorielle au niveau des éléments.</td></tr>
<tr><td>Recherche</td><td><a href="/docs/fr/range-search-with-structarray.md">Recherche par plage avec StructArray</a></td><td>Utilisez des contraintes de plage avec les sous-champs vectoriels de StructArray.</td></tr>
<tr><td>Recherche</td><td><a href="/docs/fr/grouping-search-with-structarray.md">Recherche groupée avec StructArray</a></td><td>Regrouper les résultats de recherche au niveau des éléments par clé primaire.</td></tr>
<tr><td>Recherche</td><td><a href="/docs/fr/hybrid-search-with-structarray.md">Recherche hybride avec StructArray</a></td><td>Combiner la recherche au niveau des éléments StructArray avec d'autres recherches vectorielles.</td></tr>
<tr><td>Recherche</td><td><a href="/docs/fr/filtered-search-with-structarray.md">Recherche filtrée avec StructArray</a></td><td>Utilisez les filtres StructArray dans la recherche, la requête et la recherche hybride.</td></tr>
...
<tr><td>Filtrage</td><td><a href="/docs/fr/struct-array-operators.md">Opérateurs StructArray</a></td><td>Syntaxe de référence pour les opérateurs « <code translate="no">element_filter</code> » et « <code translate="no">MATCH_*</code> ».</td></tr>
</tbody>
</table>
...
<li><p>Les sous-champs de type Vector nécessitent des index. La recherche EmbeddingList utilise les métriques de type « <code translate="no">MAX_SIM*</code> », tandis que la recherche au niveau des éléments utilise les métriques vectorielles standard.</p></li>
<li><p><code translate="no">element_filter</code> et « <code translate="no">MATCH_*</code> » s’appliquent aux sous-champs scalaires à l’intérieur des champs StructArray. N’utilisez « <code translate="no">$[subfield]</code> » qu’au sein de ces opérateurs.</p></li>
...
<li><p>Pour effectuer une recherche dans les sous-champs vectoriels de StructArray, commencez par la section « <a href="/docs/fr/basic-vector-search-with-structarray.md">Recherche vectorielle de base avec StructArray</a> ».</p></li>
<li><p>Pour filtrer les sous-champs scalaires de StructArray, consultez les sections « <a href="/docs/fr/struct-array-operators.md">Opérateurs StructArray</a> » et « <a href="/docs/fr/filtered-search-with-structarray.md">Recherche filtrée avec StructArray</a> ».</p></li>
</ol>
