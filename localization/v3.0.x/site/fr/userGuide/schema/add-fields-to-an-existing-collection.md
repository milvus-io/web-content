...
title: Modifier le schéma de la collection
summary: >-
  Modifier un schéma de collection existant en ajoutant ou en supprimant des
  champs scalaires, des champs vectoriels et des champs vectoriels générés par
  des fonctions, sans recréer la collection.
---
<h1 id="Alter-Collection-Schema" class="common-anchor-header">Modifier le schéma de la collection<button data-href="#Alter-Collection-Schema" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h1><p>Lorsqu'une collection passe de l'environnement de développement à l'environnement de production, les champs associés à chaque entité changent souvent. Vous pouvez ajouter des champs scalaires tels que « <code translate="no">source_uri</code> » ou « <code translate="no">review_status</code> » pour le filtrage et la logique d’application, ajouter un nouveau champ vectoriel pour les représentations générées par votre application, ajouter un champ vectoriel creux généré par BM25 pour la recherche lexicale sur le texte existant, ou supprimer des champs qui ne sont plus utilisés. La modification du schéma de collection vous permet d’effectuer les modifications de champs prises en charge directement, sans avoir à recréer la collection.</p>
<div class="alert note">
<p>Ce guide traite des modifications de schéma au niveau des champs dans les collections gérées, y compris les champs définis par l’utilisateur et les champs vectoriels générés par des fonctions. Pour ajouter un champ à une collection externe, reportez-vous à <a href="/docs/fr/alter-external-collection-schema.md">la section Modifier le schéma d’une collection externe</a>. Pour les modifications de propriétés de champ, telles que la modification de la valeur « <code translate="no">max_length</code> » sur un champ « <code translate="no">VARCHAR</code> » ou de la valeur « <code translate="no">max_capacity</code> » sur un champ « <code translate="no">ARRAY</code> », reportez-vous à la <a href="/docs/fr/alter-collection-field.md">section Modifier un champ de collection</a>. Pour le comportement dynamique des champs, reportez-vous aux sections « <a href="/docs/fr/enable-dynamic-field.md">Champ dynamique</a> » et « <a href="/docs/fr/modify-collection.md">Modifier une collection</a> ».</p>
</div>
...
<li><p>L’ajout de champs StructArray est pris en charge à partir de Milvus 3.0.0. Les champs StructArray ajoutés doivent être non nuls.</p></li>
<li><p>Les noms de champs doivent être uniques parmi les champs de la collection.</p></li>
</ul>
<p><strong>Ajouter des champs vectoriels générés par des fonctions</strong></p>
<ul>
...
<ul>
<li><p>Vous ne pouvez pas supprimer le champ de clé primaire, le champ de clé de partition, le champ de clé de clustering ou le dernier champ vectoriel d’une collection.</p></li>
...
<li><p>Vous ne pouvez pas supprimer directement un champ utilisé comme champ d’entrée d’une fonction ou généré comme champ de sortie d’une fonction. Pour supprimer un champ de sortie d’une fonction, supprimez la fonction qui le génère.</p></li>
</ul>
<p><strong>Supprimer les champs vectoriels générés par des fonctions</strong></p>
<ul>
<li><p>Dans ce workflow de modification de schéma, la suppression d’une fonction supprime la fonction et ses champs de sortie générés. Les champs d’entrée de la fonction restent dans le schéma de la collection.</p></li>
<li><p>La suppression d’une fonction est refusée si la suppression de ses champs de sortie laisse la collection sans aucun champ vectoriel.</p></li>
</ul>
<div class="alert note">
...
<h2 id="Add-fields-to-an-existing-collection" class="common-anchor-header">Ajouter des champs à une collection existante<button data-href="#Add-fields-to-an-existing-collection" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>Choisissez la méthode d’ajout de champ en fonction de la manière dont les valeurs du champ sont produites :</p>
<ul>
...
<li><p><a href="#add-vector-fields-generated-by-functions--milvus-30x">Ajoutez des champs vectoriels générés par des fonctions</a> lorsque Milvus doit générer des valeurs vectorielles à partir de champs existants, tels que des vecteurs creux BM25 ou des signatures MinHash issues de texte.</p></li>
</ul>
...
<p>Chaque champ vectoriel défini par l’utilisateur ajouté doit être pouvant prendre la valeur null. Les entités existantes disposent d’une valeur par défaut ( <code translate="no">NULL</code> ) pour le nouveau champ vectoriel jusqu’à ce que vous écriviez des valeurs vectorielles via un workflow « upsert » ou de « backfill ». Les nouvelles entités peuvent inclure le champ vectoriel lors de l’insertion. La recherche vectorielle ignore les entités dont la valeur vectorielle est « <code translate="no">NULL</code> ». Pour plus de détails, reportez-vous à <a href="/docs/fr/nullable-and-default.md">la section Champs pouvant prendre la valeur null</a>.</p>
...
<h3 id="Add-vector-fields-generated-by-functions--Milvus-30x" class="common-anchor-header">Ajouter des champs vectoriels générés par des fonctions<span class="beta-tag" style="background-color:rgb(0, 179, 255);color:white" translate="no">Compatible with Milvus 3.0.x</span><button data-href="#Add-vector-fields-generated-by-functions--Milvus-30x" class="anchor-icon" translate="no">
    ...
    </button></h3><p>Utilisez ce workflow lorsque Milvus doit générer un nouveau champ vectoriel à partir de données déjà stockées dans une collection existante. L’opération ajoute deux éléments de schéma associés :</p>
<ul>
<li><p>Une fonction qui lit un ou plusieurs champs d’entrée existants.</p></li>
<li><p>Un nouveau champ de sortie vectoriel qui stocke les valeurs générées par la fonction.</p></li>
</ul>
<p>Par exemple, une fonction BM25 lit un champ existant « <code translate="no">VARCHAR</code> » et génère un champ « <code translate="no">SPARSE_FLOAT_VECTOR</code> » pour la recherche lexicale. Une fonction MinHash génère un champ « <code translate="no">BINARY_VECTOR</code> » pour la détection des quasi-duplicatas. Ce workflow n’ajoute ni ne remplace le champ d’entrée de la fonction.</p>
<div class="alert note">
...
<tr><td><code translate="no">MINHASH</code></td><td><code translate="no">BINARY_VECTOR</code></td><td>Un champ « <code translate="no">VARCHAR</code> »</td><td>Détection des quasi-doublons</td></tr>
</tbody>
</table>
...
<p><strong>Exemple : ajouter un champ vectoriel clairsemé généré par BM25 pour la recherche lexicale</strong></p>
<p>L'exemple suivant ajoute une fonction BM25 nommée « <code translate="no">text_bm25</code> » et un champ vectoriel clairsemé généré nommé « <code translate="no">text_sparse</code> » à une collection existante. La collection doit déjà comporter un champ « <code translate="no">VARCHAR</code> » nommé « <code translate="no">text</code> » avec l'analyseur activé.</p>
...
<span class="highlighted-comment-line">)</span>
<button class="copy-code-btn"></button></code></pre>
<p>Après avoir ajouté la fonction BM25 et le champ généré, créez un index sur le champ vectoriel clairsemé avant de l’utiliser pour une recherche BM25 :</p>
<pre><code translate="no" class="language-python">index_params = client.prepare_index_params()
...
<button class="copy-code-btn"></button></code></pre>
<p>D’un point de vue conceptuel, cette opération ajoute les définitions de champ et de fonction suivantes :</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>Une fois la requête réussie, ` <code translate="no">describe_collection()</code> ` renvoie à la fois le nouveau champ vectoriel ` <code translate="no">text_sparse</code> ` et la fonction ` <code translate="no">text_bm25</code> ` dans le schéma de la collection. Milvus génère la sortie de la fonction pour les nouvelles entités au fur et à mesure de leur écriture. Pour les entités existantes, Milvus remplit le champ vectoriel généré de manière asynchrone via une compaction en arrière-plan. La visibilité du schéma confirme que la mise à jour du schéma a réussi, mais n’indique pas que le remplissage a été effectué pour chaque entité existante. Pour le workflow complet de recherche BM25, reportez-vous à <a href="/docs/fr/full-text-search.md">la section</a> « <a href="/docs/fr/full-text-search.md">Recherche en texte intégral</a> ».</p>
<p>Milvus prend également en charge les champs vectoriels binaires générés par MinHash pour la détection des quasi-duplicatas. Une fonction MinHash utilise l’ <code translate="no">FunctionType.MINHASH</code> et écrit dans un nouveau champ de sortie « <code translate="no">BINARY_VECTOR</code> ». Pour plus de détails sur la configuration, reportez-vous à la section « <a href="/docs/fr/minhash-function.md">Fonction MinHash</a> ».</p>
<h2 id="Drop-fields-from-an-existing-collection" class="common-anchor-header">Supprimer des champs d’une collection existante<button data-href="#Drop-fields-from-an-existing-collection" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>Vous pouvez supprimer des champs d’une collection existante de deux manières. Supprimez directement les champs scalaires ou vectoriels définis par l’utilisateur lorsqu’ils ne font plus partie de votre modèle de collection. Supprimez les champs vectoriels générés par des fonctions en supprimant la fonction qui les génère.</p>
...
<ul>
<li><p>Une fois l’opération « <code translate="no">drop_collection_field()</code> » réussie, le schéma de la collection est mis à jour : la méthode ` <code translate="no">describe_collection()</code> ` ne renvoie plus le champ supprimé, et les requêtes ou recherches ne peuvent plus renvoyer ce champ dans ` <code translate="no">output_fields</code> ` ni l’utiliser dans des expressions.</p></li>
...
<span class="highlighted-comment-line">)</span>
<button class="copy-code-btn"></button></code></pre>
<p>Après avoir supprimé un champ, vous pouvez appeler la fonction ` <code translate="no">describe_collection()</code> ` pour vérifier que le champ ne fait plus partie du schéma.</p>
...
<h3 id="Drop-vector-fields-generated-by-functions--Milvus-30x" class="common-anchor-header">Supprimer des champs vectoriels générés par des fonctions<span class="beta-tag" style="background-color:rgb(0, 179, 255);color:white" translate="no">Compatible with Milvus 3.0.x</span><button data-href="#Drop-vector-fields-generated-by-functions--Milvus-30x" class="anchor-icon" translate="no">
    ...
    </button></h3><p>Utilisez cette opération lorsque vous n’avez plus besoin d’un champ vectoriel généré par une fonction, tel qu’un champ vectoriel creux généré par BM25.</p>
<p>Pour supprimer un champ vectoriel généré, appelez <code translate="no">drop_collection_function()</code> sur la fonction qui le génère. Dans ce workflow, Milvus supprime la fonction du schéma de la collection et supprime également ses champs de sortie vectoriels générés.</p>
<p>N’appelez pas la méthode ` <code translate="no">drop_collection_field()</code> ` sur un champ d’entrée ou de sortie de fonction. Si le champ cible est un champ de sortie de fonction, appelez plutôt la méthode ` <code translate="no">drop_collection_function()</code> `. Les champs d’entrée de fonction sont conservés après la suppression de la fonction.</p>
<p><strong>Exemple : suppression d’une fonction BM25 et de son champ généré</strong></p>
<p>L’exemple suivant part du principe que ` <code translate="no">text_bm25</code> ` est une fonction BM25 de ` <code translate="no">product_catalog</code> ` et génère un champ de sortie de type vecteur creux nommé ` <code translate="no">text_sparse</code>`.</p>
<pre><code translate="no" class="language-python"><span class="hljs-keyword">from</span> pymilvus <span class="hljs-keyword">import</span> MilvusClient
...
<span class="highlighted-comment-line">)</span>
<button class="copy-code-btn"></button></code></pre>
<p>Une fois l’opération réussie, <code translate="no">describe_collection()</code> ne renvoie plus la fonction supprimée ni ses champs de sortie générés. Les champs d’entrée de la fonction restent dans le schéma.</p>
<p>Si la suppression des champs de sortie de la fonction devait laisser la collection sans aucun champ vectoriel, l’opération est rejetée.</p>
<h2 id="FAQ" class="common-anchor-header">FAQ<button data-href="#FAQ" class="anchor-icon" translate="no">
    ...
    </button></h2><h3 id="Which-add-field-method-should-I-use" class="common-anchor-header">Quelle méthode d’ajout de champ dois-je utiliser ?<button data-href="#Which-add-field-method-should-I-use" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Utilisez ` <code translate="no">add_collection_field()</code> ` pour ajouter un champ scalaire défini par l’utilisateur lorsque votre application fournit des valeurs scalaires à des fins de filtrage, de résultat de requête ou de logique d’application.</p>
...
<p>Utilisez « <code translate="no">add_collection_field()</code> » pour ajouter un champ vectoriel défini par l’utilisateur lorsque votre application génère des représentations et écrit des valeurs vectorielles dans Milvus.</p>
<p>Utilisez le workflow « generated-vector-field » lorsque Milvus doit produire des valeurs vectorielles à partir de champs existants. Ce guide présente le parcours BM25 avec « <code translate="no">add_function_field()</code> » pour la recherche lexicale. Milvus prend également en charge les champs vectoriels binaires générés par MinHash pour la détection de quasi-duplicatas.</p>
<h3 id="Why-must-added-user-defined-fields-be-nullable" class="common-anchor-header">Pourquoi les champs définis par l’utilisateur doivent-ils être de type « nullable » ?<button data-href="#Why-must-added-user-defined-fields-be-nullable" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<h3 id="Can-I-add-BM25-lexical-search-to-an-existing-collection" class="common-anchor-header">Puis-je ajouter la recherche lexicale BM25 à une collection existante ?<button data-href="#Can-I-add-BM25-lexical-search-to-an-existing-collection" class="anchor-icon" translate="no">
    ...
    </button></h3><p>Oui. Si la collection dispose déjà d’un champ « <code translate="no">VARCHAR</code> » avec l’analyseur activé, vous pouvez ajouter un champ vectoriel clairsemé généré par BM25 pour la recherche lexicale. Dans ce workflow, Milvus ajoute le nouveau champ de sortie « <code translate="no">SPARSE_FLOAT_VECTOR</code> » ainsi que la fonction BM25 qui génère des valeurs pour celui-ci. Vous ne pouvez pas utiliser un champ « <code translate="no">TEXT</code> » existant comme entrée BM25 dans ce workflow de modification de schéma. Pour utiliser une entrée « <code translate="no">TEXT</code> », définissez le champ et la fonction BM25 lors de la création de la collection.</p>
<p>Après avoir ajouté le champ vectoriel clairsemé généré par BM25, créez un index « <code translate="no">SPARSE_INVERTED_INDEX</code> » avec l’option « <code translate="no">metric_type=&quot;BM25&quot;</code> » avant d’utiliser ce champ pour une recherche BM25.</p>
<h3 id="Can-I-drop-a-vector-field-generated-by-a-function-directly" class="common-anchor-header">Puis-je supprimer directement un champ vectoriel généré par une fonction ?<button data-href="#Can-I-drop-a-vector-field-generated-by-a-function-directly" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Non. Un champ vectoriel généré par une fonction fait partie du contrat de schéma de cette fonction. Utilisez plutôt la commande « <code translate="no">drop_collection_function()</code> ». Dans ce workflow de modification de schéma, Milvus supprime à la fois la fonction et les champs vectoriels de sortie qu’elle a générés, tout en conservant les champs d’entrée.</p>
<h3 id="Do-I-need-to-wait-after-altering-a-collection-schema" class="common-anchor-header">Dois-je patienter après avoir modifié le schéma d’une collection ?<button data-href="#Do-I-need-to-wait-after-altering-a-collection-schema" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<p>Dans un déploiement distribué, il peut y avoir un court délai de propagation pendant que les composants de Milvus actualisent les métadonnées de la collection. Si une opération effectuée immédiatement après la modification du schéma échoue en raison d’une erreur liée au schéma, actualisez le schéma et réessayez l’opération.</p>
<h3 id="When-is-storage-space-reclaimed-after-dropping-a-field" class="common-anchor-header">Quand l’espace de stockage est-il libéré après la suppression d’un champ ?<button data-href="#When-is-storage-space-reclaimed-after-dropping-a-field" class="anchor-icon" translate="no">
    ...
    </button></h3><p>La suppression d’un champ le retire du schéma actuel et le rend invisible dans les requêtes et recherches normales, mais les données historiques de ce champ ne sont pas immédiatement supprimées physiquement du stockage d’objets.</p>
