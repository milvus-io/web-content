...
summary: >-
  ...
  adjacents les uns aux autres. Par exemple, une requête telle que « robotique
  apprentissage automatique » correspond à un texte du type « …modèles typiques
  de robotique et d’apprentissage automatique… », où les mots « robotique », «
  machine » et « apprentissage » apparaissent à la suite les uns des autres,
  sans aucun autre mot entre eux.
beta: Milvus 2.5.17+
---
<h1 id="Phrase-Match" class="common-anchor-header">Recherche d'expression exacte<span class="beta-tag" style="background-color:rgb(0, 179, 255);color:white" translate="no">Compatible with Milvus 2.5.17+</span><button data-href="#Phrase-Match" class="anchor-icon" translate="no">
    ...
    </button></h1><p>La recherche d'expressions vous permet de rechercher des documents contenant les termes de votre requête sous forme d'expression exacte. Par défaut, les mots doivent apparaître dans le même ordre et être directement adjacents les uns aux autres. Par exemple, une requête sur <strong>« robotique apprentissage automatique »</strong> correspond à un texte tel que <em>« …modèles typiques d’apprentissage automatique en robotique… »</em>, où les mots <strong>« robotique »</strong>, <strong>« apprentissage »</strong> et <strong>« automatique</strong> <strong>»</strong> apparaissent dans cet ordre sans aucun autre mot entre eux.</p>
<p>Cependant, dans la pratique, la correspondance exacte par expression peut s’avérer trop rigide. Vous souhaiterez peut-être trouver des textes tels que <em>« …modèles d’apprentissage automatique largement adoptés en robotique… ».</em> Ici, les mêmes mots-clés sont présents, mais ils ne sont ni côte à côte ni dans l’ordre d’origine. Pour remédier à cela, la correspondance par expression prend en charge un paramètre « <code translate="no">slop</code> », qui apporte une certaine flexibilité. La valeur « <code translate="no">slop</code> » définit le nombre de décalages positionnels autorisés entre les termes de l’expression. Par exemple, avec une valeur « <code translate="no">slop</code> » de 1, une requête sur <strong>« machine learning »</strong> peut correspondre à un texte tel que <em>« …machine deep learning… »</em>, où un mot (<strong>« deep »</strong>) sépare les termes d’origine.</p>
...
<ol>
<li><p><strong>Tokenisation des documents</strong>: lorsque vous importez des documents dans Milvus, le texte est divisé en tokens (mots ou termes individuels) à l’aide d’un analyseur, et les informations de position sont enregistrées pour chaque token. Par exemple, <strong>doc_1</strong> est tokenisé en <strong>[« machine » (pos=0), « learning » (pos=1), « boosts » (pos=2), « efficiency » (pos=3)]</strong>. Pour plus d’informations sur les analyseurs, consultez <a href="/docs/fr/analyzer-overview.md">la section Présentation des analyseurs</a>.</p></li>
<li><p><strong>Création d’un index inversé</strong>: Milvus construit un index inversé, en associant chaque token au(x) document(s) dans lequel (lesquels) il apparaît ainsi qu’aux positions de ce token dans ces documents.</p></li>
...
<h3 id="Set-enableanalyzer-and-enablematch" class="common-anchor-header">Définissez <code translate="no">enable_analyzer</code> et <code translate="no">enable_match</code><button data-href="#Set-enableanalyzer-and-enablematch" class="anchor-icon" translate="no">
    ...
    </button></h3><p>Pour activer la correspondance par expression pour un champ « <code translate="no">VARCHAR</code> » spécifique, définissez les paramètres « <code translate="no">enable_analyzer</code> » et « <code translate="no">enable_match</code> » sur « <code translate="no">True</code> » lors de la définition du schéma du champ. Cette configuration indique à Milvus de tokeniser le texte et de créer un index inversé contenant les informations de position nécessaires à une correspondance par expression efficace.</p>
...
<p>Si votre application nécessite un analyseur spécifique, configurez-le à l’aide du paramètre ` <code translate="no">analyzer_params</code> `. Voici, par exemple, comment configurer l’analyseur ` <code translate="no">english</code> ` pour la correspondance de phrases dans un texte en anglais :</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#go">   Go</a>
...
<p>L'expression « <code translate="no">PHRASE_MATCH</code> » ne tient pas compte de la casse. Vous pouvez utiliser soit <code translate="no">PHRASE_MATCH</code>, soit <code translate="no">phrase_match</code>.</p>
</div>
<h3 id="PHRASEMATCH-expression-syntax" class="common-anchor-header">Syntaxe de l’expression PHRASE_MATCH<button data-href="#PHRASEMATCH-expression-syntax" class="anchor-icon" translate="no">
    ...
    </button></h3><p>Utilisez l’expression « <code translate="no">PHRASE_MATCH</code> » pour spécifier le champ, l’expression et le niveau de flexibilité facultatif (<code translate="no">slop</code>) lors de la recherche. La syntaxe est la suivante :</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#go">   Go</a>
...
<li><p><code translate="no">slop</code> (facultatif)<strong>:</strong> un entier indiquant le nombre maximal de positions autorisées dans les tokens correspondants.</p>
<ul>
...
<li><p><code translate="no">1</code>: Autorise des variations mineures, telles qu’un terme supplémentaire ou un léger décalage de position. Exemple : un filtre pour <strong>« machine learning »</strong> correspondra à <strong>« machine boosts learning »</strong> (un token entre <strong>« machine »</strong> et <strong>« learning »</strong>), mais pas à <strong>« learning machine »</strong> (termes inversés).</p></li>
<li><p><code translate="no">2</code>: Autorise davantage de flexibilité, y compris l’inversion de l’ordre des termes ou jusqu’à deux tokens entre eux. Exemple : un filtre pour <strong>« machine learning »</strong> correspondra à <strong>« learning machine »</strong> (ordres des termes inversés) ou à <strong>« machine quickly boosts learning »</strong> (deux tokens entre <strong>« machine »</strong> et <strong>« learning »</strong>).</p></li>
</ul></li>
</ul>
...
<h3 id="Query-with-phrase-match" class="common-anchor-header">Requête avec correspondance de phrase<button data-href="#Query-with-phrase-match" class="anchor-icon" translate="no">
    ...
    </button></h3><p>Lors de l'utilisation de la méthode ` <code translate="no">query()</code> `, <strong>`PHRASE_MATCH</strong> ` agit comme un filtre scalaire. Seuls les documents contenant la phrase spécifiée (dans les limites de la tolérance autorisée) sont renvoyés.</p>
...
<table>
   ...
   <tr>
     <td><p>5</p></td>
     <td><p>« L'apprentissage d'algorithmes avancés pour les machines élargit les capacités de l'IA »</p></td>
   </tr>
</table>
...
<li><p>Les performances de la correspondance de phrases dépendent de la manière dont le texte est tokenisé. Avant d’appliquer un analyseur à l’ensemble de votre collection, utilisez la méthode ` <code translate="no">run_analyzer</code> ` pour examiner le résultat de la tokenisation. Pour plus d’informations, reportez-vous à <a href="/docs/fr/analyzer-overview.md#share-DYZvdQ2vUowWEwx1MEHcdjNNnqT">la section Présentation des analyseurs</a>.</p></li>
<li><p>Règles d'échappement dans les expressions « <code translate="no">filter</code> » :</p>
<ul>
<li><p>Les caractères placés entre guillemets doubles ou simples au sein des expressions sont interprétés comme des constantes de chaîne. Si la constante de chaîne contient des caractères d’échappement, ceux-ci doivent être représentés par une séquence d’échappement. Par exemple, utilisez ` <code translate="no">\\</code> ` pour représenter « <code translate="no">\</code> », ` <code translate="no">\\t</code> ` pour représenter une tabulation ` <code translate="no">\t</code>` et ` <code translate="no">\\n</code> ` pour représenter un saut de ligne.</p></li>
