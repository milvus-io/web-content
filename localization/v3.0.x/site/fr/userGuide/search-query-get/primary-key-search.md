...
<p>Les recommandations sont triées en fonction de leur similarité avec le mot-clé ou le produit actuel. Pour ce faire, les développeurs de la plateforme doivent récupérer la représentation vectorielle du mot-clé ou du produit actuel auprès de Milvus avant la recherche de similarité proprement dite, ce qui augmente le nombre d'allers-retours entre la plateforme et Milvus et entraîne la transmission d'un grand nombre de nombres à virgule flottante de haute dimension sur le réseau.</p>
...
<li><p>Les recherches par clé primaire s’appliquent à tous les types de données vectorielles, à l’exception des champs vectoriels clairsemés dérivés de champs VarChar, comme dans les fonctions BM25.</p></li>
...
<li><p>Pour les recherches par similarité impliquant des listes d’embeddings, vous devez toujours récupérer les vecteurs de requête, les organiser en listes d’embeddings, puis lancer les recherches.</p></li>
...
<li><p>Les clés primaires et les vecteurs de requête s’excluent mutuellement. Le fait de fournir les deux entraîne également des erreurs.</p></li>
</ul>
...
<p>Les clés primaires ne sont pas utilisées pour le filtrage ; elles servent uniquement à la récupération de vecteurs.</p>
</div>
...
<button class="copy-code-btn"></button></code></pre>
<h3 id="Example-4-Grouping-search-using-primary-keys" class="common-anchor-header">Exemple 4 : Recherche groupée à l'aide de clés primaires<button data-href="#Example-4-Grouping-search-using-primary-keys" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
