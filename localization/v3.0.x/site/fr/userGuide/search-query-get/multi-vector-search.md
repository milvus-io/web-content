...
summary: >-
  ...
  tweet contenant un texte et une image doit être trouvé si le texte ou l’image
  correspond à la sémantique de la requête de recherche. La recherche hybride
  améliore l’expérience de recherche en combinant les recherches sur ces
  différents champs. Milvus prend en charge cette fonctionnalité en permettant
  ...
  vous souhaitez effectuer une recherche à la fois sur du texte et des images,
...
<li><p><strong>Recherche multimodale par image :</strong> cette méthode consiste à interroger l’image à l’aide d’une requête textuelle utilisant des vecteurs denses. Les représentations d’images peuvent être générées à l’aide de modèles tels que <a href="https://zilliz.com/learn/exploring-openai-clip-the-future-of-multimodal-ai-learning">CLIP</a>.</p></li>
</ul>
<p>Ce guide vous présentera un exemple de recherche hybride multimodale combinant les méthodes de recherche ci-dessus, à partir de la description textuelle brute et des représentations d’images des produits. Nous vous montrerons comment stocker des données multivectorielles et effectuer des recherches hybrides à l’aide d’une stratégie de reclassement.</p>
...
<h3 id="Define-schema" class="common-anchor-header">Définir le schéma<button data-href="#Define-schema" class="anchor-icon" translate="no">
    ...
    </button></h3><p>Pour la recherche hybride multivectorielle, il convient de définir plusieurs champs vectoriels au sein du schéma de collection. Pour plus de détails sur les limites relatives au nombre de champs vectoriels autorisés dans une collection, consultez <a href="https://zilliverse.feishu.cn/wiki/PuxkwMWvbiHxvTkHsVkcMZP9n5f#E5yxdHM16okh57xV3WKcTJsYn0f">la page Limites de Zilliz Cloud</a>. Toutefois, si nécessaire, vous pouvez ajuster le <a href="/docs/fr/configure_proxy.md#proxymaxVectorFieldNum"><code translate="no">proxy.maxVectorFieldNum</code></a> pour inclure jusqu’à 10 champs vectoriels dans une collection, selon vos besoins.</p>
...
<li><p><code translate="no">image_dense_index</code>: un index de type « <code translate="no">AUTOINDEX</code> » avec le type de métrique « <code translate="no">IP</code> » est créé pour le champ vectoriel d’images dense.</p></li>
</ul>
...
<li><p><code translate="no">image_dense</code>: une liste de 512 valeurs à virgule flottante représentant l’embedding dense de l’image du produit</p></li>
</ul>
