...
<p>Comme le montre le schéma ci-dessus, la requête contient deux tokens, à savoir <code translate="no">machine</code> et <code translate="no">learning</code>, et le document dans la fenêtre comporte quatre tokens : <code translate="no">neural</code>, <code translate="no">network</code>, <code translate="no">python</code> et <code translate="no">tutorial</code>. Une fois ces tokens vectorisés, les vecteurs d’embedding de chaque token de la requête sont comparés à ceux du document afin d’obtenir une liste de scores de similarité. Les scores les plus élevés de chaque liste de scores sont ensuite additionnés pour obtenir le score final. Le processus de détermination du score final d’un document est appelé « similarité maximale » (<strong>MAX_SIM</strong>). Pour plus de détails sur la similarité maximale, consultez la section Similarité maximale.</p>
<div class="alert note">
<p>Lors de la mise en œuvre d’un système de recherche textuelle de type ColBERT dans Milvus, vous n’êtes pas obligé de diviser les documents en tokens.</p>
...
<p>Cette méthode préserve les informations non textuelles, telles que la mise en page du document, les images et les structures de tableaux, qui sont perdues lors de l’utilisation de systèmes de recherche exclusivement textuels.</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>Chaque score de similarité cosinus par paire varie entre <code translate="no">-1</code> et <code translate="no">1</code>. Le score final de l’ <code translate="no">MAX_SIM_COSINE</code> e peut être supérieur à <code translate="no">1</code> car il agrège plusieurs scores de similarité maximale au niveau des tokens.</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>Dans le code ci-dessus, le modèle ColPali redimensionne l’image à 448 x 448 pixels, puis la divise en fragments mesurant chacun 14 x 14 pixels. Enfin, ces fragments sont intégrés dans 1 031 représentations, chacune comportant 128 dimensions.</p>
