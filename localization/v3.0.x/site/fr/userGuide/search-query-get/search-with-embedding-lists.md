...
title: 'Recherche avec EmbeddingLists : ColBERT et ColPali'
summary: >-
  Ce tutoriel explique comment créer des systèmes de recherche de type ColBERT
  et ColPali à l'aide de la recherche EmbeddingList sur des sous-champs
  vectoriels StructArray dans Milvus. Utilisez-le lorsque votre requête et vos
  données stockées sont toutes deux représentées sous forme de listes de
  vecteurs et que vous souhaitez effectuer une recherche avec interaction
  tardive au niveau des entités, en utilisant les métriques MAX_SIM*.
---
<h1 id="Search-with-EmbeddingLists-ColBERT-and-ColPali" class="common-anchor-header">Recherche avec EmbeddingLists : ColBERT et ColPali<button data-href="#Search-with-EmbeddingLists-ColBERT-and-ColPali" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h1><p>Ce tutoriel explique comment créer des systèmes de recherche de type ColBERT et ColPali à l’aide de la recherche EmbeddingList sur les sous-champs vectoriels StructArray dans Milvus. Utilisez-le lorsque votre requête et vos données stockées sont toutes deux représentées sous forme de listes de vecteurs et que vous souhaitez effectuer une recherche à interaction tardive au niveau des entités avec des métriques d’ <code translate="no">MAX_SIM*</code>.</p>
<p>Pour connaître les principes de base de StructArray sur lesquels repose ce tutoriel, consultez les articles <a href="/docs/fr/create-structarray-field.md">Créer un champ StructArray</a>, <a href="/docs/fr/index-structarray-fields.md">Indexer des champs StructArray</a> et <a href="/docs/fr/basic-vector-search-with-structarray.md">Recherche vectorielle de base avec StructArray</a>. Ce tutoriel se concentre sur les workflows ColBERT et ColPali plutôt que sur la syntaxe générale de StructArray.</p>
<h2 id="Overview" class="common-anchor-header">Présentation<button data-href="#Overview" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Pour créer un système de recherche textuelle, vous devrez peut-être diviser les documents en segments et stocker chaque segment, ainsi que ses représentations, en tant qu’entité dans une base de données vectorielle afin de garantir la précision et l’exactitude, en particulier pour les documents longs où les représentations en texte intégral pourraient diluer la spécificité sémantique ou dépasser les limites d’entrée du modèle.</p>
<p>Cependant, le stockage des données par segments conduit à des résultats de recherche segment par segment, ce qui signifie que la recherche identifie dans un premier temps <em>des segments</em> pertinents plutôt que <em>des documents</em> cohérents. Pour remédier à cela, vous devez effectuer un traitement supplémentaire après la recherche.</p>
<p>ColBERT (arXiv : <a href="https://arxiv.org/abs/2004.12832">2004.12832</a>) est un système de recherche texte-texte qui offre une recherche de passages efficace et performante grâce à des interactions contextuelles tardives sur BERT. Il permet un encodage indépendant, au niveau des tokens, des requêtes et des documents, et calcule leur similarité.</p>
<h3 id="Token-wise-encoding" class="common-anchor-header">Encodage au niveau des tokens<button data-href="#Token-wise-encoding" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<h3 id="Late-interaction" class="common-anchor-header">Interaction tardive<button data-href="#Late-interaction" class="anchor-icon" translate="no">
    ...
    </button></h3><p>Une fois la vectorisation terminée, la liste d’embeddings de la requête est comparée à chacune des listes d’embeddings des documents, token par token, afin de déterminer le score de similarité final.</p>
<p>Comme le montre le schéma ci-dessus, la requête contient deux tokens, à savoir <code translate="no">machine</code> et <code translate="no">learning</code>, et le document dans la fenêtre comporte quatre tokens : <code translate="no">neural</code>, <code translate="no">network</code>, <code translate="no">python</code> et <code translate="no">tutorial</code>. Une fois ces tokens vectorisés, les vecteurs d’embeddings de chaque token de la requête sont comparés à ceux du document afin d’obtenir une liste de scores de similarité. Les scores les plus élevés de chaque liste de scores sont ensuite additionnés pour obtenir le score final. Le processus de détermination du score final d’un document est appelé « similarité maximale » (<strong>MAX_SIM</strong>). Pour plus de détails sur la similarité maximale, consultez la section « Similarité maximale ».</p>
<div class="alert note">
<p>Lors de la mise en œuvre d’un système de recherche textuelle de type ColBERT dans Milvus, vous n’êtes pas limité au découpage des documents en tokens.</p>
<p>Vous pouvez au contraire diviser les documents en segments de taille appropriée, effectuer l’embedding de chaque segment pour créer une liste d’embeddings, puis stocker le document ainsi que ses segments embeddés dans une entité.</p>
</div>
<h3 id="ColPali-extension" class="common-anchor-header">Extension ColPali<button data-href="#ColPali-extension" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>S'appuyant sur ColBERT, ColPali (arXiv : <a href="https://arxiv.org/abs/2407.01449?spm=a2ty_o01.29997173.0.0.31c4c9217HFv28&amp;file=2407.01449">2407.01449</a>) propose une approche novatrice de la recherche de documents riches en contenu visuel qui exploite les modèles vision-langage (VLM). Lors de l’ingestion des données, chaque page de document est convertie en une image haute résolution, puis divisée en « patches », plutôt que d’être tokenisée. Par exemple, l’image d’une page de document de 448 x 448 pixels peut produire 1 024 patches, mesurant chacun 14 x 14 pixels.</p>
<p>Cette méthode préserve les informations non textuelles, telles que la mise en page du document, les images et les structures des tableaux, qui sont perdues lors de l’utilisation de systèmes de recherche exclusivement textuels.</p>
<p>Le VLM utilisé dans ColPali s’appelle PaliGemma (arXiv : <a href="https://arxiv.org/html/2407.07726v2#S1">2407.07726</a>) ; il comprend un encodeur d’images (<strong>SigLIP-400M</strong>), un modèle linguistique de type « décodeur seul » (<strong>Gemma2-2B</strong>), et d’une couche linéaire qui projette la sortie de l’encodeur d’images dans l’espace vectoriel du modèle linguistique, comme le montre le schéma ci-dessus.</p>
<p>Lors de l’ingestion des données, une page de document, représentée sous forme d’image brute, est divisée en plusieurs segments visuels, dont chacun est intégré pour générer une liste de représentations vectorielles. Elles sont ensuite projetées dans l’espace vectoriel du modèle linguistique pour obtenir la liste finale d’encodages, comme dans <span class="katex"><span class="katex-mathml"><math xmlns="http://www.w3.org/1998/Math/MathML"><semantics><mrow><mo>$d \rightarrow E_d</mo><mo stretchy="false">= [</mo><msub><mrow><mn>e_{d1}</mn></mrow></msub><mo separator="true">,</mo><msub><mrow><mn>e_{d2</mn></mrow></msub><mo separator="true">},</mo><mo separator="true">\</mo><mo>dots</mo><mo separator="true">,</mo><msub><mrow><mi>e_{dn</mi></mrow></msub><mo stretchy="false">}]</mo></mrow><annotation encoding="application/x-tex">\in \mathbb{R}^{n \times d}$ \rightarrow E_d = [e_{d1}, e_{d2}, \dots, e_{dn}] ∈ \R^{n×d}</annotation></semantics></math></span><span class="katex-html" aria-hidden="true"><span class="base"><span class="strut" style="height:0.6944em;"></span><span class="mrel"></span><span class="mord mathnormal">d</span><span class="mspace" style="margin-right:0.2778em;"></span><span class="mrel">→</span></span></span></span><span class="mspace" style="margin-right:0.2778em;"></span><span class="base"><span class="mord"><span class="mord mathnormal" style="margin-right:0.05764em;"></span></span></span> <span class="katex"><span class="katex-html" aria-hidden="true"><span class="base"><span class="strut" style="height:0.8333em;vertical-align:-0.15em;"></span><span class="mord"><span class="msupsub"><span class="vlist-t vlist-t2"><span class="vlist-r"><span class="vlist" style="height:0.3361em;"><span style="top:-2.55em;margin-left:-0.0576em;margin-right:0.05em;"><span class="sizing reset-size6 size3 mtight"><span class="mord mathnormal mtight"> </span></span></span></span></span></span></span></span></span></span></span> E <span class="katex"><span class="katex-html" aria-hidden="true"><span class="base"></span></span></span><span class="pstrut" style="height:2.7em;"></span><span class="mrel"></span> <span class="katex"><span class="katex-html" aria-hidden="true"><span class="base"><span class="mord"><span class="msupsub"><span class="vlist-t vlist-t2"><span class="vlist-r"><span class="vlist-s">d​</span></span></span></span></span></span></span></span><span class="vlist-r"><span class="vlist" style="height:0.15em;"><span></span></span></span><span class="mrel"></span> <span class="katex"><span class="katex-html" aria-hidden="true"><span class="base"><span class="mspace" style="margin-right:0.2778em;"></span><span class="mrel"> </span></span></span></span>= <span class="katex"><span class="katex-html" aria-hidden="true"></span></span><span class="mspace" style="margin-right:0.2778em;"></span><span class="base"><span class="mopen"></span></span> <span class="katex"><span class="katex-html" aria-hidden="true"><span class="base"><span class="strut" style="height:1em;vertical-align:-0.25em;"></span><span class="mopen"> </span></span></span></span>[ <span class="katex"><span class="katex-html" aria-hidden="true"><span class="base"><span class="mord"><span class="mord mathnormal">e</span></span></span></span></span><span class="pstrut" style="height:2.7em;"></span><span class="mpunct"></span> <span class="katex"><span class="katex-html" aria-hidden="true"><span class="base"><span class="mord"><span class="msupsub"><span class="vlist-t vlist-t2"><span class="vlist-r"><span class="vlist-s">d1​</span></span></span></span></span></span></span></span><span class="vlist-r"><span class="vlist" style="height:0.15em;"><span></span></span></span><span class="mpunct"></span> <span class="katex"><span class="katex-html" aria-hidden="true"><span class="base"><span class="mpunct">,</span><span class="mspace" style="margin-right:0.1667em;"></span><span class="mord"><span class="mord mathnormal">e</span></span></span></span></span><span class="pstrut" style="height:2.7em;"></span><span class="mpunct"></span> <span class="katex"><span class="katex-html" aria-hidden="true"><span class="base"><span class="mord"><span class="msupsub"><span class="vlist-t vlist-t2"><span class="vlist-r"><span class="vlist-s">d2​</span></span></span></span></span></span></span></span><span class="vlist-r"><span class="vlist" style="height:0.15em;"><span></span></span></span><span class="mpunct"></span> <span class="katex"><span class="katex-html" aria-hidden="true"><span class="base"><span class="mpunct">,</span><span class="mspace" style="margin-right:0.1667em;"></span><span class="mpunct"></span><span class="minner">…</span><span class="mspace" style="margin-right:0.1667em;"></span><span class="mpunct">,</span><span class="mspace" style="margin-right:0.1667em;"></span><span class="mord"><span class="mord mathnormal">e</span></span></span></span></span><span class="pstrut" style="height:2.7em;"></span><span class="mclose"></span> <span class="katex"><span class="katex-html" aria-hidden="true"><span class="base"><span class="mord"><span class="msupsub"><span class="vlist-t vlist-t2"><span class="vlist-r"><span class="vlist-s">dn​</span></span></span></span></span></span></span></span><span class="vlist-r"><span class="vlist" style="height:0.15em;"><span></span></span></span><span class="mclose"></span> <span class="katex"><span class="katex-html" aria-hidden="true"><span class="base"><span class="mclose">]</span></span></span></span><span class="mspace" style="margin-right:0.2778em;"></span><span class="base"><span class="mord"><span class="mord mathbb"></span></span></span> <span class="katex"><span class="katex-html" aria-hidden="true"><span class="base"><span class="mrel">∈</span></span></span></span><span class="mspace" style="margin-right:0.2778em;"></span><span class="base"><span class="mord"><span class="mord mathbb"></span></span></span> <span class="katex"><span class="katex-html" aria-hidden="true"><span class="base"><span class="strut" style="height:0.8491em;"></span><span class="mord"><span class="msupsub"><span class="vlist-t"><span class="vlist-r"><span class="vlist" style="height:0.8491em;"><span style="top:-3.063em;margin-right:0.05em;"><span class="sizing reset-size6 size3 mtight"><span class="mord mtight"><span class="mord mathnormal mtight"> </span></span></span></span></span></span></span></span></span></span></span></span> R <span class="katex"><span class="katex-html" aria-hidden="true"><span class="base"><span class="mord"><span class="msupsub"><span class="vlist-t"><span class="vlist-r"><span class="vlist" style="height:0.8491em;"><span style="top:-3.063em;margin-right:0.05em;"><span class="pstrut" style="height:2.7em;"></span><span class="sizing reset-size6 size3 mtight"><span class="mord mtight"><span class="mord mathnormal mtight"></span><span class="mord mathnormal mtight">n×d</span></span></span></span></span></span></span></span></span></span></span></span>. Lorsqu’une requête arrive, elle est tokenisée, et chaque token est encodé pour générer une liste de vecteurs d’encodage, comme dans <span class="katex"><span class="katex-mathml"><math xmlns="http://www.w3.org/1998/Math/MathML"><semantics><annotation encoding="application/x-tex">$q \rightarrow E_q = [e_{q1}, e_{q2}</annotation><mrow><mo separator="true">, \</mo><mo>dots</mo><mo separator="true">,</mo></mrow><annotation encoding="application/x-tex">e_{</annotation><mrow><mo stretchy="false">qm</mo></mrow><annotation encoding="application/x-tex">}] \in \mathbb{R}^{m \times d}$, où $q \rightarrow E_q = [e_{q1}, e_{q2},</annotation><mrow><mo>\dots</mo></mrow><annotation encoding="application/x-tex">, e_{qm}] \in \mathbb{R}^{m \times d}$, $q \rightarrow E_q = [e_{q1}, e_{q2}, \dots, e_{qm}] \in \mathbb{R}^{m \times d}$, $q \rightarrow E_q = [e_{q1}, e_{q2}, \dots, e_{qm}] \</annotation></semantics></math></span></span>in \mathbb{R}^ <span class="katex"><span class="katex-mathml"><math xmlns="http://www.w3.org/1998/Math/MathML"><semantics><annotation encoding="application/x-tex">{m \times d}$, $q \rightarrow E_q = [e_{q1}, e_{q2}</annotation><mrow><mo separator="true">,</mo><mo>\dots</mo></mrow><annotation encoding="application/x-tex">, e_{</annotation><mrow><msub><mrow><mi>qm</mi></mrow></msub><mo stretchy="false">}]</mo><mo>\</mo></mrow></semantics></math></span></span>in <span class="katex"><span class="katex-mathml"><math xmlns="http://www.w3.org/1998/Math/MathML"><semantics><mrow><mo>\</mo></mrow></semantics></math></span></span>mathbb{ <span class="katex"><span class="katex-mathml"><math xmlns="http://www.w3.org/1998/Math/MathML"><semantics><annotation encoding="application/x-tex">R</annotation></semantics></math></span></span>}^ <span class="katex"><span class="katex-mathml"><math xmlns="http://www.w3.org/1998/Math/MathML"><semantics><annotation encoding="application/x-tex">{m</annotation><mrow><mo>\</mo></mrow></semantics></math></span></span>times <span class="katex"><span class="katex-mathml"><math xmlns="http://www.w3.org/1998/Math/MathML"><semantics><annotation encoding="application/x-tex">d}$, $q \rightarrow E_q = [e_{q1}, e_{q2}, \dots, e_{</annotation><mrow><msub><mrow><mi>qm</mi></mrow></msub></mrow><annotation encoding="application/x-tex">}] \in \mathbb{R}^{m \times d}$, $q \rightarrow E_q = [e_{q1}, e_{q2}, \dots, e_{qm}] \in \mathbb{R}^{m \times d}$, $q \rightarrow E_q = [e_{q1}, e_{q2}, \dots, e_{</annotation><mrow><msub><mrow><mi>qm</mi></mrow></msub></mrow><annotation encoding="application/x-tex">}</annotation><mrow><mo stretchy="false">]</mo></mrow><annotation encoding="application/x-tex">\</annotation></semantics></math></span></span>in <span class="katex"><span class="katex-mathml"><math xmlns="http://www.w3.org/1998/Math/MathML"><semantics><annotation encoding="application/x-tex">\ \dots, e_{qm}] ∈ \R^{m×d}</annotation></semantics></math></span><span class="katex-html" aria-hidden="true"><span class="base"><span class="strut" style="height:0.625em;vertical-align:-0.1944em;"></span><span class="mord mathnormal" style="margin-right:0.03588em;">q</span></span></span></span><span class="mspace" style="margin-right:0.2778em;"></span><span class="base"><span class="mord"><span class="mord mathnormal" style="margin-right:0.05764em;"></span></span></span> <span class="katex"><span class="katex-html" aria-hidden="true"><span class="base"><span class="mrel">→</span></span></span></span><span class="mspace" style="margin-right:0.2778em;"></span><span class="base"><span class="mord"><span class="mord mathnormal" style="margin-right:0.05764em;"></span></span></span> <span class="katex"><span class="katex-html" aria-hidden="true"><span class="base"><span class="strut" style="height:0.9694em;vertical-align:-0.2861em;"></span><span class="mord"><span class="msupsub"><span class="vlist-t vlist-t2"><span class="vlist-r"><span class="vlist" style="height:0.1514em;"><span style="top:-2.55em;margin-left:-0.0576em;margin-right:0.05em;"><span class="sizing reset-size6 size3 mtight"><span class="mord mathnormal mtight" style="margin-right:0.03588em;"> </span></span></span></span></span></span></span></span></span></span></span> E <span class="katex"><span class="katex-html" aria-hidden="true"><span class="base"></span></span></span><span class="pstrut" style="height:2.7em;"></span><span class="mrel"></span> <span class="katex"><span class="katex-html" aria-hidden="true"><span class="base"><span class="mord"><span class="msupsub"><span class="vlist-t vlist-t2"><span class="vlist-r"><span class="vlist-s">q​</span></span></span></span></span></span></span></span><span class="vlist-r"><span class="vlist" style="height:0.2861em;"><span></span></span></span><span class="mrel"></span> <span class="katex"><span class="katex-html" aria-hidden="true"><span class="base"><span class="mspace" style="margin-right:0.2778em;"></span><span class="mrel"> =</span></span></span></span><span class="mspace" style="margin-right:0.2778em;"></span><span class="base"><span class="mopen"></span></span> <span class="katex"><span class="katex-html" aria-hidden="true"><span class="base"><span class="strut" style="height:1.0361em;vertical-align:-0.2861em;"></span><span class="mopen"> </span></span> [<span class="base"><span class="mord"><span class="mord mathnormal">e</span></span></span></span></span><span class="pstrut" style="height:2.7em;"></span><span class="mpunct"></span> <span class="katex"><span class="katex-html" aria-hidden="true"><span class="base"><span class="mord"><span class="msupsub"><span class="vlist-t vlist-t2"><span class="vlist-r"><span class="vlist-s">q1​</span></span></span></span></span></span></span></span><span class="vlist-r"><span class="vlist" style="height:0.2861em;"><span></span></span></span><span class="mpunct"></span> <span class="katex"><span class="katex-html" aria-hidden="true"><span class="base"><span class="mpunct">,</span><span class="mspace" style="margin-right:0.1667em;"></span><span class="mord"><span class="mord mathnormal">e</span></span></span></span></span><span class="pstrut" style="height:2.7em;"></span><span class="mpunct"></span> <span class="katex"><span class="katex-html" aria-hidden="true"><span class="base"><span class="mord"><span class="msupsub"><span class="vlist-t vlist-t2"><span class="vlist-r"><span class="vlist-s">q2​</span></span></span></span></span></span></span></span><span class="vlist-r"><span class="vlist" style="height:0.2861em;"><span></span></span></span><span class="mpunct"></span> <span class="katex"><span class="katex-html" aria-hidden="true"><span class="base"><span class="mpunct">,</span><span class="mspace" style="margin-right:0.1667em;"></span><span class="mpunct"></span><span class="minner">…</span><span class="mspace" style="margin-right:0.1667em;"></span><span class="mpunct">,</span><span class="mspace" style="margin-right:0.1667em;"></span><span class="mord"><span class="mord mathnormal">e</span></span></span></span></span><span class="pstrut" style="height:2.7em;"></span><span class="mclose"></span> <span class="katex"><span class="katex-html" aria-hidden="true"><span class="base"><span class="mord"><span class="msupsub"><span class="vlist-t vlist-t2"><span class="vlist-r"><span class="vlist-s">qm​</span></span></span></span></span></span></span></span><span class="vlist-r"><span class="vlist" style="height:0.2861em;"><span></span></span></span><span class="mclose"></span> <span class="katex"><span class="katex-html" aria-hidden="true"><span class="base"><span class="mclose">]</span><span class="mspace" style="margin-right:0.2778em;"></span><span class="mrel">∈</span></span></span></span><span class="mspace" style="margin-right:0.2778em;"></span><span class="base"><span class="mord"><span class="mord mathbb"></span></span></span> <span class="katex"><span class="katex-html" aria-hidden="true"><span class="base"><span class="strut" style="height:0.8491em;"></span><span class="mord"><span class="msupsub"><span class="vlist-t"><span class="vlist-r"><span class="vlist" style="height:0.8491em;"><span style="top:-3.063em;margin-right:0.05em;"><span class="sizing reset-size6 size3 mtight"><span class="mord mtight"><span class="mord mathnormal mtight"> </span></span></span></span></span></span></span></span></span></span></span></span> R <span class="katex"><span class="katex-html" aria-hidden="true"><span class="base"><span class="mord"><span class="msupsub"><span class="vlist-t"><span class="vlist-r"><span class="vlist" style="height:0.8491em;"><span style="top:-3.063em;margin-right:0.05em;"><span class="pstrut" style="height:2.7em;"></span><span class="sizing reset-size6 size3 mtight"><span class="mord mtight"><span class="mord mathnormal mtight"></span><span class="mord mathnormal mtight">m×d</span></span></span></span></span></span></span></span></span></span></span></span>. Ensuite, <strong>l’algorithme MAX_SIM</strong> a été appliqué pour comparer les deux listes d’embeddings et obtenir le score final entre la requête et la page du document.</p>
<h2 id="ColBERT-text-retrieval-system" class="common-anchor-header">Système de recherche textuelle ColBERT<button data-href="#ColBERT-text-retrieval-system" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>Dans cette section, nous allons mettre en place un système de recherche textuelle ColBERT à l’aide de StructArray. Avant cela, configurez une instance de Milvus v2.6.x et obtenez un jeton d’accès Cohere.</p>
<h3 id="Step-1-Install-the-dependencies" class="common-anchor-header">Étape 1 : Installation des dépendances<button data-href="#Step-1-Install-the-dependencies" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Exécutez la commande suivante pour installer les dépendances.</p>
...
<h3 id="Step-2-Load-the-Cohere-dataset" class="common-anchor-header">Étape 2 : Charger le jeu de données Cohere<button data-href="#Step-2-Load-the-Cohere-dataset" class="anchor-icon" translate="no">
    ...
    </button></h3><p>Dans cet exemple, nous allons utiliser le jeu de données Wikipédia de Cohere et récupérer les 10 000 premiers enregistrements. Vous trouverez des informations sur ce jeu de données sur <a href="https://huggingface.co/datasets/Cohere/wikipedia-2023-11-embed-multilingual-v3">cette page</a>.</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>L'exécution des scripts ci-dessus permettra de télécharger le jeu de données s'il n'est pas disponible localement. Chaque enregistrement du jeu de données correspond à un paragraphe d'une page Wikipédia. Le tableau suivant présente la structure de ce jeu de données.</p>
<table>
<thead>
<tr><th>Nom de la colonne</th><th>Description</th></tr>
</thead>
<tbody>
<tr><td><code translate="no">_id</code></td><td>ID de l'enregistrement</td></tr>
<tr><td><code translate="no">url</code></td><td>L'URL de l'enregistrement actuel.</td></tr>
<tr><td><code translate="no">title</code></td><td>Le titre du document source.</td></tr>
<tr><td><code translate="no">text</code></td><td>Un paragraphe tiré du document source.</td></tr>
<tr><td><code translate="no">emb</code></td><td>Représentations du texte issu du document source.</td></tr>
</tbody>
</table>
...
<button class="copy-code-btn"></button></code></pre>
<p>Dans ce code, nous stockons les paragraphes regroupés sous forme de documents et les incluons dans la liste « <code translate="no">data</code> ». Chaque document possède une clé « <code translate="no">paragraphs</code> », qui est une liste de paragraphes ; chaque objet « paragraphe » contient des clés « <code translate="no">text</code> » et « <code translate="no">emb</code> ».</p>
<h3 id="Step-4-Create-a-collection-for-the-Cohere-dataset" class="common-anchor-header">Étape 4 : Créer une collection pour l’ensemble de données Cohere<button data-href="#Step-4-Create-a-collection-for-the-Cohere-dataset" class="anchor-icon" translate="no">
    ...
    </button></h3><p>Une fois les données prêtes, nous allons créer une collection. Dans cette collection, ` <code translate="no">paragraphs</code> ` est un champ de type `StructArray`. Pour une explication générale des schémas `StructArray`, consultez la section <a href="/docs/fr/create-structarray-field.md">Créer un champ de type `StructArray`</a>.</p>
...
<h3 id="Step-5-Insert-Cohere-dataset-into-the-collection" class="common-anchor-header">Étape 5 : Insérer l’ensemble de données Cohere dans la collection<button data-href="#Step-5-Insert-Cohere-dataset-into-the-collection" class="anchor-icon" translate="no">
    ...
    </button></h3><p>Nous pouvons désormais insérer les données préparées dans la collection que nous avons créée ci-dessus.</p>
<pre><code translate="no" class="language-python">client.insert(
    collection_name=<span class="hljs-string">&#x27;wiki_documents&#x27;</span>,
    data=data
)
<button class="copy-code-btn"></button></code></pre>
<h3 id="Step-6-Search-within-the-Cohere-dataset" class="common-anchor-header">Étape 6 : Effectuer une recherche dans l’ensemble de données Cohere<button data-href="#Step-6-Search-within-the-Cohere-dataset" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Conformément à la conception de ColBERT, le texte de la requête doit être tokenisé, puis intégré dans une EmbeddingList. Au cours de cette étape, nous utiliserons le même modèle que celui utilisé par Cohere pour générer les embeddings des paragraphes du jeu de données Wikipédia.</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>Dans le code, les textes de requête sont organisés en tokens dans ` <code translate="no">query_inputs</code> ` et intégrés dans une liste de vecteurs flottants. Vous pouvez ensuite utiliser l’`EmbeddingList` de Milvus pour effectuer une recherche par similarité comme suit.</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>Chaque score de similarité cosinus par paire varie entre <code translate="no">-1</code> et <code translate="no">1</code>. Le score final d’ <code translate="no">MAX_SIM_COSINE</code> e peut être supérieur à <code translate="no">1</code> car il agrège plusieurs scores de similarité maximale au niveau des tokens.</p>
<h2 id="ColPali-document-retrieval-system" class="common-anchor-header">Système de recherche de documents ColPali<button data-href="#ColPali-document-retrieval-system" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>Dans cette section, nous allons mettre en place un système de recherche de documents basé sur ColPali à l’aide de StructArray. Avant cela, configurez une instance de Milvus v2.6.x.</p>
<h3 id="Step-1-Install-the-dependencies" class="common-anchor-header">Étape 1 : Installez les dépendances<button data-href="#Step-1-Install-the-dependencies" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<button class="copy-code-btn"></button></code></pre>
<h3 id="Step-2-Load-the-Vidore-dataset" class="common-anchor-header">Étape 2 : Chargement du jeu de données Vidore<button data-href="#Step-2-Load-the-Vidore-dataset" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Dans cette section, nous utiliserons un ensemble de données Vidore nommé <strong>vidore_v2_finance_en</strong>. Cet ensemble de données est un corpus de rapports annuels issus du secteur bancaire, destiné à des tâches de compréhension de documents longs. Il s’agit de l’un des 10 corpus composant le benchmark ViDoRe v3. Vous trouverez plus de détails sur cet ensemble de données sur <a href="https://huggingface.co/datasets/vidore/vidore_v3_finance_en">cette page</a>.</p>
<pre><code translate="no" class="language-python"><span class="hljs-keyword">from</span> datasets <span class="hljs-keyword">import</span> load_dataset
...
<button class="copy-code-btn"></button></code></pre>
<p>L'exécution des scripts ci-dessus permettra de télécharger le jeu de données s'il n'est pas disponible localement. Chaque enregistrement du jeu de données correspond à une page d'un rapport financier. Le tableau suivant présente la structure de ce jeu de données.</p>
<table>
<thead>
<tr><th>Nom de la colonne</th><th>Description</th></tr>
</thead>
<tbody>
<tr><td><code translate="no">corpus_id</code></td><td>Un enregistrement du corpus</td></tr>
<tr><td><code translate="no">image</code></td><td>L'image de la page en octets.</td></tr>
<tr><td><code translate="no">doc_id</code></td><td>L'identifiant descriptif du document.</td></tr>
<tr><td><code translate="no">page_number_in_doc</code></td><td>Le numéro de la page actuelle dans le document.</td></tr>
</tbody>
</table>
<h3 id="Step-3-Generate-embeddings-for-the-page-images" class="common-anchor-header">Étape 3 : Générer des représentations vectorielles pour les images de page<button data-href="#Step-3-Generate-embeddings-for-the-page-images" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Comme illustré dans la section <a href="/docs/fr/search-with-embedding-lists.md#overview">« Présentation »</a>, le modèle ColPali est un VLM qui projette des images dans l’espace vectoriel d’un modèle textuel. Dans cette étape, nous utiliserons la dernière version du modèle ColPali <strong>: vidore/colpali-v1.3</strong>. Vous trouverez plus de détails sur ce modèle sur <a href="https://huggingface.co/vidore/colpali-v1.3">cette page</a>.</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>Une fois le modèle prêt, vous pouvez essayer de générer des patches pour une image spécifique comme suit.</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>Dans le code ci-dessus, le modèle ColPali redimensionne l’image à 448 x 448 pixels, puis la divise en fragments mesurant chacun 14 x 14 pixels. Enfin, ces fragments sont représentés par 1 031 vecteurs d’encodage, chacun comportant 128 dimensions.</p>
<p>Vous pouvez générer des représentations pour toutes les images à l’aide d’une boucle comme suit :</p>
...
<button class="copy-code-btn"></button></code></pre>
<div class="alert note">
<p>Cette étape est relativement longue en raison du volume important de données à représenter.</p>
</div>
<h3 id="Step-4-Create-a-collection-for-the-financial-reports-dataset" class="common-anchor-header">Étape 4 : Créer une collection pour l’ensemble de données des rapports financiers<button data-href="#Step-4-Create-a-collection-for-the-financial-reports-dataset" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Une fois les données prêtes, nous allons créer une collection. Dans cette collection, ` <code translate="no">patches</code> ` est un champ de type `StructArray`. Chaque élément de la structure stocke un encodage de segment. Pour connaître les exigences d’indexation des sous-champs vectoriels de `StructArray`, consultez la section <a href="/docs/fr/index-structarray-fields.md">Indexer les champs StructArray</a>.</p>
<pre><code translate="no" class="language-python"><span class="hljs-keyword">from</span> pymilvus <span class="hljs-keyword">import</span> MilvusClient, DataType
...
<h3 id="Step-5-Insert-the-financial-reports-into-the-collection" class="common-anchor-header">Étape 5 : Insérer les rapports financiers dans la collection<button data-href="#Step-5-Insert-the-financial-reports-into-the-collection" class="anchor-icon" translate="no">
    ...
    </button></h3><p>Nous pouvons désormais insérer les rapports financiers préparés dans la collection.</p>
<pre><code translate="no" class="language-python">client.insert(
    collection_name=<span class="hljs-string">&quot;financial_reports&quot;</span>,
    data=data
)
<button class="copy-code-btn"></button></code></pre>
<div class="alert note">
<p>L’insertion des rapports financiers peut prendre beaucoup de temps. Chaque page peut contenir plus de mille vecteurs de patch, et chaque vecteur est stocké dans le champ StructArray « <code translate="no">patches</code> ». Pour les ensembles de données plus volumineux, divisez « <code translate="no">data</code> » en lots plus petits et insérez-les un par un.</p>
</div>
<p>Dans la sortie, vous pouvez constater que toutes les pages de l’ensemble de données Vidore ont été insérées.</p>
<h3 id="Step-6-Search-within-the-financial-reports" class="common-anchor-header">Étape 6 : Effectuer une recherche dans les rapports financiers<button data-href="#Step-6-Search-within-the-financial-reports" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Une fois les données prêtes, nous pouvons effectuer des recherches dans les données de la collection comme suit :</p>
<pre><code translate="no" class="language-python"><span class="hljs-keyword">from</span> pymilvus.client.embedding_list <span class="hljs-keyword">import</span> EmbeddingList
