...
summary: >-
  ...
  recherche ANN en fonction de la valeur d'un champ et réduire le nombre
  d'entités en double.
---
...
<p>Lorsqu’on effectue une recherche par « voisin le plus proche approximatif » (ANN) sur une telle collection, les résultats de recherche peuvent inclure plusieurs paragraphes issus du même document, ce qui risque de faire passer d’autres documents inaperçus, ce qui peut ne pas correspondre au cas d’utilisation prévu.</p>
<p><span class="img-wrapper">
...
<p>Pour améliorer la diversité des résultats de recherche, vous pouvez ajouter le paramètre « <code translate="no">group_by_field</code> » dans la requête de recherche afin d’activer la recherche par regroupement. Comme le montre le schéma, vous pouvez définir « <code translate="no">group_by_field</code> » sur « <code translate="no">docId</code> ». À la réception de cette requête, Milvus :</p>
<ul>
<li><p>Effectuera une recherche ANN à partir du vecteur de requête fourni afin de trouver toutes les entités les plus similaires à la requête.</p></li>
...
<p>Par défaut, la recherche groupée ne renvoie qu’une seule entité par groupe. Si vous souhaitez augmenter le nombre de résultats à renvoyer par groupe, vous pouvez le faire à l’aide des paramètres ` <code translate="no">group_size</code> ` et ` <code translate="no">strict_group_size</code> `.</p>
</div>
...
<li><p><code translate="no">strict_group_size</code>: Ce paramètre booléen détermine si le système doit respecter strictement le nombre défini par <code translate="no">group_size</code>. Lorsque <code translate="no">strict_group_size=True</code> est activé, le système tentera d’inclure le nombre exact d’entités spécifié par <code translate="no">group_size</code> dans chaque groupe (par exemple, deux paragraphes), sauf si ce groupe ne contient pas suffisamment de données. Par défaut (<code translate="no">strict_group_size=False</code>), le système privilégie le respect du nombre de groupes spécifié par le paramètre <code translate="no">limit</code>, plutôt que de s’assurer que chaque groupe contienne <code translate="no">group_size</code> entités. Cette approche est généralement plus efficace lorsque la répartition des données est inégale.</p></li>
</ul>
...
<h2 id="Order-groups-by-a-scalar-field--Milvus-30x" class="common-anchor-header">Trier les groupes selon un champ scalaire<span class="beta-tag" style="background-color:rgb(0, 179, 255);color:white" translate="no">Compatible with Milvus 3.0.x</span><button data-href="#Order-groups-by-a-scalar-field--Milvus-30x" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Vous pouvez combiner la recherche par regroupement (Grouping Search) avec la recherche par champ scalaire ( <code translate="no">order_by_fields</code> ) pour trier les groupes selon un champ scalaire. Cela s’avère utile lorsque vous souhaitez obtenir des résultats variés entre les groupes, tout en veillant à ce que ceux-ci respectent un ordre pertinent pour l’activité, tel que le prix ou la note.</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>Dans la requête ci-dessus, « <code translate="no">limit=20</code> » signifie que Milvus sélectionne jusqu’à 20 groupes, et non 20 entités. Étant donné que « <code translate="no">group_size=3</code> », la liste plate des résultats peut contenir jusqu’à 60 entités au total.</p>
...
<li><p><strong>Nombre de groupes</strong>: le paramètre ` <code translate="no">limit</code> ` contrôle le nombre de groupes à partir desquels les résultats de recherche sont renvoyés, plutôt que le nombre spécifique d’entités au sein de chaque groupe. Définir une valeur appropriée pour ` <code translate="no">limit</code> ` permet de contrôler la diversité des résultats et les performances de la requête. Réduire la valeur de ` <code translate="no">limit</code> ` peut diminuer les coûts de calcul si les données sont densément réparties ou si les performances constituent un enjeu.</p></li>
<li><p><strong>Entités par groupe</strong>: le paramètre <code translate="no">group_size</code> contrôle le nombre d’entités renvoyées par groupe. Ajuster l’ <code translate="no">group_size</code> en fonction de votre cas d’utilisation peut enrichir les résultats de recherche. Toutefois, si les données sont réparties de manière inégale, certains groupes peuvent renvoyer moins d’entités que ce qui est spécifié par l’ <code translate="no">group_size</code>, en particulier dans les scénarios où les données sont limitées.</p></li>
<li><p><strong>Taille de groupe stricte</strong>: lorsque <code translate="no">strict_group_size=True</code> est activé, le système tentera de renvoyer le nombre d’entités spécifié (<code translate="no">group_size</code>) pour chaque groupe, sauf si les données disponibles dans ce groupe sont insuffisantes. Ce paramètre garantit un nombre d’entités constant par groupe, mais peut entraîner une dégradation des performances en cas de répartition inégale des données ou de ressources limitées. Si un nombre strict d’entités n’est pas requis, la configuration de <code translate="no">strict_group_size=False</code> peut améliorer la vitesse d’exécution des requêtes.</p></li>
<li><p>Si les vecteurs de requête existent déjà dans la collection cible, envisagez d’utiliser <code translate="no">ids</code> plutôt que de les récupérer avant les recherches. Pour plus de détails, reportez-vous à <a href="/docs/fr/primary-key-search.md">la section Recherche par clé primaire</a>.</p></li>
</ul>
