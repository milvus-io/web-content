...
<h2 id="Snapshot-anatomy" class="common-anchor-header">Anatomie d’un instantané<button data-href="#Snapshot-anatomy" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Milvus met en œuvre une architecture de snapshots basée sur des manifestes pour une capture, un stockage et une restauration efficaces des données à un instant donné, sans dupliquer les données vectorielles réelles. Cette architecture sépare la gestion des métadonnées du stockage physique des données, ce qui permet d’obtenir des snapshots légers faisant référence à des fichiers de segments existants dans le stockage objet.</p>
...
<p>Elles fournissent les informations de base nécessaires à la création du snapshot, notamment son nom et sa description, l’ID de la collection cible, ainsi que l’instant auquel le snapshot a été créé.</p></li>
...
<h2 id="Storage-impacts-and-considerations" class="common-anchor-header">Impacts sur le stockage et considérations<button data-href="#Storage-impacts-and-considerations" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Une fois que Milvus référence un segment ou un fichier d’index dans un instantané, il ne procède pas au ramassage des fichiers inutiles (garbage collection) tant que vous n’avez pas supprimé l’instantané. Les instantanés consomment de l’espace de stockage proportionnellement à la taille des collections cibles, et des coûts de stockage d’objets s’appliquent à la conservation des instantanés. Dans les cas extrêmes, un seul instantané peut même doubler vos coûts de stockage d’objets. Il est conseillé de</p>
<ul>
