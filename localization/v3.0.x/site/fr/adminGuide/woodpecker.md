...
<li>Woodpecker peut fonctionner <strong>de manière intégrée</strong> au nœud Milvus/streaming (par défaut), ou en tant que <strong>service dédié</strong> avec ses propres pods (distribué/cluster uniquement).</li>
...
<li>Ajustez l’ <code translate="no">maxInterval</code> en fonction des caractéristiques du support (sacrifiez la latence au profit du débit en optant pour une agrégation plus longue).</li>
...
<li><strong>Téléchargement immédiat vers le stockage objet après le roulement d’un segment</strong> — chaque segment suit l’intégralité de son cycle de vie et est téléchargé vers le stockage objet dès son roulement, ce qui permet de maintenir un faible encombrement sur le disque local et des coûts de stockage réduits sans compromettre la latence.</li>
