...
summary: >-
  Présentation du stockage des métadonnées, du stockage d'objets et des files
  d'attente de messages utilisés par Milvus.
---
<h1 id="Data-Infrastructure" class="common-anchor-header">Infrastructure de données<button data-href="#Data-Infrastructure" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h1><p>Milvus s'appuie sur un système de stockage de métadonnées, un système de stockage d'objets et une file d'attente de messages pour son infrastructure de données principale. Ce chapitre présente les composants que vous pouvez configurer :</p>
<ul>
