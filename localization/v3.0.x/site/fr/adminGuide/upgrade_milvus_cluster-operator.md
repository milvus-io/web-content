...
<h1 id="Upgrade-Milvus-Cluster-with-Milvus-Operator" class="common-anchor-header">Mise à niveau d'un cluster Milvus avec Milvus Operator<button data-href="#Upgrade-Milvus-Cluster-with-Milvus-Operator" class="anchor-icon" translate="no">
    ...
    </button></h1><p>Ce guide décrit comment mettre à niveau un cluster Milvus 2.6.x vers la version 3.0.1 à l’aide de Milvus Operator.</p>
<div class="alert note">
<p>Cette procédure a été validée pour une mise à niveau de Milvus 2.6.20 vers Milvus v3.0.1 avec Milvus Operator 1.3.0, MixCoord, StreamingNode, Woodpecker, etcd au sein du cluster et MinIO au sein du cluster. Si vous utilisez une autre version de Milvus 2.6.x, une autre version d’Operator, une topologie de composants différente, une file d’attente de messages différente ou une configuration des dépendances différente, testez d’abord la mise à niveau dans un environnement hors production.</p>
</div>
...
<li>Un cluster Kubernetes comprenant un cluster Milvus 2.6.x géré par Milvus Operator</li>
...
<p>Appliquez la CR Milvus complète pour cette mise à niveau. N’utilisez pas de patch de fusion contenant uniquement une image. L’Operator peut définir par défaut les champs de composants à réplique nulle qui ont été omis, ce qui peut réactiver un composant que le déploiement 2.6.x existant avait désactivé.</p>
<p>Cette procédure ne valide pas une rétrogradation ou une annulation de la mise à jour consistant à ramener l’image Milvus à la version 2.6.x. Une fois que la version 3.0.1 a écrit des données, une annulation portant uniquement sur l’image peut ne pas parvenir à lire l’état mis à jour. Si la mise à niveau échoue, arrêtez les écritures et utilisez un plan de reprise qui restaure les métadonnées et les sauvegardes de données persistantes antérieures à la mise à niveau. Validez d’abord le plan de reprise dans un environnement hors production.</p>
</div>
<h2 id="Upgrade-process" class="common-anchor-header">Processus de mise à niveau<button data-href="#Upgrade-process" class="anchor-icon" translate="no">
    ...
    </button></h2><h3 id="Step-1-Back-up-the-current-Milvus-CR" class="common-anchor-header">Étape 1 : Sauvegarder le CR Milvus actuel<button data-href="#Step-1-Back-up-the-current-Milvus-CR" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<button class="copy-code-btn"></button></code></pre>
<p>La mise à niveau validée a conservé Milvus Operator à la version 1.3.0. Conservez la version de l’Operator qui gère actuellement votre déploiement Milvus 2.6.x, sauf si votre politique de support exige une mise à niveau distincte de l’Operator. Ne revenez pas à une version antérieure d’un Operator plus récent que la version testée. Si vous devez changer de version d’Operator, utilisez la même méthode d’installation (Helm ou <code translate="no">kubectl</code> ) ainsi que le même nom de version et le même espace de noms que l’installation existante, puis validez la modification de l’Operator avant de mettre à jour le CR Milvus.</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>Vérifiez que le CR Milvus indique « <code translate="no">Healthy</code> », que tous les composants Milvus utilisent « <code translate="no">milvusdb/milvus:v3.0.1</code> », qu’aucun pod IndexNode n’est en cours d’exécution et que les collections existantes restent consultables et recherchables. Effectuez ces vérifications avant d’activer toute fonctionnalité spécifique à la version v3.0.1.</p>
