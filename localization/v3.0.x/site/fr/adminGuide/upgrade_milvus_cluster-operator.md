...
<h1 id="Upgrade-Milvus-Cluster-with-Milvus-Operator" class="common-anchor-header">Mise à niveau d'un cluster Milvus avec Milvus Operator<button data-href="#Upgrade-Milvus-Cluster-with-Milvus-Operator" class="anchor-icon" translate="no">
    ...
    </button></h1><p>Ce guide décrit comment mettre à niveau un cluster Milvus 2.6.x vers la version 3.0.2 à l'aide de Milvus Operator.</p>
<div class="alert note">
<p>Cette procédure a été validée pour une mise à niveau de Milvus 2.6.20 vers Milvus v3.0.2 avec Milvus Operator 1.3.0, MixCoord, StreamingNode, Woodpecker, etcd au sein du cluster et MinIO au sein du cluster. Si vous utilisez une autre version de patch Milvus 2.6.x, une autre version d’Operator, une autre topologie de composants, une autre file d’attente de messages ou une autre configuration des dépendances, testez d’abord la mise à niveau dans un environnement hors production.</p>
</div>
...
<button class="copy-code-btn"></button></code></pre>
<p>La mise à niveau validée a conservé Milvus Operator à la version 1.3.0. Conservez la version de l’Operator qui gère actuellement votre déploiement Milvus 2.6.x, sauf si votre politique de support exige une mise à niveau distincte de l’Operator. Ne revenez pas à une version antérieure d’un Operator plus récent vers la version testée. Si vous devez changer de version d’Operator, utilisez la même méthode d’installation (Helm ou <code translate="no">kubectl</code> ) ainsi que le même nom de version et le même espace de noms que l’installation existante, puis validez le changement d’Operator avant de mettre à jour le CR Milvus.</p>
...
<p>Avant d’appliquer la CR cible, vérifiez que <code translate="no">indexNode.replicas</code> correspond à <code translate="no">0</code>. La configuration Milvus 2.6.20 validée utilisait déjà ce paramètre. Conservez le paramètre explicite « zero-replica » dans la CR cible.</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>Vérifiez que le CR Milvus indique « <code translate="no">Healthy</code> », que tous les composants Milvus utilisent « <code translate="no">milvusdb/milvus:v3.0.2</code> », qu’aucun pod IndexNode n’est en cours d’exécution et que les collections existantes restent consultables et recherchables. Effectuez ces vérifications avant d’activer toute fonctionnalité spécifique à la version 3.0.2.</p>
