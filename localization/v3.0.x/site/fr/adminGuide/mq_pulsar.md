...
<tbody>
<tr><td>2.5.x et versions ultérieures</td><td>Pulsar v3 (recommandé) ou Pulsar v2</td><td>Pulsar v3 (via Helm / Milvus Operator)</td></tr>
<tr><td>2.4.x et antérieures</td><td>Pulsar v2</td><td>Pulsar v2</td></tr>
</tbody>
</table>
...
<h2 id="Deploy-a-Milvus-cluster-with-Pulsar-using-Helm" class="common-anchor-header">Déployer un cluster Milvus avec Pulsar à l’aide de Helm<button data-href="#Deploy-a-Milvus-cluster-with-Pulsar-using-Helm" class="anchor-icon" translate="no">
    ...
    </button></h3><p>Pour déployer un cluster Milvus utilisant la version intégrée de Pulsar (au lieu de Woodpecker), installez le chart Helm avec le nœud de streaming activé :</p>
...
<h3 id="Configure" class="common-anchor-header">Configuration<button data-href="#Configure" class="anchor-icon" translate="no">
    ...
    </button></h3><p>Pour connecter Milvus à un service Pulsar <strong>externe</strong>, désactivez le Pulsar fourni et activez la configuration « <code translate="no">externalPulsar</code> » dans une surc ion de la configuration de cluster ( <code translate="no">values.yaml</code> ) :</p>
...
<li><strong>Mise à niveau de la version 2.5.x vers la version 2.6.x :</strong> <strong>limitations de la file d’attente de messages</strong>: lors de la mise à niveau vers Milvus v3.0.1, vous devez conserver votre choix actuel de file d’attente de messages. Le passage d’un système de file d’attente de messages à un autre pendant la mise à niveau n’est pas pris en charge. La prise en charge du changement de système de file d’attente de messages sera disponible dans les versions futures.
...
<li><a href="/docs/fr/switch-pulsar-woodpecker.md">Basculer entre Pulsar et Woodpecker</a></li>
</ul>
