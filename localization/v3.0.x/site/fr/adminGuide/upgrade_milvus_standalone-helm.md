...
<h1 id="Upgrade-Milvus-Standalone-with-Helm-Chart" class="common-anchor-header">Mise à niveau de Milvus Standalone à l'aide d'un Helm Chart<button data-href="#Upgrade-Milvus-Standalone-with-Helm-Chart" class="anchor-icon" translate="no">
    ...
    </button></h1><p>Ce guide décrit comment mettre à niveau votre déploiement autonome Milvus 2.6.x vers la version v3.0.2 à l'aide de Helm.</p>
<div class="alert note">
<p>Cette procédure a été validée de Milvus 2.6.20 à Milvus v3.0.2 avec le Helm Chart Milvus 5.0.22. Si vous utilisez une autre version de Milvus 2.6.x ou une autre version du Helm Chart, validez d'abord la mise à niveau dans un environnement hors production.</p>
</div>
...
<p>Cette procédure ne valide pas de retour à une version antérieure ni de restauration consistant à ramener l’image Milvus à la version 2.6.x. Une fois que la version 3.0.2 a écrit des données, une restauration portant uniquement sur l’image peut ne pas parvenir à lire l’état mis à jour. Si la mise à niveau échoue, arrêtez les écritures et utilisez un plan de reprise qui restaure les métadonnées antérieures à la mise à niveau ainsi que les sauvegardes des données persistantes. Validez d’abord le plan de reprise dans un environnement hors production.</p>
</div>
...
<button class="copy-code-btn"></button></code></pre>
<p>Vérifiez que toutes les charges de travail requises sont prêtes, que Milvus utilise <code translate="no">v3.0.2</code> et que vos collections existantes restent consultables et recherchables. Effectuez ces vérifications avant d’activer toute fonctionnalité spécifique à la version 3.0.2.</p>
<div class="alert note">
