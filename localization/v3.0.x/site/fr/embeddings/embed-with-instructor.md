...
order: 10
summary: >-
  Cet article décrit comment utiliser la fonction InstructorEmbeddingFunction
  pour encoder des documents et des requêtes à l'aide du modèle d'encodage
  Instructor.
title: Instructeur
---
<h1 id="Instructor" class="common-anchor-header">Instructeur<button data-href="#Instructor" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h1><p><a href="https://instructor-embedding.github.io/">Instructor</a> est un modèle d'intégration de texte basé sur les instructions qui peut générer des intégrations de texte adaptées à n'importe quelle tâche (par exemple, la classification, l'extraction, le regroupement, l'évaluation de texte, etc.) et à n'importe quel domaine (par exemple, la science, la finance, etc.) en fournissant simplement les instructions relatives à la tâche, sans aucune mise au point.</p>
<p>Milvus s'intègre aux modèles d'intégration d'Instructor via la classe InstructorEmbeddingFunction. Cette classe fournit des méthodes pour coder les documents et les requêtes à l'aide des modèles d'intégration d'Instructor et pour renvoyer les intégrations sous forme de vecteurs denses compatibles avec l'indexation de Milvus.</p>
<p>Pour utiliser cette fonctionnalité, installez les dépendances nécessaires :</p>
<pre><code translate="no" class="language-python">pip install --upgrade pymilvus
pip install <span class="hljs-string">&quot;pymilvus[model]&quot;</span>
<button class="copy-code-btn"></button></code></pre>
<p>Ensuite, instanciez la classe InstructorEmbeddingFunction :</p>
...
<p>Le nom du modèle d'intégration de Mistral AI à utiliser pour l'encodage. La valeur par défaut est <code translate="no">hkunlp/instructor-xl</code>. Pour plus d'informations, voir <a href="https://github.com/xlang-ai/instructor-embedding?tab=readme-ov-file#model-list">Liste des modèles</a>.</p></li>
<li><p><code translate="no">query_instruction</code> <em>(chaîne</em>)</p>
<p>Instruction spécifique à la tâche qui guide le modèle sur la manière de générer un embedding pour une requête ou une question.</p></li>
<li><p><code translate="no">doc_instruction</code> <em>(chaîne</em>)</p>
<p>Instruction spécifique à la tâche qui guide le modèle pour générer un embedding pour un document.</p></li>
</ul>
