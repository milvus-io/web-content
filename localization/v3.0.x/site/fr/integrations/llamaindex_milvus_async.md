...
title: Créer un système RAG avec LlamaIndex et l'API asynchrone de Milvus
related_key: LlamaIndex
summary: >-
  Ce tutoriel explique comment utiliser LlamaIndex avec Milvus pour créer un
  pipeline de traitement asynchrone de documents destiné à la technologie RAG.
  LlamaIndex permet de traiter des documents et de les stocker dans une base de
  données vectorielle telle que Milvus. En tirant parti de l'API asynchrone de
  LlamaIndex et de la bibliothèque cliente Python de Milvus, nous pouvons
  augmenter le débit du pipeline afin de traiter et d'indexer efficacement de
  grands volumes de données.
---
<h1 id="RAG-with-Milvus-and-LlamaIndex-Async-API" class="common-anchor-header">RAG avec Milvus et l'API asynchrone de LlamaIndex<button data-href="#RAG-with-Milvus-and-LlamaIndex-Async-API" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<p>Ce tutoriel explique comment utiliser <a href="https://www.llamaindex.ai/">LlamaIndex</a> avec <a href="https://milvus.io/">Milvus</a> pour créer un pipeline de traitement asynchrone des documents destiné au RAG. LlamaIndex permet de traiter des documents et de les stocker dans une base de données vectorielle telle que Milvus. En tirant parti de l’API asynchrone de LlamaIndex et de la bibliothèque cliente Python de Milvus, nous pouvons augmenter le débit du pipeline afin de traiter et d’indexer efficacement de grands volumes de données.</p>
<p>Dans ce tutoriel, nous présenterons d’abord l’utilisation des méthodes asynchrones pour créer un RAG avec LlamaIndex et Milvus d’un point de vue général, puis nous aborderons l’utilisation des méthodes de bas niveau ainsi que la comparaison des performances entre les approches synchrones et asynchrones.</p>
<h2 id="Before-you-begin" class="common-anchor-header">Avant de commencer<button data-href="#Before-you-begin" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Les extraits de code de cette page nécessitent les dépendances pymilvus et llamaindex. Vous pouvez les installer à l’aide des commandes suivantes :</p>
...
<p>Nous utiliserons les modèles d’OpenAI. Vous devez définir la <a href="https://platform.openai.com/docs/quickstart">clé API</a> <code translate="no">OPENAI_API_KEY</code> en tant que variable d’environnement.</p>
<pre><code translate="no" class="language-python"><span class="hljs-keyword">import</span> os

os.environ[<span class="hljs-string">&quot;OPENAI_API_KEY&quot;</span>] = <span class="hljs-string">&quot;sk-***********&quot;</span>
<button class="copy-code-btn"></button></code></pre>
<p>Si vous utilisez Jupyter Notebook, vous devez exécuter cette ligne de code avant de lancer le code asynchrone.</p>
<pre><code translate="no" class="language-python"><span class="hljs-keyword">import</span> nest_asyncio

nest_asyncio.apply()
<button class="copy-code-btn"></button></code></pre>
<h3 id="Prepare-data" class="common-anchor-header">Préparation des données<button data-href="#Prepare-data" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Vous pouvez télécharger des exemples de données à l’aide des commandes suivantes :</p>
...
<button class="copy-code-btn"></button></code></pre>
<h2 id="Build-RAG-with-Asynchronous-Processing" class="common-anchor-header">Créer un système RAG avec traitement asynchrone<button data-href="#Build-RAG-with-Asynchronous-Processing" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>Cette section explique comment créer un système RAG capable de traiter des documents de manière asynchrone.</p>
<p>Importez les bibliothèques nécessaires et définissez l'URI Milvus ainsi que la dimension de l'embedding.</p>
...
<div class="alert note">
<ul>
<li>Si vous disposez d’un volume important de données, vous pouvez configurer un serveur Milvus performant sur <a href="https://milvus.io/docs/quickstart.md">Docker ou Kubernetes</a>. Dans cette configuration, veuillez utiliser l’URI du serveur, par exemple<code translate="no">http://localhost:19530</code>, comme <code translate="no">uri</code>.</li>
<li>Si vous souhaitez utiliser <a href="https://zilliz.com/cloud">Zilliz Cloud</a>, le service cloud entièrement géré pour Milvus, adaptez les paramètres ` <code translate="no">uri</code> ` et ` <code translate="no">token</code>`, qui correspondent respectivement au <a href="https://docs.zilliz.com/docs/on-zilliz-cloud-console#free-cluster-details">point de terminaison public et</a> à <a href="https://docs.zilliz.com/docs/on-zilliz-cloud-console#free-cluster-details">la clé API</a> dans Zilliz Cloud.</li>
<li>Dans le cas de systèmes complexes (tels que la communication réseau), le traitement asynchrone peut apporter une amélioration des performances par rapport à la synchronisation. Nous estimons donc que Milvus-Lite n’est pas adapté à l’utilisation d’interfaces asynchrones, car les scénarios d’utilisation ne s’y prêtent pas.</li>
</ul>
</div>
<p>Définissez une fonction d’initialisation que nous pourrons réutiliser pour reconstruire la collection Milvus.</p>
...
<p>Utilisez SimpleDirectoryReader pour encapsuler un objet document LlamaIndex à partir du fichier <code translate="no">paul_graham_essay.txt</code>.</p>
...
<p>Instanciez localement un modèle d’embedding Hugging Face. L’utilisation d’un modèle local évite le risque d’atteindre les limites de débit de l’API lors de l’insertion asynchrone de données, car les requêtes API simultanées peuvent rapidement s’accumuler et épuiser votre budget sur l’API publique. Toutefois, si vous disposez d’une limite de débit élevée, vous pouvez choisir d’utiliser un service de modèle distant à la place.</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>Créez un index et insérez le document.</p>
<p>Nous définissons la variable ` <code translate="no">use_async</code> ` sur ` <code translate="no">True</code> ` pour activer le mode d’insertion asynchrone.</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>Lors de la création du moteur de requête, vous pouvez également définir le paramètre ` <code translate="no">use_async</code> ` sur ` <code translate="no">True</code> ` pour activer la recherche asynchrone.</p>
...
<h2 id="Explore-the-Async-API" class="common-anchor-header">Découvrez l’API asynchrone<button data-href="#Explore-the-Async-API" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>Dans cette section, nous allons présenter l’utilisation de l’API de bas niveau et comparer les performances des exécutions synchrones et asynchrones.</p>
<h3 id="Async-add" class="common-anchor-header">Ajout asynchrone<button data-href="#Async-add" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Réinitialisez le magasin de vecteurs.</p>
...
<p>Définissons une fonction de génération de nœuds, qui servira à générer un grand nombre de nœuds de test pour l’index.</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>Définissons une fonction asynchrone pour ajouter des documents au magasin de vecteurs. Nous utilisons la fonction ` <code translate="no">async_add()</code> ` de l’instance du magasin de vecteurs Milvus.</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>Récupérons la boucle d'événements.</p>
<pre><code translate="no" class="language-python">loop = asyncio.get_event_loop()
<button class="copy-code-btn"></button></code></pre>
<p>Ajoutez des documents de manière asynchrone au magasin de vecteurs.</p>
...
<h4 id="Compare-with-synchronous-add" class="common-anchor-header">Comparez avec l’ajout synchrone.</h4><p>Définissons une fonction d’ajout synchrone. Mesurons ensuite le temps d’exécution dans les mêmes conditions.</p>
...
<p>Le résultat montre que le processus d’ajout synchrone est bien plus lent que l’ajout asynchrone.</p>
<h3 id="Async-search" class="common-anchor-header">Recherche asynchrone<button data-href="#Async-search" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Réinitialisez le magasin de vecteurs et ajoutez quelques documents avant de lancer la recherche.</p>
<pre><code translate="no" class="language-python">vector_store = init_vector_store()
...
<p>Définissez une fonction de recherche asynchrone. Nous utilisons la fonction ` <code translate="no">aquery()</code> ` de l’instance du magasin de vecteurs Milvus.</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>Effectuez une recherche asynchrone à partir du magasin Milvus.</p>
...
<h4 id="Compare-with-synchronous-search" class="common-anchor-header">Comparaison avec la recherche synchrone</h4><p>Définissez une fonction de recherche synchrone. Mesurez ensuite le temps d'exécution dans les mêmes conditions.</p>
...
<p>Les résultats montrent que le processus de recherche synchrone est nettement plus lent que celui asynchrone.</p>
