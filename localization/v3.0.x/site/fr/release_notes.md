...
<p>Milvus v3.0-beta amorce le passage de Milvus d'une base de données vectorielle à un moteur de lac sémantique-natif. Le noyau Milvus peut désormais fonctionner directement sur des données dans des formats de lac ouverts, et les capacités principales de Milvus ont été étendues à la recherche, au schéma, au cycle de vie, au langage et aux opérations.</p>
<p>La collecte externe et l'instantané sont les principaux ajouts du côté du lac. Le même noyau alimente également Zilliz Lakebase, une plateforme de données sémantique-native construite sur Milvus 3.0.</p>
...
<h4 id="External-Backfill" class="common-anchor-header">Remplissage externe</h4><p>La mise à niveau d'un modèle d'intégration, comme le passage des intégrations v1 aux intégrations v2 dans une collection existante, impliquait auparavant une reconstruction à partir de zéro. Cela entraînait des interruptions de service ou une logique de double écriture du côté de l'application.</p>
<p>Milvus 3.0 prend en charge la mise à niveau en tant que flux de travail à chaud. Vous pouvez ajouter un nouveau champ vectoriel à l'aide de <code translate="no">AddCollectionField</code>, utiliser Snapshot pour geler un point de départ cohérent, exécuter le travail d'intégration hors ligne par rapport à Snapshot et réécrire les valeurs par les voies d'ingestion normales. Une fois le nouveau champ indexé en ligne, l'application peut basculer sans interruption de service.</p>
<h4 id="Query--Search-Order-By" class="common-anchor-header">Requête / Recherche - Ordre par</h4><p>Les fonctions de recherche et d'interrogation acceptent désormais l'ordre sur plusieurs champs, le tri étant poussé vers le bas dans le noyau Milvus et <code translate="no">ASC</code> / <code translate="no">DESC</code> pouvant être configuré par champ. Cela comble une lacune fréquente dans la production : Le Top-K par distance seul ne répond souvent pas aux besoins de l'entreprise lorsque l'élément le plus similaire n'est pas le moins cher, le plus récent ou le plus populaire.</p>
