...
<h3 id="Standalone" class="common-anchor-header">Autonome<button data-href="#Standalone" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Une instance unique de Milvus qui exécute tous les composants dans un seul processus, ce qui convient aux petits ensembles de données et aux faibles charges de travail. En outre, en mode autonome, des implémentations WAL plus simples, telles que woodpecker et rocksmq, peuvent être choisies pour éliminer la nécessité de dépendances de stockage WAL tiers.</p>
<p>
...
<h3 id="Cluster" class="common-anchor-header">Cluster<button data-href="#Cluster" class="anchor-icon" translate="no">
    ...
    </button></h3><p>Mode de déploiement distribué de Milvus dans lequel chaque composant s'exécute indépendamment et peut être mis à l'échelle pour plus d'élasticité. Cette configuration convient aux grands ensembles de données et aux scénarios à forte charge.</p>
<p>
