...
<h1 id="Upgrade-Milvus-Standalone-with-Docker-Compose" class="common-anchor-header">Upgrade von Milvus Standalone mit Docker Compose<button data-href="#Upgrade-Milvus-Standalone-with-Docker-Compose" class="anchor-icon" translate="no">
    ...
    </button></h1><p>In dieser Anleitung wird beschrieben, wie Sie eine Milvus 2.6.x-Standalone-Bereitstellung mit Docker Compose auf Version 3.0.1 aktualisieren.</p>
<div class="alert note">
<p>Dieses Verfahren wurde mit der offiziellen Milvus 2.6.20 Standalone-Docker-Compose-Konfiguration getestet. Bei der Aktualisierung blieben etcd, MinIO, Woodpecker und die vorhandenen Datenverzeichnisse erhalten; lediglich das Milvus-Image wurde auf „ <code translate="no">milvusdb/milvus:v3.0.1</code> “ geändert.</p>
</div>
...
<li>Ein aktuelles Backup der Milvus-Metadaten und der persistenten Daten</li>
</ul>
...
<p>Ersetzen Sie im Rahmen dieses Vorgangs nicht Ihre aktuelle Compose-Datei und ändern Sie keine Abhängigkeitsversionen. Behalten Sie das vorhandene etcd, den Objektspeicher, die Nachrichtenwarteschlange, die Volumes und die Konfiguration bei. Aktualisieren Sie lediglich den Milvus-Image-Tag.</p>
<p>Dieses Verfahren validiert kein Downgrade oder Rollback durch die Rückführung des Milvus-Images auf 2.6.x. Nachdem v3.0.1 Daten geschrieben hat, kann es bei einem reinen Image-Rollback vorkommen, dass der aktualisierte Zustand nicht gelesen werden kann. Wenn das Upgrade fehlschlägt, stoppen Sie Schreibvorgänge und wenden Sie einen Wiederherstellungsplan an, der die Metadaten und die Backups der persistenten Daten aus der Zeit vor dem Upgrade wiederherstellt. Testen Sie den Wiederherstellungsplan zunächst in einer Nicht-Produktionsumgebung.</p>
</div>
...
<button class="copy-code-btn"></button></code></pre>
<p>Stellen Sie sicher, dass der Dienst „ <code translate="no">standalone</code> “ ordnungsgemäß funktioniert, sein Image „ <code translate="no">milvusdb/milvus:v3.0.1</code> “ lautet und die vorhandenen Sammlungen weiterhin abfragbar und durchsuchbar sind. Führen Sie diese Überprüfungen durch, bevor Sie v3.0.1-spezifische Funktionen aktivieren.</p>
