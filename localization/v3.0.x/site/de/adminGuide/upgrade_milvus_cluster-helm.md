...
summary: >-
  Erfahren Sie, wie Sie einen Milvus-Cluster mit Helm-Charts aktualisieren
  können.
...
<h1 id="Upgrade-Milvus-Cluster-with-Helm-Chart" class="common-anchor-header">Milvus-Cluster mit Helm-Chart aktualisieren<button data-href="#Upgrade-Milvus-Cluster-with-Helm-Chart" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h1><p>In dieser Anleitung wird beschrieben, wie Sie Ihren Milvus 2.6.x-Cluster mithilfe von Helm auf Version 3.0.1 aktualisieren.</p>
<div class="alert note">
<p>Dieses Verfahren wurde für den Upgrade von Milvus 2.6.20 auf Milvus v3.0.1 mit dem Milvus-Helm-Chart 5.0.22 validiert. Wenn Sie eine andere Milvus 2.6.x-Patch-Version oder eine andere Helm-Chart-Version verwenden, testen Sie das Upgrade zunächst in einer Nicht-Produktionsumgebung.</p>
</div>
...
<p><strong>Einschränkungen bei der Nachrichtenwarteschlange</strong>: Beim Upgrade auf Milvus v3.0.1 müssen Sie Ihre aktuelle Auswahl der Nachrichtenwarteschlange beibehalten. Ein Wechsel zwischen verschiedenen Nachrichtenwarteschlangensystemen während des Upgrades wird nicht unterstützt. Die Unterstützung für den Wechsel des Nachrichtenwarteschlangensystems wird in zukünftigen Versionen verfügbar sein.</p>
<div class="alert warning">
<p>Ändern oder downgraden Sie das Helm-Chart im Rahmen dieses Vorgangs nicht. Behalten Sie die bereits für Ihre Helm-Release installierte Chart-Version bei. Bei der getesteten Basisversion wurde das Helm-Chart 5.0.22 beibehalten und lediglich das Milvus-Image-Tag in „ <code translate="no">v3.0.1</code> “ geändert.</p>
<p>Dieses Verfahren validiert kein Downgrade oder Rollback, bei dem das Milvus-Image wieder auf 2.6.x zurückgesetzt wird. Nachdem v3.0.1 Daten geschrieben hat, kann es bei einem reinen Image-Rollback vorkommen, dass der aktualisierte Zustand nicht gelesen werden kann. Wenn das Upgrade fehlschlägt, stellen Sie Schreibvorgänge ein und wenden Sie einen Wiederherstellungsplan an, der die Metadaten vor dem Upgrade sowie die Backups der persistenten Daten wiederherstellt. Testen Sie den Wiederherstellungsplan zunächst in einer Nicht-Produktionsumgebung.</p>
</div>
...
<h3 id="Step-1-Confirm-the-current-topology" class="common-anchor-header">Schritt 1: Überprüfen Sie die aktuelle Topologie<button data-href="#Step-1-Confirm-the-current-topology" class="anchor-icon" translate="no">
    ...
    </button></h3><p>Speichern Sie die vollständigen Werte des aktuellen Releases und überprüfen Sie die laufenden Pods:</p>
...
<button class="copy-code-btn"></button></code></pre>
<div class="alert note">
Das Milvus-Helm-Charts-Repo unter <code translate="no">https://milvus-io.github.io/milvus-helm/</code> wurde archiviert. Verwenden Sie das neue Repo <code translate="no">https://zilliztech.github.io/milvus-helm/</code> für Chart-Versionen ab 4.0.31.
</div>
...
<button class="copy-code-btn"></button></code></pre>
<p>Die Option „ <code translate="no">--reset-then-reuse-values</code> “ behält die Werte aus der vorherigen Version bei und wendet gleichzeitig die explizite Image-Überschreibung gegenüber den Standardwerten des ausgewählten Charts an.</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>Stellen Sie sicher, dass alle erforderlichen Workloads bereit sind, alle Milvus-Komponenten „ <code translate="no">v3.0.1</code> “ verwenden und Ihre bestehenden Sammlungen weiterhin abfragbar und durchsuchbar sind. Führen Sie diese Überprüfungen durch, bevor Sie v3.0.1-spezifische Funktionen aktivieren.</p>
<div class="alert note">
