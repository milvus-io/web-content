...
---
<div class="tab-wrapper"><a href="/docs/de/upgrade_milvus_standalone-operator.md" class='active '>Milvus</a><a href="/docs/de/upgrade_milvus_standalone-docker.md" class=''>Operator</a>, Helm, Docker<a href="/docs/de/upgrade_milvus_standalone-docker.md" class=''>Compose</a></div>
<h1 id="Upgrade-Milvus-Standalone-with-Milvus-Operator" class="common-anchor-header">Upgrade von Milvus Standalone mit Milvus Operator<button data-href="#Upgrade-Milvus-Standalone-with-Milvus-Operator" class="anchor-icon" translate="no">
    ...
    </button></h1><p>In dieser Anleitung wird beschrieben, wie Sie eine Milvus 2.6.x-Standalone-Bereitstellung mit Milvus Operator auf Version 3.0.1 aktualisieren.</p>
<div class="alert note">
<p>Dieses Verfahren wurde von Milvus 2.6.20 auf Milvus v3.0.1 mit Milvus Operator 1.3.0, Woodpecker, einem clusterinternen etcd und einem clusterinternen MinIO getestet. Wenn Sie eine andere Milvus 2.6.x-Patch-Version, Operator-Version, Nachrichtenwarteschlange oder Abhängigkeitskonfiguration verwenden, testen Sie das Upgrade zunächst in einer Nicht-Produktionsumgebung.</p>
</div>
...
<li>Ein Kubernetes-Cluster mit einer eigenständigen Milvus 2.6.x-Bereitstellung, die vom Milvus Operator verwaltet wird</li>
...
<li>Die Installationsmethode und die Manifeste, die für den bestehenden Milvus-Operator verwendet werden</li>
<li>Eine aktuelle Sicherung der Milvus-Metadaten und persistenten Daten</li>
</ul>
<p><strong>Einschränkungen bei der Nachrichtenwarteschlange</strong>: Beim Upgrade auf Milvus v3.0.1 müssen Sie Ihre aktuelle Wahl der Nachrichtenwarteschlange beibehalten. Ein Wechsel zwischen verschiedenen Nachrichtenwarteschlangensystemen während des Upgrades wird nicht unterstützt. Die Unterstützung für den Wechsel von Nachrichtenwarteschlangensystemen wird in zukünftigen Versionen verfügbar sein.</p>
<div class="alert warning">
<p>Dieses Verfahren gilt nicht für ein Downgrade oder einen Rollback, bei dem das Milvus-Image wieder auf 2.6.x zurückgesetzt wird. Nachdem v3.0.1 Daten geschrieben hat, kann es bei einem reinen Image-Rollback vorkommen, dass der aktualisierte Zustand nicht gelesen werden kann. Wenn das Upgrade fehlschlägt, stoppen Sie Schreibvorgänge und wenden Sie einen Wiederherstellungsplan an, der die vor dem Upgrade erstellten Sicherungen der Metadaten und persistenten Daten wiederherstellt. Testen Sie den Wiederherstellungsplan zunächst in einer Nicht-Produktionsumgebung.</p>
</div>
...
<h3 id="Step-2-Confirm-the-Milvus-Operator-version" class="common-anchor-header">Schritt 2: Überprüfen Sie die Milvus-Operator-Version<button data-href="#Step-2-Confirm-the-Milvus-Operator-version" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<h3 id="Step-3-Update-the-Milvus-image" class="common-anchor-header">Schritt 3: Aktualisieren Sie das Milvus-Image<button data-href="#Step-3-Update-the-Milvus-image" class="anchor-icon" translate="no">
    ...
    </button></h3><p>Ändern Sie im vollständigen Milvus-CR-Manifest nur „ <code translate="no">spec.components.image</code> “. Behalten Sie den bestehenden Modus, die Komponenteneinstellungen, die Nachrichtenwarteschlange, etcd, den Speicher sowie andere Abhängigkeitseinstellungen bei. Der folgende Auszug zeigt das zu ändernde Feld; ersetzen Sie nicht Ihr gesamtes CR durch diesen Auszug.</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>Stellen Sie sicher, dass das Milvus-CR „ <code translate="no">Healthy</code> “ meldet, das aktuelle Image „ <code translate="no">milvusdb/milvus:v3.0.1</code> “ lautet und die vorhandenen Sammlungen weiterhin abfragbar und durchsuchbar sind. Führen Sie diese Überprüfungen durch, bevor Sie v3.0.1-spezifische Funktionen aktivieren.</p>
