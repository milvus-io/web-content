...
<p>Hinweis: Das Umschalten auf „ <code translate="no">mq.type</code> “ für einen laufenden Cluster ist ein Upgrade-Vorgang. Befolgen Sie die Upgrade-Anleitung sorgfältig und testen Sie die Konfiguration zunächst auf einem neuen Cluster, bevor Sie die Umstellung in der Produktionsumgebung vornehmen.</p>
...
<ul>
<li>Steuert das Verhalten beim Anhängen, rollierenden Aktualisieren und Überwachen von Segmenten auf der Client-Seite, um einen Ausgleich zwischen Durchsatz und End-to-End-Latenz zu schaffen.</li>
</ul></li>
...
<tr><td><code translate="no">local</code></td><td>In den Milvus-/Streaming-Knoten integriert</td><td>Lokales Dateisystem</td><td>Unterstützt</td><td>Eingeschränkt (alle Knoten benötigen ein gemeinsam genutztes Dateisystem, z. B. NFS)</td></tr>
...
<li><strong><code translate="no">service</code> In diesem Modus wird Woodpecker als separater, unabhängig skalierbarer Dienst ausgeführt und steht nur für verteilte/Cluster-Bereitstellungen zur Verfügung.</strong> Standalone-Bereitstellungen nutzen die eingebetteten Modi (<code translate="no">minio</code> oder <code translate="no">local</code>).</li>
</ul>
...
<p>– Cluster-Bereitstellung (empfohlene Einstellungen mit aktiviertem Woodpecker und Streaming-Knoten):</p>
...
<button class="copy-code-btn"></button></code></pre>
<h3 id="Enable-Woodpecker-service-mode-for-a-Milvus-Cluster-Helm" class="common-anchor-header">Woodpecker-Dienstmodus für einen Milvus-Cluster aktivieren (Helm)<button data-href="#Enable-Woodpecker-service-mode-for-a-Milvus-Cluster-Helm" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<li>Steuern Sie den Zeitpunkt der Aktualisierung/Indexerstellung (Batch-Erstellung vor dem Auslösen), um häufige kleine Schreibvorgänge zu vermeiden.</li>
</ul></li>
</ul>
...
<li><strong>Topologiebewusste Single-Hop-Lesevorgänge</strong> – jeder Lesevorgang wird direkt an die nächstgelegene Replik gesendet, anstatt über einen Broker weitergeleitet zu werden. Dadurch werden die zufälligen Lesevorgänge über AZ-Grenzen hinweg (≈2/3 des Leseverkehrs über AZ-Grenzen) vermieden, wie sie bei Broker-basierten Systemen auftreten.</li>
<li><strong>Sofortiges Hochladen in den Objektspeicher nach Segmentrollover</strong> – jedes Segment verfolgt seinen gesamten Lebenszyklus und wird sofort nach dem Rollover in den Objektspeicher hochgeladen, wodurch der Speicherplatzbedarf auf der lokalen Festplatte und die Speicherkosten niedrig gehalten werden, ohne dass dies zu Lasten der Latenz geht.</li>
...
<p>Bei bereichsübergreifenden (AZ) Bereitstellungen spart der Servicemodus im Vergleich zu brokerbasierten Protokollsystemen zudem etwa <strong>ein Drittel des Schreib-</strong> und <strong>zwei Drittel des Lese-Netzwerk</strong> verkehrs zwischen den AZs ein. Die vollständige Design- und Kostenanalyse finden Sie unter <a href="/docs/de/woodpecker_architecture.md">„Woodpecker-Architektur</a>“.</p>
