...
title: RAG mit LlamaIndex und der Milvus-Async-API erstellen
related_key: LlamaIndex
summary: >-
  Dieses Tutorial zeigt, wie man LlamaIndex zusammen mit Milvus einsetzt, um
  eine asynchrone Pipeline zur Dokumentenverarbeitung für RAG aufzubauen.
  LlamaIndex bietet die Möglichkeit, Dokumente zu verarbeiten und in einer
  Vektordatenbank wie Milvus zu speichern. Durch die Nutzung der asynchronen API
  von LlamaIndex und der Milvus-Python-Client-Bibliothek können wir den
  Durchsatz der Pipeline steigern, um große Datenmengen effizient zu verarbeiten
  und zu indizieren.
---
<h1 id="RAG-with-Milvus-and-LlamaIndex-Async-API" class="common-anchor-header">RAG mit Milvus und der asynchronen LlamaIndex-API<button data-href="#RAG-with-Milvus-and-LlamaIndex-Async-API" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<p>Dieses Tutorial zeigt, wie man <a href="https://www.llamaindex.ai/">LlamaIndex</a> zusammen mit <a href="https://milvus.io/">Milvus</a> nutzt, um eine asynchrone Pipeline zur Dokumentverarbeitung für RAG aufzubauen. LlamaIndex bietet die Möglichkeit, Dokumente zu verarbeiten und in einer Vektordatenbank wie Milvus zu speichern. Durch die Nutzung der asynchronen API von LlamaIndex und der Milvus-Python-Client-Bibliothek können wir den Durchsatz der Pipeline steigern, um große Datenmengen effizient zu verarbeiten und zu indizieren.</p>
<p>In diesem Tutorial stellen wir zunächst auf allgemeiner Ebene die Verwendung asynchroner Methoden zum Aufbau eines RAG mit LlamaIndex und Milvus vor und gehen anschließend auf die Verwendung von Low-Level-Methoden sowie den Leistungsvergleich zwischen synchroner und asynchroner Verarbeitung ein.</p>
<h2 id="Before-you-begin" class="common-anchor-header">Bevor Sie beginnen<button data-href="#Before-you-begin" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Die Codeausschnitte auf dieser Seite erfordern die Abhängigkeiten „pymilvus“ und „llamaindex“. Sie können diese mit den folgenden Befehlen installieren:</p>
...
<p>Wir werden die Modelle von OpenAI verwenden. Du solltest den <a href="https://platform.openai.com/docs/quickstart">API-Schlüssel</a> <code translate="no">OPENAI_API_KEY</code> als Umgebungsvariable vorbereiten.</p>
<pre><code translate="no" class="language-python"><span class="hljs-keyword">import</span> os

os.environ[<span class="hljs-string">&quot;OPENAI_API_KEY&quot;</span>] = <span class="hljs-string">&quot;sk-***********&quot;</span>
<button class="copy-code-btn"></button></code></pre>
<p>Wenn Sie Jupyter Notebook verwenden, müssen Sie diese Codezeile ausführen, bevor Sie den asynchronen Code ausführen.</p>
...
<h3 id="Prepare-data" class="common-anchor-header">Daten vorbereiten<button data-href="#Prepare-data" class="anchor-icon" translate="no">
    ...
    </button></h3><p>Mit den folgenden Befehlen können Sie Beispieldaten herunterladen:</p>
...
<h2 id="Build-RAG-with-Asynchronous-Processing" class="common-anchor-header">RAG mit asynchroner Verarbeitung erstellen<button data-href="#Build-RAG-with-Asynchronous-Processing" class="anchor-icon" translate="no">
    ...
    </button></h2><p>In diesem Abschnitt wird gezeigt, wie ein RAG-System erstellt wird, das Dokumente asynchron verarbeiten kann.</p>
<p>Importieren Sie die erforderlichen Bibliotheken und definieren Sie die Milvus-URI sowie die Dimension der Einbettung.</p>
...
<div class="alert note">
<ul>
<li>Wenn Sie über große Datenmengen verfügen, können Sie einen leistungsstarken Milvus-Server auf <a href="https://milvus.io/docs/quickstart.md">Docker oder Kubernetes</a> einrichten. Verwenden Sie in dieser Konfiguration bitte die Server-URI, z. B.<code translate="no">http://localhost:19530</code>, als „ <code translate="no">uri</code> “.</li>
...
<li>Bei komplexen Systemen (wie z. B. der Netzwerkkommunikation) kann die asynchrone Verarbeitung im Vergleich zur Synchronisation zu einer Leistungssteigerung führen. Daher sind wir der Meinung, dass Milvus-Lite für die Verwendung asynchroner Schnittstellen nicht geeignet ist, da die verwendeten Szenarien dafür nicht geeignet sind.</li>
</ul>
</div>
<p>Definieren Sie eine Initialisierungsfunktion, die wir wiederverwenden können, um die Milvus-Sammlung neu aufzubauen.</p>
...
<p>Verwenden Sie „SimpleDirectoryReader“, um ein LlamaIndex-Dokumentobjekt aus der Datei „ <code translate="no">paul_graham_essay.txt</code> “ zu verpacken.</p>
...
<p>Instanziieren Sie ein Hugging-Face-Embedding-Modell lokal. Durch die Verwendung eines lokalen Modells vermeiden Sie das Risiko, während der asynchronen Dateneingabe API-Ratenlimits zu erreichen, da sich gleichzeitige API-Anfragen schnell summieren und Ihr Budget bei öffentlichen APIs aufbrauchen können. Wenn Sie jedoch über ein hohes Ratenlimit verfügen, können Sie stattdessen einen Remote-Modelldienst verwenden.</p>
...
<p>Wir setzen „ <code translate="no">use_async</code> “ auf „ <code translate="no">True</code> “, um den asynchronen Einfügemodus zu aktivieren.</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>Initialisieren Sie das LLM.</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>Beim Erstellen der Abfrage-Engine können Sie den Parameter „ <code translate="no">use_async</code> “ ebenfalls auf „ <code translate="no">True</code> “ setzen, um die asynchrone Suche zu aktivieren.</p>
...
<h2 id="Explore-the-Async-API" class="common-anchor-header">Entdecken Sie die Async-API<button data-href="#Explore-the-Async-API" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>In diesem Abschnitt stellen wir die Nutzung der Low-Level-API vor und vergleichen die Leistung synchroner und asynchroner Ausführungen.</p>
<h3 id="Async-add" class="common-anchor-header">Asynchrones Hinzufügen<button data-href="#Async-add" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Initialisieren Sie den Vektorspeicher erneut.</p>
...
<p>Definieren wir eine Funktion zur Erzeugung von Knoten, die dazu dient, eine große Anzahl von Testknoten für den Index zu generieren.</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>Definieren Sie eine asynchrone Funktion, um Dokumente zum Vektorspeicher hinzuzufügen. Wir verwenden die Funktion „ <code translate="no">async_add()</code> “ in der Milvus-Vektorspeicher-Instanz.</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>Rufen Sie die Ereignisschleife ab.</p>
<pre><code translate="no" class="language-python">loop = asyncio.get_event_loop()
<button class="copy-code-btn"></button></code></pre>
<p>Fügen Sie Dokumente asynchron zum Vektorspeicher hinzu.</p>
...
<h4 id="Compare-with-synchronous-add" class="common-anchor-header">Vergleichen Sie dies mit dem synchronen Hinzufügen.</h4><p>Definieren Sie eine Funktion zum synchronen Hinzufügen. Messen Sie anschließend die Laufzeit unter denselben Bedingungen.</p>
...
<p>Das Ergebnis zeigt, dass der synchrone Hinzufügungsvorgang deutlich langsamer ist als der asynchrone.</p>
<h3 id="Async-search" class="common-anchor-header">Asynchrone Suche<button data-href="#Async-search" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Initialisieren Sie den Vektorspeicher neu und fügen Sie einige Dokumente hinzu, bevor Sie die Suche ausführen.</p>
<pre><code translate="no" class="language-python">vector_store = init_vector_store()
...
<button class="copy-code-btn"></button></code></pre>
<p>Führen Sie eine asynchrone Suche im Milvus-Speicher durch.</p>
...
<h4 id="Compare-with-synchronous-search" class="common-anchor-header">Vergleich mit der synchronen Suche</h4><p>Definieren Sie eine Funktion für die synchrone Suche. Messen Sie anschließend die Laufzeit unter denselben Bedingungen.</p>
...
<p>Das Ergebnis zeigt, dass der synchrone Suchvorgang deutlich langsamer ist als der asynchrone.</p>
