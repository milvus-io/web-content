...
<h1 id="Search-with-Embedding-Lists" class="common-anchor-header">Suche mit eingebetteten Listen<button data-href="#Search-with-Embedding-Lists" class="anchor-icon" translate="no">
    ...
    </button></h1><p>Auf dieser Seite wird erklärt, wie man ein ColBERT-Textrecherchesystem und ein ColPali-Textrecherchesystem einrichtet, indem man das Array of Structs in Milvus verwendet, mit dem man ein Dokument zusammen mit seinen vektorisierten Chunks in Einbettungslisten speichern kann.</p>
...
<p>Die Speicherung von Daten in Chunks führt jedoch zu chunk-artigen Suchergebnissen, d. h. die Suche identifiziert zunächst relevante <em>Segmente</em> und nicht zusammenhängende <em>Dokumente</em>. Um dies zu vermeiden, sollten Sie eine zusätzliche Verarbeitung nach der Suche durchführen.</p>
...
<h3 id="Late-interaction" class="common-anchor-header">Späte Interaktion<button data-href="#Late-interaction" class="anchor-icon" translate="no">
    ...
    </button></h3><p>Sobald die Vektorisierung abgeschlossen ist, wird die Liste der Abfrageeinbettungen mit jeder Liste der Dokumenteneinbettungen verglichen, Token für Token, um den endgültigen Ähnlichkeitswert zu bestimmen.</p>
<p>
...
<h2 id="ColPali-text-retrieval-system" class="common-anchor-header">ColPali Text Retrieval System<button data-href="#ColPali-text-retrieval-system" class="anchor-icon" translate="no">
    ...
    </button></h2><p>In diesem Abschnitt werden wir ein ColPali-basiertes Textsuchsystem einrichten, das Milvus' Array of Structs verwendet. Zuvor muss eine Instanz von Milvus v2.6.x eingerichtet werden - ein mit Milvus v2.6.x kompatibler Zilliz-Cluster.</p>
<h3 id="Step-1-Install-the-dependencies" class="common-anchor-header">Schritt 1: Installieren Sie die Abhängigkeiten<button data-href="#Step-1-Install-the-dependencies" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
