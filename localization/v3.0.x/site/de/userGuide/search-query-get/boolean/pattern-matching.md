...
<p>Wenn Sie eine exakte Zeichenfolgenübereinstimmung benötigen, empfehlen wir Ihnen, anstelle des Musterabgleichs „ <code translate="no">==</code> “ zu verwenden. Verwenden Sie „ <code translate="no">LIKE</code> “ oder „regex“ nur, wenn der Filter einem Muster entsprechen muss.</p>
<table>
<thead>
...
<p>Verwenden Sie „ <code translate="no">LIKE</code> “ für einfache Platzhalterabgleiche. Verwenden Sie reguläre Ausdrücke, wenn das Muster Zeichenklassen, Wiederholungen, Alternativen wie „ <code translate="no">error|failed</code> “, Anker oder einen Abgleich ohne Berücksichtigung der Groß-/Kleinschreibung erfordert.</p>
...
<ul>
<li><code translate="no">name LIKE r&quot;\%&quot;</code> entspricht dem wörtlichen Wert „ <code translate="no">%</code> “.</li>
...
<h2 id="Use-regex" class="common-anchor-header">Verwenden Sie reguläre Ausdrücke<span class="beta-tag" style="background-color:rgb(0, 179, 255);color:white" translate="no">Compatible with Milvus 3.0.x</span><button data-href="#Use-regex" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<button class="copy-code-btn"></button></code></pre>
<p>Wenn Sie Regex-Metazeichen wörtlich abgleichen möchten, müssen Sie diese im Regex-Muster mit einem Escape-Zeichen versehen. Um beispielsweise einen wörtlichen Punkt (<code translate="no">\.</code> in Regex) abzugleichen, schreiben Sie <code translate="no">\\.</code> in einer Python-, Java-, Go- oder Node.js-Quellzeichenfolge:</p>
<div class="multipleCode">
 <a href="#python">Python</a>
 <a href="#java"> Java</a>
 <a href="#go"> Go</a>
...
<p>Bei JSON-Pfaden verhalten sich Regex-Filter unterschiedlich, wenn der Pfad fehlt, den Wert „null“ hat oder zu einem Nicht-String-Wert aufgelöst wird:</p>
<table>
<thead>
