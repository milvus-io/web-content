...
<h3 id="Single-vector-search" class="common-anchor-header">Ein-Vektor-Suche<button data-href="#Single-vector-search" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Die Ein-Vektor-Suche ist die einfachste Form der <code translate="no">search</code> Operationen in Milvus und dient dazu, die ähnlichsten Vektoren zu einem bestimmten Abfragevektor zu finden.</p>
...
<h3 id="Bulk-vector-search" class="common-anchor-header">Bulk-Vektor-Suche<button data-href="#Bulk-vector-search" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Die Massenvektorsuche erweitert das Konzept der <a href="https://milvus.io/docs/single-vector-search.md#Single-Vector-Search">Einzelvektorsuche</a>, indem sie die Suche nach mehreren Abfragevektoren in einer einzigen Anfrage ermöglicht. Diese Art der Suche ist ideal für Szenarien, in denen ähnliche Vektoren für eine Reihe von Abfragevektoren gefunden werden müssen, was den Zeit- und Rechenaufwand erheblich reduziert.</p>
...
<h3 id="Partition-search" class="common-anchor-header">Partitionierte Suche<button data-href="#Partition-search" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Die Partitionssuche grenzt den Umfang Ihrer Suche auf eine bestimmte Teilmenge oder Partition Ihrer Sammlung ein. Dies ist besonders nützlich für organisierte Datensätze, bei denen die Daten in logische oder kategorische Unterteilungen segmentiert sind, was schnellere Suchvorgänge ermöglicht, da die zu durchsuchende Datenmenge reduziert wird.</p>
...
<h3 id="Search-with-output-fields" class="common-anchor-header">Suche mit Ausgabefeldern<button data-href="#Search-with-output-fields" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Bei der Suche mit Ausgabefeldern können Sie angeben, welche Attribute oder Felder der übereinstimmenden Vektoren in die Suchergebnisse aufgenommen werden sollen.</p>
...
<h3 id="Use-the-like-operator" class="common-anchor-header">Verwenden Sie den <code translate="no">like</code> Operator<button data-href="#Use-the-like-operator" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Der <code translate="no">like</code> Operator verbessert die Zeichenkettensuche durch die Auswertung von Mustern, einschließlich Präfixen, Infixen und Suffixen:</p>
<ul>
...
<h3 id="Special-characters-in-search-strings" class="common-anchor-header">Sonderzeichen in Suchzeichenketten<button data-href="#Special-characters-in-search-strings" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<tbody>
<tr><td><code translate="no">L2</code></td><td>Kleinere L2-Distanzen zeigen eine höhere Ähnlichkeit an.</td><td>Um die nächstgelegenen Vektoren von den Ergebnissen auszuschließen, stellen Sie sicher, dass:<br/> <code translate="no">range_filter</code> &lt;= Abstand &lt; <code translate="no">radius</code></td></tr>
<tr><td><code translate="no">IP</code></td><td>Größere IP-Abstände weisen auf eine höhere Ähnlichkeit hin.</td><td>Um die nächstgelegenen Vektoren aus den Ergebnissen auszuschließen, stellen Sie sicher, dass:<br/> <code translate="no">radius</code> &lt; Abstand &lt;= <code translate="no">range_filter</code></td></tr>
<tr><td><code translate="no">COSINE</code></td><td>Ein größerer Kosinuswert weist auf eine höhere Ähnlichkeit hin.</td><td>Um die nächstgelegenen Vektoren aus den Ergebnissen auszuschließen, stellen Sie sicher, dass:<br/> <code translate="no">radius</code> &lt; Abstand &lt;= <code translate="no">range_filter</code></td></tr>
<tr><td><code translate="no">JACCARD</code></td><td>Kleinere Jaccard-Distanzen weisen auf eine höhere Ähnlichkeit hin.</td><td>Um die engsten Vektoren von den Ergebnissen auszuschließen, stellen Sie sicher, dass:<br/> <code translate="no">range_filter</code> &lt;= Abstand &lt; <code translate="no">radius</code></td></tr>
<tr><td><code translate="no">HAMMING</code></td><td>Kleinere Hamming-Distanzen weisen auf eine höhere Ähnlichkeit hin.</td><td>Um die engsten Vektoren von den Ergebnissen auszuschließen, stellen Sie sicher, dass:<br/> <code translate="no">range_filter</code> &lt;= Abstand &lt; <code translate="no">radius</code></td></tr>
</tbody>
</table>
<p>Weitere Informationen zu den Abstandsmetriken finden Sie unter <a href="/docs/de/metric.md">Ähnlichkeitsmetriken</a>.</p>
