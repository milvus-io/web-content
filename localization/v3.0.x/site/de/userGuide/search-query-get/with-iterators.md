...
summary: >-
  ...
  reine Verwendung der einfachen ANN-Suche reicht möglicherweise nicht aus, um
  die Anforderungen einer groß angelegten Abfrage zu erfüllen. Bei
...
<h1 id="Search-Iterator" class="common-anchor-header">Such-Iterator<button data-href="#Search-Iterator" class="anchor-icon" translate="no">
    ...
    </button></h1><p>Die ANN-Suche unterliegt einer Obergrenze für die Anzahl der Entitäten, die bei einer einzelnen Abfrage abgerufen werden können, und die einfache Verwendung der grundlegenden ANN-Suche reicht unter Umständen nicht aus, um die Anforderungen einer groß angelegten Suche zu erfüllen. Bei ANN-Suchanfragen, bei denen „topK“ den Wert 16.384 überschreitet, empfiehlt es sich, den Einsatz des SearchIterators in Betracht zu ziehen. In diesem Abschnitt werden die Verwendung des SearchIterators und damit verbundene Überlegungen vorgestellt.</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>In den obigen Codebeispielen haben Sie eine Endlosschleife erstellt und innerhalb der Schleife die Methode <strong>`next()`</strong> aufgerufen, um die Suchergebnisse in einer Variablen zu speichern. Sobald ` <strong>next()`</strong> nichts mehr zurückgibt, haben Sie den Iterator geschlossen.</p>
