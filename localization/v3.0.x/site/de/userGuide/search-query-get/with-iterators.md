...
title: Such-Iterator
summary: >-
  Die ANN-Suche unterliegt einer Obergrenze hinsichtlich der Anzahl der
  Entitäten, die bei einer einzelnen Abfrage abgerufen werden können, und die
  bloße Verwendung der einfachen ANN-Suche reicht möglicherweise nicht aus, um
  den Anforderungen einer groß angelegten Suche gerecht zu werden. Bei
  ANN-Search-Anfragen, bei denen „topK“ den Wert 16.384 überschreitet, empfiehlt
  es sich, den Einsatz des SearchIterators in Betracht zu ziehen. In diesem
  Abschnitt wird die Verwendung des SearchIterators sowie damit verbundene
...
<h1 id="Search-Iterator" class="common-anchor-header">Such-Iterator<button data-href="#Search-Iterator" class="anchor-icon" translate="no">
    ...
    </button></h1><p>Die ANN-Suche unterliegt einer Obergrenze für die Anzahl der Entitäten, die bei einer einzelnen Abfrage abgerufen werden können, und die einfache Verwendung der grundlegenden ANN-Suche reicht möglicherweise nicht aus, um die Anforderungen einer groß angelegten Abfrage zu erfüllen. Bei ANN-Suchanfragen, bei denen „topK“ den Wert 16.384 überschreitet, empfiehlt es sich, den Einsatz des SearchIterators in Betracht zu ziehen. In diesem Abschnitt werden die Verwendung des SearchIterators und damit verbundene Überlegungen vorgestellt.</p>
<h2 id="Overview" class="common-anchor-header">Übersicht<button data-href="#Overview" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Eine Suchanfrage liefert Suchergebnisse zurück, während ein „SearchIterator“ einen Iterator zurückgibt. Sie können die Methode <strong>„next()“</strong> dieses Iterators aufrufen, um die Suchergebnisse abzurufen.</p>
<p>Konkret können Sie den „SearchIterator“ wie folgt verwenden:</p>
<ol>
<li><p>Erstellen Sie einen SearchIterator und legen Sie <strong>die Anzahl der pro Suchanfrage zurückzugebenden Entitäten</strong> sowie <strong>die Gesamtzahl der zurückzugebenden Entitäten</strong> fest.</p></li>
<li><p>Rufen Sie die Methode <strong>`next()`</strong> des `SearchIterator` in einer Schleife auf, um die Suchergebnisse paginiert abzurufen.</p></li>
<li><p>Rufen Sie die Methode <strong>„close()“</strong> des Iterators auf, um die Schleife zu beenden, wenn die Methode <strong>„next()“</strong> ein leeres Ergebnis zurückgibt.</p></li>
</ol>
<h2 id="Create-SearchIterator" class="common-anchor-header">SearchIterator erstellen<button data-href="#Create-SearchIterator" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Der folgende Codeausschnitt veranschaulicht, wie ein SearchIterator erstellt wird.</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>In den obigen Beispielen haben Sie die Anzahl der pro Suche zurückzugebenden Entitäten (<strong>batch_size/batchSize</strong>) auf 50 und die Gesamtzahl der zurückzugebenden Entitäten (<strong>topK</strong>) auf 20.000 festgelegt.</p>
<h2 id="Use-SearchIterator" class="common-anchor-header">SearchIterator verwenden<button data-href="#Use-SearchIterator" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Sobald der SearchIterator bereit ist, können Sie dessen next()-Methode aufrufen, um die Suchergebnisse paginiert abzurufen.</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#go">   Go</a>
...
<button class="copy-code-btn"></button></code></pre>
<p>In den obigen Code-Beispielen haben Sie eine Endlosschleife erstellt und innerhalb der Schleife die Methode <strong>`next()`</strong> aufgerufen, um die Suchergebnisse in einer Variablen zu speichern. Sobald ` <strong>next()`</strong> nichts mehr zurückgibt, haben Sie den Iterator geschlossen.</p>
