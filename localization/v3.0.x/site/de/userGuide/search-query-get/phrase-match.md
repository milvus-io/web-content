---
id: phrase-match.md
title: PhrasenübereinstimmungCompatible with Milvus 2.5.17+
summary: >-
  ...
  derselben Reihenfolge und direkt nebeneinander stehen. Beispielsweise liefert
  eine Suche nach „Robotik maschinelles Lernen“ Treffer wie „…typische
  Robotik-Modelle für maschinelles Lernen…“, bei denen die Wörter „Robotik“,
  „maschinelles“ und „Lernen“ nacheinander ohne dazwischenliegende Wörter
  auftreten.
beta: Milvus 2.5.17+
---
<h1 id="Phrase-Match" class="common-anchor-header">Phrasenübereinstimmung<span class="beta-tag" style="background-color:rgb(0, 179, 255);color:white" translate="no">Compatible with Milvus 2.5.17+</span><button data-href="#Phrase-Match" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h1><p>Mit der Phrasenübereinstimmung können Sie nach Dokumenten suchen, die Ihre Suchbegriffe als exakte Phrase enthalten. Standardmäßig müssen die Wörter in derselben Reihenfolge und direkt nebeneinander stehen. Beispielsweise liefert eine Suche nach <strong>„Robotik maschinelles Lernen“</strong> Treffer wie <em>„…typische Robotik-Modelle für maschinelles Lernen…“</em>, wobei die Wörter <strong>„Robotik“</strong>, <strong>„Maschine“</strong> und <strong>„Lernen“</strong> nacheinander und ohne dazwischenliegende Wörter erscheinen.</p>
<p>In der Praxis kann ein strikter Phrasenabgleich jedoch zu starr sein. Möglicherweise möchten Sie Text wie <em>„…in der Robotik weit verbreitete Machine-Learning-Modelle…“</em> finden <em>.</em> Hier sind zwar dieselben Schlüsselwörter vorhanden, jedoch nicht nebeneinander oder in der ursprünglichen Reihenfolge. Um dies zu berücksichtigen, unterstützt der Phrasenabgleich den Parameter „ <code translate="no">slop</code> “, der für mehr Flexibilität sorgt. Der Wert „ <code translate="no">slop</code> “ legt fest, wie viele Positionsverschiebungen zwischen den Begriffen in der Phrase zulässig sind. Bei einem Wert von 1 für „ <code translate="no">slop</code> “ kann eine Abfrage nach <strong>„machine learning“</strong> beispielsweise mit Text wie <em>„…machine deep learning…“</em> übereinstimmen, bei dem ein Wort (<strong>„deep“</strong>) die ursprünglichen Begriffe trennt.</p>
<h2 id="Overview" class="common-anchor-header">Übersicht<button data-href="#Overview" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Die Phrasenübereinstimmung basiert auf der Suchmaschinenbibliothek <a href="https://github.com/quickwit-oss/tantivy">Tantivy</a> und funktioniert durch die Analyse der Positionsinformationen von Wörtern innerhalb von Dokumenten. Das folgende Diagramm veranschaulicht den Prozess:</p>
<p><span class="img-wrapper">
   ...
   <span>Ablauf der Phrasenübereinstimmung</span>
  
 </span></p>
<ol>
<li><p><strong>Tokenisierung von Dokumenten</strong>: Wenn Sie Dokumente in Milvus einfügen, wird der Text mithilfe eines Analysators in Token (einzelne Wörter oder Begriffe) aufgeteilt, wobei für jedes Token Positionsinformationen erfasst werden. Beispielsweise wird <strong>„doc_1“</strong> in <strong>[„machine“ (pos=0), „learning“ (pos=1), „boosts“ (pos=2), „efficiency“ (pos=3)]</strong> tokenisiert. Weitere Informationen zu Analysatoren finden Sie unter <a href="/docs/de/analyzer-overview.md">„Übersicht über Analysatoren</a>“.</p></li>
<li><p><strong>Erstellung eines invertierten Index</strong>: Milvus erstellt einen invertierten Index, der jedes Token den Dokumenten zuordnet, in denen es vorkommt, sowie die Positionen der Token in diesen Dokumenten.</p></li>
<li><p><strong>Phrasenabgleich</strong>: Bei der Ausführung einer Phrasenabfrage sucht Milvus jedes Token im invertierten Index und überprüft dessen Positionen, um festzustellen, ob sie in der richtigen Reihenfolge und Nähe zueinander vorkommen. Der Parameter „ <code translate="no">slop</code> “ steuert die maximal zulässige Anzahl von Positionen zwischen übereinstimmenden Token:</p>
<ul>
<li><p><strong>slop = 0</strong> bedeutet, dass die Token <strong>in genau derselben Reihenfolge und unmittelbar nebeneinander</strong> vorkommen müssen (d. h. keine zusätzlichen Wörter dazwischen).</p>
<ul>
<li>Im Beispiel passt nur <strong>doc_1</strong> (<strong>„machine“</strong> an <strong>Pos. 0</strong>, <strong>„learning“</strong> an <strong>Pos. 1</strong>) exakt.</li>
</ul></li>
<li><p><strong>slop = 2</strong> erlaubt bis zu zwei Positionen Flexibilität oder Umstellungen zwischen übereinstimmenden Tokens.</p>
<ul>
<li><p>Dies erlaubt eine umgekehrte Reihenfolge (<strong>„learning machine“</strong>) oder eine kleine Lücke zwischen den Tokens.</p></li>
...
<h2 id="Enable-phrase-match" class="common-anchor-header">Phrasenabgleich aktivieren<button data-href="#Enable-phrase-match" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>Der Phrasenabgleich funktioniert mit dem Feldtyp „ <code translate="no">VARCHAR</code> “, dem String-Datentyp in Milvus. Um den Phrasenabgleich zu aktivieren, konfigurieren Sie Ihr Sammlungsschema, indem Sie die Parameter „ <code translate="no">enable_analyzer</code> “ und „ <code translate="no">enable_match</code> “ ähnlich wie <a href="/docs/de/keyword-match.md">beim Textabgleich</a> auf „ <code translate="no">True</code> “ setzen.</p>
<h3 id="Set-enableanalyzer-and-enablematch" class="common-anchor-header">Legen Sie „ <code translate="no">enable_analyzer</code> “ und <code translate="no">enable_match</code><button data-href="#Set-enableanalyzer-and-enablematch" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Um den Phrasenabgleich für ein bestimmtes „ <code translate="no">VARCHAR</code> “-Feld zu aktivieren, setzen Sie bei der Definition des Feldschemas die Parameter „ <code translate="no">enable_analyzer</code> “ und „ <code translate="no">enable_match</code> “ beide auf „ <code translate="no">True</code> “. Diese Konfiguration weist Milvus an, den Text zu tokenisieren und einen invertierten Index mit den für einen effizienten Phrasenabgleich erforderlichen Positionsinformationen zu erstellen.</p>
<p>Hier ist ein Beispiel für eine Schemadefinition zur Aktivierung des Phrasenabgleichs:</p>
...
<h3 id="Optional-Configure-an-analyzer" class="common-anchor-header">Optional: Konfigurieren Sie einen Analysator<button data-href="#Optional-Configure-an-analyzer" class="anchor-icon" translate="no">
    ...
    </button></h3><p>Die Genauigkeit des Phrasenabgleichs hängt maßgeblich vom Analysator ab, der zur Tokenisierung Ihrer Textdaten verwendet wird. Verschiedene Analysatoren eignen sich für unterschiedliche Sprachen und Textformate und beeinflussen die Tokenisierung sowie die Positionsgenauigkeit. Die Auswahl eines geeigneten Analysators für Ihren spezifischen Anwendungsfall optimiert Ihre Ergebnisse beim Phrasenabgleich.</p>
<p>Standardmäßig verwendet Milvus den Standard-Analyzer, der Text anhand von Leerzeichen und Satzzeichen tokenisiert, Token mit mehr als 40 Zeichen entfernt und den Text in Kleinbuchstaben umwandelt. Für die Standardnutzung sind keine zusätzlichen Parameter erforderlich. Weitere Informationen finden Sie unter <a href="/docs/de/standard-analyzer.md">„Standard-Analyzer</a> “.</p>
<p>Wenn Ihre Anwendung einen bestimmten Analysator erfordert, konfigurieren Sie diesen mithilfe des Parameters „ <code translate="no">analyzer_params</code> “. So konfigurieren Sie beispielsweise den Analysator „ <code translate="no">english</code> “ für den Phrasenabgleich in englischem Text:</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#go">   Go</a>
...
schema.<span class="hljs-built_in">AddField</span>(milvus::<span class="hljs-built_in">FieldSchema</span>(<span class="hljs-string">&quot;text&quot;</span>, milvus::DataType::VARCHAR)  <span class="hljs-comment">// Name of the field</span>
                    ...
                    .<span class="hljs-built_in">EnableMatch</span>(<span class="hljs-literal">true</span>));                <span class="hljs-comment">// Enables inverted indexing for phrase matching</span>
<button class="copy-code-btn"></button></code></pre>
<p>Milvus unterstützt mehrere Analysatoren, die auf verschiedene Sprachen und Anwendungsfälle zugeschnitten sind. Ausführliche Informationen finden Sie unter <a href="/docs/de/analyzer-overview.md">„Analyzer-Übersicht</a>“.</p>
<h2 id="Use-phrase-match" class="common-anchor-header">Phrasenabgleich verwenden<button data-href="#Use-phrase-match" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Sobald Sie den Abgleich für ein „ <code translate="no">VARCHAR</code> “-Feld in Ihrem Sammlungsschema aktiviert haben, können Sie Phrasenabgleiche mithilfe des Ausdrucks „ <code translate="no">PHRASE_MATCH</code> “ durchführen.</p>
<div class="alert note">
<p>Bei dem Ausdruck „ <code translate="no">PHRASE_MATCH</code> “ wird die Groß-/Kleinschreibung nicht berücksichtigt. Sie können entweder „ <code translate="no">PHRASE_MATCH</code> “ oder „ <code translate="no">phrase_match</code> “ verwenden.</p>
</div>
<h3 id="PHRASEMATCH-expression-syntax" class="common-anchor-header">Syntax des Ausdrucks „PHRASE_MATCH“<button data-href="#PHRASEMATCH-expression-syntax" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Verwenden Sie den Ausdruck „ <code translate="no">PHRASE_MATCH</code> “, um bei der Suche das Feld, die Phrase und optional den Flexibilitätsgrad (<code translate="no">slop</code>) anzugeben. Die Syntax lautet:</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#go">   Go</a>
...
<button class="copy-code-btn"></button></code></pre>
<ul>
<li><p><code translate="no">field_name</code><strong>:</strong> Der Name des Feldes „ <code translate="no">VARCHAR</code> “, in dem Sie nach Phrasen suchen.</p></li>
<li><p><code translate="no">phrase</code><strong>:</strong> Die genaue Phrase, nach der gesucht werden soll.</p></li>
<li><p><code translate="no">slop</code> (optional)<strong>:</strong> Eine Ganzzahl, die die maximale Anzahl der zulässigen Positionen bei der Übereinstimmung von Tokens angibt.</p>
<ul>
<li><p><code translate="no">0</code> (Standard): Es werden nur exakte Phrasen abgeglichen. Beispiel: Ein Filter für <strong>„machine learning“</strong> findet genau <strong>„machine learning“</strong>, jedoch nicht <strong>„machine boosts learning“</strong> oder <strong>„learning machine“.</strong></p></li>
<li><p><code translate="no">1</code>: Lässt geringfügige Abweichungen zu, wie z. B. einen zusätzlichen Begriff oder eine geringfügige Verschiebung der Position. Beispiel: Ein Filter für <strong>„machine learning“</strong> findet Übereinstimmungen mit <strong>„machine boosts learning“</strong> (ein Token zwischen <strong>„machine“</strong> und <strong>„learning“</strong>), jedoch nicht mit <strong>„learning machine“</strong> (umgekehrte Reihenfolge der Begriffe).</p></li>
<li><p><code translate="no">2</code>: Ermöglicht mehr Flexibilität, einschließlich umgekehrter Wortreihenfolge oder bis zu zwei Tokens dazwischen. Beispiel: Ein Filter für <strong>„machine learning“</strong> findet <strong>„learning machine“</strong> (umgekehrte Wortreihenfolge) oder <strong>„machine quickly boosts learning“</strong> (zwei Tokens zwischen <strong>„machine“</strong> und <strong>„learning“</strong>).</p></li>
</ul></li>
</ul>
<h3 id="Example-dataset" class="common-anchor-header">Beispiel-Datensatz<button data-href="#Example-dataset" class="anchor-icon" translate="no">
    ...
    </button></h3><p>Angenommen, Sie haben eine Sammlung namens <strong>„tech_articles“</strong>, die die folgenden fünf Entitäten enthält:</p>
<table>
   <tr>
   ...
   <tr>
     <td><p>2</p></td>
     <td><p>„Das Erlernen eines maschinellen Ansatzes ist entscheidend für den Fortschritt der modernen KI“</p></td>
   </tr>
   <tr>
     <td><p>3</p></td>
     <td><p>„Deep-Learning-Architekturen optimieren die Rechenlasten“</p></td>
   </tr>
   <tr>
     <td><p>4</p></td>
     <td><p>„Die Maschine verbessert zügig die Modellleistung für kontinuierliches Lernen“</p></td>
   </tr>
   <tr>
     <td><p>5</p></td>
     <td><p>„Das Erlernen fortschrittlicher Algorithmen erweitert die KI-Fähigkeiten“</p></td>
   </tr>
</table>
<h3 id="Query-with-phrase-match" class="common-anchor-header">Abfrage mit Phrasenübereinstimmung<button data-href="#Query-with-phrase-match" class="anchor-icon" translate="no">
    ...
    </button></h3><p>Bei Verwendung der Methode „ <code translate="no">query()</code> “ fungiert <strong>PHRASE_MATCH</strong> als skalarer Filter. Es werden nur Dokumente zurückgegeben, die die angegebene Phrase enthalten (unter Berücksichtigung der zulässigen Toleranz).</p>
<h4 id="Example-slop--0-exact-match" class="common-anchor-header">Beispiel: slop = 0 (exakte Übereinstimmung)</h4><p>Dieses Beispiel liefert Dokumente, die genau die Phrase <strong>„machine learning“</strong> enthalten, ohne dass dazwischen weitere Token stehen.</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#go">   Go</a>
...
<table>
   ...
   <tr>
     <td><p>1</p></td>
     <td><p>„Machine Learning steigert die Effizienz bei der Analyse großer Datenmengen“</p></td>
   </tr>
</table>
<p>Nur Dokument 1 enthält die exakte Phrase <strong>„machine learning“</strong> in der angegebenen Reihenfolge ohne zusätzliche Token.</p>
<h3 id="Search-with-phrase-match" class="common-anchor-header">Suche mit Phrasenabgleich<button data-href="#Search-with-phrase-match" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Bei Suchvorgängen wird <strong>PHRASE_MATCH</strong> verwendet, um Dokumente zu filtern, bevor die Rangfolge anhand der Vektorähnlichkeit ermittelt wird. Bei diesem zweistufigen Ansatz wird zunächst die Kandidatenmenge durch textbasierte Übereinstimmung eingegrenzt und anschließend werden diese Kandidaten auf der Grundlage von Vektor-Embeddings neu geordnet.</p>
<h4 id="Example-slop--1" class="common-anchor-header">Beispiel: slop = 1</h4><p>Hier lassen wir eine Toleranz von 1 zu. Der Filter wird auf Dokumente angewendet, die die Phrase <strong>„learning machine“</strong> mit geringer Flexibilität enthalten.</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#go">   Go</a>
...
<table>
   ...
   <tr>
     <td><p>2</p></td>
     <td><p>„Das Erlernen eines maschinenbasierten Ansatzes ist für den Fortschritt der modernen KI von entscheidender Bedeutung“</p></td>
   </tr>
   <tr>
     <td><p>3</p></td>
     <td><p>„Deep-Learning-Architekturen optimieren die Rechenlasten“</p></td>
   ...
   <tr>
     <td><p>5</p></td>
     <td><p>„Das Erlernen fortschrittlicher maschineller Algorithmen erweitert die Fähigkeiten der KI“</p></td>
   </tr>
</table>
<h4 id="Example-slop--2" class="common-anchor-header">Beispiel: Toleranz = 2</h4><p>In diesem Beispiel ist ein Slop von 2 zulässig, was bedeutet, dass zwischen den Wörtern <strong>„machine“</strong> und <strong>„learning“</strong> bis zu zwei zusätzliche Token (oder in umgekehrter Reihenfolge stehende Begriffe) erlaubt sind <strong>.</strong></p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#go">   Go</a>
...
<table>
   ...
   <tr>
     <td><p>3</p></td>
     <td><p>„Deep-Learning-Architekturen optimieren die Rechenlast“</p></td>
   </tr>
</table>
<h4 id="Example-slop--3" class="common-anchor-header">Beispiel: slop = 3</h4><p>In diesem Beispiel bietet ein Slop-Wert von 3 noch mehr Flexibilität. Der Filter sucht nach <strong>„machine learning“</strong>, wobei bis zu drei Token-Positionen zwischen den Wörtern zulässig sind.</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#go">   Go</a>
...
<button class="copy-code-btn"></button></code></pre>
<p><strong>Trefferergebnisse:</strong></p>
<table>
   <tr>
     <th><p><code translate="no">doc_id</code></p></th>
     <th><p><code translate="no">text</code></p></th>
   ...
   <tr>
     <td><p>5</p></td>
     <td><p>„Das Erlernen fortschrittlicher maschineller Algorithmen erweitert die Möglichkeiten der KI“</p></td>
   </tr>
</table>
<h2 id="Considerations" class="common-anchor-header">Zu beachtende Aspekte<button data-href="#Considerations" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<li><p>Das Aktivieren des Phrasenabgleichs für ein Feld löst die Erstellung eines invertierten Index aus, der Speicherressourcen beansprucht. Berücksichtigen Sie die Auswirkungen auf den Speicherplatz, wenn Sie sich für die Aktivierung dieser Funktion entscheiden, da diese je nach Textgröße, eindeutigen Tokens und dem verwendeten Analysator variieren.</p></li>
<li><p>Sobald Sie einen Analysator in Ihrem Schema definiert haben, sind dessen Einstellungen für diese Sammlung dauerhaft festgelegt. Wenn Sie zu dem Schluss kommen, dass ein anderer Analysator Ihren Anforderungen besser entspricht, können Sie die bestehende Sammlung löschen und eine neue mit der gewünschten Analysatorkonfiguration erstellen.</p></li>
<li><p>Die Leistung des Phrasenabgleichs hängt davon ab, wie der Text tokenisiert wird. Bevor Sie einen Analysator auf Ihre gesamte Sammlung anwenden, sollten Sie die Tokenisierungsergebnisse mit der Methode „ <code translate="no">run_analyzer</code> “ überprüfen. Weitere Informationen finden Sie unter <a href="/docs/de/analyzer-overview.md#share-DYZvdQ2vUowWEwx1MEHcdjNNnqT">„Analysator-Übersicht</a>“.</p></li>
<li><p>Escape-Regeln in „ <code translate="no">filter</code> “-Ausdrücken:</p>
<ul>
<li><p>Zeichen, die in Ausdrücken in doppelte oder einfache Anführungszeichen gesetzt sind, werden als Zeichenfolgenkonstanten interpretiert. Enthält die Zeichenfolgenkonstante Escape-Zeichen, müssen diese mit einer Escape-Sequenz dargestellt werden. Verwenden Sie beispielsweise „ <code translate="no">\\</code> “, um „ <code translate="no">\</code> “ darzustellen, „ <code translate="no">\\t</code> “, um ein Tabulatorzeichen „ <code translate="no">\t</code> “ darzustellen, und „ <code translate="no">\\n</code> “, um ein Zeilenende darzustellen.</p></li>
<li><p>Wenn eine Zeichenfolgenkonstante in einfache Anführungszeichen gesetzt ist, sollte ein einfaches Anführungszeichen innerhalb der Konstante als „ <code translate="no">\\'</code> “ dargestellt werden, während ein doppeltes Anführungszeichen entweder als „ <code translate="no">&quot;</code> “ oder „ <code translate="no">\\&quot;</code> “ dargestellt werden kann. Beispiel: „ <code translate="no">'It\\'s milvus'</code> “.</p></li>
<li><p>Wenn eine Zeichenfolgenkonstante in doppelte Anführungszeichen gesetzt ist, sollte ein doppeltes Anführungszeichen innerhalb der Konstante als ` <code translate="no">\\&quot;</code> ` dargestellt werden, während ein einfaches Anführungszeichen entweder als ` <code translate="no">'</code> ` oder als ` <code translate="no">\\'</code>` dargestellt werden kann. Beispiel: ` <code translate="no">&quot;He said \\&quot;Hi\\&quot;&quot;</code>`.</p></li>
</ul></li>
</ul>
