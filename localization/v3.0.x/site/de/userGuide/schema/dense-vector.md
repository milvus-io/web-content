...
summary: >-
  ...
  von Dimensionen, die je nach spezifischer Anwendung und Anforderungen von
  einigen Dutzend bis zu mehreren Hundert oder sogar Tausenden reicht.
---
...
<h2 id="Overview" class="common-anchor-header">Überblick<button data-href="#Overview" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Dichte Vektoren werden typischerweise als Arrays aus Gleitkommazahlen mit fester Länge dargestellt, beispielsweise <code translate="no">[0.2, 0.7, 0.1, 0.8, 0.3, ..., 0.5]</code>. Die Dimensionalität dieser Vektoren reicht in der Regel von Hunderten bis zu Tausenden, wie etwa 128, 256, 768 oder 1024. Jede Dimension erfasst spezifische semantische Merkmale eines Objekts, wodurch der Vektor durch Ähnlichkeitsberechnungen in verschiedenen Szenarien eingesetzt werden kann.</p>
...
<p>Die obige Abbildung veranschaulicht die Darstellung dichter Vektoren in einem 2D-Raum. Obwohl dichte Vektoren in realen Anwendungen oft weitaus höhere Dimensionen aufweisen, vermittelt diese 2D-Darstellung mehrere Schlüsselkonzepte auf anschauliche Weise:</p>
<ul>
<li><p><strong>Mehrdimensionale Darstellung:</strong> Jeder Punkt repräsentiert ein konzeptionelles Objekt (wie <strong>Milvus</strong>, <strong>eine Vektordatenbank</strong>, <strong>ein Abfragesystem</strong> usw.), dessen Position durch die Werte seiner Dimensionen bestimmt wird.</p></li>
...
<p>Sobald die Daten vektorisiert sind, können sie in Milvus zur Verwaltung und zum Abruf der Vektoren gespeichert werden. Das folgende Diagramm veranschaulicht den grundlegenden Ablauf.</p>
<p><span class="img-wrapper">
...
<p>Neben dichten Vektoren unterstützt Milvus auch spärliche Vektoren und binäre Vektoren. Spärliche Vektoren eignen sich für präzise Übereinstimmungen auf der Grundlage bestimmter Begriffe, wie z. B. bei der Stichwortsuche und beim Begriffsabgleich, während binäre Vektoren häufig für die effiziente Verarbeitung binarisierter Daten verwendet werden, wie z. B. beim Abgleich von Bildmustern und bei bestimmten Hash-Anwendungen. Weitere Informationen finden Sie unter <a href="/docs/de/binary-vector.md">„Binäre Vektoren</a> “ und <a href="/docs/de/sparse_vector.md">„Spärliche Vektoren</a>“.</p>
</div>
<h2 id="Use-dense-vectors" class="common-anchor-header">Verwendung dichter Vektoren<button data-href="#Use-dense-vectors" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Um dichte Vektoren in Milvus zu verwenden, definieren Sie zunächst beim Erstellen einer Sammlung ein Vektorfeld zum Speichern dichter Vektoren. Dieser Vorgang umfasst:</p>
<ol>
...
<table>
   ...
   <tr>
     ...
     <td><p>Speichert 32-Bit-Gleitkommazahlen, die üblicherweise zur Darstellung von reellen Zahlen in wissenschaftlichen Berechnungen und im maschinellen Lernen verwendet werden. Ideal für Szenarien, die eine hohe Genauigkeit erfordern, wie z. B. die Unterscheidung ähnlicher Vektoren.</p></td>
   </tr>
   <tr>
     <td><p><code translate="no">FLOAT16_VECTOR</code></p></td>
     <td><p>Speichert 16-Bit-Gleitkommazahlen mit halber Genauigkeit, die für Deep Learning und GPU-Berechnungen verwendet werden. Dies spart Speicherplatz in Szenarien, in denen die Genauigkeit weniger kritisch ist, wie beispielsweise in der Low-Precision-Recall-Phase von Empfehlungssystemen.</p></td>
   </tr>
   <tr>
...
<h3 id="Set-index-params-for-vector-field" class="common-anchor-header">Indexparameter für Vektorfeld festlegen<button data-href="#Set-index-params-for-vector-field" class="anchor-icon" translate="no">
    ...
    </button></h3><p>Um semantische Suchen zu beschleunigen, muss ein Index für das Vektorfeld erstellt werden. Die Indizierung kann die Effizienz der Abfrage großer Vektordatenmengen erheblich verbessern.</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#javascript">   NodeJS</a>
...
<button class="copy-code-btn"></button></code></pre>
<p>Im obigen Beispiel wird für das Feld „ <code translate="no">dense_vector</code> “ ein Index mit dem Namen „ <code translate="no">dense_vector_index</code> “ unter Verwendung des Indextyps „ <code translate="no">AUTOINDEX</code> “ erstellt. Die „ <code translate="no">metric_type</code> “ wird auf „ <code translate="no">IP</code> “ gesetzt, was angibt, dass das Skalarprodukt als Distanzmetrik verwendet wird.</p>
<p>Milvus bietet verschiedene Indextypen für eine optimierte Vektorsuche. AUTOINDEX ist ein spezieller Indextyp, der entwickelt wurde, um den Einstieg in die Vektorsuche zu erleichtern. Es stehen zahlreiche Indextypen zur Auswahl. Weitere Informationen finden Sie unter <a href="/docs/de/index-explained.md">„Index Explained</a>“.</p>
<p>Milvus unterstützt weitere Metriktypen. Weitere Informationen finden Sie unter <a href="/docs/de/metric.md">„Metriktypen</a>“.</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>Weitere Informationen zu den Parametern für die Ähnlichkeitssuche finden Sie unter <a href="/docs/de/single-vector-search.md">„Grundlegende ANN-Suche</a>“.</p>
