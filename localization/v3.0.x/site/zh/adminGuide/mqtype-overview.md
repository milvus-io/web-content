...
title: 消息队列概述
summary: 概述 Milvus 支持的消息队列（mqType）选项，以及在独立部署和分布式部署中应使用哪一种。
---
<h1 id="Message-Queue-Overview" class="common-anchor-header">消息队列概述<button data-href="#Message-Queue-Overview" class="anchor-icon" translate="no">
    ...
    </button></h1><p>Milvus 依赖消息队列（预写日志，WAL）来管理最近的变更日志、输出流日志，并提供日志订阅功能。在 Milvus 3.x<strong>中，Woodpecker</strong>是默认的消息队列，无需单独的消息传递基础设施。Pulsar、Kafka 和 RocksMQ 仍适用于特定场景。</p>
...
<thead>
<tr><th>消息队列</th><th style="text-align:center">Milvus Standalone</th><th style="text-align:center">Milvus Distributed（集群）</th><th>默认配置于</th><th>备注</th></tr>
</thead>
<tbody>
<tr><td><a href="/docs/zh/woodpecker.md">Woodpecker</a></td><td style="text-align:center">✔️（嵌入式）</td><td style="text-align:center">✔️（嵌入式或服务）</td><td><strong>Milvus 3.x</strong>（两种模式）</td><td>默认且推荐。基于对象存储的云原生 WAL；无需外部服务。</td></tr>
...
<ul>
<li><p>每个 Milvus 实例仅使用一条消息队列。</p></li>
...
<li><strong>现有 Pulsar 或 Kafka 用户：</strong>Pulsar 和 Kafka 仍受完全支持。您可以继续使用它们，或<a href="/docs/zh/switch-mq-type.md">切换至 Woodpecker</a>。</li>
<li><strong>RocksMQ：</strong>仅支持独立部署，且在 Milvus 3.x 中已被嵌入式 Woodpecker 取代。</li>
</ul>
