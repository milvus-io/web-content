...
<p>该流程已在 Milvus 2.6.20 至 Milvus v3.0-beta 的升级过程中经过验证，所用环境包括 Milvus Operator 1.3.0、Woodpecker、集群内 etcd 以及集群内 MinIO。 如果您使用的是其他 Milvus 2.6.x 补丁版本、Milvus Operator 版本、消息队列或依赖项配置，请先在非生产环境中验证升级是否成功。</p>
</div>
...
<li><code translate="no">kubectl</code> 可访问该集群</li>
...
<p>本操作流程不支持通过将 Milvus 镜像回滚至 2.6.x 版本来执行降级或回滚。 在 v3.0-beta 写入数据后，仅回滚镜像的操作可能无法读取更新后的状态。如果升级失败，请停止写入操作，并使用恢复计划来还原升级前的元数据和持久化数据备份。请先在非生产环境中验证该恢复计划。</p>
</div>
...
<button class="copy-code-btn"></button></code></pre>
<p>请确认 Milvus CR 报告的镜像为<code translate="no">Healthy</code> ，当前镜像为<code translate="no">milvusdb/milvus:v3.0-beta</code> ，且现有 Collections 仍可查询和搜索。在启用任何 v3.0-beta 专属功能之前，请完成这些检查。</p>
