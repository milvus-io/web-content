...
<h1 id="Upgrade-Milvus-Standalone-with-Milvus-Operator" class="common-anchor-header">使用 Milvus Operator 升级 Milvus Standalone<button data-href="#Upgrade-Milvus-Standalone-with-Milvus-Operator" class="anchor-icon" translate="no">
    ...
    </button></h1><p>本指南介绍了如何使用 Milvus Operator 将 Milvus 2.6.x Standalone 部署升级至 v3.0.1。</p>
<div class="alert note">
<p>该流程已在 Milvus 2.6.20 至 Milvus v3.0.1 的环境中经过验证，所用环境包括 Milvus Operator 1.3.0、Woodpecker、集群内 etcd 以及集群内 MinIO。 如果您使用的是其他 Milvus 2.6.x 补丁版本、Milvus Operator 版本、消息队列或依赖项配置，请先在非生产环境中验证升级过程。</p>
</div>
...
<p>本操作流程不支持通过将 Milvus 镜像降级回 2.6.x 版本来执行降级或回滚。 在 v3.0.1 写入数据后，仅回滚映像的操作可能无法读取更新后的状态。如果升级失败，请停止写入操作，并使用恢复计划来还原升级前的元数据和持久化数据备份。请先在非生产环境中验证该恢复计划。</p>
</div>
...
<p>请将现有部署的源清单用作升级清单。切勿直接应用导出的备份文件，必须先移除由服务器管理的元数据和状态字段。</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>经过验证的升级方案将 Milvus Operator 版本保持为 1.3.0。除非您的支持政策要求单独升级 Operator，否则请保留当前管理您的 Milvus 2.6.x 部署的 Operator 版本。 请勿将较新版本的 Operator 降级至已测试版本。若需更改 Operator 版本，请采用与现有安装相同的 Helm 或<code translate="no">kubectl</code> 安装方式，并使用相同的发布名称和命名空间，随后在更新 Milvus CR 之前验证 Operator 的变更。</p>
<h3 id="Step-3-Update-the-Milvus-image" class="common-anchor-header">步骤 3：更新 Milvus 镜像<button data-href="#Step-3-Update-the-Milvus-image" class="anchor-icon" translate="no">
    ...
    </button></h3><p>在完整的 Milvus CR 清单中，仅修改<code translate="no">spec.components.image</code> 字段。保留现有的模式、组件设置、消息队列、etcd、存储及其他依赖项设置。以下摘录显示了需要修改的字段；请勿用此摘录替换您的完整 CR。</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>请确认 Milvus CR 报告的镜像路径为<code translate="no">Healthy</code> ，当前镜像为<code translate="no">milvusdb/milvus:v3.0.1</code> ，且现有 Collections 仍可进行查询和搜索。在启用任何 v3.0.1 特定功能之前，请完成这些检查。</p>
