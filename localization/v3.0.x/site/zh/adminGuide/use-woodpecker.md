...
<h2 id="Latency" class="common-anchor-header">延迟<button data-href="#Latency" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Woodpecker 是一款云原生 WAL，设计用于对象存储，在吞吐量、成本和延迟之间进行权衡。目前支持的轻量级嵌入式模式优先考虑成本和吞吐量优化，因为大多数场景只要求在一定时间内写入数据，而不是要求单个写入请求的低延迟。因此，啄木鸟采用分批写入的方式，本地文件系统存储后端的默认写入间隔为 10 毫秒，类 MinIO 存储后端的默认写入间隔为 200 毫秒。在慢速写操作期间，最大延迟等于间隔时间加上刷新时间。</p>
