...
<p>本流程已在 Milvus 2.6.20 至 Milvus v3.0-beta 的升级过程中经过验证，所用组件包括 Milvus Operator 1.3.0、MixCoord、StreamingNode、Woodpecker、集群内 etcd 以及集群内 MinIO。 如果您使用的 Milvus 2.6.x 补丁版本、Milvus Operator 版本、组件拓扑、消息队列或依赖关系配置与上述不同，请先在非生产环境中验证升级是否可行。</p>
</div>
...
<p>请为本次升级应用完整的 Milvus CR。请勿使用仅包含镜像的合并补丁。操作符可能会默认填充被省略的零副本组件字段，这可能会重新启用现有 2.6.x 部署中已禁用的组件。</p>
<p>此流程不验证通过将 Milvus 镜像回滚至 2.6.x 版本进行的降级或回滚操作。 在 v3.0-beta 写入数据后，仅镜像的回滚可能无法读取更新后的状态。如果升级失败，请停止写入操作，并使用恢复计划来还原升级前的元数据和持久化数据备份。请先在非生产环境中验证该恢复计划。</p>
</div>
...
<button class="copy-code-btn"></button></code></pre>
<p>请确认 Milvus CR 报告的值为<code translate="no">Healthy</code> ，所有 Milvus 组件均使用<code translate="no">milvusdb/milvus:v3.0-beta</code> ，没有 IndexNode Pod 在运行，且现有 Collections 仍可进行查询和搜索。在启用任何 v3.0-beta 特定功能之前，请完成这些检查。</p>
