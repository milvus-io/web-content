...
<h1 id="Upgrade-Milvus-Cluster-with-Milvus-Operator" class="common-anchor-header">使用 Milvus Operator 升级 Milvus 集群<button data-href="#Upgrade-Milvus-Cluster-with-Milvus-Operator" class="anchor-icon" translate="no">
    ...
    </button></h1><p>本指南介绍了如何使用 Milvus Operator 将 Milvus 2.6.x 集群升级至 v3.0.2。</p>
<div class="alert note">
<p>该流程已在 Milvus 2.6.20 至 Milvus v3.0.2 的环境中经过验证，所用组件包括 Milvus Operator 1.3.0、MixCoord、StreamingNode、Woodpecker、集群内 etcd 以及集群内 MinIO。 如果您使用的 Milvus 2.6.x 补丁版本、Milvus Operator 版本、组件拓扑、消息队列或依赖项配置与上述不同，请先在非生产环境中验证升级操作。</p>
</div>
...
<p>此流程不支持通过将 Milvus 镜像回滚至 2.6.x 版本来执行降级或回滚。 在 v3.0.2 写入数据后，仅镜像的回滚可能无法读取更新后的状态。如果升级失败，请停止写入操作，并使用恢复计划来还原升级前的元数据和持久化数据备份。请先在非生产环境中验证该恢复计划。</p>
</div>
...
<p>将现有部署的源清单用作升级清单。请勿直接应用导出的备份文件，必须先移除由服务器管理的元数据和状态字段。</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>请验证 Milvus CR 是否报告<code translate="no">Healthy</code> ，所有 Milvus 组件是否均使用<code translate="no">milvusdb/milvus:v3.0.2</code> ，是否没有 IndexNode Pod 在运行，以及现有 Collections 是否仍可查询和搜索。在启用任何 v3.0.2 特定功能之前，请完成这些检查。</p>
