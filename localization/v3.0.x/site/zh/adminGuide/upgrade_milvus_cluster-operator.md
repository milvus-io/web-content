...
<h1 id="Upgrade-Milvus-Cluster-with-Milvus-Operator" class="common-anchor-header">使用 Milvus Operator 升级 Milvus 集群<button data-href="#Upgrade-Milvus-Cluster-with-Milvus-Operator" class="anchor-icon" translate="no">
    ...
    </button></h1><p>本指南介绍了如何使用 Milvus Operator 将 Milvus 2.6.x 集群升级至 v3.0.1。</p>
<div class="alert note">
<p>本流程已在 Milvus 2.6.20 至 Milvus v3.0.1 环境中经过验证，涉及 Milvus Operator 1.3.0、MixCoord、StreamingNode、Woodpecker、集群内 etcd 以及集群内 MinIO。 如果您使用的 Milvus 2.6.x 补丁版本、Milvus Operator 版本、组件拓扑、消息队列或依赖关系配置与上述不同，请先在非生产环境中验证升级效果。</p>
</div>
...
<li>一个由 Milvus Operator 管理的包含 Milvus 2.6.x 集群的 Kubernetes 集群</li>
...
<p><strong>消息队列限制</strong>：升级至 Milvus v3.0.1 时，您必须保留当前的消息队列选择。升级过程中不支持在不同的消息队列系统之间切换。未来版本将支持更改消息队列系统。</p>
<div class="alert warning">
<p>请为此次升级应用完整的 Milvus CR。请勿使用仅包含镜像的合并补丁。操作符可能会默认填充被省略的零副本组件字段，这可能会重新启用现有 2.6.x 部署中已禁用的组件。</p>
<p>此流程不验证通过将 Milvus 镜像回滚至 2.6.x 版本进行的降级或回滚操作。 在 v3.0.1 写入数据后，仅镜像的回滚可能无法读取更新后的状态。如果升级失败，请停止写入操作，并使用恢复计划来还原升级前的元数据和持久化数据备份。请先在非生产环境中验证该恢复计划。</p>
</div>
...
<p>请使用现有部署的源清单作为升级清单。切勿在未先移除服务器管理的元数据和状态字段的情况下，直接应用导出的备份文件。</p>
...
<h3 id="Step-3-Update-the-Milvus-image" class="common-anchor-header">步骤 3：更新 Milvus 镜像<button data-href="#Step-3-Update-the-Milvus-image" class="anchor-icon" translate="no">
    ...
    </button></h3><p>在完整的 Milvus CR 清单中，将<code translate="no">spec.components.image</code> 更改为目标版本。保留当前的模式、组件拓扑、消息队列、etcd、存储以及其他依赖项设置。以下摘录展示了需要确认的字段；请勿用此摘录替换您的完整 CR。</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>请验证 Milvus CR 是否报告<code translate="no">Healthy</code> ，所有 Milvus 组件是否均使用<code translate="no">milvusdb/milvus:v3.0.1</code> ，是否没有 IndexNode Pod 在运行，以及现有 Collections 是否仍可查询和搜索。在启用任何 v3.0.1 特定功能之前，请完成这些检查。</p>
