...
<h1 id="Upgrade-Milvus-Cluster-with-Milvus-Operator" class="common-anchor-header">使用 Milvus Operator 升级 Milvus 集群<button data-href="#Upgrade-Milvus-Cluster-with-Milvus-Operator" class="anchor-icon" translate="no">
    ...
    </button></h1><p>本指南介绍了如何使用 Milvus Operator 将 Milvus 2.6.x 集群升级至 v3.0-beta 版本。</p>
<div class="alert note">
<p>本流程已在 Milvus 2.6.20 至 Milvus v3.0-beta 的升级过程中经过验证，涉及 Milvus Operator 1.3.0、MixCoord、StreamingNode、Woodpecker、集群内 etcd 以及集群内 MinIO。 如果您使用的 Milvus 2.6.x 补丁版本、Milvus Operator 版本、组件拓扑、消息队列或依赖配置与上述不同，请先在非生产环境中验证升级是否可行。</p>
</div>
...
<p>请为此次升级应用完整的 Milvus CR。请勿使用仅包含镜像的合并补丁。操作符可能会默认填充被省略的、副本数为零的组件字段，这可能会重新启用现有 2.6.x 部署中已禁用的组件。</p>
<p>本操作流程不验证通过将 Milvus 镜像回滚至 2.6.x 版本进行的降级或回滚操作。 在 v3.0-beta 写入数据后，仅镜像的回滚操作可能无法读取已更新的状态。如果升级失败，请停止写入操作，并使用恢复计划来还原升级前的元数据和持久化数据备份。请先在非生产环境中验证恢复计划。</p>
</div>
...
<p>请将现有部署的源清单用作升级清单。请勿直接应用导出的备份文件，必须先移除服务器管理的元数据和状态字段。</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>经过验证的升级方案将 Milvus Operator 版本保持在 1.3.0。除非您的支持政策要求单独升级 Operator，否则请保留当前管理您的 Milvus 2.6.x 部署的 Operator 版本。 请勿将较新版本的 Operator 降级至已测试版本。若需更改 Operator 版本，请使用与现有安装相同的 Helm 或<code translate="no">kubectl</code> 安装方式，并采用相同的发布名称和命名空间，然后在更新 Milvus CR 之前验证 Operator 变更。</p>
...
<p>在应用目标 CR 之前，请确认<code translate="no">indexNode.replicas</code> 为<code translate="no">0</code> 。已验证的 Milvus 2.6.20 配置已使用此设置。请在目标 CR 中保留显式的零副本设置。</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>验证 Milvus CR 是否报告<code translate="no">Healthy</code> ，所有 Milvus 组件是否均使用<code translate="no">milvusdb/milvus:v3.0-beta</code> ，是否没有 IndexNode Pod 在运行，以及现有 Collections 是否仍可查询和搜索。在启用任何 v3.0-beta 特定功能之前，请完成这些检查。</p>
