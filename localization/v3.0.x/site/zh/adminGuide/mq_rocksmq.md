...
<h1 id="Use-RocksMQ-as-the-Milvus-Message-Queue" class="common-anchor-header">将 RocksMQ 用作 Milvus 消息队列<button data-href="#Use-RocksMQ-as-the-Milvus-Message-Queue" class="anchor-icon" translate="no">
    ...
    </button></h1><p>RocksMQ 是随 Milvus 捆绑提供的嵌入式消息队列（WAL），<strong>仅</strong>适用于<strong>Milvus Standalone</strong>。在早期 Milvus 版本中，它是默认的独立版消息队列；而在 Milvus 3.x 中，Milvus Standalone 默认使用嵌入式<a href="/docs/zh/woodpecker.md">Woodpecker</a>。</p>
...
<h2 id="Deploy-Milvus-Standalone-with-RocksMQ-using-Docker" class="common-anchor-header">使用 Docker 部署带 RocksMQ 的 Milvus Standalone<button data-href="#Deploy-Milvus-Standalone-with-RocksMQ-using-Docker" class="anchor-icon" translate="no">
    ...
    </button></h3><p>请参考<a href="/docs/zh/install_standalone-docker.md">《在 Docker 中运行 Milvus Standalone</a>》。在 Milvus 3.x 中，独立模式的默认消息队列为 Woodpecker，因此需显式将消息队列类型切换为 RocksMQ。引导脚本会在<strong>首次</strong>运行时（<code translate="no">start</code> ）生成新的<code translate="no">user.yaml</code> ，因此<strong>请在</strong>首次<strong>启动后</strong>设置类型，然后执行<code translate="no">restart</code> 使其生效（<code translate="no">restart</code> 会保留<code translate="no">user.yaml</code> ）：</p>
...
通过这种方式切换<code translate="no">mq.type</code> 仅适用于<b>全新</b>实例（尚未包含任何 Collection）。若要更改已包含数据的实例的消息队列，请改用<a href="/docs/zh/switch-rocksmq-woodpecker.md">切换流程</a>。
</div>
<h3 id="Configure" class="common-anchor-header">配置<button data-href="#Configure" class="anchor-icon" translate="no">
    ...
    </button></h3><p>要调整 RocksMQ 的配置，请在<code translate="no">user.yaml</code> 文件中添加<code translate="no">rocksmq</code> 部分，然后重启服务：</p>
...
由于 2.6.x 将独立部署模式的默认消息队列更改为 Woodpecker，若您希望保留 RocksMQ，请<strong>在升级前</strong>将<code translate="no">mq.type: rocksmq</code> 固定在您的<code translate="no">user.yaml</code> 文件中。</li>
