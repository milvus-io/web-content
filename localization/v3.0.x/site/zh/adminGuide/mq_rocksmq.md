...
<h2 id="Deploy-Milvus-Standalone-with-RocksMQ-using-Docker" class="common-anchor-header">使用 Docker 部署带 RocksMQ 的 Milvus Standalone<button data-href="#Deploy-Milvus-Standalone-with-RocksMQ-using-Docker" class="anchor-icon" translate="no">
    ...
    </button></h3><p>请按照《<a href="/docs/zh/install_standalone-docker.md">在 Docker 中运行 Milvus Standalone</a>》中的步骤操作。在 Milvus 3.x 中，独立部署的默认消息队列是 Woodpecker，因此需显式将消息队列类型切换为 RocksMQ。引导脚本会在<strong>首次</strong>运行时将<code translate="no">user.yaml</code> 写入<code translate="no">start</code> ，因此<strong>请在</strong>首次<strong>启动后</strong>设置类型，然后执行<code translate="no">restart</code> 以应用更改（<code translate="no">restart</code> 会保留<code translate="no">user.yaml</code> ）：</p>
