...
<p>注意：对正在运行的集群切换<code translate="no">mq.type</code> 属于升级操作。请严格遵循升级流程，并在切换至生产环境前，先在全新集群上进行验证。</p>
<h2 id="Configuration" class="common-anchor-header">配置<button data-href="#Configuration" class="anchor-icon" translate="no">
    ...
    </button></h2><p>以下是完整的Woodpecker配置块（编辑<code translate="no">milvus.yaml</code> 或在<code translate="no">user.yaml</code> 中进行覆盖）：</p>
...
<ul>
<li>控制客户端的分段追加/滚动/审计行为，以平衡吞吐量和端到端延迟。</li>
</ul></li>
...
<tbody>
<tr><td><code translate="no">minio</code> （默认）</td><td>嵌入在 Milvus/流式处理节点中</td><td>对象存储（MinIO/S3兼容）</td><td>受支持</td><td>支持</td></tr>
...
<p>注：</p>
<ul>
<li>在<code translate="no">minio</code> 中，Woodpecker与Milvus共享相同的对象存储（MinIO/S3/GCS/OSS等）。</li>
...
<li>此兼容性矩阵反映了<a href="https://github.com/zilliztech/woodpecker/discussions/150">当前讨论情况</a>，随着后端支持的进一步验证，内容可能会有所变化。</li>
</ul>
<h2 id="Deployment-guides" class="common-anchor-header">部署指南<button data-href="#Deployment-guides" class="anchor-icon" translate="no">
    ...
    </button></h3><p>安装<a href="/docs/zh/install_cluster-milvusoperator.md">Milvus Operator</a> 后，请使用官方示例启动一个启用了 Woodpecker 的 Milvus 集群：</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>此示例将 Woodpecker 配置为消息队列，并启用了流式处理节点。首次启动时可能需要一些时间来拉取镜像；请等待直至所有 Pod 准备就绪：</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>若需调整 Woodpecker 配置，请在首次启动后编辑生成的 `<code translate="no">user.yaml</code> ` 文件，并运行 `<code translate="no">bash standalone_embed.sh restart</code> ` 以应用更改（执行 `<code translate="no">start</code> ` 会重新生成 `<code translate="no">user.yaml</code>` 文件，因此请使用 `<code translate="no">restart</code>` 应用编辑内容）：</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>这将 Woodpecker 作为独立的 StatefulSet（<code translate="no">my-release-milvus-woodpecker</code> ，默认 4 个副本）进行部署，由无头服务作为前端，通过端口<code translate="no">18080</code> （服务）、<code translate="no">17946</code> （闲聊）和<code translate="no">9091</code> （指标）进行闲聊集群，并使用 MinIO 作为存储后端。 该服务需要<strong>3</strong>个节点的法定人数；默认的<strong>4</strong>个副本既能保证法定人数，又能容忍单节点故障，因此请勿将 `<code translate="no">woodpecker.replicaCount</code> ` 设置为小于 3。该集群还包含一个独立的<code translate="no">woodpecker</code> Pod 集合：</p>
...
<p>Woodpecker<code translate="no">service</code> 模式仅适用于<strong>分布式/集群部署</strong>——独立部署将运行内嵌版 Woodpecker（<code translate="no">minio</code> 或<code translate="no">local</code> ）。Milvus Operator 目前尚不支持 Woodpecker 服务模式。</p>
</div>
<h2 id="Throughput-tuning-tips" class="common-anchor-header">吞吐量调优建议<button data-href="#Throughput-tuning-tips" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Woodpecker<strong>在嵌入式</strong>模式<strong>和服务</strong>模式（Milvus 3.0 的新功能）下的吞吐量和延迟表现各不相同。以下指南按模式分类。</p>
...
<li>控制刷新/索引构建的时机（在触发前先进行批量处理），以避免频繁的小写入。</li>
</ul></li>
</ul>
...
<h2 id="Latency" class="common-anchor-header">延迟<button data-href="#Latency" class="anchor-icon" translate="no">
    ...
    </button></h3><p>Woodpecker 是一款专为对象存储设计的云原生 WAL，在吞吐量、成本和延迟之间实现了权衡。轻量级的嵌入式模式优先考虑成本和吞吐量的优化，因为大多数场景仅要求数据在一定时间内写入，而非对单个写入请求要求低延迟。 因此，Woodpecker 采用批量写入机制，针对本地文件系统存储后端，默认间隔为 10 毫秒；针对 MinIO 类存储后端，默认间隔为 200 毫秒。在写入速度较慢的情况下，最大延迟等于间隔时间加上刷新时间之和。</p>
<p>请注意，批量插入不仅由时间间隔触发，还受批处理大小影响，其默认值为 2MB。</p>
<h3 id="Service-mode-Milvus-30+" class="common-anchor-header">服务模式（Milvus 3.0+）<button data-href="#Service-mode-Milvus-30+" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
    ...
    </button></h3><p>服务模式在保持低成本的同时，实现了<strong>毫秒级写入延迟</strong>——与传统的三副本本地磁盘 WAL 处于同一量级。在典型的三副本跨可用区（AZ）部署中，写入延迟始终保持在毫秒范围内。其实现方式如下：</p>
<ul>
<li><strong>单 RTT 法定数写入</strong>——客户端驱动的复制可在单次往返内完成法定数写入，跨可用区流量固定为两个副本的数据量（相比之下，基于代理/领导者的复制通常会产生额外的约 1/3 跨可用区流量）。</li>
<li><strong>拓扑感知单跳读取</strong>——每次读取直接访问最近的副本，而非通过代理转发，从而避免了基于代理系统中随机跨可用区读取（约占跨可用区读取流量的 2/3）的情况。</li>
