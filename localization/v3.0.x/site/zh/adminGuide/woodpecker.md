...
summary: 了解 Woodpecker 如何作为 Milvus 中的默认消息队列（WAL）工作，以及如何以嵌入式或服务模式运行它。
---
...
<li>Woodpecker 可以<strong>嵌入</strong>Milvus/流处理节点中运行（默认），也可以作为具有独立 Pod 的<strong>专用服务</strong>运行（仅限分布式/集群环境）。</li>
<li>它支持三种<code translate="no">storage.type</code> 模式：对象存储（<code translate="no">minio</code> ，默认）、本地文件系统（<code translate="no">local</code> ）以及专用模式（<code translate="no">service</code> ）。请参阅<a href="#Deployment-modes">部署模式</a>。</li>
</ul>
...
<ul>
<li>控制日志分段的同步/刷新/压缩/读取策略。这些是调整吞吐量和延迟的主要控制参数。</li>
</ul></li>
...
<li><strong>rootPath</strong>：存储后端的根路径（对<code translate="no">local</code> 有效；对于<code translate="no">minio</code> ，路径由存储桶/前缀决定）。</li>
</ul></li>
</ul>
...
<tbody>
<tr><td><code translate="no">minio</code> （默认）</td><td>嵌入 Milvus/流式处理节点</td><td>对象存储（MinIO/S3兼容）</td><td>受支持</td><td>支持</td></tr>
...
<h2 id="Object-storage-compatibility-for-storagetypeminio" class="common-anchor-header">对象存储兼容性<code translate="no">storage.type=minio</code><button data-href="#Object-storage-compatibility-for-storagetypeminio" class="anchor-icon" translate="no">
    ...
    </button></h2><p>下表总结了当 Woodpecker 配置为<code translate="no">storage.type=minio</code> 时，目前已知的对象存储后端兼容性。此信息基于<a href="https://github.com/zilliztech/woodpecker/discussions/150">GitHub 讨论 #150</a>。</p>
<table>
<thead>
...
<h3 id="Enable-Woodpecker-for-Milvus-Standalone-in-Docker-storagelocal" class="common-anchor-header">在 Docker 中的 Milvus Standalone（storage=local）中启用 Woodpecker<button data-href="#Enable-Woodpecker-for-Milvus-Standalone-in-Docker-storagelocal" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>在 Milvus 3.x 中，Docker 独立部署<strong>默认</strong>使用 Woodpecker<strong>并以本地文件系统</strong>作为其 WAL 后端——无需额外配置。请按照《<a href="/docs/zh/install_standalone-docker.md">在 Docker 中运行 Milvus Stand</a>alone》中的说明操作：</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>若需调整 Woodpecker 配置，请在首次启动后编辑生成的 `<code translate="no">user.yaml</code> ` 文件，并运行 `<code translate="no">bash standalone_embed.sh restart</code> ` 以应用更改（执行 `<code translate="no">start</code> ` 会重新生成 `<code translate="no">user.yaml</code>`，因此请通过 `<code translate="no">restart</code>` 应用编辑内容）：</p>
...
<p>对于 Woodpecker 服务模式，我们建议使用即将发布的 Milvus 3.0.1 或更高版本，并搭配 Woodpecker v0.1.37 或更高版本，以获得压缩清理和组提交优化。</p>
</div>
<p>Woodpecker<strong>服务模式是</strong> <strong>Milvus 3.0</strong>的一项功能。对于分布式/集群部署，您可以通过设置<code translate="no">streaming.woodpecker.embedded=false</code> ，将 Woodpecker 作为<strong>专用服务</strong>（独立 Pod）运行，而非嵌入流式处理节点中：</p>
<pre><code translate="no" class="language-bash">helm install my-release zilliztech/milvus \
  --<span class="hljs-built_in">set</span> image.all.tag=v3.0.2 \
  --<span class="hljs-built_in">set</span> woodpecker.enabled=<span class="hljs-literal">true</span> \
...
<button class="copy-code-btn"></button></code></pre>
<p>这将把 Woodpecker 作为独立的 StatefulSet（<code translate="no">my-release-milvus-woodpecker</code> ，默认 4 个副本）进行部署，由一个无头服务作为前端，通过端口<code translate="no">18080</code> （服务）、<code translate="no">17946</code> （流言）和<code translate="no">9091</code> （指标）进行流言集群，并使用 MinIO 作为其存储后端。 该服务需要<strong>3</strong>个节点的法定人数；默认的<strong>4</strong>个副本既能保持法定人数，又能容忍单节点故障，因此请勿将 `<code translate="no">woodpecker.replicaCount</code> ` 设置为小于 3。该集群还包含一个独立的 `<code translate="no">woodpecker</code> ` pod 集合：</p>
...
<p>Woodpecker<code translate="no">service</code> 模式仅适用于<strong>分布式/集群部署</strong>——独立部署需运行嵌入式 Woodpecker（<code translate="no">minio</code> 或<code translate="no">local</code> ）。Milvus Operator 目前尚不支持 Woodpecker 服务模式。</p>
</div>
<h2 id="Throughput-tuning-tips" class="common-anchor-header">吞吐量调优提示<button data-href="#Throughput-tuning-tips" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>Woodpecker<strong>在嵌入式模式</strong> <strong>和服务</strong>模式（Milvus 3.0 的新功能）下的吞吐量和延迟表现各不相同。以下指导按模式分类。</p>
<h3 id="Embedded-mode" class="common-anchor-header">嵌入式模式<button data-href="#Embedded-mode" class="anchor-icon" translate="no">
    ...
    </button></h3><p>基于<a href="/docs/zh/woodpecker_architecture.md">Woodpecker</a> 中的基准测试结果和后端限制，请从以下方面优化端到端写入吞吐量：</p>
<ul>
<li>存储端
<ul>
<li><strong>对象存储（MinIO/S3兼容）</strong>：提高并发数并增大对象大小（避免使用微小对象）。注意网络和存储桶的带宽限制。单个基于SSD的MinIO节点本地吞吐量通常上限约为100 MB/s；单个EC2到S3的吞吐量可达GB/s。</li>
<li><strong>本地/共享文件系统（本地）</strong>：优先选用 NVMe 或高速磁盘。确保文件系统能良好处理小文件写入操作，并具备良好的 fsync 延迟表现。</li>
</ul></li>
<li>Woodpecker 配置参数
<ul>
...
<li>对于对象存储，建议增加<code translate="no">segmentRollingPolicy.maxSize</code> 值以减少分段切换。</li>
</ul></li>
<li>客户端/应用程序端
<ul>
...
<p><strong>批量插入演示</strong>——使用以下内容来测量写入吞吐量：</p>
...
<li><strong>分段滚动后立即上传至对象存储</strong>——每个分段都会追踪其完整生命周期，并在滚动后立即上传至对象存储，从而在保持低延迟的同时，有效控制本地磁盘占用空间和存储成本。</li>
