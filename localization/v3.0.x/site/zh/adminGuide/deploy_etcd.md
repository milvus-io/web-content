...
<h2 id="Configure-etcd-with-Docker-Compose" class="common-anchor-header">使用 Docker Compose 配置 etcd<button data-href="#Configure-etcd-with-Docker-Compose" class="anchor-icon" translate="no">
    ...
    </button></h2><h3 id="1-Configure-etcd" class="common-anchor-header">1.配置 etcd<button data-href="#1-Configure-etcd" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>要使用 Docker Compose 配置 etcd，请为<code translate="no">milvus.yaml</code> 文件中的<code translate="no">etcd</code> 部分提供值，该文件位于 Milvus/configs 路径下。</p>
...
<h3 id="2-Run-Milvus" class="common-anchor-header">2.运行 Milvus<button data-href="#2-Run-Milvus" class="anchor-icon" translate="no">
    ...
    </button></h3><p>运行以下命令启动使用 etcd 配置的 Milvus。</p>
...
<h2 id="Configure-etcd-on-K8s" class="common-anchor-header">在 K8s 上配置 etcd<button data-href="#Configure-etcd-on-K8s" class="anchor-icon" translate="no">
    ...
    </button></h2><p>对于 K8s 上的 Milvus 群集，可以在启动 Milvus 的同一命令中配置 etcd。或者，也可以在启动 Milvus 前，使用<a href="https://github.com/milvus-io/milvus-helm">milvus-helm</a>资源库中 /charts/milvus 路径下的<code translate="no">values.yml</code> 文件配置 etcd。</p>
<p>下表列出了在 YAML 文件中配置 etcd 的关键字。</p>
<table>
<thead>
...
<h3 id="Using-a-command" class="common-anchor-header">使用命令<button data-href="#Using-a-command" class="anchor-icon" translate="no">
    ...
    </button></h3><p>要安装 Milvus 并配置 etcd，请使用你的值运行以下命令。</p>
