...
<p>指定消息存储时还存在其他限制：</p>
<ul>
<li>每个 Milvus 实例仅支持一个消息存储。不过，我们仍向后兼容单个实例配置多个消息存储的情况。优先级顺序如下：
<ul>
...
<li><strong>升级限制</strong>：<strong>消息队列限制</strong>：升级至 Milvus v3.0.1 时，必须保留当前选择的消息队列。升级过程中不支持在不同的消息队列系统之间切换。未来版本将支持更改消息队列系统。</li>
</ul>
...
<li><code translate="no">persistence.pvcDeletion</code>: 当值为 true 时，在删除 Milvus 实例时将删除 PVC</li>
...
<h2 id="Configure-Woodpecker" class="common-anchor-header">配置 Woodpecker<button data-href="#Configure-Woodpecker" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Woodpecker 是一款专为对象存储设计的云原生预写日志（WAL）。它具有高吞吐量、低操作开销和无缝可扩展性。更多详情，请参阅<a href="/docs/zh/woodpecker.md">Woodpecker</a>。</p>
<h2 id="Configure-Pulsar" class="common-anchor-header">配置 Pulsar<button data-href="#Configure-Pulsar" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Pulsar 管理近期变更日志、输出流日志并提供日志订阅服务。无论是 Milvus Standalone 模式还是 Milvus 集群模式，均支持将 Pulsar 配置为消息存储。但若使用 Milvus Operator，则仅能将 Pulsar 配置为 Milvus 集群的消息存储。请在 `<code translate="no">spec.dependencies.pulsar</code> ` 下添加必填字段以配置 Pulsar。</p>
