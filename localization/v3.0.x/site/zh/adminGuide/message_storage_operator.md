...
summary: 了解如何使用 Milvus Operator 配置消息存储。
---
<h1 id="Configure-Message-Storage-with-Milvus-Operator" class="common-anchor-header">使用 Milvus Operator 配置消息存储<button data-href="#Configure-Message-Storage-with-Milvus-Operator" class="anchor-icon" translate="no">
    ...
    </button></h1><p>在 Milvus 3.x 中，Woodpecker 是默认的消息队列（参见<a href="/docs/zh/woodpecker.md">Woodpecker</a>）。通过 Milvus Operator，您还可以配置 RocksMQ、Pulsar 或 Kafka 来管理最新变更日志、输出流日志以及提供日志订阅。 本主题介绍在使用 Milvus Operator 安装 Milvus 时如何配置消息存储依赖项。更多详细信息，请参阅 Milvus Operator 仓库中的<a href="https://github.com/zilliztech/milvus-operator/blob/main/docs/administration/manage-dependencies/message-storage.md">《使用 Milvus Operator 配置消息存储》</a>。</p>
...
<p>您只需编辑 `<code translate="no">milvus_cluster_default.yaml</code> ` 中的代码模板即可配置第三方依赖项。以下各节将分别介绍如何配置对象存储、etcd 和 Pulsar。</p>
<h2 id="Before-you-begin" class="common-anchor-header">开始之前<button data-href="#Before-you-begin" class="anchor-icon" translate="no">
    ...
    </button></h2><p>下表展示了 Milvus Standalone 和集群模式是否支持 RocksMQ、Pulsar、Kafka 以及 Woodpecker。</p>
<table>
<thead>
...
<p>指定消息存储时还存在其他限制：</p>
<ul>
<li>每个 Milvus 实例仅支持一个消息存储。不过，我们仍向后兼容为单个实例配置多个消息存储的情况。优先级如下：
<ul>
<li>独立模式：Woodpecker（默认） &gt; RocksMQ &gt; Pulsar &gt; Kafka</li>
<li>集群模式：Woodpecker（默认）&gt; Pulsar &gt; Kafka</li>
</ul></li>
<li>Milvus 系统运行期间无法更改消息存储。</li>
<li>仅支持 Kafka 2.x 或 3.x 版本。</li>
<li><strong>升级限制</strong>：<strong>消息队列限制</strong>：升级至 Milvus v3.0-beta 时，必须保留当前的消息队列选择。升级过程中不支持在不同的消息队列系统之间切换。未来版本将支持更改消息队列系统。</li>
</ul>
<h2 id="Configure-RocksMQ" class="common-anchor-header">配置 RocksMQ<button data-href="#Configure-RocksMQ" class="anchor-icon" translate="no">
    ...
    </button></h2><p>在 Milvus Standalone 独立部署版本 2.5.x 及更早版本中，RocksMQ 是默认的消息存储（从 2.6.x 版本起被 Woodpecker 取代）。</p>
<div class="alert note">
<p>目前，您只能通过 Milvus Operator 将 RocksMQ 配置为 Milvus Standalone 的消息存储。</p>
</div>
<h4 id="Example" class="common-anchor-header">示例</h4><p>以下示例演示了如何配置 RocksMQ 服务。</p>
...
<button class="copy-code-btn"></button></code></pre>
<h5 id="Key-configuration-options" class="common-anchor-header">关键配置选项：</h5><ul>
<li><code translate="no">msgStreamType</code>: rocksmq：显式将 RocksMQ 设置为消息队列</li>
<li><code translate="no">persistence.enabled</code>: 启用 RocksMQ 数据的持久化存储</li>
<li><code translate="no">persistence.pvcDeletion</code>: 当值为 true 时，在删除 Milvus 实例时将一并删除 PVC</li>
<li><code translate="no">persistentVolumeClaim.spec</code>: 标准的 Kubernetes PVC 规范</li>
<li><code translate="no">accessModes</code>: 通常用于块存储时为<code translate="no">ReadWriteOnce</code> </li>
...
<h2 id="Configure-Woodpecker" class="common-anchor-header">配置 Woodpecker<button data-href="#Configure-Woodpecker" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Woodpecker 是一款专为对象存储设计的云原生预写日志（WAL）。它具有高吞吐量、低运维开销和无缝可扩展性。更多详细信息，请参阅<a href="/docs/zh/woodpecker.md">Woodpecker</a>。</p>
...
<h3 id="External-Pulsar" class="common-anchor-header">外部 Pulsar<button data-href="#External-Pulsar" class="anchor-icon" translate="no">
    ...
    </button></h3><p><code translate="no">external</code> 表示使用外部 Pulsar 服务。
用于配置外部 Pulsar 服务的字段包括：</p>
<ul>
...
<h4 id="Example" class="common-anchor-header">示例</h4><p>以下示例配置了一个外部 Pulsar 服务。</p>
...
<h3 id="Internal-Pulsar" class="common-anchor-header">内部 Pulsar<button data-href="#Internal-Pulsar" class="anchor-icon" translate="no">
    ...
    </button></h3><p><code translate="no">inCluster</code> 表示当 Milvus 集群启动时，Pulsar 服务会在集群内自动启动。</p>
<h4 id="Example" class="common-anchor-header">示例</h4><p>以下示例配置了一个内部 Pulsar 服务。</p>
<pre><code translate="no" class="language-YAML"><span class="hljs-attr">apiVersion:</span> <span class="hljs-string">milvus.io/v1alpha1</span>
<span class="hljs-attr">kind:</span> <span class="hljs-string">Milvus</span>
<span class="hljs-attr">metadata:</span>
  <span class="hljs-attr">name:</span> <span class="hljs-string">my-release</span>
...
<div class="alert note">请在<a href="https://artifacthub.io/packages/helm/apache/pulsar/2.7.8?modal=values">values.yaml</a> 中查找配置内部 Pulsar 服务的完整配置项。如前例所示，在 `<code translate="no">pulsar.inCluster.values</code> ` 下根据需要添加配置项。</div>
...
<h2 id="Configure-Kafka" class="common-anchor-header">配置 Kafka<button data-href="#Configure-Kafka" class="anchor-icon" translate="no">
    ...
    </button></h2><p>在 2.5.x 及更早版本中，Pulsar 是 Milvus 集群中的默认消息存储（从 2.6.x 版本起被 Woodpecker 取代）。若要使用 Kafka，请添加可选字段<code translate="no">msgStreamType</code> 来配置 Kafka。</p>
...
<ul>
<li><code translate="no">external</code>:<code translate="no">true</code> 值为 1 表示 Milvus 使用外部 Kafka 服务。</li>
<li><code translate="no">brokerList</code>：要发送消息的经纪人列表。</li>
</ul>
...
<button class="copy-code-btn"></button></code></pre>
<blockquote>
<p>操作员 v0.8.5 及更高版本支持 SASL 配置。</p>
</blockquote>
<h3 id="Internal-Kafka" class="common-anchor-header">内部 Kafka<button data-href="#Internal-Kafka" class="anchor-icon" translate="no">
    ...
    </button></h3><p><code translate="no">inCluster</code> 表示当 Milvus 集群启动时，集群内会自动启动一个 Kafka 服务。</p>
<h4 id="Example" class="common-anchor-header">示例</h4><p>以下示例配置了一个内部 Kafka 服务。</p>
<pre><code translate="no" class="language-yaml"><span class="hljs-attr">apiVersion:</span> <span class="hljs-string">milvus.io/v1alpha1</span>
<span class="hljs-attr">kind:</span> <span class="hljs-string">Milvus</span>
<span class="hljs-attr">metadata:</span>
  <span class="hljs-attr">name:</span> <span class="hljs-string">my-release</span>
...
<span class="hljs-attr">spec:</span> 
  ...
  <span class="hljs-attr">config:</span> {}
<button class="copy-code-btn"></button></code></pre>
<p><a href="https://artifacthub.io/packages/helm/bitnami/kafka">点击此处</a>查看配置内部 Kafka 服务的完整配置项。根据需要在 `<code translate="no">kafka.inCluster.values</code>` 下添加配置项。</p>
<p>假设配置文件名为<code translate="no">milvuscluster.yaml</code> ，请运行以下命令以应用配置。</p>
...
<li><a href="/docs/zh/meta_storage_operator.md">使用 Milvus Operator 配置元数据存储</a></li>
</ul>
