...
summary: 使用 Helm 或 Milvus Operator，在 Pulsar 和 Woodpecker 之间切换 Milvus 集群的消息队列。
---
<h1 id="Switch-between-Pulsar-and-Woodpecker" class="common-anchor-header">在 Pulsar 和 Woodpecker 之间切换<button data-href="#Switch-between-Pulsar-and-Woodpecker" class="anchor-icon" translate="no">
    ...
    </button></h1><p>本页面介绍了如何在<strong>Milvus 集群中</strong>将消息队列（MQ）在<strong>Pulsar</strong>（内置或外部）与<strong>Woodpecker</strong>（MinIO 后端）之间双向切换。有关一般工作流和先决条件，请参阅<a href="/docs/zh/switch-mq-type.md">《切换消息队列》</a>。</p>
<div class="alert note">
<p><strong>先决条件：</strong>切换消息队列（Switch MQ）功能仅在<strong>Milvus 3.0 及更高版本中</strong>可用。开始操作前，请将您的 Milvus 实例升级至 Milvus 3.0 或更高版本——此功能在早期版本中不可用。</p>
</div>
<div class="alert warning">
<p>切换消息队列是一项<strong>高风险操作</strong>。请选择<strong>与您的</strong>部署方式相匹配的章节<strong>——使用 Helm</strong> <strong>或使用 Milvus Operator</strong>——并按顺序从头至尾操作。切勿混用 Helm 和 Operator 命令。</p>
</div>
...
<p><strong>步骤 4：（可选）停止 Pulsar 并清理。</strong>对于<strong>内置</strong>Pulsar，请禁用 Pulsar 并启用 Woodpecker，然后删除 Pulsar PVC：</p>
...
<p><strong>步骤 2：配置目标 Pulsar 连接并重启 Milvus。</strong>此次切换要求 Milvus 已知晓 Pulsar 连接信息，因此请通过<code translate="no">extraConfigFiles</code> 将配置写入<code translate="no">user.yaml</code> ，并使用<code translate="no">helm upgrade</code> 应用配置（该命令会滚动重启 Pod）。<code translate="no">streaming.enabled=true</code> 是 Switch MQ 功能的必备条件。</p>
...
<div class="alert note">
<p>确保目标 Pulsar 中不包含先前配置中的 Milvus 主题。如果这是您首次切换至 Pulsar，请跳过此说明；否则，请先清理同名的残留 Milvus 主题。</p>
</div>
<pre><code translate="no" class="language-shell">kubectl port-forward --address 0.0.0.0 service/my-release-milvus-mixcoord 29091:9091
<button class="copy-code-btn"></button></code></pre>
<p>在另一个终端中：</p>
...
<p><strong>步骤 2：执行 MQ 切换。</strong>由于 MixCoord 服务未对外暴露，请在 MixCoord pod 内部运行切换 API：</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>切换成功时会记录日志：<code translate="no">[mqTypeValue=woodpecker]</code> 。</p>
<p><strong>步骤 4：更新 Operator 中的 MQ 类型。</strong>更新<strong>Operator</strong>管理的配置，以防止 Operator 回滚此次切换。创建<code translate="no">change_configmap.yaml</code> ：</p>
...
<button class="copy-code-btn"></button></code></pre>
<p><strong>步骤 5：（可选）停止 Pulsar 并清理。</strong>对于<strong>内置</strong>Pulsar，请卸载 Pulsar 发布版本并删除其 PVC：</p>
...
<div class="alert note">
<p>如果您计划稍后切换回 Pulsar，请先清理数据/主题以避免冲突。由于 Helm 图表的限制，目前无法切换回<strong>内置</strong>Pulsar 实例。</p>
</div>
...
<p><strong>步骤 2：配置目标 Pulsar 连接并重启 Milvus。</strong>将 Pulsar 连接配置在<code translate="no">spec.config</code> 下（该 Operator 会将<code translate="no">spec.config</code> 渲染为<code translate="no">user.yaml</code> ），并设置 MQ 类型；应用 CR 后，Pod 将根据新配置自动重启。</p>
...
<p><strong>步骤 3：执行 MQ 切换。</strong></p>
<div class="alert note">
<p>确保目标 Pulsar 中不包含来自先前配置的 Milvus 主题。如果这是您首次切换至 Pulsar，请跳过此提示；否则，请先清理同名的残留 Milvus 主题。</p>
</div>
<pre><code translate="no" class="language-shell">kubectl exec -it &lt;mixcoord-pod&gt; -- \
  curl -X POST http://localhost:9091/management/wal/alter \
  -H &quot;Content-Type: application/json&quot; \
...
<thead>
<tr><th>源消息队列</th><th>目标 MQ</th><th>Helm</th><th>Milvus Operator</th></tr>
</thead>
<tbody>
...
<tr><td>Woodpecker（MinIO）</td><td>外部Pulsar</td><td><strong>已支持</strong></td><td><strong>已支持</strong></td></tr>
