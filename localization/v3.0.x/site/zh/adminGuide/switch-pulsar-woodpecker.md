...
summary: 使用 Helm 或 Milvus Operator，将 Milvus 集群的消息队列在 Pulsar 和 Woodpecker 之间进行切换。
---
<h1 id="Switch-between-Pulsar-and-Woodpecker" class="common-anchor-header">在 Pulsar 和 Woodpecker 之间切换<button data-href="#Switch-between-Pulsar-and-Woodpecker" class="anchor-icon" translate="no">
    ...
    </button></h1><p>本页面介绍了如何在<strong>Milvus 集群中</strong>将消息队列 (MQ) 在<strong>Pulsar</strong>（内置或外部）和<strong>Woodpecker</strong>（MinIO 后端）之间双向切换。有关一般工作流和先决条件，请参阅<a href="/docs/zh/switch-mq-type.md">《切换消息队列》</a>。</p>
...
<p>切换消息队列是一项<strong>高风险操作</strong>。请选择<strong>与您的</strong>部署方式相匹配的章节<strong>——使用 Helm</strong> <strong>或使用 Milvus Operator</strong>——并按照顺序从头到尾操作。请勿混合使用 Helm 和 Operator 命令。</p>
</div>
...
<p><strong>步骤 2：配置目标 Pulsar 连接并重启 Milvus。</strong>切换操作需要 Milvus 已知晓 Pulsar 连接信息，因此请通过<code translate="no">extraConfigFiles</code> 将配置写入<code translate="no">user.yaml</code> ，并使用<code translate="no">helm upgrade</code> 应用（该命令会滚动更新 Pod）。<code translate="no">streaming.enabled=true</code> 是启用 Switch MQ 功能所必需的。</p>
...
<p><strong>步骤 5：（可选）清理 Woodpecker 数据。</strong>删除 MinIO/S3 上的 Woodpecker 数据（位于<code translate="no">&lt;rootPath&gt;/wp/...</code> 目录下，通常为<code translate="no">files/wp/...</code> ）以及 etcd 中的 Woodpecker 元数据（<code translate="no">etcdctl get woodpecker --prefix</code> ）。如果您计划日后切回 Woodpecker，请先清理这些文件。</p>
<h2 id="With-Milvus-Operator" class="common-anchor-header">使用 Milvus Operator<button data-href="#With-Milvus-Operator" class="anchor-icon" translate="no">
    ...
    </button></h2><h3 id="Switch-from-Pulsar-to-Woodpecker-Milvus-Operator" class="common-anchor-header">从 Pulsar 切换至 Woodpecker（Milvus Operator）<button data-href="#Switch-from-Pulsar-to-Woodpecker-Milvus-Operator" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<p><strong>步骤 2：执行 MQ 切换。</strong>MixCoord 服务未对外暴露，因此需在 MixCoord pod 内部运行切换 API：</p>
...
<button class="copy-code-btn"></button></code></pre>
<p><strong>步骤 5：（可选）停止 Pulsar 并进行清理。</strong>对于<strong>内置的</strong>Pulsar，请卸载 Pulsar 发布版本并删除其 PVC：</p>
...
<p><strong>步骤 2：配置目标 Pulsar 连接并重启 Milvus。</strong>将 Pulsar 连接放置在<code translate="no">spec.config</code> 下（操作符会将<code translate="no">spec.config</code> 渲染为<code translate="no">user.yaml</code> ），并设置 MQ 类型；应用 CR 后，Pod 将根据新配置进行滚动更新。</p>
...
<p><strong>步骤 3：执行 MQ 切换。</strong></p>
<div class="alert note">
<p>确保目标 Pulsar 中不包含来自先前配置的 Milvus 主题。若这是您首次切换至 Pulsar，请跳过此说明；否则，请先清理同名的残留 Milvus 主题。</p>
</div>
<pre><code translate="no" class="language-shell">kubectl exec -it &lt;mixcoord-pod&gt; -- \
  curl -X POST http://localhost:9091/management/wal/alter \
  -H &quot;Content-Type: application/json&quot; \
...
<tr><td>Pulsar</td><td>Woodpecker（本地）</td><td><strong>支持但不推荐</strong>（所有 pod 需要共享文件系统）</td><td><strong>不支持</strong></td></tr>
</tbody>
</table>
