...
<h1 id="Switch-between-Pulsar-and-Woodpecker" class="common-anchor-header">在 Pulsar 和 Woodpecker 之间切换<button data-href="#Switch-between-Pulsar-and-Woodpecker" class="anchor-icon" translate="no">
    ...
    </button></h1><p>本页面介绍了如何在<strong>Milvus 集群中</strong>双向切换消息队列（MQ）的后端，可在<strong>Pulsar</strong>（内置或外部）与<strong>Woodpecker</strong>（MinIO 后端）之间进行切换。有关一般工作流和先决条件，请参阅<a href="/docs/zh/switch-mq-type.md">《切换消息队列》</a>。</p>
