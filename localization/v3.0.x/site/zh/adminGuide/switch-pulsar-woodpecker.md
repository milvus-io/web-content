...
<p><strong>先决条件：</strong>切换消息队列（Switch MQ）功能仅在<strong>Milvus 3.0 及更高版本中</strong>提供。开始操作前，请将您的 Milvus 实例升级至 Milvus 3.0 或更高版本——此功能在更早版本中不可用。</p>
</div>
<div class="alert warning">
<p>切换消息队列是一项<strong>高风险操作</strong>。请选择<strong>与您的</strong>部署方式相匹配的章节（<strong>“使用 Helm</strong>”或<strong>“使用 Milvus Operator</strong>”，请勿混合使用 Helm 和 Operator 命令）。</p>
</div>
<h2 id="With-Helm" class="common-anchor-header">使用 Helm<button data-href="#With-Helm" class="anchor-icon" translate="no">
    ...
    </button></h3><p><strong>步骤 1：验证 Milvus 实例是否正在运行。</strong>确保您的 Milvus 集群运行正常——例如，通过创建测试 Collection、插入数据并运行查询来验证。</p>
...
<p><strong>步骤 2：配置目标 Pulsar 连接并重启 Milvus。</strong>切换操作要求 Milvus 已知晓 Pulsar 连接信息，因此请通过<code translate="no">extraConfigFiles</code> 将配置写入<code translate="no">user.yaml</code> ，并使用<code translate="no">helm upgrade</code> 应用配置（该命令会滚动更新 Pod）。<code translate="no">streaming.enabled=true</code> 是 Switch MQ 功能的必备条件。</p>
...
<h3 id="Switch-from-Woodpecker-to-Pulsar-Milvus-Operator" class="common-anchor-header">从 Woodpecker 切换至 Pulsar（Milvus Operator）<button data-href="#Switch-from-Woodpecker-to-Pulsar-Milvus-Operator" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<p><strong>步骤 2：配置目标 Pulsar 连接并重启 Milvus。</strong>将 Pulsar 连接放置在<code translate="no">spec.config</code> 下（Operator 会将<code translate="no">spec.config</code> 渲染为<code translate="no">user.yaml</code> ），并设置 MQ 类型；应用 CR 后，Pod 会根据新配置自动重启。</p>
