...
title: 脉冲星
---
<h1 id="Use-Pulsar-as-the-Milvus-Message-Queue" class="common-anchor-header">将 Pulsar 用作 Milvus 消息队列<button data-href="#Use-Pulsar-as-the-Milvus-Message-Queue" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h1><p>Apache Pulsar 是 Milvus 支持的消息队列（WAL）后端之一。 在 Milvus 3.x 中<a href="/docs/zh/woodpecker.md">，Woodpecker</a>是默认的消息队列；但对于偏好 Pulsar 的用户，Pulsar 仍提供全面支持。Pulsar 主要用于 Milvus Distributed（集群）环境；而独立部署通常使用嵌入式的 Woodpecker 或<a href="/docs/zh/mq_rocksmq.md">RocksMQ</a>。</p>
...
<thead>
<tr><th>Milvus 版本</th><th>支持的 Pulsar 版本</th><th>默认</th></tr>
</thead>
<tbody>
<tr><td>2.5.x 及更高版本</td><td>Pulsar v3（推荐）或 Pulsar v2</td><td>Pulsar v3（通过 Helm / Milvus Operator）</td></tr>
<tr><td>2.4.x 及更早版本</td><td>Pulsar v2</td><td>Pulsar v2</td></tr>
</tbody>
</table>
<p>自 Milvus 2.5 起，Milvus Helm 图表和 Milvus Operator 默认部署<strong>Pulsar v3</strong>；Pulsar v2 仍保持兼容。请参阅《<a href="/docs/zh/upgrade-pulsar-v3.md">将 Pulsar 从 v2 升级至 v3</a>》和《<a href="/docs/zh/use-pulsar-v2.md">继续使用 Pulsar v2</a>》。</p>
<h2 id="Deploy-a-Milvus-cluster-with-Pulsar-using-Helm" class="common-anchor-header">使用 Helm 部署带 Pulsar 的 Milvus 集群<button data-href="#Deploy-a-Milvus-cluster-with-Pulsar-using-Helm" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><h3 id="Install" class="common-anchor-header">安装<button data-href="#Install" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<p>在 Kubernetes v1.25 及更高版本中，如果因捆绑的 Pulsar 子图导致 PodDisruptionBudget（PDB）API 问题，请禁用 Pulsar 的 PDB 策略：</p>
<pre><code translate="no" class="language-bash">helm install my-release zilliztech/milvus \
...
<button class="copy-code-btn"></button></code></pre>
<h3 id="Configure" class="common-anchor-header">配置<button data-href="#Configure" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<button class="copy-code-btn"></button></code></pre>
<h3 id="Uninstall" class="common-anchor-header">卸载<button data-href="#Uninstall" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<button class="copy-code-btn"></button></code></pre>
<h2 id="Deploy-a-Milvus-cluster-with-Pulsar-using-Milvus-Operator" class="common-anchor-header">使用 Milvus Operator 部署带 Pulsar 的 Milvus 集群<button data-href="#Deploy-a-Milvus-cluster-with-Pulsar-using-Milvus-Operator" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>使用 Milvus Operator 时，请在<code translate="no">spec.dependencies.pulsar</code> 下配置 Pulsar（仅支持 Milvus 集群）。<code translate="no">pulsar</code> 支持<code translate="no">external</code> 和<code translate="no">inCluster</code> 。</p>
...
<button class="copy-code-btn"></button></code></pre>
<h3 id="Internal-in-cluster-Pulsar" class="common-anchor-header">内部（集群内）Pulsar<button data-href="#Internal-in-cluster-Pulsar" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<span class="hljs-attr">spec:</span>
  ...
  <span class="hljs-attr">config:</span> {}
<button class="copy-code-btn"></button></code></pre>
<p>应用配置（假设文件名为<code translate="no">milvuscluster.yaml</code> ）：</p>
...
<button class="copy-code-btn"></button></code></pre>
<h2 id="Notes" class="common-anchor-header">注意事项<button data-href="#Notes" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<li><strong>从 2.5.x 升级至 2.6.x：</strong> <strong>消息队列限制</strong>：升级至 Milvus v3.0.1 时，必须保留当前的消息队列选择。升级过程中不支持在不同的消息队列系统之间切换。未来版本将支持更改消息队列系统。
如果您正在运行 Pulsar 且希望继续使用，请在升级过程中不要更改消息队列。</li>
<li><strong>Pulsar v2 → v3：</strong>请参阅<a href="/docs/zh/upgrade-pulsar-v3.md">《将 Pulsar 从 v2 升级到 v3</a>》；若要继续<a href="/docs/zh/use-pulsar-v2.md">使用</a> v2，请参阅《<a href="/docs/zh/use-pulsar-v2.md">继续使用 Pulsar v2</a>》。</li>
</ul>
