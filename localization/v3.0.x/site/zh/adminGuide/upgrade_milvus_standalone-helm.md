...
<h1 id="Upgrade-Milvus-Standalone-with-Helm-Chart" class="common-anchor-header">使用 Helm 图表升级 Milvus Standalone<button data-href="#Upgrade-Milvus-Standalone-with-Helm-Chart" class="anchor-icon" translate="no">
    ...
    </button></h1><p>本指南介绍了如何使用 Helm 将 Milvus 2.6.x Standalone 部署升级至 v3.0.1。</p>
<div class="alert note">
<p>本流程已通过 Milvus 2.6.20 至 Milvus v3.0.1 版本的验证，所用 Milvus Helm 图表版本为 5.0.22。如果您使用的是其他 Milvus 2.6.x 补丁版本或 Helm 图表版本，请先在非生产环境中验证升级过程。</p>
</div>
...
<p><strong>消息队列限制</strong>：升级至 Milvus v3.0.1 时，必须保留当前选择的消息队列。升级过程中不支持在不同的消息队列系统之间切换。未来版本将支持更改消息队列系统。</p>
...
<p>本操作流程未验证通过将 Milvus 镜像降级回 2.6.x 版本来实现降级或回滚的情况。 在 v3.0.1 写入数据后，仅回滚镜像的操作可能无法读取更新后的状态。如果升级失败，请停止写入操作，并使用恢复计划来还原升级前的元数据和持久化数据备份。请先在非生产环境中验证该恢复计划。</p>
</div>
<h2 id="Upgrade-process" class="common-anchor-header">升级流程<button data-href="#Upgrade-process" class="anchor-icon" translate="no">
    ...
    </button></h2><h3 id="Step-1-Update-the-Helm-repository" class="common-anchor-header">步骤 1：更新 Helm 存储库<button data-href="#Step-1-Update-the-Helm-repository" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<div class="alert note">
位于<code translate="no">https://milvus-io.github.io/milvus-helm/</code> 的 Milvus Helm Charts 仓库已归档。对于 4.0.31 及更高版本的图表，请使用新仓库<code translate="no">https://zilliztech.github.io/milvus-helm/</code> 。
</div>
<h3 id="Step-2-Upgrade-Milvus" class="common-anchor-header">步骤 2：升级 Milvus<button data-href="#Step-2-Upgrade-Milvus" class="anchor-icon" translate="no">
    ...
    </button></h3><p>检查您的 Helm 发布中已安装的图表版本：</p>
...
<button class="copy-code-btn"></button></code></pre>
<p><code translate="no">--reset-then-reuse-values</code> 选项在应用显式镜像覆盖以替代所选Chart默认设置的同时，会保留上一版本的值。</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>请确认所有必需的工作负载均已就绪，Milvus 正在使用<code translate="no">v3.0.1</code> ，且现有 Collections 仍可进行查询和搜索。在启用任何 v3.0.1 专属功能之前，请完成这些检查。</p>
<div class="alert note">
<p>升级到 Milvus 3.0 不会启用 Storage V3。在验证升级成功后，请先了解<a href="/docs/zh/storage-v3.md">Storage V3，</a>然后再启用依赖于它的功能。一旦 Milvus 写入了 Storage V3 数据，将系统降级到无法读取 Storage V3 的旧版 Milvus 是不受支持的。</p>
</div>
