...
title: 切换消息队列
summary: 在不造成停机的情况下，将现有的 Milvus 部署在 Woodpecker 与另一个消息队列之间进行切换。
---
<h1 id="Switch-Message-Queue" class="common-anchor-header">切换消息队列<button data-href="#Switch-Message-Queue" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
