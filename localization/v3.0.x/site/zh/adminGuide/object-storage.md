...
<h1 id="Object-Storage" class="common-anchor-header">对象存储<button data-href="#Object-Storage" class="anchor-icon" translate="no">
    ...
    </button></h1><p>Milvus 将索引文件和二进制日志（即其数据的主体）存储在对象存储中。Milvus 支持 MinIO 以及多种 S3 兼容型和云对象存储。</p>
...
<h2 id="Additional-requirements-when-using-embedded-Woodpecker" class="common-anchor-header">使用嵌入式 Woodpecker 时的额外要求<button data-href="#Additional-requirements-when-using-embedded-Woodpecker" class="anchor-icon" translate="no">
    ...
    </button></h2><p>当您运行默认的<strong>Woodpecker</strong>消息队列及其对象存储后端（<code translate="no">storage.type=minio</code> ）时，Woodpecker 会将其预写日志写入同一对象存储，并要求<strong>严格遵守 S3 条件写入语义</strong>。 并非所有对象存储都符合要求——例如，尽管华为云 OBS 可作为常规的 Milvus 对象存储使用，但目前<strong>尚不支持</strong>将其用作 Woodpecker 的后端。</p>
<p>请参阅<a href="/docs/zh/woodpecker.md">Woodpecker</a>页面上的对象存储兼容性对照表，了解各提供商的具体要求。</p>
