...
<h2 id="Additional-requirements-when-using-embedded-Woodpecker" class="common-anchor-header">使用嵌入式 Woodpecker 时的额外要求<button data-href="#Additional-requirements-when-using-embedded-Woodpecker" class="anchor-icon" translate="no">
    ...
    </button></h2><p>当您运行默认的<strong>Woodpecker</strong>消息队列及其对象存储后端（<code translate="no">storage.type=minio</code> ）时，Woodpecker 会将其预写日志写入同一对象存储，并要求<strong>严格遵循 S3 条件写入语义</strong>。 并非所有对象存储都符合要求——例如，尽管华为云 OBS 可作为常规的 Milvus 对象存储使用，但目前<strong>尚不支持</strong>将其用作 Woodpecker 的后端。</p>
