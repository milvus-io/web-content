...
<p>通过 ServiceMonitor 定制资源定义（CRD），您可以声明式地定义如何监控一组动态服务。它还允许使用标签选择以所需配置监控哪些服务。使用 Prometheus 操作符，您可以引入约定，指定如何暴露度量。新服务可以按照您设置的约定自动发现，而无需手动重新配置。</p>
...
<h4 id="With-Milvus-Operator" class="common-anchor-header">使用 Milvus 操作符</h4><p>如果使用 Milvus 操作符安装了 Milvus，则可以如下方式启用 ServiceMonitor。</p>
<ol>
...
<li><p>将<code translate="no">spec.components.disableMetric</code> 字段编辑为<code translate="no">false</code> 。</p>
...
<h3 id="3-Check-the-metrics" class="common-anchor-header">3.检查指标<button data-href="#3-Check-the-metrics" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>启用 ServiceMonitor 后，您可以访问 Prometheus 仪表板<code translate="no">http://localhost:9090/</code> 。</p>
...
<h3 id="4-Check-the-ServiceMonitor" class="common-anchor-header">4.检查 ServiceMonitor<button data-href="#4-Check-the-ServiceMonitor" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
