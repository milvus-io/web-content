...
<h3 id="1-Installing-the-Jaeger-Operator-on-Kubernetes" class="common-anchor-header">1.在 Kubernetes 上安装 Jaeger 操作符<button data-href="#1-Installing-the-Jaeger-Operator-on-Kubernetes" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<h3 id="2-Deploy-Jaeger" class="common-anchor-header">2.部署 Jaeger<button data-href="#2-Deploy-Jaeger" class="anchor-icon" translate="no">
    ...
    </button></h3><p>创建 Jaeger 实例的最简单方法是创建类似下面示例的 YAML 文件。这将安装默认的 AllInOne 策略，在单个 pod 中部署<strong>一体化</strong>镜像（结合了<strong>jaeger-agents</strong>、<strong>jaeger-collector</strong>、<strong>jaeger</strong> <strong>-query</strong> 和 Jaeger UI），默认使用<strong>内存存储</strong>。</p>
...
<p>要将上述设置应用于新的 Milvus 部署，可以运行以下命令：</p>
