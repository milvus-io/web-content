...
title: 卡夫卡
---
<h1 id="Use-Kafka-as-the-Milvus-Message-Queue" class="common-anchor-header">将 Kafka 用作 Milvus 消息队列<button data-href="#Use-Kafka-as-the-Milvus-Message-Queue" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h1><p>Apache Kafka 是 Milvus 支持的消息队列（WAL）后端之一。 在 Milvus 3.x 中<a href="/docs/zh/woodpecker.md">，Woodpecker</a>是默认的消息队列；但对于更倾向于使用 Kafka 的用户，Kafka 仍提供全面支持。Kafka 主要与 Milvus Distributed（集群）配合使用；独立部署通常使用嵌入式的 Woodpecker 或<a href="/docs/zh/mq_rocksmq.md">RocksMQ</a>。</p>
...
<li>Milvus 仅支持<strong>Kafka 2.x 和 3.x</strong>版本。</li>
<li>可通过 Helm 或 Milvus Operator 为 Milvus Distributed（集群）配置 Kafka。</li>
</ul>
<h2 id="Deploy-a-Milvus-cluster-with-Kafka-using-Helm" class="common-anchor-header">使用 Helm 部署基于 Kafka 的 Milvus 集群<button data-href="#Deploy-a-Milvus-cluster-with-Kafka-using-Helm" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><h3 id="Install-and-configure" class="common-anchor-header">安装与配置<button data-href="#Install-and-configure" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>若要使用外部 Kafka 服务，请禁用内置的 Pulsar，并在 `<code translate="no">values.yaml</code> ` 覆盖配置中启用 `<code translate="no">externalKafka</code> `，然后使用该配置安装 Milvus：</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>有关 SASL/SSL 身份验证的详细信息，请参阅《<a href="/docs/zh/connect_kafka_ssl.md">使用 SASL/SSL 连接到 Kafka</a>》。</p>
...
<button class="copy-code-btn"></button></code></pre>
<h2 id="Deploy-a-Milvus-cluster-with-Kafka-using-Milvus-Operator" class="common-anchor-header">使用 Milvus Operator 部署带 Kafka 的 Milvus 集群<button data-href="#Deploy-a-Milvus-cluster-with-Kafka-using-Milvus-Operator" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>使用 Milvus Operator 时，请在<code translate="no">spec.dependencies.kafka</code> （仅限集群）下设置<code translate="no">spec.dependencies.msgStreamType: &quot;kafka&quot;</code> 并配置 Kafka。<code translate="no">kafka</code> 支持<code translate="no">external</code> 和<code translate="no">inCluster</code> 。</p>
...
<p>Milvus Operator v0.8.5 及更高版本支持 SASL 配置。</p>
</div>
<h3 id="Internal-in-cluster-Kafka" class="common-anchor-header">内部（集群内）Kafka<button data-href="#Internal-in-cluster-Kafka" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
