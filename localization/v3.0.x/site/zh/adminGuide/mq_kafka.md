...
<h1 id="Use-Kafka-as-the-Milvus-Message-Queue" class="common-anchor-header">将 Kafka 用作 Milvus 消息队列<button data-href="#Use-Kafka-as-the-Milvus-Message-Queue" class="anchor-icon" translate="no">
    ...
    </button></h1><p>Apache Kafka 是 Milvus 支持的消息队列（WAL）后端之一。 在 Milvus 3.x 中<a href="/docs/zh/woodpecker.md">，Woodpecker</a>是默认的消息队列；但对于偏好使用 Kafka 的用户，Kafka 仍提供全面支持。Kafka 主要与 Milvus Distributed（集群）配合使用；独立部署通常使用嵌入式的 Woodpecker 或<a href="/docs/zh/mq_rocksmq.md">RocksMQ</a>。</p>
