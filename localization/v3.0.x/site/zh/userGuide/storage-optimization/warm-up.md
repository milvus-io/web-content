...
<h1 id="Warm-Up" class="common-anchor-header">预热<span class="beta-tag" style="background-color:rgb(0, 179, 255);color:white" translate="no">Compatible with Milvus 2.6.4+</span><button data-href="#Warm-Up" class="anchor-icon" translate="no">
    ...
    </button></h1><p><strong>预热</strong>是分层存储的补充，可在段可查询前将选定字段或索引预加载到缓存中。您可以在集群、 Collections 或单个字段/索引级别配置预热，从而实现对首次查询延迟和资源使用的精细控制。</p>
...
<h3 id="Set-warmup-when-creating-a-collection" class="common-anchor-header">创建 Collection 时设置预热<button data-href="#Set-warmup-when-creating-a-collection" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<h2 id="Configure-warmup-at-field-level--Milvus-2611+" class="common-anchor-header">配置字段级预热<span class="beta-tag" style="background-color:rgb(0, 179, 255);color:white" translate="no">Compatible with Milvus 2.6.11+</span><button data-href="#Configure-warmup-at-field-level--Milvus-2611+" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
