...
summary: >-
  外部 Collection 是 Milvus 中的 Collection 类型之一，它可直接访问来自外部存储系统或数据库表（如 AWS S3 和
  Iceberg）的数据，而无需将数据复制到 Milvus 中。它作为数据湖上的查询层，同时保持与 Milvus 查询接口的兼容性。
beta: Milvus 3.0.x
---
...
<p>以下是一个示例权限策略，可附加到 IAM 角色上并包含允许的权限。您可以根据需要进行调整。</p>
<pre><code translate="no" class="language-json"><span class="hljs-punctuation">{</span>
...
<li><p>如果源数据有更新，您需要手动再次调用刷新操作，以保持 Milvus 的数据最新。</p></li>
...
<h2 id="Next-steps" class="common-anchor-header">后续步骤<button data-href="#Next-steps" class="anchor-icon" translate="no">
    ...
    </button></h2><p>刷新外部Collection后，您可以加载和释放该Collection，并在外部Collection中执行相似度搜索和查询，就像在任何托管Collection中一样，但需注意：用于按需计算的数据库中的Collection必须附加到按需集群上才能进行搜索和查询。</p>
