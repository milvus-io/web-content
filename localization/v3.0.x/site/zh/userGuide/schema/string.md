...
id: string.md
title: VarChar 字段
summary: VARCHAR 是 Milvus 中用于存储短且长度受限的字符串元数据的标量字段类型。
---
<h1 id="VarChar-Field" class="common-anchor-header">VarChar 字段<button data-href="#VarChar-Field" class="anchor-icon" translate="no">
    ...
    </button></h1><p>Milvus 通过 `<code translate="no">VARCHAR</code> ` 和 `<code translate="no">TEXT</code> ` 字段支持文本标量数据。本页介绍 `<code translate="no">VARCHAR</code>`，该字段专为名称、标签、类别和外部 ID 等短且长度受限的字符串元数据而设计。</p>
