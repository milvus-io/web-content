...
summary: >-
  ...
  字段形式实现，它会自动存储数据中未在 Schema 中显式定义的任何字段。
---
...
<p>例如，如果您的 Collection Schema 仅定义了 `<code translate="no">id</code> ` 和 `<code translate="no">vector</code>`，而您插入以下实体：</p>
...
<h3 id="Apply-indexes-to-the-collection" class="common-anchor-header">将索引应用于Collection<button data-href="#Apply-indexes-to-the-collection" class="anchor-icon" translate="no">
    ...
    </button></h3><p>定义索引参数后，您可以使用 `<code translate="no">create_index()</code>` 将其应用到 Collection：</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#javascript">   NodeJS</a>
...
<p>这有几个重要影响：</p>
<ul>
<li><p><strong>不会回退到全表扫描</strong>：如果大多数实体已成功建立索引，过滤查询将完全依赖该索引。发生类型转换失败的实体将被排除在结果集之外——即使它们在逻辑上符合过滤条件。</p></li>
<li><p><strong>搜索准确性风险</strong>：在数据质量不一致的大型数据集中（尤其是Dynamic Field字段键的情况下），此行为可能会导致意外的搜索结果缺失。在建立索引之前，确保数据格式的一致性和有效性至关重要。</p></li>
...
<h3 id="What-happens-if-my-query-uses-a-different-data-type-than-the-indexed-cast-type" class="common-anchor-header">如果我的查询使用的数据类型与索引中的转换类型不同，会发生什么情况？<button data-href="#What-happens-if-my-query-uses-a-different-data-type-than-the-indexed-cast-type" class="anchor-icon" translate="no">
    ...
    </button></h3><p>如果您的查询使用与索引中<strong>不同的数据类型</strong>来比较Dynamic Field键（例如，当索引被强制转换为<code translate="no">double</code> 时，却使用字符串比较进行查询），系统将<strong>不会使用该索引</strong>，并且<em>仅在可能的情况下</em>才会回退到全表扫描。 为获得最佳性能和准确性，请确保您的查询类型与创建索引时使用的<code translate="no">json_cast_type</code> 相匹配。</p>
