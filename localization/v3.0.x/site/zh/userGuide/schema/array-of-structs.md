...
<p>此示例中的两个向量字段分别从两种搜索视角表示同一个片段。<code translate="no">chunks[emb_list_vector]</code> 旨在支持采用<code translate="no">MAX_SIM*</code> 指标的 EmbeddingList 搜索，而<code translate="no">chunks[emb]</code> 则用于采用常规向量指标（如<code translate="no">COSINE</code> 、<code translate="no">IP</code> 或<code translate="no">L2</code> ）的元素级搜索。</p>
</div>
...
<tr><td>元素级检索</td><td>返回最相关的片段、剪辑、补丁或观测结果，并包含其数组偏移量。</td><td><code translate="no">chunks[emb]</code></td></tr>
...
<tr><td>搜索单个片段、剪辑或补丁。</td><td>使用常规向量度量进行元素级搜索。</td><td>结构元素级别，如有偏移量则包含偏移量。</td><td><a href="/docs/zh/basic-vector-search-with-structarray.md">使用 StructArray 进行基本向量搜索</a></td></tr>
...
<h2 id="Understand-the-two-search-models" class="common-anchor-header">了解两种搜索模型<button data-href="#Understand-the-two-search-models" class="anchor-icon" translate="no">
  ...
  <tbody>
    <tr>
      <td>
        <p>EmbeddingList搜索将StructArray向量子字段中的向量视为父实体的单个嵌入列表。查询同样是一个嵌入列表。Milvus使用<code translate="no">MAX_SIM*</code> 度量将查询嵌入列表与存储的嵌入列表进行比较，并返回匹配的实体。</p>
        <ul>
          <li>查询数据：嵌入列表。</li>
...
<tr><td>建模</td><td><a href="/docs/zh/insert-data-into-structarray-fields.md">向 StructArray 字段插入数据</a></td><td>准备并插入嵌套的 StructArray 数据。</td></tr>
...
<h2 id="Key-limits-to-check-first" class="common-anchor-header">首先需检查的键限制<button data-href="#Key-limits-to-check-first" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<li><p><code translate="no">element_filter</code> 以及<code translate="no">MATCH_*</code> 适用于 StructArray 字段内的标量子字段。请仅在这些操作符内部使用<code translate="no">$[subfield]</code> 。</p></li>
...
<li><p>若要过滤 StructArray 的标量子字段，请参阅《<a href="/docs/zh/struct-array-operators.md">StructArray 操作符</a>》和《<a href="/docs/zh/filtered-search-with-structarray.md">使用 StructArray 进行过滤搜索</a>》。</p></li>
</ol>
