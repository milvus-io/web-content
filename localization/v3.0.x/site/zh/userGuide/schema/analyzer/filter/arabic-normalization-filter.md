...
<h1 id="Arabic-Normalization" class="common-anchor-header">阿拉伯语规范化<span class="beta-tag" style="background-color:rgb(0, 179, 255);color:white" translate="no">Compatible with Milvus 3.0.0+</span><button data-href="#Arabic-Normalization" class="anchor-icon" translate="no">
    ...
    </button></h1><p><code translate="no">arabic_normalization</code> 过滤器是一个用于阿拉伯语文本的内置标记过滤器。它可规范化阿拉伯语特有的字母变体，并移除那些可能导致等效阿拉伯语术语在文本分析过程中显示不一致的可选标记。</p>
...
<p>要在自定义分析器中使用<code translate="no">arabic_normalization</code> 过滤器，请将其添加到<code translate="no">analyzer_params</code> 文件中的<code translate="no">filter</code> 部分：</p>
