...
title: 稠密向量
summary: >-
  稠密向量是机器学习和数据分析中广泛使用的数值数据表示形式。它们由实数数组组成，其中大部分或所有元素均不为零。与稀疏向量相比，稠密向量在相同维度下包含更多信息，因为每个维度都包含有意义的值。
  这种表示方式能够有效捕捉复杂的模式和关系，使数据在高维空间中更易于分析和处理。稠密向量通常具有固定的维度数，从几十到几百甚至几千不等，具体取决于具体的应用和要求。
---
<h1 id="Dense-Vector" class="common-anchor-header">稠密向量<button data-href="#Dense-Vector" class="anchor-icon" translate="no">
    ...
    </button></h1><p>稠密向量是机器学习和数据分析中广泛使用的数值数据表示形式。它们由实数数组组成，其中大部分或所有元素均不为零。与稀疏向量相比，在相同的维度下，稠密向量包含更多信息，因为每个维度都包含有意义的值。 这种表示方式能够有效捕捉复杂的模式和关系，使数据在高维空间中更易于分析和处理。稠密向量通常具有固定的维度数，范围从几十到几百甚至几千不等，具体取决于应用场景和需求。</p>
...
<h2 id="Overview" class="common-anchor-header">概述<button data-href="#Overview" class="anchor-icon" translate="no">
    ...
    </button></h2><p>稠密向量通常表示为固定长度的浮点数数组，例如<code translate="no">[0.2, 0.7, 0.1, 0.8, 0.3, ..., 0.5]</code> 。这些向量的维度通常在数百到数千之间，例如 128、256、768 或 1024。 每个维度都捕捉了对象的特定语义特征，从而通过相似度计算使其适用于各种场景。</p>
...
<p>上图展示了稠密向量在二维空间中的表示形式。尽管现实应用中的稠密向量维度通常要高得多，但这幅二维图示有效地传达了几个关键概念：</p>
<ul>
<li><p><strong>多维表示：</strong>每个点代表一个概念对象（如<strong>Milvus</strong>、<strong>向量数据库</strong>、<strong>检索系统等</strong>），其位置由各维度的数值决定。</p></li>
<li><p><strong>语义关系：</strong>点与点之间的距离反映了概念之间的语义相似性。距离越近，表明概念在语义上关联越紧密。</p></li>
<li><p><strong>聚类效应：</strong>相关概念（如<strong>Milvus</strong>、<strong>向量数据库和</strong> <strong>检索系统</strong>）在空间中彼此邻近，形成语义聚类。</p></li>
</ul>
...
<p>除了稠密向量外，Milvus 还支持稀疏向量和二进制向量。稀疏向量适用于基于特定术语的精确匹配，例如关键词搜索和术语匹配；而二进制向量通常用于高效处理二值化数据，例如图像模式匹配和某些哈希应用。有关更多信息，请参阅<a href="/docs/zh/binary-vector.md">“二进制向量</a>”和<a href="/docs/zh/sparse_vector.md">“稀疏向量”</a>。</p>
</div>
<h2 id="Use-dense-vectors" class="common-anchor-header">使用稠密向量<button data-href="#Use-dense-vectors" class="anchor-icon" translate="no">
    ...
    </button></h3><p>要在 Milvus 中使用稠密向量，首先需要在创建 Collection 时定义一个用于存储稠密向量的向量字段。该过程包括：</p>
<ol>
<li><p>将 `<code translate="no">datatype</code> ` 设置为受支持的密集向量数据类型。有关受支持的密集向量数据类型，请参阅“数据类型”。</p></li>
<li><p>使用 `<code translate="no">dim</code> ` 参数指定稠密向量的维度。</p></li>
</ol>
...
<table>
   ...
   <tr>
     ...
     <td><p>存储各维度元素均为 8 位整数（int8）的向量，每个元素的取值范围为 –128 到 127。 INT8_VECTOR 专为量化深度学习模型（例如 ResNet、EfficientNet）设计，可在精度损失极小的情况下缩小模型体积并加快推理速度。<br><strong>注意</strong>：此向量类型仅支持 HNSW 索引。</p></td>
   </tr>
</table>
<h3 id="Set-index-params-for-vector-field" class="common-anchor-header">设置向量字段的索引参数<button data-href="#Set-index-params-for-vector-field" class="anchor-icon" translate="no">
    ...
    </button></h3><p>为加速语义搜索，必须为向量字段创建索引。建立索引可显著提高大规模向量数据的检索效率。</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#javascript">   NodeJS</a>
...
<p>Milvus 提供了多种索引类型，以提供更好的向量搜索体验。AUTOINDEX 是一种专门设计的特殊索引类型，旨在降低向量搜索的学习门槛。您还可以从众多索引类型中进行选择。有关详细信息，请参阅<a href="/docs/zh/index-explained.md">《索引详解》</a>。</p>
<p>Milvus 还支持其他度量类型。更多信息请参阅《<a href="/docs/zh/metric.md">度量类型</a>》。</p>
<h3 id="Create-collection" class="common-anchor-header">创建Collection<button data-href="#Create-collection" class="anchor-icon" translate="no">
    ...
    </button></h3><p>完成稠密向量和索引参数的配置后，您可以创建一个包含稠密向量的Collection。下面的示例使用<code translate="no">create_collection</code> 方法创建了一个名为<code translate="no">my_collection</code> 的Collection。</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#javascript">   NodeJS</a>
...
<h3 id="Perform-similarity-search" class="common-anchor-header">执行相似度搜索<button data-href="#Perform-similarity-search" class="anchor-icon" translate="no">
    ...
    </button></h3><p>基于密集向量的语义搜索是 Milvus 的核心功能之一，它允许您根据向量之间的距离，快速找到与查询向量最相似的数据。要执行相似度搜索，请准备好查询向量和搜索参数，然后调用<code translate="no">search</code> 方法。</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#javascript">   NodeJS</a>
