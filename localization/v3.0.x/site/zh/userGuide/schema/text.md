...
<p>Milvus 提供了<code translate="no">TEXT</code> 标量字段类型，用于将长源文本直接与实体关联存储。典型值包括段落、长文档、文章正文、工单和日志。与<code translate="no">VARCHAR</code> 不同，后者要求设置固定的<code translate="no">max_length</code> ，而<code translate="no">TEXT</code> 则无需在 Schema 中设置最大字节长度。</p>
