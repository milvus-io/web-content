...
<tr><td>可空字段</td><td>可为空的 StructArray 字段受版本限制。当支持此功能时，空值适用于整个 StructArray 字段，而非独立作用于单个 Struct 元素。</td></tr>
...
<p>在规划集合 Schema 时，StructArray 向量字段应计入向量字段的总数。请确保向量字段和向量子字段的总数在目标版本和服务层级的限制范围内。</p>
...
<tr><td>范围搜索</td><td>根据搜索模式以及目标版本对索引/度量的支持情况而定。若需了解针对元素级 StructArray 请求的混合搜索范围行为，请查阅目标版本文档。</td></tr>
