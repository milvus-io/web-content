...
<li><p>外部Collection目前支持在创建后添加字段。其他模式更改（例如删除字段、重命名字段、更改字段数据类型、更改向量维度或重新映射<code translate="no">external_field</code> ）均不支持。</p></li>
...
<h3 id="Add-a-vector-field" class="common-anchor-header">添加向量字段<button data-href="#Add-a-vector-field" class="anchor-icon" translate="no">
    ...
    </button></h3><p>如果外部数据源中已包含向量值，您还可以添加向量字段。请将向量<code translate="no">data_type</code> 和<code translate="no">dim</code> 设置为与外部数据源中的向量字段相对应。</p>
