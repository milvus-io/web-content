...
<p>对于已启用<code translate="no">nullable</code> 的字段，如果无需进行任何更新，则可以在<code translate="no">upsert</code> 请求中省略这些字段。</p>
...
<p>设置 `<code translate="no">partial_update=True</code> `，并提供主键以及要更新的字段。</p>
<p>Milvus 将通过强一致性查询检索现有实体，将您的更改与存储的数据进行合并，插入合并后的实体，并删除旧实体。</p>
<p>如果主键不存在，结果取决于<code translate="no">autoID</code> 是否已启用：</p>
<ul>
<li><strong>当<code translate="no">autoID</code> 禁用时，</strong>Milvus会尝试使用您提供的主键插入新实体。若满足常规插入要求，请求即成功。若缺少必填字段，请求将因字段缺失而失败。可为空字段和具有默认值的字段可以省略，这与常规插入操作相同。</li>
<li><strong>启用<code translate="no">autoID</code> 时，</strong>请求中的每个主键都必须已存在。如果任何主键缺失，即使您提供了插入所需的所有字段，Milvus也会拒绝该请求。对于现有实体，合并模式会保持主键不变。</li>
</ul>
<p>如果部分更新因“字段缺失”错误而失败，请检查目标实体是否存在。如果没有现有的实体，Milvus 无法检索您省略的字段的值。</p>
<p>对于新实体，请使用<code translate="no">insert</code> 或覆盖模式下的upsert操作。后续对单个字段的更新请使用合并模式。</p>
<p>对于<code translate="no">ARRAY</code> 字段，在Milvus v2.6.17及更高版本中，合并模式支持两种操作符：<code translate="no">ARRAY_APPEND</code> 和<code translate="no">ARRAY_REMOVE</code> 。这些操作符允许您向现有的<code translate="no">ARRAY</code> 字段追加元素或移除匹配的元素，而无需先查询该实体以获取其当前值。有关详细信息，请参阅<a href="/docs/zh/upsert-entities.md#Upsert-ARRAY-fields-in-merge-mode">“在合并模式下对ARRAY字段执行Upsert”</a>。</p>
...
<p>假设<code translate="no">issue</code> 字段可以为空。当您对这些字段执行更新或插入操作时，请注意：</p>
<ul>
...
<p>假设示例 Collection 有一个名为<code translate="no">extras</code> 的 Schema 定义的 JSON 字段，且实体中该 JSON 字段中的键值对类似于<code translate="no">{&quot;author&quot;: &quot;John&quot;, &quot;year&quot;: 2020, &quot;tags&quot;: [&quot;fiction&quot;]}</code> 。</p>
...
<li><p><code translate="no">ARRAY_REMOVE</code> 从现有数组中移除所有与请求有效载荷中某个值匹配的元素。</p></li>
</ul>
...
<p>有关详细信息，请参阅<a href="/docs/zh/upsert-entities.md#Upsert-StructArray-field-in-merge-mode">“在合并模式下</a>对<a href="/docs/zh/upsert-entities.md#Upsert-StructArray-field-in-merge-mode">StructArray 字段进行 Upsert”</a>。</p></li>
</ul>
...
<li><p>请求中指定的所有字段都必须存在于目标Collection的Schema中。</p></li>
<li><p>请求中指定的所有字段的值必须与Schema中定义的数据类型相匹配。</p></li>
<li><p>对于通过函数从其他字段派生的字段，Milvus 将在 upsert 操作期间移除该派生字段，以便重新计算。</p></li>
</ul>
<h2 id="Upsert-entities-in-a-collection" class="common-anchor-header">在 Collection 中对实体进行 upsert 操作<button data-href="#Upsert-entities-in-a-collection" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<span class="hljs-comment"># }</span>
<button class="copy-code-btn"></button></code></pre>
<h2 id="Upsert-entities-in-a-partition" class="common-anchor-header">在分区中执行“Upsert”操作<button data-href="#Upsert-entities-in-a-partition" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>您还可以将实体更新或插入到指定的分区中。以下代码片段假设您的 Collection 中有一个名为<strong>PartitionA</strong>的分区。</p>
...
<span class="hljs-comment"># }</span>
<button class="copy-code-btn"></button></code></pre>
<h2 id="Upsert-entities-in-merge-mode--Milvus-v262+" class="common-anchor-header">在合并模式下对实体进行更新或插入<span class="beta-tag" style="background-color:rgb(0, 179, 255);color:white" translate="no">Compatible with Milvus v2.6.2+</span><button data-href="#Upsert-entities-in-merge-mode--Milvus-v262+" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>以下示例仅更新<code translate="no">my_collection</code> 中主键分别为<code translate="no">1</code> 和<code translate="no">2</code> 的实体的<code translate="no">issue</code> 字段。运行前，请确保这两个实体均已存在。它们的其他字段将保留当前值。</p>
<div class="alert note">
<p>在合并模式下执行 upsert 操作时，请确保请求中涉及的实体具有相同的一组字段。假设需要进行 upsert 操作的实体有两个或更多（如下面的代码片段所示），为了防止错误并保持数据完整性，这些实体必须包含完全相同的字段。</p>
</div>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
...
<h2 id="Upsert-ARRAY-fields-in-merge-mode--Milvus-2617+" class="common-anchor-header">在合并模式下对 ARRAY 字段进行 upsert<span class="beta-tag" style="background-color:rgb(0, 179, 255);color:white" translate="no">Compatible with Milvus 2.6.17+</span><button data-href="#Upsert-ARRAY-fields-in-merge-mode--Milvus-2617+" class="anchor-icon" translate="no">
    ...
    </button></h2><p>在 Milvus v2.6.17 之前，更新<code translate="no">ARRAY</code> 字段的一部分需要客户端执行“读取-修改-写入”流程：查询现有数组，在应用程序代码中进行修改，然后对完整的替换值执行 upsert 操作。 部分更新操作符（<code translate="no">ARRAY_APPEND</code> 和<code translate="no">ARRAY_REMOVE</code> ）允许您仅发送需要追加或删除的元素，从而减少了客户端逻辑，并避免了在upsert操作前进行额外的读取。</p>
<p>假设主键为<code translate="no">1</code> 的实体已包含<code translate="no">tags = [&quot;new&quot;, &quot;trial&quot;]</code> 。在引入部分更新操作符之前，向数组中添加元素<code translate="no">&quot;premium&quot;</code> 需要对整个替换数组执行 upsert 操作：</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#javascript">   NodeJS</a>
...
<li><p>对同一实体的并发更新插入操作在不同请求之间并非原子性的。如果两个请求同时更新同一个<code translate="no">ARRAY</code> 字段，则后写入的操作可能会覆盖先写入的操作。若需保留所有并发更改，请使用应用程序级别的协调机制。</p></li>
</ul>
<h3 id="Example" class="common-anchor-header">示例<button data-href="#Example" class="anchor-icon" translate="no">
    ...
    </button></h3><p>以下示例使用了一个小型<code translate="no">users</code> 集合，其主键为<code translate="no">pk</code> ，包含一个类型为<code translate="no">ARRAY&lt;VARCHAR&gt;</code> 的<code translate="no">tags</code> 字段，以及一个<code translate="no">embedding</code> 向量字段。该示例首先插入两个初始<code translate="no">tags</code> 值为 的实体，然后使用<code translate="no">ARRAY_APPEND</code> 和<code translate="no">ARRAY_REMOVE</code> 演示每个操作符如何更改存储的数组。</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#javascript">   NodeJS</a>
...
<p>以下示例演示了如何在合并模式下对<code translate="no">chunks</code> 字段（一个包含6个子字段的StructArray字段）执行Upsert操作。操作完成后，ID为1的实体的<code translate="no">chunks</code> 字段将被设置为请求中提供的包含两个元素的结构体数组。</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#javascript">   NodeJS</a>
