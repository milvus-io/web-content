...
<tr><td><code translate="no">lemur</code></td><td>每行一个学习得到的向量</td><td>从Embeddings列表到固定维度的行向量，学习针对特定语料库的压缩方法。</td><td>低区分度Embeddings、多模态或视觉文档检索、大型Embeddings列表。</td><td>需要训练，且可能受语料库分布和文档长度偏差的影响。</td></tr>
</tbody>
</table>
<h2 id="TokenANN" class="common-anchor-header">TokenANN<button data-href="#TokenANN" class="anchor-icon" translate="no">
    ...
    </button></h2><p><code translate="no">tokenann</code> 为嵌入列表中的每个向量建立索引。在搜索过程中，每个查询向量执行人工神经网络（ANN）检索，匹配的向量被聚合回其所在行，最终得到的行候选结果通过 MaxSim 进行重新排序。</p>
...
<li><p><strong>较不适用：</strong>非常长的文档、包含数千个补丁向量的视觉页面、内存或延迟预算严格受限的情况。</p></li>
...
<div class="alert note">
<p><strong>当学习压缩的收益足以抵消训练成本时，建议使用 LEMUR。</strong>该方法在低区分度 Embeddings 空间和多模态检索中表现良好，但需针对目标语料库进行验证，因为其结果可能受文档长度分布的影响较大。</p>
</div>
<ul>
<li><p><strong>适用场景：</strong>视觉文档检索、多模态片段嵌入、低区分度 Embeddings 空间，以及 TokenANN 难以应用的大型 Embeddings 列表。</p></li>
...
<h2 id="Configuration-Items-by-Strategy" class="common-anchor-header">按策略划分的配置项<button data-href="#Configuration-Items-by-Strategy" class="anchor-icon" translate="no">
    ...
    </button></h2><p>下表列出了各策略特有的配置项。在 Milvus 中，构建时的配置项通常在创建索引时通过<code translate="no">params</code> 映射传递。若需服务器端的默认值，应将其定义在 Milvus 配置文件的<code translate="no">knowhere</code> 部分中。</p>
<table>
<thead>
...
<h2 id="Configure-Candidate-Retrieval-at-Search-Time" class="common-anchor-header">在搜索时配置候选结果检索<button data-href="#Configure-Candidate-Retrieval-at-Search-Time" class="anchor-icon" translate="no">
    ...
    </button></h2><p>该策略决定索引的构建方式。在搜索时，使用<code translate="no">retrieval_ann_ratio</code> 参数控制在进行 MaxSim 重新排序之前检索的第一阶段候选项数量。较高的数值通常能提高召回率，但会增加延迟。</p>
...
<table>
<thead>
<tr><th>### 质量优先 首先从 `<code translate="no">tokenann</code>` 开始。将其作为 MaxSim 近似质量的基准。</th><th>### 平衡型 当您需要在不增加训练管道的情况下降低成本时，请尝试<code translate="no">muvera</code> 。</th><th>### 压缩 若学习到的行级压缩预计能优于固定随机投影，请尝试使用<code translate="no">lemur</code> 。</th></tr>
</thead>
<tbody>
</tbody>
</table>
