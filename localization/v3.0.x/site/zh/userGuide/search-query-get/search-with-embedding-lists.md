...
summary: >-
  ...
  ColPali 风格的检索系统。当您的查询和存储数据均以向量列表的形式表示，且您希望使用 MAX_SIM* 指标实现实体级晚期交互检索时，可采用此方法。
---
...
<p>ColBERT（arXiv:<a href="https://arxiv.org/abs/2004.12832">2004.12832</a>）是一种文本-文本检索系统，通过基于BERT的上下文化后期交互，提供高效且有效的段落检索。它能够对查询和文档进行独立的词元级编码，并计算它们之间的相似度。</p>
...
<p>如上图所示，查询包含两个令牌，即<code translate="no">machine</code> 和<code translate="no">learning</code> ，而窗口中的文档包含四个令牌：<code translate="no">neural</code> 、<code translate="no">network</code> 、<code translate="no">python</code> 和<code translate="no">tutorial</code> 。将这些令牌向量化后，会将每个查询令牌的向量嵌入与文档中的向量嵌入进行比较，从而获得一组相似度得分。 随后，将每个相似度列表中得分最高的项相加，从而得出最终得分。确定文档最终得分的过程称为最大相似度（<strong>MAX_SIM</strong>）。有关最大相似度的详细信息，请参阅“最大相似度”。</p>
<div class="alert note">
<p>在 Milvus 中实现类似 ColBERT 的文本检索系统时，您并不局限于将文档拆分为词元。</p>
<p>相反，您可以将文档划分为任意合适大小的段落，对每个段落进行嵌入以生成Embeddings列表，并将文档与其嵌入的段落一起存储在实体中。</p>
</div>
...
<h2 id="ColBERT-text-retrieval-system" class="common-anchor-header">ColBERT 文本检索系统<button data-href="#ColBERT-text-retrieval-system" class="anchor-icon" translate="no">
    ...
    </button></h2><p>在本节中，我们将使用 StructArray 搭建一个 ColBERT 文本检索系统。在此之前，请先部署一个 Milvus v2.6.x 实例，并获取 Cohere 访问令牌。</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>如果本地没有该数据集，运行上述脚本将自动下载数据集。数据集中的每条记录都是维基百科页面中的一段文字。下表展示了该数据集的结构。</p>
<table>
<thead>
...
<button class="copy-code-btn"></button></code></pre>
<p>在此代码中，我们将分组后的段落作为文档存储，并将其加入<code translate="no">data</code> 列表。每个文档都有一个<code translate="no">paragraphs</code> 键，该键对应一个段落列表；每个段落对象包含<code translate="no">text</code> 和<code translate="no">emb</code> 键。</p>
...
<h3 id="Step-4-Create-a-collection-for-the-financial-reports-dataset" class="common-anchor-header">步骤 4：为财务报告数据集创建 Collection<button data-href="#Step-4-Create-a-collection-for-the-financial-reports-dataset" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
