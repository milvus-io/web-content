...
<h3 id="Escaping-wildcards-in-a-LIKE-pattern" class="common-anchor-header">在 LIKE 模式中转义通配符<button data-href="#Escaping-wildcards-in-a-LIKE-pattern" class="anchor-icon" translate="no">
    ...
    </button></h3><p>在<code translate="no">LIKE</code> 模式中，<code translate="no">%</code> 匹配零个或多个字符，<code translate="no">_</code> 匹配恰好一个字符。若要精确匹配<code translate="no">%</code> 、<code translate="no">_</code> 或<code translate="no">\</code> 这些字符串，请使用反斜杠 (<code translate="no">\</code>) 对字符进行转义：</p>
<ul>
...
<h3 id="Use-raw-string-literals" class="common-anchor-header">使用原始字符串字面量<button data-href="#Use-raw-string-literals" class="anchor-icon" translate="no">
    ...
    </button></h3><p>对于包含反斜杠的正则表达式模式，建议使用原始字符串字面量。在原始字符串中（写法为<code translate="no">r&quot;...&quot;</code> 或<code translate="no">r'...'</code> ），反斜杠将原样传递给正则表达式引擎。这避免了普通字符串字面量所需的额外转义操作。</p>
<p>例如：</p>
...
<tr><td>不包含固定字符的正则表达式模式，或以字符类、短令牌或通配符为主的模式</td><td>在依赖索引加速之前请先进行基准测试</td><td>这些模式可能提供有限的索引选择性，并可能退化为更广泛的扫描。</td></tr>
</tbody>
</table>
