...
summary: >-
  ...
  混合搜索通过整合这些不同领域的搜索，从而提升搜索体验。Milvus 支持这种混合搜索，允许在多个向量字段上进行搜索，并同时执行多次近似最近邻（ANN）搜索。
  若您希望同时搜索文本和图片、描述同一对象的多个文本字段，或是同时搜索稠密向量和稀疏向量以提升搜索质量，多向量混合搜索将特别有用。
---
<h1 id="Multi-Vector-Hybrid-Search" class="common-anchor-header">多向量混合搜索<button data-href="#Multi-Vector-Hybrid-Search" class="anchor-icon" translate="no">
    ...
    </button></h1><p>在许多应用中，可以通过丰富的信息（如标题和描述）或多种模态（如文本、图像和音频）来搜索对象。例如，如果一条包含文本和图像的推文中，其文本或图像与搜索查询的语义匹配，则应将其检索出来。 混合搜索通过整合这些不同领域的搜索功能，从而提升搜索体验。Milvus 通过支持在多个向量字段上进行搜索，并同时执行多次近似最近邻（ANN）搜索，实现了这一功能。 若您希望同时搜索文本和图像、描述同一对象的多个文本字段，或者为了提高搜索质量而同时搜索稠密向量和稀疏向量，多向量混合搜索将特别有用。</p>
...
<ul>
<li><p><strong>稀疏-稠密向量搜索</strong>：<a href="/docs/zh/dense-vector.md">稠密向量</a>非常适合捕捉语义关系，而<a href="/docs/zh/sparse_vector.md">稀疏向量</a>则在精确的关键词匹配方面极为有效。 混合检索将这些方法相结合，既能提供广泛的概念理解，又能确保术语的精确相关性，从而提升搜索结果质量。通过发挥每种方法的优势，混合检索克服了单一方法的局限性，在处理复杂查询时表现更佳。以下是关于将语义搜索与全文搜索相结合的混合检索的更详细<a href="/docs/zh/full_text_search_with_milvus.md">指南</a>。</p></li>
<li><p><strong>多模态向量搜索</strong>：多模态向量搜索是一种强大的技术，允许您跨多种数据类型（包括文本、图像、音频及其他类型）进行搜索。该方法的主要优势在于能够将不同模态统一到一个无缝且连贯的搜索体验中。 例如，在产品搜索中，用户可能输入文本查询来查找同时包含文本和图片描述的产品。通过混合搜索方法将这些模态结合起来，可以提高搜索准确性或丰富搜索结果。</p></li>
</ul>
<h2 id="Example" class="common-anchor-header">示例<button data-href="#Example" class="anchor-icon" translate="no">
    ...
    </button></h2><p>让我们考虑一个现实世界中的用例：每个产品都包含一段文字描述和一张图片。基于现有数据，我们可以进行三种类型的搜索：</p>
<ul>
...
<li><p><strong>多模态图像搜索：</strong>该方法通过基于稠密向量的文本查询对图像进行检索。图像Embeddings可使用<a href="https://zilliz.com/learn/exploring-openai-clip-the-future-of-multimodal-ai-learning">CLIP</a>等模型生成。</p></li>
</ul>
<p>本指南将通过一个示例，向您演示如何结合上述搜索方法，基于产品的原始文本描述和图像Embeddings，实现多模态混合搜索。我们将展示如何存储多向量数据，并采用重新排序策略进行混合搜索。</p>
...
<p>由于我们将使用内置的 BM25 算法对文本字段进行全文搜索，因此必须将 Milvus 的<code translate="no">Function</code> 添加到 Schema 中。更多详细信息，请参阅<a href="/docs/zh/full-text-search.md">全文搜索</a>。</p>
...
<li><p><code translate="no">text_sparse_index</code>：为文本稀疏向量字段创建了一个类型为<code translate="no">SPARSE_INVERTED_INDEX</code>、度量类型为<code translate="no">BM25</code> 的索引。</p></li>
...
<p>您可以根据需要选择其他索引类型，以最适合您的需求和数据类型。有关支持的索引类型的更多信息，请参阅<a href="/docs/zh/index-vector-fields.md">可用索引类型的</a>文档。</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#go">   Go</a>
...
<h3 id="Create-collection" class="common-anchor-header">创建Collection<button data-href="#Create-collection" class="anchor-icon" translate="no">
    ...
    </button></h3><p>创建一个名为<code translate="no">demo</code> 的 Collection，其 Collection Schema 和索引配置与前两个步骤中设置的一致。</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#go">   Go</a>
...
<h2 id="Insert-data" class="common-anchor-header">插入数据<button data-href="#Insert-data" class="anchor-icon" translate="no">
    ...
    </button></h2><p>本节将根据之前定义的 Schema，向<code translate="no">my_collection</code> Collection 插入数据。插入时，请确保除自动生成的字段外，所有字段均以正确格式提供数据。在此示例中：</p>
<ul>
<li><p><code translate="no">id</code>：一个整数，表示产品 ID</p></li>
<li><p><code translate="no">text</code>: 包含产品描述的字符串</p></li>
<li><p><code translate="no">text_dense</code>: 包含768个浮点数值的列表，表示文本描述的密集嵌入</p></li>
<li><p><code translate="no">image_dense</code>: 一个包含 512 个浮点数的列表，表示产品图片的稠密嵌入</p></li>
</ul>
<p>您可以使用相同或不同的模型为每个字段生成稠密Embeddings。在此示例中，这两个稠密Embeddings的维度不同，这表明它们是由不同的模型生成的。稍后在定义每个搜索时，请务必使用相应的模型来生成合适的查询Embedding。</p>
...
<h2 id="Perform-Hybrid-Search" class="common-anchor-header">执行混合搜索<button data-href="#Perform-Hybrid-Search" class="anchor-icon" translate="no">
    ...
    </button></h3><p>混合搜索的实现是在<code translate="no">hybrid_search()</code> 函数中创建多个<code translate="no">AnnSearchRequest</code> ，其中每个<code translate="no">AnnSearchRequest</code> 代表针对特定向量字段的基本ANN搜索请求。因此，在执行混合搜索之前，必须为每个向量字段创建一个<code translate="no">AnnSearchRequest</code> 。</p>
...
<p>为了演示各种搜索向量字段的功能，我们将使用一个示例查询构建三个<code translate="no">AnnSearchRequest</code> 搜索请求。在此过程中，我们还将使用其预计算的稠密向量。这些搜索请求将针对以下向量字段：</p>
<ul>
...
<button class="copy-code-btn"></button></code></pre>
<p>鉴于参数<code translate="no">limit</code> 设置为 2，每个<code translate="no">AnnSearchRequest</code> 都会返回 2 个搜索结果。在此示例中，创建了 3 个<code translate="no">AnnSearchRequest</code> 实例，因此总共产生了 6 个搜索结果。</p>
<h3 id="Step-2-Configure-a-reranking-strategy" class="common-anchor-header">步骤 2：配置重新排序策略<button data-href="#Step-2-Configure-a-reranking-strategy" class="anchor-icon" translate="no">
    ...
    </button></h3><p>要合并并重新排序多组 ANN 搜索结果，选择合适的重新排序策略至关重要。Milvus 提供了多种类型的重新排序策略。有关这些重新排序机制的更多详细信息，请参阅<a href="/docs/zh/weighted-ranker.md">“加权排序器</a>”或<a href="/docs/zh/rrf-ranker.md">“RRF 排序器”</a>。</p>
...
<h3 id="Step-3-Perform-a-Hybrid-Search" class="common-anchor-header">步骤 3：执行混合搜索<button data-href="#Step-3-Perform-a-Hybrid-Search" class="anchor-icon" translate="no">
    ...
    </button></h3><p>在启动混合搜索之前，请确保已加载该Collection。如果Collection中的任何向量字段缺少索引或未加载到内存中，执行混合搜索方法时将会发生错误。</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#go">   Go</a>
...
<button class="copy-code-btn"></button></code></pre>
<p>当为混合搜索指定<code translate="no">limit=2</code> 参数时，Milvus将对来自三次搜索的六个结果进行重新排序。最终，系统将仅返回相似度最高的两个结果。</p>
