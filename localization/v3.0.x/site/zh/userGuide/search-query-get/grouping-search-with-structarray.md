...
summary: >-
  ...
  元素与查询条件匹配时，元素级搜索可能会返回来自同一实体的多个匹配结果。分组功能会将这些元素级匹配结果合并，从而确保每个父实体至多出现一次。
---
...
<p>本页面<a href="/docs/zh/create-structarray-field.md">使用“创建 StructArray 字段</a>”中的<code translate="no">tech_articles</code> Collection。该 Collection 包含一个名为<code translate="no">chunks</code> 的 StructArray 字段。其中<code translate="no">chunks[emb]</code> 向量已通过常规向量度量进行索引，以支持元素级搜索。</p>
