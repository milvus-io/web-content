...
summary: >-
  短语匹配可让您搜索包含查询词作为精确短语的文档。默认情况下，这些词必须按相同顺序出现，并且彼此紧邻。 例如，查询“robotics machine
  learning”将匹配“……典型的机器人学机器学习模型……”这样的文本，其中“robotics”、“machine”和“learning”这三个词按顺序出现，且之间没有其他词。
beta: Milvus 2.5.17+
---
<h1 id="Phrase-Match" class="common-anchor-header">短语匹配<span class="beta-tag" style="background-color:rgb(0, 179, 255);color:white" translate="no">Compatible with Milvus 2.5.17+</span><button data-href="#Phrase-Match" class="anchor-icon" translate="no">
    ...
    </button></h1><p>短语匹配可让您搜索包含查询词作为精确短语的文档。默认情况下，这些词必须按相同顺序出现，并且彼此紧邻。 例如，查询<strong>“robotics machine learning”</strong>将匹配诸如<em>“…典型的机器人学机器学习模型…”之类的</em>文本，其中<strong>“robotics”</strong>、<strong>“machine”</strong>和<strong>“learning”</strong>这三个词按顺序出现，且之间没有其他词语。</p>
<p>然而，在实际应用中，严格的短语匹配可能过于僵化。您可能希望匹配<em>诸如“…在机器人技术中广泛采用的机器学习模型…”</em>之类的文本<em>。</em>在此情况下，虽然包含相同的关键词，但它们并非紧邻或按原始顺序排列。为解决此问题，短语匹配支持<code translate="no">slop</code> 参数，从而增加了灵活性。<code translate="no">slop</code> 的值定义了短语中各术语之间允许的位置偏移量。例如，当<code translate="no">slop</code> 设为1时，查询<strong>“machine learning”</strong>可匹配<em>“…machine deep learning…”</em>这样的文本，其中一个单词（<strong>“deep”</strong>）将原始术语分隔开来。</p>
<h2 id="Overview" class="common-anchor-header">概述<button data-href="#Overview" class="anchor-icon" translate="no">
    ...
    </button></h2><p>短语匹配功能由<a href="https://github.com/quickwit-oss/tantivy">Tantivy</a>搜索引擎库提供支持，其工作原理是分析文档中单词的位置信息。下图说明了该过程：</p>
...
<ol>
<li><p><strong>文档分词</strong>：将文档导入 Milvus 时，系统会使用分析器将文本拆分为词元（单个单词或术语），并为每个词元记录位置信息。 例如，<strong>doc_1</strong>被分词为<strong>[“machine”（pos=0），“learning”（pos=1），“boosts”（pos=2），“efficiency”（pos=3）]</strong>。有关分析器的更多信息，请参阅《<a href="/docs/zh/analyzer-overview.md">分析器概述》</a>。</p></li>
<li><p><strong>倒排索引的创建</strong>：Milvus 会构建一个倒排索引，将每个词元映射到其出现的文档以及该词元在这些文档中的位置。</p></li>
<li><p><strong>短语匹配</strong>：当执行短语查询时，Milvus 会在倒排索引中查找每个词元，并检查其位置，以确定它们是否以正确的顺序和接近度出现。<code translate="no">slop</code> 参数控制匹配词元之间允许的最大位置差：</p>
<ul>
<li><p><strong>slop = 0</strong>表示词元必须<strong>以完全相同的顺序</strong>出现<strong>且紧邻</strong>（即中间没有多余的词）。</p>
<ul>
<li>在示例中，只有<strong>doc_1</strong>（<strong>“machine”</strong>位于<strong>pos=0</strong>，<strong>“learning”</strong>位于<strong>pos=1</strong>）完全匹配。</li>
</ul></li>
<li><p><strong>slop = 2</strong>允许匹配标记之间最多有两个位置的灵活性或重新排列。</p>
<ul>
<li><p>这允许顺序颠倒（<strong>“learning machine”</strong>）或令牌之间存在微小间隔。</p></li>
<li><p>因此，<strong>doc_1</strong>、<strong>doc_2</strong>（<strong>“learning”</strong>位于<strong>位置 0</strong> <strong>，“machine”</strong>位于<strong>位置 1</strong>）和<strong>doc_3</strong>（<strong>“learning”</strong>位于<strong>位置 1</strong> <strong>，“machine”</strong>位于<strong>位置 2</strong>）均匹配。</p></li>
</ul></li>
</ul></li>
</ol>
<h2 id="Enable-phrase-match" class="common-anchor-header">启用短语匹配<button data-href="#Enable-phrase-match" class="anchor-icon" translate="no">
    ...
    </button></h2><p>短语匹配适用于<code translate="no">VARCHAR</code> 字段类型，即Milvus中的字符串数据类型。要启用短语匹配，请配置您的Collection Schema，将<code translate="no">enable_analyzer</code> 和<code translate="no">enable_match</code> 两个参数均设置为<code translate="no">True</code> ，这与<a href="/docs/zh/keyword-match.md">文本匹配</a>类似。</p>
<h3 id="Set-enableanalyzer-and-enablematch" class="common-anchor-header">设置<code translate="no">enable_analyzer</code> 和<code translate="no">enable_match</code><button data-href="#Set-enableanalyzer-and-enablematch" class="anchor-icon" translate="no">
    ...
    </button></h3><p>若要为特定的<code translate="no">VARCHAR</code> 字段启用短语匹配，请在定义字段模式时将<code translate="no">enable_analyzer</code> 和<code translate="no">enable_match</code> 两个参数均设置为<code translate="no">True</code> 。此配置会指示 Milvus 对文本进行分词，并创建包含高效短语匹配所需位置信息的倒排索引。</p>
<p>以下是一个用于启用短语匹配的示例Schema定义：</p>
...
<h3 id="Optional-Configure-an-analyzer" class="common-anchor-header">可选：配置分析器<button data-href="#Optional-Configure-an-analyzer" class="anchor-icon" translate="no">
    ...
    </button></h3><p>短语匹配的准确性在很大程度上取决于用于对文本数据进行分词的分析器。不同的分析器适用于不同的语言和文本格式，从而影响分词结果及位置匹配的准确性。根据您的具体用例选择合适的分析器，将有助于优化短语匹配结果。</p>
<p>默认情况下，Milvus 使用标准分析器，该分析器基于空格和标点符号对文本进行分词，移除长度超过 40 个字符的词元，并将文本转换为小写。默认使用时无需额外参数。详情请参阅<a href="/docs/zh/standard-analyzer.md">“标准分析器”</a>。</p>
<p>如果您的应用需要特定的分析器，请使用<code translate="no">analyzer_params</code> 参数进行配置。例如，以下是针对英语文本的短语匹配配置<code translate="no">english</code> 分析器的方法：</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#go">   Go</a>
...
schema.<span class="hljs-built_in">AddField</span>(milvus::<span class="hljs-built_in">FieldSchema</span>(<span class="hljs-string">&quot;text&quot;</span>, milvus::DataType::VARCHAR)  <span class="hljs-comment">// Name of the field</span>
                    ...
                    .<span class="hljs-built_in">EnableMatch</span>(<span class="hljs-literal">true</span>));                <span class="hljs-comment">// Enables inverted indexing for phrase matching</span>
<button class="copy-code-btn"></button></code></pre>
<p>Milvus 支持多种针对不同语言和用例量身定制的分析器。有关详细信息，请参阅《<a href="/docs/zh/analyzer-overview.md">分析器概述</a>》。</p>
<h2 id="Use-phrase-match" class="common-anchor-header">使用短语匹配<button data-href="#Use-phrase-match" class="anchor-icon" translate="no">
    ...
    </button></h2><p>在 Collection Schema 中为 `<code translate="no">VARCHAR</code> ` 字段启用匹配功能后，即可使用 `<code translate="no">PHRASE_MATCH</code> ` 表达式执行短语匹配。</p>
<div class="alert note">
<p><code translate="no">PHRASE_MATCH</code> 表达式不区分大小写。您可以使用<code translate="no">PHRASE_MATCH</code> 或<code translate="no">phrase_match</code> 。</p>
</div>
<h3 id="PHRASEMATCH-expression-syntax" class="common-anchor-header">PHRASE_MATCH 表达式语法<button data-href="#PHRASEMATCH-expression-syntax" class="anchor-icon" translate="no">
    ...
    </button></h3><p>使用<code translate="no">PHRASE_MATCH</code> 表达式在搜索时指定字段、短语以及可选的灵活性（<code translate="no">slop</code> ）。语法如下：</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#go">   Go</a>
...
<button class="copy-code-btn"></button></code></pre>
<ul>
<li><p><code translate="no">field_name</code><strong>:</strong>用于执行短语匹配的<code translate="no">VARCHAR</code> 字段名称。</p></li>
<li><p><code translate="no">phrase</code><strong>:</strong>要进行短语匹配的xml-ph-0000@deepl.internal字段名称。</p></li>
<li><p><code translate="no">slop</code> （可选）<strong>：</strong>一个整数，用于指定匹配令牌中允许的最大位差。</p>
<ul>
<li><p><code translate="no">0</code> (默认)：仅匹配完全相同的短语。示例：<strong>针对“machine</strong>learning”的过滤器将精确<strong>匹配“machine</strong>learning<strong>”，但不会匹配“machine boosts</strong> <strong>learning”或“learning machine”。</strong></p></li>
<li><p><code translate="no">1</code>: 允许轻微变体，例如多一个术语或位置的微小变化。示例：针对<strong>“machine learning”</strong>的过滤器将匹配<strong>“machine boosts learning”</strong>（<strong>“machine”</strong>和<strong>“learning”</strong>之间有一个令牌），但不会<strong>匹配“learning machine”</strong>（术语顺序颠倒）。</p></li>
<li><p><code translate="no">2</code>：允许更大的灵活性，包括词序颠倒或“machine”和“learning”之间最多有两个词元。示例：过滤条件<strong>“machine learning”</strong>将匹配<strong>“learning machine”</strong>（词序颠倒）<strong>或“machine quickly boosts learning”</strong>（<strong>“machine”</strong>和<strong>“learning”</strong>之间有两个词元）。</p></li>
</ul></li>
</ul>
<h3 id="Example-dataset" class="common-anchor-header">示例数据集<button data-href="#Example-dataset" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>假设您有一个名为<strong>tech_articles</strong>的 Collection，其中包含以下五个实体：</p>
<table>
   <tr>
   ...
   <tr>
     <td><p>2</p></td>
     <td><p>“掌握基于机器的方法对现代人工智能的发展至关重要”</p></td>
   </tr>
   <tr>
     <td><p>3</p></td>
     <td><p>“深度学习架构可优化计算负载”</p></td>
   </tr>
   <tr>
     <td><p>4</p></td>
     <td><p>“机器能迅速提升模型性能，以支持持续学习”</p></td>
   </tr>
   <tr>
     <td><p>5</p></td>
     <td><p>“学习先进的机器算法可拓展人工智能的能力”</p></td>
   </tr>
</table>
<h3 id="Query-with-phrase-match" class="common-anchor-header">短语匹配查询<button data-href="#Query-with-phrase-match" class="anchor-icon" translate="no">
    ...
    </button></h3><p>使用<code translate="no">query()</code> 方法时，<strong>PHRASE_MATCH</strong>充当标量过滤器。仅返回包含指定短语（在允许的容差范围内）的文档。</p>
<h4 id="Example-slop--0-exact-match" class="common-anchor-header">示例：slop = 0（精确匹配）</h4><p>此示例将返回包含精确<strong>短语“machine learning”</strong>且中间没有额外词元的文档。</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#go">   Go</a>
...
<p>只有文档 1 包含完全匹配的短语<strong>“machine learning”，</strong>且词语顺序与指定一致，中间没有其他词。</p>
<h3 id="Search-with-phrase-match" class="common-anchor-header">使用短语匹配进行搜索<button data-href="#Search-with-phrase-match" class="anchor-icon" translate="no">
    ...
    </button></h3><p>在搜索操作中，<strong>PHRASE_MATCH</strong>用于在应用向量相似度排序之前对文档进行过滤。这种两步法首先通过文本匹配缩小候选集范围，然后基于向量 Embeddings 对这些候选结果进行重新排序。</p>
<h4 id="Example-slop--1" class="common-anchor-header">示例：slop = 1</h4><p>在此，我们允许 1 的容差值。该过滤器将应用于包含短语<strong>“learning machine”</strong>且具有轻微灵活性（即允许词序微小偏差）的文档。</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#go">   Go</a>
...
<table>
   ...
   <tr>
     <td><p>2</p></td>
     <td><p>“掌握基于机器的学习方法对现代人工智能的发展至关重要”</p></td>
   </tr>
   <tr>
     <td><p>3</p></td>
     <td><p>“深度学习架构能够优化计算负载”</p></td>
   </tr>
   <tr>
     <td><p>5</p></td>
     <td><p>“学习先进的机器算法能拓展人工智能的能力”</p></td>
   </tr>
</table>
<h4 id="Example-slop--2" class="common-anchor-header">示例：slop = 2</h4><p><strong>此</strong>示例允许2的容错度，这意味着在“machine”和“learning”这两个词之间最多允许出现两个额外的词元（或词序颠倒）<strong>。</strong></p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#go">   Go</a>
...
<table>
   ...
   <tr>
     <td><p>3</p></td>
     <td><p>“深度学习机器架构优化计算负载”</p></td>
   </tr>
</table>
<h4 id="Example-slop--3" class="common-anchor-header">示例：slop = 3</h4><p>在此示例中，slop 值为 3 提供了更大的灵活性。该过滤器在搜索<strong>“machine learning</strong>”时，允许两个单词之间最多有三个令牌的位置差。</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#go">   Go</a>
...
<li><p>为字段启用短语匹配会触发倒排索引的创建，这会消耗存储资源。在决定是否启用此功能时，请考虑其对存储的影响，因为该影响会因文本大小、唯一词元以及所使用的分析器而异。</p></li>
<li><p>在 Schema 中定义分析器后，其设置将对该 Collection 永久生效。若您认为其他分析器更能满足需求，可考虑删除现有 Collection，并使用所需的分析器配置创建新 Collection。</p></li>
<li><p>短语匹配的性能取决于文本的分词方式。在将分析器应用于整个 Collection 之前，请使用 `<code translate="no">run_analyzer</code> ` 方法查看分词结果。有关更多信息，请参阅《<a href="/docs/zh/analyzer-overview.md#share-DYZvdQ2vUowWEwx1MEHcdjNNnqT">分析器概述</a>》。</p></li>
<li><p><code translate="no">filter</code> 表达式中的转义规则：</p>
<ul>
<li><p>表达式中用双引号或单引号括起的字符将被解释为字符串常量。如果字符串常量包含转义字符，则必须使用转义序列来表示这些转义字符。例如，使用<code translate="no">\\</code> 表示<code translate="no">\</code> ，使用<code translate="no">\\t</code> 表示制表符<code translate="no">\t</code> ，使用<code translate="no">\\n</code> 表示换行符。</p></li>
<li><p>如果字符串常量由单引号包围，则常量内的单引号应表示为<code translate="no">\\'</code> ，而双引号可以表示为<code translate="no">&quot;</code> 或<code translate="no">\\&quot;</code> 。示例：<code translate="no">'It\\'s milvus'</code> 。</p></li>
<li><p>如果字符串常量由双引号包围，则常量中的双引号应表示为<code translate="no">\\&quot;</code> ，而单引号可以表示为<code translate="no">'</code> 或<code translate="no">\\'</code> 。示例：<code translate="no">&quot;He said \\&quot;Hi\\&quot;&quot;</code> 。</p></li>
</ul></li>
</ul>
