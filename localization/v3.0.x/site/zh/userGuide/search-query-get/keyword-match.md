...
summary: >-
  Milvus
  中的文本匹配功能可根据特定术语精确检索文档。该功能主要用于满足特定条件的过滤搜索，并可结合标量过滤来优化查询结果，从而在满足标量条件的向量中进行相似度搜索。
---
<h1 id="Text-Match" class="common-anchor-header">文本匹配<button data-href="#Text-Match" class="anchor-icon" translate="no">
    ...
    </button></h1><p>Milvus 中的文本匹配功能可基于特定术语实现精确的文档检索。该功能主要用于满足特定条件的过滤搜索，并可结合标量过滤来优化查询结果，从而在满足标量条件的向量中进行相似度搜索。</p>
<div class="alert note">
<p><code translate="no">TEXT_MATCH</code> 可查找完全匹配的已分析术语，而“近似匹配”（<code translate="no">TEXT_MATCH_FUZZY</code> ）则可容忍查询令牌与索引令牌之间存在微小的编辑距离。这两者均为布尔过滤操作，不会对匹配文档的相关性进行评分。如果您希望根据查询术语的语义含义和重要性检索最相关的文档，建议您使用<a href="/docs/zh/full-text-search.md">全文搜索</a>。</p>
</div>
<h2 id="Overview" class="common-anchor-header">概述<button data-href="#Overview" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Milvus 集成了<a href="https://github.com/quickwit-oss/tantivy">Tantivy，</a>以此驱动其底层的倒排索引和基于术语的文本搜索。对于每条文本输入，Milvus 会按照以下流程进行索引：</p>
<ol>
<li><p><a href="/docs/zh/analyzer-overview.md">分析器</a>：分析器通过将输入文本切分为单个单词（即词元）来处理文本，然后根据需要应用过滤器。这使 Milvus 能够基于这些词元构建索引。</p></li>
<li><p><a href="/docs/zh/index-explained.md">索引</a>：文本分析完成后，Milvus 会创建一个倒排索引，将每个唯一的词元映射到包含该词元的文档中。</p></li>
</ol>
<p>当用户执行文本匹配时，系统会利用倒排索引快速检索所有包含该词条的文档。这比逐个扫描每个文档要快得多。</p>
...
<h3 id="Set-enableanalyzer-and-enablematch" class="common-anchor-header">将 `<code translate="no">enable_analyzer</code> ` 和<code translate="no">enable_match</code><button data-href="#Set-enableanalyzer-and-enablematch" class="anchor-icon" translate="no">
    ...
    </button></h3><p>若要为特定的<code translate="no">VARCHAR</code> 字段启用文本匹配，请在定义字段 Schema 时将<code translate="no">enable_analyzer</code> 和<code translate="no">enable_match</code> 两个参数均设置为<code translate="no">True</code> 。这将指示 Milvus 对文本进行分词，并为指定字段创建倒排索引，从而实现快速高效的文本匹配。</p>
...
<h3 id="Optional-Configure-an-analyzer" class="common-anchor-header">可选：配置分析器<button data-href="#Optional-Configure-an-analyzer" class="anchor-icon" translate="no">
    ...
    </button></h3><p>关键词匹配的性能和准确性取决于所选的分析器。不同的分析器针对各种语言和文本结构进行了优化，因此选择合适的分析器会对您具体用例的搜索结果产生显著影响。</p>
...
<h2 id="Use-text-match" class="common-anchor-header">使用文本匹配<button data-href="#Use-text-match" class="anchor-icon" translate="no">
    ...
    </button></h2><p>在Collection模式中为<code translate="no">VARCHAR</code> 或<code translate="no">TEXT</code> 字段启用文本匹配功能后，即可使用<code translate="no">TEXT_MATCH</code> 表达式执行文本匹配。</p>
...
<li><p><code translate="no">text</code>: 要搜索的术语。多个术语可用空格分隔，或根据语言和配置的分析器使用其他适当的分隔符。</p></li>
</ul>
...
<h3 id="TEXTMATCHFUZZY-expression-syntax--Milvus-300+" class="common-anchor-header">TEXT_MATCH_FUZZY 表达式语法<span class="beta-tag" style="background-color:rgb(0, 179, 255);color:white" translate="no">Compatible with Milvus 3.0.0+</span><button data-href="#TEXTMATCHFUZZY-expression-syntax--Milvus-300+" class="anchor-icon" translate="no">
    ...
    </button></h3><p>使用<code translate="no">TEXT_MATCH_FUZZY</code> 可容忍查询词元与索引词元之间的拼写差异。Milvus会使用字段的分析器对查询文本进行分析，并对生成的每个词元应用模糊匹配。如果查询产生多个词元，当任何一个词元满足配置的编辑距离时，该表达式即匹配一个实体。</p>
<p>语法如下：</p>
...
<button class="copy-code-btn"></button></code></pre>
<ul>
<li><p><code translate="no">field_name</code>: 要进行搜索的、已启用匹配功能的<code translate="no">VARCHAR</code> 或<code translate="no">TEXT</code> 字段名称。</p></li>
<li><p><code translate="no">text</code>: 待分析和与索引词进行匹配的查询文本。</p></li>
...
<p>例如，以下表达式匹配与<code translate="no">machne</code> 相差不超过一处编辑的令牌，包括<code translate="no">machine</code> ：</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#go">   Go</a>
...
<button class="copy-code-btn"></button></code></pre>
<p><code translate="no">TEXT_MATCH_FUZZY</code> 是过滤表达式语法的一部分，因此客户端 SDK 无需专门的模糊匹配方法。在搜索或查询操作中，将该表达式作为与<code translate="no">TEXT_MATCH</code> 相同的<code translate="no">filter</code> 参数传递即可。</p>
<h3 id="Search-with-text-match" class="common-anchor-header">使用文本匹配进行搜索<button data-href="#Search-with-text-match" class="anchor-icon" translate="no">
    ...
    </button></h3><p>文本匹配可与向量相似度搜索结合使用，以缩小搜索范围并提升搜索性能。通过在向量相似度搜索之前使用文本匹配对Collection进行过滤，您可以减少需要搜索的文档数量，从而加快查询速度。</p>
<p>在此示例中，<code translate="no">filter</code> 表达式将搜索结果过滤为仅包含与指定术语<code translate="no">keyword1</code> 或<code translate="no">keyword2</code> 匹配的文档。随后，将在该过滤后的文档子集上执行向量相似度搜索。</p>
<div class="alert note">
...
<li><p>为字段启用术语匹配会触发反向索引的创建，这会消耗存储资源。在决定是否启用此功能时，请考虑其对存储空间的影响，因为该影响会因文本大小、唯一词元以及所使用的分析器而异。</p></li>
<li><p>在 Schema 中定义分析器后，其设置将对该 Collection 永久生效。若您认为其他分析器更能满足需求，可考虑删除现有 Collection，并使用所需的分析器配置创建新的 Collection。</p></li>
...
<li><p>如果字符串常量用单引号括起，则常量中的单引号应表示为<code translate="no">\\'</code> ，而双引号可以表示为<code translate="no">&quot;</code> 或<code translate="no">\\&quot;</code> 。示例：<code translate="no">'It\\'s milvus'</code> 。</p></li>
<li><p>如果字符串常量用双引号括起，则常量中的双引号应表示为<code translate="no">\\&quot;</code> ，而单引号可以表示为<code translate="no">'</code> 或<code translate="no">\\'</code> 。示例：<code translate="no">&quot;He said \\&quot;Hi\\&quot;&quot;</code> 。</p></li>
</ul></li>
</ul>
