...
summary: >-
  Milvus
  中的文本匹配功能可基于特定术语实现精确的文档检索。该功能主要用于满足特定条件的过滤搜索，并可结合标量过滤来优化查询结果，从而在满足标量条件的向量范围内进行相似度搜索。
---
<h1 id="Text-Match" class="common-anchor-header">文本匹配<button data-href="#Text-Match" class="anchor-icon" translate="no">
    ...
    </button></h1><p>Milvus 中的文本匹配功能可基于特定术语精确检索文档。该功能主要用于满足特定条件的过滤搜索，并可结合标量过滤来优化查询结果，从而在满足标量条件的向量中进行相似度搜索。</p>
<div class="alert note">
<p>文本匹配专注于查找查询词的精确匹配，而不对匹配文档的相关性进行评分。如果您希望根据查询词的语义含义和重要性检索最相关的文档，建议您使用<a href="/docs/zh/full-text-search.md">全文搜索</a>。</p>
</div>
<h2 id="Overview" class="common-anchor-header">概述<button data-href="#Overview" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Milvus 集成了<a href="https://github.com/quickwit-oss/tantivy">Tantivy，</a>以此支持其底层的倒排索引和基于术语的文本搜索。对于每条文本条目，Milvus 会按照以下流程进行索引：</p>
<ol>
<li><p><a href="/docs/zh/analyzer-overview.md">分析器</a>：分析器通过将输入文本切分为单个单词（即词元）来处理文本，然后根据需要应用过滤器。这使 Milvus 能够基于这些词元构建索引。</p></li>
<li><p><a href="/docs/zh/index-explained.md">索引</a>：文本分析完成后，Milvus 会创建一个倒排索引，将每个唯一的词素映射到包含该词素的文档上。</p></li>
</ol>
...
<h2 id="Enable-text-match" class="common-anchor-header">启用文本匹配<button data-href="#Enable-text-match" class="anchor-icon" translate="no">
    ...
    </button></h2><p>文本匹配适用于 <a href="/docs/zh/string.md"><code translate="no">VARCHAR</code></a> 字段类型上运行，该类型本质上是 Milvus 中的字符串数据类型。要启用文本匹配，请将 `<code translate="no">enable_analyzer</code> ` 和 `<code translate="no">enable_match</code> ` 均设置为 `<code translate="no">True</code> `，然后在定义 Collection Schema 时，可选地配置一个<a href="/docs/zh/analyzer-overview.md">分析器</a>用于文本分析。</p>
...
<h3 id="Optional-Configure-an-analyzer" class="common-anchor-header">可选：配置分析器<button data-href="#Optional-Configure-an-analyzer" class="anchor-icon" translate="no">
    ...
    </button></h3><p>关键词匹配的性能和准确性取决于所选的分析器。不同的分析器针对各种语言和文本结构进行了优化，因此选择合适的分析器将对您特定用例的搜索结果产生显著影响。</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>Milvus 还提供了适用于不同语言和场景的多种其他分析器。更多详细信息，请参阅《<a href="/docs/zh/analyzer-overview.md">分析器概述</a>》。</p>
<h2 id="Use-text-match" class="common-anchor-header">使用文本匹配<button data-href="#Use-text-match" class="anchor-icon" translate="no">
    ...
    </button></h2><p>在 Collection Schema 中为 VARCHAR 字段启用文本匹配功能后，即可使用<code translate="no">TEXT_MATCH</code> 表达式执行文本匹配。</p>
<h3 id="TEXTMATCH-expression-syntax" class="common-anchor-header">TEXT_MATCH 表达式语法<button data-href="#TEXTMATCH-expression-syntax" class="anchor-icon" translate="no">
    ...
    </button></h3><p><code translate="no">TEXT_MATCH</code> 表达式用于指定要搜索的字段和搜索词。其语法如下：</p>
...
<p>默认情况下，<code translate="no">TEXT_MATCH</code> 使用<strong>“或”（OR）</strong>匹配逻辑，这意味着它将返回包含任何指定术语的文档。例如，若要搜索在<code translate="no">text</code> 字段中包含术语<code translate="no">machine</code> 或<code translate="no">deep</code> 的文档，请使用以下表达式：</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#go">   Go</a>
...
<ul>
<li><p>若要搜索在“<code translate="no">text</code> ”字段中同时包含“<code translate="no">machine</code> ”和“<code translate="no">deep</code> ”的文档，请使用以下表达式：</p>
...
<h3 id="Search-with-text-match" class="common-anchor-header">使用文本匹配进行搜索<button data-href="#Search-with-text-match" class="anchor-icon" translate="no">
    ...
    </button></h3><p>文本匹配可与向量相似度搜索结合使用，以缩小搜索范围并提升搜索性能。通过在进行向量相似度搜索之前使用文本匹配对Collection进行过滤，您可以减少需要搜索的文档数量，从而加快查询速度。</p>
<p>在此示例中，表达式 `<code translate="no">filter</code> ` 将搜索结果过滤为仅包含与指定术语 `<code translate="no">keyword1</code> ` 或 `<code translate="no">keyword2</code>` 匹配的文档。随后，系统将针对这一经过过滤的文档子集执行向量相似度搜索。</p>
<div class="alert note">
...
<li><p>为字段启用术语匹配会触发倒排索引的创建，这会消耗存储资源。在决定是否启用此功能时，请考虑其对存储空间的影响，因为该影响会因文本大小、唯一词元以及所使用的分析器而异。</p></li>
<li><p>在 Schema 中定义分析器后，其设置将对该 Collection 永久生效。若您认为其他分析器更符合需求，可考虑删除现有 Collection，并使用所需的分析器配置创建新 Collection。</p></li>
