...
<p><code translate="no">TEXT_MATCH</code> 可查找完全匹配的已分析术语，而“<code translate="no">TEXT_MATCH_FUZZY</code> ”则可容忍查询令牌与索引令牌之间存在微小的编辑距离。这两者均为布尔过滤操作，不会对匹配文档的相关性进行评分。如果您希望根据查询术语的语义含义和重要性检索最相关的文档，建议您使用<a href="/docs/zh/full-text-search.md">全文搜索</a>。</p>
</div>
<h2 id="Overview" class="common-anchor-header">概述<button data-href="#Overview" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Milvus 集成了<a href="https://github.com/quickwit-oss/tantivy">Tantivy，</a>以此驱动其底层的倒排索引和基于术语的文本搜索。对于每条文本条目，Milvus 会按照以下流程进行索引：</p>
<ol>
...
<h2 id="Enable-text-match" class="common-anchor-header">启用文本匹配<button data-href="#Enable-text-match" class="anchor-icon" translate="no">
    ...
    </button></h2><p>文本匹配适用于已启用匹配功能的字符串字段。本页示例使用 <a href="/docs/zh/string.md"><code translate="no">VARCHAR</code></a>，该字段在所有客户端 SDK 中均受支持。在 Milvus 3.0.x 中， <a href="/docs/zh/text.md"><code translate="no">TEXT</code></a> 当启用 Storage V3 时，字段也支持文本匹配。对于这两种字段类型，请将 `<code translate="no">enable_analyzer</code> ` 和 `<code translate="no">enable_match</code> ` 均设置为 `<code translate="no">True</code>`，然后在定义 Collection Schema 时可选地配置<a href="/docs/zh/analyzer-overview.md">分析器</a>。</p>
...
<h3 id="Optional-Configure-an-analyzer" class="common-anchor-header">可选：配置分析器<button data-href="#Optional-Configure-an-analyzer" class="anchor-icon" translate="no">
    ...
    </button></h3><p>关键词匹配的性能和准确性取决于所选的分析器。不同的分析器针对各种语言和文本结构进行了优化，因此选择合适的分析器会对您特定用例的搜索结果产生显著影响。</p>
...
<h2 id="Use-text-match" class="common-anchor-header">使用文本匹配<button data-href="#Use-text-match" class="anchor-icon" translate="no">
    ...
    </button></h2><p>在Collection Schema中为<code translate="no">VARCHAR</code> 或<code translate="no">TEXT</code> 字段启用文本匹配功能后，即可使用<code translate="no">TEXT_MATCH</code> 表达式执行文本匹配。</p>
...
<li><p><code translate="no">text</code>: 要搜索的术语。多个术语可用空格或根据语言及配置的分析器所确定的其他适当分隔符分隔。</p></li>
</ul>
...
<h3 id="TEXTMATCHFUZZY-expression-syntax--Milvus-300+" class="common-anchor-header">TEXT_MATCH_FUZZY 表达式语法<span class="beta-tag" style="background-color:rgb(0, 179, 255);color:white" translate="no">Compatible with Milvus 3.0.0+</span><button data-href="#TEXTMATCHFUZZY-expression-syntax--Milvus-300+" class="anchor-icon" translate="no">
    ...
    </button></h3><p>使用<code translate="no">TEXT_MATCH_FUZZY</code> 可容忍查询令牌与索引令牌之间的拼写差异。Milvus 会使用字段的分析器分析查询文本，并对生成的每个令牌应用模糊匹配。如果查询产生多个令牌，当任何一个令牌满足配置的编辑距离时，该表达式即匹配一个实体。</p>
<p>语法如下：</p>
...
<button class="copy-code-btn"></button></code></pre>
<ul>
<li><p><code translate="no">field_name</code>: 要搜索的、已启用匹配功能的<code translate="no">VARCHAR</code> 或<code translate="no">TEXT</code> 字段名称。</p></li>
...
<p>例如，以下表达式匹配与<code translate="no">machne</code> 仅差一个字符的令牌，包括<code translate="no">machine</code> ：</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#go">   Go</a>
...
<h3 id="Search-with-text-match" class="common-anchor-header">使用文本匹配进行搜索<button data-href="#Search-with-text-match" class="anchor-icon" translate="no">
    ...
    </button></h3><p>文本匹配可与向量相似度搜索结合使用，以缩小搜索范围并提升搜索性能。通过在进行向量相似度搜索前使用文本匹配对Collection进行过滤，您可以减少需要搜索的文档数量，从而加快查询速度。</p>
<p>在此示例中，<code translate="no">filter</code> 表达式将搜索结果过滤为仅包含与指定术语<code translate="no">keyword1</code> 或<code translate="no">keyword2</code> 匹配的文档。随后，系统将针对这一经过过滤的文档子集执行向量相似度搜索。</p>
<div class="alert note">
...
<li><p>如果字符串常量由双引号包围，则常量内的双引号应表示为<code translate="no">\\&quot;</code> ，而单引号可以表示为<code translate="no">'</code> 或<code translate="no">\\'</code> 。示例：<code translate="no">&quot;He said \\&quot;Hi\\&quot;&quot;</code> 。</p></li>
</ul></li>
</ul>
