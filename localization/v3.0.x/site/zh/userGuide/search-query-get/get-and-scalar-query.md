...
<table>
   ...
   <tr>
     ...
     <td><p>查找满足自定义过滤条件的所有实体或指定数量的实体</p></td>
     <td><p>在分页查询中查找满足自定义过滤条件的所有实体。</p></td>
   </tr>
   <tr>
     <td><p>筛选方法</p></td>
     <td><p>按主键</p></td>
   ...
   <tr>
     ...
     <td><p>通过分页查询，返回指定Collection或分区中满足自定义筛选条件的全部实体。</p></td>
   </tr>
</table>
...
<h2 id="Use-Query" class="common-anchor-header">使用查询<button data-href="#Use-Query" class="anchor-icon" translate="no">
    ...
    </button></h3><p>当您需要根据自定义过滤条件查找实体时，请使用<strong>Query</strong>方法。以下代码示例假设存在三个名为<code translate="no">id</code> 、<code translate="no">vector</code> 和<code translate="no">color</code> 的字段，并返回指定数量的、其<code translate="no">color</code> 字段值以<code translate="no">red</code> 开头的实体。</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#go">   Go</a>
...
<h3 id="Sort-Query-Results" class="common-anchor-header">对查询结果进行排序<span class="beta-tag" style="background-color:rgb(0, 179, 255);color:white" translate="no">Compatible with Milvus 3.0.x</span><button data-href="#Sort-Query-Results" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<li><p>按可为空字段排序时，升序（NULLS LAST）中 NULL 值位于末尾，降序（NULLS FIRST）中位于开头。</p></li>
</ul>
...
<button class="copy-code-btn"></button></code></pre>
<h4 id="Pagination-with-Sort" class="common-anchor-header">带排序的分页</h4><p>结合使用<code translate="no">order_by</code> 、<code translate="no">limit</code> 和<code translate="no">offset</code> ，即可对排序后的结果进行分页。例如，若要按价格排序并分多页显示产品列表，每页将按正确的价格顺序显示下一批商品，且不会出现重复或缺失的情况。</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#go">   Go</a>
...
<button class="copy-code-btn"></button></code></pre>
<h3 id="Aggregate-Query-Results" class="common-anchor-header">聚合查询结果<span class="beta-tag" style="background-color:rgb(0, 179, 255);color:white" translate="no">Compatible with Milvus 3.0.x</span><button data-href="#Aggregate-Query-Results" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>您可以按一个或多个标量字段对查询结果进行分组，并针对每个组计算聚合结果。支持的聚合操作符包括<code translate="no">count</code> 、<code translate="no">min</code> 、<code translate="no">max</code> 、<code translate="no">sum</code> 和<code translate="no">avg</code> 。</p>
...
<li><p><code translate="no">sum</code> 以及<code translate="no">avg</code> 仅支持数值型字段。您可以将其应用于数值型字段，包括<code translate="no">FLOAT</code> 和<code translate="no">DOUBLE</code> ，但若将其应用于<code translate="no">VARCHAR</code> 字段，则会返回错误。</p></li>
</ul>
...
<button class="copy-code-btn"></button></code></pre>
<p>您可以在单次调用中请求多个聚合表达式。以下示例按<code translate="no">color</code> 分组，并返回每个组中的实体数量、平均价格和最高评分：</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#go">   Go</a>
...
<button class="copy-code-btn"></button></code></pre>
<p>您还可以将<code translate="no">group_by_fields</code> 与<code translate="no">limit</code> 结合使用，以限制返回的组数。当某个字段具有高基数，而您只需获取部分组样本时，此方法非常有用：</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#go">   Go</a>
...
<h2 id="Use-QueryIterator" class="common-anchor-header">使用 QueryIterator<button data-href="#Use-QueryIterator" class="anchor-icon" translate="no">
    ...
    </button></h2><p>当您需要通过分页查询并采用自定义筛选条件来查找实体时，请创建一个<strong>QueryIterator</strong>并使用其<strong>next()</strong>方法遍历所有实体，以筛选出符合筛选条件的实体。以下代码示例假设存在三个名为<code translate="no">id</code> 、<code translate="no">vector</code> 和<code translate="no">color</code> 的字段，并返回所有<code translate="no">color</code> 字段值为<code translate="no">red</code> 开头的实体。</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#go">   Go</a>
...
<button class="copy-code-btn"></button></code></pre>
<h2 id="Queries-in-Partitions" class="common-anchor-header">分区查询<button data-href="#Queries-in-Partitions" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<h2 id="Random-Sampling-with-Query" class="common-anchor-header">使用 Query 进行随机采样<button data-href="#Random-Sampling-with-Query" class="anchor-icon" translate="no">
    ...
    </button></h2><p>若要从Collection中提取具有代表性的数据子集以用于数据探索或开发测试，请使用<code translate="no">RANDOM_SAMPLE(sampling_factor)</code> 表达式，其中<code translate="no">sampling_factor</code> 是一个介于0到1之间的浮点数，表示要抽取的数据百分比。</p>
<div class="alert note">
