...
title: 查询
summary: 在 Milvus 中，使用 Query、Get 和 QueryIterator 来检索实体并筛选元数据。
---
<h1 id="Query" class="common-anchor-header">查询<button data-href="#Query" class="anchor-icon" translate="no">
    ...
    </button></h1><p>除了 ANN 搜索外，Milvus 还支持通过查询进行元数据过滤。本页介绍如何使用 Query、Get 和 QueryIterators 来执行元数据过滤。</p>
<div class="alert note">
<p>如果在创建Collection后添加了新字段，包含这些字段的查询将返回已定义的默认值，或者对于未显式设置值的实体，返回<code translate="no">NULL</code> 。有关详细信息，请参阅<a href="/docs/zh/add-fields-to-an-existing-collection.md">“修改Collection Schema”</a>。</p>
</div>
<h2 id="Overview" class="common-anchor-header">概述<button data-href="#Overview" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Collection 可以存储各种类型的标量字段。您可以让 Milvus 根据一个或多个标量字段对实体进行过滤。Milvus 提供了三种查询类型：Query、Get 和 QueryIterator。下表对比了这三种查询类型。</p>
<table>
   <tr>
     <th></th>
     <th><p>Get</p></th>
   ...
   <tr>
     ...
     <td><ul><li><p>分区名称</p></li><li><p>总共要返回的实体数量</p></li><li><p>输出字段</p></li></ul></td>
   </tr>
   <tr>
     <td><p>返回</p></td>
     <td><p>返回指定Collection或分区中包含指定主键的实体。</p></td>
     <td><p>返回在指定Collection或分区中满足自定义筛选条件的全部或指定数量的实体。</p></td>
     <td><p>通过分页查询，返回在指定Collection或分区中满足自定义过滤条件的所有实体。</p></td>
   </tr>
</table>
<p>有关元数据过滤的更多信息，请参阅。</p>
<h2 id="Use-Get" class="common-anchor-header">使用 Get<button data-href="#Use-Get" class="anchor-icon" translate="no">
    ...
    </button></h2><p>当需要根据主键查找实体时，可以使用<strong>Get</strong>方法。以下代码示例假设您的 Collection 中有三个名为<code translate="no">id</code> 、<code translate="no">vector</code> 和<code translate="no">color</code> 的字段。</p>
...
<h2 id="Use-Query" class="common-anchor-header">使用查询<button data-href="#Use-Query" class="anchor-icon" translate="no">
    ...
    </button></h2><h3 id="Basic-Query" class="common-anchor-header">基本查询<button data-href="#Basic-Query" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>当您需要根据自定义筛选条件查找实体时，请使用<strong>Query</strong>方法。以下代码示例假设存在三个名为<code translate="no">id</code> 、<code translate="no">vector</code> 和<code translate="no">color</code> 的字段，并返回指定数量的实体，这些实体的<code translate="no">color</code> 值以<code translate="no">red</code> 开头。</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#go">   Go</a>
...
<li><p>按可为空字段排序时，升序排序（NULLS LAST）中 NULL 值位于末尾，降序排序（NULLS FIRST）中 NULL 值位于开头。</p></li>
</ul>
...
<button class="copy-code-btn"></button></code></pre>
<h4 id="Pagination-with-Sort" class="common-anchor-header">带排序的分页</h4><p>结合使用<code translate="no">order_by</code> 、<code translate="no">limit</code> 和<code translate="no">offset</code> ，即可对已排序的结果进行分页。例如，要按价格排序并分多页显示产品列表时，每页都会按正确的价格顺序显示下一批商品，且不会出现重复或缺漏。</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#go">   Go</a>
...
<h2 id="Use-QueryIterator" class="common-anchor-header">使用 QueryIterator<button data-href="#Use-QueryIterator" class="anchor-icon" translate="no">
    ...
    </button></h2><p>当您需要通过分页查询根据自定义过滤条件查找实体时，请创建一个<strong>QueryIterator</strong>，并使用其<strong>next()</strong>方法遍历所有实体，以查找符合过滤条件的实体。以下代码示例假设存在三个名为<code translate="no">id</code> 、<code translate="no">vector</code> 和<code translate="no">color</code> 的字段，并返回所有<code translate="no">color</code> 字段值以<code translate="no">red</code> 开头的实体。</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#go">   Go</a>
