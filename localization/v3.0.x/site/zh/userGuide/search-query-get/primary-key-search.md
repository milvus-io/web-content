...
summary: >-
  进行相似度搜索时，系统总是会要求您提供一个或多个查询向量，即使这些查询向量已经存在于目标Collection中也是如此。为了避免在搜索前检索向量，您可以改用主键。
beta: Milvus 2.6.9+
---
<h1 id="Primary-Key-Search" class="common-anchor-header">主键搜索<span class="beta-tag" style="background-color:rgb(0, 179, 255);color:white" translate="no">Compatible with Milvus 2.6.9+</span><button data-href="#Primary-Key-Search" class="anchor-icon" translate="no">
    ...
    </button></h1><p>进行相似度搜索时，系统总是要求您提供一个或多个查询向量，即使这些查询向量已经存在于目标Collection中也是如此。为了避免在搜索前检索向量，您可以改用主键。</p>
<h2 id="Overview" class="common-anchor-header">概述<button data-href="#Overview" class="anchor-icon" translate="no">
    ...
    </button></h2><p>在电子商务平台上，用户可以输入关键词来检索匹配的产品。当用户查看产品详情页时，平台还会在页面底部显示一份相似产品列表，供用户进行比较。</p>
<p>这些推荐商品是根据其与关键词或当前商品的相似度进行排序的。为了实现这一点，平台开发人员需要在实际进行相似度搜索之前，从 Milvus 中检索关键词或当前商品的向量表示，这会增加平台与 Milvus 之间的往返次数，并导致大量高维浮点数通过网络传输。</p>
<p>为简化应用程序与 Milvus 之间的交互逻辑、减少往返次数，并避免通过网络传输大量高维浮点数值，建议考虑使用主键搜索。</p>
<p>在主键搜索中，您无需提供任何查询向量。取而代之的是，系统会要求您提供包含查询向量的实体的主键（<code translate="no">ids</code> ）。</p>
<h2 id="Limits--restrictions" class="common-anchor-header">限制与约束<button data-href="#Limits--restrictions" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<li><p>使用主键的搜索适用于所有向量数据类型，但 BM25 函数中由 VarChar 字段派生的稀疏向量字段除外。</p></li>
<li><p>在过滤、范围和分组搜索中，您可以使用主键代替查询向量，并可选择启用分页功能。但此功能不适用于混合搜索和搜索迭代器。</p></li>
<li><p>对于涉及嵌入列表的相似度搜索，您仍需检索查询向量，将其排列成嵌入列表，然后运行搜索。</p></li>
<li><p>在 RESTful API 中，您无法使用主键代替查询向量。</p></li>
<li><p>对于任何不存在的主键或格式错误的主键，Milvus 会提示错误。</p></li>
<li><p>主键和查询向量互斥。同时提供两者也会导致错误。</p></li>
</ul>
<h2 id="Examples" class="common-anchor-header">示例<button data-href="#Examples" class="anchor-icon" translate="no">
    ...
    </button></h2><p>以下示例假设目标 Collection 中包含所有提供的 Int64 ID。</p>
...
<h3 id="Example-1-Basic-primary-key-search" class="common-anchor-header">示例 1：基本主键搜索<button data-href="#Example-1-Basic-primary-key-search" class="anchor-icon" translate="no">
    ...
    </button></h3><p>要进行基本的主键搜索，只需将查询向量替换为主键即可。</p>
...
<button class="copy-code-btn"></button></code></pre>
<h3 id="Example-2-Filtered-search-using-primary-keys" class="common-anchor-header">示例 2：使用主键进行过滤搜索<button data-href="#Example-2-Filtered-search-using-primary-keys" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>以下示例假设 color 和 likes 是目标 Collection 中两个由 Schema 定义的字段。</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#javascript">   NodeJS</a>
...
<button class="copy-code-btn"></button></code></pre>
<h3 id="Example-4-Grouping-search-using-primary-keys" class="common-anchor-header">示例 4：使用主键进行分组搜索<button data-href="#Example-4-Grouping-search-using-primary-keys" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>以下示例假设<code translate="no">docId</code> 是目标Collection中由Schema定义的字段。</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#javascript">   NodeJS</a>
