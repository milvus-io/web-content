...
<h2 id="Data-processing-with-external-collections" class="common-anchor-header">使用外部Collection进行数据处理<button data-href="#Data-processing-with-external-collections" class="anchor-icon" translate="no">
    ...
    </button></h2><p>快照可为分析或验证工作负载提供稳定的、特定时间点的数据源。对于 Milvus 快照，请使用<code translate="no">milvus-table</code> 外部 Collection 格式，而非直接将快照文件作为通用 Spark 输入进行读取。 Milvus快照存储了Collection元数据、分段清单、删除日志和主键统计信息，因此Milvus需要快照元数据JSON以及<code translate="no">milvus-table</code> 读取器，以保留正确的Schema和删除语义。</p>
<p>此工作流会在快照数据上创建一个可查询的外部 Collection。主列数据仍从快照源进行引用，而刷新操作会将源 StorageV3 清单映射到目标外部分段。</p>
