...
<h2 id="Drop-database" class="common-anchor-header">删除数据库<button data-href="#Drop-database" class="anchor-icon" translate="no">
    ...
    </button></h2><p>要删除一个数据库，必须先删除它的所有 Collections。否则，删除将失败。</p>
...
<li>如果 Milvus 连接指定了<code translate="no">db_name</code> ，但之后的 Permission API 调用没有指定，则<strong>数据库</strong>指的是 Milvus 连接中指定名称的数据库。</li>
