...
<p>此集成使用托管版的 Hugging Face 路由器。若要使用单独部署的文本嵌入推理（TEI）服务进行重新排序，请参阅<a href="/docs/zh/tei-ranker.md">TEI Ranker</a>。</p>
...
<li><code translate="no">queries</code> 中的字符串数量必须等于搜索查询的数量（<code translate="no">nq</code> ）。</li>
</ul>
...
<li><strong>对候选文本进行重新排序。</strong>Hugging Face 为每个候选文本返回一个相似度分数。Milvus 根据分数从高到低对候选文本进行排序，并返回重新排序后的结果。</li>
</ol>
...
<li>当前由<code translate="no">hf-inference</code> 托管的、用于 <a href="https://huggingface.co/tasks/sentence-similarity"><code translate="no">sentence-similarity</code></a> 任务提供服务的模型。</li>
...
<h3 id="Step-3-Search-with-the-ranker" class="common-anchor-header">步骤 3：使用排名器进行搜索<button data-href="#Step-3-Search-with-the-ranker" class="anchor-icon" translate="no">
    ...
    </button></h3><p>将该函数作为<code translate="no">search()</code> 的<code translate="no">ranker</code> 参数传入：</p>
...
<li>有关共享模型排名器的行为和限制，请参阅《<a href="/docs/zh/model-ranker-overview.md">模型排名器概述</a>》。</li>
<li>若要通过托管的 Hugging Face 推理提供程序生成 Embeddings，请参阅<a href="/docs/zh/hugging-face.md">Hugging Face</a>。</li>
<li>若要将排序器应用于混合搜索，请参阅《<a href="/docs/zh/multi-vector-search.md">多向量混合搜索</a>》。</li>
</ul>
