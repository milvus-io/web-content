...
<h1 id="Run-Milvus-with-GPU-Support-Using-Docker-Compose" class="common-anchor-header">使用 Docker Compose 运行支持 GPU 的 Milvus<button data-href="#Run-Milvus-with-GPU-Support-Using-Docker-Compose" class="anchor-icon" translate="no">
    ...
    </button></h1><p>本页面演示了如何使用 Docker Compose 启动支持 GPU 的 Milvus 实例。</p>
...
<h3 id="1-Download-and-configure-the-YAML-file" class="common-anchor-header">1. 下载并配置 YAML 文件<button data-href="#1-Download-and-configure-the-YAML-file" class="anchor-icon" translate="no">
    ...
    </button></h3><p>下载 <a href="https://github.com/milvus-io/milvus/releases/download/v3.0-beta/milvus-standalone-docker-compose-gpu.yml"><code translate="no">milvus-standalone-docker-compose-gpu.yml</code></a> 并将其手动保存为 docker-compose.yml，或使用以下命令。</p>
...
<button class="copy-code-btn"></button></code></pre>
<div class="alert note">
<p>如果无法运行上述命令，请检查您的系统是否安装了 Docker Compose V1。如果是这种情况，建议您根据<a href="https://docs.docker.com/compose/">本页</a>的说明迁移至 Docker Compose V2。</p>
</div>
<p>Milvus 启动后，</p>
<ul>
...
<li><strong>milvus-minio</strong>容器在本地通过默认身份验证凭据提供<strong>9090</strong>和<strong>9091</strong>端口服务，并将数据映射到当前文件夹中的<strong>volumes/minio</strong>。</li>
<li><strong>Milvus Standalone</strong>容器在本地通过默认设置提供<strong>19530</strong>端口服务，并将数据映射到当前目录下的<strong>volumes/milvus</strong>卷中。</li>
</ul></li>
</ul>
...
<button class="copy-code-btn"></button></code></pre>
<p>您还可以通过<code translate="no">http://127.0.0.1:9091/webui/</code> 访问 Milvus WebUI，以进一步了解您的 Milvus 实例。有关详细信息，请参阅<a href="/docs/zh/milvus-webui.md">Milvus WebUI</a>。</p>
...
<ol>
<li><p>使用以下命令将<code translate="no">milvus.yaml</code> 从 Milvus 容器复制到本地机器。请将<code translate="no">&lt;milvus_container_id&gt;</code> 替换为您实际的 Milvus 容器 ID。</p>
...
<button class="copy-code-btn"></button></code></pre></li>
<li><p>根据需要编辑<code translate="no">initMemSize</code> 和<code translate="no">maxMemSize</code> 的设置，并保存更改：</p>
<pre><code translate="no" class="language-yaml"><span class="hljs-string">...</span>
...
<li><p>查看<a href="/docs/zh/milvus-webui.md">Milvus WebUI</a>以进一步了解 Milvus 实例。</p></li>
...
<li><p>了解<a href="/docs/zh/birdwatcher_overview.md">Birdwatcher</a>——一款用于 Milvus 调试和动态配置更新的开源工具。</p></li>
