...
<h3 id="1-Download-and-configure-the-YAML-file" class="common-anchor-header">1. 下载并配置 YAML 文件<button data-href="#1-Download-and-configure-the-YAML-file" class="anchor-icon" translate="no">
    ...
    </button></h3><p>下载 <a href="https://github.com/milvus-io/milvus/releases/download/v3.0.2/milvus-standalone-docker-compose-gpu.yml"><code translate="no">milvus-standalone-docker-compose-gpu.yml</code></a> 并手动将其保存为 docker-compose.yml，或使用以下命令进行操作。</p>
...
<ul>
<li>若要为 Milvus 分配特定的 GPU 设备，请在<code translate="no">standalone</code> 服务的定义中找到 `<code translate="no">deploy.resources.reservations.devices[0].devices_ids</code> ` 字段，并将该字段的值替换为所需 GPU 的 ID。您可以使用 NVIDIA GPU 显示驱动程序自带的<code translate="no">nvidia-smi</code> 工具来确定 GPU 设备的 ID。Milvus 支持多个 GPU 设备。</li>
</ul>
...
<button class="copy-code-btn"></button></code></pre>
<div class="alert note">
<p>如果无法运行上述命令，请检查您的系统是否安装了 Docker Compose V1。如果是这种情况，建议您根据<a href="https://docs.docker.com/compose/">本页</a>的说明迁移到 Docker Compose V2。</p>
</div>
<p>Milvus 启动后，</p>
<ul>
...
<li><strong>milvus-minio</strong>容器在本地提供<strong>9090</strong>和<strong>9091</strong>端口服务，使用默认身份验证凭据，并将数据映射到当前文件夹下的<strong>volumes/minio</strong>。</li>
...
<p>如果您在 docker-compose.yml 中为 Milvus 分配了多个 GPU 设备，则可以指定哪个 GPU 设备对 Milvus 可见或可供使用。</p>
...
<li><p>探索<a href="/docs/zh/milvus_backup_overview.md">Milvus Backup</a>，这是一款用于 Milvus 数据备份的开源工具。</p></li>
<li><p>了解<a href="/docs/zh/birdwatcher_overview.md">Birdwatcher</a>——一款用于调试 Milvus 并动态更新配置的开源工具。</p></li>
