...
<p>然后按照以下步骤从仓库获取 Milvus Helm 图表：</p>
<pre><code translate="no"><span class="hljs-variable">$ </span>helm repo update
<button class="copy-code-btn"></button></code></pre>
<p>您可以随时运行以下命令以获取最新的 Milvus Helm Charts。</p>
...
<p>以下命令将部署一个针对 v3.0.1 进行过优化的 Milvus 集群，并使用 Woodpecker 作为推荐的消息队列：</p>
<pre><code translate="no" class="language-bash">helm install my-release zilliztech/milvus \
  --<span class="hljs-built_in">set</span> image.all.tag=v3.0.1 \
  --<span class="hljs-built_in">set</span> woodpecker.enabled=<span class="hljs-literal">true</span> \
...
<ul>
<li>使用<strong>Woodpecker</strong>作为消息队列（建议使用以减少维护工作）</li>
<li>将<strong>Woodpecker 作为专用服务</strong>（单独的 StatefulSet）运行，而非嵌入流式节点中</li>
<li>启用新的<strong>流式节点</strong>组件以提升性能</li>
...
<ul>
<li><strong>版本命名</strong>：仅允许使用字母、数字和连字符（不允许使用点）</li>
</ul>
...
<button class="copy-code-btn"></button></code></pre>
<p><strong>等待所有 Pod 显示“Running”状态。</strong>使用 v3.0.1 配置时，您应看到类似以下的 Pod：</p>
...
<h3 id="3-Connect-to-Milvus" class="common-anchor-header">3. 连接到 Milvus<button data-href="#3-Connect-to-Milvus" class="anchor-icon" translate="no">
    ...
    </button></h3><p>若要在 Kubernetes 外部连接到您的 Milvus 集群，您需要配置端口转发。</p>
...
<h2 id="Access-Milvus-WebUI" class="common-anchor-header">访问 Milvus WebUI<button data-href="#Access-Milvus-WebUI" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Milvus 随附了一个名为 Milvus WebUI 的内置图形化用户界面工具，您可通过浏览器访问该工具。Milvus WebUI 通过简单直观的界面增强了系统的可观察性。您可以使用 Milvus WebUI 观察 Milvus 组件及依赖项的统计信息和指标，查看数据库和 Collection 的详细信息，并列出详细的 Milvus 配置。 有关 Milvus WebUI 的详细信息，请参阅<a href="/docs/zh/milvus-webui.md">Milvus WebUI</a></p>
...
<li>若要更改 Milvus 配置，请下载 <a href="https://raw.githubusercontent.com/milvus-io/milvus-helm/master/charts/milvus/values.yaml"><code translate="no">value.yaml</code></a> 模板，在其中填写所需设置，然后使用<code translate="no">helm template -f values.yaml my-release zilliztech/milvus &gt; milvus_manifest.yaml</code> 据此生成相应的清单。</li>
</ul>
</div>
<h3 id="2-Download-image-pulling-script" class="common-anchor-header">2. 下载镜像拉取脚本<button data-href="#2-Download-image-pulling-script" class="anchor-icon" translate="no">
    ...
    </button></h3><p>镜像拉取脚本采用 Python 编写。您应将该脚本及其依赖项一并下载到<code translate="no">requirement.txt</code> 文件中。</p>
...
<ul>
<li><p>查看<a href="/docs/zh/quickstart.md">“Hello</a>Milvus”了解 Milvus 的功能。</p></li>
...
<li><p>探索<a href="https://github.com/zilliztech/attu">Attu</a>——一款用于直观管理 Milvus 的开源图形化界面工具。</p></li>
