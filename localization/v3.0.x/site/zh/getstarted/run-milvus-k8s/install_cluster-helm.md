...
<h1 id="Run-Milvus-in-Kubernetes-with-Helm" class="common-anchor-header">使用 Helm 在 Kubernetes 中运行 Milvus<button data-href="#Run-Milvus-in-Kubernetes-with-Helm" class="anchor-icon" translate="no">
    ...
    </button></h1><p>本页面介绍了如何使用<a href="https://github.com/zilliztech/milvus-helm">Milvus Helm 图表在</a> Kubernetes 中启动一个 Milvus 实例。</p>
...
<li><p><a href="/docs/zh/prerequisite-helm.md#How-can-I-start-a-K8s-cluster-locally-for-test-purposes">创建一个 K8s 集群</a>。</p></li>
<li><p>安装<a href="https://kubernetes.io/docs/tasks/administer-cluster/change-default-storage-class/">StorageClass</a>。您可以按以下方式查看已安装的 StorageClass。</p>
...
<p>如果拉取镜像时遇到任何问题，请将问题详情发送至<a href="mailto:community@zilliz.com">community@zilliz.com</a>联系我们，我们将为您提供必要的支持。</p>
</div>
...
<div class="alert note">
<p>位于<code translate="no">https://github.com/milvus-io/milvus-helm</code> 的 Milvus Helm Charts 仓库现已归档。我们现在使用新仓库<code translate="no">https://github.com/zilliztech/milvus-helm</code> 。归档的仓库仍可提供 4.0.31 及更早版本的图表，但后续版本请使用新仓库。</p>
</div>
...
<h2 id="Online-install" class="common-anchor-header">在线安装<button data-href="#Online-install" class="anchor-icon" translate="no">
    ...
    </button></h3><p>安装 Helm 图表后，您即可在 Kubernetes 上启动 Milvus。本节将指导您部署 Milvus 集群。</p>
<div class="alert note" id="standalone-deployment-note">
<p><strong>需要独立部署吗？</strong></p>
<p>如果您希望以独立模式（单节点）部署 Milvus 用于开发或测试，请使用以下命令：</p>
...
<button class="copy-code-btn"></button></code></pre>
<p><strong>注意</strong>：独立模式默认使用 Woodpecker 作为消息队列，并启用流式处理节点（Streaming Node）组件。独立部署中，Woodpecker<strong>嵌入在</strong>Milvus Pod 中运行；而专用的 Woodpecker<strong>服务</strong>（独立 Pod）仅用于<strong>分布式/集群部署</strong>。详情请参阅《<a href="/docs/zh/architecture_overview.md">架构概述</a>》和<a href="/docs/zh/woodpecker.md">《Woodpecker》</a>。</p>
</div>
<p><strong>部署 Milvus 集群：</strong></p>
<div class="alert note">
<p>对于 Woodpecker 服务模式，我们建议使用即将发布的 Milvus 3.0.1 或更高版本，并搭配 Woodpecker v0.1.37 或更高版本，以获得紧凑化清理和组提交优化功能。</p>
</div>
<p>以下命令将部署一个针对 v3.0.2 进行过优化的 Milvus 集群，并使用 Woodpecker 作为推荐的消息队列：</p>
<pre><code translate="no" class="language-bash">helm install my-release zilliztech/milvus \
  --<span class="hljs-built_in">set</span> image.all.tag=v3.0.2 \
  --<span class="hljs-built_in">set</span> woodpecker.enabled=<span class="hljs-literal">true</span> \
...
<li>将<strong>Woodpecker 作为专用服务</strong>（独立的 StatefulSet）运行，而非嵌入流式处理节点中</li>
<li>启用新的<strong>流式处理节点</strong>组件以提升性能</li>
<li>禁用旧版<strong>索引节点</strong>（其功能现由数据节点处理）</li>
</ul>
<div class="alert note">
...
<p><strong>替代消息队列：</strong>若要使用 Pulsar、Kafka 或 RocksMQ 代替 Woodpecker 进行部署，请参阅<a href="#Optional-dependencies">“可选依赖项”</a>。</p>
<p><strong>后续步骤：</strong>
上述命令将使用推荐配置部署 Milvus。若用于生产环境：</p>
<ul>
<li>请使用<a href="https://milvus.io/tools/sizing">Milvus 容量规划工具</a>，根据您的数据规模优化设置</li>
...
<h3 id="2-Check-Milvus-cluster-status" class="common-anchor-header">2. 检查 Milvus 集群状态<button data-href="#2-Check-Milvus-cluster-status" class="anchor-icon" translate="no">
    ...
    </button></h3><p>通过检查 Pod 状态来验证部署是否成功：</p>
<pre><code translate="no" class="language-bash">kubectl get pods
<button class="copy-code-btn"></button></code></pre>
<p><strong>等待所有 Pod 显示“Running”状态。</strong>采用 v3.0.2 配置时，您应看到类似以下的 Pod：</p>
...
<button class="copy-code-btn"></button></code></pre>
<p><strong>需验证的关键组件：</strong></p>
<ul>
<li><strong>Milvus 组件</strong>：<code translate="no">mixcoord</code> 、<code translate="no">datanode</code> 、<code translate="no">querynode</code> 、<code translate="no">proxy</code> 、<code translate="no">streamingnode</code></li>
...
<div class="alert note">
<p>使用<code translate="no">streaming.woodpecker.embedded=false</code> 时，Woodpecker<strong>作为专用的 StatefulSet</strong>运行（<code translate="no">my-release-milvus-woodpecker</code> ，默认 4 个副本——3 个节点构成定额，外加一个备用节点以实现容错； 请勿将<code translate="no">woodpecker.replicaCount</code> 设置为小于 3），由一个无头服务作为前端，并使用 MinIO 作为其存储后端——因此该集群拥有独立的<code translate="no">woodpecker</code> Pod 集，与流式处理节点相互独立。</p>
</div>
...
<h3 id="3-Connect-to-Milvus" class="common-anchor-header">3. 连接到 Milvus<button data-href="#3-Connect-to-Milvus" class="anchor-icon" translate="no">
    ...
    </button></h3><p>若要在 Kubernetes 外部连接到您的 Milvus 集群，您需要设置端口转发。</p>
<p><strong>设置端口转发：</strong></p>
<pre><code translate="no" class="language-bash">kubectl port-forward service/my-release-milvus 27017:19530
<button class="copy-code-btn"></button></code></pre>
<p>此命令将本地端口<code translate="no">27017</code> 转发至 Milvus 端口<code translate="no">19530</code> 。您应看到：</p>
...
<button class="copy-code-btn"></button></code></pre></li>
<li><strong>独立部署</strong>：若使用独立模式，服务名称保持不变</li>
</ul>
</div>
<p>使用 Milvus 时请<strong>保持此终端窗口打开</strong>。现在，您可以使用任何 Milvus SDK 通过<code translate="no">localhost:27017</code> 连接到 Milvus。</p>
...
<h2 id="Access-Milvus-WebUI" class="common-anchor-header">访问 Milvus WebUI<button data-href="#Access-Milvus-WebUI" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Milvus 随附了一个名为 Milvus WebUI 的内置图形化用户界面工具，您可以通过浏览器访问该工具。Milvus WebUI 通过简单直观的界面增强了系统的可观察性。您可以使用 Milvus WebUI 观察 Milvus 组件和依赖项的统计数据及指标，查看数据库和 Collection 的详细信息，并列出详细的 Milvus 配置。 有关 Milvus WebUI 的详细信息，请参阅<a href="/docs/zh/milvus-webui.md">Milvus WebUI</a></p>
...
<h3 id="1-Get-Milvus-manifest" class="common-anchor-header">1. 获取 Milvus 配置清单<button data-href="#1-Get-Milvus-manifest" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>运行以下命令获取 Milvus 配置文件。</p>
...
<li>若要修改 Milvus 配置，请下载 <a href="https://raw.githubusercontent.com/milvus-io/milvus-helm/master/charts/milvus/values.yaml"><code translate="no">value.yaml</code></a> 模板，在其中填写所需的设置，然后使用<code translate="no">helm template -f values.yaml my-release zilliztech/milvus &gt; milvus_manifest.yaml</code> 据此生成相应的清单。</li>
</ul>
</div>
...
<button class="copy-code-btn"></button></code></pre>
<h3 id="3-Pull-and-save-images" class="common-anchor-header">3. 拉取并保存镜像<button data-href="#3-Pull-and-save-images" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<h3 id="4-Load-images" class="common-anchor-header">4. 加载图像<button data-href="#4-Load-images" class="anchor-icon" translate="no">
    ...
    </button></h3><p>现在，您可以按照以下步骤将图像加载到网络受限环境中的主机上：</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>至此，您可以按照在线安装指南中的步骤<a href="#2-Check-Milvus-cluster-status">2</a>和<a href="#3-Connect-to-Milvus">3</a>检查集群状态，并将本地端口转发至 Milvus。</p>
<h2 id="Upgrade-running-Milvus-cluster" class="common-anchor-header">升级正在运行的 Milvus 集群<button data-href="#Upgrade-running-Milvus-cluster" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>运行以下命令，将正在运行的 Milvus 集群升级至最新版本：</p>
...
<ul>
<li><p><a href="/docs/zh/quickstart.md">查看“Hello Milvus</a>”以了解 Milvus 的功能。</p></li>
...
<li><p>探索<a href="/docs/zh/milvus-webui.md">Milvus WebUI，</a>这是一个用于 Milvus 可观测性和管理的直观 Web 界面。</p></li>
