...
<p><strong>v3.0.2 版本更新内容：</strong></p>
<ul>
...
<p>若在拉取镜像时遇到任何问题，请通过<a href="mailto:community@zilliz.com">community@zilliz.com</a>联系我们，并附上详细问题描述，我们将为您提供必要的支持。</p>
</div>
<p>运行安装脚本后：</p>
<ul>
<li>一个名为 Milvus Standalone 的 Docker 容器已在<strong>19530</strong> 端口启动。</li>
<li>一个嵌入式 etcd 已随 Milvus 一起安装在同一容器中，并监听<strong>2379</strong> 端口。其配置文件映射到当前文件夹中的<strong>embedEtcd.yaml</strong>。</li>
<li>若要修改 Milvus 的默认配置，请在当前目录的<strong>user.yaml</strong>文件中添加您的设置，然后重启服务。</li>
...
<h2 id="Upgrade-Milvus" class="common-anchor-header">升级 Milvus<button data-href="#Upgrade-Milvus" class="anchor-icon" translate="no">
    ...
    </button></h2><p>您可以使用内置的升级命令将 Milvus 升级至最新版本。该命令会自动下载最新的配置和 Milvus 镜像：</p>
...
<p>这是升级 Milvus Standalone 部署的推荐方法。</p>
</div>
...
<h2 id="Optional-dependencies" class="common-anchor-header">可选依赖项<button data-href="#Optional-dependencies" class="anchor-icon" translate="no">
    ...
    </button></h2><p>默认情况下，此部署运行<strong>Woodpecker</strong>（本地文件系统 WAL）作为消息队列，并使用<strong>嵌入式 etcd</strong>管理元数据——无需安装其他组件。若要使用其他消息队列或连接外部对象存储/元数据，请参阅：</p>
<ul>
...
<li><a href="/docs/zh/insert-update-delete.md">插入、更新或插入（Upsert）及删除</a></li>
