...
<p><strong>默认部署（v3.0.0）：</strong> <code translate="no">docker compose up -d</code> 将启动三个容器——<code translate="no">milvus-etcd</code> （元数据）、<code translate="no">milvus-minio</code> （对象存储）和<code translate="no">milvus-standalone</code> 。消息队列使用<strong>Woodpecker（嵌入式，以 MinIO / 对象存储作为其 WAL 后端）</strong>，因此无需单独的消息队列容器。</p>
...
<p>请务必下载最新的 Docker Compose 配置文件，以确保与 v3.0.0 版本的功能兼容。</p>
<ul>
