...
<li>安装前<a href="/docs/zh/prerequisite-docker.md">请确认硬件和软件要求</a>。</li>
</ul>
...
<p><strong>默认部署（v3.0.1）：</strong> <code translate="no">docker compose up -d</code> 将启动三个容器——<code translate="no">milvus-etcd</code> （元数据）、<code translate="no">milvus-minio</code> （对象存储）和<code translate="no">milvus-standalone</code> 。消息队列使用的是<strong>Woodpecker（嵌入式，以 MinIO / 对象存储作为其 WAL 后端）</strong>，因此无需单独的消息队列容器。</p>
...
<p>请务必下载最新的 Docker Compose 配置文件，以确保与 v3.0.1 功能兼容。</p>
<ul>
...
<li><p>若在拉取镜像时遇到任何问题，请将详细情况发送至<a href="mailto:community@zilliz.com">community@zilliz.com</a>联系我们，我们将为您提供必要的支持。</p></li>
</ul>
</div>
<p>启动 Milvus 后，</p>
<ul>
...
<li><strong>Milvus Standalone</strong>容器在本地通过默认设置提供<strong>19530</strong>端口服务，并将数据映射到当前目录下的<strong>volumes/milvus</strong>目录中。</li>
</ul></li>
</ul>
...
<button class="copy-code-btn"></button></code></pre>
<p>您还可以访问 Milvus WebUI（<code translate="no">http://127.0.0.1:9091/webui/</code> ）以了解有关您的 Milvus 实例的更多信息。有关详细信息，请参阅<a href="/docs/zh/milvus-webui.md">Milvus WebUI</a>。</p>
...
<li><p>添加额外配置以覆盖默认设置。
下文假设您需要覆盖默认的<code translate="no">proxy.healthCheckTimeout</code> 。有关适用的配置项，请<a href="/docs/zh/system_configuration.md">参阅系统配置</a>。</p>
...
<h2 id="Upgrading-from-Milvus-25x-to-26x" class="common-anchor-header">从 Milvus 2.5.x 升级到 2.6.x<button data-href="#Upgrading-from-Milvus-25x-to-26x" class="anchor-icon" translate="no">
    ...
    </button></h2><p><strong>消息队列限制</strong>：升级至 Milvus v3.0.1 时，必须保留当前的消息队列选择。升级过程中不支持在不同的消息队列系统之间切换。未来版本将支持更改消息队列系统。</p>
...
<h2 id="Optional-dependencies" class="common-anchor-header">可选依赖项<button data-href="#Optional-dependencies" class="anchor-icon" translate="no">
    ...
    </button></h2><p>此部署使用<strong>Woodpecker</strong>（嵌入式，MinIO WAL 后端）进行消息传递，使用<strong>etcd</strong>管理元数据，并使用<strong>MinIO</strong>作为对象存储。若要使用其他消息队列或连接外部对象存储/元数据，请参阅：</p>
<ul>
...
<div class="alert note">
<p>Storage V3 默认处于禁用状态。在使用依赖该功能的相关特性前，请先启用它。有关要求和兼容性注意事项，请参阅<a href="/docs/zh/storage-v3.md">Storage V3</a>。</p>
</div>
