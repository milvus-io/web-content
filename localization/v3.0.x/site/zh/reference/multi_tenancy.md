...
<h3 id="Database-level-multi-tenancy" class="common-anchor-header">数据库级多租户<button data-href="#Database-level-multi-tenancy" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>使用数据库级多租户，每个租户都会收到一个包含一个或多个 Collections 的相应<a href="/docs/zh/manage_databases.md">数据库</a>。</p>
<p>
...
<h3 id="Collection-level-multi-tenancy" class="common-anchor-header">Collections 级多租户功能<button data-href="#Collection-level-multi-tenancy" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>有了 Collection 级多租户功能，每个租户都会被分配到一个<a href="/docs/zh/manage-collections.md">Collection</a>，从而提供强大的数据隔离功能。</p>
<p>
...
<h3 id="Partition-level-multi-tenancy" class="common-anchor-header">分区级多租户<button data-href="#Partition-level-multi-tenancy" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>在分区级多租户中，每个租户都被分配到共享 Collections 中手动创建的<a href="/docs/zh/manage-partitions.md">分区</a>。</p>
<p>
...
<ul>
<li><p><strong>可扩展性</strong>：每个 Collection 最多可容纳 1,024 个分区，允许在其中容纳相同数量的租户。</p></li>
...
<h3 id="Partition-key-level-multi-tenancy" class="common-anchor-header">Partition Key 级多租户<button data-href="#Partition-key-level-multi-tenancy" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>采用这种策略，所有租户共享一个 Collections 和 Schema，但每个租户的数据会根据<a href="/docs/zh/use-partition-key.md">分区键值</a>自动路由到 16 个物理隔离的分区中。虽然每个物理分区可以包含多个租户，但不同租户的数据在逻辑上是分开的。</p>
<p>
...
<li><p><strong>其他</strong>：在分区 Key 层不支持 RBAC。租户既可以单独查询，也可以跨多个分区查询，因此这种方法非常适合涉及跨租户分区的聚合查询或分析的场景。</p></li>
</ul>
