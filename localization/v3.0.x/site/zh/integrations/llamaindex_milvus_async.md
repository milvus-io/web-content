...
title: 使用 LlamaIndex 和 Milvus Async API 构建 RAG
related_key: LlamaIndex
summary: >-
  本教程演示了如何结合使用 LlamaIndex 和 Milvus，为 RAG 构建异步文档处理管道。LlamaIndex 提供了一种处理文档并将其存储到
  Milvus 等向量数据库中的方法。 通过利用 LlamaIndex 的异步 API 以及 Milvus Python
  客户端库，我们可以提高管道的吞吐量，从而高效地处理和索引海量数据。
---
<h1 id="RAG-with-Milvus-and-LlamaIndex-Async-API" class="common-anchor-header">基于 Milvus 和 LlamaIndex 异步 API 的 RAG<button data-href="#RAG-with-Milvus-and-LlamaIndex-Async-API" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<p>本教程演示了如何结合使用<a href="https://www.llamaindex.ai/">LlamaIndex</a>和<a href="https://milvus.io/">Milvus，</a>为 RAG 构建异步文档处理管道。LlamaIndex 提供了一种处理文档并将其存储在 Milvus 等向量数据库中的方法。 通过利用 LlamaIndex 的异步 API 以及 Milvus Python 客户端库，我们可以提升管道的吞吐量，从而高效地处理和索引海量数据。</p>
<p>在本教程中，我们将首先从高层次介绍如何使用异步方法基于 LlamaIndex 和 Milvus 构建 RAG，随后介绍低层次方法的使用，并进行同步与异步的性能对比。</p>
<h2 id="Before-you-begin" class="common-anchor-header">开始之前<button data-href="#Before-you-begin" class="anchor-icon" translate="no">
    ...
    </button></h2><p>本页面的代码片段需要 pymilvus 和 llamaindex 依赖项。您可以使用以下命令进行安装：</p>
...
<p>我们将使用 OpenAI 的模型。您应将<a href="https://platform.openai.com/docs/quickstart">API 密钥</a> <code translate="no">OPENAI_API_KEY</code> 设置为环境变量。</p>
<pre><code translate="no" class="language-python"><span class="hljs-keyword">import</span> os

os.environ[<span class="hljs-string">&quot;OPENAI_API_KEY&quot;</span>] = <span class="hljs-string">&quot;sk-***********&quot;</span>
<button class="copy-code-btn"></button></code></pre>
<p>如果您使用的是 Jupyter Notebook，在运行异步代码之前，需要先执行以下这行代码。</p>
...
<button class="copy-code-btn"></button></code></pre>
<h2 id="Build-RAG-with-Asynchronous-Processing" class="common-anchor-header">构建支持异步处理的 RAG<button data-href="#Build-RAG-with-Asynchronous-Processing" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>本节将介绍如何构建一个能够以异步方式处理文档的 RAG 系统。</p>
<p>导入必要的库，并定义 Milvus URI 以及 Embeddings 的维度。</p>
...
<div class="alert note">
<ul>
<li>如果您拥有海量数据，可以在<a href="https://milvus.io/docs/quickstart.md">Docker 或 Kubernetes</a> 上搭建高性能的 Milvus 服务器。在此配置中，请使用服务器 URI（例如<code translate="no">http://localhost:19530</code> ）作为您的<code translate="no">uri</code> 。</li>
<li>若要使用<a href="https://zilliz.com/cloud">Zilliz Cloud</a>（Milvus 的全托管云服务），请调整<code translate="no">uri</code> 和<code translate="no">token</code> ，使其与 Zilliz Cloud 中的<a href="https://docs.zilliz.com/docs/on-zilliz-cloud-console#free-cluster-details">公共端点和 API 密钥</a>相对应。</li>
<li>对于复杂系统（例如网络通信），与同步处理相比，异步处理可以提升性能。因此，我们认为 Milvus-Lite 不适合使用异步接口，因为其应用场景并不适合。</li>
</ul>
</div>
<p>定义一个初始化函数，以便我们能够再次使用它来重建 Milvus 集合。</p>
...
<p>使用 SimpleDirectoryReader 封装来自文件<code translate="no">paul_graham_essay.txt</code> 的 LlamaIndex 文档对象。</p>
...
<p>在本地实例化一个 Hugging Face Embeddings 模型。使用本地模型可以避免在异步插入数据时触及 API 速率限制的风险，因为并发 API 请求会迅速累积，从而耗尽您在公共 API 中的配额。不过，如果您拥有较高的速率限制，也可以选择使用远程模型服务。</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>在构建查询引擎时，您还可以将<code translate="no">use_async</code> 参数设置为<code translate="no">True</code> ，以启用异步搜索。</p>
...
<h2 id="Explore-the-Async-API" class="common-anchor-header">探索异步 API<button data-href="#Explore-the-Async-API" class="anchor-icon" translate="no">
    ...
    </button></h2><p>在本节中，我们将介绍底层 API 的使用方法，并比较同步和异步运行的性能。</p>
...
<p>让我们定义一个生成节点的函数，该函数将用于为索引生成大量测试节点。</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>定义一个异步函数，用于向向量存储中添加文档。我们使用 Milvus 向量存储实例中的 `<code translate="no">async_add()</code> ` 函数。</p>
...
<h4 id="Compare-with-synchronous-add" class="common-anchor-header">与同步添加进行比较</h4><p>定义一个同步添加函数。然后在相同条件下测量其运行时间。</p>
...
<h3 id="Async-search" class="common-anchor-header">异步搜索<button data-href="#Async-search" class="anchor-icon" translate="no">
    ...
    </button></h3><p>在执行搜索之前，重新初始化向量存储并添加一些文档。</p>
<pre><code translate="no" class="language-python">vector_store = init_vector_store()
...
<h4 id="Compare-with-synchronous-search" class="common-anchor-header">与同步搜索进行比较</h4><p>定义一个同步搜索函数。然后在相同条件下测量运行时间。</p>
