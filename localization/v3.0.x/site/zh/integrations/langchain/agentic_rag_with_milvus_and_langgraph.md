...
<h1 id="Agentic-RAG-with-Milvus-and-LangGraph" class="common-anchor-header">使用 Milvus 和 LangGraph 的 Agents RAG<button data-href="#Agentic-RAG-with-Milvus-and-LangGraph" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<li>对检索到的文档进行相关性分级</li>
<li>当检索到的文档不相关时重写问题</li>
<li>根据相关上下文生成高质量的答案</li>
</ul>
...
<blockquote>
<p>如果使用的是 Google Colab，要启用刚刚安装的依赖项，可能需要<strong>重新启动运行时</strong>（点击屏幕顶部的 "运行时 "菜单，从下拉菜单中选择 "重新启动会话"）。</p>
</blockquote>
...
<h2 id="Prepare-the-data" class="common-anchor-header">准备数据<button data-href="#Prepare-the-data" class="anchor-icon" translate="no">
    ...
    </button></h2><p>我们使用 Langchain<a href="https://python.langchain.com/docs/integrations/document_loaders/web_base/">WebBaseLoader</a>从<a href="https://lilianweng.github.io/">Lilian Weng 的博文</a>中加载文档，并使用<a href="https://python.langchain.com/docs/how_to/recursive_text_splitter/">RecursiveCharacterTextSplitter</a> 将它们分割成块。</p>
...
<h2 id="Create-a-retriever-tool-with-Milvus" class="common-anchor-header">使用 Milvus 创建一个检索工具<button data-href="#Create-a-retriever-tool-with-Milvus" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>现在，我们将使用 Milvus 创建一个向量存储来索引我们的文档块，并创建一个我们的 Agents 可以使用的检索工具。</p>
...
<ul>
<li>将<code translate="no">uri</code> 设置为本地文件，如<code translate="no">./milvus_agentic_rag.db</code> ，是最方便的方法，因为它会自动利用<a href="https://milvus.io/docs/milvus_lite.md">Milvus Lite</a>将所有数据存储在此文件中。</li>
...
<h2 id="Build-the-agentic-RAG-graph" class="common-anchor-header">构建 Agents RAG 图<button data-href="#Build-the-agentic-RAG-graph" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><h3 id="Define-the-graph-state" class="common-anchor-header">定义图状态<button data-href="#Define-the-graph-state" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>我们将使用 LangGraph 的<code translate="no">MessagesState</code> 来维护对话中的消息列表。</p>
...
<button class="copy-code-btn"></button></code></pre>
<h3 id="Node-1-Generate-query-or-respond" class="common-anchor-header">节点 1：生成查询或响应<button data-href="#Node-1-Generate-query-or-respond" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>该节点决定是使用检索工具搜索信息，还是直接回复用户。</p>
...
<h3 id="Node-2-Grade-documents" class="common-anchor-header">节点 2：对文档进行分级<button data-href="#Node-2-Grade-documents" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>该节点评估检索到的文档是否与用户的问题相关。</p>
...
<button class="copy-code-btn"></button></code></pre>
<h3 id="Node-3-Rewrite-question" class="common-anchor-header">节点 3：重写问题<button data-href="#Node-3-Rewrite-question" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>如果文档不相关，该节点会重写问题，以改善检索结果。</p>
...
<button class="copy-code-btn"></button></code></pre>
<h3 id="Node-4-Generate-answer" class="common-anchor-header">节点 4：生成答案<button data-href="#Node-4-Generate-answer" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>该节点根据检索到的相关文档生成最终答案。</p>
<pre><code translate="no" class="language-python"><span class="hljs-keyword">def</span> <span class="hljs-title function_">generate</span>(<span class="hljs-params">state: MessagesState</span>):
    ...
    <span class="hljs-keyword">return</span> {<span class="hljs-string">&quot;messages&quot;</span>: [response]}
<button class="copy-code-btn"></button></code></pre>
<h3 id="Assemble-the-graph" class="common-anchor-header">组装图<button data-href="#Assemble-the-graph" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>现在，我们将把所有节点连接起来，创建我们的 Agents RAG 工作流程。</p>
...
<p>
   ...
   <span class="img-wrapper"> <img translate="no" src="https://milvus-docs.s3.us-west-2.amazonaws.com/assets/agentic_rag_with_milvus_and_langgraph_21_0.png" alt="png" class="doc-image" id="png" />
    图 </span></p>
<h2 id="Run-the-agentic-RAG-system" class="common-anchor-header">运行代理 RAG 系统<button data-href="#Run-the-agentic-RAG-system" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>现在，让我们用不同类型的查询来测试我们的 Agents RAG 系统。</p>
<h3 id="Test-1-Simple-greeting-no-retrieval-needed" class="common-anchor-header">测试 1：简单问候语（无需检索）<button data-href="#Test-1-Simple-greeting-no-retrieval-needed" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<h3 id="Test-2-Question-requiring-retrieval" class="common-anchor-header">测试 2：需要检索的问题<button data-href="#Test-2-Question-requiring-retrieval" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<h3 id="Test-3-Question-that-might-trigger-rewrite" class="common-anchor-header">测试 3：可能触发重写的问题<button data-href="#Test-3-Question-that-might-trigger-rewrite" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<h2 id="Summary" class="common-anchor-header">总结<button data-href="#Summary" class="anchor-icon" translate="no">
    ...
    </button></h2><p>在本教程中，我们使用 LangGraph 和 Milvus 构建了一个代理 RAG 系统，它可以智能地决定何时检索信息、评估文档相关性并重写查询以获得更好的结果。与传统的 RAG 系统相比，这种方法具有显著优势，包括通过智能路由提供更好的用户体验、通过文档分级提供更高质量的答案，以及通过查询重写改进检索。您可以通过添加更复杂的分级逻辑、实施多种检索策略或整合其他工具和数据源来进一步扩展该系统。</p>
