...
<h3 id="Multiple-Metdata-Filters" class="common-anchor-header">多个元数据过滤器<button data-href="#Multiple-Metdata-Filters" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>您还可以将多个元数据过滤器组合起来，创建更复杂的查询。LlamaIndex 支持<code translate="no">AND</code> 和<code translate="no">OR</code> 条件来组合过滤器。这样就能根据元数据属性更精确、更灵活地检索文档。</p>
