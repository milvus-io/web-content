...
<h2 id="Preparation" class="common-anchor-header">准备工作<button data-href="#Preparation" class="anchor-icon" translate="no">
    ...
    </button></h2><h3 id="Dependency-and-Environment" class="common-anchor-header">依赖性和环境<button data-href="#Dependency-and-Environment" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<h2 id="Workflow" class="common-anchor-header">工作流程<button data-href="#Workflow" class="anchor-icon" translate="no">
    ...
    </button></h2><h3 id="Defining-the-Data-Source-Amazon-S3" class="common-anchor-header">定义数据源（亚马逊 S3）<button data-href="#Defining-the-Data-Source-Amazon-S3" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>在本例中，我们从亚马逊 S3 存储桶中提取文档。VectorETL 允许我们指定数据桶名称、文件路径和数据类型。</p>
...
<button class="copy-code-btn"></button></code></pre>
<h3 id="Configuring-the-Embedding-Model-OpenAI" class="common-anchor-header">配置嵌入模型（OpenAI）<button data-href="#Configuring-the-Embedding-Model-OpenAI" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>设置好数据源后，我们需要定义嵌入模型，将文本数据转换为向量嵌入。在本例中，我们使用 OpenAI 的<code translate="no">text-embedding-ada-002</code> 。</p>
...
<button class="copy-code-btn"></button></code></pre>
<h3 id="Setting-Up-Milvus-as-the-Target-Database" class="common-anchor-header">将 Milvus 设置为目标数据库<button data-href="#Setting-Up-Milvus-as-the-Target-Database" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>我们需要将生成的嵌入模型存储在 Milvus 中。在此，我们使用 Milvus Lite 定义 Milvus 连接参数。</p>
...
<h3 id="Specifying-Columns-for-Embedding" class="common-anchor-header">指定 Embeddings 的列<button data-href="#Specifying-Columns-for-Embedding" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>现在，我们需要指定 CSV 文件中的哪些列应转换为 Embeddings。这样可以确保只处理相关的文本字段，优化效率和存储。</p>
...
<button class="copy-code-btn"></button></code></pre>
<h3 id="Creating-and-Executing-the-VectorETL-Pipeline" class="common-anchor-header">创建并执行 VectorETL 管道<button data-href="#Creating-and-Executing-the-VectorETL-Pipeline" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>所有配置就绪后，我们现在要初始化 ETL 管道、设置数据流并执行它。</p>
...
flow.execute()
<button class="copy-code-btn"></button></code></pre>
<p>通过本教程的学习，我们成功地建立了一个端到端的 ETL 管道，使用 VectorETL 将文档从亚马逊 S3 转移到 Milvus。VectorETL 的数据源非常灵活，你可以根据自己的具体应用需求选择任何数据源。借助 VectorETL 的模块化设计，你可以轻松扩展这个管道，支持其他数据源，嵌入模型，使其成为人工智能和数据工程工作流的强大工具！</p>
