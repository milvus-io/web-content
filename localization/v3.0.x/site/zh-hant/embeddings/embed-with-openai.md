...
<h1 id="OpenAI" class="common-anchor-header">OpenAI<button data-href="#OpenAI" class="anchor-icon" translate="no">
    ...
    </button></h1><p>Milvus 透過<strong>OpenAIEmbeddingFunction</strong>類與 OpenAI 的模型整合。這個類別提供使用預先訓練的 OpenAI 模型來編碼文件和查詢的方法，並將內嵌回傳成與 Milvus 索引相容的密集向量。若要使用此功能，請在<a href="https://openai.com/api/">OpenAI</a>平台上建立帳號，從<a href="https://openai.com/api/">OpenAI</a>取得 API 金鑰。</p>
...
<p>存取 OpenAI API 的基本 URL。預設值為<strong>https://api.openai.com/v1。</strong>不過，如果您要存取不同模型提供者或本機 vLLM 範例的相容 API 端點，例如<strong>http://localhost:8080/v1，</strong>您可以在此指定 URL。</p></li>
