...
<h1 id="Gemini" class="common-anchor-header">雙子星<button data-href="#Gemini" class="anchor-icon" translate="no">
    ...
    </button></h1><p>Milvus 透過<strong>GeminiEmbeddingFunction</strong>類與<strong>Gemini</strong>的模型整合。這個類別提供了使用預先訓練的 Gemini 模型對文件和查詢進行編碼的方法，並將嵌入返回為與 Milvus 索引相容的密集向量。若要使用此功能，請在<a href="https://ai.google.dev/gemini-api/docs/api-key">Gemini</a>平台上建立帳號，從<a href="https://ai.google.dev/gemini-api/docs/api-key">Gemini</a>取得 API 金鑰。</p>
