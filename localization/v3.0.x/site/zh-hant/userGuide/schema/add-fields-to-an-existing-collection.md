...
<h1 id="Alter-Collection-Schema" class="common-anchor-header">變更集合模式<button data-href="#Alter-Collection-Schema" class="anchor-icon" translate="no">
    ...
    </button></h1><p>當集合從開發環境移轉至生產環境時，其架構通常會有所變更。您可能會新增如 `<code translate="no">source_uri</code> ` 或 `<code translate="no">review_status</code> ` 等標量欄位，以供篩選與應用程式邏輯使用；新增由應用程式生成的嵌入向量欄位；新增 BM25 函式及其生成的稀疏向量欄位，以便對現有文字進行詞彙搜尋；或是移除不再使用的欄位與函式。 「變更集合模式」功能可讓您直接對受支援的欄位和函式進行變更，而無需重新建立集合。</p>
...
<ul>
<li><p>在此模式變更工作流程中，刪除函數將移除該函數、其生成的向量欄位以及相關索引。函數的輸入欄位仍保留在集合模式中。</p></li>
...
<h2 id="Add-fields-and-Functions-to-an-existing-collection" class="common-anchor-header">向現有集合新增欄位與函數<button data-href="#Add-fields-and-Functions-to-an-existing-collection" class="anchor-icon" translate="no">
    ...
    </button></h2><p>請根據您是要新增使用者自訂欄位，還是要新增會產生向量場的函數，選擇對應的工作流程：</p>
<ul>
...
<ol>
<li><p>建立一個 Struct 模式，其中包含所需且支援資料類型的子欄位。有關適用的資料類型，請參閱<a href="/docs/zh-hant/structarray-limits.md#Supported-subfield-data-types">StructArray 限制</a>。</p></li>
...
<span class="highlighted-comment-line">)</span>
<button class="copy-code-btn"></button></code></pre>
<p>新增 StructArray 欄位後，集合中已存在的實體在所有子欄位上，其 `<code translate="no">chunks</code> ` 值皆會返回 `<code translate="no">NULL</code> `。插入新實體時，請確保所有子欄位均為 `<code translate="no">NULL</code> ` 或具有有效值。若插入的實體中，部分子欄位設定為 `<code translate="no">NULL</code> ` 而其他子欄位則設定為有效值，將會導致錯誤。</p>
...
<p>每個新增的自訂向量欄位都必須為可為空欄位。對於現有實體，其新向量欄位預設為<code translate="no">NULL</code> ，直到您透過 upsert 或後填工作流程寫入向量值為止。新實體可在插入時包含該向量欄位。向量搜尋會跳過向量值為<code translate="no">NULL</code> 的實體。詳細資訊請參閱《<a href="/docs/zh-hant/nullable-and-default.md">可為空欄位</a>》。</p>
...
<p>將函式及其生成的向量欄位新增至現有集合時，還需進行模式版本壓縮與儲存版本壓縮。若任一設定為停用狀態，Milvus 將拒絕該請求。這些額外的先決條件僅適用於修改現有集合的情況；在初始集合模式中定義函式時，並不會使用此現有資料回填工作流程。</p>
...
<span class="highlighted-comment-line">)</span>
<button class="copy-code-btn"></button></code></pre>
<p><code translate="no">index_params</code> 物件必須包含一個且僅一個針對新函數輸出欄位的索引定義。Milvus 會將該函數、其生成的向量欄位以及邊界索引定義，一併納入同一項模式變更中。請勿在執行<code translate="no">add_function_field()</code> 之後，再單獨呼叫<code translate="no">create_index()</code> 。</p>
...
<h2 id="Drop-fields-and-Functions-from-an-existing-collection" class="common-anchor-header">從現有集合中移除欄位和函式<button data-href="#Drop-fields-and-Functions-from-an-existing-collection" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>當使用者自訂欄位不再屬於您的集合模型時，您可以直接移除這些欄位。若要移除函式及其所產生的向量欄位，請刪除該函式；Milvus 會在同一項架構變更中一併移除該產生的欄位及其索引。</p>
<h3 id="Drop-user-defined-fields--Milvus-30x" class="common-anchor-header">刪除使用者自訂欄位<span class="beta-tag" style="background-color:rgb(0, 179, 255);color:white" translate="no">Compatible with Milvus 3.0.x</span><button data-href="#Drop-user-defined-fields--Milvus-30x" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<h3 id="Drop-a-Function-and-its-generated-vector-field--Milvus-30x" class="common-anchor-header">刪除函數及其生成的向量場<span class="beta-tag" style="background-color:rgb(0, 179, 255);color:white" translate="no">Compatible with Milvus 3.0.x</span><button data-href="#Drop-a-Function-and-its-generated-vector-field--Milvus-30x" class="anchor-icon" translate="no">
    ...
    </button></h3><p>當您不再需要某個函數或其所生成的向量場時（例如 BM25 函數及其生成的稀疏向量場），請使用此操作。</p>
...
<p>使用 `<code translate="no">add_function_field()</code> ` 在同一項架構變更中新增函式、其生成的向量場以及綁定索引定義。</p>
...
<h3 id="What-happens-to-existing-entities-after-I-add-a-user-defined-field" class="common-anchor-header">新增使用者自訂欄位後，現有實體會如何處理？<button data-href="#What-happens-to-existing-entities-after-I-add-a-user-defined-field" class="anchor-icon" translate="no">
    ...
    </button></h3><p>對於使用者自訂的標量欄位，除非您設定了<code translate="no">default_value</code> ，否則現有實體會傳回<code translate="no">NULL</code> 。若您設定了<code translate="no">default_value</code> ，現有實體則會傳回該預設值。</p>
...
<h3 id="Can-I-add-BM25-lexical-search-to-an-existing-collection" class="common-anchor-header">我可以為現有集合新增 BM25 詞彙搜尋功能嗎？<button data-href="#Can-I-add-BM25-lexical-search-to-an-existing-collection" class="anchor-icon" translate="no">
    ...
    </button></h3><p>可以。若集合已具備啟用分析器的<code translate="no">VARCHAR</code> 欄位，即可新增 BM25 函式及其生成的稀疏向量欄位，以進行詞彙搜尋。在此工作流程中，Milvus 會於同一項架構變更中，一併新增該函式、新的<code translate="no">SPARSE_FLOAT_VECTOR</code> 輸出欄位，以及綁定索引定義。 在此模式變更工作流程中，您無法將現有的<code translate="no">TEXT</code> 欄位用作BM25輸入。若要使用<code translate="no">TEXT</code> 輸入，請在建立集合時定義該欄位及BM25函式。</p>
<p>呼叫 `<code translate="no">add_function_field()</code>` 時，請提供一個 `<code translate="no">index_params</code> ` 物件，其中包含一個 `<code translate="no">SPARSE_INVERTED_INDEX</code> ` 索引，並為新輸出欄位設定 `<code translate="no">metric_type=&quot;BM25&quot;</code> `。Milvus 會將該索引定義綁定至生成的欄位，作為同一項資料結構變更的一部分。</p>
<h3 id="How-do-I-drop-a-Function-and-its-generated-vector-field" class="common-anchor-header">如何刪除一個函數及其生成的向量場？<button data-href="#How-do-I-drop-a-Function-and-its-generated-vector-field" class="anchor-icon" translate="no">
    ...
    </button></h3><p>呼叫<code translate="no">drop_function_field()</code> 並傳入函數名稱。在此模式變更工作流程中，Milvus 會一併移除該函數、其生成的向量欄位以及相關索引，同時保留該函數的輸入欄位。</p>
...
<p>儲存空間可在後續的壓縮過程中被釋放。壓縮是一種背景處理程序，會將現有資料檔案重新組織成更緊湊的新檔案。刪除欄位後，新壓縮的檔案將遵循當前資料結構，並省略已刪除的欄位。Milvus 無法保證在刪除欄位後能立即或於固定時間內減少儲存空間。</p>
