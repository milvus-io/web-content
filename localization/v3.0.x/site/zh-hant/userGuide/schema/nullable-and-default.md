...
title: 可為空欄位
summary: 設定可為空的欄位及其預設值，包括模式、插入、索引、搜尋及篩選行為。
---
<h1 id="Nullable-Fields" class="common-anchor-header">可為空欄位<button data-href="#Nullable-Fields" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h1><p>Milvus 支援可為空欄位，這使得欄位值可以為空，或明確設定為 NULL。可為空性是在架構層級定義的，並會一致地套用至資料匯入、建立索引、搜尋及查詢等操作中。</p>
<p>在以下情況下應使用可為空欄位：</p>
<ul>
<li>從允許缺失值的外部系統匯入資料時。</li>
<li>部分元資料為可選項，或僅適用於資料集的一部分。</li>
<li>向量嵌入是異步產生並於稍後插入。</li>
</ul>
...
<li><p>允許 NULL 值的向量欄位不支援 `<code translate="no">IS NULL</code> ` 或 `<code translate="no">IS NOT NULL</code> ` 篩選表達式。您無法根據向量欄位值是否為 NULL 來明確篩選實體。</p></li>
<li><p><a href="/docs/zh-hant/array-of-structs.md">「結構體陣列」</a>欄位不支援 NULL 值。您無法將「結構體陣列」欄位或其內嵌的任何欄位標記為可為空。</p></li>
<li><p>可為空屬性是在建立欄位時定義的，事後無法修改。您無法針對現有欄位啟用或停用可為空功能。</p></li>
<li><p>標記為可為 NULL 的欄位無法用作分區鍵。分區鍵欄位必須始終包含有效且非 NULL 的值。如需更多資訊，請參閱「<a href="/docs/zh-hant/use-partition-key.md">使用分區鍵</a>」。</p></li>
</ul>
<h2 id="What-is-a-nullable-field" class="common-anchor-header">什麼是可為空欄位？<button data-href="#What-is-a-nullable-field" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>在 Milvus 中，字段是否允許儲存 NULL 值，是由名為 `<code translate="no">nullable</code>` 的模式層級字段屬性所控制。</p>
<p>當欄位定義時指定 `<code translate="no">nullable=True</code>` 時，Milvus 允許該欄位在資料載入過程中缺失值。實際上，Milvus 會將以下兩種輸入視為等同，並將該欄位值儲存為 `NULL`：</p>
<ul>
<li>輸入實體中省略該欄位。</li>
<li>該欄位被明確設定為 NULL（例如，在 Python 中使用<code translate="no">None</code> ）。</li>
</ul>
<p>若未將欄位定義為可為空（此為預設行為），則每個實體都必須為該欄位提供有效值。省略該欄位或明確賦予 NULL 值，都會導致插入或匯入操作失敗。</p>
<p>集合模式中的<strong>標量欄位和向量欄位均</strong>支援 `nullable` 屬性。然而，結構體陣列欄位並不支援 `nullable` 屬性。</p>
<div class="alert note">
<p>可為空性決定了欄位值是否可能缺失；它並未定義當欄位缺失時會使用何種值。</p>
<ul>
<li>若可空欄位未設定預設值，省略該欄位將導致儲存為 NULL 值。</li>
<li>若已設定預設值，Milvus 可能會改為儲存該預設值。詳情請參閱<a href="/docs/zh-hant/default-values.md">《預設值》</a>。</li>
</ul>
</div>
<h2 id="Define-a-nullable-field-in-the-collection-schema" class="common-anchor-header">在集合模式中定義可為空欄位<button data-href="#Define-a-nullable-field-in-the-collection-schema" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>若要使用可為空欄位，您必須在定義集合模式時啟用可為空屬性。</p>
<p>在此範例中，集合模式定義了一個名為<code translate="no">embedding</code> 的向量欄位，其<code translate="no">nullable=True</code> 。這允許集合中的實體在資料匯入時省略向量值，或明確將其設定為 NULL。</p>
...
<p>在此模式中：</p>
<ul>
<li><code translate="no">embedding</code> 欄位已明確標記為可為空。</li>
<li>實體在插入時可省略<code translate="no">embedding</code> 欄位，或將其設定為 NULL 值。</li>
<li>是否允許 NULL 值的設定，是在建立集合時即已確定。</li>
</ul>
<p>為求清晰起見，以下範例將聚焦於可為 NULL 的向量欄位（<code translate="no">embedding</code> ）。定義可為 NULL 的標量欄位屬可選操作，且無須遵循本指南其餘內容。</p>
<p><details>
<summary>可選：定義可為 NULL 的標量欄位</summary></p>
<p>標量欄位亦可透過相同的 `<code translate="no">nullable</code> ` 屬性定義為可為 NULL，並在資料導入時遵循相同的規則。例如：</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#javascript">   NodeJS</a>
...
<button class="copy-code-btn"></button></code></pre>
<p></details></p>
<h2 id="Insert-behavior-with-missing-or-NULL-values" class="common-anchor-header">值缺失或為 NULL 時的插入行為<button data-href="#Insert-behavior-with-missing-or-NULL-values" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>一旦在集合模式中將欄位定義為可為空，Milvus 便允許在資料匯入過程中，該欄位值為缺失或明確設定為 NULL。</p>
<p>以下範例將三個實體插入至「<a href="#define-a-nullable-field-in-the-collection-schema">在集合模式中定義可為空欄位</a>」中建立的集合，以示範這些不同情況。</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#javascript">   NodeJS</a>
...
<p>在此範例中：</p>
<ul>
<li>實體<strong>id = 1</strong>提供了一個有效的向量值。</li>
<li>實體<strong>id = 2</strong>將 NULL 值明確定義賦予 `<code translate="no">embedding</code> ` 欄位。</li>
<li>實體<strong>id = 3</strong>完全省略了 `<code translate="no">embedding</code> ` 欄位；Milvus 會將其儲存為 NULL。</li>
</ul>
<h2 id="Index-behavior-on-nullable-fields" class="common-anchor-header">可為 NULL 欄位的索引行為<button data-href="#Index-behavior-on-nullable-fields" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>插入資料後，您可以照常在可為 NULL 的欄位上建立索引。主要差異在於 Milvus 在建立索引時如何處理 NULL 值：</p>
<ul>
<li>僅會將具有非 NULL 值的實體加入索引。</li>
<li>具有 NULL 值的實體將被跳過，且不會參與索引建立。</li>
</ul>
<p>對於可為 NULL 的向量欄位，這表示只有具有有效向量的實體才能透過向量相似度進行搜尋。</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#javascript">   NodeJS</a>
...
<p>此時：</p>
<ul>
<li>具有有效嵌入向量值的實體已建立索引，並可供搜尋。</li>
<li>嵌入值為 NULL 的實體仍保留在集合中，但不會被納入向量索引。</li>
</ul>
<h2 id="Search-behavior-with-nullable-fields" class="common-anchor-header">可為空欄位的搜尋行為<button data-href="#Search-behavior-with-nullable-fields" class="anchor-icon" translate="no">
    ...
    </button></h2><p>當您對可為空欄位執行搜尋操作時，Milvus 僅會評估該搜尋欄位值不為空的實體。向量欄位為 NULL 的實體會被自動跳過。</p>
<p>以本例中的<code translate="no">embedding</code> 為例，此為可為 NULL 的向量欄位：</p>
<ul>
<li>僅會評估並對具有有效向量值的實體進行排序。</li>
<li>向量為 NULL 的實體不會引發錯誤。</li>
<li>若有效向量數量少於請求的<code translate="no">topK</code> （<code translate="no">limit</code> ），Milvus 可能返回的結果數量會少於<code translate="no">limit</code> 。</li>
</ul>
<p>以下範例針對可為 NULL 的向量欄位<code translate="no">embedding</code> 執行向量搜尋：</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#javascript">   NodeJS</a>
...
<p>在此搜尋中：</p>
<ul>
<li>僅將<code translate="no">embedding</code> 值不為空的實體視為候選對象。</li>
<li><code translate="no">embedding</code> 值為 NULL 的實體將被排除在評估之外。</li>
<li>回傳的結果數量取決於集合中存在多少個有效的向量。</li>
</ul>
<h2 id="Query-and-filtering-implications" class="common-anchor-header">查詢與篩選的注意事項<button data-href="#Query-and-filtering-implications" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>前面的範例主要著重於向量欄位。本節將說明 NULL 值在<strong>標量篩選表達式中的</strong>運作方式。</p>
<p>標量欄位可以定義為<code translate="no">nullable=True</code> ，並遵循與向量欄位相同的匯入規則。然而<strong>，在篩選表達式中，NULL 標量值始終被評估為 false</strong>。</p>
<p>例如，假設有一個可為 NULL 的標量欄位<code translate="no">age</code> ，以下篩選條件會選出年齡大於 18 歲的實體：</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#javascript">   NodeJS</a>
...
<button class="copy-code-btn"></button></code></pre>
<p>由於 NULL 值不符合篩選條件，因此<code translate="no">age</code> 為 NULL 的實體將被排除在結果之外。</p>
<p>同樣地，等值檢查也不會與 NULL 值匹配。例如：</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#javascript">   NodeJS</a>
...
<button class="copy-code-btn"></button></code></pre>
<p><code translate="no">status</code> 為 NULL 的實體將被排除在結果之外。</p>
<h2 id="Nullable-fields-and-default-values" class="common-anchor-header">可為 NULL 的欄位與預設值<button data-href="#Nullable-fields-and-default-values" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>當某個欄位同時設定了 `<code translate="no">nullable</code> ` 與 `<code translate="no">default_value</code> ` 時，以下規則將決定 Milvus 在插入資料時如何處理 NULL 輸入或缺失的欄位值。</p>
<table>
<thead>
<tr><th>啟用可為 NULL 設定</th><th>預設值</th><th>使用者輸入（NULL 或省略）</th><th>結果</th></tr>
</thead>
<tbody>
<tr><td>是</td><td>是（非 NULL）</td><td>NULL 或省略</td><td>使用預設值</td></tr>
<tr><td>是</td><td>否</td><td>NULL 或未指定</td><td>儲存為 NULL</td></tr>
<tr><td>否</td><td>是（非 NULL）</td><td>NULL 或省略</td><td>使用預設值</td></tr>
<tr><td>否</td><td>否</td><td>NULL 或省略</td><td>拋出錯誤</td></tr>
<tr><td>否</td><td>是（預設為 NULL）</td><td>NULL 或未指定</td><td>會拋出錯誤</td></tr>
</tbody>
</table>
<p><strong>重點摘要：</strong></p>
<ul>
<li>當欄位具有非 NULL 的預設值時，無論是否啟用「<code translate="no">nullable</code> 」，系統都會使用該預設值。</li>
<li>當啟用「<code translate="no">nullable=True</code> 」但未設定預設值時，該欄位會儲存 NULL。</li>
<li>當啟用「<code translate="no">nullable=False</code> 」但未設定預設值時，插入操作會失敗並觸發錯誤。</li>
<li>在不可為 NULL 的欄位上設定 NULL 預設值是不合規的，並會引發錯誤。</li>
</ul>
<p>有關預設值的完整範例與 API 使用方式，請參閱《<a href="/docs/zh-hant/default-values.md">預設值》</a>。</p>
