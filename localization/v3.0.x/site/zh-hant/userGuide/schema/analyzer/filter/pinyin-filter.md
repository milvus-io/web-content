...
summary: 在文字分析過程中，拼音過濾器會將中文字元片段轉換為拼音片段，從而使中文文字能夠進行基於拼音的比對。
beta: Milvus 3.0.x
---
<h1 id="Pinyin" class="common-anchor-header">拼音<span class="beta-tag" style="background-color:rgb(0, 179, 255);color:white" translate="no">Compatible with Milvus 3.0.x</span><button data-href="#Pinyin" class="anchor-icon" translate="no">
    ...
    </button></h1><p>中文文字搜尋通常需要使用者精確輸入與索引文字中完全相同的漢字。在名稱查詢、自動完成及邊打字邊搜尋的工作流程中，使用者經常會輸入拼音而非漢字。 例如，使用者可能會輸入「<code translate="no">zuqiu</code> 」來搜尋「<code translate="no">足球</code> 」。<code translate="no">pinyin</code> 過濾器會在分析器的輸出中加入拼音標記，讓中文文字能與拼音輸入進行比對，而無需維護獨立的拼音欄位。</p>
...
<p>此簡寫設定會保留原始的中文詞元，並輸出字級的拼音詞元。除非您明確啟用相關選項，否則不會輸出連音拼音或拼音首字母。</p>
...
<tr><td><code translate="no">keep_joined_full_pinyin</code></td><td>布林值</td><td><code translate="no">false</code></td><td>針對每個來源詞元，輸出一個連接後的拼音詞元。例如，<code translate="no">中文</code> 會產生<code translate="no">zhongwen</code> 。</td></tr>
...
<p>此篩選器針對分詞器產生的標記進行處理。對於中文文本，請搭配如<code translate="no">jieba</code> 這樣的分詞器使用。</p>
...
<button class="copy-code-btn"></button></code></pre>
<h3 id="Match-Chinese-text-with-character-level-Pinyin" class="common-anchor-header">將中文文本與字元級別的拼音進行配對<button data-href="#Match-Chinese-text-with-character-level-Pinyin" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<h3 id="Match-Chinese-terms-with-joined-Pinyin" class="common-anchor-header">將中文詞彙與拼寫連字形式進行匹配<button data-href="#Match-Chinese-terms-with-joined-Pinyin" class="anchor-icon" translate="no">
    ...
    </button></h3><p>若需讓中文詞彙與其完整的拼讀形式進行比對，請啟用<code translate="no">keep_joined_full_pinyin</code> 。</p>
<pre><code translate="no" class="language-python">analyzer_params = {
    <span class="hljs-string">&quot;tokenizer&quot;</span>: <span class="hljs-string">&quot;jieba&quot;</span>,
...
<h3 id="Match-Chinese-terms-with-Pinyin-initials" class="common-anchor-header">將中文詞彙與拼音首字母進行配對<button data-href="#Match-Chinese-terms-with-Pinyin-initials" class="anchor-icon" translate="no">
    ...
    </button></h3><p>若需讓中文詞彙與其拼音首字母形式進行配對，請啟用<code translate="no">keep_separate_first_letter</code> 。</p>
<pre><code translate="no" class="language-python">analyzer_params = {
    <span class="hljs-string">&quot;tokenizer&quot;</span>: <span class="hljs-string">&quot;jieba&quot;</span>,
    <span class="hljs-string">&quot;filter&quot;</span>: [
        {
