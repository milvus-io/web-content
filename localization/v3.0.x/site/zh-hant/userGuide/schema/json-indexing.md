...
<h3 id="Understand-cast-types" class="common-anchor-header">了解轉換類型<button data-href="#Understand-cast-types" class="anchor-icon" translate="no">
    ...
    </button></h3><p><code translate="no">json_cast_type</code> 即用於解析並索引<code translate="no">json_path</code> 路徑下值所使用的資料型別。這與欄位架構型別不同：該欄位仍屬於<code translate="no">JSON</code> 欄位，但每個被索引的路徑都會被視為特定的標量、陣列或 JSON 物件型別。</p>
...
<ul>
<li><p>您需要對 JSON 陣列中的值建立索引時。</p></li>
<li><p>您維護著針對整個 JSON 物件或子物件的現有索引，並希望明確指定 `<code translate="no">INVERTED</code> ` 的行為。</p></li>
<li><p>您希望使用單一索引類型來處理等值、<code translate="no">IN</code> 、範圍、文字比對及陣列查詢。為維持相容性，仍支援對整個 JSON 物件的索引，但代價是索引大小會較大。</p></li>
</ul>
...
<h3 id="STLSORT" class="common-anchor-header">STL_SORT<button data-href="#STLSORT" class="anchor-icon" translate="no">
    ...
    </button></h3><p><code translate="no">STL_SORT</code> 會將 JSON 路徑中的值以排序順序儲存。此類型針對數值或可排序字串值的範圍篩選器進行了優化。</p>
...
<h3 id="BITMAP" class="common-anchor-header">BITMAP<button data-href="#BITMAP" class="anchor-icon" translate="no">
    ...
    </button></h3><p><code translate="no">BITMAP</code> 會針對 JSON 路徑中的每個不同值建立一個緊湊位圖。此功能針對頻繁重複的值所進行的相等性比對及<code translate="no">IN</code> 篩選已進行優化。</p>
<p><code translate="no">BITMAP</code> 僅支援<code translate="no">BOOL</code> 和<code translate="no">VARCHAR</code> 轉換類型。請在以下情況下使用：</p>
<ul>
<li><p>您的篩選條件使用 `<code translate="no">==</code> ` 或 `<code translate="no">IN</code>`。</p></li>
...
<p>對於標記為 `<code translate="no">No</code>` 的儲存格，Milvus 會在建立索引時拒絕該請求。對於陣列轉換類型，請明確使用 `<code translate="no">INVERTED</code> `（`<code translate="no">AUTOINDEX</code> ` 不適用於陣列）。</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>以下每個範例皆展示一個<code translate="no">index_params.add_index(...)</code> 呼叫。請選擇符合您資料的範例，並在同一個<code translate="no">index_params</code> 物件上執行這些呼叫。最後，透過單一的<code translate="no">client.create_index(...)</code> 呼叫套用所有設定。詳細資訊請參閱「<a href="/docs/zh-hant/json-indexing.md#apply-the-index">套用索引」</a>。</p>
...
<p><code translate="no">AUTOINDEX</code> 會透明地將 `<code translate="no">INVERTED</code> ` 用於 `<code translate="no">JSON</code> ` 轉換類型，因為扁平化與類型推斷皆屬於反向索引的功能。</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>索引整個物件會增加索引大小。對於具有深度嵌套文件和多樣化查詢模式的新工作負載，請使用路徑特定索引，或考慮採用<a href="/docs/zh-hant/json-shredding.md">JSON 拆分</a>。</p>
...
<p>針對<code translate="no">AUTOINDEX</code> 條目，此回應中的<code translate="no">index_type</code> 欄位會顯示為<code translate="no">AUTOINDEX</code> 。Milvus 目前不會揭露建置時所選用的底層佈局（<code translate="no">BITMAP</code> 或<code translate="no">STL_SORT</code> ）。請將此選擇視為內部優化：針對該路徑的等值查詢、<code translate="no">IN</code> 及範圍查詢，無論選用何種佈局皆能正常運作。</p>
