...
<tr><td>子欄位模式</td><td>同一 StructArray 字段中的所有 Struct 元素共享一個預定義的 Struct 模式。</td></tr>
...
<tr><td>子欄位路徑請為索引、搜尋目標、輸出欄位和篩選器使用 xml-ph-0000@deepl.internal 路徑，例如 xml-ph-0001@deepl.internal。</td><td>請使用<code translate="no">structArray[subfield]</code> 路徑（例如<code translate="no">chunks[emb]</code> ）作為索引、搜尋目標、輸出欄位及篩選條件。請勿使用<code translate="no">chunks.emb</code> 。</td></tr>
...
<tr><td>可為空的欄位</td><td>可為空的 StructArray 欄位受版本限制。若受支援，空值將套用至整個 StructArray 欄位，而非獨立套用至個別的 Struct 元素。</td></tr>
...
<tbody>
<tr><td>Struct 並非頂層欄位類型。</td><td>建立一個 StructArray 欄位，格式為<code translate="no">datatype=DataType.ARRAY</code> ，並設定<code translate="no">element_type=DataType.STRUCT</code> 及<code translate="no">struct_schema</code> 。</td></tr>
...
<tr><td><code translate="no">Array</code></td><td>不支援</td><td>StructArray 欄位中不支援 JSON 子欄位。</td></tr>
...
<h2 id="Nullable-and-dynamic-schema-limits" class="common-anchor-header">可為空與動態模式限制<button data-href="#Nullable-and-dynamic-schema-limits" class="anchor-icon" translate="no">
    ...
    </button></h2><p>可為空的 StructArray 行為以及動態 StructArray 欄位的新增功能，皆受版本限制。</p>
<table>
<thead>
<tr><th>功能</th><th>限制</th></tr>
</thead>
<tbody>
<tr><td>可為空的 StructArray 欄位</td><td>Milvus 3.0.0 及後續版本支援此功能。請在 StructArray 父元素上設定 `<code translate="no">nullable=True</code> `；請勿獨立將 Struct 子欄位設定為可為空。</td></tr>
...
<tr><td>空值作用域</td><td>空值適用於整個 StructArray 欄位。例如，<code translate="no">chunks=None</code> 僅在<code translate="no">chunks</code> 可設為空時才有效。</td></tr>
<tr><td>部分可為空的 StructArray 值</td><td>當 StructArray 欄位包含有效的陣列值時，請勿在同一個值中混合使用可為 null 的子欄位陣列與有效的子欄位陣列。</td></tr>
<tr><td>動態新增 StructArray 欄位</td><td>於 Milvus 3.0.0 及後續版本中支援。</td></tr>
<tr><td>動態新增的可為空要求</td><td>新增至現有集合的 StructArray 欄位必須為可為空，因為現有實體對於新欄位尚無值。</td></tr>
<tr><td>動態新增後的現有實體</td><td>現有實體針對新增的 StructArray 欄位會傳回 `<code translate="no">null</code> `。</td></tr>
</tbody>
</table>
<p>Milvus 3.0.0 及後續版本在獨立部署和分散式部署中，均支援可為空的 StructArray 欄位、可為空的向量陣列，以及動態新增 StructArray 欄位。較早版本的 Milvus 不支援這些功能。</p>
<p>在 Zilliz Cloud 中，這些功能僅適用於運行 Milvus 3.0.0 或更新版本的隨需叢集。服務叢集不支援這些功能。</p>
...
<tbody>
<tr><td>有效載荷形狀</td><td>將 StructArray 欄位作為 Struct 物件的陣列進行插入，例如<code translate="no">chunks: [{&quot;text&quot;: &quot;...&quot;, &quot;emb&quot;: [...]}]</code> 。</td></tr>
...
<h2 id="Index-and-metric-limits" class="common-anchor-header">索引與度量限制<button data-href="#Index-and-metric-limits" class="anchor-icon" translate="no">
    ...
    </button></h2><p>StructArray 向量子欄位可針對「嵌入清單搜尋」或「元素層級搜尋」進行索引。同一個向量子欄位無法同時使用這兩種度量家族，因為每個向量欄位或向量子欄位僅能接受一個索引。</p>
<table>
<thead>
...
<tr><td>範圍搜尋</td><td>支援情況取決於搜尋模式以及目標版本對索引/度量的支援狀況。關於元素層級 StructArray 請求中混合搜尋範圍的行為，請查閱您的目標版本。</td></tr>
...
<h2 id="Filter-and-operator-limits" class="common-anchor-header">篩選與運算子限制<button data-href="#Filter-and-operator-limits" class="anchor-icon" translate="no">
    ...
    </button></h2><p>StructArray 的標量篩選由 StructArray 運算子處理，例如 `<code translate="no">element_filter</code> ` 及 `<code translate="no">MATCH_*</code> ` 系列。詳細的謂詞支援對照表請參閱<a href="/docs/zh-hant/struct-array-operators.md">StructArray 運算子</a>。</p>
<p>總體而言：</p>
<ul>
...
<li><p>StructArray 元素層級的條件式不支援 JSON 路徑語法、JSON 函數、陣列容器函數、文字比對函數、幾何／GIS 函數，以及 Timestamptz 運算式。</p></li>
<li><p>請優先使用顯式的布林比較（例如 `<code translate="no">$[has_code] == true</code> `），而非未包覆的布林運算式。</p></li>
</ul>
...
<li><p>若要建立向量和標量索引，請參閱《為<a href="/docs/zh-hant/index-structarray-fields.md">StructArray 欄位建立索引》</a>。</p></li>
