...
<h1 id="StructArray-Limits" class="common-anchor-header">StructArray 的限制<button data-href="#StructArray-Limits" class="anchor-icon" translate="no">
    ...
    </button></h1><p>StructArray 的支援範圍涵蓋模式定義、插入資料、索引建立、搜尋模式，以及 StructArray 專屬的篩選器。在生產環境中依賴 StructArray 的運作行為之前，請將本頁作為限制參照。</p>
...
<tr><td>子欄位路徑請在索引、搜尋目標、輸出欄位及篩選器中使用「xml-ph-0000@deepl.internal」路徑，例如 xml-ph-0001@deepl.internal。</td><td>請使用<code translate="no">structArray[subfield]</code> 路徑（例如<code translate="no">chunks[emb]</code> ），用於索引、搜尋目標、輸出欄位和篩選器。請勿使用<code translate="no">chunks.emb</code> 。</td></tr>
...
<tr><td>函式</td><td>StructArray 字段內的字段或子字段不支援字段函數。</td></tr>
<tr><td>可為空的欄位</td><td>可為空的 StructArray 欄位受版本限制。當支援此功能時，空值將套用至整個 StructArray 欄位，而非獨立套用至個別的 Struct 元素。</td></tr>
...
<tr><td><code translate="no">max_capacity</code> 此欄位為必填欄位。</td><td>一個實體中的 Struct 元素數量不得超過為 StructArray 欄位所設定的<code translate="no">max_capacity</code> 。</td></tr>
<tr><td>現有的子欄位是固定的。</td><td>您無法將新的子欄位附加到現有的 StructArray 欄位。若要變更子欄位架構，請刪除 StructArray 欄位，然後使用更新的架構再次新增該欄位。</td></tr>
...
<p>有關模式建立的範例，請參閱「<a href="/docs/zh-hant/create-structarray-field.md">建立 StructArray 欄位</a>」。</p>
<h2 id="Supported-subfield-data-types" class="common-anchor-header">受支援的子欄位資料類型<button data-href="#Supported-subfield-data-types" class="anchor-icon" translate="no">
    ...
    </button></h2><p>StructArray 子欄位會映射至實體陣列式的儲存方式。下表列出受支援與不受支援的實體類型。</p>
<table>
<thead>
...
<tr><td><code translate="no">ArrayOfVector</code></td><td>受支援</td><td>將子欄位定義為<code translate="no">DataType.INT8_VECTOR</code> ，並將<code translate="no">dim</code> 設為。</td></tr>
...
<tr><td><code translate="no">Array</code></td><td>不支援</td><td>StructArray 欄位中不支援文字子欄位。</td></tr>
<tr><td><code translate="no">Array</code></td><td>不支援</td><td>StructArray 欄位不支援 Timestamptz 子欄位及時間特定表達式。</td></tr>
<tr><td>嵌套的<code translate="no">Array</code> 、<code translate="no">ArrayOfVector</code> 、<code translate="no">Struct</code> 或<code translate="no">ArrayOfStruct</code></td><td>不支援</td><td>StructArray 字段不支援嵌套的陣列、向量陣列、Struct 或 Array-of-Struct 子欄位。</td></tr>
</tbody>
</table>
<h2 id="Nullable-and-dynamic-schema-limits" class="common-anchor-header">可為空與動態模式的限制<button data-href="#Nullable-and-dynamic-schema-limits" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>可為空的 StructArray 行為以及動態 StructArray 欄位的新增，皆受版本限制。</p>
<table>
<thead>
<tr><th>功能</th><th>限制</th></tr>
</thead>
<tbody>
<tr><td>可為空的 StructArray 欄位</td><td>僅在包含可為空 StructArray 及可為空向量陣列支援的版本中受支援。</td></tr>
...
<tr><td>空值作用域</td><td>空值適用於整個 StructArray 欄位。例如，<code translate="no">chunks=None</code> 僅在<code translate="no">chunks</code> 為可空時才有效。</td></tr>
<tr><td>部分可為空的 StructArray 值</td><td>當 StructArray 欄位包含有效的陣列值時，請勿在同一個值中混合使用可為 null 的子欄位陣列與有效的子欄位陣列。</td></tr>
...
<tr><td>動態新增後的既有實體</td><td>現有實體會針對新增的 StructArray 欄位及其所有子欄位，回傳<code translate="no">null</code> 。</td></tr>
</tbody>
</table>
...
<h2 id="Index-and-metric-limits" class="common-anchor-header">索引與度量限制<button data-href="#Index-and-metric-limits" class="anchor-icon" translate="no">
    ...
    </button></h2><p>StructArray 向量子欄位可針對「嵌入清單搜尋」或「元素層級搜尋」進行索引。同一個向量子欄位無法同時使用這兩種度量家族，因為每個向量欄位或向量子欄位僅接受一個索引。</p>
<table>
<thead>
...
<p>在規劃集合架構時，StructArray 向量子欄位會被視為向量子欄位。請確保向量欄位與向量子欄位的總數符合目標版本及服務層級的限制。</p>
...
<tbody>
<tr><td>基本 EmbeddingList 搜尋</td><td>支援以<code translate="no">MAX_SIM*</code> 度量進行索引的StructArray向量子欄位。返回實體層級的結果。</td></tr>
...
<tr><td>分組搜尋</td><td>元素層級的分組搜尋可返回偏移量。針對元素層級 StructArray 請求的混合搜尋分組行為，取決於目標版本。</td></tr>
<tr><td>混合搜尋</td><td>混合搜尋請求僅在目標版本支援該搜尋組合時，才可包含 StructArray 向量子欄位請求。每個請求仍遵循已建立索引之向量子欄位的指標家族。</td></tr>
<tr><td>偏移量輸出</td><td>元素層級的搜尋結果可提供偏移量。EmbeddingList 搜尋會回傳實體層級的結果，且不會將元素偏移量作為主要結果單位。</td></tr>
</tbody>
</table>
<h2 id="Filter-and-operator-limits" class="common-anchor-header">篩選與運算子限制<button data-href="#Filter-and-operator-limits" class="anchor-icon" translate="no">
    ...
    </button></h2><p>StructArray 的標量篩選由 StructArray 運算子處理，例如 `<code translate="no">element_filter</code> ` 以及 `<code translate="no">MATCH_*</code> ` 家族。詳細的謂詞支援矩陣請參閱<a href="/docs/zh-hant/struct-array-operators.md">StructArray 運算子</a>。</p>
<p>總體而言：</p>
<ul>
<li><p>請僅在 StructArray 運算子內部使用 `<code translate="no">$[subfield]</code> `。</p></li>
<li><p>請使用標量子字段作為標量謂詞的輸入。</p></li>
<li><p>請勿將向量子欄位用作 `<code translate="no">$[...]</code> ` 標量判別式的輸入。</p></li>
<li><p>StructArray 元素層級的條件式不支援 JSON 路徑語法、JSON 函式、陣列容器函式、文字比對函式、幾何／GIS 函式，以及 Timestamptz 表達式。</p></li>
<li><p>請優先使用顯式的布林比較（例如<code translate="no">$[has_code] == true</code> ），而非裸露的布林運算式。</p></li>
</ul>
