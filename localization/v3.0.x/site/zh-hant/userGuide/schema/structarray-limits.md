...
<tr><td>可為空的欄位</td><td>可為空的 StructArray 欄位受版本限制。當支援此功能時，空值適用於整個 StructArray 欄位，而非獨立地套用至個別的 Struct 元素。</td></tr>
...
<li><p>請優先使用顯式的布林比較（例如 `<code translate="no">$[has_code] == true</code> `），而非裸露的布林表達式。</p></li>
</ul>
