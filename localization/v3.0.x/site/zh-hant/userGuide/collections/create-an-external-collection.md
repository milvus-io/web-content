...
summary: 透過外部集合存取 AWS S3 或 Iceberg 中的資料，無須複製到 Milvus 中。
beta: Milvus 3.0.x
---
<h1 id="Create-an-External-Collection" class="common-anchor-header">建立外部資料集<span class="beta-tag" style="background-color:rgb(0, 179, 255);color:white" translate="no">Compatible with Milvus 3.0.x</span><button data-href="#Create-an-External-Collection" class="anchor-icon" translate="no">
    ...
    </button></h1><p>外部集合是 Milvus 中的一種資料集合類型，可從 AWS S3 和 Iceberg 等外部儲存系統存取資料，而無需將資料複製到 Milvus 中。它充當資料湖上的查詢層，同時保持與 Milvus 查詢介面的相容性。</p>
...
<h2 id="Limits--restrictions" class="common-anchor-header">限制與約束<button data-href="#Limits--restrictions" class="anchor-icon" translate="no">
    ...
    </button></h2><p>由於 Milvus 不儲存原始資料，只維護元資料和原始資料之間的映射，外部資料集是唯讀的。這意味著您無法在 Milvus 端插入、上插、刪除、匯入、刷新和壓縮資料。</p>
<p>與管理式資料集相比，外部資料集有以下限制：</p>
<ul>
<li><p>您無法設定主索引鍵及其 AutoID 屬性。</p></li>
<li><p>不能啟用動態欄位。</p></li>
<li><p>不能設定分割鍵和群集鍵，因為分割不可用。</p></li>
<li><p>不能在模式中定義功能。</p></li>
<li><p>一旦建立外部資料集，就無法變更其模式。</p></li>
<li><p>您無法在 BM25 中使用文字匹配。</p></li>
<li><p>您必須在建立索引前觸發更新作業。</p></li>
</ul>
<h2 id="Step-1-Create-schema" class="common-anchor-header">步驟 1：建立模式<button data-href="#Step-1-Create-schema" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>與建立管理式資料集一樣，您也需要在建立外部資料集前建立模式。然而，模式與管理式集合略有不同。</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>要為外部集合建立模式，您需要指定來源資料 URI、資料格式和驗證設定。</p>
...
<h3 id="Use-AWS-AKSK" class="common-anchor-header">使用 AWS AK/SK<button data-href="#Use-AWS-AKSK" class="anchor-icon" translate="no">
    ...
    </button></h3><p>此選項適用於自行託管的 MinIO 或有 AK/SK 工作的情況。</p>
...
<table>
   ...
   <tr>
     ...
     <td><p>目標來源資料檔案的格式。</p><p>可能的值是<code translate="no">parquet</code>,<code translate="no">vortex</code>,<code translate="no">lance-table</code>, 和<code translate="no">iceberg-table</code> 。</p></td>
     <td><p><code translate="no">parquet</code></p></td>
   </tr>
   <tr>
     <td><p><code translate="no">extfs</code></p></td>
     <td><p>以字串化 JSON 結構顯示的外部檔案系統設定。</p></td>
     <td><p>--</p></td>
   </tr>
   <tr>
   ...
   <tr>
     ...
     <td><p>是否使用虛擬主機存取您的儲存桶。</p><p>如需詳細資訊，請參閱<a href="https://docs.aws.amazon.com/AmazonS3/latest/userguide/VirtualHosting.html">本文</a>。</p></td>
     <td><p><code translate="no">true</code></p></td>
   </tr>
</table>
<h3 id="Use-AWS-IAM" class="common-anchor-header">使用 AWS IAM<button data-href="#Use-AWS-IAM" class="anchor-icon" translate="no">
    ...
    </button></h3><p>此選項適用於 Milvus 在 EC2 實例或 EKS 集群上執行的情況。在這種情況下，您不需要硬體編碼 AK/SK。</p>
<pre><code translate="no" class="language-json"><span class="hljs-punctuation">{</span>
    <span class="hljs-attr">&quot;format&quot;</span><span class="hljs-punctuation">:</span> <span class="hljs-string">&quot;...&quot;</span><span class="hljs-punctuation">,</span>
    <span class="hljs-attr">&quot;extfs&quot;</span><span class="hljs-punctuation">:</span> <span class="hljs-punctuation">{</span>
...
<table>
   ...
   <tr>
     <td><p><code translate="no">format</code></p></td>
     <td><p>目標來源資料的格式。</p><p>可能的值為<code translate="no">parquet</code>,<code translate="no">vortex</code>,<code translate="no">lance-table</code>, 和<code translate="no">iceberg-table</code></p></td>
     <td><p><code translate="no">parquet</code></p></td>
   </tr>
   <tr>
     <td><p><code translate="no">extfs</code></p></td>
   ...
   <tr>
     ...
     <td><p>是否使用 AWS IAM。</p><p>此選項設定為<code translate="no">"true"</code> 。</p></td>
     <td><p><code translate="no">true</code></p></td>
   </tr>
   <tr>
     <td><p><code translate="no">extfs.iam_endpoint</code></p></td>
     <td><p>有效的 AWS STS 端點。 </p><p>如需詳細資訊，請參閱<a href="https://docs.aws.amazon.com/IAM/latest/UserGuide/id_credentials_temp_region-endpoints.html">本文</a>。</p></td>
...
<h3 id="Use-Milvus-global-credentials" class="common-anchor-header">使用 Milvus 全局憑證<button data-href="#Use-Milvus-global-credentials" class="anchor-icon" translate="no">
    ...
    </button></h3><p>此選項適用於在 Milvus 儲存桶中儲存外部資料時，可直接使用<code translate="no">milvus.yaml</code> 中指定的全局 MinIO 設定來存取資料。</p>
<pre><code translate="no" class="language-json"><span class="hljs-punctuation">{</span>
    <span class="hljs-attr">&quot;format&quot;</span><span class="hljs-punctuation">:</span> <span class="hljs-string">&quot;...&quot;</span><span class="hljs-punctuation">,</span>
    <span class="hljs-attr">&quot;extfs&quot;</span><span class="hljs-punctuation">:</span> <span class="hljs-punctuation">{</span>
...
<li><p>在角色的信任政策的 Condition 欄位中包含一個自定義的<code translate="no">sts:ExternalId</code> 。</p></li>
</ul>
<p>然後，水桶所有者應該提供您 IAM 角色的 ARN 和 External ID，這樣您就可以使用這些值呼叫<code translate="no">sts:AssumeRole</code> ，以承擔 IAM 角色。</p>
<p>以下是要附加到 IAM 角色的允許權限政策範例。您可以調整此政策以符合您的需求。</p>
<pre><code translate="no" class="language-json"><span class="hljs-punctuation">{</span>
...
<span class="hljs-punctuation">}</span>
<button class="copy-code-btn"></button></code></pre>
<p>而與 IAM 角色相關的信任政策則定義了誰可以擔任該角色。</p>
<pre><code translate="no" class="language-json"><span class="hljs-punctuation">{</span>
  <span class="hljs-attr">&quot;Version&quot;</span><span class="hljs-punctuation">:</span> <span class="hljs-string">&quot;2012-10-17&quot;</span><span class="hljs-punctuation">,</span>
  <span class="hljs-attr">&quot;Statement&quot;</span><span class="hljs-punctuation">:</span> <span class="hljs-punctuation">[</span>
    <span class="hljs-punctuation">{</span>
...
<span class="hljs-punctuation">}</span>
<button class="copy-code-btn"></button></code></pre>
<p>取得 IAM 角色 ARN 和外部 ID 後，就可以設定<code translate="no">external_spec</code> 參數，如下所示：</p>
<pre><code translate="no" class="language-json"><span class="hljs-punctuation">{</span>
    <span class="hljs-attr">&quot;format&quot;</span><span class="hljs-punctuation">:</span> <span class="hljs-string">&quot;...&quot;</span><span class="hljs-punctuation">,</span>
    <span class="hljs-attr">&quot;extfs&quot;</span><span class="hljs-punctuation">:</span> <span class="hljs-punctuation">{</span>
        <span class="hljs-attr">&quot;cloud_provider&quot;</span><span class="hljs-punctuation">:</span> <span class="hljs-string">&quot;aws&quot;</span><span class="hljs-punctuation">,</span>
...
<table>
   ...
   <tr>
     <td><p><code translate="no">format</code></p></td>
     <td><p>目標來源資料的格式。</p><p>可能的值是<code translate="no">parquet</code>,<code translate="no">vortex</code>,<code translate="no">lance-table</code>, 和<code translate="no">iceberg-table</code></p></td>
     <td><p><code translate="no">parquet</code></p></td>
   </tr>
   <tr>
     <td><p><code translate="no">extfs</code></p></td>
   ...
   <tr>
     <td><p><code translate="no">extfs.use_iam</code></p></td>
     <td><p>是否使用 AWS IAM。</p><p>將此選項設定為<code translate="no">"true"</code> 。</p></td>
     <td><p><code translate="no">true</code></p></td>
   </tr>
   <tr>
   ...
   <tr>
     ...
     <td><p>從水桶所有者取得的外部 ID。</p></td>
     <td><p>--</p></td>
   </tr>
   <tr>
     <td><p><code translate="no">extfs.load_frequency</code></p></td>
     <td><p>Milvus 擷取臨時認證憑證的間隔，以秒為單位。</p></td>
...
<h2 id="Step-2-Add-fields" class="common-anchor-header">步驟 2：新增欄位<button data-href="#Step-2-Add-fields" class="anchor-icon" translate="no">
    ...
    </button></h2><p>一旦模式準備就緒，您可以如下方式新增欄位：</p>
<div class="multipleCode">
   <a href="#python">Python</a> <a href="#java">Java</a> <a href="#go">Go</a> <a href="#javascript">NodeJS</a> <a href="#bash">cURL</a></div>
...
<h2 id="Step-3-Create-a-collection" class="common-anchor-header">步驟 3：建立集合<button data-href="#Step-3-Create-a-collection" class="anchor-icon" translate="no">
    ...
    </button></h2><p>將所有欄位加入模式後，您就可以建立集合。</p>
<div class="multipleCode">
   <a href="#python">Python</a> <a href="#java">Java</a> <a href="#go">Go</a> <a href="#javascript">NodeJS</a> <a href="#bash">cURL</a></div>
...
<pre><code translate="no" class="language-bash"><span class="hljs-comment"># restful</span>
<button class="copy-code-btn"></button></code></pre>
<h2 id="Step-4-Refresh-data" class="common-anchor-header">步驟 4：刷新資料<button data-href="#Step-4-Refresh-data" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>一旦集合準備就緒，您需要執行刷新，將資料中的元資料同步到 Milvus。</p>
<div class="multipleCode">
   <a href="#python">Python</a> <a href="#java">Java</a> <a href="#go">Go</a> <a href="#javascript">NodeJS</a> <a href="#bash">cURL</a></div>
...
<li><p>艙單檔記錄了 Milvus 中的元資料與外部檔案中的行之間的映射。</p></li>
<li><p>如果源資料有更新，您需要再次手動呼叫刷新，以保持 Milvus 為最新。</p></li>
<li><p>您不能在刷新完成後才為外部資料集建立索引。不過，建立索引的方法與管理式資料集相同。</p></li>
<li><p>需要移除所有作用中元資料而沒有任何插入的刷新會導致拒絕。</p></li>
</ul>
</div>
<h2 id="Follow-ups" class="common-anchor-header">跟進<button data-href="#Follow-ups" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>對外部資料集執行刷新作業且清單檔案可用後，您就可以在外部資料集中建立索引、載入/釋出資料集、進行類似性搜尋和查詢，就像在任何受管資料集中一樣。</p>
