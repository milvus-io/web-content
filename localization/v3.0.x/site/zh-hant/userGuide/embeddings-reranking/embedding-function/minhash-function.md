...
summary: 使用 MinHash 將文字轉換為二進位向量，以進行基於 Jaccard 係數的相似度搜尋及近似重複內容偵測。
beta: Milvus 3.0.x
---
...
<p><a href="https://en.wikipedia.org/wiki/MinHash">MinHash</a>是一種局部敏感哈希技術，用於估算集合之間的<a href="https://en.wikipedia.org/wiki/Jaccard_index">Jaccard 相似度</a>。在 Milvus 中，MinHash 函數遵循以下處理流程：您提供原始文字作為輸入，Milvus 則產生二進位向量作為輸出——所有中間步驟均由系統內部處理。</p>
<p>整體工作流程包含一個由文件導入與查詢處理共同使用的<strong>共用文字處理管線，</strong>隨後是針對儲存與檢索階段的特定操作。</p>
...
<li><p><strong>MinHash 簽名生成</strong>：對片段集套用多個雜湊函數（H1、H2、…、Hn，其中 n =<code translate="no">num_hashes</code> ）。 針對每個哈希函數，會從所有片段中選取最小的哈希值。這些最小值的集合便構成 MinHash 簽名——一種固定長度的表示形式，用以近似原始文件的雅卡德相似度。</p></li>
...
<h3 id="Document-ingestion" class="common-anchor-header">文件導入<button data-href="#Document-ingestion" class="anchor-icon" translate="no">
    ...
    </button></h3><p>在插入過程中，由共享管線產生的二進位向量會儲存於<code translate="no">MINHASH_LSH</code> 索引中。該索引維護一個LSH（局部敏感雜湊）表，將相似的簽名分組至相同的桶中，從而能在查詢時快速檢索候選項目。</p>
<h3 id="Query-processing" class="common-anchor-header">查詢處理<button data-href="#Query-processing" class="anchor-icon" translate="no">
    ...
    </button></h3><p>在搜尋過程中，查詢文字會經過相同的共享管線以產生二進位向量。此向量用於在<code translate="no">MINHASH_LSH</code> 索引中執行 LSH 查詢，從而快速識別出可能相似的候選對。若未啟用 Jaccard 精細化，Milvus 會回傳未依據預估 Jaccard 相似度進行排序的 LSH 候選結果。 當啟用 Jaccard 精細化時，Milvus 會利用儲存的原始 MinHash 簽名，根據估算的 Jaccard 相似度對候選結果進行排序，並返回前 K 項結果。</p>
...
<li><p><strong>二進位向量欄位</strong>(<code translate="no">BINARY_VECTOR</code>)：儲存由 MinHash 函式自動生成的二進位向量。其維度必須等於<code translate="no">32 * num_hashes</code> 。</p></li>
</ul>
...
<h3 id="Define-the-MinHash-function" class="common-anchor-header">定義 MinHash 函式<button data-href="#Define-the-MinHash-function" class="anchor-icon" translate="no">
    ...
    </button></h3><p>MinHash 函式會將分析過的文字轉換為二進位向量，用以近似文件之間的雅卡德相似度。</p>
...
<table>
   ...
   <tr>
     ...
     <td><p>分詞層級。選項：</p><ul><li><p><code translate="no">"word"</code>：使用該欄位的分析器進行分詞，然後套用 n-gram 分段法。</p></li><li><p><code translate="no">"char"</code> /<code translate="no">"character"</code>: 直接對原始字元應用 n-gram 分片（不使用分析器）。</p><p>詞級提供更強的語義和更高效率，但取決於特定語言的詞元化。字元級則不受語言限制，但會產生更高維度的切片，且語義較弱。</p></li></ul></td>
   </tr>
   <tr>
