...
summary: 本主題說明如何在 Milvus 中使用託管版的 Hugging Face 推論提供者進行文字嵌入。
beta: Milvus v2.6.20+
---
...
<p>此整合功能使用託管的 Hugging Face 路由器。若要將 Milvus 連接到獨立部署的文字嵌入推論 (TEI) 服務，請參閱<a href="/docs/zh-hant/hugging-face-tei.md">Hugging Face TEI</a>。</p>
...
<li>「函式」輸出欄位的維度必須與所選模型的輸出維度相符。</li>
</ul>
...
<li><strong>產生嵌入向量。</strong>文字嵌入函式會透過<code translate="no">hf-inference</code> 將文字傳送至 Hugging Face 的<code translate="no">feature-extraction</code> 處理流程。該函式使用<code translate="no">model_name</code> 來選取模型，並可傳遞受支援的推論選項，例如正規化與截斷。</li>
...
<p>Milvus 無法控制 Hugging Face 模型是否仍可透過<code translate="no">hf-inference</code> 取得，亦無法保證該模型是否符合您的穩定性、延遲及輸出品質要求。在將模型投入生產環境使用前，請先在 Hugging Face 上驗證該模型，並針對您的工作負載進行評估。</p>
</div>
...
<h3 id="Option-1-Configuration-file" class="common-anchor-header">選項 1：設定檔<button data-href="#Option-1-Configuration-file" class="anchor-icon" translate="no">
    ...
    </button></h3><p>請在<code translate="no">milvus.yaml</code> 的頂層<code translate="no">credential</code> 區段中定義該令牌，然後將 Hugging Face 嵌入式提供者指向該憑證標籤：</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>您亦可在函式參數中設定 `<code translate="no">credential</code> `。其值必須是 `<code translate="no">credential</code> ` 頂層區段中定義的標籤，而非憑證本身。函式層級的憑證標籤優先於提供者層級的標籤。</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>有關套用 Docker Compose 設定的詳細資訊，請參閱《<a href="/docs/zh-hant/configure-docker.md">使用 Docker Compose 配置 Milvus</a>》。</p>
...
<li>有關 Function 的一般概念及插入/搜尋行為，請參閱《<a href="/docs/zh-hant/embedding-function-overview.md">嵌入式函數概覽</a>》。</li>
