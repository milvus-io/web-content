...
<ol>
<li><p><strong>將文字分解成 n-grams</strong>：Milvus 在目標欄位的每個字串上滑動一個<em>n</em>的視窗，並擷取重疊的子串或<em>n-gram</em>。這些子串的長度在可設定的範圍內，<code translate="no">[min_gram, max_gram]</code>.</p>
<ul>
...
<p>例如，如果 2-gram<code translate="no">&quot;AI&quot;</code> 出現在 ID 為 1、5、6、8 和 9 的文件中，則索引會記錄<code translate="no">{&quot;AI&quot;: [1, 5, 6, 8, 9]}</code> 。查詢時可使用此索引快速縮小搜尋範圍。</p></li>
</ol>
<p>
...
<li><p>如果是<code translate="no">L &gt; max_gram</code> ，查詢詞會使用相等於<code translate="no">max_gram</code> 的視窗大小分解成重疊的格。</p></li>
</ul>
...
<p>如果您的使用情況需要，請將相同的規範化方式套用於擷取的文字與查詢字面意義（例如：降低大小寫、修剪）。</p></li>
</ul>
