...
title: GPU_CAGRA
summary: >-
  GPU_CAGRA 索引是一種針對 GPU 進行優化的圖式索引。相較於使用昂貴的訓練級 GPU，使用推論級 GPU 來執行 Milvus 的 GPU
  版本，在成本效益方面可能更具優勢。
---
...
<p>配置完索引參數後，您可以直接使用 `<code translate="no">create_index()</code> ` 方法建立索引，或將索引參數傳入 `<code translate="no">create_collection</code> ` 方法中。詳細資訊請參閱「<a href="/docs/zh-hant/create-collection.md">建立集合</a>」。</p>
