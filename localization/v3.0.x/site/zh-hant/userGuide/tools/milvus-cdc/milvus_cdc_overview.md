...
<p>在主-備拓撲中，其中一個叢集擔任主叢集並接受寫入操作。一個或多個備用叢集會持續接收來自主叢集的變更，並可處理讀取流量。當主叢集無法運作或需要進行維護時，您可以將服務流量切換至備用叢集。</p>
...
<tbody>
<tr><td><strong><a href="/docs/zh-hant/cdc_switchover.md">切換</a></strong></td><td>當前主節點仍可連線，或您正在進行預定維護</td><td>RPO = 0</td><td>在角色變更前，會等待剩餘的複製資料完成</td></tr>
<tr><td><strong><a href="/docs/zh-hant/cdc_failover.md">故障轉移</a></strong></td><td>當前主伺服器無法使用且無法快速恢復</td><td>可能</td><td>立即提升備用伺服器為主伺服器，以便寫入操作能恢復</td></tr>
</tbody>
</table>
<p>只要當前主伺服器仍能回應，就應使用切換。僅當恢復可用性比等待原始主伺服器更重要時，才使用故障轉移。</p>
...
<h2 id="Bulk-Import-in-CDC-Replication" class="common-anchor-header">CDC 複製中的批次匯入<button data-href="#Bulk-Import-in-CDC-Replication" class="anchor-icon" translate="no">
    ...
    </button></h2><p>在 CDC 複製拓撲中，批量匯入必須使用兩階段提交 (2PC) 模式，並搭配 `<code translate="no">auto_commit=false</code>`。僅針對主叢集執行匯入與提交，並確保主叢集和備用叢集均可存取匯入檔案。詳細資訊請參閱<a href="/docs/zh-hant/bulk_import_in_cdc_replication.md">《CDC 複製中的批量匯入</a>》。</p>
...
<h2 id="FAQ" class="common-anchor-header">常見問題<button data-href="#FAQ" class="anchor-icon" translate="no">
    ...
    </button></h2><h3 id="Can-a-standby-cluster-serve-queries" class="common-anchor-header">備用叢集可以處理查詢嗎？<button data-href="#Can-a-standby-cluster-serve-queries" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<h3 id="Does-Milvus-CDC-support-active-active-writes" class="common-anchor-header">Milvus CDC 是否支援主動-主動寫入？<button data-href="#Does-Milvus-CDC-support-active-active-writes" class="anchor-icon" translate="no">
    ...
    </button></h3><p>不。Milvus CDC 專為單一主節點拓撲設計。同時向多個叢集寫入資料可能會導致「腦裂」及資料分歧。</p>
<h3 id="Does-switchover-lose-data" class="common-anchor-header">切換時會丟失資料嗎？<button data-href="#Does-switchover-lose-data" class="anchor-icon" translate="no">
    ...
    </button></h3><p>不會。系統切換會等待剩餘資料完成複製後，才讓備用叢集成為主叢集。</p>
