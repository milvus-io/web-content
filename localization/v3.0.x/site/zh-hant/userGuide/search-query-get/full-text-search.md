...
title: 全文搜尋
summary: >-
  全文搜尋是一項功能，可從文字資料集中檢索包含特定術語或短語的文件，並根據相關性對結果進行排序。此功能克服了語義搜尋的局限性——後者可能忽略精確的術語——確保您獲得最準確且符合語境的結果。
...
<h1 id="Full-Text-Search" class="common-anchor-header">全文搜尋<button data-href="#Full-Text-Search" class="anchor-icon" translate="no">
    ...
    </button></h1><p>全文搜尋是一項功能，可從文字資料集中檢索包含特定術語或短語的文件，並根據相關性對結果進行排序。此功能克服了語義搜尋的限制——語義搜尋可能會忽略精確的術語——確保您獲得最準確且符合上下文的結果。 此外，它透過接受原始文字輸入來簡化向量搜尋，能自動將您的文字資料轉換為稀疏嵌入向量，無需手動生成向量嵌入。</p>
<p>此功能採用 BM25 演算法進行相關性評分，在「檢索增強生成」（RAG）情境中尤為實用，能優先呈現與特定搜尋詞彙高度吻合的文件。</p>
...
<h2 id="BM25-implementation" class="common-anchor-header">BM25 實作<button data-href="#BM25-implementation" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Milvus 提供基於 BM25 相關性演算法的全文搜尋功能，該演算法是資訊檢索系統中廣泛採用的評分函數，Milvus 將其整合至搜尋工作流程中，以提供精準且按相關性排序的文字結果。</p>
...
<li><p><strong>BM25 函數處理</strong>：內建函數會將這些術語轉換為針對 BM25 評分進行優化的稀疏向量表示形式。</p></li>
...
<ol>
<li><p><a href="/docs/zh-hant/full-text-search.md#Create-a-collection-for-BM25-full-text-search">建立集合</a>：設定所需欄位，並定義一個能將原始文字轉換為稀疏嵌入向量的 BM25 函式。</p></li>
<li><p><a href="/docs/zh-hant/full-text-search.md#Insert-text-data">插入資料</a>：將原始文字文件導入集合中。</p></li>
<li><p><a href="/docs/zh-hant/full-text-search.md#Perform-full-text-search">執行搜尋</a>：使用自然語言查詢文字，依據 BM25 相關性檢索排序後的結果。</p></li>
</ol>
<h2 id="Create-a-collection-for-BM25-full-text-search" class="common-anchor-header">建立用於 BM25 全文搜尋的集合<button data-href="#Create-a-collection-for-BM25-full-text-search" class="anchor-icon" translate="no">
    ...
    </button></h2><p>若要啟用由 BM25 驅動的全文搜尋，您必須準備一個包含所需欄位的集合、定義一個用於產生稀疏向量的 BM25 函式、設定索引，然後建立該集合。</p>
...
<li><p><strong>稀疏向量欄位</strong>(<code translate="no">SPARSE_FLOAT_VECTOR</code>)：儲存由 BM25 函式自動產生的稀疏嵌入向量。</p></li>
</ul>
...
<p>在上述配置中，</p>
<ul>
<li><p><code translate="no">id</code>：作為主鍵，並會透過<code translate="no">auto_id=True</code> 自動產生。</p></li>
<li><p><code translate="no">text</code>：用於儲存您用於全文檢索操作的原始文字資料。此欄位可使用 `<code translate="no">VARCHAR</code> ` 儲存有限長度的文字，或使用 `<code translate="no">TEXT</code> ` 儲存長篇來源內容。</p></li>
<li><p><code translate="no">sparse</code>：一個向量欄位，專門用於儲存全文字搜尋操作中內部生成的稀疏嵌入向量。資料型別必須為<code translate="no">SPARSE_FLOAT_VECTOR</code> 。</p></li>
</ul>
...
<table>
   ...
   <tr>
     ...
     <td><p>函式名稱。此函式會將您在「<code translate="no">text</code> 」欄位中的原始文字，轉換為相容於 BM25 的稀疏向量，並儲存於「<code translate="no">sparse</code> 」欄位中。</p></td>
   </tr>
   <tr>
...
<div class="alert note">
<p>若多個文字欄位需要進行 BM25 處理，請<strong>針對每個欄位</strong>定義<strong>一個 BM25 函式，</strong>且每個<strong>函式</strong>必須具有唯一的名稱與輸出欄位。</p>
</div>
...
<table>
   ...
   <tr>
     ...
     <td><p>此參數的值必須明確設定為<code translate="no">BM25</code> ，方能啟用全文檢索功能。</p></td>
   </tr>
   <tr>
     <td><p><code translate="no">params</code></p></td>
     <td><p>專屬於該索引的附加參數字典。</p></td>
   </tr>
   <tr>
     <td><p><code translate="no">params.inverted_index_algo</code></p></td>
     <td><p>用於建立和查詢 BM25 稀疏反向索引的演算法。有效值：</p><ul><li><p><code translate="no">"DAAT_MAXSCORE"</code> (預設)：採用「逐份文件 (Document-at-a-Time)」MaxScore 查詢處理方式。此選項適用於<em>k</em>值較高或包含大量術語的全文檢索工作負載。相關背景請參閱《<a href="https://dl.acm.org/doi/10.1016/0306-4573%2895%2900020-H">查詢評估：策略與最佳化</a>》。</p></li><li><p><code translate="no">"DAAT_WAND"</code>：採用「一次處理一份文件」的 WAND 查詢處理方式。此選項適用於<em>k</em>值較小或查詢詞彙較短的全文檢索工作負載。相關背景請參閱《<a href="https://dl.acm.org/doi/10.1145/956863.956944">使用兩級檢索流程進行高效查詢評估</a>》。</p></li><li><p><code translate="no">"TAAT_NAIVE"</code>：基本「逐詞」查詢處理。請將此選項用作基準，或當您需要讓評分機制能動態適應整體資料庫統計資料（例如平均文件長度）時，請選用此選項。</p></li><li><p><code translate="no">"BLOCK_MAX_MAXSCORE"</code>: 搭配區塊層級最高分數元資料的 MaxScore 查詢處理。有關背景資訊，請參閱《<a href="https://dl.acm.org/doi/10.1145/2009916.2010048">使用區塊最大索引加速 Top-k 文件檢索</a>》。</p></li><li><p><code translate="no">"BLOCK_MAX_WAND"</code>: 搭配區塊層級最高分數元資料的 WAND 查詢處理。有關背景資訊，請參閱《<a href="https://dl.acm.org/doi/10.1145/2009916.2010048">使用區塊最高分數索引加速 Top-k 文件檢索</a>》。</p></li></ul></td>
   </tr>
   <tr>
...
<h3 id="Create-the-collection" class="common-anchor-header">建立集合<button data-href="#Create-the-collection" class="anchor-icon" translate="no">
    ...
    </button></h3><p>現在使用已定義的資料結構與索引參數來建立集合。</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#go">   Go</a>
...
<h2 id="Insert-text-data" class="common-anchor-header">插入文字資料<button data-href="#Insert-text-data" class="anchor-icon" translate="no">
    ...
    </button></h2><p>設定完資料集與索引後，即可開始插入文字資料。在此過程中，您只需提供原始文字即可。我們先前定義的內建函式會自動為每筆文字條目產生對應的稀疏向量。</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#go">   Go</a>
...
<h2 id="Perform-full-text-search" class="common-anchor-header">執行全文檢索<button data-href="#Perform-full-text-search" class="anchor-icon" translate="no">
    ...
    </button></h2><p>將資料插入集合後，您即可使用原始文字查詢執行全文搜尋。Milvus 會自動將您的查詢轉換為稀疏向量，並使用 BM25 演算法對匹配的搜尋結果進行排序，最後返回前 K 項（<code translate="no">limit</code> ）結果。</p>
<div class="alert note">
<p>您可以透過配置文字高亮工具，在搜尋結果中標示出匹配的詞彙。詳情請參閱「<a href="/docs/zh-hant/text-highlighter.md">文字高亮工具</a>」。</p>
</div>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
...
<table>
   ...
   <tr>
     ...
     <td><p>搜尋時應忽略的低重要性詞彙比例。數值必須在 [0.0, 1.0) 範圍內。詳情請參閱「<a href="/docs/zh-hant/sparse_vector.md">稀疏向量</a>」。</p></td>
   </tr>
   <tr>
     <td></td>
     <td></td>
   ...
   <tr>
     ...
     <td><p>以自然語言撰寫的原始查詢文字。Milvus 會使用 BM25 函式自動將您的文字查詢轉換為稀疏向量——請勿提供預先計算的向量。</p></td>
   </tr>
   <tr>
   ...
   <tr>
     ...
     <td><p>搜尋結果中要回傳的欄位名稱清單。<strong>除</strong>包含由 BM25 產生嵌入<strong>向量的稀疏向量欄位外，</strong>皆支援所有欄位。常見的輸出欄位包括主鍵欄位（例如：<code translate="no">id</code> ）與原始文字欄位（例如：<code translate="no">text</code> ）。如需更多資訊，請參閱<a href="/docs/zh-hant/full-text-search.md#Can-I-output-or-access-the-sparse-vectors-generated-by-the-BM25-function-in-full-text-search">常見問題解答</a>。</p></td>
   </tr>
   <tr>
     <td><p><code translate="no">limit</code></p></td>
     <td><p>要回傳的前幾項最符合結果的最大數量。</p></td>
   </tr>
</table>
<h2 id="FAQ" class="common-anchor-header">常見問題解答<button data-href="#FAQ" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><h3 id="Can-I-output-or-access-the-sparse-vectors-generated-by-the-BM25-function-in-full-text-search" class="common-anchor-header">我可以在全文搜尋中輸出或存取由 BM25 函式生成的稀疏向量嗎？<button data-href="#Can-I-output-or-access-the-sparse-vectors-generated-by-the-BM25-function-in-full-text-search" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>不可以，在全文搜尋中無法直接存取或輸出由 BM25 函式所產生的稀疏向量。詳情如下：</p>
<ul>
<li><p>BM25 函式會於內部產生稀疏向量，用於排序與檢索</p></li>
<li><p>這些向量儲存於稀疏欄位中，但無法包含在<code translate="no">output_fields</code></p></li>
<li><p>您只能輸出原始文字欄位和元資料（例如<code translate="no">id</code> 、<code translate="no">text</code> ）</p></li>
</ul>
<p>範例：</p>
...
<h3 id="Why-do-I-need-to-define-a-sparse-vector-field-if-I-cant-access-it" class="common-anchor-header">既然無法存取稀疏向量欄位，為何還需要定義它？<button data-href="#Why-do-I-need-to-define-a-sparse-vector-field-if-I-cant-access-it" class="anchor-icon" translate="no">
    ...
    </button></h3><p>稀疏向量欄位用作內部搜尋索引，類似於使用者不會直接與之互動的資料庫索引。</p>
