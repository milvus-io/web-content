...
summary: >-
  Milvus
  中的文字比對功能可根據特定術語精確檢索文件。此功能主要用於符合特定條件的篩選搜尋，並可結合標量篩選來精確篩選查詢結果，從而實現符合標量標準的向量內相似度搜尋。
---
<h1 id="Text-Match" class="common-anchor-header">文字匹配<button data-href="#Text-Match" class="anchor-icon" translate="no">
    ...
    </button></h1><p>Milvus 中的文字匹配功能可根據特定術語精確檢索文件。此功能主要用於篩選搜尋以滿足特定條件，並可結合標量篩選來精確化查詢結果，從而實現符合標量標準的向量內相似度搜尋。</p>
<div class="alert note">
<p>文字匹配著重於查找查詢詞彙的完全匹配結果，不會對匹配文件的相關性進行評分。若您希望根據查詢詞彙的語義及重要性來檢索最相關的文件，建議您使用<a href="/docs/zh-hant/full-text-search.md">全文檢索</a>。</p>
</div>
...
<ol>
<li><p><a href="/docs/zh-hant/analyzer-overview.md">分析器</a>：分析器會將輸入的文字分割為單一詞彙（即標記），並視需要套用篩選器。這使 Milvus 能根據這些標記建立索引。</p></li>
<li><p><a href="/docs/zh-hant/index-explained.md">建立索引</a>：完成文字分析後，Milvus 會建立一個倒排索引，將每個唯一的詞元映射至包含該詞元的文件。</p></li>
</ol>
<p>當使用者執行文字比對時，系統會利用倒排索引快速檢索所有包含該術語的文件。此方式遠比逐一掃描每份文件來得快速。</p>
...
<h2 id="Enable-text-match" class="common-anchor-header">啟用文字比對<button data-href="#Enable-text-match" class="anchor-icon" translate="no">
    ...
    </button></h2><p>文字比對適用於 <a href="/docs/zh-hant/string.md"><code translate="no">VARCHAR</code></a> 欄位類型上運作，該類型本質上是 Milvus 中的字串資料類型。若要啟用文字比對，請將 `<code translate="no">enable_analyzer</code> ` 與 `<code translate="no">enable_match</code> ` 兩者皆設定為 `<code translate="no">True</code> `，然後在定義集合架構時，可選擇性地配置<a href="/docs/zh-hant/analyzer-overview.md">分析器</a>以進行文字分析。</p>
<h3 id="Set-enableanalyzer-and-enablematch" class="common-anchor-header">設定<code translate="no">enable_analyzer</code> 與<code translate="no">enable_match</code><button data-href="#Set-enableanalyzer-and-enablematch" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>若要為特定的<code translate="no">VARCHAR</code> 欄位啟用文字比對功能，請在定義欄位架構時，將<code translate="no">enable_analyzer</code> 和<code translate="no">enable_match</code> 兩個參數皆設定為<code translate="no">True</code> 。此設定會指示 Milvus 將文字進行分詞，並為指定欄位建立倒排索引，從而實現快速且高效的文字比對。</p>
...
<p>預設情況下，Milvus 使用<code translate="no">standard</code> 分析器，該分析器會根據空白字元和標點符號將文字進行分詞，移除長度超過 40 個字元的分詞，並將文字轉換為小寫。套用此預設設定無需額外參數。如需更多資訊，請參閱「<a href="/docs/zh-hant/standard-analyzer.md">標準」</a>。</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>Milvus 還提供多種其他分析器，適用於不同的語言和情境。如需更多詳細資訊，請參閱《<a href="/docs/zh-hant/analyzer-overview.md">分析器概覽》</a>。</p>
<h2 id="Use-text-match" class="common-anchor-header">使用文字比對<button data-href="#Use-text-match" class="anchor-icon" translate="no">
    ...
    </button></h2><p>在您的集合架構中為 VARCHAR 欄位啟用文字比對功能後，即可使用<code translate="no">TEXT_MATCH</code> 表達式執行文字比對。</p>
<h3 id="TEXTMATCH-expression-syntax" class="common-anchor-header">TEXT_MATCH 表達式語法<button data-href="#TEXTMATCH-expression-syntax" class="anchor-icon" translate="no">
    ...
    </button></h3><p><code translate="no">TEXT_MATCH</code> 表達式用於指定要搜尋的欄位及搜尋詞彙。其語法如下：</p>
...
<li><p><code translate="no">text</code>: 要搜尋的關鍵詞。多個關鍵詞可使用空格分隔，或根據語言及已設定的分析器使用其他適當的分隔符號。</p></li>
</ul>
<p>預設情況下，<code translate="no">TEXT_MATCH</code> 會使用「<strong>或 (OR)</strong>」匹配邏輯，這表示它將回傳包含任何指定搜尋詞的文件。例如，若要搜尋在<code translate="no">text</code> 欄位中包含<code translate="no">machine</code> 或<code translate="no">deep</code> 其中任一搜尋詞的文件，請使用以下表達式：</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#go">   Go</a>
...
<h3 id="Search-with-text-match" class="common-anchor-header">使用文字比對進行搜尋<button data-href="#Search-with-text-match" class="anchor-icon" translate="no">
    ...
    </button></h3><p>文字匹配可與向量相似度搜尋結合使用，以縮小搜尋範圍並提升搜尋效能。透過在進行向量相似度搜尋前，先使用文字匹配過濾資料庫，可減少需要搜尋的文件數量，從而加快查詢速度。</p>
<p>在此範例中，<code translate="no">filter</code> 這個表達式會將搜尋結果篩選為僅包含與指定詞彙<code translate="no">keyword1</code> 或<code translate="no">keyword2</code> 相符的文件。隨後，系統會針對這組已篩選的文件子集執行向量相似度搜尋。</p>
<div class="alert note">
...
<button class="copy-code-btn"></button></code></pre>
<h3 id="Query-with-text-match" class="common-anchor-header">使用文字比對進行查詢<button data-href="#Query-with-text-match" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>文字匹配亦可用於查詢操作中的標量篩選。透過在 `<code translate="no">query()</code> ` 方法的 `<code translate="no">expr</code> ` 參數中指定 `<code translate="no">TEXT_MATCH</code> ` 表達式，即可擷取與指定術語相符的文件。</p>
...
<li><p>為某個欄位啟用詞彙匹配功能會觸發建立倒排索引，這會消耗儲存資源。在決定是否啟用此功能時，請考量其對儲存空間的影響，因為此影響會因文字大小、唯一詞元以及所使用的分析器而異。</p></li>
<li><p>一旦在資料結構中定義了分析器，其設定將對該集合永久生效。若您認為其他分析器更能滿足需求，可考慮刪除現有集合，並使用所需的分析器設定建立新的集合。</p></li>
...
<li><p>若字串常數以雙引號包圍，常數內的雙引號應以<code translate="no">\\&quot;</code> 表示，而單引號則可使用<code translate="no">'</code> 或<code translate="no">\\'</code> 表示。範例：<code translate="no">&quot;He said \\&quot;Hi\\&quot;&quot;</code> 。</p></li>
</ul></li>
</ul>
