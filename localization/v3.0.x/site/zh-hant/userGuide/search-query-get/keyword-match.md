...
summary: >-
  Milvus
  中的文字比對功能可根據特定術語精確檢索文件。此功能主要用於篩選搜尋以滿足特定條件，並可結合標量篩選來精確篩選查詢結果，從而實現符合標量標準的向量內相似度搜尋。
---
<h1 id="Text-Match" class="common-anchor-header">文字匹配<button data-href="#Text-Match" class="anchor-icon" translate="no">
    ...
    </button></h1><p>Milvus 中的文字比對功能可根據特定術語精確檢索文件。此功能主要用於篩選搜尋以滿足特定條件，並可結合標量篩選來精細化查詢結果，允許在符合標量標準的向量內進行相似度搜尋。</p>
<div class="alert note">
<p><code translate="no">TEXT_MATCH</code> 可搜尋完全相符的已分析術語，而「<code translate="no">TEXT_MATCH_FUZZY</code> 」則能容許查詢標記與索引標記之間存在微小的編輯距離。兩者皆為布林篩選操作，且不會對匹配文件的相關性進行評分。若您希望根據查詢術語的語義含義與重要性來檢索最相關的文件，我們建議您使用「<a href="/docs/zh-hant/full-text-search.md">全文搜尋</a>」。</p>
</div>
<h2 id="Overview" class="common-anchor-header">概述<button data-href="#Overview" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Milvus 整合了<a href="https://github.com/quickwit-oss/tantivy">Tantivy，</a>以驅動其底層的反向索引及基於術語的文字搜尋功能。對於每則文字輸入，Milvus 會依照以下程序進行索引：</p>
<ol>
<li><p><a href="/docs/zh-hant/analyzer-overview.md">分析器</a>：分析器會將輸入的文字切分為單一詞彙（即標記），並視需要套用篩選器。這使 Milvus 能根據這些標記建立索引。</p></li>
<li><p><a href="/docs/zh-hant/index-explained.md">建立索引</a>：完成文字分析後，Milvus 會建立一個倒排索引，將每個唯一的詞元映射至包含該詞元的文件。</p></li>
</ol>
<p>當使用者執行文字比對時，系統會利用倒排索引快速檢索所有包含該術語的文件。此方式遠比逐一掃描每份文件來得迅速。</p>
...
<h2 id="Enable-text-match" class="common-anchor-header">啟用文字比對<button data-href="#Enable-text-match" class="anchor-icon" translate="no">
    ...
    </button></h2><p>文字比對功能適用於已啟用比對功能的字串欄位。本頁面的範例使用 <a href="/docs/zh-hant/string.md"><code translate="no">VARCHAR</code></a>，該字段在所有客戶端 SDK 中均受支援。在 Milvus 3.0.x 版本中， <a href="/docs/zh-hant/text.md"><code translate="no">TEXT</code></a> 當啟用 Storage V3 時，字段亦支援文字比對。無論使用哪種字段類型，請將 `<code translate="no">enable_analyzer</code> ` 與 `<code translate="no">enable_match</code> ` 皆設定為 `<code translate="no">True</code>`，並可在定義集合架構時選擇性地配置<a href="/docs/zh-hant/analyzer-overview.md">分析器</a>。</p>
<h3 id="Set-enableanalyzer-and-enablematch" class="common-anchor-header">設定<code translate="no">enable_analyzer</code> 和<code translate="no">enable_match</code><button data-href="#Set-enableanalyzer-and-enablematch" class="anchor-icon" translate="no">
    ...
    </button></h3><p>若要為特定的<code translate="no">VARCHAR</code> 欄位啟用文字比對功能，請在定義欄位架構時，將<code translate="no">enable_analyzer</code> 和<code translate="no">enable_match</code> 兩項參數皆設定為<code translate="no">True</code> 。此設定會指示 Milvus 將文字進行分詞，並為指定欄位建立倒排索引，從而實現快速且高效的文字比對。</p>
...
<p>預設情況下，Milvus 使用<code translate="no">standard</code> 分析器，該分析器會根據空白字元和標點符號將文字分詞，移除長度超過 40 個字元的詞元，並將文字轉為小寫。套用此預設設定無需額外參數。如需更多資訊，請參閱<a href="/docs/zh-hant/standard-analyzer.md">「標準」</a>。</p>
<p>若需使用其他分析器，可透過 `<code translate="no">analyzer_params</code> ` 參數進行配置。例如，若要套用 `<code translate="no">english</code> ` 分析器來處理英文文字：</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#go">   Go</a>
...
<li><p><code translate="no">text</code>: 要搜尋的術語。多個術語可使用空格，或根據語言及已設定的分析器，使用其他適當的分隔符號進行分隔。</p></li>
</ul>
<p>預設情況下，<code translate="no">TEXT_MATCH</code> 採用「<strong>或 (OR)</strong>」匹配邏輯，這表示它會回傳包含任何指定術語的文件。例如，若要在<code translate="no">text</code> 欄位中搜尋包含<code translate="no">machine</code> 或<code translate="no">deep</code> 術語的文件，請使用以下表達式：</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#go">   Go</a>
...
<h3 id="TEXTMATCHFUZZY-expression-syntax--Milvus-300+" class="common-anchor-header">TEXT_MATCH_FUZZY 表達式語法<span class="beta-tag" style="background-color:rgb(0, 179, 255);color:white" translate="no">Compatible with Milvus 3.0.0+</span><button data-href="#TEXTMATCHFUZZY-expression-syntax--Milvus-300+" class="anchor-icon" translate="no">
    ...
    </button></h3><p>使用 `<code translate="no">TEXT_MATCH_FUZZY</code> ` 可容許查詢詞元與索引詞元之間的拼寫差異。Milvus 會使用該欄位的分析器來分析查詢文字，並對每個產生的詞元套用模糊比對。若查詢產生多個詞元，當任何一個詞元符合設定的編輯距離時，該表達式即會匹配該實體。</p>
<p>語法如下：</p>
...
<li><p><code translate="no">max_edit_distance</code>: 每個查詢標記允許的最大編輯距離。此選項名稱必須精確為<code translate="no">max_edit_distance</code> ，且其值必須為<code translate="no">0</code> 、<code translate="no">1</code> 或<code translate="no">2</code> 。若設定為<code translate="no">0</code> ，則執行精確標記比對，相當於<code translate="no">TEXT_MATCH</code> 。</p></li>
</ul>
...
<button class="copy-code-btn"></button></code></pre>
<p><code translate="no">TEXT_MATCH_FUZZY</code> 屬於篩選表達式語法的一部分，因此客戶端 SDK 無需提供專用的模糊匹配方法。請將該表達式傳遞至搜尋或查詢操作中，與<code translate="no">TEXT_MATCH</code> 所使用的相同<code translate="no">filter</code> 參數。</p>
<h3 id="Search-with-text-match" class="common-anchor-header">使用文字比對進行搜尋<button data-href="#Search-with-text-match" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>文字比對可與向量相似度搜尋結合使用，以縮小搜尋範圍並提升搜尋效能。透過在進行向量相似度搜尋前，先使用文字比對對集合進行篩選，可減少需要搜尋的文件數量，從而加快查詢速度。</p>
...
<div class="alert note">
<p>您可以透過設定文字高亮工具，在搜尋結果中標示出匹配的術語。詳情請參閱「<a href="/docs/zh-hant/text-highlighter.md">文字高亮工具</a>」。</p>
</div>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
...
<h3 id="Query-with-text-match" class="common-anchor-header">使用文字匹配進行查詢<button data-href="#Query-with-text-match" class="anchor-icon" translate="no">
    ...
    </button></h3><p>文字匹配亦可用於查詢操作中的標量篩選。透過在<code translate="no">query()</code> 方法的<code translate="no">expr</code> 參數中指定<code translate="no">TEXT_MATCH</code> 表達式，即可檢索與指定術語相符的文件。</p>
...
<li><p>為某個欄位啟用詞彙匹配功能會觸發建立倒排索引，這會消耗儲存資源。在決定是否啟用此功能時，請考量對儲存空間的影響，因為其影響程度會因文字大小、唯一詞元以及所使用的分析器而異。</p></li>
...
<ul>
<li><p>在表達式中，以雙引號或單引號包圍的字元會被解釋為字串常數。若字串常數包含轉義字元，則必須使用轉義序列來表示這些轉義字元。例如，使用<code translate="no">\\</code> 來表示<code translate="no">\</code> ，使用<code translate="no">\\t</code> 來表示制表符<code translate="no">\t</code> ，以及使用<code translate="no">\\n</code> 來表示換行符。</p></li>
...
<li><p>若字串常數以雙引號包圍，常數內的雙引號應寫作<code translate="no">\\&quot;</code> ，而單引號則可寫作<code translate="no">'</code> 或<code translate="no">\\'</code> 。範例：<code translate="no">&quot;He said \\&quot;Hi\\&quot;&quot;</code> 。</p></li>
</ul></li>
</ul>
