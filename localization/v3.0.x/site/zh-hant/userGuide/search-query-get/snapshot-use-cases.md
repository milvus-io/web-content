...
title: 快照使用情境Compatible with Milvus 3.0.x
summary: 在本指南中，您將了解快照的常見使用情境。
beta: Milvus 3.0.x
---
<h1 id="Snapshot-Use-Cases" class="common-anchor-header">快照使用情境<span class="beta-tag" style="background-color:rgb(0, 179, 255);color:white" translate="no">Compatible with Milvus 3.0.x</span><button data-href="#Snapshot-Use-Cases" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<h2 id="Data-backup-and-restoration" class="common-anchor-header">資料備份與還原<button data-href="#Data-backup-and-restoration" class="anchor-icon" translate="no">
    ...
    </button></h2><p>快照是資料的快速、特定時間點的影像，適用於快速回滾或測試（數天至數週）。與此同時，備份則是獨立且完整的副本，會另行儲存以供長期災難復原（數週至數年），並能更有效地防範儲存裝置完全故障。</p>
<p>下表比較了快照與備份的差異。</p>
<table>
   <tr>
     <th></th>
