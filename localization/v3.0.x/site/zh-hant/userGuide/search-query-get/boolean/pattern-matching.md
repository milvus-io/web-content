...
<h1 id="Pattern-Matching" class="common-anchor-header">模式比對<button data-href="#Pattern-Matching" class="anchor-icon" translate="no">
    ...
    </button></h1><p>在基於代理的搜尋應用中，向量搜尋與 grep 風格的模式比對往往相輔相成。向量搜尋會檢索出語義相關的實體，而模式比對則透過精確的字串結構（例如錯誤代碼、日誌前綴、電子郵件網域、URL 路徑或識別碼）來縮小搜尋結果範圍。</p>
...
<p>本頁說明<code translate="no">query</code> 、<code translate="no">search</code> 以及混合搜尋所使用的標量篩選器表達式中的模式匹配。這些表達式會評估欄位值，但不會變更分析器所產生的標記。若要在文字分析期間篩選標記，請參閱「<a href="/docs/zh-hant/regex-filter.md">正規表達式分析器篩選器</a>」。</p>
</div>
<p>模式匹配表達式需寫入 `<code translate="no">filter</code> ` 參數中。例如，以下查詢會匹配包含如<code translate="no">E1001</code> 等錯誤代碼的日誌訊息：</p>
...
<table>
<thead>
<tr><th>目標</th><th><code translate="no">LIKE</code></th><th>正規表示式<code translate="no">=~</code> /<code translate="no">!~</code></th><th>注意事項</th></tr>
</thead>
<tbody>
<tr><td><code translate="no">VARCHAR</code> 欄位</td><td>是</td><td>是</td><td>字串欄位進行模式比對的典型目標。</td></tr>
<tr><td><code translate="no">JSON</code> 路徑，使用<code translate="no">VARCHAR</code> 進行型別轉換</td><td>是</td><td>是</td><td>若要獲得正向匹配，JSON 路徑值必須為字串。若您為加速目的在 JSON 路徑上建立索引，請設定<code translate="no">json_cast_type=&quot;varchar&quot;</code> 。</td></tr>
...
<tr><td>數值、布林值、向量、<code translate="no">TEXT</code> 或其他非<code translate="no">VARCHAR</code> 目標</td><td>否</td><td>否</td><td>模式匹配僅適用於<code translate="no">VARCHAR</code> 值、可解析為字串的JSON路徑，或具有索引的<code translate="no">ARRAY&lt;VARCHAR&gt;</code> 元素。</td></tr>
</tbody>
</table>
...
<p>若需進行字串完全比對，建議您使用 `<code translate="no">==</code> ` 而非模式比對。僅當篩選條件需要比對特定模式時，才應使用 `<code translate="no">LIKE</code> ` 或正規表達式。</p>
<table>
<thead>
<tr><th>需求</th><th>建議運算子</th><th>範例</th><th>說明</th></tr>
</thead>
<tbody>
<tr><td>字串完全相等</td><td><code translate="no">==</code></td><td><code translate="no">status == &quot;active&quot;</code></td><td>與字串<code translate="no">active</code> 完全匹配。</td></tr>
...
<h3 id="LIKE-matching-behavior" class="common-anchor-header">LIKE 匹配行為<button data-href="#LIKE-matching-behavior" class="anchor-icon" translate="no">
    ...
    </button></h3><p>請使用<code translate="no">LIKE</code> 進行前綴、後綴、包含及固定位置單一字元比對。<code translate="no">LIKE</code> 不支援字元類別（例如<code translate="no">[0-9]</code> ）、選擇符（例如<code translate="no">error|failed</code> ）、重複次數（例如<code translate="no">{4}</code> ）、錨點（例如<code translate="no">^</code> 或<code translate="no">$</code> ），亦不支援不區分大小寫的標誌（例如<code translate="no">(?i)</code> ）。若需使用此類模式，請改用正規表達式。</p>
...
<p>原始字串字面值（以<code translate="no">r&quot;...&quot;</code> 或<code translate="no">r'...'</code> 形式書寫）會在 Milvus 篩選器表達式中保留反斜線的原始形式。建議在<code translate="no">LIKE</code> 以及包含反斜線的正規表示式中使用此形式。若未使用原始字串，一般字串字面值在評估模式前仍會處理轉義序列，因此可能需要更多反斜線。</p>
...
<tbody>
<tr><td><code translate="no">=~</code></td><td>匹配符合正規表示式模式的值。</td><td><code translate="no">filter = 'message =~ &quot;E[0-9]{4}&quot;'</code></td></tr>
...
<h3 id="Use-raw-string-literals" class="common-anchor-header">使用原始字串文字<button data-href="#Use-raw-string-literals" class="anchor-icon" translate="no">
    ...
    </button></h3><p>對於包含反斜線的正規表達式模式，建議使用原始字串文字。在以<code translate="no">r&quot;...&quot;</code> 或<code translate="no">r'...'</code> 形式寫入的原始字串中，反斜線會原樣傳遞給正規表達式引擎。這可避免一般字串文字所需的額外轉義。</p>
<p>例如：</p>
<div class="multipleCode">
 <a href="#python">Python</a>
 <a href="#java"> Java</a>
...
<p>若 Milvus 篩選器表達式中未使用原始字串，一般字串文字會在評估正規表達式模式之前先處理轉義序列。因此，經過轉義的字元可能需要在主語言的字串中額外添加反斜線。</p>
...
<p>若要匹配多個單字中的任一個，請使用「<code translate="no">|</code> 」進行交替匹配：</p>
<div class="multipleCode">
 <a href="#python">Python</a>
 <a href="#java"> Java</a>
 <a href="#go"> Go</a>
...
<button class="copy-code-btn"></button></code></pre>
<p>若要字面匹配正規表達式的元字元，請在正規表達式模式中對其進行轉義。例如，若要匹配字面上的點（在正規表達式中為 `<code translate="no">\.</code> `），請在 Python、Java、Go 或 Node.js 的原始碼字串中寫入 `<code translate="no">\\.</code> `：</p>
<div class="multipleCode">
 <a href="#python">Python</a>
 <a href="#java"> Java</a>
 <a href="#go"> Go</a>
...
<p>正規表達式篩選器不會匹配 null 值。這適用於<code translate="no">=~</code> 和<code translate="no">!~</code> 兩種錨點。若要排除某個正規表達式模式但保留 null 值，請明確添加<code translate="no">OR field IS NULL</code> ：</p>
<div class="multipleCode">
 <a href="#python">Python</a>
 <a href="#java"> Java</a>
 <a href="#go"> Go</a>
...
<tbody>
<tr><td><code translate="no">json_field[&quot;path&quot;] =~ &quot;pattern&quot;</code></td><td>否</td><td>僅匹配符合正則表達式模式的字串值。</td></tr>
...
<h2 id="Accelerate-pattern-matching-with-indexes" class="common-anchor-header">利用索引加速模式比對<button data-href="#Accelerate-pattern-matching-with-indexes" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Milvus 支援多種適用於字串欄位的索引類型，可與<code translate="no">LIKE</code> 以及針對<code translate="no">VARCHAR</code> 欄位或 JSON 字串路徑的正規表示式篩選器搭配使用，例如<code translate="no">NGRAM</code> 、<code translate="no">STL_SORT</code> 、<code translate="no">INVERTED</code> 以及<code translate="no">BITMAP</code> 。模式匹配雖可在無索引的情況下運作，但使用索引可提升大型資料集的處理效能。</p>
...
<tbody>
<tr><td>包含固定的字面值子字串，例如<code translate="no">message =~ &quot;error.*timeout&quot;</code> 或<code translate="no">message LIKE &quot;%database%&quot;</code></td><td><code translate="no">NGRAM</code></td><td>當 Milvus 能從模式中提取有意義的字面值子字串時，此特性會有所幫助。詳情請參閱<a href="/docs/zh-hant/ngram.md">NGRAM</a>。</td></tr>
<tr><td>前綴、精確或等值類型的字串篩選器，特別適用於基數低至中等的欄位</td><td><code translate="no">STL_SORT</code>、<code translate="no">INVERTED</code> ，或<code translate="no">BITMAP</code></td><td>當欄位具有重複值，或篩選條件接近完全匹配時，此方法可能更為有效。詳情請參閱<a href="/docs/zh-hant/stl-sort.md">STL_SORT</a>、<a href="/docs/zh-hant/inverted.md">INVERTED</a> 及<a href="/docs/zh-hant/bitmap.md">BITMAP</a>。</td></tr>
