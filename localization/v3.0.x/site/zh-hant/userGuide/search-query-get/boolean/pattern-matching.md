...
<tr><td>數值、布林值、向量、<code translate="no">TEXT</code> 或其他非<code translate="no">VARCHAR</code> 目標</td><td>否</td><td>否</td><td>模式匹配僅適用於 `<code translate="no">VARCHAR</code> ` 值、解析為字串的 JSON 路徑，或已建立索引的 `<code translate="no">ARRAY&lt;VARCHAR&gt;</code> ` 元素。</td></tr>
</tbody>
</table>
...
<button class="copy-code-btn"></button></code></pre>
<p>若要字面匹配正規表達式中的元字元，請在正規表達式模式中對其進行轉義。例如，若要匹配字面上的點（在正規表達式中為 `<code translate="no">\.</code> `），請在 Python 篩選器字串中寫作 `<code translate="no">\\.</code> `：</p>
