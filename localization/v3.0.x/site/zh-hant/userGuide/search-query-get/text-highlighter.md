...
summary: >-
  ...
  通過使用自定義標籤來包裝文本字段中的匹配詞彙，從而對其進行註釋。突出顯示有助於解釋文件匹配的原因，提高結果的可讀性，並支援搜索和 RAG
  應用程式中的豐富渲染。
beta: Milvus 2.6.8+
---
<h1 id="Text-Highlighter" class="common-anchor-header">文字高亮器<span class="beta-tag" style="background-color:rgb(0, 179, 255);color:white" translate="no">Compatible with Milvus 2.6.8+</span><button data-href="#Text-Highlighter" class="anchor-icon" translate="no">
    ...
    </button></h1><p>Milvus 的文字高亮器會用可自訂的標籤包裝文字欄位中的匹配詞彙。高亮顯示有助於解釋文件匹配的原因，提高結果的可讀性，並支援搜尋和 RAG 應用程式的豐富渲染。</p>
...
<p>若要在 BM25 全文搜尋中高亮顯示搜尋詞彙，請建立<code translate="no">LexicalHighlighter</code> ，並啟用 BM25 全文搜尋的搜尋詞彙高亮顯示功能：</p>
...
<li><p><code translate="no">highlight_search_text=True</code> 告訴 Milvus 使用 BM25 全文搜尋中的搜尋詞彙作為高亮詞彙的來源。</p></li>
</ul>
...
<li><p>高亮器用<code translate="no">{</code> 和<code translate="no">}</code> 標籤包圍所有出現的 "test"。</p></li>
</ul>
...
<li><p>高亮器將所有匹配的詞彙 (<code translate="no">&quot;my&quot;</code>,<code translate="no">&quot;test&quot;</code>,<code translate="no">&quot;doc&quot;</code>) 包裝為<code translate="no">{</code> 和<code translate="no">}</code></p></li>
</ul>
<pre><code translate="no" class="language-python"><span class="highlighted-comment-line">highlighter = LexicalHighlighter(</span>
<span class="highlighted-comment-line">    pre_tags=[<span class="hljs-string">&quot;{&quot;</span>],</span>
<span class="highlighted-comment-line">    post_tags=[<span class="hljs-string">&quot;}&quot;</span>],</span>
