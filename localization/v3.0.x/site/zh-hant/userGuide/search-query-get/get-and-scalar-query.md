...
title: 查詢
summary: 在 Milvus 中，使用 Query、Get 和 QueryIterator 來擷取實體並篩選元資料。
---
...
<p>若在建立集合後新增欄位，包含這些欄位的查詢對於未明確設定值的實體，將返回已定義的預設值或 `<code translate="no">NULL</code> `。詳細資訊請參閱「<a href="/docs/zh-hant/add-fields-to-an-existing-collection.md">變更集合架構</a>」。</p>
</div>
<h2 id="Overview" class="common-anchor-header">概述<button data-href="#Overview" class="anchor-icon" translate="no">
    ...
    </button></h2><p>一個集合（Collection）可儲存各種類型的標量欄位。您可以讓 Milvus 根據一個或多個標量欄位來過濾實體（Entities）。Milvus 提供三種查詢類型：Query、Get 和 QueryIterator。下表比較了這三種查詢類型。</p>
<table>
   <tr>
     <th></th>
     <th><p>Get</p></th>
   ...
   <tr>
     ...
     <td><p>用以查找所有或指定數量符合自訂篩選條件的實體</p></td>
   ...
   <tr>
     <td><p>回傳</p></td>
     <td><p>回傳指定集合或區段中，擁有指定主鍵的實體。</p></td>
...
<h2 id="Use-Query" class="common-anchor-header">使用 Query<button data-href="#Use-Query" class="anchor-icon" translate="no">
    ...
    </button></h3><p>當您需要根據自訂篩選條件尋找實體時，請使用<strong>Query</strong>方法。以下程式碼範例假設集合中存在三個名為<code translate="no">id</code> 、<code translate="no">vector</code> 和<code translate="no">color</code> 的欄位，並會回傳指定數量、且其<code translate="no">color</code> 值以<code translate="no">red</code> 開頭的實體。</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#go">   Go</a>
...
<h3 id="Sort-Query-Results--Milvus-30x" class="common-anchor-header">排序查詢結果<span class="beta-tag" style="background-color:rgb(0, 179, 255);color:white" translate="no">Compatible with Milvus 3.0.x</span><button data-href="#Sort-Query-Results--Milvus-30x" class="anchor-icon" translate="no">
    ...
    </button></h3><p>預設情況下，Query 會以未指定的順序返回結果。請使用<code translate="no">order_by</code> 參數，依據一個或多個標量欄位對結果進行排序。使用<code translate="no">order_by</code> 時，請注意：</p>
<ul>
<li><p><code translate="no">order_by</code> 必須與 `<code translate="no">limit</code>` 參數一併使用。</p></li>
<li><p>支援的欄位類型：<code translate="no">INT8</code> 、<code translate="no">INT16</code> 、<code translate="no">INT32</code> 、<code translate="no">INT64</code> 、<code translate="no">FLOAT</code> 、<code translate="no">DOUBLE</code> 以及<code translate="no">VARCHAR</code> 。不支援根據向量、<code translate="no">JSON</code> 或<code translate="no">ARRAY</code> 欄位進行排序。</p></li>
<li><p>當依可為空欄位排序時，在升序排序下（NULLS LAST），NULL 值會置於末尾；在降序排序下（NULLS FIRST），則置於開頭。</p></li>
</ul>
<h4 id="Basic-Sort" class="common-anchor-header">基本排序</h4><p>將一組<code translate="no">&quot;field_name:direction&quot;</code> 字串傳遞給<code translate="no">order_by</code> 參數，其中<code translate="no">direction</code> 的取值為<code translate="no">asc</code> （升序）或<code translate="no">desc</code> （降序）。請注意，<code translate="no">asc</code> 和<code translate="no">desc</code> 會區分大小寫。</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#go">   Go</a>
...
<button class="copy-code-btn"></button></code></pre>
<h4 id="Multi-field-Sort" class="common-anchor-header">多欄位排序</h4><p>您可以同時依據多個欄位進行排序。結果會先根據清單中的第一個欄位進行排序。當兩行在該欄位中的值相同時，則由第二個欄位決定其順序，依此類推。</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#go">   Go</a>
...
<h2 id="Use-QueryIterator" class="common-anchor-header">使用 QueryIterator<button data-href="#Use-QueryIterator" class="anchor-icon" translate="no">
    ...
    </button></h2><p>當您需要透過分頁查詢，依據自訂篩選條件查找實體時，請建立<strong>一個 QueryIterator</strong>，並使用其<strong>next()</strong>方法遍歷所有實體，以找出符合篩選條件的實體。以下程式碼範例假設存在三個名為<code translate="no">id</code> 、<code translate="no">vector</code> 和<code translate="no">color</code> 的欄位，並會傳回所有<code translate="no">color</code> 值以<code translate="no">red</code> 開頭的實體。</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#go">   Go</a>
...
<h2 id="Queries-in-Partitions" class="common-anchor-header">分區內的查詢<button data-href="#Queries-in-Partitions" class="anchor-icon" translate="no">
    ...
    </button></h2><p>您也可以透過在 Get、Query 或 QueryIterator 請求中包含區隔名稱，在一個或多個區隔內執行查詢。以下程式碼範例假設集合中存在一個名為<strong>PartitionA</strong>的區隔。</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#go">   Go</a>
...
<h2 id="Random-Sampling-with-Query" class="common-anchor-header">使用 Query 進行隨機抽樣<button data-href="#Random-Sampling-with-Query" class="anchor-icon" translate="no">
    ...
    </button></h2><p>若要從集合中擷取具代表性的資料子集，以進行資料探索或開發測試，請使用<code translate="no">RANDOM_SAMPLE(sampling_factor)</code> 表達式，其中<code translate="no">sampling_factor</code> 為介於 0 至 1 之間的浮點數，代表要採樣的資料百分比。</p>
<div class="alert note">
