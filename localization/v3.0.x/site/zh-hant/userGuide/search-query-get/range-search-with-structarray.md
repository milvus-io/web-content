...
<tr><td>混合搜尋</td><td>當 StructArray 請求針對元素層級的向量欄位時，此功能受支援。EmbeddingList 層級的請求不支援範圍搜尋。</td><td>先進行元素層級子搜尋，再進行混合重新排序。</td></tr>
</tbody>
</table>
<div class="alert note">
<p>若您僅需最接近的 Struct 元素，請先<a href="/docs/zh-hant/basic-vector-search-with-structarray.md">使用 StructArray 進行基本向量搜尋</a>。當結果必須滿足分數或距離限制（而非僅限前 K 名排名）時，請使用範圍搜尋。</p>
</div>
...
<tr><td>元素層級的向量子欄位</td><td>目標向量子欄位為<code translate="no">chunks[emb]</code> ，而非<code translate="no">chunks[emb_list_vector]</code> 。</td></tr>
...
<tr><td><code translate="no">IP</code>,<code translate="no">COSINE</code></td><td>是的。分數越高越好。</td><td><code translate="no">radius &lt; distance &lt;= range_filter</code></td></tr>
</tbody>
</table>
<p>當僅設定 `<code translate="no">radius</code> ` 時，範圍搜尋會回傳符合該度量外界限的結果。請根據您的嵌入模型的評分或距離尺度來選擇數值。</p>
<h2 id="Run-element-level-range-search" class="common-anchor-header">執行元素層級的範圍搜尋<button data-href="#Run-element-level-range-search" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<button class="copy-code-btn"></button></code></pre>
<p>在此範例中，僅有 `<code translate="no">chunks[emb]</code> ` 子請求會使用範圍搜尋參數。StructArray 請求仍遵循元素層級語義：範圍邊界適用於 Struct 元素的搜尋結果，此後混合搜尋才會將結果合併並重新排序。</p>
<h2 id="Interpret-range-results" class="common-anchor-header">解析範圍搜尋結果<button data-href="#Interpret-range-results" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<tbody>
<tr><td><code translate="no">id</code></td><td>包含匹配 Struct 元素的實體之主鍵。</td></tr>
...
<tr><td>重複的主鍵</td><td>可能發生。同一實體中可能有多個 Struct 元素落在指定範圍內。</td></tr>
...
<li><p>預期範圍搜尋結果會根據父實體而唯一。範圍搜尋會返回符合條件的 Struct 元素結果。</p></li>
