...
<h1 id="Using-Full-Text-Search-with-LangChain-and-Milvus" class="common-anchor-header">使用 LangChain 和 Milvus 進行全文檢索<button data-href="#Using-Full-Text-Search-with-LangChain-and-Milvus" class="anchor-icon" translate="no">
    ...
    </button></h1><p><a href="https://milvus.io/docs/full-text-search.md#Full-Text-Search">全文</a>檢索是一種透過匹配文字中特定關鍵字或短語來檢索文件的傳統方法。它會根據詞彙頻率等因素計算出的相關性分數對結果進行排序。語意搜尋更擅長於理解意義和上下文，而全文搜尋則擅長於精確的關鍵字匹配，因此是語意搜尋的有效補充。BM25 演算法廣泛用於全文檢索的排序，並在檢索增強世代 (Retrieval-Augmented Generation, RAG) 中扮演關鍵角色。</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>指定您的 Milvus 伺服器<code translate="no">URI</code> (以及可選的<code translate="no">TOKEN</code>)。關於如何安裝和啟動 Milvus 伺服器，請參考本<a href="https://milvus.io/docs/install_standalone-docker-compose.md">指南</a>。</p>
...
<h2 id="Initialization-with-BM25-Function" class="common-anchor-header">使用 BM25 功能初始化<button data-href="#Initialization-with-BM25-Function" class="anchor-icon" translate="no">
    ...
    </button></h2><h3 id="Hybrid-Search" class="common-anchor-header">混合搜尋<button data-href="#Hybrid-Search" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>對於全文檢索，Milvus VectorStore 接受一個<code translate="no">builtin_function</code> 參數。透過這個參數，您可以傳入<code translate="no">BM25BuiltInFunction</code> 的一個實例。這與語意搜尋不同，語意搜尋通常會傳入密集的嵌入到<code translate="no">VectorStore</code> 、</p>
...
<ul>
<li><code translate="no">input_field_names</code> (str)：輸入欄位的名稱，預設值是<code translate="no">text</code> 。它表示這個函式讀取哪一個欄位作為輸入。</li>
<li><code translate="no">output_field_names</code> (str)：輸出欄位的名稱，預設為<code translate="no">sparse</code> 。它表示這個函式將計算結果輸出到哪一個欄位。</li>
</ul>
...
<h3 id="BM25-search-without-embedding" class="common-anchor-header">無嵌入的BM25搜尋<button data-href="#BM25-search-without-embedding" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>如果您只想使用 BM25 函式執行全文搜尋，而不使用任何基於 embedding 的語意搜尋，您可以設定 embedding 參數為<code translate="no">None</code> ，並只保留指定為 BM25 函式實例的<code translate="no">builtin_function</code> 。向量欄位只有「稀疏」欄位。舉例來說</p>
...
<h2 id="Customize-analyzer" class="common-anchor-header">自訂分析器<button data-href="#Customize-analyzer" class="anchor-icon" translate="no">
    ...
    </button></h2><p>分析器在全文檢索中是不可或缺的，它可將句子分割成字元，並執行詞彙分析，如詞幹分析和停止詞移除。分析器通常是特定語言的。您可以參考<a href="https://milvus.io/docs/analyzer-overview.md#Analyzer-Overview">本指南</a>以瞭解更多關於 Milvus 分析器的資訊。</p>
...
<h3 id="Multi-language-Analyzer" class="common-anchor-header">多語言分析器<button data-href="#Multi-language-Analyzer" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Milvus 支援多語言分析器，可分析多種語言的文件。在<code translate="no">BM25BuiltInFunction</code> 中使用<code translate="no">multi_analyzer_params</code> ：</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>詳情請參閱<a href="https://milvus.io/docs/multi-language-analyzers.md">多語言分析器文件</a>。</p>
