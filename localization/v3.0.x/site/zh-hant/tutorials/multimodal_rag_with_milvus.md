...
<p>本教學展示了由 Milvus、<a href="https://github.com/FlagOpen/FlagEmbedding/tree/master/FlagEmbedding/visual">可視化 BGE 模型及</a> <a href="https://openai.com/index/hello-gpt-4o/">GPT-4o</a> 驅動的多模態 RAG 系統。透過此系統，使用者可上傳圖片並編輯文字指令，這些指令將由 BGE 的複合檢索模型進行處理，以搜尋候選圖片。 隨後，GPT-4o 將擔任重新排序器（reranker）的角色，篩選出最合適的圖片並提供選擇的理由。這套強大的組合能提供流暢且直觀的圖片搜尋體驗，結合 Milvus 實現高效檢索、BGE 模型進行精準的圖片處理與匹配，以及 GPT-4o 執行進階的重新排序。</p>
...
<h3 id="Download-Data" class="common-anchor-header">下載資料<button data-href="#Download-Data" class="anchor-icon" translate="no">
    ...
    </button></h3><p>以下指令將下載範例資料並解壓縮至本機資料夾「./images_folder」，內容包含：</p>
<ul>
<li><strong>images</strong>：<a href="https://github.com/hyp1231/AmazonReviews2023">Amazon Reviews 2023</a>資料集的子集，包含來自「家電」、「手機與配件」及「電子產品」類別的約 900 張圖片。</li>
<li><strong>leopard.jpg</strong>：一個示例查詢圖片。</li>
</ul>
...
<button class="copy-code-btn"></button></code></pre>
<h3 id="Load-Embedding-Model" class="common-anchor-header">載入嵌入式模型<button data-href="#Load-Embedding-Model" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>我們將使用可視覺化的 BGE 模型「bge-visualized-base-en-v1.5」來為圖片和文字生成嵌入向量。</p>
...
<h2 id="Load-Data" class="common-anchor-header">載入資料<button data-href="#Load-Data" class="anchor-icon" translate="no">
    ...
    </button></h2><p>本節將把範例圖片連同對應的嵌入向量載入至資料庫中。</p>
...
<h3 id="Insert-into-Milvus" class="common-anchor-header">插入至 Milvus<button data-href="#Insert-into-Milvus" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>將圖片連同對應的路徑與嵌入向量插入 Milvus 集合中。</p>
<div class="alert note">
<p>關於 `<code translate="no">MilvusClient</code>` 的參數：</p>
<ul>
<li>將 `<code translate="no">uri</code> ` 設定為本機檔案（例如：<code translate="no">./milvus_demo.db</code> ）是最便捷的方法，因為系統會自動利用<a href="https://milvus.io/docs/milvus_lite.md">Milvus Lite</a>將所有資料儲存至該檔案中。</li>
<li>若您擁有大規模資料，可於<a href="https://milvus.io/docs/quickstart.md">Docker 或 Kubernetes</a> 上架設效能更佳的 Milvus 伺服器。在此設定中，請使用伺服器 URI（例如<code translate="no">http://localhost:19530</code> ）作為您的<code translate="no">uri</code> 。</li>
<li>若欲使用<a href="https://zilliz.com/cloud">Zilliz Cloud</a>（Milvus 的全託管雲端服務），請調整<code translate="no">uri</code> 和<code translate="no">token</code> ，這兩者分別對應 Zilliz Cloud 中的「<a href="https://docs.zilliz.com/docs/on-zilliz-cloud-console#free-cluster-details">公開端點</a>」<a href="https://docs.zilliz.com/docs/on-zilliz-cloud-console#free-cluster-details">與「API 金鑰」</a>。</li>
</ul>
</div>
...
<h2 id="Multimodal-Search-with-Generative-Reranker" class="common-anchor-header">結合生成式重新排序器的多模態搜尋<button data-href="#Multimodal-Search-with-Generative-Reranker" class="anchor-icon" translate="no">
    ...
    </button></h2><p>在本節中，我們將首先透過多模態查詢搜尋相關圖片，然後使用 LLM 服務對結果進行重新排序，並找出最佳結果及其解釋。</p>
<h3 id="Run-search" class="common-anchor-header">執行搜尋<button data-href="#Run-search" class="anchor-icon" translate="no">
    ...
    </button></h3><p>現在我們已準備好執行進階圖片搜尋，查詢資料由圖片與文字指示共同組成。</p>
...
<h3 id="Rerank-with-GPT-4o" class="common-anchor-header">使用 GPT-4o 重新排序<button data-href="#Rerank-with-GPT-4o" class="anchor-icon" translate="no">
    ...
    </button></h3><p>我們將利用大型語言模型（LLM）根據使用者查詢與檢索結果，對圖片進行排序，並為最佳結果生成解釋。</p>
<p><strong>1. 建立全景視圖</strong></p>
...
<p>我們將把合併後的圖片連同適當的提示語一併傳送至多模態大型語言模型（LLM）服務，以對檢索結果進行排序並提供解釋。若要啟用 GPT-4o 作為大型語言模型，您需要準備您的<a href="https://platform.openai.com/docs/quickstart">OpenAI API 金鑰</a>。</p>
...
<button class="copy-code-btn"></button></code></pre>
<p><strong>3. 顯示最佳結果及其解釋</strong></p>
