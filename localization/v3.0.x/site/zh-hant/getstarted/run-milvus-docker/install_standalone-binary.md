...
title: 使用 RPM/DEB 套件安裝 Milvus 獨立版本
---
<h1 id="Install-Milvus-Standalone-with-RPMDEB-Package" class="common-anchor-header">使用 RPM/DEB 套件安裝 Milvus 獨立版本<button data-href="#Install-Milvus-Standalone-with-RPMDEB-Package" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<h2 id="Download-the-RPMDEB-Package" class="common-anchor-header">下載 RPM/DEB 套件<button data-href="#Download-the-RPMDEB-Package" class="anchor-icon" translate="no">
    ...
    </button></h2><p>您可以從<a href="https://github.com/milvus-io/milvus/releases/tag/v3.0.2">Milvus 發行版頁面</a>，根據您的系統架構下載 RPM/DEB 套件。</p>
<ul>
<li>若為 x86_64/amd64 架構，請下載<strong>milvus_3.0.2-1_amd64.deb</strong>或<strong>milvus_3.0.2-1_amd64.rpm</strong>套件。</li>
<li>若為 ARM64 架構，請下載<strong>milvus_3.0.2-1_arm64.deb</strong>或<strong>milvus_3.0.2-1_arm64.rpm</strong>套件。</li>
</ul>
...
<p>對於基於 RPM 的系統（例如 CentOS、Fedora 和 RHEL），請使用<code translate="no">yum</code> 命令來安裝套件。</p>
...
<h2 id="Start-Milvus-Standalone" class="common-anchor-header">啟動 Milvus 獨立模式<button data-href="#Start-Milvus-Standalone" class="anchor-icon" translate="no">
    ...
    </button></h2><p>安裝完成後，Milvus 會以 systemd 服務的形式安裝，並可透過以下指令啟動：</p>
...
<h2 id="Optional-Update-Milvus-configurations" class="common-anchor-header">（可選）更新 Milvus 設定<button data-href="#Optional-Update-Milvus-configurations" class="anchor-icon" translate="no">
    ...
    </button></h2><p>您可修改<code translate="no">/etc/milvus/configs/milvus.yaml</code> 檔案中的 Milvus 設定。例如，若要將<code translate="no">proxy.healthCheckTimeout</code> 變更為<code translate="no">1000</code> 訊息服務，可搜尋 target 參數並據此修改。有關適用的設定項目，請參閱<a href="/docs/zh-hant/system_configuration.md">《系統設定》</a>。</p>
...
<h2 id="Uninstall-Milvus-Standalone" class="common-anchor-header">解除安裝 Milvus 獨立版<button data-href="#Uninstall-Milvus-Standalone" class="anchor-icon" translate="no">
    ...
    </button></h2><p>若要解除安裝 Milvus 獨立版本，可使用以下指令：</p>
<p>針對基於 RPM 的系統：</p>
...
<button class="copy-code-btn"></button></code></pre>
<div class="alert note">
<p>Storage V3 預設為停用狀態。在使用依賴此功能的服務前，請先啟用它。有關系統需求與相容性考量，請參閱<a href="/docs/zh-hant/storage-v3.md">Storage V3</a>。</p>
</div>
...
<li><p>探索<a href="/docs/zh-hant/birdwatcher_overview.md">Birdwatcher</a>——一款用於除錯 Milvus 及更新動態設定的開源工具。</p></li>
