...
<li><strong>架構優化</strong>：整合元件以提升效能</li>
</ul>
...
<li>一個嵌入式 etcd 已與 Milvus 一併安裝在同一容器中，並於<strong>2379</strong> 埠提供服務。其設定檔對應至當前資料夾中的<strong>embedEtcd.yaml</strong>檔案。</li>
...
<h2 id="Optional-dependencies" class="common-anchor-header">可選的依賴項<button data-href="#Optional-dependencies" class="anchor-icon" translate="no">
    ...
    </button></h2><p>預設情況下，此部署會運行<strong>Woodpecker</strong>（本地檔案系統 WAL）作為訊息佇列，並<strong>使用內嵌的 etcd</strong>來儲存元資料 — 無需安裝其他組件。若要使用不同的訊息佇列或連接外部物件儲存／元資料，請參閱：</p>
<ul>
