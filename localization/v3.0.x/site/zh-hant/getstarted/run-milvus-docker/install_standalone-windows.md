...
label: Docker
related_key: Docker
summary: 瞭解如何使用 Windows 版的 Docker Desktop 安裝 Milvus 獨立執行版本。
...
<p>預設情況下，此部署會將<strong>Woodpecker</strong>（本機檔案系統）作為訊息佇列，因此無需外部訊息佇列服務。請參閱<a href="/docs/zh-hant/woodpecker.md">Woodpecker</a>。</p>
</blockquote>
...
<h2 id="Run-Milvus-in-Docker​" class="common-anchor-header">在 Docker 中執行 Milvus​<button data-href="#Run-Milvus-in-Docker​" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Milvus 提供安裝腳本，可將其安裝為 Docker 容器。在 Microsoft Windows 上安裝 Docker Desktop 後，您即可透過以<strong>系統管理員模式運作的</strong>PowerShell 或 Windows 命令提示字元，以及透過 WSL 2 存取 Docker CLI。​</p>
...
<p>視您的網路連線狀況而定，下載 Milvus 安裝所需的映像檔可能需要一些時間。一旦名為<strong>milvus-standalone</strong>、<strong>milvus-minio</strong> 和<strong>milvus-etcd</strong>的容器啟動後，您將看到 ​</p>
<ul>
<li><p><strong>milvus-etcd</strong>容器並未向主機暴露任何端口，且將其資料映射至當前資料夾中的<strong>volumes/etcd​</strong></p></li>
<li><p><strong>milvus-minio</strong>容器在本地提供<strong>9000</strong>和<strong>9001</strong>埠，並使用預設的驗證憑證，其資料映射至當前資料夾中的<strong>volumes/minio​</strong>。​</p></li>
...
<h3 id="From-WSL-2​" class="common-anchor-header">透過 WSL 2​<button data-href="#From-WSL-2​" class="anchor-icon" translate="no">
    ...
    </button></h3><p>此操作步驟與在 Linux 系統中使用 Docker Compose 安裝 Milvus 的方式類似。​</p>
<ol>
<li><p>啟動 WSL 2。​</p>
<pre><code translate="no" class="language-powershell">C:\&gt;wsl --install​
Ubuntu already installed.​
...
<h2 id="FAQs​" class="common-anchor-header">常見問題​<button data-href="#FAQs​" class="anchor-icon" translate="no">
    ...
    </button></h3><p>在 Windows 上安裝 Docker Desktop 後，若電腦未正確設定，可能會遇到「<code translate="no">Docker Engine stopped</code> 」錯誤。此時，您可能需要進行以下檢查：​</p>
<ol>
<li><p>檢查虛擬化功能是否已啟用。​</p>
...
<p>若虛擬化功能已停用，您可能需要檢查主機板韌體的 BIOS 設定。在 BIOS 設定中啟用虛擬化功能的方法會因主機板廠商而異。以 ASUS<a href="https://www.asus.com/support/faq/1043786/">主機板為例，您可以參考這篇文章來了解如何</a>啟<a href="https://www.asus.com/support/faq/1043786/">用虛擬化功能</a>。​</p>
...
<h3 id="How-can-I-deal-with-the-volume-related-errors-prompted-during-Milvus-startup-that-reads-Read-config-failed​" class="common-anchor-header">如何處理 Milvus 啟動時出現的與卷相關的錯誤提示，內容顯示為「<code translate="no">Read config failed</code> 」？​<button data-href="#How-can-I-deal-with-the-volume-related-errors-prompted-during-Milvus-startup-that-reads-Read-config-failed​" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
   ...
   <span>Milvus 啟動時出現的「讀取設定失敗」錯誤提示</span>
  
 </span></p>
<p>若要處理 Milvus 啟動時出現的「讀取設定檔失敗（<code translate="no">docker exec</code> ）」錯誤，您需要檢查掛載至 Milvus 容器中的儲存區是否正確。若儲存區已正確掛載至容器中，您可以使用 指令進入容器，並如下所示列出<strong>/milvus/configs</strong>資料夾：​</p>
<p><span class="img-wrapper">
