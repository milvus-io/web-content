...
label: Docker
related_key: Docker
summary: 瞭解如何使用 Windows 版的 Docker Desktop 安裝 Milvus 獨立版本。
title: 在 Docker 中執行 Milvus（Windows）
---
...
<p>預設情況下，此部署會將<strong>Woodpecker</strong>（本地檔案系統）用作訊息佇列，因此無需外部訊息佇列服務。請參閱<a href="/docs/zh-hant/woodpecker.md">Woodpecker</a>。</p>
</blockquote>
<p>本頁面將示範如何使用 Docker Desktop for Windows 在 Windows 上執行 Milvus。​</p>
...
<h2 id="Run-Milvus-in-Docker​" class="common-anchor-header">在 Docker 中執行 Milvus​<button data-href="#Run-Milvus-in-Docker​" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Milvus 提供安裝腳本，可將其安裝為 Docker 容器。在 Microsoft Windows 上安裝 Docker Desktop 後，您即可透過以<strong>系統管理員模式</strong>執行的 PowerShell 或 Windows 命令提示字元，以及透過 WSL 2 存取 Docker CLI。​</p>
...
<li><p>隨 Milvus 一起安裝的嵌入式 etcd 位於同一容器中，並在<strong>2379</strong> 埠提供服務。其設定檔已映射至當前資料夾中的<strong>embedEtcd.yaml</strong>。​</p></li>
...
<h3 id="From-WSL-2​" class="common-anchor-header">透過 WSL 2​<button data-href="#From-WSL-2​" class="anchor-icon" translate="no">
    ...
    </button></h3><p>若您希望在 Windows 上透過 Linux 命令和 shell 腳本啟動 Milvus，請確保已安裝 WSL 2。有關如何安裝 WSL 2 的詳細資訊，請參閱這篇<a href="https://learn.microsoft.com/en-us/windows/wsl/install#install-wsl-command">Microsoft 文章</a>。​</p>
<ol>
<li><p>啟動 WSL 2。​</p>
...
<h2 id="Run-Milvus-with-Docker-Compose​" class="common-anchor-header">使用 Docker Compose 執行 Milvus​<button data-href="#Run-Milvus-with-Docker-Compose​" class="anchor-icon" translate="no">
    ...
    </button></h2><p>在 Microsoft Windows 上安裝 Docker Desktop 後，您可透過 PowerShell 或 Windows 命令提示字元（<strong>以系統管理員模式</strong>）存取 Docker CLI。您可以在 PowerShell、Windows 命令提示字元或 WSL 2 中執行 Docker Compose 來啟動 Milvus。​</p>
<h3 id="From-PowerShell-or-Windows-Command-Prompt​" class="common-anchor-header">透過 PowerShell 或 Windows 命令提示字元​<button data-href="#From-PowerShell-or-Windows-Command-Prompt​" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
...
<li><p>以管理員模式開啟 Docker Desktop：右鍵點擊並選擇「<strong>以管理員身分執行</strong>」。​</p></li>
<li><p>在 PowerShell 或 Windows 命令提示字元中執行以下指令，以下載 Milvus Standalone 的 Docker Compose 配置檔並啟動 Milvus。​</p>
...
<p>視您的網路連線狀況而定，下載 Milvus 安裝所需的映像檔可能需要一些時間。一旦名為<strong>milvus-standalone</strong>、<strong>milvus-minio</strong> 和<strong>milvus-etcd</strong>的容器啟動後，您會發現 ​</p>
<ul>
<li><p><strong>milvus-etcd</strong>容器並未向主機公開任何端口，且將其資料映射至當前資料夾中的<strong>volumes/etcd​</strong></p></li>
<li><p><strong>milvus-minio</strong>容器在本地提供<strong>9000</strong>和<strong>9001</strong>埠，使用預設的認證憑證，並將其資料映射至當前資料夾中的<strong>volumes/minio</strong>。​</p></li>
...
<h2 id="FAQs​" class="common-anchor-header">常見問題​<button data-href="#FAQs​" class="anchor-icon" translate="no">
    ...
    </button></h3><p>在 Windows 上安裝 Docker Desktop 後，若電腦設定不當，可能會遇到「<code translate="no">Docker Engine stopped</code> 」錯誤。此時，您可能需要進行以下檢查：​</p>
<ol>
<li><p>檢查虛擬化功能是否已啟用。​</p>
<p>您可透過「<strong>工作管理員</strong>」中的「<strong>效能</strong>」分頁來檢查虛擬化功能是否已啟用。​</p>
<p><span class="img-wrapper">
   ...
   </span>「<span class="img-wrapper"> <span>工作管理員」中的虛擬化功能</span>
  
 </span></p>
<p>若虛擬化功能已停用，您可能需要檢查主機板韌體的 BIOS 設定。在 BIOS 設定中啟用虛擬化的方法會因主機板廠商而異。以 ASUS<a href="https://www.asus.com/support/faq/1043786/">主機板為例，您可以參考這篇</a>關於啟用虛<a href="https://www.asus.com/support/faq/1043786/">擬化的文章</a>。​</p>
...
<li><p>請檢查您是否已以<strong>系統管理員</strong>模式啟動 Docker Desktop。​</p>
...
<h3 id="How-can-I-deal-with-WSL-related-issues-while-deploying-Milvus​" class="common-anchor-header">在部署 Milvus 時，該如何處理與 WSL 相關的問題？​<button data-href="#How-can-I-deal-with-WSL-related-issues-while-deploying-Milvus​" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>若您在 WSL 2 上執行 Milvus 時遇到與 WSL 相關的問題，您可能需要檢查是否已將 Docker Desktop 設定為使用基於 WSL 2 的引擎，操作步驟如下：​</p>
<ol>
...
<p><span class="img-wrapper">
   ...
   <span>在 Docker Desktop 設定中選擇 WSL 2 發行版</span>
...
<h3 id="How-can-I-deal-with-the-volume-related-errors-prompted-during-Milvus-startup-that-reads-Read-config-failed​" class="common-anchor-header">如何處理 Milvus 啟動時出現的與卷相關的錯誤提示，內容為「<code translate="no">Read config failed</code> 」？​<button data-href="#How-can-I-deal-with-the-volume-related-errors-prompted-during-Milvus-startup-that-reads-Read-config-failed​" class="anchor-icon" translate="no">
   ...
   <span>Milvus 啟動時出現「讀取設定檔失敗」錯誤提示</span>
  
 </span></p>
<p>若要處理 Milvus 啟動時出現的「讀取設定檔失敗」錯誤，您需要檢查掛載至 Milvus 容器中的卷是否正確。如果卷已正確掛載至容器中，您可以使用<code translate="no">docker exec</code> 指令進入容器，並如下列出<strong>/milvus/configs</strong>資料夾：​</p>
<p><span class="img-wrapper">
