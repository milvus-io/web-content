...
<p>預設情況下，此部署會將<strong>Woodpecker</strong>（本機檔案系統）用作訊息佇列，因此無需外部訊息佇列服務。請參閱<a href="/docs/zh-hant/woodpecker.md">Woodpecker</a>。</p>
</blockquote>
<p>本頁面將示範如何在 Windows 上使用 Docker Desktop for Windows 執行 Milvus。​</p>
...
<h2 id="Run-Milvus-in-Docker​" class="common-anchor-header">在 Docker 中執行 Milvus​<button data-href="#Run-Milvus-in-Docker​" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Milvus 提供安裝腳本，可將其安裝為 Docker 容器。在 Microsoft Windows 上安裝 Docker Desktop 後，您即可透過以<strong>管理員</strong>模式執行的 PowerShell 或 Windows 命令提示字元，以及 WSL 2 來存取 Docker CLI。​</p>
...
<li><p>隨 Milvus 一併安裝的嵌入式 etcd 位於同一容器中，並於<strong>2379</strong> 埠提供服務。其設定檔已映射至當前資料夾中的<strong>embedEtcd.yaml</strong>。​</p></li>
<li><p>Milvus 資料卷已映射至當前資料夾中的<strong>volumes/milvus​</strong></p></li>
</ul>
<p>您可以使用以下指令來管理 Milvus 容器及其儲存的資料。​</p>
...
<h2 id="Run-Milvus-with-Docker-Compose​" class="common-anchor-header">使用 Docker Compose 執行 Milvus​<button data-href="#Run-Milvus-with-Docker-Compose​" class="anchor-icon" translate="no">
    ...
    </button></h2><p>在 Microsoft Windows 上安裝 Docker Desktop 後，您可透過<strong>以系統管理員模式執行的</strong>PowerShell 或 Windows 命令提示字元存取 Docker CLI。您可以在 PowerShell、Windows 命令提示字元或 WSL 2 中執行 Docker Compose 來啟動 Milvus。​</p>
<h3 id="From-PowerShell-or-Windows-Command-Prompt​" class="common-anchor-header">透過 PowerShell 或 Windows 命令提示字元​<button data-href="#From-PowerShell-or-Windows-Command-Prompt​" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
...
<li><p>以管理員模式開啟 Docker Desktop：右鍵點擊並選擇「<strong>以系統管理員身分執行</strong>」。​</p></li>
...
<p>視您的網路連線狀況而定，下載 Milvus 安裝所需的映像檔可能需要一段時間。一旦名為<strong>milvus-standalone</strong>、<strong>milvus-minio</strong> 和<strong>milvus-etcd</strong>的容器啟動後，您將看到 ​</p>
<ul>
<li><p><strong>milvus-etcd</strong>容器並未向主機公開任何埠號，且將其資料映射至當前資料夾中的<strong>volumes/etcd​</strong></p></li>
<li><p><strong>milvus-minio</strong>容器在本地端以預設的驗證憑證提供<strong>9000</strong>和<strong>9001</strong>埠，並將其資料映射至當前資料夾中的<strong>volumes/minio</strong>。​</p></li>
...
<ol>
<li><p>檢查虛擬化功能是否已啟用。​</p>
<p>您可透過「<strong>工作管理員</strong>」中的「<strong>效能</strong>」分頁，確認虛擬化功能是否已啟用。​</p>
...
<p>若虛擬化功能已停用，您可能需要檢查主機板韌體的 BIOS 設定。在 BIOS 設定中啟用虛擬化功能的方式因主機板廠商而異。以 ASUS<a href="https://www.asus.com/support/faq/1043786/">主機板為例，您可以參考這篇文章來了解</a>如何啟用虛<a href="https://www.asus.com/support/faq/1043786/">擬化功能</a>。​</p>
...
<h3 id="How-can-I-deal-with-WSL-related-issues-while-deploying-Milvus​" class="common-anchor-header">在部署 Milvus 時，如何處理與 WSL 相關的問題？​<button data-href="#How-can-I-deal-with-WSL-related-issues-while-deploying-Milvus​" class="anchor-icon" translate="no">
    ...
    </button></h3><p>若您在 WSL 2 上執行 Milvus 時遇到與 WSL 相關的問題，您可能需要檢查是否已依下列步驟將 Docker Desktop 設定為使用基於 WSL 2 的引擎：​</p>
<ol>
...
<h3 id="How-can-I-deal-with-the-volume-related-errors-prompted-during-Milvus-startup-that-reads-Read-config-failed​" class="common-anchor-header">如何處理 Milvus 啟動時出現的與儲存區相關的錯誤提示，內容為「<code translate="no">Read config failed</code> 」？​<button data-href="#How-can-I-deal-with-the-volume-related-errors-prompted-during-Milvus-startup-that-reads-Read-config-failed​" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<p>若要處理 Milvus 啟動時出現的「讀取設定檔失敗」錯誤，您需要檢查掛載至 Milvus 容器中的卷是否正確。若卷已正確掛載至容器中，您可以使用 `<code translate="no">docker exec</code> ` 指令進入容器，並依下述方式列出 `<strong>/milvus/configs</strong>` 資料夾：​</p>
<p><span class="img-wrapper">
