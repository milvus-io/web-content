...
summary: 了解如何使用 Milvus Operator 在 Kubernetes 上安裝 Milvus 叢集
title: 使用 Milvus Operator 安裝 Milvus 叢集
---
<h1 id="Run-Milvus-in-Kubernetes-with-Milvus-Operator" class="common-anchor-header">使用 Milvus Operator 在 Kubernetes 中執行 Milvus<button data-href="#Run-Milvus-in-Kubernetes-with-Milvus-Operator" class="anchor-icon" translate="no">
    ...
    </button></h1><p>本頁面說明如何使用<a href="https://github.com/zilliztech/milvus-operator">Milvus Operator</a> 在 Kubernetes 中啟動 Milvus 實例。</p>
<h2 id="Overview" class="common-anchor-header">概述<button data-href="#Overview" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Milvus Operator 是一套解決方案，可協助您在目標 Kubernetes (K8s) 叢集中部署並管理完整的 Milvus 服務堆疊。此堆疊包含所有 Milvus 元件及相關依賴項（如 etcd 和 MinIO），並內建 Woodpecker 作為訊息佇列。</p>
...
<p>若在拉取映像檔時遇到任何問題，請將問題詳情寄至<a href="mailto:community@zilliz.com">community@zilliz.com</a>聯絡我們，我們將為您提供必要的支援。</p>
</div>
<h2 id="Install-Milvus-Operator" class="common-anchor-header">安裝 Milvus Operator<button data-href="#Install-Milvus-Operator" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<p>執行以下指令，透過 Helm 安裝 Milvus Operator。</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>若您先前已安裝 Milvus Operator，請使用以下指令進行升級：</p>
...
<p>執行以下命令，透過<code translate="no">kubectl</code> 安裝 Milvus Operator。</p>
...
<h2 id="Deploy-Milvus" class="common-anchor-header">部署 Milvus<button data-href="#Deploy-Milvus" class="anchor-icon" translate="no">
    ...
    </button></h3><p>一旦 Milvus Operator Pod 開始運行，您即可依照以下步驟部署 Milvus 叢集。</p>
<pre><code translate="no" class="language-shell"><span class="hljs-meta prompt_">$ </span><span class="language-bash">kubectl apply -f https://raw.githubusercontent.com/zilliztech/milvus-operator/main/config/samples/milvus_cluster_woodpecker.yaml</span>
<button class="copy-code-btn"></button></code></pre>
<p>上述指令會部署一個以<strong>Woodpecker</strong>作為訊息佇列（建議用於 v3.0-beta）的 Milvus 叢集，並包含所有新的架構元件，包括串流節點。</p>
<p><strong>此部署中的架構重點：</strong></p>
<ul>
<li><strong>訊息佇列</strong>：<a href="/docs/zh-hant/woodpecker.md">使用 Woodpecker</a>（可減少基礎架構維護工作）</li>
<li><strong>串流節點</strong>：已啟用以強化資料處理能力</li>
<li><strong>Mix 協調器</strong>：整合協調器元件以提升效率</li>
</ul>
<p>若要自訂這些設定，建議您使用<a href="https://milvus.io/tools/sizing">Milvus 規模規劃工具</a>，根據實際資料大小調整配置，然後下載對應的 YAML 檔案。如需進一步了解配置參數，請參閱《<a href="https://milvus.io/docs/system_configuration.md">Milvus 系統配置檢查清單》</a>。</p>
<div class="alert note">
<ul>
<li>版本名稱僅應包含字母、數字和連字號。版本名稱中不允許出現點號。</li>
<li>您亦可將 Milvus 實例部署於獨立模式，此時所有元件皆包含於單一 Pod 之中。若要如此操作，請將上述指令中的配置檔案 URL 變更為<code translate="no">https://raw.githubusercontent.com/zilliztech/milvus-operator/main/config/samples/milvus_default.yaml</code></li>
</ul>
</div>
...
<button class="copy-code-btn"></button></code></pre>
<p>當您的 Milvus 叢集就緒後，上述命令的輸出應類似於以下內容。若「<code translate="no">status.status</code> 」欄位仍顯示為「<code translate="no">Unhealthy</code> 」，表示您的 Milvus 叢集仍在建立中。</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>Milvus Operator 會先建立 Milvus 依賴項（例如 etcd 和 MinIO），接著再建立 Milvus 組件（例如代理伺服器、協調器及節點）。</p>
<p>當您的 Milvus 叢集準備就緒後，Milvus 叢集中所有 Pod 的狀態應類似於以下內容。</p>
...
<h3 id="3-Forward-a-local-port-to-Milvus" class="common-anchor-header">3. 將本地埠轉發至 Milvus<button data-href="#3-Forward-a-local-port-to-Milvus" class="anchor-icon" translate="no">
    ...
    </button></h3><p>執行以下指令以取得您的 Milvus 叢集所使用的埠號。</p>
...
19530
<button class="copy-code-btn"></button></code></pre>
<p>輸出結果顯示，該 Milvus 實例於預設埠<strong>19530</strong> 提供服務。</p>
<div class="alert note">
<p>若您是以獨立模式部署 Milvus，請將 Pod 名稱從<code translate="no">my-release-milvus-proxy-xxxxxxxxxx-xxxxx</code> 變更為<code translate="no">my-release-milvus-xxxxxxxxxx-xxxxx</code> 。</p>
</div>
<p>接著，執行以下命令，將本機埠轉發至 Milvus 的服務埠。</p>
...
<p>預設情況下，kubectl 的端口轉發僅監聽<code translate="no">localhost</code> 。若要讓 Milvus 監聽指定或所有 IP 位址，請使用<code translate="no">address</code> 標誌。以下指令會讓端口轉發監聽主機上的所有 IP 位址。</p>
...
<h2 id="Optional-Update-Milvus-configurations" class="common-anchor-header">（可選）更新 Milvus 設定<button data-href="#Optional-Update-Milvus-configurations" class="anchor-icon" translate="no">
    ...
    </button></h2><p>您可以透過以下方式呼叫<code translate="no">patch</code> 命令，檢視並更新 Milvus 叢集的設定：</p>
<ol>
<li><p>執行以下命令以預覽預設的設定。</p>
<p>以下範例假設您要將 `<code translate="no">spec.components.disableMetric</code> ` 參數更新為<code translate="no">false</code> 毫秒。</p>
...
<h2 id="Access-Milvus-WebUI" class="common-anchor-header">存取 Milvus WebUI<button data-href="#Access-Milvus-WebUI" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Milvus 內建名為「Milvus WebUI」的圖形化使用者介面工具，您可透過瀏覽器存取。Milvus WebUI 透過簡單直覺的介面，提升系統的可觀察性。您可以使用 Milvus WebUI 觀察 Milvus 各元件與依賴項的統計資料和指標、檢查資料庫與收集項的詳細資訊，並列出詳細的 Milvus 設定。 有關 Milvus WebUI 的詳細資訊，請參閱<a href="/docs/zh-hant/milvus-webui.md">Milvus WebUI</a></p>
<p>若要啟用對 Milvus WebUI 的存取，您需要將代理 pod 進行埠轉發至本機埠。</p>
<pre><code translate="no" class="language-shell"><span class="hljs-meta prompt_">$ </span><span class="language-bash">kubectl port-forward --address 0.0.0.0 service/my-release-milvus 27018:9091</span>
Forwarding from 0.0.0.0:27018 -&gt; 9091
<button class="copy-code-btn"></button></code></pre>
<p>現在，您可透過<code translate="no">http://localhost:27018</code> 存取 Milvus Web UI。</p>
<h2 id="Uninstall-Milvus" class="common-anchor-header">解除安裝 Milvus<button data-href="#Uninstall-Milvus" class="anchor-icon" translate="no">
    ...
    </button></h2><p>執行以下指令以解除安裝 Milvus 叢集。</p>
...
<div class="alert note">
<ul>
<li>若您使用預設設定刪除 Milvus 叢集，etcd 和 MinIO 等依賴項將不會被刪除。因此，下次安裝相同的 Milvus 叢集實例時，這些依賴項將會再次被使用。</li>
<li>若要連同 Milvus 叢集一併刪除依賴項和持久化卷聲明 (PVC)，請參閱<a href="https://github.com/zilliztech/milvus-operator/blob/main/config/samples/milvus_deletion.yaml">配置檔案</a>。</li>
</ul>
</div>
<h2 id="Uninstall-Milvus-Operator" class="common-anchor-header">解除安裝 Milvus Operator<button data-href="#Uninstall-Milvus-Operator" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>此外，還有兩種方式可以卸載 Milvus Operator。</p>
<ul>
<li><a href="#Uninstall-with-Helm">使用 Helm 解除安裝</a></li>
<li><a href="#Uninstall-with-kubectl">使用 kubectl 解除安裝</a></li>
</ul>
<h4 id="Uninstall-with-Helm" class="common-anchor-header">使用 Helm 解除安裝</h4><pre><code translate="no" class="language-shell"><span class="hljs-meta prompt_">$ </span><span class="language-bash">helm -n milvus-operator uninstall milvus-operator</span>
<button class="copy-code-btn"></button></code></pre>
<h4 id="Uninstall-with-kubectl" class="common-anchor-header">使用 kubectl 解除安裝</h4><pre><code translate="no" class="language-shell"><span class="hljs-meta prompt_">$ </span><span class="language-bash">kubectl delete -f https://raw.githubusercontent.com/zilliztech/milvus-operator/v1.3.0/deploy/manifests/deployment.yaml</span>
<button class="copy-code-btn"></button></code></pre>
<h2 id="Optional-dependencies" class="common-anchor-header">可選依賴項<button data-href="#Optional-dependencies" class="anchor-icon" translate="no">
    ...
    </button></h2><p>此部署使用<strong>Woodpecker</strong>作為訊息佇列、<strong>etcd</strong>作為元資料儲存，以及<strong>MinIO</strong>作為物件儲存，所有資源均由 Milvus Operator 進行配置。若要搭配 Operator 使用其他後端，請參閱：</p>
<ul>
<li>訊息佇列：<a href="/docs/zh-hant/woodpecker.md">Woodpecker</a>（預設）·<a href="/docs/zh-hant/message_storage_operator.md#Configure-Pulsar">Pulsar</a>·<a href="/docs/zh-hant/message_storage_operator.md#Configure-Kafka">Kafka</a>·<a href="/docs/zh-hant/message_storage_operator.md#Configure-RocksMQ">RocksMQ</a></li>
<li>物件儲存：<a href="/docs/zh-hant/object_storage_operator.md">透過 Milvus Operator 配置物件儲存</a></li>
<li>元資料：<a href="/docs/zh-hant/meta_storage_operator.md">透過 Milvus Operator 設定 etcd</a></li>
</ul>
<div class="alert note">
<p>Storage V3 預設為停用狀態。在使用依賴於它的功能之前，請先啟用它。有關需求與相容性考量，請參閱<a href="/docs/zh-hant/storage-v3.md">Storage V3</a>。</p>
</div>
...
<ul>
<li><p>請參閱《<a href="/docs/zh-hant/quickstart.md">Hello Milvus</a>》，了解 Milvus 的功能。</p></li>
