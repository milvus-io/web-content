...
<div class="alert note">
<p>位於<code translate="no">https://github.com/milvus-io/milvus-helm</code> 的 Milvus Helm Charts 儲存庫已歸檔。我們現在使用位於<code translate="no">https://github.com/zilliztech/milvus-helm</code> 的新儲存庫。歸檔的儲存庫仍可提供 4.0.31 及更早版本的 Charts，但後續版本請使用新儲存庫。</p>
</div>
...
<button class="copy-code-btn"></button></code></pre>
<p><strong>注意</strong>：獨立模式預設使用 Woodpecker 作為訊息佇列，並啟用「串流節點」元件。獨立部署會將 Woodpecker<strong>內嵌於</strong>Milvus Pod 中運行；而專用的 Woodpecker<strong>服務</strong>（獨立 Pod）僅適用於<strong>分散式／叢集部署</strong>。詳細資訊請參閱《<a href="/docs/zh-hant/architecture_overview.md">架構概覽</a>》與<a href="/docs/zh-hant/woodpecker.md">《Woodpecker》</a>。</p>
</div>
<p><strong>部署 Milvus 叢集：</strong></p>
<div class="alert note">
<p>若採用 Woodpecker 服務模式，我們建議使用即將推出的 Milvus 3.0.1 或更新版本，並搭配 Woodpecker v0.1.37 或更新版本，以獲得壓縮清理與群組提交的優化效果。</p>
</div>
<p>以下指令將部署一個針對 v3.0.1 進行優化設定的 Milvus 叢集，並使用 Woodpecker 作為推薦的消息佇列：</p>
<pre><code translate="no" class="language-bash">helm install my-release zilliztech/milvus \
  --<span class="hljs-built_in">set</span> image.all.tag=v3.0.1 \
  --<span class="hljs-built_in">set</span> woodpecker.enabled=<span class="hljs-literal">true</span> \
...
<li>啟用新的<strong>串流節點 (Streaming Node</strong>) 元件以提升效能</li>
...
<p>完整的架構詳情，請參閱《<a href="/docs/zh-hant/architecture_overview.md">架構概述</a>》。</p>
</div>
...
<button class="copy-code-btn"></button></code></pre>
<p><strong>請等待所有 Pod 顯示「Running」狀態。</strong>採用 v3.0.1 配置時，您應會看到類似以下的 Pod：</p>
...
<li><strong>依賴項</strong>：<code translate="no">etcd</code> （元數據）、<code translate="no">minio</code> （物件儲存）、<code translate="no">woodpecker</code> （訊息佇列）</li>
</ul>
<div class="alert note">
<p>透過<code translate="no">streaming.woodpecker.embedded=false</code> ，Woodpecker<strong>會以專用的 StatefulSet</strong>形式運行（<code translate="no">my-release-milvus-woodpecker</code> ，預設為 4 個複本 — 包含 3 個節點的法定人數，外加一個備用節點以確保容錯能力； 請勿將<code translate="no">woodpecker.replicaCount</code> 設定低於 3）並由無頭服務作為前端，採用 MinIO 作為儲存後端——因此該叢集擁有獨立的<code translate="no">woodpecker</code> Pod 組，與串流節點相互區隔。</p>
</div>
...
<h2 id="Access-Milvus-WebUI" class="common-anchor-header">存取 Milvus WebUI<button data-href="#Access-Milvus-WebUI" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Milvus 內建名為「Milvus WebUI」的圖形化介面工具，您可透過瀏覽器存取。Milvus WebUI 透過簡單直觀的介面，提升系統的可觀察性。您可以使用 Milvus WebUI 觀察 Milvus 各元件及依賴項的統計資料與指標、檢查資料庫與收集的詳細資訊，並列出詳細的 Milvus 配置。 有關 Milvus WebUI 的詳細資訊，請參閱<a href="/docs/zh-hant/milvus-webui.md">Milvus WebUI</a></p>
...
<h2 id="Offline-install" class="common-anchor-header">離線安裝<button data-href="#Offline-install" class="anchor-icon" translate="no">
    ...
    </button></h2><p>若您身處網路受限的環境，請依照本節的步驟啟動 Milvus 叢集。</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>上述命令會為 Milvus 叢集渲染圖表範本，並將輸出結果儲存至名為<code translate="no">milvus_manifest.yaml</code> 的清單檔案中。透過此清單，您可以安裝一個 Milvus 叢集，其各組件與依賴項將分別置於不同的 Pod 中。</p>
<div class="alert note">
<ul>
<li>若要在獨立模式下安裝 Milvus 實例（即所有 Milvus 組件均包含於單一 Pod 中），您應改為執行<code translate="no">helm template my-release --set cluster.enabled=false --set etcd.replicaCount=1 --set minio.mode=standalone --set pulsarv3.enabled=false --set standalone.messageQueue=woodpecker --set woodpecker.enabled=true --set streaming.enabled=true zilliztech/milvus &gt; milvus_manifest.yaml</code> ，以生成獨立模式下 Milvus 實例的圖表範本。</li>
<li>若要變更 Milvus 設定，請下載 <a href="https://raw.githubusercontent.com/milvus-io/milvus-helm/master/charts/milvus/values.yaml"><code translate="no">value.yaml</code></a> 範本，在其中填入您所需的設定，並使用<code translate="no">helm template -f values.yaml my-release zilliztech/milvus &gt; milvus_manifest.yaml</code> 據此渲染對應的宣言檔。</li>
</ul>
</div>
...
<button class="copy-code-btn"></button></code></pre>
<p>至此，您可以依照線上安裝指南的步驟<a href="#2-Check-Milvus-cluster-status">2</a>和<a href="#3-Connect-to-Milvus">3，</a>檢查叢集狀態並將本地埠口轉發至 Milvus。</p>
...
<li><p>探索<a href="/docs/zh-hant/birdwatcher_overview.md">Birdwatcher，這</a>是一款用於 Milvus 除錯與動態設定更新的開源工具。</p></li>
<li><p>探索<a href="https://github.com/zilliztech/attu">Attu</a>——一款用於直觀管理 Milvus 的開源圖形化介面工具。</p></li>
