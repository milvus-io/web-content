...
<button class="copy-code-btn"></button></code></pre>
<p><strong>注意</strong>：獨立模式預設使用 Woodpecker 作為訊息佇列，並啟用「串流節點」（Streaming Node）元件。獨立部署會將 Woodpecker<strong>內嵌於</strong>Milvus Pod 中執行；而專用的 Woodpecker<strong>服務</strong>（獨立 Pod）僅適用<strong>於分散式／叢集部署</strong>。詳細資訊請參閱《<a href="/docs/zh-hant/architecture_overview.md">架構概覽</a>》及<a href="/docs/zh-hant/woodpecker.md">《Woodpecker》</a>。</p>
</div>
...
<ul>
<li>使用<strong>Woodpecker</strong>作為訊息佇列（建議採用以減少維護工作量）</li>
...
<div class="alert note">
<p>透過<code translate="no">streaming.woodpecker.embedded=false</code> ，Woodpecker<strong>會以專用的 StatefulSet</strong>形式運行（<code translate="no">my-release-milvus-woodpecker</code> ，預設為 4 個複本——3 個節點的決議數加上 1 個備用節點以實現容錯； 請勿將<code translate="no">woodpecker.replicaCount</code> 設定低於 3）並由無頭服務作為前端，採用 MinIO 作為儲存後端——因此該叢集擁有獨立的<code translate="no">woodpecker</code> Pod 集合，與串流節點相互區隔。</p>
</div>
...
<p>使用 Milvus 期間<strong>請保持此終端機開啟</strong>。現在您可透過任何 Milvus SDK 連線至<code translate="no">localhost:27017</code> 來存取 Milvus。</p>
...
<h2 id="Access-Milvus-WebUI" class="common-anchor-header">存取 Milvus WebUI<button data-href="#Access-Milvus-WebUI" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Milvus 內建名為「Milvus WebUI」的圖形化使用者介面工具，您可透過瀏覽器存取。Milvus WebUI 透過簡單直覺的介面，強化系統的可觀察性。您可以使用 Milvus WebUI 觀察 Milvus 各元件及依賴項的統計資料與指標、檢查資料庫與資料收集的詳細資訊，並列出詳細的 Milvus 設定。 有關 Milvus WebUI 的詳細資訊，請參閱<a href="/docs/zh-hant/milvus-webui.md">Milvus WebUI</a></p>
...
<button class="copy-code-btn"></button></code></pre>
<p>上述命令會為 Milvus 叢集渲染圖表模板，並將輸出結果儲存至名為<code translate="no">milvus_manifest.yaml</code> 的清單檔案中。透過此清單，您可以安裝一個 Milvus 叢集，其組件與依賴項將分別置於不同的 Pod 中。</p>
<div class="alert note">
<ul>
<li>若要以獨立模式安裝 Milvus 實例（即所有 Milvus 元件均包含於單一 Pod 中），您應改為執行<code translate="no">helm template my-release --set cluster.enabled=false --set etcd.replicaCount=1 --set minio.mode=standalone --set pulsarv3.enabled=false --set standalone.messageQueue=woodpecker --set woodpecker.enabled=true --set streaming.enabled=true zilliztech/milvus &gt; milvus_manifest.yaml</code> ，以渲染獨立模式下 Milvus 實例的圖表範本。</li>
...
<button class="copy-code-btn"></button></code></pre>
<p>至此，您可以依照線上安裝指南的步驟<a href="#2-Check-Milvus-cluster-status">2</a>和<a href="#3-Connect-to-Milvus">3，</a>檢查叢集狀態並將本機埠口轉發至 Milvus。</p>
