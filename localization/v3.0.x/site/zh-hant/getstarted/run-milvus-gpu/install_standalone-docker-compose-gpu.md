...
<h2 id="Install-Milvus" class="common-anchor-header">安裝 Milvus<button data-href="#Install-Milvus" class="anchor-icon" translate="no">
    ...
    </button></h2><p>若要使用 Docker Compose 安裝具備 GPU 支援的 Milvus，請按照以下步驟操作。</p>
<h3 id="1-Download-and-configure-the-YAML-file" class="common-anchor-header">1. 下載並設定 YAML 檔案<button data-href="#1-Download-and-configure-the-YAML-file" class="anchor-icon" translate="no">
    ...
    </button></h3><p>下載 <a href="https://github.com/milvus-io/milvus/releases/download/v3.0.1/milvus-standalone-docker-compose-gpu.yml"><code translate="no">milvus-standalone-docker-compose-gpu.yml</code></a> 並手動將其儲存為 docker-compose.yml，或使用以下指令執行。</p>
...
<ul>
<li>若要將特定 GPU 裝置指派給 Milvus，請在<code translate="no">standalone</code> 服務的定義中找到<code translate="no">deploy.resources.reservations.devices[0].devices_ids</code> 欄位，並將其值替換為所需 GPU 的 ID。您可以使用 NVIDIA GPU 顯示驅動程式內附的<code translate="no">nvidia-smi</code> 工具，來確定 GPU 裝置的 ID。Milvus 支援多個 GPU 裝置。</li>
</ul>
...
<button class="copy-code-btn"></button></code></pre>
<div class="alert note">
<p>若無法執行上述指令，請檢查您的系統是否已安裝 Docker Compose V1。若確實如此，建議您根據<a href="https://docs.docker.com/compose/">此頁面</a>中的說明，遷移至 Docker Compose V2。</p>
</div>
<p>啟動 Milvus 後，</p>
<ul>
...
<li><strong>milvus-standalone</strong>容器以預設設定在本地端提供<strong>19530</strong>埠的服務，並將其資料映射至當前資料夾中的<strong>volumes/milvus</strong>。</li>
</ul></li>
</ul>
...
<ol>
<li><p>請使用以下指令，將 Milvus 容器中的<code translate="no">milvus.yaml</code> 複製到您的本地端機器。請將<code translate="no">&lt;milvus_container_id&gt;</code> 替換為您實際的 Milvus 容器 ID。</p>
...
<li><p>查看<a href="/docs/zh-hant/milvus-webui.md">Milvus</a>WebUI，進一步了解 Milvus 執行個體。</p></li>
