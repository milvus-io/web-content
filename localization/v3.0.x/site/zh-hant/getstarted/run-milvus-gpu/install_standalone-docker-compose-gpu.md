...
<li>安裝前<a href="/docs/zh-hant/prerequisite-gpu.md">請先確認硬體與軟體的系統需求</a>。</li>
</ul>
...
<h3 id="1-Download-and-configure-the-YAML-file" class="common-anchor-header">1. 下載並設定 YAML 檔案<button data-href="#1-Download-and-configure-the-YAML-file" class="anchor-icon" translate="no">
    ...
    </button></h3><p>下載 <a href="https://github.com/milvus-io/milvus/releases/download/v3.0.2/milvus-standalone-docker-compose-gpu.yml"><code translate="no">milvus-standalone-docker-compose-gpu.yml</code></a> 並手動將其儲存為 docker-compose.yml，或使用以下指令執行。</p>
...
<p>您需要對 YAML 檔案中獨立服務的環境變數進行如下修改：</p>
<ul>
<li>若要將特定 GPU 裝置指派給 Milvus，請在<code translate="no">standalone</code> 服務的定義中找到<code translate="no">deploy.resources.reservations.devices[0].devices_ids</code> 欄位，並將其值替換為所需 GPU 的 ID。您可以使用 NVIDIA GPU 顯示驅動程式中內建的<code translate="no">nvidia-smi</code> 工具，來確定 GPU 裝置的 ID。Milvus 支援多個 GPU 裝置。</li>
</ul>
...
<button class="copy-code-btn"></button></code></pre>
<div class="alert note">
<p>若無法執行上述指令，請檢查您的系統是否已安裝 Docker Compose V1。若屬此情況，建議您根據<a href="https://docs.docker.com/compose/">此頁面中的</a>注意事項，遷移至 Docker Compose V2。</p>
</div>
<p>啟動 Milvus 後，</p>
<ul>
<li>名為<strong>milvus-standalone</strong>、<strong>milvus-minio</strong> 及<strong>milvus-etcd</strong>的容器即已啟動。
<ul>
<li><strong>milvus-etcd</strong>容器不會向主機公開任何端口，並將其資料映射至當前資料夾中的<strong>volumes/etcd</strong>。</li>
<li><strong>milvus-minio</strong>容器使用預設的驗證憑證在本地端提供<strong>9090</strong>和<strong>9091</strong>埠，並將其資料映射至當前資料夾中的<strong>volumes/minio</strong>。</li>
<li><strong>milvus-standalone</strong>容器使用預設設定在本地端提供<strong>19530</strong>埠，並將其資料映射至當前資料夾中的<strong>volumes/milvus</strong>。</li>
</ul></li>
</ul>
...
<button class="copy-code-btn"></button></code></pre>
<p>您也可以透過<code translate="no">http://127.0.0.1:9091/webui/</code> 存取 Milvus WebUI，以進一步了解您的 Milvus 實例。詳細資訊請參閱<a href="/docs/zh-hant/milvus-webui.md">Milvus WebUI</a>。</p>
<p>若您已在 `docker-compose.yml` 檔案中為 Milvus 指派了多個 GPU 裝置，您可以指定哪些 GPU 裝置對 Milvus 可見或可供使用。</p>
...
<ol>
<li><p>請使用以下指令，將<code translate="no">milvus.yaml</code> 從 Milvus 容器複製到您的本地端電腦。請將<code translate="no">&lt;milvus_container_id&gt;</code> 替換為您實際的 Milvus 容器 ID。</p>
<pre><code translate="no" class="language-shell">docker cp &lt;milvus_container_id&gt;:/milvus/configs/milvus.yaml milvus.yaml
<button class="copy-code-btn"></button></code></pre></li>
<li><p>使用您偏好的文字編輯器開啟已複製的<code translate="no">milvus.yaml</code> 檔案。例如，使用 vim：</p>
<pre><code translate="no" class="language-shell">vim milvus.yaml
<button class="copy-code-btn"></button></code></pre></li>
<li><p>根據需要編輯<code translate="no">initMemSize</code> 及<code translate="no">maxMemSize</code> 的設定，並儲存變更：</p>
<pre><code translate="no" class="language-yaml"><span class="hljs-string">...</span>
...
<li><p>查看<a href="/docs/zh-hant/milvus-webui.md">Milvus WebUI</a>以進一步了解 Milvus 執行個體。</p></li>
...
<li><p>探索<a href="/docs/zh-hant/birdwatcher_overview.md">Birdwatcher</a>——一款用於 Milvus 除錯及動態配置更新的開源工具。</p></li>
