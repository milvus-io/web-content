...
<h1 id="Run-Milvus-with-GPU-Support-Using-Helm-Chart" class="common-anchor-header">使用 Helm Chart 執行具備 GPU 支援的 Milvus<button data-href="#Run-Milvus-with-GPU-Support-Using-Helm-Chart" class="anchor-icon" translate="no">
    ...
    </button></h1><p>本頁面說明如何透過 Helm Chart 啟動具備 GPU 支援的 Milvus 實例。</p>
<h2 id="Overview" class="common-anchor-header">概述<button data-href="#Overview" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Helm 使用一種稱為「圖表」（charts）的封裝格式。 Chart 是一組檔案的集合，用於描述一組相關的 Kubernetes 資源。Milvus 提供了一組 Chart，以協助您部署 Milvus 的依賴項與組件。<a href="https://artifacthub.io/packages/helm/milvus-helm/milvus">Milvus Helm Chart</a>是一項解決方案，可透過 Helm 套件管理器在 Kubernetes (K8s) 叢集上初始化 Milvus 的部署。</p>
...
<p>您應透過 Helm 指定發行版名稱、圖表以及預期要變更的參數來啟動 Milvus。在本指南中，我們使用<code translate="no">my-release</code> 作為發行版名稱。若要使用不同的發行版名稱，請將以下指令中的<code translate="no">my-release</code> 替換為您正在使用的名稱。</p>
...
19530
<button class="copy-code-btn"></button></code></pre>
<p>接著，執行以下指令將本機埠轉發至 Milvus 提供服務的埠號。</p>
<pre><code translate="no" class="language-bash">$ kubectl port-forward service/my-release-milvus 27017:19530
Forwarding from 127.0.0.1:27017 -&gt; 19530
<button class="copy-code-btn"></button></code></pre>
<p>若需，您可在上述指令中使用<code translate="no">:19530</code> 取代<code translate="no">27017:19530</code> ，讓<code translate="no">kubectl</code> 為您分配一個本地端埠，如此便無需自行處理埠號衝突問題。</p>
