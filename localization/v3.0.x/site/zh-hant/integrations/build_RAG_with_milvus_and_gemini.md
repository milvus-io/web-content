...
summary: >-
  在本教學中，我們將向您展示如何使用 Milvus 和 Gemini 建置一個 RAG（檢索增強生成）管道。我們將利用 Gemini
  模型根據給定的查詢來生成文字，並使用 Milvus 來儲存和檢索生成的文字。
title: 使用 Milvus 和 Gemini 建置 RAG
---
...
<h1 id="Build-RAG-with-Milvus-and-Gemini" class="common-anchor-header">使用 Milvus 和 Gemini 建置 RAG<button data-href="#Build-RAG-with-Milvus-and-Gemini" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h1><p><a href="https://ai.google.dev/gemini-api/docs">Gemini API</a>與<a href="https://ai.google.dev/aistudio">Google AI Studio</a>可協助您開始運用 Google 最新的模型，並將您的構想轉化為可擴展的應用程式。 Gemini 提供對強大語言模型（如<code translate="no">Gemini-2.5-Flash</code> 和<code translate="no">Gemini-2.5-Pro</code> ）的存取權限，適用於文字生成、文件處理、視覺、音訊分析等任務。它還提供<code translate="no">Gemini Embedding 2</code> ，這是一個多模態嵌入模型，支援文字、圖片、影片、音訊及 PDF 文件，並透過 Matryoshka 表徵學習（Matryoshka Representation Learning）提供靈活的輸出維度。 透過此 API，您可以輸入包含數百萬個標記的長上下文、針對特定任務微調模型、產生 JSON 等結構化輸出，並運用語義檢索與程式碼執行等功能。</p>
<p>在本教學中，我們將向您展示如何使用 Milvus 和 Gemini 建置 RAG（檢索增強生成）處理流程。我們將運用 Gemini 模型，根據給定的查詢生成回應，並結合從 Milvus 檢索到的相關資訊進行增強。</p>
<h2 id="Preparation" class="common-anchor-header">準備工作<button data-href="#Preparation" class="anchor-icon" translate="no">
    ...
    </button></h2><h3 id="Dependencies-and-Environment" class="common-anchor-header">依賴項與環境<button data-href="#Dependencies-and-Environment" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<p>您應先登入 Google AI Studio 平台，並將<a href="https://aistudio.google.com/apikey">API 金鑰</a> <code translate="no">GEMINI_API_KEY</code> 設定為環境變數。</p>
...
<h3 id="Prepare-the-data" class="common-anchor-header">準備資料<button data-href="#Prepare-the-data" class="anchor-icon" translate="no">
    ...
    </button></h3><p>我們將<a href="https://github.com/milvus-io/milvus-docs/releases/download/v2.4.6-preview/milvus_docs_2.4.x_en.zip">Milvus 文件 2.4.x</a>中的常見問題 (FAQ) 頁面用作 RAG 中的私有知識，這對於簡單的 RAG 管道而言是個很好的資料來源。</p>
<p>下載 ZIP 檔案，並將文件解壓縮至<code translate="no">milvus_docs</code> 資料夾中。</p>
<pre><code translate="no" class="language-shell"><span class="hljs-meta prompt_">$ </span><span class="language-bash">wget https://github.com/milvus-io/milvus-docs/releases/download/v2.4.6-preview/milvus_docs_2.4.x_en.zip</span>
<span class="hljs-meta prompt_">$ </span><span class="language-bash">unzip -q milvus_docs_2.4.x_en.zip -d milvus_docs</span>
<button class="copy-code-btn"></button></code></pre>
<p>我們將從<code translate="no">milvus_docs/en/faq</code> 資料夾中載入所有 Markdown 檔案。對於每個文件，我們僅需使用「# 」來分隔檔案中的內容，這能大致區分 Markdown 檔案中各主要部分的內容。</p>
...
<button class="copy-code-btn"></button></code></pre>
<h3 id="Prepare-the-LLM-and-Embedding-Model" class="common-anchor-header">準備大型語言模型（LLM）與嵌入模型<button data-href="#Prepare-the-LLM-and-Embedding-Model" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>我們使用<code translate="no">gemini-2.5-flash</code> 作為 LLM，並採用<code translate="no">gemini-embedding-2-preview</code> 作為嵌入模型。<code translate="no">gemini-embedding-2-preview</code> 是 Google 最新的多模態嵌入模型，透過 Matryoshka 表徵學習（Matryoshka Representation Learning），支援文字、圖片、影片、音訊及 PDF 文件，並具備靈活的輸出維度（128–3,072）。</p>
<p>讓我們試著從 LLM 生成一個測試回應：</p>
...
<h2 id="Load-data-into-Milvus" class="common-anchor-header">將資料載入 Milvus<button data-href="#Load-data-into-Milvus" class="anchor-icon" translate="no">
    ...
    </button></h2><h3 id="Create-the-Collection" class="common-anchor-header">建立 Collection<button data-href="#Create-the-Collection" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>讓我們初始化 Milvus 客戶端並設定我們的 Collection：</p>
...
<ul>
<li>將 `<code translate="no">uri</code> ` 設定為本機檔案（例如 `<code translate="no">./milvus.db</code>`）是最方便的方法，因為它會自動利用<a href="https://milvus.io/docs/milvus_lite.md">Milvus Lite</a>將所有資料儲存至此檔案中。</li>
<li>若您擁有大量資料，可於<a href="https://milvus.io/docs/quickstart.md">Docker 或 Kubernetes</a> 上架設效能更佳的 Milvus 伺服器。在此設定中，請將伺服器 URI（例如<code translate="no">http://localhost:19530</code> ）設為您的<code translate="no">uri</code> 。</li>
<li>若您想使用<a href="https://zilliz.com/cloud">Zilliz Cloud</a>（Milvus 的全託管雲端服務），請調整<code translate="no">uri</code> 和<code translate="no">token</code> ，這兩者分別對應 Zilliz Cloud 中的「<a href="https://docs.zilliz.com/docs/on-zilliz-cloud-console#free-cluster-details">公開端點</a>」<a href="https://docs.zilliz.com/docs/on-zilliz-cloud-console#free-cluster-details">和「API 金鑰」</a>。</li>
</ul>
</div>
<p>檢查該集合是否已存在，若已存在則刪除它。</p>
<pre><code translate="no" class="language-python"><span class="hljs-keyword">if</span> milvus_client.has_collection(collection_name):
    milvus_client.drop_collection(collection_name)
<button class="copy-code-btn"></button></code></pre>
<p>根據指定的參數建立新的集合。</p>
<p>若未指定任何欄位資訊，Milvus 將自動建立一個預設的<code translate="no">id</code> 欄位作為主鍵，並建立一個<code translate="no">vector</code> 欄位來儲存向量資料。系統會使用一個預留的 JSON 欄位來儲存非模式定義的欄位及其值。</p>
...
<h3 id="Insert-data" class="common-anchor-header">插入資料<button data-href="#Insert-data" class="anchor-icon" translate="no">
    ...
    </button></h3><p>遍歷文字行，建立嵌入向量，然後將資料插入 Milvus。</p>
<p>此處出現了一個新的<code translate="no">text</code> 欄位，這是集合模式中未定義的欄位。它將自動新增至預留的 JSON 動態欄位中，從高層次來看，該欄位可視為一般欄位。</p>
...
<h2 id="Build-RAG" class="common-anchor-header">建立 RAG<button data-href="#Build-RAG" class="anchor-icon" translate="no">
    ...
    </button></h2><h3 id="Retrieve-data-for-a-query" class="common-anchor-header">針對查詢擷取資料<button data-href="#Retrieve-data-for-a-query" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>讓我們指定一個關於 Milvus 的常見問題。</p>
<pre><code translate="no" class="language-python">question = <span class="hljs-string">&quot;How is data stored in milvus?&quot;</span>
<button class="copy-code-btn"></button></code></pre>
<p>在集合中搜尋該問題，並擷取語義上最相關的前三項結果。</p>
...
<h3 id="Use-LLM-to-get-a-RAG-response" class="common-anchor-header">使用大型語言模型 (LLM) 生成 RAG 回應<button data-href="#Use-LLM-to-get-a-RAG-response" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>將檢索到的文件轉換為字串格式。</p>
<pre><code translate="no" class="language-python">context = <span class="hljs-string">&quot;\n&quot;</span>.join(
    [line_with_distance[<span class="hljs-number">0</span>] <span class="hljs-keyword">for</span> line_with_distance <span class="hljs-keyword">in</span> retrieved_lines_with_distances]
)
<button class="copy-code-btn"></button></code></pre>
<p>為語言模型定義系統提示與使用者提示。此提示是根據從 Milvus 檢索到的文件所組建的。</p>
...
<h2 id="Multimodal-Search" class="common-anchor-header">多模態搜尋<button data-href="#Multimodal-Search" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>由於<code translate="no">gemini-embedding-2-preview</code> 將文字、圖像及其他模態映射至相同的嵌入空間，因此我們可以進行跨模態搜尋——例如，使用文字查詢來尋找最相關的圖像。</p>
<h3 id="Prepare-image-data" class="common-anchor-header">準備圖像資料<button data-href="#Prepare-image-data" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>我們從 Milvus Bootcamp 儲存庫下載了一組 RAG 架構圖，作為我們的圖像資料集。</p>
...
<h3 id="Embed-images-and-store-in-Milvus" class="common-anchor-header">對圖片進行嵌入並儲存至 Milvus<button data-href="#Embed-images-and-store-in-Milvus" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>我們將每張圖片讀取為位元組，並傳遞給<code translate="no">gemini-embedding-2-preview</code> 以產生嵌入向量，隨後將其儲存至新的 Milvus 集合中。</p>
<pre><code translate="no" class="language-python"><span class="hljs-keyword">from</span> google.genai <span class="hljs-keyword">import</span> types
...
<h3 id="Cross-modal-search-Text-query-→-Image-results" class="common-anchor-header">跨模態搜尋：文字查詢 → 圖片結果<button data-href="#Cross-modal-search-Text-query-→-Image-results" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>現在，讓我們使用文字查詢來跨圖像嵌入進行搜尋。由於文字和圖像都映射到相同的嵌入空間中，因此我們可以直接進行比對。</p>
...
<p>太棒了！我們已成功利用 Milvus 和 Gemini 建置了一條 RAG 處理流程，並展示了透過文字查詢檢索相關圖片的跨模態搜尋功能——所有這些皆由<code translate="no">gemini-embedding-2-preview</code> 的統一嵌入空間所驅動。</p>
