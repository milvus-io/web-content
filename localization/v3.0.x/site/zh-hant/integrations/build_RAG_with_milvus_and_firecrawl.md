...
<h2 id="Preparation" class="common-anchor-header">準備工作<button data-href="#Preparation" class="anchor-icon" translate="no">
    ...
    </button></h3><p>若要開始使用，請執行下列指令來安裝所需的相依性：</p>
...
<h3 id="Setting-Up-API-Keys" class="common-anchor-header">設定 API 金鑰<button data-href="#Setting-Up-API-Keys" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>要使用 Firecrawl 從指定的 URL 搜刮資料，您需要取得<a href="https://www.firecrawl.dev/">FIRECRAWL_API_KEY</a>，並將其設定為環境變數。此外，在本範例中，我們會使用 OpenAI 作為 LLM。您也應該準備<a href="https://platform.openai.com/docs/quickstart">OPENAI_API_KEY</a>作為環境變數。</p>
...
<h2 id="Scrape-Data-Using-Firecrawl" class="common-anchor-header">使用 Firecrawl 搜刮資料<button data-href="#Scrape-Data-Using-Firecrawl" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><h3 id="Initialize-the-Firecrawl-Application" class="common-anchor-header">初始化 Firecrawl 應用程式<button data-href="#Initialize-the-Firecrawl-Application" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>我們將使用<code translate="no">firecrawl</code> 函式庫以 markdown 格式從指定的 URL 搜刮資料。首先初始化 Firecrawl 應用程式：</p>
...
<button class="copy-code-btn"></button></code></pre>
<h3 id="Scrape-the-Target-Website" class="common-anchor-header">抓取目標網站<button data-href="#Scrape-the-Target-Website" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>從目標 URL 搜刮內容。網站<a href="https://lilianweng.github.io/posts/2023-06-23-agent/">LLM-powered Autonomous Agents</a>深入探討使用大型語言模型 (LLM) 建立的自治代理系統。我們將使用這些內容建立 RAG 系統。</p>
...
<button class="copy-code-btn"></button></code></pre>
<h3 id="Process-the-Scraped-Content" class="common-anchor-header">處理 Scraped 內容<button data-href="#Process-the-Scraped-Content" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>為了讓 Scraped 內容可以管理，以便插入 Milvus，我們簡單地使用「#」來分隔內容，這可以大致分隔 Scraped Markdown 檔案中每個主要部分的內容。</p>
...
<h2 id="Build-RAG" class="common-anchor-header">建立 RAG<button data-href="#Build-RAG" class="anchor-icon" translate="no">
    ...
    </button></h3><p>讓我們指定一個關於我們剛剛搜刮到的網站的查詢問題。</p>
