...
<p>n8n 中的<strong>Milvus Vector Store</strong>節點可將<a href="https://milvus.io/">Milvus</a>整合到您的自動化工作流程中。這可讓您執行語意搜尋、強化檢索增量生成 (RAG) 系統，以及建立智慧型 AI 應用程式 - 全都在 n8n 的生態系統中。</p>
...
<h3 id="Use-as-a-regular-node-to-insert-and-retrieve-documents" class="common-anchor-header">作為一般節點來使用，以插入和擷取文件<button data-href="#Use-as-a-regular-node-to-insert-and-retrieve-documents" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>你可以使用 Milvus Vector Store 作為一般節點來插入或取得文件。此模式將 Milvus Vector Store 置於一般連線流程中，不需要使用代理程式。</p>
...
<h3 id="Connect-directly-to-an-AI-agent-as-a-tool" class="common-anchor-header">直接連接 AI 代理作為工具<button data-href="#Connect-directly-to-an-AI-agent-as-a-tool" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>您可以將 Milvus 向量儲存節點直接連接到<a href="https://docs.n8n.io/integrations/builtin/cluster-nodes/root-nodes/n8n-nodes-langchain.agent/">AI 代理</a>的工具連接器，以便在回答查詢時使用向量儲存作為資源。</p>
...
<h3 id="Use-a-retriever-to-fetch-documents" class="common-anchor-header">使用擷取器擷取文檔<button data-href="#Use-a-retriever-to-fetch-documents" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<p>查看此<a href="https://n8n.io/workflows/3574-create-a-paul-graham-essay-qanda-system-with-openai-and-milvus-vector-database/">工作流程範例</a>，瞭解如何將外部資料擷取至 Milvus，並建立以聊天為基礎的語意問答系統。</p>
<h3 id="Use-the-Vector-Store-Question-Answer-Tool-to-answer-questions" class="common-anchor-header">使用 Vector Store 問題解答工具來回答問題<button data-href="#Use-the-Vector-Store-Question-Answer-Tool-to-answer-questions" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>另一種模式是使用<a href="https://docs.n8n.io/integrations/builtin/cluster-nodes/sub-nodes/n8n-nodes-langchain.toolvectorstore/">Vector Store Question Answer Tool</a>來總結結果，並回答來自 Milvus Vector Store 節點的問題。這種模式不是直接連接 Milvus Vector Store 作為工具，而是使用專門設計來總結向量商店資料的工具。</p>
...
<hr>
<h3 id="Operation-Modes-Overview" class="common-anchor-header">操作模式概述<button data-href="#Operation-Modes-Overview" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Milvus Vector Store 節點支援四種不同的模式：</p>
<ul>
...
<h3 id="Additional-Node-Options" class="common-anchor-header">其他節點選項<button data-href="#Additional-Node-Options" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<h3 id="Related-Resources" class="common-anchor-header">相關資源<button data-href="#Related-Resources" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
