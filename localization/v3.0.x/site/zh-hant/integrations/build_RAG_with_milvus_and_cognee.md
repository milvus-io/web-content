...
<h3 id="Build-RAG-with-Milvus-and-Cognee" class="common-anchor-header">使用 Milvus 和 Cognee 建立 RAG<button data-href="#Build-RAG-with-Milvus-and-Cognee" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p><a href="https://www.cognee.ai">Cognee</a>是一個開發者至上的平台，可透過可擴充的模組化 ECL（Extract、Cognify、Load）管道簡化 AI 應用程式開發。透過與 Milvus 的無縫整合，Cognee 可實現對話、文件和轉錄的高效連接和檢索，從而減少幻覺並優化營運成本。</p>
<p>憑藉對 Milvus、圖形資料庫和 LLM 等向量儲存的強大支援，Cognee 為建立檢索增強生成 (RAG) 系統提供了靈活且可定制的框架。它的生產就緒架構可確保為人工智能驅動的應用程式提高準確性和效率。</p>
<p>在本教程中，我們將告訴您如何使用 Milvus 和 Cognee 建立 RAG（檢索-增強生成）管道。</p>
...
<h3 id="Prepare-the-data" class="common-anchor-header">準備資料<button data-href="#Prepare-the-data" class="anchor-icon" translate="no">
    ...
    </button></h3><p>我們使用<a href="https://github.com/milvus-io/milvus-docs/releases/download/v2.4.6-preview/milvus_docs_2.4.x_en.zip">Milvus 文件 2.4.x</a>中的常見問題頁面作為 RAG 中的私有知識，這是簡單 RAG 管道的良好資料來源。</p>
...
<h2 id="Build-RAG" class="common-anchor-header">建立 RAG<button data-href="#Build-RAG" class="anchor-icon" translate="no">
    ...
    </button></h2><h3 id="Resetting-Cognee-Data" class="common-anchor-header">重置 Cognee 資料<button data-href="#Resetting-Cognee-Data" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<h3 id="Adding-Data-and-Cognifying" class="common-anchor-header">新增資料與認知<button data-href="#Adding-Data-and-Cognifying" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<h3 id="Querying-for-Summaries" class="common-anchor-header">查詢摘要<button data-href="#Querying-for-Summaries" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>現在資料已經處理完成，讓我們來查詢知識圖表。</p>
...
<h3 id="Querying-for-Chunks" class="common-anchor-header">查詢大塊<button data-href="#Querying-for-Chunks" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>摘要提供了高層次的洞察力，但若要瞭解更仔細的細節，我們可以直接從處理過的資料集中查詢特定的資料塊。這些資料塊來自於在建立知識圖表時新增和分析的原始資料。</p>
<pre><code translate="no" class="language-python"><span class="hljs-keyword">from</span> cognee.api.v1.search <span class="hljs-keyword">import</span> SearchType
...
<h3 id="Reset-Cognee" class="common-anchor-header">重置 Cognee<button data-href="#Reset-Cognee" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<span class="hljs-keyword">await</span> cognee.prune.prune_system(metadata=<span class="hljs-literal">True</span>)
<button class="copy-code-btn"></button></code></pre>
<h3 id="Adding-the-Focused-Dataset" class="common-anchor-header">新增焦點資料集<button data-href="#Adding-the-Focused-Dataset" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>在這裡，我們添加並處理了一個只有一行文字的較小數據集，以確保知識圖表聚焦且易於詮釋。</p>
...
<span class="hljs-keyword">await</span> cognee.cognify()
<button class="copy-code-btn"></button></code></pre>
<h3 id="Querying-for-Insights" class="common-anchor-header">查詢洞察力<button data-href="#Querying-for-Insights" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>透過聚焦於這個較小的資料集，我們現在可以清楚地分析知識圖形中的關係和結構。</p>
