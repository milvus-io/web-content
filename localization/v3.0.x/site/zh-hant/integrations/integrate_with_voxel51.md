...
<h3 id="Keyword-arguments" class="common-anchor-header">關鍵字參數<button data-href="#Keyword-arguments" class="anchor-icon" translate="no">
    ...
    </button></h3><p>您可以手動提供您的 Milvus 認證作為關鍵字參數，每次您呼叫方法如 <a href="https://docs.voxel51.com/api/fiftyone.brain.html#fiftyone.brain.compute_similarity"><code translate="no">compute_similarity()</code></a>等需要連線到 Milvus 的方法時，您可以手動提供您的 Milvus 認證作為關鍵字參數：</p>
<pre><code translate="no" class="language-python"><span class="hljs-keyword">import</span> fiftyone.brain <span class="hljs-keyword">as</span> fob
...
<span class="hljs-punctuation">}</span>
<button class="copy-code-btn"></button></code></pre>
<p>不過，通常這些參數會直接傳給 <a href="https://docs.voxel51.com/api/fiftyone.brain.html#fiftyone.brain.compute_similarity"><code translate="no">compute_similarity()</code></a>來設定特定的新索引：</p>
