...
<li>由於 Milvus 採用大規模平行處理 (MPP) 架構，因此 Proxy 會先彙集中間結果並進行後處理，再將最終結果傳回給用戶端。</li>
</ul>
...
<li><strong>歷史資料管理</strong>：將離線工作（如壓縮和建立索引）分派給資料節點，並管理區段的拓樸結構和資料檢視。</li>
</ul>
...
<h3 id="Streaming-node" class="common-anchor-header">串流節點<button data-href="#Streaming-node" class="anchor-icon" translate="no">
    ...
    </button></h3><p>Streaming Node 充當 shard 級的「迷你大腦」，提供 shard 級的一致性保證，並根據底層 WAL 儲存進行故障復原。同時，Streaming Node 也負責成長資料查詢和產生查詢計畫。此外，它也負責將成長中的資料轉換為封存（歷史）資料。</p>
