...
<h3 id="Standalone" class="common-anchor-header">單機<button data-href="#Standalone" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Milvus 的單一實體，在一個進程中執行所有元件，適用於小資料集、低工作負載的情況。 此外，在 standalone 模式下，可以選擇較簡單的 WAL 實作，例如 woodpecker 和 rocksmq，以消除對第三方 WAL 儲存的依賴。</p>
<p>
...
<p>目前，即使 WAL 儲存後端支援叢集模式，您也無法從獨立的 Milvus 實例執行線上升級到 Milvus 叢集。</p>
<h3 id="Cluster" class="common-anchor-header">叢集<button data-href="#Cluster" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Milvus 的分散式部署模式，每個元件獨立運作，並可擴充彈性。此設定適用於大型資料集和高負載的情況。</p>
<p>
