...
summary: 使用 Helm 或 Milvus Operator，將 Milvus 叢集的消息佇列在 Pulsar 與 Woodpecker 之間切換。
---
...
<h2 id="With-Helm" class="common-anchor-header">使用 Helm<button data-href="#With-Helm" class="anchor-icon" translate="no">
    ...
    </button></h3><p><strong>步驟 1：確認 Milvus 實例正在執行中。</strong>請確保您的 Milvus 叢集運作正常 —— 例如，建立測試集合、插入資料並執行查詢。</p>
...
<p><strong>步驟 2：配置目標 Pulsar 連線並重新啟動 Milvus。</strong>此切換操作需要 Milvus 已知曉 Pulsar 連線設定，因此請透過<code translate="no">extraConfigFiles</code> 將設定寫入<code translate="no">user.yaml</code> ，並使用<code translate="no">helm upgrade</code> 套用（此指令會重新部署 Pod）。<code translate="no">streaming.enabled=true</code> 對於 Switch MQ 功能是必要的。</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>若切換成功，系統會記錄<code translate="no">[mqTypeValue=woodpecker]</code> 。</p>
<p><strong>步驟 4：更新 Operator 中的 MQ 類型。</strong>更新由<strong>Operator</strong>管理的設定，以防止 Operator 撤銷此切換。建立<code translate="no">change_configmap.yaml</code> ：</p>
...
<div class="alert note">
<p>若您計劃日後切回 Pulsar，請先清理資料/主題以避免衝突。由於 Helm 圖表的限制，目前無法切回<strong>內建的</strong>Pulsar 實例。</p>
</div>
...
<p><strong>步驟 2：配置目標 Pulsar 連線並重新啟動 Milvus。</strong>將 Pulsar 連線置於<code translate="no">spec.config</code> 下（該 Operator 會將<code translate="no">spec.config</code> 渲染為<code translate="no">user.yaml</code> ），並設定 MQ 類型；套用 CR 後，系統會根據新配置重新啟動 Pod。</p>
...
<p><strong>步驟 3：執行 MQ 切換。</strong></p>
<div class="alert note">
<p>請確保目標 Pulsar 中不包含來自先前配置的 Milvus 主題。若這是您首次切換至 Pulsar，請跳過此注意事項；否則，請先清理名稱相同的殘留 Milvus 主題。</p>
</div>
<pre><code translate="no" class="language-shell">kubectl exec -it &lt;mixcoord-pod&gt; -- \
  curl -X POST http://localhost:9091/management/wal/alter \
  -H &quot;Content-Type: application/json&quot; \
