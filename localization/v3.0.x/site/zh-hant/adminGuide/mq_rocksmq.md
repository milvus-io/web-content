...
<h2 id="Deploy-Milvus-Standalone-with-RocksMQ-using-Docker" class="common-anchor-header">使用 Docker 部署搭配 RocksMQ 的 Milvus 獨立執行版<button data-href="#Deploy-Milvus-Standalone-with-RocksMQ-using-Docker" class="anchor-icon" translate="no">
    ...
    </button></h3><p>請參照《<a href="/docs/zh-hant/install_standalone-docker.md">在 Docker 中執行 Milvus</a>》。在 Milvus 3.x 中，獨立模式的預設訊息佇列為 Woodpecker，因此請明確將訊息佇列類型切換為 RocksMQ。初始化腳本會在<strong>首次執行</strong> <code translate="no">start</code> 時建立新的<code translate="no">user.yaml</code> ，因此<strong>請在</strong>首次<strong>啟動後設定</strong>類型，然後執行<code translate="no">restart</code> 以套用設定（執行<code translate="no">restart</code> 可保留<code translate="no">user.yaml</code> 中的資料）：</p>
...
以這種方式切換<code translate="no">mq.type</code> 是針對<b>全新</b>實例（尚未建立任何集合）。若要變更已存有資料的實例之訊息佇列，請改為遵循<a href="/docs/zh-hant/switch-rocksmq-woodpecker.md">切換程序</a>。
</div>
