...
<h1 id="Use-RocksMQ-as-the-Milvus-Message-Queue" class="common-anchor-header">將 RocksMQ 用作 Milvus 訊息佇列<button data-href="#Use-RocksMQ-as-the-Milvus-Message-Queue" class="anchor-icon" translate="no">
    ...
    </button></h1><p>RocksMQ 是一套隨 Milvus 一併內建的嵌入式訊息佇列（WAL），<strong>僅</strong>適用於<strong>Milvus 獨立執行模式</strong>。在較早的 Milvus 版本中，它是預設的獨立執行模式訊息佇列；而在 Milvus 3.x 版本中，Milvus 獨立執行模式預設會使用內建的<a href="/docs/zh-hant/woodpecker.md">Woodpecker</a>。</p>
...
<h2 id="Deploy-Milvus-Standalone-with-RocksMQ-using-Docker" class="common-anchor-header">使用 Docker 部署搭配 RocksMQ 的 Milvus Standalone<button data-href="#Deploy-Milvus-Standalone-with-RocksMQ-using-Docker" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>請參照《<a href="/docs/zh-hant/install_standalone-docker.md">在 Docker 中執行 Milvus</a>》。在 Milvus 3.x 中，獨立執行模式的預設值為 Woodpecker，因此請明確將訊息佇列類型切換為 RocksMQ。初始化腳本會在<strong>首次執行</strong> <code translate="no">start</code> 時建立新的<code translate="no">user.yaml</code> ，因此<strong>請在</strong>首次<strong>啟動後</strong>設定類型，然後執行<code translate="no">restart</code> 以套用設定（執行<code translate="no">restart</code> 可保留<code translate="no">user.yaml</code> 中的資料）：</p>
...
此種切換<code translate="no">mq.type</code> 的方式，是針對<b>全新</b>實例（尚未建立任何集合）而設計。若要變更已存有資料之實例的訊息佇列，請改為遵循<a href="/docs/zh-hant/switch-rocksmq-woodpecker.md">切換程序</a>。
</div>
<h3 id="Configure" class="common-anchor-header">設定<button data-href="#Configure" class="anchor-icon" translate="no">
    ...
    </button></h3><p>若要調整 RocksMQ 的設定，請在<code translate="no">user.yaml</code> 檔案中新增<code translate="no">rocksmq</code> 區段，並重新啟動服務：</p>
