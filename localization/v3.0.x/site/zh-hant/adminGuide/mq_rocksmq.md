...
<h1 id="Use-RocksMQ-as-the-Milvus-Message-Queue" class="common-anchor-header">將 RocksMQ 用作 Milvus 訊息佇列<button data-href="#Use-RocksMQ-as-the-Milvus-Message-Queue" class="anchor-icon" translate="no">
    ...
    </button></h1><p>RocksMQ 是一套隨 Milvus 捆綁提供的內嵌式訊息佇列（WAL），<strong>僅</strong>適用於<strong>Milvus 獨立執行模式</strong>。在較早的 Milvus 版本中，它曾是預設的獨立執行模式訊息佇列；而在 Milvus 3.x 版本中，Milvus 獨立執行模式預設使用內嵌式<a href="/docs/zh-hant/woodpecker.md">Woodpecker</a>。</p>
...
以這種方式切換<code translate="no">mq.type</code> 僅適用於<b>全新</b>實例（尚未建立任何集合）。若要變更已存有資料的實例之訊息佇列，請改為遵循<a href="/docs/zh-hant/switch-rocksmq-woodpecker.md">切換程序</a>。
</div>
...
<li><strong>從 2.5.x 升級至 2.6.x：</strong> <strong>訊息佇列限制</strong>：升級至 Milvus v3.0-beta 時，必須維持當前的訊息佇列選擇。升級過程中不支援在不同的訊息佇列系統之間切換。未來版本將支援變更訊息佇列系統。
由於 2.6.x 將獨立執行模式的預設值變更為 Woodpecker，若您希望保留 RocksMQ，請<strong>在升級前於</strong> <code translate="no">user.yaml</code> 中將<code translate="no">mq.type: rocksmq</code> 固定為預設值。</li>
<li>若要變更正在運作的實例所使用的訊息佇列，請參閱<a href="/docs/zh-hant/switch-rocksmq-woodpecker.md">《從 RocksMQ 切換至 Woodpecker</a>》。</li>
</ul>
