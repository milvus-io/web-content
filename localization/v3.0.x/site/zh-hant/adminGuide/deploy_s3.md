...
<h2 id="Configure-S3-with-Docker-Compose" class="common-anchor-header">使用 Docker Compose 設定 S3<button data-href="#Configure-S3-with-Docker-Compose" class="anchor-icon" translate="no">
    ...
    </button></h2><h3 id="1-Configure-S3" class="common-anchor-header">1.設定 S3<button data-href="#1-Configure-S3" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p><a href="https://min.io/product/overview">MinIO</a>與 S3 相容。要使用 Docker Compose 設定 S3，請在 milvus/configs 路徑上的<code translate="no">milvus.yaml</code> 檔案中提供<code translate="no">minio</code> 部分的值。</p>
...
<h3 id="2-Refine-docker-composeyaml" class="common-anchor-header">2.完善 docker-compose.yaml<button data-href="#2-Refine-docker-composeyaml" class="anchor-icon" translate="no">
    ...
    </button></h3><p>您也可以移除<code translate="no">MINIO_ADDRESS</code> 環境變數，讓 milvus 服務在<code translate="no">docker-compose.yaml</code> 。預設的情況下，milvus 會使用本機的 minio 而不是外部的 S3。</p>
<h3 id="3-Run-Milvus" class="common-anchor-header">3.執行 Milvus<button data-href="#3-Run-Milvus" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>執行下列指令啟動使用 S3 配置的 Milvus。</p>
...
<button class="copy-code-btn"></button></code></pre>
<h3 id="Using-a-command" class="common-anchor-header">使用指令<button data-href="#Using-a-command" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>要安裝 Milvus 並配置 S3，請使用您的值執行以下指令。</p>
