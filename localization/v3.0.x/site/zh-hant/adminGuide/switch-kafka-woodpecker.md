...
<p><strong>步驟 4：更新 Operator 中的訊息佇列類型。</strong>更新由<strong>Operator</strong>管理的設定，以防止 Operator 還原此切換。建立<code translate="no">change_configmap.yaml</code> ：</p>
...
<p><strong>步驟 3：執行 MQ 切換。</strong></p>
<div class="alert note">
<p>請確保目標 Kafka 中不包含來自先前設定的 Milvus 主題。若這是您首次切換至 Kafka，請跳過此注意事項；否則請先清理名稱相同的殘留 Milvus 主題。</p>
</div>
<pre><code translate="no" class="language-shell">kubectl exec -it &lt;mixcoord-pod&gt; -- \
  curl -X POST http://localhost:9091/management/wal/alter \
  -H &quot;Content-Type: application/json&quot; \
