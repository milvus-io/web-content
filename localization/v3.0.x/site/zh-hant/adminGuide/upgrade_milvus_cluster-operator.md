...
<p>此程序不驗證透過將 Milvus 映像還原至 2.6.x 版本所進行的降級或回滾操作。 在 v3.0-beta 寫入資料後，若執行僅映像檔的回滾，可能會無法讀取已更新的狀態。若升級失敗，請停止寫入操作，並採用能還原升級前元資料及持久化資料備份的復原方案。請先在非生產環境中驗證該復原方案。</p>
</div>
...
<p>在套用目標 CR 之前，請確認<code translate="no">indexNode.replicas</code> 設定為<code translate="no">0</code> 。已驗證的 Milvus 2.6.20 配置已採用此設定。請在目標 CR 中保留此明確的「零副本」設定。</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>請確認 Milvus CR 回報的<code translate="no">Healthy</code> 值、所有 Milvus 元件均使用<code translate="no">milvusdb/milvus:v3.0-beta</code> 設定、沒有任何 IndexNode Pod 正在執行，且現有集合仍可進行查詢與搜尋。在啟用任何 v3.0-beta 專屬功能之前，請先完成這些檢查。</p>
