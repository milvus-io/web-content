...
<h1 id="Upgrade-Milvus-Cluster-with-Milvus-Operator" class="common-anchor-header">使用 Milvus Operator 升級 Milvus 叢集<button data-href="#Upgrade-Milvus-Cluster-with-Milvus-Operator" class="anchor-icon" translate="no">
    ...
    </button></h1><p>本指南說明如何使用 Milvus Operator 將 Milvus 2.6.x 叢集升級至 v3.0.0。</p>
<div class="alert note">
<p>此程序已針對從 Milvus 2.6.20 升級至 Milvus v3.0.0 的情境進行驗證，所使用的環境包含 Milvus Operator 1.3.0、MixCoord、StreamingNode、Woodpecker、叢集內 etcd 以及叢集內 MinIO。 若您使用其他 Milvus 2.6.x 修補版本、Operator 版本、元件拓撲、訊息佇列或依賴項配置，請先在非生產環境中驗證升級流程。</p>
</div>
...
<p>此程序不驗證透過將 Milvus 映像還原至 2.6.x 版本所進行的降級或回滾操作。 在 v3.0.0 寫入資料後，若執行僅映像檔的回滾，可能會無法讀取已更新的狀態。若升級失敗，請停止寫入操作，並採用能還原升級前元資料及持久化資料備份的復原方案。請先在非生產環境中驗證該復原方案。</p>
</div>
<h2 id="Upgrade-process" class="common-anchor-header">升級流程<button data-href="#Upgrade-process" class="anchor-icon" translate="no">
    ...
    </button></h3><p>在變更部署設定前，請先儲存當前的 CR：</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>經過驗證的升級方案將 Milvus Operator 維持在 1.3.0 版本。除非您的支援政策要求進行獨立的 Operator 升級，否則請維持目前管理您的 Milvus 2.6.x 部署的 Operator 版本。 請勿將較新版本的 Operator 降級至已測試的版本。若需變更 Operator 版本，請採用與現有安裝相同的 Helm 或<code translate="no">kubectl</code> 安裝方式，並使用相同的發行版本名稱與命名空間，然後在更新 Milvus CR 之前驗證 Operator 的變更。</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>請確認 Milvus CR 回報的<code translate="no">Healthy</code> 值、所有 Milvus 元件均使用<code translate="no">milvusdb/milvus:v3.0.0</code> 設定、沒有任何 IndexNode Pod 正在執行，且現有集合仍可進行查詢與搜尋。在啟用任何 v3.0.0 專屬功能之前，請先完成這些檢查。</p>
