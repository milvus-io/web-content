...
---
<div class="tab-wrapper"><a href="/docs/zh-hant/upgrade_milvus_standalone-operator.md" class=''>Milvus</a><a href="/docs/zh-hant/upgrade_milvus_standalone-docker.md" class=''>操作員</a>、Helm、D<a href="/docs/zh-hant/upgrade_milvus_standalone-docker.md" class=''>ocker Compose</a></div>
<h1 id="Upgrade-Milvus-Standalone-with-Helm-Chart" class="common-anchor-header">使用 Helm Chart 升級 Milvus 獨立部署版本<button data-href="#Upgrade-Milvus-Standalone-with-Helm-Chart" class="anchor-icon" translate="no">
    ...
    </button></h1><p>本指南說明如何使用 Helm 將您的 Milvus 2.6.x 獨立部署升級至 v3.0.1。</p>
<div class="alert note">
<p>此程序已針對從 Milvus 2.6.20 升級至 Milvus v3.0.1（搭配 Milvus Helm Chart 5.0.22）進行驗證。若您使用其他 Milvus 2.6.x 修補版本或 Helm Chart 版本，請先在非生產環境中驗證升級流程。</p>
</div>
...
<p>此程序未驗證將 Milvus 映像降級回 2.6.x 的操作。 在 v3.0.1 寫入資料後，若僅還原映像檔，可能無法讀取已更新的狀態。若升級失敗，請停止寫入操作，並採用能還原升級前元資料及持久化資料備份的復原方案。請先於非生產環境中驗證該復原方案。</p>
</div>
...
<h3 id="Step-2-Upgrade-Milvus" class="common-anchor-header">步驟 2：升級 Milvus<button data-href="#Step-2-Upgrade-Milvus" class="anchor-icon" translate="no">
    ...
    </button></h3><p>檢查您的 Helm 發行版已安裝的 Charts 版本：</p>
<pre><code translate="no" class="language-bash">helm list --namespace &lt;namespace&gt;
<button class="copy-code-btn"></button></code></pre>
<p>在「<code translate="no">CHART</code> 」欄位中，從值中移除「<code translate="no">milvus-</code> 」前綴，並將剩餘的版本號改為「<code translate="no">&lt;current-chart-version&gt;</code> 」。接著執行升級指令：</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>確認所有必要的工作負載均已就緒，Milvus 已採用<code translate="no">v3.0.1</code> ，且現有集合仍可進行查詢與搜尋。在啟用任何 v3.0.1 專屬功能之前，請先完成這些檢查。</p>
<div class="alert note">
<p>升級至 Milvus 3.0 並不會啟用 Storage V3。驗證升級完成後，請先審閱<a href="/docs/zh-hant/storage-v3.md">Storage V3</a>相關資訊，再啟用依賴於該功能的特性。一旦 Milvus 寫入 Storage V3 資料，便不支援降級至無法讀取 Storage V3 的舊版 Milvus。</p>
</div>
