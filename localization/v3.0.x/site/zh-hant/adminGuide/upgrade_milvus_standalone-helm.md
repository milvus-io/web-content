...
<h1 id="Upgrade-Milvus-Standalone-with-Helm-Chart" class="common-anchor-header">使用 Helm Chart 升級 Milvus 獨立部署<button data-href="#Upgrade-Milvus-Standalone-with-Helm-Chart" class="anchor-icon" translate="no">
    ...
    </button></h1><p>本指南說明如何使用 Helm Chart，將您的 Milvus 獨立部署從 v2.5.x 升級至 v3.0-beta。</p>
...
自 Milvus Helm 圖表版本 4.2.21 起，我們已將 pulsar-v3.x 圖表納入為依賴項。為確保向後相容性，請將您的 Helm 升級至 v3.14 或更新版本，並務必在每次使用 `<code translate="no">helm upgrade</code>` 時，加入 `<code translate="no">--reset-then-reuse-values</code> ` 選項。
</div>
...
<h3 id="Step-3-Upgrade-to-v30-beta" class="common-anchor-header">步驟 3：升級至 v3.0-beta<button data-href="#Step-3-Upgrade-to-v30-beta" class="anchor-icon" translate="no">
    ...
    </button></h3><p>當 v2.5.16 成功運行後，請升級至 v3.0-beta：</p>
<pre><code translate="no" class="language-bash">helm upgrade my-release zilliztech/milvus \
