...
title: 資料基礎架構
summary: Milvus 所使用的元資料儲存、物件儲存及訊息佇列的概述。
---
<h1 id="Data-Infrastructure" class="common-anchor-header">資料基礎架構<button data-href="#Data-Infrastructure" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h1><p>Milvus 的核心資料基礎架構仰賴元資料儲存、物件儲存以及訊息佇列。本章將介紹您可以進行設定的元件：</p>
<ul>
<li><strong><a href="/docs/zh-hant/etcd.md">元資料</a></strong>— Milvus 將元資料（集合模式、節點狀態、消耗檢查點）儲存於 etcd 中。</li>
<li><strong><a href="/docs/zh-hant/object-storage.md">物件儲存</a></strong>— Milvus 將索引檔案和二進位日誌儲存於 MinIO、AWS S3 或其他相容於 S3 的雲端物件儲存服務中。</li>
