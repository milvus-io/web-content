...
<p>若要使用 Woodpecker 服務模式，我們建議採用即將推出的 Milvus 3.0.1 或更新版本，並搭配 Woodpecker v0.1.36 或更新版本，以獲得壓縮清理與群組提交的優化效果。</p>
</div>
...
<h3 id="Service-mode-Milvus-30+" class="common-anchor-header">服務模式（Milvus 3.0+）<button data-href="#Service-mode-Milvus-30+" class="anchor-icon" translate="no">
    ...
    </button></h3><p>服務模式在維持以物件儲存為後端的 WAL 高寫入吞吐量的同時，還增加了低延遲（請參閱「<a href="#Latency">延遲」</a>）。 上述儲存端與客戶端側的調校方式依然適用；此外，由於 Woodpecker 以獨立服務形式運行，您可透過增加複本（<code translate="no">woodpecker.replicaCount</code> ，預設為 4）來水平擴展寫入容量，且寫入操作能受益於單 RTT 法定人數複製，以及可避免經由中介伺服器轉發的拓撲感知讀取機制。</p>
...
<ul>
<li><strong>單 RTT 法定人數寫入</strong>— 由客戶端驅動的複製可在單次往返內完成法定人數寫入，且跨可用區域的流量固定為兩個副本的資料量（相較於基於中介/領導節點的複製通常會產生額外約 1/3 的跨可用區域流量）。</li>
