...
<h2 id="Version" class="common-anchor-header">版本<button data-href="#Version" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Milvus 已針對<strong>etcd 3.5.x</strong> 進行過相容性驗證。當您使用 Helm 安裝 Milvus 時，預設捆綁的 etcd 映像檔為<code translate="no">milvusdb/etcd:3.5.25-r1</code> 。</p>
...
<p>若使用外部 etcd，或需了解 Docker Compose / Helm / Milvus Operator 的詳細設定，請參閱<a href="/docs/zh-hant/deploy_etcd.md">《使用 Docker Compose 或 Helm 設定元資料儲存》</a>及<a href="/docs/zh-hant/meta_storage_operator.md">《使用 Milvus Operator 設定元資料儲存</a>》。</p>
