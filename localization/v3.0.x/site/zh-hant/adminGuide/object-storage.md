...
<h2 id="Additional-requirements-when-using-embedded-Woodpecker" class="common-anchor-header">使用內嵌式 Woodpecker 時的額外要求<button data-href="#Additional-requirements-when-using-embedded-Woodpecker" class="anchor-icon" translate="no">
    ...
    </button></h2><p>當您執行預設的<strong>Woodpecker</strong>訊息佇列及其物件儲存後端（<code translate="no">storage.type=minio</code> ）時，Woodpecker 會將其預寫日誌寫入相同的物件儲存中，並需要<strong>嚴格遵循 S3 條件寫入語義</strong>。 並非所有物件儲存服務都符合資格——例如，儘管華為雲 OBS 可作為一般的 Milvus 物件儲存使用，但目前<strong>尚不支援</strong>作為 Woodpecker 的後端。</p>
