...
<h1 id="Manage-Resource-Groups" class="common-anchor-header">管理資源群組<button data-href="#Manage-Resource-Groups" class="anchor-icon" translate="no">
    ...
    </button></h1><p>在 Milvus 中，您可以透過資源群組將某些查詢節點與其他節點進行物理隔離。本指南將引導您了解如何建立和管理自訂資源群組，以及如何在資源群組之間移動節點。</p>
<h2 id="What-is-a-resource-group" class="common-anchor-header">何謂資源群組<button data-href="#What-is-a-resource-group" class="anchor-icon" translate="no">
    ...
    </button></h2><p>一個資源群組可以包含 Milvus 叢集中的部分或所有查詢節點。 您可以根據自身需求，決定如何將查詢節點分配至各資源群組。例如，在多資料集的場景中，您可以將適當數量的查詢節點分配給每個資源群組，並將資料集載入至不同的資源群組，藉此使每個資料集內的操作在物理上獨立於其他資料集的操作。</p>
<p>請注意，Milvus 實例在啟動時會維護一個預設資源群組來容納所有查詢節點，並將其命名為<strong>__default_resource_group</strong>。</p>
<p>自 2.4.1 版本起，Milvus 提供宣告式資源群組 API，而舊版資源群組 API 已宣告為過時。新的宣告式 API 讓使用者能夠實現冪等性，並更輕鬆地在雲原生環境中進行次要開發。</p>
...
<p>一旦資源群組的配置發生變更，Milvus 將盡可能根據新配置調整當前的查詢節點資源，以確保所有資源群組最終都能滿足以下條件：</p>
...
<li>當 Milvus 叢集中的查詢節點數量過多時，即<code translate="no">NumOfQueryNode &gt; sum(.limits.nodeNum)</code> ，多餘的查詢節點將始終優先配置至<strong>__default_resource_group</strong>。</li>
</ul>
<p>當然，若叢集中的 QueryNodes 數量發生變化，Milvus 會持續嘗試進行調整以符合最終條件。因此，您可以先套用資源群組的配置變更，然後再執行 QueryNodes 的擴展。</p>
...
<p>本頁所有程式碼範例均採用 PyMilvus 3.0.2 版本。執行前請先升級您的 PyMilvus 安裝版本。</p>
</div>
<ol>
<li><p>建立資源群組。</p>
...
<button class="copy-code-btn"></button></code></pre></li>
<li><p>在資源群組之間移動節點。</p>
<p>您可能會注意到，該資源群組目前尚無任何查詢節點。請依照以下步驟，將部分節點從預設資源群組移至您所建立的資源群組：
假設叢集的<strong>__default_resource_group</strong>目前有 1 個 QueryNodes，而我們想要將其中一個節點移至已建立的<strong>資源群組中</strong>。<code translate="no">update_resource_groups</code> 可確保多項配置變更的原子性，因此 Milvus 不會看到任何中間狀態。</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>此外，您也可以僅將一個分區載入至資源群組，並讓其副本分佈於多個資源群組中。以下範例假設已存在一個名為<code translate="no">Books</code> 的集合，且該集合含有名為<code translate="no">Novels</code> 的分區。</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>請注意，<code translate="no">_resource_groups</code> 為可選參數，若未指定此參數，Milvus 會將複本載入至預設資源群組中的查詢節點。</p>
<p>若要讓 Milvus 將集合的每個副本載入至不同的資源群組中，請確保資源群組的數量等於副本的數量。</p></li>
<li><p>在資源群組之間移轉副本。</p>
<p>Milvus 透過<a href="/docs/zh-hant/replica.md">副本</a>在分佈於多個查詢節點<a href="/docs/zh-hant/glossary.md#Segment">的區段</a>之間實現負載平衡。您可以依照以下步驟，將集合的特定副本從一個資源群組移至另一個資源群組：</p>
<pre><code translate="no" class="language-python">source = <span class="hljs-string">&#x27;__default_resource_group&#x27;</span>
target = <span class="hljs-string">&#x27;rg&#x27;</span>
...
<button class="copy-code-btn"></button></code></pre></li>
<li><p>刪除資源群組。</p>
<p>您可以隨時刪除未包含任何查詢節點的資源群組（<code translate="no">limits.node_num = 0</code> ）。在本指南中，資源群組<code translate="no">rg</code> 目前包含一個查詢節點。您需先將該資源群組的配置<code translate="no">limits.node_num</code> 設定為零。</p>
...
<h2 id="A-good-practice-to-manage-cluster-scaling" class="common-anchor-header">管理叢集擴展的良好實務<button data-href="#A-good-practice-to-manage-cluster-scaling" class="anchor-icon" translate="no">
    ...
    </button></h2><p>目前，Milvus 無法在雲原生環境中獨立進行擴展與縮減。然而，透過結合使用「<strong>宣告式資源群組 API</strong>」與容器調度，Milvus 能輕鬆實現對 QueryNodes 的資源隔離與管理。
以下是在雲端環境中管理 QueryNodes 的最佳實務：</p>
<ol>
<li><p>預設情況下，Milvus 會建立一個<strong>__default_resource_group</strong>。此資源群組無法刪除，同時也作為所有集合的預設載入資源群組，且冗餘的 QueryNodes 會始終被指派至該資源群組。 因此，我們可以建立一個「待處理」資源群組來存放閒置的 QueryNode 資源，防止這些資源被<strong>__default_resource_group</strong> 佔用。</p>
<p>此外，若嚴格執行<code translate="no">sum(.requests.nodeNum) &lt;= queryNodeNum</code> 這項限制，即可精確控制叢集內 QueryNodes 的分配。假設叢集中目前僅有一台 QueryNode，並初始化該叢集。
以下為設定範例：</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>利用上述範例程式碼，我們建立了一個名為<strong>__pending_nodes 的</strong>資源群組，用以存放額外的 QueryNodes。我們也建立兩個名為<strong>rg1</strong>和<strong>rg2</strong> 的使用者專用資源群組。此外，我們確保其他資源群組會優先從<strong>__pending_nodes</strong> 中恢復缺失或冗餘的 QueryNodes。</p></li>
<li><p>叢集水平擴展</p>
<p>假設我們有以下擴展函式：</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>我們可以利用 API 將特定資源群組擴展至指定的 QueryNodes 數量，同時不影響其他資源群組。</p>
...
<button class="copy-code-btn"></button></code></pre></li>
<li><p>叢集縮減</p>
<p>同樣地，我們可以建立縮減規則，優先從<strong>__pending_nodes</strong>資源群組中選取 QueryNodes。此資訊可透過<code translate="no">describe_resource_group</code> API 取得，藉此達成縮減指定資源群組的目標。</p>
...
<li>單一集合的副本與資源群組之間存在 N 對 N 的關係。</li>
