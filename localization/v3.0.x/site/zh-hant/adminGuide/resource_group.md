...
<h2 id="What-is-a-resource-group" class="common-anchor-header">何謂資源群組<button data-href="#What-is-a-resource-group" class="anchor-icon" translate="no">
    ...
    </button></h2><p>一個資源群組可以包含 Milvus 叢集中的部分或所有查詢節點。 您可以根據自身需求，決定如何將查詢節點分配至各資源群組。例如，在多集合的場景中，您可以將適當數量的查詢節點分配給每個資源群組，並將集合載入至不同的資源群組，使每個集合內的操作在物理上獨立於其他集合的操作。</p>
...
<p>本頁所有程式碼範例均基於 PyMilvus 3.0.1。執行前請先升級您的 PyMilvus 安裝版本。</p>
</div>
<ol>
<li><p>建立資源群組。</p>
...
<p>此外，若嚴格執行<code translate="no">sum(.requests.nodeNum) &lt;= queryNodeNum</code> 此限制條件，即可精確控制叢集內 QueryNodes 的分配。假設叢集中目前僅有一台 QueryNode，並初始化該叢集。
以下為設定範例：</p>
