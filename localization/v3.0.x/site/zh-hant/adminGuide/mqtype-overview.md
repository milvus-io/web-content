...
<h1 id="Message-Queue-Overview" class="common-anchor-header">訊息佇列概覽<button data-href="#Message-Queue-Overview" class="anchor-icon" translate="no">
    ...
    </button></h1><p>Milvus 仰賴訊息佇列（預寫日誌，WAL）來管理近期變更的日誌、輸出串流日誌，並提供日誌訂閱功能。在 Milvus 3.x 中<strong>，Woodpecker</strong>是預設的訊息佇列，無需額外的訊息傳遞基礎架構。針對特定情境，Pulsar、Kafka 及 RocksMQ 仍持續受到支援。</p>
...
<li><strong>新部署（Milvus 3.x）：</strong>請使用<strong>Woodpecker</strong>（預設值）。獨立執行環境會以內嵌方式運行；對於分散式（叢集）環境，建議的預設方式是透過 Helm 部署專用<a href="/docs/zh-hant/woodpecker.md#Deployment-modes">服務</a>，同時也支援內嵌模式。</li>
...
<li><strong>RocksMQ：</strong>僅限獨立執行模式，且已在 Milvus 3.x 中被內嵌的 Woodpecker 所取代。</li>
</ul>
