...
<li><strong>新部署（Milvus 3.x）：</strong>請使用<strong>Woodpecker</strong>（預設）。獨立執行模式會以內嵌方式運行；對於分散式（叢集）模式，建議的預設方式是透過 Helm 部署專用<a href="/docs/zh-hant/woodpecker.md#Deployment-modes">服務</a>，同時也支援內嵌模式。</li>
<li><strong>現有 Pulsar 或 Kafka 使用者：</strong>Pulsar 和 Kafka 仍獲得完整支援。您可以繼續使用，或切換至 Woodpecker。</li>
