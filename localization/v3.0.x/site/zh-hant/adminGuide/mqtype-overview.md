...
<li><strong>新部署（Milvus 3.x）：</strong>請使用<strong>Woodpecker</strong>（預設值）。獨立執行環境會以內嵌方式運行；對於分散式（叢集）環境，建議的預設值是透過 Helm 部署的專用<a href="/docs/zh-hant/woodpecker.md#Deployment-modes">服務</a>，同時也支援內嵌模式。</li>
