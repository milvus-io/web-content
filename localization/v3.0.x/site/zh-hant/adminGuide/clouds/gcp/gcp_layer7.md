...
<p>本指南將教您如何為已經在第 4 層負載平衡器後面執行的 Milvus 集群設定第 7 層負載平衡器。</p>
...
<h3 id="Tweak-Milvus-configurations" class="common-anchor-header">調整 Milvus 配置<button data-href="#Tweak-Milvus-configurations" class="anchor-icon" translate="no">
    ...
    </button></h3><p>本指南假設您已<a href="/docs/zh-hant/gcp.md">在 GCP 的第 4 層負載平衡器後部署 Milvus 叢集</a>。</p>
...
<h3 id="Set-up-a-health-check-endpoint" class="common-anchor-header">設定健康檢查端點<button data-href="#Set-up-a-health-check-endpoint" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>為了確保服務可用性，GCP 上的 Layer-7 負載平衡需要探測後端服務的健康狀況。因此，我們需要設定一個 BackendConfig 來包裝健康檢查端點，並透過註解將 BackendConfig 與 Milvus 服務關聯。</p>
...
<li><p>至於第二個註解、</p>
<p>Milvus 只透過 gRPC 和 HTTP/1 提供健康檢查端點。我們需要設定一個 BackendConfig 來包裝健康檢查端點，並將其與 Milvus 服務關聯，以便 Layer-7 負載平衡器探測此端點，以瞭解 Milvus 的健康狀況。</p></li>
<li><p>至於第三個註解、</p>
...
<h3 id="Prepare-TLS-certificates" class="common-anchor-header">準備 TLS 憑證<button data-href="#Prepare-TLS-certificates" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>TLS 需要憑證才能運作。<strong>有兩種建立憑證的方式，即自我管理和 Google 管理。</strong></p>
...
<h3 id="Create-an-Ingress-to-generate-a-Layer-7-Load-Balancer" class="common-anchor-header">建立一個 Ingress 來產生第七層負載平衡器<button data-href="#Create-an-Ingress-to-generate-a-Layer-7-Load-Balancer" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>使用下列其中一個片段建立 YAML 檔案。</p>
<ul>
<li><p>使用自我管理的憑證</p>
