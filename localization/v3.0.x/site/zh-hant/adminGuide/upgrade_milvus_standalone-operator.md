...
<h1 id="Upgrade-Milvus-Standalone-with-Milvus-Operator" class="common-anchor-header">使用 Milvus Operator 升級 Milvus 獨立部署<button data-href="#Upgrade-Milvus-Standalone-with-Milvus-Operator" class="anchor-icon" translate="no">
    ...
    </button></h1><p>本指南說明如何使用 Milvus Operator 將 Milvus 2.6.x 獨立部署升級至 v3.0-beta。</p>
<div class="alert note">
<p>此程序已通過驗證，適用於從 Milvus 2.6.20 升級至 Milvus v3.0-beta，並搭配 Milvus Operator 1.3.0、Woodpecker、叢集內 etcd 以及叢集內 MinIO。 若您使用其他 Milvus 2.6.x 修補版本、Operator 版本、訊息佇列或依賴項配置，請先在非生產環境中驗證升級流程。</p>
</div>
...
<li>用於現有部署的完整 Milvus 自訂資源 (CR) 宣言</li>
...
<p>此程序不支援透過將 Milvus 映像還原至 2.6.x 版本來進行降級或回滾。 在 v3.0-beta 寫入資料後，若僅執行映像檔層級的回滾，可能會無法讀取更新後的狀態。若升級失敗，請停止寫入操作，並採用能還原升級前元資料與持久化資料備份的復原方案。請先在非生產環境中驗證該復原方案。</p>
</div>
...
<p>請將現有部署的來源清單用作升級清單。請勿直接套用匯出的備份檔案，務必先移除伺服器管理的元資料與狀態欄位。</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>經過驗證的升級方案將 Milvus Operator 維持在 1.3.0 版本。除非您的支援政策要求進行獨立的 Operator 升級，否則請保留目前管理您的 Milvus 2.6.x 部署的 Operator 版本。 請勿將較新的 Operator 降級至已測試的版本。若需變更 Operator 版本，請使用與現有安裝相同的 Helm 或 `<code translate="no">kubectl</code> ` 安裝方式，並採用相同的發行版名稱和命名空間，然後在更新 Milvus CR 之前驗證 Operator 的變更。</p>
<h3 id="Step-3-Update-the-Milvus-image" class="common-anchor-header">步驟 3：更新 Milvus 映像檔<button data-href="#Step-3-Update-the-Milvus-image" class="anchor-icon" translate="no">
    ...
    </button></h3><p>在完整的 Milvus CR 清單中，僅需修改<code translate="no">spec.components.image</code> 部分。請保留現有的模式、元件設定、訊息佇列、etcd、儲存空間及其他依賴項設定。以下摘錄顯示需修改的欄位；請勿以此摘錄取代您的完整 CR。</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>請確認 Milvus CR 報告的<code translate="no">Healthy</code> ，當前映像為<code translate="no">milvusdb/milvus:v3.0-beta</code> ，且現有集合仍可查詢與搜尋。在啟用任何 v3.0-beta 專屬功能之前，請先完成這些檢查。</p>
