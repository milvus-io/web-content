...
summary: 瞭解如何使用 Milvus Operator 升級 Milvus 獨立版本。
...
<h1 id="Upgrade-Milvus-Standalone-with-Milvus-Operator" class="common-anchor-header">使用 Milvus Operator 升級 Milvus 獨立部署<button data-href="#Upgrade-Milvus-Standalone-with-Milvus-Operator" class="anchor-icon" translate="no">
    ...
    </button></h1><p>本指南說明如何透過 Milvus Operator 將 Milvus 2.6.x 獨立部署升級至 v3.0.2。</p>
<div class="alert note">
<p>此程序已針對從 Milvus 2.6.20 升級至 Milvus v3.0.2 的情境進行驗證，所使用的環境包含 Milvus Operator 1.3.0、Woodpecker、叢集內 etcd 以及叢集內 MinIO。 若您使用其他 Milvus 2.6.x 修補版本、Operator 版本、訊息佇列或依賴項配置，請先在非生產環境中驗證升級流程。</p>
</div>
...
<li>現有 Milvus Operator 所使用的安裝方式與配置檔</li>
<li>Milvus 元資料與持久化資料的最新備份</li>
</ul>
<p><strong>訊息佇列限制</strong>：升級至 Milvus v3.0.2 時，必須維持目前所選用的訊息佇列。升級過程中不支援在不同的訊息佇列系統之間切換。未來版本將支援變更訊息佇列系統。</p>
<div class="alert warning">
<p>此程序不驗證透過將 Milvus 映像回滾至 2.6.x 版本所進行的降級或回滾操作。 在 v3.0.2 寫入資料後，若僅執行映像檔層級的還原，可能會無法讀取已更新的狀態。若升級失敗，請停止寫入作業，並採用能還原升級前元資料與持久資料備份的恢復方案。請先於非生產環境中驗證該恢復方案。</p>
</div>
...
<button class="copy-code-btn"></button></code></pre>
<p>經過驗證的升級方案將 Milvus Operator 版本維持在 1.3.0。除非您的支援政策要求進行獨立的 Operator 升級，否則請維持目前管理您的 Milvus 2.6.x 部署的 Operator 版本。 請勿將較新版本的 Operator 降級至已測試的版本。若需變更 Operator 版本，請採用與現有安裝相同的 Helm 或<code translate="no">kubectl</code> 安裝方式，並使用與現有安裝相同的發行版本名稱及命名空間，然後在更新 Milvus CR 之前驗證 Operator 的變更。</p>
<h3 id="Step-3-Update-the-Milvus-image" class="common-anchor-header">步驟 3：更新 Milvus 映像檔<button data-href="#Step-3-Update-the-Milvus-image" class="anchor-icon" translate="no">
    ...
    </button></h3><p>在完整的 Milvus CR 清單中，僅需修改<code translate="no">spec.components.image</code> 部分。請保留現有的模式、元件設定、訊息佇列、etcd、儲存空間及其他依賴項設定。以下摘錄顯示了需修改的欄位；請勿以此摘錄取代您的完整 CR。</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>確認 Milvus CR 報告的<code translate="no">Healthy</code> 是否正確，當前映像檔為<code translate="no">milvusdb/milvus:v3.0.2</code> ，且現有集合仍可查詢與搜尋。在啟用任何 v3.0.2 專屬功能之前，請先完成這些檢查。</p>
