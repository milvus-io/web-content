...
<h1 id="Upgrade-Milvus-Cluster-with-Helm-Chart" class="common-anchor-header">使用 Helm 圖表升級 Milvus 叢集<button data-href="#Upgrade-Milvus-Cluster-with-Helm-Chart" class="anchor-icon" translate="no">
    ...
    </button></h1><p>本指南說明如何使用 Helm 將您的 Milvus 2.6.x 叢集升級至 v3.0.0。</p>
<div class="alert note">
<p>此程序已針對從 Milvus 2.6.20 升級至 Milvus v3.0.0，並使用 Milvus Helm Chart 5.0.22 進行驗證。若您使用其他 Milvus 2.6.x 修補版本或 Helm Chart 版本，請先在非生產環境中驗證升級流程。</p>
</div>
...
<p>請勿在此程序中變更或降級 Helm Chart。請保留您 Helm 發行版中已安裝的 Chart 版本。經測試的基準配置保留了 Helm Chart 5.0.22，僅將 Milvus 映像標籤變更為<code translate="no">v3.0.0</code> 。</p>
<p>此程序未驗證將 Milvus 映像降級回 2.6.x 的操作。 在 v3.0.0 寫入資料後，若僅還原映像檔，可能無法讀取更新後的狀態。若升級失敗，請停止寫入操作，並採用能還原升級前元資料及持久化資料備份的復原方案。請先在非生產環境中驗證該復原方案。</p>
</div>
<h2 id="Upgrade-process" class="common-anchor-header">升級流程<button data-href="#Upgrade-process" class="anchor-icon" translate="no">
    ...
    </button></h2><p>此經驗證的 Milvus 2.6.20 部署是使用 Helm Chart 5.0.22 建立的，採用 MixCoord 和 StreamingNode，且未運行 IndexNode。若您的部署採用相同的拓撲結構，則無需執行獨立的 coordinator-migration 步驟。</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>在「<code translate="no">CHART</code> 」欄位中，從值中移除「<code translate="no">milvus-</code> 」前綴，並將剩餘的版本號以「<code translate="no">&lt;current-chart-version&gt;</code> 」格式表示。接著執行升級指令：</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>確認所有必要的工作負載均已就緒、所有 Milvus 元件皆使用<code translate="no">v3.0.0</code> ，且現有集合仍可進行查詢與搜尋。在啟用任何 v3.0.0 專屬功能之前，請先完成這些檢查。</p>
<div class="alert note">
