...
title: 切換訊息佇列
summary: 在不造成服務中斷的情況下，將現有的 Milvus 部署在 Woodpecker 與另一條訊息佇列之間切換。
---
<h1 id="Switch-Message-Queue" class="common-anchor-header">切換訊息佇列<button data-href="#Switch-Message-Queue" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h1><p>本指南說明如何將現有 Milvus 部署中的訊息佇列 (MQ)<strong>在 Woodpecker 與其他訊息佇列之間</strong>進行切換，且能在線上運作且無停機時間的情況下完成。</p>
...
<h2 id="Scope" class="common-anchor-header">適用範圍<button data-href="#Scope" class="anchor-icon" translate="no">
    ...
    </button></h2><p>本指南僅涵蓋<strong>在 Woodpecker 與其他訊息佇列之間</strong>進行切換。Pulsar 與 Kafka 之間的直接切換不在本指南範圍內。</p>
<ul>
...
<p>切換前，請確保目標 MQ 中不包含與當前 Milvus 執行個體所用名稱相同的話題。若目標 MQ 曾被其他 Milvus 執行個體使用過，此點尤為重要，因為話題名稱衝突可能導致預期之外的行為。</p>
</div>
...
<tbody>
<tr><td>RocksMQ</td><td>Woodpecker（本地／MinIO）</td><td>獨立部署 (Docker Compose)</td><td><strong>支援</strong></td></tr>
