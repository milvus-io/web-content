...
<h1 id="Configure-Message-Storage-with-Milvus-Operator" class="common-anchor-header">使用 Milvus Operator 設定訊息儲存<button data-href="#Configure-Message-Storage-with-Milvus-Operator" class="anchor-icon" translate="no">
    ...
    </button></h1><p>在 Milvus 3.x 中，Woodpecker 是預設的訊息佇列（請參閱<a href="/docs/zh-hant/woodpecker.md">Woodpecker</a>）。透過 Milvus Operator，您亦可配置 RocksMQ、Pulsar 或 Kafka，用以管理最新變更的日誌、輸出串流日誌，並提供日誌訂閱服務。 本主題介紹在透過 Milvus Operator 安裝 Milvus 時，如何配置訊息儲存依賴項。如需更多詳細資訊，請參閱 Milvus Operator 儲存庫中的《<a href="https://github.com/zilliztech/milvus-operator/blob/main/docs/administration/manage-dependencies/message-storage.md">使用 Milvus Operator 配置訊息儲存</a>》。</p>
...
<h2 id="Before-you-begin" class="common-anchor-header">開始之前<button data-href="#Before-you-begin" class="anchor-icon" translate="no">
    ...
    </button></h2><p>下表顯示 Milvus 獨立執行模式與叢集模式是否支援 RocksMQ、Pulsar、Kafka 及 Woodpecker。</p>
<table>
<thead>
...
<li><strong>升級限制</strong>：<strong>訊息佇列限制</strong>：升級至 Milvus v3.0.1 時，您必須維持當前的訊息佇列選項。升級過程中不支援在不同的訊息佇列系統之間切換。未來版本將支援變更訊息佇列系統的功能。</li>
</ul>
<h2 id="Configure-RocksMQ" class="common-anchor-header">設定 RocksMQ<button data-href="#Configure-RocksMQ" class="anchor-icon" translate="no">
    ...
    </button></h2><p>在 Milvus 獨立部署版本 2.5.x 之前，RocksMQ 曾是預設的消息儲存系統（自 2.6.x 起由 Woodpecker 取代）。</p>
<div class="alert note">
<p>目前，您僅能透過 Milvus Operator 將 RocksMQ 設定為 Milvus 獨立執行版的訊息儲存系統。</p>
</div>
...
<li><code translate="no">persistence.pvcDeletion</code>: 若設定為 true，當 Milvus 實例被刪除時，對應的 PVC 也會被刪除</li>
...
<h4 id="Example" class="common-anchor-header">範例</h4><p>以下範例展示如何設定內部 Pulsar 服務。</p>
<pre><code translate="no" class="language-YAML"><span class="hljs-attr">apiVersion:</span> <span class="hljs-string">milvus.io/v1alpha1</span>
<span class="hljs-attr">kind:</span> <span class="hljs-string">Milvus</span>
<span class="hljs-attr">metadata:</span>
  <span class="hljs-attr">name:</span> <span class="hljs-string">my-release</span>
...
<span class="hljs-attr">spec:</span>
  ...
  <span class="hljs-attr">config:</span> {}            
<button class="copy-code-btn"></button></code></pre>
<div class="alert note">此範例指定了 Pulsar 各元件的副本數、Pulsar BookKeeper 的運算資源，以及其他設定。</div>
<div class="alert note">請參閱<a href="https://artifacthub.io/packages/helm/apache/pulsar/2.7.8?modal=values">values.yaml</a> 檔案，以了解用於配置內部 Pulsar 服務的完整配置項目。如前例所示，請根據需要在<code translate="no">pulsar.inCluster.values</code> 下新增配置項目。</div>
...
<h4 id="Example" class="common-anchor-header">範例</h4><p>以下範例展示如何配置內部 Kafka 服務。</p>
<pre><code translate="no" class="language-yaml"><span class="hljs-attr">apiVersion:</span> <span class="hljs-string">milvus.io/v1alpha1</span>
<span class="hljs-attr">kind:</span> <span class="hljs-string">Milvus</span>
<span class="hljs-attr">metadata:</span>
  <span class="hljs-attr">name:</span> <span class="hljs-string">my-release</span>
