...
<p>指定訊息儲存時還存在其他限制：</p>
<ul>
<li>每個 Milvus 實例僅支援一個訊息儲存空間。不過，我們仍維持向後相容性，允許為單一實例設定多個訊息儲存空間。其優先順序如下：
<ul>
...
<li><strong>升級限制</strong>：<strong>訊息佇列限制</strong>：升級至 Milvus v3.0.2 時，必須維持當前的訊息佇列選擇。升級過程中不支援在不同的訊息佇列系統之間切換。未來版本將支援變更訊息佇列系統。</li>
</ul>
<h2 id="Configure-RocksMQ" class="common-anchor-header">設定 RocksMQ<button data-href="#Configure-RocksMQ" class="anchor-icon" translate="no">
    ...
    </button></h2><p>在 Milvus 獨立執行模式 2.5.x 版本之前，RocksMQ 為預設訊息儲存系統（自 2.6.x 起由 Woodpecker 取代）。</p>
...
<h2 id="Configure-Pulsar" class="common-anchor-header">配置 Pulsar<button data-href="#Configure-Pulsar" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Pulsar 負責管理近期變更的日誌、輸出串流日誌，並提供日誌訂閱服務。無論是 Milvus 獨立執行模式還是 Milvus 叢集模式，皆支援將 Pulsar 配置為訊息儲存方案。然而，若使用 Milvus Operator，您僅能將 Pulsar 配置為 Milvus 叢集的訊息儲存方案。請在 `<code translate="no">spec.dependencies.pulsar</code> ` 下新增必要欄位以配置 Pulsar。</p>
...
<ul>
<li><code translate="no">external</code>:  若<code translate="no">true</code> 欄位為「啟用」，表示 Milvus 使用外部 Pulsar 服務。</li>
...
<h3 id="Internal-Pulsar" class="common-anchor-header">內部 Pulsar<button data-href="#Internal-Pulsar" class="anchor-icon" translate="no">
    ...
    </button></h3><p><code translate="no">inCluster</code> 表示當 Milvus 叢集啟動時，Pulsar 服務會自動在叢集中啟動。</p>
<h4 id="Example" class="common-anchor-header">範例</h4><p>以下範例展示如何配置內部 Pulsar 服務。</p>
<pre><code translate="no" class="language-YAML"><span class="hljs-attr">apiVersion:</span> <span class="hljs-string">milvus.io/v1alpha1</span>
<span class="hljs-attr">kind:</span> <span class="hljs-string">Milvus</span>
<span class="hljs-attr">metadata:</span>
  <span class="hljs-attr">name:</span> <span class="hljs-string">my-release</span>
...
<span class="hljs-attr">spec:</span>
  ...
  <span class="hljs-attr">config:</span> {}            
<button class="copy-code-btn"></button></code></pre>
<div class="alert note">此範例指定了 Pulsar 各元件的副本數量、Pulsar BookKeeper 的運算資源，以及其他設定。</div>
<div class="alert note">請參閱<a href="https://artifacthub.io/packages/helm/apache/pulsar/2.7.8?modal=values">values.yaml</a> 檔案，以查找用於配置內部 Pulsar 服務的完整配置項目。如前例所示，請根據需要在<code translate="no">pulsar.inCluster.values</code> 之下新增配置項目。</div>
...
<h2 id="Configure-Kafka" class="common-anchor-header">設定 Kafka<button data-href="#Configure-Kafka" class="anchor-icon" translate="no">
    ...
    </button></h2><p>在 Milvus 叢集版本 2.5.x 之前，Pulsar 是預設的訊息儲存方案（自 2.6.x 起已被 Woodpecker 取代）。若要使用 Kafka，請新增可選欄位<code translate="no">msgStreamType</code> 來配置 Kafka。</p>
...
<li><code translate="no">brokerList</code>：要傳送訊息的仲介伺服器清單。</li>
</ul>
...
<button class="copy-code-btn"></button></code></pre>
<blockquote>
<p>SASL 配置僅支援在 operator v0.8.5 或更高版本中。</p>
</blockquote>
...
<span class="hljs-attr">spec:</span> 
  ...
  <span class="hljs-attr">config:</span> {}
<button class="copy-code-btn"></button></code></pre>
<p>請<a href="https://artifacthub.io/packages/helm/bitnami/kafka">在此處</a>查閱用於設定內部 Kafka 服務的完整設定項目。請根據需要，在 `<code translate="no">kafka.inCluster.values</code>` 目錄下新增設定項目。</p>
<p>假設配置檔名為<code translate="no">milvuscluster.yaml</code> ，請執行以下命令以套用配置。</p>
