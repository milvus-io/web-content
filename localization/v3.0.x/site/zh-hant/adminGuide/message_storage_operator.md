...
summary: 瞭解如何使用 Milvus Operator 設定訊息儲存。
---
<h1 id="Configure-Message-Storage-with-Milvus-Operator" class="common-anchor-header">使用 Milvus Operator 設定訊息儲存<button data-href="#Configure-Message-Storage-with-Milvus-Operator" class="anchor-icon" translate="no">
    ...
    </button></h1><p>在 Milvus 3.x 中，Woodpecker 是預設的訊息佇列（請參閱<a href="/docs/zh-hant/woodpecker.md">Woodpecker</a>）。透過 Milvus Operator，您亦可配置 RocksMQ、Pulsar 或 Kafka，以管理近期變更的日誌、輸出串流日誌，並提供日誌訂閱服務。 本主題介紹在透過 Milvus Operator 安裝 Milvus 時，如何配置訊息儲存依賴項。如需更多詳細資訊，請參閱 Milvus Operator 儲存庫中的《<a href="https://github.com/zilliztech/milvus-operator/blob/main/docs/administration/manage-dependencies/message-storage.md">使用 Milvus Operator 配置訊息儲存</a>》。</p>
<p>本主題假設您已部署 Milvus Operator。</p>
<div class="alert note">如需更多資訊，請參閱《<a href="https://milvus.io/docs/v2.2.x/install_cluster-milvusoperator.md">部署 Milvus Operator</a>》。 </div>
<p>您需要指定一個配置檔案，才能使用 Milvus Operator 啟動 Milvus 叢集。</p>
<pre><code translate="no" class="language-YAML"><span class="hljs-string">kubectl</span> <span class="hljs-string">apply</span> <span class="hljs-string">-f</span> <span class="hljs-string">https://raw.githubusercontent.com/zilliztech/milvus-operator/main/config/samples/milvus_cluster_default.yaml</span>
<button class="copy-code-btn"></button></code></pre>
<p>您只需編輯<code translate="no">milvus_cluster_default.yaml</code> 中的程式碼範本，即可配置第三方依賴項。以下各節將分別介紹如何配置物件儲存、etcd 以及 Pulsar。</p>
<h2 id="Before-you-begin" class="common-anchor-header">開始之前<button data-href="#Before-you-begin" class="anchor-icon" translate="no">
    ...
    </button></h2><p>下表列出 Milvus 獨立執行模式與叢集模式是否支援 RocksMQ、Pulsar、Kafka 及 Woodpecker。</p>
<table>
<thead>
...
<tbody>
<tr><td style="text-align:center">獨立模式</td><td style="text-align:center">✔️</td><td style="text-align:center">✔️</td><td style="text-align:center">✔️</td><td style="text-align:center">✔️</td></tr>
...
<p>指定訊息儲存時還有其他限制：</p>
<ul>
<li>每個 Milvus 實例僅支援一個訊息儲存。不過，我們仍維持向後相容性，允許為單一實例設定多個訊息儲存。其優先順序如下：
<ul>
<li>獨立模式：Woodpecker（預設） &gt; RocksMQ &gt; Pulsar &gt; Kafka</li>
<li>叢集模式：Woodpecker（預設） &gt; Pulsar &gt; Kafka</li>
</ul></li>
<li>Milvus 系統運行期間無法變更訊息儲存。</li>
<li>僅支援 Kafka 2.x 或 3.x 版本。</li>
<li><strong>升級限制</strong>：<strong>訊息佇列限制</strong>：升級至 Milvus v3.0-beta 時，您必須維持當前的訊息佇列選擇。升級過程中不支援在不同的訊息佇列系統之間切換。未來版本將支援變更訊息佇列系統。</li>
</ul>
<h2 id="Configure-RocksMQ" class="common-anchor-header">設定 RocksMQ<button data-href="#Configure-RocksMQ" class="anchor-icon" translate="no">
    ...
    </button></h2><p>在 Milvus 獨立部署版本 2.5.x 之前，RocksMQ 曾是預設的訊息儲存方案（自 2.6.x 起由 Woodpecker 取代）。</p>
<div class="alert note">
<p>目前，您只能透過 Milvus Operator 將 RocksMQ 設定為 Milvus 獨立執行版的訊息儲存方案。</p>
</div>
...
<li><code translate="no">msgStreamType</code>: rocksmq：明確將 RocksMQ 設定為訊息佇列</li>
<li><code translate="no">persistence.enabled</code>: 啟用 RocksMQ 資料的持久化儲存</li>
<li><code translate="no">persistence.pvcDeletion</code>: 若設定為 true，當 Milvus 實例被刪除時，PVC 亦將被刪除</li>
<li><code translate="no">persistentVolumeClaim.spec</code>: 標準的 Kubernetes PVC 規格</li>
<li><code translate="no">accessModes</code>: 通常用於區塊儲存的<code translate="no">ReadWriteOnce</code> </li>
<li><code translate="no">storageClassName</code>: 您的叢集所使用的儲存類別</li>
<li><code translate="no">storage</code>: 持久化卷的大小</li>
</ul>
<h2 id="Configure-Woodpecker" class="common-anchor-header">設定 Woodpecker<button data-href="#Configure-Woodpecker" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>Woodpecker 是一款專為物件儲存設計的雲原生預寫日誌（WAL）。它具備高吞吐量、低運作開銷及無縫擴展性。如需更多詳細資訊，請參閱<a href="/docs/zh-hant/woodpecker.md">Woodpecker</a>。</p>
<h2 id="Configure-Pulsar" class="common-anchor-header">配置 Pulsar<button data-href="#Configure-Pulsar" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Pulsar 負責管理近期變更的日誌、輸出串流日誌，並提供日誌訂閱服務。無論是 Milvus 獨立執行模式還是 Milvus 叢集模式，均支援將 Pulsar 配置為訊息儲存。然而，若使用 Milvus Operator，您僅能將 Pulsar 配置為 Milvus 叢集的訊息儲存。請在 `<code translate="no">spec.dependencies.pulsar</code> ` 下新增必要欄位以配置 Pulsar。</p>
...
<h3 id="External-Pulsar" class="common-anchor-header">外部 Pulsar<button data-href="#External-Pulsar" class="anchor-icon" translate="no">
    ...
    </button></h3><p><code translate="no">external</code> 表示使用外部 Pulsar 服務。
用於配置外部 Pulsar 服務的欄位包括：</p>
<ul>
<li><code translate="no">external</code>：若<code translate="no">true</code> 欄位設為「是」，表示 Milvus 將使用外部 Pulsar 服務。</li>
<li><code translate="no">endpoints</code>: Pulsar 的端點。</li>
</ul>
<h4 id="Example" class="common-anchor-header">範例</h4><p>以下範例展示如何配置外部 Pulsar 服務。</p>
...
<h3 id="Internal-Pulsar" class="common-anchor-header">內部 Pulsar<button data-href="#Internal-Pulsar" class="anchor-icon" translate="no">
    ...
    </button></h3><p><code translate="no">inCluster</code> 表示當 Milvus 叢集啟動時，叢集內的 Pulsar 服務會自動啟動。</p>
<h4 id="Example" class="common-anchor-header">範例</h4><p>以下範例說明如何配置內部 Pulsar 服務。</p>
<pre><code translate="no" class="language-YAML"><span class="hljs-attr">apiVersion:</span> <span class="hljs-string">milvus.io/v1alpha1</span>
<span class="hljs-attr">kind:</span> <span class="hljs-string">Milvus</span>
<span class="hljs-attr">metadata:</span>
  <span class="hljs-attr">name:</span> <span class="hljs-string">my-release</span>
...
<span class="hljs-attr">spec:</span>
  ...
  <span class="hljs-attr">config:</span> {}            
<button class="copy-code-btn"></button></code></pre>
<div class="alert note">此範例指定了 Pulsar 各元件的複本數、Pulsar BookKeeper 的運算資源，以及其他設定。</div>
<div class="alert note">請參閱<a href="https://artifacthub.io/packages/helm/apache/pulsar/2.7.8?modal=values">values.yaml</a> 檔案，以查找用於配置內部 Pulsar 服務的完整配置項目。如前例所示，請根據需要在 `<code translate="no">pulsar.inCluster.values</code> ` 下方新增配置項目。</div>
<p>假設配置檔案名為<code translate="no">milvuscluster.yaml</code> ，請執行以下命令以套用配置。</p>
<pre><code translate="no" class="language-Shell">kubectl apply -f milvuscluster.yaml
<button class="copy-code-btn"></button></code></pre>
<h2 id="Configure-Kafka" class="common-anchor-header">設定 Kafka<button data-href="#Configure-Kafka" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>在 2.5.x 版本之前，Pulsar 是 Milvus 叢集的預設訊息儲存方案（自 2.6.x 起由 Woodpecker 取代）。若要使用 Kafka，請新增可選欄位<code translate="no">msgStreamType</code> 來配置 Kafka。</p>
<p><code translate="no">kafka</code> 支援<code translate="no">external</code> 及<code translate="no">inCluster</code> 。</p>
...
<ul>
<li><code translate="no">external</code>：<code translate="no">true</code> 的值表示 Milvus 使用外部 Kafka 服務。</li>
...
<button class="copy-code-btn"></button></code></pre>
<blockquote>
<p>SASL 配置僅支援 operator v0.8.5 或更高版本。</p>
</blockquote>
<h3 id="Internal-Kafka" class="common-anchor-header">內部 Kafka<button data-href="#Internal-Kafka" class="anchor-icon" translate="no">
    ...
    </button></h3><p><code translate="no">inCluster</code> 表示當 Milvus 叢集啟動時，叢集內的 Kafka 服務會自動啟動。</p>
<h4 id="Example" class="common-anchor-header">範例</h4><p>以下範例說明如何設定內部 Kafka 服務。</p>
<pre><code translate="no" class="language-yaml"><span class="hljs-attr">apiVersion:</span> <span class="hljs-string">milvus.io/v1alpha1</span>
<span class="hljs-attr">kind:</span> <span class="hljs-string">Milvus</span>
<span class="hljs-attr">metadata:</span>
  <span class="hljs-attr">name:</span> <span class="hljs-string">my-release</span>
...
<span class="hljs-attr">spec:</span> 
  ...
  <span class="hljs-attr">config:</span> {}
<button class="copy-code-btn"></button></code></pre>
<p><a href="https://artifacthub.io/packages/helm/bitnami/kafka">請在此處</a>查閱配置內部 Kafka 服務所需的完整配置項目。請根據需要，在 `<code translate="no">kafka.inCluster.values</code>` 下新增配置項目。</p>
<p>假設配置檔名為<code translate="no">milvuscluster.yaml</code> ，請執行以下指令以套用配置。</p>
...
<h2 id="Whats-next" class="common-anchor-header">下一步<button data-href="#Whats-next" class="anchor-icon" translate="no">
    ...
    </button></h2><p>瞭解如何使用 Milvus Operator 配置其他 Milvus 依賴項：</p>
<ul>
<li><a href="/docs/zh-hant/object_storage_operator.md">使用 Milvus Operator 配置物件儲存</a></li>
<li><a href="/docs/zh-hant/meta_storage_operator.md">使用 Milvus Operator 配置元資料儲存</a></li>
</ul>
