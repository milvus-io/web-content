...
<h1 id="Configure-Message-Storage-with-Milvus-Operator" class="common-anchor-header">使用 Milvus Operator 設定訊息儲存<button data-href="#Configure-Message-Storage-with-Milvus-Operator" class="anchor-icon" translate="no">
    ...
    </button></h1><p>在 Milvus 3.x 中，Woodpecker 是預設的訊息佇列（請參閱<a href="/docs/zh-hant/woodpecker.md">Woodpecker</a>）。透過 Milvus Operator，您亦可配置 RocksMQ、Pulsar 或 Kafka，用以管理近期變更的日誌、輸出串流日誌，以及提供日誌訂閱服務。 本主題介紹在透過 Milvus Operator 安裝 Milvus 時，如何配置訊息儲存依賴項。如需更多詳細資訊，請參閱 Milvus Operator 儲存庫中的《<a href="https://github.com/zilliztech/milvus-operator/blob/main/docs/administration/manage-dependencies/message-storage.md">使用 Milvus Operator 配置訊息儲存</a>》。</p>
...
<li><strong>升級限制</strong>：<strong>訊息佇列限制</strong>：升級至 Milvus v3.0.0 時，您必須維持當前的訊息佇列選擇。升級過程中不支援在不同的訊息佇列系統之間切換。未來版本將支援變更訊息佇列系統。</li>
</ul>
<h2 id="Configure-RocksMQ" class="common-anchor-header">設定 RocksMQ<button data-href="#Configure-RocksMQ" class="anchor-icon" translate="no">
    ...
    </button></h2><p>在 Milvus 獨立部署版本 2.5.x 之前，RocksMQ 為預設訊息儲存方案（自 2.6.x 起由 Woodpecker 取代）。</p>
<div class="alert note">
<p>目前，您僅能透過 Milvus Operator 將 RocksMQ 設定為 Milvus 獨立執行版的訊息儲存方案。</p>
</div>
...
<h2 id="Configure-Pulsar" class="common-anchor-header">配置 Pulsar<button data-href="#Configure-Pulsar" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Pulsar 負責管理近期變更的日誌、輸出串流日誌，並提供日誌訂閱功能。無論是 Milvus 獨立執行模式或 Milvus 叢集模式，皆支援將 Pulsar 設定為訊息儲存。然而，若使用 Milvus Operator，您僅能將 Pulsar 設定為 Milvus 叢集的訊息儲存。請在 `<code translate="no">spec.dependencies.pulsar</code> ` 下新增必要欄位以設定 Pulsar。</p>
...
<p>假設設定檔名為<code translate="no">milvuscluster.yaml</code> ，請執行以下指令以套用設定。</p>
