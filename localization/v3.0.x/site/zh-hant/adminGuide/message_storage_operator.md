...
<h1 id="Configure-Message-Storage-with-Milvus-Operator" class="common-anchor-header">使用 Milvus Operator 設定訊息儲存<button data-href="#Configure-Message-Storage-with-Milvus-Operator" class="anchor-icon" translate="no">
    ...
    </button></h1><p>在 Milvus 3.x 中，Woodpecker 是預設的消息佇列（請參閱<a href="/docs/zh-hant/woodpecker.md">Woodpecker</a>）。透過 Milvus Operator，您亦可配置 RocksMQ、Pulsar 或 Kafka，用以管理近期變更的日誌、輸出串流日誌，以及提供日誌訂閱服務。 本主題介紹在透過 Milvus Operator 安裝 Milvus 時，如何配置訊息儲存依存項。如需更多詳細資訊，請參閱 Milvus Operator 儲存庫中的《<a href="https://github.com/zilliztech/milvus-operator/blob/main/docs/administration/manage-dependencies/message-storage.md">使用 Milvus Operator 配置訊息儲存</a>》。</p>
...
<p>您需要指定一個配置檔案，才能透過 Milvus Operator 啟動 Milvus 叢集。</p>
...
<p>指定訊息儲存時還存在其他限制：</p>
<ul>
<li>每個 Milvus 實例僅支援一個訊息儲存。不過，我們仍向下相容於單一實例設定多個訊息儲存的情形。優先順序如下：
<ul>
...
<li><strong>升級限制</strong>：<strong>訊息佇列限制</strong>：升級至 Milvus v3.0-beta 時，您必須維持目前的訊息佇列選擇。升級過程中不支援在不同的訊息佇列系統之間切換。未來版本將支援變更訊息佇列系統。</li>
</ul>
...
<li><code translate="no">persistence.pvcDeletion</code>: 若設定為 true，當 Milvus 實例被刪除時，該 PVC 亦將被刪除</li>
...
<h2 id="Configure-Woodpecker" class="common-anchor-header">設定 Woodpecker<button data-href="#Configure-Woodpecker" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Woodpecker 是一款專為物件儲存設計的雲原生預寫日誌 (WAL)。它具備高吞吐量、低運作開銷及無縫可擴展性。如需更多詳細資訊，請參閱<a href="/docs/zh-hant/woodpecker.md">Woodpecker</a>。</p>
...
<ul>
<li><code translate="no">external</code>:  若<code translate="no">true</code> 欄位設為「是」，表示 Milvus 會使用外部 Pulsar 服務。</li>
...
<div class="alert note">請參閱<a href="https://artifacthub.io/packages/helm/apache/pulsar/2.7.8?modal=values">values.yaml</a> 檔案，以查看用於配置內部 Pulsar 服務的完整配置項目。如前例所示，請根據需要在 `<code translate="no">pulsar.inCluster.values</code> ` 下方新增配置項目。</div>
<p>假設配置檔案名為<code translate="no">milvuscluster.yaml</code> ，請執行以下指令以套用配置。</p>
...
<h2 id="Configure-Kafka" class="common-anchor-header">設定 Kafka<button data-href="#Configure-Kafka" class="anchor-icon" translate="no">
    ...
    </button></h2><p>在 Milvus 叢集的 2.5.x 版本之前，Pulsar 是預設的訊息儲存方案（自 2.6.x 版本起由 Woodpecker 取代）。若要使用 Kafka，請新增可選欄位<code translate="no">msgStreamType</code> 來配置 Kafka。</p>
...
<h4 id="Example" class="common-anchor-header">範例</h4><p>以下範例示範如何設定外部 Kafka 服務。</p>
<pre><code translate="no" class="language-yaml"><span class="hljs-attr">apiVersion:</span> <span class="hljs-string">milvus.io/v1alpha1</span>
<span class="hljs-attr">kind:</span> <span class="hljs-string">Milvus</span>
<span class="hljs-attr">metadata:</span>
  <span class="hljs-attr">name:</span> <span class="hljs-string">my-release</span>
...
<h4 id="Example" class="common-anchor-header">範例</h4><p>以下範例展示如何設定內部 Kafka 服務。</p>
<pre><code translate="no" class="language-yaml"><span class="hljs-attr">apiVersion:</span> <span class="hljs-string">milvus.io/v1alpha1</span>
<span class="hljs-attr">kind:</span> <span class="hljs-string">Milvus</span>
<span class="hljs-attr">metadata:</span>
  <span class="hljs-attr">name:</span> <span class="hljs-string">my-release</span>
