...
<h1 id="Switch-between-RocksMQ-and-Woodpecker" class="common-anchor-header">在 RocksMQ 與 Woodpecker 之間切換<button data-href="#Switch-between-RocksMQ-and-Woodpecker" class="anchor-icon" translate="no">
    ...
    </button></h1><p>本頁面說明如何將<strong>Milvus 獨立部署（Docker Compose）</strong>中的訊息佇列（MQ）在<strong>RocksMQ</strong>與<strong>Woodpecker</strong>（本機或 MinIO 後端）之間進行雙向切換。有關一般工作流程與先決條件，請參閱<a href="/docs/zh-hant/switch-mq-type.md">《切換訊息佇列》</a>。</p>
...
<h2 id="Switch-from-RocksMQ-to-Woodpecker" class="common-anchor-header">從 RocksMQ 切換至 Woodpecker<button data-href="#Switch-from-RocksMQ-to-Woodpecker" class="anchor-icon" translate="no">
    ...
    </button></h3><p>請確認您的 Milvus 獨立 Docker Compose 實例運作正常 — 例如，建立測試集合、插入資料並執行查詢。</p>
...
<li><strong>儲存資料：</strong>在<strong>MinIO 模式下</strong>，請刪除桶中<code translate="no">&lt;rootPath&gt;/wp/...</code> 下的日誌資料（通常位於<code translate="no">files/wp/...</code> ）；在<strong>本地模式下</strong>，資料位於本機磁碟的<code translate="no">volumes/milvus/data/wp/...</code> 。</li>
</ul>
...
<tr><td>Woodpecker 本地</td><td>Woodpecker MinIO</td><td><strong>不支援</strong></td><td>與上文相同。</td></tr>
