...
summary: 了解如何使用 Docker Compose 升級 Milvus 獨立版本。
title: 使用 Docker Compose 升級 Milvus 獨立部署版本
---
<div class="tab-wrapper"><a href="/docs/zh-hant/upgrade_milvus_standalone-operator.md" class=''>Milvus</a><a href="/docs/zh-hant/upgrade_milvus_standalone-docker.md" class='active '>操作員</a>、Helm、<a href="/docs/zh-hant/upgrade_milvus_standalone-docker.md" class='active '>Docker Compose</a></div>
<h1 id="Upgrade-Milvus-Standalone-with-Docker-Compose" class="common-anchor-header">使用 Docker Compose 升級 Milvus 獨立部署版本<button data-href="#Upgrade-Milvus-Standalone-with-Docker-Compose" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h1><p>本指南說明如何使用 Docker Compose 將 Milvus 2.6.x 獨立部署版本升級至 v3.0.1。</p>
<div class="alert note">
<p>此程序已針對官方 Milvus 2.6.20 獨立部署版 Docker Compose 配置進行驗證。升級過程保留了 etcd、MinIO、Woodpecker 以及現有的資料目錄，僅將 Milvus 映像檔更換為<code translate="no">milvusdb/milvus:v3.0.1</code> 。</p>
</div>
...
<li>現有部署所使用的 Docker Compose 檔案與設定</li>
<li>Milvus 元資料與持久化資料的最新備份</li>
</ul>
<p><strong>訊息佇列限制</strong>：升級至 Milvus v3.0.1 時，必須維持當前的訊息佇列選擇。升級過程中不支援在不同的訊息佇列系統之間切換。未來版本將支援變更訊息佇列系統。</p>
<div class="alert warning">
<p>請勿在此程序中替換您目前的 Compose 檔案或變更依賴項版本。請保留現有的 etcd、物件儲存、訊息佇列、卷宗及配置。僅需更新 Milvus 映像標籤。</p>
<p>此程序不支援透過將 Milvus 映像標籤改回 2.6.x 來執行降級或還原。 在 v3.0.1 寫入資料後，僅還原映像檔的回滾操作可能無法讀取已更新的狀態。若升級失敗，請停止寫入作業，並採用能還原升級前元資料及持久化資料備份的復原方案。請先在非生產環境中驗證該復原方案。</p>
</div>
...
<button class="copy-code-btn"></button></code></pre>
<p>確認<code translate="no">standalone</code> 服務狀態正常，其映像為<code translate="no">milvusdb/milvus:v3.0.1</code> ，且現有的集合仍可進行查詢與搜尋。在啟用任何 v3.0.1 專屬功能之前，請先完成這些檢查。</p>
