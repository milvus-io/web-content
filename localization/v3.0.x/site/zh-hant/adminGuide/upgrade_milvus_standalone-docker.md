...
<h1 id="Upgrade-Milvus-Standalone-with-Docker-Compose" class="common-anchor-header">使用 Docker Compose 升級 Milvus 獨立部署<button data-href="#Upgrade-Milvus-Standalone-with-Docker-Compose" class="anchor-icon" translate="no">
    ...
    </button></h1><p>本指南說明如何使用 Docker Compose 將 Milvus 2.6.x 獨立部署版本升級至 v3.0.2。</p>
<div class="alert note">
<p>此程序已透過官方 Milvus 2.6.20 獨立部署版 Docker Compose 配置進行驗證。升級過程保留了 etcd、MinIO、Woodpecker 以及現有的資料目錄，僅將 Milvus 映像檔更換為<code translate="no">milvusdb/milvus:v3.0.2</code> 。</p>
</div>
...
<p>此程序不支援透過將 Milvus 映像回滾至 2.6.x 版本來進行降級或回滾。 在 v3.0.2 寫入資料後，若執行僅還原映像檔的回滾操作，可能會無法讀取已更新的狀態。若升級失敗，請停止寫入操作，並採用能還原升級前元資料及持久化資料備份的復原方案。請先在非生產環境中驗證該復原方案。</p>
</div>
...
<button class="copy-code-btn"></button></code></pre>
<h3 id="Step-2-Update-the-Milvus-image" class="common-anchor-header">步驟 2：更新 Milvus 映像<button data-href="#Step-2-Update-the-Milvus-image" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<button class="copy-code-btn"></button></code></pre>
<p>Docker Compose 會讓現有的 etcd 和 object-storage 容器繼續運行，並重複使用已配置的資料目錄。</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>確認<code translate="no">standalone</code> 服務狀態正常，其映像為<code translate="no">milvusdb/milvus:v3.0.2</code> ，且現有的集合仍可查詢與搜尋。在啟用任何 v3.0.2 專屬功能之前，請先完成這些檢查。</p>
