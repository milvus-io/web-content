...
title: milvusのAGENTS.md
summary: PyMilvusを使用してMilvusベクトルデータベースコードを生成、レビュー、デバッグするAIコーディングエージェントのルールとパターン。
---
<h1 id="AGENTSmd--Milvus" class="common-anchor-header">AGENTS.md - Milvus（ミルバス<button data-href="#AGENTSmd--Milvus" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h1><p>Milvusは類似検索、ハイブリッド検索、RAGのためのオープンソースのベクトルデータベースです。PyMilvus SDKの<code translate="no">MilvusClient</code> インタフェースを通して操作します。以下のプロンプトをAIツールにコピーすると、これらのルールが自動的に適用されます。タスク固有のプロンプトの詳細については、<a href="/docs/ja/milvus_for_agents.md">AIプロンプトを</a>参照してください。</p>
<h2 id="How-to-use-this-prompt" class="common-anchor-header">このプロンプトの使用方法<button data-href="#How-to-use-this-prompt" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<li>以下の「<a href="#full-prompt">完全なプロンプト」</a>セクションから完全なプロンプトを<strong>コピー</strong>します。</li>
<li>このプロンプトをAIツールが指定する場所に<strong>保存</strong>します。</li>
<li>AIアシスタントはMilvusコードを生成またはレビューする際にこれらのルールを自動的に適用します。</li>
</ol>
<p><strong>Cursorを</strong>お使いの場合: プロンプトを<a href="#full-prompt">フルプロンプトセクションから</a>コピーし、プロジェクトの<code translate="no">.cursor/rules/</code> 。</p>
<h2 id="Full-prompt" class="common-anchor-header">フルプロンプト<button data-href="#Full-prompt" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
