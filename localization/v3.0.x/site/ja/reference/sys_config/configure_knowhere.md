...
summary: Milvus用のknowhereの設定方法について説明します。
---
<h1 id="knowhere-related-Configurations" class="common-anchor-header">knowhere 関連設定<button data-href="#knowhere-related-Configurations" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h1><p>knowhere ベクター検索エンジンに関連するすべての設定</p>
<h2 id="knowhereenable" class="common-anchor-header"><code translate="no">knowhere.enable</code><button data-href="#knowhereenable" class="anchor-icon" translate="no">
  ...
  <tbody>
    <tr>
      <td>        この設定を有効にすると、以下に定義するインデックス・パラメータが自動的にインデックス・パラメータとして入力されます。      </td>
      <td>true</td>
    </tr>
  </tbody>
</table>
...
<h2 id="knowhereDISKANNsearchbeamwidthratio" class="common-anchor-header"><code translate="no">knowhere.DISKANN.search.beam_width_ratio</code><button data-href="#knowhereDISKANNsearchbeamwidthratio" class="anchor-icon" translate="no">
  ...
  <tbody>
    <tr>
      <td>        検索反復あたりの最大IOリクエスト数とCPU数の比率      </td>
      <td>4</td>
    </tr>
  </tbody>
</table>
