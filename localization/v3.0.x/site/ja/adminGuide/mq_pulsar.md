...
<p>Milvus 2.5 以降、Milvus Helm チャートおよび Milvus Operator はデフォルトで<strong>Pulsar v3</strong>をデプロイしますが、Pulsar v2 との互換性は維持されています。「<a href="/docs/ja/upgrade-pulsar-v3.md">Pulsar を v2 から v3 にアップグレードする</a>」および「<a href="/docs/ja/use-pulsar-v2.md">Pulsar v2 を引き続き使用する</a>」を参照してください。</p>
<h2 id="Deploy-a-Milvus-cluster-with-Pulsar-using-Helm" class="common-anchor-header">Helm を使用して Pulsar を搭載した Milvus クラスタをデプロイする<button data-href="#Deploy-a-Milvus-cluster-with-Pulsar-using-Helm" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<h3 id="Configure" class="common-anchor-header">設定<button data-href="#Configure" class="anchor-icon" translate="no">
    ...
    </button></h3><p>Milvusを<strong>外部の</strong>Pulsarサービスに接続するには、<code translate="no">values.yaml</code> のオーバーライドで、バンドルされたPulsarを無効化し、<code translate="no">externalPulsar</code> を有効にしてください:</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>バンドルされたPulsarを使用しており、その永続化データを削除したい場合は、PulsarのPVC（<code translate="no">my-release-pulsarv3-*</code> という名前）を削除してください:</p>
<pre><code translate="no" class="language-bash">kubectl get pvc | grep my-release-pulsarv3
kubectl delete pvc &lt;pulsar-pvc-name&gt; ...
<button class="copy-code-btn"></button></code></pre>
<h2 id="Deploy-a-Milvus-cluster-with-Pulsar-using-Milvus-Operator" class="common-anchor-header">Milvus Operator を使用して Pulsar を含む Milvus クラスターをデプロイする<button data-href="#Deploy-a-Milvus-cluster-with-Pulsar-using-Milvus-Operator" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>Milvus Operator を使用する場合は、<code translate="no">spec.dependencies.pulsar</code> の下でPulsarを設定します（Milvusクラスタでのみサポートされています）。<code translate="no">pulsar</code> は、<code translate="no">external</code> および<code translate="no">inCluster</code> をサポートしています。</p>
...
<li><strong>2.5.x から 2.6.x へのアップグレード:</strong> <strong>メッセージキューの制限事項</strong>: Milvus v3.0.2 へアップグレードする際は、現在のメッセージキューの設定を維持する必要があります。アップグレード中に異なるメッセージキューシステムへ切り替えることはサポートされていません。メッセージキューシステムの変更機能は、将来のバージョンで提供される予定です。
...
<li><a href="/docs/ja/switch-pulsar-woodpecker.md">PulsarとWoodpeckerの切り替え</a></li>
</ul>
