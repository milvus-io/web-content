...
title: パルサー
---
<h1 id="Use-Pulsar-as-the-Milvus-Message-Queue" class="common-anchor-header">MilvusのメッセージキューとしてPulsarを使用する<button data-href="#Use-Pulsar-as-the-Milvus-Message-Queue" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<thead>
<tr><th>Milvusのバージョン</th><th>サポートされるPulsarのバージョン</th><th>デフォルト</th></tr>
</thead>
<tbody>
<tr><td>2.5.x 以降</td><td>Pulsar v3（推奨）または Pulsar v2</td><td>Pulsar v3（Helm / Milvus Operator 経由）</td></tr>
<tr><td>2.4.x およびそれ以前</td><td>Pulsar v2</td><td>Pulsar v2</td></tr>
</tbody>
</table>
<p>Milvus 2.5 以降、Milvus Helm チャートおよび Milvus Operator ではデフォルトで<strong>Pulsar v3</strong>がデプロイされますが、Pulsar v2 との互換性は維持されています。「<a href="/docs/ja/upgrade-pulsar-v3.md">Pulsar を v2 から v3 にアップグレードする</a>」および「<a href="/docs/ja/use-pulsar-v2.md">Pulsar v2 を引き続き使用する</a>」を参照してください。</p>
<h2 id="Deploy-a-Milvus-cluster-with-Pulsar-using-Helm" class="common-anchor-header">Helm を使用して Pulsar を搭載した Milvus クラスターをデプロイする<button data-href="#Deploy-a-Milvus-cluster-with-Pulsar-using-Helm" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>（Woodpeckerの代わりに）バンドルされたPulsarを使用するMilvusクラスターをデプロイするには、Streaming Nodeを有効にしたHelmチャートをインストールします:</p>
...
<p>Kubernetes v1.25以降で、バンドルされたPulsarサブチャートからPodDisruptionBudget（PDB）APIに関する問題が発生した場合は、PulsarのPDBポリシーを無効にしてください：</p>
<pre><code translate="no" class="language-bash">helm install my-release zilliztech/milvus \
...
<h3 id="Configure" class="common-anchor-header">設定<button data-href="#Configure" class="anchor-icon" translate="no">
    ...
    </button></h3><p>Milvusを<strong>外部の</strong>Pulsarサービスに接続するには、<code translate="no">values.yaml</code> のオーバーライドで、バンドルされたPulsarを無効にし、<code translate="no">externalPulsar</code> を有効にします:</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>バンドルされたPulsarを使用しており、その永続化データを削除したい場合は、PulsarのPVC（<code translate="no">my-release-pulsarv3-*</code> という名前のもの）を削除してください:</p>
<pre><code translate="no" class="language-bash">kubectl get pvc | grep my-release-pulsarv3
kubectl delete pvc &lt;pulsar-pvc-name&gt; ...
<button class="copy-code-btn"></button></code></pre>
<h2 id="Deploy-a-Milvus-cluster-with-Pulsar-using-Milvus-Operator" class="common-anchor-header">Milvus Operator を使用して Pulsar を備えた Milvus クラスターをデプロイする<button data-href="#Deploy-a-Milvus-cluster-with-Pulsar-using-Milvus-Operator" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>Milvus Operator を使用する場合は、<code translate="no">spec.dependencies.pulsar</code> の下で Pulsar を設定します（Milvus クラスタでのみサポートされています）。<code translate="no">pulsar</code> では、<code translate="no">external</code> および<code translate="no">inCluster</code> がサポートされています。</p>
...
<span class="hljs-attr">spec:</span>
  ...
  <span class="hljs-attr">config:</span> {}
<button class="copy-code-btn"></button></code></pre>
<p>設定を適用します（ファイルが<code translate="no">milvuscluster.yaml</code> であると仮定します）：</p>
...
<li><strong>2.5.x から 2.6.x へのアップグレード:</strong> <strong>メッセージキューの制限事項</strong>: Milvus v3.0.1 へアップグレードする際は、現在のメッセージキューの設定を維持する必要があります。アップグレード中に異なるメッセージキューシステムへ切り替えることはサポートされていません。メッセージキューシステムの変更機能は、将来のバージョンで提供される予定です。
Pulsar を実行しており、それを継続して使用したい場合は、アップグレード中にメッセージキューを変更しないでください。</li>
<li><strong>Pulsar v2 → v3:</strong>「<a href="/docs/ja/upgrade-pulsar-v3.md">Pulsarをv2からv3へアップグレードする</a>」を参照してください。v2のまま使用を続ける場合は、「<a href="/docs/ja/use-pulsar-v2.md">Pulsar v2の継続使用</a>」を参照してください。</li>
</ul>
...
<li><a href="/docs/ja/switch-pulsar-woodpecker.md">Pulsar と Woodpecker の切り替え</a></li>
</ul>
