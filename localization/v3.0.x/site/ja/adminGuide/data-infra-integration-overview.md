...
title: データインフラストラクチャ
summary: Milvus が使用するメタデータストレージ、オブジェクトストレージ、およびメッセージキューの概要。
---
<h1 id="Data-Infrastructure" class="common-anchor-header">データインフラストラクチャ<button data-href="#Data-Infrastructure" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h1><p>Milvusは、中核となるデータインフラストラクチャとして、メタデータストレージ、オブジェクトストレージ、およびメッセージキューを利用しています。この章では、設定可能なコンポーネントについて説明します：</p>
<ul>
<li><strong><a href="/docs/ja/etcd.md">メタデータ</a></strong>— Milvus は、メタデータ（コレクションスキーマ、ノードステータス、消費チェックポイント）を etcd に格納します。</li>
<li><strong><a href="/docs/ja/object-storage.md">オブジェクトストレージ</a></strong>— Milvusは、インデックスファイルとバイナリログをMinIO、AWS S3、またはその他のS3互換／クラウドオブジェクトストレージに格納します。</li>
