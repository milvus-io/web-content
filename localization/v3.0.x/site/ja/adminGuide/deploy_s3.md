...
<h2 id="Configure-S3-with-Docker-Compose" class="common-anchor-header">Docker ComposeでS3を設定する<button data-href="#Configure-S3-with-Docker-Compose" class="anchor-icon" translate="no">
    ...
    </button></h2><h3 id="1-Configure-S3" class="common-anchor-header">1.S3の構成<button data-href="#1-Configure-S3" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p><a href="https://min.io/product/overview">MinIOは</a>S3と互換性があります。Docker ComposeでS3を設定するには、milvus/configパスの<code translate="no">milvus.yaml</code> ファイルの<code translate="no">minio</code> セクションに値を指定します。</p>
...
<h3 id="2-Refine-docker-composeyaml" class="common-anchor-header">2.docker-compose.yaml を洗練させる。<button data-href="#2-Refine-docker-composeyaml" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>また、milvusサービスの環境変数<code translate="no">MINIO_ADDRESS</code> を<code translate="no">docker-compose.yaml</code> で削除します。デフォルトでは、milvusは外部のS3ではなくローカルのminioを使用する。</p>
<h3 id="3-Run-Milvus" class="common-anchor-header">3.milvusの実行<button data-href="#3-Run-Milvus" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>以下のコマンドを実行して、S3の設定を使用するMilvusを起動する。</p>
...
<p>次の表は、YAMLファイル内のS3を設定するためのキーの一覧です。</p>
<table>
<thead>
...
<h3 id="Using-the-YAML-file" class="common-anchor-header">YAMLファイルを使う<button data-href="#Using-the-YAML-file" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<button class="copy-code-btn"></button></code></pre>
<h3 id="Using-a-command" class="common-anchor-header">コマンドの使用<button data-href="#Using-a-command" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>MilvusのインストールとS3の設定を行うには、以下のコマンドを実行します。</p>
