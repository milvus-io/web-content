...
<h2 id="Deploy-a-Milvus-cluster-with-Kafka-using-Milvus-Operator" class="common-anchor-header">Milvus Operator を使用して Kafka を含む Milvus クラスターをデプロイする<button data-href="#Deploy-a-Milvus-cluster-with-Kafka-using-Milvus-Operator" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Milvus Operatorを使用する場合は、`<code translate="no">spec.dependencies.msgStreamType: &quot;kafka&quot;</code> `を設定し、<code translate="no">spec.dependencies.kafka</code> （クラスタのみ）でKafkaを設定します。<code translate="no">kafka</code> は、<code translate="no">external</code> および<code translate="no">inCluster</code> をサポートしています。</p>
...
<li><strong>2.5.x から 2.6.x へのアップグレード:</strong> <strong>メッセージキューの制限事項</strong>: Milvus v3.0.2 へアップグレードする際は、現在のメッセージキューの選択を維持する必要があります。アップグレード中に異なるメッセージキューシステムへ切り替えることはサポートされていません。メッセージキューシステムの変更機能は、将来のバージョンで提供される予定です。
