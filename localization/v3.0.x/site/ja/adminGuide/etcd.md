...
<h2 id="Change-the-etcd-image-with-Helm" class="common-anchor-header">Helm を使用して etcd イメージを変更する<button data-href="#Change-the-etcd-image-with-Helm" class="anchor-icon" translate="no">
    ...
    </button></h2><p>etcd イメージのバージョンを固定または置き換えるには、Helm を使用してインストールまたはアップグレードする際に、<code translate="no">etcd.image.tag</code> （および必要に応じて<code translate="no">etcd.image.repository</code> ）を上書きします。</p>
