...
title: Milvus を利用したマルチモーダル RAG
---
<h1 id="Multimodal-RAG-with-Milvus" class="common-anchor-header">Milvus を利用したマルチモーダル RAG<button data-href="#Multimodal-RAG-with-Milvus" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<h3 id="Download-Data" class="common-anchor-header">データのダウンロード<button data-href="#Download-Data" class="anchor-icon" translate="no">
    ...
    </button></h3><p>次のコマンドを実行すると、サンプルデータがダウンロードされ、ローカルフォルダ「./images_folder」に以下の画像が抽出されます：</p>
<ul>
<li><strong>images</strong>:<a href="https://github.com/hyp1231/AmazonReviews2023">Amazon Reviews 2023</a>の一部で、「Appliance」、「Cell_Phones_and_Accessories」、「Electronics」の各カテゴリから約 900 枚の画像が含まれています。</li>
...
<h2 id="Load-Data" class="common-anchor-header">データの読み込み<button data-href="#Load-Data" class="anchor-icon" translate="no">
    ...
    </button></h2><p>このセクションでは、サンプル画像を対応する埋め込みデータとともにデータベースに読み込みます。</p>
<h3 id="Generate-embeddings" class="common-anchor-header">埋め込みの生成<button data-href="#Generate-embeddings" class="anchor-icon" translate="no">
    ...
    </button></h3><p>データディレクトリからすべてのJPEG画像を読み込み、エンコーダを適用して画像を埋め込みベクトルに変換します。</p>
...
<h3 id="Insert-into-Milvus" class="common-anchor-header">Milvusへの挿入<button data-href="#Insert-into-Milvus" class="anchor-icon" translate="no">
    ...
    </button></h3><p>対応するパスと埋め込みベクトルとともに、画像をMilvusコレクションに挿入します。</p>
<div class="alert note">
<p><code translate="no">MilvusClient</code> の引数については：</p>
<ul>
<li><code translate="no">uri</code> をローカルファイル（例：<code translate="no">./milvus_demo.db</code> ）として設定するのが最も便利な方法です。これにより、<a href="https://milvus.io/docs/milvus_lite.md">Milvus Liteが</a>自動的にこのファイルにすべてのデータを保存します。</li>
<li>大規模なデータをお持ちの場合は、<a href="https://milvus.io/docs/quickstart.md">Docker や Kubernetes</a> 上でより高性能な Milvus サーバーを構築できます。この設定では、<code translate="no">uri</code> の代わりにサーバーの URI（例:<code translate="no">http://localhost:19530</code> ）を使用してください。</li>
<li>Milvus向けのフルマネージドクラウドサービス<a href="https://zilliz.com/cloud">であるZilliz Cloud</a>をご利用になる場合は、Zilliz Cloudの「<a href="https://docs.zilliz.com/docs/on-zilliz-cloud-console#free-cluster-details">Public Endpoint」および「APIキー</a>」に対応する<code translate="no">uri</code> と<code translate="no">token</code> を調整してください。</li>
</ul>
</div>
...
<h3 id="Run-search" class="common-anchor-header">検索を実行<button data-href="#Run-search" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>これで、画像とテキストの指示の両方からなるクエリデータを用いた高度な画像検索を実行する準備が整いました。</p>
...
<p><strong>1. パノラマビューを作成する</strong></p>
...
<button class="copy-code-btn"></button></code></pre>
<p>クエリ画像と検索結果の画像を、インデックス付きでパノラマビューにまとめます。</p>
...
<p>結合した画像を、適切なプロンプトと共にマルチモーダルLLMサービスに送信し、検索結果の再ランク付けと説明を生成します。LLMとしてGPT-4oを利用するには、<a href="https://platform.openai.com/docs/quickstart">OpenAI APIキー</a>の準備が必要です。</p>
