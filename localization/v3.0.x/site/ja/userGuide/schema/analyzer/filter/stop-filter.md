...
<h3 id="Load-stop-words-from-a-file-resource--Milvus-30x" class="common-anchor-header">ファイルリソースからストップワードを読み込む<span class="beta-tag" style="background-color:rgb(0, 179, 255);color:white" translate="no">Compatible with Milvus 3.0.x</span><button data-href="#Load-stop-words-from-a-file-resource--Milvus-30x" class="anchor-icon" translate="no">
    ...
    </button></h3><p>大規模なカスタムストップワードリスト（言語固有のリスト、ドメイン固有の語彙、または多くのコレクションで共有したいリストなど）については、単語をファイルに保存し、そのファイルをリモートファイルリソースとして登録してから、<code translate="no">stop_words_file</code> パラメータを介してフィルターから参照します。<code translate="no">stop_words_file</code> は単独で使用することも、インラインの<code translate="no">stop_words</code> と併用することもできます。両方が設定されている場合、フィルターは2つのソースを1つのストップワードリストに統合します。</p>
<p>ファイルは、<strong>1行につき1つのストップワード</strong>が記載されたプレーンなUTF-8テキストである必要があります。例：</p>
<pre><code translate="no" class="language-plaintext">the
of
for
<button class="copy-code-btn"></button></code></pre>
<p>ファイルを、Milvusクラスタで使用するように設定されているオブジェクトストアにアップロードし、登録します:</p>
