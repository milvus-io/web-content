...
<li><p><a href="/docs/ja/array-of-structs.md">Array of Structs</a>フィールドは NULL 値をサポートしません。Array of Structs フィールドや、その内部にネストされたフィールドを NULLable としてマークすることはできません。</p></li>
...
<h2 id="Nullable-fields-and-default-values" class="common-anchor-header">NULL可能なフィールドとデフォルト値<button data-href="#Nullable-fields-and-default-values" class="anchor-icon" translate="no">
    ...
    </button></h2><p><code translate="no">nullable</code> 、<code translate="no">default_value</code> の両方がフィールドに設定されている場合、Milvusは挿入時にNULL入力やフィールド値がない場合の処理方法を以下のルールで決定します。</p>
<table>
<thead>
...
<tr><td>いいえ</td><td>いいえ</td><td>NULLまたは省略</td><td>エラーをスローする</td></tr>
