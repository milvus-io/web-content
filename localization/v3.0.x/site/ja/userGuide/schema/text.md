...
<p>フィールドが定義されると、各エンティティはそのフィールドに文字列値を格納できるようになります。<code translate="no">TEXT</code> の値は他のスカラーフィールドと同様に挿入でき、クエリや検索結果から取得するには、<code translate="no">output_fields</code> でそのフィールドを指定します。</p>
<div class="alert note">
