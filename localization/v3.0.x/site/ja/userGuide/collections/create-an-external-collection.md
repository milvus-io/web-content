...
summary: Milvusにコピーすることなく、外部コレクションからAWS S3やIcebergのデータにアクセスできます。
beta: Milvus 3.0.x
---
<h1 id="Create-an-External-Collection" class="common-anchor-header">外部コレクションの作成<span class="beta-tag" style="background-color:rgb(0, 179, 255);color:white" translate="no">Compatible with Milvus 3.0.x</span><button data-href="#Create-an-External-Collection" class="anchor-icon" translate="no">
    ...
    </button></h1><p>外部コレクションはMilvusのデータコレクションの一種であり、AWS S3やIcebergなどの外部ストレージシステムからMilvusにコピーすることなくデータにアクセスします。Milvusのクエリインタフェースとの互換性を保ちながら、データレイクのクエリレイヤーとして機能します。</p>
...
<h2 id="Limits--restrictions" class="common-anchor-header">制限と制約<button data-href="#Limits--restrictions" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Milvusは生データを保存せず、メタデータと生データのマッピングのみを保持するため、外部コレクションは読み取り専用です。つまり、Milvus側でデータを挿入、アップサート、削除、インポート、フラッシュ、コンパクト化することはできません。</p>
<p>マネージドコレクションと比較した場合、外部コレクションには以下の制限があります：</p>
<ul>
<li><p>主キーとその AutoID 属性を設定できない。</p></li>
<li><p>ダイナミックフィールドを有効にすることはできません。</p></li>
<li><p>パーティションが使用できないため、パーティション・キーとクラスタリング・キーを設定できません。</p></li>
<li><p>スキーマに関数を定義することはできません。</p></li>
<li><p>一度作成した外部コレクションのスキーマは変更できません。</p></li>
<li><p>BM25 とのテキスト一致は使用できません。</p></li>
<li><p>インデックスを作成する前に、リフレッシュ操作をトリガする必要があります。</p></li>
</ul>
<h2 id="Step-1-Create-schema" class="common-anchor-header">ステップ 1: スキーマの作成<button data-href="#Step-1-Create-schema" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>管理コレクションの作成と同様に、外部コレクションを作成する前にスキーマを作成する必要があります。ただし、スキーマはマネージド・コレクションとは若干異なります。</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>外部コレクションのスキーマを作成するには、ソースデータ URI、データ形式、認証設定を指定する必要があります。</p>
<p><details summary="Authentication Options"></p>
<p>認証設定を設定するには、以下のオプションがあります：</p>
<h3 id="Use-AWS-AKSK" class="common-anchor-header">Use AWS AK/SK<button data-href="#Use-AWS-AKSK" class="anchor-icon" translate="no">
    ...
    </button></h3><p>このオプションは、セルフホストMinIO、または作業用にAK/SKを持つシナリオに適用されます。</p>
...
<table>
   ...
   <tr>
     ...
     <td><p>文字列化された JSON 構造の外部ファイル・システム設定。</p></td>
     <td><p>--</p></td>
   </tr>
   <tr>
     <td><p><code translate="no">extfs.access_key_id</code></p></td>
     <td><p>アクセス・キー ID</p></td>
   ...
   <tr>
     ...
     <td><p>クラウド・リージョンID</p></td>
     <td><p><code translate="no">us-west-2</code></p></td>
   </tr>
   <tr>
     <td><p><code translate="no">extfs.cloud_provider</code></p></td>
     <td><p>クラウド・プロバイダーID</p></td>
     <td><p><code translate="no">aws</code></p></td>
   </tr>
   <tr>
     <td><p><code translate="no">extfs.use_ssl</code></p></td>
     <td><p>接続の確立にSSLを使うかどうか</p></td>
     <td><p><code translate="no">true</code></p></td>
   </tr>
   <tr>
     <td><p><code translate="no">extfs.use_virtual_host</code></p></td>
     <td><p>バケットへのアクセスにバーチャルホスティングを利用するかどうか。</p><p>詳細は<a href="https://docs.aws.amazon.com/AmazonS3/latest/userguide/VirtualHosting.html">こちらの</a>記事を参照。</p></td>
     <td><p><code translate="no">true</code></p></td>
   </tr>
</table>
<h3 id="Use-AWS-IAM" class="common-anchor-header">AWS IAMを使用する<button data-href="#Use-AWS-IAM" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>このオプションは、MilvusがEC2インスタンスまたはEKSクラスタ上で動作する場合に適用されます。この場合、AK/SKをハードコーディングする必要はありません。</p>
<pre><code translate="no" class="language-json"><span class="hljs-punctuation">{</span>
    <span class="hljs-attr">&quot;format&quot;</span><span class="hljs-punctuation">:</span> <span class="hljs-string">&quot;...&quot;</span><span class="hljs-punctuation">,</span>
    <span class="hljs-attr">&quot;extfs&quot;</span><span class="hljs-punctuation">:</span> <span class="hljs-punctuation">{</span>
...
<table>
   ...
   <tr>
     <td><p><code translate="no">format</code></p></td>
     <td><p>ターゲットソースデータのフォーマット。</p><p>指定可能な値は、<code translate="no">parquet</code> 、<code translate="no">vortex</code> 、<code translate="no">lance-table</code> 、および<code translate="no">iceberg-table</code></p></td>
     <td><p><code translate="no">parquet</code></p></td>
   </tr>
   <tr>
     <td><p><code translate="no">extfs</code></p></td>
     <td><p>外部ファイル・システムの設定</p></td>
     <td><p>--</p></td>
   </tr>
   <tr>
     <td><p><code translate="no">extfs.use_iam</code></p></td>
     <td><p>AWS IAM を使用するかどうか。</p><p>このオプションには<code translate="no">"true"</code> を設定します。</p></td>
     <td><p><code translate="no">true</code></p></td>
   </tr>
   <tr>
     <td><p><code translate="no">extfs.iam_endpoint</code></p></td>
     <td><p>有効な AWS STS エンドポイント。 </p><p>詳細は<a href="https://docs.aws.amazon.com/IAM/latest/UserGuide/id_credentials_temp_region-endpoints.html">こちらの</a>記事を参照。</p></td>
     <td><p><code translate="no">https:&ast;//&ast;sts.&lt;region&gt;.amazonaws.com</code></p></td>
   </tr>
   <tr>
     <td><p><code translate="no">extfs.region</code></p></td>
     <td><p>クラウドリージョンID</p></td>
     <td><p><code translate="no">us-west-2</code></p></td>
   </tr>
   <tr>
     <td><p><code translate="no">extfs.cloud_provider</code></p></td>
     <td><p>クラウドプロバイダーID</p></td>
     <td><p><code translate="no">aws</code></p></td>
   </tr>
   <tr>
     <td><p><code translate="no">extfs.use_ssl</code></p></td>
...
<h3 id="Use-Milvus-global-credentials" class="common-anchor-header">Milvusグローバル認証情報を使用する<button data-href="#Use-Milvus-global-credentials" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Milvusバケットに外部データを保存し、<code translate="no">milvus.yaml</code> で指定したグローバルMinIO設定を直接使用してデータにアクセスできる場合に適用します。</p>
<pre><code translate="no" class="language-json"><span class="hljs-punctuation">{</span>
    <span class="hljs-attr">&quot;format&quot;</span><span class="hljs-punctuation">:</span> <span class="hljs-string">&quot;...&quot;</span><span class="hljs-punctuation">,</span>
    <span class="hljs-attr">&quot;extfs&quot;</span><span class="hljs-punctuation">:</span> <span class="hljs-punctuation">{</span>
...
<span class="hljs-punctuation">}</span>
<button class="copy-code-btn"></button></code></pre>
<h3 id="Use-IAM-Role-ARN" class="common-anchor-header">IAMロールARNを使用する<button data-href="#Use-IAM-Role-ARN" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>このオプションは、組織がMilvusクラスタとターゲットデータファイルを保持するバケットを管理するために異なるAWSアカウントを使用する場合に適用されます。</p>
<p>この場合、バケットオーナーは以下のような IAM ロールを作成する必要があります。</p>
<ul>
...
<span class="hljs-punctuation">}</span>
<button class="copy-code-btn"></button></code></pre>
<p>そして、IAMロールに関連付けられた信頼ポリシーは、誰がIAMロールを引き受けることを許可されるかを定義します。</p>
<pre><code translate="no" class="language-json"><span class="hljs-punctuation">{</span>
  <span class="hljs-attr">&quot;Version&quot;</span><span class="hljs-punctuation">:</span> <span class="hljs-string">&quot;2012-10-17&quot;</span><span class="hljs-punctuation">,</span>
  <span class="hljs-attr">&quot;Statement&quot;</span><span class="hljs-punctuation">:</span> <span class="hljs-punctuation">[</span>
    <span class="hljs-punctuation">{</span>
...
<span class="hljs-punctuation">}</span>
<button class="copy-code-btn"></button></code></pre>
<p>IAMロールのARNと外部IDを取得したら、<code translate="no">external_spec</code> パラメータを以下のように設定します：</p>
<pre><code translate="no" class="language-json"><span class="hljs-punctuation">{</span>
    <span class="hljs-attr">&quot;format&quot;</span><span class="hljs-punctuation">:</span> <span class="hljs-string">&quot;...&quot;</span><span class="hljs-punctuation">,</span>
    <span class="hljs-attr">&quot;extfs&quot;</span><span class="hljs-punctuation">:</span> <span class="hljs-punctuation">{</span>
        <span class="hljs-attr">&quot;cloud_provider&quot;</span><span class="hljs-punctuation">:</span> <span class="hljs-string">&quot;aws&quot;</span><span class="hljs-punctuation">,</span>
...
<table>
   ...
   <tr>
     <td><p><code translate="no">format</code></p></td>
     <td><p>ターゲット・ソース・データの形式。</p><p>設定可能な値は<code translate="no">parquet</code> 、<code translate="no">vortex</code> 、<code translate="no">lance-table</code> 、および<code translate="no">iceberg-table</code></p></td>
     <td><p><code translate="no">parquet</code></p></td>
   </tr>
   <tr>
     <td><p><code translate="no">extfs</code></p></td>
...
<h2 id="Step-2-Add-fields" class="common-anchor-header">ステップ 2: フィールドの追加<button data-href="#Step-2-Add-fields" class="anchor-icon" translate="no">
    ...
    </button></h2><p>スキーマの準備ができたら、以下のようにフィールドを追加していく：</p>
<div class="multipleCode">
   <a href="#python">Python</a> <a href="#java">Java</a> <a href="#go">Go</a> <a href="#javascript">NodeJS</a> <a href="#bash">cURL</a></div>
...
<pre><code translate="no" class="language-bash"><span class="hljs-comment"># restful</span>
<button class="copy-code-btn"></button></code></pre>
<h2 id="Step-3-Create-a-collection" class="common-anchor-header">ステップ3：コレクションの作成<button data-href="#Step-3-Create-a-collection" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>スキーマにすべてのフィールドを追加したら、コレクションを作成します。</p>
<div class="multipleCode">
   <a href="#python">Python</a> <a href="#java">Java</a> <a href="#go">Go</a> <a href="#javascript">NodeJS</a> <a href="#bash">cURL</a></div>
...
<pre><code translate="no" class="language-bash"><span class="hljs-comment"># restful</span>
<button class="copy-code-btn"></button></code></pre>
<h2 id="Step-4-Refresh-data" class="common-anchor-header">ステップ 4: データの更新<button data-href="#Step-4-Refresh-data" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>コレクションの準備ができたら、データからmilvusにメタデータを同期するためにリフレッシュを実行する必要があります。</p>
<div class="multipleCode">
   <a href="#python">Python</a> <a href="#java">Java</a> <a href="#go">Go</a> <a href="#javascript">NodeJS</a> <a href="#bash">cURL</a></div>
...
<li><p>挿入を伴わない、すべてのアクティブなメタデータの削除を必要とする更新は拒否されます。</p></li>
</ul>
</div>
<h2 id="Follow-ups" class="common-anchor-header">フォローアップ<button data-href="#Follow-ups" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>外部コレクションで更新操作を実行し、マニフェスト・ファイルが利用可能になると、管理対象コレクションと同様に、外部コレクションでインデックスを作成し、コレクションをロード/リリースし、類似検索およびクエリを実行できます。</p>
