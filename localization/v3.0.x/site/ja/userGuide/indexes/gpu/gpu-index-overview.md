...
<p>デフォルトの<code translate="no">initMemSize</code> は通常Milvus起動時のGPUメモリの半分で、<code translate="no">maxMemSize</code> はGPUメモリ全体がデフォルトです。GPU メモリプールサイズは、最初は<code translate="no">initMemSize</code> に設定され、必要に応じて<code translate="no">maxMemSize</code> に自動的に拡張されます。</p>
