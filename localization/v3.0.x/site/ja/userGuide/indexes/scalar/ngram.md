...
<li><p><strong>各グラムを検索し、共通部分を抽出</strong>：Milvusは、クエリの各グラムを逆引きインデックスで検索し、その結果得られたドキュメントIDリストの共通部分を抽出することで、候補ドキュメントの小さなセットを見つけ出します。これらの候補には、クエリに含まれるすべてのグラムが含まれています。</p></li>
