...
title: 検索イテレータ
summary: >-
  ANN検索では、1回のクエリで検索結果として取得できるエンティティ数に上限があり、基本的なANN検索を単に利用しただけでは、大規模な検索の要件を満たせない場合があります。
  topKが16,384を超えるANN
  Searchリクエストについては、SearchIteratorの使用を検討することをお勧めします。このセクションでは、SearchIteratorの使用方法と関連する考慮事項について紹介します。
---
<h1 id="Search-Iterator" class="common-anchor-header">検索イテレータ<button data-href="#Search-Iterator" class="anchor-icon" translate="no">
    ...
    </button></h1><p>ANN検索では、1回のクエリで検索結果として取得できるエンティティ数に上限があり、基本的なANN検索を単に利用しただけでは、大規模な検索の要件を満たせない場合があります。 topKが16,384を超えるANN検索リクエストについては、SearchIteratorの使用を検討することをお勧めします。このセクションでは、SearchIteratorの使用方法と関連する留意点について説明します。</p>
<h2 id="Overview" class="common-anchor-header">概要<button data-href="#Overview" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Search リクエストは検索結果を返すのに対し、SearchIterator はイテレータを返します。このイテレータの<strong>next()</strong>メソッドを呼び出すことで、検索結果を取得できます。</p>
...
<h2 id="Create-SearchIterator" class="common-anchor-header">SearchIteratorの作成<button data-href="#Create-SearchIterator" class="anchor-icon" translate="no">
    ...
    </button></h2><p>以下のコードスニペットは、SearchIterator の作成方法を示しています。</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>上記の例では、1回の検索で返すエンティティの数（<strong>batch_size/batchSize</strong>）を50に、返すエンティティの総数（<strong>topK</strong>）を20,000に設定しています。</p>
<h2 id="Use-SearchIterator" class="common-anchor-header">SearchIteratorの使用<button data-href="#Use-SearchIterator" class="anchor-icon" translate="no">
    ...
    </button></h2><p>SearchIteratorの準備が整ったら、next() メソッドを呼び出すことで、検索結果をページ単位で取得できます。</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#go">   Go</a>
