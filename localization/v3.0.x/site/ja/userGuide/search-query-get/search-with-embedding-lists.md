...
<h3 id="Late-interaction" class="common-anchor-header">遅い相互作用<button data-href="#Late-interaction" class="anchor-icon" translate="no">
    ...
    </button></h3><p>ベクトル化が完了すると、クエリー埋め込みリストと各文書埋め込みリストがトークンごとに比較され、最終的な類似度スコアが決定される。</p>
<p>
...
<button class="copy-code-btn"></button></code></pre>
<p>このコードでは、グループ化された段落を文書として格納し、<code translate="no">data</code> リストに含めます。各文書は段落のリストである<code translate="no">paragraphs</code> ・キーを持っています。各段落オブジェクトは<code translate="no">text</code> ・キーと<code translate="no">emb</code> ・キーを持っています。</p>
