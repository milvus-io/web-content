...
summary: >-
  このチュートリアルでは、MilvusのStructArrayベクトルサブフィールドに対するEmbeddingList検索を用いて、ColBERTスタイルおよびColPaliスタイルの検索システムを構築する方法について説明します。クエリと保存データの両方がベクトルのリストとして表現されており、MAX_SIM*メトリクスを用いたエンティティレベルのレイトインタラクション検索を行いたい場合に、この方法をご利用ください。
---
...
<h2 id="Overview" class="common-anchor-header">概要<button data-href="#Overview" class="anchor-icon" translate="no">
    ...
    </button></h2><p>テキスト検索システムを構築する際、特に全文の埋め込みが意味的な特異性を希薄化したり、モデルの入力制限を超えたりする可能性がある長いドキュメントについては、精度と正確性を確保するために、ドキュメントをチャンクに分割し、各チャンクをその埋め込み情報とともにベクトルデータベース内のエンティティとして保存する必要がある場合があります。</p>
...
<h3 id="ColPali-extension" class="common-anchor-header">ColPali 拡張機能<button data-href="#ColPali-extension" class="anchor-icon" translate="no">
    ...
    </button></h3><p>ColBERT に基づいて、ColPali (arXiv:<a href="https://arxiv.org/abs/2407.01449?spm=a2ty_o01.29997173.0.0.31c4c9217HFv28&amp;file=2407.01449">2407.01449</a>) は、ビジョン・言語モデル（VLM）を活用した、視覚的に豊かな文書検索への新しいアプローチを提案しています。 データ取り込みの際、各ドキュメントのページは高解像度の画像としてレンダリングされ、トークン化されるのではなく、パッチに分割されます。たとえば、448 x 448 ピクセルのドキュメントページ画像からは、14 x 14 ピクセルずつ、1,024 個のパッチが生成されます。</p>
...
<h3 id="Step-6-Search-within-the-Cohere-dataset" class="common-anchor-header">ステップ 6: Cohere データセット内を検索する<button data-href="#Step-6-Search-within-the-Cohere-dataset" class="anchor-icon" translate="no">
    ...
    </button></h3><p>ColBERTの設計に従い、クエリテキストはトークン化され、EmbeddingListに埋め込まれる必要があります。このステップでは、CohereがWikipediaデータセットの段落に対して埋め込みを生成するために使用したのと同じモデルを使用します。</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>コード内では、クエリテキストが<code translate="no">query_inputs</code> でトークン化され、floatベクトルのリストに埋め込まれます。その後、MilvusのEmbeddingListを使用して、次のように類似度検索を実行できます。</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>上記のスクリプトを実行すると、データセットがローカルに存在しない場合はダウンロードされます。データセット内の各レコードは、財務報告書の1ページです。以下の表に、このデータセットの構造を示します。</p>
<table>
<thead>
<tr><th>列名</th><th>説明</th></tr>
</thead>
...
<button class="copy-code-btn"></button></code></pre>
<div class="alert note">
<p>財務レポートの挿入には時間がかかる場合があります。各ページには1,000以上のパッチベクトルが含まれる可能性があり、各ベクトルは<code translate="no">patches</code> のStructArrayフィールド内に格納されます。大規模なデータセットの場合は、<code translate="no">data</code> を小さなバッチに分割し、1回に1バッチずつ挿入してください。</p>
</div>
