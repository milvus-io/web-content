...
<button class="copy-code-btn"></button></code></pre>
<p>関数レベルの `<code translate="no">credential</code> ` パラメータは、プロバイダレベルのラベルを上書きできます。その値は、トークンそのものではなく、<code translate="no">milvus.yaml</code> で定義された認証情報のラベルでなければなりません。</p>
