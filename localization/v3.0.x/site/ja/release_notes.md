...
<p>Milvus v3.0-betaは、Milvusのベクトルデータベースからセマンティックネイティブなレイクエンジンへの移行を開始します。Milvusカーネルは、オープンレイクフォーマットのデータを直接操作できるようになり、Milvusのコア機能は、検索、スキーマ、ライフサイクル、言語、操作にわたって拡張された。</p>
<p>外部コレクションとスナップショットは、レイク側で追加された主要な機能である。同じカーネルは、Milvus 3.0をベースに構築されたセマンティック・ネイティブなデータプラットフォームであるZilliz Lakebaseにも搭載されている。</p>
<h3 id="Key-Features" class="common-anchor-header">主な機能<button data-href="#Key-Features" class="anchor-icon" translate="no">
    ...
    </button></h3><h4 id="External-Collection" class="common-anchor-header">外部コレクション</h4><p>一般的なAIデータパイプラインでは、何テラバイトものエンベッディングとメタデータが、すでにParquet、Lance、Icebergテーブルとしてオブジェクトストレージ上に置かれている。そのデータをMilvusにコピーすると、ストレージコストが2倍になり、同期を維持しなければならないETLパイプラインが追加され、データガバナンスが顧客から遠ざかります。</p>
...
<h4 id="External-Backfill" class="common-anchor-header">外部バックフィル</h4><p>既存のコレクションでv1エンベッディングからv2エンベッディングに移行するなど、エンベッディング・モデルをアップグレードする場合、以前はゼロから再構築する必要がありました。そのため、サービスのダウンタイムやアプリケーション側の二重書き込みロジックを余儀なくされていました。</p>
<p>Milvus3.0は、ホットワークフローとしてアップグレードをサポートします。新しいベクターフィールドを<code translate="no">AddCollectionField</code> で追加し、スナップショットを使用して一貫性のある開始点をフリーズし、スナップショットに対してオフラインで埋め込みジョブを実行し、通常の取り込みパスで値を書き戻すことができます。新しいフィールドがオンラインでインデックス化された後、アプリケーションはダウンタイムなしで切り替えることができます。</p>
<h4 id="Query--Search-Order-By" class="common-anchor-header">クエリ/検索順序</h4><p>Milvusカーネルにソートがプッシュダウンされ、<code translate="no">ASC</code> /<code translate="no">DESC</code> 、フィールド毎に設定可能です。これは一般的な生産上のギャップを埋めるものです：最も類似したアイテムが最も安く、最も新しく、最も人気があるわけではない場合、距離によるトップKだけではビジネスニーズにマッチしないことがよくあります。</p>
<p>アプリケーションは、複合ランキングを表現するために、結果をオーバーフェッチしたり、クライアント上で再ソートしたりする必要がなくなります。</p>
<p>詳細については、<a href="/docs/ja/single-vector-search.md#Sort-Search-Results-by-Scalar-Fields--Milvus-30x">Sort Search Results by Scalar Fields</a>および<a href="/docs/ja/get-and-scalar-query.md#Sort-Query-Results--Milvus-30x">Sort Query Resultsを</a>参照してください。</p>
<h4 id="Null-Vector" class="common-anchor-header">Null Vector</h4><p>エンベッディングは多くの場合非同期で生成されるため、エンティティがベクトルより先に到着することがあります。マルチモーダルデータには、キャプションのないビデオや画像のない製品など、自然なギャップもあります。以前のバージョンでは、ベクターの準備ができるまで書き込みを遅らせるか、プレースホルダーベクターで埋めるかのどちらかしかありませんでした。</p>
