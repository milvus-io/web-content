...
title: ExaとmilvusによるデュアルソースRAGエージェントの構築
---
...
<h1 id="Building-a-Dual-Source-RAG-Agent-with-Exa-and-Milvus" class="common-anchor-header">ExaとmilvusによるデュアルソースRAGエージェントの構築<button data-href="#Building-a-Dual-Source-RAG-Agent-with-Exa-and-Milvus" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<p><a href="https://dashboard.exa.ai/api-keys">Exaと</a> <a href="https://platform.openai.com/api-keys">OpenAIの</a>APIキーが必要です。環境変数として設定してください：</p>
<pre><code translate="no" class="language-shell">import os

os.environ[&quot;EXA_API_KEY&quot;] = &quot;***********&quot;
os.environ[&quot;OPENAI_API_KEY&quot;] = &quot;sk-***********&quot;
<button class="copy-code-btn"></button></code></pre>
<h2 id="Initialize-Clients" class="common-anchor-header">クライアントの初期化<button data-href="#Initialize-Clients" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<p>エンベッディングを生成するヘルパー関数を定義します。これはインデックス作成とクエリのためにノートブック全体で再利用する：</p>
...
<button class="copy-code-btn"></button></code></pre>
<h2 id="Build-the-Private-Knowledge-Base-Milvus" class="common-anchor-header">プライベート知識ベース（milvus）の構築<button data-href="#Build-the-Private-Knowledge-Base-Milvus" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>製品仕様書、ポリシー、業績報告書、APIドキュメントなど、一般公開されない社内ドキュメントのセットをシミュレートします。実際のシナリオでは、これらは社内のWiki、データベース、文書管理システムから来るかもしれません。</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>Milvusコレクションを明示的なスキーマで作成し、ドキュメントを埋め込み、挿入します：</p>
...
<p>簡単なテストクエリで検索が機能するか確認してみましょう：</p>
...
<h2 id="Explore-Exa-Search-Capabilities" class="common-anchor-header">Exaの検索機能を調べる<button data-href="#Explore-Exa-Search-Capabilities" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>エージェントを構築する前に、Exaの検索機能を調べてみよう。Exa は異なるシナリオに役立つ複数の検索モードをサポートしています。</p>
<p>コンテンツ抽出による<strong>セマンティック検索</strong>- Exa はリンクだけでなく、記事本文、主要なハイライト、AI が生成した要約を一度のリクエストで返すことができます：</p>
...
<p><strong>カテゴリーベースのフィルタリング</strong>- 結果を<code translate="no">&quot;research paper&quot;</code> 、<code translate="no">&quot;news&quot;</code> 、<code translate="no">&quot;company&quot;</code> 、<code translate="no">&quot;tweet&quot;</code> などの特定のコンテンツタイプに制限することができます。 これは、高品質のソースが必要で、ノイズを避けたい場合に便利です：</p>
...
<p><strong>類似記事の検索</strong>- URL を指定すると、類似した内容の他の記事を検索することができます。これは、良い出発点から研究を広げるのに役立ちます：</p>
...
<h2 id="Define-the-Agent-Tools" class="common-anchor-header">エージェントツールの定義<button data-href="#Define-the-Agent-Tools" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>ここで、エージェントが使用する2つのツール機能を定義します。プライベートKBツールはベクトル類似度を用いてMilvusを検索し、ウェブツールはExaを介してパブリックインターネットを検索します：</p>
...
<button class="copy-code-btn"></button></code></pre>
<h2 id="Build-the-Agent" class="common-anchor-header">エージェントの構築<button data-href="#Build-the-Agent" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>エージェントはOpenAIの<a href="https://platform.openai.com/docs/guides/function-calling">関数呼び出しを使って</a>、どのツールを呼び出すかを決定する。LLMはユーザのクエリを受け取り、どのツールを呼び出すかを決定し（もしあれば）、実行し、そして取得したコンテキストから最終的な答えを合成する。</p>
...
<button class="copy-code-btn"></button></code></pre>
<h2 id="Demo" class="common-anchor-header">デモ<button data-href="#Demo" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>それでは、異なるルーティング動作を示す3つのシナリオでエージェントをテストしてみましょう。</p>
<h3 id="Scenario-A-Internal-question-routes-to-Milvus" class="common-anchor-header">シナリオA: 内部的な質問(Milvusへのルーティング)<button data-href="#Scenario-A-Internal-question-routes-to-Milvus" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>内部ポリシーについて質問する - エージェントは<code translate="no">search_private_kb</code> を呼び出し、我々のプライベートドキュメントから答えを取得する：</p>
...
<h3 id="Scenario-B-External-question-routes-to-Exa" class="common-anchor-header">シナリオB: 外部からの質問(Exaへのルート)<button data-href="#Scenario-B-External-question-routes-to-Exa" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>外部のトレンドについて質問する - エージェントは<code translate="no">search_web</code> 、パブリックインターネットから最新情報を取得する：</p>
...
<h3 id="Scenario-C-Hybrid-question-routes-to-both" class="common-anchor-header">シナリオC: ハイブリッド質問(両方へのルート)<button data-href="#Scenario-C-Hybrid-question-routes-to-both" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>内部スペックと外部ベンチマークの両方を必要とする質問をする - エージェントは両方のツールを呼び出し、比較を合成する：</p>
...
<h2 id="Cleanup" class="common-anchor-header">クリーンアップ<button data-href="#Cleanup" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>終わったら、リソースを解放するためにコレクションを削除する。</p>
...
<p>このパターンは、AIアシスタントが社内の機密文書とリアルタイムの外部情報の両方にアクセスする必要がある企業ユースケースに適用可能です。</p>
