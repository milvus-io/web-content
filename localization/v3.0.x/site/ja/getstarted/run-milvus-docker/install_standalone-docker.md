...
<li><strong>Woodpecker MQ（デフォルト）</strong>：この Docker デプロイメントでは、メッセージキューとして Woodpecker を実行し、WAL バックエンドとして<strong>ローカルファイルシステム</strong>を使用するため、外部のメッセージキューサービスは不要です。<a href="/docs/ja/woodpecker.md">Woodpecker を</a>参照してください。</li>
...
<h2 id="Optional-dependencies" class="common-anchor-header">オプションの依存関係<button data-href="#Optional-dependencies" class="anchor-icon" translate="no">
    ...
    </button></h2><p>デフォルトでは、このデプロイメントはメッセージキューとして<strong>Woodpecker</strong>（ローカルファイルシステム WAL）、メタデータ用に<strong>組み込みの etcd</strong>を実行しており、他にインストールする必要はありません。別のメッセージキューを使用する場合や、外部のオブジェクトストレージ／メタデータに接続する場合は、以下を参照してください：</p>
<ul>
