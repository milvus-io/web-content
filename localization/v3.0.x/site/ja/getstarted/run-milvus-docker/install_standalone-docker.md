...
<p><strong>v3.0.2の新機能：</strong></p>
<ul>
<li><strong>ストリーミングノード</strong>：データ処理機能の強化</li>
<li><strong>Woodpecker MQ（デフォルト）</strong>：このDockerデプロイメントでは、メッセージキューとしてWoodpeckerを実行し、WALバックエンドとして<strong>ローカルファイルシステム</strong>を使用するため、外部のメッセージキューサービスは不要です。<a href="/docs/ja/woodpecker.md">Woodpeckerを</a>参照してください。</li>
...
<p>インストールスクリプトの実行後：</p>
<ul>
<li>「milvus-standalone」という名前の Docker コンテナがポート<strong>19530</strong> で起動されました。</li>
...
<p><code translate="no">http://127.0.0.1:9091/webui/</code> から Milvus WebUI にアクセスして、お使いの Milvus インスタンスについて詳しく確認できます。詳細については、<a href="/docs/ja/milvus-webui.md">Milvus WebUI</a> を参照してください。</p>
...
<h2 id="Optional-dependencies" class="common-anchor-header">オプションの依存関係<button data-href="#Optional-dependencies" class="anchor-icon" translate="no">
    ...
    </button></h2><p>デフォルトでは、このデプロイメントはメッセージキューとして<strong>Woodpecker</strong>（ローカルファイルシステム WAL）、メタデータ用に<strong>組み込みの etcd</strong>を実行します。これ以外にインストールする必要はありません。別のメッセージキューを使用する場合、または外部のオブジェクトストレージ／メタデータに接続する場合は、以下を参照してください：</p>
<ul>
...
<li><p><a href="/docs/ja/scaleout.md">Milvusクラスタをスケーリングする</a>。</p></li>
