...
<h2 id="Optional-dependencies" class="common-anchor-header">オプションの依存関係<button data-href="#Optional-dependencies" class="anchor-icon" translate="no">
    ...
    </button></h2><p>デフォルトでは、このデプロイメントはメッセージキューとして<strong>Woodpecker</strong>（ローカルファイルシステム WAL）、メタデータ用に<strong>組み込みの etcd</strong>を実行しており、他にインストールする必要はありません。別のメッセージキューを使用する場合、または外部のオブジェクトストレージ／メタデータに接続する場合は、以下を参照してください：</p>
<ul>
...
<div class="alert note">
<p>Storage V3はデフォルトで無効になっています。Storage V3に依存する機能を使用する前に、有効にしてください。要件や互換性に関する考慮事項については、「<a href="/docs/ja/storage-v3.md">Storage V3</a>」を参照してください。</p>
</div>
...
<ul>
<li><p>「<a href="/docs/ja/quickstart.md">クイックスタート</a>」を確認して、Milvusの機能を確認してください。</p></li>
