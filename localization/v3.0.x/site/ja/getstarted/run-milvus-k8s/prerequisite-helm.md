...
<p>Kubernetes クラスタをローカルで実行する場合は、minikube が必要です。minikube には Docker が依存関係として必要です。Helm を使用して Milvus をインストールする前に、Docker をインストールしてください。詳細については、<a href="https://docs.docker.com/get-docker">「Docker の入手方法</a>」を参照してください。</p>
<table>
<thead>
...
<tr><td>Woodpecker</td><td>Milvus にバンドル（サービスモード：<code translate="no">v</code> 以降）</td><td>デフォルトのメッセージキュー。分散デプロイメントの場合、Woodpecker は専用<strong>サービス</strong>として実行できます。<code translate="no">--set woodpecker.image.tag</code> を使用してバージョンを固定してください。サービスモードは、Woodpecker<code translate="no">v</code> 以降でサポートされています。</td></tr>
...
<ol start="2">
<li>minikube を使用して K8s クラスターを起動する</li>
</ol>
