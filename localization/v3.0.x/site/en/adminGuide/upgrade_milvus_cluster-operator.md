...
<h1 id="Upgrade-Milvus-Cluster-with-Milvus-Operator" class="common-anchor-header">Upgrade Milvus Cluster with Milvus Operator<button data-href="#Upgrade-Milvus-Cluster-with-Milvus-Operator" class="anchor-icon" translate="no">
    ...
    </button></h1><p>This guide describes how to upgrade a Milvus 2.6.x cluster to v3.0.0 with Milvus Operator.</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>Verify that the Milvus CR reports <code translate="no">Healthy</code>, all Milvus components use <code translate="no">milvusdb/milvus:v3.0.0</code>, no IndexNode Pod is running, and the existing collections remain queryable and searchable. Complete these checks before you enable any v3.0.0-specific feature.</p>
