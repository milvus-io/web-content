...
<h1 id="Upgrade-Milvus-Standalone-with-Milvus-Operator" class="common-anchor-header">Upgrade Milvus Standalone with Milvus Operator<button data-href="#Upgrade-Milvus-Standalone-with-Milvus-Operator" class="anchor-icon" translate="no">
    ...
    </button></h1><p>This guide describes how to upgrade a Milvus 2.6.x standalone deployment to v3.0.0 with Milvus Operator.</p>
<div class="alert note">
<p>This procedure has been validated from Milvus 2.6.20 to Milvus v3.0.0 with Milvus Operator 1.3.0, Woodpecker, in-cluster etcd, and in-cluster MinIO. If you use another Milvus 2.6.x patch release, Operator version, message queue, or dependency configuration, validate the upgrade in a non-production environment first.</p>
</div>
...
<button class="copy-code-btn"></button></code></pre>
<p>Verify that the Milvus CR reports <code translate="no">Healthy</code>, the current image is <code translate="no">milvusdb/milvus:v3.0.0</code>, and the existing collections remain queryable and searchable. Complete these checks before you enable any v3.0.0-specific feature.</p>
