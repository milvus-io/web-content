...
<tr><td>Woodpecker</td><td>Bundled with Milvus (service mode: <code translate="no">v</code>+)</td><td>Default message queue. For distributed deployments, Woodpecker can run as a dedicated <strong>service</strong>; pin its version with <code translate="no">--set woodpecker.image.tag</code>. Service mode is supported from Woodpecker <code translate="no">v</code> onward.</td></tr>
