...
summary: >-
  Questo tutorial mostra come costruire un agente che cerca sia nel web pubblico
  (tramite Exa) sia in una base di conoscenza privata (tramite Milvus), per poi
  sintetizzare una risposta unificata. L'agente utilizza le chiamate di funzione
  di OpenAI per decidere automaticamente quale fonte interrogare in base alla
  domanda dell'utente.
title: Costruire un agente RAG a doppia fonte con Exa e Milvus
---
...
<h1 id="Building-a-Dual-Source-RAG-Agent-with-Exa-and-Milvus" class="common-anchor-header">Costruire un agente RAG a doppia fonte con Exa e Milvus<button data-href="#Building-a-Dual-Source-RAG-Agent-with-Exa-and-Milvus" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h1><p>Questo tutorial mostra come costruire un agente che cerca sia nel <strong>web pubblico</strong> (tramite <a href="https://exa.ai/">Exa</a>) sia in <strong>una base di conoscenza privata</strong> (tramite <a href="https://milvus.io/">Milvus</a>), per poi sintetizzare una risposta unificata. L'agente utilizza le chiamate di funzione di OpenAI per decidere automaticamente quale fonte interrogare in base alla domanda dell'utente.</p>
<p><a href="https://exa.ai/">Exa</a> è un'API di ricerca progettata per le applicazioni di intelligenza artificiale, che è orgogliosamente alimentata da <a href="https://zilliz.com/cloud">Zilliz Cloud</a> (completamente gestita da Milvus). A differenza dei tradizionali motori di ricerca basati su parole chiave, Exa supporta la ricerca semantica (neurale): l'utente descrive ciò che desidera in linguaggio naturale e il motore capisce il suo intento. Fornisce inoltre l'estrazione dei contenuti, l'evidenziazione e il filtraggio per categorie. <a href="https://milvus.io/">Milvus</a> è un database vettoriale open-source costruito per la ricerca di similarità scalabile. Combinandoli con un agente LLM, è possibile costruire un sistema che recupera sia i dati proprietari interni sia le informazioni web aggiornate in un unico flusso di lavoro.</p>
...
<p>Sono necessarie le chiavi API di <a href="https://dashboard.exa.ai/api-keys">Exa</a> e <a href="https://platform.openai.com/api-keys">OpenAI</a>. Impostarle come variabili d'ambiente:</p>
<pre><code translate="no" class="language-shell">import os

os.environ[&quot;EXA_API_KEY&quot;] = &quot;***********&quot;
os.environ[&quot;OPENAI_API_KEY&quot;] = &quot;sk-***********&quot;
<button class="copy-code-btn"></button></code></pre>
<h2 id="Initialize-Clients" class="common-anchor-header">Inizializzare i client<button data-href="#Initialize-Clients" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>Configurare i client Exa, OpenAI e Milvus. Utilizziamo il modello <code translate="no">text-embedding-3-small</code> di OpenAI per generare embeddings vettoriali e Milvus Lite per l'archiviazione locale dei vettori, senza alcuna configurazione dell'infrastruttura.</p>
...
<p>Definire una funzione di aiuto per generare gli embeddings. La riutilizzeremo in tutto il notebook sia per l'indicizzazione che per l'interrogazione:</p>
...
<button class="copy-code-btn"></button></code></pre>
<h2 id="Build-the-Private-Knowledge-Base-Milvus" class="common-anchor-header">Costruire la base di conoscenza privata (Milvus)<button data-href="#Build-the-Private-Knowledge-Base-Milvus" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>Simuliamo un insieme di documenti interni all'azienda - specifiche di prodotto, politiche, rapporti sui guadagni e documenti API - che non appaiono sul web pubblico. In uno scenario reale, questi documenti potrebbero provenire da wiki interni, database o sistemi di gestione dei documenti.</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>Creiamo la collezione Milvus con uno schema esplicito, incorporiamo i documenti e li inseriamo:</p>
...
<p>Verifichiamo che il recupero funzioni con una rapida query di prova:</p>
...
<h2 id="Explore-Exa-Search-Capabilities" class="common-anchor-header">Esplorare le capacità di ricerca di Exa<button data-href="#Explore-Exa-Search-Capabilities" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>Prima di costruire l'agente, esploriamo le funzionalità di ricerca di Exa. Exa supporta diverse modalità di ricerca, utili per diversi scenari.</p>
<p><strong>Ricerca semantica</strong> con estrazione di contenuti - Exa può restituire non solo i link, ma anche il testo dell'articolo, i punti salienti e i riassunti generati dall'intelligenza artificiale in un'unica richiesta:</p>
...
<p><strong>Filtro basato sulla categoria</strong> - è possibile limitare i risultati a tipi di contenuto specifici, come <code translate="no">&quot;research paper&quot;</code>, <code translate="no">&quot;news&quot;</code>, <code translate="no">&quot;company&quot;</code> o <code translate="no">&quot;tweet&quot;</code>. Questo è utile quando si vogliono fonti di alta qualità e si vuole evitare il rumore:</p>
...
<p><strong>Trova articoli simili</strong> - dato un URL, Exa può trovare altri articoli con contenuti simili. Questo è utile per ampliare la ricerca partendo da un buon punto di partenza:</p>
...
<h2 id="Define-the-Agent-Tools" class="common-anchor-header">Definire gli strumenti dell'agente<button data-href="#Define-the-Agent-Tools" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>Ora definiamo le due funzioni degli strumenti che l'agente utilizzerà. Lo strumento KB privato cerca in Milvus utilizzando la similarità vettoriale, mentre lo strumento web cerca in Internet pubblica tramite Exa:</p>
...
<button class="copy-code-btn"></button></code></pre>
<h2 id="Build-the-Agent" class="common-anchor-header">Costruire l'agente<button data-href="#Build-the-Agent" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>L'agente utilizza la <a href="https://platform.openai.com/docs/guides/function-calling">chiamata di funzione</a> di OpenAI per decidere quale strumento invocare. Segue un semplice ciclo: l'LLM riceve la domanda dell'utente, decide quali strumenti chiamare (se ce ne sono), li esegue e quindi sintetizza una risposta finale dal contesto recuperato.</p>
...
<h2 id="Demo" class="common-anchor-header">Demo<button data-href="#Demo" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Ora testiamo l'agente con tre scenari che dimostrano diversi comportamenti di routing.</p>
<h3 id="Scenario-A-Internal-question-routes-to-Milvus" class="common-anchor-header">Scenario A: Domanda interna (instradamento verso Milvus)<button data-href="#Scenario-A-Internal-question-routes-to-Milvus" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Domanda su una politica interna - l'agente deve chiamare <code translate="no">search_private_kb</code> e recuperare la risposta dai nostri documenti privati:</p>
...
<h3 id="Scenario-B-External-question-routes-to-Exa" class="common-anchor-header">Scenario B: Domanda esterna (indirizzata a Exa)<button data-href="#Scenario-B-External-question-routes-to-Exa" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Domanda sulle tendenze esterne: l'agente deve chiamare <code translate="no">search_web</code> per ottenere informazioni aggiornate dalla rete Internet pubblica:</p>
...
<h3 id="Scenario-C-Hybrid-question-routes-to-both" class="common-anchor-header">Scenario C: Domanda ibrida (indirizzata a entrambi)<button data-href="#Scenario-C-Hybrid-question-routes-to-both" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Domanda che richiede sia specifiche interne che benchmark esterni: l'agente deve chiamare entrambi gli strumenti e sintetizzare un confronto:</p>
...
<h2 id="Cleanup" class="common-anchor-header">Pulizia<button data-href="#Cleanup" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>Al termine, abbandonare la raccolta per liberare risorse.</p>
