...
<p>Milvus 3.0-beta estende il database vettoriale Milvus con una nuova integrazione nell'ecosistema open lake: External Collection consente a Milvus di interrogare tabelle di laghi esterni a copia zero e Spark può leggere le collezioni Milvus direttamente tramite Snapshot. Il rilascio porta anche un recupero più ricco, uno schema più espressivo, una personalizzazione più profonda della ricerca testuale, controlli più precisi sul ciclo di vita dei dati e dei modelli e più controlli sul lato operatore. Milvus 3.0 è il nucleo centrale di Zilliz Lakebase e ne alimenta il servizio unificato, la scoperta e il batch.</p>
<p>Fate clic qui sotto per partecipare al nostro webinar per maggiori dettagli su Milvus 3.0 e AMA con i manutentori del nucleo:</p>
<p><a href="https://zilliz.com/event/whats-new-in-milvus-3-0-beta">
 ...
 </span>  <span class="img-wrapper"> <span>Webinar 3.0</span> </span></a></p>
...
<p>Snapshot crea una vista point-in-time, di sola lettura, di una Collection facendo riferimento ai segmenti esistenti invece di copiare i dati, in modo che il costo marginale di archiviazione sia prossimo allo zero. I lavori batch possono leggere dall'istantanea con un isolamento di tipo MVCC, mentre la raccolta attiva continua ad accettare le scritture.</p>
...
<h4 id="Query-Aggregation" class="common-anchor-header">Aggregazione delle query</h4><p>Per produrre statistiche sulla distribuzione degli affittuari, sul conteggio dei campi o sull'avanzamento del rollout delle versioni da una Milvus Collection era necessario riportare le entità corrispondenti al client e aggregarle lì. Milvus 3.0 introduce nel kernel l'aggregazione scalare in stile SQL. Una chiamata di query accetta <code translate="no">group_by_fields</code> e le espressioni di aggregazione in <code translate="no">output_fields</code>, comprese <code translate="no">count(*)</code>, <code translate="no">count(&lt;field&gt;)</code>, <code translate="no">sum(&lt;field&gt;)</code>, <code translate="no">avg(&lt;field&gt;)</code>, <code translate="no">min(&lt;field&gt;)</code> e <code translate="no">max(&lt;field&gt;)</code>. L'aggregazione viene valutata lato server dopo il filtraggio.</p>
<p>Per ulteriori informazioni, consultare <a href="/docs/it/get-and-scalar-query.md#Aggregate-Query-Results--Milvus-30x">Aggregazione dei risultati delle query</a>.</p>
<h4 id="Null-Vector" class="common-anchor-header">Vettore nullo</h4><p>Le incorporazioni sono spesso prodotte in modo asincrono, quindi un'entità può arrivare prima del suo vettore. Anche i dati multimodali presentano lacune naturali, come un video senza didascalie o un prodotto senza immagine. Le versioni precedenti non avevano una buona risposta: le applicazioni o ritardavano la scrittura fino a quando il vettore non era pronto o riempivano un vettore segnaposto, ed entrambe le scelte danneggiavano la qualità del recupero.</p>
<p>Milvus 3.0 supporta il NULL nei campi vettoriali per tutti e sei i tipi di vettore. La ricerca salta automaticamente i vettori NULL, la qualità del recupero è inalterata e i vettori NULL non occupano spazio. <code translate="no">AddField</code> si estende anche ai campi vettoriali con questa modifica: con <code translate="no">nullable=True</code>, una raccolta esistente può far crescere nuovi campi vettoriali online senza doverli ricostruire.</p>
<p>Per ulteriori informazioni, consultare <a href="/docs/it/nullable-and-default.md">Campi nulli</a>.</p>
<h4 id="Custom-Dictionary--Synonym-Dictionary" class="common-anchor-header">Dizionario personalizzato e dizionario dei sinonimi</h4><p>I tokenizer disponibili non sempre soddisfano i requisiti di qualità della ricerca di produzione. Il cinese, i domini verticali come la medicina, la legge e la chimica e i corpora multilingue possono trarre notevoli vantaggi dai dizionari personalizzati e dalle tabelle dei sinonimi. Fino ad oggi, queste risorse erano per lo più utilizzate come riscritture di query dal lato dell'applicazione.</p>
...
<h4 id="Storage-V3" class="common-anchor-header">Archiviazione V3</h4><p>Milvus 3.0 introduce Storage V3, un motore di archiviazione colonnare basato su manifest in cui i dati e i metadati vivono su uno storage di oggetti compatibile con S3. Ogni versione del dataset viene acquisita come snapshot immutabile del manifest, un file codificato Avro che registra i gruppi di colonne, i log delta e le statistiche che compongono il dataset.</p>
<p>I manifest sono file Avro compatti e i delta log registrano le cancellazioni a livello di entità senza riscrivere i file di dati. In questo modo, l'overhead dei metadati rimane ridotto man mano che i set di dati crescono. Il manifesto disaccoppia inoltre il tracciamento dei metadati dal percorso della query, consentendo a una Collection di gestire più segmenti senza ridurre le prestazioni della query.</p>
<p>Poiché gli stati sono memorizzati su uno storage a oggetti, il set di dati è autodescrittivo: qualsiasi lettore con accesso al percorso di storage può scoprirlo e interpretarlo senza un catalogo centrale. Questa proprietà è alla base delle raccolte esterne, degli snapshot e delle future integrazioni con i laghi.</p>
