...
<p>External Collection e Snapshot sono le principali novità sul fronte del data lake. Lo stesso kernel è alla base anche di Zilliz Lakebase, una piattaforma dati nativa semantica basata su Milvus 3.0.</p>
...
<h4 id="External-Backfill" class="common-anchor-header">Backfill esterno</h4><p>L’aggiornamento di un modello di embedding, come il passaggio dagli embedding v1 a quelli v2 su una collezione esistente, in passato comportava la ricostruzione da zero. Ciò costringeva a un periodo di inattività del servizio o all’adozione di una logica di doppia scrittura a livello di applicazione.</p>
...
<p>Milvus 3.0 supporta il valore NULL nei campi vettoriali per tutti e sei i tipi di vettori. La ricerca salta automaticamente i vettori NULL, la qualità del recupero non ne risente e i vettori NULL non occupano praticamente spazio di archiviazione. Con questa modifica, la funzione " <code translate="no">AddField</code> " si estende anche ai campi vettoriali: con l'opzione ` <code translate="no">nullable=True</code>`, una collezione esistente può aggiungere nuovi campi vettoriali online senza necessità di ricostruzione.</p>
