...
<h1 id="Upgrade-Milvus-Standalone-with-Helm-Chart" class="common-anchor-header">Aggiornamento di Milvus Standalone con Helm Chart<button data-href="#Upgrade-Milvus-Standalone-with-Helm-Chart" class="anchor-icon" translate="no">
    ...
    </button></h1><p>Questa guida descrive come aggiornare la distribuzione standalone di Milvus 2.6.x alla versione v3.0.2 utilizzando Helm.</p>
<div class="alert note">
<p>Questa procedura è stata verificata dal Milvus 2.6.20 al Milvus v3.0.2 con il Helm Chart di Milvus 5.0.22. Se si utilizza un'altra versione patch di Milvus 2.6.x o una versione diversa del Helm Chart, verificare prima l'aggiornamento in un ambiente non di produzione.</p>
</div>
...
<li>I valori Helm utilizzati per la distribuzione esistente</li>
<li>Un backup aggiornato dei metadati e dei dati persistenti di Milvus</li>
</ul>
<p><strong>Limitazioni relative alle code dei messaggi</strong>: durante l'aggiornamento a Milvus v3.0.2, è necessario mantenere la scelta corrente relativa alla coda dei messaggi. Il passaggio da un sistema di code dei messaggi a un altro durante l'aggiornamento non è supportato. Il supporto per la modifica dei sistemi di code dei messaggi sarà disponibile nelle versioni future.</p>
<div class="alert warning">
<p>Non modificare né effettuare il downgrade dell’Helm Chart nell’ambito di questa procedura. Mantieni la versione del Chart già installata per la tua release Helm. La baseline testata ha mantenuto l’Helm Chart 5.0.22 e ha modificato solo il tag dell’immagine Milvus in <code translate="no">v3.0.2</code>.</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>L’opzione <code translate="no">--reset-then-reuse-values</code> mantiene i valori della versione precedente, applicando al contempo la sostituzione esplicita dell’immagine rispetto alle impostazioni predefinite del Chart selezionato.</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>Verifica che tutti i carichi di lavoro richiesti siano pronti, che Milvus utilizzi <code translate="no">v3.0.2</code> e che le tue raccolte esistenti rimangano interrogabili e ricercabili. Completa questi controlli prima di abilitare qualsiasi funzionalità specifica della versione v3.0.2.</p>
<div class="alert note">
