...
<h1 id="Upgrade-Milvus-Standalone-with-Docker-Compose" class="common-anchor-header">Aggiornamento di Milvus Standalone con Docker Compose<button data-href="#Upgrade-Milvus-Standalone-with-Docker-Compose" class="anchor-icon" translate="no">
    ...
    </button></h1><p>Questa guida descrive come aggiornare una distribuzione standalone di Milvus 2.6.x alla versione v3.0.2 con Docker Compose.</p>
<div class="alert note">
<p>Questa procedura è stata verificata con la configurazione ufficiale di Docker Compose per Milvus 2.6.20 standalone. L'aggiornamento ha mantenuto etcd, MinIO, Woodpecker e le directory dei dati esistenti, modificando solo l'immagine di Milvus in <code translate="no">milvusdb/milvus:v3.0.2</code>.</p>
</div>
...
<p><strong>Limiti della coda dei messaggi</strong>: durante l'aggiornamento a Milvus v3.0.2, è necessario mantenere la coda dei messaggi attualmente in uso. Il passaggio a sistemi di code dei messaggi diversi durante l'aggiornamento non è supportato. Il supporto per la modifica dei sistemi di code dei messaggi sarà disponibile nelle versioni future.</p>
<div class="alert warning">
<p>Non sostituire il file Compose attuale né modificare le versioni delle dipendenze nell’ambito di questa procedura. Mantieni l’etcd, l’object storage, la coda dei messaggi, i volumi e la configurazione esistenti. Aggiorna solo il tag dell’immagine di Milvus.</p>
<p>Questa procedura non garantisce la validità di un downgrade o di un rollback ottenuto riportando l’immagine di Milvus alla versione 2.6.x. Dopo che la v3.0.2 ha scritto i dati, un rollback che riguarda solo l’immagine potrebbe non riuscire a leggere lo stato aggiornato. Se l’aggiornamento fallisce, interrompere le operazioni di scrittura e utilizzare un piano di ripristino che ripristini i metadati precedenti all’aggiornamento e i backup dei dati persistenti. Verificare prima il piano di ripristino in un ambiente non di produzione.</p>
</div>
...
<p>Verificare che i container attuali siano in buono stato prima di avviare l'aggiornamento:</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>Verifica che il servizio <code translate="no">standalone</code> sia in buono stato, che la sua immagine sia <code translate="no">milvusdb/milvus:v3.0.2</code> e che le collezioni esistenti rimangano interrogabili e ricercabili. Completa questi controlli prima di abilitare qualsiasi funzionalità specifica della versione v3.0.2.</p>
