...
<h1 id="Upgrade-Milvus-Standalone-with-Milvus-Operator" class="common-anchor-header">Aggiornamento di Milvus Standalone con Milvus Operator<button data-href="#Upgrade-Milvus-Standalone-with-Milvus-Operator" class="anchor-icon" translate="no">
    ...
    </button></h1><p>Questa guida descrive come aggiornare una distribuzione standalone di Milvus 2.6.x alla versione v3.0.1 utilizzando Milvus Operator.</p>
<div class="alert note">
<p>Questa procedura è stata verificata dal Milvus 2.6.20 al Milvus v3.0.1 con Milvus Operator 1.3.0, Woodpecker, etcd all'interno del cluster e MinIO all'interno del cluster. Se si utilizza un’altra versione patch di Milvus 2.6.x, una versione diversa di Operator, una coda di messaggi o una configurazione delle dipendenze, verificare prima l’aggiornamento in un ambiente non di produzione.</p>
</div>
...
<p>Questa procedura non convalida un downgrade o un rollback che comporti il ripristino dell'immagine di Milvus alla versione 2.6.x. Dopo che la versione v3.0.1 ha scritto i dati, un rollback che riguarda solo l’immagine potrebbe non riuscire a leggere lo stato aggiornato. Se l’aggiornamento fallisce, interrompere le operazioni di scrittura e utilizzare un piano di ripristino che riporti i metadati e i backup dei dati persistenti precedenti all’aggiornamento. Verificare prima il piano di ripristino in un ambiente non di produzione.</p>
</div>
<h2 id="Upgrade-process" class="common-anchor-header">Processo di aggiornamento<button data-href="#Upgrade-process" class="anchor-icon" translate="no">
    ...
    </button></h2><h3 id="Step-1-Back-up-the-current-Milvus-CR" class="common-anchor-header">Passaggio 1: Eseguire il backup dell'attuale CR di Milvus<button data-href="#Step-1-Back-up-the-current-Milvus-CR" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<button class="copy-code-btn"></button></code></pre>
<p>L'aggiornamento convalidato ha mantenuto Milvus Operator alla versione 1.3.0. Mantieni la versione di Operator che attualmente gestisce la tua distribuzione Milvus 2.6.x, a meno che la tua politica di supporto non richieda un aggiornamento separato di Operator. Non eseguire il downgrade di un Operator più recente alla versione testata. Se è necessario modificare la versione dell’Operator, utilizzare lo stesso metodo di installazione (Helm o <code translate="no">kubectl</code> ) e lo stesso nome di release e namespace dell’installazione esistente, quindi convalidare la modifica dell’Operator prima di aggiornare il CR di Milvus.</p>
<h3 id="Step-3-Update-the-Milvus-image" class="common-anchor-header">Passaggio 3: aggiornare l’immagine Milvus<button data-href="#Step-3-Update-the-Milvus-image" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<button class="copy-code-btn"></button></code></pre>
<p>Verificare che il CR di Milvus riporti ` <code translate="no">Healthy</code>`, che l'immagine corrente sia ` <code translate="no">milvusdb/milvus:v3.0.1</code>` e che le raccolte esistenti rimangano interrogabili e ricercabili. Completare questi controlli prima di abilitare qualsiasi funzionalità specifica della versione 3.0.1.</p>
