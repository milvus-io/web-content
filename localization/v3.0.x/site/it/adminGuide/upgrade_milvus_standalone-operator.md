...
summary: >-
  Scopri come effettuare l'aggiornamento di Milvus standalone con Milvus
  Operator.
...
<h1 id="Upgrade-Milvus-Standalone-with-Milvus-Operator" class="common-anchor-header">Aggiornamento di Milvus Standalone con Milvus Operator<button data-href="#Upgrade-Milvus-Standalone-with-Milvus-Operator" class="anchor-icon" translate="no">
    ...
    </button></h1><p>Questa guida descrive come aggiornare una distribuzione standalone di Milvus 2.6.x alla versione v3.0.2 utilizzando Milvus Operator.</p>
<div class="alert note">
<p>Questa procedura è stata verificata dal Milvus 2.6.20 al Milvus v3.0.2 con Milvus Operator 1.3.0, Woodpecker, etcd all'interno del cluster e MinIO all'interno del cluster. Se si utilizza un’altra versione patch di Milvus 2.6.x, una versione diversa di Operator, una coda di messaggi diversa o una configurazione delle dipendenze diversa, verificare prima l’aggiornamento in un ambiente non di produzione.</p>
</div>
...
<h3 id="Step-3-Update-the-Milvus-image" class="common-anchor-header">Passaggio 3: aggiornare l’immagine di Milvus<button data-href="#Step-3-Update-the-Milvus-image" class="anchor-icon" translate="no">
    ...
    </button></h3><p>Nel manifesto CR completo di Milvus, modificare solo <code translate="no">spec.components.image</code>. Mantenere la modalità esistente, le impostazioni dei componenti, la coda dei messaggi, etcd, lo storage e le altre impostazioni relative alle dipendenze. Il seguente estratto mostra il campo da modificare; non sostituire il CR completo con questo estratto.</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>Verificare che il CR di Milvus riporti ` <code translate="no">Healthy</code>`, che l'immagine attuale sia ` <code translate="no">milvusdb/milvus:v3.0.2</code>` e che le collezioni esistenti rimangano interrogabili e ricercabili. Completare questi controlli prima di abilitare qualsiasi funzionalità specifica della versione v3.0.2.</p>
