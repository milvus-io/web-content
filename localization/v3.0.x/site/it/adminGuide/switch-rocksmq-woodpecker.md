...
summary: >-
  Passare dalla coda dei messaggi di RocksMQ a quella di Woodpecker in
  un'installazione Milvus Standalone (Docker Compose).
---
<h1 id="Switch-between-RocksMQ-and-Woodpecker" class="common-anchor-header">Passaggio da RocksMQ a Woodpecker<button data-href="#Switch-between-RocksMQ-and-Woodpecker" class="anchor-icon" translate="no">
    ...
    </button></h1><p>Questa pagina descrive come passare dalla coda dei messaggi (MQ) di una distribuzione <strong>Milvus Standalone (Docker Compose)</strong> da <strong>RocksMQ</strong> a <strong>Woodpecker</strong> (backend locale o MinIO) e viceversa. Per il flusso di lavoro generale e i prerequisiti, consultare <a href="/docs/it/switch-mq-type.md">Passare da una coda dei messaggi</a> all'altra.</p>
<div class="alert note">
<ul>
<li><strong>Prerequisito:</strong> la funzionalità "Cambio di coda dei messaggi" è disponibile in <strong>Milvus 3.0 e versioni successive</strong>. Aggiornare l’istanza di Milvus a Milvus 3.0 o versioni successive prima di iniziare: la funzionalità non è disponibile nelle versioni precedenti.</li>
<li>Il cambio di coda dei messaggi richiede la distribuzione Docker <strong>Compose</strong> (che abilita una sorgente di configurazione etcd). La distribuzione Docker a container singolo non supporta il cambio.</li>
</ul>
</div>
<h2 id="Switch-from-RocksMQ-to-Woodpecker" class="common-anchor-header">Passaggio da RocksMQ a Woodpecker<button data-href="#Switch-from-RocksMQ-to-Woodpecker" class="anchor-icon" translate="no">
    ...
    </button></h3><p>Verifica che la tua istanza Milvus Standalone Docker Compose sia in esecuzione corretta — ad esempio, creando una raccolta di prova, inserendo dati ed eseguendo una query.</p>
<h3 id="Step-2-Configure-Woodpecker-storage" class="common-anchor-header">Passaggio 2: configurare l’archiviazione Woodpecker<button data-href="#Step-2-Configure-Woodpecker-storage" class="anchor-icon" translate="no">
    ...
    </button></h3><p>Aggiungere le impostazioni di Woodpecker alla configurazione di Milvus <strong>senza</strong> modificare il valore di ` <code translate="no">mqType</code> `. Eseguire ` <code translate="no">docker exec -it milvus-standalone bash</code> ` per accedere al container, quindi modificare ` <code translate="no">/milvus/configs/user.yaml</code>`:</p>
...
<h3 id="Step-5-Optional-Clean-up-RocksMQ-data" class="common-anchor-header">Passaggio 5: (Facoltativo) Rimuovere i dati di RocksMQ<button data-href="#Step-5-Optional-Clean-up-RocksMQ-data" class="anchor-icon" translate="no">
    ...
    </button></h3><p>I dati di RocksMQ si trovano nelle directory <code translate="no">volumes/milvus/rdb_data</code> e <code translate="no">volumes/milvus/rdb_data_meta_kv</code> definite in <code translate="no">docker-compose.yaml</code>. Se si prevede di tornare a RocksMQ in un secondo momento, eliminare prima questi file per evitare conflitti.</p>
...
<p>Assicurarsi che l’istanza non contenga dati residui di RocksMQ provenienti da un’esecuzione precedente. Se è la prima volta che si passa a RocksMQ, ignorare questa nota; in caso contrario, eliminare prima i metadati e i dati relativi a RocksMQ.</p>
</div>
<pre><code translate="no" class="language-shell">curl -X POST http://&lt;mixcoord_addr&gt;:&lt;mixcoord_port&gt;/management/wal/alter \
  -H &quot;Content-Type: application/json&quot; \
...
<tr><td>RocksMQ / Woodpecker</td><td>Pulsar / Kafka esterno</td><td><strong>Supportato ma non consigliato</strong></td><td>Mantenere le istanze standalone il più semplici possibile.</td></tr>
</tbody>
</table>
