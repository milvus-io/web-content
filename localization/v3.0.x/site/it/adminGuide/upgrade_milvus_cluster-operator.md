...
<h1 id="Upgrade-Milvus-Cluster-with-Milvus-Operator" class="common-anchor-header">Aggiornamento del cluster Milvus con Milvus Operator<button data-href="#Upgrade-Milvus-Cluster-with-Milvus-Operator" class="anchor-icon" translate="no">
    ...
    </button></h1><p>Questa guida descrive come aggiornare un cluster Milvus 2.6.x alla versione 3.0.2 con Milvus Operator.</p>
<div class="alert note">
<p>Questa procedura è stata verificata dal Milvus 2.6.20 al Milvus v3.0.2 con Milvus Operator 1.3.0, MixCoord, StreamingNode, Woodpecker, etcd all'interno del cluster e MinIO all'interno del cluster. Se si utilizza un’altra versione patch di Milvus 2.6.x, una versione diversa di Operator, una topologia dei componenti diversa, una coda di messaggi diversa o una configurazione delle dipendenze diversa, verificare prima l’aggiornamento in un ambiente non di produzione.</p>
</div>
...
<button class="copy-code-btn"></button></code></pre>
<p>L'aggiornamento convalidato ha mantenuto Milvus Operator alla versione 1.3.0. Mantenere la versione di Operator che attualmente gestisce la distribuzione di Milvus 2.6.x, a meno che la politica di supporto non richieda un aggiornamento separato di Operator. Non eseguire il downgrade di un Operator più recente alla versione testata. Se è necessario modificare la versione dell’Operator, utilizzare lo stesso metodo di installazione (Helm o <code translate="no">kubectl</code> ) e lo stesso nome di release e namespace dell’installazione esistente, quindi convalidare la modifica dell’Operator prima di aggiornare il CR di Milvus.</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>Verificare che il CR di Milvus riporti <code translate="no">Healthy</code>, che tutti i componenti di Milvus utilizzino <code translate="no">milvusdb/milvus:v3.0.2</code>, che nessun Pod IndexNode sia in esecuzione e che le collezioni esistenti rimangano interrogabili e ricercabili. Completare questi controlli prima di abilitare qualsiasi funzionalità specifica della versione v3.0.2.</p>
