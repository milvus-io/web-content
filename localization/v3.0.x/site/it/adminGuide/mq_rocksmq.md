...
<h1 id="Use-RocksMQ-as-the-Milvus-Message-Queue" class="common-anchor-header">Utilizzare RocksMQ come coda di messaggi di Milvus<button data-href="#Use-RocksMQ-as-the-Milvus-Message-Queue" class="anchor-icon" translate="no">
    ...
    </button></h1><p>RocksMQ è una coda di messaggi integrata (WAL) inclusa in Milvus, disponibile <strong>solo</strong> per <strong>Milvus Standalone</strong>. Nelle versioni precedenti di Milvus era la coda di messaggi predefinita per la versione standalone; in Milvus 3.x, Milvus Standalone utilizza <a href="/docs/it/woodpecker.md">Woodpecker</a> integrato come impostazione predefinita.</p>
...
<li>RocksMQ è incluso in Milvus, quindi non è necessario installarne una versione separata.</li>
<li>Era la coda di messaggi predefinita per la versione Standalone nelle precedenti versioni di Milvus ed è stata sostituita da Woodpecker integrato in Milvus 3.x.</li>
</ul>
<h2 id="Deploy-Milvus-Standalone-with-RocksMQ-using-Docker" class="common-anchor-header">Distribuzione di Milvus Standalone con RocksMQ tramite Docker<button data-href="#Deploy-Milvus-Standalone-with-RocksMQ-using-Docker" class="anchor-icon" translate="no">
    ...
    </button></h3><p>Seguire le istruzioni riportate <a href="/docs/it/install_standalone-docker.md">in "Eseguire Milvus in Docker</a>". In Milvus 3.x, l’impostazione predefinita per la modalità standalone è Woodpecker, pertanto è necessario impostare esplicitamente il tipo di coda di messaggi su RocksMQ. Lo script di avvio crea un nuovo file ` <code translate="no">user.yaml</code> ` al <strong>primo</strong> avvio ( <code translate="no">start</code>), quindi impostare il tipo <strong>dopo</strong> tale primo avvio e successivamente eseguire ` <code translate="no">restart</code> ` per applicare la modifica (il comando ` <code translate="no">restart</code> ` mantiene il contenuto di ` <code translate="no">user.yaml</code>`):</p>
...
La modifica dell'<code translate="no">mq.type</code> in questo modo è pensata per un'istanza <b>nuova di zecca</b> (senza collezioni). Per modificare la coda dei messaggi di un'istanza che contiene già dati, segui invece la <a href="/docs/it/switch-rocksmq-woodpecker.md">procedura di cambio</a>.
</div>
...
<li><strong>Aggiornamento dalla versione 2.5.x alla 2.6.x:</strong> <strong>limitazioni relative alle code di messaggi</strong>: quando si esegue l'aggiornamento a Milvus v3.0.2, è necessario mantenere la scelta attuale della coda di messaggi. Il passaggio tra diversi sistemi di code di messaggi durante l'aggiornamento non è supportato. Il supporto per la modifica dei sistemi di code di messaggi sarà disponibile nelle versioni future.
Poiché la versione 2.6.x imposta Woodpecker come predefinito per l’esecuzione standalone, se si desidera mantenere RocksMQ è necessario fissare <code translate="no">mq.type: rocksmq</code> nel file <code translate="no">user.yaml</code> <strong>prima</strong> dell’aggiornamento.</li>
