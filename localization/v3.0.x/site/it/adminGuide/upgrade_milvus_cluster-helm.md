...
<h1 id="Upgrade-Milvus-Cluster-with-Helm-Chart" class="common-anchor-header">Aggiornamento del cluster Milvus con Helm Chart<button data-href="#Upgrade-Milvus-Cluster-with-Helm-Chart" class="anchor-icon" translate="no">
    ...
    </button></h1><p>Questa guida descrive come aggiornare il cluster Milvus 2.6.x alla versione 3.0.2 utilizzando Helm.</p>
<div class="alert note">
<p>Questa procedura è stata verificata per il passaggio da Milvus 2.6.20 a Milvus v3.0.2 utilizzando il grafico Helm di Milvus 5.0.22. Se si utilizza un'altra versione patch di Milvus 2.6.x o una versione diversa del grafico Helm, verificare prima l'aggiornamento in un ambiente non di produzione.</p>
</div>
...
<p><strong>Limitazioni relative alla coda dei messaggi</strong>: durante l'aggiornamento a Milvus v3.0.2, è necessario mantenere la scelta corrente della coda dei messaggi. Il passaggio tra diversi sistemi di coda dei messaggi durante l'aggiornamento non è supportato. Il supporto per la modifica dei sistemi di coda dei messaggi sarà disponibile nelle versioni future.</p>
<div class="alert warning">
<p>Non modificare né effettuare il downgrade dell’Helm Chart nell’ambito di questa procedura. Mantieni la versione del Chart già installata per la tua release di Helm. La baseline testata ha mantenuto l’Helm Chart 5.0.22 e ha modificato solo il tag dell’immagine Milvus in <code translate="no">v3.0.2</code>.</p>
<p>Questa procedura non convalida un downgrade o un rollback che comporti il ripristino dell’immagine Milvus alla versione 2.6.x. Dopo che la v3.0.2 ha scritto i dati, un rollback che riguarda solo l’immagine potrebbe non riuscire a leggere lo stato aggiornato. Se l’aggiornamento fallisce, interrompere le operazioni di scrittura e utilizzare un piano di ripristino che ripristini i metadati precedenti all’aggiornamento e i backup dei dati persistenti. Verificare prima il piano di ripristino in un ambiente non di produzione.</p>
</div>
...
<button class="copy-code-btn"></button></code></pre>
<p>Verifica che tutti i carichi di lavoro richiesti siano pronti, che tutti i componenti Milvus utilizzino <code translate="no">v3.0.2</code> e che le tue collezioni esistenti rimangano interrogabili e ricercabili. Completa questi controlli prima di abilitare qualsiasi funzionalità specifica della v3.0.2.</p>
<div class="alert note">
