...
<h1 id="Woodpecker" class="common-anchor-header">Woodpecker<button data-href="#Woodpecker" class="anchor-icon" translate="no">
    ...
    </button></h1><p>Woodpecker è la <strong>coda di messaggi predefinita (write-ahead log, WAL)</strong> in Milvus 3.x. Si tratta di un WAL nativo per il cloud progettato per l'archiviazione a oggetti, che offre un elevato throughput, un basso overhead operativo e una scalabilità senza soluzione di continuità. Per i dettagli sull'architettura e sui benchmark, consultare <a href="/docs/it/woodpecker_architecture.md">Woodpecker</a>.</p>
...
<li>In Milvus 3.x, Woodpecker è il WAL/coda di messaggi <strong>predefinito</strong>, che fornisce scritture ordinate e funzionalità di ripristino in qualità di servizio di registrazione. Non è richiesto alcun servizio esterno di coda di messaggi (come Pulsar o Kafka).</li>
...
<p>Nota: la migrazione a <code translate="no">mq.type</code> per un cluster in esecuzione costituisce un’operazione di aggiornamento. Seguire attentamente la procedura di aggiornamento e verificare il funzionamento su un cluster nuovo prima di effettuare il passaggio in produzione.</p>
...
<ul>
<li>Controlla le politiche di sincronizzazione, svuotamento, compattazione e lettura per i segmenti di log. Queste sono le impostazioni principali per l’ottimizzazione della larghezza di banda e della latenza.</li>
</ul></li>
...
<h3 id="Enable-Woodpecker-for-Milvus-Standalone-in-Docker-storagelocal" class="common-anchor-header">Abilitare Woodpecker per Milvus in modalità standalone su Docker (storage=local)<button data-href="#Enable-Woodpecker-for-Milvus-Standalone-in-Docker-storagelocal" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<button class="copy-code-btn"></button></code></pre>
<p>In questo modo Woodpecker viene distribuito come uno StatefulSet dedicato (<code translate="no">my-release-milvus-woodpecker</code>, 4 repliche per impostazione predefinita) con un servizio headless in primo piano, in cluster gossip sulle porte <code translate="no">18080</code> (servizio), <code translate="no">17946</code> (gossip) e <code translate="no">9091</code> (metriche), con MinIO come backend di archiviazione. Il servizio richiede un quorum di <strong>3</strong> nodi; l’impostazione predefinita di <strong>4</strong> repliche garantisce il quorum pur tollerando il guasto di un singolo nodo, pertanto non impostare il parametro ` <code translate="no">woodpecker.replicaCount</code> ` su un valore inferiore a 3. Il cluster include quindi un insieme separato di pod <code translate="no">woodpecker</code>:</p>
...
<h2 id="Throughput-tuning-tips" class="common-anchor-header">Suggerimenti per l’ottimizzazione del throughput<button data-href="#Throughput-tuning-tips" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Il profilo di throughput e latenza di Woodpecker varia tra la modalità <strong>embedded</strong> e la modalità <strong>di servizio</strong> (una funzionalità di Milvus 3.0). Le indicazioni riportate di seguito sono organizzate per modalità.</p>
...
<ul>
<li>Aumentare i valori di ` <code translate="no">logstore.segmentSyncPolicy.maxFlushSize</code> ` e ` <code translate="no">maxFlushThreads</code> ` per operazioni di flush più grandi e un parallelismo maggiore.</li>
<li>Ottimizzare <code translate="no">maxInterval</code> in base alle caratteristiche del supporto (scambiare latenza con throughput con un'aggregazione più lunga).</li>
...
<p><strong>Dimostrazione di inserimento in batch</strong> — utilizzare quanto segue per misurare il throughput in scrittura:</p>
...
<ul>
<li><strong>Scritture con quorum a un RTT</strong>: la replica guidata dal client completa una scrittura con quorum entro un singolo round trip, con il traffico tra zone (cross-AZ) limitato a una quantità di dati pari a due repliche (rispetto al traffico cross-AZ aggiuntivo pari a circa 1/3, tipico della replica basata su broker/leader).</li>
<li><strong>Letture a salto singolo sensibili alla topologia</strong> — ogni lettura va direttamente alla replica più vicina invece di essere inoltrata tramite un broker, evitando le letture casuali tra le AZ (≈2/3 del traffico di lettura tra le AZ) tipiche dei sistemi basati su broker.</li>
...
<li><strong>Nessuna replica continua da nodo a nodo</strong> — i log vengono conservati nell’object storage che funge da storage condiviso, quindi il failover ricarica solo le repliche sopravvissute (senza copia dell’intero nodo); lo scaling non è vincolato dalla larghezza di banda di replica tra i nodi e la sostituzione di nodi su larga scala non causa picchi di replica.</li>
</ul>
<p>Nelle distribuzioni cross-AZ, la modalità di servizio consente inoltre di risparmiare circa <strong>1/3 del</strong> traffico di rete <strong>in scrittura</strong> e <strong>2/3 di quello in lettura</strong> rispetto ai sistemi di log basati su broker. Per l’analisi completa del design e dei costi, consultare <a href="/docs/it/woodpecker_architecture.md">Architettura di Woodpecker</a>.</p>
