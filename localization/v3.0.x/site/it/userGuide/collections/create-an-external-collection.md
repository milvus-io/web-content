...
summary: >-
  Accedere ai dati in AWS S3 o Iceberg attraverso una raccolta esterna senza
  copiarli in Milvus.
beta: Milvus 3.0.x
---
<h1 id="Create-an-External-Collection" class="common-anchor-header">Creare una raccolta esterna<span class="beta-tag" style="background-color:rgb(0, 179, 255);color:white" translate="no">Compatible with Milvus 3.0.x</span><button data-href="#Create-an-External-Collection" class="anchor-icon" translate="no">
    ...
    </button></h1><p>Una raccolta esterna è un tipo di raccolta di dati in Milvus che accede ai dati da sistemi di archiviazione esterni come AWS S3 e Iceberg senza copiarli in Milvus. Agisce come un livello di interrogazione sui data lake, mantenendo la compatibilità con le interfacce di interrogazione di Milvus.</p>
...
<h2 id="Limits--restrictions" class="common-anchor-header">Limiti e restrizioni<button data-href="#Limits--restrictions" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Poiché Milvus non memorizza i dati grezzi e mantiene solo una mappatura tra i metadati e i dati grezzi, le raccolte esterne sono di sola lettura. Ciò significa che non è possibile inserire, aggiornare, cancellare, importare, svuotare e compattare i dati sul lato Milvus.</p>
<p>Rispetto alle raccolte gestite, le raccolte esterne hanno i seguenti limiti:</p>
<ul>
<li><p>Non è possibile impostare la chiave primaria e gli attributi AutoID.</p></li>
<li><p>Non è possibile abilitare il campo dinamico.</p></li>
<li><p>Non è possibile impostare la chiave di partizione e la chiave di clustering, perché le partizioni non sono disponibili.</p></li>
<li><p>Non si possono definire funzioni nello schema.</p></li>
<li><p>Non è possibile modificare lo schema di una collezione esterna dopo averla creata.</p></li>
<li><p>Non è possibile utilizzare la corrispondenza di testo con BM25.</p></li>
<li><p>È necessario attivare un'operazione di aggiornamento prima di creare gli indici.</p></li>
</ul>
...
<button class="copy-code-btn"></button></code></pre>
<p>Per creare lo schema di una raccolta esterna, è necessario specificare l'URI dei dati di origine, il formato dei dati e le impostazioni di autenticazione.</p>
...
<table>
   ...
   <tr>
     ...
     <td><p>Formato dei file di dati di origine di destinazione.</p><p>I valori possibili sono <code translate="no">parquet</code>, <code translate="no">vortex</code>, <code translate="no">lance-table</code> e <code translate="no">iceberg-table</code>.</p></td>
...
<table>
   ...
   <tr>
     <td><p><code translate="no">format</code></p></td>
     <td><p>Formato dei dati di origine di destinazione.</p><p>I valori possibili sono <code translate="no">parquet</code>, <code translate="no">vortex</code>, <code translate="no">lance-table</code>, e <code translate="no">iceberg-table</code></p></td>
     <td><p><code translate="no">parquet</code></p></td>
   </tr>
   <tr>
     <td><p><code translate="no">extfs</code></p></td>
     <td><p>Impostazioni del file system esterno</p></td>
     <td><p>--</p></td>
   </tr>
   <tr>
...
<table>
   ...
   <tr>
     <td><p><code translate="no">extfs.region</code></p></td>
     <td><p>ID regione del cloud</p></td>
     <td><p><code translate="no">us-west-2</code></p></td>
   </tr>
   <tr>
     <td><p><code translate="no">extfs.use_ssl</code></p></td>
...
<h2 id="Step-3-Create-a-collection" class="common-anchor-header">Passo 3: creare una collezione<button data-href="#Step-3-Create-a-collection" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Dopo aver aggiunto tutti i campi allo schema, si può creare la collezione.</p>
<div class="multipleCode">
   <a href="#python">Python</a> <a href="#java">Java</a> <a href="#go">Go</a> <a href="#javascript">NodeJS</a> <a href="#bash">cURL</a></div>
...
<pre><code translate="no" class="language-bash"><span class="hljs-comment"># restful</span>
<button class="copy-code-btn"></button></code></pre>
<h2 id="Step-4-Refresh-data" class="common-anchor-header">Passo 4: Aggiornare i dati<button data-href="#Step-4-Refresh-data" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>Una volta che la collezione è pronta, è necessario eseguire un aggiornamento per sincronizzare i metadati dai dati a Milvus.</p>
<div class="multipleCode">
   <a href="#python">Python</a> <a href="#java">Java</a> <a href="#go">Go</a> <a href="#javascript">NodeJS</a> <a href="#bash">cURL</a></div>
...
<pre><code translate="no" class="language-bash"><span class="hljs-comment"># restful</span>
<button class="copy-code-btn"></button></code></pre>
<p>L'operazione di aggiornamento è asincrona, quindi è necessario impostare un'iterazione per monitorare i suoi progressi.</p>
...
<li><p>Non è possibile indicizzare una collezione esterna solo al termine dell'aggiornamento. Tuttavia, la modalità di creazione degli indici è la stessa di una collezione gestita.</p></li>
...
<h2 id="Follow-ups" class="common-anchor-header">Operazioni successive<button data-href="#Follow-ups" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>Una volta eseguita un'operazione di aggiornamento sulla raccolta esterna e una volta che i file manifest sono disponibili, è possibile creare indici, caricare/rilasciare raccolte ed eseguire ricerche e query di similarità nella raccolta esterna come in qualsiasi raccolta gestita.</p>
