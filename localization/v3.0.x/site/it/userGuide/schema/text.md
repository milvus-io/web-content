...
<p>Milvus fornisce il tipo di campo scalare ` <code translate="no">TEXT</code> ` per memorizzare testi di origine di lunga durata direttamente insieme alle entità. I valori tipici includono brani, documenti lunghi, corpi di articoli, ticket e log. A differenza di ` <code translate="no">VARCHAR</code>`, che richiede una lunghezza massima in byte ( <code translate="no">max_length</code>) fissa, ` <code translate="no">TEXT</code> ` non richiede di impostare una lunghezza massima in byte nello schema della collezione.</p>
...
<p>Milvus rifiuta uno schema di raccolta che contenga un campo " <code translate="no">TEXT</code> " se Storage V3 è disabilitato.</p>
<pre><code translate="no" class="language-python">schema.add_field(
    field_name=<span class="hljs-string">&quot;content&quot;</span>,
<span class="highlighted-wrapper-line">    datatype=DataType.TEXT,</span>
)
<button class="copy-code-btn"></button></code></pre>
<p>Una volta definito il campo, ogni entità può includere un valore stringa in quel campo. I valori di " <code translate="no">TEXT</code> " si inseriscono come gli altri campi scalari e vengono restituiti dai risultati delle query o delle ricerche elencando il campo in " <code translate="no">output_fields</code>".</p>
<div class="alert note">
<p><code translate="no">TEXT</code> I campi supportano i valori null. Per abilitare questa funzionalità, impostare ` <code translate="no">nullable</code> ` su ` <code translate="no">True</code>`. Per ulteriori dettagli, consultare la sezione <a href="/docs/it/nullable-and-default.md">"Campo nullable</a>".</p>
</div>
