...
title: Analizzatore predefinito
summary: >-
  L'analizzatore standard è l'analizzatore predefinito in Milvus, che viene
  applicato automaticamente ai campi di testo se non viene specificato alcun
  analizzatore. Combina il tokenizzatore standard con il filtro per la
  conversione in minuscolo.
---
<h1 id="Standard-Analyzer" class="common-anchor-header">Analizzatore predefinito<button data-href="#Standard-Analyzer" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h1><p>L'analizzatore " <code translate="no">standard</code> " è l'analizzatore predefinito in Milvus, che viene applicato automaticamente ai campi di testo se non viene specificato alcun analizzatore. Combina il tokenizzatore standard con il filtro per il passaggio alle minuscole.</p>
<div class="alert note">
<p>L'analizzatore " <code translate="no">standard</code> " è adatto alle lingue che utilizzano separatori (come spazi e segni di punteggiatura) per definire i confini delle parole. Tuttavia, lingue come il cinese, l'arabo, il thailandese, il giapponese e il coreano richiedono una tokenizzazione o una normalizzazione specifica per la lingua. In questi casi, utilizzare un analizzatore specifico per la lingua, come <a href="/docs/it/chinese-analyzer.md"><code translate="no">chinese</code></a>, <a href="/docs/it/arabic-analyzer.md"><code translate="no">arabic</code></a>, oppure <a href="/docs/it/thai-analyzer.md"><code translate="no">thai</code></a>, oppure analizzatori personalizzati con tokenizzatori specializzati come <a href="/docs/it/lindera-tokenizer.md"><code translate="no">lindera</code></a> e <a href="/docs/it/icu-tokenizer.md"><code translate="no">icu</code></a>.</p>
</div>
<h2 id="Definition" class="common-anchor-header">Definizione<button data-href="#Definition" class="anchor-icon" translate="no">
    ...
    </button></h2><p>L’analizzatore <code translate="no">standard</code> è costituito da:</p>
<ul>
<li><p><strong>Tokenizzatore</strong>: utilizza il tokenizzatore " <code translate="no">standard</code> " per mantenere le lettere Unicode e i caratteri numerici consecutivi all’interno dei token e per effettuare la suddivisione in presenza di altri caratteri. Per le regole esatte relative ai caratteri, consultare la sezione " <a href="/docs/it/standard-tokenizer.md#Tokenization-rules">Tokenizzatore standard</a>".</p></li>
<li><p><strong>Filtro</strong>: utilizza il filtro " <code translate="no">lowercase</code> " per convertire tutti i token in minuscolo, consentendo ricerche che non distinguono tra maiuscole e minuscole. Per ulteriori informazioni, consultare la sezione <a href="/docs/it/lowercase-filter.md">"Minuscolo"</a>.</p></li>
</ul>
<p>La funzionalità dell’analizzatore ` <code translate="no">standard</code> ` è equivalente alla seguente configurazione di analizzatore personalizzato:</p>
...
<h2 id="Configuration" class="common-anchor-header">Configurazione<button data-href="#Configuration" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Per applicare l’analizzatore “ <code translate="no">standard</code> ” a un campo, è sufficiente impostare ` <code translate="no">type</code> ` su ` <code translate="no">standard</code> ` in ` <code translate="no">analyzer_params</code>` e includere i parametri opzionali secondo necessità.</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#javascript">   NodeJS</a>
...
<table>
   ...
   <tr>
     ...
     <td><p>Un array contenente un elenco di parole da escludere, che verranno rimosse dalla tokenizzazione. Il valore predefinito è <code translate="no">_english_</code>, un insieme integrato di parole da escludere comuni in inglese.</p></td>
   </tr>
</table>
<p>Esempio di configurazione delle parole di stop personalizzate:</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#javascript">   NodeJS</a>
...
<pre><code translate="no" class="language-bash"><span class="hljs-comment"># restful</span>
<button class="copy-code-btn"></button></code></pre>
<p>Dopo aver definito <code translate="no">analyzer_params</code>, è possibile applicarle a un campo <code translate="no">VARCHAR</code> durante la definizione dello schema di una collezione. Ciò consente a Milvus di elaborare il testo in quel campo utilizzando l'analizzatore specificato per una tokenizzazione e un filtraggio efficienti. Per ulteriori informazioni, consultare <a href="/docs/it/analyzer-overview.md#Example-use">Esempio di utilizzo</a>.</p>
