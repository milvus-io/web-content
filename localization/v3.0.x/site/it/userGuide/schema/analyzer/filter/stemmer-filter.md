...
title: Stemmer
summary: >-
  Il filtro "stemmer" riduce le parole alla loro forma base o radice (processo
  noto come "stemming"), facilitando l'abbinamento di parole con significati
  simili nelle diverse forme flesse. Il filtro "stemmer" supporta più lingue,
  consentendo una ricerca e un'indicizzazione efficaci in vari contesti
  linguistici.
---
<h1 id="Stemmer" class="common-anchor-header">Stemmer<button data-href="#Stemmer" class="anchor-icon" translate="no">
    ...
    </button></h1><p>Il filtro " <code translate="no">stemmer</code> " riduce le parole alla loro forma base o radice (operazione nota come stemming), facilitando l'abbinamento di parole con significati simili in diverse forme flesse. Il filtro " <code translate="no">stemmer</code> " supporta più lingue, consentendo una ricerca e un'indicizzazione efficaci in vari contesti linguistici.</p>
<h2 id="Configuration" class="common-anchor-header">Configurazione<button data-href="#Configuration" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Il filtro <code translate="no">stemmer</code> è un filtro personalizzato in Milvus. Per utilizzarlo, specificare <code translate="no">&quot;type&quot;: &quot;stemmer&quot;</code> nella configurazione del filtro, insieme al parametro <code translate="no">language</code> per selezionare la lingua desiderata per lo stemming.</p>
...
<table>
   ...
   <tr>
     ...
     <td><p>Specifica la lingua per il processo di stemming. Le lingue supportate includono: <code translate="no">"arabic"</code>, <code translate="no">"danish"</code>, <code translate="no">"dutch"</code>, <code translate="no">"english"</code>, <code translate="no">"finnish"</code>, <code translate="no">"french"</code>, <code translate="no">"german"</code>, <code translate="no">"greek"</code>, <code translate="no">"hungarian"</code>, <code translate="no">"italian"</code>, <code translate="no">"norwegian"</code>, <code translate="no">"portuguese"</code>, <code translate="no">"romanian"</code>, <code translate="no">"russian"</code>, <code translate="no">"spanish"</code>, <code translate="no">"swedish"</code>, <code translate="no">"tamil"</code>, <code translate="no">"turkish"</code></p></td>
   </tr>
</table>
<p>Il filtro " <code translate="no">stemmer</code> " opera sui termini generati dal tokenizer, pertanto deve essere utilizzato in combinazione con un tokenizer.</p>
<p>Per il testo in arabo, utilizzare l’analizzatore integrato <a href="/docs/it/arabic-analyzer.md"><code translate="no">arabic</code></a> analizzatore integrato. L’analizzatore integrato include lo stemming arabo insieme alla normalizzazione araba, alla normalizzazione delle cifre decimali e alla rimozione delle parole vuote arabe. Utilizzare direttamente il filtro <code translate="no">stemmer</code> solo quando è necessario creare una pipeline di analizzatori personalizzata.</p>
