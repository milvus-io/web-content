...
<tr><td><code translate="no">page</code></td><td><code translate="no">INT64</code></td><td>Numero di pagina o posizione logica del chunk.</td></tr>
...
<h2 id="Create-a-collection-with-a-StructArray-field" class="common-anchor-header">Creare una raccolta con un campo StructArray<button data-href="#Create-a-collection-with-a-StructArray-field" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<ol>
<li><p>Creare lo schema della collezione.</p></li>
...
<li><p>Impostare ` <code translate="no">max_capacity</code> ` per limitare il numero di elementi Struct che ogni entità può memorizzare nel campo.</p></li>
</ol>
...
<tr><td><code translate="no">chunks[emb_list_vector]</code></td><td>Il sottocampo vettoriale utilizzato come lista di embedding.</td><td>Ricerca in EmbeddingList con l'<code translate="no">MAX_SIM*</code>.</td></tr>
...
<tr><td>Sono consentiti solo i tipi di sottocampo supportati.</td><td>Utilizzare i tipi di sottocampo scalari e vettoriali supportati da StructArray. Non definire sottocampi JSON, Geometry, Text, Timestamptz, SparseFloatVector o sottocampi Struct / Array annidati.</td></tr>
...
<tr><td>Un sottocampo vettoriale ha un solo indice.</td><td>Se sono necessarie sia la ricerca EmbeddingList che la ricerca a livello di elemento, creare due sottocampi vettoriali separati.</td></tr>
...
<li><p>Aggiungere solo sottocampi vettoriali e tralasciare i sottocampi scalari necessari per il filtraggio, come <code translate="no">section</code>, <code translate="no">quality_score</code> o <code translate="no">has_code</code>.</p></li>
...
<li><p>Ipotesi che sia possibile aggiungere nuovi sottocampi a un campo StructArray esistente dopo la creazione del campo stesso.</p></li>
