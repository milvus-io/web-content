...
title: Campi StructArray dell'indice
summary: >-
  ...
  l'oggetto dell'indice è un percorso di sottocampo, ad esempio
  chunks[emb_list_vector], chunks[emb] o chunks[section].
---
<h1 id="Index-StructArray-Fields" class="common-anchor-header">Campi StructArray dell'indice<button data-href="#Index-StructArray-Fields" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h1><p>Creare indici sui sottocampi di StructArray prima di eseguire una ricerca vettoriale o accelerare il filtraggio scalare. Per un campo StructArray, la destinazione dell'indice è un percorso di sottocampo, ad esempio <code translate="no">chunks[emb_list_vector]</code>, <code translate="no">chunks[emb]</code> o <code translate="no">chunks[section]</code>.</p>
...
<tbody>
<tr><td>Ricerca nell'EmbeddingList</td><td><code translate="no">chunks[emb_list_vector]</code></td><td>Una famiglia di metriche di tipo " <code translate="no">MAX_SIM*</code> ".</td></tr>
...
<button class="copy-code-btn"></button></code></pre>
<p>Gli indici scalari sono opzionali ma utili quando i sottocampi scalari di StructArray compaiono frequentemente nei filtri, come <code translate="no">element_filter(chunks, $[quality_score] &gt; 0.9)</code> o <code translate="no">MATCH_ANY(chunks, $[section] == &quot;index&quot;)</code>.</p>
...
<h3 id="EmbeddingList-search" class="common-anchor-header">Ricerca EmbeddingList<button data-href="#EmbeddingList-search" class="anchor-icon" translate="no">
    ...
    </button></h3><p>La ricerca EmbeddingList utilizza metriche di tipo <code translate="no">MAX_SIM*</code>. Considera i vettori presenti in un sottocampo vettoriale StructArray come una lista di embedding e restituisce risultati a livello di entità.</p>
<table>
<thead>
...
<tr><td>Tenere presenti i limiti dei campi vettoriali.</td><td>Il numero totale di campi vettoriali e sottocampi vettoriali è limitato. Consulta Limiti di StructArray prima di aggiungere molti sottocampi vettoriali.</td></tr>
</tbody>
</table>
...
<li><p>Riutilizzare un sottocampo vettoriale sia per le metriche " <code translate="no">MAX_SIM*</code> " che per quelle vettoriali standard.</p></li>
<li><p>Dimenticare gli indici scalari per i filtri StructArray molto utilizzati.</p></li>
