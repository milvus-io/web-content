...
<h1 id="Yandex-Cloud" class="common-anchor-header">Yandex Cloud<span class="beta-tag" style="background-color:rgb(0, 179, 255);color:white" translate="no">Compatible with Milvus 2.6.x</span><button data-href="#Yandex-Cloud" class="anchor-icon" translate="no">
    ...
    </button></h1><p>In questa sezione viene descritto come configurare e utilizzare le funzioni di embedding di Yandex Cloud in Milvus.</p>
<h2 id="Choose-an-embedding-model" class="common-anchor-header">Scegliere un modello di embedding<button data-href="#Choose-an-embedding-model" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<h2 id="Configure-credentials" class="common-anchor-header">Configurare le credenziali<button data-href="#Configure-credentials" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Milvus deve conoscere la chiave API di Yandex Cloud prima di poter richiedere gli embedding. È possibile configurare la chiave API in <code translate="no">milvus.yaml</code> o tramite una variabile d’ambiente.</p>
<h3 id="Option-1-Configuration-file" class="common-anchor-header">Opzione 1: File di configurazione<button data-href="#Option-1-Configuration-file" class="anchor-icon" translate="no">
    ...
    </button></h3><p>Salvare la chiave API in <code translate="no">milvus.yaml</code> e indirizzare il provider Yandex Cloud all’etichetta delle credenziali.</p>
...
<h3 id="Option-2-Environment-variable" class="common-anchor-header">Opzione 2: Variabile d’ambiente<button data-href="#Option-2-Environment-variable" class="anchor-icon" translate="no">
    ...
    </button></h3><p>Se in <code translate="no">milvus.yaml</code> non è configurata alcuna credenziale corrispondente, Milvus può leggere la chiave API di Yandex Cloud dalla seguente variabile d'ambiente:</p>
<table>
   <tr>
     <th><p>Variabile</p></th>
   ...
   <tr>
     ...
     <td><p>Sì</p></td>
     <td><p>Chiave API di Yandex Cloud utilizzata dal servizio Milvus per richiamare Yandex Cloud AI Studio.</p></td>
   </tr>
</table>
<h2 id="Use-embedding-function" class="common-anchor-header">Utilizza la funzione di embedding<button data-href="#Use-embedding-function" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>Una volta configurate le credenziali, definire uno schema con un campo di testo in ingresso e un campo vettoriale in uscita, quindi aggiungere una funzione di embedding di Yandex Cloud allo schema.</p>
...
<button class="copy-code-btn"></button></code></pre>
<h3 id="Yandex-Cloud-specific-parameters" class="common-anchor-header">Parametri specifici di Yandex Cloud<button data-href="#Yandex-Cloud-specific-parameters" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
   ...
   <tr>
     ...
     <td><p>Sì</p></td>
     <td><p>Il provider del modello di embedding da utilizzare.</p></td>
     <td><p><code translate="no">"yc"</code></p></td>
   </tr>
   <tr>
     <td><p><code translate="no">model_name</code></p></td>
     <td><p>Sì</p></td>
     <td><p>L'URI del modello Yandex Cloud da chiamare.</p></td>
   ...
   <tr>
     ...
     <td><p>No</p></td>
     <td><p>La dimensione del vettore di output. Se specificato, il valore deve corrispondere alla dimensione del campo del vettore di output.</p></td>
...
<h2 id="Next-steps" class="common-anchor-header">Passi successivi<button data-href="#Next-steps" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Dopo aver configurato la funzione di embedding, consultare <a href="/docs/it/embedding-function-overview.md">la panoramica sulla funzione di embedding</a> per indicazioni su come creare indici, inserire dati ed eseguire la ricerca semantica.</p>
