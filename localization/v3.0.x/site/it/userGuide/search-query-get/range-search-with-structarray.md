...
<tr><td>Ricerca a livello di elemento</td><td>Utilizzare una query vettoriale standard con l'<code translate="no">radius</code> e, facoltativamente, l'<code translate="no">range_filter</code>.</td><td>Livello degli elementi della struttura.</td></tr>
...
<p>Se sono necessari solo gli elementi Struct più vicini, iniziare con <a href="/docs/it/basic-vector-search-with-structarray.md">la ricerca vettoriale di base con StructArray</a>. Utilizzare la ricerca per intervallo quando il risultato deve soddisfare un limite di punteggio o di distanza anziché solo una classifica top-K.</p>
</div>
...
<h2 id="Run-element-level-range-search" class="common-anchor-header">Eseguire una ricerca per intervallo a livello di elemento<button data-href="#Run-element-level-range-search" class="anchor-icon" translate="no">
    ...
    </button></h2><p>L'esempio seguente ricerca singoli blocchi i cui vettori di " <code translate="no">chunks[emb]</code> " siano sufficientemente simili al vettore di query. Ogni risultato trovato rappresenta un elemento Struct corrispondente.</p>
