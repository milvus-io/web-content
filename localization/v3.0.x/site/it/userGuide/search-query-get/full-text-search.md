...
<li><p><strong>Archiviazione nella collezione</strong>: Milvus archivia gli embedding sparsi risultanti in una collezione per un rapido recupero e ordinamento.</p></li>
<li><p><strong>Punteggio di rilevanza BM25</strong>: al momento della ricerca, Milvus applica la funzione di punteggio BM25 per calcolare la rilevanza dei documenti e restituire i risultati ordinati che corrispondono meglio ai termini della query.</p></li>
</ol>
...
<ol>
<li><p><a href="/docs/it/full-text-search.md#Create-a-collection-for-BM25-full-text-search">Creazione di una collezione</a>: configurare i campi richiesti e definire una funzione BM25 che converta il testo grezzo in rappresentazioni sparse.</p></li>
<li><p><a href="/docs/it/full-text-search.md#Insert-text-data">Inserire i dati</a>: importare i documenti di testo grezzo nella collezione.</p></li>
...
<h2 id="Create-a-collection-for-BM25-full-text-search" class="common-anchor-header">Creazione di una raccolta per la ricerca full-text con BM25<button data-href="#Create-a-collection-for-BM25-full-text-search" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<li><p><code translate="no">text</code>: memorizza i dati di testo grezzi per le operazioni di ricerca full-text. Il campo può utilizzare <code translate="no">VARCHAR</code> per testi di dimensioni limitate oppure <code translate="no">TEXT</code> per contenuti sorgente di grandi dimensioni.</p></li>
...
<table>
   ...
   <tr>
     ...
     <td><p>Il nome del campo <code translate="no">VARCHAR</code> o <code translate="no">TEXT</code> per cui è richiesta la conversione da testo a vettore sparso. Per <code translate="no">FunctionType.BM25</code>, questo parametro accetta solo un nome di campo.</p></td>
   </tr>
   <tr>
   ...
   <tr>
     ...
     <td><p>Il tipo della funzione da utilizzare. Deve essere <code translate="no">FunctionType.BM25</code>.</p></td>
   </tr>
</table>
<div class="alert note">
<p>Se più campi di testo richiedono l’elaborazione BM25, definire <strong>una funzione BM25 per ciascun campo</strong>, ciascuna con un nome e un campo di output univoci.</p>
</div>
...
<table>
   ...
   <tr>
     ...
     <td><p>Il nome del campo vettoriale da indicizzare. Per la ricerca full-text, questo dovrebbe essere il campo che memorizza i vettori sparsi generati. In questo esempio, imposta il valore su <code translate="no">sparse</code>.</p></td>
   </tr>
   <tr>
...
<h2 id="Insert-text-data" class="common-anchor-header">Inserire dati di testo<button data-href="#Insert-text-data" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Dopo aver configurato la collezione e l’indice, sei pronto per inserire i dati di testo. In questa fase, devi solo fornire il testo grezzo. La funzione integrata che abbiamo definito in precedenza genera automaticamente il vettore sparso corrispondente per ogni voce di testo.</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#go">   Go</a>
...
<h2 id="Perform-full-text-search" class="common-anchor-header">Eseguire una ricerca full-text<button data-href="#Perform-full-text-search" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Una volta inseriti i dati nella collezione, è possibile eseguire ricerche full-text utilizzando query di testo grezzo. Milvus converte automaticamente la query in un vettore sparso e ordina i risultati della ricerca corrispondenti utilizzando l’algoritmo BM25, per poi restituire i primi K (<code translate="no">limit</code>) risultati.</p>
<div class="alert note">
<p>È possibile evidenziare i termini corrispondenti nei risultati della ricerca configurando uno strumento di evidenziazione del testo. Per ulteriori dettagli, consultare la sezione <a href="/docs/it/text-highlighter.md">Evidenziazione del testo</a>.</p>
</div>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
...
<table>
   ...
   <tr>
     ...
     <td><p>Numero massimo di corrispondenze migliori da restituire.</p></td>
   </tr>
</table>
<h2 id="FAQ" class="common-anchor-header">Domande frequenti<button data-href="#FAQ" class="anchor-icon" translate="no">
    ...
    </button></h2><h3 id="Can-I-output-or-access-the-sparse-vectors-generated-by-the-BM25-function-in-full-text-search" class="common-anchor-header">È possibile generare o accedere ai vettori sparsi generati dalla funzione BM25 nella ricerca full-text?<button data-href="#Can-I-output-or-access-the-sparse-vectors-generated-by-the-BM25-function-in-full-text-search" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<li><p>Questi vettori vengono memorizzati nel campo «sparse», ma non possono essere inclusi in <code translate="no">output_fields</code></p></li>
