...
title: Ricerca tramite chiave primariaCompatible with Milvus 2.6.9+
summary: >-
...
beta: Milvus 2.6.9+
---
<h1 id="Primary-Key-Search" class="common-anchor-header">Ricerca tramite chiave primaria<span class="beta-tag" style="background-color:rgb(0, 179, 255);color:white" translate="no">Compatible with Milvus 2.6.9+</span><button data-href="#Primary-Key-Search" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h1><p>Quando si eseguono ricerche di similarità, viene sempre richiesto di fornire uno o più vettori di query, anche se questi sono già presenti nella raccolta di destinazione. Per evitare di recuperare i vettori prima della ricerca, è possibile utilizzare invece le chiavi primarie.</p>
...
<p>I suggerimenti sono ordinati in base alla loro somiglianza con la parola chiave o con il prodotto corrente. Per ottenere questo risultato, gli sviluppatori della piattaforma devono recuperare da Milvus la rappresentazione vettoriale della parola chiave o del prodotto corrente prima della ricerca di similarità vera e propria; ciò aumenta il numero di scambi di dati tra la piattaforma e Milvus e comporta la trasmissione attraverso la rete di un gran numero di valori in virgola mobile ad alta dimensionalità.</p>
<p>Per semplificare la logica di interazione tra le vostre applicazioni e Milvus, ridurre il numero di scambi di dati ed evitare la trasmissione di grandi quantità di valori in virgola mobile ad alta dimensionalità attraverso la rete, prendete in considerazione l’utilizzo delle ricerche per chiave primaria.</p>
...
<h3 id="Example-1-Basic-primary-key-search" class="common-anchor-header">Esempio 1: Ricerca di base con chiave primaria<button data-href="#Example-1-Basic-primary-key-search" class="anchor-icon" translate="no">
    ...
    </button></h3><p>Per eseguire una ricerca di base tramite chiave primaria, è sufficiente sostituire i vettori di query con le chiavi primarie.</p>
...
<h3 id="Example-4-Grouping-search-using-primary-keys" class="common-anchor-header">Esempio 4: Ricerca per raggruppamento utilizzando le chiavi primarie<button data-href="#Example-4-Grouping-search-using-primary-keys" class="anchor-icon" translate="no">
    ...
    </button></h3><p>L'esempio seguente presuppone c <code translate="no">docId</code> sia un campo definito dallo schema nella collezione di destinazione.</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#javascript">   NodeJS</a>
