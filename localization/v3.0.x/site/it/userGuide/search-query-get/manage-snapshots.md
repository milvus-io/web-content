...
title: Gestione degli snapshotCompatible with Milvus 3.0.x
summary: >-
  Scopri come creare, elencare, descrivere, fissare, ripristinare ed eliminare
  le istantanee e come monitorare i processi di ripristino.
beta: Milvus 3.0.x
---
<h1 id="Manage-Snapshots" class="common-anchor-header">Gestione degli snapshot<span class="beta-tag" style="background-color:rgb(0, 179, 255);color:white" translate="no">Compatible with Milvus 3.0.x</span><button data-href="#Manage-Snapshots" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h1><p>In questa guida imparerai come creare e gestire le istantanee, tra cui</p>
<ul>
<li><a href="#Create-snapshot">Creare uno snapshot</a>,</li>
<li><a href="#List-snapshots">Elencare gli snapshot</a>,</li>
<li><a href="#Describe-snapshot">Descrivere uno snapshot</a>,</li>
<li><a href="#Pinunpin-snapshot-data">Fissare/sbloccare i dati di uno snapshot</a>,</li>
<li><a href="#Restore-snapshot">ripristinare uno snapshot</a>,</li>
<li><a href="#Drop-snapshot">Eliminare uno snapshot</a>,</li>
<li><a href="#List-restoration-jobs">Elencare i processi di ripristino</a> e</li>
<li><a href="#Get-restoration-state">Ottenere lo stato del ripristino</a>.</li>
</ul>
<h2 id="Create-snapshot" class="common-anchor-header">Creare uno snapshot<button data-href="#Create-snapshot" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>Prima di creare uno snapshot, si consiglia di interrompere la scrittura dei dati nella raccolta di destinazione e di chiamare <code translate="no">flush()</code> per evitare possibili perdite di dati.</p>
<div class="alert note">
<p>L'esecuzione di ` <code translate="no">flush()</code> ` non è obbligatoria, ma è altamente raccomandata per evitare la perdita di dati. Se si salta questo passaggio, lo snapshot conterrà solo i dati che sono già stati salvati.</p>
</div>
<p>Quando si assegna un nome a uno snapshot, utilizzare nomi chiari e descrittivi, come <code translate="no">&quot;daily_backup_20240101&quot;</code> o <code translate="no">&quot;v2.1_production_release&quot;</code>, ed evitare termini generici, come <code translate="no">&quot;backup1&quot;</code> e <code translate="no">&quot;test&quot;</code>. Utilizzare i nomi degli snapshot con attenzione per distinguerli tra versioni, ambienti e fasi.</p>
<p>Gli esempi di codice riportati di seguito presuppongono che si disponga già di una raccolta denominata <code translate="no">my_collection</code>.</p>
...
<button class="copy-code-btn"></button></code></pre>
<h2 id="List-snapshots" class="common-anchor-header">Elencare gli snapshot<button data-href="#List-snapshots" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>È possibile elencare i nomi degli snapshot esistenti.</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#go">   Go</a>
...
<button class="copy-code-btn"></button></code></pre>
<h2 id="Describe-snapshot" class="common-anchor-header">Descrizione di uno snapshot<button data-href="#Describe-snapshot" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>È possibile ottenere informazioni dettagliate su uno snapshot specifico.</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#go">   Go</a>
...
<pre><code translate="no" class="language-bash"><span class="hljs-comment"># restful</span>
<button class="copy-code-btn"></button></code></pre>
<h2 id="Pinunpin-snapshot-data" class="common-anchor-header">Bloccare/sbloccare i dati di uno snapshot<button data-href="#Pinunpin-snapshot-data" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>Durante il ripristino, è possibile fissare uno snapshot per proteggere temporaneamente i dati sottostanti dalla garbage collection e sbloccarlo per liberare i dati.</p>
<p>È inoltre possibile impostare una durata (time-to-live, TTL) per l'operazione di blocco, in modo che i dati bloccati vengano rilasciati allo scadere di tale durata.</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#go">   Go</a>
...
<pre><code translate="no" class="language-bash"><span class="hljs-comment"># restful</span>
<button class="copy-code-btn"></button></code></pre>
<h2 id="Restore-snapshot" class="common-anchor-header">Ripristino di uno snapshot<button data-href="#Restore-snapshot" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>È possibile ripristinare uno snapshot in una nuova raccolta. Questa operazione è asincrona e restituisce un ID del processo per monitorare lo stato di avanzamento del ripristino.</p>
<p>Il ripristino utilizza un meccanismo <strong>di copia dei segmenti</strong> anziché l’importazione dei dati, il che risulta più efficiente poiché</p>
<ul>
<li>copia direttamente i file di segmento (binlog, deltalog, file di indice) dall’archivio degli snapshot</li>
<li>conserva gli ID dei campi e gli ID degli indici per garantire la compatibilità con i file di dati esistenti</li>
<li>evita la riscrittura dei dati e la ricostruzione degli indici, con tempi di ripristino notevolmente più rapidi, e</li>
<li>garantisce un aumento delle prestazioni da 10 a 100 volte rispetto ai metodi tradizionali di backup e ripristino</li>
</ul>
<p>Per ripristinare uno snapshot, procedere come segue:</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#go">   Go</a>
...
<pre><code translate="no" class="language-bash"><span class="hljs-comment"># restful</span>
<button class="copy-code-btn"></button></code></pre>
<p>Per ulteriori dettagli sul monitoraggio dello stato di avanzamento di un'operazione di ripristino, consultare la sezione <a href="#Get-restoration-state">Ottenere lo stato del ripristino</a>.</p>
<h2 id="Drop-snapshot" class="common-anchor-header">Eliminazione di uno snapshot<button data-href="#Drop-snapshot" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>È possibile eliminare uno snapshot se non è più necessario. Si consiglia di rimuovere regolarmente gli snapshot obsoleti per risparmiare spazio di archiviazione.</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#go">   Go</a>
...
<pre><code translate="no" class="language-bash"><span class="hljs-comment"># restful</span>
<button class="copy-code-btn"></button></code></pre>
<h2 id="List-restoration-jobs" class="common-anchor-header">Elenco dei processi di ripristino<button data-href="#List-restoration-jobs" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>È possibile utilizzare questa API per ottenere un elenco degli snapshot già creati per la raccolta di destinazione.</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#go">   Go</a>
...
<pre><code translate="no" class="language-bash"><span class="hljs-comment"># restful</span>
<button class="copy-code-btn"></button></code></pre>
<h2 id="Get-restoration-state" class="common-anchor-header">Ottenere lo stato del ripristino<button data-href="#Get-restoration-state" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>Una volta ottenuto l'ID di un'operazione di ripristino, è possibile utilizzarlo per recuperare lo stato di avanzamento del ripristino.</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#go">   Go</a>
