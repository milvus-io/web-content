...
<li><p><strong>Voci restituite:</strong> mantenere il numero massimo configurato di voci di risultato pari o inferiore a 10.000. Calcolare questo massimo come segue:</p>
...
<p>Milvus ignora <code translate="no">limit</code> quando è impostato <code translate="no">search_aggregation</code>. Utilizza il valore di root <code translate="no">SearchAggregation.size</code> per controllare il numero di bucket di primo livello.</p>
...
<tbody>
<tr><td>Alias di una metrica</td><td>Ordina in base a un valore calcolato in <code translate="no">metrics</code> allo stesso livello di aggregazione, come ad esempio <code translate="no">avg_price</code>.</td></tr>
...
└── (Brand C)
<button class="copy-code-btn"></button></code></pre>
<p>Ogni livello può utilizzare più campi in modo indipendente. Ad esempio, l’utilizzo di <code translate="no">fields=[&quot;brand&quot;, &quot;color&quot;]</code> nell’aggregazione figlia creerebbe chiavi figlie composite come <code translate="no">(Brand A, black)</code>.</p>
...
<ul>
<li>Il bucket radice <code translate="no">running_shoes</code> contiene quattro candidati selezionati tra le sue chiavi composte figlie. I suoi <code translate="no">metrics</code> contengono i valori di livello radice <code translate="no">avg_price</code> e <code translate="no">product_count</code>.</li>
