...
summary: >-
  ...
  di documenti in base a termini specifici. Questa funzionalità viene utilizzata
  principalmente per effettuare ricerche filtrate che soddisfino condizioni
...
<p>La corrispondenza testuale si concentra sull’individuazione delle occorrenze esatte dei termini di ricerca, senza valutare la rilevanza dei documenti trovati. Se si desidera recuperare i documenti più rilevanti in base al significato semantico e all’importanza dei termini di ricerca, si consiglia di utilizzare <a href="/docs/it/full-text-search.md">la ricerca full-text</a>.</p>
</div>
...
<h2 id="Enable-text-match" class="common-anchor-header">Abilita la ricerca testuale<button data-href="#Enable-text-match" class="anchor-icon" translate="no">
    ...
    </button></h2><p>La corrispondenza testuale funziona sul <a href="/docs/it/string.md"><code translate="no">VARCHAR</code></a> , che in Milvus corrisponde essenzialmente al tipo di dati stringa. Per abilitare la corrispondenza testuale, impostare sia ` <code translate="no">enable_analyzer</code> ` che ` <code translate="no">enable_match</code> ` su ` <code translate="no">True</code> ` e, facoltativamente, configurare un <a href="/docs/it/analyzer-overview.md">analizzatore</a> per l’analisi testuale durante la definizione dello schema della collezione.</p>
...
<h3 id="Optional-Configure-an-analyzer" class="common-anchor-header">Opzionale: configurare un analizzatore<button data-href="#Optional-Configure-an-analyzer" class="anchor-icon" translate="no">
    ...
    </button></h3><p>Le prestazioni e l’accuratezza della corrispondenza delle parole chiave dipendono dall’analizzatore selezionato. Analizzatori diversi sono ottimizzati per varie lingue e strutture di testo, quindi la scelta di quello giusto può influire in modo significativo sui risultati di ricerca per il vostro caso d’uso specifico.</p>
...
<p>Nei casi in cui sia richiesto un analizzatore diverso, è possibile configurarne uno utilizzando il parametro ` <code translate="no">analyzer_params</code> `. Ad esempio, per applicare l’analizzatore ` <code translate="no">english</code> ` per l’elaborazione di testo in inglese:</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#go">   Go</a>
...
<h2 id="Use-text-match" class="common-anchor-header">Utilizzare la corrispondenza testuale<button data-href="#Use-text-match" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Una volta abilitata la corrispondenza testuale per un campo VARCHAR nello schema della collezione, è possibile eseguire corrispondenze testuali utilizzando l’espressione <code translate="no">TEXT_MATCH</code>.</p>
...
<h3 id="Search-with-text-match" class="common-anchor-header">Ricerca con corrispondenza testuale<button data-href="#Search-with-text-match" class="anchor-icon" translate="no">
    ...
    </button></h3><p>La corrispondenza testuale può essere utilizzata in combinazione con la ricerca per similarità vettoriale per restringere l'ambito della ricerca e migliorare le prestazioni. Filtrando la raccolta tramite la corrispondenza testuale prima della ricerca per similarità vettoriale, è possibile ridurre il numero di documenti da esaminare, con conseguente riduzione dei tempi di elaborazione delle query.</p>
...
<h3 id="Query-with-text-match" class="common-anchor-header">Query con corrispondenza testuale<button data-href="#Query-with-text-match" class="anchor-icon" translate="no">
    ...
    </button></h3><p>La corrispondenza testuale può essere utilizzata anche per il filtraggio scalare nelle operazioni di query. Specificando un’espressione <code translate="no">TEXT_MATCH</code> nel parametro <code translate="no">expr</code> del metodo <code translate="no">query()</code>, è possibile recuperare i documenti che corrispondono ai termini specificati.</p>
