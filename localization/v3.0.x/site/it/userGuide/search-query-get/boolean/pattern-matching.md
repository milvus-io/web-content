...
summary: >-
  Milvus supporta la corrispondenza di stringhe con modelli con caratteri jolly
  LIKE ed espressioni regolari RE2. Utilizza i filtri di corrispondenza per
  individuare prefissi, suffissi, sottostringhe, codici strutturati, domini
  e-mail, percorsi URL e altri modelli di stringhe nei campi VARCHAR, nei
...
<tr><td><code translate="no">ARRAY&lt;VARCHAR&gt;</code> elemento</td><td>Sì</td><td>Sì</td><td>Corrisponde a un elemento specifico in base all’indice, ad esempio <code translate="no">tags[0]</code>. La corrispondenza con il pattern <strong>non</strong> esegue la scansione di tutti gli elementi; si applica solo all’elemento all’indice specificato.</td></tr>
<tr><td>Destinazioni numeriche, booleane, vettoriali, <code translate="no">TEXT</code> o altre destinazioni non<code translate="no">VARCHAR</code> </td><td>No</td><td>No</td><td>La corrispondenza con il pattern è disponibile solo per valori di tipo " <code translate="no">VARCHAR</code> ", percorsi JSON che si risolvono in stringhe o elementi " <code translate="no">ARRAY&lt;VARCHAR&gt;</code> " indicizzati.</td></tr>
</tbody>
</table>
<h2 id="Choose-LIKE-or-regex" class="common-anchor-header">Scegliere LIKE o regex<button data-href="#Choose-LIKE-or-regex" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Scegli l’operatore più semplice che esprima il pattern di cui hai bisogno.</p>
<p>Se è necessaria una corrispondenza esatta della stringa, si consiglia di utilizzare la ricerca esatta ( <code translate="no">==</code> ) anziché la corrispondenza dei pattern. Utilizzare " <code translate="no">LIKE</code> " o "regex" solo quando il filtro deve corrispondere a un pattern.</p>
<table>
<thead>
...
<tr><td>Corrispondenza di pattern senza distinzione tra maiuscole e minuscole</td><td><code translate="no">=~</code> con <code translate="no">(?i)</code></td><td><code translate="no">message =~ &quot;(?i)error&quot;</code></td><td>Trova <code translate="no">error</code>, <code translate="no">ERROR</code> o altre varianti con maiuscole/minuscole.</td></tr>
...
<p>Utilizza <code translate="no">LIKE</code> per una semplice corrispondenza con caratteri jolly. Utilizza l’espressione regolare quando il pattern richiede classi di caratteri, ripetizioni, alternative come <code translate="no">error|failed</code>, ancore o corrispondenze che non tengono conto delle maiuscole e minuscole.</p>
...
<h3 id="Common-LIKE-patterns" class="common-anchor-header">Modelli LIKE comuni<button data-href="#Common-LIKE-patterns" class="anchor-icon" translate="no">
    ...
    </button></h3><p>Utilizzare la posizione di <code translate="no">%</code> e <code translate="no">_</code> per controllare dove appare il testo fisso nella stringa corrispondente.</p>
<table>
<thead>
...
<h2 id="Use-regex--Milvus-30x" class="common-anchor-header">Utilizzare le espressioni regolari<span class="beta-tag" style="background-color:rgb(0, 179, 255);color:white" translate="no">Compatible with Milvus 3.0.x</span><button data-href="#Use-regex--Milvus-30x" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Utilizza i filtri regex quando il pattern richiede funzionalità delle espressioni regolari quali classi di caratteri, ripetizioni, alternanze, ancore o corrispondenze che non distinguono tra maiuscole e minuscole. Milvus applica un'espressione regolare <a href="https://github.com/google/re2/wiki/syntax">RE2</a> a un valore stringa.</p>
<p>Il lato destro di <code translate="no">=~</code> o <code translate="no">!~</code> deve essere una stringa letterale.</p>
<table>
<thead>
...
<h3 id="Use-raw-string-literals" class="common-anchor-header">Utilizzare stringhe letterali "raw"<button data-href="#Use-raw-string-literals" class="anchor-icon" translate="no">
    ...
    </button></h3><p>I letterali stringa raw sono consigliati per i modelli regex che contengono barre rovesciate. In una stringa raw, scritta come <code translate="no">r&quot;...&quot;</code> o <code translate="no">r'...'</code>, le barre rovesciate vengono passate al motore regex alla lettera. Ciò evita l’escape aggiuntivo richiesto dai normali letterali stringa.</p>
<p>Ad esempio:</p>
<div class="multipleCode">
 <a href="#python">Python</a>
 <a href="#java"> Java</a>
...
<button class="copy-code-btn"></button></code></pre>
<p>Questo trova le stringhe che finiscono con <code translate="no">.json</code>, come <code translate="no">report.json</code>.</p>
...
<h3 id="Common-regex-patterns" class="common-anchor-header">Modelli di espressioni regolari comuni<button data-href="#Common-regex-patterns" class="anchor-icon" translate="no">
    ...
    </button></h3><p>Gli esempi seguenti utilizzano la sintassi RE2 comune nelle espressioni dei filtri Milvus. Per la sintassi completa delle espressioni regolari, consultare il riferimento <a href="https://github.com/google/re2/wiki/syntax">alla sintassi RE2</a>.</p>
<table>
<thead>
...
<button class="copy-code-btn"></button></code></pre>
<p>Nota: i filtri regex di Milvus seguono la sintassi RE2. Se un pattern regex utilizza una sintassi non supportata da RE2 o è comunque non valido, Milvus rifiuta l’espressione del filtro. Per i dettagli sui metacaratteri regex, i flag e il comportamento di corrispondenza, consultare il riferimento <a href="https://github.com/google/re2/wiki/syntax">alla sintassi RE2</a>.</p>
