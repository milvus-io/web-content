...
title: Ricerca raggruppata
summary: >-
  Utilizza la ricerca per raggruppamento per aggregare i risultati della ricerca
  ANN in base al valore di un campo e ridurre il numero di entità duplicate.
---
<h1 id="Grouping-Search" class="common-anchor-header">Ricerca raggruppata<button data-href="#Grouping-Search" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h1><p>Una ricerca per raggruppamento consente a Milvus di raggruppare i risultati della ricerca in base ai valori presenti in un campo specificato, al fine di aggregare i dati a un livello superiore. Ad esempio, è possibile utilizzare una ricerca ANN di base per trovare libri simili a quello in esame, ma è possibile utilizzare una ricerca raggruppata per individuare le categorie di libri che potrebbero riguardare gli argomenti trattati in quel libro. Questo argomento descrive come utilizzare la ricerca raggruppata e le considerazioni chiave da tenere presenti.</p>
<h2 id="Overview" class="common-anchor-header">Panoramica<button data-href="#Overview" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Quando le entità nei risultati di ricerca condividono lo stesso valore in un campo scalare, ciò indica che sono simili in un particolare attributo, il che può influire negativamente sui risultati di ricerca.</p>
<p>Supponiamo che una raccolta contenga più documenti (indicati con <strong>docId</strong>). Per conservare quante più informazioni semantiche possibili durante la conversione dei documenti in vettori, ogni documento viene suddiviso in paragrafi (o <strong>blocchi</strong>) più piccoli e gestibili e memorizzato come entità separate. Anche se il documento è suddiviso in sezioni più piccole, gli utenti sono spesso comunque interessati a identificare quali documenti siano più rilevanti per le loro esigenze.</p>
<p><span class="img-wrapper">
   ...
   <span>Ricerca Ann</span>
  
 </span></p>
<p>Quando si esegue una ricerca "Approximate Nearest Neighbor" (ANN) su una collezione di questo tipo, i risultati della ricerca potrebbero includere diversi paragrafi dello stesso documento, con il rischio che altri documenti vengano trascurati, il che potrebbe non essere in linea con il caso d’uso previsto.</p>
<p><span class="img-wrapper">
   ...
   <span>Ricerca raggruppata</span>
  
 </span></p>
<p>Per migliorare la diversità dei risultati di ricerca, è possibile aggiungere il parametro ` <code translate="no">group_by_field</code> ` nella richiesta di ricerca per abilitare la ricerca raggruppata. Come mostrato nel diagramma, è possibile impostare ` <code translate="no">group_by_field</code> ` su ` <code translate="no">docId</code>`. Una volta ricevuta questa richiesta, Milvus:</p>
<ul>
<li><p>Eseguirà una ricerca ANN basata sul vettore di query fornito per individuare tutte le entità più simili alla query.</p></li>
<li><p>Raggrupperà i risultati della ricerca in base al parametro ` <code translate="no">group_by_field</code>` specificato, ad esempio ` <code translate="no">docId</code>`.</p></li>
<li><p>Restituirà i risultati principali per ciascun gruppo, come definito dal parametro <code translate="no">limit</code>, indicando l’entità più simile di ciascun gruppo.</p></li>
</ul>
<div class="alert note">
<p>Per impostazione predefinita, la ricerca raggruppata restituisce solo un'entità per gruppo. Se si desidera aumentare il numero di risultati da restituire per gruppo, è possibile controllarlo tramite i parametri ` <code translate="no">group_size</code> ` e ` <code translate="no">strict_group_size</code> `.</p>
</div>
<h2 id="Perform-Grouping-Search" class="common-anchor-header">Esecuzione della ricerca raggruppata<button data-href="#Perform-Grouping-Search" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>Questa sezione fornisce un codice di esempio per illustrare l’uso della ricerca raggruppata. L’esempio seguente presuppone che la raccolta includa i campi <code translate="no">id</code>, <code translate="no">vector</code>, <code translate="no">chunk</code> e <code translate="no">docId</code>.</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>Nella richiesta sopra riportata, <code translate="no">limit=3</code> indica che il sistema restituirà i risultati della ricerca suddivisi in tre gruppi, ciascuno dei quali conterrà l’entità più simile al vettore di query.</p>
<h2 id="Configure-group-size" class="common-anchor-header">Configurazione della dimensione dei gruppi<button data-href="#Configure-group-size" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>Per impostazione predefinita, la ricerca raggruppata restituisce una sola entità per gruppo. Se si desiderano più risultati per gruppo, modificare i parametri ` <code translate="no">group_size</code> ` e ` <code translate="no">strict_group_size</code> `.</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#go">   Go</a>
...
<button class="copy-code-btn"></button></code></pre>
<p>Nell’esempio sopra:</p>
<ul>
<li><p><code translate="no">group_size</code>: Specifica il numero desiderato di entità da restituire per gruppo. Ad esempio, impostando <code translate="no">group_size=2</code> si indica che ogni gruppo (o ogni <code translate="no">docId</code>) dovrebbe idealmente restituire due dei paragrafi (o <strong>segmenti</strong>) più simili. Se <code translate="no">group_size</code> non è impostato, il sistema restituisce per impostazione predefinita un risultato per gruppo.</p></li>
<li><p><code translate="no">strict_group_size</code>: Questo parametro booleano controlla se il sistema debba applicare rigorosamente il conteggio impostato da <code translate="no">group_size</code>. Quando è <code translate="no">strict_group_size=True</code>, il sistema tenterà di includere in ciascun gruppo il numero esatto di entità specificato da <code translate="no">group_size</code> (ad esempio, due paragrafi), a meno che non vi siano dati sufficienti in quel gruppo. Per impostazione predefinita (<code translate="no">strict_group_size=False</code>), il sistema dà la priorità al rispetto del numero di gruppi specificato dal parametro <code translate="no">limit</code>, piuttosto che garantire che ogni gruppo contenga <code translate="no">group_size</code> entità. Questo approccio è generalmente più efficiente nei casi in cui la distribuzione dei dati sia disomogenea.</p></li>
</ul>
<p>Per ulteriori dettagli sui parametri, consultare <a href="https://docs.zilliz.com/reference/python/python/Vector-search">search</a>.</p>
<h2 id="Order-groups-by-a-scalar-field--Milvus-30x" class="common-anchor-header">Ordinare i gruppi in base a un campo scalare<span class="beta-tag" style="background-color:rgb(0, 179, 255);color:white" translate="no">Compatible with Milvus 3.0.x</span><button data-href="#Order-groups-by-a-scalar-field--Milvus-30x" class="anchor-icon" translate="no">
    ...
    </button></h2><p>È possibile combinare la ricerca raggruppata con <code translate="no">order_by_fields</code> per ordinare i gruppi in base a un campo scalare. Ciò risulta utile quando si desiderano risultati diversi tra i vari gruppi, ma si vuole comunque che i gruppi seguano un ordine rilevante dal punto di vista aziendale, come il prezzo o la valutazione.</p>
<p>L’esempio seguente raggruppa i risultati della ricerca in base a « <code translate="no">category</code> », restituisce fino a tre entità per gruppo e ordina i gruppi restituiti in base a « <code translate="no">price</code> » dal valore più basso a quello più alto.</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#javascript">   NodeJS</a>
...
<button class="copy-code-btn"></button></code></pre>
<p>Nella richiesta sopra riportata, ` <code translate="no">limit=20</code> ` significa che Milvus seleziona fino a 20 gruppi, non 20 entità. Poiché ` <code translate="no">group_size=3</code>`, l’elenco dei risultati in formato piatto può contenere fino a 60 entità in totale.</p>
<p>Quando si utilizza <code translate="no">order_by_fields</code> con <code translate="no">group_by_field</code>, Milvus ordina i gruppi in base al valore del campo scalare specificato dell’entità principale di ciascun gruppo. All’interno di ciascun gruppo, le entità rimangono ordinate in base al loro punteggio di similarità rispetto al vettore di query.</p>
...
<li><p><strong>Indicizzazione</strong>: Questa funzionalità di raggruppamento funziona solo per le collezioni indicizzate con i seguenti tipi di indice: <strong>FLAT</strong>, <strong>IVF_FLAT</strong>, <strong>IVF_SQ8</strong>, <strong>HNSW</strong>, <strong>HNSW_PQ</strong>, <strong>HNSW_PRQ</strong>, <strong>HNSW_SQ</strong>, <strong>DISKANN</strong>, <strong>SPARSE_INVERTED_INDEX</strong>.</p></li>
<li><p><strong>Numero di gruppi</strong>: il parametro ` <code translate="no">limit</code> ` controlla il numero di gruppi da cui vengono restituiti i risultati della ricerca, piuttosto che il numero specifico di entità all’interno di ciascun gruppo. Impostare un valore appropriato per ` <code translate="no">limit</code> ` aiuta a controllare la diversità della ricerca e le prestazioni della query. Ridurre ` <code translate="no">limit</code> ` può ridurre i costi di calcolo se i dati sono distribuiti in modo denso o se le prestazioni sono un fattore critico.</p></li>
<li><p><strong>Entità per gruppo</strong>: il parametro <code translate="no">group_size</code> controlla il numero di entità restituite per gruppo. Regolare il valore di <code translate="no">group_size</code> in base al proprio caso d’uso può aumentare la ricchezza dei risultati di ricerca. Tuttavia, se i dati sono distribuiti in modo non uniforme, alcuni gruppi potrebbero restituire un numero di entità inferiore a quello specificato da <code translate="no">group_size</code>, in particolare in scenari con dati limitati.</p></li>
<li><p><strong>Dimensione rigida del gruppo</strong>: quando si imposta ` <code translate="no">strict_group_size=True</code>`, il sistema tenterà di restituire il numero specificato di entità (`<code translate="no">group_size</code>`) per ciascun gruppo, a meno che non vi siano dati sufficienti in quel gruppo. Questa impostazione garantisce un numero costante di entità per gruppo, ma può comportare un calo delle prestazioni in caso di distribuzione non uniforme dei dati o risorse limitate. Se non è necessario un numero rigido di entità, impostare ` <code translate="no">strict_group_size=False</code> ` può migliorare la velocità della query.</p></li>
<li><p>Se i vettori di query esistono già nella raccolta di destinazione, valutare l’utilizzo di <code translate="no">ids</code> invece di recuperarli prima delle ricerche. Per ulteriori dettagli, consultare <a href="/docs/it/primary-key-search.md">Ricerca per chiave primaria</a>.</p></li>
</ul>
