...
<h3 id="Upsert-in-override-mode" class="common-anchor-header">Upsert in modalità "override"<button data-href="#Upsert-in-override-mode" class="anchor-icon" translate="no">
    ...
    </button></h3><p>Una richiesta di upsert in modalità "override" combina un inserimento e un'eliminazione. Quando viene ricevuta una richiesta di " <code translate="no">upsert</code> " per un'entità esistente, Milvus inserisce i dati contenuti nel payload della richiesta ed elimina contemporaneamente l’entità esistente con la chiave primaria originale specificata nei dati.</p>
...
<p>Se la collezione di destinazione ha l'<code translate="no">autoID</code> abilitato sul proprio campo primario, la richiesta <code translate="no">upsert</code> deve comunque includere la chiave primaria dell’entità di destinazione. Milvus utilizza la chiave primaria fornita per individuare l’entità da sostituire e genera una nuova chiave primaria per i dati contenuti nel payload della richiesta prima di inserirli.</p>
<p>Per i campi con l’ <code translate="no">nullable</code> e abilitato, è possibile ometterli nella richiesta <code translate="no">upsert</code> se non richiedono alcun aggiornamento.</p>
<h3 id="Upsert-in-merge-mode" class="common-anchor-header">Upsert in modalità merge<span class="beta-tag" style="background-color:rgb(0, 179, 255);color:white" translate="no">Compatible with Milvus v2.6.2+</span><button data-href="#Upsert-in-merge-mode" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Utilizzare la modalità di unione per aggiornare campi specifici di un’entità esistente, lasciando invariati gli altri campi.</p>
<p><span class="img-wrapper">
...
<p>Impostare ` <code translate="no">partial_update=True</code> ` e specificare la chiave primaria e i campi che si desidera aggiornare.</p>
<p>Milvus recupera l’entità esistente con una query a forte coerenza, unisce le modifiche ai dati memorizzati, inserisce l’entità risultante dall’unione ed elimina l’entità precedente.</p>
<p>Se la chiave primaria non esiste, il risultato dipende dal fatto che " <code translate="no">autoID</code> " sia abilitato o meno:</p>
<ul>
<li><strong>Con l'<code translate="no">autoID</code> disabilitato</strong>, Milvus tenta di inserire una nuova entità con la chiave primaria fornita. La richiesta va a buon fine se soddisfa i normali requisiti di inserimento. Se manca un campo obbligatorio, la richiesta fallisce con un errore di campo mancante. I campi nullabili e quelli con valori predefiniti possono essere omessi, proprio come in un normale inserimento.</li>
<li><strong>Con l’opzione « <code translate="no">autoID</code> » abilitata</strong>, ogni chiave primaria presente nella richiesta deve già esistere. Milvus rifiuta la richiesta se manca una qualsiasi chiave primaria, anche se vengono forniti tutti i campi necessari per l’inserimento. Per le entità esistenti, la modalità di unione mantiene invariata la chiave primaria.</li>
</ul>
<p>Se un aggiornamento parziale fallisce con un errore di campo mancante, verificare se l’entità di destinazione esiste. In assenza di un’entità esistente, Milvus non può recuperare i valori dei campi omessi.</p>
<p>Per le nuove entità, utilizzare l’ <code translate="no">insert</code> o un upsert in modalità override. Utilizzare la modalità di unione per gli aggiornamenti successivi ai singoli campi.</p>
<p>Per i campi di tipo " <code translate="no">ARRAY</code> ", la modalità di unione supporta due operatori in Milvus v2.6.17 e versioni successive: <code translate="no">ARRAY_APPEND</code> e <code translate="no">ARRAY_REMOVE</code>. Questi operatori consentono di aggiungere elementi o rimuovere elementi corrispondenti da un campo " <code translate="no">ARRAY</code> " esistente, senza dover prima interrogare l'entità per recuperarne il valore corrente. Per ulteriori dettagli, consultare la sezione <a href="/docs/it/upsert-entities.md#Upsert-ARRAY-fields-in-merge-mode">"Upsert dei campi ARRAY in modalità di unione"</a>.</p>
...
<ul>
<li><p>Se si omette il campo ` <code translate="no">issue</code> ` nella richiesta ` <code translate="no">upsert</code> ` e si disabilita ` <code translate="no">partial_update</code>`, il campo ` <code translate="no">issue</code> ` verrà aggiornato a ` <code translate="no">null</code> ` anziché mantenere il proprio valore originale.</p></li>
...
<p>Quando si esegue l'upsert dell'entità con chiavi quali <code translate="no">author</code>, <code translate="no">year</code> o <code translate="no">tags</code>, oppure si aggiungono altre chiavi, tenere presente quanto segue:</p>
<ul>
<li><p>Se si esegue un'operazione "upsert" con <code translate="no">partial_update</code> disabilitato, il comportamento predefinito è la <strong>sovrascrittura</strong>. Ciò significa che il valore del campo dinamico verrà sovrascritto da tutti i campi non definiti dallo schema inclusi nella richiesta e dai relativi valori.</p>
...
<p>Quando si esegue l’upsert del campo <code translate="no">extras</code> di un’entità con dati JSON modificati, si noti che il campo JSON viene trattato come un unico insieme e non è possibile aggiornare singole chiavi in modo selettivo. In altre parole, il campo JSON <strong>NON</strong> supporta l’upsert in modalità <strong>di unione</strong>.</p></li>
<li><p><strong>Eseguire l’upsert di un</strong> <strong>campo</strong> ` <code translate="no">ARRAY</code> <strong>`.</strong></p>
<p>Per impostazione predefinita, un campo <code translate="no">ARRAY</code> in modalità merge segue la semantica <strong>REPLACE</strong>: il valore contenuto nella richiesta sovrascrive l’array esistente. Per aggiornamenti più granulari, Milvus v2.6.17 e versioni successive supportano anche due operatori:</p>
<ul>
<li><p><code translate="no">ARRAY_APPEND</code> aggiunge gli elementi presenti nel payload della richiesta all’array esistente.</p></li>
<li><p><code translate="no">ARRAY_REMOVE</code> rimuove dall’array esistente ogni elemento che corrisponda a un valore presente nel payload della richiesta.</p></li>
</ul>
<p>Per la sintassi degli operatori, i tipi di elementi supportati e altri vincoli, consultare <a href="/docs/it/upsert-entities.md#Upsert-ARRAY-fields-in-merge-mode">Upsert dei campi ARRAY in modalità merge</a>.</p></li>
<li><p><strong>Upsert di un campo StructArray.</strong></p>
<p>L'upsert di un campo StructArray in un'entità sovrascrive il valore del campo. A tal fine, è necessario fornire un elenco di dizionari, ciascuno dei quali contiene tutti i sottocampi definiti nello schema della struttura, anche quando si esegue l'upsert in modalità merge.</p>
<p>Per ulteriori dettagli, consultare <a href="/docs/it/upsert-entities.md#Upsert-StructArray-field-in-merge-mode">Upsert</a> di <a href="/docs/it/upsert-entities.md#Upsert-StructArray-field-in-merge-mode">un campo StructArray in modalità merge</a>.</p></li>
</ul>
...
<ul>
<li><p>La richiesta " <code translate="no">upsert</code> " deve sempre includere le chiavi primarie delle entità di destinazione, anche quando è abilitata l'opzione " <code translate="no">autoID</code> ". Per le collezioni " <code translate="no">autoID</code> ", la gestione delle chiavi primarie dipende dalla modalità di upsert:</p>
<ul>
<li><p>In modalità "override", la chiave primaria identifica l’entità esistente da sostituire e Milvus genera una nuova chiave primaria per l’entità sostitutiva.</p></li>
<li><p>In modalità "merge", la chiave primaria identifica l'entità esistente da aggiornare e rimane invariata. Se la chiave primaria non esiste, la richiesta fallisce invece di inserire una nuova entità.</p></li>
</ul></li>
...
<h2 id="Upsert-entities-in-a-collection" class="common-anchor-header">Upsert di entità in una raccolta<button data-href="#Upsert-entities-in-a-collection" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<p>Le tre entità, se presenti nella raccolta, verranno sovrascritte da quelle incluse nella richiesta di upsert.</p>
...
<h2 id="Upsert-entities-in-a-partition" class="common-anchor-header">Upsert di entità in una partizione<button data-href="#Upsert-entities-in-a-partition" class="anchor-icon" translate="no">
    ...
    </button></h2><p>È inoltre possibile eseguire l'upsert di entità in una partizione specificata. I seguenti frammenti di codice presuppongono che nella propria collezione sia presente una partizione denominata <strong>PartitionA</strong>.</p>
...
<h2 id="Upsert-entities-in-merge-mode" class="common-anchor-header">Inserimento o aggiornamento di entità in modalità merge<span class="beta-tag" style="background-color:rgb(0, 179, 255);color:white" translate="no">Compatible with Milvus v2.6.2+</span><button data-href="#Upsert-entities-in-merge-mode" class="anchor-icon" translate="no">
    ...
    </button></h2><p>L'esempio seguente aggiorna solo il campo ` <code translate="no">issue</code> ` delle entità con chiavi primarie ` <code translate="no">1</code> ` e ` <code translate="no">2</code> ` in ` <code translate="no">my_collection</code>`. Prima di eseguirlo, assicurarsi che entrambe le entità esistano già. Gli altri campi mantengono i valori attuali.</p>
...
<h2 id="Upsert-ARRAY-fields-in-merge-mode" class="common-anchor-header">Upsert dei campi ARRAY in modalità merge<span class="beta-tag" style="background-color:rgb(0, 179, 255);color:white" translate="no">Compatible with Milvus 2.6.17+</span><button data-href="#Upsert-ARRAY-fields-in-merge-mode" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Prima di Milvus v2.6.17, l’aggiornamento di una parte di un campo di tipo « <code translate="no">ARRAY</code> » richiedeva un flusso di lettura-modifica-scrittura lato client: interrogare l’array esistente, modificarlo nel codice dell’applicazione ed eseguire l’upsert del valore di sostituzione completo. Gli operatori di aggiornamento parziale (<code translate="no">ARRAY_APPEND</code> e <code translate="no">ARRAY_REMOVE</code>) consentono di inviare solo gli elementi da aggiungere o rimuovere, riducendo così la logica lato client ed evitando la lettura aggiuntiva prima dell’upsert.</p>
<p>Supponiamo che l’entità con chiave primaria <code translate="no">1</code> contenga già <code translate="no">tags = [&quot;new&quot;, &quot;trial&quot;]</code>. Prima dell’introduzione degli operatori di aggiornamento parziale, l’aggiunta dell’elemento <code translate="no">&quot;premium&quot;</code> a un array richiedeva l’inserimento o l’aggiornamento dell’intero array sostitutivo:</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#javascript">   NodeJS</a>
...
<pre><code translate="no" class="language-bash"><span class="hljs-comment"># restful</span>
<button class="copy-code-btn"></button></code></pre>
<p>Con ` <code translate="no">ARRAY_REMOVE</code>`, è sufficiente inviare solo l'elemento corrispondente da rimuovere:</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#javascript">   NodeJS</a>
...
<button class="copy-code-btn"></button></code></pre>
<div class="alert note">
<p>L'associazione di uno dei due operatori a un campo tramite <code translate="no">field_ops</code> abilita implicitamente la semantica dell'aggiornamento parziale. Pertanto, <strong>non</strong> è necessario passare <code translate="no">partial_update=True</code> insieme a <code translate="no">field_ops</code>.</p>
</div>
...
<li><p>I valori del payload devono corrispondere all’ <code translate="no">element_type</code> del campo di destinazione <code translate="no">ARRAY</code>. Ad esempio, se il campo di destinazione è <code translate="no">ARRAY&lt;VARCHAR&gt;</code>, il payload deve contenere valori di tipo stringa.</p></li>
<li><p>In Milvus v2.6.17 e versioni successive, <code translate="no">ARRAY_APPEND</code> e <code translate="no">ARRAY_REMOVE</code> supportano campi <code translate="no">ARRAY</code> il cui <code translate="no">element_type</code> è <code translate="no">BOOL</code>, <code translate="no">INT8</code>, <code translate="no">INT16</code>, <code translate="no">INT32</code>, <code translate="no">INT64</code>, <code translate="no">FLOAT</code>, <code translate="no">DOUBLE</code> o <code translate="no">VARCHAR</code>.</p></li>
<li><p>Dopo un’operazione di tipo “ <code translate="no">ARRAY_APPEND</code> ”, la lunghezza dell’array risultante non deve superare il valore specificato nel campo “ <code translate="no">max_capacity</code> ”.</p></li>
<li><p>Gli upsert simultanei sulla stessa entità non sono atomici tra le richieste. Se due richieste aggiornano contemporaneamente lo stesso campo <code translate="no">ARRAY</code>, la scrittura successiva può sovrascrivere quella precedente. Utilizzare il coordinamento a livello di applicazione se è necessario preservare tutte le modifiche simultanee.</p></li>
</ul>
...
<pre><code translate="no" class="language-bash"><span class="hljs-comment"># restful</span>
<button class="copy-code-btn"></button></code></pre>
<h2 id="Upsert-StructArray-field-in-merge-mode" class="common-anchor-header">Upsert del campo StructArray in modalità merge<button data-href="#Upsert-StructArray-field-in-merge-mode" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>L'upsert di un campo StructArray in un'entità sovrascrive il valore del campo. Ciò significa che è necessario includere tutti i sottocampi definiti nello schema della struttura quando si esegue l'upsert di un campo StructArray.</p>
<p>L'esempio seguente mostra come eseguire l'`upsert` del campo ` <code translate="no">chunks</code> ` in modalità merge, un campo StructArray con 6 sottocampi. Al termine dell'operazione, il campo ` <code translate="no">chunks</code> ` dell'entità con id 1 viene impostato sull'array contenente le strutture a due elementi fornite nella richiesta.</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#javascript">   NodeJS</a>
