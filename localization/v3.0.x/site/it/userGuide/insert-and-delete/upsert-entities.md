...
summary: >-
  L'operazione "upsert" offre un modo pratico per inserire o aggiornare le
  entità in una raccolta.
---
...
<h3 id="Upsert-in-override-mode" class="common-anchor-header">Upsert in modalità "override"<button data-href="#Upsert-in-override-mode" class="anchor-icon" translate="no">
    ...
    </button></h3><p>Una richiesta di upsert in modalità "override" combina un'operazione di inserimento e una di eliminazione. Quando viene ricevuta una richiesta di " <code translate="no">upsert</code> " per un'entità esistente, Milvus inserisce i dati contenuti nel payload della richiesta ed elimina contemporaneamente l'entità esistente con la chiave primaria originale specificata nei dati.</p>
...
<h3 id="Upsert-in-merge-mode--Milvus-v262+" class="common-anchor-header">Upsert in modalità di unione<span class="beta-tag" style="background-color:rgb(0, 179, 255);color:white" translate="no">Compatible with Milvus v2.6.2+</span><button data-href="#Upsert-in-merge-mode--Milvus-v262+" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<p>Imposta l'<code translate="no">partial_update=True</code> e e specifica la chiave primaria e i campi che desideri aggiornare.</p>
...
<p>Se la chiave primaria non esiste, il risultato dipende dal fatto che la modalità " <code translate="no">autoID</code> " sia abilitata o meno:</p>
<ul>
...
<li><strong>Con l’opzione « <code translate="no">autoID</code> » abilitata</strong>, ogni chiave primaria nella richiesta deve già esistere. Milvus rifiuta la richiesta se manca una qualsiasi chiave primaria, anche se si forniscono tutti i campi richiesti per l’inserimento. Per le entità esistenti, la modalità di unione mantiene invariata la chiave primaria.</li>
</ul>
...
<ul>
<li><p>Se si omette il campo ` <code translate="no">issue</code> ` nella richiesta ` <code translate="no">upsert</code> ` e si disabilita ` <code translate="no">partial_update</code>`, il campo ` <code translate="no">issue</code> ` verrà aggiornato a ` <code translate="no">null</code> ` invece di mantenere il proprio valore originale.</p></li>
...
<p>Quando si esegue un upsert dell’entità con chiavi quali <code translate="no">author</code>, <code translate="no">year</code> o <code translate="no">tags</code>, oppure si aggiungono altre chiavi, tenere presente che:</p>
<ul>
<li><p>Se si esegue un'operazione di upsert con l'opzione " <code translate="no">partial_update</code> " disabilitata, il comportamento predefinito è la <strong>sovrascrittura</strong>. Ciò significa che il valore del campo dinamico verrà sovrascritto da tutti i campi non definiti dallo schema inclusi nella richiesta e dai relativi valori.</p>
...
<li><p>Se si esegue un upsert con " <code translate="no">partial_update</code> " abilitato, il comportamento predefinito è la <strong>fusione</strong>. Ciò significa che il valore del campo dinamico si fonderà con tutti i campi non definiti dallo schema inclusi nella richiesta e con i relativi valori.</p>
...
<li><p>Esegu<strong>ire l’upsert di un</strong> <strong>campo</strong> ` <code translate="no">ARRAY</code> <strong>`.</strong></p>
...
<li><p><code translate="no">ARRAY_REMOVE</code> rimuove dall’array esistente ogni elemento che corrisponda a un valore nel payload della richiesta.</p></li>
</ul>
...
<p>Per ulteriori dettagli, consultare <a href="/docs/it/upsert-entities.md#Upsert-StructArray-field-in-merge-mode">Upsert</a> del <a href="/docs/it/upsert-entities.md#Upsert-StructArray-field-in-merge-mode">campo StructArray in modalità di unione</a>.</p></li>
</ul>
...
<h2 id="Upsert-entities-in-merge-mode--Milvus-v262+" class="common-anchor-header">Inserimento o aggiornamento di entità in modalità merge<span class="beta-tag" style="background-color:rgb(0, 179, 255);color:white" translate="no">Compatible with Milvus v2.6.2+</span><button data-href="#Upsert-entities-in-merge-mode--Milvus-v262+" class="anchor-icon" translate="no">
    ...
    </button></h2><p>L'esempio seguente aggiorna solo il campo ` <code translate="no">issue</code> ` delle entità con chiavi primarie ` <code translate="no">1</code> ` e ` <code translate="no">2</code> ` presenti in ` <code translate="no">my_collection</code>`. Prima di eseguirlo, assicurarsi che entrambe le entità esistano già. Gli altri campi mantengono i valori attuali.</p>
<div class="alert note">
<p>Quando si esegue un upsert in modalità merge, assicurarsi che le entità coinvolte nella richiesta abbiano lo stesso insieme di campi. Supponendo che vi siano due o più entità da sottoporre a upsert, come mostrato nel seguente frammento di codice, è importante che includano campi identici per prevenire errori e mantenere l’integrità dei dati.</p>
</div>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
...
<pre><code translate="no" class="language-bash"><span class="hljs-comment"># restful</span>
<button class="copy-code-btn"></button></code></pre>
<p>Con ` <code translate="no">ARRAY_REMOVE</code>`, invia solo l'elemento corrispondente da rimuovere:</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#javascript">   NodeJS</a>
...
<li><p>I valori del payload devono corrispondere all’ <code translate="no">element_type</code> del campo <code translate="no">ARRAY</code> di destinazione. Ad esempio, se il campo di destinazione è <code translate="no">ARRAY&lt;VARCHAR&gt;</code>, il payload deve contenere valori di tipo stringa.</p></li>
...
<li><p>Dopo un’operazione di tipo “ <code translate="no">ARRAY_APPEND</code> ”, la lunghezza dell’array risultante non deve superare il valore specificato in “ <code translate="no">max_capacity</code> ” del campo.</p></li>
