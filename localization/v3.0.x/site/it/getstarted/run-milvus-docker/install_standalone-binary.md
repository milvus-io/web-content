...
summary: >-
  Scopri come installare Milvus in modalità standalone utilizzando un pacchetto
  RPM/DEB precompilato.
title: Installazione di Milvus in modalità standalone con pacchetto RPM/DEB
---
<h1 id="Install-Milvus-Standalone-with-RPMDEB-Package" class="common-anchor-header">Installazione di Milvus in modalità standalone con pacchetto RPM/DEB<button data-href="#Install-Milvus-Standalone-with-RPMDEB-Package" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h1><p>Questa pagina illustra come installare Milvus in modalità standalone utilizzando un pacchetto RPM/DEB precompilato.</p>
...
<h2 id="Download-the-RPMDEB-Package" class="common-anchor-header">Scaricare il pacchetto RPM/DEB<button data-href="#Download-the-RPMDEB-Package" class="anchor-icon" translate="no">
    ...
    </button></h2><p>È possibile scaricare il pacchetto RPM/DEB in base all'architettura del proprio sistema dalla <a href="https://github.com/milvus-io/milvus/releases/tag/v3.0.2">pagina delle versioni di Milvus</a>.</p>
<ul>
<li>Per x86_64/amd64, scaricare il pacchetto <strong>milvus_3.0.2-1_amd64.deb</strong> o <strong>milvus_3.0.2-1_amd64.rpm</strong>.</li>
<li>Per ARM64, scaricare il pacchetto <strong>milvus_3.0.2-1_arm64.deb</strong> o <strong>milvus_3.0.2-1_arm64.rpm</strong>.</li>
</ul>
...
<h2 id="Start-Milvus-Standalone" class="common-anchor-header">Avvio di Milvus in modalità standalone<button data-href="#Start-Milvus-Standalone" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Una volta completata l’installazione, Milvus viene installato come servizio systemd e può essere avviato utilizzando il seguente comando:</p>
<pre><code translate="no" class="language-shell">systemctl start milvus
<button class="copy-code-btn"></button></code></pre>
<p>È possibile verificare lo stato del servizio Milvus utilizzando il seguente comando:</p>
<pre><code translate="no" class="language-shell">systemctl status milvus
<button class="copy-code-btn"></button></code></pre>
<p>Se Milvus è in esecuzione correttamente, dovrebbe apparire il seguente output:</p>
...
<h2 id="Optional-Update-Milvus-configurations" class="common-anchor-header">(Facoltativo) Aggiornamento delle configurazioni di Milvus<button data-href="#Optional-Update-Milvus-configurations" class="anchor-icon" translate="no">
    ...
    </button></h2><p>È possibile modificare le configurazioni di Milvus nel file <code translate="no">/etc/milvus/configs/milvus.yaml</code>. Ad esempio, per cambiare il servizio <code translate="no">proxy.healthCheckTimeout</code> in <code translate="no">1000</code>, è possibile cercare il parametro target e modificarlo di conseguenza. Per le voci di configurazione applicabili, fare riferimento a <a href="/docs/it/system_configuration.md">Configurazione di sistema</a>.</p>
<h2 id="Stop-Milvus-Standalone" class="common-anchor-header">Arrestare Milvus Standalone<button data-href="#Stop-Milvus-Standalone" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Per arrestare Milvus Standalone, è possibile utilizzare il seguente comando:</p>
<pre><code translate="no" class="language-shell">systemctl stop milvus
<button class="copy-code-btn"></button></code></pre>
<h2 id="Uninstall-Milvus-Standalone" class="common-anchor-header">Disinstallare Milvus Standalone<button data-href="#Uninstall-Milvus-Standalone" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>Per disinstallare Milvus Standalone, è possibile utilizzare il seguente comando:</p>
...
<h2 id="Whats-next" class="common-anchor-header">Passi successivi<button data-href="#Whats-next" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
