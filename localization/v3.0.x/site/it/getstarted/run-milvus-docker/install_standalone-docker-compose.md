...
<h2 id="Install-Milvus" class="common-anchor-header">Installare Milvus<button data-href="#Install-Milvus" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Milvus fornisce un file di configurazione Docker Compose nel repository di Milvus. Per installare Milvus utilizzando Docker Compose, è sufficiente eseguire</p>
...
<p><strong>Distribuzione predefinita (v3.0.1):</strong> <code translate="no">docker compose up -d</code> avvia tre container: <code translate="no">milvus-etcd</code> (metadati), <code translate="no">milvus-minio</code> (archiviazione a oggetti) e <code translate="no">milvus-standalone</code>. La coda dei messaggi è <strong>Woodpecker (integrata, con MinIO / archiviazione a oggetti come backend WAL)</strong>, quindi non è necessario un container separato <strong>per</strong> la coda dei messaggi.</p>
...
<p>Scarica sempre l'ultima configurazione di Docker Compose per garantire la compatibilità con le funzionalità della versione 3.0.1.</p>
<ul>
<li><p>Se non sei riuscito a eseguire il comando sopra indicato, verifica se sul tuo sistema è installata la versione 1 di Docker Compose. In tal caso, ti consigliamo di migrare a Docker Compose V2 in base alle indicazioni riportate in <a href="https://docs.docker.com/compose/">questa pagina</a>.</p></li>
...
<h2 id="Upgrading-from-Milvus-25x-to-26x" class="common-anchor-header">Aggiornamento da Milvus 2.5.x a 2.6.x<button data-href="#Upgrading-from-Milvus-25x-to-26x" class="anchor-icon" translate="no">
    ...
    </button></h2><p><strong>Limiti della coda dei messaggi</strong>: durante l’aggiornamento a Milvus v3.0.1, è necessario mantenere la coda dei messaggi attualmente in uso. Il passaggio tra diversi sistemi di code dei messaggi durante l’aggiornamento non è supportato. Il supporto per la modifica dei sistemi di code dei messaggi sarà disponibile nelle versioni future.</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>Per cambiare la coda dei messaggi <em>dopo</em> l’aggiornamento, consultare la sezione " <a href="/docs/it/switch-mq-type.md">Cambio della coda dei messaggi</a>".</p>
