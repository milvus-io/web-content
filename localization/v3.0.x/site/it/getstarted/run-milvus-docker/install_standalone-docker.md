...
<p><strong>Novità della versione 3.0.1:</strong></p>
<ul>
<li><strong>Nodo di streaming</strong>: funzionalità di elaborazione dei dati migliorate</li>
<li><strong>Woodpecker MQ (impostazione predefinita)</strong>: questa distribuzione Docker utilizza Woodpecker come coda dei messaggi con il <strong>filesystem locale</strong> come backend WAL, quindi non è richiesto alcun servizio esterno di coda dei messaggi. Vedi <a href="/docs/it/woodpecker.md">Woodpecker</a>.</li>
...
<p>Se si riscontrano problemi durante il recupero dell’immagine, contattarci all’indirizzo <a href="mailto:community@zilliz.com">community@zilliz.com</a> fornendo i dettagli del problema e forniremo il supporto necessario.</p>
</div>
...
<li>Insieme a Milvus, nello stesso container è installato un etcd integrato, attivo sulla porta <strong>2379</strong>. Il suo file di configurazione è mappato su <strong>embedEtcd.yaml</strong> nella cartella corrente.</li>
<li>Per modificare la configurazione predefinita di Milvus, aggiungi le tue impostazioni al file <strong>user.yaml</strong> nella cartella corrente e poi riavvia il servizio.</li>
<li>Il volume dati di Milvus è mappato su <strong>volumes/milvus</strong> nella cartella corrente.</li>
</ul>
<p>È possibile accedere all’interfaccia Web di Milvus all’indirizzo <code translate="no">http://127.0.0.1:9091/webui/</code> per ottenere ulteriori informazioni sull’istanza di Milvus in uso. Per i dettagli, consultare la documentazione relativa <a href="/docs/it/milvus-webui.md">all’interfaccia Web di Milvus</a>.</p>
<h2 id="Optional-Update-Milvus-configurations" class="common-anchor-header">(Facoltativo) Aggiornamento delle configurazioni di Milvus<button data-href="#Optional-Update-Milvus-configurations" class="anchor-icon" translate="no">
    ...
    </button></h2><p>È possibile modificare le configurazioni di Milvus nel file <strong>user.yaml</strong> nella cartella corrente. Ad esempio, per modificare l'URL da <code translate="no">proxy.healthCheckTimeout</code> a <code translate="no">1000</code>, è possibile modificare il file come segue:</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>Per le voci di configurazione applicabili, consultare <a href="/docs/it/system_configuration.md">Configurazione di sistema</a>.</p>
