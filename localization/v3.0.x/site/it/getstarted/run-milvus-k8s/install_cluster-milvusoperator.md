...
<h2 id="Overview" class="common-anchor-header">Panoramica<button data-href="#Overview" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Milvus Operator è una soluzione che consente di distribuire e gestire uno stack completo di servizi Milvus su cluster Kubernetes (K8s). Lo stack include tutti i componenti di Milvus e le dipendenze correlate, come etcd e MinIO, con Woodpecker come coda di messaggi integrata.</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>Il comando sopra riportato distribuisce un cluster Milvus con <strong>Woodpecker</strong> come coda di messaggi (consigliato per la versione 3.0.2) e tutti i nuovi componenti architetturali, incluso lo Streaming Node.</p>
<p><strong>Caratteristiche principali dell’architettura in questa distribuzione:</strong></p>
<ul>
<li><strong>Coda dei messaggi</strong>: <a href="/docs/it/woodpecker.md">utilizza Woodpecker</a> (riduce la manutenzione dell’infrastruttura)</li>
...
<h3 id="2-Check-Milvus-cluster-status" class="common-anchor-header">2. Verifica dello stato del cluster Milvus<button data-href="#2-Check-Milvus-cluster-status" class="anchor-icon" translate="no">
    ...
    </button></h3><p>Esegui il seguente comando per verificare lo stato del cluster Milvus</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>Una volta che il cluster Milvus è pronto, l’output del comando sopra riportato dovrebbe essere simile a quanto segue. Se il campo " <code translate="no">status.status</code> " rimane " <code translate="no">Unhealthy</code>", il cluster Milvus è ancora in fase di creazione.</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>Milvus Operator crea le dipendenze di Milvus, come etcd e MinIO, e successivamente i componenti di Milvus, quali proxy, coordinatori e nodi.</p>
...
<h3 id="3-Forward-a-local-port-to-Milvus" class="common-anchor-header">3. Reindirizzare una porta locale a Milvus<button data-href="#3-Forward-a-local-port-to-Milvus" class="anchor-icon" translate="no">
    ...
    </button></h3><p>Esegui il seguente comando per ottenere la porta su cui è in esecuzione il tuo cluster Milvus.</p>
...
<div class="alert note">
<p>Se hai distribuito Milvus in modalità standalone, modifica il nome del pod da <code translate="no">my-release-milvus-proxy-xxxxxxxxxx-xxxxx</code> a <code translate="no">my-release-milvus-xxxxxxxxxx-xxxxx</code>.</p>
</div>
...
<button class="copy-code-btn"></button></code></pre>
<p>Facoltativamente, è possibile utilizzare <code translate="no">:19530</code> al posto di <code translate="no">27017:19530</code> nel comando sopra riportato per consentire a <code translate="no">kubectl</code> di allocare una porta locale automaticamente, in modo da non dover gestire eventuali conflitti di porte.</p>
...
<h2 id="Optional-Update-Milvus-configurations" class="common-anchor-header">(Facoltativo) Aggiornamento delle configurazioni di Milvus<button data-href="#Optional-Update-Milvus-configurations" class="anchor-icon" translate="no">
    ...
    </button></h2><p>È possibile visualizzare e aggiornare le configurazioni del proprio cluster Milvus richiamando il comando ` <code translate="no">patch</code> ` come segue:</p>
<ol>
...
<button class="copy-code-btn"></button></code></pre>
<p>Per le voci di configurazione applicabili, fare riferimento a <a href="/docs/it/system_configuration.md">Configurazione di sistema</a>.</p></li>
...
<div class="alert note">
<ul>
<li>Quando si elimina il cluster Milvus utilizzando la configurazione predefinita, le dipendenze come etcd e MinIO non vengono eliminate. Pertanto, la prossima volta che si installerà la stessa istanza del cluster Milvus, queste dipendenze verranno riutilizzate.</li>
...
<h2 id="Optional-dependencies" class="common-anchor-header">Dipendenze opzionali<button data-href="#Optional-dependencies" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Questa distribuzione utilizza <strong>Woodpecker</strong> come coda di messaggi, <strong>etcd</strong> per i metadati e <strong>MinIO</strong> per l'archiviazione degli oggetti, tutti forniti da Milvus Operator. Per utilizzare un backend diverso con l'Operator, consultare:</p>
<ul>
<li>Coda di messaggi: <a href="/docs/it/woodpecker.md">Woodpecker</a> (impostazione predefinita) · <a href="/docs/it/message_storage_operator.md#Configure-Pulsar">Pulsar</a> · <a href="/docs/it/message_storage_operator.md#Configure-Kafka">Kafka</a> · <a href="/docs/it/message_storage_operator.md#Configure-RocksMQ">RocksMQ</a></li>
<li>Archiviazione a oggetti: <a href="/docs/it/object_storage_operator.md">Configurare l’archiviazione a oggetti con Milvus Operator</a></li>
<li>Metadati: <a href="/docs/it/meta_storage_operator.md">Configurare etcd con Milvus Operator</a></li>
</ul>
<div class="alert note">
