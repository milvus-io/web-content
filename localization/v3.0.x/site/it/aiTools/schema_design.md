...
<h1 id="Schema-Design" class="common-anchor-header">Progettazione dello schema<button data-href="#Schema-Design" class="anchor-icon" translate="no">
    ...
    </button></h1><p>Regole e linee guida per la progettazione di schemi di raccolta Milvus corretti, inclusi i tipi di campo, le chiavi primarie, la configurazione BM25 e i vincoli di immutabilità dello schema. Copia il prompt completo riportato di seguito nel tuo strumento di IA per applicare automaticamente queste regole. Per una panoramica di tutti i prompt, consulta <a href="/docs/it/milvus_for_agents.md">Prompt IA</a>.</p>
...
<p>Per gli utenti <strong>di Cursor</strong>: copia il prompt dalla sezione " <a href="#full-prompt">Prompt completo</a> " e salvalo in " <code translate="no">.cursor/rules/</code> " all’interno del tuo progetto.</p>
