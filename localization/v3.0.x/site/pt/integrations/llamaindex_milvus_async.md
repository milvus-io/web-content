...
title: Criar um RAG com o LlamaIndex e a API assíncrona do Milvus
related_key: LlamaIndex
summary: >-
  Este tutorial demonstra como utilizar o LlamaIndex com o Milvus para criar um
  pipeline de processamento assíncrono de documentos para RAG. O LlamaIndex
  permite processar documentos e armazená-los numa base de dados vetorial, como
  o Milvus. Ao tirar partido da API assíncrona do LlamaIndex e da biblioteca
  cliente Python do Milvus, podemos aumentar a capacidade de processamento do
  pipeline para processar e indexar eficientemente grandes volumes de dados.
---
<h1 id="RAG-with-Milvus-and-LlamaIndex-Async-API" class="common-anchor-header">RAG com o Milvus e a API assíncrona do LlamaIndex<button data-href="#RAG-with-Milvus-and-LlamaIndex-Async-API" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<p>Este tutorial demonstra como utilizar <a href="https://www.llamaindex.ai/">o LlamaIndex</a> com <a href="https://milvus.io/">o Milvus</a> para criar um pipeline de processamento assíncrono de documentos para RAG. O LlamaIndex permite processar documentos e armazená-los numa base de dados vetorial, como o Milvus. Ao tirar partido da API assíncrona do LlamaIndex e da biblioteca cliente Python do Milvus, podemos aumentar a capacidade de processamento do pipeline para processar e indexar eficientemente grandes volumes de dados.</p>
<p>Neste tutorial, iremos primeiro apresentar, a um nível geral, a utilização de métodos assíncronos para criar um RAG com o LlamaIndex e o Milvus e, em seguida, abordar a utilização de métodos de baixo nível e a comparação de desempenho entre os métodos síncronos e assíncronos.</p>
<h2 id="Before-you-begin" class="common-anchor-header">Antes de começar<button data-href="#Before-you-begin" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Os trechos de código nesta página requerem as dependências pymilvus e llamaindex. Pode instalá-las utilizando os seguintes comandos:</p>
...
<p>Iremos utilizar os modelos da OpenAI. Deve preparar a <a href="https://platform.openai.com/docs/quickstart">chave API</a> <code translate="no">OPENAI_API_KEY</code> como variável de ambiente.</p>
<pre><code translate="no" class="language-python"><span class="hljs-keyword">import</span> os

os.environ[<span class="hljs-string">&quot;OPENAI_API_KEY&quot;</span>] = <span class="hljs-string">&quot;sk-***********&quot;</span>
<button class="copy-code-btn"></button></code></pre>
<p>Se estiver a utilizar o Jupyter Notebook, terá de executar esta linha de código antes de executar o código assíncrono.</p>
<pre><code translate="no" class="language-python"><span class="hljs-keyword">import</span> nest_asyncio

nest_asyncio.apply()
<button class="copy-code-btn"></button></code></pre>
<h3 id="Prepare-data" class="common-anchor-header">Preparar os dados<button data-href="#Prepare-data" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Pode descarregar dados de exemplo com os seguintes comandos:</p>
...
<button class="copy-code-btn"></button></code></pre>
<h2 id="Build-RAG-with-Asynchronous-Processing" class="common-anchor-header">Criar um RAG com processamento assíncrono<button data-href="#Build-RAG-with-Asynchronous-Processing" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>Esta secção mostra como criar um sistema RAG capaz de processar documentos de forma assíncrona.</p>
...
<div class="alert note">
<ul>
<li>Se tiver uma grande quantidade de dados, pode configurar um servidor Milvus de alto desempenho no <a href="https://milvus.io/docs/quickstart.md">Docker ou</a> no <a href="https://milvus.io/docs/quickstart.md">Kubernetes</a>. Nesta configuração, utilize o URI do servidor, por exemplo,<code translate="no">http://localhost:19530</code>, como o seu <code translate="no">uri</code>.</li>
...
<li>No caso de sistemas complexos (como a comunicação em rede), o processamento assíncrono pode trazer uma melhoria de desempenho em comparação com a sincronização. Por isso, consideramos que o Milvus-Lite não é adequado para a utilização de interfaces assíncronas, uma vez que os cenários utilizados não são adequados.</li>
</ul>
</div>
<p>Defina uma função de inicialização que possamos utilizar novamente para reconstruir a coleção do Milvus.</p>
...
<p>Utilize o SimpleDirectoryReader para encapsular um objeto de documento LlamaIndex a partir do ficheiro <code translate="no">paul_graham_essay.txt</code>.</p>
...
<p>Instancie um modelo de embedding do Hugging Face localmente. A utilização de um modelo local evita o risco de atingir os limites de taxa da API durante a inserção assíncrona de dados, uma vez que os pedidos simultâneos à API podem acumular-se rapidamente e esgotar o seu orçamento na API pública. No entanto, se tiver um limite de taxa elevado, pode optar por utilizar um serviço de modelo remoto.</p>
...
<p>Definimos o parâmetro ` <code translate="no">use_async</code> ` para ` <code translate="no">True</code> ` para ativar o modo de inserção assíncrona.</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>Ao criar o motor de pesquisa, também pode definir o parâmetro « <code translate="no">use_async</code> » como « <code translate="no">True</code> » para ativar a pesquisa assíncrona.</p>
...
<h2 id="Explore-the-Async-API" class="common-anchor-header">Explorar a API assíncrona<button data-href="#Explore-the-Async-API" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Nesta secção, iremos apresentar a utilização da API de nível inferior e comparar o desempenho das execuções síncronas e assíncronas.</p>
<h3 id="Async-add" class="common-anchor-header">Adição assíncrona<button data-href="#Async-add" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Reinicialize o armazenamento de vetores.</p>
...
<p>Vamos definir uma função de produção de nós, que será utilizada para gerar um grande número de nós de teste para o índice.</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>Defina uma função assíncrona para adicionar documentos ao armazenamento de vetores. Utilizamos a função ` <code translate="no">async_add()</code> ` na instância do armazenamento de vetores do Milvus.</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>Adicione documentos de forma assíncrona ao armazenamento de vetores.</p>
...
<h4 id="Compare-with-synchronous-add" class="common-anchor-header">Compare com a adição síncrona</h4><p>Defina uma função de adição síncrona. Em seguida, meça o tempo de execução nas mesmas condições.</p>
...
<p>O resultado mostra que o processo de adição síncrona é muito mais lento do que o assíncrono.</p>
<h3 id="Async-search" class="common-anchor-header">Pesquisa assíncrona<button data-href="#Async-search" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Reinicialize o armazenamento de vetores e adicione alguns documentos antes de executar a pesquisa.</p>
<pre><code translate="no" class="language-python">vector_store = init_vector_store()
...
<p>Defina uma função de pesquisa assíncrona. Utilizamos a função « <code translate="no">aquery()</code> » na instância do armazenamento de vetores Milvus.</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>Efetue uma pesquisa assíncrona a partir do armazenamento Milvus.</p>
...
<h4 id="Compare-with-synchronous-search" class="common-anchor-header">Compare com a pesquisa síncrona</h4><p>Defina uma função de pesquisa síncrona. Em seguida, meça o tempo de execução nas mesmas condições.</p>
...
<p>O resultado mostra que o processo de pesquisa síncrona é muito mais lento do que o assíncrono.</p>
