...
<h2 id="Major-Components-of-Airbyte" class="common-anchor-header">Principais componentes do Airbyte<button data-href="#Major-Components-of-Airbyte" class="anchor-icon" translate="no">
    ...
    </button></h2><h3 id="1-Connector-Catalog" class="common-anchor-header">1. Catálogo de conectores<button data-href="#1-Connector-Catalog" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<h3 id="2-The-Platform" class="common-anchor-header">2. A plataforma<button data-href="#2-The-Platform" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>A plataforma da Airbyte fornece todos os serviços horizontais necessários para configurar e escalar operações de movimentação de dados, disponíveis como <a href="https://airbyte.com/product/airbyte-cloud">geridos na nuvem</a> ou <a href="https://airbyte.com/product/airbyte-enterprise">autogeridos</a>.</p>
<h3 id="3-The-User-Interface" class="common-anchor-header">3. A interface de utilizador<button data-href="#3-The-User-Interface" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<li>Conta Airbyte ou instância local</li>
...
<h3 id="Set-Up-Source" class="common-anchor-header">Configurar a fonte<button data-href="#Set-Up-Source" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Quando sua instância estiver em execução, precisamos configurar a conexão - clique em "Nova conexão" e escolha o conetor "Zendesk Support" como a origem. Depois de clicar no botão "Testar e salvar", o Airbyte verificará se a conexão pode ser estabelecida.</p>
...
<h3 id="Set-Up-Destination" class="common-anchor-header">Configurar o destino<button data-href="#Set-Up-Destination" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Se tudo estiver a funcionar corretamente, o próximo passo é configurar o destino para onde mover os dados. Aqui, escolha o conetor "Milvus".</p>
...
<li><strong>Indexação</strong> - Depois de ter vectorizado os pedaços, pode carregá-los na base de dados. Para o fazer, insira as informações que obteve aquando da configuração do seu cluster e da sua coleção no cluster Milvus. <div><img translate="no" src="https://milvus-docs.s3.us-west-2.amazonaws.com/assets/airbyte_with_milvus_1.png" width="40%"/></div> Clicando em "Testar e guardar", verificará se tudo está corretamente alinhado (credenciais válidas, a coleção existe e tem a mesma dimensionalidade vetorial que o embedding configurado, etc.).</li>
</ul>
<h3 id="Set-up-stream-sync-flow" class="common-anchor-header">Configurar o fluxo de sincronização de fluxos<button data-href="#Set-up-stream-sync-flow" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>O último passo antes de os dados estarem prontos para fluir é selecionar quais os "fluxos" a sincronizar. Um fluxo é uma coleção de registros na origem. Como o Zendesk oferece suporte a um grande número de fluxos que não são relevantes para o nosso caso de uso, vamos selecionar apenas "tickets" e "artigos" e desativar todos os outros para economizar largura de banda e garantir que apenas as informações relevantes apareçam nas pesquisas:<div><img translate="no" src="https://milvus-docs.s3.us-west-2.amazonaws.com/assets/airbyte_with_milvus_2.png" width="40%"/></div> É possível selecionar os campos a extrair da fonte clicando no nome do fluxo. O modo de sincronização "Incremental | Append + Deduped" significa que as execuções de conexão subsequentes mantêm o Zendesk e o Milvus em sincronia enquanto transferem o mínimo de dados (apenas os artigos e tickets que foram alterados desde a última execução).</p>
...
<h3 id="Check-flow" class="common-anchor-header">Fluxo de verificação<button data-href="#Check-flow" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<p>Em ambos os casos, vamos aproveitar a pesquisa semântica usando as incorporações do OpenAI. Para tal, a descrição do problema que o utilizador introduziu também é incorporada e utilizada para obter entidades semelhantes a partir do agrupamento Milvus. Se existirem resultados relevantes, estes são apresentados por baixo do formulário.</p>
<h3 id="Set-up-UI-environment" class="common-anchor-header">Configurar o ambiente da IU<button data-href="#Set-up-UI-environment" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>É necessária uma instalação local do Python, uma vez que vamos utilizar o Streamlit para implementar a aplicação.</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>Isto irá renderizar um formulário básico:<div><img translate="no" src="https://milvus-docs.s3.us-west-2.amazonaws.com/assets/airbyte_with_milvus_4.png" width="40%"/></div>O código para este exemplo também pode ser encontrado no <a href="https://github.com/airbytehq/tutorial-similarity-search/blob/main/1_basic_support_form.py">GitHub</a>.</p>
<h3 id="Set-up-backend-query-service" class="common-anchor-header">Configurar o serviço de consulta de backend<button data-href="#Set-up-backend-query-service" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Em seguida, vamos verificar se há tíquetes abertos existentes que possam ser relevantes. Para isso, incorporamos o texto que o usuário digitou usando o OpenAI e, em seguida, fizemos uma pesquisa de similaridade em nossa coleção, filtrando os tíquetes ainda abertos. Se houver um com uma distância muito baixa entre o tíquete fornecido e o tíquete existente, informamos o usuário e não enviamos:</p>
<pre><code translate="no" class="language-python"><span class="hljs-keyword">import</span> streamlit <span class="hljs-keyword">as</span> st
...
<button class="copy-code-btn"></button></code></pre>
<p>Ao tentar submeter um bilhete que já existe, o resultado será o seguinte:<div><img translate="no" src="https://milvus-docs.s3.us-west-2.amazonaws.com/assets/airbyte_with_milvus_5.png" width="40%"/></div> O código para este exemplo também pode ser encontrado no <a href="https://github.com/airbytehq/tutorial-similarity-search/blob/main/2_open_ticket_check.py">GitHub</a>.</p>
<h3 id="Show-more-relevant-information" class="common-anchor-header">Mostrar mais informações relevantes<button data-href="#Show-more-relevant-information" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
