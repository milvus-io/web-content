...
<h3 id="Before-cluster-deployment-global-configuration" class="common-anchor-header">Antes da implantação do cluster: configuração global<button data-href="#Before-cluster-deployment-global-configuration" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<h3 id="During-cluster-operation-dynamic-configuration" class="common-anchor-header">Durante o funcionamento do cluster: configuração dinâmica<button data-href="#During-cluster-operation-dynamic-configuration" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Durante o tempo de execução do cluster, é possível ajustar dinamicamente as definições de mapeamento de memória ao nível da coleção ou do índice.</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>Para as definições <strong>de nível de índice</strong>, o mapeamento de memória pode ser aplicado especificamente a índices de vetor sem afetar outros tipos de dados. Esta funcionalidade é valiosa para colecções que requerem um desempenho optimizado para pesquisas vectoriais.</p>
