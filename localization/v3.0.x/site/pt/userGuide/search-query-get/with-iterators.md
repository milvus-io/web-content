...
summary: >-
  ...
  recuperadas numa única consulta, pelo que a simples utilização da Pesquisa ANN
  básica poderá não satisfazer as exigências de uma recuperação em grande
  escala. Para pedidos de pesquisa ANN em que o topK exceda 16 384, é
  aconselhável considerar a utilização do SearchIterator. Esta secção irá
  apresentar como utilizar o SearchIterator e as considerações relacionadas.
---
...
<p>Mais especificamente, pode utilizar os SearchIterators da seguinte forma:</p>
<ol>
...
<li><p>Chame o método <strong>next()</strong> do SearchIterator num ciclo para obter o resultado da pesquisa de forma paginada.</p></li>
...
<h2 id="Create-SearchIterator" class="common-anchor-header">Criar um SearchIterator<button data-href="#Create-SearchIterator" class="anchor-icon" translate="no">
    ...
    </button></h2><p>O seguinte fragmento de código demonstra como criar um SearchIterator.</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>Nos exemplos de código acima, criou-se um ciclo infinito e chamou-se o método <strong>next()</strong> dentro do ciclo para armazenar os resultados da pesquisa numa variável, tendo-se encerrado o iterador quando o método <strong>next()</strong> não devolveu nenhum resultado.</p>
