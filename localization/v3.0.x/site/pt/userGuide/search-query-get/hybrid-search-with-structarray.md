...
<p>A função «Collapse» utiliza apenas os resultados dos elementos devolvidos por essa função « <code translate="no">AnnSearchRequest</code> » ao nível do elemento do StructArray. Não analisa todos os elementos Struct da entidade após a pesquisa ANN. Defina a função « <code translate="no">limit</code> » da solicitação num valor suficientemente elevado para disponibilizar os elementos que pretende para a função «Collapse».</p>
