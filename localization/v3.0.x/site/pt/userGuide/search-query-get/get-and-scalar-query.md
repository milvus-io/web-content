...
title: Consulta
summary: >-
  Utilize as funções Query, Get e QueryIterator para recuperar entidades,
  filtrar metadados, ordenar resultados de consultas e agregar valores escalares
  no Milvus.
---
<h1 id="Query" class="common-anchor-header">Consulta<button data-href="#Query" class="anchor-icon" translate="no">
    ...
    </button></h1><p>Para além das pesquisas ANN, o Milvus também suporta a filtragem de metadados através de consultas. Esta página apresenta como utilizar as funções Query, Get e QueryIterators para recuperar entidades, filtrar metadados, ordenar resultados de consultas e agregar valores escalares.</p>
<div class="alert note">
<p>Se adicionar novos campos após a criação da coleção, as consultas que incluírem esses campos devolverão os valores predefinidos ou « <code translate="no">NULL</code> » para as entidades que não tenham valores explicitamente definidos. Para mais detalhes, consulte <a href="/docs/pt/add-fields-to-an-existing-collection.md">«Alterar o esquema da coleção</a>».</p>
</div>
...
<table>
   ...
   <tr>
     ...
     <td><p>Para encontrar todas as entidades ou um número específico de entidades que satisfaçam as condições de filtragem personalizadas</p></td>
...
<p>Para mais informações sobre a filtragem de metadados, consulte <a href="/docs/pt/basic-operators.md">Regras</a> de <a href="/docs/pt/basic-operators.md">expressões booleanas</a>.</p>
...
<h3 id="Sort-Query-Results" class="common-anchor-header">Ordenar os resultados da consulta<span class="beta-tag" style="background-color:rgb(0, 179, 255);color:white" translate="no">Compatible with Milvus 3.0.x</span><button data-href="#Sort-Query-Results" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<h3 id="Aggregate-Query-Results" class="common-anchor-header">Agregar resultados de consultas<span class="beta-tag" style="background-color:rgb(0, 179, 255);color:white" translate="no">Compatible with Milvus 3.0.x</span><button data-href="#Aggregate-Query-Results" class="anchor-icon" translate="no">
    ...
    </button></h3><p>Pode agrupar os resultados da consulta por um ou mais campos escalares e calcular agregações por grupo. Os operadores de agregação suportados são <code translate="no">count</code>, <code translate="no">min</code>, <code translate="no">max</code>, <code translate="no">sum</code> e <code translate="no">avg</code>.</p>
<p>Ao utilizar ` <code translate="no">group_by_fields</code>`, tenha em atenção que:</p>
<ul>
<li><p>Tipos de campo suportados para <code translate="no">group_by_fields</code>: <code translate="no">INT8</code>, <code translate="no">INT16</code>, <code translate="no">INT32</code>, <code translate="no">INT64</code>, <code translate="no">VARCHAR</code> e <code translate="no">TIMESTAMPTZ</code>. O agrupamento por campos <code translate="no">FLOAT</code>, <code translate="no">DOUBLE</code>, vector, <code translate="no">JSON</code> ou <code translate="no">ARRAY</code> devolve um erro.</p></li>
<li><p><code translate="no">sum</code> e <code translate="no">avg</code> são exclusivamente numéricos. Pode aplicá-los a campos numéricos, incluindo <code translate="no">FLOAT</code> e <code translate="no">DOUBLE</code>, mas a sua aplicação a um campo <code translate="no">VARCHAR</code> devolve um erro.</p></li>
</ul>
...
<p>O exemplo seguinte agrupa as entidades pelo campo « <code translate="no">color</code> » e devolve o número de entidades em cada grupo de cores:</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#go">   Go</a>
...
<button class="copy-code-btn"></button></code></pre>
<p>É possível solicitar várias expressões de agregação numa única chamada. O exemplo seguinte agrupa por « <code translate="no">color</code> » e devolve a contagem de entidades, o preço médio e a classificação máxima para cada grupo:</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#go">   Go</a>
...
<button class="copy-code-btn"></button></code></pre>
<p>Passe mais do que um campo para ` <code translate="no">group_by_fields</code> ` para calcular grupos compostos. O exemplo seguinte agrupa por ` <code translate="no">(color, rating)</code> ` e calcula o intervalo de preços em cada grupo:</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#go">   Go</a>
...
<button class="copy-code-btn"></button></code></pre>
<p>Também pode combinar ` <code translate="no">group_by_fields</code> ` com ` <code translate="no">limit</code> ` para limitar o número de grupos devolvidos. Isto é útil quando um campo tem elevada cardinalidade e apenas necessita de uma amostra de grupos:</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#go">   Go</a>
...
<button class="copy-code-btn"></button></code></pre>
<h2 id="Use-QueryIterator" class="common-anchor-header">Utilize o QueryIterator<button data-href="#Use-QueryIterator" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>Quando precisar de encontrar entidades através de condições de filtragem personalizadas por meio de consultas paginadas, crie um <strong>QueryIterator</strong> e utilize o seu método <strong>next()</strong> para percorrer todas as entidades e encontrar aquelas que cumprem as condições de filtragem. Os exemplos de código seguintes partem do princípio de que existem três campos denominados <code translate="no">id</code>, <code translate="no">vector</code> e <code translate="no">color</code> e devolvem todas as entidades que contêm um valor <code translate="no">color</code> que comece por <code translate="no">red</code>.</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#go">   Go</a>
...
<button class="copy-code-btn"></button></code></pre>
<h2 id="Random-Sampling-with-Query" class="common-anchor-header">Amostragem aleatória com a função «Query»<button data-href="#Random-Sampling-with-Query" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
