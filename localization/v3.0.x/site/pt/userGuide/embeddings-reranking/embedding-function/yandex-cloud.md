...
summary: >-
  Este tópico descreve como configurar e utilizar as funções de incorporação do
  Yandex Cloud no Milvus.
beta: Milvus 2.6.x
---
<h1 id="Yandex-Cloud" class="common-anchor-header">Yandex Cloud<span class="beta-tag" style="background-color:rgb(0, 179, 255);color:white" translate="no">Compatible with Milvus 2.6.x</span><button data-href="#Yandex-Cloud" class="anchor-icon" translate="no">
    ...
    </button></h1><p>Este tópico descreve como configurar e utilizar as funções de incorporação do Yandex Cloud no Milvus.</p>
...
<p>Por exemplo, o Yandex Text Embeddings para documentos utiliza um URI de modelo como <code translate="no">emb://&lt;folder_ID&gt;/text-search-doc/latest</code> e devolve vetores de 256 dimensões. Para conhecer os URIs de modelo e dimensões disponíveis, consulte <a href="https://aistudio.yandex.ru/docs/en/ai-studio/concepts/embeddings">Modelos de vetorização de texto</a>.</p>
<h2 id="Configure-credentials" class="common-anchor-header">Configurar credenciais<button data-href="#Configure-credentials" class="anchor-icon" translate="no">
    ...
    </button></h2><p>O Milvus precisa de conhecer a sua chave de API do Yandex Cloud antes de poder solicitar embeddings. Pode configurar a chave de API em <code translate="no">milvus.yaml</code> ou através de uma variável de ambiente.</p>
<h3 id="Option-1-Configuration-file" class="common-anchor-header">Opção 1: Ficheiro de configuração<button data-href="#Option-1-Configuration-file" class="anchor-icon" translate="no">
    ...
    </button></h3><p>Guarde a sua chave de API em <code translate="no">milvus.yaml</code> e indique ao fornecedor Yandex Cloud o rótulo da credencial.</p>
...
<table>
   ...
   <tr>
     ...
     <td><p>Sim</p></td>
     <td><p>Chave da API do Yandex Cloud utilizada pelo serviço Milvus para aceder ao Yandex Cloud AI Studio.</p></td>
   </tr>
</table>
<h2 id="Use-embedding-function" class="common-anchor-header">Utilizar função de incorporação<button data-href="#Use-embedding-function" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>Depois de configurar as credenciais, defina um esquema com um campo de texto de entrada e um campo de vetor de saída e, em seguida, adicione uma função de incorporação do Yandex Cloud ao esquema.</p>
...
<button class="copy-code-btn"></button></code></pre>
<h3 id="Yandex-Cloud-specific-parameters" class="common-anchor-header">Parâmetros específicos do Yandex Cloud<button data-href="#Yandex-Cloud-specific-parameters" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
   ...
   <tr>
     ...
     <td><p>Sim</p></td>
     <td><p>O fornecedor do modelo de incorporação a utilizar.</p></td>
     <td><p><code translate="no">"yc"</code></p></td>
   </tr>
   <tr>
     <td><p><code translate="no">model_name</code></p></td>
     <td><p>Sim</p></td>
     <td><p>O URI do modelo do Yandex Cloud a ser chamado.</p></td>
   ...
   <tr>
     ...
     <td><p>Não</p></td>
     <td><p>A dimensão do vetor de saída. Se definida, o valor deve corresponder à dimensão do campo do vetor de saída.</p></td>
...
<h2 id="Next-steps" class="common-anchor-header">Próximos passos<button data-href="#Next-steps" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Após configurar a função de incorporação, consulte a <a href="/docs/pt/embedding-function-overview.md">Visão geral da função de incorporação</a> para obter orientações sobre como criar índices, inserir dados e executar a pesquisa semântica.</p>
