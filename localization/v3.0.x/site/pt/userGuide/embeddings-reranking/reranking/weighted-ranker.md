...
<li><p>Resultados da pesquisa ANN em textos （topK=5)：</p>
<p><table>
<tr>
<th><p><strong>ID</strong></p></th>
...
<li><p>Utilize o WeightedRanker para atribuir pesos aos resultados da pesquisa de imagens e de texto. Suponha que o peso para a pesquisa ANN de imagens é 0,6 e o peso para a pesquisa de texto é 0,4.</p>
<p><table>
<tr>
<th><p><strong>ID</strong></p></th>
<th><p><strong>Pontuação (imagem)</strong></p></th>
...
<li><p>Os resultados finais após o reranking（topK=5)：</p>
<p><table>
<tr>
...
<h3 id="Create-a-Weighted-Ranker" class="common-anchor-header">Criar um classificador ponderado<button data-href="#Create-a-Weighted-Ranker" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Por exemplo, suponha que existam dois pedidos de pesquisa ANN básicos numa Pesquisa Híbrida: pesquisa de texto e pesquisa de imagem. Se a pesquisa de texto for considerada mais importante, deve ser-lhe atribuído um peso maior.</p>
<div class="alert note">
<p>O Milvus 2.6.x e posteriores permitem-lhe configurar estratégias de reranking diretamente através da API <code translate="no">Function</code>. Se estiver a utilizar uma versão anterior (antes da v2.6.0), consulte a documentação <a href="https://milvus.io/docs/v2.5.x/reranking.md#Usage-of-WeightedRanker">do Reranking</a> para obter instruções de configuração.</p>
</div>
...
<table>
   ...
   <tr>
     ...
     <td><p>Sim</p></td>
     <td><p>Especifica o método de reordenação a utilizar.</p><p>Deve ser definido como <code translate="no">weighted</code> para utilizar a classificação ponderada.</p></td>
     <td><p><code translate="no">"weighted"</code></p></td>
   </tr>
   <tr>
     <td><p><code translate="no">params.weights</code></p></td>
     <td><p>Sim</p></td>
     <td><p>Matriz de pesos correspondentes a cada caminho de pesquisa; valores ∈ [0,1].</p><p>Para obter detalhes, consulte <a href="/docs/pt/weighted-ranker.md#Mechanism-of-Weighted-Ranker">Mecanismo do Weighted Ranker</a>.</p></td>
     <td><p><code translate="no">[0.1, 0.9]</code></p></td>
   </tr>
   <tr>
     <td><p><code translate="no">params.norm_score</code></p></td>
     <td><p>Não</p></td>
     <td><p>Se a pontuação bruta deve ser normalizada (usando arctan) antes da ponderação.</p><p>Para obter detalhes, consulte <a href="/docs/pt/weighted-ranker.md#Mechanism-of-Weighted-Ranker">Mecanismo do classificador ponderado</a>.</p></td>
