...
<p>O Milvus disponibiliza o tipo de campo escalar « <code translate="no">TEXT</code> » para armazenar texto de origem extenso diretamente com as entidades. Os valores típicos incluem passagens, documentos longos, corpos de artigos, tickets e registos. Ao contrário do « <code translate="no">VARCHAR</code> », que requer um comprimento máximo fixo de bytes ( <code translate="no">max_length</code>), o « <code translate="no">TEXT</code> » não exige que se defina um comprimento máximo de bytes no esquema da coleção.</p>
...
<button class="copy-code-btn"></button></code></pre>
<h2 id="Step-4-Perform-BM25-full-text-search" class="common-anchor-header">Passo 4: Efetuar pesquisa de texto completo com BM25<button data-href="#Step-4-Perform-BM25-full-text-search" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
