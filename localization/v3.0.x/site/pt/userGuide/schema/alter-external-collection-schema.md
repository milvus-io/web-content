...
<li><p>Atualmente, as coleções externas suportam a adição de campos após a sua criação. Outras alterações ao esquema, tais como eliminar campos, renomear campos, alterar tipos de dados de campos, alterar dimensões de vetores ou remapear um <code translate="no">external_field</code>, não são suportadas.</p></li>
...
<h2 id="Refresh-the-external-collection" class="common-anchor-header">Atualizar a coleção externa<button data-href="#Refresh-the-external-collection" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Após alterar o esquema de uma coleção externa, atualize a coleção externa para que o Milvus atualize os metadados da coleção externa e torne a alteração do esquema efetiva nos resultados de consultas, pesquisas e filtragem.</p>
