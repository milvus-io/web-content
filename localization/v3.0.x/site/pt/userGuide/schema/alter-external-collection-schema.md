...
<li><p>Atualmente, as coleções externas suportam a adição de campos após a sua criação. Outras alterações ao esquema, tais como eliminar campos, renomear campos, alterar tipos de dados de campos, alterar dimensões vetoriais ou remapear <code translate="no">external_field</code>, não são suportadas.</p></li>
...
<li><p>A adição de campos StructArray a uma coleção externa existente não é suportada. Se a sua coleção externa necessitar de um campo StructArray, defina-o no esquema da coleção quando criar a coleção.</p></li>
</ul>
