...
title: Campos nulos
summary: >-
  Configure campos que podem ser nulos e valores por predefinição, incluindo o
  esquema e os comportamentos de inserção, indexação, pesquisa e filtragem.
---
...
<li><p>Os campos vetoriais que permitem valores NULL não suportam expressões de filtro do tipo « <code translate="no">IS NULL</code> » ou « <code translate="no">IS NOT NULL</code> ». Não é possível filtrar explicitamente entidades com base no facto de o valor de um campo vetorial ser NULL.</p></li>
<li><p>Os campos<a href="/docs/pt/array-of-structs.md">«Array of Structs»</a> não suportam valores NULL. Não é possível marcar um campo «Array of Structs» ou qualquer campo aninhado no seu interior como nulo.</p></li>
<li><p>O atributo «nullable» é definido quando um campo é criado e não pode ser modificado posteriormente. Não é possível ativar ou desativar a nulabilidade para um campo existente.</p></li>
...
<h2 id="What-is-a-nullable-field" class="common-anchor-header">O que é um campo nulo?<button data-href="#What-is-a-nullable-field" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<p>Se um campo não for definido como nulo (o comportamento predefinido), todas as entidades devem fornecer um valor válido para esse campo. A omissão do campo ou a atribuição explícita de um valor NULL fará com que a operação de inserção ou importação falhe.</p>
<p>O atributo «nullable» é suportado tanto para <strong>campos escalares como</strong> para <strong>campos vetoriais</strong> num esquema de coleção. No entanto, os campos «Array of Structs» não suportam o atributo «nullable».</p>
...
<ul>
<li>Se um campo nulo for configurado sem um valor por defeito, a omissão do campo resulta num valor NULL armazenado.</li>
<li>Se for configurado um valor predefinido, o Milvus poderá armazenar esse valor em vez do valor nulo. Para mais detalhes, consulte <a href="/docs/pt/default-values.md">Valores predefinidos</a>.</li>
</ul>
</div>
...
<p>Para maior clareza, os exemplos seguintes centram-se num campo vetorial que pode ser nulo (<code translate="no">embedding</code>). A definição de campos escalares que podem ser nulos é opcional e não é necessária para seguir o resto deste guia.</p>
<p><details>
...
<h2 id="Insert-behavior-with-missing-or-NULL-values" class="common-anchor-header">Comportamento de inserção com valores em falta ou NULL<button data-href="#Insert-behavior-with-missing-or-NULL-values" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Assim que um campo é definido como nulo no esquema da coleção, o Milvus permite que o valor do campo esteja em falta ou seja explicitamente definido como NULL durante a ingestão de dados.</p>
...
<p>No caso de um campo vetorial nulo, isto significa que apenas as entidades com vetores válidos podem ser pesquisadas por semelhança vetorial.</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#javascript">   NodeJS</a>
...
<li>As entidades cujo embedding é NULL permanecem na coleção, mas não são incluídas no índice vetorial.</li>
</ul>
...
<p>Nesta pesquisa:</p>
<ul>
<li>Apenas as entidades com valores não nulos para <code translate="no">embedding</code> são consideradas candidatas.</li>
...
<p>Por exemplo, dado um campo escalar que pode assumir o valor NULL <code translate="no">age</code>, o filtro seguinte seleciona entidades cuja idade é superior a 18:</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#javascript">   NodeJS</a>
...
<h2 id="Nullable-fields-and-default-values" class="common-anchor-header">Campos nulos e valores por defeito<button data-href="#Nullable-fields-and-default-values" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<li>Quando o « <code translate="no">nullable=True</code> » está ativado, mas não está definido nenhum valor predefinido, o campo armazena NULL.</li>
<li>Quando a opção « <code translate="no">nullable=False</code> » está definida, mas não existe um valor predefinido, a inserção falha com um erro.</li>
<li>Definir um valor predefinido NULL num campo não nulo é inválido e provoca um erro.</li>
</ul>
<p>Para exemplos completos e utilização da API para valores por defeito, consulte <a href="/docs/pt/default-values.md">Valores por defeito</a>.</p>
