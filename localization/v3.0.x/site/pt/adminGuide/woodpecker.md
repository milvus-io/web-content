...
<h1 id="Woodpecker" class="common-anchor-header">Woodpecker<button data-href="#Woodpecker" class="anchor-icon" translate="no">
    ...
    </button></h1><p>O Woodpecker é a <strong>fila de mensagens predefinida (registo de gravação antecipada, WAL)</strong> no Milvus 3.x. Trata-se de um WAL nativo da nuvem concebido para armazenamento de objetos, que oferece elevado débito, baixo custo operacional e escalabilidade contínua. Para obter detalhes sobre a arquitetura e os testes de desempenho, consulte <a href="/docs/pt/woodpecker_architecture.md">Woodpecker</a>.</p>
...
<li><strong>prefix</strong>: O prefixo da chave para os metadados. Padrão: <code translate="no">woodpecker</code>.</li>
</ul></li>
...
<p>Notas:</p>
<ul>
<li>Com o modo « <code translate="no">minio</code> », o Woodpecker partilha o mesmo armazenamento de objetos com o Milvus (MinIO/S3/GCS/OSS, etc.).</li>
...
<li><strong><code translate="no">service</code> O modo executa o Woodpecker como um serviço separado e escalável de forma independente, estando disponível apenas para implementações distribuídas/em cluster.</strong> As implementações autónomas utilizam os modos incorporados (<code translate="no">minio</code> ou <code translate="no">local</code>).</li>
</ul>
