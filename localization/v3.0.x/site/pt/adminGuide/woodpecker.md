...
<h1 id="Woodpecker" class="common-anchor-header">Woodpecker<button data-href="#Woodpecker" class="anchor-icon" translate="no">
    ...
    </button></h1><p>O Woodpecker é a <strong>fila de mensagens predefinida (registo antecipado, WAL)</strong> no Milvus 3.x. Trata-se de um WAL nativo da nuvem concebido para armazenamento de objetos, que oferece elevado débito, baixo custo operacional e escalabilidade contínua. Para obter detalhes sobre a arquitetura e os testes de desempenho, consulte <a href="/docs/pt/woodpecker_architecture.md">Woodpecker</a>.</p>
...
<pre><code translate="no" class="language-bash">helm install my-release zilliztech/milvus \
  ...
  --<span class="hljs-built_in">set</span> streaming.enabled=<span class="hljs-literal">true</span>
<button class="copy-code-btn"></button></code></pre>
<p>Após a implementação, siga a documentação para fazer o redirecionamento de portas e estabelecer a ligação. Para ajustar os parâmetros do Woodpecker, siga as configurações descritas em <a href="#Configuration">«Configuração</a>».</p>
...
<ul>
<li>Aumente os parâmetros ` <code translate="no">logstore.segmentSyncPolicy.maxFlushSize</code> ` e ` <code translate="no">maxFlushThreads</code> ` para obter flushes maiores e maior paralelismo.</li>
