...
<p>Basta editar o modelo de código em <code translate="no">milvus_cluster_default.yaml</code> para configurar as dependências de terceiros. As secções seguintes apresentam como configurar o armazenamento de objetos, o etcd e o Pulsar, respetivamente.</p>
...
<ul>
<li>É suportado apenas um armazenamento de mensagens por instância do Milvus. No entanto, ainda existe compatibilidade retroativa com a configuração de vários armazenamentos de mensagens para uma única instância. A prioridade é a seguinte:
<ul>
<li>modo autónomo: Woodpecker (predefinição) &gt; RocksMQ &gt; Pulsar &gt; Kafka</li>
<li>modo de cluster: Woodpecker (predefinido) &gt; Pulsar &gt; Kafka</li>
</ul></li>
...
<li><strong>Limitações da atualização</strong>: <strong>Limitações da fila de mensagens</strong>: Ao atualizar para o Milvus v3.0.2, deve manter a sua escolha atual de fila de mensagens. Não é suportada a mudança entre diferentes sistemas de filas de mensagens durante a atualização. O suporte para a alteração de sistemas de filas de mensagens estará disponível em versões futuras.</li>
</ul>
<h2 id="Configure-RocksMQ" class="common-anchor-header">Configurar o RocksMQ<button data-href="#Configure-RocksMQ" class="anchor-icon" translate="no">
    ...
    </button></h2><p>O RocksMQ era o armazenamento de mensagens predefinido no Milvus autónomo até à versão 2.5.x (substituído pelo Woodpecker a partir da versão 2.6.x).</p>
<div class="alert note">
<p>Atualmente, só é possível configurar o RocksMQ como armazenamento de mensagens para o Milvus autónomo com o Milvus Operator.</p>
</div>
...
<ul>
<li><code translate="no">external</code>:  Um valor « <code translate="no">true</code> » indica que o Milvus utiliza um serviço Pulsar externo.</li>
...
<h2 id="Configure-Kafka" class="common-anchor-header">Configurar o Kafka<button data-href="#Configure-Kafka" class="anchor-icon" translate="no">
    ...
    </button></h2><p>O Pulsar era o armazenamento de mensagens predefinido num cluster Milvus até à versão 2.5.x (substituído pelo Woodpecker a partir da versão 2.6.x). Se pretender utilizar o Kafka, adicione o campo opcional « <code translate="no">msgStreamType</code> » para configurar o Kafka.</p>
...
<h3 id="Internal-Kafka" class="common-anchor-header">Kafka interno<button data-href="#Internal-Kafka" class="anchor-icon" translate="no">
    ...
    </button></h3><p><code translate="no">inCluster</code> indica que, quando um cluster Milvus é iniciado, um serviço Kafka é iniciado automaticamente nesse cluster.</p>
...
<li><a href="/docs/pt/meta_storage_operator.md">Configurar o armazenamento de metadados com o Milvus Operator</a></li>
</ul>
