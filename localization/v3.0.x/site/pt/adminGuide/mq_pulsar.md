...
<h1 id="Use-Pulsar-as-the-Milvus-Message-Queue" class="common-anchor-header">Utilizar o Pulsar como fila de mensagens do Milvus<button data-href="#Use-Pulsar-as-the-Milvus-Message-Queue" class="anchor-icon" translate="no">
    ...
    </button></h1><p>O Apache Pulsar é um dos backends de fila de mensagens (WAL) suportados pelo Milvus. No Milvus 3.x, <a href="/docs/pt/woodpecker.md">o Woodpecker</a> é a fila de mensagens predefinida; o Pulsar continua a ser totalmente suportado para os utilizadores que o preferirem. O Pulsar é utilizado principalmente com o Milvus Distributed (cluster); as implementações autónomas utilizam normalmente o Woodpecker incorporado ou <a href="/docs/pt/mq_rocksmq.md">o RocksMQ</a>.</p>
...
<h2 id="Deploy-a-Milvus-cluster-with-Pulsar-using-Helm" class="common-anchor-header">Implementar um cluster Milvus com o Pulsar utilizando o Helm<button data-href="#Deploy-a-Milvus-cluster-with-Pulsar-using-Helm" class="anchor-icon" translate="no">
    ...
    </button></h2><h3 id="Install" class="common-anchor-header">Instalar<button data-href="#Install" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<p>No Kubernetes v1.25 e versões posteriores, se enfrentar problemas com a API do PodDisruptionBudget (PDB) decorrentes do sub-chart do Pulsar incluído, desative as políticas de PDB do Pulsar:</p>
<pre><code translate="no" class="language-bash">helm install my-release zilliztech/milvus \
...
<h3 id="Configure" class="common-anchor-header">Configurar<button data-href="#Configure" class="anchor-icon" translate="no">
    ...
    </button></h3><p>Para ligar o Milvus a um serviço Pulsar <strong>externo</strong>, desative o Pulsar incluído e ative o « <code translate="no">externalPulsar</code> » numa substituição do « <code translate="no">values.yaml</code> »:</p>
...
<li><strong>Atualização da versão 2.5.x para a 2.6.x:</strong> <strong>Limitações da fila de mensagens</strong>: Ao atualizar para o Milvus v3.0.1, deve manter a sua escolha atual de fila de mensagens. Não é suportada a mudança entre diferentes sistemas de fila de mensagens durante a atualização. O suporte à alteração de sistemas de fila de mensagens estará disponível em versões futuras.
