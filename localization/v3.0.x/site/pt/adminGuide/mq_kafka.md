...
<h2 id="Deploy-a-Milvus-cluster-with-Kafka-using-Milvus-Operator" class="common-anchor-header">Implantar um cluster Milvus com o Kafka utilizando o Milvus Operator<button data-href="#Deploy-a-Milvus-cluster-with-Kafka-using-Milvus-Operator" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Com o Milvus Operator, defina o parâmetro « <code translate="no">spec.dependencies.msgStreamType: &quot;kafka&quot;</code> » e configure o Kafka em « <code translate="no">spec.dependencies.kafka</code> » (apenas no cluster). O « <code translate="no">kafka</code> » suporta « <code translate="no">external</code> » e « <code translate="no">inCluster</code> ».</p>
...
<span class="hljs-attr">spec:</span>
  ...
  <span class="hljs-attr">config:</span> {}
<button class="copy-code-btn"></button></code></pre>
<p>Aplique a configuração (assumindo que o ficheiro é <code translate="no">milvuscluster.yaml</code>):</p>
