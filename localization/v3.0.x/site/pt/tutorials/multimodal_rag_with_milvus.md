...
summary: RAG multimodal com Milvus
title: RAG multimodal com o Milvus
---
<h1 id="Multimodal-RAG-with-Milvus" class="common-anchor-header">RAG multimodal com o Milvus<button data-href="#Multimodal-RAG-with-Milvus" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<p>Se quiser ver o resultado final deste tutorial, pode aceder diretamente à <a href="https://demos.milvus.io/multimodal-image-search/">demonstração online</a>.</p>
...
<p>Este tutorial apresenta o RAG multimodal, que utiliza o Milvus, <a href="https://github.com/FlagOpen/FlagEmbedding/tree/master/FlagEmbedding/visual">o modelo BGE visualizado</a> e <a href="https://openai.com/index/hello-gpt-4o/">o GPT-4o</a>. Com este sistema, os utilizadores podem carregar uma imagem e editar instruções de texto, que são processadas pelo modelo de recuperação composto do BGE para pesquisar imagens candidatas. O GPT-4o atua então como um reclassificador, selecionando a imagem mais adequada e apresentando a justificação por trás da escolha. Esta poderosa combinação permite uma experiência de pesquisa de imagens fluida e intuitiva, tirando partido do Milvus para uma recuperação eficiente, do modelo BGE para um processamento e correspondência precisos de imagens e do GPT-4o para uma reclassificação avançada.</p>
...
<h3 id="Download-Data" class="common-anchor-header">Descarregar dados<button data-href="#Download-Data" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>O comando seguinte irá descarregar os dados de exemplo e extraí-los para uma pasta local «./images_folder», incluindo:</p>
<ul>
<li><strong>imagens</strong>: Um subconjunto do <a href="https://github.com/hyp1231/AmazonReviews2023">Amazon Reviews 2023</a> contendo aproximadamente 900 imagens das categorias «Appliance», «Cell_Phones_and_Accessories» e «Electronics».</li>
<li><strong>leopard.jpg</strong>: Uma imagem de exemplo de consulta.</li>
</ul>
...
<button class="copy-code-btn"></button></code></pre>
<h3 id="Load-Embedding-Model" class="common-anchor-header">Carregar modelo de embedding<button data-href="#Load-Embedding-Model" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Iremos utilizar o modelo BGE visualizado «bge-visualized-base-en-v1.5» para gerar embeddings tanto para imagens como para texto.</p>
<p><strong>1. Descarregar os pesos</strong></p>
...
<button class="copy-code-btn"></button></code></pre>
<p><strong>2. Construir o codificador</strong></p>
...
<h2 id="Load-Data" class="common-anchor-header">Carregar dados<button data-href="#Load-Data" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Esta secção irá carregar imagens de exemplo na base de dados com as representações correspondentes.</p>
<h3 id="Generate-embeddings" class="common-anchor-header">Gerar embeddings<button data-href="#Generate-embeddings" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Carregar todas as imagens JPEG do diretório de dados e aplicar o codificador para converter as imagens em representações.</p>
...
<h3 id="Insert-into-Milvus" class="common-anchor-header">Inserir no Milvus<button data-href="#Insert-into-Milvus" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Inserir imagens com os caminhos e embeddings correspondentes na coleção do Milvus.</p>
<div class="alert note">
<p>Quanto ao argumento de « <code translate="no">MilvusClient</code> »:</p>
<ul>
<li>Definir o ` <code translate="no">uri</code> ` como um ficheiro local, por exemplo, ` <code translate="no">./milvus_demo.db</code>`, é o método mais conveniente, uma vez que utiliza automaticamente <a href="https://milvus.io/docs/milvus_lite.md">o Milvus Lite</a> para armazenar todos os dados neste ficheiro.</li>
...
<h2 id="Multimodal-Search-with-Generative-Reranker" class="common-anchor-header">Pesquisa multimodal com reclassificador generativo<button data-href="#Multimodal-Search-with-Generative-Reranker" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>Nesta secção, iremos, em primeiro lugar, procurar imagens relevantes através de uma consulta multimodal e, em seguida, utilizar o serviço LLM para reclassificar os resultados e encontrar o melhor, com explicação.</p>
<h3 id="Run-search" class="common-anchor-header">Executar pesquisa<button data-href="#Run-search" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Agora estamos prontos para realizar a pesquisa avançada de imagens com dados de consulta compostos tanto por imagens como por instruções de texto.</p>
...
<h3 id="Rerank-with-GPT-4o" class="common-anchor-header">Reordenar com o GPT-4o<button data-href="#Rerank-with-GPT-4o" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Iremos utilizar um LLM para classificar as imagens e gerar uma explicação para o melhor resultado com base na consulta do utilizador e nos resultados obtidos.</p>
<p><strong>1. Criar uma vista panorâmica</strong></p>
...
<button class="copy-code-btn"></button></code></pre>
<p>Combinar a imagem da consulta e as imagens recuperadas com índices numa vista panorâmica.</p>
...
<p><span class="img-wrapper">
   ...
   <span>Criar uma vista panorâmica</span>
  
 </span></p>
<p><strong>2. Reorganizar a classificação e explicar</strong></p>
<p>Enviaremos a imagem combinada para o serviço LLM multimodal, juntamente com prompts adequados, para classificar os resultados recuperados com uma explicação. Para ativar o GPT-4o como LLM, é necessário preparar <a href="https://platform.openai.com/docs/quickstart">a</a> sua <a href="https://platform.openai.com/docs/quickstart">chave API da OpenAI</a>.</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>Obtenha os índices das imagens após a classificação e a razão para o melhor resultado:</p>
...
<button class="copy-code-btn"></button></code></pre>
<p><strong>3. Apresentar o melhor resultado com explicação</strong></p>
...
<p><span class="img-wrapper">
   ...
   <span>O melhor resultado</span>
  
 </span></p>
<h3 id="Quick-Deploy" class="common-anchor-header">Implementação rápida<button data-href="#Quick-Deploy" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Para saber como iniciar uma demonstração online com este tutorial, consulte <a href="https://github.com/milvus-io/bootcamp/tree/master/tutorials/quickstart/apps/multimodal_rag_with_milvus">a aplicação de exemplo</a>.</p>
