...
summary: Milvus를 활용한 다중 모달 RAG
title: Milvus를 활용한 다중 모달 RAG
---
<h1 id="Multimodal-RAG-with-Milvus" class="common-anchor-header">Milvus를 활용한 다중 모달 RAG<button data-href="#Multimodal-RAG-with-Milvus" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<p>이 튜토리얼의 최종 결과를 직접 체험해 보고 싶으시다면, <a href="https://demos.milvus.io/multimodal-image-search/">온라인 데모로</a> 바로 이동하실 수 있습니다.</p>
...
<p>이 튜토리얼에서는 Milvus, <a href="https://github.com/FlagOpen/FlagEmbedding/tree/master/FlagEmbedding/visual">Visualized BGE 모델</a>, <a href="https://openai.com/index/hello-gpt-4o/">GPT-4o를</a> 기반으로 한 멀티모달 RAG를 소개합니다. 이 시스템을 통해 사용자는 이미지를 업로드하고 텍스트 지침을 편집할 수 있으며, BGE의 복합 검색 모델이 이를 처리하여 후보 이미지를 검색합니다. 그 후 GPT-4o가 재순위 지정(reranking) 역할을 수행하여 가장 적합한 이미지를 선정하고, 해당 선택의 근거를 제공합니다. 이 강력한 조합은 Milvus를 활용한 효율적인 검색, BGE 모델을 통한 정밀한 이미지 처리 및 매칭, GPT-4o를 이용한 고급 재순위 지정을 바탕으로 매끄럽고 직관적인 이미지 검색 경험을 제공합니다.</p>
...
<h2 id="Preparation" class="common-anchor-header">준비<button data-href="#Preparation" class="anchor-icon" translate="no">
    ...
    </button></h2><h3 id="Install-Dependencies" class="common-anchor-header">의존성 설치<button data-href="#Install-Dependencies" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<h3 id="Download-Data" class="common-anchor-header">데이터 다운로드<button data-href="#Download-Data" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>다음 명령어를 실행하면 예제 데이터를 다운로드하여 “./images_folder”라는 로컬 폴더에 추출할 수 있습니다. 이 폴더에는 다음이 포함됩니다:</p>
<ul>
<li><strong>images</strong>: “Appliance”, “Cell_Phones_and_Accessories”, “Electronics” 카테고리의 이미지 약 900장이 포함된 <a href="https://github.com/hyp1231/AmazonReviews2023">Amazon Reviews 2023의</a> 하위 집합입니다.</li>
<li><strong>leopard.jpg</strong>: 예제 쿼리 이미지입니다.</li>
</ul>
...
<button class="copy-code-btn"></button></code></pre>
<h3 id="Load-Embedding-Model" class="common-anchor-header">임베딩 모델 불러오기<button data-href="#Load-Embedding-Model" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>이미지와 텍스트 모두에 대한 임베딩을 생성하기 위해 시각화된 BGE 모델 “bge-visualized-base-en-v1.5”를 사용할 것입니다.</p>
<p><strong>1. 가중치 다운로드</strong></p>
...
<button class="copy-code-btn"></button></code></pre>
<h2 id="Load-Data" class="common-anchor-header">데이터 불러오기<button data-href="#Load-Data" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<h3 id="Generate-embeddings" class="common-anchor-header">임베딩 생성<button data-href="#Generate-embeddings" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>데이터 디렉터리에서 모든 jpeg 이미지를 불러온 다음, 인코더를 적용하여 이미지를 임베딩으로 변환합니다.</p>
...
<h3 id="Insert-into-Milvus" class="common-anchor-header">Milvus에 삽입<button data-href="#Insert-into-Milvus" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>해당 경로와 임베딩을 가진 이미지를 Milvus 컬렉션에 삽입합니다.</p>
<div class="alert note">
<p><code translate="no">MilvusClient</code> 의 인자에 대해서는 다음과 같습니다:</p>
<ul>
<li><code translate="no">uri</code> 를 로컬 파일(예: <code translate="no">./milvus_demo.db</code>)로 설정하는 것이 가장 편리한 방법입니다. 이 경우 <a href="https://milvus.io/docs/milvus_lite.md">Milvus Lite를</a> 자동으로 활용하여 모든 데이터를 해당 파일에 저장하기 때문입니다.</li>
<li>대규모 데이터를 보유한 경우, <a href="https://milvus.io/docs/quickstart.md">Docker나 Kubernetes에서</a> 더 높은 성능을 발휘하는 Milvus 서버를 구축할 수 있습니다. 이 설정에서는 <code translate="no">uri</code> 대신 서버 URI(예:<code translate="no">http://localhost:19530</code>)를 사용하십시오.</li>
<li>Milvus 전용 풀 매니지드 클라우드 서비스인 <a href="https://zilliz.com/cloud">Zilliz Cloud를</a> 사용하려면, Zilliz Cloud의 <a href="https://docs.zilliz.com/docs/on-zilliz-cloud-console#free-cluster-details">공용 엔드포인트(Public Endpoint) 및 API 키에</a> 해당하는 <code translate="no">uri</code> 와 <code translate="no">token</code> 를 조정하십시오.</li>
</ul>
</div>
...
<h2 id="Multimodal-Search-with-Generative-Reranker" class="common-anchor-header">생성형 재순위 지정기를 활용한 다중 모달 검색<button data-href="#Multimodal-Search-with-Generative-Reranker" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>이 섹션에서는 먼저 다중 모달 쿼리를 통해 관련 이미지를 검색한 다음, LLM 서비스를 사용하여 결과의 순위를 재조정하고 설명과 함께 가장 적합한 이미지를 찾아보겠습니다.</p>
<h3 id="Run-search" class="common-anchor-header">검색 실행<button data-href="#Run-search" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>이제 이미지와 텍스트 지침으로 구성된 쿼리 데이터를 사용하여 고급 이미지 검색을 수행할 준비가 되었습니다.</p>
...
<h3 id="Rerank-with-GPT-4o" class="common-anchor-header">GPT-4o를 활용한 재순위 지정<button data-href="#Rerank-with-GPT-4o" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>LLM을 사용하여 이미지를 순위 매기고, 사용자 쿼리와 검색 결과를 바탕으로 최상의 결과에 대한 설명을 생성할 것입니다.</p>
<p><strong>1. 파노라마 뷰 생성</strong></p>
...
<button class="copy-code-btn"></button></code></pre>
<p>쿼리 이미지와 검색된 이미지를 인덱스와 함께 파노라마 뷰로 결합합니다.</p>
...
<p><strong>2. 재순위 지정 및 설명 제공</strong></p>
<p>결합된 이미지를 적절한 프롬프트와 함께 멀티모달 LLM 서비스로 전송하여, 검색된 결과의 순위를 매기고 그 이유를 설명하도록 합니다. GPT-4o를 LLM으로 사용하려면 <a href="https://platform.openai.com/docs/quickstart">OpenAI API 키를</a> 준비해야 합니다.</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>순위 지정 후 이미지 인덱스와 최우수 결과의 이유를 확인합니다:</p>
...
<h3 id="Quick-Deploy" class="common-anchor-header">빠른 배포<button data-href="#Quick-Deploy" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>이 튜토리얼을 통해 온라인 데모를 시작하는 방법에 대해 알아보려면 <a href="https://github.com/milvus-io/bootcamp/tree/master/tutorials/quickstart/apps/multimodal_rag_with_milvus">예제 애플리케이션을</a> 참조하십시오.</p>
