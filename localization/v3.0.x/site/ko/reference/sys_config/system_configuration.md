...
<p>Milvus는 시스템을 구성하는 상당수의 매개변수를 관리합니다. 각 구성 항목에는 기본값이 설정되어 있어 이를 바로 사용할 수 있습니다. Milvus가 사용자의 애플리케이션에 더 잘 부합하도록 이러한 매개변수를 유연하게 수정할 수 있습니다. 자세한 내용은 <a href="/docs/ko/configure-docker.md">‘Milvus 구성’을</a> 참조하십시오.</p>
...
<h2 id="Sections" class="common-anchor-header">섹션<button data-href="#Sections" class="anchor-icon" translate="no">
    ...
    </button></h2><p>유지 관리를 용이하게 하기 위해, Milvus는 구성 요소를 기반으로 구성 항목을 %s 섹션으로 분류합니다.</p>
...
<p>메타스토어에 TiKV가 활성화된 경우에도 서비스 검색을 위해서는 etcd가 여전히 필요합니다.</p>
...
<h3 id="localStorage" class="common-anchor-header"><code translate="no">localStorage</code><button data-href="#localStorage" class="anchor-icon" translate="no">
    ...
    </button></h3><p>이 섹션에 있는 각 매개 변수에 대한 자세한 설명은 <a href="/docs/ko/configure_localstorage.md">localStorage 관련 구성을</a> 참조하십시오.</p>
<h3 id="minio" class="common-anchor-header"><code translate="no">minio</code><button data-href="#minio" class="anchor-icon" translate="no">
    ...
    </button></h3><p>MinIO/S3/GCS 또는 S3 API를 지원하는 기타 서비스에 대한 관련 구성으로, 이는 Milvus의 데이터 영속성을 담당합니다.</p>
...
<p>mq.type 필드를 기본값으로 설정하지 않은 경우, 이 파일에서 여러 MQ를 구성할 때 우선순위를 활성화해야 한다는 참고 사항이 있습니다.</p>
<ol>
...
<h3 id="proxy" class="common-anchor-header"><code translate="no">proxy</code><button data-href="#proxy" class="anchor-icon" translate="no">
    ...
    </button></h3><p>클라이언트 요청을 검증하고 반환되는 결과를 축소하는 데 사용되는 proxy 관련 구성입니다.</p>
...
<h3 id="queryCoord" class="common-anchor-header"><code translate="no">queryCoord</code><button data-href="#queryCoord" class="anchor-icon" translate="no">
    ...
    </button></h3><p>쿼리 노드의 토폴로지 및 부하 분산을 관리하고, 성장 중인 세그먼트에서 봉인된 세그먼트로 핸드오프하는 데 사용되는 queryCoord 관련 구성.</p>
...
<p>이 섹션에 있는 각 매개 변수에 대한 자세한 설명은 <a href="/docs/ko/configure_querynode.md">queryNode 관련 구성을</a> 참조하십시오.</p>
...
<p>이 섹션에 있는 각 매개변수에 대한 자세한 설명은 <a href="/docs/ko/configure_log.md">log 관련 구성을</a> 참조하십시오.</p>
...
<p>#실제 메모리 수요가 maxMemSize로 설정된 값을 초과할 경우 Milvus가 중단될 수 있습니다.</p>
...
<p>이 섹션에 포함된 각 매개변수에 대한 자세한 설명은 <a href="/docs/ko/configure_streamingnode.md">‘스트리밍 노드 관련 구성’을</a> 참조하십시오.</p>
