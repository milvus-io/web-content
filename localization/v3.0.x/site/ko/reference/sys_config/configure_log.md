...
summary: Milvus용 로그를 구성하는 방법을 알아보세요.
---
<h1 id="log-related-Configurations" class="common-anchor-header">로그 관련 구성<button data-href="#log-related-Configurations" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h1><p>시스템 로그 출력을 구성합니다.</p>
<h2 id="loglevel" class="common-anchor-header"><code translate="no">log.level</code><button data-href="#loglevel" class="anchor-icon" translate="no">
  ...
  <tbody>
    <tr>
      <td>
        <li>밀버스 로그 수준. 옵션: 디버그, 정보, 경고, 오류, 패닉, 치명적. </li>      
        <li>테스트 및 개발 환경에서는 디버그 수준을, 프로덕션 환경에서는 정보 수준을 사용하는 것이 좋습니다.</li>      </td>
      <td>info</td>
    </tr>
  </tbody>
</table>
<h2 id="logfilerootPath" class="common-anchor-header"><code translate="no">log.file.rootPath</code><button data-href="#logfilerootPath" class="anchor-icon" translate="no">
  ...
  <tbody>
    <tr>
      <td>
        <li>로그 파일의 루트 경로입니다.</li>      
        <li>기본값은 비워져 있으며, 로그 파일을 표준 출력(stdout) 및 표준 오류(stderr)로 출력하도록 지정합니다.</li>      
        <li>이 매개변수를 유효한 로컬 경로로 설정하면 Milvus는 이 경로에 로그 파일을 쓰고 저장합니다.</li>      
        <li>이 매개변수를 쓰기 권한이 있는 경로로 설정하세요.</li>      </td>
      <td></td>
    </tr>
  </tbody>
</table>
...
<h2 id="logfilemaxAge" class="common-anchor-header"><code translate="no">log.file.maxAge</code><button data-href="#logfilemaxAge" class="anchor-icon" translate="no">
  ...
  <tbody>
    <tr>
      <td>        로그 파일이 자동으로 지워지기 전 최대 보존 시간(단위: 일)입니다. 최소값은 1입니다.      </td>
      <td>10</td>
    </tr>
  </tbody>
</table>
<h2 id="logfilemaxBackups" class="common-anchor-header"><code translate="no">log.file.maxBackups</code><button data-href="#logfilemaxBackups" class="anchor-icon" translate="no">
  ...
  <tbody>
    <tr>
      <td>        백업할 최대 로그 파일 수(단위: 일)입니다. 최소값은 1입니다.      </td>
      <td>20</td>
    </tr>
  </tbody>
</table>
<h2 id="logformat" class="common-anchor-header"><code translate="no">log.format</code><button data-href="#logformat" class="anchor-icon" translate="no">
  ...
  <tbody>
    <tr>
      <td>        Milvus 로그 형식입니다. 옵션: 텍스트 및 JSON    </td>
      <td>text</td>
    </tr>
  </tbody>
</table>
