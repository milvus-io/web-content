...
<p>요소 수준 하이브리드 그룹화는 모든 <code translate="no">AnnSearchRequest</code> 객체가 동일한 StructArray 필드 하위의 요소 수준 벡터 필드를 대상으로 할 때만 지원되며, <code translate="no">group_by_field</code> 는 주 키여야 합니다. 요청에 컬렉션 수준 벡터 필드, 서로 다른 StructArray 필드 또는 EmbeddingList 수준 요청이 혼합된 경우 하이브리드 그룹화는 지원되지 않습니다. 범위 검색과 그룹화를 함께 사용하지 마십시오.</p>
