...
title: 검색 반복자
summary: >-
  ANN 검색에는 단일 쿼리에서 검색할 수 있는 엔티티 수에 상한선이 있으며, 단순히 기본 ANN 검색만으로는 대규모 검색 요구 사항을
  충족하지 못할 수 있습니다. topK가 16,384를 초과하는 ANN 검색 요청의 경우, SearchIterator 사용을 고려하는 것이
  좋습니다. 이 섹션에서는 SearchIterator 사용 방법과 관련 고려 사항을 소개합니다.
---
<h1 id="Search-Iterator" class="common-anchor-header">검색 반복자<button data-href="#Search-Iterator" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h1><p>ANN 검색은 단일 쿼리에서 검색할 수 있는 엔티티 수에 최대 제한이 있으므로, 기본적인 ANN 검색만으로는 대규모 검색 요구 사항을 충족하지 못할 수 있습니다. topK가 16,384를 초과하는 ANN 검색 요청의 경우, SearchIterator 사용을 고려하는 것이 좋습니다. 이 섹션에서는 SearchIterator 사용 방법과 관련 고려 사항을 소개합니다.</p>
<h2 id="Overview" class="common-anchor-header">개요<button data-href="#Overview" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Search 요청은 검색 결과를 반환하는 반면, SearchIterator는 이터레이터를 반환합니다. 이 이터레이터의 <strong>next()</strong> 메서드를 호출하여 검색 결과를 가져올 수 있습니다.</p>
<p>구체적으로 SearchIterator는 다음과 같이 사용할 수 있습니다.</p>
<ol>
<li><p>SearchIterator를 생성하고, <strong>검색 요청당 반환할 엔티티 수</strong> 와 <strong>반환할 총 엔티티 수를</strong> 설정합니다.</p></li>
<li><p>루프 내에서 SearchIterator의 <strong>next()</strong> 메서드를 호출하여 페이징 방식으로 검색 결과를 가져옵니다.</p></li>
<li><p><strong>next()</strong> 메서드가 빈 결과를 반환하면, 이터레이터의 <strong>close()</strong> 메서드를 호출하여 루프를 종료합니다.</p></li>
</ol>
<h2 id="Create-SearchIterator" class="common-anchor-header">SearchIterator 생성<button data-href="#Create-SearchIterator" class="anchor-icon" translate="no">
    ...
    </button></h2><p>다음 코드 예제는 SearchIterator를 생성하는 방법을 보여줍니다.</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>위의 예제에서는 검색당 반환할 엔티티 수(<strong>batch_size/batchSize</strong>)를 50개로, 반환할 총 엔티티 수(<strong>topK</strong>)를 20,000개로 설정했습니다.</p>
<h2 id="Use-SearchIterator" class="common-anchor-header">SearchIterator 사용<button data-href="#Use-SearchIterator" class="anchor-icon" translate="no">
    ...
    </button></h2><p>SearchIterator가 준비되면 next() 메서드를 호출하여 검색 결과를 페이지 단위로 가져올 수 있습니다.</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#go">   Go</a>
...
<button class="copy-code-btn"></button></code></pre>
<p>위의 코드 예제에서는 무한 루프를 생성하고, 루프 내에서 <strong>next()</strong> 메서드를 호출하여 검색 결과를 변수에 저장한 후, <strong>next()</strong> 가 아무것도 반환하지 않을 때 이터레이터를 닫았습니다.</p>
