...
<h2 id="Overview" class="common-anchor-header">개요<button data-href="#Overview" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Elasticsearch에서 쿼리 컨텍스트의 작업은 정확도 점수를 생성하지만 필터 컨텍스트의 작업은 정확도 점수를 생성하지 않습니다. 마찬가지로, Milvus 검색은 유사성 점수를 생성하지만 필터와 같은 쿼리는 그렇지 않습니다. 코드 기반을 Elasticsearch에서 Milvus로 마이그레이션할 때 핵심 원칙은 유사성 점수 생성이 가능하도록 Elasticsearch의 쿼리 컨텍스트에서 사용되는 필드를 벡터 필드로 변환하는 것입니다.</p>
...
<p>다음은 일치 쿼리가 포함된 Elasticsearch 검색 요청의 예입니다.</p>
