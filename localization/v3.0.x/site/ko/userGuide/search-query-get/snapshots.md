...
<h2 id="Snapshot-anatomy" class="common-anchor-header">스냅샷 구조<button data-href="#Snapshot-anatomy" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Milvus는 실제 벡터 데이터를 중복 저장하지 않으면서도 데이터를 특정 시점 기준으로 효율적으로 캡처, 저장 및 복원하기 위해 매니페스트 기반 스냅샷 아키텍처를 구현합니다. 이 아키텍처는 메타데이터 관리와 물리적 데이터 저장을 분리하여, 오브젝트 스토리지 내의 기존 세그먼트 파일을 참조하는 경량 스냅샷을 가능하게 합니다.</p>
...
<p>스냅샷 생성은 일반적으로 밀리초 단위로 소요되며, 복원에는 데이터 양에 따라 몇 초에서 몇 분 정도 걸립니다.</p>
...
<li><a href="/docs/ko/manage-snapshots.md">스냅샷 관리</a> — 스냅샷 생성, 나열, 설명, 고정, 복원 및 삭제.</li>
