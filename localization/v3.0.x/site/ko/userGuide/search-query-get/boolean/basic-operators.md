...
<h3 id="Example-2-Filtering-with-not-equal-to-" class="common-anchor-header">예제 2: '<code translate="no">!=</code>'에 대한 '아님' 조건으로 필터링<button data-href="#Example-2-Filtering-with-not-equal-to-" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<button class="copy-code-btn"></button></code></pre>
<h3 id="Example-6-Filtering-with-less-than-or-equal-to" class="common-anchor-header">예 6: ‘보다 작거나 같음’을 사용한 필터링<button data-href="#Example-6-Filtering-with-less-than-or-equal-to" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<h2 id="Range-operators" class="common-anchor-header">범위 연산자<button data-href="#Range-operators" class="anchor-icon" translate="no">
    ...
    </button></h2><p>범위 연산자는 특정 값 집합을 기준으로 데이터를 필터링하는 데 도움이 됩니다. Milvus는 집합 소속 여부 확인을 위해 <code translate="no">IN</code> 를 지원합니다.</p>
...
<p><code translate="no">(permissions &amp; 4) == 4</code> 와 같이 결과를 비교하기 전에는 항상 비트 연산을 괄호로 묶어야 합니다. Milvus 3.0.0은 필터 표현식에서 <code translate="no">&amp;</code>, <code translate="no">|</code> 및 <code translate="no">^</code> 을 지원합니다. 비트 NOT 연산(<code translate="no">~</code>)과 시프트 연산자(<code translate="no">&lt;&lt;</code> 및 <code translate="no">&gt;&gt;</code>)는 지원되지 않습니다.</p>
</div>
...
<p>예를 들어, <code translate="no">product</code> 필드에 <code translate="no">price</code>, <code translate="no">model</code>, <code translate="no">tags</code> 와 같은 여러 키가 포함된 경우, 항상 키를 직접 참조해야 합니다:</p>
