...
<p>Milvus에서는 간단한 와일드카드 일치를 위해 ` <code translate="no">LIKE</code> `을, <a href="https://github.com/google/re2/wiki/syntax">RE2</a> 정규식을 위해 ` <code translate="no">=~</code> ` 또는 ` <code translate="no">!~</code> `을 사용하여 스칼라 필터에서 이러한 패턴 제약 조건을 표현할 수 있습니다. 이러한 필터를 ` <code translate="no">query</code>`, ` <code translate="no">search</code>` 또는 하이브리드 검색과 결합할 수 있습니다.</p>
...
<p>Milvus 정규 표현식 일치는 부분 문자열 의미를 사용합니다. 패턴이 필드 값 전체와 일치할 필요는 없습니다. 예를 들어, 다음 필터는 <code translate="no">E1001</code> 과 <code translate="no">failed with E1001 after retry</code> 모두에 일치합니다:</p>
