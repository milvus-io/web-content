...
<p>Milvus에서는 이러한 패턴 제약 조건을 스칼라 필터 내에서 ` <code translate="no">LIKE</code> `을 사용하여 간단한 와일드카드 일치를 지정하거나, ` <code translate="no">=~</code> ` 또는 ` <code translate="no">!~</code> `을 사용하여 <a href="https://github.com/google/re2/wiki/syntax">RE2</a> 정규식을 지정할 수 있습니다. 이러한 필터는 ` <code translate="no">query</code>`, ` <code translate="no">search</code>` 또는 하이브리드 검색과 결합하여 사용할 수 있습니다.</p>
