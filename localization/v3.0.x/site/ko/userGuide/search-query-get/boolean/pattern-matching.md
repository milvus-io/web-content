...
<p>Milvus에서는 간단한 와일드카드 일치를 위해 <code translate="no">LIKE</code> 를, <a href="https://github.com/google/re2/wiki/syntax">RE2</a> 정규식을 위해 <code translate="no">=~</code> 또는 <code translate="no">!~</code> 를 사용하여 스칼라 필터 내에서 이러한 패턴 제약 조건을 표현할 수 있습니다. 이러한 필터를 <code translate="no">query</code>, <code translate="no">search</code> 또는 하이브리드 검색과 결합할 수 있습니다.</p>
<div class="alert note">
<p>이 페이지에서는 <code translate="no">query</code>, <code translate="no">search</code> 및 하이브리드 검색에서 사용되는 스칼라 필터 표현식의 패턴 일치에 대해 설명합니다. 이러한 표현식은 필드 값을 평가할 뿐, 분석기가 생성한 토큰을 변경하지 않습니다. 텍스트 분석 중에 토큰을 필터링하려면 <a href="/docs/ko/regex-filter.md">정규식 분석기 필터를</a> 참조하십시오.</p>
</div>
<p>패턴 매칭 표현식은 <code translate="no">filter</code> 매개변수에 작성됩니다. 예를 들어, 다음 쿼리는 <code translate="no">E1001</code> 과 같은 오류 코드가 포함된 로그 메시지와 일치합니다.</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>이 페이지의 예제는 ` <code translate="no">filter</code>` 매개변수에 할당된 표현식에 중점을 둡니다. ` <code translate="no">query</code>`, ` <code translate="no">search</code>`, 하이브리드 검색과 같이 스칼라 필터를 허용하는 Milvus 작업에서도 동일한 필터 표현식 구문을 사용할 수 있습니다.</p>
...
<tr><td><code translate="no">JSON</code> <code translate="no">VARCHAR</code> 캐스트 유형이 지정된 경로</td><td>예</td><td>예</td><td>정확한 일치를 위해서는 JSON 경로 값이 문자열이어야 합니다. 성능을 높이기 위해 JSON 경로에 인덱스를 생성하는 경우, <code translate="no">json_cast_type=&quot;varchar&quot;</code> 를 설정하십시오.</td></tr>
...
<h2 id="Choose-LIKE-or-regex" class="common-anchor-header">LIKE 또는 정규 표현식(regex) 중 하나를 선택하십시오.<button data-href="#Choose-LIKE-or-regex" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<tbody>
<tr><td>문자열의 정확한 동일성</td><td><code translate="no">==</code></td><td><code translate="no">status == &quot;active&quot;</code></td><td><code translate="no">active</code> 라는 문자열과 정확히 일치합니다.</td></tr>
...
<tr><td>단순 포함 일치</td><td><code translate="no">LIKE</code></td><td><code translate="no">description LIKE &quot;%vector database%&quot;</code></td><td>문자열 내 어디에나 <code translate="no">vector database</code> 이 포함된 값과 일치합니다.</td></tr>
...
<h2 id="Use-LIKE" class="common-anchor-header">LIKE 사용<button data-href="#Use-LIKE" class="anchor-icon" translate="no">
    ...
    </button></h2><p><code translate="no">LIKE</code> 연산자는 문자열 값에 대한 간단한 와일드카드 일치에 사용됩니다. 다음 와일드카드만 지원합니다:</p>
<table>
<thead>
...
<h3 id="Common-LIKE-patterns" class="common-anchor-header">일반적인 LIKE 패턴<button data-href="#Common-LIKE-patterns" class="anchor-icon" translate="no">
    ...
    </button></h3><p><code translate="no">%</code> 와 <code translate="no">_</code> 의 위치를 사용하여 일치하는 문자열에서 고정 텍스트가 표시될 위치를 제어합니다.</p>
<table>
<thead>
...
<h3 id="LIKE-matching-behavior" class="common-anchor-header">LIKE 일치 동작<button data-href="#LIKE-matching-behavior" class="anchor-icon" translate="no">
    ...
    </button></h3><p>접두사, 접미사, 포함, 고정 위치의 단일 문자 일치를 위해서는 <code translate="no">LIKE</code> 를 사용하십시오. <code translate="no">LIKE</code> 는 <code translate="no">[0-9]</code> 와 같은 문자 클래스, <code translate="no">error|failed</code> 와 같은 선택, <code translate="no">{4}</code> 와 같은 반복 횟수, <code translate="no">^</code> 또는 <code translate="no">$</code> 와 같은 앵커, <code translate="no">(?i)</code> 와 같은 대소문자 구분 없음 플래그를 지원하지 않습니다. 이러한 패턴의 경우 정규 표현식(regex)을 사용하십시오.</p>
<p>전체 문자열이 정확히 일치하는 경우 <code translate="no">==</code> 를 사용하십시오. 필터에 와일드카드 일치가 필요한 경우에만 <code translate="no">LIKE</code> 를 사용하십시오.</p>
<h3 id="Escaping-wildcards-in-a-LIKE-pattern" class="common-anchor-header">LIKE 패턴에서 와일드카드 이스케이프 처리<button data-href="#Escaping-wildcards-in-a-LIKE-pattern" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p><code translate="no">LIKE</code> 패턴에서 <code translate="no">%</code> 는 0개 이상의 문자와 일치하고, <code translate="no">_</code> 는 정확히 하나의 문자와 일치합니다. <code translate="no">%</code>, <code translate="no">_</code> 또는 <code translate="no">\</code> 를 문자 그대로 일치시키려면, 백슬래시(<code translate="no">\</code>)로 문자를 이스케이프 처리하십시오:</p>
<ul>
<li><code translate="no">name LIKE r&quot;\%&quot;</code> <code translate="no">%</code> 라는 리터럴 값과 일치합니다.</li>
<li><code translate="no">name LIKE r&quot;\_%&quot;</code> 리터럴 <code translate="no">_</code> 로 시작하는 값과 일치합니다.</li>
<li><code translate="no">name LIKE r&quot;\\%&quot;</code> 리터럴 백슬래시로 시작하는 값과 일치합니다.</li>
</ul>
<p><code translate="no">r&quot;...&quot;</code> 또는 <code translate="no">r'...'</code> 형태로 작성된 원시 문자열 리터럴은 Milvus 필터 표현식에서 백슬래시를 그대로 유지합니다. 백슬래시가 포함된 <code translate="no">LIKE</code> 및 정규식 패턴의 경우 원시 문자열을 사용하는 것이 권장됩니다. 원시 문자열을 사용하지 않으면 일반 문자열 리터럴은 패턴이 평가되기 전에 이스케이프 시퀀스를 처리하므로, 더 많은 백슬래시가 필요할 수 있습니다.</p>
<h2 id="Use-regex--Milvus-30x" class="common-anchor-header">정규 표현식 사용<span class="beta-tag" style="background-color:rgb(0, 179, 255);color:white" translate="no">Compatible with Milvus 3.0.x</span><button data-href="#Use-regex--Milvus-30x" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<p><code translate="no">=~</code> 또는 <code translate="no">!~</code> 의 우측은 문자열 리터럴이어야 합니다.</p>
<table>
<thead>
...
<h3 id="Use-raw-string-literals" class="common-anchor-header">원시 문자열 리터럴 사용<button data-href="#Use-raw-string-literals" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>백슬래시가 포함된 정규 표현식 패턴의 경우 원시 문자열 리터럴을 사용하는 것이 좋습니다. <code translate="no">r&quot;...&quot;</code> 또는 <code translate="no">r'...'</code> 형태로 작성된 원시 문자열에서는 백슬래시가 그대로 정규 표현식 엔진으로 전달됩니다. 이를 통해 일반 문자열 리터럴에서 필요한 추가 이스케이프 처리를 피할 수 있습니다.</p>
<p>예:</p>
<div class="multipleCode">
 <a href="#python">Python</a>
 <a href="#java"> Java</a>
...
<button class="copy-code-btn"></button></code></pre>
<p>이는 <code translate="no">report.json</code> 과 같이 <code translate="no">.json</code> 으로 끝나는 문자열과 일치합니다.</p>
<p>Milvus 필터 표현식에 원시 문자열이 포함되지 않은 경우, 일반 문자열 리터럴은 정규식 패턴이 평가되기 전에 이스케이프 시퀀스를 처리합니다. 따라서 이스케이프 처리된 리터럴 문자의 경우 호스트 언어 문자열에 추가적인 백슬래시가 필요할 수 있습니다.</p>
<h3 id="Common-regex-patterns" class="common-anchor-header">일반적인 정규 표현식 패턴<button data-href="#Common-regex-patterns" class="anchor-icon" translate="no">
    ...
    </button></h3><p>다음 예제는 Milvus 필터 표현식에서 흔히 사용되는 RE2 구문을 보여줍니다. 전체 정규식 구문에 대해서는 <a href="https://github.com/google/re2/wiki/syntax">RE2 구문</a> 참조를 참조하십시오.</p>
<table>
<thead>
...
<tr><td>대소문자를 구분하지 않고 일치시킵니다</td><td><code translate="no">(?i)error</code></td><td><code translate="no">filter = 'message =~ &quot;(?i)error&quot;'</code></td></tr>
<tr><td>전체 문자열과 일치</td><td><code translate="no">^prod-[0-9]+$</code></td><td><code translate="no">filter = 'name =~ &quot;^prod-[0-9]+$&quot;'</code></td></tr>
</tbody>
</table>
<p>여러 단어 중 하나와 일치시키려면, ` <code translate="no">|</code>`을 사용하여 선택지를 지정합니다:</p>
<div class="multipleCode">
 <a href="#python">Python</a>
 <a href="#java"> Java</a>
 <a href="#go"> Go</a>
...
<button class="copy-code-btn"></button></code></pre>
<p>정규 표현식 메타문자를 리터럴로 일치시키려면, 정규 표현식 패턴에서 해당 문자를 이스케이프 처리하십시오. 예를 들어, 리터럴 점(정규 표현식에서<code translate="no">\.</code> )을 일치시키려면 Python, Java, Go 또는 Node.js 소스 문자열에 <code translate="no">\\.</code> 을 작성하십시오:</p>
<div class="multipleCode">
 <a href="#python">Python</a>
 <a href="#java"> Java</a>
 <a href="#go"> Go</a>
...
<p>정규식 필터는 null 값과 일치하지 않습니다. 이는 <code translate="no">=~</code> 및 <code translate="no">!~</code> 모두에 적용됩니다. 정규식 패턴을 제외하고 null 값은 유지하려면 <code translate="no">OR field IS NULL</code> 을 명시적으로 추가하십시오:</p>
<div class="multipleCode">
 <a href="#python">Python</a>
 <a href="#java"> Java</a>
 <a href="#go"> Go</a>
...
<table>
<thead>
<tr><th>필터</th><th>누락/null/문자열이 아닌 값을 포함합니까?</th><th>참고</th></tr>
</thead>
<tbody>
...
<h2 id="Accelerate-pattern-matching-with-indexes" class="common-anchor-header">인덱스를 활용한 패턴 매칭 가속화<button data-href="#Accelerate-pattern-matching-with-indexes" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Milvus는 문자열 필드에 대해 여러 인덱스 유형을 지원하며, 이는 <code translate="no">VARCHAR</code> 필드 또는 JSON 문자열 경로(예: <code translate="no">NGRAM</code>, <code translate="no">STL_SORT</code>, <code translate="no">INVERTED</code>, <code translate="no">BITMAP</code>)에 대한 <code translate="no">LIKE</code> 및 정규식 필터와 함께 사용할 수 있습니다. 인덱스 없이도 패턴 매칭이 가능하지만, 인덱스를 사용하면 대용량 데이터셋에서 성능이 향상될 수 있습니다.</p>
<p>인덱스의 효과는 패턴 표현식, Milvus가 고정된 리터럴 부분 문자열을 추출할 수 있는지 여부, 그리고 대상 필드의 카디널리티와 분포에 따라 달라집니다. <code translate="no">name LIKE &quot;Prod%&quot;</code> 과 같은 접두사 스타일 패턴은 <code translate="no">description LIKE &quot;%vector%&quot;</code> 또는 <code translate="no">filename LIKE &quot;%.json&quot;</code> 과 같은 중위 또는 접미사 패턴과는 다른 인덱스 전략을 적용할 때 더 나은 성능을 보일 수 있습니다.</p>
...
<tbody>
<tr><td><code translate="no">message =~ &quot;error.*timeout&quot;</code> 와 같이 고정된 리터럴 부분 문자열을 포함하는 경우, 또는 <code translate="no">message LIKE &quot;%database%&quot;</code></td><td><code translate="no">NGRAM</code></td><td>Milvus가 패턴에서 의미 있는 리터럴 부분 문자열을 추출할 수 있을 때 유용합니다. 자세한 내용은 <a href="/docs/ko/ngram.md">NGRAM을</a> 참조하십시오.</td></tr>
...
<tr><td>고정 리터럴이 없는 정규식 패턴, 또는 문자 클래스, 짧은 토큰, 와일드카드가 주를 이루는 패턴</td><td>인덱스 가속화를 활용하기 전에 벤치마크를 수행하십시오</td><td>이러한 패턴은 인덱스 선택성이 제한적일 수 있으며, 더 광범위한 스캔으로 전환될 수 있습니다.</td></tr>
</tbody>
</table>
