...
<h1 id="Pattern-Matching" class="common-anchor-header">패턴 매칭<button data-href="#Pattern-Matching" class="anchor-icon" translate="no">
    ...
    </button></h1><p>에이전트 기반 검색 애플리케이션에서 벡터 검색과 grep 스타일의 패턴 매칭은 종종 서로를 보완합니다. 벡터 검색은 의미적으로 관련된 엔티티를 검색하는 반면, 패턴 매칭은 오류 코드, 로그 접두사, 이메일 도메인, URL 경로 또는 식별자 등과 같은 정확한 문자열 구조를 기준으로 해당 결과를 좁혀줍니다.</p>
<p>Milvus에서는 간단한 와일드카드 일치를 위해 <code translate="no">LIKE</code> 를, <a href="https://github.com/google/re2/wiki/syntax">RE2</a> 정규 표현식을 위해 <code translate="no">=~</code> 또는 <code translate="no">!~</code> 를 사용하여 스칼라 필터 내에서 이러한 패턴 제약 조건을 표현할 수 있습니다. 이러한 필터를 <code translate="no">query</code>, <code translate="no">search</code> 또는 하이브리드 검색과 결합할 수 있습니다.</p>
<div class="alert note">
<p>이 페이지에서는 <code translate="no">query</code>, <code translate="no">search</code> 및 하이브리드 검색에서 사용되는 스칼라 필터 표현식의 패턴 일치에 대해 설명합니다. 이러한 표현식은 필드 값을 평가할 뿐, 분석기가 생성한 토큰을 변경하지는 않습니다. 텍스트 분석 중에 토큰을 필터링하려면 <a href="/docs/ko/regex-filter.md">정규식 분석기 필터를</a> 참조하십시오.</p>
</div>
<p>패턴 매칭 표현식은 <code translate="no">filter</code> 매개변수에 작성됩니다. 예를 들어, 다음 쿼리는 <code translate="no">E1001</code> 와 같은 오류 코드가 포함된 로그 메시지와 일치합니다:</p>
...
<tr><td><code translate="no">JSON</code> <code translate="no">VARCHAR</code> 로 형변환된 경로</td><td>예</td><td>예</td><td>정확한 일치를 위해서는 JSON 경로 값이 문자열이어야 합니다. 성능 향상을 위해 JSON 경로에 인덱스를 생성하는 경우, ` <code translate="no">json_cast_type=&quot;varchar&quot;</code>`를 설정하십시오.</td></tr>
<tr><td><code translate="no">ARRAY&lt;VARCHAR&gt;</code> element</td><td>예</td><td>예</td><td><code translate="no">tags[0]</code> 와 같이 인덱스를 사용하여 특정 요소와 일치시킵니다. 패턴 일치는 모든 요소를 스캔하지 <strong>않으며</strong>, 지정된 인덱스의 요소에만 적용됩니다.</td></tr>
<tr><td>숫자, 부울, 벡터, <code translate="no">TEXT</code> 또는 기타<code translate="no">VARCHAR</code> 대상이 아닌 대상</td><td>아니요</td><td>아니요</td><td>패턴 매칭은 <code translate="no">VARCHAR</code> 값, 문자열로 변환되는 JSON 경로 또는 인덱스가 지정된 <code translate="no">ARRAY&lt;VARCHAR&gt;</code> 요소에만 사용할 수 있습니다.</td></tr>
</tbody>
</table>
...
<p>간단한 와일드카드 일치에는 <code translate="no">LIKE</code> 를 사용하십시오. 패턴에 문자 클래스, 반복, <code translate="no">error|failed</code> 와 같은 선택, 앵커 또는 대소문자를 구분하지 않는 일치가 필요한 경우에는 정규식을 사용하십시오.</p>
...
<h3 id="Common-LIKE-patterns" class="common-anchor-header">일반적인 LIKE 패턴<button data-href="#Common-LIKE-patterns" class="anchor-icon" translate="no">
    ...
    </button></h3><p><code translate="no">%</code> 와 <code translate="no">_</code> 의 위치를 사용하여 일치하는 문자열에서 고정된 텍스트가 나타날 위치를 제어합니다.</p>
<table>
<thead>
...
<p>전체 문자열이 정확히 일치하는 경우 <code translate="no">==</code> 을 사용하십시오. 필터에 와일드카드 일치가 필요한 경우에만 <code translate="no">LIKE</code> 을 사용하십시오.</p>
...
<h2 id="Use-regex" class="common-anchor-header">정규 표현식 사용<span class="beta-tag" style="background-color:rgb(0, 179, 255);color:white" translate="no">Compatible with Milvus 3.0.x</span><button data-href="#Use-regex" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>패턴에 문자 클래스, 반복, 대안, 앵커 또는 대소문자 구분 없는 일치와 같은 정규 표현식 기능이 필요한 경우 정규 표현식 필터를 사용하십시오. Milvus는 문자열 값에 <a href="https://github.com/google/re2/wiki/syntax">RE2</a> 정규 표현식을 적용합니다.</p>
<p><code translate="no">=~</code> 또는 <code translate="no">!~</code> 의 우측은 반드시 문자열 리터럴이어야 합니다.</p>
<table>
<thead>
...
<button class="copy-code-btn"></button></code></pre>
<p>이는 <code translate="no">report.json</code> 과 같이 <code translate="no">.json</code> 로 끝나는 문자열과 일치합니다.</p>
<p>Milvus 필터 표현식에 원시 문자열이 포함되지 않은 경우, 일반 문자열 리터럴은 정규식 패턴이 평가되기 전에 이스케이프 시퀀스를 처리합니다. 따라서 이스케이프된 리터럴 문자의 경우 호스트 언어 문자열에 추가적인 백슬래시가 필요할 수 있습니다.</p>
...
<tr><td>접두사로 시작하는 경우</td><td><code translate="no">^ERR</code></td><td><code translate="no">filter = 'code =~ &quot;^ERR&quot;'</code></td></tr>
...
<button class="copy-code-btn"></button></code></pre>
<p>정규 표현식 메타문자를 리터럴로 일치시키려면 정규 표현식 패턴에서 해당 문자를 이스케이프 처리해야 합니다. 예를 들어, 리터럴 점(정규 표현식에서 `<code translate="no">\.</code> `)과 일치시키려면 Python, Java, Go 또는 Node.js 소스 문자열에 ` <code translate="no">\\.</code> `을 작성합니다:</p>
<div class="multipleCode">
 <a href="#python">Python</a>
 <a href="#java"> Java</a>
 <a href="#go"> Go</a>
...
<p>Milvus의 정규 표현식 일치는 부분 문자열 의미를 사용합니다. 패턴이 필드 값 전체와 일치할 필요는 없습니다. 예를 들어, 다음 필터는 <code translate="no">E1001</code> 과 <code translate="no">failed with E1001 after retry</code> 모두에 일치합니다:</p>
<div class="multipleCode">
 <a href="#python">Python</a>
 <a href="#java"> Java</a>
 <a href="#go"> Go</a>
...
<p>정규식 필터는 null 값과 일치하지 않습니다. 이는 <code translate="no">=~</code> 및 <code translate="no">!~</code> 모두에 적용됩니다. 정규식 패턴은 제외하되 null 값은 유지하려면 <code translate="no">OR field IS NULL</code> 를 명시적으로 추가하십시오:</p>
<div class="multipleCode">
 <a href="#python">Python</a>
 <a href="#java"> Java</a>
 <a href="#go"> Go</a>
...
<h2 id="Accelerate-pattern-matching-with-indexes" class="common-anchor-header">인덱스를 사용하여 패턴 일치 속도 향상<button data-href="#Accelerate-pattern-matching-with-indexes" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>Milvus는 문자열 필드에 대해 여러 인덱스 유형을 지원하며, 이러한 인덱스는 <code translate="no">VARCHAR</code> 필드 또는 JSON 문자열 경로에서 <code translate="no">LIKE</code> 및 정규식 필터와 함께 사용할 수 있습니다(예: <code translate="no">NGRAM</code>, <code translate="no">STL_SORT</code>, <code translate="no">INVERTED</code>, <code translate="no">BITMAP</code>). 패턴 매칭은 인덱스 없이도 작동할 수 있지만, 인덱스를 사용하면 대용량 데이터셋에서 성능이 향상될 수 있습니다.</p>
