...
title: 패턴 매칭
summary: >-
  Milvus는 LIKE 와일드카드 패턴 및 RE2 정규식을 사용한 문자열 패턴 일치 기능을 지원합니다. 패턴 필터를 사용하여 VARCHAR
  필드, JSON 문자열 경로 또는 ARRAY 요소 내에서 접두사, 접미사, 부분 문자열, 구조화된 코드, 이메일 도메인, URL 경로 및
  기타 문자열 패턴과 일치시킬 수 있습니다.
---
<h1 id="Pattern-Matching" class="common-anchor-header">패턴 매칭<button data-href="#Pattern-Matching" class="anchor-icon" translate="no">
    ...
    </button></h1><p>에이전트 기반 검색 애플리케이션에서 벡터 검색과 grep 스타일의 패턴 매칭은 종종 서로를 보완합니다. 벡터 검색은 의미적으로 관련성이 있는 엔티티를 검색하는 반면, 패턴 매칭은 오류 코드, 로그 접두사, 이메일 도메인, URL 경로 또는 식별자와 같은 정확한 문자열 구조를 통해 해당 결과를 좁혀줍니다.</p>
<p>Milvus에서는 간단한 와일드카드 일치를 위해 <code translate="no">LIKE</code> 를, <a href="https://github.com/google/re2/wiki/syntax">RE2</a> 정규식을 위해 <code translate="no">=~</code> 또는 <code translate="no">!~</code> 를 사용하여 스칼라 필터에서 이러한 패턴 제약 조건을 표현할 수 있습니다. 이러한 필터를 <code translate="no">query</code>, <code translate="no">search</code> 또는 하이브리드 검색과 결합할 수 있습니다.</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>이 페이지의 예제는 ` <code translate="no">filter</code>` 매개변수에 할당된 표현식에 중점을 둡니다. ` <code translate="no">query</code>`, ` <code translate="no">search</code>` 및 하이브리드 검색과 같이 스칼라 필터를 허용하는 Milvus 작업에서도 동일한 필터 표현식 구문을 사용할 수 있습니다.</p>
<h2 id="Supported-field-types" class="common-anchor-header">지원되는 필드 유형<button data-href="#Supported-field-types" class="anchor-icon" translate="no">
    ...
    </button></h2><p>패턴 매칭은 문자열 값에 사용할 수 있습니다.</p>
<table>
<thead>
...
<tr><td><code translate="no">JSON</code> <code translate="no">VARCHAR</code> 의 캐스트 유형이 지정된 경로</td><td>예</td><td>예</td><td>정확한 일치를 위해서는 JSON 경로 값이 문자열이어야 합니다. 성능 향상을 위해 JSON 경로에 인덱스를 생성하는 경우, ` <code translate="no">json_cast_type=&quot;varchar&quot;</code>`을 설정하십시오.</td></tr>
...
<tr><td>단순 포함 일치</td><td><code translate="no">LIKE</code></td><td><code translate="no">description LIKE &quot;%vector database%&quot;</code></td><td>문자열 내 어디서든 <code translate="no">vector database</code> 를 포함하는 값과 일치합니다.</td></tr>
...
<h3 id="Common-LIKE-patterns" class="common-anchor-header">일반적인 LIKE 패턴<button data-href="#Common-LIKE-patterns" class="anchor-icon" translate="no">
    ...
    </button></h3><p><code translate="no">%</code> 와 <code translate="no">_</code> 의 위치를 사용하여 일치하는 문자열에서 고정 텍스트가 나타날 위치를 제어합니다.</p>
<table>
<thead>
...
<h3 id="Use-raw-string-literals" class="common-anchor-header">원시 문자열 리터럴 사용<button data-href="#Use-raw-string-literals" class="anchor-icon" translate="no">
    ...
    </button></h3><p>백슬래시가 포함된 정규 표현식 패턴의 경우 원시 문자열 리터럴을 사용하는 것이 좋습니다. <code translate="no">r&quot;...&quot;</code> 또는 <code translate="no">r'...'</code> 형태로 작성된 원시 문자열에서는 백슬래시가 그대로 정규 표현식 엔진으로 전달됩니다. 이를 통해 일반 문자열 리터럴에서 필요한 추가적인 이스케이프 처리를 피할 수 있습니다.</p>
<p>예:</p>
<div class="multipleCode">
 <a href="#python">Python</a>
 <a href="#java"> Java</a>
...
<p>Milvus 필터 표현식에 원시 문자열이 없는 경우, 일반 문자열 리터럴은 정규식 패턴이 평가되기 전에 이스케이프 시퀀스를 처리합니다. 따라서 이스케이프된 리터럴 문자의 경우 호스트 언어 문자열에 추가적인 백슬래시가 필요할 수 있습니다.</p>
<h3 id="Common-regex-patterns" class="common-anchor-header">일반적인 정규 표현식 패턴<button data-href="#Common-regex-patterns" class="anchor-icon" translate="no">
    ...
    </button></h3><p>다음 예제는 Milvus 필터 표현식에서 흔히 사용되는 RE2 구문을 보여줍니다. 전체 정규 표현식 구문에 대해서는 <a href="https://github.com/google/re2/wiki/syntax">RE2 구문</a> 참조를 참조하십시오.</p>
<table>
<thead>
...
<p>여러 단어 중 하나와 일치시키려면 <code translate="no">|</code> 를 사용하여 선택 기호를 사용하십시오:</p>
<div class="multipleCode">
 <a href="#python">Python</a>
 <a href="#java"> Java</a>
 <a href="#go"> Go</a>
...
<button class="copy-code-btn"></button></code></pre>
<p>정규 표현식 메타문자를 리터럴로 일치시키려면, 정규 표현식 패턴 내에서 해당 문자를 이스케이프 처리해야 합니다. 예를 들어, 리터럴 점(정규 표현식에서 `<code translate="no">\.</code> `)을 일치시키려면, Python, Java, Go 또는 Node.js 소스 문자열에 ` <code translate="no">\\.</code> `를 작성합니다:</p>
<div class="multipleCode">
 <a href="#python">Python</a>
 <a href="#java"> Java</a>
 <a href="#go"> Go</a>
...
<button class="copy-code-btn"></button></code></pre>
<p>참고: Milvus 정규식 필터는 RE2 구문을 따릅니다. 정규식 패턴이 RE2에서 지원하지 않는 구문을 사용하거나 다른 이유로 유효하지 않은 경우, Milvus는 해당 필터 표현식을 거부합니다. 정규식 메타문자, 플래그 및 일치 동작에 대한 자세한 내용은 <a href="https://github.com/google/re2/wiki/syntax">RE2 구문</a> 참조를 참조하십시오.</p>
...
<p>Milvus 정규식 일치 처리는 부분 문자열 의미를 사용합니다. 패턴이 필드 값 전체와 일치할 필요는 없습니다. 예를 들어, 다음 필터는 <code translate="no">E1001</code> 과 <code translate="no">failed with E1001 after retry</code> 모두에 일치합니다:</p>
<div class="multipleCode">
 <a href="#python">Python</a>
 <a href="#java"> Java</a>
 <a href="#go"> Go</a>
...
<h2 id="Accelerate-pattern-matching-with-indexes" class="common-anchor-header">인덱스를 사용하여 패턴 일치 속도 향상<button data-href="#Accelerate-pattern-matching-with-indexes" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Milvus는 <code translate="no">NGRAM</code>, <code translate="no">STL_SORT</code>, <code translate="no">INVERTED</code>, <code translate="no">BITMAP</code> 와 같이 <code translate="no">VARCHAR</code> 필드 또는 JSON 문자열 경로에 대한 <code translate="no">LIKE</code> 및 정규식 필터와 함께 사용할 수 있는 여러 가지 문자열 필드 인덱스 유형을 지원합니다. 패턴 매칭은 인덱스 없이도 작동할 수 있지만, 인덱스를 사용하면 대용량 데이터 세트에서 성능이 향상될 수 있습니다.</p>
<p>인덱스의 효과는 패턴 표현식, Milvus가 고정된 리터럴 부분 문자열을 추출할 수 있는지 여부, 그리고 대상 필드의 카디널리티와 분포에 따라 달라집니다. <code translate="no">name LIKE &quot;Prod%&quot;</code> 와 같은 접두사 스타일 패턴은 <code translate="no">description LIKE &quot;%vector%&quot;</code> 또는 <code translate="no">filename LIKE &quot;%.json&quot;</code> 와 같은 중위사 또는 접미사 패턴과는 다른 인덱스 전략을 적용하는 것이 유리할 수 있습니다.</p>
