...
<h3 id="Basic-Operators-in-JSON-Fields" class="common-anchor-header">JSON 필드의 기본 연산자<button data-href="#Basic-Operators-in-JSON-Fields" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>다음 표에서는 <code translate="no">json_key</code> 라는 JSON 필드 값에 <code translate="no">A</code> 이라는 키가 있다고 가정합니다. JSON 필드 키를 사용하여 부울 표현식을 작성할 때 참조로 사용하세요.</p>
<table>
<thead>
...
<h3 id="Advanced-Operators" class="common-anchor-header">고급 연산자<button data-href="#Advanced-Operators" class="anchor-icon" translate="no">
    ...
    </button></h3><p>다음 연산자는 JSON 필드에만 해당합니다:</p>
<ul>
