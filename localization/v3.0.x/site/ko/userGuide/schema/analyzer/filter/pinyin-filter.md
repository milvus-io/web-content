...
<p>이 약식 설정은 원본 중국어 토큰을 유지하면서 문자 단위의 병음 토큰을 출력합니다. 해당 옵션을 명시적으로 활성화하지 않는 한, 결합된 병음이나 병음 초성 형태는 출력되지 않습니다.</p>
...
<tr><td><code translate="no">keep_separate_first_letter</code></td><td>부울</td><td><code translate="no">false</code></td><td>각 소스 토큰에 대해 피inyin 이니셜 토큰을 출력합니다. 예를 들어, <code translate="no">中文</code> 는 <code translate="no">zw</code> 를 생성합니다.</td></tr>
</tbody>
</table>
...
<h3 id="Match-Chinese-text-with-character-level-Pinyin" class="common-anchor-header">중국어 텍스트를 문자 단위의 병음과 매칭<button data-href="#Match-Chinese-text-with-character-level-Pinyin" class="anchor-icon" translate="no">
    ...
    </button></h3><p>기본 <code translate="no">pinyin</code> 필터는 원본 중국어 토큰을 유지하면서 문자 단위의 병음 토큰을 출력합니다.</p>
<pre><code translate="no" class="language-python">analyzer_params = {
    <span class="hljs-string">&quot;tokenizer&quot;</span>: <span class="hljs-string">&quot;jieba&quot;</span>,
...
<h3 id="Match-Chinese-terms-with-Pinyin-initials" class="common-anchor-header">중국어 용어와 병음 초성 일치<button data-href="#Match-Chinese-terms-with-Pinyin-initials" class="anchor-icon" translate="no">
    ...
    </button></h3><p>중국어 용어를 해당 병음 형태의 초성과 일치시켜야 할 경우 ‘ <code translate="no">keep_separate_first_letter</code> ’를 활성화하십시오.</p>
<pre><code translate="no" class="language-python">analyzer_params = {
    <span class="hljs-string">&quot;tokenizer&quot;</span>: <span class="hljs-string">&quot;jieba&quot;</span>,
    <span class="hljs-string">&quot;filter&quot;</span>: [
        {
