...
title: 스템머
summary: >-
  스템머 필터는 단어를 기본형 또는 어근 형태로 변환(이를 ‘스템밍’이라고 함)하여, 다양한 어형 변화에도 불구하고 유사한 의미를 가진
  단어들을 더 쉽게 매칭할 수 있게 해줍니다. 이 스템머 필터는 여러 언어를 지원하므로, 다양한 언어적 맥락에서 효과적인 검색 및 색인 생성이
  가능합니다.
---
<h1 id="Stemmer" class="common-anchor-header">스템머<button data-href="#Stemmer" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h1><p><code translate="no">stemmer</code> 필터는 단어를 기본형 또는 어근 형태로 변환(스템밍이라고 함)하여, 다양한 어형 변화에도 불구하고 유사한 의미를 가진 단어들을 더 쉽게 매칭할 수 있게 해줍니다. <code translate="no">stemmer</code> 필터는 여러 언어를 지원하므로, 다양한 언어적 맥락에서 효과적인 검색 및 색인 생성이 가능합니다.</p>
<h2 id="Configuration" class="common-anchor-header">구성<button data-href="#Configuration" class="anchor-icon" translate="no">
    ...
    </button></h2><p><code translate="no">stemmer</code> 필터는 Milvus의 사용자 정의 필터입니다. 이 필터를 사용하려면 필터 구성에서 <code translate="no">&quot;type&quot;: &quot;stemmer&quot;</code> 을 지정하고, 스템밍에 사용할 언어를 선택하기 위해 <code translate="no">language</code> 매개변수를 함께 지정해야 합니다.</p>
...
<button class="copy-code-btn"></button></code></pre>
<p><code translate="no">stemmer</code> 필터는 다음과 같은 구성 가능한 매개변수를 지원합니다.</p>
<table>
   <tr>
     <th><p>매개변수</p></th>
     <th><p>설명</p></th>
   ...
   <tr>
     ...
     <td><p>어근 추출 프로세스에 사용할 언어를 지정합니다. 지원되는 언어는 다음과 같습니다: <code translate="no">"arabic"</code>, <code translate="no">"danish"</code>, <code translate="no">"dutch"</code>, <code translate="no">"english"</code>, <code translate="no">"finnish"</code>, <code translate="no">"french"</code>, <code translate="no">"german"</code>, <code translate="no">"greek"</code>, <code translate="no">"hungarian"</code>, <code translate="no">"italian"</code>, <code translate="no">"norwegian"</code>, <code translate="no">"portuguese"</code>, <code translate="no">"romanian"</code>, <code translate="no">"russian"</code>, <code translate="no">"spanish"</code>, <code translate="no">"swedish"</code>, <code translate="no">"tamil"</code>, <code translate="no">"turkish"</code></p></td>
   </tr>
</table>
<p><code translate="no">stemmer</code> 필터는 토큰화기가 생성한 용어를 대상으로 작동하므로, 반드시 토큰화기와 함께 사용해야 합니다.</p>
<p>아랍어 텍스트의 경우, 대부분의 경우 내장된 <a href="/docs/ko/arabic-analyzer.md"><code translate="no">arabic</code></a> 분석기를 사용하십시오. 이 내장 분석기에는 아랍어 어근 추출 기능과 아랍어 정규화, 소수점 숫자 정규화, 아랍어 스톱워드 제거 기능이 포함되어 있습니다. 사용자 정의 분석기 파이프라인을 구축해야 하는 경우에만 <code translate="no">stemmer</code> 필터를 직접 사용하십시오.</p>
<p><code translate="no">analyzer_params</code> 를 정의한 후에는 컬렉션 스키마를 정의할 때 <code translate="no">VARCHAR</code> 필드에 이를 적용할 수 있습니다. 이를 통해 Milvus는 지정된 분석기를 사용하여 해당 필드의 텍스트를 처리함으로써 효율적인 토큰화 및 필터링을 수행할 수 있습니다. 자세한 내용은 <a href="/docs/ko/analyzer-overview.md#Example-use">사용 예제를</a> 참조하십시오.</p>
