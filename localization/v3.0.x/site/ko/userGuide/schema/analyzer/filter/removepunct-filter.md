...
summary: >-
  removepunct 필터는 구두점이나 공백이 포함된 토큰을 제거합니다. 단어 내부의 구두점을 그대로 유지하는 토큰화기를 사용하는 경우, 이
  필터는 해당 토큰 전체를 제거합니다.
beta: Milvus 2.5.11+
---
<h1 id="Remove-Punct" class="common-anchor-header">구두점 제거<span class="beta-tag" style="background-color:rgb(0, 179, 255);color:white" translate="no">Compatible with Milvus 2.5.11+</span><button data-href="#Remove-Punct" class="anchor-icon" translate="no">
    ...
    </button></h1><p><code translate="no">removepunct</code> 필터는 토큰 스트림에서 구두점이나 공백을 포함하는 토큰을 제거합니다. 구두점보다는 의미 있는 내용어에 초점을 맞춘 더 깔끔한 텍스트 처리를 원할 때 이 필터를 사용하십시오.</p>
<div class="alert note">
<p>이 필터는 구두점을 별도의 토큰으로 보존하는 <code translate="no">jieba</code>, <code translate="no">lindera</code> 및 <code translate="no">icu</code> 토큰화기와 함께 사용할 때 가장 효과적입니다(예: <code translate="no">&quot;Hello!&quot;</code> → <code translate="no">[&quot;Hello&quot;, &quot;!&quot;]</code>). <code translate="no">standard</code> 토큰화기는 토큰화 과정에서 구두점을 제거합니다. <code translate="no">whitespace</code> 토큰화기는 토큰 내부의 구두점을 포함하여 구두점을 그대로 유지합니다. <code translate="no">whitespace</code> 와 결합될 경우, <code translate="no">removepunct</code> 는 구두점이나 공백이 포함된 토큰 전체를 제거하며, 토큰에서 개별 문자를 제거하지는 않습니다.</p>
</div>
<h2 id="Configuration" class="common-anchor-header">구성<button data-href="#Configuration" class="anchor-icon" translate="no">
    ...
    </button></h2><p><code translate="no">removepunct</code> 필터는 Milvus에 내장되어 있습니다. 이를 사용하려면 <code translate="no">analyzer_params</code> 파일의 <code translate="no">filter</code> 섹션에 해당 이름을 지정하기만 하면 됩니다.</p>
...
<button class="copy-code-btn"></button></code></pre>
<p><code translate="no">removepunct</code> 필터는 토큰화기가 생성한 용어에 대해 작동하므로, 토큰화기와 함께 사용해야 합니다.</p>
