...
<h1 id="Remove-Punct" class="common-anchor-header">구두점 제거<span class="beta-tag" style="background-color:rgb(0, 179, 255);color:white" translate="no">Compatible with Milvus 2.5.11+</span><button data-href="#Remove-Punct" class="anchor-icon" translate="no">
    ...
    </button></h1><p><code translate="no">removepunct</code> 필터는 토큰 스트림에서 독립형 구두점 토큰을 제거합니다. 구두점보다는 의미 있는 콘텐츠 단어에 초점을 맞춘 깔끔한 텍스트 처리를 원할 때 이 필터를 사용하세요.</p>
<div class="alert note">
<p>이 필터는 구두점을 별도의 토큰으로 보존하는 <code translate="no">jieba</code>, <code translate="no">lindera</code>, <code translate="no">icu</code> 토큰화기에 가장 효과적입니다(예: <code translate="no">&quot;Hello!&quot;</code> → <code translate="no">[&quot;Hello&quot;, &quot;!&quot;]</code>). <code translate="no">standard</code> 및 <code translate="no">whitespace</code> 과 같은 다른 토큰화 도구는 토큰화 중에 구두점을 삭제하므로 <code translate="no">removepunct</code> 는 영향을 미치지 않습니다.</p>
</div>
<h2 id="Configuration" class="common-anchor-header">구성<button data-href="#Configuration" class="anchor-icon" translate="no">
    ...
    </button></h2><p><code translate="no">removepunct</code> 필터는 Milvus에 내장되어 있습니다. 이를 사용하려면 <code translate="no">analyzer_params</code> 내의 <code translate="no">filter</code> 섹션에 이름을 지정하기만 하면 됩니다.</p>
...
<button class="copy-code-btn"></button></code></pre>
<p><code translate="no">removepunct</code> 필터는 토큰화 도구에서 생성된 조건에서 작동하므로 토큰화 도구와 함께 사용해야 합니다.</p>
