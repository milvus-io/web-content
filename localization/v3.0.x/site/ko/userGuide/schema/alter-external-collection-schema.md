...
<h3 id="Add-a-scalar-field" class="common-anchor-header">스칼라 필드 추가<button data-href="#Add-a-scalar-field" class="anchor-icon" translate="no">
    ...
    </button></h3><p>쿼리 결과에 필드를 반환하거나 필터에서 사용하려는 경우 <code translate="no">add_collection_field()</code> 을 사용하여 스칼라 필드를 추가합니다. 다음 예제는 외부 데이터 소스의 <code translate="no">score</code> 필드에 매핑되는 <code translate="no">score</code> 필드를 추가합니다.</p>
