...
title: 동적 필드
summary: >-
  Milvus는 ‘동적 필드(dynamic field)’라는 특수 기능을 통해 유연하고 진화하는 구조를 가진 엔티티를 삽입할 수 있게
  해줍니다. 이 필드는 $meta라는 이름의 숨겨진 JSON 필드로 구현되며, 컬렉션 스키마에 명시적으로 정의되지 않은 데이터 내의 모든
  필드를 자동으로 저장합니다.
---
...
<li><p><strong>JSON 경로 필터를 사용하여 검색 또는 쿼리 수행</strong></p>
...
<h3 id="When-indexing-a-dynamic-field-key-what-if-the-data-casting-fails" class="common-anchor-header">동적 필드 키에 인덱스를 생성할 때 데이터 형 변환이 실패하면 어떻게 되나요?<button data-href="#When-indexing-a-dynamic-field-key-what-if-the-data-casting-fails" class="anchor-icon" translate="no">
    ...
    </button></h3><p>동적 필드 키에 인덱스를 생성하는 과정에서 데이터 형변환이 실패하는 경우(예: <code translate="no">double</code> 로 형변환되어야 하는 값이 <code translate="no">&quot;abc&quot;</code>와 같은 비숫자 문자열인 경우), 해당 특정 값들은 <strong>인덱스 생성 시 아무런 오류 메시지 없이 건너뜁니다</strong>. 이러한 값은 인덱스에 나타나지 않으므로, 해당 인덱스에 의존하는 <strong>필터 기반 검색이나 쿼리 결과에도 반환되지 않습니다</strong>.</p>
<p>이는 몇 가지 중요한 의미를 갖습니다:</p>
<ul>
...
<li><p><strong>캐스팅 함수는 신중하게 사용하십시오</strong>: 인덱싱 중에 ` <code translate="no">json_cast_function</code> `를 사용하여 문자열을 숫자로 변환하는 경우, 문자열 값이 안정적으로 변환 가능한지 확인하십시오. ` <code translate="no">json_cast_type</code> `와 실제 변환된 유형 간의 불일치는 오류나 항목 생략을 초래합니다.</p></li>
</ul>
