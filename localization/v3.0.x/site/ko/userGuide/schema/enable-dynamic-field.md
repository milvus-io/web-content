...
title: 동적 필드
summary: >-
  Milvus에서는 ‘동적 필드(dynamic field)’라는 특수 기능을 통해 유연하고 진화하는 구조를 가진 엔티티를 삽입할 수 있습니다.
  이 필드는 $meta라는 이름의 숨겨진 JSON 필드로 구현되며, 컬렉션 스키마에 명시적으로 정의되지 않은 데이터 내의 모든 필드를 자동으로
  저장합니다.
---
<h1 id="Dynamic-Field" class="common-anchor-header">동적 필드<button data-href="#Dynamic-Field" class="anchor-icon" translate="no">
    ...
    </button></h1><p>Milvus는 <strong>‘동적 필드</strong>’라는 특수 기능을 통해 유연하고 진화하는 구조를 가진 엔티티를 삽입할 수 있게 해줍니다. 이 필드는 <code translate="no">$meta</code> 라는 숨겨진 JSON 필드로 구현되며, 컬렉션 스키마에 <strong>명시적으로 정의되지 않은</strong> 데이터 내의 모든 필드를 자동으로 저장합니다.</p>
<h2 id="How-it-works" class="common-anchor-header">작동 원리<button data-href="#How-it-works" class="anchor-icon" translate="no">
    ...
    </button></h2><p>동적 필드가 활성화되면 Milvus는 각 엔티티에 숨겨진 <code translate="no">$meta</code> 필드를 추가합니다. 이 필드는 JSON 유형이므로 JSON과 호환되는 모든 데이터 구조를 저장할 수 있으며, JSON 경로 구문을 사용하여 인덱싱할 수 있습니다.</p>
<p>데이터 삽입 시, 스키마에 선언되지 않은 모든 필드는 이 동적 필드 내에 키-값 쌍으로 자동 저장됩니다.</p>
<p><code translate="no">$meta</code> 를 수동으로 관리할 필요는 없습니다. Milvus가 이를 투명하게 처리합니다.</p>
<p>예를 들어, 컬렉션 스키마에 <code translate="no">id</code> 와 <code translate="no">vector</code> 만 정의되어 있고, 다음과 같은 엔티티를 삽입하는 경우:</p>
...
<p>동적 필드 기능이 활성화되어 있으면, Milvus는 이를 내부적으로 다음과 같이 저장합니다:</p>
<pre><code translate="no" class="language-json"><span class="hljs-punctuation">{</span>
  <span class="hljs-attr">&quot;id&quot;</span><span class="hljs-punctuation">:</span> <span class="hljs-number">1</span><span class="hljs-punctuation">,</span>
  <span class="hljs-attr">&quot;vector&quot;</span><span class="hljs-punctuation">:</span> <span class="hljs-punctuation">[</span><span class="hljs-number">0.1</span><span class="hljs-punctuation">,</span> <span class="hljs-number">0.2</span><span class="hljs-punctuation">,</span> <span class="hljs-number">0.3</span><span class="hljs-punctuation">]</span><span class="hljs-punctuation">,</span>
...
<span class="hljs-punctuation">}</span>
<button class="copy-code-btn"></button></code></pre>
<p>이를 통해 스키마를 변경하지 않고도 데이터 구조를 발전시킬 수 있습니다.</p>
<p>일반적인 사용 사례는 다음과 같습니다:</p>
<ul>
<li><p>선택적 필드나 자주 조회되지 않는 필드 저장</p></li>
...
<h2 id="Supported-data-types" class="common-anchor-header">지원되는 데이터 유형<button data-href="#Supported-data-types" class="anchor-icon" translate="no">
    ...
    </button></h2><p>동적 필드는 단순 값과 복합 값을 포함하여 Milvus에서 제공하는 모든 스칼라 데이터 유형을 지원합니다. 이러한 데이터 유형은 ** <code translate="no">$meta</code> 에 저장된 키의 값**에 적용됩니다.</p>
...
<li><p>스칼라 값의 배열 (<code translate="no">ARRAY</code>)</p></li>
...
<span class="hljs-punctuation">}</span>
<button class="copy-code-btn"></button></code></pre>
<p>위의 각 키와 값은 <code translate="no">$meta</code> 필드 내에 저장됩니다.</p>
<h2 id="Enable-dynamic-field" class="common-anchor-header">동적 필드 활성화<button data-href="#Enable-dynamic-field" class="anchor-icon" translate="no">
    ...
    </button></h2><p>동적 필드 기능을 사용하려면 컬렉션 스키마를 생성할 때 ` <code translate="no">enable_dynamic_field=True</code> `를 설정하십시오:</p>
...
<h2 id="Insert-entities-to-the-collection" class="common-anchor-header">컬렉션에 엔티티 삽입<button data-href="#Insert-entities-to-the-collection" class="anchor-icon" translate="no">
    ...
    </button></h2><p>동적 필드를 사용하면 스키마에 정의되지 않은 추가 필드를 삽입할 수 있습니다. 이러한 필드는 자동으로 ` <code translate="no">$meta</code>`에 저장됩니다.</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#javascript">   NodeJS</a>
...
<button class="copy-code-btn"></button></code></pre>
<h2 id="Index-keys-in-the-dynamic-field--Milvus-2511+" class="common-anchor-header">동적 필드의 키 인덱싱<span class="beta-tag" style="background-color:rgb(0, 179, 255);color:white" translate="no">Compatible with Milvus 2.5.11+</span><button data-href="#Index-keys-in-the-dynamic-field--Milvus-2511+" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>Milvus에서는 <strong>JSON 경로 인덱싱을</strong> 사용하여 동적 필드 내의 특정 키에 대한 인덱스를 생성할 수 있습니다. 이러한 키는 스칼라 값이거나 JSON 객체 내의 중첩된 값일 수 있습니다.</p>
<div class="alert note">
<p>동적 필드 키에 대한 인덱싱은 <strong>선택</strong> 사항입니다. 인덱스가 없어도 동적 필드 키를 기준으로 쿼리하거나 필터링할 수 있지만, 무차별 대입 검색으로 인해 성능이 저하될 수 있습니다.</p>
</div>
<h3 id="JSON-path-indexing-syntax" class="common-anchor-header">JSON 경로 인덱싱 구문<button data-href="#JSON-path-indexing-syntax" class="anchor-icon" translate="no">
    ...
    </button></h3><p>JSON 경로 인덱스를 생성하려면 다음을 지정하십시오:</p>
<ul>
<li><p><strong>JSON 경로</strong> (<code translate="no">json_path</code>): 인덱싱하려는 JSON 객체 내의 키 또는 중첩된 필드로 이동하는 경로입니다.</p>
<ul>
<li><p>예: <code translate="no">metadata[&quot;category&quot;]</code></p>
<p>이는 인덱싱 엔진이 JSON 구조 내에서 어디를 검색해야 하는지를 정의합니다.</p></li>
</ul></li>
<li><p><strong>JSON 캐스트 유형</strong> (<code translate="no">json_cast_type</code>): Milvus가 지정된 경로의 값을 해석하고 색인화할 때 사용해야 하는 데이터 유형입니다.</p>
<ul>
<li><p>이 유형은 인덱싱되는 필드의 실제 데이터 유형과 일치해야 합니다.</p></li>
<li><p>전체 목록은 <a href="/docs/ko/use-json-fields.md#Supported-JSON-cast-types">지원되는 JSON 형 변환 유형</a>을 참조하십시오.</p></li>
</ul></li>
</ul>
<h3 id="Use-JSON-path-to-index-dynamic-field-keys" class="common-anchor-header">JSON 경로를 사용하여 동적 필드 키 색인 생성<button data-href="#Use-JSON-path-to-index-dynamic-field-keys" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>동적 필드는 JSON 필드이므로, JSON 경로 구문을 사용하여 그 안의 모든 키를 색인화할 수 있습니다. 이는 간단한 스칼라 값과 복잡한 중첩 구조 모두에서 작동합니다.</p>
<p><strong>JSON 경로 예시:</strong></p>
<ul>
<li><p>단순 키의 경우: <code translate="no">overview</code>, <code translate="no">words</code></p></li>
...
<h3 id="Use-JSON-cast-functions-for-type-conversion--Milvus-2514+" class="common-anchor-header">형식 변환을 위해 JSON 캐스트 함수 사용<span class="beta-tag" style="background-color:rgb(0, 179, 255);color:white" translate="no">Compatible with Milvus 2.5.14+</span><button data-href="#Use-JSON-cast-functions-for-type-conversion--Milvus-2514+" class="anchor-icon" translate="no">
    ...
    </button></h3><p>동적 필드 키에 잘못된 형식의 값이 포함된 경우(예: 숫자가 문자열로 저장된 경우), 캐스트 함수를 사용하여 변환할 수 있습니다:</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#javascript">   NodeJS</a>
...
<div class="alert note">
<ul>
<li><p>형식 변환에 실패하는 경우(예: 값 <code translate="no">&quot;not_a_number&quot;</code> 을 숫자로 변환할 수 없는 경우), 해당 값은 건너뛰어지고 인덱싱되지 않습니다.</p></li>
<li><p>캐스트 함수 매개변수에 대한 자세한 내용은 <a href="/docs/ko/use-json-fields.md#Use-JSON-cast-functions-for-type-conversion">JSON 필드를</a> 참조하십시오.</p></li>
</ul>
</div>
<h3 id="Apply-indexes-to-the-collection" class="common-anchor-header">컬렉션에 인덱스 적용<button data-href="#Apply-indexes-to-the-collection" class="anchor-icon" translate="no">
    ...
    </button></h3><p>인덱스 매개변수를 정의한 후, ` <code translate="no">create_index()</code>`를 사용하여 컬렉션에 인덱스를 적용할 수 있습니다:</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#javascript">   NodeJS</a>
...
<button class="copy-code-btn"></button></code></pre>
<h2 id="Filter-by-dynamic-field-keys" class="common-anchor-header">동적 필드 키를 사용하여 필터링<button data-href="#Filter-by-dynamic-field-keys" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>동적 필드 키를 가진 엔티티를 삽입한 후에는 표준 필터 표현식을 사용하여 필터링할 수 있습니다.</p>
<ul>
<li><p>JSON이 아닌 키(예: 문자열, 숫자, 부울)의 경우 키 이름을 직접 참조할 수 있습니다.</p></li>
<li><p>JSON 객체를 저장하는 키의 경우, JSON 경로 구문을 사용하여 중첩된 값에 접근할 수 있습니다.</p></li>
</ul>
...
<button class="copy-code-btn"></button></code></pre>
<p><strong>동적 필드 키 가져오기</strong>: 검색 또는 쿼리 결과에서 동적 필드 키를 반환하려면, 필터링과 동일한 JSON 경로 구문을 사용하여 ` <code translate="no">output_fields</code> ` 매개변수에 해당 키를 명시적으로 지정해야 합니다:</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#javascript">   NodeJS</a>
...
<button class="copy-code-btn"></button></code></pre>
<div class="alert note">
<p>동적 필드 키는 기본적으로 결과에 포함되지 않으며, 명시적으로 요청해야 합니다.</p>
</div>
<p>지원되는 연산자 및 필터 표현식의 전체 목록은 <a href="/docs/ko/filtered-search.md">‘필터링된 검색’을</a> 참조하십시오.</p>
<h2 id="Put-it-all-together" class="common-anchor-header">종합해 보기<button data-href="#Put-it-all-together" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>지금까지 스키마에 정의되지 않은 키를 동적 필드를 사용하여 유연하게 저장하고 색인화하는 방법을 배웠습니다. 동적 필드 키가 삽입되면, 특별한 구문 없이 다른 필드와 마찬가지로 필터 표현식에서 사용할 수 있습니다.</p>
<p>실제 애플리케이션에서 이 워크플로를 완료하려면 다음 작업도 수행해야 합니다.</p>
<ul>
<li><p><strong>벡터 필드에 인덱스를 생성합니다</strong> (각 컬렉션마다 필수).</p>
<p><a href="/docs/ko/create-collection.md#Optional-Set-Index-Parameters">'인덱스 매개변수 설정</a>'을 참조하십시오</p></li>
<li><p><strong>컬렉션 로드</strong></p>
<p><a href="/docs/ko/load-and-release.md">'로드 및 릴리스</a>' 참조</p></li>
<li><p><strong>JSON 경로 필터를 사용하여 검색하거나 쿼리하기</strong></p>
...
<h2 id="FAQ" class="common-anchor-header">자주 묻는 질문<button data-href="#FAQ" class="anchor-icon" translate="no">
    ...
    </button></h2><h3 id="When-should-I-define-a-field-explicitly-in-the-schema-instead-of-using-a-dynamic-field-key" class="common-anchor-header">동적 필드 키를 사용하는 대신 스키마에서 필드를 명시적으로 정의해야 하는 경우는 언제인가요?<button data-href="#When-should-I-define-a-field-explicitly-in-the-schema-instead-of-using-a-dynamic-field-key" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>다음과 같은 경우에는 동적 필드 키를 사용하는 대신 스키마에서 필드를 명시적으로 정의해야 합니다.</p>
<ul>
<li><p><strong>해당 필드가 output_fields에 자주 포함되는 경우</strong>: 명시적으로 정의된 필드만이 <code translate="no">output_fields</code> 를 통해 효율적으로 검색될 수 있습니다. 동적 필드 키는 고빈도 검색에 최적화되어 있지 않아 성능 오버헤드가 발생할 수 있습니다.</p></li>
<li><p><strong>필드에 자주 액세스하거나 필터링하는 경우</strong>: 동적 필드 키에 인덱스를 생성하면 고정 스키마 필드와 유사한 필터링 성능을 제공할 수 있지만, 명시적으로 정의된 필드는 더 명확한 구조와 우수한 유지 관리성을 제공합니다.</p></li>
<li><p><strong>필드 동작을 완전히 제어해야 하는 경우</strong>: 명시적 필드는 스키마 수준의 제약 조건, 유효성 검사 및 명확한 타입 지정을 지원하며, 이는 데이터 무결성과 일관성을 관리하는 데 유용할 수 있습니다.</p></li>
<li><p><strong>인덱싱 불일치를 방지하려는 경우</strong>: 동적 필드 키의 데이터는 유형이나 구조 면에서 불일치가 발생하기 쉽습니다. 고정 스키마를 사용하면 특히 인덱싱이나 형 변환을 사용할 계획인 경우 데이터 품질을 보장하는 데 도움이 됩니다.</p></li>
</ul>
<p>기존 컬렉션에서 동적 필드 키를 명시적 스칼라 필드로 변경하기로 결정한 경우, <a href="/docs/ko/add-fields-to-an-existing-collection.md">‘컬렉션 스키마 변경’을</a> 참조하십시오. 기존 컬렉션 수준의 동적 필드 설정은 컬렉션 속성을 통해 관리되며, 자세한 내용은 <a href="/docs/ko/modify-collection.md">‘컬렉션 수정’을</a> 참조하십시오.</p>
<h3 id="Can-I-create-multiple-indexes-on-the-same-dynamic-field-key-with-different-data-types" class="common-anchor-header">동일한 동적 필드 키에 서로 다른 데이터 유형을 가진 인덱스를 여러 개 생성할 수 있습니까?<button data-href="#Can-I-create-multiple-indexes-on-the-same-dynamic-field-key-with-different-data-types" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>아니요, <strong>JSON 경로당 하나의 인덱스만</strong> 생성할 수 있습니다. 동적 필드 키에 혼합 유형의 값(예: 일부는 문자열, 일부는 숫자)이 포함되어 있더라도 해당 경로에 인덱스를 생성할 때는 단일 데이터 유형( <code translate="no">json_cast_type</code> )을 선택해야 합니다. 현재 동일한 키에 대해 서로 다른 유형의 인덱스를 여러 개 생성하는 기능은 지원되지 않습니다.</p>
<h3 id="When-indexing-a-dynamic-field-key-what-if-the-data-casting-fails" class="common-anchor-header">동적 필드 키에 인덱스를 생성할 때 데이터 형 변환이 실패하면 어떻게 되나요?<button data-href="#When-indexing-a-dynamic-field-key-what-if-the-data-casting-fails" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>동적 필드 키에 인덱스를 생성하는 과정에서 데이터 형 변환이 실패하는 경우(예: <code translate="no">double</code> 로 변환되어야 할 값이 <code translate="no">&quot;abc&quot;</code>와 같은 숫자가 아닌 문자열인 경우), 해당 특정 값들은 <strong>인덱스 생성 시 아무런 오류 메시지 없이 건너뜁니다</strong>. 이러한 값은 인덱스에 나타나지 않으므로, 해당 인덱스에 의존하는 <strong>필터 기반 검색이나 쿼리 결과에도 반환되지 않습니다</strong>.</p>
<p>이는 몇 가지 중요한 의미를 내포합니다:</p>
<ul>
<li><p><strong>전체 스캔으로의 대체 없음</strong>: 대부분의 엔티티가 성공적으로 인덱싱된 경우, 필터링 쿼리는 전적으로 인덱스에 의존합니다. 형 변환에 실패한 엔티티는 논리적으로 필터 조건과 일치하더라도 결과 집합에서 제외됩니다.</p></li>
<li><p><strong>검색 정확도 위험</strong>: 데이터 품질이 일관되지 않은 대규모 데이터 세트(특히 동적 필드 키의 경우)에서 이러한 동작은 예상치 못한 결과 누락으로 이어질 수 있습니다. 인덱싱 전에 데이터 형식이 일관되고 유효한지 확인하는 것이 매우 중요합니다.</p></li>
<li><p><strong>캐스팅 함수는 신중하게 사용하십시오</strong>: 인덱싱 중에 ` <code translate="no">json_cast_function</code> `를 사용하여 문자열을 숫자로 변환하는 경우, 문자열 값이 안정적으로 변환 가능한지 확인하십시오. ` <code translate="no">json_cast_type</code> `와 실제 변환된 유형 간의 불일치는 오류나 항목 생략으로 이어집니다.</p></li>
</ul>
<h3 id="What-happens-if-my-query-uses-a-different-data-type-than-the-indexed-cast-type" class="common-anchor-header">쿼리에서 인덱싱된 캐스트 유형과 다른 데이터 유형을 사용하면 어떻게 되나요?<button data-href="#What-happens-if-my-query-uses-a-different-data-type-than-the-indexed-cast-type" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>쿼리에서 인덱스에 사용된 <strong>데이터 유형과 다른 데이터 유형을</strong> 사용하여 동적 필드 키를 비교하는 경우(예: 인덱스가 <code translate="no">double</code> 로 캐스팅된 상태에서 문자열 비교를 통해 쿼리하는 경우), 시스템은 <strong>인덱스를 사용하지 않으며</strong>, <em>가능한 경우에만</em> 전체 스캔으로 대체될 수 있습니다. 최상의 성능과 정확도를 얻으려면 쿼리 유형이 인덱스 생성 시 사용된 ` <code translate="no">json_cast_type</code> `와 일치하는지 확인하십시오.</p>
