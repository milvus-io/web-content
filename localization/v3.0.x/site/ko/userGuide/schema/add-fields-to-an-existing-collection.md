...
title: 컬렉션 스키마 변경
summary: >-
  컬렉션을 다시 생성하지 않고도 스칼라 필드, 벡터 필드 및 함수에 의해 생성된 벡터 필드를 추가하거나 삭제하여 기존 컬렉션 스키마를
  수정합니다.
---
...
<p><strong>함수에 의해 생성된 벡터 필드 추가</strong></p>
<ul>
<li><p>스키마 업데이트마다 하나의 함수와 하나의 생성된 벡터 필드만 추가할 수 있습니다.</p></li>
<li><p>지원되는 함수에 따라 생성된 벡터 필드의 유형이 결정됩니다. 즉, ` <code translate="no">BM25</code> `는 ` <code translate="no">SPARSE_FLOAT_VECTOR</code> ` 필드를 생성하고, ` <code translate="no">MINHASH</code> `는 ` <code translate="no">BINARY_VECTOR</code> ` 필드를 생성합니다.</p></li>
<li><p>생성된 벡터 필드는 반드시 새로운 필드여야 합니다. 컬렉션 스키마에 이미 존재하는 필드를 가리킬 수는 없습니다.</p></li>
<li><p>생성된 벡터 필드는 null 허용 필드일 수 없습니다.</p></li>
<li><p>이 함수가 사용하는 입력 필드는 컬렉션에 이미 존재해야 합니다.</p></li>
<li><p>기존 컬렉션에 BM25 또는 MinHash 함수를 추가할 때, 함수 입력은 <code translate="no">VARCHAR</code> 필드여야 합니다. Milvus는 해당 입력 유형을 사용하여 기존 엔티티에 대해 생성된 출력을 백필할 수 없기 때문에, 이 워크플로에서는 <code translate="no">TEXT</code> 입력을 지원하지 않습니다.</p></li>
</ul>
...
<p><strong>함수에 의해 생성된 벡터 필드 삭제</strong></p>
<ul>
<li><p>이 스키마 변경 워크플로에서 함수를 삭제하면 해당 함수와 함수가 생성한 출력 필드가 모두 제거됩니다. 함수 입력 필드는 컬렉션 스키마에 그대로 남아 있습니다.</p></li>
<li><p>함수의 출력 필드를 제거하면 컬렉션에 벡터 필드가 하나도 남지 않게 되는 경우, 함수 삭제는 거부됩니다.</p></li>
</ul>
<div class="alert note">
<p>지원되는 추가 및 삭제 작업 범위를 벗어난 스키마 변경의 경우, 컬렉션을 다시 생성하거나 마이그레이션하십시오.</p>
</div>
<h2 id="Add-fields-to-an-existing-collection" class="common-anchor-header">기존 컬렉션에 필드 추가<button data-href="#Add-fields-to-an-existing-collection" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>필드 값이 생성되는 방식에 따라 필드 추가 경로를 선택하십시오:</p>
<ul>
<li><p>필터링, 쿼리 결과 또는 애플리케이션 로직을 위해 새로운 메타데이터가 필요한 경우<a href="#add-user-defined-scalar-fields--milvus-26x">사용자 정의 스칼라 필드를 추가하십시오</a>.</p></li>
<li><p>요소들이 동일한 Struct 스키마를 공유하는 배열 필드가 필요한 경우<a href="#add-structarray-fields--milvus-300">StructArray 필드를 추가하십시오</a>.</p></li>
<li><p>애플리케이션이 임베딩을 생성하고 벡터 값을 Milvus에 기록하는 경우<a href="#add-user-defined-vector-fields--milvus-2618">사용자 정의 벡터 필드를 추가하십시오</a>.</p></li>
<li><p>Milvus가 기존 필드(예: 텍스트의 BM25 스파스 벡터 또는 MinHash 시그니처)로부터 벡터 값을 생성해야 하는 경우,<a href="#add-vector-fields-generated-by-functions--milvus-30x">함수에 의해 생성된 벡터 필드를 추가하십시오</a>.</p></li>
</ul>
...
<h3 id="Add-user-defined-scalar-fields--Milvus-26x" class="common-anchor-header">사용자 정의 스칼라 필드 추가<span class="beta-tag" style="background-color:rgb(0, 179, 255);color:white" translate="no">Compatible with Milvus 2.6.x</span><button data-href="#Add-user-defined-scalar-fields--Milvus-26x" class="anchor-icon" translate="no">
    ...
    </button></h3><p><code translate="no">add_collection_field()</code> 를 사용하여 기존 컬렉션에 사용자 정의 스칼라 필드를 추가하십시오.</p>
...
<li><p><code translate="no">nullable=True</code> 를 지정하고 <code translate="no">default_value</code> 를 함께 지정한 스칼라 필드를 추가하면, 기존 엔티티는 새 필드에 대해 <code translate="no">NULL</code> 를 반환합니다.</p></li>
</ul>
...
<p>추가된 모든 사용자 정의 벡터 필드는 nullable이어야 합니다. 기존 엔티티의 경우, upsert 또는 백필(backfill) 워크플로를 통해 벡터 값을 기록할 때까지 새 벡터 필드에 대해 <code translate="no">NULL</code> 가 적용됩니다. 새 엔티티는 삽입 시 해당 벡터 필드를 포함할 수 있습니다. 벡터 검색은 벡터 값이 <code translate="no">NULL</code> 인 엔티티를 건너뜁니다. 자세한 내용은 <a href="/docs/ko/nullable-and-default.md">Nullable Fields를</a> 참조하십시오.</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>기존 엔티티는 <code translate="no">embedding_v2</code> 에 대해 <code translate="no">NULL</code> 값을 가지며, 이 필드를 기준으로 검색할 때 건너뜁니다. 기존 엔티티를 <code translate="no">embedding_v2</code> 를 통해 검색 가능하게 하려면, 업서트(upsert) 또는 백필(backfill) 워크플로를 통해 NULL이 아닌 벡터 값을 기록하십시오. 새 엔티티는 삽입 시 <code translate="no">embedding_v2</code> 를 포함할 수 있습니다.</p>
<h3 id="Add-vector-fields-generated-by-functions--Milvus-30x" class="common-anchor-header">함수로 생성된 벡터 필드 추가<span class="beta-tag" style="background-color:rgb(0, 179, 255);color:white" translate="no">Compatible with Milvus 3.0.x</span><button data-href="#Add-vector-fields-generated-by-functions--Milvus-30x" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Milvus가 기존 컬렉션에 이미 저장된 데이터로부터 새로운 벡터 필드를 생성해야 할 때 이 워크플로를 사용합니다. 이 작업은 두 개의 관련 스키마 요소를 추가합니다:</p>
<ul>
<li><p>하나 이상의 기존 입력 필드를 읽는 함수.</p></li>
<li><p>함수에서 생성된 값을 저장하는 새로운 벡터 출력 필드.</p></li>
</ul>
<p>예를 들어, BM25 함수는 기존 <code translate="no">VARCHAR</code> 필드를 읽고 어휘 검색을 위한 <code translate="no">SPARSE_FLOAT_VECTOR</code> 필드를 생성합니다. MinHash 함수는 유사 중복 감지를 위한 <code translate="no">BINARY_VECTOR</code> 필드를 생성합니다. 이 워크플로는 함수 입력 필드를 추가하거나 대체하지 않습니다.</p>
<div class="alert note">
...
<p>지원되는 함수에 따라 생성되는 벡터 필드의 유형이 결정됩니다.</p>
<table>
<thead>
...
<p>생성된 벡터 필드는 컬렉션에 이미 존재해서는 안 되며, null이 허용되는 필드일 수 없습니다. 함수 입력 필드는 이미 존재해야 합니다.</p>
<p><strong>예: 어휘 검색을 위해 BM25로 생성된 스파스 벡터 필드 추가</strong></p>
<p>다음 예제에서는 <code translate="no">text_bm25</code> 이라는 BM25 함수와 <code translate="no">text_sparse</code> 이라는 생성된 스파스 벡터 필드를 기존 컬렉션에 추가합니다. 컬렉션에는 이미 분석기가 활성화된 <code translate="no">text</code> 이라는 <code translate="no">VARCHAR</code> 필드가 존재해야 합니다.</p>
...
<span class="highlighted-comment-line">)</span>
<button class="copy-code-btn"></button></code></pre>
<p>BM25 함수와 생성된 필드를 추가한 후, BM25 검색에 사용하기 전에 스파스 벡터 필드에 인덱스를 생성하십시오:</p>
<pre><code translate="no" class="language-python">index_params = client.prepare_index_params()
...
<button class="copy-code-btn"></button></code></pre>
<p>개념적으로 이 작업은 다음과 같은 필드 및 함수 정의를 추가합니다:</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>요청이 성공하면 ` <code translate="no">describe_collection()</code> `는 컬렉션 스키마에 새로운 ` <code translate="no">text_sparse</code> ` 벡터 필드와 ` <code translate="no">text_bm25</code> ` 함수를 모두 반환합니다. Milvus는 새로운 엔티티가 기록될 때마다 해당 함수의 출력을 생성합니다. 기존 엔티티의 경우, Milvus는 백그라운드 압축을 통해 생성된 벡터 필드를 비동기적으로 채웁니다. 스키마 가시성은 스키마 업데이트가 성공했음을 확인해 주지만, 모든 기존 엔티티에 대한 백필이 완료되었음을 나타내지는 않습니다. 전체 BM25 검색 워크플로에 대해서는 <a href="/docs/ko/full-text-search.md">‘전체 텍스트 검색(Full Text Search</a>)’을 참조하십시오.</p>
<p>Milvus는 또한 유사 중복 감지를 위해 MinHash로 생성된 이진 벡터 필드를 지원합니다. MinHash 함수는 <code translate="no">FunctionType.MINHASH</code> 를 사용하여 새로운 <code translate="no">BINARY_VECTOR</code> 출력 필드에 데이터를 기록합니다. 구성에 대한 자세한 내용은 <a href="/docs/ko/minhash-function.md">MinHash 함수를</a> 참조하십시오.</p>
<h2 id="Drop-fields-from-an-existing-collection" class="common-anchor-header">기존 컬렉션에서 필드 삭제<button data-href="#Drop-fields-from-an-existing-collection" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>기존 컬렉션에서 필드를 제거하는 방법에는 두 가지가 있습니다. 사용자 정의 스칼라 또는 벡터 필드가 더 이상 컬렉션 모델의 일부가 아닌 경우 해당 필드를 직접 삭제할 수 있습니다. 함수에 의해 생성된 벡터 필드의 경우, 해당 필드를 생성하는 함수를 삭제하여 제거할 수 있습니다.</p>
...
<ul>
<li><p><code translate="no">drop_collection_field()</code> 이 성공적으로 실행된 후, 컬렉션 스키마가 업데이트됩니다. 즉, ` <code translate="no">describe_collection()</code> `은 더 이상 삭제된 필드를 반환하지 않으며, 쿼리나 검색을 통해 ` <code translate="no">output_fields</code> `에서 해당 필드를 반환하거나 표현식에서 사용할 수 없게 됩니다.</p></li>
...
<p>다음 예제는 ` <code translate="no">experiment_tag</code> `가 ` <code translate="no">product_catalog</code>`에 있는 사용자 정의 스칼라 필드라고 가정하며, 이 필드를 컬렉션에서 삭제합니다.</p>
...
<p><code translate="no">drop_collection_field()</code> 메서드를 사용하여 벡터 필드를 삭제할 수 있지만, 삭제 후에도 컬렉션에는 최소한 하나의 벡터 필드가 남아 있어야 합니다. 이는 일시적으로 여러 벡터 표현을 포함하고 있다가 나중에 그중 하나로 표준화하는 컬렉션에 유용합니다.</p>
...
<h3 id="Drop-vector-fields-generated-by-functions--Milvus-30x" class="common-anchor-header">함수에 의해 생성된 벡터 필드 삭제<span class="beta-tag" style="background-color:rgb(0, 179, 255);color:white" translate="no">Compatible with Milvus 3.0.x</span><button data-href="#Drop-vector-fields-generated-by-functions--Milvus-30x" class="anchor-icon" translate="no">
    ...
    </button></h3><p>BM25로 생성된 스파스 벡터 필드와 같이 함수에 의해 생성된 벡터 필드가 더 이상 필요하지 않을 때 이 작업을 사용합니다.</p>
<p>생성된 벡터 필드를 제거하려면, 해당 필드를 생성한 함수에 대해 <code translate="no">drop_collection_function()</code> 를 호출하십시오. 이 워크플로우에서 Milvus는 컬렉션 스키마에서 함수를 제거하고, 함수가 생성한 벡터 출력 필드도 함께 제거합니다.</p>
<p>함수 입력 필드나 함수 출력 필드에 대해서는 ` <code translate="no">drop_collection_field()</code> `를 호출하지 마십시오. 대상 필드가 함수 출력 필드인 경우, 대신 ` <code translate="no">drop_collection_function()</code> `를 호출하십시오. 함수 입력 필드는 함수가 삭제된 후에도 유지됩니다.</p>
<p><strong>예: BM25 함수 및 해당 함수가 생성한 필드 삭제</strong></p>
...
<span class="highlighted-comment-line">)</span>
<button class="copy-code-btn"></button></code></pre>
<p>작업이 성공적으로 완료된 후, <code translate="no">describe_collection()</code> 는 삭제된 함수나 해당 함수가 생성한 출력 필드를 더 이상 반환하지 않습니다. 함수 입력 필드는 스키마에 그대로 유지됩니다.</p>
...
<h2 id="FAQ" class="common-anchor-header">자주 묻는 질문<button data-href="#FAQ" class="anchor-icon" translate="no">
    ...
    </button></h2><h3 id="Which-add-field-method-should-I-use" class="common-anchor-header">어떤 add-field 메서드를 사용해야 합니까?<button data-href="#Which-add-field-method-should-I-use" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>애플리케이션에서 필터링, 쿼리 출력 또는 애플리케이션 로직을 위해 스칼라 값을 제공하는 경우, <code translate="no">add_collection_field()</code> 를 사용하여 사용자 정의 스칼라 필드를 추가하십시오.</p>
...
<p>애플리케이션이 임베딩을 생성하고 벡터 값을 Milvus에 기록하는 경우, ` <code translate="no">add_collection_field()</code> `를 사용하여 사용자 정의 벡터 필드를 추가하십시오.</p>
<p>Milvus가 기존 필드에서 벡터 값을 생성해야 하는 경우 generated-vector-field 워크플로를 사용하십시오. 이 가이드에서는 어휘 검색을 위해 <code translate="no">add_function_field()</code> 를 사용하는 BM25 경로를 보여줍니다. Milvus는 또한 유사 중복 탐지를 위해 MinHash로 생성된 이진 벡터 필드도 지원합니다.</p>
<h3 id="Why-must-added-user-defined-fields-be-nullable" class="common-anchor-header">추가된 사용자 정의 필드는 왜 null 허용(nullable)이어야 합니까?<button data-href="#Why-must-added-user-defined-fields-be-nullable" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>기존 엔티티는 새 필드가 생성되기 전에 삽입되었기 때문에 해당 필드의 값을 가지고 있지 않습니다. ` <code translate="no">nullable=True</code> `를 설정하면, 애플리케이션이 값을 기록할 때까지(스칼라 필드의 경우 기본값이 적용될 때까지) Milvus가 누락된 값을 ` <code translate="no">NULL</code> `로 표현합니다.</p>
<p>이 규칙은 ` <code translate="no">add_collection_field()</code>`로 추가된 사용자 정의 스칼라 필드 및 사용자 정의 벡터 필드와, ` <code translate="no">add_collection_struct_field()</code>`로 추가된 StructArray 필드에 적용됩니다. 함수에 의해 생성된 벡터 필드에는 적용되지 않으며, 이러한 필드는 nullable일 수 없습니다.</p>
<h3 id="What-happens-to-existing-entities-after-I-add-a-user-defined-field" class="common-anchor-header">사용자 정의 필드를 추가하면 기존 엔티티에는 어떤 변화가 발생하나요?<button data-href="#What-happens-to-existing-entities-after-I-add-a-user-defined-field" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>사용자 정의 스칼라 필드의 경우, <code translate="no">default_value</code> 를 설정하지 않으면 기존 엔티티는 <code translate="no">NULL</code> 를 반환합니다. <code translate="no">default_value</code> 를 설정하면 기존 엔티티는 해당 기본값을 반환합니다.</p>
<p>사용자 정의 벡터 필드의 경우, 기존 엔티티는 새로운 벡터 필드에 대해 ` <code translate="no">NULL</code> ` 값을 갖습니다. 추가된 필드에 대한 벡터 검색은 벡터 값이 ` <code translate="no">NULL</code>`인 엔티티를 건너뜁니다. 기존 엔티티를 새로운 벡터 필드를 통해 검색 가능하게 하려면, upsert 또는 백필(backfill) 워크플로를 통해 NULL이 아닌 벡터 값을 기록해야 합니다. 새 엔티티는 삽입 시 새로운 벡터 필드를 포함할 수 있습니다.</p>
<p>StructArray 필드의 경우, 기존 엔티티는 해당 StructArray 필드의 모든 하위 필드에 대해 <code translate="no">NULL</code> 을 반환합니다. 새 엔티티는 모든 하위 필드에 대해 <code translate="no">NULL</code> 을 제공하거나, 모든 하위 필드에 유효한 값을 제공해야 합니다.</p>
<h3 id="Can-I-add-BM25-lexical-search-to-an-existing-collection" class="common-anchor-header">기존 컬렉션에 BM25 어휘 검색을 추가할 수 있나요?<button data-href="#Can-I-add-BM25-lexical-search-to-an-existing-collection" class="anchor-icon" translate="no">
    ...
    </button></h3><p>네. 컬렉션에 이미 분석기가 활성화된 <code translate="no">VARCHAR</code> 필드가 있는 경우, 어휘 검색을 위해 BM25로 생성된 스파스 벡터 필드를 추가할 수 있습니다. 이 워크플로우에서 Milvus는 새로운 <code translate="no">SPARSE_FLOAT_VECTOR</code> 출력 필드와 해당 필드의 값을 생성하는 BM25 함수를 추가합니다. 이 스키마 변경 워크플로우에서는 기존 ` <code translate="no">TEXT</code> ` 필드를 BM25 입력으로 사용할 수 없습니다. ` <code translate="no">TEXT</code> ` 입력을 사용하려면 컬렉션을 생성할 때 해당 필드와 BM25 함수를 정의해야 합니다.</p>
<p>BM25에서 생성된 스파스 벡터 필드를 추가한 후, 해당 필드를 BM25 검색에 사용하기 전에 <code translate="no">metric_type=&quot;BM25&quot;</code> 를 사용하여 <code translate="no">SPARSE_INVERTED_INDEX</code> 인덱스를 생성하십시오.</p>
<h3 id="Can-I-drop-a-vector-field-generated-by-a-function-directly" class="common-anchor-header">함수로 생성된 벡터 필드를 직접 삭제할 수 있나요?<button data-href="#Can-I-drop-a-vector-field-generated-by-a-function-directly" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>아니요. 함수에 의해 생성된 벡터 필드는 해당 함수의 스키마 계약의 일부입니다. 대신 ` <code translate="no">drop_collection_function()</code> `을 사용하십시오. 이 스키마 변경 워크플로우에서 Milvus는 입력 필드는 유지한 채, 함수와 함수가 생성한 벡터 출력 필드를 함께 제거합니다.</p>
<h3 id="Do-I-need-to-wait-after-altering-a-collection-schema" class="common-anchor-header">컬렉션 스키마를 수정한 후 기다려야 하나요?<button data-href="#Do-I-need-to-wait-after-altering-a-collection-schema" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<p>분산 배포 환경에서는 Milvus 구성 요소가 컬렉션 메타데이터를 새로 고치는 동안 짧은 전파 시간이 발생할 수 있습니다. 스키마 변경 직후 수행한 작업이 스키마 관련 오류로 실패할 경우, 스키마를 새로 고친 후 작업을 다시 시도하십시오.</p>
...
<p>저장 공간은 나중에 압축 과정에서 회수될 수 있습니다. 압축은 기존 데이터 파일을 더 간결한 새 파일로 재구성하는 백그라운드 프로세스입니다. 필드가 삭제된 후, 새로 압축된 파일은 현재 스키마를 따르며 삭제된 필드는 제외됩니다. Milvus는 필드 삭제 후 저장 공간이 즉시 또는 정해진 시간 내에 감소할 것을 보장하지 않습니다.</p>
<h3 id="What-happens-if-I-add-a-scalar-field-with-the-same-name-as-a-dynamic-field-key" class="common-anchor-header">동적 필드 키와 이름이 동일한 스칼라 필드를 추가하면 어떻게 됩니까?<button data-href="#What-happens-if-I-add-a-scalar-field-with-the-same-name-as-a-dynamic-field-key" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>동적 필드가 활성화된 경우, 기존 동적 필드 키와 동일한 이름을 가진 스칼라 필드를 추가할 수 있습니다. 새로운 스칼라 필드는 일반 쿼리 출력에서 동적 필드 키를 가리게 되지만, 원래의 동적 데이터는 <code translate="no">$meta</code> 에 보존됩니다.</p>
<p>예를 들어, 기존 엔티티에 <code translate="no">source</code> 라는 동적 키가 저장되어 있고, 나중에 <code translate="no">source</code> 라는 스칼라 필드를 추가한 경우, <code translate="no">source</code> 에 대한 일반 출력에서는 스칼라 필드를 참조합니다. 원래의 동적 값에 액세스하려면 <code translate="no">$meta[&quot;source&quot;]</code> 와 같이 <code translate="no">$meta</code> 경로 구문을 사용하십시오.</p>
