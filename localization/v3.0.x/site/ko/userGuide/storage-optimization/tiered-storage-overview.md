...
<p><em>계층형 스토리지는</em> 데이터 캐싱과 세그먼트 로딩을 분리하여 이러한 문제를 해결합니다. 이제 쿼리 노드는 모든 데이터를 한 번에 로드하는 대신 가벼운 <em>메타데이터만</em> 처음에 로드하고 필요에 따라 필드 데이터를 동적으로 가져오거나 내보냅니다. 이렇게 하면 로드 시간이 크게 단축되고, 로컬 리소스 사용률이 최적화되며, 쿼리 노드가 물리적 메모리나 디스크 용량을 훨씬 초과하는 데이터 세트를 처리할 수 있습니다.</p>
