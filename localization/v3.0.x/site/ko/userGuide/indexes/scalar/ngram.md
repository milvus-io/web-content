...
title: NGRAM
summary: >-
  Milvus의 NGRAM 인덱스는 VARCHAR 필드 또는 JSON 필드 내의 특정 JSON 경로에 대한 LIKE 쿼리와 해당 정규식 필터를
  가속화합니다. 인덱스를 생성하기 전에 Milvus는 텍스트를 고정 길이 n의 짧고 중복되는 부분 문자열, 즉 n-그램으로 분할합니다. 쿼리
  실행 시, Milvus는 원래 필터 조건을 검증하기 전에 이러한 n-그램을 사용하여 후보 엔티티의 범위를 좁힙니다.
---
...
<p>다음과 같이 빠른 접두사, 접미사, 삽입어, 와일드카드 또는 유효한 정규식 필터링이 필요한 경우 이 기능을 사용하십시오.</p>
<ul>
...
<ol>
<li><p><strong>텍스트를 n-그램으로 분해</strong>: Milvus는 대상 필드의 각 문자열에 <em>n</em> 크기의 윈도우를 이동시키며, 겹치는 부분 문자열, 즉 <em>n-그램을</em> 추출합니다. 이러한 부분 문자열의 길이는 구성 가능한 범위( <code translate="no">[min_gram, max_gram]</code>) 내에 속합니다.</p>
<ul>
...
<li><p><strong>각 그램 검색 및 교집합 계산</strong>: Milvus는 인버티드 인덱스에서 쿼리 그램을 하나씩 조회한 다음, 결과로 얻은 문서 ID 목록을 교집합하여 소수의 후보 문서 집합을 찾습니다. 이 후보 문서들은 쿼리에 포함된 모든 그램을 포함하고 있습니다.</p></li>
<li><p><strong>결과 검증 및 반환:</strong> 이후, 정확한 일치 항목을 찾기 위해 소수의 후보 집합에만 원래의 <code translate="no">LIKE</code> 또는 정규식 필터를 적용하여 최종 검증을 수행합니다.</p></li>
</ol>
...
<li><p><strong>대소문자 구분 없는 정규 표현식</strong>: <code translate="no">(?i)</code> 가 포함된 정규 표현식 패턴은 지원되지만, 인덱스가 원래의 대소문자 구분을 유지하기 때문에 <code translate="no">NGRAM</code> 최적화가 생략될 수 있습니다.</p></li>
...
<li><p><strong>불변성</strong>: <code translate="no">min_gram</code> 및 <code translate="no">max_gram</code> 는 현장에서 변경할 수 없으므로, 이를 조정하려면 인덱스를 다시 구축해야 합니다.</p></li>
</ul>
...
<li><p><code translate="no">max_gram</code> 은 의미 있는 부분 문자열의 일반적인 길이에 가깝게 설정하십시오. <code translate="no">max_gram</code> 값을 크게 설정하면 필터링 성능은 향상되지만 저장 공간이 증가합니다.</p></li>
</ul></li>
...
<p>사용 사례에 필요한 경우, 수집된 텍스트와 쿼리 리터럴에 동일한 정규화 처리(예: 소문자 변환, 공백 제거)를 적용하십시오.</p></li>
</ul>
