...
<li>함수 출력 필드는 ` <code translate="no">FLOAT_VECTOR</code> ` 데이터 유형을 사용해야 합니다. Milvus의 Hugging Face 임베딩은 ` <code translate="no">INT8_VECTOR</code>`, ` <code translate="no">BINARY_VECTOR</code>`, ` <code translate="no">FLOAT16_VECTOR</code>` 또는 ` <code translate="no">BFLOAT16_VECTOR</code> ` 출력 필드를 지원하지 않습니다.</li>
...
<button class="copy-code-btn"></button></code></pre>
<p>Docker Compose 설정 적용에 대한 자세한 내용은 <a href="/docs/ko/configure-docker.md">‘Docker Compose를 사용하여 Milvus 구성’을</a> 참조하십시오.</p>
...
<p>출력 필드에 대한 인덱스를 구성한 다음 컬렉션을 생성합니다:</p>
...
<li>Function의 일반적인 개념 및 삽입/검색 동작에 대해서는 <a href="/docs/ko/embedding-function-overview.md">‘임베딩 함수 개요’를</a> 참조하십시오.</li>
<li>호스팅된 Hugging Face 문장 유사도 점수를 사용하여 벡터 검색 후보를 재순위 지정하려면 <a href="/docs/ko/hugging-face-ranker.md">Hugging Face Ranker를</a> 참조하십시오.</li>
</ul>
