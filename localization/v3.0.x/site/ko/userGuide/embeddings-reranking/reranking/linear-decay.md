...
summary: >-
  ...
  접근 방식은 특정 경계를 벗어난 항목이 결과에서 완전히 제외되도록 하여 명확한 컷오프와 함께 일관된 감쇠율을 원하는 경우에 이상적입니다.
beta: Milvus 2.6.x
---
...
<h3 id="Create-a-decay-ranker" class="common-anchor-header">감쇠 랭커 생성하기<button data-href="#Create-a-decay-ranker" class="anchor-icon" translate="no">
    ...
    </button></h3><p>컬렉션에 숫자 필드(이 예에서는 <code translate="no">event_date</code> 를 지금부터 초로 설정)를 설정한 후 선형 감쇠 순위자를 만듭니다:</p>
<div class="alert note">
