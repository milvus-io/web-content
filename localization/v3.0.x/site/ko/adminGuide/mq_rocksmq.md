...
<h2 id="Deploy-Milvus-Standalone-with-RocksMQ-using-Docker" class="common-anchor-header">Docker를 사용하여 RocksMQ와 함께 Milvus Standalone 배포하기<button data-href="#Deploy-Milvus-Standalone-with-RocksMQ-using-Docker" class="anchor-icon" translate="no">
    ...
    </button></h3><p><a href="/docs/ko/install_standalone-docker.md">'Docker에서 Milvus 실행하기'를</a> 따르십시오. Milvus 3.x에서는 독립 실행형 모드의 기본값이 Woodpecker이므로, 메시지 큐 유형을 명시적으로 RocksMQ로 전환해야 합니다. 부트스트랩 스크립트는 <strong>첫 번째</strong> 시작 시( <code translate="no">start</code>) 새로운 <code translate="no">user.yaml</code> 파일을 생성하므로, 첫 시작 <strong>후</strong> 유형을 설정한 다음 <code translate="no">restart</code> 를 실행하여 적용하십시오( <code translate="no">restart</code> 는 <code translate="no">user.yaml</code> 를 보존합니다):</p>
