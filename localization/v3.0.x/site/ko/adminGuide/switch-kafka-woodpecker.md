...
<p><strong>2단계: 대상 Kafka 연결을 구성하고 Milvus를 다시 시작합니다.</strong> 전환을 위해서는 Milvus가 이미 Kafka 연결 정보를 알고 있어야 하므로, <code translate="no">extraConfigFiles</code> 를 통해 <code translate="no">user.yaml</code> 에 해당 정보를 작성한 후 <code translate="no">helm upgrade</code> 를 실행하여 적용합니다(이 명령은 포드를 롤링합니다). Switch MQ 기능을 사용하려면 <code translate="no">streaming.enabled=true</code> 가 필요합니다. SASL/SSL에 대한 자세한 내용은 <a href="/docs/ko/connect_kafka_ssl.md">SASL/SSL을 사용하여 Kafka에 연결하기를</a> 참조하십시오.</p>
