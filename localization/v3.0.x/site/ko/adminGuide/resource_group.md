...
<h1 id="Manage-Resource-Groups" class="common-anchor-header">리소스 그룹 관리<button data-href="#Manage-Resource-Groups" class="anchor-icon" translate="no">
    ...
    </button></h1><p>Milvus에서는 리소스 그룹을 사용하여 특정 쿼리 노드를 다른 노드들로부터 물리적으로 분리할 수 있습니다. 이 가이드에서는 사용자 지정 리소스 그룹을 생성 및 관리하는 방법과 그룹 간에 노드를 이동하는 방법을 단계별로 안내합니다.</p>
...
<li>Milvus 클러스터의 쿼리 노드 수가 과도할 경우( <code translate="no">NumOfQueryNode &gt; sum(.limits.nodeNum)</code>), 중복된 쿼리 노드는 항상 <strong>__default_resource_group에</strong> 우선 배치됩니다.</li>
</ul>
...
<p>이 페이지의 모든 코드 예제는 PyMilvus 3.0.1 버전입니다. 코드를 실행하기 전에 PyMilvus를 최신 버전으로 업그레이드하십시오.</p>
</div>
<ol>
<li><p>리소스 그룹 생성하기.</p>
...
<p>리소스 그룹에 쿼리 노드가 포함되면, 이 리소스 그룹에 컬렉션을 로드할 수 있습니다. 다음 코드 조각은 <code translate="no">demo</code> 이라는 이름의 컬렉션이 이미 존재한다고 가정합니다.</p>
