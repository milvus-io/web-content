...
<p>버전 2.4.1부터 Milvus는 선언형 리소스 그룹 API를 제공하며, 기존 리소스 그룹 API는 더 이상 사용되지 않습니다. 새로운 선언형 API를 통해 사용자는 항등성을 확보할 수 있으며, 클라우드 네이티브 환경에서 2차 개발을 더 쉽게 수행할 수 있습니다.</p>
...
<li><strong>transfer_from</strong> 및 <strong>transfer_to</strong> 속성은 리소스 그룹이 우선적으로 리소스를 확보해야 할 리소스 그룹과 리소스를 이전해야 할 리소스 그룹을 각각 설명합니다.</li>
</ul>
...
<p>단, 다음의 경우는 예외입니다:</p>
<ul>
<li>Milvus 클러스터의 쿼리 노드(QueryNodes) 수가 불충분한 경우, 즉 <code translate="no">NumOfQueryNode &lt; sum(.requests.nodeNum)</code> 인 경우, 항상 충분한 쿼리 노드를 확보하지 못한 리소스 그룹이 존재하게 됩니다.</li>
...
<p>이 페이지의 모든 코드 예제는 PyMilvus 3.0.2 버전입니다. 코드를 실행하기 전에 PyMilvus를 최신 버전으로 업그레이드하십시오.</p>
</div>
<ol>
<li><p>리소스 그룹 생성.</p>
...
클러스터의 <strong>__default_resource_group에</strong> 현재 1개의 QueryNode가 있고, 생성한 <strong>rg로</strong> 노드 하나를 이동한다고 가정해 봅시다.<code translate="no">update_resource_groups</code> 은 여러 구성 변경에 대한 원자성을 보장하므로, Milvus에서는 중간 상태가 표시되지 않습니다.</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>또한, 리소스 그룹에 파티션 하나만 로드하고 그 복제본을 여러 리소스 그룹에 분산시킬 수도 있습니다. 다음 예제는 <code translate="no">Books</code> 이라는 이름의 컬렉션이 이미 존재하며, 이 컬렉션에 <code translate="no">Novels</code> 이라는 이름의 파티션이 있다고 가정합니다.</p>
...
<p>Milvus가 컬렉션의 각 복제본을 별도의 리소스 그룹에 로드하도록 하려면, 리소스 그룹의 수가 복제본의 수와 같도록 설정해야 합니다.</p></li>
<li><p>리소스 그룹 간에 복제본을 이동합니다.</p>
<p>Milvus는 여러 쿼리 노드에 분산된 <a href="/docs/ko/glossary.md#Segment">세그먼트</a> 간에 부하 분산을 달성하기 위해 <a href="/docs/ko/replica.md">복제본을</a> 사용합니다. 다음과 같이 컬렉션의 특정 복제본을 한 리소스 그룹에서 다른 리소스 그룹으로 이동할 수 있습니다.</p>
<pre><code translate="no" class="language-python">source = <span class="hljs-string">&#x27;__default_resource_group&#x27;</span>
target = <span class="hljs-string">&#x27;rg&#x27;</span>
...
<button class="copy-code-btn"></button></code></pre></li>
<li><p>리소스 그룹 삭제.</p>
<p>쿼리 노드를 포함하지 않는 리소스 그룹(<code translate="no">limits.node_num = 0</code>)은 언제든지 삭제할 수 있습니다. 이 가이드에서 리소스 그룹 <code translate="no">rg</code> 에는 현재 쿼리 노드가 하나 있습니다. 먼저 리소스 그룹의 구성 <code translate="no">limits.node_num</code> 을 0으로 변경해야 합니다.</p>
...
<h2 id="A-good-practice-to-manage-cluster-scaling" class="common-anchor-header">클러스터 스케일링 관리를 위한 모범 사례<button data-href="#A-good-practice-to-manage-cluster-scaling" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>현재 Milvus는 클라우드 네이티브 환경에서 독립적으로 확장 및 축소가 불가능합니다. 그러나 <strong>선언형 리소스 그룹 API를</strong> 컨테이너 오케스트레이션과 함께 사용하면 Milvus는 쿼리 노드에 대한 리소스 격리 및 관리를 쉽게 구현할 수 있습니다.
...
<button class="copy-code-btn"></button></code></pre>
<p>위의 예제 코드를 사용하여 추가 QueryNode를 보관할 <strong>__pending_nodes라는</strong> 리소스 그룹을 생성합니다. 또한 <strong>rg1과 rg2라는</strong> 두 개의 사용자 전용 리소스 그룹을 생성합니다. 아울러 다른 리소스 그룹이 누락되거나 중복된 QueryNode를 <strong>__pending_nodes에서</strong> 우선적으로 복구하도록 보장합니다.</p></li>
<li><p>클러스터 스케일 아웃</p>
...
<button class="copy-code-btn"></button></code></pre></li>
<li><p>클러스터 스케일 인</p>
<p>마찬가지로, <strong>__pending_nodes</strong> 리소스 그룹에서 쿼리 노드를 우선적으로 선택하도록 하는 스케일 인 규칙을 설정할 수 있습니다. 이 정보는 <code translate="no">describe_resource_group</code> API를 통해 얻을 수 있습니다. 이를 통해 지정된 리소스 그룹의 스케일 인 목표를 달성할 수 있습니다.</p>
...
<li>단일 컬렉션의 여러 복제본이 하나의 리소스 그룹에 로드되면, 해당 리소스 그룹의 쿼리 노드는 복제본들 사이에 균등하게 분산되어 각 복제본이 보유한 쿼리 노드 수의 차이가 1을 초과하지 않도록 합니다.</li>
</ul>
