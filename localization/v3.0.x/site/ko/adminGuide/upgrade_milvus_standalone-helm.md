...
title: Helm 차트를 사용하여 Milvus 독립형 환경 업그레이드하기
---
...
<h1 id="Upgrade-Milvus-Standalone-with-Helm-Chart" class="common-anchor-header">Helm 차트를 사용하여 Milvus 독립형 환경 업그레이드하기<button data-href="#Upgrade-Milvus-Standalone-with-Helm-Chart" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h1><p>이 가이드에서는 Helm을 사용하여 Milvus 2.6.x 독립형 배포를 v3.0-beta로 업그레이드하는 방법을 설명합니다.</p>
<div class="alert note">
<p>이 절차는 Milvus 5.0.22 Helm 차트를 사용하여 Milvus 2.6.20에서 Milvus v3.0-beta로 업그레이드하는 과정에서 검증되었습니다. 다른 Milvus 2.6.x 패치 릴리스나 Helm 차트 버전을 사용하는 경우, 먼저 비생산 환경에서 업그레이드가 정상적으로 이루어지는지 확인하십시오.</p>
</div>
...
<p>이 절차의 일환으로 Helm 차트를 변경하거나 다운그레이드하지 마십시오. Helm 릴리스에 이미 설치된 차트 버전을 유지하십시오. 테스트된 기준 환경에서는 Helm 차트 5.0.22를 유지하고 Milvus 이미지 태그만 <code translate="no">v3.0-beta</code> 로 변경했습니다.</p>
...
<h3 id="Step-2-Upgrade-Milvus" class="common-anchor-header">2단계: Milvus 업그레이드<button data-href="#Step-2-Upgrade-Milvus" class="anchor-icon" translate="no">
    ...
    </button></h3><p>Helm 릴리스에 설치된 차트 버전을 확인하세요:</p>
<pre><code translate="no" class="language-bash">helm list --namespace &lt;namespace&gt;
<button class="copy-code-btn"></button></code></pre>
<p><code translate="no">CHART</code> 열에서 값의 <code translate="no">milvus-</code> 접두사를 제거하고, 남은 버전을 <code translate="no">&lt;current-chart-version&gt;</code> 형식으로 사용합니다. 그런 다음 업그레이드 명령을 실행하십시오:</p>
