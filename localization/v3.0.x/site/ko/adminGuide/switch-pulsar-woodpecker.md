...
title: Pulsar와 Woodpecker 간 전환
summary: Helm 또는 Milvus Operator를 사용하여 Milvus 클러스터의 메시지 큐를 Pulsar와 Woodpecker 간에 전환합니다.
---
<h1 id="Switch-between-Pulsar-and-Woodpecker" class="common-anchor-header">Pulsar와 Woodpecker 간 전환<button data-href="#Switch-between-Pulsar-and-Woodpecker" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h1><p>이 페이지에서는 <strong>Milvus 클러스터의</strong> 메시지 큐(MQ)를 <strong>Pulsar</strong> (내장형 또는 외부)와 <strong>Woodpecker</strong> (MinIO 백엔드) 간에 양방향으로 전환하는 방법을 설명합니다. 일반적인 워크플로 및 필수 조건에 대해서는 <a href="/docs/ko/switch-mq-type.md">'메시지 큐 전환'을</a> 참조하십시오.</p>
...
<p>메시지 큐 전환은 <strong>위험도가 높은 작업입니다</strong>. <strong>배포 방식</strong> ( <strong>Helm 사용</strong> 또는 <strong>Milvus Operator 사용</strong> ) <strong>에</strong> 해당하는 섹션을 선택하고, 해당 섹션을 처음부터 끝까지 순서대로 따르십시오. Helm 명령어와 Operator 명령어를 혼용하지 마십시오.</p>
</div>
<h2 id="With-Helm" class="common-anchor-header">Helm 사용<button data-href="#With-Helm" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><h3 id="Switch-from-Pulsar-to-Woodpecker-Helm" class="common-anchor-header">Pulsar에서 Woodpecker로 전환(Helm)<button data-href="#Switch-from-Pulsar-to-Woodpecker-Helm" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<p><strong>4단계: (선택 사항) Pulsar를 중지하고 정리합니다.</strong> <strong>내장형</strong> Pulsar의 경우, Pulsar를 비활성화하고 Woodpecker를 활성화한 다음, Pulsar PVC를 삭제합니다:</p>
...
<pre><code translate="no" class="language-shell">helm upgrade -i my-release zilliztech/milvus \
  ...
  -f values.yaml
<button class="copy-code-btn"></button></code></pre>
<p>모든 파드가 준비될 때까지 기다린 후, Pulsar 액세스 구성이 Milvus 구성에 반영되었는지 확인하십시오.</p>
<p><strong>3단계: MQ 전환을 실행합니다.</strong></p>
<div class="alert note">
<p>대상 Pulsar에 이전 구성의 Milvus 토픽이 포함되어 있지 않은지 확인하십시오. Pulsar로 처음 전환하는 경우 이 참고 사항을 건너뛰고, 그렇지 않은 경우 먼저 동일한 이름을 가진 잔여 Milvus 토픽을 정리하십시오.</p>
</div>
<pre><code translate="no" class="language-shell">kubectl port-forward --address 0.0.0.0 service/my-release-milvus-mixcoord 29091:9091
<button class="copy-code-btn"></button></code></pre>
<p>다른 터미널에서:</p>
...
<p><strong>5단계: (선택 사항) Woodpecker 데이터를 정리합니다.</strong> MinIO/S3에 있는 Woodpecker 데이터( <code translate="no">&lt;rootPath&gt;/wp/...</code> 폴더 내, 일반적으로 <code translate="no">files/wp/...</code>)와 etcd에 있는 Woodpecker 메타데이터(<code translate="no">etcdctl get woodpecker --prefix</code>)를 삭제합니다. 나중에 다시 Woodpecker로 전환할 계획이라면, 먼저 이 파일들을 정리하십시오.</p>
<h2 id="With-Milvus-Operator" class="common-anchor-header">Milvus Operator 사용 시<button data-href="#With-Milvus-Operator" class="anchor-icon" translate="no">
    ...
    </button></h2><h3 id="Switch-from-Pulsar-to-Woodpecker-Milvus-Operator" class="common-anchor-header">Pulsar에서 Woodpecker로 전환(Milvus Operator)<button data-href="#Switch-from-Pulsar-to-Woodpecker-Milvus-Operator" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<button class="copy-code-btn"></button></code></pre>
<p><strong>5단계: (선택 사항) Pulsar를 중지하고 정리합니다.</strong> <strong>내장형</strong> Pulsar의 경우, Pulsar 릴리스를 제거하고 해당 PVC를 삭제합니다:</p>
<pre><code translate="no" class="language-shell">helm uninstall my-release-pulsar
kubectl get pvc | grep my-release-pulsar
kubectl delete pvc &lt;pulsar-pvc-name&gt; ...
<button class="copy-code-btn"></button></code></pre>
<p><strong>외부</strong> Pulsar의 경우, Milvus 토픽을 정리합니다( <code translate="no">&lt;cluster_prefix&gt;-dml_&lt;seqNo&gt;_&lt;TimeTick&gt;&lt;Version&gt;</code> 형식).</p>
<div class="alert note">
...
<h3 id="Switch-from-Woodpecker-to-Pulsar-Milvus-Operator" class="common-anchor-header">Woodpecker에서 Pulsar로 전환 (Milvus Operator)<button data-href="#Switch-from-Woodpecker-to-Pulsar-Milvus-Operator" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<pre><code translate="no" class="language-shell">kubectl patch -f change_configmap.yaml --patch-file change_configmap.yaml --type merge
<button class="copy-code-btn"></button></code></pre>
<p>모든 파드가 준비될 때까지 기다린 후, Pulsar 액세스 구성이 Milvus 구성으로 반영되었는지 확인합니다.</p>
<p><strong>3단계: MQ 전환을 실행합니다.</strong></p>
<div class="alert note">
<p>대상 Pulsar에 이전 구성의 Milvus 토픽이 포함되어 있지 않은지 확인하십시오. Pulsar로 처음 전환하는 경우 이 참고 사항을 건너뛰고, 그렇지 않은 경우 먼저 동일한 이름을 가진 잔여 Milvus 토픽을 정리하십시오.</p>
</div>
...
<tbody>
<tr><td>내장형 Pulsar</td><td>Woodpecker (MinIO)</td><td><strong>지원됨</strong></td><td><strong>지원됨</strong></td></tr>
<tr><td>외부 Pulsar</td><td>Woodpecker (MinIO)</td><td><strong>지원됨</strong></td><td><strong>지원됨</strong></td></tr>
<tr><td>Woodpecker (MinIO)</td><td>외부 Pulsar</td><td><strong>지원됨</strong></td><td><strong>지원됨</strong></td></tr>
<tr><td>Pulsar</td><td>Woodpecker (로컬)</td><td><strong>지원되지만 권장하지 않음</strong> (모든 포드에 공유 파일 시스템이 필요함)</td><td><strong>지원되지 않음</strong></td></tr>
</tbody>
</table>
