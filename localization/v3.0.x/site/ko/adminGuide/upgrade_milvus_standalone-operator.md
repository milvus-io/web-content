...
<p>이 절차는 Milvus Operator 1.3.0(Woodpecker), 클러스터 내 etcd, 클러스터 내 MinIO를 사용하여 Milvus 2.6.20에서 Milvus v3.0-beta로 업그레이드하는 경우 검증되었습니다. 다른 Milvus 2.6.x 패치 릴리스, Operator 버전, 메시지 큐 또는 종속성 구성을 사용하는 경우, 먼저 비생산 환경에서 업그레이드를 검증하십시오.</p>
</div>
...
<p>이 절차는 Milvus 이미지를 2.6.x 버전으로 되돌리는 다운그레이드 또는 롤백을 검증하지 않습니다. v3.0-beta가 데이터를 기록한 후에는, 이미지 전용 롤백 시 업데이트된 상태를 읽지 못할 수 있습니다. 업그레이드가 실패할 경우, 쓰기 작업을 중지하고 업그레이드 전의 메타데이터 및 영구 데이터 백업을 복원하는 복구 계획을 사용하십시오. 먼저 비생산 환경에서 복구 계획을 검증하십시오.</p>
</div>
...
<button class="copy-code-btn"></button></code></pre>
<p>Milvus CR에 <code translate="no">Healthy</code> 가 보고되는지, 현재 이미지가 <code translate="no">milvusdb/milvus:v3.0-beta</code> 인지, 기존 컬렉션을 계속 쿼리하고 검색할 수 있는지 확인하십시오. v3.0-beta 전용 기능을 활성화하기 전에 이러한 확인 작업을 완료하십시오.</p>
