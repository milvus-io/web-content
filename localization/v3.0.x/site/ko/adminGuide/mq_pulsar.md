...
<p>Milvus 2.5부터 Milvus Helm 차트 및 Milvus Operator는 기본적으로 <strong>Pulsar v3를</strong> 배포하며, Pulsar <a href="/docs/ko/upgrade-pulsar-v3.md">v2는</a> 계속 호환됩니다. <a href="/docs/ko/upgrade-pulsar-v3.md">‘Pulsar v2에서 v3로 업그레이드하기</a> ’ 및 <a href="/docs/ko/use-pulsar-v2.md">‘Pulsar v2 계속 사용하기’를</a> 참조하십시오.</p>
<h2 id="Deploy-a-Milvus-cluster-with-Pulsar-using-Helm" class="common-anchor-header">Helm을 사용하여 Pulsar가 포함된 Milvus 클러스터 배포<button data-href="#Deploy-a-Milvus-cluster-with-Pulsar-using-Helm" class="anchor-icon" translate="no">
    ...
    </button></h3><p>(Woodpecker 대신) 번들된 Pulsar를 사용하는 Milvus 클러스터를 배포하려면, 스트리밍 노드가 활성화된 Helm 차트를 설치하십시오:</p>
...
<p>Kubernetes v1.25 이상에서 번들된 Pulsar 하위 차트로 인해 PodDisruptionBudget(PDB) API 문제가 발생하면, Pulsar PDB 정책을 비활성화하십시오:</p>
<pre><code translate="no" class="language-bash">helm install my-release zilliztech/milvus \
...
<h3 id="Configure" class="common-anchor-header">구성<button data-href="#Configure" class="anchor-icon" translate="no">
    ...
    </button></h3><p>Milvus를 <strong>외부</strong> Pulsar 서비스에 연결하려면, 번들된 Pulsar를 비활성화하고 ` <code translate="no">values.yaml</code> ` 오버라이드에서 ` <code translate="no">externalPulsar</code> `를 활성화하십시오:</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>번들된 Pulsar를 사용했고 그 지속 저장 데이터를 제거하려면, Pulsar PVC(이름: <code translate="no">my-release-pulsarv3-*</code>)를 삭제하십시오:</p>
...
<li><strong>2.5.x에서 2.6.x로 업그레이드:</strong> <strong>메시지 큐 제한 사항</strong>: Milvus v3.0.2로 업그레이드할 때는 현재 선택한 메시지 큐를 유지해야 합니다. 업그레이드 과정에서 다른 메시지 큐 시스템으로 전환하는 것은 지원되지 않습니다. 메시지 큐 시스템 변경 기능은 향후 버전에서 제공될 예정입니다.
