...
<h1 id="Upgrade-Milvus-Cluster-with-Helm-Chart" class="common-anchor-header">Helm 차트를 사용하여 Milvus 클러스터 업그레이드하기<button data-href="#Upgrade-Milvus-Cluster-with-Helm-Chart" class="anchor-icon" translate="no">
    ...
    </button></h1><p>이 가이드에서는 Helm 차트를 사용하여 Milvus 클러스터를 v2.5.x에서 v3.0-beta로 업그레이드하는 방법을 설명합니다.</p>
<h2 id="Before-you-start" class="common-anchor-header">시작하기 전에<button data-href="#Before-you-start" class="anchor-icon" translate="no">
    ...
    </button></h3><p>Milvus 2.5.x에서 3.0-beta로 업그레이드하면 다음과 같은 주요 아키텍처 변경 사항이 적용됩니다:</p>
<ul>
...
<p>이 업그레이드 프로세스는 새로운 아키텍처로의 원활한 마이그레이션을 보장합니다. 아키텍처 변경 사항에 대한 자세한 내용은 <a href="/docs/ko/architecture_overview.md">Milvus 아키텍처 개요를</a> 참조하십시오.</p>
...
<li>v3.0-beta로 업그레이드하기 전에, <code translate="no">mixCoordinator</code> 가 활성화된 상태에서 v2.5.16 이상으로 먼저 업그레이드 <strong>해야 합니다</strong>.</li>
</ul>
...
kubectl get pods
<button class="copy-code-btn"></button></code></pre>
<h3 id="Step-3-Upgrade-to-v30-beta" class="common-anchor-header">3단계: v3.0-beta로 업그레이드<button data-href="#Step-3-Upgrade-to-v30-beta" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p><code translate="no">mixCoordinator</code> 를 사용하여 v2.5.16이 정상적으로 실행되면 v3.0-beta로 업그레이드하십시오:</p>
<pre><code translate="no" class="language-bash">helm upgrade my-release zilliztech/milvus \
