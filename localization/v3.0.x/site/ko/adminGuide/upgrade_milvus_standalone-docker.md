...
summary: Docker Compose를 사용하여 Milvus 독립 실행형을 업그레이드하는 방법을 알아보세요.
...
<h1 id="Upgrade-Milvus-Standalone-with-Docker-Compose" class="common-anchor-header">Docker Compose를 사용하여 Milvus 독립형 버전 업그레이드하기<button data-href="#Upgrade-Milvus-Standalone-with-Docker-Compose" class="anchor-icon" translate="no">
    ...
    </button></h1><p>이 가이드에서는 Docker Compose를 사용하여 Milvus 2.6.x 독립형 배포 환경을 v3.0.2로 업그레이드하는 방법을 설명합니다.</p>
<div class="alert note">
<p>이 절차는 공식 Milvus 2.6.20 독립형 Docker Compose 구성에서 검증되었습니다. 업그레이드 과정에서 etcd, MinIO, Woodpecker 및 기존 데이터 디렉터리는 그대로 유지되었으며, Milvus 이미지만 <code translate="no">milvusdb/milvus:v3.0.2</code> 로 변경되었습니다.</p>
</div>
...
<p>이 절차는 Milvus 이미지를 2.6.x 버전으로 되돌리는 다운그레이드 또는 롤백을 검증하지 않습니다. v3.0.2가 데이터를 기록한 후에는, 이미지 전용 롤백 시 업데이트된 상태를 읽지 못할 수 있습니다. 업그레이드가 실패하면 쓰기 작업을 중지하고, 업그레이드 이전의 메타데이터 및 영구 데이터 백업을 복원하는 복구 계획을 사용하십시오. 먼저 비생산 환경에서 복구 계획을 검증하십시오.</p>
</div>
...
<button class="copy-code-btn"></button></code></pre>
<p><code translate="no">standalone</code> 서비스가 정상적으로 작동하는지, 해당 이미지가 <code translate="no">milvusdb/milvus:v3.0.2</code> 인지, 그리고 기존 컬렉션을 계속 쿼리하고 검색할 수 있는지 확인하십시오. v3.0.2 전용 기능을 활성화하기 전에 이러한 확인 절차를 완료하십시오.</p>
