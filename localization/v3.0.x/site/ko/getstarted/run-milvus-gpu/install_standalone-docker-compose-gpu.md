...
<h3 id="1-Download-and-configure-the-YAML-file" class="common-anchor-header">1. YAML 파일을 다운로드하고 구성하십시오.<button data-href="#1-Download-and-configure-the-YAML-file" class="anchor-icon" translate="no">
    ...
    </button></h3><p>다운로드 <a href="https://github.com/milvus-io/milvus/releases/download/v3.0-beta/milvus-standalone-docker-compose-gpu.yml"><code translate="no">milvus-standalone-docker-compose-gpu.yml</code></a> 파일을 다운로드하여 수동으로 또는 다음 명령어를 사용하여 docker-compose.yml로 저장하십시오.</p>
...
<p>YAML 파일 내의 독립 실행형 서비스 환경 변수를 다음과 같이 약간 수정해야 합니다:</p>
<ul>
...
<button class="copy-code-btn"></button></code></pre>
<div class="alert note">
<p>위의 명령어를 실행하는 데 실패했다면, 시스템에 Docker Compose V1이 설치되어 있는지 확인하십시오. 설치되어 있는 경우, <a href="https://docs.docker.com/compose/">이 페이지의</a> 참고 사항에 따라 Docker Compose V2로 마이그레이션하는 것이 좋습니다.</p>
</div>
<p>Milvus가 시작된 후,</p>
<ul>
...
<p>docker-compose.yml에서 Milvus에 여러 GPU 장치를 할당했다면, 어떤 GPU 장치를 노출하거나 사용할 수 있도록 할지 지정할 수 있습니다.</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>GPU 장치 <code translate="no">0</code> 및 <code translate="no">1</code> 를 Milvus에서 인식할 수 있도록 설정하려면:</p>
...
<h2 id="Configure-memory-pool" class="common-anchor-header">메모리 풀 구성<button data-href="#Configure-memory-pool" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Milvus가 실행된 후, ` <code translate="no">milvus.yaml</code> ` 파일 내의 ` <code translate="no">initMemSize</code> ` 및 ` <code translate="no">maxMemSize</code> ` 설정을 수정하여 메모리 풀을 사용자 정의할 수 있습니다.</p>
<div class="alert note">
