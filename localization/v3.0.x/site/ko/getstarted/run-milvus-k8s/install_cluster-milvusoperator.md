...
<h2 id="Overview" class="common-anchor-header">개요<button data-href="#Overview" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Milvus Operator는 대상 Kubernetes(K8s) 클러스터에 전체 Milvus 서비스 스택을 배포하고 관리할 수 있도록 지원하는 솔루션입니다. 이 스택에는 모든 Milvus 구성 요소와 etcd, MinIO와 같은 관련 종속성이 포함되어 있으며, Woodpecker가 내장 메시지 큐로 사용됩니다.</p>
...
<li><p>Milvus를 설치하기 전에 <a href="https://milvus.io/tools/sizing">Milvus Sizing Tool을</a> 사용하여 데이터 크기에 기반한 하드웨어 요구 사항을 추정하는 것이 좋습니다. 이를 통해 Milvus 설치에 대한 최적의 성능과 리소스 할당을 보장할 수 있습니다.</p></li>
</ul>
...
<button class="copy-code-btn"></button></code></pre>
<p>위의 명령어는 <strong>Woodpecker를</strong> 메시지 큐로 사용하는(v3.0-beta 권장) Milvus 클러스터와 스트리밍 노드를 포함한 모든 새로운 아키텍처 구성 요소를 배포합니다.</p>
<p><strong>이 배포의 아키텍처 주요 특징:</strong></p>
<ul>
<li><strong>메시지 큐</strong>: <a href="/docs/ko/woodpecker.md">Woodpecker 사용</a> (인프라 유지 관리 부담 감소)</li>
...
<li>모든 구성 요소가 단일 파드에 포함된 독립 실행 모드(standalone mode)로 Milvus 인스턴스를 배포할 수도 있습니다. 이를 위해서는 위 명령어의 구성 파일 URL을 다음과 같이 변경하십시오. <code translate="no">https://raw.githubusercontent.com/zilliztech/milvus-operator/main/config/samples/milvus_default.yaml</code></li>
</ul>
</div>
...
<button class="copy-code-btn"></button></code></pre>
<p>Milvus 클러스터가 준비되면 위 명령어의 출력 결과는 다음과 유사해야 합니다. ` <code translate="no">status.status</code> ` 필드가 ` <code translate="no">Unhealthy</code>`로 유지된다면, Milvus 클러스터가 아직 생성 중인 것입니다.</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>Milvus Operator는 etcd 및 MinIO와 같은 Milvus 종속성을 생성한 다음, 프록시, 코디네이터, 노드와 같은 Milvus 구성 요소를 생성합니다.</p>
...
<p>그런 다음, 다음 명령을 실행하여 로컬 포트를 Milvus가 서비스를 제공하는 포트로 포워딩하십시오.</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>선택적으로, 위 명령에서 <code translate="no">27017:19530</code> 대신 <code translate="no">:19530</code> 을 사용하여 <code translate="no">kubectl</code> 가 로컬 포트를 자동으로 할당하도록 할 수 있습니다. 이렇게 하면 포트 충돌을 직접 관리할 필요가 없습니다.</p>
...
<ol>
<li><p>다음 명령을 실행하여 적용될 구성 설정을 미리 확인할 수 있습니다.</p>
...
<h2 id="Access-Milvus-WebUI" class="common-anchor-header">Milvus WebUI에 접속하기<button data-href="#Access-Milvus-WebUI" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>Milvus에는 브라우저를 통해 액세스할 수 있는 Milvus WebUI라는 내장 GUI 도구가 함께 제공됩니다. Milvus Web UI는 간단하고 직관적인 인터페이스를 통해 시스템 가시성을 향상시킵니다. Milvus Web UI를 사용하여 Milvus의 구성 요소 및 종속성에 대한 통계와 메트릭을 관찰하고, 데이터베이스 및 수집 세부 정보를 확인하며, 자세한 Milvus 구성을 나열할 수 있습니다. Milvus Web UI에 대한 자세한 내용은 <a href="/docs/ko/milvus-webui.md">Milvus WebUI를</a> 참조하십시오.</p>
<p>Milvus WebUI에 액세스하려면 프록시 포드를 로컬 포트로 포트 포워딩해야 합니다.</p>
<pre><code translate="no" class="language-shell"><span class="hljs-meta prompt_">$ </span><span class="language-bash">kubectl port-forward --address 0.0.0.0 service/my-release-milvus 27018:9091</span>
Forwarding from 0.0.0.0:27018 -&gt; 9091
<button class="copy-code-btn"></button></code></pre>
<p>이제 <code translate="no">http://localhost:27018</code> 에서 Milvus WebUI에 접속할 수 있습니다.</p>
...
<div class="alert note">
<ul>
<li>기본 구성을 사용하여 Milvus 클러스터를 삭제할 경우, etcd 및 MinIO와 같은 종속성은 삭제되지 않습니다. 따라서 다음에 동일한 Milvus 클러스터 인스턴스를 설치할 때 이러한 종속성이 다시 사용됩니다.</li>
...
<button class="copy-code-btn"></button></code></pre>
<h4 id="Uninstall-with-kubectl" class="common-anchor-header">kubectl을 사용하여 제거</h4><pre><code translate="no" class="language-shell"><span class="hljs-meta prompt_">$ </span><span class="language-bash">kubectl delete -f https://raw.githubusercontent.com/zilliztech/milvus-operator/v1.3.0/deploy/manifests/deployment.yaml</span>
<button class="copy-code-btn"></button></code></pre>
<h2 id="Optional-dependencies" class="common-anchor-header">선택적 종속성<button data-href="#Optional-dependencies" class="anchor-icon" translate="no">
    ...
    </button></h2><p>이 배포는 메시지 큐로 <strong>Woodpecker</strong>, 메타데이터로 <strong>etcd</strong>, 오브젝트 스토리지로 <strong>MinIO를</strong> 사용하며, 이 모든 구성 요소는 Milvus Operator에 의해 프로비저닝됩니다. Operator와 함께 다른 백엔드를 사용하려면 다음을 참조하십시오:</p>
<ul>
<li>메시지 큐: <a href="/docs/ko/woodpecker.md">Woodpecker</a> (기본값) · <a href="/docs/ko/message_storage_operator.md#Configure-Pulsar">Pulsar</a> · <a href="/docs/ko/message_storage_operator.md#Configure-Kafka">Kafka</a> · <a href="/docs/ko/message_storage_operator.md#Configure-RocksMQ">RocksMQ</a></li>
<li>오브젝트 스토리지: <a href="/docs/ko/object_storage_operator.md">Milvus Operator를 사용하여 오브젝트 스토리지 구성</a></li>
<li>메타데이터: <a href="/docs/ko/meta_storage_operator.md">Milvus Operator를 사용하여 etcd 구성</a></li>
</ul>
<div class="alert note">
