...
<h2 id="Overview" class="common-anchor-header">개요<button data-href="#Overview" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Milvus Operator는 대상 Kubernetes(K8s) 클러스터에 전체 Milvus 서비스 스택을 배포하고 관리하는 데 도움을 주는 솔루션입니다. 이 스택에는 모든 Milvus 구성 요소와 etcd, MinIO와 같은 관련 종속성이 포함되어 있으며, Woodpecker가 내장 메시지 큐로 사용됩니다.</p>
...
<p>이미지 가져오기 과정에서 문제가 발생하면, 문제에 대한 자세한 내용을 <a href="mailto:community@zilliz.com">community@zilliz.com으로</a> 문의해 주시면 필요한 지원을 제공해 드리겠습니다.</p>
</div>
...
<button class="copy-code-btn"></button></code></pre>
<p>위의 명령어는 <strong>Woodpecker를</strong> 메시지 큐로 사용하고(v3.0.2 권장), 스트리밍 노드를 포함한 모든 새로운 아키텍처 구성 요소를 갖춘 Milvus 클러스터를 배포합니다.</p>
...
<p>이러한 설정을 사용자 정의하려면 <a href="https://milvus.io/tools/sizing">Milvus Sizing Tool을</a> 사용하여 실제 데이터 크기에 따라 구성을 조정한 후, 해당 YAML 파일을 다운로드할 것을 권장합니다. 구성 매개변수에 대한 자세한 내용은 <a href="https://milvus.io/docs/system_configuration.md">Milvus 시스템 구성 체크리스트를</a> 참조하십시오.</p>
<div class="alert note">
<ul>
...
<button class="copy-code-btn"></button></code></pre>
<p>Milvus 클러스터가 준비되면 위 명령어의 출력 결과는 다음과 유사하게 표시됩니다. ‘ <code translate="no">status.status</code> ’ 필드가 계속 ‘ <code translate="no">Unhealthy</code> ’로 표시된다면, Milvus 클러스터가 아직 생성 중인 것입니다.</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>선택적으로, 위 명령어에서 <code translate="no">27017:19530</code> 대신 <code translate="no">:19530</code> 를 사용하여 <code translate="no">kubectl</code> 가 로컬 포트를 자동으로 할당하도록 할 수 있습니다. 이렇게 하면 포트 충돌을 직접 관리할 필요가 없습니다.</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>이제 <code translate="no">http://localhost:27018</code> 에서 Milvus 웹 UI에 접속할 수 있습니다.</p>
...
<div class="alert note">
<p>Storage V3는 기본적으로 비활성화되어 있습니다. Storage V3에 의존하는 기능을 사용하기 전에 이를 활성화하십시오. 요구 사항 및 호환성 고려 사항에 대해서는 <a href="/docs/ko/storage-v3.md">Storage V3를</a> 참조하십시오.</p>
</div>
