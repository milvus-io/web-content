...
label: Docker
related_key: Docker
summary: Windows용 Docker Desktop을 사용하여 Milvus 독립 실행형 버전을 설치하는 방법을 알아보세요.
title: Docker에서 Milvus 실행하기 (Windows)
---
...
<p>기본적으로 이 배포 환경은 <strong>Woodpecker</strong> (로컬 파일 시스템)를 메시지 큐로 사용하므로, 별도의 외부 메시지 큐 서비스가 필요하지 않습니다. <a href="/docs/ko/woodpecker.md">Woodpecker를</a> 참조하십시오.</p>
</blockquote>
<p>이 페이지에서는 Windows용 Docker Desktop을 사용하여 Windows에서 Milvus를 실행하는 방법을 설명합니다.</p>
...
<li><p><a href="https://docs.docker.com/desktop/setup/install/windows-install/">Docker Desktop을 설치하십시오</a>.</p></li>
<li><p><a href="https://learn.microsoft.com/en-us/windows/wsl/install#install-wsl-command">Windows Subsystem for Linux 2(WSL 2)를 설치하십시오</a>.</p></li>
<li><p>Python 3.8 이상을 설치하십시오.</p></li>
</ul>
<h2 id="Run-Milvus-in-Docker​" class="common-anchor-header">Docker에서 Milvus 실행​<button data-href="#Run-Milvus-in-Docker​" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Milvus는 Docker 컨테이너로 설치할 수 있는 설치 스크립트를 제공합니다. Microsoft Windows에 Docker Desktop을 설치한 후에는 <strong>관리자</strong> 모드의 PowerShell 또는 Windows 명령 프롬프트, 그리고 WSL 2에서 Docker CLI에 액세스할 수 있습니다. ​</p>
<h3 id="From-PowerShell-or-Windows-Command-Prompt​" class="common-anchor-header">PowerShell 또는 Windows 명령 프롬프트에서​<button data-href="#From-PowerShell-or-Windows-Command-Prompt​" class="anchor-icon" translate="no">
    ...
    </button></h3><p>PowerShell이나 Windows 명령 프롬프트에 더 익숙하다면, 명령 프롬프트 사용 방법은 다음과 같습니다.​</p>
<ol>
<li><p>Docker Desktop을 마우스 오른쪽 버튼으로 클릭하고 <strong>‘관리자 권한으로 실행’을</strong> 선택하여 관리자 모드로 엽니다.​</p></li>
<li><p>설치 스크립트를 다운로드하여 <code translate="no">standalone.bat</code> 로 저장합니다.​</p>
...
<h3 id="From-WSL-2​" class="common-anchor-header">WSL 2에서​<button data-href="#From-WSL-2​" class="anchor-icon" translate="no">
    ...
    </button></h3><p>Windows에서 Linux 명령어와 셸 스크립트를 사용하여 Milvus를 시작하려는 경우, WSL 2가 이미 설치되어 있는지 확인하십시오. WSL 2 설치 방법에 대한 자세한 내용은 이 <a href="https://learn.microsoft.com/en-us/windows/wsl/install#install-wsl-command">Microsoft 문서를</a> 참조하십시오.​</p>
<ol>
<li><p>WSL 2를 시작합니다.​</p>
...
<li><p>Milvus를 Docker 컨테이너로 시작합니다.​</p>
...
<h2 id="Run-Milvus-with-Docker-Compose​" class="common-anchor-header">Docker Compose를 사용하여 Milvus 실행​<button data-href="#Run-Milvus-with-Docker-Compose​" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>Microsoft Windows에 Docker Desktop을 설치하면 <strong>관리자</strong> 모드의 PowerShell 또는 Windows 명령 프롬프트에서 Docker CLI에 액세스할 수 있습니다. PowerShell, Windows 명령 프롬프트 또는 WSL 2에서 Docker Compose를 실행하여 Milvus를 시작할 수 있습니다.​</p>
<h3 id="From-PowerShell-or-Windows-Command-Prompt​" class="common-anchor-header">PowerShell 또는 Windows 명령 프롬프트에서​<button data-href="#From-PowerShell-or-Windows-Command-Prompt​" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
...
<p>네트워크 연결 상태에 따라 Milvus 설치용 이미지 다운로드에 다소 시간이 걸릴 수 있습니다. <strong>milvus-standalone</strong>, <strong>milvus-minio</strong>, <strong>milvus-etcd라는</strong> 이름의 컨테이너가 실행되면 다음을 확인할 수 있습니다. ​</p>
<ul>
<li><p><strong>milvus-etcd</strong> 컨테이너는 호스트에 어떤 포트도 노출하지 않으며, 데이터를 현재 폴더의 <strong>volumes/etcd에</strong> 매핑합니다.​</p></li>
<li><p><strong>milvus-minio</strong> 컨테이너는 기본 인증 자격 증명을 사용하여 로컬에서 <strong>9000번</strong> 및 <strong>9001번</strong> 포트를 제공하며, 데이터를 현재 폴더의 <strong>volumes/minio에</strong> 매핑합니다.​</p></li>
<li><p><strong>milvus-standalone</strong> 컨테이너는 기본 설정으로 로컬에서 <strong>19530</strong> 포트를 제공하며, 데이터를 현재 폴더의 <strong>volumes/milvus에</strong> 매핑합니다.​</p></li>
</ul></li>
</ol>
<p>WSL 2가 설치되어 있다면 Linux 버전의 Docker Compose 명령어를 호출할 수도 있습니다.​</p>
<h3 id="From-WSL-2​" class="common-anchor-header">WSL 2에서​<button data-href="#From-WSL-2​" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
    ...
    </button></h3><p>이 절차는 Linux 시스템에서 Docker Compose를 사용하여 Milvus를 설치하는 방법과 유사합니다.​</p>
<ol>
<li><p>WSL 2를 시작합니다.​</p>
<pre><code translate="no" class="language-powershell">C:\&gt;wsl --install​
Ubuntu already installed.​
...
<h2 id="FAQs​" class="common-anchor-header">자주 묻는 질문​<button data-href="#FAQs​" class="anchor-icon" translate="no">
    ...
    </button></h2><h3 id="How-can-I-deal-with-the-Docker-Engine-stopped-error​" class="common-anchor-header"><code translate="no">Docker Engine stopped</code> 오류는 어떻게 해결할 수 있나요?​<button data-href="#How-can-I-deal-with-the-Docker-Engine-stopped-error​" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Windows에 Docker Desktop을 설치한 후, 컴퓨터가 올바르게 구성되지 않은 경우 ' <code translate="no">Docker Engine stopped</code> ' 오류가 발생할 수 있습니다. 이 경우 다음 사항을 확인해 보시기 바랍니다:​</p>
<ol>
<li><p>가상화가 활성화되어 있는지 확인하십시오.​</p>
<p><strong>작업 관리자의</strong> <strong>‘성능’</strong> 탭을 확인하여 가상화가 활성화되어 있는지 확인할 수 있습니다.​</p>
<p><span class="img-wrapper">
   ...
   <span>작업 관리자의 가상화</span>
  
 </span></p>
<p>가상화가 비활성화되어 있다면, 마더보드 펌웨어의 BIOS 설정을 확인해야 할 수 있습니다. BIOS 설정에서 가상화를 활성화하는 방법은 마더보드 제조사에 따라 다릅니다. 예를 들어, ASUS 마더보드의 경우 가상화 활성화에 관한 <a href="https://www.asus.com/support/faq/1043786/">이 문서를</a> 참조할 수 있습니다.​</p>
<p>그런 다음 컴퓨터를 다시 시작하고 Hyper-V를 활성화해야 합니다. 자세한 내용은 이 <a href="https://learn.microsoft.com/en-us/virtualization/hyper-v-on-windows/quick-start/enable-hyper-v#enable-the-hyper-v-role-through-settings">Microsoft 문서를</a> 참조하십시오.​</p></li>
<li><p>Docker Desktop 서비스가 시작되었는지 확인하십시오.</p>
<p>다음 명령을 실행하여 Docker Desktop 서비스를 시작할 수 있습니다.​</p>
<pre><code translate="no" class="language-powershell">C:\&gt;net start com.docker.service​
The Docker for Windows Service service is starting.​
The Docker for Windows Service service was started successfully.​

</code></pre></li>
<li><p>WSL이 제대로 설치되었는지 확인하십시오.</p>
<p>다음 명령을 실행하여 WSL 2를 설치하거나 업데이트할 수 있습니다.</p>
<pre><code translate="no" class="language-powershell">C:\&gt;wsl --update​
Checking for updates.​
The most recent version of Windows Subsystem for Linux is already installed.​

</code></pre></li>
<li><p>Docker 데몬이 실행 중인지 확인하십시오.</p>
<p>Docker Desktop의 설치 디렉터리로 이동하여 ` <code translate="no">.\DockerCli.exe -SwitchDaemon</code> ` 명령을 실행하여 Docker 데몬을 시작해야 합니다.​</p>
<pre><code translate="no" class="language-powershell">C:\&gt;cd &quot;C:\Program Files\Docker\Docker&quot;​
C:\Program Files\Docker\Docker&gt;.\DockerCli.exe -SwitchDaemon​
Switching to windows engine: Post &quot;http://ipc/engine/switch&quot;: open \\.\pipe\dockerBackendApiServer: The system cannot find the file specified.​

</code></pre></li>
<li><p>Docker Desktop을 <strong>관리자</strong> 모드로 시작했는지 확인하십시오.</p>
<p>Docker Desktop을 관리자 모드로 시작했는지 확인하십시오. 이를 위해 <strong>Docker Desktop을</strong> 마우스 오른쪽 버튼으로 클릭하고 <strong>‘관리자 권한으로 실행’을</strong> 선택하십시오.​</p>
<p><span class="img-wrapper">
   ...
   <span>관리자 권한으로 Docker Desktop 시작</span>
  
 </span></p></li>
</ol>
<h3 id="How-can-I-deal-with-WSL-related-issues-while-deploying-Milvus​" class="common-anchor-header">Milvus 배포 중 WSL 관련 문제가 발생하면 어떻게 해결해야 하나요?​<button data-href="#How-can-I-deal-with-WSL-related-issues-while-deploying-Milvus​" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<li><p><strong>설정</strong> &gt; <strong>리소스</strong> &gt; <strong>WSL 통합으로</strong> 이동하여, Docker 통합을 활성화할 설치된 WSL 2 배포판을 선택하십시오.</p>
<p><span class="img-wrapper">
   ...
   <span>Docker Desktop 설정에서 WSL 2 배포판 선택</span>
  
 </span></p></li>
</ol>
<h3 id="How-can-I-deal-with-the-volume-related-errors-prompted-during-Milvus-startup-that-reads-Read-config-failed​" class="common-anchor-header">Milvus 시작 시 “ <code translate="no">Read config failed</code> ”라는 볼륨 관련 오류가 표시되면 어떻게 해결해야 하나요?​<button data-href="#How-can-I-deal-with-the-volume-related-errors-prompted-during-Milvus-startup-that-reads-Read-config-failed​" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<p>Milvus 시작 시 “Read config failed”라는 오류 메시지가 표시될 경우, Milvus 컨테이너에 마운트된 볼륨이 올바른지 확인해야 합니다. 볼륨이 컨테이너에 올바르게 마운트되어 있다면, <code translate="no">docker exec</code> 명령어를 사용하여 컨테이너에 진입한 후 다음과 같이 <strong>/milvus/configs</strong> 폴더의 내용을 확인할 수 있습니다:​</p>
<p><span class="img-wrapper">
   ...
   <span>Milvus 구성 파일 열거</span>
  
 </span></p>
<p>​</p>
<div class="alert note">
<p>Storage V3는 기본적으로 비활성화되어 있습니다. Storage V3에 의존하는 기능을 사용하기 전에 이를 활성화해야 합니다. 요구 사항 및 호환성 관련 사항은 <a href="/docs/ko/storage-v3.md">Storage V3를</a> 참조하십시오.</p>
</div>
