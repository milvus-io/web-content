...
<h3 id="From-PowerShell-or-Windows-Command-Prompt​" class="common-anchor-header">PowerShell 또는 Windows 명령 프롬프트에서​<button data-href="#From-PowerShell-or-Windows-Command-Prompt​" class="anchor-icon" translate="no">
    ...
    </button></h3><p>PowerShell이나 Windows 명령 프롬프트에 더 익숙하신 경우, 명령 프롬프트 사용법은 다음과 같습니다.​</p>
<ol>
...
<li><p>Milvus 데이터 볼륨은 현재 폴더의 ` <strong>volumes/milvus</strong> `에 매핑되어 있습니다.​</p></li>
</ul>
...
<li><p><strong>milvus-minio</strong> 컨테이너는 기본 인증 자격 증명을 사용하여 로컬에서 포트 <strong>9000</strong> 및 <strong>9001을</strong> 제공하며, 데이터를 현재 폴더의 <strong>volumes/minio에</strong> 매핑합니다.​</p></li>
...
<p><strong>작업 관리자의</strong> <strong>‘성능’</strong> 탭에서 가상화가 활성화되어 있는지 확인할 수 있습니다.​</p>
...
<p>Docker Desktop의 설치 디렉터리로 이동하여 ` <code translate="no">.\DockerCli.exe -SwitchDaemon</code> ` 명령을 실행하면 Docker 데몬이 시작됩니다.​</p>
...
<h3 id="How-can-I-deal-with-the-volume-related-errors-prompted-during-Milvus-startup-that-reads-Read-config-failed​" class="common-anchor-header">Milvus 시작 시 ‘ <code translate="no">Read config failed</code> ’라는 볼륨 관련 오류 메시지가 표시되면 어떻게 해결해야 하나요?​<button data-href="#How-can-I-deal-with-the-volume-related-errors-prompted-during-Milvus-startup-that-reads-Read-config-failed​" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<p>Milvus 시작 시 “Read config failed”라는 오류가 표시되는 경우, Milvus 컨테이너에 마운트된 볼륨이 올바른지 확인해야 합니다. 볼륨이 컨테이너에 올바르게 마운트된 경우, <code translate="no">docker exec</code> 명령을 사용하여 컨테이너로 진입한 후 다음과 같이 <strong>/milvus/configs</strong> 폴더의 내용을 확인할 수 있습니다:​</p>
<p><span class="img-wrapper">
...
<ul>
<li><p>'빠른 시작( <a href="/docs/ko/quickstart.md">Quickstart</a> )'을 확인하여 Milvus의 기능을 살펴보세요.</p></li>
...
<li><p><a href="/docs/ko/monitor.md">Prometheus를 사용하여 Milvus를 모니터링해 보세요</a>.</p></li>
</ul>
