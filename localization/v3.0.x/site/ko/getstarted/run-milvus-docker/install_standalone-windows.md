...
label: Docker
related_key: Docker
summary: Windows용 Docker Desktop을 사용하여 Milvus 독립 실행형을 설치하는 방법을 알아보세요.
...
<h2 id="Run-Milvus-in-Docker​" class="common-anchor-header">Docker에서 Milvus 실행​<button data-href="#Run-Milvus-in-Docker​" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Milvus는 Docker 컨테이너로 설치할 수 있는 설치 스크립트를 제공합니다. Microsoft Windows에 Docker Desktop을 설치하면, <strong>관리자</strong> 모드의 PowerShell 또는 Windows 명령 프롬프트와 WSL 2에서 Docker CLI에 액세스할 수 있습니다. ​</p>
...
<ol>
<li><p>마우스 오른쪽 버튼을 클릭하고 <strong>‘관리자 권한으로 실행’을</strong> 선택하여 Docker Desktop을 <strong>관리자</strong> 모드로 엽니다.​</p></li>
...
<li><p>PowerShell 또는 Windows 명령 프롬프트에서 다음 명령을 실행하여 Milvus Standalone용 Docker Compose 구성 파일을 다운로드하고 Milvus를 시작하세요.</p>
...
<p>네트워크 연결 상태에 따라 Milvus 설치용 이미지를 다운로드하는 데 시간이 다소 걸릴 수 있습니다. <strong>milvus-standalone</strong>, <strong>milvus-minio</strong>, <strong>milvus-etcd라는</strong> 이름의 컨테이너가 실행되면 다음을 확인할 수 있습니다. ​</p>
<ul>
...
<li><p><strong>milvus-standalone</strong> 컨테이너는 기본 설정으로 로컬에서 <strong>19530번</strong> 포트를 제공하며, 데이터를 현재 폴더의 <strong>volumes/milvus에</strong> 매핑합니다.​</p></li>
</ul></li>
</ol>
...
<h2 id="FAQs​" class="common-anchor-header">자주 묻는 질문​<button data-href="#FAQs​" class="anchor-icon" translate="no">
    ...
    </button></h2><h3 id="How-can-I-deal-with-the-Docker-Engine-stopped-error​" class="common-anchor-header"><code translate="no">Docker Engine stopped</code> 오류는 어떻게 해결하나요?​<button data-href="#How-can-I-deal-with-the-Docker-Engine-stopped-error​" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Windows에 Docker Desktop을 설치한 후, 컴퓨터가 제대로 구성되지 않은 경우 ' <code translate="no">Docker Engine stopped</code> ' 오류가 발생할 수 있습니다. 이 경우 다음 사항을 확인해 보시기 바랍니다:​</p>
<ol>
<li><p>가상화가 활성화되어 있는지 확인하십시오.​</p>
<p><strong>작업 관리자의</strong> <strong>‘성능’</strong> 탭에서 가상화 기능이 활성화되어 있는지 확인할 수 있습니다.​</p>
...
<p>다음 명령어를 실행하여 Docker Desktop 서비스를 시작할 수 있습니다.​</p>
...
<li><p>Docker 데몬이 시작되었는지 확인하십시오.</p>
...
<h3 id="How-can-I-deal-with-WSL-related-issues-while-deploying-Milvus​" class="common-anchor-header">Milvus 배포 시 WSL 관련 문제를 어떻게 해결할 수 있나요?​<button data-href="#How-can-I-deal-with-WSL-related-issues-while-deploying-Milvus​" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<ol>
<li><p><strong>'설정</strong> &gt; <strong>일반</strong>'에서 “WSL 2 기반 엔진 사용” 항목이 선택되어 있는지 확인하십시오. ​</p>
<p><span class="img-wrapper">
...
<h3 id="How-can-I-deal-with-the-volume-related-errors-prompted-during-Milvus-startup-that-reads-Read-config-failed​" class="common-anchor-header">Milvus 시작 시 “ <code translate="no">Read config failed</code> ”라는 볼륨 관련 오류가 표시되면 어떻게 해결해야 하나요?​<button data-href="#How-can-I-deal-with-the-volume-related-errors-prompted-during-Milvus-startup-that-reads-Read-config-failed​" class="anchor-icon" translate="no">
   ...
   <span>Milvus 시작 시 ‘ ’ 오류 메시지</span>
  
 </span></p>
<p>Milvus 시작 시 “Read config failed”라는 오류 메시지가 표시될 경우, Milvus 컨테이너에 마운트된 볼륨이 올바른지 확인해야 합니다. 볼륨이 컨테이너에 올바르게 마운트되어 있다면, <code translate="no">docker exec</code> 명령어를 사용하여 컨테이너로 진입한 후 다음과 같이 <strong>/milvus/configs</strong> 폴더의 내용을 확인할 수 있습니다:​</p>
<p><span class="img-wrapper">
...
<li><a href="/docs/ko/insert-update-delete.md">삽입, 업데이트 및 삭제</a></li>
