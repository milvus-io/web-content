...
<h3 id="From-PowerShell-or-Windows-Command-Prompt​" class="common-anchor-header">PowerShell 또는 Windows 명령 프롬프트에서​<button data-href="#From-PowerShell-or-Windows-Command-Prompt​" class="anchor-icon" translate="no">
    ...
    </button></h3><p>PowerShell이나 Windows 명령 프롬프트에 더 익숙한 경우, 명령 프롬프트 사용 방법은 다음과 같습니다.​</p>
<ol>
...
<h3 id="From-WSL-2​" class="common-anchor-header">WSL 2에서​<button data-href="#From-WSL-2​" class="anchor-icon" translate="no">
    ...
    </button></h3><p>Windows에서 Linux 명령어와 셸 스크립트를 사용하여 Milvus를 시작하려는 경우, WSL 2 명령어가 이미 설치되어 있는지 확인하십시오. WSL 2 명령어 설치 방법에 대한 자세한 내용은 이 <a href="https://learn.microsoft.com/en-us/windows/wsl/install#install-wsl-command">Microsoft 문서를</a> 참조하십시오.​</p>
<ol>
<li><p>WSL 2를 시작합니다.​</p>
...
<p>그런 다음 컴퓨터를 다시 시작한 후 Hyper-V를 활성화해야 합니다. 자세한 내용은 이 <a href="https://learn.microsoft.com/en-us/virtualization/hyper-v-on-windows/quick-start/enable-hyper-v#enable-the-hyper-v-role-through-settings">Microsoft 문서를</a> 참조하십시오.​</p></li>
...
<ol>
<li><p><strong>'설정'</strong> &gt; <strong>'일반'</strong>에서 “WSL 2 기반 엔진 사용”이 선택되어 있는지 확인하십시오. ​</p>
<p><span class="img-wrapper">
...
<li><p><strong>설정</strong> &gt; <strong>리소스</strong> &gt; <strong>WSL 통합으로</strong> 이동하여, 설치된 WSL 2 배포판 중에서 Docker 통합을 활성화할 배포판을 선택하십시오.​</p>
<p><span class="img-wrapper">
...
<p>Milvus 시작 시 “Read config failed”라는 오류 메시지가 표시되는 문제를 해결하려면, Milvus 컨테이너에 마운트된 볼륨이 올바른지 확인해야 합니다. 볼륨이 컨테이너에 올바르게 마운트된 경우, <code translate="no">docker exec</code> 명령어를 사용하여 컨테이너로 진입한 후 다음과 같이 <strong>/milvus/configs</strong> 폴더의 내용을 확인할 수 있습니다:​</p>
<p><span class="img-wrapper">
