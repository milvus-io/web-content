...
<h2 id="Install-Milvus" class="common-anchor-header">Instal Milvus<button data-href="#Install-Milvus" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Milvus menyediakan berkas konfigurasi Docker Compose di repositori Milvus. Untuk menginstal Milvus menggunakan Docker Compose, jalankan perintah berikut</p>
...
<p><strong>Penerapan default (v3.0-beta):</strong> <code translate="no">docker compose up -d</code> memulai tiga kontainer — <code translate="no">milvus-etcd</code> (metadata), <code translate="no">milvus-minio</code> (penyimpanan objek), dan <code translate="no">milvus-standalone</code>. Antrian pesan adalah <strong>Woodpecker (tertanam, dengan MinIO / penyimpanan objek sebagai backend WAL-nya)</strong>, sehingga tidak diperlukan kontainer antrian pesan terpisah.</p>
...
<li><strong>2.6.x dan yang lebih baru</strong> — antrian pesan default adalah <strong>Woodpecker (terintegrasi)</strong>.</li>
</ul>
<p>Selalu unduh konfigurasi Docker Compose terbaru untuk memastikan kompatibilitas dengan fitur v3.0-beta.</p>
<ul>
<li><p>Jika Anda gagal menjalankan perintah di atas, silakan periksa apakah sistem Anda telah menginstal Docker Compose V1. Jika demikian, Anda disarankan untuk beralih ke Docker Compose V2 sesuai dengan catatan pada <a href="https://docs.docker.com/compose/">halaman ini</a>.</p></li>
...
Berikut ini mengasumsikan bahwa Anda perlu menggantikan konfigurasi default di ` <code translate="no">proxy.healthCheckTimeout</code>`. Untuk item konfigurasi yang berlaku, lihat <a href="/docs/id/system_configuration.md">Konfigurasi Sistem</a>.</p>
...
<h2 id="Optional-dependencies" class="common-anchor-header">Ketergantungan opsional<button data-href="#Optional-dependencies" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Penerapan ini menjalankan <strong>Woodpecker</strong> (terintegrasi, backend MinIO WAL) untuk pesan, <strong>etcd</strong> untuk metadata, dan <strong>MinIO</strong> untuk penyimpanan objek. Untuk menggunakan antrian pesan yang berbeda atau menghubungkan penyimpanan objek / metadata eksternal, lihat:</p>
<ul>
