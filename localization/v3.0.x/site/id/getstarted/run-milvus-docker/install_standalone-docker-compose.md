...
<p><strong>Penerapan default (v3.0.1):</strong> <code translate="no">docker compose up -d</code> memulai tiga kontainer — <code translate="no">milvus-etcd</code> (metadata), <code translate="no">milvus-minio</code> (penyimpanan objek), dan <code translate="no">milvus-standalone</code>. Antrian pesan menggunakan <strong>Woodpecker (terintegrasi, dengan MinIO / penyimpanan objek sebagai backend WAL-nya)</strong>, sehingga tidak diperlukan kontainer antrian pesan terpisah.</p>
...
<p>Selalu unduh konfigurasi Docker Compose terbaru untuk memastikan kompatibilitas dengan fitur v3.0.1.</p>
<ul>
