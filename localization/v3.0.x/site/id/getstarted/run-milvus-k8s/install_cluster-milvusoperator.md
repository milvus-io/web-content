...
summary: >-
  Pelajari cara menginstal klaster Milvus di Kubernetes menggunakan Milvus
  Operator
title: Instal Milvus Cluster dengan Milvus Operator
---
<h1 id="Run-Milvus-in-Kubernetes-with-Milvus-Operator" class="common-anchor-header">Menjalankan Milvus di Kubernetes dengan Milvus Operator<button data-href="#Run-Milvus-in-Kubernetes-with-Milvus-Operator" class="anchor-icon" translate="no">
    ...
    </button></h1><p>Halaman ini menjelaskan cara memulai instance Milvus di Kubernetes menggunakan <a href="https://github.com/zilliztech/milvus-operator">Milvus Operator</a>.</p>
<h2 id="Overview" class="common-anchor-header">Gambaran Umum<button data-href="#Overview" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Milvus Operator adalah solusi yang membantu Anda menerapkan dan mengelola tumpukan layanan Milvus lengkap untuk menargetkan klaster Kubernetes (K8s). Tumpukan tersebut mencakup semua komponen Milvus dan dependensi yang relevan seperti etcd dan MinIO, dengan Woodpecker sebagai antrian pesan bawaan.</p>
...
<h2 id="Install-Milvus-Operator" class="common-anchor-header">Instal Milvus Operator<button data-href="#Install-Milvus-Operator" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>Milvus Operator mendefinisikan sumber daya khusus klaster Milvus di atas <a href="https://kubernetes.io/docs/concepts/extend-kubernetes/api-extension/custom-resources/">Sumber Daya Khusus Kubernetes</a>. Ketika sumber daya khusus telah didefinisikan, Anda dapat menggunakan API K8s secara deklaratif dan mengelola tumpukan penyebaran Milvus untuk memastikan skalabilitas dan ketersediaan tinggi.</p>
<div class="filter">
 <a href="#helm">Helm</a>
...
<p>Anda akan melihat hasil yang mirip dengan berikut ini setelah proses instalasi selesai.</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>Jika Anda telah menginstal Milvus Operator sebelumnya, perbarui menggunakan perintah berikut:</p>
...
<p>Jalankan perintah berikut untuk menginstal Milvus Operator deng <code translate="no">kubectl</code>.</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>Anda dapat memeriksa apakah pod Milvus Operator sedang berjalan sebagai berikut:</p>
...
<h2 id="Deploy-Milvus" class="common-anchor-header">Menerapkan Milvus<button data-href="#Deploy-Milvus" class="anchor-icon" translate="no">
    ...
    </button></h3><p>Setelah pod Milvus Operator berjalan, Anda dapat melakukan deployment kluster Milvus sebagai berikut.</p>
<pre><code translate="no" class="language-shell"><span class="hljs-meta prompt_">$ </span><span class="language-bash">kubectl apply -f https://raw.githubusercontent.com/zilliztech/milvus-operator/main/config/samples/milvus_cluster_woodpecker.yaml</span>
<button class="copy-code-btn"></button></code></pre>
<p>Perintah di atas mengimplementasikan kluster Milvus dengan <strong>Woodpecker</strong> sebagai antrian pesan (direkomendasikan untuk v3.0.2) dan semua komponen arsitektur baru termasuk Streaming Node.</p>
<p><strong>Fitur utama arsitektur dalam penyebaran ini:</strong></p>
<ul>
<li><strong>Antrian Pesan</strong>: <a href="/docs/id/woodpecker.md">Menggunakan Woodpecker</a> (mengurangi pemeliharaan infrastruktur)</li>
<li><strong>Streaming Node</strong>: Diaktifkan untuk pemrosesan data yang lebih baik</li>
<li><strong>Mix Coordinator</strong>: Komponen koordinator yang digabungkan untuk meningkatkan efisiensi</li>
</ul>
<p>Untuk menyesuaikan pengaturan ini, kami menyarankan Anda menggunakan <a href="https://milvus.io/tools/sizing">Milvus Sizing Tool</a> untuk menyesuaikan konfigurasi berdasarkan ukuran data aktual Anda, lalu mengunduh berkas YAML yang sesuai. Untuk informasi lebih lanjut tentang parameter konfigurasi, lihat <a href="https://milvus.io/docs/system_configuration.md">Milvus System Configurations Checklist</a>.</p>
<div class="alert note">
<ul>
...
<h3 id="2-Check-Milvus-cluster-status" class="common-anchor-header">2. Periksa status kluster Milvus<button data-href="#2-Check-Milvus-cluster-status" class="anchor-icon" translate="no">
    ...
    </button></h3><p>Jalankan perintah berikut untuk memeriksa status kluster Milvus</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>Setelah kluster Milvus Anda siap, hasil perintah di atas seharusnya mirip dengan berikut ini. Jika kolom " <code translate="no">status.status</code> " tetap menunjukkan " <code translate="no">Unhealthy</code>", kluster Milvus Anda masih dalam proses pembuatan.</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>Milvus Operator membuat dependensi Milvus, seperti etcd dan MinIO, kemudian komponen Milvus, seperti proxy, koordinator, dan node.</p>
<p>Setelah kluster Milvus Anda siap, status semua pod di kluster Milvus seharusnya mirip dengan berikut ini.</p>
...
<h3 id="3-Forward-a-local-port-to-Milvus" class="common-anchor-header">3. Alihkan port lokal ke Milvus<button data-href="#3-Forward-a-local-port-to-Milvus" class="anchor-icon" translate="no">
    ...
    </button></h3><p>Jalankan perintah berikut untuk mengetahui port yang digunakan oleh kluster Milvus Anda.</p>
...
19530
<button class="copy-code-btn"></button></code></pre>
<p>Hasilnya menunjukkan bahwa instance Milvus beroperasi di port default <strong>19530</strong>.</p>
<div class="alert note">
<p>Jika Anda telah menerapkan Milvus dalam mode mandiri, ubah nama pod dari <code translate="no">my-release-milvus-proxy-xxxxxxxxxx-xxxxx</code> menjadi <code translate="no">my-release-milvus-xxxxxxxxxx-xxxxx</code>.</p>
</div>
<p>Kemudian, jalankan perintah berikut untuk meneruskan port lokal ke port tempat Milvus beroperasi.</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>Secara opsional, Anda dapat menggunakan <code translate="no">:19530</code> alih-alih <code translate="no">27017:19530</code> dalam perintah di atas agar <code translate="no">kubectl</code> mengalokasikan port lokal untuk Anda sehingga Anda tidak perlu mengelola konflik port.</p>
...
<h2 id="Optional-Update-Milvus-configurations" class="common-anchor-header">(Opsional) Perbarui konfigurasi Milvus<button data-href="#Optional-Update-Milvus-configurations" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Anda dapat melihat dan memperbarui konfigurasi kluster Milvus Anda dengan menjalankan perintah ` <code translate="no">patch</code> ` sebagai berikut:</p>
<ol>
<li><p>Jalankan perintah berikut untuk melihat pratinjau konfigurasi yang akan diterapkan.</p>
<p>Berikut ini mengasumsikan bahwa Anda ingin memperbarui parameter ` <code translate="no">spec.components.disableMetric</code> ` menjadi ` <code translate="no">false</code> ` ms.</p>
...
<h2 id="Uninstall-Milvus" class="common-anchor-header">Hapus Instalasi Milvus<button data-href="#Uninstall-Milvus" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Jalankan perintah berikut untuk menghapus instalasi kluster Milvus.</p>
...
<div class="alert note">
<ul>
<li>Saat Anda menghapus kluster Milvus menggunakan konfigurasi default, dependensi seperti etcd dan MinIO tidak akan dihapus. Oleh karena itu, saat Anda menginstal instance kluster Milvus yang sama di lain waktu, dependensi ini akan digunakan kembali.</li>
<li>Untuk menghapus dependensi dan klaim volume persisten (PVC) bersama dengan kluster Milvus, lihat <a href="https://github.com/zilliztech/milvus-operator/blob/main/config/samples/milvus_deletion.yaml">berkas konfigurasi</a>.</li>
</ul>
</div>
<h2 id="Uninstall-Milvus-Operator" class="common-anchor-header">Hapus Instalasi Milvus Operator<button data-href="#Uninstall-Milvus-Operator" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<h2 id="Optional-dependencies" class="common-anchor-header">Dependensi opsional<button data-href="#Optional-dependencies" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Deployment ini menggunakan <strong>Woodpecker</strong> sebagai antrian pesan, <strong>etcd</strong> untuk metadata, dan <strong>MinIO</strong> untuk penyimpanan objek, semuanya disediakan oleh Milvus Operator. Untuk menggunakan backend yang berbeda dengan Operator, lihat:</p>
<ul>
<li>Antrian pesan: <a href="/docs/id/woodpecker.md">Woodpecker</a> (default) · <a href="/docs/id/message_storage_operator.md#Configure-Pulsar">Pulsar</a> · <a href="/docs/id/message_storage_operator.md#Configure-Kafka">Kafka</a> · <a href="/docs/id/message_storage_operator.md#Configure-RocksMQ">RocksMQ</a></li>
<li>Penyimpanan objek: <a href="/docs/id/object_storage_operator.md">Konfigurasikan penyimpanan objek dengan Milvus Operator</a></li>
<li>Metadata: <a href="/docs/id/meta_storage_operator.md">Konfigurasikan etcd dengan Milvus Operator</a></li>
</ul>
<div class="alert note">
<p>Storage V3 dinonaktifkan secara default. Aktifkan sebelum menggunakan fitur yang bergantung padanya. Untuk persyaratan dan pertimbangan kompatibilitas, lihat <a href="/docs/id/storage-v3.md">Storage V3</a>.</p>
</div>
