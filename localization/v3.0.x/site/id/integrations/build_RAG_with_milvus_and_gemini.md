...
summary: >-
  Dalam tutorial ini, kami akan menunjukkan kepada Anda cara membangun alur
  kerja RAG (Retrieval-Augmented Generation) menggunakan Milvus dan Gemini. Kami
  akan menggunakan model Gemini untuk menghasilkan teks berdasarkan kueri yang
  diberikan. Kami juga akan menggunakan Milvus untuk menyimpan dan mengambil
  teks yang dihasilkan.
title: Bangun RAG dengan Milvus dan Gemini
---
...
<h1 id="Build-RAG-with-Milvus-and-Gemini" class="common-anchor-header">Bangun RAG dengan Milvus dan Gemini<button data-href="#Build-RAG-with-Milvus-and-Gemini" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h1><p><a href="https://ai.google.dev/gemini-api/docs">API Gemini</a> dan <a href="https://ai.google.dev/aistudio">Google AI Studio</a> membantu Anda mulai bekerja dengan model-model terbaru Google dan mengubah ide-ide Anda menjadi aplikasi yang dapat diskalakan. Gemini menyediakan akses ke model bahasa yang kuat seperti <code translate="no">Gemini-2.5-Flash</code> dan <code translate="no">Gemini-2.5-Pro</code> untuk tugas-tugas seperti pembuatan teks, pemrosesan dokumen, penglihatan, analisis audio, dan banyak lagi. Gemini juga menawarkan <code translate="no">Gemini Embedding 2</code>, model embedding multimodal yang mendukung teks, gambar, video, audio, dan dokumen PDF dengan dimensi keluaran yang fleksibel melalui Matryoshka Representation Learning. API ini memungkinkan Anda memasukkan konteks panjang dengan jutaan token, menyesuaikan model untuk tugas-tugas spesifik, menghasilkan keluaran terstruktur seperti JSON, serta memanfaatkan kemampuan seperti pencarian semantik dan eksekusi kode.</p>
<p>Dalam tutorial ini, kami akan menunjukkan kepada Anda cara membangun pipa RAG (Retrieval-Augmented Generation) dengan Milvus dan Gemini. Kami akan menggunakan model Gemini untuk menghasilkan respons berdasarkan kueri yang diberikan, yang dilengkapi dengan informasi relevan yang diambil dari Milvus.</p>
<h2 id="Preparation" class="common-anchor-header">Persiapan<button data-href="#Preparation" class="anchor-icon" translate="no">
    ...
    </button></h2><h3 id="Dependencies-and-Environment" class="common-anchor-header">Ketergantungan dan Lingkungan<button data-href="#Dependencies-and-Environment" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Pertama, instal paket yang diperlukan:</p>
...
<p>Anda harus terlebih dahulu masuk ke platform Google AI Studio dan menyiapkan <a href="https://aistudio.google.com/apikey">kunci API</a> <code translate="no">GEMINI_API_KEY</code> sebagai variabel lingkungan.</p>
...
<h3 id="Prepare-the-data" class="common-anchor-header">Siapkan data<button data-href="#Prepare-the-data" class="anchor-icon" translate="no">
    ...
    </button></h3><p>Kami menggunakan halaman FAQ dari <a href="https://github.com/milvus-io/milvus-docs/releases/download/v2.4.6-preview/milvus_docs_2.4.x_en.zip">Dokumentasi Milvus 2.4.x</a> sebagai pengetahuan pribadi dalam RAG kami, yang merupakan sumber data yang baik untuk pipa RAG sederhana.</p>
<p>Unduh berkas ZIP dan ekstrak dokumen ke folder <code translate="no">milvus_docs</code>.</p>
<pre><code translate="no" class="language-shell"><span class="hljs-meta prompt_">$ </span><span class="language-bash">wget https://github.com/milvus-io/milvus-docs/releases/download/v2.4.6-preview/milvus_docs_2.4.x_en.zip</span>
<span class="hljs-meta prompt_">$ </span><span class="language-bash">unzip -q milvus_docs_2.4.x_en.zip -d milvus_docs</span>
<button class="copy-code-btn"></button></code></pre>
<p>Kami memuat semua berkas markdown dari folder <code translate="no">milvus_docs/en/faq</code>. Untuk setiap dokumen, kami cukup menggunakan "# " untuk memisahkan isi dalam berkas, yang secara kasar dapat memisahkan isi setiap bagian utama berkas markdown.</p>
...
<button class="copy-code-btn"></button></code></pre>
<h3 id="Prepare-the-LLM-and-Embedding-Model" class="common-anchor-header">Siapkan LLM dan Model Embedding<button data-href="#Prepare-the-LLM-and-Embedding-Model" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Kami menggunakan <code translate="no">gemini-2.5-flash</code> sebagai LLM, dan <code translate="no">gemini-embedding-2-preview</code> sebagai model embedding. <code translate="no">gemini-embedding-2-preview</code> adalah model embedding multimodal terbaru dari Google, yang mendukung teks, gambar, video, audio, dan dokumen PDF dengan dimensi keluaran yang fleksibel (128–3.072) melalui Matryoshka Representation Learning.</p>
<p>Mari kita coba menghasilkan respons uji dari LLM:</p>
...
<p>Buat embedding uji dan tampilkan dimensinya serta beberapa elemen pertamanya.</p>
...
<h2 id="Load-data-into-Milvus" class="common-anchor-header">Muat data ke dalam Milvus<button data-href="#Load-data-into-Milvus" class="anchor-icon" translate="no">
    ...
    </button></h2><h3 id="Create-the-Collection" class="common-anchor-header">Buat Koleksi<button data-href="#Create-the-Collection" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Mari kita inisialisasi klien Milvus dan atur koleksi kita:</p>
...
<ul>
<li>Menetapkan ` <code translate="no">uri</code> ` sebagai file lokal, misalnya `<code translate="no">./milvus.db</code>`, adalah metode yang paling praktis, karena secara otomatis menggunakan <a href="https://milvus.io/docs/milvus_lite.md">Milvus Lite</a> untuk menyimpan semua data dalam file tersebut.</li>
<li>Jika Anda memiliki data dalam skala besar, Anda dapat menyiapkan server Milvus yang lebih berperforma di <a href="https://milvus.io/docs/quickstart.md">Docker atau Kubernetes</a>. Dalam pengaturan ini, harap gunakan URI server, misalnya<code translate="no">http://localhost:19530</code>, sebagai <code translate="no">uri</code> Anda.</li>
...
<p>Periksa apakah koleksi tersebut sudah ada dan hapus jika sudah ada.</p>
<pre><code translate="no" class="language-python"><span class="hljs-keyword">if</span> milvus_client.has_collection(collection_name):
    milvus_client.drop_collection(collection_name)
<button class="copy-code-btn"></button></code></pre>
<p>Buat koleksi baru dengan parameter yang ditentukan.</p>
<p>Jika kita tidak menentukan informasi bidang apa pun, Milvus akan secara otomatis membuat bidang <code translate="no">id</code> default untuk kunci utama, dan bidang <code translate="no">vector</code> untuk menyimpan data vektor. Bidang JSON yang dicadangkan digunakan untuk menyimpan bidang yang tidak ditentukan dalam skema beserta nilainya.</p>
...
<h3 id="Insert-data" class="common-anchor-header">Sisipkan data<button data-href="#Insert-data" class="anchor-icon" translate="no">
    ...
    </button></h3><p>Lakukan iterasi melalui baris-baris teks, buat embedding, lalu masukkan data ke dalam Milvus.</p>
<p>Berikut adalah bidang baru <code translate="no">text</code>, yang merupakan bidang yang tidak didefinisikan dalam skema koleksi. Bidang ini akan secara otomatis ditambahkan ke bidang dinamis JSON yang dicadangkan, yang pada tingkat tinggi dapat diperlakukan sebagai bidang biasa.</p>
...
<h2 id="Build-RAG" class="common-anchor-header">Bangun RAG<button data-href="#Build-RAG" class="anchor-icon" translate="no">
    ...
    </button></h2><h3 id="Retrieve-data-for-a-query" class="common-anchor-header">Mengambil data untuk kueri<button data-href="#Retrieve-data-for-a-query" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Mari tentukan pertanyaan yang sering diajukan tentang Milvus.</p>
<pre><code translate="no" class="language-python">question = <span class="hljs-string">&quot;How is data stored in milvus?&quot;</span>
<button class="copy-code-btn"></button></code></pre>
<p>Cari pertanyaan tersebut dalam koleksi dan ambil 3 hasil pencocokan semantik teratas.</p>
...
<h3 id="Use-LLM-to-get-a-RAG-response" class="common-anchor-header">Gunakan LLM untuk mendapatkan respons RAG<button data-href="#Use-LLM-to-get-a-RAG-response" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Konversikan dokumen yang diambil ke dalam format string.</p>
<pre><code translate="no" class="language-python">context = <span class="hljs-string">&quot;\n&quot;</span>.join(
    [line_with_distance[<span class="hljs-number">0</span>] <span class="hljs-keyword">for</span> line_with_distance <span class="hljs-keyword">in</span> retrieved_lines_with_distances]
)
<button class="copy-code-btn"></button></code></pre>
<p>Tentukan prompt sistem dan pengguna untuk Model Bahasa. Prompt ini disusun menggunakan dokumen yang diambil dari Milvus.</p>
...
&quot;&quot;&quot;</span>
<button class="copy-code-btn"></button></code></pre>
<p>Gunakan Gemini untuk menghasilkan respons berdasarkan prompt tersebut.</p>
...
<h2 id="Multimodal-Search" class="common-anchor-header">Pencarian Multimodal<button data-href="#Multimodal-Search" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>Karena <code translate="no">gemini-embedding-2-preview</code> memetakan teks, gambar, dan modalitas lainnya ke dalam ruang embedding yang sama, kita dapat melakukan pencarian lintas-modal — misalnya, menggunakan kueri teks untuk menemukan gambar yang paling relevan.</p>
<h3 id="Prepare-image-data" class="common-anchor-header">Siapkan data gambar<button data-href="#Prepare-image-data" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Kami mengunduh serangkaian diagram arsitektur RAG dari repositori Milvus Bootcamp untuk digunakan sebagai dataset gambar kami.</p>
...
<h3 id="Embed-images-and-store-in-Milvus" class="common-anchor-header">Lakukan embedding pada gambar dan simpan di Milvus<button data-href="#Embed-images-and-store-in-Milvus" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Kami membaca setiap gambar sebagai byte dan meneruskannya ke <code translate="no">gemini-embedding-2-preview</code> untuk menghasilkan embedding, lalu menyimpannya dalam koleksi Milvus baru.</p>
<pre><code translate="no" class="language-python"><span class="hljs-keyword">from</span> google.genai <span class="hljs-keyword">import</span> types
...
<h3 id="Cross-modal-search-Text-query-→-Image-results" class="common-anchor-header">Pencarian lintas-modal: Kueri teks → Hasil gambar<button data-href="#Cross-modal-search-Text-query-→-Image-results" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Sekarang, mari gunakan kueri teks untuk mencari di seluruh embedding gambar. Karena teks dan gambar sama-sama dipetakan ke ruang embedding yang sama, kita dapat membandingkannya secara langsung.</p>
...
<p><span class="img-wrapper">
   ...
   <span>Pencarian dan Penilaian Ulang Hibrida</span>
  
 </span></p>
<p>Hebat! Kita telah berhasil membangun pipa RAG dengan Milvus dan Gemini, serta mendemonstrasikan pencarian lintas-modal menggunakan kueri teks untuk mengambil gambar yang relevan — semuanya didukung oleh ruang embedding terpadu dari <code translate="no">gemini-embedding-2-preview</code>.</p>
