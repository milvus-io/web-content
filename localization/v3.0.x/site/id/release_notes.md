...
<p>Milvus v3.0-beta memulai pergeseran Milvus dari basis data vektor ke mesin lake semantik-asli. Kernel Milvus sekarang dapat beroperasi secara langsung pada data dalam format lake terbuka, dan kemampuan inti Milvus telah diperluas di seluruh pengambilan, skema, siklus hidup, bahasa, dan operasi.</p>
<p>Koleksi Eksternal dan Snapshot adalah tambahan utama di sisi danau. Kernel yang sama juga mendukung Zilliz Lakebase, sebuah platform data semantik-asli yang dibangun di atas Milvus 3.0.</p>
...
<h4 id="Snapshot" class="common-anchor-header">Cuplikan</h4><p>Penyajian dan penemuan kumpulan sering kali membutuhkan Koleksi yang sama pada waktu yang sama. Evaluasi model A/B, deduplikasi skala besar, validasi pengisian ulang, dan rollback versi semuanya membutuhkan tampilan Collection yang stabil saat penulisan masih berlangsung.</p>
...
<h4 id="External-Backfill" class="common-anchor-header">Pengisian Ulang Eksternal</h4><p>Memutakhirkan model penyematan, seperti berpindah dari penyematan v1 ke penyematan v2 pada Koleksi yang sudah ada, biasanya berarti membangun ulang dari awal. Hal ini memaksa adanya waktu henti layanan atau logika penulisan ganda di sisi aplikasi.</p>
<p>Milvus 3.0 mendukung peningkatan sebagai alur kerja yang cepat. Anda dapat menambahkan bidang vektor baru dengan <code translate="no">AddCollectionField</code>, menggunakan Snapshot untuk membekukan titik awal yang konsisten, menjalankan pekerjaan penyematan secara offline terhadap Snapshot, dan menulis nilainya kembali melalui jalur konsumsi normal. Setelah bidang baru diindeks secara online, aplikasi dapat beralih tanpa waktu henti.</p>
<h4 id="Query--Search-Order-By" class="common-anchor-header">Perintah Kueri / Pencarian Berdasarkan</h4><p>Pencarian dan Query sekarang menerima pengurutan multi-bidang, dengan pengurutan yang didorong ke dalam kernel Milvus dan <code translate="no">ASC</code> / <code translate="no">DESC</code> yang dapat diatur per bidang. Hal ini menutup kesenjangan produksi yang umum terjadi: Top-K berdasarkan jarak saja sering kali tidak sesuai dengan kebutuhan bisnis ketika item yang paling mirip bukanlah yang termurah, terbaru, atau terpopuler.</p>
...
<h4 id="Null-Vector" class="common-anchor-header">Vektor Nol</h4><p>Penyematan sering kali dibuat secara tidak sinkron, sehingga sebuah entitas dapat muncul sebelum vektornya muncul. Data multimodal juga memiliki kesenjangan alami, seperti video tanpa teks atau produk tanpa gambar. Versi sebelumnya tidak memiliki jawaban yang baik: aplikasi menunda penulisan hingga vektor siap atau mengisi vektor placeholder, dan kedua pilihan tersebut merusak kualitas pengambilan.</p>
