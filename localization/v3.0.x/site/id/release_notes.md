...
<p>Milvus v3.0-beta menandai awal peralihan Milvus dari database vektor menjadi mesin data lake yang native semantik. Kernel Milvus kini dapat beroperasi langsung pada data dalam format data lake terbuka, dan kemampuan inti Milvus telah diperluas mencakup pengambilan data, skema, siklus hidup, bahasa, dan operasi.</p>
...
<h3 id="Key-Features" class="common-anchor-header">Fitur Utama<button data-href="#Key-Features" class="anchor-icon" translate="no">
    ...
    </button></h3><h4 id="External-Collection" class="common-anchor-header">Pengumpulan Eksternal</h4><p>Dalam pipa data AI pada umumnya, terabyte embedding dan metadata sudah tersimpan di penyimpanan objek sebagai tabel Parquet, Lance, atau Iceberg. Menyalin data tersebut ke Milvus akan menggandakan biaya penyimpanan, menambah pipa ETL yang harus tetap disinkronkan, serta mengalihkan tata kelola data dari pelanggan.</p>
<p>Pengumpulan Eksternal menghilangkan proses penyalinan tersebut. Koleksi Milvus dapat merujuk ke file di lokasi aslinya, dan Milvus hanya mengelola skema, indeks, serta eksekusi kueri. Penyegaran bertahap memastikan Koleksi tetap selaras dengan file-file dasarnya. Pelanggan yang datanya tidak dapat dipindahkan dari data lake, seperti tim keuangan dan kesehatan, dapat menjalankan pencarian vektor terhadap data tersebut di lokasi aslinya. Satu set data yang berada di data lake juga dapat disajikan dari beberapa instans Milvus secara bersamaan.</p>
<p>Untuk informasi lebih lanjut, lihat <a href="/docs/id/create-an-external-collection.md">Membuat Koleksi Eksternal</a>.</p>
<h4 id="Snapshot" class="common-anchor-header">Snapshot</h4><p>Penyajian dan penemuan batch seringkali memerlukan Koleksi yang sama pada waktu yang bersamaan. Evaluasi model A/B, deduplikasi skala besar, validasi pengisian ulang, dan rollback versi semuanya memerlukan tampilan Koleksi yang stabil sementara penulisan data masih berlangsung.</p>
...
<p>Aplikasi tidak lagi perlu mengambil hasil secara berlebihan dan menyortir ulang di sisi klien untuk menampilkan peringkat gabungan.</p>
...
<h4 id="Null-Vector" class="common-anchor-header">Vektor Null</h4><p>Embedding sering kali dihasilkan secara asinkron, sehingga suatu entitas dapat tiba sebelum vektornya. Data multimodal juga memiliki celah alami, seperti video tanpa teks atau produk tanpa gambar. Versi sebelumnya tidak memiliki solusi yang memadai: aplikasi harus menunda penulisan hingga vektor siap atau mengisi vektor tempatholder, dan kedua pilihan tersebut merugikan kualitas pengambilan data.</p>
<p>Milvus 3.0 mendukung nilai NULL pada bidang vektor di seluruh enam jenis vektor. Pencarian secara otomatis melewati vektor NULL, kualitas pengambilan data tidak terpengaruh, dan vektor NULL secara efektif tidak memakan ruang penyimpanan. " <code translate="no">AddField</code> " juga berlaku untuk bidang vektor dalam perubahan ini: dengan " <code translate="no">nullable=True</code>", Koleksi yang sudah ada dapat menambahkan bidang vektor baru secara online tanpa perlu rebuild.</p>
...
<h4 id="MinHash-DIDO-Doc-in-Doc-out" class="common-anchor-header">MinHash DIDO (Doc-in, Doc-out)</h4><p>Milvus 2.6 menambahkan indeks ` <code translate="no">MINHASH_LSH</code> ` untuk deteksi duplikat serupa berbasis himpunan, namun aplikasi masih harus menghitung tanda tangan MinHash sebelum menulis data ke Milvus.</p>
...
<h4 id="Force-Merge" class="common-anchor-header">Paksakan Penggabungan</h4><p>Beban kerja produksi menumpuk fragmentasi segmen seiring waktu, yang menyebabkan fluktuasi latensi kueri dan pembengkakan penyimpanan.</p>
<p>Milvus 3.0 menambahkan kemampuan untuk memicu kompresi segmen secara eksplisit selama jendela di luar jam sibuk, baik dalam mode sinkron maupun asinkron.</p>
<p>Untuk informasi lebih lanjut, lihat " <a href="/docs/id/force-merge.md">Force Merge Compaction</a>".</p>
