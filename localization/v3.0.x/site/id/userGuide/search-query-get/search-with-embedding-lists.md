...
summary: >-
  ...
  dengan interaksi terlambat pada tingkat entitas menggunakan metrik MAX_SIM*.
---
...
<p>ColBERT (arXiv: <a href="https://arxiv.org/abs/2004.12832">2004.12832</a>) adalah sistem pencarian teks-teks yang menawarkan pencarian bagian teks yang efisien dan efektif melalui interaksi kontekstual pada tahap akhir menggunakan BERT. Sistem ini memungkinkan pengkodean token-per-token yang independen untuk kueri dan dokumen serta menghitung kemiripannya.</p>
...
<h3 id="ColPali-extension" class="common-anchor-header">Ekstensi ColPali<button data-href="#ColPali-extension" class="anchor-icon" translate="no">
    ...
    </button></h3><p>Berdasarkan ColBERT, ColPali (arXiv: <a href="https://arxiv.org/abs/2407.01449?spm=a2ty_o01.29997173.0.0.31c4c9217HFv28&amp;file=2407.01449">2407.01449</a>) mengusulkan pendekatan baru untuk pencarian dokumen yang kaya secara visual yang memanfaatkan Model Visi-Bahasa (VLMs). Selama proses pengambilan data, setiap halaman dokumen dirender menjadi gambar beresolusi tinggi, kemudian dipecah menjadi patch, bukan ditokenisasi. Misalnya, gambar halaman dokumen berukuran 448 x 448 piksel dapat menghasilkan 1.024 patch, masing-masing berukuran 14 x 14 piksel.</p>
...
<h3 id="Step-6-Search-within-the-Cohere-dataset" class="common-anchor-header">Langkah 6: Melakukan pencarian di dalam dataset Cohere<button data-href="#Step-6-Search-within-the-Cohere-dataset" class="anchor-icon" translate="no">
    ...
    </button></h3><p>Sesuai dengan desain ColBERT, teks kueri harus ditokenisasi dan kemudian disematkan ke dalam EmbeddingList. Pada langkah ini, kita akan menggunakan model yang sama dengan yang digunakan Cohere untuk menghasilkan embedding untuk paragraf-paragraf dalam dataset Wikipedia.</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>Menjalankan skrip di atas akan mengunduh dataset jika dataset tersebut belum tersedia secara lokal. Setiap catatan dalam dataset ini merupakan satu halaman dari laporan keuangan. Tabel berikut menunjukkan struktur dataset ini.</p>
<table>
<thead>
<tr><th>Nama Kolom</th><th>Deskripsi</th></tr>
</thead>
...
<button class="copy-code-btn"></button></code></pre>
<div class="alert note">
<p>Proses penyisipan laporan keuangan bisa memakan waktu lama. Setiap halaman bisa berisi lebih dari seribu vektor patch, dan setiap vektor disimpan di dalam bidang StructArray ` <code translate="no">patches</code> `. Untuk dataset yang lebih besar, bagi ` <code translate="no">data</code> ` menjadi batch yang lebih kecil dan sisipkan satu batch pada satu waktu.</p>
</div>
