...
<tr><td><code translate="no">ARRAY&lt;VARCHAR&gt;</code> elemen</td><td>Ya</td><td>Ya</td><td>Cocokkan elemen tertentu berdasarkan indeks, seperti <code translate="no">tags[0]</code>. Pencocokan pola <strong>tidak</strong> memindai semua elemen; pencocokan ini hanya berlaku untuk elemen pada indeks yang ditentukan.</td></tr>
...
<tr><td>Mencocokkan kode terstruktur atau pola panjang tetap</td><td><code translate="no">=~</code></td><td><code translate="no">code =~ &quot;E[0-9]{4}&quot;</code></td><td>Mencocokkan string yang secara peka huruf besar/kecil mengandung <code translate="no">E</code> diikuti oleh empat digit, seperti <code translate="no">E1001</code>.</td></tr>
...
<button class="copy-code-btn"></button></code></pre>
<p>Ini mencocokkan string yang berisi nilai mirip tanggal seperti <code translate="no">2026-07-01</code>.</p>
<p>Tanpa string mentah, literal string biasa memproses urutan pelarian sebelum pola regex dievaluasi, sehingga pola seperti <code translate="no">\d</code>, <code translate="no">\s</code>, atau karakter literal yang dilindungi mungkin memerlukan tanda garis miring terbalik tambahan.</p>
...
<tr><td>Cocok dengan urutan angka</td><td><code translate="no">[0-9]+</code></td><td><code translate="no">filter = 'message =~ &quot;[0-9]+&quot;'</code></td></tr>
...
<button class="copy-code-btn"></button></code></pre>
<p>Saat mencocokkan karakter meta regex secara harfiah, lakukan escape pada pola regex tersebut. Misalnya, untuk mencocokkan titik (<code translate="no">\.</code> ) dalam regex, tulis <code translate="no">\\.</code> dalam string filter Python:</p>
...
<p>Filter regex tidak cocok dengan nilai null. Hal ini berlaku untuk <code translate="no">=~</code> dan <code translate="no">!~</code>. Jika Anda ingin mengecualikan pola regex tetapi tetap mempertahankan nilai null, tambahkan <code translate="no">OR field IS NULL</code> secara eksplisit:</p>
