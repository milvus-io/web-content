...
<h3 id="Example-Filtering-by-Permission-Bits" class="common-anchor-header">Contoh: Penyaringan berdasarkan Bit Izin<button data-href="#Example-Filtering-by-Permission-Bits" class="anchor-icon" translate="no">
    ...
    </button></h3><p>Untuk menemukan entitas yang bidang integer- <code translate="no">permissions</code> -nya memiliki bit " <code translate="no">SHARE</code> " yang diaktifkan, gunakan operator bitwise AND (<code translate="no">&amp;</code>):</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>Filter Regex menggunakan pencocokan substring. Untuk memastikan seluruh nilai bidang cocok dengan pola, tambahkan penanda <code translate="no">^</code> dan <code translate="no">$</code>. Untuk detailnya, lihat <a href="/docs/id/pattern-matching.md">Pencocokan Pola</a>.</p>
...
<h3 id="JSON-field-specific-operators" class="common-anchor-header">Operator khusus bidang JSON<button data-href="#JSON-field-specific-operators" class="anchor-icon" translate="no">
    ...
    </button></h3><p>Milvus menawarkan operator lanjutan untuk melakukan kueri pada kolom JSON, sehingga memungkinkan penyaringan yang tepat di dalam struktur JSON yang kompleks:</p>
<p><code translate="no">JSON_CONTAINS(identifier, jsonExpr)</code>: Memeriksa apakah ekspresi JSON terdapat dalam kolom tersebut.</p>
...
<button class="copy-code-btn"></button></code></pre>
<p><code translate="no">ARRAY_CONTAINS_ALL</code>: Menyaring entitas di mana semua elemen dalam daftar tersebut terdapat di dalamnya.</p>
...
<ol>
<li><p><strong>Analyzer</strong>: Menokenisasi dan memproses teks masukan.</p></li>
