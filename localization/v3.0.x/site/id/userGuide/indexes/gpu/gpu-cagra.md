...
<li><p><code translate="no">metric_type</code>: Metode yang digunakan untuk menghitung jarak antar vektor. Untuk detailnya, lihat <a href="/docs/id/metric.md">Jenis Metrik</a>.</p></li>
<li><p><code translate="no">params</code>: Opsi konfigurasi tambahan untuk membangun indeks. Untuk mempelajari lebih lanjut tentang parameter pembangunan yang tersedia untuk indeks ` <code translate="no">GPU_CAGRA</code> `, lihat <a href="/docs/id/gpu-cagra.md#Index-building-params">Parameter Pembangunan Indeks</a>.</p></li>
</ul>
<p>Setelah parameter indeks dikonfigurasi, Anda dapat membuat indeks dengan menggunakan metode ` <code translate="no">create_index()</code> ` secara langsung atau dengan meneruskan parameter indeks ke dalam metode ` <code translate="no">create_collection</code> `. Untuk detailnya, lihat <a href="/docs/id/create-collection.md">Buat Koleksi</a>.</p>
...
<ul>
<li><p>Ketika ` <code translate="no">load.adapt_for_cpu</code> ` diatur ke ` <code translate="no">true</code>`, Milvus mengonversi indeks ` <strong>GPU_CAGRA</strong> ` menjadi format yang dapat dieksekusi di CPU (seperti HNSW) selama proses pemuatan.</p></li>
...
<p>Gunakan adaptasi CPU saat pemuatan di lingkungan hybrid atau yang sensitif terhadap biaya, di mana sumber daya GPU dicadangkan untuk pembuatan indeks tetapi pencarian dijalankan di CPU.</p>
</div>
...
<table>
   ...
   <tr>
     ...
     <td><p>Menentukan apakah dataset asli akan disimpan dalam cache memori GPU. Nilai yang mungkin:</p><ul><li><p><code translate="no">"true"</code>: Menyimpan dataset asli ke dalam cache untuk meningkatkan recall dengan menyempurnakan hasil pencarian.</p></li><li><p><code translate="no">"false"</code>: Tidak menyimpan dataset asli ke dalam cache untuk menghemat memori GPU.</p></li></ul></td>
...
<h3 id="Index-specific-search-params" class="common-anchor-header">Parameter pencarian khusus indeks<button data-href="#Index-specific-search-params" class="anchor-icon" translate="no">
    ...
    </button></h3><p>Tabel berikut mencantumkan parameter yang dapat dikonfigurasi di <code translate="no">search_params.params</code> saat <a href="/docs/id/gpu-cagra.md#Search-on-index">melakukan pencarian pada indeks</a>.</p>
<table>
   <tr>
     <th><p>Parameter</p></th>
     <th><p>Deskripsi</p></th>
   ...
   <tr>
     ...
     <td><p>Menentukan jumlah titik masuk ke dalam grafik CAGRA selama proses pencarian. Meningkatkan nilai ini dapat meningkatkan recall tetapi mungkin memengaruhi kinerja pencarian (misalnya 1, 2, 4, 8, 16, 32).</p></td>
     <td><p>Kosong</p></td>
   </tr>
   <tr>
     <td><p><code translate="no">num_random_samplings</code></p></td>
     <td><p>Mengontrol seberapa banyak pengambilan sampel acak yang dilakukan CAGRA saat memilih titik masuk awal untuk pencarian graf. Nilai yang lebih besar memberi CAGRA lebih banyak peluang untuk memulai dari titik yang lebih baik, sehingga meningkatkan recall dengan mengorbankan latensi pencarian yang lebih tinggi. Nilai ini harus setidaknya <code translate="no">1</code>. Tersedia di Milvus 2.6.20+.</p></td>
