...
<ul>
<li><p>Ketika ` <code translate="no">load.adapt_for_cpu</code> ` diatur ke ` <code translate="no">true</code>`, Milvus akan mengonversi indeks ` <strong>GPU_CAGRA</strong> ` menjadi format yang dapat dieksekusi di CPU (seperti HNSW) selama proses pemuatan.</p></li>
...
<table>
   ...
   <tr>
     ...
     <td><p>Menentukan keseimbangan antara waktu pencarian dan akurasi. Nilai ` <code translate="no">ef</code> ` yang lebih tinggi menghasilkan pencarian yang lebih akurat namun lebih lambat.</p><p>Parameter ini wajib diisi jika Anda menetapkan ` <code translate="no">adapt_for_cpu</code> ` ke ` <code translate="no">true</code> ` saat membangun indeks.</p></td>
