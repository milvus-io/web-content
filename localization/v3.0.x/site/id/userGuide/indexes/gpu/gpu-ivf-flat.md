...
title: GPU_IVF_FLAT
summary: >-
  Indeks GPU_IVF_FLAT adalah versi akselerasi GPU dari indeks IVF_FLAT, yang
  dirancang khusus untuk lingkungan GPU. Indeks ini mempartisi data vektor ke
  dalam unit klaster nlist dan menghitung kemiripan dengan terlebih dahulu
  membandingkan vektor kueri target dengan pusat setiap klaster. Dengan menyetel
  parameter nprobe, hanya cluster yang paling menjanjikan yang dicari, yang
  mengurangi waktu kueri sekaligus menjaga keseimbangan antara akurasi dan
  kecepatan. Untuk informasi lebih lanjut mengenai konsep dasar, lihat IVF_FLAT.
---
<h1 id="GPUIVFFLAT" class="common-anchor-header">GPU_IVF_FLAT<button data-href="#GPUIVFFLAT" class="anchor-icon" translate="no">
    ...
    </button></h1><p>Indeks <strong>GPU_IVF_FLAT</strong> adalah versi akselerasi GPU dari indeks IVF_FLAT, yang dirancang khusus untuk lingkungan GPU. Indeks ini mempartisi data vektor ke dalam unit klaster <code translate="no">nlist</code> dan menghitung kemiripan dengan terlebih dahulu membandingkan vektor kueri target dengan pusat setiap klaster. Dengan menyetel parameter <code translate="no">nprobe</code>, hanya klaster yang paling menjanjikan yang dicari, yang mengurangi waktu kueri sekaligus menjaga keseimbangan antara akurasi dan kecepatan. Untuk informasi lebih lanjut tentang konsep dasar, lihat <a href="/docs/id/ivf-flat.md">IVF_FLAT</a>.</p>
<h2 id="Build-index" class="common-anchor-header">Membangun indeks<button data-href="#Build-index" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Untuk membangun indeks <code translate="no">GPU_IVF_FLAT</code> pada bidang vektor di Milvus, gunakan metode <code translate="no">add_index()</code>, tentukan parameter <code translate="no">index_type</code>, <code translate="no">metric_type</code>, dan parameter tambahan untuk indeks.</p>
...
<ul>
<li><p><code translate="no">index_type</code>: Jenis indeks yang akan dibangun. Dalam contoh ini, tetapkan nilainya ke <code translate="no">GPU_IVF_FLAT</code>.</p></li>
...
<p>Untuk mempelajari lebih lanjut parameter pembuatan yang tersedia untuk indeks <code translate="no">GPU_IVF_FLAT</code>, lihat Parameter <a href="/docs/id/gpu-ivf-flat.md#Index-building-params">pembuatan indeks</a>.</p></li>
</ul>
...
<p>Untuk mempelajari lebih lanjut parameter pencarian yang tersedia untuk indeks <code translate="no">GPU_IVF_FLAT</code>, lihat Parameter <a href="/docs/id/gpu-ivf-flat.md#Index-specific-search-params">pencarian khusus indeks</a>.</p></li>
</ul>
...
<h3 id="Index-building-params" class="common-anchor-header">Parameter pembangunan indeks</h3><p>Tabel berikut mencantumkan parameter yang dapat dikonfigurasi di <code translate="no">params</code> saat <a href="/docs/id/gpu-ivf-flat.md#Build-index">membangun indeks.</a></p>
<table>
   <tr>
     <th><p>Parameter</p></th>
     <th><p>Deskripsi</p></th>
...
<h3 id="Index-specific-search-params" class="common-anchor-header">Parameter pencarian khusus indeks</h3><p>Tabel berikut mencantumkan parameter yang dapat dikonfigurasi di <code translate="no">search_params.params</code> saat melakukan <a href="/docs/id/gpu-ivf-flat.md#Search-on-index">pencarian di indeks</a>.</p>
<table>
   <tr>
     <th><p>Parameter</p></th>
     <th><p>Deskripsi</p></th>
