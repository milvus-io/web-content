...
<h1 id="JSON-Field-Overview" class="common-anchor-header">Gambaran Umum Bidang JSON<button data-href="#JSON-Field-Overview" class="anchor-icon" translate="no">
    ...
    </button></h1><p>Saat membangun aplikasi seperti katalog produk, sistem manajemen konten, atau mesin preferensi pengguna, Anda sering kali perlu menyimpan metadata yang fleksibel bersama dengan embedding vektor Anda. Atribut produk bervariasi menurut kategori, preferensi pengguna berubah seiring waktu, dan properti dokumen memiliki struktur bersarang yang kompleks. Bidang JSON di Milvus mengatasi tantangan ini dengan memungkinkan Anda menyimpan dan menanyakan data terstruktur yang fleksibel tanpa mengorbankan kinerja.</p>
...
<table>
   ...
   <tr>
     ...
     <td><p>Sebuah bidang skalar yang harus dinyatakan secara eksplisit dalam skema koleksi dengan tipe <code translate="no">DataType.JSON</code>.</p></td>
...
<h2 id="Basic-operations" class="common-anchor-header">Operasi dasar<button data-href="#Basic-operations" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Alur kerja dasar untuk menggunakan bidang JSON melibatkan mendefinisikannya dalam skema Anda, memasukkan data, dan kemudian melakukan kueri data menggunakan ekspresi filter tertentu.</p>
...
<h2 id="Next-Accelerate-JSON-queries" class="common-anchor-header">Selanjutnya: Mempercepat kueri JSON<button data-href="#Next-Accelerate-JSON-queries" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Secara default, kueri pada bidang JSON tanpa akselerasi akan melakukan pemindaian penuh terhadap semua baris, yang dapat memakan waktu lama pada kumpulan data besar. Untuk mempercepat kueri JSON, Milvus menyediakan fitur pengindeksan lanjutan dan optimasi penyimpanan.</p>
<div class="alert warning">
<p>Mulai dari Milvus 3.0.0, pengindeksan JSON seluruh objek (<code translate="no">json_cast_type=&quot;JSON&quot;</code>), yang juga dikenal sebagai pengindeksan JSON datar, tidak lagi direkomendasikan. Indeks yang sudah ada dan permintaan pembuatan indeks baru tetap didukung demi kompatibilitas, tetapi mode ini tidak lagi direkomendasikan untuk beban kerja baru. Gunakan pengindeksan jalur JSON untuk jalur kueri yang sudah diketahui, atau pertimbangkan <a href="/docs/id/json-shredding.md">JSON Shredding</a> untuk percepatan kueri secara luas pada dokumen yang kompleks atau terus berkembang.</p>
</div>
...
<p><strong>Tip:</strong> Anda dapat menggabungkan pendekatan ini—misalnya, gunakan JSON shredding untuk mempercepat kueri luas, pengindeksan JSON untuk kunci array yang sering digunakan, dan pengindeksan NGRAM untuk pencarian teks yang fleksibel.</p>
