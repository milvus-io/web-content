...
title: Pengurai
summary: >-
  Gunakan filter pengurai untuk memisahkan kata majemuk dengan kamus sebaris
  atau sumber daya file terdaftar.
---
<h1 id="Decompounder" class="common-anchor-header">Pengurai<button data-href="#Decompounder" class="anchor-icon" translate="no">
    ...
    </button></h1><p>Filter <code translate="no">decompounder</code> membagi kata majemuk menjadi beberapa komponen berdasarkan kamus yang ditentukan, sehingga lebih mudah untuk mencari bagian dari istilah majemuk. Filter ini sangat berguna untuk bahasa yang sering menggunakan kata majemuk, seperti bahasa Jerman. Kamus komponen dapat disediakan sebaris melalui parameter <code translate="no">word_list</code> atau dimuat dari <a href="/docs/id/manage-file-resources.md">sumber daya file terdaftar</a> melalui parameter <code translate="no">word_list_file</code>.</p>
<h2 id="Configuration" class="common-anchor-header">Konfigurasi<button data-href="#Configuration" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Filter <code translate="no">decompounder</code> menerima kamus komponennya secara sebaris melalui parameter <code translate="no">word_list</code> atau dari sumber daya file yang terdaftar melalui parameter <code translate="no">word_list_file</code>.</p>
<h3 id="Inline-word-list" class="common-anchor-header">Daftar kata sebaris<button data-href="#Inline-word-list" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Filter <code translate="no">decompounder</code> adalah filter khusus di Milvus. Untuk menggunakannya, tentukan <code translate="no">&quot;type&quot;: &quot;decompounder&quot;</code> dalam konfigurasi filter, bersama dengan parameter <code translate="no">word_list</code> yang menyediakan kamus komponen kata yang akan dikenali.</p>
...
<h3 id="Load-word-components-from-a-file-resource--Milvus-30x" class="common-anchor-header">Memuat komponen kata dari sumber daya file<span class="beta-tag" style="background-color:rgb(0, 179, 255);color:white" translate="no">Compatible with Milvus 3.0.x</span><button data-href="#Load-word-components-from-a-file-resource--Milvus-30x" class="anchor-icon" translate="no">
    ...
    </button></h3><p>Untuk kamus komponen yang besar - terutama daftar kata dalam bahasa lengkap - simpan komponen dalam sebuah berkas dan daftarkan berkas tersebut sebagai sumber daya berkas jarak jauh, lalu rujuk dari filter melalui parameter <code translate="no">word_list_file</code>. Anda dapat menggunakan <code translate="no">word_list_file</code> sendiri atau bersama dengan <code translate="no">word_list</code> sebaris; ketika keduanya diatur, filter menggabungkan kedua sumber menjadi satu daftar komponen.</p>
<p>Berkas tersebut berupa teks UTF-8 biasa dengan <strong>satu kata komponen per baris</strong>. Sebagai contoh:</p>
...
automat
<button class="copy-code-btn"></button></code></pre>
<p>Unggah berkas ke penyimpanan objek yang dikonfigurasikan untuk digunakan oleh cluster Milvus Anda, lalu daftarkan:</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>Rujuk sumber daya yang terdaftar dalam filter melalui <code translate="no">word_list_file</code>:</p>
<pre><code translate="no" class="language-python">analyzer_params = {
    <span class="hljs-string">&quot;tokenizer&quot;</span>: <span class="hljs-string">&quot;standard&quot;</span>,
    <span class="hljs-string">&quot;filter&quot;</span>: [{
        <span class="hljs-string">&quot;type&quot;</span>: <span class="hljs-string">&quot;decompounder&quot;</span>,
...
<button class="copy-code-btn"></button></code></pre>
<p>Parameter <code translate="no">word_list_file</code> menerima objek dengan bidang berikut ini:</p>
<table>
   <tr>
   ...
   <tr>
     ...
     <td><p>Bagian nama file dari jalur penyimpanan objek sumber daya yang terdaftar (misalnya, <code translate="no">"decompounder.txt"</code> jika sumber daya terdaftar dengan <code translate="no">path="file/decompounder.txt"</code>).</p></td>
   </tr>
</table>
