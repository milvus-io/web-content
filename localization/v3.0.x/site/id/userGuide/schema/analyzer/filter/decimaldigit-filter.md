...
title: Angka DesimalCompatible with Milvus 3.0.0+
summary: Filter digit desimal mengubah digit desimal Unicode menjadi digit ASCII.
beta: Milvus 3.0.0+
---
<h1 id="Decimal-Digit" class="common-anchor-header">Angka Desimal<span class="beta-tag" style="background-color:rgb(0, 179, 255);color:white" translate="no">Compatible with Milvus 3.0.0+</span><button data-href="#Decimal-Digit" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h1><p>Filter ` <code translate="no">decimaldigit</code> ` adalah filter token bawaan yang mengubah digit desimal Unicode dari skrip yang didukung menjadi digit ASCII. Hal ini memastikan konsistensi token numerik di seluruh bahasa dan sistem penulisan.</p>
...
<p>Filter ` <code translate="no">decimaldigit</code> ` tidak memiliki parameter yang dapat dikonfigurasi.</p>
<p>Filter ini mengubah digit desimal Unicode, termasuk digit Arab-India, Thailand, Devanagari, Bengali, dan digit lebar penuh, menjadi digit ASCII. Filter ini beroperasi pada token yang dihasilkan oleh tokenizer. Konfigurasi di atas sengaja dibuat sebagai contoh penganalisis khusus dan tidak mencakup pipa pemrosesan bahasa Arab secara lengkap.</p>
