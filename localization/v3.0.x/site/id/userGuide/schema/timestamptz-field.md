...
<h3 id="Accelerate-queries" class="common-anchor-header">Mempercepat kueri<button data-href="#Accelerate-queries" class="anchor-icon" translate="no">
    ...
    </button></h3><p>Secara default, kueri pada kolom ` <code translate="no">TIMESTAMPTZ</code> ` yang tidak memiliki indeks akan melakukan pemindaian penuh terhadap semua baris, yang dapat memakan waktu lama pada dataset besar. Untuk mempercepat kueri berdasarkan cap waktu, buat indeks ` <code translate="no">STL_SORT</code> ` pada kolom ` <code translate="no">TIMESTAMPTZ</code> ` Anda.</p>
