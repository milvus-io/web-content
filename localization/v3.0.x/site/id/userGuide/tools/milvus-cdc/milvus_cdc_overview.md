...
<h2 id="Bulk-Import-in-CDC-Replication" class="common-anchor-header">Impor Massal dalam Replikasi CDC<button data-href="#Bulk-Import-in-CDC-Replication" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Dalam topologi replikasi CDC, impor massal harus menggunakan mode two-phase commit (2PC) dengan opsi ` <code translate="no">auto_commit=false</code>`. Jalankan proses impor dan commit hanya pada kluster primer, serta pastikan berkas impor tersedia bagi kluster primer dan kluster cadangan. Untuk detailnya, lihat bagian " <a href="/docs/id/bulk-import-in-cdc-replication.md">Impor Massal dalam Replikasi CDC</a>".</p>
...
<li>Failover mungkin menyebabkan hilangnya data yang telah ditulis ke primer lama tetapi belum direplikasi ke kluster siaga.</li>
...
<h2 id="FAQ" class="common-anchor-header">Pertanyaan Umum<button data-href="#FAQ" class="anchor-icon" translate="no">
    ...
    </button></h3><p>Ya. Kluster siaga dapat melayani lalu lintas baca. Kluster tersebut tidak dapat menerima penulisan hingga menjadi kluster primer.</p>
...
<h3 id="Does-switchover-lose-data" class="common-anchor-header">Apakah proses peralihan (switchover) menyebabkan kehilangan data?<button data-href="#Does-switchover-lose-data" class="anchor-icon" translate="no">
    ...
    </button></h3><p>Tidak. Proses peralihan akan menunggu hingga data yang tersisa direplikasi sebelum kluster siaga menjadi kluster utama.</p>
