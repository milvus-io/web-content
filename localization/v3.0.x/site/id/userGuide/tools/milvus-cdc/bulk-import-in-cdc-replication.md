...
summary: >-
  Pelajari cara melakukan impor massal ke klaster Milvus yang menggunakan
  replikasi CDC.
...
<h1 id="Bulk-Import-in-CDC-Replication" class="common-anchor-header">Impor Massal dalam Replikasi CDC<button data-href="#Bulk-Import-in-CDC-Replication" class="anchor-icon" translate="no">
    ...
    </button></h1><p>Panduan ini menjelaskan cara menjalankan impor massal pada kluster Milvus yang merupakan bagian dari topologi replikasi CDC. Dalam kluster replikasi, impor massal harus menggunakan dua-fase komit (2PC) agar impor tersebut dikonfirmasi sebagai satu titik yang terurut di seluruh kluster primer dan kluster cadangan.</p>
...
<p>Sebagai gantinya, jalankan impor dalam mode komit dua fase dengan mengatur ` <code translate="no">auto_commit=false</code>`:</p>
<ol>
<li><p><strong>Fase impor</strong>: Milvus memuat data pada kluster primer dan mereplikasi impor ke kluster cadangan, tetapi data yang diimpor tetap tidak terlihat. Tugas impor berhenti pada status " <code translate="no">Uncommitted</code> " dan menunggu.</p></li>
<li><p><strong>Fase komit</strong>: Anda secara eksplisit mengkomit pekerjaan impor di kluster primer. Komit direplikasi ke kluster siaga sebagai satu pagar terurut, sehingga kedua kluster membuat data yang diimpor terlihat pada titik logis yang sama.</p></li>
</ol>
<h2 id="Step-1-Enable-import-in-a-replicating-cluster" class="common-anchor-header">Langkah 1: Aktifkan impor pada kluster replikasi<button data-href="#Step-1-Enable-import-in-a-replicating-cluster" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>Impor dalam kluster replikasi dinonaktifkan secara default. Aktifkan dengan mengatur ` <code translate="no">dataCoord.import.enableInReplicatingCluster</code> ` menjadi ` <code translate="no">true</code> ` pada kluster primer dan kluster siaga.</p>
<p>Jika Anda menerapkan Milvus dengan Milvus Operator, tambahkan pengaturan berikut ke <code translate="no">spec.config</code> pada setiap sumber daya <code translate="no">Milvus</code>:</p>
...
<pre><code translate="no" class="language-yaml"><span class="hljs-attr">dataCoord:</span>
  <span class="hljs-attr">import:</span>
    <span class="hljs-attr">enableInReplicatingCluster:</span> <span class="hljs-literal">true</span>
<button class="copy-code-btn"></button></code></pre>
<p>Pengaturan ini dapat diperbarui, sehingga dapat berlaku tanpa perlu me-restart secara penuh.</p>
...
<tr><td><code translate="no">auto_commit=true</code> telah dikirim</td><td><code translate="no">auto_commit=true import in replicating cluster is not supported</code></td></tr>
</tbody>
</table>
<h2 id="Step-2-Run-a-2PC-import" class="common-anchor-header">Langkah 2: Jalankan impor 2PC<button data-href="#Step-2-Run-a-2PC-import" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Jalankan semua panggilan impor ke kluster primer. Data yang diimpor dan keputusan komit akan direplikasi ke kluster cadangan secara otomatis, jadi jangan kirimkan atau lakukan komit impor di kluster cadangan sendiri.</p>
<p>Setiap klaster membaca berkas impor dari penyimpanan objeknya sendiri. Pastikan berkas yang akan diimpor ada di penyimpanan objek primer dan cadangan. Anda dapat mengunggah berkas ke kedua klaster, atau menggunakan penyimpanan objek yang dapat diakses oleh kedua klaster. Jika berkas tidak ditemukan di klaster cadangan, proses impor yang direplikasi akan gagal di sana dengan kesalahan "objek tidak ditemukan".</p>
...
<h3 id="Why-wait-for-Uncommitted-on-both-clusters" class="common-anchor-header">Mengapa harus menunggu hingga <code translate="no">Uncommitted</code> di kedua kluster<button data-href="#Why-wait-for-Uncommitted-on-both-clusters" class="anchor-icon" translate="no">
    ...
    </button></h3><p>Melakukan komit sebelum kluster siaga selesai mengimpor tidak akan merusak data, tetapi kluster siaga masih dalam proses mengejar ketertinggalan saat komit diterapkan. Menunggu hingga kluster utama dan kluster siaga sama-sama melaporkan " <code translate="no">Uncommitted</code> " memastikan bahwa data yang diimpor telah direplikasi sepenuhnya dan kedua kluster siap menampilkannya secara bersamaan.</p>
<h2 id="Step-3-Verify-the-data" class="common-anchor-header">Langkah 3: Verifikasi data<button data-href="#Step-3-Verify-the-data" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Setelah pekerjaan mencapai status " <code translate="no">Completed</code>", entitas yang diimpor akan terlihat di kedua kluster. Muat dan jalankan kueri pada koleksi di kluster primer, lalu jalankan kueri yang sama di kluster standby tanpa memuat koleksi tersebut secara manual di sana, dan pastikan entitas yang diimpor terdapat di kedua kluster.</p>
<p>Cluster siaga bersifat hanya baca selama masih berstatus siaga. Jangan mengirimkan impor, komit, atau operasi DDL atau DCL lainnya secara langsung ke cluster siaga. Lakukan operasi ini di cluster utama dan biarkan replikasi CDC menerapkannya ke cluster siaga.</p>
<h2 id="FAQ" class="common-anchor-header">FAQ<button data-href="#FAQ" class="anchor-icon" translate="no">
    ...
    </button></h3><p>Jalankan impor dan commit di klaster utama. Klaster siaga menerima baik data yang diimpor maupun commit melalui replikasi CDC.</p>
...
<h3 id="Why-does-my-import-fail-with-import-in-replicating-cluster-is-not-supported-yet" class="common-anchor-header">Mengapa proses impor saya gagal dengan pesan " <code translate="no">import in replicating cluster is not supported yet</code>"?<button data-href="#Why-does-my-import-fail-with-import-in-replicating-cluster-is-not-supported-yet" class="anchor-icon" translate="no">
    ...
    </button></h3><p><code translate="no">dataCoord.import.enableInReplicatingCluster</code> tidak diaktifkan di cluster tersebut. Atur ke " <code translate="no">true</code> " pada cluster primer dan cluster standby.</p>
<h3 id="Why-does-my-import-fail-with-autocommittrue-import-in-replicating-cluster-is-not-supported" class="common-anchor-header">Mengapa impor saya gagal dengan pesan " <code translate="no">auto_commit=true import in replicating cluster is not supported</code>"?<button data-href="#Why-does-my-import-fail-with-autocommittrue-import-in-replicating-cluster-is-not-supported" class="anchor-icon" translate="no">
    ...
    </button></h3><p>Dalam kluster yang melakukan replikasi, hanya impor 2PC dengan ` <code translate="no">auto_commit=false</code> ` yang diterima. Tetapkan ` <code translate="no">options={&quot;auto_commit&quot;: &quot;false&quot;}</code> ` pada permintaan impor.</p>
