...
title: Migrasi Antar Instance Lintas Bucket
---
<h1 id="Migrate-Between-Instances-Across-Buckets" class="common-anchor-header">Migrasi Antar Instance Lintas Bucket<button data-href="#Migrate-Between-Instances-Across-Buckets" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h1><p>Topik ini merinci proses mencadangkan koleksi dari satu instans Milvus dan mengembalikannya ke instans lainnya, dengan masing-masing instans menggunakan bucket yang berbeda di dalam penyimpanan objek yang sama.</p>
<h2 id="Overview" class="common-anchor-header">Gambaran Umum<button data-href="#Overview" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Diagram di bawah ini mengilustrasikan proses pencadangan dan pemulihan menggunakan bucket yang berbeda dalam penyimpanan objek yang sama.</p>
<p>
   ...
   </span> <span class="img-wrapper"> <span>pencadangan-dan-pemulihan-silang-bucket.png</span> </span></p>
<p>Asumsikan kita memiliki dua instance Milvus, <code translate="no">milvus_A</code> dan <code translate="no">milvus_B</code>, keduanya menggunakan mesin penyimpanan MinIO default untuk penyimpanan objek. Kedua contoh ini menggunakan bucket yang berbeda, yaitu bucket_A dan <code translate="no">bucket_B</code> dalam penyimpanan objek yang sama. Dalam contoh ini, tujuan kita adalah menyelesaikan tugas-tugas berikut:</p>
<ol>
<li><p>Buat cadangan (<code translate="no">my_backup</code>) untuk koleksi <code translate="no">coll</code> di <code translate="no">bucket_A</code> dan simpan cadangan di <code translate="no">bucket_B</code>.</p></li>
<li><p>Di <code translate="no">bucket_B</code>, pulihkan dari cadangan dan beri nama koleksi yang dipulihkan <code translate="no">coll_bak</code>.</p></li>
</ol>
...
<li><p>Pastikan alat <strong>milvus-backup</strong> telah diinstal.</p></li>
...
<h2 id="Back-up-a-collection-from-milvusA" class="common-anchor-header">Mencadangkan koleksi dari milvus_A<button data-href="#Back-up-a-collection-from-milvusA" class="anchor-icon" translate="no">
    ...
    </button></h2><h3 id="Step-1-Prepare-configuration" class="common-anchor-header">Langkah 1: Siapkan konfigurasi<button data-href="#Step-1-Prepare-configuration" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Masuk ke direktori proyek milvus-backup dan buat direktori bernama configs:</p>
...
└── README.md
<button class="copy-code-btn"></button></code></pre>
<h3 id="Step-2-Edit-configuration-file" class="common-anchor-header">Langkah 2: Edit file konfigurasi<button data-href="#Step-2-Edit-configuration-file" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Ubah file backup.yaml untuk mengatur konfigurasi yang sesuai untuk<code translate="no">milvus_A</code>:</p>
<ul>
...
<li><p><code translate="no">minio.bucketName</code>: Nama bucket yang digunakan untuk penyimpanan data di <code translate="no">milvus_A</code>. Dalam contoh ini, diatur ke <code translate="no">bucket_A</code>.</p></li>
<li><p><code translate="no">minio.rootPath</code>: Jalur root di dalam bucket tempat data dari <code translate="no">milvus_A</code> disimpan. Dalam contoh ini, atur ke <code translate="no">files</code>.</p></li>
<li><p><code translate="no">minio.backupBucketName</code>: Nama bucket yang digunakan untuk penyimpanan cadangan di<code translate="no">milvus_B</code>. Dalam contoh ini, <code translate="no">milvus_A</code> dan <code translate="no">milvus_B</code> menggunakan bucket yang berbeda. Oleh karena itu, atur ke <code translate="no">bucket_B</code>.</p></li>
...
<h3 id="Step-3-Create-backup" class="common-anchor-header">Langkah 3: Buat cadangan<button data-href="#Step-3-Create-backup" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Setelah backup.yaml disimpan, buat cadangan bernama <code translate="no">my_backup</code>:</p>
<pre><code translate="no" class="language-shell">./milvus-backup create -c coll -n my_backup
<button class="copy-code-btn"></button></code></pre>
<p>Perintah ini membuat cadangan <code translate="no">bucket_B/backup/my_backup</code> di penyimpanan objek untuk koleksi coll.</p>
<h2 id="Restore-the-backup-to-milvusB" class="common-anchor-header">Kembalikan cadangan ke milvus_B<button data-href="#Restore-the-backup-to-milvusB" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><h3 id="Step-1-Configure-restoration-settings" class="common-anchor-header">Langkah 1: Konfigurasikan pengaturan pemulihan<button data-href="#Step-1-Configure-restoration-settings" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Ulangi langkah 2 untuk memodifikasi konfigurasi untuk pemulihan ke <code translate="no">milvus_B</code>, memastikan <code translate="no">minio.bucketName</code> diatur ke <code translate="no">bucket_B</code>.</p>
<p>Berikut ini contoh konfigurasi:</p>
...
<button class="copy-code-btn"></button></code></pre>
<h3 id="Step-2-Restore-backup" class="common-anchor-header">Langkah 2: Pulihkan cadangan<button data-href="#Step-2-Restore-backup" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Pulihkan cadangan ke <code translate="no">milvus_B</code>:</p>
<pre><code translate="no" class="language-shell">./milvus-backup restore -c coll -n my_backup -s _bak
<button class="copy-code-btn"></button></code></pre>
<p>Perintah ini mengembalikan cadangan ke dalam koleksi baru bernama <code translate="no">coll_bak</code> di <code translate="no">milvus_B</code>, dengan data yang disimpan di <code translate="no">bucket_B/files/insert_log/[ID of new collection]</code>.</p>
