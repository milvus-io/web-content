...
summary: >-
  Operasi upsert menawarkan cara yang praktis untuk menyisipkan atau memperbarui
  entitas dalam sebuah koleksi.
---
...
<p>Jika koleksi target telah mengaktifkan " <code translate="no">autoid</code> " pada bidang primernya, Milvus akan menghasilkan kunci utama baru untuk data yang terdapat dalam payload permintaan sebelum menyisipkannya.</p>
...
<h3 id="Upsert-in-merge-mode--Milvus-v262+" class="common-anchor-header">Upsert dalam mode penggabungan<span class="beta-tag" style="background-color:rgb(0, 179, 255);color:white" translate="no">Compatible with Milvus v2.6.2+</span><button data-href="#Upsert-in-merge-mode--Milvus-v262+" class="anchor-icon" translate="no">
    ...
    </button></h3><p>Gunakan mode penggabungan untuk memperbarui kolom tertentu dari entitas yang sudah ada sambil mempertahankan kolom lainnya tetap tidak berubah.</p>
<p><span class="img-wrapper">
...
<p>Tentukan " <code translate="no">partial_update=True</code> " dan berikan kunci utama serta bidang-bidang yang ingin Anda perbarui.</p>
<p>Milvus mengambil entitas yang sudah ada dengan kueri konsistensi kuat, menggabungkan perubahan Anda dengan data yang tersimpan, menyisipkan entitas yang telah digabungkan, dan menghapus entitas lama.</p>
<p>Jika kunci utama tidak ada, hasilnya bergantung pada apakah mode " <code translate="no">autoID</code> " diaktifkan:</p>
<ul>
<li><strong>Jika " <code translate="no">autoID</code> " dinonaktifkan</strong>, Milvus akan mencoba menyisipkan entitas baru dengan kunci utama yang Anda berikan. Permintaan akan berhasil jika memenuhi persyaratan penyisipan normal. Jika bidang yang wajib ada tidak terisi, permintaan akan gagal dengan kesalahan "missing-field". Bidang yang dapat bernilai null dan bidang dengan nilai default dapat diabaikan, sama seperti pada penyisipan normal.</li>
<li><strong>Jika " <code translate="no">autoID</code> " diaktifkan</strong>, setiap kunci utama dalam permintaan harus sudah ada. Milvus akan menolak permintaan jika ada kunci utama yang hilang, meskipun Anda telah menyediakan semua bidang yang diperlukan untuk penyisipan. Untuk entitas yang sudah ada, mode penggabungan (merge mode) akan mempertahankan kunci utama tetap sama.</li>
</ul>
<p>Jika pembaruan parsial gagal dengan kesalahan "missing-field", periksa apakah entitas tujuan sudah ada. Tanpa entitas yang sudah ada, Milvus tidak dapat mengambil nilai bidang yang Anda abaikan.</p>
<p>Untuk entitas baru, gunakan ` <code translate="no">insert</code> ` atau `upsert` dalam mode override. Gunakan mode penggabungan untuk pembaruan selanjutnya pada bidang individu.</p>
...
<ul>
<li><p>Jika Anda mengabaikan bidang ` <code translate="no">issue</code> ` dalam permintaan ` <code translate="no">upsert</code> ` dan menonaktifkan ` <code translate="no">partial_update</code>`, bidang ` <code translate="no">issue</code> ` akan diperbarui menjadi ` <code translate="no">null</code> ` alih-alih mempertahankan nilai aslinya.</p></li>
...
<ul>
<li><p>Jika Anda melakukan upsert dengan opsi ` <code translate="no">partial_update</code> ` dinonaktifkan, perilaku defaultnya adalah <strong>mengganti nilai</strong>. Artinya, nilai bidang dinamis akan diganti oleh semua bidang yang tidak didefinisikan dalam skema yang termasuk dalam permintaan beserta nilainya.</p>
...
<p>Misalkan koleksi contoh memiliki bidang JSON yang didefinisikan oleh skema bernama <code translate="no">extras</code>, dan pasangan kunci-nilai dalam bidang JSON entitas ini mirip dengan <code translate="no">{&quot;author&quot;: &quot;John&quot;, &quot;year&quot;: 2020, &quot;tags&quot;: [&quot;fiction&quot;]}</code>.</p>
...
<p>Secara default, bidang <code translate="no">ARRAY</code> dalam mode merge mengikuti semantik <strong>REPLACE</strong>: nilai yang dibawa dalam permintaan akan menimpa array yang ada. Untuk pembaruan yang lebih terperinci, Milvus v2.6.17 dan yang lebih baru juga mendukung dua operator:</p>
<ul>
...
<p>Melakukan upsert pada bidang StructArray dalam suatu entitas akan menimpa nilai bidang tersebut. Untuk melakukannya, Anda perlu menyediakan daftar kamus, yang masing-masing berisi semua subbidang yang didefinisikan dalam skema struktur, bahkan saat Anda melakukan upsert dalam mode penggabungan.</p>
...
<ul>
<li><p>Permintaan " <code translate="no">upsert</code> " harus selalu menyertakan kunci utama entitas tujuan.</p></li>
...
<li><p>Nilai semua kolom yang ditentukan dalam permintaan harus sesuai dengan tipe data yang didefinisikan dalam skema.</p></li>
<li><p>Untuk bidang apa pun yang diturunkan dari bidang lain menggunakan fungsi, Milvus akan menghapus bidang turunan tersebut selama proses upsert untuk memungkinkan perhitungan ulang.</p></li>
</ul>
...
<h2 id="Upsert-ARRAY-fields-in-merge-mode--Milvus-2617+" class="common-anchor-header">Upsert bidang ARRAY dalam mode penggabungan<span class="beta-tag" style="background-color:rgb(0, 179, 255);color:white" translate="no">Compatible with Milvus 2.6.17+</span><button data-href="#Upsert-ARRAY-fields-in-merge-mode--Milvus-2617+" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Sebelum Milvus v2.6.17, memperbarui sebagian dari bidang ` <code translate="no">ARRAY</code> ` memerlukan alur baca-modifikasi-tulis di sisi klien: mengambil array yang ada, mengubahnya dalam kode aplikasi, dan melakukan `upsert` terhadap nilai pengganti secara keseluruhan. Operator pembaruan parsial (<code translate="no">ARRAY_APPEND</code> dan <code translate="no">ARRAY_REMOVE</code>) memungkinkan Anda hanya mengirim elemen yang akan ditambahkan atau dihapus, yang mengurangi logika sisi klien dan menghindari pembacaan tambahan sebelum upsert.</p>
<p>Misalkan entitas dengan kunci utama ` <code translate="no">1</code> ` sudah memiliki ` <code translate="no">tags = [&quot;new&quot;, &quot;trial&quot;]</code>`. Sebelum adanya operator pembaruan parsial, menambahkan elemen ` <code translate="no">&quot;premium&quot;</code> ` ke dalam array memerlukan proses `upsert` untuk array pengganti secara keseluruhan:</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#javascript">   NodeJS</a>
...
<button class="copy-code-btn"></button></code></pre>
<div class="alert note">
<p>Menambahkan salah satu operator ke bidang melalui <code translate="no">field_ops</code> secara implisit mengaktifkan semantik pembaruan parsial. Oleh karena itu, Anda <strong>tidak</strong> perlu mengirimkan <code translate="no">partial_update=True</code> bersama dengan <code translate="no">field_ops</code>.</p>
</div>
...
<li><p>Nilai payload harus sesuai dengan tipe data ( <code translate="no">element_type</code> ) dari bidang target yang menggunakan tipe data string ( <code translate="no">ARRAY</code> ). Misalnya, jika bidang target adalah <code translate="no">ARRAY&lt;VARCHAR&gt;</code>, payload harus berisi nilai string.</p></li>
...
<h2 id="Upsert-StructArray-field-in-merge-mode" class="common-anchor-header">Upsert bidang StructArray dalam mode penggabungan<button data-href="#Upsert-StructArray-field-in-merge-mode" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Melakukan upsert pada bidang StructArray dalam suatu entitas akan menimpa nilai bidang tersebut. Artinya, Anda perlu menyertakan semua subbidang yang didefinisikan dalam skema struktur saat melakukan upsert pada bidang StructArray.</p>
