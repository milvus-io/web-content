...
summary: >-
  Operasi upsert menyediakan cara yang praktis untuk menyisipkan atau
  memperbarui entitas dalam sebuah koleksi.
---
<h1 id="Upsert-Entities" class="common-anchor-header">Entitas Upsert<button data-href="#Upsert-Entities" class="anchor-icon" translate="no">
    ...
    </button></h1><p>Operasi ` <code translate="no">upsert</code> ` menyediakan cara yang praktis untuk menyisipkan atau memperbarui entitas dalam sebuah koleksi.</p>
<h2 id="Overview" class="common-anchor-header">Gambaran Umum<button data-href="#Overview" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Anda dapat menggunakan ` <code translate="no">upsert</code> ` untuk menyisipkan entitas baru atau memperbarui entitas yang sudah ada, tergantung pada apakah kunci utama yang disediakan dalam permintaan `upsert` terdapat dalam koleksi. Jika kunci utama tidak ditemukan, operasi penyisipan akan dilakukan. Sebaliknya, operasi pembaruan akan dilakukan. Pembaruan parsial pada koleksi ` <code translate="no">autoID</code> ` merupakan pengecualian: pembaruan hanya dilakukan pada entitas yang sudah ada, seperti dijelaskan di bawah ini.</p>
<p>Upsert di Milvus bekerja dalam mode penggantian ( <strong>override</strong> ) atau <strong>penggabungan (merge)</strong>.</p>
<h3 id="Upsert-in-override-mode" class="common-anchor-header">Upsert dalam mode "override"<button data-href="#Upsert-in-override-mode" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<p>Jika koleksi target telah mengaktifkan " <code translate="no">autoID</code> " pada bidang primernya, permintaan <code translate="no">upsert</code> tetap harus menyertakan kunci primer entitas target. Milvus menggunakan kunci primer yang disediakan untuk menemukan entitas yang akan diganti, dan menghasilkan kunci primer baru untuk data yang terdapat dalam muatan permintaan sebelum menyisipkannya.</p>
...
<h3 id="Upsert-in-merge-mode" class="common-anchor-header">Upsert dalam mode penggabungan<span class="beta-tag" style="background-color:rgb(0, 179, 255);color:white" translate="no">Compatible with Milvus v2.6.2+</span><button data-href="#Upsert-in-merge-mode" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Gunakan mode penggabungan (merge) untuk memperbarui bidang tertentu dari entitas yang sudah ada sambil mempertahankan bidang lainnya tetap tidak berubah.</p>
<p><span class="img-wrapper">
   ...
   <span>Upsert dalam Mode Merge</span>
  
 </span></p>
<p>Atur " <code translate="no">partial_update=True</code> " dan berikan kunci utama serta bidang-bidang yang ingin Anda perbarui.</p>
<p>Milvus mengambil entitas yang ada dengan kueri konsistensi kuat, menggabungkan perubahan Anda dengan data yang tersimpan, menyisipkan entitas yang telah digabungkan, dan menghapus entitas lama.</p>
<p>Jika kunci utama tidak ada, hasilnya bergantung pada apakah " <code translate="no">autoID</code> " diaktifkan:</p>
<ul>
<li><strong>Jika " <code translate="no">autoID</code> " dinonaktifkan</strong>, Milvus akan mencoba menyisipkan entitas baru dengan kunci utama yang Anda berikan. Permintaan akan berhasil jika memenuhi persyaratan penyisipan normal. Jika bidang yang diwajibkan tidak ada, permintaan akan gagal dengan kesalahan "missing-field". Bidang yang dapat bernilai null dan bidang dengan nilai default dapat diabaikan, sama seperti pada penyisipan normal.</li>
<li><strong>Jika mode penggabungan ( <code translate="no">autoID</code> ) diaktifkan</strong>, setiap kunci utama dalam permintaan harus sudah ada. Milvus akan menolak permintaan jika ada kunci utama yang hilang, bahkan jika Anda menyediakan semua bidang yang diperlukan untuk penyisipan. Untuk entitas yang sudah ada, mode penggabungan akan mempertahankan kunci utama tetap sama.</li>
</ul>
<p>Jika pembaruan sebagian gagal dengan kesalahan "missing-field", periksa apakah entitas target sudah ada. Tanpa entitas yang sudah ada, Milvus tidak dapat mengambil nilai bidang yang Anda abaikan.</p>
<p>Untuk entitas baru, gunakan ` <code translate="no">insert</code> ` atau `upsert` dalam mode override. Gunakan mode penggabungan untuk pembaruan selanjutnya pada kolom individual.</p>
<p>Untuk bidang ` <code translate="no">ARRAY</code> `, mode penggabungan mendukung dua operator di Milvus v2.6.17 dan versi selanjutnya: ` <code translate="no">ARRAY_APPEND</code> ` dan ` <code translate="no">ARRAY_REMOVE</code>`. Operator ini memungkinkan Anda menambahkan elemen ke atau menghapus elemen yang cocok dari bidang ` <code translate="no">ARRAY</code> ` yang sudah ada, tanpa perlu terlebih dahulu melakukan kueri terhadap entitas untuk mengambil nilainya saat ini. Untuk detailnya, lihat <a href="/docs/id/upsert-entities.md#Upsert-ARRAY-fields-in-merge-mode">"Upsert bidang ARRAY dalam mode penggabungan</a>".</p>
<h3 id="Upsert-behaviors-special-notes" class="common-anchor-header">Perilaku Upsert: catatan khusus<button data-href="#Upsert-behaviors-special-notes" class="anchor-icon" translate="no">
    ...
    </button></h3><p>Ada beberapa catatan khusus yang perlu Anda pertimbangkan sebelum menggunakan fitur penggabungan. Kasus-kasus berikut mengasumsikan bahwa Anda memiliki koleksi dengan dua bidang skalar bernama <code translate="no">title</code> dan <code translate="no">issue</code>, bersama dengan kunci utama <code translate="no">id</code> dan bidang vektor bernama <code translate="no">vector</code>.</p>
<ul>
<li><p><strong>Mel</strong> akukan `<strong>upsert` pada bidang dengan</strong> opsi ` <code translate="no">nullable</code> ` <strong>diaktifkan.</strong></p>
...
<ul>
<li><p>Jika Anda mengabaikan bidang ` <code translate="no">issue</code> ` dalam permintaan ` <code translate="no">upsert</code> ` dan menonaktifkan ` <code translate="no">partial_update</code>`, bidang ` <code translate="no">issue</code> ` akan diperbarui menjadi ` <code translate="no">null</code> ` alih-alih mempertahankan nilainya yang asli.</p></li>
<li><p>Untuk mempertahankan nilai asli bidang <code translate="no">issue</code>, Anda perlu mengaktifkan <code translate="no">partial_update</code> dan mengabaikan bidang <code translate="no">issue</code>, atau menyertakan bidang <code translate="no">issue</code> dengan nilai aslinya dalam permintaan <code translate="no">upsert</code>.</p></li>
</ul></li>
<li><p><strong>Kunci upsert di bidang dinamis</strong>.</p>
<p>Misalkan Anda telah mengaktifkan kunci dinamis dalam koleksi contoh, dan pasangan kunci-nilai di bidang dinamis suatu entitas mirip dengan <code translate="no">{&quot;author&quot;: &quot;John&quot;, &quot;year&quot;: 2020, &quot;tags&quot;: [&quot;fiction&quot;]}</code>.</p>
...
<ul>
<li><p>Jika Anda melakukan upsert dengan opsi ` <code translate="no">partial_update</code> ` dinonaktifkan, perilaku defaultnya adalah <strong>mengganti (override)</strong>. Artinya, nilai bidang dinamis akan diganti oleh semua bidang yang tidak didefinisikan dalam skema yang termasuk dalam permintaan beserta nilainya.</p>
...
<p>Misalnya, jika data yang disertakan dalam permintaan adalah <code translate="no">{&quot;author&quot;: &quot;John&quot;, &quot;year&quot;: 2020, &quot;tags&quot;: [&quot;fiction&quot;]}</code>, pasangan kunci-nilai di bidang dinamis entitas tujuan akan menjadi <code translate="no">{&quot;author&quot;: &quot;John&quot;, &quot;year&quot;: 2020, &quot;tags&quot;: [&quot;fiction&quot;], &quot;genre&quot;: &quot;fantasy&quot;}</code> setelah upsert.</p></li>
</ul></li>
<li><p><strong>Melakukan upsert pada bidang JSON.</strong></p>
<p>Misalkan koleksi contoh memiliki bidang JSON yang ditentukan oleh skema bernama ` <code translate="no">extras</code>`, dan pasangan kunci-nilai dalam bidang JSON entitas ini mirip dengan ` <code translate="no">{&quot;author&quot;: &quot;John&quot;, &quot;year&quot;: 2020, &quot;tags&quot;: [&quot;fiction&quot;]}</code>`.</p>
<p>Saat Anda melakukan upsert pada bidang <code translate="no">extras</code> suatu entitas dengan data JSON yang dimodifikasi, perhatikan bahwa bidang JSON diperlakukan sebagai satu kesatuan, dan Anda tidak dapat memperbarui kunci individu secara selektif. Dengan kata lain, bidang JSON <strong>TIDAK</strong> mendukung upsert dalam mode <strong>penggabungan</strong>.</p></li>
<li><p>Melakukan<strong>upsert pada</strong> <strong>bidang</strong> ` <code translate="no">ARRAY</code> ` <strong>.</strong></p>
<p>Secara default, bidang ` <code translate="no">ARRAY</code> ` dalam mode penggabungan mengikuti semantik <strong>REPLACE</strong>: nilai yang dibawa dalam permintaan akan menimpa array yang ada. Untuk pembaruan yang lebih terperinci, Milvus v2.6.17 dan versi selanjutnya juga mendukung dua operator:</p>
<ul>
<li><p><code translate="no">ARRAY_APPEND</code> menambahkan elemen-elemen dalam muatan permintaan ke array yang ada.</p></li>
...
<ul>
<li><p>Permintaan " <code translate="no">upsert</code> " harus selalu menyertakan kunci utama entitas tujuan, bahkan saat " <code translate="no">autoID</code> " diaktifkan. Untuk koleksi " <code translate="no">autoID</code> ", penanganan kunci utama bergantung pada mode "upsert":</p>
<ul>
<li><p>Dalam mode override, kunci utama mengidentifikasi entitas yang sudah ada untuk diganti, dan Milvus menghasilkan kunci utama baru untuk entitas pengganti.</p></li>
<li><p>Dalam mode penggabungan (merge mode), kunci utama mengidentifikasi entitas yang sudah ada untuk diperbarui dan tetap tidak berubah. Jika kunci utama tidak ada, permintaan akan gagal alih-alih menyisipkan entitas baru.</p></li>
</ul></li>
<li><p>Koleksi tujuan harus sudah dimuat dan tersedia untuk kueri.</p></li>
...
<h2 id="Upsert-entities-in-a-collection" class="common-anchor-header">Upsert entitas dalam koleksi<button data-href="#Upsert-entities-in-a-collection" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Pada bagian ini, kita akan melakukan upsert entitas ke dalam koleksi bernama <code translate="no">my_collection</code>. Koleksi ini hanya memiliki dua kolom, yaitu <code translate="no">id</code>, <code translate="no">vector</code>, <code translate="no">title</code>, dan <code translate="no">issue</code>. Kolom <code translate="no">id</code> adalah kolom utama, sedangkan kolom <code translate="no">title</code> dan <code translate="no">issue</code> adalah kolom skalar.</p>
...
<span class="hljs-comment"># }</span>
<button class="copy-code-btn"></button></code></pre>
<h2 id="Upsert-entities-in-a-partition" class="common-anchor-header">Upsert entitas dalam partisi<button data-href="#Upsert-entities-in-a-partition" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>Anda juga dapat melakukan upsert entitas ke dalam partisi tertentu. Potongan kode berikut mengasumsikan bahwa Anda memiliki partisi bernama <strong>PartitionA</strong> dalam koleksi Anda.</p>
...
<h2 id="Upsert-entities-in-merge-mode" class="common-anchor-header">Menambahkan atau memperbarui entitas dalam mode penggabungan<span class="beta-tag" style="background-color:rgb(0, 179, 255);color:white" translate="no">Compatible with Milvus v2.6.2+</span><button data-href="#Upsert-entities-in-merge-mode" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Contoh berikut hanya memperbarui bidang ` <code translate="no">issue</code> ` dari entitas dengan kunci utama ` <code translate="no">1</code> ` dan ` <code translate="no">2</code> ` di ` <code translate="no">my_collection</code>`. Sebelum menjalankannya, pastikan kedua entitas tersebut sudah ada. Bidang lainnya tetap mempertahankan nilai saat ini.</p>
<div class="alert note">
<p>Saat melakukan upsert dalam mode merge, pastikan entitas yang terlibat dalam permintaan memiliki kumpulan bidang yang sama. Misalkan ada dua atau lebih entitas yang akan di-upsert, seperti yang ditunjukkan dalam potongan kode berikut, penting bagi entitas-entitas tersebut untuk menyertakan bidang yang identik guna mencegah kesalahan dan menjaga integritas data.</p>
</div>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
...
<button class="copy-code-btn"></button></code></pre>
<p>Dengan <code translate="no">ARRAY_APPEND</code>, cukup kirim elemen yang akan ditambahkan:</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#javascript">   NodeJS</a>
...
<li><p>Nilai muatan (payload) harus sesuai dengan tipe data ( <code translate="no">element_type</code> ) dari bidang <code translate="no">ARRAY</code> yang dituju. Misalnya, jika bidang yang dituju adalah <code translate="no">ARRAY&lt;VARCHAR&gt;</code>, muatan harus berisi nilai string.</p></li>
...
<h3 id="Example" class="common-anchor-header">Contoh<button data-href="#Example" class="anchor-icon" translate="no">
    ...
    </button></h3><p>Contoh berikut menggunakan koleksi kecil ` <code translate="no">users</code> ` dengan kunci utama ` <code translate="no">pk</code>`, bidang ` <code translate="no">tags</code> ` bertipe ` <code translate="no">ARRAY&lt;VARCHAR&gt;</code>`, dan bidang vektor ` <code translate="no">embedding</code> `. Contoh ini pertama-tama menyisipkan dua entitas dengan nilai awal ` <code translate="no">tags</code> `, lalu menggunakan ` <code translate="no">ARRAY_APPEND</code> ` dan ` <code translate="no">ARRAY_REMOVE</code> ` untuk menunjukkan bagaimana setiap operator mengubah array yang tersimpan.</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#javascript">   NodeJS</a>
...
<p>Contoh berikut menunjukkan cara melakukan `upsert` pada bidang ` <code translate="no">chunks</code> ` dalam mode penggabungan, yaitu bidang StructArray dengan 6 subbidang. Setelah operasi selesai, bidang ` <code translate="no">chunks</code> ` dari entitas dengan id 1 akan disetel ke array yang berisi struktur berelemen dua yang disertakan dalam permintaan.</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#javascript">   NodeJS</a>
