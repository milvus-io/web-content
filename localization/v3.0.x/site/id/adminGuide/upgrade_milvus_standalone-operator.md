...
summary: Pelajari cara melakukan upgrade Milvus standalone menggunakan Milvus Operator.
...
<h1 id="Upgrade-Milvus-Standalone-with-Milvus-Operator" class="common-anchor-header">Memperbarui Milvus Standalone dengan Milvus Operator<button data-href="#Upgrade-Milvus-Standalone-with-Milvus-Operator" class="anchor-icon" translate="no">
    ...
    </button></h1><p>Panduan ini menjelaskan cara meng-upgrade deployment Milvus 2.6.x standalone ke v3.0.2 menggunakan Milvus Operator.</p>
<div class="alert note">
<p>Prosedur ini telah diverifikasi dari Milvus 2.6.20 hingga Milvus v3.0.2 dengan Milvus Operator 1.3.0, Woodpecker, etcd dalam kluster, dan MinIO dalam kluster. Jika Anda menggunakan rilis patch Milvus 2.6.x lain, versi Operator, antrian pesan, atau konfigurasi dependensi yang berbeda, uji coba pembaruan ini terlebih dahulu di lingkungan non-produksi.</p>
</div>
...
<p><strong>Batasan antrian pesan</strong>: Saat melakukan upgrade ke Milvus v3.0.2, Anda harus mempertahankan pilihan antrian pesan saat ini. Beralih antara sistem antrian pesan yang berbeda selama proses upgrade tidak didukung. Dukungan untuk mengganti sistem antrian pesan akan tersedia pada versi mendatang.</p>
<div class="alert warning">
<p>Prosedur ini tidak memvalidasi penurunan versi atau rollback dengan mengubah gambar Milvus kembali ke 2.6.x. Setelah v3.0.2 menulis data, rollback yang hanya melibatkan gambar dapat gagal membaca status yang telah diperbarui. Jika peningkatan gagal, hentikan penulisan dan gunakan rencana pemulihan yang mengembalikan cadangan metadata dan data persisten sebelum peningkatan. Validasi rencana pemulihan di lingkungan non-produksi terlebih dahulu.</p>
</div>
...
<p>Gunakan manifest sumber untuk penyebaran yang ada sebagai manifest peningkatan versi. Jangan menerapkan file cadangan yang diekspor secara langsung tanpa terlebih dahulu menghapus metadata yang dikelola server dan bidang status.</p>
...
<h3 id="Step-3-Update-the-Milvus-image" class="common-anchor-header">Langkah 3: Perbarui gambar Milvus<button data-href="#Step-3-Update-the-Milvus-image" class="anchor-icon" translate="no">
    ...
    </button></h3><p>Dalam manifest CR Milvus yang lengkap, ubah hanya <code translate="no">spec.components.image</code>. Pertahankan pengaturan mode, komponen, antrian pesan, etcd, penyimpanan, dan pengaturan ketergantungan lainnya yang sudah ada. Kutipan berikut menunjukkan bidang yang perlu diubah; jangan mengganti CR lengkap Anda dengan kutipan ini.</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>Pastikan bahwa CR Milvus melaporkan ` <code translate="no">Healthy</code>`, gambar saat ini adalah ` <code translate="no">milvusdb/milvus:v3.0.2</code>`, dan koleksi yang ada tetap dapat di-query dan dicari. Selesaikan pemeriksaan ini sebelum Anda mengaktifkan fitur apa pun yang spesifik untuk v3.0.2.</p>
