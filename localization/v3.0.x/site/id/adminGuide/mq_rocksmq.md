...
title: RocksMQ
---
<h1 id="Use-RocksMQ-as-the-Milvus-Message-Queue" class="common-anchor-header">Gunakan RocksMQ sebagai Antrian Pesan Milvus<button data-href="#Use-RocksMQ-as-the-Milvus-Message-Queue" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h1><p>RocksMQ adalah antrian pesan tertanam (WAL) yang disertakan dalam Milvus, tersedia <strong>hanya</strong> untuk <strong>Milvus Standalone</strong>. Antrian pesan ini merupakan antrian pesan bawaan pada versi Milvus sebelumnya; pada Milvus 3.x, Milvus Standalone menggunakan <a href="/docs/id/woodpecker.md">Woodpecker</a> tertanam sebagai antrian pesan bawaan.</p>
...
<li><strong>Hanya untuk Standalone</strong> — RocksMQ <strong>tidak</strong> didukung di Milvus Distributed (kluster). Lihat <a href="/docs/id/mqtype-overview.md#Supported-message-queues">matriks dukungan antrian pesan</a>.</li>
<li>RocksMQ sudah disertakan dalam Milvus, sehingga tidak perlu menginstal versi terpisah.</li>
<li>RocksMQ merupakan antrian pesan standalone default pada versi Milvus sebelumnya, dan digantikan oleh Woodpecker tertanam pada Milvus 3.x.</li>
</ul>
<h2 id="Deploy-Milvus-Standalone-with-RocksMQ-using-Docker" class="common-anchor-header">Deploy Milvus Standalone dengan RocksMQ menggunakan Docker<button data-href="#Deploy-Milvus-Standalone-with-RocksMQ-using-Docker" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><h3 id="Install" class="common-anchor-header">Instal<button data-href="#Install" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Ikuti <a href="/docs/id/install_standalone-docker.md">panduan "Run Milvus in Docker"</a>. Pada Milvus 3.x, antrian pesan default untuk standalone adalah Woodpecker, jadi ubah jenis antrian pesan menjadi RocksMQ secara eksplisit. Skrip bootstrap akan menulis file ` <code translate="no">user.yaml</code> ` baru pada <strong>saat pertama kali dijalankan</strong> (` <code translate="no">start</code>`), jadi atur jenisnya <strong>setelah</strong> startup pertama, lalu jalankan ` <code translate="no">restart</code> ` untuk menerapkan perubahan (perintah ` <code translate="no">restart</code> ` akan mempertahankan ` <code translate="no">user.yaml</code>`):</p>
...
Mengubah <code translate="no">mq.type</code> dengan cara ini ditujukan untuk instance yang <b>benar-benar baru</b> (belum memiliki koleksi). Untuk mengubah antrian pesan pada instance yang sudah menyimpan data, ikuti <a href="/docs/id/switch-rocksmq-woodpecker.md">prosedur peralihan</a> sebagai gantinya.
</div>
<h3 id="Configure" class="common-anchor-header">Konfigurasi<button data-href="#Configure" class="anchor-icon" translate="no">
    ...
    </button></h3><p>Untuk mengoptimalkan RocksMQ, tambahkan bagian <code translate="no">rocksmq</code> ke <code translate="no">user.yaml</code> dan mulai ulang layanan:</p>
...
<button class="copy-code-btn"></button></code></pre>
<h2 id="Notes" class="common-anchor-header">Catatan<button data-href="#Notes" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<li><strong>Peningkatan dari 2.5.x ke 2.6.x:</strong> <strong>Batasan Antrian Pesan</strong>: Saat melakukan peningkatan ke Milvus v3.0-beta, Anda harus mempertahankan pilihan antrian pesan saat ini. Pergantian antara sistem antrian pesan yang berbeda selama proses peningkatan tidak didukung. Dukungan untuk mengganti sistem antrian pesan akan tersedia pada versi mendatang.
