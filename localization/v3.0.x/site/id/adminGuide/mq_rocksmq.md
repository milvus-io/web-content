...
<h2 id="Deploy-Milvus-Standalone-with-RocksMQ-using-Docker" class="common-anchor-header">Deploy Milvus Standalone dengan RocksMQ menggunakan Docker<button data-href="#Deploy-Milvus-Standalone-with-RocksMQ-using-Docker" class="anchor-icon" translate="no">
    ...
    </button></h3><p>Ikuti <a href="/docs/id/install_standalone-docker.md">panduan "Run Milvus in Docker"</a>. Pada Milvus 3.x, antrian pesan default untuk mode standalone adalah Woodpecker, jadi ubah jenis antrian pesan menjadi RocksMQ secara eksplisit. Skrip bootstrap akan menulis file ` <code translate="no">user.yaml</code> ` baru pada <strong>saat pertama kali dijalankan</strong> ( <code translate="no">start</code>), jadi atur jenis antrian pesan <strong>setelah</strong> proses startup pertama, lalu jalankan ` <code translate="no">restart</code> ` untuk menerapkan perubahan (perintah ` <code translate="no">restart</code> ` akan mempertahankan ` <code translate="no">user.yaml</code>`):</p>
...
Mengubah <code translate="no">mq.type</code> dengan cara ini ditujukan untuk instance yang <b>benar-benar baru</b> (belum ada koleksi). Untuk mengubah antrian pesan pada instance yang sudah menyimpan data, ikuti prosedur peralihan sebagai gantinya.
</div>
