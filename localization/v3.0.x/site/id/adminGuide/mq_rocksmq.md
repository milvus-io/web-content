...
<li>RocksMQ merupakan antrian pesan standalone bawaan pada versi Milvus sebelumnya, dan telah digantikan oleh Woodpecker tertanam di Milvus 3.x.</li>
</ul>
<h2 id="Deploy-Milvus-Standalone-with-RocksMQ-using-Docker" class="common-anchor-header">Deploy Milvus Standalone dengan RocksMQ menggunakan Docker<button data-href="#Deploy-Milvus-Standalone-with-RocksMQ-using-Docker" class="anchor-icon" translate="no">
    ...
    </button></h3><p>Ikuti <a href="/docs/id/install_standalone-docker.md">panduan "Run Milvus in Docker"</a>. Pada Milvus 3.x, antrian pesan standalone default adalah Woodpecker, jadi ubah jenis antrian pesan ke RocksMQ secara eksplisit. Skrip bootstrap akan menulis file konfigurasi baru ` <code translate="no">user.yaml</code> ` saat <strong>pertama kali</strong> dijalankan ( <code translate="no">start</code>), jadi atur jenisnya <strong>setelah</strong> proses startup pertama tersebut, lalu jalankan ` <code translate="no">restart</code> ` untuk menerapkan perubahan (perintah ` <code translate="no">restart</code> ` akan mempertahankan file ` <code translate="no">user.yaml</code>`):</p>
...
<li><strong>Peningkatan dari 2.5.x ke 2.6.x:</strong> <strong>Batasan Antrian Pesan</strong>: Saat melakukan peningkatan ke Milvus v3.0.1, Anda harus mempertahankan pilihan antrian pesan saat ini. Pergantian antara sistem antrian pesan yang berbeda selama proses peningkatan tidak didukung. Dukungan untuk mengganti sistem antrian pesan akan tersedia pada versi mendatang.
