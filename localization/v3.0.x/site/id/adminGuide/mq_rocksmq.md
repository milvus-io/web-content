...
<h1 id="Use-RocksMQ-as-the-Milvus-Message-Queue" class="common-anchor-header">Gunakan RocksMQ sebagai Antrian Pesan Milvus<button data-href="#Use-RocksMQ-as-the-Milvus-Message-Queue" class="anchor-icon" translate="no">
    ...
    </button></h1><p>RocksMQ adalah antrian pesan tertanam (WAL) yang disertakan dalam Milvus, dan <strong>hanya</strong> tersedia untuk <strong>Milvus Standalone</strong>. Antrian pesan ini dulunya merupakan antrian pesan bawaan untuk Milvus Standalone pada versi-versi Milvus sebelumnya; pada Milvus 3.x, Milvus Standalone secara default menggunakan <a href="/docs/id/woodpecker.md">Woodpecker</a> yang tertanam.</p>
...
<li><strong>Hanya untuk Standalone</strong> — RocksMQ <strong>tidak</strong> didukung di Milvus Distributed (cluster). Lihat <a href="/docs/id/mqtype-overview.md#Supported-message-queues">matriks dukungan antrian pesan</a>.</li>
<li>RocksMQ sudah disertakan dalam Milvus, sehingga tidak perlu menginstal versi terpisah.</li>
<li>RocksMQ merupakan antrian pesan standalone default pada versi Milvus sebelumnya, dan telah digantikan oleh Woodpecker tertanam di Milvus 3.x.</li>
</ul>
<h2 id="Deploy-Milvus-Standalone-with-RocksMQ-using-Docker" class="common-anchor-header">Deploy Milvus Standalone dengan RocksMQ menggunakan Docker<button data-href="#Deploy-Milvus-Standalone-with-RocksMQ-using-Docker" class="anchor-icon" translate="no">
    ...
    </button></h3><p>Ikuti <a href="/docs/id/install_standalone-docker.md">panduan "Run Milvus in Docker"</a>. Pada Milvus 3.x, antrian pesan standalone default adalah Woodpecker, jadi ubah jenis antrian pesan menjadi RocksMQ secara eksplisit. Skrip bootstrap akan menulis berkas ` <code translate="no">user.yaml</code> ` baru pada <strong>saat pertama kali dijalankan</strong> ( <code translate="no">start</code>), jadi atur jenisnya <strong>setelah</strong> proses startup pertama, lalu jalankan ` <code translate="no">restart</code> ` untuk menerapkan perubahan (perintah ` <code translate="no">restart</code> ` akan mempertahankan berkas ` <code translate="no">user.yaml</code>`):</p>
...
Mengubah <code translate="no">mq.type</code> dengan cara ini ditujukan untuk instance <b>baru</b> (belum ada koleksi). Untuk mengubah antrian pesan pada instance yang sudah menyimpan data, ikuti <a href="/docs/id/switch-rocksmq-woodpecker.md">prosedur peralihan</a> sebagai gantinya.
</div>
<h3 id="Configure" class="common-anchor-header">Konfigurasi<button data-href="#Configure" class="anchor-icon" translate="no">
    ...
    </button></h3><p>Untuk mengoptimalkan RocksMQ, tambahkan bagian <code translate="no">rocksmq</code> ke berkas <code translate="no">user.yaml</code> dan mulai ulang layanan:</p>
...
<li><strong>Peningkatan dari 2.5.x ke 2.6.x:</strong> <strong>Batasan Antrian Pesan</strong>: Saat meningkatkan ke Milvus v3.0.2, Anda harus mempertahankan pilihan antrian pesan saat ini. Pergantian antara sistem antrian pesan yang berbeda selama proses peningkatan tidak didukung. Dukungan untuk mengganti sistem antrian pesan akan tersedia pada versi mendatang.
