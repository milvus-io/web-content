...
<thead>
<tr><th>Versi Milvus</th><th>Versi Pulsar yang didukung</th><th>Default</th></tr>
</thead>
<tbody>
<tr><td>2.5.x dan yang lebih baru</td><td>Pulsar v3 (disarankan) atau Pulsar v2</td><td>Pulsar v3 (melalui Helm / Operator Milvus)</td></tr>
...
<h2 id="Deploy-a-Milvus-cluster-with-Pulsar-using-Helm" class="common-anchor-header">Deploy kluster Milvus dengan Pulsar menggunakan Helm<button data-href="#Deploy-a-Milvus-cluster-with-Pulsar-using-Helm" class="anchor-icon" translate="no">
    ...
    </button></h3><p>Untuk mengimplementasikan kluster Milvus yang menggunakan Pulsar yang disertakan (bukan Woodpecker), pasang chart Helm dengan Streaming Node diaktifkan:</p>
...
<p>Pada Kubernetes v1.25 dan yang lebih baru, jika Anda mengalami masalah API PodDisruptionBudget dari sub-chart Pulsar yang disertakan, nonaktifkan kebijakan PDB Pulsar:</p>
<pre><code translate="no" class="language-bash">helm install my-release zilliztech/milvus \
...
<button class="copy-code-btn"></button></code></pre>
<p>Jika Anda menggunakan Pulsar yang disertakan dan ingin menghapus data yang tersimpan, hapus PVC Pulsar (bernama <code translate="no">my-release-pulsarv3-*</code>):</p>
<pre><code translate="no" class="language-bash">kubectl get pvc | grep my-release-pulsarv3
kubectl delete pvc &lt;pulsar-pvc-name&gt; ...
<button class="copy-code-btn"></button></code></pre>
<h2 id="Deploy-a-Milvus-cluster-with-Pulsar-using-Milvus-Operator" class="common-anchor-header">Deploy kluster Milvus dengan Pulsar menggunakan Milvus Operator<button data-href="#Deploy-a-Milvus-cluster-with-Pulsar-using-Milvus-Operator" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>Dengan Milvus Operator, konfigurasikan Pulsar di bawah <code translate="no">spec.dependencies.pulsar</code> (hanya didukung untuk kluster Milvus). <code translate="no">pulsar</code> mendukung <code translate="no">external</code> dan <code translate="no">inCluster</code>.</p>
...
<li><strong>Meningkatkan versi dari 2.5.x ke 2.6.x:</strong> <strong>Batasan Antrian Pesan</strong>: Saat meningkatkan versi ke Milvus v3.0.2, Anda harus mempertahankan pilihan antrian pesan Anda saat ini. Beralih di antara sistem antrian pesan yang berbeda selama proses peningkatan versi tidak didukung. Dukungan untuk mengubah sistem antrian pesan akan tersedia di versi mendatang.
...
<li><a href="/docs/id/switch-pulsar-woodpecker.md">Beralih antara Pulsar dan Woodpecker</a></li>
</ul>
