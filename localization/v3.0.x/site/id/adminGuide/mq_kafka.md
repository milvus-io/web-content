...
title: Kafka
---
<h1 id="Use-Kafka-as-the-Milvus-Message-Queue" class="common-anchor-header">Gunakan Kafka sebagai Antrian Pesan Milvus<button data-href="#Use-Kafka-as-the-Milvus-Message-Queue" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<li>Milvus hanya mendukung <strong>Kafka versi 2.x dan 3.x</strong>.</li>
<li>Kafka dikonfigurasi untuk Milvus Distributed (kluster) melalui Helm atau Milvus Operator.</li>
</ul>
<h2 id="Deploy-a-Milvus-cluster-with-Kafka-using-Helm" class="common-anchor-header">Deploy kluster Milvus dengan Kafka menggunakan Helm<button data-href="#Deploy-a-Milvus-cluster-with-Kafka-using-Helm" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><h3 id="Install-and-configure" class="common-anchor-header">Instal dan konfigurasikan<button data-href="#Install-and-configure" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Untuk menggunakan layanan Kafka eksternal, nonaktifkan Pulsar yang disertakan dan aktifkan opsi " <code translate="no">externalKafka</code> " dalam pengaturan override di ` <code translate="no">values.yaml</code> `, lalu instal Milvus dengan konfigurasi tersebut:</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>Untuk detail otentikasi SASL/SSL, lihat <a href="/docs/id/connect_kafka_ssl.md">Menghubungkan ke Kafka dengan SASL/SSL</a>.</p>
...
<button class="copy-code-btn"></button></code></pre>
<h2 id="Deploy-a-Milvus-cluster-with-Kafka-using-Milvus-Operator" class="common-anchor-header">Deploy kluster Milvus dengan Kafka menggunakan Milvus Operator<button data-href="#Deploy-a-Milvus-cluster-with-Kafka-using-Milvus-Operator" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>Dengan Milvus Operator, atur " <code translate="no">spec.dependencies.msgStreamType: &quot;kafka&quot;</code> " dan konfigurasikan Kafka di bawah " <code translate="no">spec.dependencies.kafka</code> " (hanya untuk kluster). " <code translate="no">kafka</code> " mendukung " <code translate="no">external</code> " dan " <code translate="no">inCluster</code>".</p>
...
<span class="hljs-attr">spec:</span>
  ...
  <span class="hljs-attr">config:</span> {}
<button class="copy-code-btn"></button></code></pre>
<p>Terapkan konfigurasi (dengan asumsi berkasnya adalah <code translate="no">milvuscluster.yaml</code>):</p>
...
<li><strong>Peningkatan versi dari 2.5.x ke 2.6.x:</strong> <strong>Batasan antrian pesan</strong>: Saat melakukan peningkatan ke Milvus v3.0-beta, Anda harus mempertahankan pilihan antrian pesan saat ini. Pergantian antara sistem antrian pesan yang berbeda selama proses peningkatan tidak didukung. Dukungan untuk mengganti sistem antrian pesan akan tersedia pada versi mendatang.
