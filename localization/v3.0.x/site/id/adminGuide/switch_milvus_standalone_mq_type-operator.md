...
summary: Pelajari cara mengganti jenis antrean pesan untuk Milvus mandiri.
title: Mengganti Jenis MQ untuk Milvus Standalone
---
<h1 id="Switch-MQ-Type-for-Milvus-Standalone" class="common-anchor-header">Mengganti Jenis MQ untuk Milvus Standalone<button data-href="#Switch-MQ-Type-for-Milvus-Standalone" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<h2 id="General-workflow" class="common-anchor-header">Alur kerja umum<button data-href="#General-workflow" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Alur kerja umum untuk mengganti jenis MQ adalah sebagai berikut:</p>
<ol>
<li>Pastikan instans Milvus berjalan dengan benar.</li>
<li>Konfirmasikan jenis MQ sumber dan jenis MQ target.</li>
<li>Konfigurasikan pengaturan akses MQ target ke dalam konfigurasi Milvus tanpa mengubah nilai <code translate="no">mqType</code>.</li>
<li>Picu sakelar dengan memanggil API alter WAL.</li>
...
<h2 id="Switch-from-RocksMQ-to-Woodpecker-Local-Storage" class="common-anchor-header">Beralih dari RocksMQ ke Woodpecker (Penyimpanan Lokal)<button data-href="#Switch-from-RocksMQ-to-Woodpecker-Local-Storage" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Prosedur ini berlaku untuk penerapan <strong>Docker Mandiri Milvus</strong> yang menggunakan RocksMQ secara default.</p>
<h3 id="Step-1-Verify-the-Milvus-instance-is-running" class="common-anchor-header">Langkah 1: Pastikan instans Milvus berjalan<button data-href="#Step-1-Verify-the-Milvus-instance-is-running" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Pastikan instans Milvus Standalone Docker Anda berjalan dengan benar. Anda dapat memverifikasi ini dengan membuat koleksi pengujian, menyisipkan data, dan menjalankan kueri.</p>
<h3 id="Step-2-Configure-Woodpecker-with-local-storage" class="common-anchor-header">Langkah 2: Mengonfigurasi Woodpecker dengan penyimpanan lokal<button data-href="#Step-2-Configure-Woodpecker-with-local-storage" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Perbarui konfigurasi Milvus untuk menambahkan pengaturan Woodpecker <strong>tanpa</strong> mengubah nilai <code translate="no">mqType</code>. Buat atau perbarui berkas <code translate="no">user.yaml</code> dengan konten berikut:</p>
...
<p>Kemudian, mulai ulang instans Milvus untuk menerapkan konfigurasi:</p>
...
<button class="copy-code-btn"></button></code></pre>
<div class="alert note">
<p>Ganti <code translate="no">&lt;mixcoord_addr&gt;</code> dengan alamat sebenarnya dari layanan MixCoord Anda (secara default, <code translate="no">localhost</code> untuk penerapan mandiri).</p>
</div>
<h3 id="Step-4-Verify-the-switch-is-complete" class="common-anchor-header">Langkah 4: Verifikasi pengalihan telah selesai<button data-href="#Step-4-Verify-the-switch-is-complete" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<button class="copy-code-btn"></button></code></pre>
<div class="alert note">
<p>Pada pesan log di atas, <code translate="no">&lt;MQ1&gt;</code> adalah jenis MQ sumber (<code translate="no">rocksmq</code>), dan <code translate="no">&lt;MQ2&gt;</code> adalah jenis MQ target (<code translate="no">woodpecker</code>).</p>
<ul>
...
<h2 id="Switch-from-RocksMQ-to-Woodpecker-MinIO-Storage" class="common-anchor-header">Beralih dari RocksMQ ke Woodpecker (Penyimpanan MinIO)<button data-href="#Switch-from-RocksMQ-to-Woodpecker-MinIO-Storage" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<h3 id="Step-1-Verify-the-Milvus-instance-is-running" class="common-anchor-header">Langkah 1: Verifikasi bahwa instans Milvus telah berjalan<button data-href="#Step-1-Verify-the-Milvus-instance-is-running" class="anchor-icon" translate="no">
    ...
    </button></h3><p>Pastikan instans Milvus Standalone Docker Compose Anda berjalan dengan baik.</p>
<h3 id="Step-2-Optional-Verify-Woodpecker-configuration" class="common-anchor-header">Langkah 2: (Opsional) Verifikasi konfigurasi Woodpecker<button data-href="#Step-2-Optional-Verify-Woodpecker-configuration" class="anchor-icon" translate="no">
    ...
    </button></h3><p>Konfigurasi default Milvus sudah menetapkan jenis penyimpanan Woodpecker ke MinIO, sehingga tidak ada konfigurasi tambahan yang diperlukan dalam banyak kasus.</p>
<p>Namun, jika Anda sebelumnya telah menyesuaikan konfigurasi Woodpecker, Anda harus memastikan bahwa <code translate="no">woodpecker.storage.type</code> disetel ke <code translate="no">minio</code>. Buat atau perbarui file <code translate="no">user.yaml</code> dengan konten berikut:</p>
<pre><code translate="no" class="language-yaml"><span class="hljs-attr">woodpecker:</span>
  <span class="hljs-attr">storage:</span>
