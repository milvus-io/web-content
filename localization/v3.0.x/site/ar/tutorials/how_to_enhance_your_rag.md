...
summary: >-
  ...
  بتحسين أدائها. تقدم هذه المقالة جميع الطرق الممكنة لتحسين خطوط أنابيب RAG
...
<li><strong>تحسين الفهرسة</strong>: تحسين إنشاء فهارس التقطيع باستخدام تقنيات مثل الفهرسة متعددة التقطيع أو الفهرسة التدريجية أو الفهرسة متعددة الاتجاهات.</li>
...
<h3 id="Creating-Hypothetical-Questions" class="common-anchor-header">إنشاء أسئلة افتراضية<button data-href="#Creating-Hypothetical-Questions" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<h3 id="HyDE-Hypothetical-Document-Embeddings" class="common-anchor-header">HyDE (تضمينات المستندات الافتراضية)<button data-href="#HyDE-Hypothetical-Document-Embeddings" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<p>تشبه هذه الطريقة تقنية السؤال الافتراضي في معالجة عدم التماثل بين المجالات في عمليات البحث المتجهة. ومع ذلك، فإن لها أيضًا عيوبًا، مثل التكاليف الحسابية الإضافية والشكوك المتعلقة بتوليد إجابات وهمية.</p>
<p>لمزيد من المعلومات، راجع ورقة <a href="https://arxiv.org/abs/2212.10496">HyDE</a>.</p>
<h3 id="Creating-Sub-Queries" class="common-anchor-header">إنشاء استعلامات فرعية<button data-href="#Creating-Sub-Queries" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>عندما يكون استعلام المستخدم معقدًا للغاية، يمكننا استخدام LLM لتقسيمه إلى استعلامات فرعية أبسط قبل تمريرها إلى قاعدة بيانات المتجهات و LLM. لنلقِ نظرة على مثال.</p>
...
<h3 id="Creating-Stepback-Prompts" class="common-anchor-header">إنشاء موجهات متدرجة<button data-href="#Creating-Stepback-Prompts" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>هناك طريقة أخرى لتبسيط استعلامات المستخدم المعقدة وهي إنشاء <strong><em>مطالبات متدرجة</em></strong>. تتضمن هذه التقنية تجريد استفسارات المستخدم المعقدة إلى <em><em>"</em>أسئلة متدرجة</em>"** باستخدام LLM. بعد ذلك، تستخدم قاعدة البيانات المتجهة هذه الأسئلة المتدرجة لاسترداد أجزاء المستندات الأكثر صلة. وأخيرًا، تُنشئ LLM إجابة أكثر دقة بناءً على أجزاء المستندات المسترجعة هذه.</p>
...
<h2 id="Indexing-Enhancement" class="common-anchor-header">تحسين الفهرسة<button data-href="#Indexing-Enhancement" class="anchor-icon" translate="no">
    ...
    </button></h2><p>تحسين الفهرسة هي استراتيجية أخرى لتحسين أداء تطبيقات RAG الخاصة بك. دعنا نستكشف ثلاث تقنيات لتحسين الفهرسة.</p>
<h3 id="Merging-Document-Chunks-Automatically" class="common-anchor-header">دمج أجزاء المستند تلقائيًا<button data-href="#Merging-Document-Chunks-Automatically" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<h3 id="Constructing-Hierarchical-Indices" class="common-anchor-header">إنشاء مؤشرات هرمية<button data-href="#Constructing-Hierarchical-Indices" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>عند إنشاء فهارس للمستندات، يمكننا إنشاء فهرس من مستويين: فهرس لملخصات المستندات وآخر لقطع المستندات. تتألف عملية البحث المتجه من مرحلتين: في البداية، نقوم بتصفية المستندات ذات الصلة بناءً على الملخص، وبعد ذلك، نسترجع أجزاء المستندات المقابلة حصريًا ضمن هذه المستندات ذات الصلة.</p>
<p>
  <span class="img-wrapper">
...
<p>ويثبت هذا النهج فائدته في الحالات التي تتضمن أحجامًا كبيرة من البيانات أو الحالات التي تكون فيها البيانات ذات تسلسل هرمي، مثل استرجاع المحتوى ضمن مجموعة مكتبات.</p>
<h3 id="Hybrid-Retrieval-and-Reranking" class="common-anchor-header">الاسترجاع وإعادة الترتيب الهجين<button data-href="#Hybrid-Retrieval-and-Reranking" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<h3 id="Sentence-Window-Retrieval" class="common-anchor-header">استرجاع نافذة الجملة<button data-href="#Sentence-Window-Retrieval" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>في نظام RAG الأساسي، يكون جزء المستند المُعطى إلى LLM عبارة عن نافذة أكبر تشمل جزء التضمين المسترجع. وهذا يضمن أن تتضمن المعلومات المقدمة إلى آلية استرجاع المستندات مجموعة أوسع من التفاصيل السياقية، مما يقلل من فقدان المعلومات. تعمل تقنية استرجاع نافذة الجملة على فصل جزء المستند المستخدم في استرجاع التضمين عن الجزء المقدم إلى آلية إدارة التعلم الآلي.</p>
<p>
  <span class="img-wrapper">
...
<h3 id="Meta-data-Filtering" class="common-anchor-header">تصفية البيانات الوصفية<button data-href="#Meta-data-Filtering" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>لضمان الحصول على إجابات أكثر دقة، يمكننا تنقيح المستندات المسترجعة عن طريق تصفية البيانات الوصفية مثل الوقت والفئة قبل تمريرها إلى LLM. على سبيل المثال، إذا تم استرجاع التقارير المالية التي تغطي سنوات متعددة، فإن التصفية بناءً على السنة المطلوبة ستعمل على تنقيح المعلومات لتلبية متطلبات محددة. تثبت هذه الطريقة فعاليتها في الحالات التي تحتوي على بيانات شاملة وبيانات وصفية مفصلة، مثل استرجاع المحتوى في مجموعات المكتبات.</p>
<p>
  <span class="img-wrapper">
...
<h3 id="Compressing-the-LLM-prompt" class="common-anchor-header">ضغط موجه LLM<button data-href="#Compressing-the-LLM-prompt" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<h3 id="Adjusting-the-chunk-order-in-the-prompt" class="common-anchor-header">ضبط ترتيب القطع في المطالبة<button data-href="#Adjusting-the-chunk-order-in-the-prompt" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<h3 id="Self-reflection" class="common-anchor-header">الانعكاس الذاتي<button data-href="#Self-reflection" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>يتضمن هذا النهج مفهوم الانعكاس الذاتي داخل وكلاء الذكاء الاصطناعي. إذن، كيف تعمل هذه التقنية؟</p>
...
<h3 id="Query-Routing-with-an-Agent" class="common-anchor-header">توجيه الاستعلام مع وكيل<button data-href="#Query-Routing-with-an-Agent" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>في بعض الأحيان، لا نضطر إلى استخدام نظام RAG للإجابة على الأسئلة البسيطة لأنه قد يؤدي إلى مزيد من سوء الفهم والاستدلال من المعلومات المضللة. في مثل هذه الحالات، يمكننا استخدام وكيل كموجه في مرحلة الاستعلام. يقوم هذا الوكيل بتقييم ما إذا كان الاستعلام يحتاج إلى المرور عبر خط أنابيب RAG. إذا كان الأمر كذلك، يتم البدء في خط أنابيب RAG اللاحق؛ وإلا فإن LLM يعالج الاستعلام مباشرةً.</p>
<p>
  <span class="img-wrapper">
