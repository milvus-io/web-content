...
<h3 id="BM25-search-without-embedding" class="common-anchor-header">بحث BM25 بدون تضمين<button data-href="#BM25-search-without-embedding" class="anchor-icon" translate="no">
    ...
    </button></h3><p>إذا كنت ترغب في إجراء بحث بنص كامل فقط باستخدام دالة BM25 دون استخدام أي بحث دلالي قائم على التضمين، يمكنك تعيين معلمة التضمين إلى <code translate="no">None</code> والاحتفاظ فقط بـ <code translate="no">builtin_function</code> المحدد كمثيل دالة BM25. يحتوي حقل المتجه على حقل "متناثر" فقط. على سبيل المثال:</p>
...
<h3 id="Multi-language-Analyzer" class="common-anchor-header">محلل متعدد اللغات<button data-href="#Multi-language-Analyzer" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>يدعم ميلفوس محلل متعدد اللغات للمستندات بلغات متعددة. استخدم <code translate="no">multi_analyzer_params</code> في <code translate="no">BM25BuiltInFunction</code>:</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>لمزيد من التفاصيل، راجع <a href="https://milvus.io/docs/multi-language-analyzers.md">وثائق المحلل متعدد اللغات</a>.</p>
<h2 id="Using-Hybrid-Search-and-Reranking-in-RAG" class="common-anchor-header">استخدام البحث المختلط وإعادة الترتيب في RAG<button data-href="#Using-Hybrid-Search-and-Reranking-in-RAG" class="anchor-icon" translate="no">
    ...
    </button></h2><p>لقد تعلمنا كيفية استخدام دالة BM25 الأساسية المدمجة في LangChain وMilvus. دعونا نقدم تطبيق RAG الأمثل مع البحث الهجين وإعادة الترتيب.</p>
<p>
