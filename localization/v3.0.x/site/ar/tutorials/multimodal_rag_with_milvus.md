...
summary: RAG متعدد الوسائط باستخدام Milvus
title: RAG متعدد الوسائط باستخدام Milvus
---
<h1 id="Multimodal-RAG-with-Milvus" class="common-anchor-header">RAG متعدد الوسائط باستخدام Milvus<button data-href="#Multimodal-RAG-with-Milvus" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<p>إذا كنت ترغب في تجربة النتيجة النهائية لهذا البرنامج التعليمي، يمكنك الانتقال مباشرةً إلى <a href="https://demos.milvus.io/multimodal-image-search/">العرض التوضيحي عبر الإنترنت</a>.</p>
...
<p>يعرض هذا البرنامج التعليمي تقنية RAG متعددة الوسائط المدعومة بـ Milvus <a href="https://github.com/FlagOpen/FlagEmbedding/tree/master/FlagEmbedding/visual">ونموذج BGE المرئي</a> و <a href="https://openai.com/index/hello-gpt-4o/">GPT-4o</a>. باستخدام هذا النظام، يمكن للمستخدمين تحميل صورة وتحرير التعليمات النصية، والتي تتم معالجتها بواسطة نموذج الاسترجاع المركب من BGE للبحث عن الصور المرشحة. ثم يعمل GPT-4o كأداة إعادة ترتيب، حيث يختار الصورة الأنسب ويقدم الأسباب المنطقية وراء هذا الاختيار. تتيح هذه المجموعة القوية تجربة بحث سلسة وبديهية عن الصور، مستفيدة من Milvus للاسترجاع الفعال، ونموذج BGE للمعالجة والمطابقة الدقيقة للصور، وGPT-4o لإعادة الترتيب المتقدمة.</p>
...
<h3 id="Download-Data" class="common-anchor-header">تنزيل البيانات<button data-href="#Download-Data" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>سيؤدي الأمر التالي إلى تنزيل البيانات النموذجية واستخراجها إلى مجلد محلي باسم «./images_folder» يتضمن:</p>
<ul>
<li><strong>الصور</strong>: مجموعة فرعية من <a href="https://github.com/hyp1231/AmazonReviews2023">Amazon Reviews 2023</a> تحتوي على حوالي 900 صورة من فئات "Appliance" و"Cell_Phones_and_Accessories" و"Electronics".</li>
<li><strong>leopard.jpg</strong>: صورة استعلام نموذجية.</li>
</ul>
...
<button class="copy-code-btn"></button></code></pre>
<h3 id="Load-Embedding-Model" class="common-anchor-header">تحميل نموذج التضمين<button data-href="#Load-Embedding-Model" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>سنستخدم نموذج Visualized BGE «bge-visualized-base-en-v1.5» لتوليد التضمينات لكل من الصور والنصوص.</p>
<p><strong>1. تنزيل الأوزان</strong></p>
...
<button class="copy-code-btn"></button></code></pre>
<p><strong>2. إنشاء المشفر</strong></p>
...
<h2 id="Load-Data" class="common-anchor-header">تحميل البيانات<button data-href="#Load-Data" class="anchor-icon" translate="no">
    ...
    </button></h2><p>سيتم في هذا القسم تحميل صور أمثلة إلى قاعدة البيانات مع التضمينات المقابلة لها.</p>
<h3 id="Generate-embeddings" class="common-anchor-header">إنشاء التضمينات<button data-href="#Generate-embeddings" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>تحميل جميع الصور بتنسيق jpeg من دليل البيانات وتطبيق المشفر لتحويل الصور إلى التضمينات.</p>
...
<h3 id="Insert-into-Milvus" class="common-anchor-header">الإدراج في Milvus<button data-href="#Insert-into-Milvus" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>إدراج الصور مع المسارات والتضمينات المقابلة في مجموعة Milvus.</p>
<div class="alert note">
<p>بالنسبة لمعلمة <code translate="no">MilvusClient</code>:</p>
<ul>
<li>يعد تعيين <code translate="no">uri</code> كملف محلي، على سبيل المثال <code translate="no">./milvus_demo.db</code> ، الطريقة الأكثر ملاءمة، حيث يستخدم تلقائيًا <a href="https://milvus.io/docs/milvus_lite.md">Milvus Lite</a> لتخزين جميع البيانات في هذا الملف.</li>
<li>إذا كانت لديك بيانات ضخمة الحجم، فيمكنك إعداد خادم Milvus أكثر كفاءة على <a href="https://milvus.io/docs/quickstart.md">Docker أو Kubernetes</a>. في هذا الإعداد، يرجى استخدام عنوان URI للخادم، على سبيل المثال<code translate="no">http://localhost:19530</code> ، كـ <code translate="no">uri</code>.</li>
...
<h2 id="Multimodal-Search-with-Generative-Reranker" class="common-anchor-header">البحث متعدد الوسائط باستخدام أداة إعادة الترتيب التوليدية<button data-href="#Multimodal-Search-with-Generative-Reranker" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>في هذا القسم، سنبحث أولاً عن الصور ذات الصلة باستخدام استعلام متعدد الوسائط، ثم نستخدم خدمة LLM لإعادة ترتيب النتائج والعثور على أفضلها مع تفسير.</p>
<h3 id="Run-search" class="common-anchor-header">تشغيل البحث<button data-href="#Run-search" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>نحن الآن جاهزون لإجراء بحث متقدم عن الصور باستخدام بيانات استعلام تتألف من كل من الصورة والتعليمات النصية.</p>
...
<h3 id="Rerank-with-GPT-4o" class="common-anchor-header">إعادة الترتيب باستخدام GPT-4o<button data-href="#Rerank-with-GPT-4o" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>سنستخدم نموذج اللغة الكبير (LLM) لترتيب الصور وتوليد تفسير لأفضل نتيجة بناءً على استعلام المستخدم والنتائج المسترجعة.</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>دمج صورة الاستعلام والصور المسترجعة مع الفهارس في عرض بانورامي.</p>
...
<p><span class="img-wrapper">
   ...
   <span>إنشاء عرض بانورامي</span>
  
 </span></p>
<p><strong>2. إعادة الترتيب وتقديم التفسير</strong></p>
<p>سنرسل الصورة المدمجة إلى خدمة نموذج اللغة الكبير متعدد الوسائط (LLM) مع المطالبات المناسبة لترتيب النتائج المسترجعة مع تفسيرها. لتمكين GPT-4o كنموذج لغة كبير (LLM)، تحتاج إلى إعداد <a href="https://platform.openai.com/docs/quickstart">مفتاح واجهة برمجة تطبيقات OpenAI</a> الخاص بك.</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>احصل على مؤشرات الصور بعد الترتيب وسبب اختيار أفضل نتيجة:</p>
...
<button class="copy-code-btn"></button></code></pre>
<p><strong>3. عرض أفضل نتيجة مع تفسير</strong></p>
...
<h3 id="Quick-Deploy" class="common-anchor-header">النشر السريع<button data-href="#Quick-Deploy" class="anchor-icon" translate="no">
    ...
    </button></h3><p>لمعرفة كيفية بدء عرض توضيحي عبر الإنترنت باستخدام هذا البرنامج التعليمي، يرجى الرجوع إلى <a href="https://github.com/milvus-io/bootcamp/tree/master/tutorials/quickstart/apps/multimodal_rag_with_milvus">التطبيق النموذجي</a>.</p>
