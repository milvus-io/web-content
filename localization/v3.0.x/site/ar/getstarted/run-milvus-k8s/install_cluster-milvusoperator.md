...
<h2 id="Overview" class="common-anchor-header">نظرة عامة<button data-href="#Overview" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Milvus Operator هو حل يساعدك على نشر وإدارة مجموعة خدمات Milvus الكاملة لاستهداف مجموعات Kubernetes (K8s). تتضمن المجموعة جميع مكونات Milvus والتبعيات ذات الصلة مثل etcd و MinIO، مع استخدام Woodpecker كقائمة انتظار رسائل مدمجة.</p>
...
<li><p>قبل تثبيت Milvus، يُنصح باستخدام <a href="https://milvus.io/tools/sizing">أداة تحديد حجم Milvus (Milvus Sizing Tool</a> ) لتقدير متطلبات الأجهزة بناءً على حجم البيانات لديك. وهذا يساعد على ضمان الأداء الأمثل وتخصيص الموارد بشكل مثالي لتثبيت Milvus الخاص بك.</p></li>
</ul>
...
<button class="copy-code-btn"></button></code></pre>
<p>يمكنك التحقق من تشغيل بود Milvus Operator على النحو التالي:</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>يقوم الأمر أعلاه بنشر مجموعة Milvus باستخدام <strong>Woodpecker</strong> كقائمة انتظار الرسائل (موصى به للإصدار v3.0-beta) وجميع المكونات المعمارية الجديدة بما في ذلك Streaming Node.</p>
...
<p>لتخصيص هذه الإعدادات، نوصيك باستخدام <a href="https://milvus.io/tools/sizing">أداة تحديد حجم Milvus (Milvus Sizing Tool)</a> لتعديل التكوينات بناءً على حجم بياناتك الفعلي، ثم تنزيل ملف YAML المقابل. لمعرفة المزيد عن معلمات التكوين، راجع <a href="https://milvus.io/docs/system_configuration.md">قائمة مراجعة تكوينات نظام Milvus (Milvus System Configurations Checklist</a>).</p>
<div class="alert note">
<ul>
...
<button class="copy-code-btn"></button></code></pre>
<p>بمجرد أن تصبح مجموعة Milvus جاهزة، يجب أن يكون ناتج الأمر أعلاه مشابهًا لما يلي. إذا ظل حقل « <code translate="no">status.status</code> » (تاريخ الإعداد) على « <code translate="no">Unhealthy</code> » (قيد الإنشاء)، فإن مجموعة Milvus لا تزال قيد الإنشاء.</p>
...
<h2 id="Access-Milvus-WebUI" class="common-anchor-header">الوصول إلى واجهة المستخدم الرسومية لـ Milvus<button data-href="#Access-Milvus-WebUI" class="anchor-icon" translate="no">
    ...
    </button></h2><p>يأتي Milvus مزودًا بأداة واجهة مستخدم رسومية مدمجة تسمى Milvus WebUI يمكنك الوصول إليها من خلال متصفحك. تعزز واجهة Milvus Web UI قابلية مراقبة النظام بواجهة بسيطة وسهلة الاستخدام. يمكنك استخدام واجهة Milvus Web UI لمراقبة الإحصائيات والمقاييس الخاصة بمكونات Milvus وتبعياته، والتحقق من تفاصيل قاعدة البيانات والتجميعات، وعرض قائمة بتكوينات Milvus التفصيلية. للحصول على تفاصيل حول واجهة المستخدم على الويب لـ Milvus، راجع <a href="/docs/ar/milvus-webui.md">واجهة المستخدم على الويب لـ Milvus</a></p>
<p>لتمكين الوصول إلى واجهة Milvus WebUI، تحتاج إلى إعادة توجيه منفذ بود الوكيل إلى منفذ محلي.</p>
...
<div class="alert note">
<ul>
<li>عند حذف مجموعة Milvus باستخدام التكوين الافتراضي، لا يتم حذف التبعيات مثل etcd و MinIO. لذلك، في المرة التالية التي تقوم فيها بتثبيت نفس مثيل مجموعة Milvus، سيتم استخدام هذه التبعيات مرة أخرى.</li>
...
<ul>
<li><p>تحقق من <a href="/docs/ar/quickstart.md">Hello Milvus</a> لمعرفة ما يمكن أن يفعله Milvus.</p></li>
