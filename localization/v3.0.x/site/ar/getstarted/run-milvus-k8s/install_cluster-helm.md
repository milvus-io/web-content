...
<li><p>تثبيت <a href="https://kubernetes.io/docs/tasks/administer-cluster/change-default-storage-class/">StorageClass</a>. يمكنك التحقق من StorageClass المثبتة كما يلي.</p>
...
<p>يقوم الأمر التالي بنشر مجموعة Milvus بإعدادات محسّنة للإصدار v3.0.1، باستخدام Woodpecker كقائمة انتظار الرسائل الموصى بها:</p>
<pre><code translate="no" class="language-bash">helm install my-release zilliztech/milvus \
  --<span class="hljs-built_in">set</span> image.all.tag=v3.0.1 \
  --<span class="hljs-built_in">set</span> woodpecker.enabled=<span class="hljs-literal">true</span> \
...
<li><strong>المكونات المدمجة</strong>: تم دمج " <strong>عقدة الفهرس</strong> " و" <strong>عقدة البيانات</strong> " في " <strong>عقدة بيانات</strong>" واحدة</li>
</ul>
...
<button class="copy-code-btn"></button></code></pre>
<p><strong>انتظر حتى تظهر حالة «قيد التشغيل» لجميع البودات.</strong> مع تكوين الإصدار v3.0.1، من المفترض أن ترى بودات مشابهة لما يلي:</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>الآن، يمكنك الوصول إلى واجهة مستخدم الويب Milvus على <code translate="no">http://localhost:27018</code>.</p>
...
<h3 id="1-Get-Milvus-manifest" class="common-anchor-header">1. الحصول على ملف بيان Milvus<button data-href="#1-Get-Milvus-manifest" class="anchor-icon" translate="no">
    ...
    </button></h3><p>قم بتشغيل الأمر التالي للحصول على ملف بيانات Milvus.</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>يعمل الأمر أعلاه على عرض قوالب المخططات لمجموعة Milvus وحفظ الناتج في ملف قائمة تسمى <code translate="no">milvus_manifest.yaml</code>. باستخدام قائمة البيانات هذه، يمكنك تثبيت مجموعة Milvus مع مكوناتها وتبعياتها في وحدات منفصلة.</p>
<div class="alert note">
<ul>
...
<li>لتغيير تكوينات Milvus، قم بتنزيل <a href="https://raw.githubusercontent.com/milvus-io/milvus-helm/master/charts/milvus/values.yaml"><code translate="no">value.yaml</code></a> القالب، وأدخل الإعدادات المطلوبة فيه، ثم استخدم <code translate="no">helm template -f values.yaml my-release zilliztech/milvus &gt; milvus_manifest.yaml</code> لتوليد ملف البيان وفقًا لذلك.</li>
</ul>
</div>
...
<button class="copy-code-btn"></button></code></pre>
<p>حتى الآن، يمكنك اتباع الخطوتين <a href="#2-Check-Milvus-cluster-status">2</a> <a href="#3-Connect-to-Milvus">و3</a> من التثبيت عبر الإنترنت للتحقق من حالة المجموعة وتوجيه منفذ محلي إلى Milvus.</p>
