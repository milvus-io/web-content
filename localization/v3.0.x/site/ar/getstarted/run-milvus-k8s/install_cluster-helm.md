...
<li><p>قبل تثبيت Milvus، يُنصح باستخدام <a href="https://milvus.io/tools/sizing">أداة تحديد حجم Milvus (Milvus Sizing Tool</a> ) لتقدير متطلبات الأجهزة بناءً على حجم البيانات لديك. يساعد ذلك في ضمان الأداء الأمثل وتخصيص الموارد بشكل مثالي لتثبيت Milvus الخاص بك.</p></li>
</ul>
...
<p>يقوم الأمر التالي بنشر مجموعة Milvus بإعدادات مُحسَّنة للإصدار v3.0.2، باستخدام Woodpecker كقائمة انتظار الرسائل الموصى بها:</p>
<pre><code translate="no" class="language-bash">helm install my-release zilliztech/milvus \
  --<span class="hljs-built_in">set</span> image.all.tag=v3.0.2 \
  --<span class="hljs-built_in">set</span> woodpecker.enabled=<span class="hljs-literal">true</span> \
...
<li>تشغيل <strong>Woodpecker كخدمة مخصصة</strong> (StatefulSet منفصل)، وليس مدمجًا في عقدة البث</li>
...
<li><strong>مكون جديد</strong>: تم تقديم " <strong>عقدة البث" (Streaming Node</strong> ) وتم تمكينها بشكل افتراضي</li>
<li><strong>المكونات المدمجة</strong>: تم دمج " <strong>Index Node</strong> " و" <strong>Data Node</strong> " في " <strong>Data Node</strong>" واحد</li>
</ul>
...
<h3 id="2-Check-Milvus-cluster-status" class="common-anchor-header">2. تحقق من حالة مجموعة Milvus<button data-href="#2-Check-Milvus-cluster-status" class="anchor-icon" translate="no">
    ...
    </button></h3><p>تحقق من نجاح عملية النشر عن طريق التحقق من حالة البودات:</p>
<pre><code translate="no" class="language-bash">kubectl get pods
<button class="copy-code-btn"></button></code></pre>
<p><strong>انتظر حتى تظهر حالة «قيد التشغيل» لجميع البودات.</strong> مع التكوين v3.0.2، من المفترض أن ترى بودات مشابهة لما يلي:</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>يقوم هذا الأمر بتوجيه المنفذ المحلي <code translate="no">27017</code> إلى منفذ Milvus <code translate="no">19530</code>. ستظهر لك الرسالة التالية:</p>
...
<h2 id="Offline-install" class="common-anchor-header">التثبيت دون اتصال بالإنترنت<button data-href="#Offline-install" class="anchor-icon" translate="no">
    ...
    </button></h2><p>إذا كنت في بيئة مقيدة بالشبكة، فاتبع الإجراء الموضح في هذا القسم لبدء تشغيل مجموعة Milvus.</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>حتى الآن، يمكنك اتباع الخطوتين <a href="#2-Check-Milvus-cluster-status">2</a> و <a href="#3-Connect-to-Milvus">3</a> من التثبيت عبر الإنترنت للتحقق من حالة المجموعة وتوجيه منفذ محلي إلى Milvus.</p>
...
<div class="alert note">
<p>يتم تعطيل «Storage V3» افتراضيًا. قم بتفعيله قبل استخدام الميزات التي تعتمد عليه. للاطلاع على المتطلبات واعتبارات التوافق، راجع <a href="/docs/ar/storage-v3.md">«Storage V3</a>».</p>
</div>
