...
<p>يحتوي Milvus على عدد كبير من المعلمات التي تُستخدم لتكوين النظام. لكل معلمة قيمة افتراضية يمكن استخدامها مباشرةً. يمكنك تعديل هذه المعلمات بمرونة حتى يتمكن Milvus من خدمة تطبيقك بشكل أفضل. انظر <a href="/docs/ar/configure-docker.md">تكوين Milvus</a> لمزيد من المعلومات.</p>
...
<h2 id="Sections" class="common-anchor-header">الأقسام<button data-href="#Sections" class="anchor-icon" translate="no">
    ...
    </button></h2><p>لتسهيل الصيانة، يصنف Milvus تكويناته إلى %s أقسام بناءً على مكوناته وتبعياته واستخدامه العام.</p>
...
<h3 id="metastore" class="common-anchor-header"><code translate="no">metastore</code><button data-href="#metastore" class="anchor-icon" translate="no">
    ...
    </button></h3><p>انظر <a href="/docs/ar/configure_metastore.md">«إعدادات متعلقة بـ metastore»</a> للحصول على وصف تفصيلي لكل معلمة ضمن هذا القسم.</p>
...
<p>لاحظ أنه عند تمكين TiKV لمخزن البيانات الوصفية (metastore)، لا تزال بحاجة إلى etcd لاكتشاف الخدمات.</p>
<p>يُعد TiKV خيارًا جيدًا عندما يتطلب حجم البيانات الوصفية قابلية توسع أفقية أفضل.</p>
<p>انظر <a href="/docs/ar/configure_tikv.md">«الإعدادات» المتعلقة بـ tikv</a> للحصول على وصف تفصيلي لكل معلمة في هذا القسم.</p>
<h3 id="localStorage" class="common-anchor-header"><code translate="no">localStorage</code><button data-href="#localStorage" class="anchor-icon" translate="no">
    ...
    </button></h3><p>انظر <a href="/docs/ar/configure_localstorage.md">التكوينات المتعلقة بـ localStorage</a> للحصول على وصف تفصيلي لكل معلمة في هذا القسم.</p>
...
<p>نُشير إلى خدمة التخزين باسم MinIO/S3 في الوصف التالي للتبسيط.</p>
...
<p>إذا لم تقم بتعيين حقل mq.type على القيمة الافتراضية، فهناك ملاحظة حول تمكين الأولوية في حالة تكوين عدة أنظمة MQ في هذا الملف.</p>
<ol>
...
<li><p>يمكن استخدام Woodpecker في كل من الوضع المستقل ووضع الكتلة عن طريق تعيين mq.type إلى woodpecker.</p></li>
</ol>
...
<h3 id="pulsar" class="common-anchor-header"><code translate="no">pulsar</code><button data-href="#pulsar" class="anchor-icon" translate="no">
    ...
    </button></h3><p>التكوينات ذات الصلة بـ Pulsar، والتي تُستخدم لإدارة سجلات Milvus لعمليات التغيير الأخيرة، وسجل تدفق المخرجات، وتوفير خدمات نشر السجلات والاشتراك فيها.</p>
...
<p>انظر <a href="/docs/ar/configure_querynode.md">«التكوينات المتعلقة بـ queryNode</a> » للاطلاع على وصف تفصيلي لكل معلمة ضمن هذا القسم.</p>
...
<p>انظر " <a href="/docs/ar/configure_msgchannel.md">الإعدادات المتعلقة بـ msgChannel</a> " للحصول على وصف تفصيلي لكل معلمة ضمن هذا القسم.</p>
<h3 id="log" class="common-anchor-header"><code translate="no">log</code><button data-href="#log" class="anchor-icon" translate="no">
    ...
    </button></h3><p>يُستخدم لتكوين إخراج سجل النظام.</p>
...
<p>انظر <a href="/docs/ar/configure_tls.md">إعدادات TLS</a> للحصول على وصف تفصيلي لكل معلمة ضمن هذا القسم.</p>
...
<h3 id="common" class="common-anchor-header"><code translate="no">common</code><button data-href="#common" class="anchor-icon" translate="no">
    ...
    </button></h3><p>انظر <a href="/docs/ar/configure_common.md">"التكوينات" المتعلقة بـ common</a> للحصول على وصف تفصيلي لكل معلمة في هذا القسم.</p>
<h3 id="quotaAndLimits" class="common-anchor-header"><code translate="no">quotaAndLimits</code><button data-href="#quotaAndLimits" class="anchor-icon" translate="no">
    ...
    </button></h3><p>QuotaConfig، إعدادات حصص Milvus وحدودها.</p>
...
<li><p>تحديد عدد عمليات DDL وDQL في الثانية (qps/rps)؛</p></li>
...
<p>انظر <a href="/docs/ar/configure_gpu.md">«التكوينات المتعلقة بوحدة معالجة الرسومات (GPU)</a> » للحصول على وصف تفصيلي لكل معلمة في هذا القسم.</p>
...
<p>انظر <a href="/docs/ar/configure_streamingnode.md">«الإعدادات المتعلقة بـ streamingNode</a> » للحصول على وصف تفصيلي لكل معلمة في هذا القسم.</p>
<h3 id="streaming" class="common-anchor-header"><code translate="no">streaming</code><button data-href="#streaming" class="anchor-icon" translate="no">
    ...
    </button></h3><p>أي إعدادات تتعلق بخدمة البث.</p>
<p>انظر " <a href="/docs/ar/configure_streaming.md">الإعدادات المتعلقة بالبث</a> " للحصول على وصف تفصيلي لكل معلمة ضمن هذا القسم.</p>
