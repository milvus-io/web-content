...
<h3 id="Enable-Woodpecker-for-Milvus-Standalone-in-Docker-storagelocal" class="common-anchor-header">تمكين Woodpecker لـ Milvus المستقل في Docker (storage=local)<button data-href="#Enable-Woodpecker-for-Milvus-Standalone-in-Docker-storagelocal" class="anchor-icon" translate="no">
    ...
    </button></h3><p>في Milvus 3.x، يستخدم النشر المستقل في Docker Woodpecker مع <strong>نظام الملفات المحلي</strong> كخلفية WAL <strong>بشكل افتراضي</strong> — ولا يلزم أي تكوين إضافي. اتبع <a href="/docs/ar/install_standalone-docker.md">إرشادات تشغيل Milvus في Docker</a>:</p>
...
<li>جانب التخزين
<ul>
<li><strong>تخزين الكائنات (متوافق مع MinIO/S3)</strong>: قم بزيادة التزامن وحجم الكائنات (تجنب الكائنات الصغيرة جدًا). راقب حدود النطاق الترددي للشبكة وحاوية التخزين. غالبًا ما يبلغ الحد الأقصى لعقدة MinIO واحدة على SSD حوالي 100 ميجابايت/ثانية محليًا؛ بينما يمكن أن تصل سرعة نقل البيانات من EC2 إلى S3 إلى جيجابايت/ثانية.</li>
...
<button class="copy-code-btn"></button></code></pre>
<h2 id="Latency" class="common-anchor-header">الزمن المستغرق<button data-href="#Latency" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<li><strong>عمليات قراءة أحادية القفزة تراعي التوبولوجيا</strong> — تذهب كل عملية قراءة مباشرةً إلى النسخة المتماثلة الأقرب بدلاً من إعادة توجيهها عبر وسيط، مما يتجنب عمليات القراءة العشوائية عبر المناطق التشغيلية (≈2/3 من حركة مرور القراءة عبر المناطق التشغيلية) في الأنظمة القائمة على الوسيط.</li>
