...
<li>في Milvus 3.x، يُعد Woodpecker سجل <strong>الكتابة</strong> المسبق/قائمة انتظار الرسائل <strong>الافتراضية،</strong> حيث يوفر عمليات الكتابة المرتبة والاستعادة بصفته خدمة التسجيل. ولا يلزم وجود خدمة خارجية لقائمة انتظار الرسائل (مثل Pulsar أو Kafka).</li>
...
<p>ملاحظات:</p>
<ul>
<li>مع <code translate="no">minio</code> ، يشارك Woodpecker نفس تخزين الكائنات مع Milvus (MinIO/S3/GCS/OSS، إلخ).</li>
...
<p>يُستخدم وضع Woodpecker <code translate="no">service</code> للنشر <strong>الموزع/المجمّع</strong> فقط — أما عمليات النشر المستقلة فتعمل باستخدام Woodpecker المدمج (<code translate="no">minio</code> أو <code translate="no">local</code>). لا يدعم Milvus Operator حتى الآن وضع خدمة Woodpecker.</p>
</div>
...
<li>جانب التخزين
<ul>
<li><strong>تخزين الكائنات (متوافق مع MinIO/S3)</strong>: قم بزيادة التزامن وحجم الكائنات (تجنب الكائنات الصغيرة جدًا). راقب حدود النطاق الترددي للشبكة وحاوية التخزين. غالبًا ما يبلغ الحد الأقصى لعقدة MinIO واحدة على SSD حوالي 100 ميجابايت/ثانية محليًا؛ بينما يمكن أن يصل EC2 واحد إلى S3 إلى جيجابايت/ثانية.</li>
...
<ul>
<li><strong>عمليات الكتابة ذات النصاب القانوني</strong> في جولة<strong>واحدة (One-RTT)</strong> — حيث يكمل النسخ المتماثل الذي يحركه العميل عملية الكتابة ذات النصاب القانوني خلال جولة واحدة، مع تثبيت حركة المرور عبر المناطق (AZ) عند حجم بيانات نسختين متماثلتين (مقارنةً بحركة المرور الإضافية عبر المناطق التي تبلغ حوالي 1/3، وهي النسبة النموذجية للنسخ المتماثل القائم على الوسيط/القائد).</li>
...
<li><strong>لا يوجد تكرار مستمر من عقدة إلى أخرى</strong> — يتم الاحتفاظ بالسجلات في تخزين الكائنات الذي يعمل كوحدة تخزين مشتركة، لذا فإن التحويل التلقائي في حالة الفشل يعيد تحميل النسخ المتبقية فقط (بدون نسخ العقدة بأكملها)، ولا يكون التوسع مقيدًا بعرض النطاق الترددي للتكرار بين العقد، ولا يتسبب استبدال العقد على نطاق واسع في حدوث «عواصف تكرار».</li>
</ul>
