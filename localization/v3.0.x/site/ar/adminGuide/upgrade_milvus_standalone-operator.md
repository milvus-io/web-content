...
summary: تعرف على كيفية ترقية الإصدار المستقل من Milvus باستخدام Milvus Operator.
...
<h1 id="Upgrade-Milvus-Standalone-with-Milvus-Operator" class="common-anchor-header">ترقية Milvus Standalone باستخدام Milvus Operator<button data-href="#Upgrade-Milvus-Standalone-with-Milvus-Operator" class="anchor-icon" translate="no">
    ...
    </button></h1><p>يصف هذا الدليل كيفية ترقية نشر Milvus 2.6.x المستقل إلى الإصدار v3.0.2 باستخدام Milvus Operator.</p>
<div class="alert note">
<p>تم التحقق من صحة هذا الإجراء من Milvus 2.6.20 إلى Milvus v3.0.2 باستخدام Milvus Operator 1.3.0 و Woodpecker و etcd داخل المجموعة و MinIO داخل المجموعة. إذا كنت تستخدم إصدار تصحيح آخر لـ Milvus 2.6.x، أو إصدار Operator آخر، أو قائمة انتظار رسائل أخرى، أو تكوين تبعيات مختلف، فقم بالتحقق من صحة الترقية في بيئة غير إنتاجية أولاً.</p>
</div>
...
<li>مجموعة Kubernetes مع نشر مستقل لـ Milvus 2.6.x يديره Milvus Operator</li>
...
<li>طريقة التثبيت وقوائم البيانات المستخدمة في Milvus Operator الحالي</li>
...
<p>استخدم ملف البيانات المرجعية (manifest) المصدر للنشر الحالي كملف بيانات مرجعية للترقية. لا تقم بتطبيق ملف النسخ الاحتياطي المصدّر مباشرةً دون إزالة البيانات الوصفية التي يديرها الخادم وحقول الحالة أولاً.</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>أبقت عملية الترقية التي تم التحقق من صحتها على إصدار Milvus Operator عند 1.3.0. احتفظ بإصدار Operator الذي يدير حاليًا نشر Milvus 2.6.x الخاص بك ما لم تتطلب سياسة الدعم الخاصة بك ترقية Operator منفصلة. لا تقم بتخفيض إصدار Operator الأحدث إلى الإصدار الذي تم اختباره. إذا كنت بحاجة إلى تغيير إصدار Operator، فاستخدم نفس طريقة التثبيت Helm أو <code translate="no">kubectl</code> ونفس اسم الإصدار ومساحة الاسم مثل التثبيت الحالي، ثم تحقق من صحة تغيير Operator قبل تحديث Milvus CR.</p>
<h3 id="Step-3-Update-the-Milvus-image" class="common-anchor-header">الخطوة 3: تحديث صورة Milvus<button data-href="#Step-3-Update-the-Milvus-image" class="anchor-icon" translate="no">
    ...
    </button></h3><p>في بيان Milvus CR الكامل، قم بتغيير <code translate="no">spec.components.image</code> فقط. احتفظ بالوضع الحالي وإعدادات المكونات وقائمة انتظار الرسائل وetcd والتخزين وإعدادات التبعيات الأخرى. يوضح المقتطف التالي الحقل المطلوب تغييره؛ لا تستبدل CR الكامل بهذا المقتطف.</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>قم بتطبيق ملف بيان CR الكامل:</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>تأكد من أن CR الخاص بـ Milvus يبلغ عن <code translate="no">Healthy</code> ، وأن الصورة الحالية هي <code translate="no">milvusdb/milvus:v3.0.2</code> ، وأن المجموعات الحالية لا تزال قابلة للاستعلام والبحث. أكمل هذه الفحوصات قبل تمكين أي ميزة خاصة بالإصدار v3.0.2.</p>
