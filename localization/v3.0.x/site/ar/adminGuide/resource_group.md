...
<p>جميع أمثلة الكود في هذه الصفحة مكتوبة بلغة PyMilvus 3.0.1. قم بترقية إصدار PyMilvus المثبت لديك قبل تشغيلها.</p>
</div>
<ol>
...
يضمن الأمر<code translate="no">update_resource_groups</code> التزامن التام لتغييرات التكوين المتعددة، لذلك لن تظهر أي حالات وسيطة لـ Milvus.</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>كما يمكنك تحميل قسم واحد فقط إلى مجموعة موارد وتوزيع نسخه المتماثلة على عدة مجموعات موارد. يفترض المقتطف التالي وجود مجموعة باسم <code translate="no">Books</code> بالفعل وأنها تحتوي على قسم باسم <code translate="no">Novels</code>.</p>
...
<p>بالإضافة إلى ذلك، إذا قمنا بفرض القيد <code translate="no">sum(.requests.nodeNum) &lt;= queryNodeNum</code> بصرامة، يمكننا التحكم بدقة في تخصيص عقدات الاستعلام (QueryNodes) في المجموعة. لنفترض أن هناك حاليًا عقدة استعلام واحدة فقط في المجموعة، ثم نقوم بتهيئة المجموعة.
