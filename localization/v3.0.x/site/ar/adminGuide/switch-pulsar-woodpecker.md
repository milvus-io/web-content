...
<p><strong>الخطوة 2: تنفيذ عملية التبديل إلى MQ.</strong> قم بكشف واجهة إدارة MixCoord، ثم استدعِ واجهة برمجة التطبيقات (API) الخاصة بالتبديل:</p>
...
<p><strong>الخطوة 4: (اختياري) إيقاف Pulsar وتنظيفه.</strong> بالنسبة إلى Pulsar <strong>المدمج،</strong> قم بتعطيل Pulsar وتمكين Woodpecker، ثم احذف PVCs الخاصة بـ Pulsar:</p>
...
<p><strong>الخطوة 4: قم بتحديث نوع MQ في Operator.</strong> قم بتحديث التكوين الذي يديره<strong>Operator</strong> حتى لا يعيد Operator عملية التبديل. قم بإنشاء <code translate="no">change_configmap.yaml</code>:</p>
...
<button class="copy-code-btn"></button></code></pre>
<p><strong>الخطوة 5: (اختياري) إيقاف Pulsar وتنظيفه.</strong> بالنسبة لـ Pulsar <strong>المدمج،</strong> قم بإلغاء تثبيت إصدار Pulsar وحذف PVCs الخاصة به:</p>
...
<p><strong>الخطوة 5: (اختياري) قم بإزالة بيانات Woodpecker.</strong> احذف بيانات Woodpecker الموجودة على MinIO/S3 (تحت <code translate="no">&lt;rootPath&gt;/wp/...</code> ، وعادةً ما تكون <code translate="no">files/wp/...</code>) وبيانات تعريف Woodpecker في etcd (<code translate="no">etcdctl get woodpecker --prefix</code>). إذا كنت تخطط للعودة إلى استخدام Woodpecker لاحقًا، فقم بإزالة هذه الملفات أولاً.</p>
