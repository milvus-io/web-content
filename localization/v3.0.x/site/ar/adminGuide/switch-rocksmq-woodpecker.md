...
<h3 id="Step-2-Configure-Woodpecker-storage" class="common-anchor-header">الخطوة 2: تكوين تخزين Woodpecker<button data-href="#Step-2-Configure-Woodpecker-storage" class="anchor-icon" translate="no">
    ...
    </button></h3><p>أضف إعدادات Woodpecker إلى تكوين Milvus <strong>دون</strong> تغيير قيمة <code translate="no">mqType</code>. قم بتشغيل <code translate="no">docker exec -it milvus-standalone bash</code> للدخول إلى الحاوية، ثم قم بتحرير <code translate="no">/milvus/configs/user.yaml</code>:</p>
...
<p>إذا كانت هذه هي المرة الأولى التي تقوم فيها بالتحويل إلى Woodpecker، فتخط هذه الملاحظة. وإلا، فقم بتنظيف البيانات والبيانات الوصفية المتبقية لـ Woodpecker قبل التحويل مرة أخرى — فقد تتسبب البيانات المتبقية في حدوث سلوك غير متوقع.</p>
</div>
...
<h3 id="Step-4-Verify-the-switch-is-complete" class="common-anchor-header">الخطوة 4: التحقق من اكتمال التبديل<button data-href="#Step-4-Verify-the-switch-is-complete" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<p>تأكد من عدم وجود أي بيانات RocksMQ متبقية في المثيل من تشغيل سابق. إذا كانت هذه هي المرة الأولى التي تقوم فيها بالتحويل إلى RocksMQ، فتخط هذه الملاحظة؛ وإلا فقم أولاً بتنظيف البيانات والبيانات الوصفية ذات الصلة بـ RocksMQ.</p>
</div>
<pre><code translate="no" class="language-shell">curl -X POST http://&lt;mixcoord_addr&gt;:&lt;mixcoord_port&gt;/management/wal/alter \
  -H &quot;Content-Type: application/json&quot; \
...
<button class="copy-code-btn"></button></code></pre>
<p>يتم تسجيل التبديل الناجح في <code translate="no">[mqTypeValue=rocksmq]</code>.</p>
...
<tr><td>Woodpecker المحلي</td><td>Woodpecker MinIO</td><td><strong>غير مدعوم</strong></td><td>كما هو مذكور أعلاه.</td></tr>
