...
<p>يستخدم هذا التكامل جهاز التوجيه (router) المستضاف من Hugging Face. لإعادة الترتيب باستخدام خدمة استدلال تضمين النص (TEI) التي تم نشرها بشكل منفصل، راجع <a href="/docs/ar/tei-ranker.md">TEI Ranker</a>.</p>
...
<p>لا يتحكم Milvus في ما إذا كان نموذج Hugging Face سيظل متاحًا عبر <code translate="no">hf-inference</code> ، أو ما إذا كان النموذج يفي بمتطلباتك فيما يتعلق بالاستقرار وزمن الاستجابة وجودة المخرجات. تحقق من النموذج على Hugging Face وقم بتقييمه بالنسبة لحمل العمل الخاص بك قبل استخدامه في بيئة الإنتاج.</p>
</div>
...
<h2 id="Configure-credentials" class="common-anchor-header">تكوين بيانات الاعتماد<button data-href="#Configure-credentials" class="anchor-icon" translate="no">
    ...
    </button></h2><p>يمكنك تكوين رمز وصول المستخدم (User Access Token) الخاص بـ Hugging Face في <code translate="no">milvus.yaml</code> أو من خلال متغير بيئة.</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>يمكن لمعلمة <code translate="no">credential</code> على مستوى الوظيفة أن تتجاوز التسمية على مستوى المزود. يجب أن تكون قيمتها تسمية بيانات اعتماد محددة في <code translate="no">milvus.yaml</code> ، وليس الرمز نفسه.</p>
...
<h3 id="Step-2-Define-the-rerank-Function" class="common-anchor-header">الخطوة 2: تعريف دالة إعادة الترتيب<button data-href="#Step-2-Define-the-rerank-Function" class="anchor-icon" translate="no">
    ...
    </button></h3><p>قم بتعريف دالة إعادة الترتيب ( <code translate="no">RERANK</code> ) التي تقرأ النص المرشح من <code translate="no">document</code> وتستخدم نص الاستعلام الموجود في <code translate="no">queries</code>:</p>
...
<h3 id="The-number-of-query-strings-does-not-match-the-search-request" class="common-anchor-header">عدد سلاسل الاستعلام لا يتطابق مع طلب البحث<button data-href="#The-number-of-query-strings-does-not-match-the-search-request" class="anchor-icon" translate="no">
    ...
    </button></h3><p>يجب أن يساوي عدد سلاسل الاستعلامات في <code translate="no">queries</code> عدد استعلامات البحث (<code translate="no">nq</code>). للبحث باستخدام متجه استعلام واحد، أدخل سلسلة استعلام واحدة فقط.</p>
...
<li>للاطلاع على سلوك مصنف النماذج المشترك وحدوده، راجع <a href="/docs/ar/model-ranker-overview.md">نظرة عامة على مصنف النماذج</a>.</li>
<li>لإنشاء التضمينات من خلال مزودي الاستدلال المستضافين من Hugging Face، راجع <a href="/docs/ar/hugging-face.md">Hugging Face</a>.</li>
<li>لتطبيق أداة التصنيف على البحث الهجين، راجع " <a href="/docs/ar/multi-vector-search.md">البحث الهجين متعدد المتجهات</a>".</li>
</ul>
