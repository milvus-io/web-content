...
title: البحث حسب المجموعات
summary: >-
  استخدم «البحث المجمَّع» لتجميع نتائج بحث الشبكة العصبية الاصطناعية (ANN) حسب
  قيمة حقل معين وتقليل الكيانات المكررة.
---
<h1 id="Grouping-Search" class="common-anchor-header">البحث حسب المجموعات<button data-href="#Grouping-Search" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h1><p>يتيح البحث التجميعي لبرنامج Milvus تجميع نتائج البحث حسب القيم الموجودة في حقل محدد لتجميع البيانات على مستوى أعلى. على سبيل المثال، يمكنك استخدام بحث ANN الأساسي للعثور على كتب مشابهة للكتاب الذي بين يديك، ولكن يمكنك استخدام البحث حسب المجموعات للعثور على فئات الكتب التي قد تتضمن الموضوعات التي تناقشها تلك الكتب. يصف هذا الموضوع كيفية استخدام البحث حسب المجموعات بالإضافة إلى الاعتبارات الرئيسية.</p>
...
<p>عند إجراء بحث «الأقرب تقريبًا» (ANN) على مثل هذه المجموعة، قد تتضمن نتائج البحث عدة فقرات من نفس المستند، مما قد يؤدي إلى تجاهل مستندات أخرى، وهو ما قد لا يتوافق مع حالة الاستخدام المقصودة.</p>
<p><span class="img-wrapper">
...
<ul>
<li><p>إجراء بحث باستخدام الشبكة العصبية الاصطناعية (ANN) استنادًا إلى متجه الاستعلام المقدم للعثور على جميع الكيانات الأكثر تشابهًا مع الاستعلام.</p></li>
<li><p>تجميع نتائج البحث وفقًا لـ <code translate="no">group_by_field</code> المحدد، مثل <code translate="no">docId</code>.</p></li>
...
<p>بشكل افتراضي، لا يعرض "البحث المجمّع" سوى كيان واحد لكل مجموعة. إذا كنت ترغب في زيادة عدد النتائج التي يتم عرضها لكل مجموعة، فيمكنك التحكم في ذلك باستخدام المعلمتين <code translate="no">group_size</code> و <code translate="no">strict_group_size</code>.</p>
</div>
<h2 id="Perform-Grouping-Search" class="common-anchor-header">تنفيذ البحث التجميعي<button data-href="#Perform-Grouping-Search" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>يقدم هذا القسم مثالاً على الكود لتوضيح استخدام البحث التجميعي. يفترض المثال التالي أن المجموعة تتضمن حقولاً لـ <code translate="no">id</code> و <code translate="no">vector</code> و <code translate="no">chunk</code> و <code translate="no">docId</code>.</p>
...
<h2 id="Configure-group-size" class="common-anchor-header">تكوين حجم المجموعة<button data-href="#Configure-group-size" class="anchor-icon" translate="no">
    ...
    </button></h2><p>بشكل افتراضي، لا يُرجع «البحث المجمّع» سوى كيان واحد لكل مجموعة. إذا كنت ترغب في الحصول على نتائج متعددة لكل مجموعة، فقم بضبط المعلمتين <code translate="no">group_size</code> و <code translate="no">strict_group_size</code>.</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#go">   Go</a>
...
<p>في المثال أعلاه:</p>
<ul>
<li><p><code translate="no">group_size</code>: يحدد العدد المطلوب من الكيانات التي سيتم إرجاعها لكل مجموعة. على سبيل المثال، تعني تعيين القيمة <code translate="no">group_size=2</code> أن كل مجموعة (أو كل <code translate="no">docId</code>) يجب أن ترجع، في الحالة المثالية، فقرتين (أو <strong>مقطعين</strong>) من أكثر الفقرات تشابهاً. إذا لم يتم تعيين <code translate="no">group_size</code> ، فسيعرض النظام افتراضيًا نتيجة واحدة لكل مجموعة.</p></li>
<li><p><code translate="no">strict_group_size</code>: يتحكم هذا المعامل المنطقي في ما إذا كان يجب على النظام تطبيق العدد المحدد بواسطة <code translate="no">group_size</code> بصرامة أم لا. عند تفعيل <code translate="no">strict_group_size=True</code> ، سيحاول النظام تضمين العدد الدقيق للكيانات المحددة بواسطة <code translate="no">group_size</code> في كل مجموعة (على سبيل المثال، فقرتان)، ما لم تكن البيانات في تلك المجموعة غير كافية. بشكل افتراضي (<code translate="no">strict_group_size=False</code>)، يعطي النظام الأولوية لتلبية عدد المجموعات المحددة بواسطة المعلمة <code translate="no">limit</code> ، بدلاً من ضمان احتواء كل مجموعة على <code translate="no">group_size</code> كيانات. ويُعد هذا النهج أكثر كفاءة بشكل عام في الحالات التي يكون فيها توزيع البيانات غير متساوٍ.</p></li>
</ul>
...
<h2 id="Order-groups-by-a-scalar-field--Milvus-30x" class="common-anchor-header">ترتيب المجموعات حسب حقل عدد<span class="beta-tag" style="background-color:rgb(0, 179, 255);color:white" translate="no">Compatible with Milvus 3.0.x</span><button data-href="#Order-groups-by-a-scalar-field--Milvus-30x" class="anchor-icon" translate="no">
    ...
    </button></h2><p>يمكنك الجمع بين "Grouping Search" و" <code translate="no">order_by_fields</code> " لترتيب المجموعات حسب حقل عدداني. وهذا مفيد عندما تريد نتائج متنوعة عبر المجموعات، ولكنك تريد في الوقت نفسه أن تتبع المجموعات ترتيبًا ذا صلة بالأعمال، مثل السعر أو التقييم.</p>
<p>يقوم المثال التالي بتجميع نتائج البحث حسب « <code translate="no">category</code> » (المعالج)، ويعرض ما يصل إلى ثلاثة كيانات لكل مجموعة، ويصنف المجموعات المعروضة حسب « <code translate="no">price</code> » (التقييم) من الأقل إلى الأعلى.</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#javascript">   NodeJS</a>
...
<button class="copy-code-btn"></button></code></pre>
<p>في الطلب أعلاه، تعني عبارة <code translate="no">limit=20</code> أن Milvus يختار ما يصل إلى 20 مجموعة، وليس 20 كيانًا. ونظرًا لأن <code translate="no">group_size=3</code> ، يمكن أن تحتوي قائمة النتائج المسطحة على ما يصل إلى 60 كيانًا في المجموع.</p>
...
<li><p><strong>عدد المجموعات</strong>: يتحكم المعلمة <code translate="no">limit</code> <strong>في</strong> عدد المجموعات التي يتم إرجاع نتائج البحث منها، بدلاً من العدد المحدد للكيانات داخل كل مجموعة. يساعد تعيين قيمة مناسبة لـ <code translate="no">limit</code> في التحكم في تنوع البحث وأداء الاستعلام. يمكن أن يؤدي تقليل <code translate="no">limit</code> إلى خفض تكاليف الحساب إذا كانت البيانات موزعة بكثافة أو إذا كان الأداء يمثل مصدر قلق.</p></li>
<li><p><strong>الكيانات لكل مجموعة</strong>: يتحكم المعامل <code translate="no">group_size</code> في عدد الكيانات التي يتم إرجاعها لكل مجموعة. يمكن أن يؤدي تعديل المعامل <code translate="no">group_size</code> بناءً على حالة الاستخدام الخاصة بك إلى زيادة ثراء نتائج البحث. ومع ذلك، إذا كانت البيانات موزعة بشكل غير متساوٍ، فقد ترجع بعض المجموعات عددًا أقل من الكيانات مقارنةً بما هو محدد بواسطة المعامل <code translate="no">group_size</code> ، خاصةً في سيناريوهات البيانات المحدودة.</p></li>
<li><p><strong>حجم المجموعة الصارم</strong>: عند تعيين المعلمة <code translate="no">strict_group_size=True</code> ، سيحاول النظام إرجاع العدد المحدد من الكيانات (<code translate="no">group_size</code>) لكل مجموعة، ما لم تكن البيانات في تلك المجموعة غير كافية. يضمن هذا الإعداد اتساق عدد الكيانات لكل مجموعة، ولكنه قد يؤدي إلى تدهور الأداء في حالة التوزيع غير المتساوي للبيانات أو محدودية الموارد. إذا لم يكن عدد الكيانات الصارم مطلوبًا، فإن تعيين المعلمة <code translate="no">strict_group_size=False</code> يمكن أن يحسّن سرعة الاستعلام.</p></li>
<li><p>إذا كانت متجهات الاستعلام موجودة بالفعل في المجموعة المستهدفة، ففكر في استخدام <code translate="no">ids</code> بدلاً من استردادها قبل إجراء عمليات البحث. لمزيد من التفاصيل، راجع <a href="/docs/ar/primary-key-search.md">البحث</a> باستخدام <a href="/docs/ar/primary-key-search.md">المفتاح الأساسي</a>.</p></li>
</ul>
