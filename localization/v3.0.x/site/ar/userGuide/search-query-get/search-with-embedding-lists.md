...
summary: >-
  ...
  باستخدام البحث عبر EmbeddingList في الحقول الفرعية للمتجهات في StructArray في
  Milvus. استخدمه عندما يتم تمثيل كل من استعلامك والبيانات المخزنة على شكل قوائم
  من المتجهات، وترغب في إجراء استرجاع قائم على التفاعل المتأخر على مستوى
  الكيانات باستخدام مقاييس MAX_SIM*.
---
...
<p>ColBERT (arXiv: <a href="https://arxiv.org/abs/2004.12832">2004.12832</a>) هو نظام استرجاع نص-نص يوفر بحثًا فعالاً وناجعًا عن المقاطع من خلال التفاعلات المتأخرة في السياق عبر BERT. وهو يتيح الترميز المستقل للاستعلامات والمستندات على مستوى الرموز ويحسب مدى تشابهها.</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>تتراوح كل درجة تشابه جيب التمام بين <code translate="no">-1</code> و <code translate="no">1</code>. ويمكن أن تكون درجة <code translate="no">MAX_SIM_COSINE</code> النهائية أكبر من <code translate="no">1</code> لأنها تجمع بين عدة درجات تشابه قصوى على مستوى الرموز.</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>سيؤدي تشغيل البرامج النصية المذكورة أعلاه إلى تنزيل مجموعة البيانات إذا لم تكن متوفرة محليًّا. كل سجل في مجموعة البيانات يمثل صفحة من تقرير مالي. يوضح الجدول التالي بنية مجموعة البيانات هذه.</p>
<table>
<thead>
<tr><th>اسم العمود</th><th>الوصف</th></tr>
</thead>
...
<h3 id="Step-4-Create-a-collection-for-the-financial-reports-dataset" class="common-anchor-header">الخطوة 4: إنشاء مجموعة لمجموعة بيانات التقارير المالية<button data-href="#Step-4-Create-a-collection-for-the-financial-reports-dataset" class="anchor-icon" translate="no">
    ...
    </button></h3><p>بمجرد أن تصبح البيانات جاهزة، سنقوم بإنشاء مجموعة. في المجموعة، يُعد « <code translate="no">patches</code> » حقلًا من نوع StructArray. يخزن كل عنصر من عناصر Struct تضمينًا واحدًا للرقعة. للاطلاع على متطلبات الفهرسة الخاصة بالحقول الفرعية للمتجه StructArray، راجع <a href="/docs/ar/index-structarray-fields.md">فهرسة حقول StructArray</a>.</p>
<pre><code translate="no" class="language-python"><span class="hljs-keyword">from</span> pymilvus <span class="hljs-keyword">import</span> MilvusClient, DataType
