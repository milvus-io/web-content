...
title: مكرر البحث
summary: >-
  يضع «البحث باستخدام الشبكات العصبية» حدًا أقصى لعدد الكيانات التي يمكن
  استرجاعها في استعلام واحد، وقد لا يلبي استخدام «البحث باستخدام الشبكات
  العصبية» الأساسي وحده متطلبات عمليات الاسترجاع واسعة النطاق. بالنسبة لطلبات
  البحث باستخدام شبكة ANN التي يتجاوز فيها عدد topK 16,384، يُنصح بالنظر في
  استخدام SearchIterator. سيقدم هذا القسم شرحًا لكيفية استخدام SearchIterator
...
<h2 id="Create-SearchIterator" class="common-anchor-header">إنشاء SearchIterator<button data-href="#Create-SearchIterator" class="anchor-icon" translate="no">
    ...
    </button></h2><p>يوضح مقتطف الشفرة التالي كيفية إنشاء SearchIterator.</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>في الأمثلة أعلاه، قمت بتعيين عدد الكيانات المراد إرجاعها لكل عملية بحث (<strong>batch_size/batchSize</strong>) إلى 50، وإجمالي عدد الكيانات المراد إرجاعها (<strong>topK</strong>) إلى 20,000.</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>في أمثلة الكود أعلاه، قمت بإنشاء حلقة لا نهائية واستدعيت الطريقة <strong>next()</strong> داخل الحلقة لتخزين نتائج البحث في متغير، وأغلقت المكرر عندما لم تُرجع الطريقة <strong>next()</strong> أي نتيجة.</p>
