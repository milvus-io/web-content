...
<p>وبهذا المعنى، فإن «البحث التقديري» ( <code translate="no">emb_list_strategy</code> ) هو في الأساس استراتيجية لبناء الفهرس واسترجاع المرشحين. يتم تكوينها عند بناء الفهرس، وهي تحدد كيفية إنتاج مجموعة المرشحين من الشبكة العصبية الاصطناعية (ANN) في المرحلة الأولى. ثم تتحكم المعلمات الخاصة بوقت البحث، مثل «الحد الأقصى لعدد المرشحين» ( <code translate="no">retrieval_ann_ratio</code> ) و«الحد الأقصى لعدد المرات» ( <code translate="no">emb_list_rerank</code> )، في عدد المرشحين الذين يتم استرجاعهم وما إذا كان سيتم تطبيق إعادة الترتيب باستخدام MaxSim أم لا.</p>
<hr>
...
<ul>
<li><p><strong>مناسب تمامًا:</strong> أجزاء نصية قصيرة، صفوف تحتوي على عدد صغير أو معتدل من المتجهات، فصل دلالي قوي على مستوى الرموز، خطوط أساس حساسة للجودة.</p></li>
