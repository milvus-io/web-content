...
title: إدارة اللقطاتCompatible with Milvus 3.0.x
summary: >-
  تعرف على كيفية إنشاء اللقطات وإدراجها ووصفها وتثبيتها واستعادتها وإلغائها،
  بالإضافة إلى مراقبة مهام الاستعادة.
beta: Milvus 3.0.x
---
<h1 id="Manage-Snapshots" class="common-anchor-header">إدارة اللقطات<span class="beta-tag" style="background-color:rgb(0, 179, 255);color:white" translate="no">Compatible with Milvus 3.0.x</span><button data-href="#Manage-Snapshots" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h1><p>في هذا الدليل، ستتعلم كيفية إنشاء اللقطات وإدارتها، بما في ذلك</p>
<ul>
<li><a href="#Create-snapshot">إنشاء لقطة</a>،</li>
<li><a href="#List-snapshots">سرد اللقطات</a>،</li>
<li><a href="#Describe-snapshot">وصف اللقطة</a>،</li>
<li><a href="#Pinunpin-snapshot-data">تثبيت/إلغاء تثبيت بيانات اللقطة</a>،</li>
<li><a href="#Restore-snapshot">استعادة لقطة</a>،</li>
<li><a href="#Drop-snapshot">إلغاء لقطة</a>،</li>
<li><a href="#List-restoration-jobs">سرد مهام الاستعادة</a>، و</li>
<li><a href="#Get-restoration-state">الحصول على حالة الاستعادة</a>.</li>
</ul>
<h2 id="Create-snapshot" class="common-anchor-header">إنشاء لقطة<button data-href="#Create-snapshot" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>قبل إنشاء لقطة، يُنصح بإيقاف كتابة البيانات إلى المجموعة المستهدفة واستدعاء <code translate="no">flush()</code> لتجنب احتمال فقدان البيانات.</p>
<div class="alert note">
<p>استدعاء <code translate="no">flush()</code> ليس إلزاميًا ولكنه موصى به بشدة لتجنب فقدان البيانات. إذا تخطيت هذه الخطوة، فستحتوي اللقطة فقط على البيانات التي تم مسحها بالفعل.</p>
</div>
...
<p>تفترض أمثلة الكود أدناه أن لديك بالفعل مجموعة باسم <code translate="no">my_collection</code>.</p>
...
<button class="copy-code-btn"></button></code></pre>
<h2 id="List-snapshots" class="common-anchor-header">سرد اللقطات<button data-href="#List-snapshots" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>يمكنك سرد أسماء اللقطات الموجودة.</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#go">   Go</a>
...
<button class="copy-code-btn"></button></code></pre>
<h2 id="Describe-snapshot" class="common-anchor-header">وصف اللقطة<button data-href="#Describe-snapshot" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>يمكنك الحصول على معلومات تفصيلية حول لقطة معينة.</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#go">   Go</a>
...
<pre><code translate="no" class="language-bash"><span class="hljs-comment"># restful</span>
<button class="copy-code-btn"></button></code></pre>
<h2 id="Pinunpin-snapshot-data" class="common-anchor-header">تثبيت/إلغاء تثبيت بيانات اللقطة<button data-href="#Pinunpin-snapshot-data" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>أثناء الاستعادة، يمكنك تثبيت لقطة لحماية البيانات الأساسية فيها مؤقتًا من عملية جمع القمامة، وإلغاء تثبيتها لتحرير البيانات.</p>
<p>يمكنك أيضًا تعيين مدة صلاحية (TTL) لعملية التثبيت بحيث يتم تحرير البيانات المثبتة عند انتهاء هذه المدة.</p>
<div class="multipleCode">
...
<pre><code translate="no" class="language-bash"><span class="hljs-comment"># restful</span>
<button class="copy-code-btn"></button></code></pre>
<h2 id="Restore-snapshot" class="common-anchor-header">استعادة اللقطة<button data-href="#Restore-snapshot" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>يمكنك استعادة لقطة إلى مجموعة جديدة. هذه العملية غير متزامنة وتُرجع معرّف المهمة لتتبع تقدم عملية الاستعادة.</p>
<p>تستخدم عملية الاستعادة آلية <strong>نسخ المقاطع</strong> بدلاً من استيراد البيانات، وهي أكثر كفاءة لأنها</p>
<ul>
<li>تنسخ ملفات المقاطع (سجلات binlog وdeltalog وملفات الفهرس) مباشرةً من تخزين اللقطة</li>
<li>تحافظ على معرّفات الحقول ومعرّفات الفهرس لضمان التوافق مع ملفات البيانات الموجودة</li>
<li>تتجنب إعادة كتابة البيانات وإعادة بناء الفهرس، مما يؤدي إلى أوقات استعادة أسرع بشكل ملحوظ، و</li>
<li>تضمن زيادة في الأداء تتراوح بين 10 و100 ضعف مقارنةً بطرق النسخ الاحتياطي والاستعادة التقليدية</li>
</ul>
<p>لاستعادة لقطة، اتبع الخطوات التالية:</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#go">   Go</a>
...
<pre><code translate="no" class="language-bash"><span class="hljs-comment"># restful</span>
<button class="copy-code-btn"></button></code></pre>
<p>للحصول على تفاصيل حول مراقبة تقدم مهمة الاستعادة، راجع <a href="#Get-restoration-state">الحصول</a> على <a href="#Get-restoration-state">حالة الاستعادة</a>.</p>
<h2 id="Drop-snapshot" class="common-anchor-header">إزالة اللقطة<button data-href="#Drop-snapshot" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>يمكنك حذف لقطة إذا لم تعد هناك حاجة إليها. يُنصح بإزالة اللقطات القديمة بانتظام لتوفير مساحة التخزين.</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#go">   Go</a>
...
<pre><code translate="no" class="language-bash"><span class="hljs-comment"># restful</span>
<button class="copy-code-btn"></button></code></pre>
<h2 id="List-restoration-jobs" class="common-anchor-header">سرد مهام الاستعادة<button data-href="#List-restoration-jobs" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>يمكنك استخدام واجهة برمجة التطبيقات (API) هذه للحصول على قائمة باللقطات التي تم إنشاؤها بالفعل للمجموعة المستهدفة.</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#go">   Go</a>
...
<pre><code translate="no" class="language-bash"><span class="hljs-comment"># restful</span>
<button class="copy-code-btn"></button></code></pre>
<h2 id="Get-restoration-state" class="common-anchor-header">الحصول على حالة الاستعادة<button data-href="#Get-restoration-state" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>بمجرد حصولك على معرّف مهمة الاستعادة، يمكنك استخدامه لاسترجاع تقدم عملية الاستعادة.</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#go">   Go</a>
