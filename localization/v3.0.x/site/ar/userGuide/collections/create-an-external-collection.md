...
summary: >-
  الوصول إلى البيانات في AWS S3 أو Iceberg من خلال مجموعة خارجية دون نسخها إلى
  Milvus.
beta: Milvus 3.0.x
---
<h1 id="Create-an-External-Collection" class="common-anchor-header">إنشاء مجموعة خارجية<span class="beta-tag" style="background-color:rgb(0, 179, 255);color:white" translate="no">Compatible with Milvus 3.0.x</span><button data-href="#Create-an-External-Collection" class="anchor-icon" translate="no">
    ...
    </button></h1><p>المجموعة الخارجية هي نوع من مجموعات البيانات في Milvus التي تصل إلى البيانات من أنظمة التخزين الخارجية مثل AWS S3 و Iceberg دون نسخها إلى Milvus. تعمل كطبقة استعلام فوق بحيرات البيانات مع الحفاظ على التوافق مع واجهات استعلام Milvus.</p>
...
<h2 id="Limits--restrictions" class="common-anchor-header">الحدود والقيود<button data-href="#Limits--restrictions" class="anchor-icon" translate="no">
    ...
    </button></h2><p>نظرًا لأن Milvus لا يقوم بتخزين البيانات الأولية ويحتفظ فقط بتعيين بين البيانات الوصفية والبيانات الأولية، فإن المجموعات الخارجية للقراءة فقط. وهذا يعني أنه لا يمكنك إدراج البيانات وإدراجها وحذفها واستيرادها ومسحها وضغطها من جانب Milvus.</p>
<p>عند مقارنتها بالمجموعات المدارة، فإن المجموعات الخارجية لها الحدود التالية:</p>
<ul>
<li><p>لا يمكنك تعيين المفتاح الأساسي وسماته AutoID.</p></li>
<li><p>لا يمكنك تمكين الحقل الديناميكي.</p></li>
<li><p>لا يمكنك تعيين مفتاح القسم ومفتاح التجميع، لأن الأقسام غير متوفرة.</p></li>
<li><p>لا يمكنك تعريف الدوال في المخطط.</p></li>
<li><p>لا يمكنك تغيير مخطط مجموعة خارجية بمجرد إنشائها.</p></li>
<li><p>لا يمكنك استخدام مطابقة النص مع BM25.</p></li>
<li><p>يجب تشغيل عملية تحديث قبل إنشاء الفهارس.</p></li>
</ul>
...
<table>
   ...
   <tr>
     ...
     <td><p>تنسيق ملفات البيانات المصدر الهدف.</p><p>القيم الممكنة هي <code translate="no">parquet</code> و <code translate="no">vortex</code> و <code translate="no">lance-table</code> و <code translate="no">iceberg-table</code>.</p></td>
...
<table>
   ...
   <tr>
     <td><p><code translate="no">format</code></p></td>
     <td><p>تنسيق بيانات المصدر الهدف.</p><p>القيم الممكنة هي <code translate="no">parquet</code> ، <code translate="no">vortex</code> ، ، <code translate="no">lance-table</code> ، و <code translate="no">iceberg-table</code></p></td>
     <td><p><code translate="no">parquet</code></p></td>
   </tr>
   <tr>
     <td><p><code translate="no">extfs</code></p></td>
     <td><p>إعدادات نظام الملفات الخارجية</p></td>
     <td><p>--</p></td>
   </tr>
   <tr>
     <td><p><code translate="no">extfs.use_iam</code></p></td>
     <td><p>ما إذا كان يجب استخدام AWS IAM.</p><p>اضبط هذا على <code translate="no">"true"</code> لهذا الخيار.</p></td>
     <td><p><code translate="no">true</code></p></td>
   </tr>
   <tr>
...
<p>في هذه الحالة، يجب على مالك الدلو إنشاء دور IAM الذي</p>
<ul>
...
<table>
   ...
   <tr>
     <td><p><code translate="no">format</code></p></td>
     <td><p>تنسيق بيانات المصدر الهدف المصدر.</p><p>القيم الممكنة هي <code translate="no">parquet</code> ، <code translate="no">vortex</code> ، ، <code translate="no">lance-table</code> ، و <code translate="no">iceberg-table</code></p></td>
     <td><p><code translate="no">parquet</code></p></td>
   </tr>
   <tr>
     <td><p><code translate="no">extfs</code></p></td>
...
<h2 id="Step-3-Create-a-collection" class="common-anchor-header">الخطوة 3: إنشاء مجموعة<button data-href="#Step-3-Create-a-collection" class="anchor-icon" translate="no">
    ...
    </button></h2><p>بعد إضافة جميع الحقول إلى المخطط، يمكنك إنشاء المجموعة.</p>
<div class="multipleCode">
...
<pre><code translate="no" class="language-bash"><span class="hljs-comment"># restful</span>
<button class="copy-code-btn"></button></code></pre>
<h2 id="Step-4-Refresh-data" class="common-anchor-header">الخطوة 4: تحديث البيانات<button data-href="#Step-4-Refresh-data" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>بمجرد أن تصبح المجموعة جاهزة، ستحتاج إلى إجراء تحديث لمزامنة البيانات الوصفية من بياناتك إلى Milvus.</p>
<div class="multipleCode">
   <a href="#python">بايثون</a> <a href="#java">جافا جافا</a> <a href="#go">جو</a> <a href="#javascript">نودجيس</a> <a href="#bash">cURL</a></div>
...
<pre><code translate="no" class="language-bash"><span class="hljs-comment"># restful</span>
<button class="copy-code-btn"></button></code></pre>
<p>تكون عملية التحديث غير متزامنة، لذا تحتاج إلى إعداد تكرار لمراقبة تقدمها.</p>
<div class="alert note">
<ul>
<li><p>تفحص عملية التحديث البيانات الوصفية لملفات البيانات وتُنشئ ملفات البيان وفقًا لذلك. تستغرق عادةً 150-250 مللي ثانية.</p></li>
...
<li><p>لا يمكنك فهرسة مجموعة خارجية إلا بعد اكتمال التحديث. ومع ذلك، فإن طريقة إنشاء الفهارس هي نفس طريقة إنشاء الفهارس الخاصة بمجموعة مُدارة.</p></li>
<li><p>يؤدي التحديث الذي يتطلب إزالة جميع البيانات الوصفية النشطة دون أي عمليات إدراج إلى الرفض.</p></li>
</ul>
</div>
<h2 id="Follow-ups" class="common-anchor-header">عمليات المتابعة<button data-href="#Follow-ups" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>بمجرد إجراء عملية تحديث على المجموعة الخارجية وإتاحة ملفات البيانات، يمكنك إنشاء فهارس وتحميل/إصدار مجموعات وإجراء عمليات بحث واستعلامات تشابه في المجموعة الخارجية كما تفعل في أي مجموعات مُدارة.</p>
