...
summary: توفر عملية «upsert» طريقة ملائمة لإدراج الكيانات أو تحديثها في مجموعة.
---
...
<h2 id="Overview" class="common-anchor-header">نظرة عامة<button data-href="#Overview" class="anchor-icon" translate="no">
    ...
    </button></h2><p>يمكنك استخدام " <code translate="no">upsert</code> " لإدراج كيان جديد أو تحديث كيان موجود، اعتمادًا على ما إذا كان المفتاح الأساسي المقدم في طلب "upsert" موجودًا في المجموعة أم لا. إذا لم يتم العثور على المفتاح الأساسي، تتم عملية الإدراج. خلاف ذلك، سيتم تنفيذ عملية التحديث. تعتبر التحديثات الجزئية على مجموعات " <code translate="no">autoID</code> " استثناءً: فهي تقوم بتحديث الكيانات الموجودة فقط، كما هو موضح أدناه.</p>
...
<h3 id="Upsert-in-override-mode" class="common-anchor-header">عملية «upsert» في وضع «التجاوز»<button data-href="#Upsert-in-override-mode" class="anchor-icon" translate="no">
    ...
    </button></h3><p>يجمع طلب الإدراج أو التحديث الذي يعمل في وضع التجاوز بين الإدراج والحذف. عند تلقي طلب « <code translate="no">upsert</code> » لكيان موجود، يقوم Milvus بإدراج البيانات الموجودة في حمولة الطلب وحذف الكيان الموجود الذي يحمل المفتاح الأساسي الأصلي المحدد في البيانات في نفس الوقت.</p>
...
<p>بالنسبة للحقول التي تم تمكين ميزة " <code translate="no">nullable</code> " (إدراج إذا لم يكن موجودًا) لها، يمكنك حذفها من طلب " <code translate="no">upsert</code> " (إدراج/تحديث) إذا لم تتطلب أي تحديثات.</p>
<h3 id="Upsert-in-merge-mode--Milvus-v262+" class="common-anchor-header">Eupsert في وضع الدمج<span class="beta-tag" style="background-color:rgb(0, 179, 255);color:white" translate="no">Compatible with Milvus v2.6.2+</span><button data-href="#Upsert-in-merge-mode--Milvus-v262+" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<p>قم بتعيين <code translate="no">partial_update=True</code> (تحديث أو إدراج) وحدد المفتاح الأساسي والحقول التي تريد تحديثها.</p>
<p>يسترد Milvus الكيان الموجود باستخدام استعلام ذي اتساق قوي، ويدمج تغييراتك مع البيانات المخزنة، ويقوم بإدراج الكيان المدمج، وحذف الكيان القديم.</p>
<p>إذا كان المفتاح الأساسي غير موجود، فإن النتيجة تعتمد على ما إذا كان " <code translate="no">autoID</code> " ممكّنًا أم لا:</p>
<ul>
...
<p>لنفترض أنك قمت بتمكين المفتاح الديناميكي في مجموعة الأمثلة، وأن أزواج القيم والمفاتيح في الحقل الديناميكي لكيان ما مشابهة لـ <code translate="no">{&quot;author&quot;: &quot;John&quot;, &quot;year&quot;: 2020, &quot;tags&quot;: [&quot;fiction&quot;]}</code>.</p>
...
<p>على سبيل المثال، إذا كانت البيانات المتضمنة في الطلب هي <code translate="no">{&quot;author&quot;: &quot;Jane&quot;, &quot;genre&quot;: &quot;fantasy&quot;}</code> ، فسيتم تحديث أزواج القيم والمفاتيح في الحقل الديناميكي للكيان الهدف لتتوافق مع ذلك.</p></li>
...
<p>بشكل افتراضي، يتبع حقل <code translate="no">ARRAY</code> في وضع الدمج دلالات <strong>REPLACE</strong>: حيث تحل القيمة المضمنة في الطلب محل المصفوفة الحالية. لإجراء تحديثات أكثر دقة، يدعم Milvus v2.6.17 والإصدارات الأحدث أيضًا عاملين:</p>
<ul>
<li><p><code translate="no">ARRAY_APPEND</code> يُلحق العناصر الموجودة في حمولة الطلب بالمصفوفة الحالية.</p></li>
<li><p><code translate="no">ARRAY_REMOVE</code> إزالة كل عنصر من المصفوفة الحالية يطابق قيمة في حمولة الطلب.</p></li>
</ul>
<p>للاطلاع على صيغة المشغل وأنواع العناصر المدعومة والقيود الأخرى، راجع <a href="/docs/ar/upsert-entities.md#Upsert-ARRAY-fields-in-merge-mode">«Upsert حقول ARRAY في وضع الدمج</a>».</p></li>
<li><p><strong>إجراء عملية Upsert لحقل StructArray.</strong></p>
<p>يؤدي إجراء عملية «Upsert» لحقل StructArray في كيان ما إلى استبدال قيمة الحقل. للقيام بذلك، تحتاج إلى توفير قائمة من القواميس، يحتوي كل منها على جميع الحقول الفرعية المحددة في مخطط البنية، حتى عند إجراء عملية «Upsert» في وضع الدمج.</p>
...
<h2 id="Upsert-entities-in-merge-mode--Milvus-v262+" class="common-anchor-header">إدراج أو تحديث الكيانات في وضع الدمج<span class="beta-tag" style="background-color:rgb(0, 179, 255);color:white" translate="no">Compatible with Milvus v2.6.2+</span><button data-href="#Upsert-entities-in-merge-mode--Milvus-v262+" class="anchor-icon" translate="no">
    ...
    </button></h2><p>يُحدّث المثال التالي حقل « <code translate="no">issue</code> » فقط للكيانات ذات المفاتيح الأساسية <code translate="no">1</code> و <code translate="no">2</code> في <code translate="no">my_collection</code>. قبل تشغيله، تأكد من وجود الكيانين بالفعل. وتحتفظ الحقول الأخرى الخاصة بهما بقيمها الحالية.</p>
<div class="alert note">
<p>عند إجراء عملية «upsert» في وضع الدمج، تأكد من أن الكيانات المعنية في الطلب تحتوي على نفس مجموعة الحقول. لنفترض أن هناك كيانين أو أكثر سيتم إجراء عملية «upsert» عليهما، كما هو موضح في مقتطف الشفرة التالي، فمن المهم أن تتضمن هذه الكيانات حقولًا متطابقة لمنع حدوث أخطاء والحفاظ على سلامة البيانات.</p>
</div>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
...
<p>لنفترض أن الكيان ذي المفتاح الأساسي <code translate="no">1</code> يحتوي بالفعل على <code translate="no">tags = [&quot;new&quot;, &quot;trial&quot;]</code>. قبل ظهور عوامل التحديث الجزئي، كانت إضافة العنصر <code translate="no">&quot;premium&quot;</code> إلى المصفوفة تتطلب إجراء عملية «إدراج أو تحديث» (upsert) للمصفوفة البديلة بالكامل:</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#javascript">   NodeJS</a>
...
<button class="copy-code-btn"></button></code></pre>
<p>مع <code translate="no">ARRAY_APPEND</code> ، ما عليك سوى إرسال العنصر المراد إضافته:</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#javascript">   NodeJS</a>
...
<li><p>يجب أن تتطابق قيم الحمولة مع <code translate="no">element_type</code> للحقل <code translate="no">ARRAY</code> المستهدف. على سبيل المثال، إذا كان الحقل المستهدف هو <code translate="no">ARRAY&lt;VARCHAR&gt;</code> ، فيجب أن تحتوي الحمولة على قيم سلاسل نصية.</p></li>
...
<li><p>بعد عملية « <code translate="no">ARRAY_APPEND</code> »، يجب ألا يتجاوز طول المصفوفة الناتجة الحد الأقصى المسمى « <code translate="no">max_capacity</code> » الخاص بالحقل.</p></li>
<li><p>عمليات الإدراج والتحديث المتزامنة لنفس الكيان ليست متجانسة عبر الطلبات. إذا قام طلبان بتحديث نفس الحقل <code translate="no">ARRAY</code> في نفس الوقت، فقد تقوم عملية الكتابة اللاحقة بالكتابة فوق العملية السابقة. استخدم التنسيق على مستوى التطبيق إذا كنت بحاجة إلى الحفاظ على جميع التغييرات المتزامنة.</p></li>
</ul>
...
<h2 id="Upsert-StructArray-field-in-merge-mode" class="common-anchor-header">إدراج أو تحديث حقل StructArray في وضع الدمج<button data-href="#Upsert-StructArray-field-in-merge-mode" class="anchor-icon" translate="no">
    ...
    </button></h2><p>يؤدي إجراء «Upsert» لحقل StructArray في كيان ما إلى استبدال قيمة الحقل. وهذا يعني أنك تحتاج إلى تضمين جميع الحقول الفرعية المحددة في مخطط البنية (struct schema) عند إجراء «Upsert» لحقل StructArray.</p>
<p>يوضح المثال التالي كيفية إجراء عملية «إدراج أو تحديث» لحقل « <code translate="no">chunks</code> » في وضع الدمج، وهو حقل StructArray يحتوي على 6 حقول فرعية. عند اكتمال العملية، يتم تعيين حقل « <code translate="no">chunks</code> » للكيان ذي المعرف 1 إلى المصفوفة التي تحتوي على البنيات المكونة من عنصرين المحددة في الطلب.</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#javascript">   NodeJS</a>
