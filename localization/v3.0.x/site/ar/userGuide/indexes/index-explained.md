...
<h1 id="Index-Explained" class="common-anchor-header">شرح الفهرس<button data-href="#Index-Explained" class="anchor-icon" translate="no">
    ...
    </button></h1><p>الفهرس هو بنية إضافية تُبنى فوق البيانات. وتعتمد بنيته الداخلية على خوارزمية البحث عن أقرب جار تقريبي المستخدمة. يعمل الفهرس على تسريع عملية البحث، ولكنه يستهلك وقتًا إضافيًا في المعالجة المسبقة ومساحة إضافية وذاكرة وصول عشوائي (RAM) أثناء البحث. علاوة على ذلك، يؤدي استخدام الفهرس عادةً إلى انخفاض معدل الاسترجاع (على الرغم من أن التأثير ضئيل، إلا أنه لا يزال مهمًا). لذلك، تشرح هذه المقالة كيفية تقليل تكاليف استخدام الفهرس إلى الحد الأدنى مع تعظيم فوائده.</p>
<h2 id="Overview" class="common-anchor-header">نظرة عامة<button data-href="#Overview" class="anchor-icon" translate="no">
    ...
    </button></h2><p>في Milvus، تكون الفهارس خاصة بالحقول، وتختلف أنواع الفهارس القابلة للتطبيق وفقًا لأنواع بيانات الحقول المستهدفة. بصفته قاعدة بيانات متجهة احترافية، يركز Milvus على تحسين أداء عمليات البحث المتجهة والتصفية القياسية، ولهذا السبب يوفر أنواعًا متنوعة من الفهارس.</p>
...
<h3 id="Summary" class="common-anchor-header">ملخص<button data-href="#Summary" class="anchor-icon" translate="no">
    ...
    </button></h3><p>تسمح هذه البنية المتدرجة — التي تتألف من التصفية الأولية عبر هياكل البيانات، والحساب الفعال من خلال التكمية، وضبط الدقة عبر عملية التحسين — لـ Milvus بتحسين التوازن بين الدقة والأداء بشكل تكيفي.</p>
...
<ul>
<li><p>عادةً ما تتفوق<strong>أنواع الفهارس القائمة على الرسوم البيانية</strong> على <strong>متغيرات IVF</strong> من حيث <strong>QPS</strong>.</p></li>
...
<li><p>يساعد استخدام محركات الأقراص الثابتة لجزء من الفهرس (كما في <strong>DiskANN</strong>) في إدارة مجموعات البيانات الكبيرة، ولكنه يؤدي أيضًا إلى اختناقات محتملة في IOPS.</p></li>
</ul>
...
<ul>
<li><p>بالنسبة لعملية البحث ذات top-K صغير (على سبيل المثال، 2,000) التي تتطلب معدل استرجاع عالي، تتفوق أنواع الفهارس القائمة على الرسوم البيانية على متغيرات IVF.</p></li>
...
<li><p>بالنسبة لعملية البحث التي يكون فيها top-K متوسط الحجم ونسبة التصفية عالية، تُعد متغيرات IVF خيارات أفضل.</p></li>
</ul>
...
<table>
   ...
   <tr>
     ...
     <td><p>استخدم HNSW للحصول على معدل استرجاع منخفض (<code translate="no">k</code>) / معدل استرجاع مرتفع (high recall).</p></td>
   </tr>
   <tr>
...
<p>يركز هذا القسم على حساب استهلاك الذاكرة لنوع معين من الفهارس ويتضمن العديد من التفاصيل الفنية. يمكنك تخطي هذا القسم بأمان إذا لم يكن ضمن اهتماماتك.</p>
</div>
...
<h3 id="IVF-index-memory-usage" class="common-anchor-header">استخدام ذاكرة فهرس IVF<button data-href="#IVF-index-memory-usage" class="anchor-icon" translate="no">
    ...
    </button></h3><p>تحقق فهارس IVF التوازن بين كفاءة الذاكرة وأداء البحث عن طريق تقسيم البيانات إلى مجموعات. فيما يلي تفصيل للذاكرة المستخدمة من قبل مليون متجه ذي 128 بُعدًا تم فهرستها باستخدام متغيرات IVF.</p>
<ol>
...
<p>يتم تعيين كل تضمين متجه إلى مجموعة وتخزينه كمعرفات عددية. بالنسبة لـ 2,000 مجموعة، يكفي عدد صحيح مكون من بايتين. يمكن حساب استخدام الذاكرة على النحو التالي:</p>
...
<ul>
<li><p>استخدام PQ مع 8 مكمّنات فرعية</p>
...
<p>يسرد الجدول التالي الاستخدام التقديري للذاكرة مع التكوينات المختلفة:</p>
<p><table>
<tr>
<th><p>التكوين</p></th>
...
<p>عند استخدام HNSW لفهرسة مليون تضمين متجه ذي 128 بُعدًا، ستكون الذاكرة الإجمالية المستخدمة هي <strong>128 ميغابايت (الرسم البياني) + 512 ميغابايت (المتجهات) = 640 ميغابايت</strong>.</p></li>
<li><p><strong>احسب الضغط الناتج عن التكمية.</strong></p>
...
<h3 id="Other-considerations" class="common-anchor-header">اعتبارات أخرى<button data-href="#Other-considerations" class="anchor-icon" translate="no">
    ...
    </button></h3><p>بينما تعمل الفهارس القائمة على IVF والرسوم البيانية على تحسين استخدام الذاكرة من خلال التكمية، فإن الملفات المعينة في الذاكرة (mmap) وDiskANN تعالج السيناريوهات التي تتجاوز فيها مجموعات البيانات سعة ذاكرة الوصول العشوائي (RAM) المتاحة.</p>
<h4 id="DiskANN" class="common-anchor-header">DiskANN</h4><p>DiskANN هو فهرس قائم على مخطط Vamana يربط نقاط البيانات من أجل التنقل الفعال أثناء البحث مع تطبيق PQ لتقليل حجم المتجهات وتمكين الحساب التقريبي السريع للمسافة بين المتجهات.</p>
