...
<p>وبدلاً من مطالبة المستخدم بتحويل أحداثه الهيكلية الذرية إلى نماذج بيانات مسطحة، يقدم ميلفوس مصفوفة الهياكل، حيث يمكن لكل هيكل في المصفوفة أن يحتوي على كميات قياسية ومتجهات مع الحفاظ على التكامل الدلالي.</p>
...
<ul>
<li><p><strong>البيانات الهرمية</strong>: كيانات أصلية ذات سجلات فرعية متعددة، مثل كتاب يحتوي على العديد من الأجزاء النصية، أو مقطع فيديو يحتوي على العديد من الإطارات المشروحة.</p></li>
...
<p>على عكس الحلول التقليدية التي تخزن نقاط JSON أو تقسم البيانات عبر مجموعات متعددة، توفر مصفوفة الهياكل تطبيقًا أصليًا للمخطط وفهرسة المتجهات وتخزينًا فعالاً داخل Milvus.</p>
...
<h3 id="Index-vector-fields-in-Structs" class="common-anchor-header">فهرسة الحقول المتجهة في الهياكل<button data-href="#Index-vector-fields-in-Structs" class="anchor-icon" translate="no">
    ...
    </button></h3><p>تعد الفهرسة إلزامية للحقول المتجهة، بما في ذلك كل من الحقول المتجهة في مجموعة وتلك المحددة في بنية. بالنسبة لحقول المتجهات في هيكل، يجب استخدام <code translate="no">AUTOINDEX</code> أو <code translate="no">HNSW</code> كنوع الفهرس و <code translate="no">MAX_SIM</code> سلسلة كنوع القياس.</p>
...
<h2 id="A-real-world-example-Modeling-the-CoVLA-dataset-for-autonomous-driving" class="common-anchor-header">مثال واقعي: نمذجة مجموعة بيانات CoVLA للقيادة الذاتية<button data-href="#A-real-world-example-Modeling-the-CoVLA-dataset-for-autonomous-driving" class="anchor-icon" translate="no">
    ...
    </button></h2><p>توفر مجموعة بيانات الرؤية-اللغة-الحركة الشاملة (CoVLA)، التي قدمتها شركة <a href="https://tur.ing/posts/s1QUA1uh">Turing Motors</a> وتم قبولها في المؤتمر الشتوي لتطبيقات الرؤية الحاسوبية (WACV) لعام 2025، أساساً غنياً لتدريب وتقييم نماذج الرؤية-اللغة-الإجراء (VLA) في القيادة الذاتية. لا تحتوي كل نقطة بيانات، والتي عادةً ما تكون مقطع فيديو، على مدخلات بصرية أولية فحسب، بل تحتوي أيضاً على تعليقات منظمة تصف</p>
<ul>
...
<li><p>جميع السمات الأخرى للإطار، مثل <code translate="no">road</code> و <code translate="no">weather</code> و و <code translate="no">is_tunnel</code> و <code translate="no">has_pedestrain</code> و وغيرها...</p></li>
</ul></li>
<li><p><code translate="no">traffic_lights</code> هو جسم JSON يحتوي على جميع إشارات المرور الضوئية المحددة في الإطار الحالي.</p></li>
...
<p>على الرغم من أن السيارة الأمامية لا تتضمن تضم تضمين متجه، إلا أنك لا تزال بحاجة إلى تضمينها كمصفوفة من Struct لأن حجم البيانات يتجاوز الحد الأقصى لحقل JSON.</p>
<p></div></p>
