...
<h1 id="Nullable-Fields" class="common-anchor-header">الحقول القابلة للقيمة الفارغة<button data-href="#Nullable-Fields" class="anchor-icon" translate="no">
    ...
    </button></h1><p>يدعم Milvus الحقول القابلة للفراغ، والتي تسمح بغياب قيمة الحقل أو تعيينها صراحةً إلى NULL. يتم تعريف قابلية الفراغ على مستوى المخطط وتُطبق بشكل متسق عبر عمليات استيعاب البيانات والفهرسة والبحث والاستعلام.</p>
...
<li><p>لا تدعم الحقول المتجهة التي تسمح بقيم NULL تعبيرات التصفية من نوع " <code translate="no">IS NULL</code> " أو " <code translate="no">IS NOT NULL</code> ". لا يمكنك تصفية الكيانات بشكل صريح بناءً على ما إذا كانت قيمة الحقل المتجه NULL أم لا.</p></li>
<li><p>لا تدعم حقول<a href="/docs/ar/array-of-structs.md">"مصفوفة الهياكل"</a> القيم NULL. لا يمكنك وضع علامة "قابلة للقيمة NULL" على حقل "مصفوفة الهياكل" أو أي حقل متداخل بداخله.</p></li>
<li><p>يتم تعريف السمة «قابلة للقيمة NULL» عند إنشاء الحقل ولا يمكن تعديلها لاحقًا. لا يمكنك تمكين أو تعطيل إمكانية القيمة NULL لحقل موجود.</p></li>
<li><p>لا يمكن استخدام الحقول التي تم وضع علامة "قابلة للقيمة الفارغة" عليها كمفاتيح تقسيم. يجب أن تحتوي حقول مفاتيح التقسيم دائمًا على قيم صالحة وغير فارغة. لمزيد من المعلومات، راجع <a href="/docs/ar/use-partition-key.md">استخدام مفتاح التقسيم</a>.</p></li>
</ul>
<h2 id="What-is-a-nullable-field" class="common-anchor-header">ما هو الحقل القابل للقيمة NULL؟<button data-href="#What-is-a-nullable-field" class="anchor-icon" translate="no">
    ...
    </button></h2><p>في Milvus، يتم التحكم في ما إذا كان يُسمح لحقل ما بتخزين قيمة NULL من خلال سمة حقل على مستوى المخطط تسمى « <code translate="no">nullable</code> ».</p>
...
<li>تعيين الحقل صراحةً على NULL (على سبيل المثال، <code translate="no">None</code> في لغة Python).</li>
</ul>
<p>إذا لم يتم تعريف الحقل على أنه قابل للفراغ (السلوك الافتراضي)، فيجب أن توفر كل كيان قيمة صالحة لهذا الحقل. سيؤدي حذف الحقل أو تعيين قيمة NULL صراحةً إلى فشل عملية الإدراج أو الاستيراد.</p>
<p>يتم دعم السمة nullable لكل <strong>من الحقول القياسية والمتجهة</strong> في مخطط المجموعة. ومع ذلك، لا تدعم حقول Array of Structs السمة nullable.</p>
<div class="alert note">
<p>تحدد خاصية القابلية للفراغ ما إذا كان من الممكن أن تكون قيمة الحقل مفقودة؛ وهي لا تحدد القيمة التي يتم استخدامها عند فقدان الحقل.</p>
<ul>
...
<h2 id="Define-a-nullable-field-in-the-collection-schema" class="common-anchor-header">تحديد حقل قابل للفراغ في مخطط المجموعة<button data-href="#Define-a-nullable-field-in-the-collection-schema" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>لاستخدام الحقول القابلة للقيمة الفارغة، يجب تمكين السمة "nullable" عند تعريف مخطط المجموعة.</p>
...
<p>للتوضيح، تركز الأمثلة التالية على حقل متجه قابل للقيمة الفارغة (<code translate="no">embedding</code>). يعد تعريف الحقول القياسية القابلة للقيمة الفارغة أمرًا اختياريًا وليس مطلوبًا لمتابعة بقية هذا الدليل.</p>
<p><details>
<summary>اختياري: تعريف حقل عدد قياسي قابل للقيمة NULL</summary></p>
<p>يمكن أيضًا تعريف الحقول القياسية على أنها قابلة للقيمة NULL باستخدام نفس السمة <code translate="no">nullable</code> واتباع نفس القواعد أثناء الاستيعاب. على سبيل المثال:</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#javascript">   NodeJS</a>
...
<h2 id="Insert-behavior-with-missing-or-NULL-values" class="common-anchor-header">سلوك الإدراج مع القيم المفقودة أو القيم NULL<button data-href="#Insert-behavior-with-missing-or-NULL-values" class="anchor-icon" translate="no">
    ...
    </button></h2><p>بمجرد تعريف حقل على أنه قابل للقيمة null في مخطط المجموعة، يسمح Milvus بأن تكون قيمة الحقل مفقودة أو محددة صراحةً بقيمة NULL أثناء استيعاب البيانات.</p>
...
<li>الكيان <strong>ذو المعرف = 3</strong> يغفل حقل <code translate="no">embedding</code> تمامًا؛ ويقوم Milvus بتخزينه كقيمة NULL.</li>
</ul>
...
<ul>
<li>تُضاف إلى الفهرس الكيانات التي تحتوي على قيم غير NULL فقط.</li>
<li>يتم تخطي الكيانات ذات القيم NULL ولا تشارك في إنشاء الفهرس.</li>
</ul>
<p>بالنسبة لحقل متجه قابل للقيمة NULL، يعني هذا أن الكيانات التي تحتوي على متجهات صالحة هي فقط التي تصبح قابلة للبحث عن طريق تشابه المتجهات.</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#javascript">   NodeJS</a>
...
<p>في هذه المرحلة:</p>
<ul>
<li>يتم فهرسة الكيانات ذات قيم التضمين الصالحة وتكون جاهزة للبحث.</li>
<li>تظل الكيانات التي تكون قيم التضمين الخاصة بها NULL ضمن المجموعة، ولكنها لا تُدرج في فهرس المتجهات.</li>
</ul>
<h2 id="Search-behavior-with-nullable-fields" class="common-anchor-header">سلوك البحث مع الحقول التي يمكن أن تكون قيمها فارغة<button data-href="#Search-behavior-with-nullable-fields" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>عند إجراء عمليات بحث على حقل قابل للقيمة الفارغة، يقوم Milvus بتقييم الكيانات التي تحتوي على قيم غير فارغة فقط في الحقل المستخدم في البحث. يتم تخطي الكيانات التي يكون حقلها المتجه NULL تلقائيًا.</p>
...
<li>إذا كان عدد المتجهات الصالحة أقل من عدد الكيانات المطلوبة ( <code translate="no">topK</code> ) (<code translate="no">limit</code>)، فقد يعرض Milvus نتائج أقل من <code translate="no">limit</code>.</li>
</ul>
<p>يقوم المثال التالي بإجراء بحث متجهي في حقل المتجهات القابل للقيمة NULL <code translate="no">embedding</code>:</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#javascript">   NodeJS</a>
...
<p>في هذا البحث:</p>
<ul>
<li>يتم اعتبار الكيانات التي تحتوي على قيم غير فارغة لـ <code translate="no">embedding</code> كمرشحات فقط.</li>
<li>يتم استبعاد الكيانات التي تحتوي على قيم NULL لـ <code translate="no">embedding</code> من التقييم.</li>
<li>يعتمد عدد النتائج المعروضة على عدد المتجهات الصالحة الموجودة في المجموعة.</li>
</ul>
...
<p>على سبيل المثال، في حالة وجود حقل سكالي قابل للقيمة NULL <code translate="no">age</code> ، فإن التصفية التالية تختار الكيانات التي يزيد عمرها عن 18:</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#javascript">   NodeJS</a>
...
<p>وبالمثل، لا تتطابق اختبارات المساواة مع القيم NULL. على سبيل المثال:</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#javascript">   NodeJS</a>
...
<tbody>
<tr><td>نعم</td><td>نعم (غير NULL)</td><td>NULL أو تم تجاهله</td><td>يستخدم القيمة الافتراضية</td></tr>
...
<tr><td>لا</td><td>لا</td><td>NULL أو تم تجاهله</td><td>يُحدث خطأً</td></tr>
<tr><td>لا</td><td>نعم (القيمة الافتراضية NULL)</td><td>NULL أو تم حذفه</td><td>يُحدث خطأً</td></tr>
</tbody>
</table>
<p><strong>النقاط الرئيسية:</strong></p>
<ul>
<li>عندما يكون للحقل قيمة افتراضية غير NULL، يتم استخدام تلك القيمة بغض النظر عما إذا كان الخيار " <code translate="no">nullable</code> " (تحديد القيمة الافتراضية) ممكّنًا أم لا.</li>
<li>عندما تكون ميزة " <code translate="no">nullable=True</code> " مفعّلة دون تعيين قيمة افتراضية، يخزن الحقل القيمة NULL.</li>
<li>عندما يتم تعيين " <code translate="no">nullable=False</code> " دون تعيين قيمة افتراضية، يفشل الإدراج ويظهر خطأ.</li>
<li>يُعد تعيين قيمة افتراضية NULL لحقل غير قابل للقيمة NULL غير صحيح ويؤدي إلى ظهور خطأ.</li>
</ul>
