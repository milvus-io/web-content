...
<tr><td><code translate="no">quality_score</code></td><td><code translate="no">FLOAT</code></td><td>النتيجة على مستوى المقطع المستخدمة في أمثلة التصفية القياسية والنطاق.</td></tr>
...
<p>لا يقبل الحقل المتجه أو الحقل الفرعي المتجه سوى مؤشر واحد. إذا كنت بحاجة إلى كل من البحث في قائمة التضمين (EmbeddingList) والبحث على مستوى العنصر، فحدد حقلين فرعيين متجهين منفصلين. في هذا المثال، يُستخدم " <code translate="no">chunks[emb_list_vector]</code> " للبحث في قائمة التضمين (EmbeddingList)، ويُستخدم " <code translate="no">chunks[emb]</code> " للبحث على مستوى العنصر.</p>
</div>
<h2 id="Supported-subfield-data-types" class="common-anchor-header">أنواع بيانات الحقول الفرعية المدعومة<button data-href="#Supported-subfield-data-types" class="anchor-icon" translate="no">
    ...
    </button></h2><p>يخزن حقل StructArray قيمة مصفوفة واحدة لكل حقل فرعي من نوع Struct. عند تعريف مخطط Struct، اختر أنواع الحقول الفرعية من العائلات القياسية والمتجهة المدعومة.</p>
<table>
<thead>
...
<p>للحصول على معلومات حول الدعم الخاص بالإصدارات وسلوك القيم القابلة للصفر والقيود الأخرى، راجع " <a href="/docs/ar/structarray-limits.md">قيود StructArray</a>".</p>
<h2 id="Create-a-collection-with-a-StructArray-field" class="common-anchor-header">إنشاء مجموعة تحتوي على حقل StructArray<button data-href="#Create-a-collection-with-a-StructArray-field" class="anchor-icon" translate="no">
    ...
    </button></h2><p>لإنشاء حقل StructArray، حدد أولاً مخطط Struct الذي يستخدمه كل عنصر. ثم أضف حقل Array وقم بتعيين نوع العنصر الخاص به إلى Struct.</p>
<ol>
...
<h2 id="Understand-StructArray-field-paths" class="common-anchor-header">فهم مسارات حقل StructArray<button data-href="#Understand-StructArray-field-paths" class="anchor-icon" translate="no">
    ...
    </button></h2><p>بعد إنشاء حقل StructArray، قم بالإشارة إلى حقوله الفرعية باستخدام صيغة مسار <code translate="no">structArray[subfield]</code>. استخدم هذه الصيغة عند إنشاء الفهارس، أو البحث في الحقول الفرعية المتجهة، أو إخراج الحقول الفرعية، أو إنشاء مرشحات قيمية.</p>
<table>
<thead>
...
<tr><td><code translate="no">chunks[section]</code></td><td>تسمية القسم لكل مقطع.</td><td>تصفية القيم القياسية.</td></tr>
...
<tbody>
<tr><td>يُستخدم Struct كنوع عنصر Array.</td><td>قم بإنشاء حقل StructArray كحقل Array باستخدام <code translate="no">element_type=STRUCT</code>. لا تقم بإنشاء Struct كحقل مجموعة من المستوى الأعلى.</td></tr>
<tr><td>تشترك جميع العناصر في مخطط واحد.</td><td>يتبع كل عنصر Struct في حقل StructArray نفسه مخطط Struct المحدد لهذا الحقل.</td></tr>
...
<tr><td>تحتاج الحقول الفرعية المتجهة إلى فهارس قبل البحث.</td><td>قم بإنشاء فهارس على مسارات مثل <code translate="no">chunks[emb_list_vector]</code> أو <code translate="no">chunks[emb]</code> قبل تشغيل البحث المتجه.</td></tr>
...
<li><p>معاملة الحقول الفرعية المتجهة كمدخلات مسلّمة قياسية من نوع <code translate="no">$[...]</code>. استخدام الحقول الفرعية المتجهة للبحث المتجه، والحقول الفرعية القياسية للمسلّمات القياسية.</p></li>
<li><p>افتراض إمكانية إضافة حقول فرعية جديدة إلى حقل StructArray موجود بعد إنشاء الحقل.</p></li>
