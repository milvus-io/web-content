...
<h1 id="Insert-Data-into-StructArray-Fields" class="common-anchor-header">إدراج البيانات في حقول StructArray<button data-href="#Insert-Data-into-StructArray-Fields" class="anchor-icon" translate="no">
    ...
    </button></h1><p>أدخل البيانات في حقل StructArray عندما يحتوي كل كيان على قائمة مرتبة من العناصر المنظمة. في حمولة الإدخال، يُمثَّل حقل StructArray كمصفوفة من الكائنات. يمثل كل كائن عنصر Struct واحدًا ويستخدم أسماء الحقول الفرعية لـ Struct المُعرَّفة في مخطط المجموعة.</p>
<p>تستخدم هذه الصفحة مجموعة « <code translate="no">tech_articles</code> » المأخوذة من <a href="/docs/ar/create-structarray-field.md">«إنشاء حقل StructArray</a>». كل كيان يمثل مقالًا تقنيًّا، ويخزن حقل « <code translate="no">chunks</code> » أجزاء المقال كعناصر Struct.</p>
...
<tr><td><code translate="no">quality_score</code></td><td><code translate="no">FLOAT</code></td><td>النتيجة على مستوى المقطع.</td></tr>
...
<h2 id="Understand-the-insert-payload-shape" class="common-anchor-header">فهم شكل حمولة الإدراج<button data-href="#Understand-the-insert-payload-shape" class="anchor-icon" translate="no">
    ...
    </button></h2><p>تعد قيمة « <code translate="no">chunks</code> » (قائمة التضمين) مصفوفة من عناصر «Struct». كل عنصر هو كائن تكون مفاتيحه أسماء حقول فرعية.</p>
...
<h2 id="Insert-rows" class="common-anchor-header">إدراج صفوف<button data-href="#Insert-rows" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<div class="alert note">
<p>تحذير
تتوفر حقول StructArray القابلة للتعيين بقيمة null فقط في Milvus الإصدار 3.0.x. إذا أضفت حقل StructArray ديناميكيًا إلى مجموعة موجودة، فيجب أن يكون الحقل المضاف قابلاً للتعيين بقيمة null، ويجب أن تُرجع الكيانات الموجودة قيمة " <code translate="no">null</code> " للحقل الجديد عبر جميع حقوله الفرعية.</p>
</div>
...
<button class="copy-code-btn"></button></code></pre>
<p>استخدم مسارات حقول StructArray، مثل <code translate="no">chunks[text]</code> ، فقط عند الاستعلام أو البحث أو التصفية أو إنشاء الفهارس. يجب أن تستمر حمولات الإدراج في استخدام الكائنات المتداخلة تحت <code translate="no">chunks</code>.</p>
...
<tbody>
<tr><td>استخدم مصفوفة من الكائنات لحقل StructArray.</td><td>قيمة <code translate="no">chunks</code> عبارة عن قائمة، وكل عنصر في القائمة هو عنصر Struct.</td></tr>
...
<tr><td>تطابق أبعاد المتجهات.</td><td>يجب أن تتطابق قيم المتجهات مع <code translate="no">dim</code> المُعدة لحقولها الفرعية المتجهة.</td></tr>
<tr><td>يجب مراعاة <code translate="no">max_capacity</code>.</td><td>يجب ألا يتجاوز عدد عناصر Struct في كيان واحد <code translate="no">max_capacity</code> الخاص بحقل StructArray.</td></tr>
<tr><td>استخدم حقول فرعية متجهة منفصلة لأوضاع البحث المنفصلة.</td><td>إذا كان البحث في قائمة التضمين (EmbeddingList) والبحث على مستوى العناصر مطلوبين، فاكتب قيم المتجهات في كل من الحقول الفرعية للمتجهات.</td></tr>
...
<li><p>كتابة المتجهات فقط إلى <code translate="no">emb_list_vector</code> ثم محاولة إجراء بحث على مستوى العنصر على <code translate="no">chunks[emb]</code>.</p></li>
<li><p>كتابة المتجهات فقط إلى <code translate="no">emb</code> ثم محاولة إجراء بحث EmbeddingList على <code translate="no">chunks[emb_list_vector]</code>.</p></li>
</ul>
...
<li><p>للبحث في الحقول الفرعية للمتجهات في StructArray، اقرأ " <a href="/docs/ar/basic-vector-search-with-structarray.md">البحث الأساسي في المتجهات باستخدام StructArray</a>".</p></li>
<li><p>لمراجعة سلوك القيم القابلة للـ null والقيود الخاصة بالإصدارات، اقرأ " <a href="/docs/ar/structarray-limits.md">حدود StructArray</a>".</p></li>
</ol>
