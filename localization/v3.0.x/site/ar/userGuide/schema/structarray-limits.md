...
<p>تأتي معظم حدود StructArray من أحد ثلاثة مصادر: نموذج مخطط StructArray، ووضع البحث الذي تختاره لحقول المتجهات الفرعية، وإصدار Milvus الذي تعمل عليه مجموعتك.</p>
...
<tbody>
<tr><td>شكل المخطط</td><td>لا يمكن استخدام Struct إلا كنوع عنصر لحقل Array. لا يتم دعم Struct كحقل مجموعة من المستوى الأعلى.</td></tr>
<tr><td>مخطط الحقل الفرعي</td><td>تشترك جميع عناصر Struct الموجودة في حقل StructArray نفسه في مخطط Struct واحد محدد مسبقًا.</td></tr>
...
<tr><td>فهارس المتجهات</td><td>يقبل حقل المتجه أو الحقل الفرعي للمتجه فهرسًا واحدًا فقط. استخدم حقولًا فرعية منفصلة للمتجه للبحث في EmbeddingList والبحث على مستوى العناصر.</td></tr>
<tr><td>الدوال</td><td>لا يتم دعم وظائف الحقول للحقول أو الحقول الفرعية داخل حقل StructArray.</td></tr>
<tr><td>الحقول القابلة للقيمة الفارغة</td><td>تخضع حقول StructArray القابلة للقيمة الفارغة لقيود الإصدار. وعندما تكون مدعومة، تنطبق القيمة الفارغة على حقل StructArray بأكمله، وليس على عنصر Struct فردي بشكل مستقل.</td></tr>
<tr><td>حقل الإضافة الديناميكي</td><td>تخضع إضافة حقل StructArray إلى مجموعة موجودة للإصدار وتتطلب أن يكون الحقل المضاف قابلاً للقيمة الفارغة.</td></tr>
</tbody>
</table>
...
<tr><td><code translate="no">max_capacity</code> مطلوب.</td><td>يجب ألا يتجاوز عدد عناصر Struct في كيان واحد عدد <code translate="no">max_capacity</code> المُعد لحقل StructArray.</td></tr>
<tr><td>الحقول الفرعية الحالية ثابتة.</td><td>لا يمكنك إضافة حقول فرعية جديدة إلى حقل StructArray موجود. لتغيير مخطط الحقول الفرعية، قم بإزالة حقل StructArray ثم أضفه مرة أخرى باستخدام المخطط المحدث.</td></tr>
...
<h2 id="Supported-subfield-data-types" class="common-anchor-header">أنواع بيانات الحقول الفرعية المدعومة<button data-href="#Supported-subfield-data-types" class="anchor-icon" translate="no">
    ...
    </button></h2><p>يتم تعيين الحقول الفرعية لـ StructArray إلى تخزين مادي بنمط المصفوفة. يسرد الجدول التالي الأنواع المادية المدعومة وغير المدعومة.</p>
<table>
<thead>
...
<tbody>
<tr><td>حقل StructArray القابل للإفراغ</td><td>مدعوم في Milvus 3.0.0 والإصدارات الأحدث. قم بتعيين <code translate="no">nullable=True</code> على الكائن الأصلي لـ StructArray؛ ولا تقم بتكوين الحقول الفرعية لـ Struct على أنها قابلة للفراغ بشكل مستقل.</td></tr>
...
<tr><td>نطاق القيمة الفارغة</td><td>تنطبق القيمة الفارغة على حقل StructArray بأكمله. على سبيل المثال، <code translate="no">chunks=None</code> صالحة فقط عندما يكون <code translate="no">chunks</code> قابلاً للقيمة الفارغة.</td></tr>
<tr><td>قيمة StructArray "null" جزئيًا</td><td>عندما يحتوي حقل StructArray على قيمة صفيف صالحة، لا تخلط صفيفات الحقول الفرعية التي يمكن أن تكون null مع صفيفات الحقول الفرعية الصالحة في نفس القيمة.</td></tr>
<tr><td>إضافة حقل StructArray ديناميكيًا</td><td>مدعوم في Milvus 3.0.0 والإصدارات الأحدث.</td></tr>
<tr><td>متطلبات القابلية للقيمة الفارغة للإضافة الديناميكية</td><td>يجب أن يكون حقل StructArray المضاف إلى مجموعة موجودة قابلاً للقيمة null لأن الكيانات الموجودة لا تحتوي على قيمة للحقل الجديد.</td></tr>
<tr><td>الكيانات الموجودة بعد الإضافة الديناميكية</td><td>تُرجع الكيانات الموجودة قيمة " <code translate="no">null</code> " للحقل StructArray المضاف.</td></tr>
</tbody>
</table>
<p>يدعم Milvus 3.0.0 والإصدارات الأحدث حقول StructArray القابلة للقيمة الفارغة، ومصفوفات المتجهات القابلة للقيمة الفارغة، والإضافة الديناميكية لحقول StructArray في كل من عمليات النشر المستقلة والموزعة. لا تدعم إصدارات Milvus السابقة هذه الإمكانيات.</p>
<p>في Zilliz Cloud، تتوفر هذه الإمكانيات على المجموعات حسب الطلب (On-Demand Clusters) التي تعمل بنظام Milvus 3.0.0 أو أحدث. ولا تدعمها مجموعات الخدمة (Serving Clusters).</p>
<p>للاطلاع على أمثلة الإدراج التي تحتوي على حقول StructArray القابلة للقيمة null، راجع <a href="/docs/ar/insert-data-into-structarray-fields.md">إدراج البيانات في حقول StructArray</a>.</p>
...
<h2 id="Index-and-metric-limits" class="common-anchor-header">حدود الفهرس والمقياس<button data-href="#Index-and-metric-limits" class="anchor-icon" translate="no">
    ...
    </button></h2><p>يمكن فهرسة حقل فرعي متجه من نوع StructArray إما للبحث في EmbeddingList أو للبحث على مستوى العناصر. ولا يمكن لنفس الحقل الفرعي المتجه استخدام مجموعتي المقاييس معًا لأن كل حقل متجه أو حقل فرعي متجه لا يقبل سوى فهرس واحد.</p>
<table>
<thead>
...
<tr><td>البحث على مستوى العناصر</td><td>مقاييس متجهة عادية مثل <code translate="no">L2</code> ، <code translate="no">IP</code> ، <code translate="no">COSINE</code> ، <code translate="no">HAMMING</code> ، أو <code translate="no">JACCARD</code></td><td>نتائج على مستوى العنصر التي يمكن أن تتضمن إزاحة العنصر المطابق.</td></tr>
</tbody>
</table>
...
<p>تُعتبر الحقول الفرعية للمتجهات في StructArray حقولًا فرعية للمتجهات عند تخطيط مخطط المجموعة. حافظ على إجمالي عدد حقول المتجهات والحقول الفرعية للمتجهات ضمن حدود الإصدار المستهدف ومستوى الخدمة.</p>
...
<tr><td>البحث الأساسي على مستوى العناصر</td><td>مدعوم في الحقول الفرعية للمتجهات StructArray المفهرسة باستخدام مقاييس المتجهات العادية. يمكنه إرجاع إزاحات العناصر المطابقة.</td></tr>
...
<tr><td>البحث بالتجميع</td><td>يمكن أن يُرجع البحث بالتجميع على مستوى العناصر إزاحات. يعتمد سلوك التجميع في البحث المختلط لطلبات StructArray على مستوى العناصر على الإصدار.</td></tr>
...
<h2 id="Filter-and-operator-limits" class="common-anchor-header">حدود التصفية والمشغلات<button data-href="#Filter-and-operator-limits" class="anchor-icon" translate="no">
    ...
    </button></h2><p>تتم معالجة تصفية القيم العددية لـ StructArray بواسطة عوامل StructArray، مثل <code translate="no">element_filter</code> ومجموعة <code translate="no">MATCH_*</code>. توجد مصفوفة دعم المسندات التفصيلية في <a href="/docs/ar/struct-array-operators.md">StructArray Operators</a>.</p>
<p>على مستوى عام:</p>
<ul>
<li><p>استخدم <code translate="no">$[subfield]</code> فقط داخل مشغلات StructArray.</p></li>
<li><p>استخدم الحقول الفرعية القياسية للمسلّطات القياسية.</p></li>
<li><p>لا تستخدم الحقول الفرعية المتجهة كمدخلات للمسلّطات القياسية في <code translate="no">$[...]</code>.</p></li>
<li><p>لا يتم دعم صيغة مسار JSON، ووظائف JSON، ووظائف حاويات المصفوفات، ووظائف مطابقة النص، ووظائف الهندسة / نظم المعلومات الجغرافية، وتعبيرات Timestamptz للمسلّطات على مستوى عناصر StructArray.</p></li>
