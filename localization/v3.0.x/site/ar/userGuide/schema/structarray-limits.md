...
<tr><td>فهارس المتجهات</td><td>يقبل حقل المتجه أو حقل المتجه الفرعي فهرسًا واحدًا فقط. استخدم حقول متجه فرعية منفصلة للبحث في EmbeddingList والبحث على مستوى العناصر.</td></tr>
<tr><td>الدوال</td><td>لا يتم دعم وظائف الحقول للحقول أو الحقول الفرعية داخل حقل StructArray.</td></tr>
<tr><td>الحقول القابلة للقيمة "Null"</td><td>تخضع حقول StructArray القابلة للصفر لقيود الإصدار. وعندما تكون مدعومة، ينطبق القيمة الصفرية على حقل StructArray بأكمله، وليس على عنصر Struct فردي بشكل مستقل.</td></tr>
...
<tr><td><code translate="no">Array</code></td><td>مدعوم</td><td>حدد الحقل الفرعي كـ <code translate="no">DataType.INT8</code> أو <code translate="no">DataType.INT16</code> أو <code translate="no">DataType.INT32</code> أو <code translate="no">DataType.INT64</code>.</td></tr>
...
<tr><td><code translate="no">Array</code></td><td>غير مدعوم</td><td>لا يتم دعم الحقول الفرعية من نوع JSON في حقول StructArray.</td></tr>
<tr><td><code translate="no">Array</code></td><td>غير مدعوم</td><td>الحقول الفرعية الخاصة بالهندسة ووظائف نظم المعلومات الجغرافية (GIS) غير مدعومة في حقول StructArray.</td></tr>
...
<h2 id="Nullable-and-dynamic-schema-limits" class="common-anchor-header">حدود المخطط القابل للقيمة الفارغة والديناميكي<button data-href="#Nullable-and-dynamic-schema-limits" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<tr><td>نطاق القيمة الفارغة</td><td>تنطبق القيمة الصفرية على حقل StructArray بأكمله. على سبيل المثال، لا يكون <code translate="no">chunks=None</code> صالحًا إلا عندما يكون <code translate="no">chunks</code> قابلاً للقيمة الصفرية.</td></tr>
<tr><td>قيمة StructArray فارغة جزئيًا</td><td>عندما يحتوي حقل StructArray على قيمة مصفوفة صالحة، لا تخلط مصفوفات الحقول الفرعية ذات القيمة null مع مصفوفات الحقول الفرعية الصالحة في نفس القيمة.</td></tr>
...
<h2 id="Index-and-metric-limits" class="common-anchor-header">حدود الفهرس والمقياس<button data-href="#Index-and-metric-limits" class="anchor-icon" translate="no">
    ...
    </button></h2><p>يمكن فهرسة حقل فرعي متجه من نوع StructArray إما للبحث في EmbeddingList أو للبحث على مستوى العناصر. ولا يمكن للحقل الفرعي المتجه نفسه استخدام مجموعتي المقاييس معًا لأن كل حقل متجه أو حقل فرعي متجه يقبل فهرسًا واحدًا فقط.</p>
<table>
<thead>
...
<p>تُعتبر الحقول الفرعية المتجهة في StructArray حقولًا فرعية متجهة عند تخطيط مخطط المجموعة. حافظ على إجمالي عدد الحقول المتجهة والحقول الفرعية المتجهة ضمن حدود الإصدار المستهدف ومستوى الخدمة.</p>
<p>للاطلاع على مصفوفة أنواع الفهارس والمقاييس المدعومة، راجع <a href="/docs/ar/index-structarray-fields.md">حقول StructArray للفهرس</a>.</p>
...
<tr><td>البحث في النطاق</td><td>مدعوم وفقًا لوضع البحث ودعم الفهرس/المقياس في الإصدار المستهدف. للتعرف على سلوك نطاق البحث المختلط في طلبات StructArray على مستوى العناصر، تحقق من الإصدار المستهدف.</td></tr>
<tr><td>البحث المجمّع</td><td>يمكن أن يُرجع البحث التجميعي على مستوى العناصر إزاحات. يعتمد سلوك التجميع في البحث المختلط لطلبات StructArray على مستوى العناصر على الإصدار.</td></tr>
<tr><td>البحث المختلط</td><td>يمكن أن يتضمن طلب البحث الهجين طلبات الحقول الفرعية للمتجه StructArray فقط عندما يدعم الإصدار المستهدف تركيبة البحث هذه. ولا يزال كل طلب يتبع مجموعة المقاييس الخاصة بالحقل الفرعي للمتجه المفهرس.</td></tr>
...
<li><p>استخدم الحقول الفرعية القياسية للمسافات القياسية.</p></li>
