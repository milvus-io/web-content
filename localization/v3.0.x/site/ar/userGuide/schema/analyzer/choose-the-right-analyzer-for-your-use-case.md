...
<ol>
<li><p><strong>التقطيع إلى رموز (إجباري):</strong> تُطبق في هذه المرحلة الأولية أداة <strong>التقطيع</strong> لتقسيم سلسلة نصية متصلة إلى وحدات منفصلة ذات معنى تُسمى الرموز. قد تختلف طريقة التقطيع إلى رموز بشكل كبير اعتمادًا على اللغة ونوع المحتوى.</p></li>
...
<table>
   ...
   <tr>
     ...
     <td><p>استخدم <a href="/docs/ar/standard-tokenizer.md"><code translate="no">standard</code></a> أداة تجزئة لتقسيم العبارة بناءً على علامات الترقيم والمسافات؛ واستخدم مرشحًا مخصصًا <a href="/docs/ar/regex-filter.md">يع</a> تمد على <a href="/docs/ar/regex-filter.md">التعبيرات النمطية</a>.</p></td>
   </tr>
   <tr>
...
<table>
   ...
   <tr>
     ...
     <td><p>❌ لا</p></td>
     <td><p>الكلمات الصينية لا تحتوي على مسافات وسيتم التعامل معها كوحدة واحدة.</p></td>
...
<h3 id="Available-built-in-analyzers" class="common-anchor-header">المحللات المدمجة المتاحة<button data-href="#Available-built-in-analyzers" class="anchor-icon" translate="no">
   ...
   <tr>
     ...
     <td><p>محلل للأغراض العامة لمعالجة النص الأولية. بالنسبة للسيناريوهات أحادية اللغة، توفر المحللات الخاصة باللغة (مثل <code translate="no">english</code>) أداءً أفضل.</p></td>
   </tr>
   <tr>
     <td><p><a href="/docs/ar/english-analyzer.md"><code translate="no">english</code></a></p></td>
     <td><p>مخصص للغة الإنجليزية، ويطبق عملية استخلاص الجذور وإزالة الكلمات الممنوعة لتحسين المطابقة الدلالية باللغة الإنجليزية</p></td>
...
<table>
   ...
   <tr>
     ...
     <td><p>المحتوى الدولي، المحتوى الذي ينشئه المستخدمون</p></td>
...
<table>
   ...
   <tr>
     ...
     <td><p>تنظيف النتائج الناتجة عن أدوات تحليل العلامات التالية: <code translate="no">jieba</code> ، <code translate="no">lindera</code> ، <code translate="no">icu</code> ، والتي تعرض علامات الترقيم كوحدات منفردة</p></td>
...
<h3 id="Chinese-content" class="common-anchor-header">المحتوى الصيني<button data-href="#Chinese-content" class="anchor-icon" translate="no">
    ...
    </button></h3><p>استخدم أداة تحليل الكلمات " <code translate="no">jieba</code> " وقم بتطبيق مرشح الأحرف للاحتفاظ فقط بالأحرف الصينية والأحرف اللاتينية والأرقام.</p>
<pre><code translate="no" class="language-python">analyzer_params = {
...
<h3 id="Korean-content" class="common-anchor-header">المحتوى الكوري<button data-href="#Korean-content" class="anchor-icon" translate="no">
    ...
    </button></h3><p>على غرار اللغة اليابانية، استخدم أداة تحليل الرموز <code translate="no">lindera</code> مع القاموس الكوري:</p>
...
<h3 id="Mixed-or-multilingual-content" class="common-anchor-header">المحتوى المختلط أو متعدد اللغات<button data-href="#Mixed-or-multilingual-content" class="anchor-icon" translate="no">
    ...
    </button></h3><p>عند التعامل مع محتوى يمتد عبر لغات متعددة أو يستخدم أنظمة كتابة غير متوقعة، ابدأ باستخدام محلل <code translate="no">icu</code>. يتعامل هذا المحلل المتوافق مع Unicode مع أنظمة الكتابة والرموز المختلطة بفعالية.</p>
