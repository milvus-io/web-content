...
<h2 id="Preparation" class="common-anchor-header">التحضير<button data-href="#Preparation" class="anchor-icon" translate="no">
    ...
    </button></h2><h3 id="Dependency-and-Environment" class="common-anchor-header">التبعية والبيئة<button data-href="#Dependency-and-Environment" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<h2 id="Workflow" class="common-anchor-header">سير العمل<button data-href="#Workflow" class="anchor-icon" translate="no">
    ...
    </button></h2><h3 id="Defining-the-Data-Source-Amazon-S3" class="common-anchor-header">تحديد مصدر البيانات (Amazon S3)<button data-href="#Defining-the-Data-Source-Amazon-S3" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>في هذه الحالة، نحن نقوم باستخراج المستندات من دلو Amazon S3. يسمح لنا VectorETL بتحديد اسم الدلو، ومسار الملفات، ونوع البيانات التي نعمل معها.</p>
...
<button class="copy-code-btn"></button></code></pre>
<h3 id="Configuring-the-Embedding-Model-OpenAI" class="common-anchor-header">تكوين نموذج التضمين (OpenAI)<button data-href="#Configuring-the-Embedding-Model-OpenAI" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>بمجرد إعداد مصدر البيانات لدينا، نحتاج إلى تحديد نموذج التضمين الذي سيحول بياناتنا النصية إلى تضمينات متجهة. هنا، نستخدم OpenAI <code translate="no">text-embedding-ada-002</code> في هذا المثال.</p>
...
<button class="copy-code-btn"></button></code></pre>
<h3 id="Setting-Up-Milvus-as-the-Target-Database" class="common-anchor-header">إعداد ميلفوس كقاعدة البيانات المستهدفة<button data-href="#Setting-Up-Milvus-as-the-Target-Database" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>نحن بحاجة إلى تخزين التضمينات التي تم إنشاؤها في Milvus. هنا، نحدد معلمات اتصال Milvus الخاصة بنا باستخدام Milvus Lite.</p>
...
<h3 id="Specifying-Columns-for-Embedding" class="common-anchor-header">تحديد الأعمدة للتضمين<button data-href="#Specifying-Columns-for-Embedding" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>الآن، نحتاج إلى تحديد الأعمدة من ملفات CSV التي يجب تحويلها إلى تضمينات. يضمن ذلك معالجة الحقول النصية ذات الصلة فقط، مما يحسّن الكفاءة والتخزين على حد سواء.</p>
...
<button class="copy-code-btn"></button></code></pre>
<h3 id="Creating-and-Executing-the-VectorETL-Pipeline" class="common-anchor-header">إنشاء خط أنابيب VectorETL وتنفيذه<button data-href="#Creating-and-Executing-the-VectorETL-Pipeline" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>مع وجود جميع التكوينات في مكانها الصحيح، نقوم الآن بتهيئة خط أنابيب ETL، وإعداد تدفق البيانات، وتنفيذه.</p>
