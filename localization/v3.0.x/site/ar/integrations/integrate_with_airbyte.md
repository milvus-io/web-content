...
<h2 id="Major-Components-of-Airbyte" class="common-anchor-header">المكونات الرئيسية ل Airbyte<button data-href="#Major-Components-of-Airbyte" class="anchor-icon" translate="no">
    ...
    </button></h2><h3 id="1-Connector-Catalog" class="common-anchor-header">1. كتالوج الموصلات<button data-href="#1-Connector-Catalog" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<h3 id="2-The-Platform" class="common-anchor-header">2. المنصة<button data-href="#2-The-Platform" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>توفر منصة Airbyte جميع الخدمات الأفقية المطلوبة لتهيئة عمليات نقل البيانات وتوسيع نطاقها، وهي متاحة كخدمة <a href="https://airbyte.com/product/airbyte-cloud">مُدارة سحابياً</a> أو <a href="https://airbyte.com/product/airbyte-enterprise">مُدارة ذاتياً</a>.</p>
<h3 id="3-The-User-Interface" class="common-anchor-header">3. واجهة المستخدم<button data-href="#3-The-User-Interface" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<h3 id="Set-Up-Source" class="common-anchor-header">إعداد المصدر<button data-href="#Set-Up-Source" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>بمجرد تشغيل المثيل الخاص بك، نحتاج إلى إعداد الاتصال - انقر فوق "اتصال جديد" واختر موصل "دعم Zendesk" كمصدر. بعد النقر فوق الزر "اختبار وحفظ"، ستتحقق Airbyte من إمكانية إنشاء الاتصال.</p>
<p>على سحابة Airbyte، يمكنك المصادقة بسهولة بالنقر على زر "مصادقة". عند استخدام مثيل Airbyte محلي، اتبع الإرشادات الموضحة في صفحة <a href="https://docs.airbyte.com/integrations/sources/zendesk-support#airbyte-open-source-enable-api-token-access-and-generate-a-token">التوثيق</a>.</p>
<h3 id="Set-Up-Destination" class="common-anchor-header">إعداد الوجهة<button data-href="#Set-Up-Destination" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>إذا كان كل شيء يعمل بشكل صحيح، فإن الخطوة التالية هي إعداد الوجهة لنقل البيانات إليها. هنا، اختر موصل "Milvus".</p>
...
<li><strong>الفهرسة</strong> - بمجرد الانتهاء من فهرسة القطع، يمكنك تحميلها في قاعدة البيانات. للقيام بذلك، أدخل المعلومات التي حصلت عليها عند إعداد مجموعتك ومجموعتك في مجموعة Milvus العنقودية. <div><img translate="no" src="https://milvus-docs.s3.us-west-2.amazonaws.com/assets/airbyte_with_milvus_1.png" width="40%"/></div> سيؤدي النقر فوق "اختبار وحفظ" إلى التحقق مما إذا كان كل شيء مصطفًا بشكل صحيح (بيانات اعتماد صالحة، المجموعة موجودة ولها نفس الأبعاد المتجهة مثل التضمين الذي تم تكوينه، وما إلى ذلك).</li>
</ul>
<h3 id="Set-up-stream-sync-flow" class="common-anchor-header">إعداد تدفق مزامنة الدفق<button data-href="#Set-up-stream-sync-flow" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<h3 id="Check-flow" class="common-anchor-header">التحقق من التدفق<button data-href="#Check-flow" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<h3 id="Set-up-UI-environment" class="common-anchor-header">إعداد بيئة واجهة المستخدم<button data-href="#Set-up-UI-environment" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>ستحتاج إلى تثبيت بايثون محلي لأننا سنستخدم Streamlit لتنفيذ التطبيق.</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>سيؤدي ذلك إلى تصيير نموذج أساسي:<div><img translate="no" src="https://milvus-docs.s3.us-west-2.amazonaws.com/assets/airbyte_with_milvus_4.png" width="40%"/></div>يمكن أيضًا العثور على الكود الخاص بهذا المثال على <a href="https://github.com/airbytehq/tutorial-similarity-search/blob/main/1_basic_support_form.py">GitHub</a>.</p>
<h3 id="Set-up-backend-query-service" class="common-anchor-header">إعداد خدمة استعلام الواجهة الخلفية<button data-href="#Set-up-backend-query-service" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<li>يتم إجراء بحث عن التشابه، وتصفية النتائج حسب حالة التذكرة ومعرف منتدى المجموعة (حيث أن التذاكر المفتوحة لنفس منتدى المجموعة هي فقط ذات الصلة).</li>
...
<button class="copy-code-btn"></button></code></pre>
<p>عند محاولة إرسال تذكرة موجودة بالفعل، هكذا ستبدو النتيجة:<div><img translate="no" src="https://milvus-docs.s3.us-west-2.amazonaws.com/assets/airbyte_with_milvus_5.png" width="40%"/></div> يمكن أيضًا العثور على الكود الخاص بهذا المثال على <a href="https://github.com/airbytehq/tutorial-similarity-search/blob/main/2_open_ticket_check.py">GitHub</a>.</p>
<h3 id="Show-more-relevant-information" class="common-anchor-header">إظهار المزيد من المعلومات ذات الصلة<button data-href="#Show-more-relevant-information" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<p>بالإضافة إلى حالة الاستخدام الكلاسيكية للبحث الدلالي الموضحة في هذه المقالة، يمكن أيضًا استخدام الإعداد العام لبناء روبوت دردشة للإجابة عن الأسئلة باستخدام طريقة RAG (التوليد المعزز للاسترجاع)، أو أنظمة التوصية، أو المساعدة في جعل الإعلانات أكثر ملاءمة وفعالية.</p>
