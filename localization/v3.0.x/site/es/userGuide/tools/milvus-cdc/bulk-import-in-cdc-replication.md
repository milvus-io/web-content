...
<p>En esta guía, el clúster primario es el clúster de Milvus de origen, y el clúster de reserva es el clúster de Milvus de destino.</p>
...
<ol>
<li><p><strong>Fase de importación</strong>: Milvus carga los datos en el clúster primario y replica la importación al clúster de reserva, pero los datos importados permanecen ocultos. La tarea de importación se detiene en el estado « <code translate="no">Uncommitted</code> » y queda en espera.</p></li>
...
<h2 id="Step-2-Run-a-2PC-import" class="common-anchor-header">Paso 2: Ejecutar una importación 2PC<button data-href="#Step-2-Run-a-2PC-import" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Ejecuta todas las llamadas de importación en el clúster primario. Los datos importados y la decisión de confirmación se replican automáticamente en el clúster de reserva, por lo que no debes enviar ni confirmar la importación tú mismo en el clúster de reserva.</p>
<p>Cada clúster lee los archivos de importación desde su propio almacenamiento de objetos. Asegúrate de que los archivos que se van a importar existan tanto en el almacenamiento de objetos primario como en el de espera. Puedes subir los archivos a ambos clústeres o utilizar un almacenamiento de objetos al que ambos clústeres puedan acceder. Si faltan archivos en el clúster de espera, la importación replicada fallará en ese clúster con un error de «objeto no encontrado».</p>
...
<h3 id="Why-wait-for-Uncommitted-on-both-clusters" class="common-anchor-header">¿Por qué esperar a que <code translate="no">Uncommitted</code> se ejecute en ambos clústeres?<button data-href="#Why-wait-for-Uncommitted-on-both-clusters" class="anchor-icon" translate="no">
    ...
    </button></h3><p>Realizar la confirmación antes de que el clúster de reserva haya terminado de importar no daña los datos, pero el clúster de reserva aún se está poniendo al día cuando se aplica la confirmación. Esperar hasta que tanto el clúster primario como el de reserva indiquen <code translate="no">Uncommitted</code> confirma que los datos importados se han replicado por completo y que ambos clústeres están listos para hacerlos visibles conjuntamente.</p>
...
<h3 id="Do-I-need-to-commit-on-the-standby-cluster" class="common-anchor-header">¿Tengo que realizar la confirmación en el clúster en espera?<button data-href="#Do-I-need-to-commit-on-the-standby-cluster" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>No. Al realizar la confirmación en el clúster primario, esta se replica en el clúster de espera como una única barrera ordenada.</p>
<h3 id="Why-does-my-import-fail-with-import-in-replicating-cluster-is-not-supported-yet" class="common-anchor-header">¿Por qué falla mi importación con el error « <code translate="no">import in replicating cluster is not supported yet</code> »?<button data-href="#Why-does-my-import-fail-with-import-in-replicating-cluster-is-not-supported-yet" class="anchor-icon" translate="no">
    ...
    </button></h3><p><code translate="no">dataCoord.import.enableInReplicatingCluster</code> no está habilitado en ese clúster. Configúralo en « <code translate="no">true</code> » tanto en el clúster primario como en el de reserva.</p>
<h3 id="Why-does-my-import-fail-with-autocommittrue-import-in-replicating-cluster-is-not-supported" class="common-anchor-header">¿Por qué falla mi importación con « <code translate="no">auto_commit=true import in replicating cluster is not supported</code> »?<button data-href="#Why-does-my-import-fail-with-autocommittrue-import-in-replicating-cluster-is-not-supported" class="anchor-icon" translate="no">
    ...
    </button></h3><p>En un clúster de replicación, solo se aceptan importaciones 2PC con « <code translate="no">auto_commit=false</code> ». Establece « <code translate="no">options={&quot;auto_commit&quot;: &quot;false&quot;}</code> » en la solicitud de importación.</p>
