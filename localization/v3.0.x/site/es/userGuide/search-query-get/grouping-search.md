...
title: Búsqueda por agrupaciones
summary: >-
...
---
<h1 id="Grouping-Search" class="common-anchor-header">Búsqueda por agrupaciones<button data-href="#Grouping-Search" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<p>De forma predeterminada, la búsqueda por grupos devuelve solo una entidad por grupo. Si desea aumentar el número de resultados que se devuelven por grupo, puede controlarlo mediante los parámetros « <code translate="no">group_size</code> » y « <code translate="no">strict_group_size</code> ».</p>
</div>
...
<h2 id="Configure-group-size" class="common-anchor-header">Configurar el tamaño del grupo<button data-href="#Configure-group-size" class="anchor-icon" translate="no">
    ...
    </button></h2><p>De forma predeterminada, la búsqueda por grupos devuelve solo una entidad por grupo. Si deseas obtener varios resultados por grupo, ajusta los parámetros <code translate="no">group_size</code> y <code translate="no">strict_group_size</code>.</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#go">   Go</a>
...
<li><p><code translate="no">strict_group_size</code>: Este parámetro booleano controla si el sistema debe aplicar estrictamente el recuento establecido por <code translate="no">group_size</code>. Cuando se establece en <code translate="no">strict_group_size=True</code>, el sistema intentará incluir el número exacto de entidades especificado por <code translate="no">group_size</code> en cada grupo (por ejemplo, dos párrafos), a menos que no haya datos suficientes en ese grupo. Por defecto (<code translate="no">strict_group_size=False</code>), el sistema da prioridad a cumplir el número de grupos especificado por el parámetro <code translate="no">limit</code>, en lugar de garantizar que cada grupo contenga <code translate="no">group_size</code> entidades. Este enfoque suele ser más eficiente en los casos en que la distribución de los datos es desigual.</p></li>
</ul>
...
<h2 id="Order-groups-by-a-scalar-field--Milvus-30x" class="common-anchor-header">Ordenar grupos por un campo escalar<span class="beta-tag" style="background-color:rgb(0, 179, 255);color:white" translate="no">Compatible with Milvus 3.0.x</span><button data-href="#Order-groups-by-a-scalar-field--Milvus-30x" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Puede combinar la búsqueda por agrupación (Grouping Search) con la búsqueda por campo escalar ( <code translate="no">order_by_fields</code> ) para ordenar los grupos según un campo escalar. Esto resulta útil cuando se desean resultados diversos entre los grupos, pero se quiere que estos sigan un orden relevante para el negocio, como el precio o la valoración.</p>
...
<li><p><strong>Tamaño de grupo estricto</strong>: Cuando se establece <code translate="no">strict_group_size=True</code>, el sistema intentará devolver el número especificado de entidades (<code translate="no">group_size</code>) para cada grupo, a menos que no haya datos suficientes en dicho grupo. Esta configuración garantiza un recuento de entidades coherente por grupo, pero puede provocar una disminución del rendimiento si la distribución de los datos es desigual o los recursos son limitados. Si no se requiere un recuento estricto de entidades, establecer <code translate="no">strict_group_size=False</code> puede mejorar la velocidad de las consultas.</p></li>
<li><p>Si los vectores de consulta ya existen en la colección de destino, plantéate utilizar <code translate="no">ids</code> en lugar de recuperarlos antes de las búsquedas. Para obtener más detalles, consulta <a href="/docs/es/primary-key-search.md">«Búsqueda</a> por <a href="/docs/es/primary-key-search.md">clave primaria</a>».</p></li>
</ul>
