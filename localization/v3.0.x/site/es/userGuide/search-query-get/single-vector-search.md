...
summary: >-
  Ejecute búsquedas RNA básicas en Milvus con vectores de consulta, campos de
  salida, filtros, rangos e iteradores.
---
...
<li><p><a href="/docs/es/single-vector-search.md#ANN-Search-in-Partition">Búsqueda RNA en particiones</a></p></li>
<li><p><a href="/docs/es/single-vector-search.md#Use-Output-Fields">Utilización de campos de salida</a></p></li>
...
<h2 id="Primary-Key-Search--Milvus-269+" class="common-anchor-header">Búsqueda por clave primaria<span class="beta-tag" style="background-color:rgb(0, 179, 255);color:white" translate="no">Compatible with Milvus 2.6.9+</span><button data-href="#Primary-Key-Search--Milvus-269+" class="anchor-icon" translate="no">
    ...
    </button></h2><p>En lugar de establecer vectores de consulta, puede utilizar claves primarias si los vectores de consulta ya existen en la colección de destino.</p>
<div class="multipleCode">
...
<h2 id="ANN-Search-in-Partition" class="common-anchor-header">Búsqueda RNA en Partición<button data-href="#ANN-Search-in-Partition" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Supongamos que ha creado múltiples particiones en una colección, y puede limitar el ámbito de búsqueda a un número específico de particiones. En ese caso, puedes incluir los nombres de las particiones de destino en la petición de búsqueda para restringir el ámbito de búsqueda dentro de las particiones especificadas. Reducir el número de particiones implicadas en la búsqueda mejora el rendimiento de la misma.</p>
...
<h2 id="Sort-Search-Results-by-Scalar-Fields--Milvus-30x" class="common-anchor-header">Ordenar resultados de búsqueda por campos escalares<span class="beta-tag" style="background-color:rgb(0, 179, 255);color:white" translate="no">Compatible with Milvus 3.0.x</span><button data-href="#Sort-Search-Results-by-Scalar-Fields--Milvus-30x" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Por defecto, Milvus ordena los resultados de la búsqueda por su puntuación de similitud con el vector de consulta. Si desea que las entidades devueltas sigan un orden de campos escalares, añada <code translate="no">order_by_fields</code> a la petición de búsqueda.</p>
<p>Cada elemento de <code translate="no">order_by_fields</code> especifica un campo escalar y una dirección de ordenación. Utilice <code translate="no">&quot;asc&quot;</code> para el orden ascendente o <code translate="no">&quot;desc&quot;</code> para el descendente. Si omite <code translate="no">order</code>, Milvus ordena el campo en orden ascendente.</p>
<p>El siguiente ejemplo ordena los resultados de la búsqueda por <code translate="no">price</code> de menor a mayor. Incluya el campo de ordenación en <code translate="no">output_fields</code> si desea inspeccionar el valor del campo en la respuesta.</p>
<div class="multipleCode">
   <a href="#python">Python</a> <a href="#java">Java</a> <a href="#javascript">NodeJS</a> <a href="#go">Go</a> <a href="#bash">cURL</a></div>
<pre><code translate="no" class="language-python">res = client.search(
...
<pre><code translate="no" class="language-bash"><span class="hljs-comment"># restful</span>
<button class="copy-code-btn"></button></code></pre>
<p>También puede ordenar por múltiples campos escalares. Milvus aplica los campos en el orden que usted especifique. En el siguiente ejemplo, Milvus ordena los resultados por <code translate="no">price</code> en orden ascendente. Para entidades con el mismo <code translate="no">price</code>, Milvus ordena entonces por <code translate="no">rating</code> en orden descendente.</p>
<div class="multipleCode">
   <a href="#python">Python</a> <a href="#java">Java</a> <a href="#javascript">NodeJS</a> <a href="#go">Go</a> <a href="#bash">cURL</a></div>
<pre><code translate="no" class="language-python">res = client.search(
    collection_name=<span class="hljs-string">&quot;product_catalog&quot;</span>,
...
<pre><code translate="no" class="language-bash"><span class="hljs-comment"># restful</span>
<button class="copy-code-btn"></button></code></pre>
<p>Para entidades con los mismos valores en todos los campos de orden especificados, Milvus mantiene el orden original de puntuación de similitud.</p>
<h2 id="Use-Limit-and-Offset" class="common-anchor-header">Uso de límite y desplazamiento<button data-href="#Use-Limit-and-Offset" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>Usted puede notar que el parámetro <code translate="no">limit</code> llevado en las peticiones de búsqueda determina el número de entidades a incluir en los resultados de la búsqueda. Este parámetro especifica el número máximo de entidades a devolver en una sola búsqueda, y suele denominarse <strong>top-K</strong>.</p>
