...
summary: >-
  ...
  conjunto de información, como el título y la descripción, o mediante múltiples
  modalidades, como texto, imágenes y audio. Por ejemplo, un tuit que contenga
  un fragmento de texto y una imagen se considerará relevante si el texto o la
  imagen coinciden con la semántica de la consulta de búsqueda. La búsqueda
  híbrida mejora la experiencia de búsqueda al combinar búsquedas en estos
  diversos campos. Milvus permite esto al facilitar la búsqueda en múltiples
  campos vectoriales, realizando varias búsquedas de «vecino más cercano
  aproximado» (ANN) simultáneamente. La búsqueda híbrida multivectorial resulta
  especialmente útil si se desea buscar tanto en texto como en imágenes, en
  varios campos de texto que describan el mismo objeto, o en vectores densos y
  dispersos para mejorar la calidad de la búsqueda.
---
...
<li><p><strong>Búsqueda vectorial multimodal</strong>: La búsqueda vectorial multimodal es una técnica potente que permite buscar en diversos tipos de datos, incluyendo texto, imágenes, audio y otros. La principal ventaja de este enfoque es su capacidad para unificar diferentes modalidades en una experiencia de búsqueda fluida y cohesionada. Por ejemplo, en la búsqueda de productos, un usuario podría introducir una consulta de texto para encontrar productos descritos tanto con texto como con imágenes. Al combinar estas modalidades mediante un método de búsqueda híbrida, se puede mejorar la precisión de la búsqueda o enriquecer los resultados de la misma.</p></li>
</ul>
...
<li><p><code translate="no">text_dense</code>: Se utiliza para almacenar vectores densos de los textos. Este campo es de tipo « <code translate="no">FLOAT_VECTOR</code> » con una dimensión vectorial de 768.</p></li>
<li><p><code translate="no">text_sparse</code>: Se utiliza para almacenar vectores dispersos de los textos. Este campo es del tipo de datos <code translate="no">SPARSE_FLOAT_VECTOR</code>.</p></li>
...
<p>Dado que utilizaremos el algoritmo BM25 integrado para realizar una búsqueda de texto completo en el campo de texto, es necesario añadir el campo « <code translate="no">Function</code> » de Milvus al esquema. Para obtener más detalles, consulta <a href="/docs/es/full-text-search.md">Búsqueda de texto completo</a>.</p>
...
<ul>
<li><p><code translate="no">text_dense_index</code>: se crea un índice de tipo « <code translate="no">AUTOINDEX</code> » con el tipo de métrica « <code translate="no">IP</code> » para el campo de texto de vector denso.</p></li>
<li><p><code translate="no">text_sparse_index</code>: se utiliza un índice de tipo «<code translate="no">SPARSE_INVERTED_INDEX</code>» con el tipo de métrica « <code translate="no">BM25</code> » para el campo vectorial de texto disperso.</p></li>
<li><p><code translate="no">image_dense_index</code>: se crea un índice de tipo « <code translate="no">AUTOINDEX</code> » con el tipo de métrica « <code translate="no">IP</code> » para el campo vectorial denso de imágenes.</p></li>
</ul>
...
<h2 id="Insert-data" class="common-anchor-header">Insertar datos<button data-href="#Insert-data" class="anchor-icon" translate="no">
    ...
    </button></h2><p>En esta sección se insertan datos en la colección « <code translate="no">my_collection</code> » según el esquema definido anteriormente. Durante la inserción, asegúrate de que todos los campos, excepto aquellos con valores generados automáticamente, se rellenen con datos en el formato correcto. En este ejemplo:</p>
<ul>
<li><p><code translate="no">id</code>: un entero que representa el ID del producto</p></li>
...
<ul>
<li><p><code translate="no">text_dense</code> para la búsqueda semántica de texto, lo que permite la comprensión contextual y la recuperación basada en el significado, en lugar de la coincidencia directa de palabras clave;</p></li>
...
<h3 id="Step-2-Configure-a-reranking-strategy" class="common-anchor-header">Paso 2: Configurar una estrategia de reclasificación<button data-href="#Step-2-Configure-a-reranking-strategy" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<p>El valor de <code translate="no">timezone</code> debe ser un <a href="https://en.wikipedia.org/wiki/List_of_tz_database_time_zones">identificador de zona horaria IANA</a> válido (por ejemplo, <strong>Asia/Shanghai</strong>, <strong>America/Chicago</strong> o <strong>UTC</strong>). Para obtener más información sobre cómo utilizar el campo <code translate="no">TIMESTAMPTZ</code>, consulta <a href="/docs/es/timestamptz-field.md">el campo TIMESTAMPTZ</a>.</p>
