...
summary: >-
  ...
  función supera las limitaciones de la búsqueda semántica, que podría pasar por
  alto términos concretos, lo que garantiza que obtengas los resultados más
  precisos y contextualmente relevantes. Además, simplifica las búsquedas
  vectoriales al aceptar entradas de texto sin procesar, convirtiendo
  automáticamente sus datos de texto en incrustaciones dispersas sin necesidad
...
<h1 id="Full-Text-Search" class="common-anchor-header">Búsqueda de texto completo<button data-href="#Full-Text-Search" class="anchor-icon" translate="no">
    ...
    </button></h1><p>La búsqueda de texto completo es una función que recupera documentos que contienen términos o frases específicos en conjuntos de datos de texto y, a continuación, clasifica los resultados en función de su relevancia. Esta función supera las limitaciones de la búsqueda semántica, que podría pasar por alto términos precisos, garantizando que obtengas los resultados más precisos y contextualmente relevantes. Además, simplifica las búsquedas vectoriales al aceptar entradas de texto sin procesar, convirtiendo automáticamente sus datos de texto en representaciones dispersas sin necesidad de generar manualmente representaciones vectoriales.</p>
...
<h2 id="BM25-implementation" class="common-anchor-header">Implementación de BM25<button data-href="#BM25-implementation" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Milvus ofrece búsqueda de texto completo basada en el algoritmo de relevancia BM25, una función de puntuación ampliamente adoptada en los sistemas de recuperación de información, y Milvus la integra en el flujo de trabajo de búsqueda para ofrecer resultados de texto precisos y ordenados por relevancia.</p>
<p>La búsqueda de texto completo en Milvus sigue el siguiente flujo de trabajo:</p>
<ol>
<li><p><strong>Introducción de texto sin procesar</strong>: se insertan documentos de texto o se introduce una consulta utilizando texto sin formato, sin necesidad de modelos de incrustación.</p></li>
<li><p><strong>Análisis de texto</strong>: Milvus utiliza un <a href="/docs/es/analyzer-overview.md">analizador</a> para procesar el texto y convertirlo en términos significativos que puedan indexarse y buscarse.</p></li>
<li><p><strong>Procesamiento de la función BM25</strong>: una función integrada transforma estos términos en representaciones vectoriales dispersas optimizadas para la puntuación BM25.</p></li>
<li><p><strong>Almacenamiento en la colección</strong>: Milvus almacena las incrustaciones dispersas resultantes en una colección para una rápida recuperación y clasificación.</p></li>
<li><p><strong>Puntuación de relevancia BM25</strong>: En el momento de la búsqueda, Milvus aplica la función de puntuación BM25 para calcular la relevancia de los documentos y devolver resultados clasificados que mejor se ajusten a los términos de la consulta.</p></li>
</ol>
<p><span class="img-wrapper">
   ...
   <span>Búsqueda de texto completo</span>
  
 </span></p>
<p>Para utilizar la búsqueda de texto completo, sigue estos pasos principales:</p>
<ol>
...
<li><p><a href="/docs/es/full-text-search.md#Perform-full-text-search">Realizar búsquedas</a>: Utiliza texto de consulta en lenguaje natural para recuperar resultados ordenados según la relevancia BM25.</p></li>
</ol>
<h2 id="Create-a-collection-for-BM25-full-text-search" class="common-anchor-header">Crear una colección para la búsqueda de texto completo con BM25<button data-href="#Create-a-collection-for-BM25-full-text-search" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Para habilitar la búsqueda de texto completo basada en BM25, debes preparar una colección con los campos necesarios, definir una función BM25 para generar vectores dispersos, configurar un índice y, a continuación, crear la colección.</p>
...
<li><p><code translate="no">text</code>: almacena los datos de texto sin procesar para las operaciones de búsqueda de texto completo. El campo puede utilizar <code translate="no">VARCHAR</code> para texto acotado o <code translate="no">TEXT</code> para contenido de origen extenso.</p></li>
...
<table>
   ...
   <tr>
     ...
     <td><p>El nombre de la función. Esta función convierte el texto sin procesar del campo « <code translate="no">text</code> » en vectores dispersos compatibles con BM25 que se almacenarán en el campo « <code translate="no">sparse</code> ».</p></td>
   </tr>
   <tr>
   ...
   <tr>
     ...
     <td><p>El tipo de función que se va a utilizar. Debe ser <code translate="no">FunctionType.BM25</code>.</p></td>
   </tr>
</table>
<div class="alert note">
<p>Si varios campos de texto requieren procesamiento BM25, define <strong>una función BM25 por campo</strong>, cada una con un nombre y un campo de salida únicos.</p>
</div>
<h3 id="Configure-the-index" class="common-anchor-header">Configurar el índice<button data-href="#Configure-the-index" class="anchor-icon" translate="no">
    ...
    </button></h3><p>Tras definir el esquema con los campos necesarios y la función integrada, configura el índice para tu colección.</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#go">   Go</a>
...
<table>
   ...
   <tr>
     ...
     <td><p>El algoritmo utilizado para crear y consultar el índice invertido disperso BM25. Valores válidos:</p><ul><li><p><code translate="no">"DAAT_MAXSCORE"</code> (por defecto): Procesamiento de consultas «Document-at-a-Time» con MaxScore. Esta opción es adecuada para cargas de trabajo de búsqueda de texto completo con valores <em>k</em> altos o consultas con muchos términos. Para obtener más información, consulte <a href="https://dl.acm.org/doi/10.1016/0306-4573%2895%2900020-H">«Evaluación de consultas: estrategias y optimizaciones</a>».</p></li><li><p><code translate="no">"DAAT_WAND"</code>: Procesamiento de consultas «Document-at-a-Time» WAND. Esta opción es adecuada para cargas de trabajo de búsqueda de texto completo con valores <em>k</em> pequeños o consultas cortas. Para más información, consulta <a href="https://dl.acm.org/doi/10.1145/956863.956944">«Evaluación eficiente de consultas mediante un proceso de recuperación de dos niveles</a>».</p></li><li><p><code translate="no">"TAAT_NAIVE"</code>: Procesamiento básico de consultas «término por término». Utilice esta opción como referencia o cuando necesite que la puntuación se adapte dinámicamente a las estadísticas globales de la colección, como la longitud media de los documentos.</p></li><li><p><code translate="no">"BLOCK_MAX_MAXSCORE"</code>: Procesamiento de consultas MaxScore con metadatos de puntuación máxima a nivel de bloque. Para más información, consulte <a href="https://dl.acm.org/doi/10.1145/2009916.2010048">«Recuperación más rápida</a> de <a href="https://dl.acm.org/doi/10.1145/2009916.2010048">los k mejores documentos mediante índices Block-Max</a>».</p></li><li><p><code translate="no">"BLOCK_MAX_WAND"</code>: Procesamiento de consultas WAND con metadatos de puntuación máxima a nivel de bloque. Para obtener más información, consulta <a href="https://dl.acm.org/doi/10.1145/2009916.2010048">«Recuperación más rápida</a> de <a href="https://dl.acm.org/doi/10.1145/2009916.2010048">los k documentos principales mediante índices Block-Max</a>».</p></li></ul></td>
   </tr>
   <tr>
     <td><p><code translate="no">params.bm25_k1</code></p></td>
     <td><p>Controla la saturación de la frecuencia de los términos. Los valores más altos aumentan la importancia de la frecuencia de los términos en la clasificación de los documentos. Rango recomendado: [1,2; 2,0]. Valor predeterminado: 1,2.</p></td>
   </tr>
   <tr>
     <td><p><code translate="no">params.bm25_b</code></p></td>
     <td><p>Controla el grado de normalización de la longitud de los documentos. Normalmente se utilizan valores entre 0 y 1, con un valor por defecto de 0,75. Un valor de 0 significa que no hay normalización de la longitud, mientras que un valor de 1 significa normalización completa de la longitud.</p></td>
   </tr>
</table>
<h3 id="Create-the-collection" class="common-anchor-header">Crear la colección<button data-href="#Create-the-collection" class="anchor-icon" translate="no">
    ...
    </button></h3><p>Ahora crea la colección utilizando el esquema y los parámetros de índice definidos.</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#go">   Go</a>
...
<h2 id="Insert-text-data" class="common-anchor-header">Insertar datos de texto<button data-href="#Insert-text-data" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Una vez configuradas la colección y el índice, ya estás listo para insertar datos de texto. En este proceso, solo tienes que proporcionar el texto sin procesar. La función integrada que hemos definido anteriormente genera automáticamente el vector disperso correspondiente para cada entrada de texto.</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#go">   Go</a>
...
<button class="copy-code-btn"></button></code></pre>
<h2 id="Perform-full-text-search" class="common-anchor-header">Realizar búsquedas de texto completo<button data-href="#Perform-full-text-search" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>Una vez que hayas insertado datos en tu colección, podrás realizar búsquedas de texto completo utilizando consultas de texto sin procesar. Milvus convierte automáticamente tu consulta en un vector disperso y clasifica los resultados de búsqueda coincidentes utilizando el algoritmo BM25, para luego devolver los resultados topK (<code translate="no">limit</code>).</p>
<div class="alert note">
...
<table>
   ...
   <tr>
     ...
     <td><p>Un diccionario que contiene los parámetros de búsqueda.</p></td>
   </tr>
   <tr>
     <td><p><code translate="no">params.drop_ratio_search</code></p></td>
     <td><p>Proporción de términos de baja importancia que se deben ignorar durante la búsqueda. El valor debe estar en el intervalo [0,0; 1,0). Para más detalles, consulta <a href="/docs/es/sparse_vector.md">«Vector disperso</a>».</p></td>
   </tr>
   <tr>
     <td></td>
     <td></td>
   ...
   <tr>
     ...
     <td><p>Texto de consulta sin procesar en lenguaje natural. Milvus convierte automáticamente tu consulta de texto en vectores dispersos utilizando la función BM25; no proporciones vectores precalculados.</p></td>
   </tr>
   <tr>
   ...
   <tr>
     ...
     <td><p>Número máximo de coincidencias principales que se deben devolver.</p></td>
   </tr>
</table>
<h2 id="FAQ" class="common-anchor-header">Preguntas frecuentes<button data-href="#FAQ" class="anchor-icon" translate="no">
    ...
    </button></h3><p>No, no es posible acceder directamente a los vectores dispersos generados por la función BM25 ni mostrarlos en la búsqueda de texto completo. A continuación se detallan los motivos:</p>
<ul>
<li><p>La función BM25 genera vectores dispersos internamente para la clasificación y la recuperación</p></li>
<li><p>Estos vectores se almacenan en el campo «sparse», pero no pueden incluirse en <code translate="no">output_fields</code></p></li>
<li><p>Solo se pueden exportar los campos de texto originales y los metadatos (como <code translate="no">id</code> y <code translate="no">text</code>)</p></li>
</ul>
<p>Ejemplo:</p>
...
milvus::SearchRequest request = milvus::<span class="hljs-built_in">SearchRequest</span>()
    ...
    .<span class="hljs-built_in">AddOutputField</span>(<span class="hljs-string">&quot;text&quot;</span>);
<button class="copy-code-btn"></button></code></pre>
<h3 id="Why-do-I-need-to-define-a-sparse-vector-field-if-I-cant-access-it" class="common-anchor-header">¿Por qué tengo que definir un campo de vectores dispersos si no puedo acceder a él?<button data-href="#Why-do-I-need-to-define-a-sparse-vector-field-if-I-cant-access-it" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>El campo de vectores dispersos sirve como índice de búsqueda interno, de forma similar a los índices de las bases de datos con los que los usuarios no interactúan directamente.</p>
<p><strong>Fundamento del diseño</strong>:</p>
<ul>
<li><p>Separación de preocupaciones: tú trabajas con texto (entrada/salida), Milvus se encarga de los vectores (procesamiento interno).</p></li>
<li><p>Rendimiento: los vectores dispersos precalculados permiten una clasificación rápida mediante BM25 durante las consultas</p></li>
<li><p>Experiencia de usuario: oculta las complejas operaciones vectoriales tras una sencilla interfaz de texto</p></li>
</ul>
<p><strong>Si necesitas acceder a los vectores</strong>:</p>
<ul>
<li><p>Utiliza operaciones manuales con vectores dispersos en lugar de la búsqueda de texto completo</p></li>
<li><p>Crea colecciones independientes para flujos de trabajo personalizados con vectores dispersos</p></li>
</ul>
<p>Para obtener más información, consulta <a href="/docs/es/sparse_vector.md">«Vector disperso</a>».</p>
