...
<ol>
<li><p><strong>Recuperación de candidatos.</strong> Milvus ejecuta una búsqueda ANN para encontrar las entidades más cercanas al vector de consulta. A continuación, la etapa de agrupación retiene un número limitado de candidatos para cada clave compuesta completa. Este límite de candidatos por clave es el mayor valor de « <code translate="no">TopHits.size</code> » en cualquier parte del árbol de agregación, o « <code translate="no">1</code> » cuando ningún nivel configura « <code translate="no">top_hits</code> ».</p></li>
...
<p>Con « <code translate="no">sub_aggregation</code> », Milvus repite los pasos 2 y 3 dentro de cada compartimento principal. Los cambios en la recuperación de la red neuronal artificial (ANN) o en el presupuesto de candidatos por clave pueden modificar el número de compartimentos, las métricas, el orden, los resultados y los resultados anidados.</p>
...
<li><p><strong>Combinaciones no admitidas:</strong> La agregación de búsqueda no se puede combinar con <code translate="no">offset</code>, iteradores de búsqueda, búsqueda híbrida, un resaltador ni búsqueda por agrupación.</p></li>
...
<p>Utilice « <code translate="no">1</code> » como último factor cuando ningún nivel configure « <code translate="no">TopHits</code> ». Por ejemplo, un vector de consulta, 10 buckets raíz, cinco buckets secundarios por cada bucket raíz y dos resultados por cada bucket secundario producen un máximo configurado de:</p>
...
<p>Los ejemplos que se muestran a continuación utilizan una colección de productos con campos de marca, categoría, color, precio y valoración. Todos los nombres de marcas, nombres de productos, precios, valoraciones y resultados de búsqueda son datos de ejemplo sintéticos. Despliega la siguiente sección para crear la colección y definir las variables de búsqueda compartidas.</p>
<p><details></p>
...
<button class="copy-code-btn"></button></code></pre>
<p>Cuando se establece « <code translate="no">search_aggregation</code> », PyMilvus no devuelve resultados de entidades ordinarias en « <code translate="no">result[0]</code> ». En su lugar, lee la respuesta del grupo en « <code translate="no">result.agg_buckets[0]</code> ». El parámetro « <code translate="no">output_fields</code> » controla qué campos escalares aparecen en cada asignación « <code translate="no">AggregationHit.fields</code> » devuelta; Milvus puede seguir utilizando campos de origen de métricas y de ordenación que no figuren en « <code translate="no">output_fields</code> ».</p>
<p><details></p>
...
<button class="copy-code-btn"></button></code></pre>
<p>Con <code translate="no">L2</code>, calcula el valor mínimo de <code translate="no">_score</code> y ordena el alias de la métrica en orden ascendente, de modo que los intervalos con la distancia más baja aparezcan en primer lugar.</p>
<p></details></p>
<p><details></p>
...
<h2 id="FAQ" class="common-anchor-header">Preguntas frecuentes<button data-href="#FAQ" class="anchor-icon" translate="no">
    ...
    </button></h3><p>La agregación de búsqueda resume los candidatos de la red neuronal artificial (ANN) retenidos. No ejecuta una agregación de toda la colección.</p>
...
<p>La aproximación cobra mayor importancia cuando « <code translate="no">order</code> » utiliza un alias de métrica. Los cambios en la recuperación de la búsqueda pueden modificar los valores de las métricas y, por lo tanto, alterar qué compartimentos encajan en « <code translate="no">SearchAggregation.size</code> ». La agregación anidada puede amplificar este efecto, ya que cada nivel secundario opera sobre las entidades disponibles en su compartimento principal.</p>
