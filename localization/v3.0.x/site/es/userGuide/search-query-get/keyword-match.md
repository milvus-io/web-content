...
<h2 id="Enable-text-match" class="common-anchor-header">Activar la concordancia de texto<button data-href="#Enable-text-match" class="anchor-icon" translate="no">
    ...
    </button></h2><p>La concordancia de texto funciona en el <a href="/docs/es/string.md"><code translate="no">VARCHAR</code></a> que es esencialmente el tipo de datos cadena en Milvus. Para habilitar la coincidencia de texto, establezca <code translate="no">enable_analyzer</code> y <code translate="no">enable_match</code> en <code translate="no">True</code> y luego, opcionalmente, configure un <a href="/docs/es/analyzer-overview.md">analizador</a> para el análisis de texto cuando defina el esquema de su colección.</p>
