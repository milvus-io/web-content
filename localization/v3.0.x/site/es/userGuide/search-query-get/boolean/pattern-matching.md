...
summary: >-
  ...
  y expresiones regulares RE2. Utiliza filtros de patrones para encontrar
  coincidencias con prefijos, sufijos, subcadenas, códigos estructurados,
  dominios de correo electrónico, rutas URL y otros patrones de cadenas en
...
<button class="copy-code-btn"></button></code></pre>
<p>Los ejemplos de esta página se centran en la expresión asignada a « <code translate="no">filter</code> ». Puede utilizar la misma sintaxis de expresión de filtro en operaciones de Milvus que acepten un filtro escalar, como « <code translate="no">query</code> », « <code translate="no">search</code> » y la búsqueda híbrida.</p>
...
<tr><td>Números, valores booleanos, vectores, <code translate="no">TEXT</code> u otros objetivos que no sean de tipo<code translate="no">VARCHAR</code> </td><td>No</td><td>No</td><td>La coincidencia de patrones solo está disponible para valores de tipo « <code translate="no">VARCHAR</code> », rutas JSON que se resuelven en cadenas o elementos indexados de tipo « <code translate="no">ARRAY&lt;VARCHAR&gt;</code> ».</td></tr>
</tbody>
</table>
...
<p>Si necesitas una coincidencia exacta de cadena, te recomendamos que utilices « <code translate="no">==</code> » en lugar de la coincidencia de patrones. Utiliza « <code translate="no">LIKE</code> » o expresiones regulares solo cuando el filtro tenga que coincidir con un patrón.</p>
<table>
<thead>
<tr><th>Requisito</th><th>Operador recomendado</th><th>Ejemplo</th><th>Descripción</th></tr>
</thead>
<tbody>
<tr><td>Igualdad exacta de cadena</td><td><code translate="no">==</code></td><td><code translate="no">status == &quot;active&quot;</code></td><td>Coincidencia exacta de la cadena « <code translate="no">active</code> ».</td></tr>
...
<h3 id="Escaping-wildcards-in-a-LIKE-pattern" class="common-anchor-header">Escapar los comodines en un patrón «LIKE»<button data-href="#Escaping-wildcards-in-a-LIKE-pattern" class="anchor-icon" translate="no">
    ...
    </button></h3><p>En los patrones <code translate="no">LIKE</code>, <code translate="no">%</code> coincide con cero o más caracteres y <code translate="no">_</code> coincide exactamente con un carácter. Para que <code translate="no">%</code>, <code translate="no">_</code> o <code translate="no">\</code> coincidan literalmente, escapa el carácter con una barra invertida (<code translate="no">\</code>):</p>
<ul>
...
<h2 id="Use-regex" class="common-anchor-header">Utiliza expresiones regulares<span class="beta-tag" style="background-color:rgb(0, 179, 255);color:white" translate="no">Compatible with Milvus 3.0.x</span><button data-href="#Use-regex" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<tr><td><code translate="no">!~</code></td><td>Excluye los valores que satisfacen el patrón de expresión regular.</td><td><code translate="no">filter = 'message !~ &quot;^DEBUG&quot;'</code></td></tr>
</tbody>
</table>
<h3 id="Use-raw-string-literals" class="common-anchor-header">Utiliza literales de cadena sin formatear<button data-href="#Use-raw-string-literals" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Se recomienda el uso de literales de cadena sin escapar para patrones de expresiones regulares que contengan barras invertidas. En una cadena sin escapar, escrita como <code translate="no">r&quot;...&quot;</code> o <code translate="no">r'...'</code>, las barras invertidas se pasan tal cual al motor de expresiones regulares. Esto evita el escape adicional que requieren los literales de cadena normales.</p>
<p>Por ejemplo:</p>
<div class="multipleCode">
 <a href="#python">Python</a>
 <a href="#java"> Java</a>
...
<button class="copy-code-btn"></button></code></pre>
<p>Esto coincide con cadenas que terminan en <code translate="no">.json</code>, como <code translate="no">report.json</code>.</p>
<p>Sin una cadena sin procesar en la expresión del filtro de Milvus, los literales de cadena normales procesan las secuencias de escape antes de que se evalúe el patrón de expresión regular. Por lo tanto, los caracteres literales escapados pueden requerir barras invertidas adicionales en la cadena del lenguaje de host.</p>
<h3 id="Common-regex-patterns" class="common-anchor-header">Patrones comunes de expresiones regulares<button data-href="#Common-regex-patterns" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<button class="copy-code-btn"></button></code></pre>
<p>Cuando se busquen metacaracteres de expresiones regulares de forma literal, hay que escaparlos en el patrón de expresión regular. Por ejemplo, para buscar un punto literal (<code translate="no">\.</code> en una expresión regular), escribe <code translate="no">\\.</code> en una cadena de código fuente de Python, Java, Go o Node.js:</p>
<div class="multipleCode">
 <a href="#python">Python</a>
 <a href="#java"> Java</a>
 <a href="#go"> Go</a>
...
<button class="copy-code-btn"></button></code></pre>
<p>Para coincidir con el valor completo del campo, utiliza los anclajes <code translate="no">^</code> y <code translate="no">$</code>:</p>
<div class="multipleCode">
 <a href="#python">Python</a>
 <a href="#java"> Java</a>
 <a href="#go"> Go</a>
...
<p>Los filtros de expresiones regulares no coinciden con valores nulos. Esto se aplica tanto a « <code translate="no">=~</code> » como a « <code translate="no">!~</code> ». Si quieres excluir un patrón de expresión regular pero conservar los valores nulos, añade explícitamente « <code translate="no">OR field IS NULL</code> »:</p>
<div class="multipleCode">
 <a href="#python">Python</a>
 <a href="#java"> Java</a>
 <a href="#go"> Go</a>
...
<h2 id="Accelerate-pattern-matching-with-indexes" class="common-anchor-header">Acelera la búsqueda de coincidencias con índices<button data-href="#Accelerate-pattern-matching-with-indexes" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<tr><td>Filtros de cadena de tipo prefijo, exactos o de igualdad, especialmente en campos con cardinalidad baja o moderada</td><td><code translate="no">STL_SORT</code>, « <code translate="no">INVERTED</code> » o <code translate="no">BITMAP</code></td><td>Puede resultar más eficaz cuando el campo tiene valores repetidos o cuando el filtro se aproxima a una coincidencia exacta. Para más detalles, consulta <a href="/docs/es/stl-sort.md">STL_SORT</a>, <a href="/docs/es/inverted.md">INVERTED</a> y <a href="/docs/es/bitmap.md">BITMAP</a>.</td></tr>
<tr><td>Patrones de expresiones regulares sin literales fijos, o patrones dominados por clases de caracteres, tokens cortos o comodines</td><td>Realice pruebas de rendimiento antes de confiar en la aceleración por índice</td><td>Estos patrones pueden ofrecer una selectividad de índice limitada y pueden recurrir a exploraciones más amplias.</td></tr>
</tbody>
</table>
