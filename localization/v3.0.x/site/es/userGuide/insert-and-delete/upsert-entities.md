...
summary: >-
  La operación «upsert» ofrece una forma cómoda de insertar o actualizar
...
<h2 id="Overview" class="common-anchor-header">Descripción general<button data-href="#Overview" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Puedes utilizar « <code translate="no">upsert</code> » para insertar una nueva entidad o actualizar una ya existente, dependiendo de si la clave principal proporcionada en la solicitud de «upsert» existe en la colección. Si no se encuentra la clave principal, se lleva a cabo una operación de inserción. De lo contrario, se realizará una operación de actualización. Las actualizaciones parciales en colecciones de tipo « <code translate="no">autoID</code> » son una excepción: solo actualizan entidades existentes, tal y como se describe a continuación.</p>
...
<p>Si la colección de destino tiene habilitado el « <code translate="no">autoid</code> » en su campo primario, Milvus generará una nueva clave primaria para los datos incluidos en la carga útil de la solicitud antes de insertarlos.</p>
<p>En el caso de los campos en los que esté habilitada la opción « <code translate="no">nullable</code> », puedes omitirlos en la solicitud « <code translate="no">upsert</code> » si no requieren ninguna actualización.</p>
<h3 id="Upsert-in-merge-mode--Milvus-v262+" class="common-anchor-header">Upsert en modo de fusión<span class="beta-tag" style="background-color:rgb(0, 179, 255);color:white" translate="no">Compatible with Milvus v2.6.2+</span><button data-href="#Upsert-in-merge-mode--Milvus-v262+" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Utilice el modo «merge» para actualizar campos específicos de una entidad existente sin modificar los demás campos.</p>
<p><span class="img-wrapper">
   ...
   <span>Upsert en modo «merge»</span>
...
<p>Si la clave primaria no existe, el resultado depende de si está habilitada la opción « <code translate="no">autoID</code> »:</p>
<ul>
<li><strong> <code translate="no">autoID</code> Si la inserción de la clave primaria está desactivada</strong>, Milvus intenta insertar una nueva entidad con la clave primaria que has proporcionado. La solicitud se realiza correctamente si cumple los requisitos normales de inserción. Si falta algún campo obligatorio, la solicitud falla con un error de campo faltante. Los campos nulos y los campos con valores predeterminados pueden omitirse, al igual que en una inserción normal.</li>
...
<p>Si una actualización parcial falla con un error de campo faltante, comprueba si existe la entidad de destino. Sin una entidad existente, Milvus no puede recuperar los valores de los campos que has omitido.</p>
<p>Para las entidades nuevas, utiliza « <code translate="no">insert</code> » o un «upsert» en modo de anulación. Utiliza el modo de fusión para las actualizaciones posteriores de campos individuales.</p>
<p>Para los campos « <code translate="no">ARRAY</code> », el modo de fusión admite dos operadores en Milvus v2.6.17 y versiones posteriores: « <code translate="no">ARRAY_APPEND</code> » y « <code translate="no">ARRAY_REMOVE</code> ». Estos operadores te permiten añadir elementos o eliminar elementos coincidentes de un campo « <code translate="no">ARRAY</code> » existente, sin necesidad de consultar primero la entidad para recuperar su valor actual. Para más detalles, consulta <a href="/docs/es/upsert-entities.md#Upsert-ARRAY-fields-in-merge-mode">«Upsert de campos ARRAY en modo de fusión</a>».</p>
...
<p>Supongamos que el campo <code translate="no">issue</code> puede ser nulo. Al realizar una operación de «upsert» en estos campos, ten en cuenta que:</p>
<ul>
...
<li><p>Para conservar el valor original del campo « <code translate="no">issue</code> », debe habilitar « <code translate="no">partial_update</code> » y omitir el campo « <code translate="no">issue</code> », o bien incluir el campo « <code translate="no">issue</code> » con su valor original en la solicitud « <code translate="no">upsert</code> ».</p></li>
</ul></li>
...
<p>De forma predeterminada, un campo ` <code translate="no">ARRAY</code> ` en modo «merge» sigue la semántica <strong>de REPLACE</strong>: el valor incluido en la solicitud sobrescribe la matriz existente. Para actualizaciones más precisas, Milvus v2.6.17 y versiones posteriores también admiten dos operadores:</p>
<ul>
<li><p><code translate="no">ARRAY_APPEND</code> añade los elementos de la carga útil de la solicitud a la matriz existente.</p></li>
<li><p><code translate="no">ARRAY_REMOVE</code> elimina del conjunto existente todos los elementos que coincidan con un valor de la carga útil de la solicitud.</p></li>
</ul>
...
<p>Al realizar un «upsert» de un campo StructArray en una entidad, se sobrescribe el valor del campo. Para ello, es necesario proporcionar una lista de diccionarios, cada uno de los cuales contiene todos los subcampos definidos en el esquema de la estructura, incluso cuando se realiza el «upsert» en modo de fusión.</p>
...
<h3 id="Limits--Restrictions" class="common-anchor-header">Límites y restricciones<button data-href="#Limits--Restrictions" class="anchor-icon" translate="no">
    ...
    </button></h3><p>Según lo expuesto anteriormente, hay varios límites y restricciones que deben tenerse en cuenta:</p>
<ul>
...
<li><p>En el caso de cualquier campo derivado de otro mediante funciones, Milvus eliminará el campo derivado durante la operación «upsert» para permitir su recálculo.</p></li>
</ul>
<h2 id="Upsert-entities-in-a-collection" class="common-anchor-header">Upsert de entidades en una colección<button data-href="#Upsert-entities-in-a-collection" class="anchor-icon" translate="no">
    ...
    </button></h2><p>En esta sección, realizaremos una operación de «upsert» de entidades en una colección denominada « <code translate="no">my_collection</code> ». Esta colección solo tiene dos campos, denominados « <code translate="no">id</code> », « <code translate="no">vector</code> », « <code translate="no">title</code> » y « <code translate="no">issue</code> ». El campo « <code translate="no">id</code> » es el campo principal, mientras que los campos « <code translate="no">title</code> » y « <code translate="no">issue</code> » son campos escalares.</p>
...
<h2 id="Upsert-ARRAY-fields-in-merge-mode--Milvus-2617+" class="common-anchor-header">Upsert de campos ARRAY en modo «merge»<span class="beta-tag" style="background-color:rgb(0, 179, 255);color:white" translate="no">Compatible with Milvus 2.6.17+</span><button data-href="#Upsert-ARRAY-fields-in-merge-mode--Milvus-2617+" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Antes de Milvus v2.6.17, actualizar parte de un campo de tipo « <code translate="no">ARRAY</code> » requería un flujo de lectura-modificación-escritura del lado del cliente: consultar la matriz existente, modificarla en el código de la aplicación y realizar un «upsert» con el valor de sustitución completo. Los operadores de actualización parcial (<code translate="no">ARRAY_APPEND</code> y <code translate="no">ARRAY_REMOVE</code>) permiten enviar solo los elementos que se desean añadir o eliminar, lo que reduce la lógica del lado del cliente y evita una lectura adicional antes de la inserción o actualización.</p>
<p>Supongamos que la entidad con clave primaria ` <code translate="no">1</code> ` ya tiene ` <code translate="no">tags = [&quot;new&quot;, &quot;trial&quot;]</code>`. Antes de los operadores de actualización parcial, añadir el elemento ` <code translate="no">&quot;premium&quot;</code> ` a un array requería realizar una operación «upsert» del array de sustitución completo:</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#javascript">   NodeJS</a>
...
<button class="copy-code-btn"></button></code></pre>
<p>Con <code translate="no">ARRAY_APPEND</code>, solo hay que enviar el elemento que se va a añadir:</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#javascript">   NodeJS</a>
...
<li><p>En Milvus v2.6.17 y versiones posteriores, los campos « <code translate="no">ARRAY_APPEND</code> » y « <code translate="no">ARRAY_REMOVE</code> » admiten campos « <code translate="no">ARRAY</code> » cuyo « <code translate="no">element_type</code> » sea « <code translate="no">BOOL</code> », « <code translate="no">INT8</code> », « <code translate="no">INT16</code> », « <code translate="no">INT32</code> », « <code translate="no">INT64</code> », « <code translate="no">FLOAT</code> », « <code translate="no">DOUBLE</code> » o « <code translate="no">VARCHAR</code> ».</p></li>
...
<pre><code translate="no" class="language-bash"><span class="hljs-comment"># restful</span>
<button class="copy-code-btn"></button></code></pre>
<h2 id="Upsert-StructArray-field-in-merge-mode" class="common-anchor-header">Upsert del campo StructArray en modo «merge»<button data-href="#Upsert-StructArray-field-in-merge-mode" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>Al realizar una operación «upsert» en un campo StructArray de una entidad, se sobrescribe el valor del campo. Esto significa que debes incluir todos los subcampos definidos en el esquema de la estructura al realizar una operación «upsert» en un campo StructArray.</p>
<p>El siguiente ejemplo muestra cómo realizar un «upsert» del campo « <code translate="no">chunks</code> » en modo de fusión, un campo «StructArray» con 6 subcampos. Cuando finaliza la operación, el campo « <code translate="no">chunks</code> » de la entidad con id 1 se establece en el array con las estructuras de dos elementos proporcionadas en la solicitud.</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#javascript">   NodeJS</a>
