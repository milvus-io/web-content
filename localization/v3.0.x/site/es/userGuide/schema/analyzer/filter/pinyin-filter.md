...
summary: >-
  El filtro de pinyin convierte los tokens de caracteres chinos en tokens de
  pinyin durante el análisis del texto, lo que permite realizar búsquedas
  basadas en pinyin en textos chinos.
beta: Milvus 3.0.x
---
...
<h2 id="Configuration" class="common-anchor-header">Configuración<button data-href="#Configuration" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Para utilizar las opciones predeterminadas, especifica « <code translate="no">&quot;pinyin&quot;</code> » en la sección « <code translate="no">filter</code> » de « <code translate="no">analyzer_params</code> ».</p>
...
<p>Esta configuración abreviada conserva los tokens chinos originales y genera tokens pinyin a nivel de carácter. No genera pinyin unido ni iniciales pinyin a menos que se habiliten esas opciones de forma explícita.</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>El filtro admite los siguientes parámetros.</p>
<table>
<thead>
<tr><th>Parámetro</th><th>Tipo</th><th>Valor por defecto</th><th>Descripción</th></tr>
</thead>
<tbody>
...
<p>El filtro actúa sobre los tokens generados por el tokenizador. Para texto en chino, utilízalo con un tokenizador como <code translate="no">jieba</code>.</p>
<h2 id="Examples" class="common-anchor-header">Ejemplos<button data-href="#Examples" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Antes de aplicar la configuración del analizador al esquema de su colección, compruebe su comportamiento con <code translate="no">run_analyzer</code>.</p>
...
<h3 id="Match-Chinese-text-with-character-level-Pinyin" class="common-anchor-header">Hacer coincidir texto en chino con tokens de pinyin a nivel de carácter<button data-href="#Match-Chinese-text-with-character-level-Pinyin" class="anchor-icon" translate="no">
    ...
    </button></h3><p>El filtro predeterminado <code translate="no">pinyin</code> conserva los tokens chinos originales y genera tokens de pinyin a nivel de carácter.</p>
<pre><code translate="no" class="language-python">analyzer_params = {
    <span class="hljs-string">&quot;tokenizer&quot;</span>: <span class="hljs-string">&quot;jieba&quot;</span>,
...
<button class="copy-code-btn"></button></code></pre>
<h3 id="Match-Chinese-terms-with-joined-Pinyin" class="common-anchor-header">Coincidir términos chinos con pinyin unido<button data-href="#Match-Chinese-terms-with-joined-Pinyin" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Activa <code translate="no">keep_joined_full_pinyin</code> cuando necesites que un término chino coincida con su forma completa de pinyin unificado.</p>
<pre><code translate="no" class="language-python">analyzer_params = {
    <span class="hljs-string">&quot;tokenizer&quot;</span>: <span class="hljs-string">&quot;jieba&quot;</span>,
...
<button class="copy-code-btn"></button></code></pre>
<h3 id="Match-Chinese-terms-with-Pinyin-initials" class="common-anchor-header">Coincidir términos chinos con las iniciales del pinyin<button data-href="#Match-Chinese-terms-with-Pinyin-initials" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Activa la opción « <code translate="no">keep_separate_first_letter</code> » cuando necesites que un término chino coincida con las iniciales de su forma en pinyin.</p>
<pre><code translate="no" class="language-python">analyzer_params = {
    <span class="hljs-string">&quot;tokenizer&quot;</span>: <span class="hljs-string">&quot;jieba&quot;</span>,
    <span class="hljs-string">&quot;filter&quot;</span>: [
        {
