...
summary: >-
  El filtro «removepunct» elimina los tokens que contienen signos de puntuación
  o espacios en blanco. En el caso de los tokenizadores que conservan los signos
  de puntuación dentro de las palabras, elimina el token completo afectado.
beta: Milvus 2.5.11+
---
<h1 id="Remove-Punct" class="common-anchor-header">Eliminar signos de puntuación<span class="beta-tag" style="background-color:rgb(0, 179, 255);color:white" translate="no">Compatible with Milvus 2.5.11+</span><button data-href="#Remove-Punct" class="anchor-icon" translate="no">
    ...
    </button></h1><p>El filtro « <code translate="no">removepunct</code> » elimina de la secuencia de tokens aquellos que contienen signos de puntuación o espacios en blanco. Úsalo cuando quieras un procesamiento de texto más limpio que se centre en las palabras de contenido significativo en lugar de en los signos de puntuación.</p>
<div class="alert note">
<p>Este filtro resulta más eficaz con los tokenizadores « <code translate="no">jieba</code> », « <code translate="no">lindera</code> » y « <code translate="no">icu</code> », que conservan la puntuación como tokens independientes (por ejemplo, « <code translate="no">&quot;Hello!&quot;</code> » → « <code translate="no">[&quot;Hello&quot;, &quot;!&quot;]</code> »). El tokenizador <code translate="no">standard</code> descarta la puntuación durante la tokenización. El tokenizador <code translate="no">whitespace</code> conserva la puntuación, incluida la que se encuentra dentro de un token. Cuando se combina con <code translate="no">whitespace</code>, <code translate="no">removepunct</code> elimina todo el token si contiene puntuación o espacios en blanco; no elimina caracteres individuales del token.</p>
</div>
<h2 id="Configuration" class="common-anchor-header">Configuración<button data-href="#Configuration" class="anchor-icon" translate="no">
    ...
    </button></h2><p>El filtro « <code translate="no">removepunct</code> » está integrado en Milvus. Para utilizarlo, basta con especificar su nombre en la sección « <code translate="no">filter</code> » dentro de « <code translate="no">analyzer_params</code> ».</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>El filtro « <code translate="no">removepunct</code> » opera sobre los términos generados por el tokenizador, por lo que debe utilizarse en combinación con uno.</p>
<p>Una vez definidos los « <code translate="no">analyzer_params</code> », puedes aplicarlos a un campo « <code translate="no">VARCHAR</code> » al definir el esquema de una colección. Esto permite a Milvus procesar el texto de ese campo utilizando el analizador especificado para una tokenización y un filtrado eficientes. Para más detalles, consulta <a href="/docs/es/analyzer-overview.md#Example-use">«Ejemplo de uso</a>».</p>
