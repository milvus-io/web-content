...
<p>Milvus proporciona el tipo de campo escalar « <code translate="no">TEXT</code> » para almacenar texto fuente extenso directamente junto con las entidades. Entre los valores típicos se incluyen pasajes, documentos largos, cuerpos de artículos, tickets y registros. A diferencia de « <code translate="no">VARCHAR</code> », que requiere una longitud máxima fija de bytes ( <code translate="no">max_length</code>), « <code translate="no">TEXT</code> » no exige establecer una longitud máxima de bytes en el esquema de la colección.</p>
...
<li>En Milvus 3.0.0, los campos « <code translate="no">TEXT</code> » no son compatibles con las colecciones externas.</li>
...
<tr><td>Filtrado</td><td>Se utiliza para metadatos de cadenas cortas que deben aparecer en expresiones de filtro, como « <code translate="no">category == &quot;news&quot;</code> » o « <code translate="no">tag in [&quot;ai&quot;, &quot;database&quot;]</code> ».</td><td>No está pensado para el filtrado habitual de metadatos.</td></tr>
</tbody>
</table>
<p>Para obtener más información sobre los campos « <code translate="no">VARCHAR</code> », consulta el <a href="/docs/es/string.md">campo VarChar</a>.</p>
