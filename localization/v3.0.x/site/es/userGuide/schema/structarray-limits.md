...
<tr><td>Ruta de los subcampos</td><td>Utiliza rutas del tipo <code translate="no">structArray[subfield]</code>, como <code translate="no">chunks[emb]</code>, para índices, objetivos de búsqueda, campos de salida y filtros. No utilices <code translate="no">chunks.emb</code>.</td></tr>
...
<tr><td>Añadir campo dinámicamente</td><td>La adición de un campo StructArray a una colección existente depende de la versión y requiere que el campo añadido sea nulo.</td></tr>
</tbody>
</table>
...
<tbody>
<tr><td>«Struct» no es un tipo de campo de primer nivel.</td><td>Crea un campo StructArray como « <code translate="no">datatype=DataType.ARRAY</code> » con « <code translate="no">element_type=DataType.STRUCT</code> » y « <code translate="no">struct_schema</code> ».</td></tr>
...
<tr><td>No se admiten funciones dentro de StructArray.</td><td>No defina funciones de campo para los campos StructArray ni para sus subcampos.</td></tr>
</tbody>
</table>
...
<h2 id="Supported-subfield-data-types" class="common-anchor-header">Tipos de datos de subcampos compatibles<button data-href="#Supported-subfield-data-types" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>Los subcampos de StructArray se asignan a un almacenamiento físico de tipo matriz. En la siguiente tabla se enumeran los tipos físicos compatibles y no compatibles.</p>
<table>
<thead>
...
<tr><td><code translate="no">Array</code></td><td>No compatible</td><td>Utiliza <code translate="no">VARCHAR</code>, no <code translate="no">String</code>.</td></tr>
...
<tbody>
<tr><td>Campo StructArray nulo</td><td>Solo se admite en versiones que incluyan compatibilidad con StructArray nulos y matrices vectoriales nulas.</td></tr>
...
<tr><td>Adición dinámica de un campo StructArray</td><td>La adición de un campo StructArray a una colección existente solo está admitida en versiones que incluyan compatibilidad con campos StructArray dinámicos.</td></tr>
...
<tr><td>Alineación con el esquema</td><td>Cada elemento de Struct debe coincidir con el esquema de Struct.</td></tr>
...
<h2 id="Index-and-metric-limits" class="common-anchor-header">Límites de índices y métricas<button data-href="#Index-and-metric-limits" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Un subcampo vectorial de tipo StructArray puede indexarse tanto para la búsqueda en EmbeddingList como para la búsqueda a nivel de elemento. Un mismo subcampo vectorial no puede utilizar ambas familias de métricas, ya que cada campo vectorial o subcampo vectorial solo admite un índice.</p>
<table>
<thead>
...
<p>Para conocer la matriz de tipos de índice y de métricas admitidos, consulta <a href="/docs/es/index-structarray-fields.md">Campos StructArray de índice</a>.</p>
...
<tbody>
<tr><td>Búsqueda básica en EmbeddingList</td><td>Compatible con subcampos vectoriales de StructArray indexados con métricas « <code translate="no">MAX_SIM*</code> ». Devuelve resultados a nivel de entidad.</td></tr>
...
<tr><td>Salida de desplazamiento</td><td>El desplazamiento está disponible para los resultados de búsqueda a nivel de elemento. La búsqueda en EmbeddingList devuelve resultados a nivel de entidad y no utiliza desplazamientos de elementos como unidad principal de resultado.</td></tr>
</tbody>
</table>
