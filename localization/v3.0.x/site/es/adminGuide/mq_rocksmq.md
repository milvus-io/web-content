...
<h2 id="Deploy-Milvus-Standalone-with-RocksMQ-using-Docker" class="common-anchor-header">Implementar Milvus Standalone con RocksMQ mediante Docker<button data-href="#Deploy-Milvus-Standalone-with-RocksMQ-using-Docker" class="anchor-icon" translate="no">
    ...
    </button></h3><p>Sigue las instrucciones de <a href="/docs/es/install_standalone-docker.md">«Ejecutar Milvus en Docker</a>». En Milvus 3.x, la opción predeterminada para la versión independiente es Woodpecker, por lo que debes cambiar explícitamente el tipo de cola de mensajes a RocksMQ. El script de arranque escribe un nuevo archivo ` <code translate="no">user.yaml</code> ` en la <strong>primera</strong> ejecución de ` <code translate="no">start</code>`, así que configura el tipo <strong>tras</strong> ese primer inicio y, a continuación, ejecuta ` <code translate="no">restart</code> ` para aplicar el cambio (un ` <code translate="no">restart</code> ` conserva ` <code translate="no">user.yaml</code>`):</p>
