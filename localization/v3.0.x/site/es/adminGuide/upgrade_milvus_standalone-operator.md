...
summary: Descubre cómo actualizar Milvus en modo autónomo con Milvus Operator.
...
<h1 id="Upgrade-Milvus-Standalone-with-Milvus-Operator" class="common-anchor-header">Actualizar Milvus Standalone con Milvus Operator<button data-href="#Upgrade-Milvus-Standalone-with-Milvus-Operator" class="anchor-icon" translate="no">
    ...
    </button></h1><p>Esta guía describe cómo actualizar una implementación independiente de Milvus 2.6.x a la versión v3.0.1 con Milvus Operator.</p>
<div class="alert note">
<p>Este procedimiento se ha validado desde Milvus 2.6.20 hasta Milvus v3.0.1 con Milvus Operator 1.3.0, Woodpecker, etcd dentro del clúster y MinIO dentro del clúster. Si utilizas otra versión de parche de Milvus 2.6.x, otra versión de Operator, otra cola de mensajes o una configuración de dependencias diferente, comprueba primero la actualización en un entorno que no sea de producción.</p>
</div>
...
<h2 id="Upgrade-process" class="common-anchor-header">Proceso de actualización<button data-href="#Upgrade-process" class="anchor-icon" translate="no">
    ...
    </button></h2><h3 id="Step-1-Back-up-the-current-Milvus-CR" class="common-anchor-header">Paso 1: Realice una copia de seguridad del CR actual de Milvus<button data-href="#Step-1-Back-up-the-current-Milvus-CR" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<p>Utilice el manifiesto de origen de su implementación actual como manifiesto de actualización. No aplique directamente el archivo de copia de seguridad exportado sin eliminar primero los metadatos gestionados por el servidor y los campos de estado.</p>
<h3 id="Step-2-Confirm-the-Milvus-Operator-version" class="common-anchor-header">Paso 2: Confirma la versión de Milvus Operator<button data-href="#Step-2-Confirm-the-Milvus-Operator-version" class="anchor-icon" translate="no">
    ...
    </button></h3><p>Comprueba la imagen utilizada por Milvus Operator instalado:</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>La actualización validada mantuvo Milvus Operator en la versión 1.3.0. Mantén la versión de Operator que gestiona actualmente tu despliegue de Milvus 2.6.x, a menos que tu política de soporte requiera una actualización independiente de Operator. No rebajes la versión de un Operator más reciente a la versión probada. Si necesitas cambiar la versión de Operator, utiliza el mismo método de instalación (Helm o <code translate="no">kubectl</code> ) y el mismo nombre de versión y espacio de nombres que la instalación existente; a continuación, valida el cambio de Operator antes de actualizar el CR de Milvus.</p>
<h3 id="Step-3-Update-the-Milvus-image" class="common-anchor-header">Paso 3: Actualizar la imagen de Milvus<button data-href="#Step-3-Update-the-Milvus-image" class="anchor-icon" translate="no">
    ...
    </button></h3><p>En el manifiesto completo de la CR de Milvus, cambia únicamente <code translate="no">spec.components.image</code>. Mantén el modo existente, la configuración de los componentes, la cola de mensajes, etcd, el almacenamiento y el resto de ajustes de dependencias. El siguiente fragmento muestra el campo que hay que cambiar; no sustituyas tu CR completa por este fragmento.</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>Aplica el manifiesto completo de la solicitud de creación (CR):</p>
...
<h2 id="Verify-the-upgrade" class="common-anchor-header">Verifica la actualización<button data-href="#Verify-the-upgrade" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Comprueba el estado de la CR, el estado de los pods y las imágenes de contenedor:</p>
<pre><code translate="no" class="language-bash">kubectl get milvus &lt;instance-name&gt; \
  --namespace &lt;namespace&gt; \
...
<button class="copy-code-btn"></button></code></pre>
<p>Comprueba que el CR de Milvus indique « <code translate="no">Healthy</code> », que la imagen actual sea « <code translate="no">milvusdb/milvus:v3.0.1</code> » y que las colecciones existentes sigan siendo consultables y buscables. Realiza estas comprobaciones antes de habilitar cualquier función específica de la versión 3.0.1.</p>
