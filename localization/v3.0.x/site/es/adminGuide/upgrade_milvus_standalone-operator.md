...
<h1 id="Upgrade-Milvus-Standalone-with-Milvus-Operator" class="common-anchor-header">Actualizar Milvus Standalone con Milvus Operator<button data-href="#Upgrade-Milvus-Standalone-with-Milvus-Operator" class="anchor-icon" translate="no">
    ...
    </button></h1><p>Esta guía describe cómo actualizar una implementación de Milvus 2.6.x independiente a la versión 3.0.2 con Milvus Operator.</p>
<div class="alert note">
<p>Este procedimiento se ha validado desde Milvus 2.6.20 hasta Milvus v3.0.2 con Milvus Operator 1.3.0, Woodpecker, etcd dentro del clúster y MinIO dentro del clúster. Si utilizas otra versión de parche de Milvus 2.6.x, otra versión de Operator, otra cola de mensajes o una configuración de dependencias diferente, comprueba primero la actualización en un entorno que no sea de producción.</p>
</div>
...
<button class="copy-code-btn"></button></code></pre>
<p>La actualización validada mantuvo Milvus Operator en la versión 1.3.0. Mantén la versión de Operator que gestiona actualmente tu despliegue de Milvus 2.6.x, a menos que tu política de soporte técnico requiera una actualización independiente de Operator. No revierta un Operator más reciente a la versión probada. Si necesita cambiar la versión de Operator, utilice el mismo método de instalación (Helm o <code translate="no">kubectl</code> ) y el mismo nombre de versión y espacio de nombres que la instalación existente; a continuación, valide el cambio de Operator antes de actualizar el CR de Milvus.</p>
<h3 id="Step-3-Update-the-Milvus-image" class="common-anchor-header">Paso 3: Actualizar la imagen de Milvus<button data-href="#Step-3-Update-the-Milvus-image" class="anchor-icon" translate="no">
    ...
    </button></h3><p>En el manifiesto completo de Milvus CR, cambia únicamente <code translate="no">spec.components.image</code>. Mantén el modo existente, la configuración de los componentes, la cola de mensajes, etcd, el almacenamiento y el resto de configuraciones de dependencias. El siguiente fragmento muestra el campo que hay que cambiar; no sustituyas tu CR completo por este fragmento.</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>Comprueba que el CR de Milvus indique « <code translate="no">Healthy</code> », que la imagen actual sea « <code translate="no">milvusdb/milvus:v3.0.2</code> » y que las colecciones existentes sigan siendo consultables y buscables. Realiza estas comprobaciones antes de habilitar cualquier función específica de la versión 3.0.2.</p>
