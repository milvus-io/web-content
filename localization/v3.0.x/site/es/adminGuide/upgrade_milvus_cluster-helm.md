...
<h1 id="Upgrade-Milvus-Cluster-with-Helm-Chart" class="common-anchor-header">Actualizar el clúster de Milvus con Helm Chart<button data-href="#Upgrade-Milvus-Cluster-with-Helm-Chart" class="anchor-icon" translate="no">
    ...
    </button></h1><p>Esta guía describe cómo actualizar tu clúster de Milvus 2.6.x a la versión 3.0.2 utilizando Helm.</p>
<div class="alert note">
<p>Este procedimiento se ha validado desde Milvus 2.6.20 hasta Milvus v3.0.2 con el Helm Chart de Milvus 5.0.22. Si utilizas otra versión de parche de Milvus 2.6.x u otra versión del Helm Chart, comprueba primero la actualización en un entorno que no sea de producción.</p>
</div>
...
<p><strong>Limitaciones de la cola de mensajes</strong>: al actualizar a Milvus v3.0.2, debes mantener tu elección actual de cola de mensajes. No se admite el cambio entre diferentes sistemas de colas de mensajes durante la actualización. El soporte para cambiar de sistema de colas de mensajes estará disponible en futuras versiones.</p>
<div class="alert warning">
<p>No modifique ni revierta la versión del Helm Chart como parte de este procedimiento. Mantenga la versión del Chart ya instalada para su versión de Helm. La línea de base probada conservó el Helm Chart 5.0.22 y solo cambió la etiqueta de la imagen de Milvus a <code translate="no">v3.0.2</code>.</p>
<p>Este procedimiento no valida una degradación o una reversión que implique cambiar la imagen de Milvus de nuevo a la versión 2.6.x. Después de que la v3.0.2 escriba datos, una reversión que afecte solo a la imagen puede no leer correctamente el estado actualizado. Si la actualización falla, detén las escrituras y utiliza un plan de recuperación que restaure los metadatos previos a la actualización y las copias de seguridad de los datos persistentes. Valida primero el plan de recuperación en un entorno que no sea de producción.</p>
</div>
<h2 id="Upgrade-process" class="common-anchor-header">Proceso de actualización<button data-href="#Upgrade-process" class="anchor-icon" translate="no">
    ...
    </button></h2><p>La implementación validada de Milvus 2.6.20 creada con Helm Chart 5.0.22 utilizaba MixCoord y StreamingNode, y no ejecutaba IndexNode. No es necesario realizar un paso independiente de migración del coordinador cuando la implementación utiliza la misma topología.</p>
