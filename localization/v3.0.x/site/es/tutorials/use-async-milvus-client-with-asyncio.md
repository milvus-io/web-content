...
<h2 id="Create-an-event-loop​" class="common-anchor-header">Crear un bucle de eventos<button data-href="#Create-an-event-loop​" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Las aplicaciones que utilizan asyncio suelen utilizar el bucle de eventos como orquestador para gestionar tareas asíncronas y operaciones de E/S. En este tutorial, obtendremos un bucle de eventos de asyncio y lo utilizaremos como orquestador.</p>
...
<h2 id="Query​" class="common-anchor-header">Consulta<button data-href="#Query​" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Una vez que la colección está cargada y llena de datos, puede realizar búsquedas y consultas en ella. En esta sección, va a buscar el número de entidades en el campo <code translate="no">text</code> que empiezan por la palabra <code translate="no">random</code> en la colección denominada <code translate="no">my_collection</code>.</p>
