...
<h3 id="Prepare-the-data" class="common-anchor-header">Подготовьте данные<button data-href="#Prepare-the-data" class="anchor-icon" translate="no">
    ...
    </button></h3><p>Импортируйте необходимые пакеты в этот блокнот. Затем подготовьте несколько примеров документов.</p>
...
<h2 id="BM25-search-without-embedding" class="common-anchor-header">BM25 поиск без встраивания<button data-href="#BM25-search-without-embedding" class="anchor-icon" translate="no">
    ...
    </button></h2><h3 id="Create-the-indexing-Pipeline" class="common-anchor-header">Создание конвейера индексирования<button data-href="#Create-the-indexing-Pipeline" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<h3 id="Create-the-retrieval-pipeline" class="common-anchor-header">Создание конвейера поиска<button data-href="#Create-the-retrieval-pipeline" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Создайте конвейер поиска, который будет извлекать документы из хранилища документов Milvus с помощью <code translate="no">MilvusSparseEmbeddingRetriever</code>, который является оберткой для <code translate="no">document_store</code>.</p>
...
<h2 id="Hybrid-Search-with-semantic-search-and-full-text-search" class="common-anchor-header">Гибридный поиск с семантическим поиском и полнотекстовым поиском<button data-href="#Hybrid-Search-with-semantic-search-and-full-text-search" class="anchor-icon" translate="no">
    ...
    </button></h3><p>В гибридном поиске мы используем функцию BM25 для выполнения полнотекстового поиска и задаем плотное векторное поле <code translate="no">vector</code> для выполнения семантического поиска.</p>
...
<h3 id="Create-the-retrieval-pipeline" class="common-anchor-header">Создание конвейера поиска<button data-href="#Create-the-retrieval-pipeline" class="anchor-icon" translate="no">
    ...
    </button></h3><p>Создайте конвейер поиска, который извлекает документы из хранилища документов Milvus с помощью <code translate="no">MilvusHybridRetriever</code>, который содержит <code translate="no">document_store</code> и получает параметры гибридного поиска.</p>
