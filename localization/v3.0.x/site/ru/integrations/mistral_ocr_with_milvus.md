...
summary: >-
  В этом руководстве показано, как построить систему понимания документов с
...
<h1 id="Document-Understanding-with-Mistral-OCR-and-Milvus" class="common-anchor-header">Понимание документов с помощью Mistral OCR и Milvus<button data-href="#Document-Understanding-with-Mistral-OCR-and-Milvus" class="anchor-icon" translate="no">
    ...
    </button></h1><p>В этом руководстве показано, как построить систему понимания документов с помощью:</p>
<h2 id="Mistral-OCR" class="common-anchor-header">Mistral OCR<button data-href="#Mistral-OCR" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Мощный сервис оптического распознавания символов, который:</p>
<ul>
<li>обрабатывает PDF-файлы, изображения и другие форматы документов</li>
<li>Сохраняет структуру и форматирование документа</li>
<li>Работает с многостраничными документами</li>
<li>Распознает таблицы, списки и другие сложные элементы.</li>
</ul>
...
<li>Преобразование текста в числовые представления:</li>
<li>Преобразует текст в 1024-мерные векторы</li>
<li>Улавливает семантические связи между понятиями</li>
<li>Обеспечивает сопоставление по сходству на основе смысла</li>
<li>Обеспечивает основу для семантического поиска</li>
</ul>
<h2 id="Milvus-Vector-Database" class="common-anchor-header">Векторная база данных Milvus<button data-href="#Milvus-Vector-Database" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>Специализированная база данных для поиска векторного сходства:</p>
<ul>
<li>Open-Source</li>
<li>Выполняет эффективный векторный поиск</li>
<li>Масштабируется до больших коллекций документов</li>
<li>Поддерживает гибридный поиск (векторное сходство + фильтрация метаданных)</li>
<li>Оптимизирована для приложений искусственного интеллекта</li>
</ul>
<p>К концу этого урока у вас будет система, которая сможет</p>
<ol>
<li>обрабатывать документы (PDF-файлы/изображения) по URL-адресам</li>
<li>Извлекать текст с помощью OCR</li>
<li>Хранить текст и векторные вложения в Milvus</li>
<li>Выполнять семантический поиск по коллекции документов.</li>
</ol>
<hr>
<h2 id="Setup-and-Dependencies" class="common-anchor-header">Установка и зависимости<button data-href="#Setup-and-Dependencies" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>Сначала установим необходимые пакеты:</p>
...
<ol>
<li>API-ключ Mistral (получите его на сайте https://console.mistral.ai/).</li>
<li>Milvus, запущенный локально через <a href="https://milvus.io/docs/install_standalone-docker.md">Docker</a> или с помощью <a href="https://zilliz.com">Zilliz Cloud</a>.</li>
</ol>
<p>Давайте настроим наше окружение:</p>
...
<h2 id="Setting-Up-Milvus-Collection" class="common-anchor-header">Настройка коллекции Milvus<button data-href="#Setting-Up-Milvus-Collection" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>Теперь давайте создадим коллекцию Milvus для хранения данных о документах. Коллекция будет содержать следующие поля:</p>
<ul>
<li><code translate="no">id</code>: Первичный ключ (генерируется автоматически)</li>
<li><code translate="no">url</code>: URL-адрес источника документа</li>
<li><code translate="no">page_num</code>: Номер страницы в документе</li>
<li><code translate="no">content</code>: Содержание извлеченного текста</li>
<li><code translate="no">embedding</code>: Векторное представление содержимого (1024 размера)</li>
</ul>
...
<h2 id="Core-Functionality" class="common-anchor-header">Основные функции<button data-href="#Core-Functionality" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>Давайте реализуем основные функции для нашей системы понимания документов:</p>
...
<span class="hljs-keyword">def</span> <span class="hljs-title function_">process_document</span>(<span class="hljs-params">url</span>):
    ...
    <span class="hljs-keyword">return</span> ocr_result
<button class="copy-code-btn"></button></code></pre>
<h2 id="Search-Functionality" class="common-anchor-header">Функциональность поиска<button data-href="#Search-Functionality" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>Теперь давайте реализуем функцию поиска для получения релевантного содержимого документа:</p>
...
<button class="copy-code-btn"></button></code></pre>
<h2 id="Demo-Processing-Documents" class="common-anchor-header">Демо: Обработка документов<button data-href="#Demo-Processing-Documents" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>Давайте обработаем несколько примеров документов. Вы можете заменить эти URL на свои собственные документы.</p>
...
<p>Также обработаем изображение:</p>
...
<h2 id="Demo-Searching-Documents" class="common-anchor-header">Демо: Поиск в документах<button data-href="#Demo-Searching-Documents" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>Теперь, когда мы обработали несколько документов, давайте выполним поиск по ним:</p>
...
<h2 id="Conclusion" class="common-anchor-header">Заключение<button data-href="#Conclusion" class="anchor-icon" translate="no">
    ...
    </button></h2><p>В этом руководстве мы создали полноценную систему понимания документов, используя Mistral OCR и Milvus. Эта система может:</p>
<ol>
...
<p>Такой подход обеспечивает мощные возможности понимания документов, выходящие за рамки простого подбора ключевых слов, позволяя пользователям находить информацию на основе смысла, а не точного совпадения текста.</p>
