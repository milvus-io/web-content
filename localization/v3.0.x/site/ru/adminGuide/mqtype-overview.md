...
<h1 id="Message-Queue-Overview" class="common-anchor-header">Обзор очереди сообщений<button data-href="#Message-Queue-Overview" class="anchor-icon" translate="no">
    ...
    </button></h1><p>Milvus использует очередь сообщений (журнал предварительной записи, WAL) для управления журналами последних изменений, журналами выходных потоков и предоставления подписок на журналы. В Milvus 3.x <strong>Woodpecker</strong> является очередью сообщений по умолчанию и не требует отдельной инфраструктуры обмена сообщениями. Pulsar, Kafka и RocksMQ по-прежнему поддерживаются для конкретных сценариев.</p>
...
<li><p><strong>Ограничения очереди сообщений</strong>: при обновлении до Milvus v3.0-beta необходимо сохранить текущий выбор очереди сообщений. Переключение между различными системами очередей сообщений во время обновления не поддерживается. Поддержка смены систем очередей сообщений будет доступна в будущих версиях.</p></li>
