...
<li><p><strong>Ограничения очереди сообщений</strong>: При обновлении до Milvus v3.0.1 необходимо сохранить текущий выбор очереди сообщений. Переключение между различными системами очередей сообщений во время обновления не поддерживается. Поддержка смены систем очередей сообщений будет доступна в будущих версиях.</p></li>
<li><p>Чтобы изменить очередь сообщений запущенного экземпляра, см. раздел <a href="/docs/ru/switch-mq-type.md">«Смена очереди сообщений</a>». Функция «Смена MQ» доступна в <strong>Milvus 3.0 и более поздних версиях</strong> — сначала обновите Milvus до версии 3.0 или более поздней.</p></li>
</ul>
</div>
...
<li><strong>Новые развёртывания (Milvus 3.x):</strong> используйте <strong>Woodpecker</strong> (по умолчанию). В автономном режиме он работает встроенным; для распределённой (кластерной) среды рекомендуемым вариантом по умолчанию является выделенный <a href="/docs/ru/woodpecker.md#Deployment-modes">сервис</a>, развёрнутый с помощью Helm, при этом также поддерживается встроенный вариант.</li>
