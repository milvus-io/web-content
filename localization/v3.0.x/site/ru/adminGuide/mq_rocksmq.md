...
Переключение <code translate="no">mq.type</code> таким образом предназначено для <b>совершенно нового</b> экземпляра (в котором ещё нет коллекций). Чтобы изменить очередь сообщений экземпляра, в котором уже хранятся данные, воспользуйтесь <a href="/docs/ru/switch-rocksmq-woodpecker.md">процедурой переключения</a>.
</div>
...
Поскольку в версии 2.6.x по умолчанию для автономного режима используется Woodpecker, зафиксируйте значение <code translate="no">mq.type: rocksmq</code> в файле <code translate="no">user.yaml</code> <strong>перед</strong> обновлением, если хотите сохранить RocksMQ.</li>
