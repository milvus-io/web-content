...
<h2 id="Create-your-own-certificate" class="common-anchor-header">Создание собственного сертификата<button data-href="#Create-your-own-certificate" class="anchor-icon" translate="no">
    ...
    </button></h3><p>Убедитесь, что OpenSSL установлен. Если он не установлен, сначала <a href="https://github.com/openssl/openssl/blob/master/INSTALL.md">соберите и установите</a> OpenSSL.</p>
...
<button class="copy-code-btn"></button></code></pre>
<h3 id="Create-files" class="common-anchor-header">Создайте файлы<button data-href="#Create-files" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<h3 id="Run-gensh-to-generate-certificate" class="common-anchor-header">Запустите файл <code translate="no">gen.sh</code> для создания сертификата<button data-href="#Run-gensh-to-generate-certificate" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Запустите файл <code translate="no">gen.sh</code> для создания сертификата.</p>
...
<h3 id="Renew-certificates-optional" class="common-anchor-header">Обновление сертификатов (необязательно)<button data-href="#Renew-certificates-optional" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Если вы хотите обновить сертификаты в некоторых случаях, например, если срок их действия скоро истечет, вы можете использовать следующий скрипт.</p>
...
<h3 id="Setup-for-Docker-Compose" class="common-anchor-header">Настройка для Docker Compose<button data-href="#Setup-for-Docker-Compose" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><h4 id="1-Modify-the-Milvus-server-configuration" class="common-anchor-header">1. Измените конфигурацию сервера Milvus</h4><p>Чтобы включить внешний TLS, добавьте следующие конфигурации в файл <code translate="no">milvus.yaml</code>:</p>
...
<h3 id="Setup-for-Milvus-Operator" class="common-anchor-header">Setup for Milvus Operator<button data-href="#Setup-for-Milvus-Operator" class="anchor-icon" translate="no">
    ...
    </button></h3><p>Поместите файлы сертификатов в рабочий каталог. Структура каталога должна выглядеть следующим образом:</p>
...
<button class="copy-code-btn"></button></code></pre>
<h3 id="setup-for-Milvus-Helm" class="common-anchor-header">настройка для Milvus Helm<button data-href="#setup-for-Milvus-Helm" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<h3 id="One-way-TLS-connection" class="common-anchor-header">Одностороннее TLS-соединение<button data-href="#One-way-TLS-connection" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Укажите путь к <code translate="no">server.pem</code> и убедитесь, что <code translate="no">server_name</code> совпадает с <code translate="no">CommonName</code>, указанным в сертификате.</p>
...
<button class="copy-code-btn"></button></code></pre>
<h3 id="Two-way-TLS-connection" class="common-anchor-header">Двустороннее TLS-соединение<button data-href="#Two-way-TLS-connection" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Укажите пути к <code translate="no">client.pem</code>, <code translate="no">client.key</code> и <code translate="no">ca.pem</code> и убедитесь, что <code translate="no">server_name</code> соответствует <code translate="no">CommonName</code>, указанному в сертификате.</p>
<pre><code translate="no" class="language-python"><span class="hljs-keyword">from</span> pymilvus <span class="hljs-keyword">import</span> MilvusClient
