...
<p><strong>Шаг 2: Настройте подключение к целевому Kafka и перезапустите Milvus.</strong> Укажите подключение к Kafka в разделе <code translate="no">spec.config</code> (оператор преобразует <code translate="no">spec.config</code> в <code translate="no">user.yaml</code>) и задайте тип MQ; при применении CR поды будут перезапущены с новой конфигурацией. Подробности о SASL/SSL см. в разделе <a href="/docs/ru/connect_kafka_ssl.md">«Подключение к Kafka с помощью SASL/SSL</a>».</p>
