...
<tr><td>Kafka</td><td>Woodpecker (локальный)</td><td><strong>Поддерживается, но не рекомендуется</strong> (для всех подсборок требуется общая файловая система)</td><td><strong>Не поддерживается</strong></td></tr>
</tbody>
</table>
