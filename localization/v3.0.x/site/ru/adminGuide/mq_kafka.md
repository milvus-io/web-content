...
<h2 id="Deploy-a-Milvus-cluster-with-Kafka-using-Helm" class="common-anchor-header">Развертывание кластера Milvus с Kafka с помощью Helm<button data-href="#Deploy-a-Milvus-cluster-with-Kafka-using-Helm" class="anchor-icon" translate="no">
    ...
    </button></h3><p>Чтобы использовать внешний сервис Kafka, отключите входящий в состав Pulsar и включите параметр « <code translate="no">externalKafka</code> » в файле переопределения « <code translate="no">values.yaml</code> », а затем установите Milvus с его использованием:</p>
