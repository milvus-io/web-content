...
<h1 id="Use-Kafka-as-the-Milvus-Message-Queue" class="common-anchor-header">Использование Kafka в качестве очереди сообщений Milvus<button data-href="#Use-Kafka-as-the-Milvus-Message-Queue" class="anchor-icon" translate="no">
    ...
    </button></h1><p>Apache Kafka — один из бэкэндов очереди сообщений (WAL), поддерживаемых Milvus. В Milvus 3.x по умолчанию в качестве очереди сообщений используется <a href="/docs/ru/woodpecker.md">Woodpecker</a>; при этом Kafka по-прежнему полностью поддерживается для пользователей, предпочитающих именно его. Kafka в основном используется с Milvus Distributed (кластер); в автономных развертываниях обычно используется встроенный Woodpecker или <a href="/docs/ru/mq_rocksmq.md">RocksMQ</a>.</p>
...
<h2 id="Deploy-a-Milvus-cluster-with-Kafka-using-Helm" class="common-anchor-header">Развертывание кластера Milvus с Kafka с помощью Helm<button data-href="#Deploy-a-Milvus-cluster-with-Kafka-using-Helm" class="anchor-icon" translate="no">
    ...
    </button></h3><p>Чтобы использовать внешний сервис Kafka, отключите встроенный Pulsar и включите параметр « <code translate="no">externalKafka</code> » в файле переопределения <code translate="no">values.yaml</code>, а затем установите Milvus с его использованием:</p>
...
<li><strong>Обновление с версии 2.5.x до 2.6.x:</strong> <strong>Ограничения</strong>, связанные с <strong>очередью сообщений</strong>: при обновлении до Milvus v3.0.1 необходимо сохранить текущий выбор системы очереди сообщений. Переключение между различными системами очередей сообщений во время обновления не поддерживается. Поддержка смены систем очередей сообщений будет доступна в будущих версиях.
