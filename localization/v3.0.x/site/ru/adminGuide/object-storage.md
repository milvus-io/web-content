...
<h2 id="Additional-requirements-when-using-embedded-Woodpecker" class="common-anchor-header">Дополнительные требования при использовании встроенного Woodpecker<button data-href="#Additional-requirements-when-using-embedded-Woodpecker" class="anchor-icon" translate="no">
    ...
    </button></h2><p>При запуске стандартной очереди сообщений <strong>Woodpecker</strong> с бэкендом объектного хранилища (<code translate="no">storage.type=minio</code>) Woodpecker записывает свой журнал предварительной записи в то же объектное хранилище, что требует <strong>строгой семантики S3 Conditional-Write</strong>. Не каждое объектное хранилище подходит для этой цели — например, Huawei Cloud OBS в настоящее время <strong>не</strong> поддерживается в качестве бэкэнда Woodpecker, хотя и работает как обычное объектное хранилище Milvus.</p>
<p>Точные требования для каждого поставщика см. в таблице совместимости объектных хранилищ на странице <a href="/docs/ru/woodpecker.md">Woodpecker</a>.</p>
