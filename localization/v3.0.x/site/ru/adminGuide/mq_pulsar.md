...
title: Пульсар
---
<h1 id="Use-Pulsar-as-the-Milvus-Message-Queue" class="common-anchor-header">Использование Pulsar в качестве бэкэнда очереди сообщений Milvus<button data-href="#Use-Pulsar-as-the-Milvus-Message-Queue" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<thead>
<tr><th>Версия Milvus</th><th>Поддерживаемая версия Pulsar</th><th>По умолчанию</th></tr>
</thead>
<tbody>
<tr><td>2.5.x и более поздние</td><td>Pulsar v3 (рекомендуется) или Pulsar v2</td><td>Pulsar v3 (через Helm / Milvus Operator)</td></tr>
<tr><td>2.4.x и более ранние версии</td><td>Pulsar v2</td><td>Pulsar v2</td></tr>
</tbody>
</table>
<p>Начиная с Milvus 2.5, диаграмма Helm для Milvus и Milvus Operator по умолчанию развертывают <strong>Pulsar v3</strong>; Pulsar v2 по-прежнему поддерживается. См. разделы <a href="/docs/ru/upgrade-pulsar-v3.md">«Обновление Pulsar с версии v2 до v3</a> » и <a href="/docs/ru/use-pulsar-v2.md">«Продолжение использования Pulsar v2</a>».</p>
<h2 id="Deploy-a-Milvus-cluster-with-Pulsar-using-Helm" class="common-anchor-header">Развертывание кластера Milvus с Pulsar с помощью Helm<button data-href="#Deploy-a-Milvus-cluster-with-Pulsar-using-Helm" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><h3 id="Install" class="common-anchor-header">Установка<button data-href="#Install" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<button class="copy-code-btn"></button></code></pre>
<p>Если вы использовали встроенный Pulsar и хотите удалить его сохраненные данные, удалите PVC Pulsar (с именем <code translate="no">my-release-pulsarv3-*</code>):</p>
<pre><code translate="no" class="language-bash">kubectl get pvc | grep my-release-pulsarv3
kubectl delete pvc &lt;pulsar-pvc-name&gt; ...
<button class="copy-code-btn"></button></code></pre>
<h2 id="Deploy-a-Milvus-cluster-with-Pulsar-using-Milvus-Operator" class="common-anchor-header">Разверните кластер Milvus с Pulsar с помощью Milvus Operator<button data-href="#Deploy-a-Milvus-cluster-with-Pulsar-using-Milvus-Operator" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<button class="copy-code-btn"></button></code></pre>
<h3 id="Internal-in-cluster-Pulsar" class="common-anchor-header">Внутренний (внутрикластерный) Pulsar<button data-href="#Internal-in-cluster-Pulsar" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<li><a href="/docs/ru/switch-pulsar-woodpecker.md">Переключение между Pulsar и Woodpecker</a></li>
</ul>
