...
<h1 id="Upgrade-Milvus-Standalone-with-Milvus-Operator" class="common-anchor-header">Обновление автономной версии Milvus с помощью Milvus Operator<button data-href="#Upgrade-Milvus-Standalone-with-Milvus-Operator" class="anchor-icon" translate="no">
    ...
    </button></h1><p>В данном руководстве описано, как обновить автономную установку Milvus 2.6.x до версии v3.0.2 с помощью Milvus Operator.</p>
<div class="alert note">
<p>Эта процедура проверялась при обновлении с Milvus 2.6.20 до версии v3.0.2 с использованием Milvus Operator 1.3.0, Woodpecker, etcd внутри кластера и MinIO внутри кластера. Если вы используете другой патч-релиз Milvus 2.6.x, версию Operator, очередь сообщений или конфигурацию зависимостей, сначала проверьте обновление в тестовой среде.</p>
</div>
...
<h3 id="Step-3-Update-the-Milvus-image" class="common-anchor-header">Шаг 3: Обновите образ Milvus<button data-href="#Step-3-Update-the-Milvus-image" class="anchor-icon" translate="no">
    ...
    </button></h3><p>В полном манифесте CR Milvus изменяйте только поле ` <code translate="no">spec.components.image</code>`. Сохраните существующие настройки режима, компонентов, очереди сообщений, etcd, хранилища и других зависимостей. В приведенном ниже фрагменте показано поле, которое необходимо изменить; не заменяйте весь манифест CR этим фрагментом.</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>Убедитесь, что в CR Milvus указано <code translate="no">Healthy</code>, текущий образ — <code translate="no">milvusdb/milvus:v3.0.2</code>, а существующие коллекции по-прежнему доступны для запросов и поиска. Выполните эти проверки, прежде чем включать какие-либо функции, специфичные для версии v3.0.2.</p>
