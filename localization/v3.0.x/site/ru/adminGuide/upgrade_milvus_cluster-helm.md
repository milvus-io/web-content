...
<h1 id="Upgrade-Milvus-Cluster-with-Helm-Chart" class="common-anchor-header">Обновление кластера Milvus с помощью Helm Chart<button data-href="#Upgrade-Milvus-Cluster-with-Helm-Chart" class="anchor-icon" translate="no">
    ...
    </button></h1><p>В данном руководстве описано, как обновить кластер Milvus 2.6.x до версии v3.0-beta с помощью Helm.</p>
<div class="alert note">
<p>Эта процедура проверена при обновлении с Milvus 2.6.20 до Milvus v3.0-beta с использованием Helm-чарта Milvus версии 5.0.22. Если вы используете другой патч-релиз Milvus 2.6.x или другую версию Helm-чарта, сначала проверьте обновление в тестовой среде.</p>
</div>
...
<p><strong>Ограничения</strong>, связанные с<strong>очередью сообщений</strong>: при обновлении до Milvus v3.0-beta необходимо сохранить текущий выбор очереди сообщений. Переключение между различными системами очередей сообщений во время обновления не поддерживается. Поддержка смены систем очередей сообщений будет доступна в будущих версиях.</p>
<div class="alert warning">
<p>Не изменяйте и не понижайте версию Helm Chart в рамках этой процедуры. Сохраните версию Chart, уже установленную для вашего релиза Helm. В протестированном базовом сценарии была сохранена версия Helm Chart 5.0.22, а был изменен только тег образа Milvus на <code translate="no">v3.0-beta</code>.</p>
<p>Данная процедура не предусматривает перехода на более раннюю версию или отката путем смены образа Milvus обратно на версию 2.6.x. После того как v3.0-beta запишет данные, откат, затрагивающий только образ, может не смочь прочитать обновленное состояние. Если обновление завершится сбоем, остановите запись данных и воспользуйтесь планом восстановления, который восстанавливает метаданные, существовавшие до обновления, и резервные копии постоянных данных. Сначала проверьте план восстановления в тестовой среде.</p>
</div>
...
<p>Убедитесь, что кластер использует MixCoord и StreamingNode и что ни один Pod IndexNode не запущен. Команда обновления, приведенная далее в этом руководстве, сохраняет существующие значения Helm. Если ваши текущие значения включают IndexNode или используют другую топологию компонентов, не выполняйте это обновление, касающееся только образа. Сначала воспроизведите топологию в тестовой среде и получите план миграции, одобренный инженерным отделом.</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>Опция <code translate="no">--reset-then-reuse-values</code> сохраняет значения из предыдущего релиза, одновременно применяя явное переопределение образа вместо значений по умолчанию, указанных в выбранном Chart.</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>Убедитесь, что все необходимые рабочие нагрузки готовы, все компоненты Milvus используют « <code translate="no">v3.0-beta</code> », а существующие коллекции по-прежнему доступны для запросов и поиска. Завершите эти проверки, прежде чем включать какие-либо функции, специфичные для версии v3.0-beta.</p>
<div class="alert note">
