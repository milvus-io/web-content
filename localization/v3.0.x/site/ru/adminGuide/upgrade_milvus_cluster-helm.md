...
title: Обновление кластера Milvus с помощью диаграммы Helm
---
...
<h1 id="Upgrade-Milvus-Cluster-with-Helm-Chart" class="common-anchor-header">Обновление кластера Milvus с помощью диаграммы Helm<button data-href="#Upgrade-Milvus-Cluster-with-Helm-Chart" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h1><p>В данном руководстве описано, как обновить кластер Milvus 2.6.x до версии v3.0.2 с помощью Helm.</p>
<div class="alert note">
<p>Эта процедура была протестирована при обновлении с Milvus 2.6.20 до Milvus v3.0.2 с использованием Helm Chart 5.0.22. Если вы используете другой патч-релиз Milvus 2.6.x или другую версию Helm Chart, сначала проверьте обновление в тестовой среде.</p>
</div>
...
<p>Не изменяйте и не понижайте версию диаграммы Helm в рамках данной процедуры. Сохраните версию диаграммы, уже установленную для вашего выпуска Helm. В протестированной базовой конфигурации была сохранена диаграмма Helm 5.0.22, а был изменен только тег образа Milvus на <code translate="no">v3.0.2</code>.</p>
<p>Данная процедура не проверяет возможность перехода на более раннюю версию или отката путем возврата образа Milvus к версии 2.6.x. После записи данных версией v3.0.2 откат, задействующий только образ, может не смочь прочитать обновленное состояние. Если обновление завершится сбоем, остановите запись данных и воспользуйтесь планом восстановления, который восстанавливает метаданные, существовавшие до обновления, и резервные копии постоянных данных. Сначала проверьте план восстановления в тестовой среде.</p>
</div>
<h2 id="Upgrade-process" class="common-anchor-header">Процесс обновления<button data-href="#Upgrade-process" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Проверенное развертывание Milvus 2.6.20, созданное с помощью Helm Chart 5.0.22, использовало MixCoord и StreamingNode и не запускало IndexNode. Вам не требуется отдельный этап миграции координатора, если ваше развертывание использует ту же топологию.</p>
<h3 id="Step-1-Confirm-the-current-topology" class="common-anchor-header">Шаг 1: Проверьте текущую топологию<button data-href="#Step-1-Confirm-the-current-topology" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Сохраните все значения текущего релиза и проверьте запущенные Pod’ы:</p>
...
<p>Убедитесь, что кластер использует MixCoord и StreamingNode и что ни один Pod IndexNode не запущен. Команда обновления, приведенная далее в этом руководстве, сохраняет существующие значения Helm. Если ваши текущие значения включают IndexNode или используют другую топологию компонентов, не выполняйте это обновление только образа. Сначала воспроизведите топологию в непроизводственной среде и получите план миграции, одобренный инженерной службой.</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>Опция ` <code translate="no">--reset-then-reuse-values</code> ` сохраняет значения из предыдущего релиза, одновременно применяя явное переопределение образа вместо значений по умолчанию, заданных в выбранном чарте.</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>Убедитесь, что все необходимые рабочие нагрузки готовы, все компоненты Milvus используют <code translate="no">v3.0.2</code>, а существующие коллекции по-прежнему доступны для запросов и поиска. Завершите эти проверки, прежде чем включать какие-либо функции, специфичные для версии v3.0.2.</p>
<div class="alert note">
<p>Обновление до Milvus 3.0 не приводит к включению Storage V3. После проверки обновления ознакомьтесь с <a href="/docs/ru/storage-v3.md">Storage V3</a>, прежде чем включать функции, зависящие от него. После того как Milvus запишет данные в Storage V3, переход на более старую версию Milvus, не поддерживающую чтение данных из Storage V3, не поддерживается.</p>
</div>
