...
<h2 id="Configure-S3-with-Docker-Compose" class="common-anchor-header">Настройка S3 с помощью Docker Compose<button data-href="#Configure-S3-with-Docker-Compose" class="anchor-icon" translate="no">
    ...
    </button></h2><h3 id="1-Configure-S3" class="common-anchor-header">1. Настроить S3<button data-href="#1-Configure-S3" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p><a href="https://min.io/product/overview">MinIO</a> совместим с S3. Чтобы настроить S3 с помощью Docker Compose, укажите значения для секции <code translate="no">minio</code> в файле <code translate="no">milvus.yaml</code> по пути milvus/configs.</p>
...
<h3 id="2-Refine-docker-composeyaml" class="common-anchor-header">2. Доработка docker-compose.yaml<button data-href="#2-Refine-docker-composeyaml" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Вы также удалите переменную окружения <code translate="no">MINIO_ADDRESS</code> для службы milvus по адресу <code translate="no">docker-compose.yaml</code>. По умолчанию milvus будет использовать локальный minio вместо внешнего S3.</p>
<h3 id="3-Run-Milvus" class="common-anchor-header">3. Запустите Milvus<button data-href="#3-Run-Milvus" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Выполните следующую команду, чтобы запустить Milvus, который использует конфигурации S3.</p>
...
<button class="copy-code-btn"></button></code></pre>
<h3 id="Using-a-command" class="common-anchor-header">Использование команды<button data-href="#Using-a-command" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Чтобы установить Milvus и настроить S3, выполните следующую команду, используя ваши значения.</p>
