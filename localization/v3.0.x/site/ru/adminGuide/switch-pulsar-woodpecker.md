...
summary: >-
  Переключите очередь сообщений кластера Milvus между Pulsar и Woodpecker с
  помощью Helm или Milvus Operator.
---
...
<h2 id="With-Helm" class="common-anchor-header">С помощью Helm<button data-href="#With-Helm" class="anchor-icon" translate="no">
    ...
    </button></h3><p><strong>Шаг 1: Убедитесь, что экземпляр Milvus работает.</strong> Убедитесь, что ваш кластер Milvus работает правильно — например, создав тестовую коллекцию, вставив данные и запустив запрос.</p>
...
<p><strong>Шаг 2: Настройте целевое соединение с Pulsar и перезапустите Milvus.</strong> Для перехода необходимо, чтобы Milvus уже знал соединение с Pulsar, поэтому запишите его в файл « <code translate="no">user.yaml</code> » с помощью команды ` <code translate="no">extraConfigFiles</code> ` и примените с помощью команды ` <code translate="no">helm upgrade</code> ` (которая выполняет перезапуск под). Для работы функции Switch MQ требуется файл « <code translate="no">streaming.enabled=true</code> ».</p>
...
<pre><code translate="no" class="language-shell">kubectl logs &lt;mixcoord-pod&gt; | grep &quot;successfully updated mq.type configuration in etcd&quot;
<button class="copy-code-btn"></button></code></pre>
<p>При успешном переходе в журнале регистрируется сообщение « <code translate="no">[mqTypeValue=pulsar]</code> ».</p>
...
<p><strong>Шаг 2: Выполните переключение MQ.</strong> Служба MixCoord не доступна извне, поэтому запустите API-команд переключения изнутри пода MixCoord:</p>
...
<pre><code translate="no" class="language-shell">kubectl logs &lt;mixcoord-pod&gt; | grep &quot;successfully updated mq.type configuration in etcd&quot;
<button class="copy-code-btn"></button></code></pre>
<p>В случае успешного переключения в журнале записывается сообщение « <code translate="no">[mqTypeValue=woodpecker]</code> ».</p>
...
<p><strong>Шаг 2: Настройте подключение к целевому Pulsar и перезапустите Milvus.</strong> Укажите подключение к Pulsar в файле <code translate="no">spec.config</code> (оператор преобразует <code translate="no">spec.config</code> в <code translate="no">user.yaml</code>) и задайте тип MQ; при применении CR поды будут перезапущены с новой конфигурацией.</p>
