...
<div class="alert note">
<p>Если вы планируете позже вернуться к Pulsar, сначала очистите данные/темы, чтобы избежать конфликтов. Из-за ограничений диаграмм Helm переход обратно к <strong>встроенному</strong> экземпляру Pulsar в настоящее время невозможен.</p>
</div>
...
<p><strong>Шаг 2: Настройте подключение к целевому Pulsar и перезапустите Milvus.</strong> Добавьте подключение к Pulsar в файл <code translate="no">spec.config</code> (оператор преобразует <code translate="no">spec.config</code> в <code translate="no">user.yaml</code>) и установите тип MQ; приложение CR перезапустит поды с новой конфигурацией.</p>
