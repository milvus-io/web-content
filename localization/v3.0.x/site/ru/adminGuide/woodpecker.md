...
<ul>
<li><strong>Записи с кворумом за один RTT</strong> — репликация, управляемая клиентом, завершает запись с кворумом за один цикл обмена данными, при этом межазональный трафик фиксирован на уровне объема данных двух реплик (по сравнению с дополнительным трафиком ~1/3, характерным для репликации на основе брокера или лидера).</li>
<li><strong>Чтение с одним прыжком с учётом топологии</strong> — каждое чтение направляется непосредственно к ближайшей реплике, а не пересылается через брокер, что позволяет избежать случайных чтений между зонами (≈2/3 трафика чтения между зонами) в системах на основе брокера.</li>
