...
<li>Woodpecker может работать <strong>встроенным</strong> в узел Milvus/streaming (по умолчанию) или в качестве <strong>отдельного сервиса</strong> со своими собственными под (только для распределенной архитектуры/кластера).</li>
...
<ul>
<li>Управляет поведением клиента при добавлении, ротации и аудите сегментов для обеспечения баланса между пропускной способностью и сквозной задержкой.</li>
</ul></li>
<li><code translate="no">woodpecker.logstore</code>
<ul>
<li>Управляет политиками синхронизации/сброса/уплотнения/чтения для сегментов журнала. Это основные параметры для настройки пропускной способности и задержки.</li>
</ul></li>
...
<table>
<thead>
<tr><th><code translate="no">storage.type</code></th><th>Как работает Woodpecker</th><th>Бэкэнд WAL</th><th>Автономный Milvus</th><th>Распределенная версия Milvus (кластер)</th></tr>
</thead>
<tbody>
...
<tbody>
<tr><td>Хранилище Blob в Azure</td><td>Поддерживается</td><td>Использует собственный SDK Azure.</td></tr>
...
<button class="copy-code-btn"></button></code></pre>
<p>В этом примере Woodpecker настраивается в качестве очереди сообщений и включается Streaming Node. При первом запуске загрузка образов может занять некоторое время; подождите, пока все поды будут готовы:</p>
...
<h3 id="Enable-Woodpecker-for-Milvus-Standalone-in-Docker-storagelocal" class="common-anchor-header">Включение Woodpecker для автономного Milvus в Docker (storage=local)<button data-href="#Enable-Woodpecker-for-Milvus-Standalone-in-Docker-storagelocal" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<p>Для режима службы Woodpecker мы рекомендуем использовать предстоящую версию Milvus 3.0.1 или более позднюю версию с Woodpecker v0.1.37 или более поздней версией для оптимизации уборки при компактировании и групповой фиксации.</p>
</div>
...
<button class="copy-code-btn"></button></code></pre>
<p>При этом Woodpecker развертывается в виде выделенного StatefulSet (<code translate="no">my-release-milvus-woodpecker</code>, по умолчанию 4 реплики), представленного безголовым сервисом, объединенным в кластер по алгоритму «gossip» на портах <code translate="no">18080</code> (сервис), <code translate="no">17946</code> (gossip) и <code translate="no">9091</code> (метрики), с MinIO в качестве бэкэнда хранения. Сервису требуется кворум из <strong>3</strong> узлов; значение по умолчанию — <strong>4</strong> реплики — обеспечивает кворум и позволяет выдержать отказ одного узла, поэтому не устанавливайте значение параметра ` <code translate="no">woodpecker.replicaCount</code> ` ниже 3. В этом случае кластер включает отдельный набор под-контейнеров <code translate="no">woodpecker</code>:</p>
...
<h3 id="Embedded-mode" class="common-anchor-header">Встроенный режим<button data-href="#Embedded-mode" class="anchor-icon" translate="no">
    ...
    </button></h3><p>Основываясь на результатах тестов и ограничениях бэкэнда в <a href="/docs/ru/woodpecker_architecture.md">Woodpecker</a>, оптимизируйте сквозную пропускную способность записи с учетом следующих аспектов:</p>
<ul>
<li>Со стороны хранилища
<ul>
<li><strong>Объектное хранилище (совместимое с MinIO/S3)</strong>: увеличьте количество параллельных операций и размер объектов (избегайте очень маленьких объектов). Следите за ограничениями пропускной способности сети и корзины. Один узел MinIO на SSD часто ограничивается скоростью около 100 МБ/с локально; один EC2, подключенный к S3, может достигать скорости в гигабайтах в секунду.</li>
<li><strong>Локальные/общие файловые системы (локальные)</strong>: отдавайте предпочтение NVMe/быстрым дискам. Убедитесь, что файловая система хорошо обрабатывает мелкие записи и имеет низкую задержку fsync.</li>
</ul></li>
...
<li>Настройте <code translate="no">maxInterval</code> в соответствии с характеристиками носителя (пожертвуйте задержкой ради пропускной способности за счет более длительной агрегации).</li>
...
<h2 id="Latency" class="common-anchor-header">Задержка<button data-href="#Latency" class="anchor-icon" translate="no">
    ...
    </button></h3><p>Woodpecker — это облачный WAL, разработанный для объектных хранилищ, в котором найден компромисс между пропускной способностью, стоимостью и задержкой. Легкий встроенный режим ставит во главу угла оптимизацию стоимости и пропускной способности, поскольку в большинстве сценариев требуется лишь запись данных в течение определённого времени, а не низкая задержка для отдельных запросов на запись. Поэтому Woodpecker использует пакетную запись с интервалами по умолчанию 10 мс для бэкэндов хранения на базе локальной файловой системы и 200 мс для бэкэндов хранения типа MinIO. Во время медленных операций записи максимальная задержка равна времени интервала плюс время сброса.</p>
...
<ul>
<li><strong>Записи с кворумом One-RTT</strong> — репликация, управляемая клиентом, завершает запись с кворумом за один цикл обмена данными, при этом межазональный трафик фиксирован на уровне объема данных двух реплик (по сравнению с дополнительным трафиком ~1/3 между зонами, типичным для репликации на основе брокера/лидера).</li>
...
<li><strong>Отсутствие непрерывной репликации между узлами</strong> — журналы сохраняются в объектном хранилище, выступающем в качестве общего хранилища, поэтому при отработке отказа перезагружаются только уцелевшие реплики (без копирования всего узла), масштабирование не ограничивается пропускной способностью межузловой репликации, а замена большого количества узлов не вызывает «штормов» репликации.</li>
</ul>
