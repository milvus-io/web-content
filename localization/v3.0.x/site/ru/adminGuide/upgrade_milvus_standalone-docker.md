...
<h1 id="Upgrade-Milvus-Standalone-with-Docker-Compose" class="common-anchor-header">Обновление автономной версии Milvus с помощью Docker Compose<button data-href="#Upgrade-Milvus-Standalone-with-Docker-Compose" class="anchor-icon" translate="no">
    ...
    </button></h1><p>В данном руководстве описано, как обновить автономную версию Milvus 2.6.x до версии v3.0.2 с помощью Docker Compose.</p>
<div class="alert note">
<p>Эта процедура проверена на официальной конфигурации Docker Compose для автономной версии Milvus 2.6.20. При обновлении были сохранены etcd, MinIO, Woodpecker и существующие каталоги данных, а изменен был только образ Milvus на <code translate="no">milvusdb/milvus:v3.0.2</code>.</p>
</div>
...
<button class="copy-code-btn"></button></code></pre>
<p>Docker Compose поддерживает работу существующих контейнеров etcd и object-storage и повторно использует настроенные каталоги данных.</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>Убедитесь, что служба <code translate="no">standalone</code> работает исправно, её образ — <code translate="no">milvusdb/milvus:v3.0.2</code>, а существующие коллекции по-прежнему доступны для запросов и поиска. Выполните эти проверки, прежде чем включать какие-либо функции, специфичные для версии v3.0.2.</p>
