...
<p><strong>Ограничения</strong>, связанные с<strong>очередями сообщений</strong>: при обновлении до Milvus v3.0-beta необходимо сохранить текущий выбор системы очередей сообщений. Переключение между различными системами очередей сообщений во время обновления не поддерживается. Поддержка смены систем очередей сообщений будет доступна в будущих версиях.</p>
<div class="alert warning">
<p>Примените полный манифест пользовательского ресурса Milvus для данного обновления. Не используйте патч слияния, содержащий только образ. Оператор может по умолчанию восстановить пропущенные поля компонентов с нулевым количеством реплик, что может привести к повторной активации компонента, отключенного в существующем развертывании версии 2.6.x.</p>
<p>Данная процедура не обеспечивает проверку правильности перехода на более раннюю версию или отката путем возврата образа Milvus к версии 2.6.x. После записи данных версией v3.0-beta откат, выполняемый только на уровне образа, может не прочитать обновлённое состояние. Если обновление завершится неудачей, остановите запись данных и воспользуйтесь планом восстановления, который восстанавливает метаданные и резервные копии постоянных данных, существовавшие до обновления. Сначала проверьте план восстановления в тестовой среде.</p>
</div>
...
<h3 id="Step-3-Update-the-Milvus-image" class="common-anchor-header">Шаг 3: Обновите образ Milvus<button data-href="#Step-3-Update-the-Milvus-image" class="anchor-icon" translate="no">
    ...
    </button></h3><p>В полном манифесте CR Milvus замените <code translate="no">spec.components.image</code> на целевую версию. Сохраните текущие настройки режима, топологии компонентов, очереди сообщений, etcd, хранилища и других зависимостей. В следующем отрывке показаны поля, которые необходимо проверить; не заменяйте свой полный CR этим отрывком.</p>
<p>Перед применением целевого CR убедитесь, что <code translate="no">indexNode.replicas</code> имеет значение <code translate="no">0</code>. В проверенной конфигурации Milvus 2.6.20 уже использовался этот параметр. Сохраните явное значение «zero-replica» в целевом CR.</p>
