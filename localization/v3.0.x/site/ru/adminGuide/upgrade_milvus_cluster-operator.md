...
<h1 id="Upgrade-Milvus-Cluster-with-Milvus-Operator" class="common-anchor-header">Обновление кластера Milvus с помощью Milvus Operator<button data-href="#Upgrade-Milvus-Cluster-with-Milvus-Operator" class="anchor-icon" translate="no">
    ...
    </button></h1><p>В данном руководстве описано, как обновить кластер Milvus версии 2.6.x до версии 3.0.1 с помощью Milvus Operator.</p>
<div class="alert note">
<p>Эта процедура была проверена при обновлении с Milvus 2.6.20 до Milvus v3.0.1 с использованием Milvus Operator 1.3.0, MixCoord, StreamingNode, Woodpecker, etcd внутри кластера и MinIO внутри кластера. Если вы используете другой патч-релиз Milvus 2.6.x, версию Operator, топологию компонентов, очередь сообщений или конфигурацию зависимостей, сначала проверьте обновление в тестовой среде.</p>
</div>
...
<li>Кластер Kubernetes с кластером Milvus 2.6.x, управляемым Milvus Operator</li>
...
<p><strong>Ограничения очередей сообщений</strong>: при обновлении до Milvus v3.0.1 необходимо сохранить текущий выбор системы очередей сообщений. Переключение между различными системами очередей сообщений во время обновления не поддерживается. Поддержка смены систем очередей сообщений будет доступна в будущих версиях.</p>
<div class="alert warning">
<p>Примените полный манифест пользовательского ресурса Milvus для данного обновления. Не используйте патч слияния, содержащий только образ. Оператор может по умолчанию заполнить пропущенные поля компонентов с нулевым количеством реплик, что может привести к повторной активации компонента, отключенного в существующем развертывании версии 2.6.x.</p>
<p>Данная процедура не проверяет возможность перехода на более раннюю версию или отката путем возврата образа Milvus к версии 2.6.x. После записи данных версией v3.0.1 откат, включающий только образ, может не прочитать обновлённое состояние. Если обновление завершилось неудачей, остановите запись данных и воспользуйтесь планом восстановления, который восстанавливает метаданные, существовавшие до обновления, и резервные копии постоянных данных. Сначала проверьте план восстановления в тестовой среде.</p>
</div>
...
<p>Используйте исходный манифест вашего существующего развертывания в качестве манифеста обновления. Не применяйте экспортированный файл резервной копии напрямую, не удалив сначала управляемые сервером метаданные и поля статуса.</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>В ходе проверенного обновления версия Milvus Operator осталась 1.3.0. Сохраните версию Operator, которая в настоящее время управляет вашим развертыванием Milvus 2.6.x, если только ваша политика поддержки не требует отдельного обновления Operator. Не переходите с более новой версии Operator на тестируемую версию. Если вам необходимо изменить версию Operator, используйте тот же метод установки (Helm или <code translate="no">kubectl</code> ), а также то же имя выпуска и пространство имён, что и в существующей установке, а затем проверьте изменение Operator перед обновлением Milvus CR.</p>
<h3 id="Step-3-Update-the-Milvus-image" class="common-anchor-header">Шаг 3: Обновите образ Milvus<button data-href="#Step-3-Update-the-Milvus-image" class="anchor-icon" translate="no">
    ...
    </button></h3><p>В полном манифесте CR Milvus замените <code translate="no">spec.components.image</code> на целевую версию. Сохраните текущие настройки режима, топологии компонентов, очереди сообщений, etcd, хранилища и других зависимостей. В приведенном ниже фрагменте показаны поля, которые необходимо проверить; не заменяйте полный CR этим фрагментом.</p>
<p>Перед применением целевого CR убедитесь, что <code translate="no">indexNode.replicas</code> имеет значение <code translate="no">0</code>. В проверенной конфигурации Milvus 2.6.20 уже использовался этот параметр. Сохраните явную настройку с нулевым количеством реплик в целевом CR.</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>Убедитесь, что CR Milvus сообщает значение <code translate="no">Healthy</code>, все компоненты Milvus используют <code translate="no">milvusdb/milvus:v3.0.1</code>, ни один Pod IndexNode не запущен, а существующие коллекции по-прежнему доступны для запросов и поиска. Выполните эти проверки, прежде чем включать какие-либо функции, специфичные для версии v3.0.1.</p>
