...
<h2 id="Deploy-monitoring-services-on-Kubernetes" class="common-anchor-header">Развертывание служб мониторинга на Kubernetes<button data-href="#Deploy-monitoring-services-on-Kubernetes" class="anchor-icon" translate="no">
    ...
    </button></h2><h3 id="1-Access-the-dashboards" class="common-anchor-header">1. Получите доступ к панелям мониторинга<button data-href="#1-Access-the-dashboards" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Перенаправьте службу Prometheus на порт <code translate="no">9090</code>, а службу Grafana - на порт <code translate="no">3000</code>.</p>
...
<button class="copy-code-btn"></button></code></pre>
<h3 id="2-Enable-ServiceMonitor" class="common-anchor-header">2. Включите ServiceMonitor<button data-href="#2-Enable-ServiceMonitor" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>По умолчанию ServiceMonitor не включен в Milvus Helm. После установки Prometheus Operator в кластере Kubernetes вы можете включить его, добавив параметр <code translate="no">metrics.serviceMonitor.enabled=true</code>.</p>
...
<li><p>Отредактируйте поле <code translate="no">spec.components.disableMetric</code> на <code translate="no">false</code>.</p>
...
<h3 id="3-Check-the-metrics" class="common-anchor-header">3. Проверьте метрики<button data-href="#3-Check-the-metrics" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>После включения ServiceMonitor вы можете получить доступ к приборной панели Prometheus по адресу <code translate="no">http://localhost:9090/</code>.</p>
...
<h3 id="4-Check-the-ServiceMonitor" class="common-anchor-header">4. Проверьте ServiceMonitor<button data-href="#4-Check-the-ServiceMonitor" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
