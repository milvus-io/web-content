  ...
  <tbody>
    <tr>
      <td>
        ...
        <li>Слишком низкое значение этого параметра приводит к тому, что система слишком часто сохраняет небольшие объёмы данных. Слишком высокое значение увеличивает потребность системы в памяти.</li>      </td>
      <td>16777216</td>
    </tr>
  </tbody>
</table>
...
<h2 id="dataNodeimportmaxConcurrentTaskNum" class="common-anchor-header"><code translate="no">dataNode.import.maxConcurrentTaskNum</code><button data-href="#dataNodeimportmaxConcurrentTaskNum" class="anchor-icon" translate="no">
  ...
  <tbody>
    <tr>
      <td>        Максимальное количество задач импорта и предварительного импорта, которые разрешено выполнять одновременно на узле хранения данных.      </td>
      <td>16</td>
    </tr>
  </tbody>
</table>
<h2 id="dataNodeimportmaxImportFileSizeInGB" class="common-anchor-header"><code translate="no">dataNode.import.maxImportFileSizeInGB</code><button data-href="#dataNodeimportmaxImportFileSizeInGB" class="anchor-icon" translate="no">
  ...
  <tbody>
    <tr>
      <td>        Максимальный размер файла (в ГБ) для файла импорта, где под файлом импорта понимается либо файл на основе строк, либо набор файлов на основе столбцов.      </td>
      <td>16</td>
    </tr>
  </tbody>
</table>
...
<h2 id="dataNodetextinlineThreshold" class="common-anchor-header"><code translate="no">dataNode.text.inlineThreshold</code><button data-href="#dataNodetextinlineThreshold" class="anchor-icon" translate="no">
  ...
  <tbody>
    <tr>
      <td>        Пороговое значение размера (в байтах), используемое для выбора пути хранения значения поля TEXT. Значения, меньшие этого порогового значения, хранятся непосредственно в данных поля. Значения, равные или превышающие это пороговое значение, хранятся отдельно в виде полезных нагрузок LOB, а в данных поля сохраняется внутренняя ссылка.      </td>
      <td>65536</td>
    </tr>
  </tbody>
</table>
