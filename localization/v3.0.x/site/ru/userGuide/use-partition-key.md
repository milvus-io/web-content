...
title: Использование ключа раздела
---
<h1 id="Use-Partition-Key" class="common-anchor-header">Использование ключа раздела<button data-href="#Use-Partition-Key" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h1><p>В этом руководстве рассказывается об использовании ключа раздела для ускорения извлечения данных из коллекции.</p>
<h2 id="Overview" class="common-anchor-header">Обзор<button data-href="#Overview" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Вы можете задать определенное поле в коллекции в качестве ключа раздела, чтобы Milvus распределял входящие сущности по разным разделам в соответствии с их соответствующими значениями в этом поле. Это позволяет группировать сущности с одинаковым значением ключа в одном разделе, ускоряя производительность поиска за счет отсутствия необходимости сканирования нерелевантных разделов при фильтрации по ключевому полю. По сравнению с традиционными методами фильтрации ключ раздела может значительно повысить производительность запросов.</p>
<p>Ключ раздела можно использовать для реализации многопользовательского доступа. Подробнее о многопользовательстве читайте в статье <a href="https://milvus.io/docs/multi_tenancy.md">Многопользовательство</a>.</p>
<h2 id="Enable-partition-key" class="common-anchor-header">Включение ключа раздела<button data-href="#Enable-partition-key" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>Чтобы задать поле в качестве ключа раздела, укажите <code translate="no">partition_key_field</code> при создании схемы коллекции.</p>
<p>В приведенном ниже примере кода <code translate="no">num_partitions</code> определяет количество создаваемых разделов. По умолчанию он установлен на <code translate="no">64</code>. Мы рекомендуем сохранить значение по умолчанию.</p>
...
<p>Дополнительные сведения о параметрах см. <a href="https://milvus.io/api-reference/java/v2.4.x/v2/Client/MilvusClientV2.md"><code translate="no">MilvusClientV2</code></a>, <a href="https://milvus.io/api-reference/java/v2.4.x/v2/Collections/createSchema.md"><code translate="no">createSchema()</code></a>, и <a href="https://milvus.io/api-reference/java/v2.4.x/v2/CollectionSchema/addField.md"><code translate="no">addField()</code></a> в справочнике SDK.</p>
</div>
<div class="language-javascript">
<p>Для получения дополнительной информации о параметрах см. <a href="https://milvus.io/api-reference/node/v2.4.x/Client/MilvusClient.md"><code translate="no">MilvusClient</code></a> и <a href="https://milvus.io/api-reference/node/v2.4.x/Collections/createCollection.md"><code translate="no">createCollection()</code></a> в справочнике SDK.</p>
</div>
...
<button class="copy-code-btn"></button></code></pre>
<p>После того как вы определили поля, настройте параметры индекса.</p>
<div class="multipleCode">
   <a href="#python">Python </a> <a href="#java">Java</a> <a href="#javascript">Node.js</a></div>
...
<button class="copy-code-btn"></button></code></pre>
<p>Наконец, вы можете создать коллекцию.</p>
<div class="multipleCode">
   <a href="#python">Python </a> <a href="#java">Java</a> <a href="#javascript">Node.js</a></div>
...
<h2 id="List-partitions" class="common-anchor-header">Список разделов<button data-href="#List-partitions" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Как только поле коллекции используется в качестве ключа раздела, Milvus создает указанное количество разделов и управляет ими от вашего имени. Таким образом, вы больше не можете манипулировать разделами в этой коллекции.</p>
<p>Следующий фрагмент демонстрирует создание 64 разделов в коллекции после использования одного из ее полей в качестве ключа раздела.</p>
<h2 id="Insert-data" class="common-anchor-header">Вставка данных<button data-href="#Insert-data" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Когда коллекция готова, начните вставлять данные следующим образом:</p>
<h3 id="Prepare-data" class="common-anchor-header">Подготовить данные<button data-href="#Prepare-data" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<button class="copy-code-btn"></button></code></pre>
<h3 id="Insert-data" class="common-anchor-header">Вставить данные<button data-href="#Insert-data" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<p>Используйте метод <a href="https://milvus.io/api-reference/pymilvus/v2.4.x/MilvusClient/Vector/insert.md"><code translate="no">insert()</code></a> для вставки данных в коллекцию.</p>
</div>
<div class="language-java">
<p>Используйте метод <a href="https://milvus.io/api-reference/java/v2.4.x/v2/Vector/insert.md"><code translate="no">insert()</code></a> для вставки данных в коллекцию.</p>
</div>
<div class="language-javascript">
...
<h2 id="Use-partition-key" class="common-anchor-header">Использование ключа раздела<button data-href="#Use-partition-key" class="anchor-icon" translate="no">
    ...
    </button></h2><p>После индексации и загрузки коллекции, а также вставки данных можно выполнить поиск по сходству с помощью ключа раздела.</p>
<div class="language-python">
<p>Для получения дополнительной информации о параметрах см. <a href="https://milvus.io/api-reference/pymilvus/v2.4.x/MilvusClient/Vector/search.md"><code translate="no">search()</code></a> в справочнике SDK.</p>
</div>
<div class="language-java">
...
<p><b>примечания</b></p>
<p>Чтобы выполнить поиск по сходству с использованием ключа раздела, необходимо включить в логическое выражение поискового запроса одно из следующих условий:</p>
<ul>
...
<span class="hljs-comment">// </span>
<button class="copy-code-btn"></button></code></pre>
<h2 id="Typical-use-cases" class="common-anchor-header">Типичные случаи использования<button data-href="#Typical-use-cases" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>Функцию ключа раздела можно использовать для повышения производительности поиска и обеспечения многопользовательского доступа. Это можно сделать, назначив в качестве поля ключа раздела для каждой сущности значение, характерное для конкретного арендатора. При поиске или запросе коллекции можно отфильтровать сущности по значению для конкретного арендатора, включив поле ключа раздела в булево выражение. Такой подход обеспечивает изоляцию данных по арендаторам и позволяет избежать сканирования ненужных разделов.</p>
