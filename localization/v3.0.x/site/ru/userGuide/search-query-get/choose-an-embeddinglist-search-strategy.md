...
<p>В этом смысле « <code translate="no">emb_list_strategy</code> » — это в основном стратегия построения индекса и извлечения кандидатов. Она настраивается при построении индекса и определяет, как формируется набор кандидатов ANN на первом этапе. Параметры времени поиска, такие как <code translate="no">retrieval_ann_ratio</code> и <code translate="no">emb_list_rerank</code>, затем контролируют, сколько кандидатов извлекается и применяется ли переранжирование с помощью MaxSim.</p>
<hr>
...
<h2 id="TokenANN" class="common-anchor-header">TokenANN<button data-href="#TokenANN" class="anchor-icon" translate="no">
    ...
    </button></h2><p><code translate="no">tokenann</code> индексирует каждый вектор в списке вложений. Во время поиска каждый вектор запроса выполняет поиск с помощью нейронной сети (ANN), совпадающие векторы агрегируются обратно в свои строки, а полученные кандидаты-строки переранжируются с помощью MaxSim.</p>
...
<h2 id="MUVERA" class="common-anchor-header">MUVERA<button data-href="#MUVERA" class="anchor-icon" translate="no">
    ...
    </button></h2><p><code translate="no">muvera</code> кодирует каждый список вложений в вектор фиксированной размерности с помощью случайных проекций. Это превращает поиск на первом этапе в стандартный поиск по векторам на уровне строк. Затем кандидаты переранжируются с помощью MaxSim.</p>
<div class="alert note">
...
<ul>
<li><p><strong>Хорошо подходит для:</strong> длинных текстовых документов, пространств вложений с высокой дискриминацией, рабочих нагрузок, требующих меньшего размера индекса, чем у TokenANN.</p></li>
...
<ul>
<li><p><strong>Хорошо подходит для:</strong> поиска визуальных документов, мультимодальных вложений фрагментов, пространств вложений с низкой дискриминацией, больших списков вложений, где использование TokenANN нецелесообразно.</p></li>
...
<h2 id="Default-Behavior-and-Configuration" class="common-anchor-header">Поведение и настройки по умолчанию<button data-href="#Default-Behavior-and-Configuration" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Стратегией EmbeddingList по умолчанию в Knowhere является <code translate="no">tokenann</code>. Если вы не укажете <code translate="no">emb_list_strategy</code>, Knowhere будет использовать TokenANN. Значения по умолчанию при поиске включают <code translate="no">retrieval_ann_ratio=3.0</code> и <code translate="no">emb_list_rerank=true</code>.</p>
...
<tr><td><code translate="no">muvera</code></td><td><code translate="no">muvera_num_repeats</code></td><td>Создание индекса</td><td><code translate="no">7</code></td><td>Регулирует количество объединяемых независимых кодировок FDE. Более высокие значения могут повысить отказоустойчивость, но увеличат затраты на индексирование и поиск.</td></tr>
<tr><td><code translate="no">muvera</code></td><td><code translate="no">muvera_seed</code></td><td>Создание индекса</td><td><code translate="no">42</code></td><td>Устанавливается для получения воспроизводимых случайных проекций, особенно при тестировании и сравнительных тестах.</td></tr>
...
<tr><td><code translate="no">lemur</code></td><td><code translate="no">lemur_num_train_samples</code></td><td>Создание индекса</td><td><code translate="no">20000</code></td><td>Увеличивайте этот параметр, если корпус разнообразен, а обученное сжатие не обеспечивает полного покрытия; уменьшайте его только для небольших тестов или более быстрого построения.</td></tr>
<tr><td><code translate="no">lemur</code></td><td><code translate="no">lemur_num_epochs</code></td><td>Построение индекса</td><td><code translate="no">50</code></td><td>Увеличивайте, если обучение не сходится; уменьшайте, когда время построения является основным ограничением.</td></tr>
...
<tr><td><code translate="no">lemur</code></td><td><code translate="no">lemur_seed</code></td><td>Построение индекса</td><td><code translate="no">42</code></td><td>Установите для обеспечения воспроизводимости циклов обучения.</td></tr>
...
<tr><td>Все стратегии</td><td><code translate="no">retrieval_ann_ratio</code></td><td>Поиск</td><td><code translate="no">3.0</code></td><td>Увеличивайте, чтобы найти больше кандидатов на первом этапе и повысить полноту обнаружения; уменьшайте, чтобы сократить задержку.</td></tr>
...
<button class="copy-code-btn"></button></code></pre>
<div class="alert note">
<p><strong>Для выбора стратегии предпочтительно использовать параметры на уровне отдельного индекса.</strong> Значения по умолчанию из конфигурационного файла Milvus применяются в общем случае ко всем индексам данного типа и этапа. Используйте параметры <code translate="no">create_index</code>, когда для разных коллекций или полей требуются разные стратегии EmbeddingList.</p>
</div>
...
<tr><td><code translate="no">emb_list_rerank</code></td><td>Поиск</td><td><code translate="no">true</code></td><td>Необходимость переранжирования найденных кандидатов с помощью MaxSim.</td></tr>
</tbody>
</table>
<div class="alert note">
<p><strong>Примечания по совместимости:</strong> MUVERA и LEMUR в настоящее время поддерживают данные fp32 в Knowhere. DiskANN поддерживает EmbeddingList только со стратегией TokenANN. Если вы используете векторные типы, отличные от fp32, или DiskANN, проверьте поддержку стратегии перед изменением значения по умолчанию.</p>
</div>
<hr>
...
<tbody>
<tr><td>Вам нужен высококачественный базовый вариант?</td><td>Вы хотите определить наилучшую практическую аппроксимацию перед оптимизацией затрат.</td><td><code translate="no">tokenann</code></td></tr>
...
<tr><td>Является ли TokenANN слишком большим или слишком медленным?</td><td>Узким местом является размер индекса или задержка поиска на первом этапе.</td><td><code translate="no">muvera</code></td></tr>
<tr><td>Вам нужно сжатие без обучения?</td><td>Вам нужна более простая операционная модель и воспроизводимое кодирование.</td><td><code translate="no">muvera</code></td></tr>
...
<li><p>Начните с <code translate="no">tokenann</code> в качестве эталона качества, если размер набора данных это позволяет.</p></li>
...
<li><p>Попробуйте использовать <code translate="no">lemur</code>, если список вложений большой, пространство вложений зашумлено или рабочая нагрузка является визуальной или мультимодальной.</p></li>
...
<table>
<thead>
<tr><th>### «Качество прежде всего» Начните с параметра ` <code translate="no">tokenann</code>`. Используйте его в качестве базового значения для оценки качества аппроксимации MaxSim.</th><th>### Сбалансированный подход Попробуйте <code translate="no">muvera</code>, если вам нужна более низкая стоимость без добавления конвейера обучения.</th><th>### Сжатие. Попробуйте <code translate="no">lemur</code>, если обученное сжатие на уровне строк, вероятно, превзойдет фиксированную случайную проекцию.</th></tr>
</thead>
<tbody>
</tbody>
</table>
...
<li><p>Определения параметров Knowhere для значений по умолчанию и поддерживаемых названий стратегий.</p></li>
...
<div class="alert note">
<p><strong>Примечание по публикации:</strong> перед внешней публикацией убедитесь, какие параметры официально поддерживаются в целевом выпуске Milvus и хочет ли продукт раскрывать все низкоуровневые параметры Knowhere или только более небольшое задокументированное подмножество.</p>
</div>
