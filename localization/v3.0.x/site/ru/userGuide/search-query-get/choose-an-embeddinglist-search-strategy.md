...
<li><p><strong>Поведение на уровне элементов:</strong> TokenANN может извлекать кандидаты из отдельных векторов перед их агрегированием обратно в строки. Окончательный результат поиска EmbeddingList по-прежнему находится на уровне строк после оценки по алгоритму MaxSim.</p></li>
</ul>
...
<li><p>Настройте параметр ` <code translate="no">retrieval_ann_ratio</code> `, прежде чем менять слишком много параметров, задаваемых на этапе сборки. Увеличьте его, если коэффициент восстановления низкий; уменьшите — если задержка слишком высокая.</p></li>
...
<table>
<thead>
<tr><th>### «Качество прежде всего» Начните с параметра ` <code translate="no">tokenann</code>`. Используйте его в качестве базового значения для оценки качества аппроксимации MaxSim.</th><th>### Сбалансированный подход Попробуйте <code translate="no">muvera</code>, если вам нужна меньшая вычислительная нагрузка без добавления конвейера обучения.</th><th>### Сжатие. Попробуйте <code translate="no">lemur</code>, если обученное сжатие на уровне строк, вероятно, превзойдет фиксированную случайную проекцию.</th></tr>
</thead>
<tbody>
</tbody>
</table>
...
<div class="alert note">
<p><strong>Примечание по публикации:</strong> перед публикацией во внешних источниках убедитесь, какие параметры официально поддерживаются в целевом выпуске Milvus и планирует ли продукт раскрывать все низкоуровневые параметры Knowhere или только более небольшое документированное подмножество.</p>
</div>
