...
summary: >-
  Стратегии поиска EmbeddingList определяют, как Milvus строит приблизительный
  ...
  по-прежнему формируется с помощью переранжирования MaxSim, если включен
  параметр `emb_list_rerank`.
---
...
<tr><td><code translate="no">muvera</code></td><td>Один закодированный вектор на строку</td><td>Сжимает список вложений в представление FDE фиксированной размерности без обучения.</td><td>Более длинные документы, вложения с высокой дискриминационной способностью, случаи, когда TokenANN слишком ресурсоемкий.</td><td>Случайная проекция вносит потерю при аппроксимации; размерность FDE влияет на задержку.</td></tr>
<tr><td><code translate="no">lemur</code></td><td>Один обученный вектор на строку</td><td>Обучается специфическому для корпуса сжатию списков вложений в векторы строк фиксированной размерности.</td><td>Встраивания с низкой дискриминацией, мультимодальный поиск или поиск визуальных документов, большие списки встраиваний.</td><td>Требует обучения и может быть чувствителен к распределению корпуса и смещению, связанному с длиной документов.</td></tr>
</tbody>
</table>
...
<ul>
<li><p><strong>Хорошо подходит для:</strong> коротких фрагментов текста, строк с небольшим или умеренным количеством векторов, сильного семантического разделения на уровне токенов, базовых сценариев, чувствительных к качеству.</p></li>
...
<h2 id="Default-Behavior-and-Configuration" class="common-anchor-header">Поведение и настройки по умолчанию<button data-href="#Default-Behavior-and-Configuration" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Стратегией EmbeddingList по умолчанию в Knowhere является <code translate="no">tokenann</code>. Если вы не укажете <code translate="no">emb_list_strategy</code>, Knowhere использует TokenANN. По умолчанию при поиске используются <code translate="no">retrieval_ann_ratio=3.0</code> и <code translate="no">emb_list_rerank=true</code>.</p>
...
<tr><td><code translate="no">lemur</code></td><td><code translate="no">lemur_hidden_dim</code></td><td>Создание индекса</td><td><code translate="no">256</code></td><td>Регулирует размер сжатого представления. Увеличьте значение для увеличения емкости; уменьшите — для снижения потребления памяти и ускорения извлечения данных.</td></tr>
...
<tr><td>Является ли пространство вложений низкодискриминационным?</td><td>Кандидаты ANN на уровне токенов содержат много шума, а метод случайной проекции не сохраняет достаточно сигнала.</td><td><code translate="no">lemur</code></td></tr>
...
<li><p>Попробуйте использовать <code translate="no">lemur</code>, если список вложений большой, пространство вложений зашумлено или рабочая нагрузка носит визуальный или мультимодальный характер.</p></li>
<li><p>Настройте параметр ` <code translate="no">retrieval_ann_ratio</code> `, прежде чем менять слишком много параметров, задаваемых на этапе сборки. Увеличьте его, если коэффициент восстановления низкий; уменьшите — если задержка слишком высока.</p></li>
...
<li><p>Проверки совместимости Knowhere для поддержки MUVERA/LEMUR только с fp32 и DiskANN только с TokenANN.</p></li>
