...
summary: >-
  ...
  ограничения семантического поиска, при котором могут быть упущены точные
...
<button class="copy-code-btn"></button></code></pre>
<p>В приведенной выше конфигурации</p>
<ul>
...
<div class="alert note">
<p>Если обработка по алгоритму BM25 требуется для нескольких текстовых полей, определите <strong>по одной функции BM25 для каждого поля</strong>, каждая из которых должна иметь уникальное имя и поле вывода.</p>
</div>
...
<table>
   ...
   <tr>
     ...
     <td><p>Тип создаваемого индекса. Для полнотекстового поиска по алгоритму BM25 в Milvus установите для этого параметра значение <code translate="no">SPARSE_INVERTED_INDEX</code>. Дополнительную информацию см. в разделе <a href="/docs/ru/sparse-inverted-index.md">SPARSE_INVERTED_INDEX</a>.</p></td>
   </tr>
   <tr>
...
<table>
   ...
   <tr>
     ...
     <td><p>Доля терминов с низкой значимостью, которые следует игнорировать при поиске. Значение должно находиться в диапазоне [0,0; 1,0). Подробности см. в разделе <a href="/docs/ru/sparse_vector.md">«Разреженный вектор</a>».</p></td>
   </tr>
   <tr>
     <td></td>
     <td></td>
   ...
   <tr>
     ...
     <td><p>Список имен полей, которые необходимо вернуть в результатах поиска. Поддерживаются все поля, <strong>кроме поля разреженных векторов</strong>, содержащего вложения, сгенерированные с помощью BM25. К типичным полям вывода относятся поле первичного ключа (например, <code translate="no">id</code>) и поле исходного текста (например, <code translate="no">text</code>). Дополнительные сведения см. в <a href="/docs/ru/full-text-search.md#Can-I-output-or-access-the-sparse-vectors-generated-by-the-BM25-function-in-full-text-search">разделе «Часто задаваемые вопросы</a>».</p></td>
   </tr>
   <tr>
...
<li><p>Вы можете выводить только исходные текстовые поля и метаданные (например, <code translate="no">id</code>, <code translate="no">text</code>)</p></li>
</ul>
<p>Пример:</p>
