...
<ol>
<li><p><strong>Ввод исходного текста</strong>: вы вставляете текстовые документы или вводите запрос в виде простого текста; модели встраивания не требуются.</p></li>
...
<li><p><strong>Хранение в коллекции</strong>: Milvus сохраняет полученные разреженные вложения в коллекции для быстрого поиска и ранжирования.</p></li>
...
<h2 id="Create-a-collection-for-BM25-full-text-search" class="common-anchor-header">Создание коллекции для полнотекстового поиска BM25<button data-href="#Create-a-collection-for-BM25-full-text-search" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Чтобы включить полнотекстовый поиск на основе BM25, необходимо подготовить коллекцию с требуемыми полями, определить функцию BM25 для генерации разреженных векторов, настроить индекс, а затем создать коллекцию.</p>
...
<li><p><code translate="no">text</code>: хранит исходные текстовые данные для операций полнотекстового поиска. Тип данных должен быть <code translate="no">VARCHAR</code>, так как <code translate="no">VARCHAR</code> — это строковый тип данных Milvus для хранения текста.</p></li>
...
<h3 id="Define-the-BM25-function" class="common-anchor-header">Определение функции BM25<button data-href="#Define-the-BM25-function" class="anchor-icon" translate="no">
    ...
    </button></h3><p>Функция BM25 преобразует токенизированный текст в разреженные векторы, поддерживающие подсчёт оценки по алгоритму BM25.</p>
...
<table>
   ...
   <tr>
     ...
     <td><p>Имя функции. Эта функция преобразует исходный текст из поля « <code translate="no">text</code> » в разреженные векторы, совместимые с BM25, которые будут храниться в поле « <code translate="no">sparse</code> ».</p></td>
   </tr>
   <tr>
     <td><p><code translate="no">input_field_names</code></p></td>
     <td><p>Имя поля « <code translate="no">VARCHAR</code> », текст которого необходимо преобразовать в разреженные векторы. Для параметра « <code translate="no">FunctionType.BM25</code> » допускается указание только одного имени поля.</p></td>
   </tr>
   <tr>
...
<div class="alert note">
<p>Если обработка по алгоритму BM25 требуется для нескольких полей типа « <code translate="no">VARCHAR</code> », определите <strong>по одной функции BM25 для каждого поля</strong>, причём каждая из них должна иметь уникальное имя и поле вывода.</p>
</div>
<h3 id="Configure-the-index" class="common-anchor-header">Настройте индекс<button data-href="#Configure-the-index" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>После определения схемы с необходимыми полями и встроенной функцией настройте индекс для вашей коллекции.</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#go">   Go</a>
...
<table>
   ...
   <tr>
     ...
     <td><p>Имя векторного поля для индексирования. Для полнотекстового поиска это должно быть поле, в котором хранятся сгенерированные разреженные векторы. В данном примере установите значение <code translate="no">sparse</code>.</p></td>
   </tr>
   <tr>
     <td><p><code translate="no">index_type</code></p></td>
     <td><p>Тип создаваемого индекса. Значение « <code translate="no">AUTOINDEX</code> » позволяет Milvus автоматически оптимизировать настройки индекса. Если вам требуется более точный контроль над настройками индекса, вы можете выбрать один из различных типов индексов, доступных для разреженных векторов в Milvus. Дополнительную информацию см. в разделе <a href="/docs/ru/index.md#Indexes-supported-in-Milvus">«Индексы, поддерживаемые в Milvus</a>».</p></td>
   </tr>
   <tr>
     <td><p><code translate="no">metric_type</code></p></td>
     <td><p>Значение этого параметра должно быть установлено именно на « <code translate="no">BM25</code> » для обеспечения функциональности полнотекстового поиска.</p></td>
   </tr>
   <tr>
   ...
   <tr>
     ...
     <td><p>Алгоритм, используемый для построения индекса и выполнения запросов по нему. Допустимые значения:</p><ul><li><p><code translate="no">"DAAT_MAXSCORE"</code> (по умолчанию): Оптимизированная обработка запросов по принципу «документ за раз» (DAAT) с использованием алгоритма MaxScore. MaxScore обеспечивает более высокую производительность при высоких значениях <em>k</em> или при запросах с большим количеством терминов за счет пропуска терминов и документов, которые, вероятно, имеют минимальное влияние. Это достигается за счёт разделения терминов на существенные и несущественные группы на основе их максимальных оценок влияния, при этом основное внимание уделяется терминам, которые могут повлиять на результаты top-k.</p></li><li><p><code translate="no">"DAAT_WAND"</code>: Оптимизированная обработка запросов по методу «документ за раз» (DAAT) с использованием алгоритма WAND. WAND оценивает меньшее количество найденных документов, используя максимальные оценки влияния для пропуска неконкурентоспособных документов, но при этом имеет более высокие накладные расходы на каждый найденный документ. Это делает WAND более эффективным для запросов с небольшими значениями <em>k</em> или коротких запросов, где пропуск является более целесообразным.</p></li><li><p><code translate="no">"TAAT_NAIVE"</code>: Базовая обработка запросов по методу «Term-at-a-Time» (TAAT). Хотя этот метод работает медленнее по сравнению с методами « <code translate="no">DAAT_MAXSCORE</code> » и « <code translate="no">DAAT_WAND</code> », « <code translate="no">TAAT_NAIVE</code> » обладает уникальным преимуществом. В отличие от алгоритмов DAAT, которые используют кэшированные максимальные оценки влияния, остающиеся неизменными независимо от изменений глобального параметра коллекции (avgdl), « <code translate="no">TAAT_NAIVE</code> » динамически адаптируется к таким изменениям.</p></li></ul></td>
   </tr>
   <tr>
     <td><p><code translate="no">params.bm25_k1</code></p></td>
     <td><p>Регулирует степень насыщения частоты терминов. Более высокие значения повышают значимость частоты терминов при ранжировании документов. Диапазон значений: [1,2; 2,0].</p></td>
   </tr>
   <tr>
     <td><p><code translate="no">params.bm25_b</code></p></td>
     <td><p>Регулирует степень нормализации длины документа. Обычно используются значения от 0 до 1, значение по умолчанию — 0,75. Значение 0 означает отсутствие нормализации длины, а значение 1 — полную нормализацию длины.</p></td>
   </tr>
</table>
<h3 id="Create-the-collection" class="common-anchor-header">Создание коллекции<button data-href="#Create-the-collection" class="anchor-icon" translate="no">
    ...
    </button></h3><p>Теперь создайте коллекцию, используя определённые параметры схемы и индекса.</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#go">   Go</a>
...
<h2 id="Perform-full-text-search" class="common-anchor-header">Выполнение полнотекстового поиска<button data-href="#Perform-full-text-search" class="anchor-icon" translate="no">
    ...
    </button></h2><p>После вставки данных в коллекцию вы можете выполнять полнотекстовый поиск с использованием запросов в виде необработанного текста. Milvus автоматически преобразует ваш запрос в разреженный вектор и ранжирует найденные результаты поиска с помощью алгоритма BM25, а затем возвращает topK (<code translate="no">limit</code>) результатов.</p>
<div class="alert note">
<p>Вы можете выделять найденные термины в результатах поиска, настроив инструмент выделения текста. Подробности см. в разделе <a href="/docs/ru/text-highlighter.md">«Инструмент выделения текста</a> ».</p>
</div>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
...
<table>
   ...
   <tr>
     ...
     <td><p>Доля терминов с низкой значимостью, которые следует игнорировать при поиске. Подробности см. в разделе <a href="/docs/ru/sparse_vector.md">«Разреженный вектор</a>».</p></td>
   </tr>
   <tr>
     <td></td>
     <td></td>
   ...
   <tr>
     ...
     <td><p>Исходный текст запроса на естественном языке. Milvus автоматически преобразует ваш текстовый запрос в разреженные векторы с помощью функции BM25 — не предоставляйте заранее вычисленные векторы.</p></td>
   </tr>
   <tr>
   ...
   <tr>
     ...
     <td><p>Максимальное количество лучших совпадений для возврата.</p></td>
   </tr>
</table>
<h2 id="FAQ" class="common-anchor-header">Часто задаваемые вопросы<button data-href="#FAQ" class="anchor-icon" translate="no">
    ...
    </button></h2><h3 id="Can-I-output-or-access-the-sparse-vectors-generated-by-the-BM25-function-in-full-text-search" class="common-anchor-header">Можно ли вывести или получить доступ к разреженным векторам, сгенерированным функцией BM25 в полнотекстовом поиске?<button data-href="#Can-I-output-or-access-the-sparse-vectors-generated-by-the-BM25-function-in-full-text-search" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Нет, к разреженным векторам, сгенерированным функцией BM25, в полнотекстовом поиске нет прямого доступа, и их невозможно вывести. Подробности приведены ниже:</p>
<ul>
...
<p>Подробности см. в разделе <a href="/docs/ru/sparse_vector.md">«Редкие векторы</a>».</p>
