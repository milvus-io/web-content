...
title: Поиск по первичному ключуCompatible with Milvus 2.6.9+
summary: >-
  При выполнении поиска по схожести вам всегда предлагается указать один или
  несколько векторов запроса, даже если эти векторы уже присутствуют в целевой
  коллекции. Чтобы избежать извлечения векторов перед началом поиска, можно
  вместо них использовать первичные ключи.
beta: Milvus 2.6.9+
---
<h1 id="Primary-Key-Search" class="common-anchor-header">Поиск по первичному ключу<span class="beta-tag" style="background-color:rgb(0, 179, 255);color:white" translate="no">Compatible with Milvus 2.6.9+</span><button data-href="#Primary-Key-Search" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h1><p>При выполнении поиска по схожести всегда требуется указать один или несколько векторов запроса, даже если эти векторы уже присутствуют в целевой коллекции. Чтобы избежать извлечения векторов перед поиском, можно вместо них использовать первичные ключи.</p>
<h2 id="Overview" class="common-anchor-header">Обзор<button data-href="#Overview" class="anchor-icon" translate="no">
    ...
    </button></h2><p>На платформах электронной коммерции пользователи могут ввести ключевое слово, чтобы найти соответствующие ему товары. Как только пользователь переходит на страницу с подробной информацией о товаре, платформа также отображает внизу страницы список похожих товаров для тех, кто хочет их сравнить.</p>
<p>Рекомендации сортируются по степени сходства с ключевым словом или текущим товаром. Для этого разработчикам платформы необходимо извлечь векторное представление ключевого слова или текущего товара из Milvus до начала собственно поиска по сходству, что увеличивает количество обменных циклов между платформой и Milvus и приводит к передаче по сети большого количества высокоразмерных чисел с плавающей запятой.</p>
<p>Чтобы упростить логику взаимодействия между вашими приложениями и Milvus, сократить количество циклов обмена данными и избежать передачи по сети больших объемов высокоразмерных чисел с плавающей запятой, рассмотрите возможность использования поиска по первичному ключу.</p>
<p>При поиске по первичному ключу вам не нужно предоставлять какие-либо векторы запроса. Вместо этого вам предлагается указать первичные ключи (<code translate="no">ids</code>) сущностей, содержащих векторы запроса.</p>
<h2 id="Limits--restrictions" class="common-anchor-header">Ограничения<button data-href="#Limits--restrictions" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<li><p>Поиск с использованием первичных ключей применим ко всем типам векторных данных, за исключением разреженных векторных полей, полученных из полей VarChar, как, например, в функциях BM25.</p></li>
<li><p>Вы можете использовать первичные ключи вместо векторов запроса в фильтрованном поиске, поиске по диапазону и группированном поиске, при желании с включенной пагинацией. Однако эта функция не распространяется на гибридный поиск и итераторы поиска.</p></li>
<li><p>Для поиска по схожести с использованием списков вложений по-прежнему необходимо извлечь векторы запроса, сгруппировать их в списки вложений и запустить поиск.</p></li>
<li><p>В RESTful API нельзя использовать первичные ключи вместо векторов запроса.</p></li>
<li><p>При наличии несуществующих первичных ключей или ключей в некорректном формате Milvus выдаст сообщение об ошибке.</p></li>
<li><p>Первичные ключи и векторы запросов являются взаимоисключающими. Указание обоих также приводит к ошибкам.</p></li>
</ul>
<h2 id="Examples" class="common-anchor-header">Примеры<button data-href="#Examples" class="anchor-icon" translate="no">
    ...
    </button></h2><p>В следующих примерах предполагается, что все указанные идентификаторы Int64 присутствуют в целевой коллекции.</p>
<div class="alert note">
<p>Первичные ключи не используются для фильтрации; они используются только для поиска векторов.</p>
</div>
<h3 id="Example-1-Basic-primary-key-search" class="common-anchor-header">Пример 1: Базовый поиск по первичному ключу<button data-href="#Example-1-Basic-primary-key-search" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Чтобы выполнить базовый поиск по первичному ключу, просто замените векторы запроса на первичные ключи.</p>
...
<button class="copy-code-btn"></button></code></pre>
<h3 id="Example-2-Filtered-search-using-primary-keys" class="common-anchor-header">Пример 2: Поиск с фильтрацией по первичным ключам<button data-href="#Example-2-Filtered-search-using-primary-keys" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>В следующем примере предполагается, что color и likes — это два поля, определённые в схеме целевой коллекции.</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#javascript">   NodeJS</a>
...
<button class="copy-code-btn"></button></code></pre>
<h3 id="Example-3-Range-search-using-primary-keys" class="common-anchor-header">Пример 3: Поиск по диапазону с использованием первичных ключей<button data-href="#Example-3-Range-search-using-primary-keys" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<button class="copy-code-btn"></button></code></pre>
<h3 id="Example-4-Grouping-search-using-primary-keys" class="common-anchor-header">Пример 4: Групповой поиск с использованием первичных ключей<button data-href="#Example-4-Grouping-search-using-primary-keys" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>В следующем примере предполагается, что « <code translate="no">docId</code> » — это поле, определённое в схеме целевой коллекции.</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#javascript">   NodeJS</a>
