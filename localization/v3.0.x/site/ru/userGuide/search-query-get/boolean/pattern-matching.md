...
<h1 id="Pattern-Matching" class="common-anchor-header">Сопоставление шаблонов<button data-href="#Pattern-Matching" class="anchor-icon" translate="no">
    ...
    </button></h1><p>В приложениях агентного поиска векторный поиск и сопоставление шаблонов в стиле grep часто дополняют друг друга. Векторный поиск извлекает сущности, имеющие семантическое отношение, в то время как сопоставление шаблонов сужает эти результаты с помощью точных строковых структур, таких как коды ошибок, префиксы журналов, домены электронной почты, пути URL или идентификаторы.</p>
...
<tr><td>Поиск шаблона без учета регистра</td><td><code translate="no">=~</code> с <code translate="no">(?i)</code></td><td><code translate="no">message =~ &quot;(?i)error&quot;</code></td><td>Находит <code translate="no">error</code>, <code translate="no">ERROR</code> или другие варианты с учетом регистра.</td></tr>
...
<h3 id="LIKE-matching-behavior" class="common-anchor-header">Поведение сопоставления LIKE<button data-href="#LIKE-matching-behavior" class="anchor-icon" translate="no">
    ...
    </button></h3><p>Используйте <code translate="no">LIKE</code> для префиксов, суффиксов, содержания и совпадений одного символа в фиксированной позиции. <code translate="no">LIKE</code> не поддерживает классы символов, такие как <code translate="no">[0-9]</code>, альтернативы, такие как <code translate="no">error|failed</code>, количество повторений, такие как <code translate="no">{4}</code>, якоря, такие как <code translate="no">^</code> или <code translate="no">$</code>, а также флаги, игнорирующие регистр, такие как <code translate="no">(?i)</code>. Для этих шаблонов используйте регулярные выражения.</p>
...
<h3 id="Common-regex-patterns" class="common-anchor-header">Распространенные шаблоны регулярных выражений<button data-href="#Common-regex-patterns" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
