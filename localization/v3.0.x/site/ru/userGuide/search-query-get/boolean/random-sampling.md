...
title: Случайная выборкаCompatible with Milvus 2.6.x
summary: >-
  При работе с большими массивами данных часто не требуется обрабатывать все
  данные, чтобы получить представление о них или протестировать логику
  фильтрации. Случайная выборка позволяет работать со статистически
  репрезентативным подмножеством данных, значительно сокращая время запроса и
  расход ресурсов.
beta: Milvus 2.6.x
---
<h1 id="Random-Sampling" class="common-anchor-header">Случайная выборка<span class="beta-tag" style="background-color:rgb(0, 179, 255);color:white" translate="no">Compatible with Milvus 2.6.x</span><button data-href="#Random-Sampling" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h1><p>При работе с большими массивами данных часто не требуется обрабатывать все данные, чтобы получить представление о них или проверить логику фильтрации. Случайная выборка позволяет работать со статистически репрезентативным подмножеством данных, значительно сокращая время выполнения запросов и потребление ресурсов.</p>
<p>Случайная выборка работает на уровне сегмента, обеспечивая эффективную производительность при сохранении случайности выборки во всем распределении данных в вашей коллекции.</p>
<p><strong>Ключевые примеры использования:</strong></p>
<ul>
<li><p><strong>Изучение данных</strong>: Быстрый предварительный просмотр структуры и содержимого коллекции с минимальным использованием ресурсов.</p></li>
<li><p><strong>Тестирование при разработке</strong>: Тестирование сложной логики фильтрации на управляемых выборках данных перед полным развертыванием.</p></li>
<li><p><strong>Оптимизация ресурсов</strong>: Сокращение вычислительных затрат для поисковых запросов и статистического анализа.</p></li>
</ul>
<h2 id="Syntax" class="common-anchor-header">Синтаксис<button data-href="#Syntax" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<ul>
<li><code translate="no">sampling_factor</code>: Коэффициент выборки в диапазоне (0, 1), без учета границ. Например, <code translate="no">RANDOM_SAMPLE(0.001)</code> отбирает примерно 0,1 % результатов.</li>
</ul>
<p><strong>Важные правила:</strong></p>
<ul>
<li><p>Выражение не зависит от регистра (<code translate="no">RANDOM_SAMPLE</code> или <code translate="no">random_sample</code>).</p></li>
<li><p>Коэффициент выборки должен находиться в диапазоне (0, 1), исключая границы.</p></li>
</ul>
<h2 id="Combine-with-other-filters" class="common-anchor-header">Комбинирование с другими фильтрами<button data-href="#Combine-with-other-filters" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>Оператор случайной выборки должен комбинироваться с другими выражениями фильтрации с помощью логического <code translate="no">AND</code>. При комбинировании фильтров Milvus сначала применяет другие условия, а затем выполняет случайную выборку в наборе результатов.</p>
...
<h2 id="Examples" class="common-anchor-header">Примеры<button data-href="#Examples" class="anchor-icon" translate="no">
    ...
    </button></h2><h3 id="Example-1-Data-exploration" class="common-anchor-header">Пример 1: Исследование данных<button data-href="#Example-1-Data-exploration" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Быстрый предварительный просмотр структуры коллекции:</p>
<div class="multipleCode">
   <a href="#python">Python</a> <a href="#java">Java</a> <a href="#go">Go</a> <a href="#javascript">NodeJS</a> <a href="#bash">cURL</a></div>
...
<pre><code translate="no" class="language-bash"><span class="hljs-comment"># restful</span>
<button class="copy-code-btn"></button></code></pre>
<h3 id="Example-2-Combined-filtering-with-random-sampling" class="common-anchor-header">Пример 2: Комбинированная фильтрация со случайной выборкой<button data-href="#Example-2-Combined-filtering-with-random-sampling" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Проверьте логику фильтрации на управляемом подмножестве:</p>
<div class="multipleCode">
   <a href="#python">Python</a> <a href="#java">Java</a> <a href="#go">Go</a> <a href="#javascript">NodeJS</a> <a href="#bash">cURL</a></div>
...
<pre><code translate="no" class="language-bash"><span class="hljs-comment"># restful</span>
<button class="copy-code-btn"></button></code></pre>
<h3 id="Example-3-Quick-analytics" class="common-anchor-header">Пример 3: Быстрая аналитика<button data-href="#Example-3-Quick-analytics" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Выполните быстрый статистический анализ отфильтрованных данных:</p>
<div class="multipleCode">
   <a href="#python">Python</a> <a href="#java">Java</a> <a href="#go">Go</a> <a href="#javascript">NodeJS</a> <a href="#bash">cURL</a></div>
...
<pre><code translate="no" class="language-bash"><span class="hljs-comment"># restful</span>
<button class="copy-code-btn"></button></code></pre>
<h3 id="Example-4-Combined-with-vector-search" class="common-anchor-header">Пример 4: Комбинация с векторным поиском<button data-href="#Example-4-Combined-with-vector-search" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Использование случайной выборки в сценариях поиска с фильтрацией:</p>
<div class="multipleCode">
   <a href="#python">Python</a> <a href="#java">Java</a> <a href="#go">Go</a> <a href="#javascript">NodeJS</a> <a href="#bash">cURL</a></div>
...
<li><p><strong>Начните с малого</strong>: для первоначального изучения начните с меньших коэффициентов выборки (0,001-0,01).</p></li>
<li><p><strong>Рабочий процесс разработки</strong>: Используйте выборку во время разработки, а для производственных запросов удалите.</p></li>
<li><p><strong>Статистическая достоверность</strong>: Большие выборки обеспечивают более точное статистическое представление</p></li>
<li><p><strong>Тестирование производительности</strong>: Мониторинг производительности запросов и корректировка коэффициентов выборки по мере необходимости</p></li>
</ul>
