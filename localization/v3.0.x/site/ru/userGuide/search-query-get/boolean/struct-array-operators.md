...
<p>Используйте скалярные подполя в операторах StructArray. Векторные подполя используются в векторных путях поиска и не являются входными данными для скалярных предикатов.</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>При сочетании предиката на уровне сущности с <code translate="no">element_filter</code> помещайте <code translate="no">element_filter</code> в конец выражения:</p>
...
<tbody>
<tr><td><code translate="no">BOOL</code></td><td>Скалярные предикаты, такие как <code translate="no">$[has_code] == true</code> или <code translate="no">!($[has_code] == true)</code>. Следует избегать использования булевых выражений без предефинирования, таких как <code translate="no">$[has_code]</code>.</td></tr>
...
<li><p><a href="/docs/ru/filtered-search-with-structarray.md">Фильтрованный поиск с использованием StructArray</a></p></li>
