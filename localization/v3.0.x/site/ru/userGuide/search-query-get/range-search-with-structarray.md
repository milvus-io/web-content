...
<p>Если установлен только параметр « <code translate="no">radius</code> », поиск по диапазону возвращает результаты, удовлетворяющие внешней границе метрики. Выбирайте значения в соответствии со шкалой оценок или расстояний ваших вложений.</p>
<h2 id="Run-element-level-range-search" class="common-anchor-header">Запуск поиска по диапазону на уровне элементов<button data-href="#Run-element-level-range-search" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
