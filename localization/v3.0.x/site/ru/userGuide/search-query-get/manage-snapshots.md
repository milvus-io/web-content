...
<p>Вызов <code translate="no">flush()</code> не является обязательным, но настоятельно рекомендуется во избежание потери данных. Если это пропустить, моментальный снимок будет содержать только те данные, которые уже были выгружены.</p>
</div>
...
<ul>
<li><p>непосредственно копирует файлы сегментов (бинлоги, дельталоги, индексные файлы) из хранилища моментальных снимков</p></li>
