...
summary: >-
  Узнайте, как создавать, отображать в списке, описывать, закреплять,
  восстанавливать и удалять моментальные снимки, а также отслеживать задания на
  восстановление.
beta: Milvus 3.0.x
---
<h1 id="Manage-Snapshots" class="common-anchor-header">Управление моментальными снимками<span class="beta-tag" style="background-color:rgb(0, 179, 255);color:white" translate="no">Compatible with Milvus 3.0.x</span><button data-href="#Manage-Snapshots" class="anchor-icon" translate="no">
    ...
    </button></h1><p>В этом руководстве вы узнаете, как создавать и управлять моментальными снимками, в том числе</p>
<ul>
<li><a href="#Create-snapshot">создать моментальный снимок</a>,</li>
<li>просмотр<a href="#List-snapshots">списка снимков</a>,</li>
<li><a href="#Describe-snapshot">описать моментальный снимок</a>,</li>
<li><a href="#Pinunpin-snapshot-data">закрепить/открепить данные моментального снимка</a>,</li>
<li><a href="#Restore-snapshot">восстановление моментального снимка</a>,</li>
<li><a href="#Drop-snapshot">Удалить моментальный снимок</a>,</li>
<li><a href="#List-restoration-jobs">Отобразить список заданий восстановления</a>, и</li>
<li><a href="#Get-restoration-state">Получить информацию о состоянии восстановления</a>.</li>
</ul>
<h2 id="Create-snapshot" class="common-anchor-header">Создание моментального снимка<button data-href="#Create-snapshot" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>Перед созданием моментального снимка рекомендуется приостановить запись данных в целевую коллекцию и вызвать функцию <code translate="no">flush()</code>, чтобы избежать возможной потери данных.</p>
<div class="alert note">
<p>Вызов команды ` <code translate="no">flush()</code> ` не является обязательным, но настоятельно рекомендуется для предотвращения потери данных. Если вы пропустите этот шаг, моментальный снимок будет содержать только те данные, которые уже были сброшены в кэш.</p>
</div>
<p>При присвоении имени моментальному снимку используйте понятные и описательные имена, такие как <code translate="no">&quot;daily_backup_20240101&quot;</code> или <code translate="no">&quot;v2.1_production_release&quot;</code>, и избегайте общих терминов, таких как <code translate="no">&quot;backup1&quot;</code> и <code translate="no">&quot;test&quot;</code>. Внимательно подбирайте имена моментальных снимков, чтобы различать их по версиям, средам и этапам.</p>
...
<button class="copy-code-btn"></button></code></pre>
<h2 id="List-snapshots" class="common-anchor-header">Вывод списка снимков<button data-href="#List-snapshots" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>Вы можете отобразить список имен существующих снимков.</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#go">   Go</a>
...
<button class="copy-code-btn"></button></code></pre>
<h2 id="Describe-snapshot" class="common-anchor-header">Описание моментального снимка<button data-href="#Describe-snapshot" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>Вы можете получить подробную информацию о конкретном снимке.</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#go">   Go</a>
...
<pre><code translate="no" class="language-bash"><span class="hljs-comment"># restful</span>
<button class="copy-code-btn"></button></code></pre>
<h2 id="Pinunpin-snapshot-data" class="common-anchor-header">Закрепить/открепить данные моментального снимка<button data-href="#Pinunpin-snapshot-data" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>Во время восстановления вы можете закрепить моментальный снимок, чтобы временно защитить его базовые данные от сборки мусора, и открепить его, чтобы освободить данные.</p>
<p>Вы также можете установить срок действия (TTL) для операции фиксации, чтобы зафиксированные данные были освобождены по истечении этого срока.</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#go">   Go</a>
...
<pre><code translate="no" class="language-bash"><span class="hljs-comment"># restful</span>
<button class="copy-code-btn"></button></code></pre>
<h2 id="Restore-snapshot" class="common-anchor-header">Восстановление моментального снимка<button data-href="#Restore-snapshot" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>Вы можете восстановить моментальный снимок в новую коллекцию. Эта операция выполняется асинхронно и возвращает идентификатор задания для отслеживания хода восстановления.</p>
<p>При восстановлении используется механизм <strong>копирования сегментов</strong> вместо импорта данных, что более эффективно, поскольку</p>
<ul>
<li>непосредственно копирует файлы сегментов (binlogs, deltalogs, файлы индексов) из хранилища моментальных снимков</li>
<li>сохраняет идентификаторы полей и индексов, обеспечивая совместимость с существующими файлами данных</li>
<li>избегает перезаписи данных и перестроения индексов, что приводит к значительному сокращению времени восстановления, а также</li>
<li>обеспечивает повышение производительности в 10–100 раз по сравнению с традиционными методами резервного копирования и восстановления</li>
</ul>
...
<pre><code translate="no" class="language-bash"><span class="hljs-comment"># restful</span>
<button class="copy-code-btn"></button></code></pre>
<p>Подробнее о мониторинге хода задания восстановления см. в разделе <a href="#Get-restoration-state">«Получение состояния восстановления</a>».</p>
<h2 id="Drop-snapshot" class="common-anchor-header">Удаление моментального снимка<button data-href="#Drop-snapshot" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>Вы можете удалить моментальный снимок, если он больше не нужен. Рекомендуется регулярно удалять старые моментальные снимки, чтобы сэкономить место на хранилище.</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#go">   Go</a>
...
<pre><code translate="no" class="language-bash"><span class="hljs-comment"># restful</span>
<button class="copy-code-btn"></button></code></pre>
<h2 id="List-restoration-jobs" class="common-anchor-header">Список заданий восстановления<button data-href="#List-restoration-jobs" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>С помощью этого API можно получить список снимков, уже созданных для целевой коллекции.</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#go">   Go</a>
...
<pre><code translate="no" class="language-bash"><span class="hljs-comment"># restful</span>
<button class="copy-code-btn"></button></code></pre>
<h2 id="Get-restoration-state" class="common-anchor-header">Получение информации о состоянии восстановления<button data-href="#Get-restoration-state" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>Получив идентификатор задания восстановления, вы можете использовать его для получения информации о ходе восстановления.</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#go">   Go</a>
