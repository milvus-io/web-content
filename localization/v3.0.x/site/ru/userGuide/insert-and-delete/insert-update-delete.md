...
<li><p><strong>Обработка дубликатов</strong>: стандартная операция <code translate="no">insert</code> не проверяет наличие дубликатов первичных ключей. Вставка данных с уже существующим первичным ключом приводит к созданию новой сущности с тем же ключом, что вызывает дублирование данных и может вызвать проблемы в приложении. Чтобы обновить существующие сущности или избежать дубликатов, используйте вместо этого операцию <strong><code translate="no">upsert</code></strong> . Дополнительные сведения см. в разделе <a href="/docs/ru/upsert-entities.md">«Upsert сущностей</a>».</p></li>
</ul>
</div>
...
<p>При вставке сущности в коллекцию вставляемая сущность может быть успешно добавлена только в том случае, если она содержит все поля, определённые в схеме. Вставленная сущность попадает в партицию с именем <strong>_default</strong> в порядке вставки. При наличии определенной партиции вы также можете вставлять сущности в эту партицию, указав имя партиции в запросе на вставку.</p>
