...
<h3 id="Option-2-Environment-variable" class="common-anchor-header">Вариант 2: Переменная среды<button data-href="#Option-2-Environment-variable" class="anchor-icon" translate="no">
    ...
    </button></h3><p>Если в файле <code translate="no">milvus.yaml</code> не настроено соответствующее учетное имя, Milvus может считывать ключ API DashScope из следующей переменной среды:</p>
<table>
   <tr>
