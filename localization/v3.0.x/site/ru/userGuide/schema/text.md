...
<tr><td>Параметр «Длина»</td><td>Требуется параметр <code translate="no">max_length</code>, который определяет максимальное количество байтов, которое может хранить поле. Максимальное значение составляет <code translate="no">65,535</code> байт. Если значение может превысить этот предел, используйте параметр <code translate="no">TEXT</code>.</td><td>Не требует параметра <code translate="no">max_length</code>, поэтому в схеме не требуется фиксированное ограничение на количество байтов для текстового значения.</td></tr>
