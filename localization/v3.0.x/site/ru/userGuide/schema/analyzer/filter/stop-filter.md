...
title: Остановка
summary: >-
  Используйте стоп-фильтр, чтобы удалить сконфигурированные стоп-слова из
  токенизированного текста во время анализа.
---
<h1 id="Stop" class="common-anchor-header">Остановка<button data-href="#Stop" class="anchor-icon" translate="no">
    ...
    </button></h1><p>Фильтр <code translate="no">stop</code> удаляет заданные стоп-слова из токенизированного текста, помогая устранить обычные, менее значимые слова. Список стоп-слов можно настроить с помощью параметра <code translate="no">stop_words</code>.</p>
<h2 id="Configuration" class="common-anchor-header">Конфигурация<button data-href="#Configuration" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Фильтр <code translate="no">stop</code> получает список стоп-слов либо в строке с помощью параметра <code translate="no">stop_words</code>, либо из зарегистрированного файлового ресурса с помощью параметра <code translate="no">stop_words_file</code>.</p>
<h3 id="Inline-stop-words-list" class="common-anchor-header">Встроенный список стоп-слов<button data-href="#Inline-stop-words-list" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Чтобы использовать фильтр <code translate="no">stop</code> с инлайн-списком, укажите в конфигурации фильтра параметр <code translate="no">&quot;type&quot;: &quot;stop&quot;</code>, а также параметр <code translate="no">stop_words</code>, который предоставляет список стоп-слов.</p>
...
<table>
   ...
   <tr>
     ...
     <td><p>Список слов, которые должны быть удалены из токенизации. По умолчанию фильтр использует встроенный словарь <code translate="no">_english_</code>. Вы можете переопределить или расширить его тремя способами:</p><ul><li><p><strong>Встроенные словари</strong> - укажите один из этих языковых псевдонимов, чтобы использовать предопределенный словарь:</p><p><code translate="no">"_english_"</code>, <code translate="no">"_danish_"</code>, <code translate="no">"_dutch_"</code>, <code translate="no">"_finnish_"</code>, <code translate="no">"_french_"</code>, <code translate="no">"_german_"</code>, <code translate="no">"_hungarian_"</code>, <code translate="no">"_italian_"</code>, <code translate="no">"_norwegian_"</code>, <code translate="no">"_portuguese_"</code>, <code translate="no">"_russian_"</code>, <code translate="no">"_spanish_"</code>, <code translate="no">"_swedish_"</code></p></li><li><p><strong>Пользовательский список</strong> - передайте массив собственных терминов, например, <code translate="no">["foo", "bar", "baz"]</code>.</p></li><li><p><strong>Смешанный список</strong> - объединяет псевдонимы и пользовательские термины, например <code translate="no">["of", "to", "_english_"]</code>.</p><p>Подробнее о точном содержании каждого предопределенного словаря см. в разделе <a href="https://github.com/milvus-io/milvus/blob/master/internal/core/thirdparty/tantivy/tantivy-binding/src/analyzer/filter/stop_words.rs">stop_words</a>.</p></li></ul></td>
   </tr>
</table>
...
<h3 id="Load-stop-words-from-a-file-resource--Milvus-30x" class="common-anchor-header">Загрузка стоп-слов из файлового ресурса<span class="beta-tag" style="background-color:rgb(0, 179, 255);color:white" translate="no">Compatible with Milvus 3.0.x</span><button data-href="#Load-stop-words-from-a-file-resource--Milvus-30x" class="anchor-icon" translate="no">
    ...
    </button></h3><p>Для создания больших пользовательских списков стоп-слов - списков для конкретного языка, доменных словарей или списков, которые вы хотите использовать во многих коллекциях, - сохраните слова в файле и зарегистрируйте его как удаленный файловый ресурс, а затем ссылайтесь на него в фильтре с помощью параметра <code translate="no">stop_words_file</code>. Вы можете использовать <code translate="no">stop_words_file</code> сам по себе или вместе со встроенным <code translate="no">stop_words</code>; если заданы оба параметра, фильтр объединяет два источника в один список стоп-слов.</p>
<p>Файл представляет собой обычный текст в формате UTF-8 с <strong>одним стоп-словом на строку</strong>. Например:</p>
<pre><code translate="no" class="language-plaintext">the
of
for
<button class="copy-code-btn"></button></code></pre>
<p>Загрузите файл в хранилище объектов, на использование которого настроен кластер Milvus, а затем зарегистрируйте его:</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>Ссылайтесь на зарегистрированный ресурс в фильтре через <code translate="no">stop_words_file</code>:</p>
<pre><code translate="no" class="language-python">analyzer_params = {
    <span class="hljs-string">&quot;tokenizer&quot;</span>: <span class="hljs-string">&quot;standard&quot;</span>,
    <span class="hljs-string">&quot;filter&quot;</span>: [{
        <span class="hljs-string">&quot;type&quot;</span>: <span class="hljs-string">&quot;stop&quot;</span>,
...
<table>
   ...
   <tr>
     ...
     <td><p>Часть имени файла в пути к объектному хранилищу зарегистрированного ресурса (например, <code translate="no">"stop_words.txt"</code>, если ресурс был зарегистрирован с помощью <code translate="no">path="file/stop_words.txt"</code>).</p></td>
   </tr>
</table>
<h2 id="Examples" class="common-anchor-header">Примеры<button data-href="#Examples" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Прежде чем применять конфигурацию анализатора к схеме коллекции, проверьте ее поведение с помощью метода <code translate="no">run_analyzer</code>.</p>
