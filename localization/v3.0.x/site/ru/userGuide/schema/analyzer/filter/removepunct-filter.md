...
title: Удаление знаков препинанияCompatible with Milvus 2.5.11+
summary: >-
  Фильтр `removepunct` удаляет лексемы, содержащие знаки препинания или пробелы.
  При использовании токенизаторов, которые сохраняют знаки препинания внутри
  слов, он удаляет всю соответствующую лексему целиком.
beta: Milvus 2.5.11+
---
<h1 id="Remove-Punct" class="common-anchor-header">Удаление знаков препинания<span class="beta-tag" style="background-color:rgb(0, 179, 255);color:white" translate="no">Compatible with Milvus 2.5.11+</span><button data-href="#Remove-Punct" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h1><p>Фильтр « <code translate="no">removepunct</code> » удаляет из потока токенов те, которые содержат знаки препинания или пробелы. Используйте его, если вам нужна более чистая обработка текста, ориентированная на содержательные слова, а не на знаки препинания.</p>
<div class="alert note">
<p>Этот фильтр наиболее эффективен в сочетании с токенизаторами <code translate="no">jieba</code>, <code translate="no">lindera</code> и <code translate="no">icu</code>, которые сохраняют знаки препинания в виде отдельных токенов (например, <code translate="no">&quot;Hello!&quot;</code> → <code translate="no">[&quot;Hello&quot;, &quot;!&quot;]</code>). Токенизатор <code translate="no">standard</code> удаляет знаки препинания при токенизации. Токенизатор <code translate="no">whitespace</code> сохраняет знаки препинания, в том числе знаки препинания внутри токена. В сочетании с <code translate="no">whitespace</code> функция <code translate="no">removepunct</code> удаляет весь токен, если он содержит знаки препинания или пробелы; она не удаляет отдельные символы из токена.</p>
</div>
<h2 id="Configuration" class="common-anchor-header">Настройка<button data-href="#Configuration" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Фильтр « <code translate="no">removepunct</code> » встроен в Milvus. Чтобы использовать его, просто укажите его имя в разделе « <code translate="no">filter</code> » в файле « <code translate="no">analyzer_params</code> ».</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>Фильтр « <code translate="no">removepunct</code> » работает с терминами, сгенерированными токенизатором, поэтому его необходимо использовать в сочетании с токенизатором.</p>
