---
id: pinyin-filter.md
title: ПиньиньCompatible with Milvus 3.0.x
summary: >-
  Фильтр пиньинь преобразует лексемы, состоящие из китайских иероглифов, в
  лексемы пиньинь в ходе анализа текста, что позволяет осуществлять
  сопоставление китайского текста на основе пиньинь.
beta: Milvus 3.0.x
---
<h1 id="Pinyin" class="common-anchor-header">Пиньинь<span class="beta-tag" style="background-color:rgb(0, 179, 255);color:white" translate="no">Compatible with Milvus 3.0.x</span><button data-href="#Pinyin" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<h2 id="Configuration" class="common-anchor-header">Настройка<button data-href="#Configuration" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Чтобы использовать параметры по умолчанию, укажите <code translate="no">&quot;pinyin&quot;</code> в разделе « <code translate="no">filter</code> » файла <code translate="no">analyzer_params</code>.</p>
...
<p>Эта сокращённая запись сохраняет исходные китайские токены и генерирует токены пиньинь на уровне символов. Она не генерирует соединенные формы пиньинь или инициалы пиньинь, если вы явно не включите эти параметры.</p>
<p>Для полного контроля укажите фильтр в виде объекта и настройте формы токенов пиньинь, которые генерирует Milvus.</p>
<pre><code translate="no" class="language-python">analyzer_params = {
    <span class="hljs-string">&quot;tokenizer&quot;</span>: <span class="hljs-string">&quot;jieba&quot;</span>,
...
<button class="copy-code-btn"></button></code></pre>
<p>Фильтр принимает следующие параметры.</p>
<table>
<thead>
...
<tr><td><code translate="no">keep_separate_first_letter</code></td><td>Булево</td><td><code translate="no">false</code></td><td>Выдает токен с инициалами пиньинь для каждого исходного токена. Например, <code translate="no">中文</code> выдает <code translate="no">zw</code>.</td></tr>
</tbody>
</table>
...
<h2 id="Examples" class="common-anchor-header">Примеры<button data-href="#Examples" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Прежде чем применять конфигурацию анализатора к схеме вашей коллекции, проверьте его поведение с помощью <code translate="no">run_analyzer</code>.</p>
...
<button class="copy-code-btn"></button></code></pre>
<h3 id="Match-Chinese-text-with-character-level-Pinyin" class="common-anchor-header">Сопоставление китайского текста с токенами пиньинь на уровне символов<button data-href="#Match-Chinese-text-with-character-level-Pinyin" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Фильтр <code translate="no">pinyin</code> по умолчанию сохраняет исходные китайские токены и выдает токены пиньинь на уровне символов.</p>
<pre><code translate="no" class="language-python">analyzer_params = {
    <span class="hljs-string">&quot;tokenizer&quot;</span>: <span class="hljs-string">&quot;jieba&quot;</span>,
...
<h3 id="Match-Chinese-terms-with-Pinyin-initials" class="common-anchor-header">Сопоставление китайских терминов с начальными буквами пиньинь<button data-href="#Match-Chinese-terms-with-Pinyin-initials" class="anchor-icon" translate="no">
    ...
    </button></h3><p>Включите функцию « <code translate="no">keep_separate_first_letter</code> », если вам нужно, чтобы китайский термин соответствовал начальным буквам его формы пиньинь.</p>
<pre><code translate="no" class="language-python">analyzer_params = {
    <span class="hljs-string">&quot;tokenizer&quot;</span>: <span class="hljs-string">&quot;jieba&quot;</span>,
    <span class="hljs-string">&quot;filter&quot;</span>: [
        {
