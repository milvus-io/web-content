...
title: Расщепитель
summary: >-
  Используйте фильтр decompounder для разделения сложных слов с помощью
  встроенного словаря или зарегистрированного файлового ресурса.
---
<h1 id="Decompounder" class="common-anchor-header">Расщепитель<button data-href="#Decompounder" class="anchor-icon" translate="no">
    ...
    </button></h1><p>Фильтр <code translate="no">decompounder</code> разделяет сложные слова на отдельные компоненты на основе заданного словаря, облегчая поиск частей сложных терминов. Этот фильтр особенно полезен для языков, в которых часто используются составные слова, например, для немецкого. Словарь компонентов может быть задан в строке с помощью параметра <code translate="no">word_list</code> или загружен из <a href="/docs/ru/manage-file-resources.md">зарегистрированного файлового ресурса</a> с помощью параметра <code translate="no">word_list_file</code>.</p>
<h2 id="Configuration" class="common-anchor-header">Конфигурация<button data-href="#Configuration" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Фильтр <code translate="no">decompounder</code> принимает свой словарь компонентов либо в строке через параметр <code translate="no">word_list</code>, либо из зарегистрированного файлового ресурса через параметр <code translate="no">word_list_file</code>.</p>
<h3 id="Inline-word-list" class="common-anchor-header">Встроенный список слов<button data-href="#Inline-word-list" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Фильтр <code translate="no">decompounder</code> - это пользовательский фильтр в Milvus. Чтобы использовать его, укажите <code translate="no">&quot;type&quot;: &quot;decompounder&quot;</code> в конфигурации фильтра, а также параметр <code translate="no">word_list</code>, который предоставляет словарь компонентов слов для распознавания.</p>
...
<h3 id="Load-word-components-from-a-file-resource--Milvus-30x" class="common-anchor-header">Загрузка компонентов слова из файлового ресурса<span class="beta-tag" style="background-color:rgb(0, 179, 255);color:white" translate="no">Compatible with Milvus 3.0.x</span><button data-href="#Load-word-components-from-a-file-resource--Milvus-30x" class="anchor-icon" translate="no">
    ...
    </button></h3><p>Для больших словарей компонентов - особенно полноязычных списков слов - храните компоненты в файле и регистрируйте файл как удаленный файловый ресурс, а затем ссылайтесь на него в фильтре через параметр <code translate="no">word_list_file</code>. Вы можете использовать <code translate="no">word_list_file</code> самостоятельно или вместе со встроенным <code translate="no">word_list</code>; если заданы оба параметра, фильтр объединяет два источника в один список компонентов.</p>
<p>Файл представляет собой обычный текст в формате UTF-8 с <strong>одним словом компонента в строке</strong>. Например:</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>Ссылайтесь на зарегистрированный ресурс в фильтре через <code translate="no">word_list_file</code>:</p>
<pre><code translate="no" class="language-python">analyzer_params = {
    <span class="hljs-string">&quot;tokenizer&quot;</span>: <span class="hljs-string">&quot;standard&quot;</span>,
    <span class="hljs-string">&quot;filter&quot;</span>: [{
        <span class="hljs-string">&quot;type&quot;</span>: <span class="hljs-string">&quot;decompounder&quot;</span>,
...
<button class="copy-code-btn"></button></code></pre>
<p>Параметр <code translate="no">word_list_file</code> принимает объект со следующими полями:</p>
<table>
   <tr>
   ...
   <tr>
     ...
     <td><p>Часть имени файла в пути к объектному хранилищу зарегистрированного ресурса (например, <code translate="no">"decompounder.txt"</code>, если ресурс был зарегистрирован с помощью <code translate="no">path="file/decompounder.txt"</code>).</p></td>
   </tr>
</table>
