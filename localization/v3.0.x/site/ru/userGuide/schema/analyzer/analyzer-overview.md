...
<div class="alert note">
<ul>
<li>Если при создании коллекции не указаны настройки анализатора, Milvus по умолчанию использует анализатор « <code translate="no">standard</code> » для всей обработки текста. Подробности см. в разделе <a href="/docs/ru/standard-analyzer.md">«Стандартный анализатор</a>».</li>
...
<p>Например, чтобы использовать встроенный анализатор « <code translate="no">standard</code> », просто укажите его имя <code translate="no">standard</code> в качестве параметра <code translate="no">type</code> и, при необходимости, добавьте дополнительные настройки, характерные для этого типа анализатора, такие как <code translate="no">stop_words</code>:</p>
...
<p>Прежде чем включить эти конфигурации в вашу коллекцию, вы проверите каждый анализатор с помощью метода ` <code translate="no">run_analyzer</code> `.</p>
