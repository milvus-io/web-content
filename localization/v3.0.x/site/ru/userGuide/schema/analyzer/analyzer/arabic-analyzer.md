...
<li><strong>Нормализация арабского текста</strong>: использует фильтр <code translate="no">arabic_normalization</code> для нормализации вариантов буквы «Алеф», «Тех Марбута» и «Алеф Максура», а также для удаления знаков «Харакат» и «Татвиль».</li>
<li><strong>Стеминг</strong>: использует фильтр <code translate="no">stemmer</code> с параметром « <code translate="no">language</code> » (Стеминг-слова), установленным в значение « <code translate="no">arabic</code> » (Стеминг по корням).</li>
