...
summary: >-
  ...
  объединяет стандартный токенизатор с фильтром преобразования в нижний регистр.
---
<h1 id="Standard-Analyzer" class="common-anchor-header">Стандартный анализатор<button data-href="#Standard-Analyzer" class="anchor-icon" translate="no">
    ...
    </button></h1><p>Анализатор « <code translate="no">standard</code> » является анализатором по умолчанию в Milvus, который автоматически применяется к текстовым полям, если не указан другой анализатор. Он сочетает в себе стандартный токенизатор и фильтр, преобразующий символы в нижний регистр.</p>
<div class="alert note">
<p>Анализатор « <code translate="no">standard</code> » подходит для языков, в которых границы слов определяются разделителями (такими как пробелы и знаки препинания). Однако для таких языков, как китайский, арабский, тайский, японский и корейский, требуется токенизация или нормализация с учётом особенностей конкретного языка. В таких случаях следует использовать языкоспецифический анализатор, например <a href="/docs/ru/chinese-analyzer.md"><code translate="no">chinese</code></a>, <a href="/docs/ru/arabic-analyzer.md"><code translate="no">arabic</code></a>, или <a href="/docs/ru/thai-analyzer.md"><code translate="no">thai</code></a>, либо пользовательские анализаторы со специализированными токенизаторами, например <a href="/docs/ru/lindera-tokenizer.md"><code translate="no">lindera</code></a> и <a href="/docs/ru/icu-tokenizer.md"><code translate="no">icu</code></a>.</p>
</div>
...
<ul>
<li><p><strong>Токенизатор</strong>: использует токенизатор « <code translate="no">standard</code> » для сохранения последовательностей букв и цифровых символов Unicode в токенах и разделения по другим символам. Точные правила обработки символов см. в разделе <a href="/docs/ru/standard-tokenizer.md#Tokenization-rules">«Стандартный токенизатор</a>».</p></li>
<li><p><strong>Фильтр</strong>: использует фильтр « <code translate="no">lowercase</code> » для преобразования всех токенов в нижний регистр, что позволяет выполнять поиск без учета регистра. Дополнительные сведения см. в разделе <a href="/docs/ru/lowercase-filter.md">«Lowercase</a>».</p></li>
</ul>
...
<h2 id="Configuration" class="common-anchor-header">Настройка<button data-href="#Configuration" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Чтобы применить анализатор « <code translate="no">standard</code> » к полю, просто установите для параметра ` <code translate="no">type</code> ` значение ` <code translate="no">standard</code> ` в файле ` <code translate="no">analyzer_params</code>` и добавьте дополнительные параметры по мере необходимости.</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#javascript">   NodeJS</a>
...
<table>
   ...
   <tr>
     ...
     <td><p>Массив, содержащий список стоп-слов, которые будут исключены при токенизации. По умолчанию используется <code translate="no">_english_</code> — встроенный набор распространённых английских стоп-слов.</p></td>
   </tr>
</table>
