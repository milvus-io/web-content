...
<h1 id="Standard-Analyzer" class="common-anchor-header">Стандартный анализатор<button data-href="#Standard-Analyzer" class="anchor-icon" translate="no">
    ...
    </button></h1><p>Анализатор « <code translate="no">standard</code> » является аналитиком по умолчанию в Milvus, который автоматически применяется к текстовым полям, если не указан другой анализатор. Он использует токенизацию на основе грамматики, что делает его эффективным для большинства языков.</p>
