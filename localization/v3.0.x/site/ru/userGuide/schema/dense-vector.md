...
summary: >-
  ...
  высокомерных пространствах. Плотные векторы обычно имеют фиксированное
...
<h1 id="Dense-Vector" class="common-anchor-header">Плотный вектор<button data-href="#Dense-Vector" class="anchor-icon" translate="no">
    ...
    </button></h1><p>Плотные векторы — это представления числовых данных, широко используемые в машинном обучении и анализе данных. Они состоят из массивов действительных чисел, в которых большинство или все элементы отличаются от нуля. По сравнению с разреженными векторами плотные векторы содержат больше информации при одинаковом количестве измерений, поскольку каждое измерение содержит значимые значения. Такое представление позволяет эффективно фиксировать сложные закономерности и взаимосвязи, что упрощает анализ и обработку данных в высокомерных пространствах. Плотные векторы, как правило, имеют фиксированное количество измерений — от нескольких десятков до нескольких сотен или даже тысяч — в зависимости от конкретного применения и требований.</p>
...
<li><p><strong>Семантические отношения:</strong> расстояния между точками отражают семантическое сходство между понятиями. Более близко расположенные точки указывают на понятия, которые более тесно связаны в семантическом плане.</p></li>
<li><p><strong>Эффект кластеризации:</strong> связанные концепции (такие как <strong>Milvus</strong>, <strong>векторная база данных</strong> и <strong>система поиска</strong>) располагаются в пространстве близко друг к другу, образуя семантический кластер.</p></li>
</ul>
...
<p>После векторизации данные можно хранить в Milvus для управления и поиска по векторам. На приведенной ниже схеме показан основной процесс.</p>
<p><span class="img-wrapper">
...
<table>
   ...
   <tr>
     ...
     <td><p>Хранит 32-битные числа с плавающей запятой, которые обычно используются для представления действительных чисел в научных вычислениях и машинном обучении. Идеально подходит для сценариев, требующих высокой точности, например, для различения похожих векторов.</p></td>
   </tr>
   <tr>
     <td><p><code translate="no">FLOAT16_VECTOR</code></p></td>
     <td><p>Хранит 16-битные числа с плавающей запятой половинной точности, используемые для глубокого обучения и вычислений на графическом процессоре (GPU). Это позволяет сэкономить место на диске в сценариях, где точность не является критичной, например, на этапе низкоточной рекоркусии в системах рекомендаций.</p></td>
   </tr>
   <tr>
   ...
   <tr>
     ...
     <td><p>Хранит векторы, отдельные элементы которых в каждом измерении представляют собой 8-битные целые числа (int8), причем диапазон значений каждого элемента составляет от –128 до 127. Разработанный для квантованных моделей глубокого обучения (например, ResNet, EfficientNet), INT8_VECTOR уменьшает размер модели и ускоряет инференс с минимальной потерей точности.<br><strong>Примечание</strong>: данный тип вектора поддерживается только для индексов HNSW.</p></td>
   </tr>
</table>
...
<p>Milvus предоставляет различные типы индексов для более эффективного векторного поиска. AUTOINDEX — это специальный тип индекса, разработанный для упрощения освоения векторного поиска. Вам доступно множество типов индексов на выбор. Подробности см. в разделе <a href="/docs/ru/index-explained.md">«Объяснение индексов</a>».</p>
