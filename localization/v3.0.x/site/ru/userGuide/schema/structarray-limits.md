...
<tr><td>Схема подполя</td><td>Все элементы Struct в одном и том же поле StructArray используют одну предопределенную схему Struct.</td></tr>
...
<h2 id="Supported-subfield-data-types" class="common-anchor-header">Поддерживаемые типы данных подполей<button data-href="#Supported-subfield-data-types" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Поле-подобра StructArray сопоставляются физическому хранению в виде массива. В следующей таблице перечислены поддерживаемые и неподдерживаемые физические типы.</p>
<table>
<thead>
...
<h2 id="Nullable-and-dynamic-schema-limits" class="common-anchor-header">Ограничения, связанные с допускаемыми значениями null и динамическими схемами<button data-href="#Nullable-and-dynamic-schema-limits" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<tbody>
<tr><td>Базовый поиск по EmbeddingList</td><td>Поддерживается для векторных подполей StructArray, индексированных с помощью метрик типа « <code translate="no">MAX_SIM*</code> ». Возвращает результаты на уровне объектов.</td></tr>
...
<tr><td>Поиск по диапазону</td><td>Поддерживается в зависимости от режима поиска и поддержки индексов/метрик целевой версии. Для получения информации о поведении гибридного поиска по диапазону в запросах StructArray на уровне элементов проверьте вашу целевую версию.</td></tr>
...
<h2 id="Filter-and-operator-limits" class="common-anchor-header">Ограничения фильтров и операторов<button data-href="#Filter-and-operator-limits" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Скалярная фильтрация StructArray осуществляется с помощью операторов StructArray, таких как « <code translate="no">element_filter</code> » и семейства « <code translate="no">MATCH_*</code> ». Подробная матрица поддержки предикатов приведена в разделе <a href="/docs/ru/struct-array-operators.md">«Операторы StructArray</a>».</p>
<p>В общих чертах:</p>
<ul>
