...
<p>Большинство ограничений StructArray обусловлены одним из трёх факторов: моделью схемы StructArray, выбранным режимом поиска для подполей вектора и версией Milvus, на которой работает ваша коллекция.</p>
...
<tr><td>Поля, допускающие значение null</td><td>Поля StructArray, допускающие значение null, зависят от версии. Если такая возможность поддерживается, значение null применяется ко всему полю StructArray, а не к отдельному элементу Struct независимо.</td></tr>
...
<tr><td>Все элементы используют одну схему.</td><td>Каждый элемент Struct в поле StructArray имеет тот же список подполей и те же типы данных подполей.</td></tr>
...
<tr><td>Вложенные StructArray не поддерживаются.</td><td>Поле StructArray не может содержать вложенных подполей типа <code translate="no">Array</code>, <code translate="no">ArrayOfVector</code>, <code translate="no">Struct</code> или <code translate="no">ArrayOfStruct</code>.</td></tr>
...
<h2 id="Supported-subfield-data-types" class="common-anchor-header">Поддерживаемые типы данных подполей<button data-href="#Supported-subfield-data-types" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Поподполя StructArray отображаются в памяти как физические массивы. В следующей таблице приведены поддерживаемые и неподдерживаемые физические типы.</p>
<table>
<thead>
...
<tr><td><code translate="no">ArrayOfVector</code></td><td>Поддерживается</td><td>Определите подполе как <code translate="no">DataType.FLOAT_VECTOR</code> и установите значение <code translate="no">dim</code>.</td></tr>
...
<h2 id="Nullable-and-dynamic-schema-limits" class="common-anchor-header">Ограничения, связанные с допускаемыми нулевыми значениями и динамическими схемами<button data-href="#Nullable-and-dynamic-schema-limits" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Поведение StructArray с допустимыми нулевыми значениями и динамическое добавление полей в StructArray зависят от версии.</p>
<table>
<thead>
<tr><th>Возможности</th><th>Ограничения</th></tr>
</thead>
<tbody>
<tr><td>Поле StructArray с возможностью принятия нулевого значения</td><td>Поддерживается только в версиях, в которых реализована поддержка StructArray с допустимыми нулевыми значениями и векторных массивов с допустимыми нулевыми значениями.</td></tr>
...
<tr><td>Частично нулевое значение StructArray</td><td>Если поле StructArray содержит допустимое значение массива, не смешивайте массивы подполей с нулевым значением с массивами подполей с допустимым значением в одном и том же значении.</td></tr>
...
<tr><td>Требование к допускаемости нулевых значений при динамическом добавлении</td><td>Поле StructArray, добавляемое в существующую коллекцию, должно допускать значение null, поскольку существующие сущности не имеют значения для нового поля.</td></tr>
...
<p>В Milvus v3.0.x доступны поля StructArray с возможностью присвоения значения null, векторные массивы с возможностью присвоения значения null и динамическое добавление полей StructArray.</p>
...
<p>Используйте отдельные векторные подполя, если требуются оба режима. Например, используйте <code translate="no">chunks[emb_list_vector]</code> для поиска по EmbeddingList и <code translate="no">chunks[emb]</code> для поиска на уровне элементов.</p>
...
<p>Поддерживаемые матрицы типа индекса и типа метрики см. в разделе <a href="/docs/ru/index-structarray-fields.md">«Поля StructArray индекса</a>».</p>
...
<tr><td>Поиск по диапазону</td><td>Поддерживается в зависимости от режима поиска и поддержки индексов/метрик целевой версии. Для получения информации о поведении гибридного поиска по диапазону при запросах StructArray на уровне элементов проверьте вашу целевую версию.</td></tr>
...
<li><p>Чтобы создать поле StructArray, ознакомьтесь со статьей <a href="/docs/ru/create-structarray-field.md">«Создание поля StructArray</a>».</p></li>
...
<li><p>Чтобы ознакомиться с синтаксисом фильтров StructArray, прочитайте раздел <a href="/docs/ru/struct-array-operators.md">«Операторы StructArray</a>».</p></li>
</ol>
