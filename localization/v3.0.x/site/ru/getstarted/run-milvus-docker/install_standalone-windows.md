...
summary: >-
  ...
  Windows.
title: Запуск Milvus в Docker (Windows)
---
...
<p>По умолчанию в этом развертывании в качестве очереди сообщений используется <strong>Woodpecker</strong> (локальная файловая система), поэтому внешний сервис очереди сообщений не требуется. См. раздел <a href="/docs/ru/woodpecker.md">«Woodpecker</a>».</p>
</blockquote>
<p>На этой странице показано, как запустить Milvus в Windows с помощью Docker Desktop для Windows.</p>
...
<h2 id="Run-Milvus-in-Docker​" class="common-anchor-header">Запуск Milvus в Docker​<button data-href="#Run-Milvus-in-Docker​" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Milvus предоставляет скрипт установки для развертывания в виде контейнера Docker. После установки Docker Desktop в Microsoft Windows вы можете получить доступ к интерфейсу командной строки Docker (CLI) из PowerShell или командной строки Windows в режиме <strong>администратора</strong>, а также из WSL 2. ​</p>
<h3 id="From-PowerShell-or-Windows-Command-Prompt​" class="common-anchor-header">Из PowerShell или командной строки Windows​<button data-href="#From-PowerShell-or-Windows-Command-Prompt​" class="anchor-icon" translate="no">
    ...
    </button></h3><p>Если вы лучше знакомы с PowerShell или командной строкой Windows, выполните следующие действия:​</p>
<ol>
...
<ul>
<li><p>Контейнер Docker с именем <strong>milvus-standalone</strong> запущен на порте <strong>19530</strong>.</p></li>
...
<li><p>Том данных Milvus сопоставлен с <strong>папкой volumes/milvus</strong> в текущей папке.​</p></li>
</ul>
...
<h3 id="From-WSL-2​" class="common-anchor-header">Из WSL 2​<button data-href="#From-WSL-2​" class="anchor-icon" translate="no">
    ...
    </button></h3><p>Если вы предпочитаете запускать Milvus с помощью команд Linux и скриптов оболочки в Windows, убедитесь, что у вас уже установлен WSL 2. Подробные сведения об установке WSL 2 см. в этой <a href="https://learn.microsoft.com/en-us/windows/wsl/install#install-wsl-command">статье Microsoft</a>.​</p>
<ol>
...
<li><p>Контейнер <strong>milvus-minio</strong> обслуживает порты <strong>9000</strong> и <strong>9001</strong> локально с использованием учетных данных по умолчанию и сопоставляет свои данные с <strong>каталогом volumes/minio</strong> в текущей папке.​</p></li>
...
<p>Если виртуализация отключена, возможно, вам потребуется проверить настройки BIOS прошивки материнской платы. Способ включения виртуализации в настройках BIOS зависит от производителя материнской платы. Например, для материнских плат ASUS вы можете ознакомиться с <a href="https://www.asus.com/support/faq/1043786/">этой статьей</a> по включению виртуализации.​</p>
...
<p>Для запуска демона Docker вам необходимо перейти в каталог установки Docker Desktop и выполнить команду « <code translate="no">.\DockerCli.exe -SwitchDaemon</code> ».</p>
...
<p>Чтобы устранить ошибку, возникающую при запуске Milvus с текстом «Read config failed», необходимо проверить, правильно ли смонтирован том в контейнере Milvus. Если том правильно смонтирован в контейнере, вы можете использовать команду <code translate="no">docker exec</code>, чтобы войти в контейнер и отобразить папку <strong>/milvus/configs</strong> следующим образом:​</p>
<p><span class="img-wrapper">
...
<p>Storage V3 по умолчанию отключен. Включите его перед использованием функций, которые от него зависят. Требования и сведения о совместимости см. в разделе <a href="/docs/ru/storage-v3.md">«Storage V3</a>».</p>
</div>
