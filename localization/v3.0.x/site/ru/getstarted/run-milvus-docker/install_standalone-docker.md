...
label: Docker
related_key: Docker
summary: 'Узнайте, как установить автономную версию Milvus с помощью Docker.'
...
<h1 id="Run-Milvus-in-Docker-Linux" class="common-anchor-header">Запуск Milvus в Docker (Linux)<button data-href="#Run-Milvus-in-Docker-Linux" class="anchor-icon" translate="no">
    ...
    </button></h1><p>На этой странице описано, как запустить экземпляр Milvus в Docker.</p>
...
<h2 id="Install-Milvus-in-Docker" class="common-anchor-header">Установка Milvus в Docker<button data-href="#Install-Milvus-in-Docker" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Milvus предоставляет скрипт установки для развертывания в виде контейнера Docker. Скрипт доступен в <a href="https://raw.githubusercontent.com/milvus-io/milvus/master/scripts/standalone_embed.sh">репозитории Milvus</a>. Чтобы установить Milvus в Docker, просто выполните команду</p>
...
<p><strong>Что нового в версии 3.0.2:</strong></p>
<ul>
<li><strong>Узел потоковой передачи</strong>: расширенные возможности обработки данных</li>
<li><strong>Woodpecker MQ (по умолчанию)</strong>: в этом развертывании Docker в качестве очереди сообщений используется Woodpecker, а в качестве бэкэнда WAL — <strong>локальная файловая система</strong>, поэтому внешний сервис очереди сообщений не требуется. См. раздел <a href="/docs/ru/woodpecker.md">«Woodpecker</a>».</li>
<li><strong>Оптимизированная архитектура</strong>: объединение компонентов для повышения производительности</li>
</ul>
<p>Всегда загружайте последнюю версию скрипта, чтобы получить самые свежие настройки и улучшения архитектуры.</p>
...
<ul>
<li>Запущен контейнер Docker с именем milvus-standalone на порту <strong>19530</strong>.</li>
<li>Вместе с Milvus в том же контейнере установлен встроенный etcd, работающий на порту <strong>2379</strong>. Его файл конфигурации сопоставлен файлу <strong>embedEtcd.yaml</strong> в текущей папке.</li>
...
<li>Том данных Milvus сопоставлен папке <strong>volumes/milvus</strong> в текущей папке.</li>
</ul>
<p>Вы можете перейти в веб-интерфейс Milvus по адресу <code translate="no">http://127.0.0.1:9091/webui/</code>, чтобы узнать больше о вашем экземпляре Milvus. Подробности см. в разделе <a href="/docs/ru/milvus-webui.md">«Веб-интерфейс Milvus</a>».</p>
<h2 id="Optional-Update-Milvus-configurations" class="common-anchor-header">(Необязательно) Обновление конфигураций Milvus<button data-href="#Optional-Update-Milvus-configurations" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Вы можете изменить настройки Milvus в файле <strong>user.yaml</strong>, расположенном в текущей папке. Например, чтобы изменить адрес <code translate="no">proxy.healthCheckTimeout</code> на <code translate="no">1000</code> ms, можно отредактировать файл следующим образом:</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>Информацию о соответствующих элементах конфигурации см. в разделе <a href="/docs/ru/system_configuration.md">«Системная конфигурация</a>».</p>
...
<button class="copy-code-btn"></button></code></pre>
<h2 id="Optional-dependencies" class="common-anchor-header">Необязательные зависимости<button data-href="#Optional-dependencies" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>По умолчанию в этом развертывании в качестве очереди сообщений используется <strong>Woodpecker</strong> (WAL в локальной файловой системе), а для метаданных — <strong>встроенный etcd</strong> — ничего больше устанавливать не требуется. Чтобы использовать другую очередь сообщений или подключить внешнее хранилище объектов / метаданных, см.:</p>
<ul>
