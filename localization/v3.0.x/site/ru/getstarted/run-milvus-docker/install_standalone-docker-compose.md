...
<li>Перед установкой<a href="/docs/ru/prerequisite-docker.md">проверьте системные требования к аппаратному и программному обеспечению</a>.</li>
</ul>
...
<p>Всегда загружайте последнюю конфигурацию Docker Compose, чтобы обеспечить совместимость с функциями версии v3.0-beta.</p>
<ul>
<li><p>Если у вас не удалось выполнить приведённую выше команду, проверьте, установлена ли в вашей системе версия Docker Compose V1. В этом случае рекомендуется перейти на Docker Compose V2 в соответствии с примечаниями на <a href="https://docs.docker.com/compose/">этой странице</a>.</p></li>
...
<li>Контейнер <strong>milvus-minio</strong> обслуживает локальные порты <strong>9000</strong> и <strong>9001</strong> с использованием учетных данных аутентификации по умолчанию и отображает свои данные в <strong>папку volumes/minio</strong> в текущем каталоге.</li>
...
<h2 id="Optional-Update-Milvus-configurations" class="common-anchor-header">(Необязательно) Обновление настроек Milvus<button data-href="#Optional-Update-Milvus-configurations" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Чтобы обновить конфигурацию Milvus в соответствии с вашими потребностями, необходимо изменить файл ` <code translate="no">/milvus/configs/user.yaml</code> ` внутри контейнера ` <code translate="no">milvus-standalone</code> `.</p>
<ol>
...
Ниже описано, как переопределить настройки по умолчанию в файле ` <code translate="no">proxy.healthCheckTimeout</code>`. Сведения о соответствующих элементах конфигурации см. в разделе <a href="/docs/ru/system_configuration.md">«Конфигурация системы</a>».</p>
...
<h2 id="Optional-dependencies" class="common-anchor-header">Дополнительные зависимости<button data-href="#Optional-dependencies" class="anchor-icon" translate="no">
    ...
    </button></h2><p>В данном развёртывании для обмена сообщениями используется <strong>Woodpecker</strong> (встроенный, бэкенд MinIO WAL), для метаданных — <strong>etcd</strong>, а для объектного хранилища — <strong>MinIO</strong>. Чтобы использовать другую очередь сообщений или подключить внешнее объектное хранилище/метаданные, см.:</p>
<ul>
