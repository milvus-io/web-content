...
<button class="copy-code-btn"></button></code></pre>
<p>Приведенная выше команда развертывает кластер Milvus с <strong>Woodpecker</strong> в качестве очереди сообщений (рекомендуется для v3.0-beta) и всеми новыми архитектурными компонентами, включая Streaming Node.</p>
