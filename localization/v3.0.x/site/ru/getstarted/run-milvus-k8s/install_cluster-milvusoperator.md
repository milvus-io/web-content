...
<button class="copy-code-btn"></button></code></pre>
<p>Приведенная выше команда развертывает кластер Milvus с использованием <strong>Woodpecker</strong> в качестве очереди сообщений (рекомендуется для версии v3.0-beta) и всех новых архитектурных компонентов, включая Streaming Node.</p>
<p><strong>Основные особенности архитектуры в данном развертывании:</strong></p>
<ul>
<li><strong>Очередь сообщений</strong>: <a href="/docs/ru/woodpecker.md">используется Woodpecker</a> (упрощает обслуживание инфраструктуры)</li>
<li><strong>Узел потоковой обработки (Streaming Node</strong>): включён для расширенной обработки данных</li>
...
<li>Вы также можете развернуть экземпляр Milvus в автономном режиме, когда все его компоненты находятся в одном поде. Для этого измените URL файла конфигурации в приведенной выше команде на <code translate="no">https://raw.githubusercontent.com/zilliztech/milvus-operator/main/config/samples/milvus_default.yaml</code></li>
</ul>
</div>
...
<button class="copy-code-btn"></button></code></pre>
<p>Как только ваш кластер Milvus будет готов, вывод вышеуказанной команды должен выглядеть примерно так. Если поле « <code translate="no">status.status</code> » (Состояние кластера) по-прежнему отображает « <code translate="no">Unhealthy</code> », ваш кластер Milvus все еще находится в процессе создания.</p>
...
<p>Как только кластер Milvus будет готов, статус всех подів в кластере Milvus должен выглядеть примерно так.</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>По желанию вы можете использовать <code translate="no">:19530</code> вместо <code translate="no">27017:19530</code> в приведённой выше команде, чтобы <code translate="no">kubectl</code> автоматически выделил вам локальный порт, избавив вас от необходимости решать проблемы с конфликтами портов.</p>
...
<h2 id="Optional-Update-Milvus-configurations" class="common-anchor-header">(Необязательно) Обновление конфигураций Milvus<button data-href="#Optional-Update-Milvus-configurations" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Вы можете просмотреть и обновить конфигурации своего кластера Milvus, выполнив команду <code translate="no">patch</code> следующим образом:</p>
<ol>
...
<div class="alert note">
<ul>
<li>При удалении кластера Milvus с использованием конфигурации по умолчанию такие зависимости, как etcd и MinIO, не удаляются. Поэтому при следующей установке того же экземпляра кластера Milvus эти зависимости будут использоваться вновь.</li>
...
<h2 id="Optional-dependencies" class="common-anchor-header">Дополнительные зависимости<button data-href="#Optional-dependencies" class="anchor-icon" translate="no">
    ...
    </button></h2><p>В данном развертывании в качестве очереди сообщений используется <strong>Woodpecker</strong>, для метаданных — <strong>etcd</strong>, а для объектного хранилища — <strong>MinIO</strong>; все эти компоненты предоставляются оператором Milvus. Чтобы использовать другой бэкэнд с оператором, см.:</p>
<ul>
