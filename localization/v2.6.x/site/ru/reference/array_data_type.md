...
title: Использование полей массива
---
<h1 id="Use-Array-Fields" class="common-anchor-header">Использование полей массива<button data-href="#Use-Array-Fields" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h1><p>В этом руководстве рассказывается об использовании полей массива, в том числе о вставке значений массива, создании индексов для полей вектора и массива, а также о поиске и запросе в полях массива с помощью базовых и расширенных операторов.</p>
<h2 id="Prerequisites" class="common-anchor-header">Необходимые условия<button data-href="#Prerequisites" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Убедитесь, что у вас есть все необходимое:</p>
<ul>
<li>Установлен и запущен Milvus. Информацию о том, как установить Milvus, см. в разделе <a href="/docs/ru/install-overview.md">Установка Milvus</a>.</li>
<li>Один из SDK Milvus установлен в вашей среде. Подробную информацию см. в разделе <a href="/docs/ru/install-pymilvus.md">Установка SDK</a>.</li>
</ul>
<h2 id="Prepare-data-with-an-array-field" class="common-anchor-header">Подготовка данных с помощью поля массива<button data-href="#Prepare-data-with-an-array-field" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>Milvus поддерживает массивы в качестве одного из типов данных полей. Массив в коллекции Milvus всегда должен содержать элементы с одинаковым типом данных, а тип данных для элементов массива может быть любым из поддерживаемых в Milvus типов данных. Список поддерживаемых типов данных см. в разделе <a href="https://milvus.io/docs/schema.md#Supported-data-types">Поддерживаемые типы данных</a>.</p>
<p>Следующий фрагмент кода генерирует случайный набор данных, содержащий поле массива с именем <code translate="no">color_coord</code>, все элементы которого имеют тип данных interger.</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>Этот фрагмент кода подготавливает список случайных цветов и генерирует набор данных, содержащий 1 000 сущностей. Каждая сущность имеет идентификатор, вектор из пяти чисел с плавающей точкой, цвет, цветовую метку и поле массива <code translate="no">color_coord</code>, содержащее от 3 до 5 целых значений. Образец данных распечатывается для проверки его структуры.</p>
<p>Структура выходных данных:</p>
...
<button class="copy-code-btn"></button></code></pre>
<h2 id="Set-up-MilvusClient" class="common-anchor-header">Настройка MilvusClient<button data-href="#Set-up-MilvusClient" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>Чтобы взаимодействовать с Milvus, настройте клиент Milvus, указав адрес сервера.</p>
<div class="multipleCode">
   <a href="#python">Python </a> <a href="#java">Java</a> <a href="#javascript">Node.js</a></div>
...
<button class="copy-code-btn"></button></code></pre>
<h2 id="Create-a-collection-with-an-array-field" class="common-anchor-header">Создание коллекции с полем-массивом<button data-href="#Create-a-collection-with-an-array-field" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><h3 id="Define-collection-schema" class="common-anchor-header">Определение схемы коллекции<button data-href="#Define-collection-schema" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Схема определяет структуру коллекции, включая поля и их типы данных. В примере ниже определена схема коллекции, соответствующая образцу данных, сгенерированному в <a href="#prepare-data-with-an-array-field">предыдущем разделе</a>.</p>
<p>Чтобы настроить поле массива в коллекции:</p>
<div class="language-python">
<ol>
<li>Задайте <code translate="no">datatype</code>: Настройте его как <code translate="no">DataType.ARRAY</code>.</li>
<li>Укажите <code translate="no">element_type</code>: Выберите тип данных для элементов массива. Элементы в поле массива должны иметь одинаковый тип данных. В этом примере для <code translate="no">element_type</code> установлено значение <code translate="no">DataType.INT64</code>.</li>
<li>Определить <code translate="no">max_capacity</code>: Задайте этот параметр, чтобы указать максимальное количество элементов, которое может содержать поле массива.</li>
</ol>
</div>
<div class="language-java">
<ol>
<li>Установить <code translate="no">dataType</code>: Настройте его как <code translate="no">DataType.Array</code>.</li>
<li>Укажите <code translate="no">elementType</code>: Выберите тип данных для элементов массива. Все элементы в поле массива должны иметь одинаковый тип данных. В этом примере для <code translate="no">elementType</code> установлено значение <code translate="no">DataType.Int64</code>.</li>
<li>Определить <code translate="no">maxCapacity</code>: Задайте этот параметр, чтобы указать максимальное количество элементов, которое может содержать поле массива.</li>
</ol>
</div>
<div class="language-javascript">
<ol>
<li>Установить <code translate="no">data_type</code>: Настройте его как <code translate="no">DataType.Array</code>.</li>
<li>Укажите <code translate="no">element_type</code>: Выберите тип данных для элементов массива. Все элементы в поле массива должны иметь одинаковый тип данных. В этом примере для <code translate="no">element_type</code> установлено значение <code translate="no">DataType.Int64</code>.</li>
<li>Определите <code translate="no">max_capacity</code>: Задайте этот параметр, чтобы указать максимальное количество элементов, которое может содержать поле массива.</li>
</ol>
</div>
<p>Приведенный ниже пример определяет схему коллекции с полем массива <code translate="no">color_coord</code>, в котором может быть не более 5 элементов и каждый элемент имеет тип данных integer.</p>
<div class="multipleCode">
   <a href="#python">Python </a> <a href="#java">Java</a> <a href="#javascript">Node.js</a></div>
...
<button class="copy-code-btn"></button></code></pre>
<div class="language-python">
<p>Дополнительные сведения о методах и параметрах см. в разделах <a href="https://milvus.io/api-reference/pymilvus/v2.4.x/MilvusClient/Collections/create_schema.md">create_schema</a> и <a href="https://milvus.io/api-reference/pymilvus/v2.4.x/MilvusClient/CollectionSchema/add_field.md">add_field</a>.</p>
</div>
<div class="language-java">
<p>Дополнительные сведения о методах и параметрах см. в разделах <a href="https://milvus.io/api-reference/java/v2.4.x/v2/Collections/createSchema.md">createSchema</a> и <a href="https://milvus.io/api-reference/java/v2.4.x/v2/CollectionSchema/addField.md">addField</a>.</p>
</div>
<div class="language-javascript">
<p>Дополнительные сведения о методах и параметрах см. в разделе <a href="https://milvus.io/api-reference/node/v2.4.x/Collections/createCollection.md">createCollection</a>.</p>
</div>
<h3 id="Create-the-collection" class="common-anchor-header">Создание коллекции<button data-href="#Create-the-collection" class="anchor-icon" translate="no">
    ...
    </button></h3><p>Затем создайте коллекцию, используя определенную схему.</p>
<div class="multipleCode">
   <a href="#python">Python </a> <a href="#java">Java</a> <a href="#javascript">Node.js</a></div>
...
<button class="copy-code-btn"></button></code></pre>
<div class="language-python">
<p>Дополнительные сведения о методах и параметрах см. в разделах <a href="https://milvus.io/api-reference/pymilvus/v2.4.x/MilvusClient/Collections/create_collection.md">create_collection</a> и <a href="https://milvus.io/api-reference/pymilvus/v2.4.x/MilvusClient/Collections/list_collections.md">list_collections</a>.</p>
</div>
<div class="language-java">
<p>Дополнительные сведения о методах и параметрах см. в разделе <a href="https://milvus.io/api-reference/java/v2.4.x/v2/Collections/createCollection.md">createCollection</a>.</p>
</div>
<div class="language-javascript">
<p>Дополнительные сведения о методах и параметрах см. в разделах <a href="https://milvus.io/api-reference/node/v2.4.x/Collections/createCollection.md">createCollection</a> и <a href="https://milvus.io/api-reference/node/v2.4.x/Collections/listCollections.md">listCollections</a>.</p>
</div>
<h2 id="Create-indexes" class="common-anchor-header">Создание индексов<button data-href="#Create-indexes" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>Индексы повышают производительность операций поиска и запросов. В Milvus можно создавать индексы как для векторных, так и для скалярных полей. В этом примере мы создадим индекс <code translate="no">IVF_FLAT</code> на векторном поле <code translate="no">vector</code> и индекс <code translate="no">INVERTED</code> на поле массива <code translate="no">color_coord</code>. Дополнительные сведения об индексах см. в разделах <a href="https://milvus.io/docs/index-vector-fields.md?tab=floating">Индексные векторные поля</a> и <a href="https://milvus.io/docs/index-scalar-fields.md">Индексные скалярные поля</a>.</p>
<h3 id="Index-vector-field" class="common-anchor-header">Индексное векторное поле<button data-href="#Index-vector-field" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Создание индекса на векторном поле может улучшить производительность поиска векторного подобия, что необходимо для каждой операции поиска.</p>
<p>В приведенном ниже примере создается индекс типа <code translate="no">IVF_FLAT</code> на векторном поле <code translate="no">vector</code>.</p>
<div class="multipleCode">
   <a href="#python">Python </a> <a href="#java">Java</a> <a href="#javascript">Node.js</a></div>
...
<button class="copy-code-btn"></button></code></pre>
<div class="language-python">
<p>Дополнительные сведения о методах и параметрах см. в разделах <a href="https://milvus.io/api-reference/pymilvus/v2.4.x/MilvusClient/Management/prepare_index_params.md">prepare_index_params</a>, <a href="https://milvus.io/api-reference/pymilvus/v2.4.x/MilvusClient/Management/create_index.md">create_index</a> и <a href="https://milvus.io/api-reference/pymilvus/v2.4.x/MilvusClient/Management/describe_index.md">describe_index</a>.</p>
</div>
<div class="language-java">
<p>Дополнительные сведения о методах и параметрах см. в разделах <a href="https://milvus.io/api-reference/java/v2.4.x/v2/Management/IndexParam.md">IndexParam</a> и <a href="https://milvus.io/api-reference/java/v2.4.x/v2/Management/createIndex.md">createIndex</a>.</p>
</div>
<div class="language-javascript">
<p>Дополнительные сведения о методах и параметрах см. в разделах <a href="https://milvus.io/api-reference/node/v2.4.x/Management/createIndex.md">createIndex</a> и <a href="https://milvus.io/api-reference/node/v2.4.x/Management/describeIndex.md">describeIndex</a>.</p>
</div>
<h3 id="Index-array-field" class="common-anchor-header">Индексное поле массива<button data-href="#Index-array-field" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Создание индекса на скалярном поле может улучшить производительность запросов на это поле, что необязательно, но рекомендуется для больших наборов данных.</p>
<p>В этом примере мы создадим инвертированный индекс на поле массива <code translate="no">color_coord</code>. Это позволит нам ускорить фильтрацию по этому полю. Инвертированный индекс демонстрирует отличную общую производительность, значительно превосходя фильтрацию грубой силой по необработанным данным, когда данные извлекаются нечасто, и сохраняя сопоставимую производительность при частых операциях извлечения. Более подробную информацию об инвертированных индексах можно найти в разделе <a href="/docs/ru/scalar_index.md#Inverted-indexing">"Скалярный индекс"</a>.</p>
<div class="multipleCode">
   <a href="#python">Python </a> <a href="#java">Java</a> <a href="#javascript">Node.js</a></div>
<pre><code translate="no" class="language-python">index_params = client.prepare_index_params()
...
<h2 id="Insert-data" class="common-anchor-header">Вставка данных<button data-href="#Insert-data" class="anchor-icon" translate="no">
    ...
    </button></h2><p>После создания коллекции и индексов мы можем вставить данные в коллекцию. Этот шаг вставляет 1 000 сущностей в коллекцию <code translate="no">test_collection</code>.</p>
<div class="multipleCode">
   <a href="#python">Python </a> <a href="#java">Java</a> <a href="#javascript">Node.js</a></div>
...
<button class="copy-code-btn"></button></code></pre>
<h2 id="Load-the-collection" class="common-anchor-header">Загрузка коллекции<button data-href="#Load-the-collection" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>После вставки данных нам нужно загрузить коллекцию, чтобы сделать ее доступной для операций поиска и запросов.</p>
<div class="multipleCode">
   <a href="#python">Python </a> <a href="#java">Java</a> <a href="#javascript">Node.js</a></div>
...
<button class="copy-code-btn"></button></code></pre>
<h2 id="Basic-scalar-filtering" class="common-anchor-header">Базовая скалярная фильтрация<button data-href="#Basic-scalar-filtering" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>После добавления всех данных вы можете выполнять поиск и запросы по элементам поля-массива так же, как и по стандартному скалярному полю.</p>
<div class="language-python">
<p>Дополнительные сведения о параметрах см. <a href="https://milvus.io/api-reference/pymilvus/v2.4.x/MilvusClient/Vector/search.md"><code translate="no">search()</code></a> в справочнике SDK.</p>
</div>
<div class="language-java">
<p>Дополнительные сведения о параметрах см. <a href="https://milvus.io/api-reference/java/v2.4.x/v2/Vector/search.md"><code translate="no">search()</code></a> в справке SDK.</p>
</div>
<div class="language-javascript">
<p>Дополнительные сведения о параметрах см. <a href="https://milvus.io/api-reference/node/v2.4.x/Vector/search.md"><code translate="no">search()</code></a> в справочнике SDK.</p>
</div>
<div class="multipleCode">
   <a href="#python">Python </a> <a href="#java">Java</a> <a href="#javascript">Node.js</a></div>
...
<span class="hljs-comment">// ]</span>
<button class="copy-code-btn"></button></code></pre>
<h2 id="Advanced-filtering" class="common-anchor-header">Расширенная фильтрация<button data-href="#Advanced-filtering" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>Как и в случае с полем JSON, Milvus также предоставляет операторы расширенной фильтрации для массивов, а именно <code translate="no">ARRAY_CONTAINS</code>, <code translate="no">ARRAY_CONTAINS_ALL</code>, <code translate="no">ARRAY_CONTAINS_ANY</code> и <code translate="no">ARRAY_LENGTH</code>. Более подробную информацию об операторах можно найти в разделе <a href="#reference-on-array-filters">Справочник по фильтрам массивов</a>.</p>
<ul>
<li><p>Фильтрует все сущности, в значениях <code translate="no">color_coord</code> которых присутствует <code translate="no">10</code>.</p>
...
<li><p>Фильтрует все сущности, у которых в значениях <code translate="no">color_coord</code> есть <code translate="no">7</code> и <code translate="no">8</code>.</p>
<p><div class="multipleCode">
<a href="#python">Python </a><a href="#java">Java</a><a href="#javascript">Node.js</a></div></p>
...
<span class="hljs-comment">// </span>
<button class="copy-code-btn"></button></code></pre></li>
<li><p>Фильтрует все сущности, в значениях <code translate="no">color_coord</code> которых есть 7, 8 или 9.</p>
<p><div class="multipleCode">
<a href="#python">Python </a><a href="#java">Java</a><a href="#javascript">Node.js</a></div></p>
<pre><code translate="no" class="language-python">res = client.query(
    collection_name=<span class="hljs-string">&quot;test_collection&quot;</span>,
...
<span class="hljs-comment">// </span>
<button class="copy-code-btn"></button></code></pre></li>
<li><p>Фильтрует сущности, имеющие ровно четыре элемента.</p>
<p><div class="multipleCode">
<a href="#python">Python </a><a href="#java">Java</a><a href="#javascript">Node.js</a></div></p>
<pre><code translate="no" class="language-python">res = client.query(
    collection_name=<span class="hljs-string">&quot;test_collection&quot;</span>,
...
<li><p>Элементы в поле ARRAY должны иметь одинаковый тип данных, указанный на <code translate="no">element_type</code>. В качестве <code translate="no">element_type</code> можно использовать любой допустимый тип данных, доступный для скалярных полей в Milvus. Список поддерживаемых типов данных см. в разделе <a href="https://milvus.io/docs/schema.md#Supported-data-types">Поддерживаемые типы данных</a>.</p></li>
<li><p>Количество элементов в поле ARRAY должно быть меньше или равно максимальной емкости поля массива, указанной на <code translate="no">max_capacity</code>.</p></li>
</ul>
<h2 id="Reference-on-array-filters" class="common-anchor-header">Справочник по фильтрам массивов<button data-href="#Reference-on-array-filters" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>При работе с полями массива строковое значение можно заключить в двойные кавычки ("") или одинарные кавычки (''). Важно отметить, что Milvus хранит строковые значения в поле массива как есть, не выполняя семантического экранирования или преобразования. Например, <strong>'a'b',</strong> <strong>'a'b',</strong> <strong>'a'b'</strong> и <strong>'a'b'</strong> будут сохранены как есть, а <strong>'a'b'</strong> и <strong>'a'b'</strong> будут рассматриваться как недопустимые значения.</p>
<p>Предположим, что определены два поля массива <code translate="no">int_array</code> и <code translate="no">var_array</code>. В следующей таблице описаны поддерживаемые булевы выражения, которые можно использовать в <code translate="no">expr</code> при поиске по полям массива.</p>
<table>
<thead>
<tr><th>Оператор</th><th>Примеры</th><th>Примечания</th></tr>
</thead>
<tbody>
<tr><td>&lt;</td><td><code translate="no">‘int_array[0] < 3’</code></td><td>Это выражение равно true, если значение <code translate="no">int_array[0]</code> меньше 3.</td></tr>
<tr><td>&gt;</td><td><code translate="no">‘int_array[0] > 5’</code></td><td>Это выражение считается истинным, если значение <code translate="no">int_array[0]</code> больше 5.</td></tr>
<tr><td>==</td><td><code translate="no">‘int_array[0] == 0’</code></td><td>Это выражение считается истинным, если значение <code translate="no">int_array[0]</code> равно 0.</td></tr>
<tr><td>!=</td><td><code translate="no">‘var_array[0] != "a"’</code></td><td>Это выражение считается истинным, если значение <code translate="no">var_array[0]</code> не равно <code translate="no">“a”</code>.</td></tr>
<tr><td>&lt;=</td><td><code translate="no">‘int_array[0] <= 3’</code></td><td>Это выражение считается истинным, если значение <code translate="no">int_array[0]</code> меньше или равно 3.</td></tr>
<tr><td>&gt;=</td><td><code translate="no">‘int_array[0] >= 10’</code></td><td>Это выражение считается истинным, если значение <code translate="no">int_array[0]</code> больше или равно 10.</td></tr>
<tr><td>в</td><td><code translate="no">'var_array[0] in ["str1", “str2”]'</code></td><td>Это выражение равно true, если значение <code translate="no">var_array[0]</code> равно <code translate="no">“str1”</code> или <code translate="no">“str2”</code>.</td></tr>
<tr><td>не в</td><td><code translate="no">'int_array[0] not in [1, 2, 3]'</code></td><td>Это выражение равно true, если значение <code translate="no">int_array[0]</code> не равно 1, 2 или 3.</td></tr>
<tr><td>+, -, *, /, %, **</td><td><code translate="no">‘int_array[0] + 100 > 200’</code></td><td>Это выражение равно true, если значение <code translate="no">int_array[0] + 100</code> больше 200.</td></tr>
<tr><td>как (LIKE)</td><td><code translate="no">‘var_array[0] like "prefix%"’</code></td><td>Это выражение считается истинным, если значение <code translate="no">var_array[0]</code> имеет префикс <code translate="no">“prefix”</code>.</td></tr>
<tr><td>и (&amp;&amp;)</td><td><code translate="no">‘var_array[0] like “prefix%” && int_array[0] <= 100’</code></td><td>Это выражение считается истинным, если значение <code translate="no">var_array[0]</code> имеет префикс <code translate="no">“prefix”</code>, а значение <code translate="no">int_array[0]</code> меньше или равно 100.</td></tr>
<tr><td>или (||)</td><td><code translate="no">‘var_array[0] like “prefix%” || int_array[0] <= 100’</code></td><td>Это выражение считается истинным, если значение <code translate="no">var_array[0]</code> имеет префикс <code translate="no">“prefix”</code>, или значение <code translate="no">int_array[0]</code> меньше или равно 100.</td></tr>
<tr><td>array_contains (ARRAY_CONTAINS)</td><td><code translate="no">'array_contains(int_array, 100)'</code></td><td>Это выражение принимает значение true, если <code translate="no">int_array</code> содержит элемент <code translate="no">100</code>.</td></tr>
<tr><td>array_contains_all (ARRAY_CONTAINS_ALL)</td><td><code translate="no">'array_contains_all(int_array, [1, 2, 3])'</code></td><td>Это выражение равно true, если <code translate="no">int_array</code> содержит все элементы <code translate="no">1</code>, <code translate="no">2</code> и <code translate="no">3</code>.</td></tr>
<tr><td>array_contains_any (ARRAY_CONTAINS_ANY)</td><td><code translate="no">'array_contains_any(var_array, ["a", "b", “c”])'</code></td><td>Это выражение равно true, если <code translate="no">var_array</code> содержит любой элемент из <code translate="no">“a”</code>, <code translate="no">“b”</code> и <code translate="no">“c”</code>.</td></tr>
<tr><td>длина_массива</td><td><code translate="no">‘array_length(int_array) == 10’</code></td><td>Это выражение принимает значение true, если <code translate="no">int_array</code> содержит ровно 10 элементов.</td></tr>
</tbody>
</table>
