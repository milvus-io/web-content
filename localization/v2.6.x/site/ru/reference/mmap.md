...
<h3 id="Before-cluster-deployment-global-configuration" class="common-anchor-header">Перед развертыванием кластера: глобальная конфигурация<button data-href="#Before-cluster-deployment-global-configuration" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Перед развертыванием кластера настройки на <strong>уровне кластера</strong> применяют отображение памяти для всего кластера. Благодаря этому все новые объекты будут автоматически соответствовать этим конфигурациям. Важно отметить, что изменение этих настроек требует перезапуска кластера, чтобы они вступили в силу.</p>
...
<h3 id="During-cluster-operation-dynamic-configuration" class="common-anchor-header">Во время работы кластера: динамическая конфигурация<button data-href="#During-cluster-operation-dynamic-configuration" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Во время работы кластера можно динамически изменять параметры отображения памяти на уровне коллекции или индекса.</p>
...
<p>Сопоставление памяти рекомендуется при ограниченном объеме памяти или при умеренных требованиях к производительности. Включение этой функции увеличивает пропускную способность для загрузки данных. Например, при конфигурации из 2 процессоров и 8 ГБ памяти включение функции отображения памяти позволяет загружать в 4 раза больше данных по сравнению с отсутствием этой функции. Влияние на производительность различно:</p>
<ul>
...
<p>Да, для включения mmap рекомендуется HNSW. Мы уже тестировали индексы серий HNSW, IVF_FLAT, IVF_PQ/SQ, производительность индексов серии IVF сильно упала, в то время как падение производительности при включении mmap для индексов HNSW все еще находится в пределах ожиданий.</p></li>
