...
<p>Следующая команда развертывает кластер Milvus с оптимизированными настройками для версии 2.6.11, используя Woodpecker в качестве рекомендуемой очереди сообщений:</p>
<pre><code translate="no" class="language-bash">helm install my-release zilliztech/milvus \
  --<span class="hljs-built_in">set</span> image.all.tag=v2.6.11 \
  --<span class="hljs-built_in">set</span> pulsarv3.enabled=<span class="hljs-literal">false</span> \
  --<span class="hljs-built_in">set</span> woodpecker.enabled=<span class="hljs-literal">true</span> \
...
<button class="copy-code-btn"></button></code></pre>
<p><strong>Дождитесь, пока все поды покажут статус "Running".</strong> В конфигурации v2.6.11 вы должны увидеть поды, похожие на эти:</p>
