...
<h2 id="Online-install" class="common-anchor-header">Онлайн-установка<button data-href="#Online-install" class="anchor-icon" translate="no">
    ...
    </button></h3><p>После установки диаграммы Helm вы можете запустить Milvus на Kubernetes. В этом разделе мы рассмотрим шаги по запуску Milvus.</p>
...
<h3 id="2-Check-Milvus-cluster-status" class="common-anchor-header">2. Проверка состояния кластера Milvus<button data-href="#2-Check-Milvus-cluster-status" class="anchor-icon" translate="no">
    ...
    </button></h3><p>Выполните следующую команду, чтобы проверить состояние всех стручков в вашем кластере Milvus.</p>
<pre><code translate="no">$ kubectl <span class="hljs-keyword">get</span> pods
<button class="copy-code-btn"></button></code></pre>
<p>Если все поды запущены, результат выполнения команды должен быть похож на следующий:</p>
...
<button class="copy-code-btn"></button></code></pre>
<h3 id="3-Forward-a-local-port-to-Milvus" class="common-anchor-header">3. Перенаправление локального порта в Milvus<button data-href="#3-Forward-a-local-port-to-Milvus" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
