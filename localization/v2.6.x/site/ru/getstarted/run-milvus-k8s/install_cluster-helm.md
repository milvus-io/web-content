...
<h2 id="Overview" class="common-anchor-header">Обзор<button data-href="#Overview" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Helm использует формат упаковки, называемый «чартами». Чарт представляет собой набор файлов, описывающих связанный набор ресурсов Kubernetes. Milvus предоставляет набор чартов, помогающих развернуть зависимости и компоненты Milvus.</p>
...
<h2 id="Install-Milvus-Helm-Chart" class="common-anchor-header">Установка Helm-чарта Milvus<button data-href="#Install-Milvus-Helm-Chart" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<li>Отключает устаревший <strong>узел индексации</strong> (эту функцию теперь выполняет узел данных)</li>
...
Вышеуказанная команда развертывает Milvus с рекомендуемыми настройками. Для использования в производственной среде:</p>
<ul>
...
<li>Ознакомьтесь с <a href="https://milvus.io/docs/system_configuration.md">контрольным списком настроек системы Milvus</a> для продвинутых параметров конфигурации</li>
</ul>
<div class="alert note">
...
<p>Дополнительную информацию см. в <a href="https://helm.sh/docs/">документации</a> по <a href="https://artifacthub.io/packages/helm/milvus/milvus">Milvus Helm Chart</a> и <a href="https://helm.sh/docs/">Helm</a>.</p>
</div>
...
<p>Вы также можете получить доступ к <strong>веб-интерфейсу Milvus</strong> по адресу <code translate="no">http://127.0.0.1:9091/webui/</code> после настройки переадресации портов (см. следующий шаг). Подробности см. в разделе <a href="/docs/ru/v2.6.x/milvus-webui.md">«Веб-интерфейс Milvus</a>».</p>
...
<h2 id="Offline-install" class="common-anchor-header">Установка в автономном режиме<button data-href="#Offline-install" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Если вы находитесь в среде с ограниченным доступом к сети, выполните процедуру, описанную в этом разделе, чтобы запустить кластер Milvus.</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>Вышеуказанная команда сгенерирует шаблоны диаграмм для кластера Milvus и сохранит результат в файл манифеста с именем <code translate="no">milvus_manifest.yaml</code>. Используя этот манифест, вы сможете установить кластер Milvus, при этом его компоненты и зависимости будут размещены в отдельных подах.</p>
<div class="alert note">
<ul>
...
<h3 id="2-Download-image-pulling-script" class="common-anchor-header">2. Загрузите скрипт для извлечения образа<button data-href="#2-Download-image-pulling-script" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Скрипт для загрузки образов написан на Python. Вам следует загрузить этот скрипт вместе с его зависимостями из файла <code translate="no">requirement.txt</code>.</p>
