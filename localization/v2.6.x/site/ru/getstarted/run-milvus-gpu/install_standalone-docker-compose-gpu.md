...
<li>Перед установкой<a href="/docs/ru/v2.6.x/prerequisite-gpu.md">проверьте требования к аппаратному и программному обеспечению</a>.</li>
</ul>
...
<h3 id="1-Download-and-configure-the-YAML-file" class="common-anchor-header">1. Загрузите и настройте файл YAML<button data-href="#1-Download-and-configure-the-YAML-file" class="anchor-icon" translate="no">
    ...
    </button></h3><p>Скачайте <a href="https://github.com/milvus-io/milvus/releases/download/v2.6.24/milvus-standalone-docker-compose-gpu.yml"><code translate="no">milvus-standalone-docker-compose-gpu.yml</code></a> и сохраните его как docker-compose.yml вручную или с помощью следующей команды.</p>
...
<ul>
<li>контейнеры с именами <strong>milvus-standalone</strong>, <strong>milvus-minio</strong> и <strong>milvus-etcd</strong> запущены.
<ul>
...
<li>Контейнер ` <strong>milvus-minio</strong> ` обслуживает локальные порты <strong>9090</strong> и <strong>9091</strong> с использованием учетных данных по умолчанию и сопоставляет свои данные с <strong>папкой `volumes/minio`</strong> в текущей директории.</li>
...
<p>Чтобы настроить пул памяти, измените настройки <code translate="no">initMemSize</code> и <code translate="no">maxMemSize</code> в файле <code translate="no">milvus.yaml</code> следующим образом.</p>
<ol>
<li><p>Используйте следующую команду, чтобы скопировать файл ` <code translate="no">milvus.yaml</code> ` из контейнера Milvus на ваш локальный компьютер. Замените ` <code translate="no">&lt;milvus_container_id&gt;</code> ` на фактический идентификатор вашего контейнера Milvus.</p>
...
<li><p>Ознакомьтесь с <a href="/docs/ru/v2.6.x/milvus-webui.md">веб-интерфейсом Milvus</a>, чтобы узнать больше об экземпляре Milvus.</p></li>
