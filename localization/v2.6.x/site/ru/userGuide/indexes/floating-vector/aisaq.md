...
<p>Режим производительности: оптимизирован для приложений, которым требуется низкая задержка и высокая пропускная способность в масштабе, например для онлайнового семантического поиска.</p>
...
<table>
   ...
   <tr>
     ...
     <td><p>Количество векторов PQ, хранящихся в строке для каждого узла индекса (считывается при обращении к узлу, чтобы сократить количество операций ввода-вывода)</p></td>
...
<table>
   ...
   <tr>
     ...
     <td><p>Большие значения увеличивают параллелизм, что может ускорить поиск на системах с мощными процессорами и SSD. Однако слишком высокое значение может привести к чрезмерному потреблению ресурсов, поскольку каждая соседняя группа векторов PQ может содержать до max_degree векторов.</p><p>В большинстве случаев мы рекомендуем устанавливать значение 1.</p></td>
   </tr>
   <tr>
