...
summary: >-
  В этом руководстве рассказывается об операциях манипулирования данными в
  коллекции, включая вставку, добавление и удаление.
title: 'Вставка, вставка и удаление'
---
<h1 id="Insert-Upsert--Delete" class="common-anchor-header">Вставка, вставка и удаление<button data-href="#Insert-Upsert--Delete" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h1><p>В этом руководстве рассказывается об операциях по работе с данными в коллекции, включая вставку, вставку и удаление.</p>
...
<h2 id="Preparations" class="common-anchor-header">Подготовка<button data-href="#Preparations" class="anchor-icon" translate="no">
    ...
    </button></h2><p>В приведенном ниже фрагменте кода используется существующий код для установки соединения с кластером Milvus и быстрой настройки коллекции.</p>
<div class="language-python">
<p>Для подготовки используйте <a href="https://milvus.io/api-reference/pymilvus/v2.4.x/MilvusClient/Client/MilvusClient.md"><code translate="no">MilvusClient</code></a> для подключения к серверу Milvus и <a href="https://milvus.io/api-reference/pymilvus/v2.4.x/MilvusClient/Collections/create_collection.md"><code translate="no">create_collection()</code></a> для создания коллекции в режиме быстрой настройки.</p>
</div>
...
<p><b>заметки</b></p>
<p>Коллекция, созданная в приведенном выше коде, содержит только два поля: <code translate="no">id</code> (в качестве первичного ключа) и <code translate="no">vector</code> (в качестве векторного поля), при этом параметры <code translate="no">auto_id</code> и <code translate="no">enable_dynamic_field</code> включены по умолчанию. При вставке данных,</p>
<ul>
<li><p>Не нужно включать <strong>id</strong> в вставляемые данные, поскольку первичное поле автоматически увеличивается по мере вставки данных.</p></li>
<li><p>Поля, не определяемые схемой, будут сохраняться в виде пар ключ-значение в зарезервированном JSON-поле с именем <strong>$meta</strong>.</p></li>
</ul>
</div>
<h2 id="Insert-entities" class="common-anchor-header">Вставка сущностей<button data-href="#Insert-entities" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>Чтобы вставить сущности, вам нужно организовать данные в список словарей, где каждый словарь представляет сущность. Каждый словарь содержит ключи, соответствующие как предопределенным, так и динамическим полям целевой коллекции.</p>
<div class="language-python">
<p>Чтобы вставить сущности в коллекцию, используйте метод <a href="https://milvus.io/api-reference/pymilvus/v2.4.x/MilvusClient/Vector/insert.md"><code translate="no">insert()</code></a> метод.</p>
</div>
<div class="language-java">
<p>Чтобы вставить сущности в коллекцию, используйте метод <a href="https://milvus.io/api-reference/java/v2.4.x/v2/Vector/insert.md"><code translate="no">insert()</code></a> метод.</p>
</div>
<div class="language-javascript">
<p>Чтобы вставить сущности в коллекцию, используйте <a href="https://milvus.io/api-reference/node/v2.4.x/Vector/insert.md"><code translate="no">insert()</code></a> метод.</p>
</div>
<div class="multipleCode">
   <a href="#python">Python </a> <a href="#java">Java</a> <a href="#javascript">Node.js</a></div>
...
<span class="hljs-comment">// </span>
<button class="copy-code-btn"></button></code></pre>
<h3 id="Insert-into-partitions" class="common-anchor-header">Вставка в разделы<button data-href="#Insert-into-partitions" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Чтобы вставить данные в определенный раздел, вы можете указать имя раздела в запросе на вставку следующим образом:</p>
<div class="multipleCode">
   <a href="#python">Python </a> <a href="#java">Java</a> <a href="#javascript">Node.js</a></div>
...
<h2 id="Upsert-entities" class="common-anchor-header">Вставка сущностей<button data-href="#Upsert-entities" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Операция апсерт данных - это комбинация операций обновления и вставки. В Milvus операция upsert выполняет действие на уровне данных для вставки или обновления сущности на основе того, существует ли ее первичный ключ в коллекции. А именно:</p>
<ul>
<li><p>Если первичный ключ сущности уже существует в коллекции, существующая сущность будет перезаписана.</p></li>
<li><p>Если первичный ключ не существует в коллекции, будет вставлена новая сущность.</p></li>
</ul>
<div class="alert note">
<ul>
<li>Операции Upsert не обновляют первичные ключи.</li>
<li>Если вы планируете использовать операцию <code translate="no">upsert</code> вместо <code translate="no">insert</code> для получения больших объемов данных (например, миллионов векторов), имейте в виду, что это может привести к большим затратам памяти на узлах данных Milvus.</li>
</ul>
</div>
<div class="language-python">
...
<div class="language-javascript">
<p>Чтобы вставить сущности, используйте <a href="https://milvus.io/api-reference/node/v2.4.x/Vector/upsert.md"><code translate="no">upsert()</code></a> метод.</p>
</div>
<div class="multipleCode">
   <a href="#python">Python </a> <a href="#java">Java</a> <a href="#javascript">Node.js</a></div>
...
<span class="hljs-comment">// </span>
<button class="copy-code-btn"></button></code></pre>
<h3 id="Upsert-data-in-partitions" class="common-anchor-header">Апсертирование данных в разделы<button data-href="#Upsert-data-in-partitions" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<p>Если сущность больше не нужна, вы можете удалить ее из коллекции с помощью команды <a href="https://milvus.io/api-reference/pymilvus/v2.4.x/MilvusClient/Vector/delete.md"><code translate="no">delete()</code></a>.</p>
</div>
<div class="language-java">
<p>Если сущность больше не нужна, вы можете удалить ее из коллекции с помощью команды <a href="https://milvus.io/api-reference/java/v2.4.x/v2/Vector/delete.md"><code translate="no">delete()</code></a>.</p>
</div>
<div class="language-javascript">
<p>Если сущность больше не нужна, вы можете удалить ее из коллекции, используя <a href="https://milvus.io/api-reference/node/v2.4.x/Vector/delete.md"><code translate="no">delete()</code></a>.</p>
</div>
<p>Milvus предлагает два способа определения сущностей для удаления.</p>
<ul>
<li><p><strong>Удаление сущностей по фильтру.</strong></p>
   <div class='alert note'>
<p>При использовании выражений фильтрации для удаления сущностей убедитесь, что коллекция загружена. В противном случае Milvus вернет ошибку.</p>
   </div>
...
<li><p><strong>Удаление сущностей по идентификаторам.</strong></p>
<p>В следующих фрагментах показано, как удалять сущности по идентификаторам из определенного раздела. Это также работает, если вы оставите имя раздела неуказанным.</p>
<p><div class="multipleCode">
<a href="#python">Python </a><a href="#java">Java</a><a href="#javascript">Node.js</a></div></p>
