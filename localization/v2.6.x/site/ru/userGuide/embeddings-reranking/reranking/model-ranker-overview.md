...
<h1 id="Model-Ranker-Overview" class="common-anchor-header">Обзор Model Ranker<span class="beta-tag" style="background-color:rgb(0, 179, 255);color:white" translate="no">Compatible with Milvus 2.6.x</span><button data-href="#Model-Ranker-Overview" class="anchor-icon" translate="no">
    ...
    </button></h1><p>Традиционный векторный поиск ранжирует результаты исключительно по математическому сходству — то есть по степени совпадения векторов в многомерном пространстве. Несмотря на свою эффективность, этот подход зачастую упускает истинную семантическую релевантность. Рассмотрим поиск по <strong>запросу «лучшие практики оптимизации баз данных»:</strong> вы можете получить документы с высоким векторным сходством, в которых эти термины упоминаются часто, но которые на самом деле не содержат практических стратегий оптимизации.</p>
<p>Model Ranker преобразует поиск Milvus за счет интеграции передовых языковых моделей, которые понимают семантические связи между запросами и документами. Вместо того чтобы полагаться исключительно на векторное сходство, он оценивает смысл и контекст контента, чтобы предоставлять более интеллектуальные и релевантные результаты.</p>
...
<h2 id="Choose-a-model-provider-for-your-needs" class="common-anchor-header">Выберите поставщика моделей, соответствующего вашим потребностям<button data-href="#Choose-a-model-provider-for-your-needs" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<table>
   ...
   <tr>
     ...
     <td><p>Система управления контентом, требующая эффективных возможностей переранжирования при стандартных требованиях</p></td>
   </tr>
   <tr>
     <td><p>Cohere</p></td>
     <td><p>Корпоративные приложения, в которых приоритет отдается надежности и простоте интеграции</p></td>
     <td><ul><li><p>Надежность и масштабируемость корпоративного уровня</p></li><li><p>Управляемый сервис без необходимости обслуживания инфраструктуры</p></li><li><p>Возможности переранжирования на нескольких языках</p></li><li><p>Встроенные функции ограничения скорости и обработки ошибок</p></li></ul></td>
   ...
   <tr>
     ...
     <td><p>Система поиска по технической документации, обрабатывающая объемные руководства и статьи, требующие интеллектуальной сегментации и управления перекрытием</p></td>
   </tr>
</table>
<p>Подробную информацию о внедрении каждого сервиса модели см. в соответствующей документации:</p>
<ul>
...
<h2 id="Implementation" class="common-anchor-header">Внедрение<button data-href="#Implementation" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<li><p>Работающий внешний сервис моделей, доступный для вашего экземпляра Milvus</p></li>
...
<table>
   ...
   <tr>
     ...
     <td><p>Да</p></td>
     <td><p>Словарь, содержащий настройки функции переранжирования на основе модели. Доступные параметры (ключи) зависят от поставщика услуг.</p></td>
   ...
   <tr>
     ...
     <td><p>Да</p></td>
     <td><p>Список строк запроса, используемых моделью переранжирования для расчета оценок релевантности.</p><p>Количество строк запроса должно точно соответствовать количеству запросов в вашей операции поиска (даже при использовании векторов запросов вместо текста), в противном случае будет выдана ошибка.</p></td>
...
<h3 id="Apply-to-standard-vector-search" class="common-anchor-header">Применить к стандартному векторному поиску<button data-href="#Apply-to-standard-vector-search" class="anchor-icon" translate="no">
    ...
    </button></h3><p>После определения рангера на основе модели вы можете применять его во время операций поиска, передавая его в параметр ranker:</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#javascript">   NodeJS</a>
