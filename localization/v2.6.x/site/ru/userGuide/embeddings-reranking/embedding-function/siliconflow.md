...
<h1 id="SiliconFLow" class="common-anchor-header">SiliconFLow<span class="beta-tag" style="background-color:rgb(0, 179, 255);color:white" translate="no">Compatible with Milvus 2.6.x</span><button data-href="#SiliconFLow" class="anchor-icon" translate="no">
    ...
    </button></h1><p>В этой теме описывается, как настраивать и использовать функции встраивания SiliconFLow в Milvus.</p>
<h2 id="Choose-an-embedding-model" class="common-anchor-header">Выбор модели встраивания<button data-href="#Choose-an-embedding-model" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Milvus поддерживает модели встраивания, предоставляемые SiliconFLow. Ниже приведены доступные в настоящее время модели встраивания SiliconFLow для быстрого ознакомления:</p>
<table>
   <tr>
   ...
   <tr>
     ...
     <td><p>1,024</p></td>
     <td><p>512</p></td>
     <td><p>Модель встраивания большого китайского текста, входящая в серию BGE (BAAI General Embedding).</p></td>
   </tr>
   <tr>
     <td><p>BAAI/bge-large-en-v1.5</p></td>
     <td><p>1,024</p></td>
     <td><p>512</p></td>
     <td><p>Большая модель встраивания английского текста, входящая в серию BGE (BAAI General Embedding).</p></td>
   </tr>
   <tr>
     <td><p>netease-youdao/bce-embedding-base_v1</p></td>
     <td><p>768</p></td>
     <td><p>512</p></td>
     <td><p>Двуязычная и межъязыковая модель встраивания, разработанная NetEase Youdao. Модель демонстрирует отличную производительность в задачах семантического представления и поиска на китайском и английском языках, особенно хорошо проявляя себя в кросс-языковых сценариях.</p></td>
   </tr>
   <tr>
     <td><p>BAAI/bge-m3</p></td>
     <td><p>1,024</p></td>
     <td><p>8,192</p></td>
     <td><p>Многофункциональная, многоязычная и многогранная модель встраивания текста. Поддерживает три общие функции поиска: плотный поиск, многовекторный поиск и разреженный поиск.</p></td>
   </tr>
   <tr>
     <td><p>Pro/BAAI/bge-m3</p></td>
     <td><p>1,024</p></td>
     <td><p>8,192</p></td>
     <td><p>Многофункциональная, многоязычная, многогранная модель встраивания текста. Поддерживает три общие функции поиска: плотный поиск, многовекторный поиск и разреженный поиск. Модель может обрабатывать вводимые данные на более чем 100 языках и способна работать с различными гранулярностями.</p></td>
   </tr>
</table>
<h2 id="Configure-credentials" class="common-anchor-header">Настройка учетных данных<button data-href="#Configure-credentials" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Milvus должен знать ваш ключ API SiliconFlow, прежде чем он сможет запрашивать вкрапления. Milvus предоставляет два способа настройки учетных данных:</p>
<ul>
...
<h3 id="Option-1-Configuration-file" class="common-anchor-header">Вариант 1: Конфигурационный файл</h3><p>Храните ваши API-ключи в <code translate="no">milvus.yaml</code>; Milvus считывает их при запуске и переопределяет любую переменную окружения для того же провайдера.</p>
<ol>
...
<p>В том же файле укажите провайдеру SiliconFlow на метку, которую вы хотите использовать.</p>
...
<table>
   ...
   <tr>
     ...
     <td><p>Да</p></td>
     <td><p>Ваш действительный ключ API SiliconFlow.</p></td>
   </tr>
</table>
<p>В файле <strong>docker-compose.yaml</strong> установите переменную окружения <code translate="no">MILVUSAI_SILICONFLOW_API_KEY</code>.</p>
...
<p>В примере ниже добавлен модуль Function (<code translate="no">siliconflow_embedding</code>), который преобразует скалярное поле <code translate="no">&quot;document&quot;</code> в эмбеддинги, сохраняя полученные векторы в векторном поле <code translate="no">&quot;dense&quot;</code>, определенном ранее.</p>
