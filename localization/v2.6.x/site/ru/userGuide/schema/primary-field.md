...
<p>Первичное поле в коллекции не имеет значения по умолчанию и не может быть нулевым.</p>
<div class="alert note">
<ul>
<li>Стандартная операция <code translate="no">insert</code> с первичным ключом, который уже существует в коллекции, не перезапишет старую запись. Вместо этого будет создана новая, отдельная сущность с тем же первичным ключом. Это может привести к неожиданным результатам поиска и избыточности данных.</li>
<li>Если в вашем случае требуется обновить существующие данные или вы подозреваете, что данные, которые вы вставляете, уже существуют, настоятельно рекомендуется использовать операцию upsert. Операция upsert интеллектуально обновит сущность, если первичный ключ существует, или вставит новый, если его нет. Для получения более подробной информации см. раздел <a href="/docs/ru/upsert-entities.md">Upsert Entities</a>.</li>
</ul>
</div>
