...
<h2 id="How-it-works" class="common-anchor-header">Как это работает<button data-href="#How-it-works" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Когда динамическое поле включено, Milvus добавляет скрытое поле <code translate="no">$meta</code> к каждой сущности. При вставке данных любое поле, не объявленное в схеме, автоматически сохраняется в виде пары ключ-значение в этом динамическом поле.</p>
...
<h3 id="Index-non-JSON-keys-in-the-dynamic-field" class="common-anchor-header">Индексирование неJSON-ключей в динамическом поле</h3><p>Вы можете индексировать любой не-JSON-ключ (например, <code translate="no">overview</code>, <code translate="no">words</code>), ссылаясь на его имя непосредственно в <code translate="no">json_path</code>:</p>
<div class="multipleCode">
   <a href="#python">Python</a> <a href="#java">Java</a> <a href="#javascript">NodeJS</a> <a href="#go">Go</a> <a href="#bash">cURL</a></div>
...
<pre><code translate="no" class="language-bash"><span class="hljs-comment"># restful</span>
<button class="copy-code-btn"></button></code></pre>
<h3 id="Index-JSON-keys-in-the-dynamic-field" class="common-anchor-header">Индексирование JSON-ключей в динамическом поле</h3><p>Когда ключ в динамическом поле (<code translate="no">$meta</code>) хранит объект JSON, вы можете индексировать его внутренние ключи, используя скобочную нотацию в <code translate="no">json_path</code>.</p>
<div class="multipleCode">
   <a href="#python">Python</a> <a href="#java">Java</a> <a href="#javascript">NodeJS</a> <a href="#go">Go</a> <a href="#bash">cURL</a></div>
...
<h2 id="Put-it-all-together" class="common-anchor-header">Соберите все вместе<button data-href="#Put-it-all-together" class="anchor-icon" translate="no">
    ...
    </button></h2><p>К этому моменту вы узнали, как использовать динамическое поле для гибкого хранения и индексации ключей, не определенных в схеме. После вставки ключа динамического поля вы можете использовать его так же, как и любое другое поле в выражениях фильтрации - специальный синтаксис не требуется.</p>
