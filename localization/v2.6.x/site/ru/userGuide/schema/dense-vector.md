...
<h1 id="Dense-Vector" class="common-anchor-header">Плотный вектор<button data-href="#Dense-Vector" class="anchor-icon" translate="no">
    ...
    </button></h1><p>Плотные векторы — это представления числовых данных, широко используемые в машинном обучении и анализе данных. Они состоят из массивов с вещественными числами, в которых большинство или все элементы отличаются от нуля. По сравнению с разреженными векторами плотные векторы содержат больше информации при одинаковом количестве измерений, поскольку каждое измерение содержит значимые значения. Такое представление позволяет эффективно отражать сложные закономерности и взаимосвязи, что упрощает анализ и обработку данных в высокоразмерных пространствах. Плотные векторы, как правило, имеют фиксированное количество измерений — от нескольких десятков до нескольких сотен или даже тысяч, в зависимости от конкретного применения и требований.</p>
...
<li><p><strong>Семантические отношения:</strong> расстояния между точками отражают семантическое сходство между понятиями. Более близкие точки указывают на понятия, которые более тесно связаны семантически.</p></li>
...
<table>
   ...
   <tr>
     ...
     <td><p>Хранит векторы, отдельные элементы которых в каждом измерении представляют собой 8-битные целые числа (int8), причем диапазон значений каждого элемента составляет от –128 до 127. Разработанный для квантованных моделей глубокого обучения (например, ResNet, EfficientNet), тип INT8_VECTOR уменьшает размер модели и ускоряет инференс с минимальной потерей точности.<br><strong>Примечание</strong>: этот тип вектора поддерживается только для индексов HNSW.</p></td>
   </tr>
</table>
...
<button class="copy-code-btn"></button></code></pre>
<p>В приведенном выше примере для поля <code translate="no">dense_vector</code> создается индекс с именем <code translate="no">dense_vector_index</code> с использованием типа индекса <code translate="no">AUTOINDEX</code>. Параметр <code translate="no">metric_type</code> устанавливается в значение <code translate="no">IP</code>, что указывает на использование внутреннего произведения в качестве метрики расстояния.</p>
<p>Milvus предоставляет различные типы индексов для более эффективного векторного поиска. AUTOINDEX — это специальный тип индекса, разработанный для упрощения освоения векторного поиска. Вам доступно множество типов индексов на выбор. Подробности см. в xxx.</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>Дополнительную информацию о параметрах поиска по сходству см. в разделе <a href="/docs/ru/v2.6.x/single-vector-search.md">«Базовый поиск с использованием нейронных сетей</a>».</p>
