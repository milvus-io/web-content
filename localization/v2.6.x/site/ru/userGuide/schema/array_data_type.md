...
<h1 id="Array-Field" class="common-anchor-header">Поле «Массив»<button data-href="#Array-Field" class="anchor-icon" translate="no">
    ...
    </button></h1><p>Поле ARRAY хранит упорядоченный набор элементов одного и того же типа данных. Вот пример того, как поля ARRAY хранят данные:</p>
...
<li><p><strong>Значения по умолчанию</strong>: поля ARRAY не поддерживают значения по умолчанию. Однако вы можете установить атрибут « <code translate="no">nullable</code> » в значение « <code translate="no">True</code> », чтобы разрешить нулевые значения. Подробности см. в разделе <a href="/docs/ru/v2.6.x/nullable-and-default.md">«Nullable &amp; Default</a>».</p></li>
...
<p>Если при определении схемы установить параметр ` <code translate="no">enable_dynamic_fields=True</code> `, Milvus позволит вставлять скалярные поля, которые не были определены заранее. Однако это может увеличить сложность запросов и управления, что потенциально может повлиять на производительность. Дополнительную информацию см. в разделе <a href="/docs/ru/v2.6.x/enable-dynamic-field.md">«Динамическое поле</a>».</p>
</div>
