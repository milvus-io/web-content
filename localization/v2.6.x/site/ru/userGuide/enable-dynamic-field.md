...
title: Включение динамического поля
---
<h1 id="Enable-Dynamic-Field" class="common-anchor-header">Включение динамического поля<button data-href="#Enable-Dynamic-Field" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h1><p>На этой странице объясняется, как использовать динамическое поле в коллекции для гибкой вставки и извлечения данных.</p>
<h2 id="Overview" class="common-anchor-header">Обзор<button data-href="#Overview" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Milvus позволяет вам определить схему коллекции, задав имя и тип данных каждого конкретного поля, чтобы вы могли создать индексы в этих полях для повышения производительности поиска.</p>
<p>После того как поле определено, его необходимо включать при вставке данных. Что делать, если некоторые поля не всегда присутствуют во всех ваших записях данных? Здесь на помощь приходит динамическое поле.</p>
<p>Динамическое поле в коллекции - это зарезервированное поле JSON с именем $meta. В нем могут храниться не определенные схемой поля и их значения в виде пар ключ-значение. С помощью динамического поля можно искать и запрашивать как определенные схемой поля, так и не определенные схемой поля.</p>
<h2 id="Enable-dynamic-field" class="common-anchor-header">Включение динамического поля<button data-href="#Enable-dynamic-field" class="anchor-icon" translate="no">
    ...
    </button></h2><p>При определении схемы для коллекции вы можете установить значение <code translate="no">enable_dynamic_field</code> на <code translate="no">True</code>, чтобы включить зарезервированное динамическое поле, указывающее, что любые не определенные схемой поля и их значения, вставленные позже, будут сохранены как пары ключ-значение в зарезервированном динамическом поле.</p>
<p>Следующий фрагмент создает коллекцию с двумя определенными схемой полями, а именно id и vector, и включает динамическое поле.</p>
<div class="language-python">
<p>Для получения дополнительной информации о параметрах см. <a href="https://milvus.io/api-reference/pymilvus/v2.4.x/MilvusClient/Collections/create_collection.md"><code translate="no">create_collection()</code></a> в справочнике SDK.</p>
</div>
<div class="language-java">
<p>Для получения дополнительной информации о параметрах см. <a href="https://milvus.io/api-reference/java/v2.4.x/v2/Collections/createCollection.md"><code translate="no">createCollection()</code></a> в справочнике SDK.</p>
</div>
<div class="language-javascript">
<p>Для получения дополнительной информации о параметрах см. <a href="https://milvus.io/api-reference/node/v2.4.x/Collections/createCollection.md"><code translate="no">createCollection()</code></a> в справочнике SDK.</p>
</div>
...
<span class="hljs-comment">// </span>
<button class="copy-code-btn"></button></code></pre>
<h2 id="Insert-dynamic-data" class="common-anchor-header">Вставка динамических данных<button data-href="#Insert-dynamic-data" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>После создания коллекции можно приступать к вставке в нее данных, в том числе динамических.</p>
<h3 id="Prepare-data" class="common-anchor-header">Подготовка данных<button data-href="#Prepare-data" class="anchor-icon" translate="no">
    ...
    </button></h3><p>В этом разделе вам нужно подготовить несколько случайно сгенерированных данных для последующей вставки.</p>
<div class="multipleCode">
   <a href="#python">Python </a> <a href="#java">Java</a> <a href="#javascript">Node.js</a></div>
...
<h3 id="Insert-data" class="common-anchor-header">Вставить данные<button data-href="#Insert-data" class="anchor-icon" translate="no">
    ...
    </button></h3><p>После этого вы можете смело вставлять данные в коллекцию.</p>
<div class="language-python">
<p>Для получения дополнительной информации о параметрах см. <a href="https://milvus.io/api-reference/pymilvus/v2.4.x/MilvusClient/Vector/insert.md"><code translate="no">insert()</code></a> в справочнике SDK.</p>
</div>
<div class="language-java">
<p>Дополнительные сведения о параметрах см. <a href="https://milvus.io/api-reference/java/v2.4.x/v2/Vector/insert.md"><code translate="no">insert()</code></a> в справке SDK.</p>
</div>
<div class="language-javascript">
<p>Дополнительные сведения о параметрах см. <a href="https://milvus.io/api-reference/node/v2.4.x/Vector/insert.md"><code translate="no">insert()</code></a> в справочнике SDK.</p>
</div>
<div class="multipleCode">
   <a href="#python">Python </a> <a href="#java">Java</a> <a href="#javascript">Node.js</a></div>
...
<button class="copy-code-btn"></button></code></pre>
<h2 id="Search-with-dynamic-fields" class="common-anchor-header">Поиск с динамическими полями<button data-href="#Search-with-dynamic-fields" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>Если вы создали коллекцию с включенным динамическим полем и вставили в нее поля, не определяемые схемой, вы можете использовать эти поля в выражении фильтра поиска или запроса следующим образом.</p>
<div class="language-python">
...
<span class="hljs-comment">// </span>
<button class="copy-code-btn"></button></code></pre>
<h2 id="Recaps" class="common-anchor-header">Отзывы<button data-href="#Recaps" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>Стоит отметить, что <strong>color</strong>, <strong>tag</strong> и <strong>color_tag</strong> не присутствуют при определении схемы коллекции, но вы можете использовать их как определенные схемой поля при выполнении поиска и запросов.</p>
<p>Если имя поля, не определяемого схемой, содержит символы, отличные от цифр, букв и знаков подчеркивания, такие как знаки плюс (+), звездочки (*) или доллары (<span class="katex-error" title="ParseError: KaTeX parse error: Expected group after &#x27;_&#x27; at position 39: …the key within _̲_" style="color:#cc0000">), то</span>при использовании его в булевом выражении или включении в поля вывода<span class="katex-error" title="ParseError: KaTeX parse error: Expected group after &#x27;_&#x27; at position 39: …the key within _̲_" style="color:#cc0000"> необходимо включить ключ в __meta</span>[]__, как показано в следующем фрагменте кода.</p>
