...
summary: >-
  В этом руководстве показано, как построить продвинутую систему с расширенной
  генерацией (Retrieval-Augmented Generation, RAG) с помощью LangGraph и Milvus.
  В отличие от традиционных систем RAG, которые просто извлекают и генерируют
  информацию, агентные системы RAG могут принимать интеллектуальные решения о
  том, когда извлекать информацию, как обрабатывать нерелевантные документы и
  когда переписывать запросы для получения лучших результатов.
title: Агентный RAG с Milvus и LangGraph
---
...
<h1 id="Agentic-RAG-with-Milvus-and-LangGraph" class="common-anchor-header">Агентный RAG с Milvus и LangGraph<button data-href="#Agentic-RAG-with-Milvus-and-LangGraph" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h1><p>В этом руководстве показано, как построить продвинутую систему с расширенной генерацией (Retrieval-Augmented Generation, RAG) с помощью LangGraph и Milvus. В отличие от традиционных систем RAG, которые просто извлекают и генерируют информацию, агентские системы RAG могут принимать интеллектуальные решения о том, когда извлекать информацию, как обрабатывать нерелевантные документы и когда переписывать запросы для получения лучших результатов.</p>
<p>
   ...
   </span> <span class="img-wrapper"> <span>Архитектура агентной RAG-системы с использованием LangGraph и Milvus</span> </span></p>
<p><a href="https://langchain-ai.github.io/langgraph/">LangGraph</a> - это библиотека для создания многоакторных приложений с состоянием и LLM, построенная на основе LangChain. <a href="https://milvus.io/">Milvus</a> - самая продвинутая в мире векторная база данных с открытым исходным кодом, созданная для работы с приложениями поиска сходства встраивания и искусственного интеллекта.</p>
<p>В этом руководстве мы создадим агентную систему RAG, которая сможет:</p>
<ul>
<li>Принимать решения о том, извлекать ли документы или отвечать напрямую на простые запросы</li>
<li>оценивать полученные документы по релевантности</li>
<li>Переписывать вопросы, если найденные документы не являются релевантными</li>
<li>Генерировать высококачественные ответы на основе релевантного контекста</li>
</ul>
...
<h2 id="Prepare-the-data" class="common-anchor-header">Подготовьте данные<button data-href="#Prepare-the-data" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Мы используем Langchain <a href="https://python.langchain.com/docs/integrations/document_loaders/web_base/">WebBaseLoader</a> для загрузки документов из <a href="https://lilianweng.github.io/">постов блога Лилиан Венг</a> и разбиваем их на фрагменты с помощью <a href="https://python.langchain.com/docs/how_to/recursive_text_splitter/">RecursiveCharacterTextSplitter</a>.</p>
...
<h2 id="Create-a-retriever-tool-with-Milvus" class="common-anchor-header">Создание ретривера с помощью Milvus<button data-href="#Create-a-retriever-tool-with-Milvus" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>Теперь мы создадим векторное хранилище с помощью Milvus, чтобы проиндексировать наши фрагменты документов и создать инструмент для извлечения, который сможет использовать наш агент.</p>
...
<ul>
<li>Установка <code translate="no">uri</code> в качестве локального файла, например<code translate="no">./milvus_agentic_rag.db</code>, является наиболее удобным методом, поскольку он автоматически использует <a href="https://milvus.io/docs/milvus_lite.md">Milvus Lite</a> для хранения всех данных в этом файле.</li>
...
<h2 id="Build-the-agentic-RAG-graph" class="common-anchor-header">Построение агентного графа RAG<button data-href="#Build-the-agentic-RAG-graph" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><h3 id="Define-the-graph-state" class="common-anchor-header">Определите состояние графа<button data-href="#Define-the-graph-state" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Мы будем использовать граф LangGraph <code translate="no">MessagesState</code>, который хранит список сообщений в разговоре.</p>
...
<button class="copy-code-btn"></button></code></pre>
<h3 id="Node-1-Generate-query-or-respond" class="common-anchor-header">Узел 1: генерировать запрос или отвечать<button data-href="#Node-1-Generate-query-or-respond" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Этот узел решает, использовать ли инструмент retriever для поиска информации или ответить пользователю напрямую.</p>
...
<h3 id="Node-2-Grade-documents" class="common-anchor-header">Узел 2: Оценка документов<button data-href="#Node-2-Grade-documents" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Этот узел оценивает, насколько найденные документы соответствуют вопросу пользователя.</p>
...
<button class="copy-code-btn"></button></code></pre>
<h3 id="Node-3-Rewrite-question" class="common-anchor-header">Узел 3: Переписать вопрос<button data-href="#Node-3-Rewrite-question" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Если документы не релевантны, этот узел переписывает вопрос, чтобы улучшить результаты поиска.</p>
...
<button class="copy-code-btn"></button></code></pre>
<h3 id="Node-4-Generate-answer" class="common-anchor-header">Узел 4: Генерировать ответ<button data-href="#Node-4-Generate-answer" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Этот узел генерирует окончательный ответ на основе найденных релевантных документов.</p>
<pre><code translate="no" class="language-python"><span class="hljs-keyword">def</span> <span class="hljs-title function_">generate</span>(<span class="hljs-params">state: MessagesState</span>):
    ...
    <span class="hljs-keyword">return</span> {<span class="hljs-string">&quot;messages&quot;</span>: [response]}
<button class="copy-code-btn"></button></code></pre>
<h3 id="Assemble-the-graph" class="common-anchor-header">Сборка графа<button data-href="#Assemble-the-graph" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Теперь мы соединим все узлы вместе, чтобы создать наш агентный рабочий процесс RAG.</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>Давайте визуализируем структуру графа, чтобы понять суть рабочего процесса:</p>
...
<h2 id="Run-the-agentic-RAG-system" class="common-anchor-header">Запуск агентной системы RAG<button data-href="#Run-the-agentic-RAG-system" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>Теперь давайте протестируем нашу агентскую систему RAG с помощью различных типов запросов.</p>
<h3 id="Test-1-Simple-greeting-no-retrieval-needed" class="common-anchor-header">Тест 1: Простое приветствие (поиск не требуется)<button data-href="#Test-1-Simple-greeting-no-retrieval-needed" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<h3 id="Test-2-Question-requiring-retrieval" class="common-anchor-header">Тест 2: Вопрос, требующий поиска<button data-href="#Test-2-Question-requiring-retrieval" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<h3 id="Test-3-Question-that-might-trigger-rewrite" class="common-anchor-header">Тест 3: Вопрос, который может вызвать повторную запись<button data-href="#Test-3-Question-that-might-trigger-rewrite" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<h2 id="Summary" class="common-anchor-header">Резюме<button data-href="#Summary" class="anchor-icon" translate="no">
    ...
    </button></h2><p>В этом руководстве мы создали агентную RAG-систему на основе LangGraph и Milvus, которая может самостоятельно решать, когда нужно получить информацию, оценивать релевантность документов и переписывать запросы для получения лучших результатов. Такой подход обеспечивает значительные преимущества по сравнению с традиционными RAG-системами, включая улучшение пользовательского опыта за счет интеллектуальной маршрутизации, более высокое качество ответов за счет оценки документов и улучшение поиска за счет переписывания запросов. Вы можете расширить эту систему, добавив более сложную логику оценки, реализовав несколько стратегий поиска или включив дополнительные инструменты и источники данных.</p>
