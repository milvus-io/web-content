...
---
<div class="tab-wrapper"><a href="/docs/ru/v2.6.x/upgrade_milvus_cluster-operator.md" class='active '>Milvus</a><a href="/docs/ru/v2.6.x/upgrade_milvus_cluster-helm.md" class=''>OperatorHelm</a></div>
<h1 id="Upgrade-Milvus-Cluster-with-Milvus-Operator" class="common-anchor-header">Обновление кластера Milvus с помощью Milvus Operator<button data-href="#Upgrade-Milvus-Cluster-with-Milvus-Operator" class="anchor-icon" translate="no">
    ...
    </button></h1><p>В этом руководстве описано, как обновить кластер Milvus с версии v2.5.x до v2.6.24 с помощью Milvus Operator.</p>
<h2 id="Before-you-start" class="common-anchor-header">Перед началом<button data-href="#Before-you-start" class="anchor-icon" translate="no">
    ...
    </button></h2><h3 id="Whats-new-in-v2624" class="common-anchor-header">Что нового в версии v2.6.24<button data-href="#Whats-new-in-v2624" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Обновление с Milvus 2.5.x до 2.6.24 сопряжено со значительными изменениями в архитектуре:</p>
<ul>
...
<li><strong>Новые компоненты</strong>: введена узел потоковой обработки (Streaming Node) для усовершенствованной обработки данных</li>
...
<ul>
<li>Кластер Kubernetes с Milvus, развернутым с помощью Milvus Operator</li>
...
<ul>
<li>Milvus v2.6.0-rc1 <strong>несовместим</strong> с версией v2.6.24. Прямое обновление с кандидатских версий не поддерживается.</li>
...
<li>Перед обновлением до версии v2.6.24 <strong>необходимо</strong> выполнить обновление до версии v2.5.16 или более поздней с включенной функцией « <code translate="no">mixCoord</code> ».</li>
</ul>
<p><strong>Ограничения</strong>, связанные с<strong>очередями сообщений</strong>: при обновлении до Milvus v2.6.24 необходимо сохранить текущий выбор системы очередей сообщений. Переключение между различными системами очередей сообщений во время обновления не поддерживается. Поддержка смены систем очередей сообщений будет доступна в будущих версиях.</p>
<h2 id="Upgrade-process" class="common-anchor-header">Процесс обновления<button data-href="#Upgrade-process" class="anchor-icon" translate="no">
    ...
    </button></h2><h3 id="Step-1-Upgrade-Milvus-Operator" class="common-anchor-header">Шаг 1: Обновление Milvus Operator<button data-href="#Step-1-Upgrade-Milvus-Operator" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Сначала обновите Milvus Operator до версии v1.3.7:</p>
...
<button class="copy-code-btn"></button></code></pre>
<h3 id="Step-2-Upgrade-your-Milvus-cluster" class="common-anchor-header">Шаг 2: Обновите кластер Milvus<button data-href="#Step-2-Upgrade-your-Milvus-cluster" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><h4 id="21-Check-current-coordinator-configuration" class="common-anchor-header">2.1 Проверьте текущую конфигурацию координатора</h4><p>Проверьте, используется ли в вашем кластере уже <code translate="no">mixCoord</code>:</p>
<pre><code translate="no" class="language-bash">kubectl get pods
<button class="copy-code-btn"></button></code></pre>
<p>Если вместо этого вы видите отдельные поды координатора (<code translate="no">datacoord</code>, <code translate="no">querycoord</code>, <code translate="no">indexcoord</code>), вам необходимо включить <code translate="no">mixCoord</code> на следующем шаге.</p>
<h4 id="22-Upgrade-to-v2516-with-mixCoord" class="common-anchor-header">2.2 Обновление до версии v2.5.16 с помощью mixCoord</h4><div class="alert-note">
<p>Пропустите этот шаг, если в вашем кластере уже работает версия v2.5.16 или выше с включенной функцией <code translate="no">mixCoord</code>.</p>
</div>
<p>Создайте файл конфигурации <code translate="no">milvusupgrade.yaml</code>, чтобы включить <code translate="no">mixCoord</code> и выполнить обновление до версии v2.5.16:</p>
...
kubectl get pods
<button class="copy-code-btn"></button></code></pre>
<h4 id="23-Upgrade-to-v2624" class="common-anchor-header">2.3 Обновление до версии v2.6.24</h4><p>Как только версия v2.5.16 начнёт успешно работать с функцией « <code translate="no">mixCoord</code> », обновите систему до версии v2.6.24:</p>
