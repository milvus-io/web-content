---
id: abs.md
title: Настройка доступа к хранилищу Blob Storage по идентификатору рабочей нагрузки
related_key: 'blob storage, workload identity, iam'
summary: 'Узнайте, как настроить Blob Storage с помощью Workload Identity.'
---
<h1 id="Configure-Blob-Storage-Access-by-Workload-Identity" class="common-anchor-header">Настройка доступа к хранилищу Blob Storage по идентификатору рабочей нагрузки<button data-href="#Configure-Blob-Storage-Access-by-Workload-Identity" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h1><p>В этой теме рассказывается о том, как настроить доступ к Azure Blob Storage по Workload Identity при установке Milvus с помощью helm. Для получения дополнительной информации см. раздел <a href="https://azure.github.io/azure-workload-identity/docs/introduction.html">Workload Identity</a>.</p>
<h2 id="Configure-applications-to-use-Workload-Identity" class="common-anchor-header">Настройте приложения для использования Workload Identity<button data-href="#Configure-applications-to-use-Workload-Identity" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<ul>
<li>Обновите кластер AKS с помощью OIDC Issuer и Workload Identity.</li>
</ul>
...
<button class="copy-code-btn"></button></code></pre>
<ul>
<li>Получите URL-адрес эмитента OIDC.</li>
</ul>
...
<button class="copy-code-btn"></button></code></pre>
<ul>
<li>Создайте управляемую идентификацию пользователя и назначьте роль.</li>
</ul>
...
<button class="copy-code-btn"></button></code></pre>
<ul>
<li>Создайте учетную запись службы.</li>
</ul>
