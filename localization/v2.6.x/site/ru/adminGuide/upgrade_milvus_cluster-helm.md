...
<h1 id="Upgrade-Milvus-Cluster-with-Helm-Chart" class="common-anchor-header">Обновление кластера Milvus с помощью Helm Chart<button data-href="#Upgrade-Milvus-Cluster-with-Helm-Chart" class="anchor-icon" translate="no">
    ...
    </button></h1><p>В этом руководстве описано, как обновить кластер Milvus с версии v2.5.x до v2.6.24 с помощью Helm Chart.</p>
...
<li><strong>Новые компоненты</strong>: введено узел потоковой обработки (Streaming Node) для усовершенствованной обработки данных</li>
...
<li>Если вы в настоящее время используете версию v2.6.0-rc1 и вам необходимо сохранить свои данные, обратитесь к <a href="https://github.com/milvus-io/milvus/issues/43538#issuecomment-3112808997">этому руководству сообщества</a> для получения помощи по миграции.</li>
<li>Перед обновлением до версии v2.6.24 <strong>необходимо</strong> выполнить обновление до версии v2.5.16 или более поздней с включенной функцией « <code translate="no">mixCoordinator</code> ».</li>
</ul>
...
<p>Чтобы проверить совместимость версии Helm Chart с версиями Milvus:</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>Если вы видите отдельные под-координаторы (<code translate="no">datacoord</code>, <code translate="no">querycoord</code>, <code translate="no">indexcoord</code>), обновите систему до версии v2.5.16 и включите <code translate="no">mixCoordinator</code>:</p>
...
kubectl get pods
<button class="copy-code-btn"></button></code></pre>
<h3 id="Step-3-Upgrade-to-v2624" class="common-anchor-header">Шаг 3: Обновление до версии v2.6.24<button data-href="#Step-3-Upgrade-to-v2624" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Как только версия v2.5.16 начнет успешно работать с <code translate="no">mixCoordinator</code>, обновите систему до версии v2.6.24:</p>
<pre><code translate="no" class="language-bash">helm upgrade my-release zilliztech/milvus \
