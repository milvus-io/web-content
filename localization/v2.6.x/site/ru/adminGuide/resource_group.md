...
<h2 id="What-is-a-resource-group" class="common-anchor-header">Что такое группа ресурсов<button data-href="#What-is-a-resource-group" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Группа ресурсов может содержать несколько или все узлы запросов в кластере Milvus. Вы сами решаете, как распределить узлы запросов между группами ресурсов, исходя из того, что наиболее целесообразно в вашем случае. Например, в сценарии с несколькими коллекциями вы можете распределить соответствующее количество узлов запросов по каждой группе ресурсов и загрузить коллекции в разные группы ресурсов, чтобы операции внутри каждой коллекции были физически независимы от операций в других коллекциях.</p>
...
<p>Все примеры кода на этой странице приведены для PyMilvus версии 2.6.16. Перед их запуском обновите свою установку PyMilvus.</p>
</div>
<ol>
...
<p>Кроме того, если строго соблюдать ограничение <code translate="no">sum(.requests.nodeNum) &lt;= queryNodeNum</code>, можно точно контролировать распределение QueryNodes в кластере. Предположим, что в данный момент в кластере находится только один QueryNode, и инициализируем кластер.
...
<button class="copy-code-btn"></button></code></pre>
<p>Мы можем использовать API для масштабирования конкретной группы ресурсов до заданного количества узлов QueryNode, не затрагивая при этом другие группы ресурсов.</p>
