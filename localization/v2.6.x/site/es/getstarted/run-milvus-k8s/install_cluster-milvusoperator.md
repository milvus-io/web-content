...
<h2 id="Overview" class="common-anchor-header">Descripción general<button data-href="#Overview" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Milvus Operator es una solución que te ayuda a implementar y gestionar una pila completa de servicios de Milvus para clústeres de Kubernetes (K8s). La pila incluye todos los componentes de Milvus y las dependencias pertinentes, como etcd, Pulsar y MinIO.</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>El comando anterior implementa un clúster de Milvus con <strong>Woodpecker</strong> como cola de mensajes (recomendado para la versión 2.6.17) y todos los nuevos componentes arquitectónicos, incluido el nodo de streaming.</p>
<p><strong>Aspectos destacados de la arquitectura en esta implementación:</strong></p>
<ul>
<li><strong>Cola de mensajes</strong>: <a href="/docs/es/v2.6.x/use-woodpecker.md">utiliza Woodpecker</a> (reduce el mantenimiento de la infraestructura)</li>
...
<button class="copy-code-btn"></button></code></pre>
<h3 id="3-Forward-a-local-port-to-Milvus" class="common-anchor-header">3. Reenvía un puerto local a Milvus<button data-href="#3-Forward-a-local-port-to-Milvus" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<button class="copy-code-btn"></button></code></pre>
<p>Si lo prefieres, puedes utilizar <code translate="no">:19530</code> en lugar de <code translate="no">27017:19530</code> en el comando anterior para que <code translate="no">kubectl</code> te asigne un puerto local y así evitar tener que gestionar conflictos de puertos.</p>
...
<div class="alert note">
<ul>
<li>Al eliminar el clúster de Milvus utilizando la configuración predeterminada, no se eliminan dependencias como etcd, Pulsar y MinIO. Por lo tanto, la próxima vez que instale la misma instancia del clúster de Milvus, estas dependencias se volverán a utilizar.</li>
