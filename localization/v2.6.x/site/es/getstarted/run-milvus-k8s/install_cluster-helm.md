...
<h2 id="Online-install" class="common-anchor-header">Instalación en línea<button data-href="#Online-install" class="anchor-icon" translate="no">
    ...
    </button></h2><h3 id="1-Deploy-a-Milvus-cluster" class="common-anchor-header">1. Despliegue un cluster Milvus</h3><p>Una vez que haya instalado la carta Helm, puede iniciar Milvus en Kubernetes. Esta sección le guía a través del despliegue de un cluster Milvus.</p>
...
<button class="copy-code-btn"></button></code></pre>
<p><strong>Nota</strong>: El modo autónomo utiliza Woodpecker como cola de mensajes predeterminada y habilita el componente Streaming Node. Para más detalles, consulte la <a href="/docs/es/architecture_overview.md">Visión general de la arquitectura</a>.</p>
</div>
<p><strong>Despliegue del cluster Milvus:</strong></p>
<p>El siguiente comando despliega un cluster Milvus con la configuración optimizada para v2.6.0, utilizando WoodPecker como cola de mensajes recomendada:</p>
<pre><code translate="no" class="language-bash">helm install my-release zilliztech/milvus \
  --<span class="hljs-built_in">set</span> image.all.tag=v2.6.0 \
  --<span class="hljs-built_in">set</span> pulsarv3.enabled=<span class="hljs-literal">false</span> \
  --<span class="hljs-built_in">set</span> woodpecker.enabled=<span class="hljs-literal">true</span> \
...
<li><strong>Nuevo componente</strong>: Se introduce <strong>Streaming Node</strong> y se activa por defecto</li>
...
<h3 id="2-Check-Milvus-cluster-status" class="common-anchor-header">2. Compruebe el estado del cluster Milvus</h3><p>Verifique que su despliegue se ha realizado correctamente comprobando el estado del pod:</p>
...
<h3 id="3-Connect-to-Milvus" class="common-anchor-header">3. Conectarse a Milvus</h3><p>Para conectarse a su cluster Milvus desde fuera de Kubernetes, necesita configurar el reenvío de puertos.</p>
