...
<ul>
<li><p>Para desplegar una instancia de Milvus en modo autónomo, ejecute el siguiente comando:</p>
...
<button class="copy-code-btn"></button></code></pre>
  <div class="alert note">
<p>A partir de Milvus 2.6.x, se han realizado los siguientes cambios en la arquitectura en modo autónomo:</p>
<ul>
<li>La cola de mensajes (MQ) por defecto es <strong>Woodpecker</strong>.</li>
<li>Se introduce el componente <strong>Streaming Node</strong> y se activa por defecto.</li>
</ul>
<p>Para más detalles, consulte la <a href="/docs/es/architecture_overview.md">Descripción general de la arquitectura</a>.</p>
  </div>
</li>
<li><p>Para desplegar una instancia de Milvus en modo cluster, ejecute el siguiente comando:</p>
<p>Puede utilizar <code translate="no">--set</code> para instalar el clúster Milvus con configuraciones personalizadas. El siguiente comando establece <code translate="no">streaming.enabled</code> en <code translate="no">true</code> para habilitar el servicio de streaming y establece <code translate="no">indexNode.enabled</code> en <code translate="no">false</code> para deshabilitar el servicio de índice. En este caso, el nodo de streaming será responsable de todas las tareas de procesamiento e indexación de datos.</p>
<pre><code translate="no" class="language-bash">helm install my-release milvus/milvus \
  --<span class="hljs-built_in">set</span> image.all.tag=v2.6.0-rc1 \
  --<span class="hljs-built_in">set</span> streaming.enabled=<span class="hljs-literal">true</span> \
...
<button class="copy-code-btn"></button></code></pre>
  <div class="alert note">
<p>A partir de Milvus 2.6.x, se han realizado los siguientes cambios en la arquitectura en modo cluster:</p>
<ul>
<li>El MQ por defecto sigue siendo <strong>Pulsar</strong>.</li>
<li>Se introduce el componente <strong>Streaming Node</strong> y se activa por defecto.</li>
<li>El <strong>Nodo de Índice</strong> y el <strong>Nodo de Datos</strong> se fusionan en un único componente <strong>Nodo de Datos</strong>.</li>
</ul>
<p>Para obtener más información, consulte la <a href="/docs/es/architecture_overview.md">Descripción general de la arquitectura</a>.</p>
  </div>
</li>
</ul>
...
<p>Los comandos anteriores despliegan una instancia de Milvus con sus componentes y dependencias utilizando configuraciones predeterminadas. Para personalizar estas configuraciones, le recomendamos que utilice la <a href="https://milvus.io/tools/sizing">herramienta Milvus Sizing Tool</a> para ajustar las configuraciones en función del tamaño real de sus datos y, a continuación, descargue el archivo YAML correspondiente. Para saber más sobre los parámetros de configuración, consulte <a href="https://milvus.io/docs/system_configuration.md">Milvus System Configurations Checklist</a>.</p>
<div class="alert note">
  <ul>
...
<ol>
<li>Cree un archivo <code translate="no">values.yaml</code> con las configuraciones deseadas.</li>
</ol>
