...
summary: >-
  Esta guía muestra cómo construir un sistema avanzado de Generación Mejorada de
  Recuperación (RAG) utilizando LangGraph y Milvus. A diferencia de los sistemas
  RAG tradicionales que simplemente recuperan y generan, los sistemas RAG
  agénticos pueden tomar decisiones inteligentes sobre cuándo recuperar
  información, cómo manejar los documentos irrelevantes y cuándo reescribir las
  consultas para obtener mejores resultados.
title: RAG Agentico con Milvus y LangGraph
---
...
<h1 id="Agentic-RAG-with-Milvus-and-LangGraph" class="common-anchor-header">RAG Agentico con Milvus y LangGraph<button data-href="#Agentic-RAG-with-Milvus-and-LangGraph" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h1><p>Esta guía muestra cómo construir un sistema avanzado de Generación Mejorada de Recuperación (RAG) utilizando LangGraph y Milvus. A diferencia de los sistemas RAG tradicionales que simplemente recuperan y generan, los sistemas RAG agénticos pueden tomar decisiones inteligentes sobre cuándo recuperar información, cómo manejar los documentos irrelevantes y cuándo reescribir las consultas para obtener mejores resultados.</p>
<p><a href="https://langchain-ai.github.io/langgraph/">LangGraph</a> es una biblioteca para crear aplicaciones con múltiples actores y estado con LLM, construida sobre LangChain. <a href="https://milvus.io/">Milvus</a> es la base de datos vectorial de código abierto más avanzada del mundo, creada para potenciar la búsqueda de similitudes y las aplicaciones de IA.</p>
...
<li>Calificar la relevancia de los documentos recuperados</li>
<li>Reescribir las preguntas cuando los documentos recuperados no sean relevantes.</li>
<li>Generar respuestas de alta calidad basadas en el contexto relevante</li>
</ul>
...
<blockquote>
<p>Si utilizas Google Colab, para activar las dependencias que acabas de instalar, es posible que tengas que <strong>reiniciar el tiempo de ejecución</strong> (haz clic en el menú "Tiempo de ejecución" situado en la parte superior de la pantalla y selecciona "Reiniciar sesión" en el menú desplegable).</p>
</blockquote>
...
<h2 id="Prepare-the-data" class="common-anchor-header">Preparar los datos<button data-href="#Prepare-the-data" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Usamos el Langchain <a href="https://python.langchain.com/docs/integrations/document_loaders/web_base/">WebBaseLoader</a> para cargar los documentos de <a href="https://lilianweng.github.io/">los posts del blog de Lilian Weng</a> y dividirlos en trozos usando el <a href="https://python.langchain.com/docs/how_to/recursive_text_splitter/">RecursiveCharacterTextSplitter</a>.</p>
...
<h2 id="Create-a-retriever-tool-with-Milvus" class="common-anchor-header">Crear una herramienta de recuperación con Milvus<button data-href="#Create-a-retriever-tool-with-Milvus" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>Ahora crearemos un almacén vectorial utilizando Milvus para indexar nuestros trozos de documentos y crear una herramienta recuperadora que nuestro agente pueda utilizar.</p>
...
<ul>
<li>Establecer el <code translate="no">uri</code> como un archivo local, por ejemplo<code translate="no">./milvus_agentic_rag.db</code>, es el método más conveniente, ya que utiliza automáticamente <a href="https://milvus.io/docs/milvus_lite.md">Milvus Lite</a> para almacenar todos los datos en este archivo.</li>
...
<h2 id="Build-the-agentic-RAG-graph" class="common-anchor-header">Construir el grafo RAG agéntico<button data-href="#Build-the-agentic-RAG-graph" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><h3 id="Define-the-graph-state" class="common-anchor-header">Definir el estado del grafo<button data-href="#Define-the-graph-state" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Usaremos <code translate="no">MessagesState</code> de LangGraph que mantiene una lista de mensajes en la conversación.</p>
...
<button class="copy-code-btn"></button></code></pre>
<h3 id="Node-1-Generate-query-or-respond" class="common-anchor-header">Nodo 1: Generar consulta o responder<button data-href="#Node-1-Generate-query-or-respond" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Este nodo decide si utilizar la herramienta retriever para buscar información o responder directamente al usuario.</p>
...
<h3 id="Node-2-Grade-documents" class="common-anchor-header">Nodo 2: Calificar los documentos<button data-href="#Node-2-Grade-documents" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Este nodo evalúa si los documentos recuperados son relevantes para la pregunta del usuario.</p>
...
<button class="copy-code-btn"></button></code></pre>
<h3 id="Node-3-Rewrite-question" class="common-anchor-header">Nodo 3: Reescribir la pregunta<button data-href="#Node-3-Rewrite-question" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Si los documentos no son relevantes, este nodo reescribe la pregunta para mejorar los resultados de la recuperación.</p>
...
<button class="copy-code-btn"></button></code></pre>
<h3 id="Node-4-Generate-answer" class="common-anchor-header">Nodo 4: Generar respuesta<button data-href="#Node-4-Generate-answer" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Este nodo genera la respuesta final basándose en los documentos relevantes recuperados.</p>
<pre><code translate="no" class="language-python"><span class="hljs-keyword">def</span> <span class="hljs-title function_">generate</span>(<span class="hljs-params">state: MessagesState</span>):
    ...
    <span class="hljs-keyword">return</span> {<span class="hljs-string">&quot;messages&quot;</span>: [response]}
<button class="copy-code-btn"></button></code></pre>
<h3 id="Assemble-the-graph" class="common-anchor-header">Ensamblar el grafo<button data-href="#Assemble-the-graph" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Ahora conectaremos todos los nodos para crear nuestro flujo de trabajo RAG.</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>Visualicemos la estructura del grafo para entender el flujo de trabajo:</p>
...
<h2 id="Run-the-agentic-RAG-system" class="common-anchor-header">Ejecutar el sistema RAG<button data-href="#Run-the-agentic-RAG-system" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>Ahora vamos a probar nuestro sistema RAG con diferentes tipos de consultas.</p>
<h3 id="Test-1-Simple-greeting-no-retrieval-needed" class="common-anchor-header">Prueba 1: Saludo simple (no requiere recuperación)<button data-href="#Test-1-Simple-greeting-no-retrieval-needed" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<h3 id="Test-2-Question-requiring-retrieval" class="common-anchor-header">Prueba 2: Pregunta que requiere recuperación<button data-href="#Test-2-Question-requiring-retrieval" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<h3 id="Test-3-Question-that-might-trigger-rewrite" class="common-anchor-header">Prueba 3: Pregunta que podría desencadenar la reescritura<button data-href="#Test-3-Question-that-might-trigger-rewrite" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<h2 id="Summary" class="common-anchor-header">Resumen<button data-href="#Summary" class="anchor-icon" translate="no">
    ...
    </button></h2><p>En este tutorial, construimos un sistema RAG agéntico usando LangGraph y Milvus que puede decidir inteligentemente cuándo recuperar información, evaluar la relevancia de los documentos y reescribir las consultas para obtener mejores resultados. Este enfoque proporciona ventajas significativas sobre los sistemas RAG tradicionales, incluyendo una mejor experiencia de usuario a través del enrutamiento inteligente, respuestas de mayor calidad con la clasificación de documentos, y una mejor recuperación a través de la reescritura de consultas. Puede ampliar este sistema añadiendo una lógica de clasificación más sofisticada, implementando múltiples estrategias de recuperación o incorporando herramientas y fuentes de datos adicionales.</p>
