...
<h2 id="Enable-text-match" class="common-anchor-header">Activar la concordancia de texto<button data-href="#Enable-text-match" class="anchor-icon" translate="no">
    ...
    </button></h2><p>La concordancia de texto funciona en el tipo de campo <code translate="no">VARCHAR</code>, que es esencialmente el tipo de datos de cadena en Milvus. Para habilitar la concordancia de texto, configure tanto <code translate="no">enable_analyzer</code> como <code translate="no">enable_match</code> en <code translate="no">True</code> y luego, opcionalmente, configure un <a href="/docs/es/analyzer-overview.md">analizador</a> para el análisis de texto al definir el esquema de su colección.</p>
