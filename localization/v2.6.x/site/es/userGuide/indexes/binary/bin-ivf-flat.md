...
title: BIN_IVF_FLAT
summary: >-
  El índice BIN_IVF_FLAT es una variante del índice IVF_FLAT exclusivamente para
  incrustaciones binarias. Mejora la eficacia de la consulta dividiendo primero
  los datos vectoriales en varios clústeres (unidades nlist) y comparando
  después el vector de entrada objetivo con el centro de cada clúster.
  BIN_IVF_FLAT reduce significativamente el tiempo de consulta al tiempo que
  permite a los usuarios ajustar con precisión el equilibrio entre precisión y
  velocidad. Para más información, consulte IVF_FLAT.
---
...
<h2 id="Build-index" class="common-anchor-header">Creación de índices<button data-href="#Build-index" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Para construir un índice <code translate="no">BIN_IVF_FLAT</code> sobre un campo vectorial en Milvus, utilice el método <code translate="no">add_index()</code>, especificando los parámetros <code translate="no">index_type</code>, <code translate="no">metric_type</code>, y adicionales para el índice.</p>
...
<ul>
<li><p><code translate="no">index_type</code>: El tipo de índice a construir. En este ejemplo, establezca el valor <code translate="no">BIN_IVF_FLAT</code>.</p></li>
...
<p>Para conocer más parámetros de construcción disponibles para el índice <code translate="no">BIN_IVF_FLAT</code>, consulte <a href="/docs/es/bin-ivf-flat.md#Index-building-params">Parámetros de construcción del índice</a>.</p></li>
</ul>
...
<p>Para conocer más parámetros de búsqueda disponibles para el índice <code translate="no">BIN_IVF_FLAT</code>, consulte <a href="/docs/es/bin-ivf-flat.md#Index-specific-search-params">Parámetros de búsqueda específicos del índice</a>.</p></li>
</ul>
...
<h3 id="Index-building-params" class="common-anchor-header">Parámetros de creación de índices</h3><p>La siguiente tabla enumera los parámetros que pueden configurarse en <code translate="no">params</code> al <a href="/docs/es/bin-ivf-flat.md#Index-building-params">crear un índice</a>.</p>
<table>
   <tr>
     <th><p>Parámetro</p></th>
...
<h3 id="Index-specific-search-params" class="common-anchor-header">Parámetros de búsqueda específicos del índice</h3><p>La siguiente tabla enumera los parámetros que pueden configurarse en <code translate="no">search_params.params</code> al <a href="/docs/es/bin-ivf-flat.md#Search-on-index">buscar en el índice</a>.</p>
<table>
   <tr>
     <th><p>Parámetro</p></th>
     <th><p>Descripción</p></th>
