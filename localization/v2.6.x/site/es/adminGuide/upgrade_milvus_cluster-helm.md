...
<h1 id="Upgrade-Milvus-Cluster-with-Helm-Chart" class="common-anchor-header">Actualizar el clúster de Milvus con Helm Chart<button data-href="#Upgrade-Milvus-Cluster-with-Helm-Chart" class="anchor-icon" translate="no">
    ...
    </button></h1><p>Esta guía describe cómo actualizar tu clúster de Milvus de la versión v2.5.x a la v2.6.24 utilizando Helm Chart.</p>
...
<li><strong>Debes</strong> actualizar a la versión v2.5.16 o posterior con la opción « <code translate="no">mixCoordinator</code> » habilitada antes de actualizar a la v2.6.24.</li>
</ul>
...
<button class="copy-code-btn"></button></code></pre>
<p>Esta guía da por hecho que vas a instalar la última versión. Si necesitas instalar una versión específica, especifica el parámetro <code translate="no">--version</code> como corresponda.</p>
<h3 id="Step-2-Upgrade-to-v2516-with-mixCoordinator" class="common-anchor-header">Paso 2: Actualizar a la v2.5.16 con mixCoordinator<button data-href="#Step-2-Upgrade-to-v2516-with-mixCoordinator" class="anchor-icon" translate="no">
    ...
    </button></h3><p>Comprueba si tu clúster utiliza actualmente coordinadores independientes:</p>
<pre><code translate="no" class="language-bash">kubectl get pods
<button class="copy-code-btn"></button></code></pre>
<p>Si ves pods de coordinador independientes (<code translate="no">datacoord</code>, <code translate="no">querycoord</code>, <code translate="no">indexcoord</code>), actualiza a la v2.5.16 y habilita <code translate="no">mixCoordinator</code>:</p>
...
<button class="copy-code-btn"></button></code></pre>
<div class="alert-note">
<p>Si su clúster ya utiliza <code translate="no">mixCoordinator</code>, simplemente actualice la imagen:</p>
<pre><code translate="no" class="language-bash">helm upgrade my-release zilliztech/milvus \
  --<span class="hljs-built_in">set</span> image.all.tag=<span class="hljs-string">&quot;v2.5.16&quot;</span> \
  --reset-then-reuse-values \
  --version=4.2.58
...
kubectl get pods
<button class="copy-code-btn"></button></code></pre>
<h3 id="Step-3-Upgrade-to-v2624" class="common-anchor-header">Paso 3: Actualizar a la versión 2.6.24<button data-href="#Step-3-Upgrade-to-v2624" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Una vez que la versión 2.5.16 funcione correctamente con <code translate="no">mixCoordinator</code>, actualiza a la versión 2.6.24:</p>
<pre><code translate="no" class="language-bash">helm upgrade my-release zilliztech/milvus \
