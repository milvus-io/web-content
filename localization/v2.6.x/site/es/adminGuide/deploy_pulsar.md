...
<p><strong>Limitaciones de las colas de mensajes</strong>: al actualizar a Milvus v2.6.23, debe mantener su elección actual de cola de mensajes. No se admite el cambio entre diferentes sistemas de colas de mensajes durante la actualización. La compatibilidad con el cambio de sistemas de colas de mensajes estará disponible en futuras versiones.</p>
</div>
...
<li>Configure la sección « <code translate="no">externalConfigFiles</code> » del archivo « <code translate="no">values.yaml</code> ».</li>
</ol>
...
<p>Para obtener más información sobre cómo configurar Milvus mediante Helm, consulta <a href="/docs/es/v2.6.x/configure-helm.md">«Configurar Milvus con Helm Charts</a>». Para obtener más información sobre los elementos de configuración relacionados con Woodpecker, consulta <a href="/docs/es/v2.6.x/use-woodpecker.md">«Configuraciones relacionadas con Woodpecker</a>».
|</p>
<h3 id="Using-the-YAML-file" class="common-anchor-header">Uso del archivo YAML<button data-href="#Using-the-YAML-file" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
...
<h2 id="Configure-Kafka-with-Helm" class="common-anchor-header">Configurar Kafka con Helm<button data-href="#Configure-Kafka-with-Helm" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Para los clústeres de Milvus en K8s, puedes configurar Kafka en el mismo comando con el que se inicia Milvus. Como alternativa, puedes configurar Kafka utilizando el archivo <code translate="no">values.yml</code>, ubicado en la ruta /charts/milvus del repositorio <a href="https://github.com/milvus-io/milvus-helm">milvus-helm</a>, antes de iniciar Milvus.</p>
...
<h2 id="Configure-RocksMQ-with-Helm" class="common-anchor-header">Configurar RocksMQ con Helm<button data-href="#Configure-RocksMQ-with-Helm" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Milvus en modo independiente utiliza RocksMQ como almacenamiento de mensajes predeterminado. Para conocer los pasos detallados sobre cómo configurar Milvus con Helm, consulta <a href="/docs/es/v2.6.x/configure-helm.md">«Configurar Milvus con Helm Charts</a>». Para obtener más información sobre los elementos de configuración relacionados con RocksMQ, consulta <a href="/docs/es/v2.6.x/configure_rocksmq.md">«Configuraciones relacionadas con RocksMQ</a>».</p>
<ul>
