...
summary: >-
  ...
  Génération Assistée par Récupération (RAG) en utilisant Milvus et DeepSeek.
title: Construire RAG avec Milvus et DeepSeek
---
<h1 id="Build-RAG-with-Milvus-and-DeepSeek" class="common-anchor-header">Construire RAG avec Milvus et DeepSeek<button data-href="#Build-RAG-with-Milvus-and-DeepSeek" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<p><a href="https://www.deepseek.com/">DeepSeek</a> permet aux développeurs de créer et de mettre à l'échelle des applications d'IA avec des modèles de langage très performants. Il offre une inférence efficace, des API flexibles et des architectures avancées de mélange d'experts (MoE) pour des tâches de raisonnement et de recherche robustes.</p>
<p>Dans ce tutoriel, nous allons vous montrer comment construire un pipeline Retrieval-Augmented Generation (RAG) en utilisant Milvus et DeepSeek.</p>
...
<p>DeepSeek active l'API de type OpenAI. Vous pouvez vous connecter à son site officiel et préparer la <a href="https://platform.deepseek.com/api_keys">clé api</a> <code translate="no">DEEPSEEK_API_KEY</code> comme variable d'environnement.</p>
...
<h3 id="Prepare-the-LLM-and-Embedding-Model" class="common-anchor-header">Préparer le LLM et le modèle d'intégration<button data-href="#Prepare-the-LLM-and-Embedding-Model" class="anchor-icon" translate="no">
    ...
    </button></h3><p>DeepSeek active l'API de type OpenAI, et vous pouvez utiliser la même API avec des ajustements mineurs pour appeler le LLM.</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>Définissez un modèle d'intégration pour générer des intégrations de texte à l'aide de <code translate="no">milvus_model</code>. Nous utilisons le modèle <code translate="no">DefaultEmbeddingFunction</code> comme exemple, qui est un modèle d'intégration pré-entraîné et léger.</p>
...
&quot;&quot;&quot;</span>
<button class="copy-code-btn"></button></code></pre>
<p>Utiliser le modèle <code translate="no">deepseek-chat</code> fourni par DeepSeek pour générer une réponse basée sur les invites.</p>
...
<p>Génial ! Nous avons réussi à construire un pipeline RAG avec Milvus et DeepSeek.</p>
