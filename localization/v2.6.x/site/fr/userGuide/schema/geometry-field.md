...
title: Champ géométriqueCompatible with Milvus 2.6.4+
summary: >-
  Lors de la création d'applications telles que les systèmes d'information
  géographique (SIG), les outils de cartographie ou les services basés sur la
  localisation, il est souvent nécessaire de stocker et d'interroger des données
  géométriques. Le type de données GEOMETRY de Milvus résout ce problème en
  fournissant un moyen natif de stocker et d'interroger des données géométriques
  flexibles.
beta: Milvus 2.6.4+
---
<h1 id="Geometry-Field" class="common-anchor-header">Champ géométrique<span class="beta-tag" style="background-color:rgb(0, 179, 255);color:white" translate="no">Compatible with Milvus 2.6.4+</span><button data-href="#Geometry-Field" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<p>Le champ GEOMETRY nécessite PyMilvus 2.7.0rc46 ou une version ultérieure. Cette version n'est actuellement disponible qu'en construisant à partir des sources. Pour les instructions, voir <a href="https://github.com/milvus-io/pymilvus#faq">Comment construire PyMilvus à partir des sources</a>.</p>
</div>
<h2 id="What-is-a-GEOMETRY-field" class="common-anchor-header">Qu'est-ce qu'un champ GEOMETRIE ?<button data-href="#What-is-a-GEOMETRY-field" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>Un champ GEOMETRY est un type de données défini par le schéma (<code translate="no">DataType.GEOMETRY</code>) dans Milvus qui stocke des données géométriques. Lorsque vous travaillez avec des champs géométriques, vous interagissez avec les données à l'aide du format <a href="https://en.wikipedia.org/wiki/Well-known_text_representation_of_geometry">Well-Known Text (WKT)</a>, une représentation lisible par l'homme utilisée à la fois pour l'insertion de données et l'interrogation. En interne, Milvus convertit WKT en <a href="https://en.wikipedia.org/wiki/Well-known_text_representation_of_geometry#Well-known_binary">Well-Known Binary (WKB</a> ) pour un stockage et un traitement efficaces, mais vous n'avez pas besoin de gérer WKB directement.</p>
<p>Le type de données <code translate="no">GEOMETRY</code> prend en charge les objets géométriques suivants :</p>
<ul>
<li><p><strong>POINT</strong>: <code translate="no">POINT (x y)</code>; par exemple, <code translate="no">POINT (13.403683 52.520711)</code> où <code translate="no">x</code> = longitude et <code translate="no">y</code> = latitude</p></li>
<li><p><strong>LINESTRING</strong>: <code translate="no">LINESTRING (x1 y1, x2 y2, …)</code>; par exemple, <code translate="no">LINESTRING (13.40 52.52, 13.41 52.51)</code></p></li>
<li><p><strong>POLYGONE</strong>: <code translate="no">POLYGON ((x1 y1, x2 y2, x3 y3, x1 y1))</code>; par exemple, <code translate="no">POLYGON ((30 10, 40 40, 20 40, 10 20, 30 10))</code></p></li>
<li><p><strong>MULTIPOINT</strong>: <code translate="no">MULTIPOINT ((x1 y1), (x2 y2), …)</code>, par exemple, <code translate="no">MULTIPOINT ((10 40), (40 30), (20 20), (30 10))</code></p></li>
<li><p><strong>MULTILINESTRING</strong>: <code translate="no">MULTILINESTRING ((x1 y1, …), (xk yk, …))</code>, par exemple, <code translate="no">MULTILINESTRING ((10 10, 20 20, 10 40), (40 40, 30 30, 40 20, 30 10))</code></p></li>
<li><p><strong>MULTIPOLYGON</strong>: <code translate="no">MULTIPOLYGON (((outer ring ...)), ((outer ring ...)))</code>, par exemple, <code translate="no">MULTIPOLYGON (((30 20, 45 40, 10 40, 30 20)), ((15 5, 40 10, 10 20, 5 10, 15 5)))</code></p></li>
<li><p><strong>GEOMETRYCOLLECTION</strong>: <code translate="no">GEOMETRYCOLLECTION(POINT(x y), LINESTRING(x1 y1, x2 y2), ...)</code>, par exemple, <code translate="no">GEOMETRYCOLLECTION (POINT (40 10), LINESTRING (10 10, 20 20, 10 40), POLYGON ((40 40, 20 45, 45 30, 40 40)))</code></p></li>
</ul>
<h2 id="Basic-operations" class="common-anchor-header">Opérations de base<button data-href="#Basic-operations" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Pour utiliser un champ <code translate="no">GEOMETRY</code>, il faut le définir dans le schéma de la collection, insérer des données géométriques, puis interroger les données à l'aide d'expressions de filtrage spécifiques.</p>
<h3 id="Step-1-Define-a-GEOMETRY-field" class="common-anchor-header">Étape 1 : Définition d'un champ GEOMETRY<button data-href="#Step-1-Define-a-GEOMETRY-field" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Pour utiliser un champ <code translate="no">GEOMETRY</code>, définissez-le explicitement dans votre schéma de collection lors de la création de la collection. L'exemple suivant montre comment créer une collection avec un champ <code translate="no">geo</code> de type <code translate="no">DataType.GEOMETRY</code>.</p>
...
<button class="copy-code-btn"></button></code></pre>
<div class="alert note">
<p>Dans cet exemple, le champ <code translate="no">GEOMETRY</code> défini dans le schéma de la collection autorise les valeurs nulles avec <code translate="no">nullable=True</code>. Pour plus de détails, voir <a href="/docs/fr/nullable-and-default.md">Nullable &amp; Default</a>.</p>
</div>
<h3 id="Step-2-Insert-data" class="common-anchor-header">Étape 2 : Insérer des données<button data-href="#Step-2-Insert-data" class="anchor-icon" translate="no">
    ...
    </button></h3><p>Insérez des entités avec des données géométriques au format <a href="https://en.wikipedia.org/wiki/Well-known_text_representation_of_geometry">WKT</a>. Voici un exemple avec plusieurs points géo :</p>
<div class="multipleCode">
   <a href="#python">Python</a> <a href="#java">Java</a> <a href="#javascript">NodeJS</a> <a href="#go">Go</a> <a href="#bash">cURL</a></div>
...
<pre><code translate="no" class="language-bash"><span class="hljs-comment"># restful</span>
<button class="copy-code-btn"></button></code></pre>
<h3 id="Step-3-Filtering-operations" class="common-anchor-header">Etape 3 : Opérations de filtrage<button data-href="#Step-3-Filtering-operations" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Avant de pouvoir effectuer des opérations de filtrage sur les champs <code translate="no">GEOMETRY</code>, il faut s'assurer que</p>
<ul>
...
<div class="alert note">
<p>Certains opérateurs, tels que <code translate="no">ST_DWITHIN</code>, peuvent nécessiter des paramètres supplémentaires. Pour plus de détails et d'exemples d'utilisation de chaque opérateur, reportez-vous à la section <a href="/docs/fr/geometry-operators.md">Opérateurs de géométrie</a>.</p>
</div>
...
<h4 id="Example-1-Find-entities-within-a-rectangular-area" class="common-anchor-header">Exemple 1 : Trouver des entités à l'intérieur d'une zone rectangulaire</h4><div class="multipleCode">
   <a href="#python">Python</a> <a href="#java">Java</a> <a href="#javascript">NodeJS</a> <a href="#go">Go</a> <a href="#bash">cURL</a></div>
...
<pre><code translate="no" class="language-bash"><span class="hljs-comment"># restful</span>
<button class="copy-code-btn"></button></code></pre>
<h4 id="Example-2-Find-entities-within-1km-of-a-central-point" class="common-anchor-header">Exemple 2 : Trouver des entités dans un rayon de 1 km autour d'un point central</h4><div class="multipleCode">
   <a href="#python">Python</a> <a href="#java">Java</a> <a href="#javascript">NodeJS</a> <a href="#go">Go</a> <a href="#bash">cURL</a></div>
...
<pre><code translate="no" class="language-bash"><span class="hljs-comment"># restful</span>
<button class="copy-code-btn"></button></code></pre>
<h4 id="Example-3-Combine-vector-similarity-with-a-spatial-filter" class="common-anchor-header">Exemple 3 : Combiner la similarité vectorielle avec un filtre spatial</h4><div class="multipleCode">
   <a href="#python">Python</a> <a href="#java">Java</a> <a href="#javascript">NodeJS</a> <a href="#go">Go</a> <a href="#bash">cURL</a></div>
...
<pre><code translate="no" class="language-bash"><span class="hljs-comment"># restful</span>
<button class="copy-code-btn"></button></code></pre>
<h2 id="Next-Accelerate-queries" class="common-anchor-header">Suivant : Accélérer les requêtes<button data-href="#Next-Accelerate-queries" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<h2 id="FAQ" class="common-anchor-header">FAQ<button data-href="#FAQ" class="anchor-icon" translate="no">
    ...
    </button></h2><h3 id="If-Ive-enabled-the-dynamic-field-feature-for-my-collection-can-I-insert-geometric-data-into-a-dynamic-field-key" class="common-anchor-header">Si j'ai activé la fonctionnalité de champ dynamique pour ma collection, puis-je insérer des données géométriques dans une clé de champ dynamique ?<button data-href="#If-Ive-enabled-the-dynamic-field-feature-for-my-collection-can-I-insert-geometric-data-into-a-dynamic-field-key" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Non, les données géométriques ne peuvent pas être insérées dans un champ dynamique. Avant d'insérer des données géométriques, assurez-vous que le champ <code translate="no">GEOMETRY</code> a été explicitement défini dans le schéma de votre collection.</p>
<h3 id="Does-the-GEOMETRY-field-support-the-mmap-feature" class="common-anchor-header">Le champ GEOMETRY prend-il en charge la fonction mmap ?<button data-href="#Does-the-GEOMETRY-field-support-the-mmap-feature" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Oui, le champ <code translate="no">GEOMETRY</code> prend en charge la fonction mmap. Pour plus d'informations, reportez-vous à la section <a href="https://zilliverse.feishu.cn/wiki/P3wrwSMNNihy8Vkf9p6cTsWYnTb">Utiliser mmap.</a></p>
<h3 id="Can-I-define-the-GEOMETRY-field-as-nullable-or-set-a-default-value" class="common-anchor-header">Puis-je définir la rubrique GEOMETRY comme nullable ou lui attribuer une valeur par défaut ?<button data-href="#Can-I-define-the-GEOMETRY-field-as-nullable-or-set-a-default-value" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Oui, le champ GEOMETRY prend en charge l'attribut <code translate="no">nullable</code> et une valeur par défaut au format WKT. Pour plus d'informations, reportez-vous à la section <a href="/docs/fr/nullable-and-default.md">Nullable &amp; Default</a>.</p>
