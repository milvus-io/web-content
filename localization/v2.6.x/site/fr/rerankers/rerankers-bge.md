...
<h1 id="BGE" class="common-anchor-header">BGE<button data-href="#BGE" class="anchor-icon" translate="no">
    ...
    </button></h1><p>Milvus prend en charge les <a href="https://github.com/FlagOpen/FlagEmbedding/tree/master/FlagEmbedding/reranker">modèles de reranker BGE</a> par le biais de la classe <code translate="no">BGERerankFunction</code>. Cette fonctionnalité vous permet d'évaluer efficacement la pertinence des paires requête-document.</p>
<p>Pour utiliser cette fonctionnalité, installez les dépendances nécessaires :</p>
<pre><code translate="no" class="language-bash">pip install --upgrade pymilvus
pip install <span class="hljs-string">&quot;pymilvus[model]&quot;</span>
<button class="copy-code-btn"></button></code></pre>
<p>Ensuite, instanciez la classe <code translate="no">BGERerankFunction</code>:</p>
...
<p>Le nom du modèle à utiliser. Vous pouvez spécifier n'importe quel nom de modèle de reranker BGE disponible, par exemple, <code translate="no">BAAI/bge-reranker-base</code>, <code translate="no">BAAI/bge-reranker-large</code>, etc. Si ce paramètre n'est pas spécifié, <code translate="no">BAAI/bge-reranker-v2-m3</code> sera utilisé. Pour obtenir la liste des modèles disponibles, reportez-vous à la section <a href="https://github.com/FlagOpen/FlagEmbedding/tree/master/FlagEmbedding/llm_reranker#model-list">Liste des modèles</a>.</p></li>
<li><p><code translate="no">device</code> <em>(chaîne</em>)</p>
<p>Facultatif. L'appareil à utiliser pour exécuter le modèle. S'il n'est pas spécifié, le modèle sera exécuté sur l'unité centrale. Vous pouvez spécifier <code translate="no">cpu</code> pour l'unité centrale et <code translate="no">cuda:n</code> pour le nième périphérique GPU.</p></li>
</ul>
