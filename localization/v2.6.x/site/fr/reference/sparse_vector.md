...
<p>Pour générer des embeddings, vous pouvez également utiliser le paquetage <code translate="no">model</code> intégré à la bibliothèque PyMilvus, qui offre une gamme de fonctions d'embedding. Pour plus d'informations, reportez-vous à la section <a href="/docs/fr/embeddings.md">Embeddings</a>.</p>
...
<li><p><code translate="no">params.drop_ratio_build</code>: Paramètre d'index utilisé spécifiquement pour les vecteurs peu denses. Il contrôle la proportion de petites valeurs vectorielles qui sont exclues au cours du processus d'indexation. Ce paramètre permet d'affiner le compromis entre efficacité et précision en ignorant les petites valeurs lors de la construction de l'index. Par exemple, si <code translate="no">drop_ratio_build = 0.3</code>, toutes les valeurs de tous les vecteurs épars sont rassemblées et triées lors de la construction de l'index. Les 30 % les plus petites de ces valeurs ne sont pas incluses dans l'index, ce qui réduit la charge de travail informatique pendant la recherche.</p></li>
</ul>
...
<ul>
<li><code translate="no">params.drop_ratio_search</code>: Le paramètre de recherche utilisé spécifiquement pour les vecteurs épars. Cette option permet d'affiner le processus de recherche en spécifiant le ratio des plus petites valeurs du vecteur de la requête à ignorer. Elle permet d'équilibrer la précision de la recherche et les performances. Plus la valeur de <code translate="no">drop_ratio_search</code> est petite, moins ces petites valeurs contribuent au résultat final. En ignorant certaines petites valeurs, les performances de la recherche peuvent être améliorées avec un impact minimal sur la précision.</li>
</ul>
