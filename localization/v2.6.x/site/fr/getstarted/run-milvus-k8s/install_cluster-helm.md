...
<button class="copy-code-btn"></button></code></pre>
<p><strong>Remarque</strong>: le mode autonome utilise Woodpecker comme file d’attente de messages par défaut et active le composant Streaming Node. Pour plus de détails, consultez les sections « <a href="/docs/fr/v2.6.x/architecture_overview.md">Présentation de l’architecture</a> » et « <a href="/docs/fr/v2.6.x/use-woodpecker.md">Utilisation de Woodpecker</a> ».</p>
</div>
<p><strong>Déployer un cluster Milvus :</strong></p>
<p>La commande suivante déploie un cluster Milvus avec des paramètres optimisés pour la version 2.6.24, en utilisant Woodpecker comme file d'attente de messages recommandée :</p>
<pre><code translate="no" class="language-bash">helm install my-release zilliztech/milvus \
  --<span class="hljs-built_in">set</span> image.all.tag=v2.6.24 \
  --<span class="hljs-built_in">set</span> pulsarv3.enabled=<span class="hljs-literal">false</span> \
  --<span class="hljs-built_in">set</span> woodpecker.enabled=<span class="hljs-literal">true</span> \
...
<button class="copy-code-btn"></button></code></pre>
<p><strong>Attendez que tous les pods affichent le statut « Running ».</strong> Avec la configuration v2.6.24, vous devriez voir des pods similaires à ceux-ci :</p>
...
<h3 id="2-Download-image-pulling-script" class="common-anchor-header">2. Télécharger le script de récupération d'images<button data-href="#2-Download-image-pulling-script" class="anchor-icon" translate="no">
    ...
    </button></h3><p>Le script de récupération des images est écrit en Python. Vous devez télécharger ce script ainsi que ses dépendances à partir du fichier <code translate="no">requirement.txt</code>.</p>
...
<li><p>Découvrez <a href="/docs/fr/v2.6.x/milvus-webui.md">Milvus WebUI</a>, une interface web intuitive dédiée à la surveillance et à la gestion de Milvus.</p></li>
...
<li><p>Découvrez <a href="/docs/fr/v2.6.x/birdwatcher_overview.md">Birdwatcher</a>, un outil open source permettant le débogage de Milvus et la mise à jour dynamique de la configuration.</p></li>
