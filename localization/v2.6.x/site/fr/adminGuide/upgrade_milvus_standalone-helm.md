...
summary: >-
  ...
  Chart.
title: Mise à jour de Milvus en mode autonome à l'aide d'un Helm Chart
---
...
<h1 id="Upgrade-Milvus-Standalone-with-Helm-Chart" class="common-anchor-header">Mise à jour de Milvus en mode autonome à l'aide d'un Helm Chart<button data-href="#Upgrade-Milvus-Standalone-with-Helm-Chart" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h1><p>Ce guide explique comment mettre à niveau votre déploiement Milvus autonome de la version 2.5.x à la version 2.6.24 à l'aide d'un Helm Chart.</p>
<h2 id="Before-you-start" class="common-anchor-header">Avant de commencer<button data-href="#Before-you-start" class="anchor-icon" translate="no">
    ...
    </button></h2><h3 id="Whats-new-in-v2624" class="common-anchor-header">Nouveautés de la version 2.6.24<button data-href="#Whats-new-in-v2624" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>La mise à niveau de Milvus 2.5.x vers la version 2.6.24 implique des changements architecturaux importants :</p>
<ul>
...
<p>Ce processus de mise à niveau garantit une migration correcte vers la nouvelle architecture. Pour plus d’informations sur les changements d’architecture, reportez-vous à <a href="/docs/fr/v2.6.x/architecture_overview.md">la présentation de l’architecture de Milvus</a>.</p>
...
<li>Milvus autonome déployé via Helm Chart</li>
</ul>
<p><strong>Exigences de compatibilité :</strong></p>
<ul>
<li>Milvus v2.6.0-rc1 <strong>n'</strong> est <strong>pas compatible</strong> avec la version v2.6.24. Les mises à jour directes à partir de versions candidates ne sont pas prises en charge.</li>
...
<li>Vous <strong>devez</strong> effectuer une mise à niveau vers la version v2.5.16 ou une version ultérieure avant de passer à la version v2.6.24.</li>
</ul>
...
Depuis la version 4.2.21 du Helm chart de Milvus, nous avons introduit le chart pulsar-v3.x en tant que dépendance. Pour assurer la rétrocompatibilité, veuillez mettre à niveau votre Helm vers la version 3.14 ou une version ultérieure, et veillez à ajouter l’option <code translate="no">--reset-then-reuse-values</code> chaque fois que vous utilisez <code translate="no">helm upgrade</code>.
</div>
<h2 id="Upgrade-process" class="common-anchor-header">Processus de mise à niveau<button data-href="#Upgrade-process" class="anchor-icon" translate="no">
    ...
    </button></h2><h3 id="Step-1-Upgrade-Helm-Chart" class="common-anchor-header">Étape 1 : Mise à jour du chart Helm<button data-href="#Step-1-Upgrade-Helm-Chart" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Commencez par mettre à niveau votre chart Helm Milvus vers la version 5.0.22 :</p>
...
<h3 id="Step-2-Upgrade-to-v2516" class="common-anchor-header">Étape 2 : Mise à niveau vers la version 2.5.16<button data-href="#Step-2-Upgrade-to-v2516" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<p>Mettez à niveau votre Milvus autonome vers la version 2.5.16 :</p>
...
kubectl get pods
<button class="copy-code-btn"></button></code></pre>
<h3 id="Step-3-Upgrade-to-v2624" class="common-anchor-header">Étape 3 : Mise à niveau vers la version 2.6.24<button data-href="#Step-3-Upgrade-to-v2624" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Une fois que la version 2.5.16 fonctionne correctement, effectuez la mise à niveau vers la version 2.6.24 :</p>
<pre><code translate="no" class="language-bash">helm upgrade my-release zilliztech/milvus \
...
<h2 id="Verify-the-upgrade" class="common-anchor-header">Vérifiez la mise à jour<button data-href="#Verify-the-upgrade" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Vérifiez que votre déploiement autonome utilise bien la nouvelle version :</p>
