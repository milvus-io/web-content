...
<h2 id="Download-a-configuration-file" class="common-anchor-header">Télécharger un fichier de configuration<button data-href="#Download-a-configuration-file" class="anchor-icon" translate="no">
    ...
    </button></h2><p><a href="https://raw.githubusercontent.com/milvus-io/milvus/v2.6.17/configs/milvus.yaml">Téléchargez</a> directement le fichier « <code translate="no">milvus.yaml</code> » ou utilisez la commande suivante.</p>
...
<h2 id="Modify-the-configuration-file" class="common-anchor-header">Modifier le fichier de configuration<button data-href="#Modify-the-configuration-file" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Configurez votre instance Milvus en fonction de vos scénarios d'application en ajustant les paramètres correspondants dans le fichier « <code translate="no">milvus.yaml</code> ».</p>
<p>Consultez les liens suivants pour plus d’informations sur chaque paramètre.</p>
<p>Trié par :</p>
<div class="filter">
<a href="#component">Composants ou dépendances</a> <a href="#purpose">Objectifs de configuration</a> 
</div>
  ...
  <tr>
    ...
    <td>
        <ul>
            ...
            <li><a href="/docs/fr/v2.6.x/configure_datacoord.md">Coordonnées de données</a></li>
            ...
            <li><a href="/docs/fr/v2.6.x/configure_quotaandlimits.md">Quotas et limites</a></li>
        </ul>
    </td>
  </tr>
</tbody>
...
<h2 id="Download-an-installation-file" class="common-anchor-header">Télécharger un fichier d'installation<button data-href="#Download-an-installation-file" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Téléchargez le fichier d'installation de Milvus <a href="https://github.com/milvus-io/milvus/releases/download/v2.6.17/milvus-standalone-docker-compose.yml">en version autonome</a> et enregistrez-le sous le nom « <code translate="no">docker-compose.yml</code> ».</p>
...
<h2 id="Modify-the-installation-file" class="common-anchor-header">Modifier le fichier d'installation<button data-href="#Modify-the-installation-file" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Dans le fichier « <code translate="no">docker-compose.yml</code> », ajoutez une section « <code translate="no">volumes</code> » sous chaque élément « <code translate="no">milvus-standalone</code> ».</p>
<p>Faites correspondre le chemin d'accès local à votre fichier <code translate="no">milvus.yaml</code> aux chemins d'accès correspondants dans le conteneur Docker vers les fichiers de configuration <code translate="no">/milvus/configs/milvus.yaml</code> sous toutes les sections <code translate="no">volumes</code>.</p>
...
<button class="copy-code-btn"></button></code></pre>
<div class="alert note">
Les données sont stockées dans le dossier <code translate="no">/volumes</code> conformément à la configuration par défaut décrite dans <code translate="no">docker-compose.yml</code>. Pour modifier le dossier de stockage des données, modifiez le fichier <code translate="no">docker-compose.yml</code> ou exécutez la commande <code translate="no">$ export DOCKER_VOLUME_DIRECTORY=</code>.
</div>
<h2 id="Start-Milvus" class="common-anchor-header">Démarrer Milvus<button data-href="#Start-Milvus" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Une fois les modifications apportées au fichier de configuration et au fichier d'installation terminées, vous pouvez lancer Milvus.</p>
...
<li>Découvrez comment gérer les dépendances suivantes de Milvus avec Docker Compose ou Helm :
<ul>
