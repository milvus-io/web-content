...
summary: >-
  Découvrez comment mettre à niveau Milvus en mode autonome à l'aide de Milvus
  Operator.
title: Mise à niveau de Milvus Standalone avec Milvus Operator
---
<div class="tab-wrapper"><a href="/docs/fr/v2.6.x/upgrade_milvus_standalone-operator.md" class='active '>Milvus</a><a href="/docs/fr/v2.6.x/upgrade_milvus_standalone-docker.md" class=''>Operator</a>, Helm, Docker<a href="/docs/fr/v2.6.x/upgrade_milvus_standalone-docker.md" class=''>Compose</a></div>
<h1 id="Upgrade-Milvus-Standalone-with-Milvus-Operator" class="common-anchor-header">Mise à niveau de Milvus Standalone avec Milvus Operator<button data-href="#Upgrade-Milvus-Standalone-with-Milvus-Operator" class="anchor-icon" translate="no">
    ...
    </button></h1><p>Ce guide décrit comment mettre à niveau votre déploiement autonome de Milvus de la version v2.5.x à la version v2.6.24 à l'aide de Milvus Operator.</p>
...
<ul>
<li>Milvus v2.6.0-rc1 <strong>n'</strong> est <strong>pas compatible</strong> avec la version v2.6.24. Les mises à niveau directes à partir de versions candidates ne sont pas prises en charge.</li>
...
<li>Vous <strong>devez</strong> effectuer une mise à jour vers la version v2.5.16 ou une version ultérieure avant de passer à la version v2.6.24.</li>
</ul>
...
<h2 id="Upgrade-process" class="common-anchor-header">Processus de mise à niveau<button data-href="#Upgrade-process" class="anchor-icon" translate="no">
    ...
    </button></h2><h3 id="Step-1-Upgrade-Milvus-Operator" class="common-anchor-header">Étape 1 : Mise à niveau de Milvus Operator<button data-href="#Step-1-Upgrade-Milvus-Operator" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Commencez par mettre à niveau votre Milvus Operator vers la version 1.3.7 :</p>
...
<p>Vérifiez la mise à niveau de l’opérateur :</p>
<pre><code translate="no" class="language-bash">kubectl -n milvus-operator get pods
<button class="copy-code-btn"></button></code></pre>
<h3 id="Step-2-Upgrade-your-Milvus-standalone" class="common-anchor-header">Étape 2 : Mise à niveau de votre instance Milvus autonome<button data-href="#Step-2-Upgrade-your-Milvus-standalone" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><h4 id="21-Upgrade-to-v2516" class="common-anchor-header">2.1 Mise à niveau vers la version v2.5.16</h4><div class="alert-note">
<p>Ignorez cette étape si votre déploiement autonome fonctionne déjà sous la version 2.5.16 ou une version supérieure.</p>
</div>
<p>Créez un fichier de configuration <code translate="no">milvusupgrade.yaml</code> pour effectuer la mise à jour vers la version 2.5.16 :</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>Attendez que l'opération soit terminée :</p>
<pre><code translate="no" class="language-bash"><span class="hljs-comment"># Verify all pods are ready</span>
kubectl get pods
<button class="copy-code-btn"></button></code></pre>
<h4 id="22-Upgrade-to-v2624" class="common-anchor-header">2.2 Mise à niveau vers la version 2.6.24</h4><p>Une fois que la version 2.5.16 fonctionne correctement, effectuez la mise à niveau vers la version 2.6.24 :</p>
