...
<h1 id="Upgrade-Milvus-Standalone-with-Docker-Compose" class="common-anchor-header">Mise à niveau de Milvus en mode autonome avec Docker Compose<button data-href="#Upgrade-Milvus-Standalone-with-Docker-Compose" class="anchor-icon" translate="no">
    ...
    </button></h1><p>Ce guide décrit comment mettre à niveau votre déploiement Milvus autonome de la version v2.5.x à la version v2.6.22 à l'aide de Docker Compose.</p>
<h2 id="Before-you-start" class="common-anchor-header">Avant de commencer<button data-href="#Before-you-start" class="anchor-icon" translate="no">
    ...
    </button></h2><h3 id="Whats-new-in-v2622" class="common-anchor-header">Nouveautés de la version 2.6.22<button data-href="#Whats-new-in-v2622" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>La mise à niveau de Milvus 2.5.x vers la version 2.6.22 implique des changements architecturaux importants :</p>
<ul>
...
<ul>
<li>Milvus v2.6.0-rc1 <strong>n'</strong> est <strong>pas compatible</strong> avec la version v2.6.22. Les mises à jour directes à partir de versions candidates ne sont pas prises en charge.</li>
...
<li>Vous <strong>devez</strong> effectuer une mise à niveau vers la version v2.5.16 ou une version ultérieure avant de passer à la version v2.6.22.</li>
</ul>
...
<p>Pour des raisons de sécurité, Milvus met à jour son MinIO vers la version RELEASE.2024-12-18T13-15-44Z avec la sortie de la version v2.6.22.</p>
</div>
<h2 id="Upgrade-process" class="common-anchor-header">Processus de mise à niveau<button data-href="#Upgrade-process" class="anchor-icon" translate="no">
    ...
    </button></h2><h3 id="Step-1-Upgrade-to-v2516" class="common-anchor-header">Étape 1 : Mise à niveau vers la version v2.5.16<button data-href="#Step-1-Upgrade-to-v2516" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<p>Ignorez cette étape si votre déploiement autonome exécute déjà la version v2.5.16 ou une version supérieure.</p>
</div>
<ol>
<li><p>Modifiez votre fichier ` <code translate="no">docker-compose.yaml</code> ` existant et mettez à jour la balise de l’image Milvus vers la version v2.5.16 :</p>
...
<li><p>Appliquez la mise à niveau vers la version 2.5.16 :</p>
<pre><code translate="no" class="language-bash">docker compose down
docker compose up -d
<button class="copy-code-btn"></button></code></pre></li>
<li><p>Vérifiez la mise à niveau vers la version 2.5.16 :</p>
<pre><code translate="no" class="language-bash">docker compose ps
<button class="copy-code-btn"></button></code></pre></li>
</ol>
<h3 id="Step-2-Upgrade-to-v2622" class="common-anchor-header">Étape 2 : Mise à niveau vers la version 2.6.22<button data-href="#Step-2-Upgrade-to-v2622" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Une fois que la version v2.5.16 fonctionne correctement, effectuez la mise à niveau vers la version v2.6.22 :</p>
<ol>
...
<button class="copy-code-btn"></button></code></pre></li>
<li><p>Appliquez la mise à jour finale :</p>
<pre><code translate="no" class="language-bash">docker compose down
docker compose up -d
<button class="copy-code-btn"></button></code></pre></li>
</ol>
...
<li>Vous souhaiterez peut-être également découvrir comment :
<ul>
<li><a href="/docs/fr/v2.6.x/scaleout.md">Faire évoluer un cluster Milvus</a></li>
</ul></li>
<li>Si vous êtes prêt à déployer votre cluster sur le cloud :
<ul>
<li>Découvrez comment <a href="/docs/fr/v2.6.x/eks.md">déployer Milvus sur Amazon EKS avec Terraform</a></li>
<li>Découvrez comment <a href="/docs/fr/v2.6.x/gcp.md">déployer un cluster Milvus sur GCP avec Kubernetes</a></li>
<li>Découvrez comment <a href="/docs/fr/v2.6.x/azure.md">déployer Milvus sur Microsoft Azure avec Kubernetes</a></li>
</ul></li>
</ul>
