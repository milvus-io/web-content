...
<h1 id="Upgrade-Milvus-Cluster-with-Helm-Chart" class="common-anchor-header">Upgrade Milvus Cluster with Helm Chart<button data-href="#Upgrade-Milvus-Cluster-with-Helm-Chart" class="anchor-icon" translate="no">
    ...
    </button></h1><p>This guide describes how to upgrade your Milvus cluster from v2.5.x to v2.6.12 using Helm Chart.</p>
<h2 id="Before-you-start" class="common-anchor-header">Before you start<button data-href="#Before-you-start" class="anchor-icon" translate="no">
    ...
    </button></h2><h3 id="Whats-new-in-v2612" class="common-anchor-header">What’s new in v2.6.12<button data-href="#Whats-new-in-v2612" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Upgrading from Milvus 2.5.x to 2.6.12 involves significant architectural changes:</p>
<ul>
...
<ul>
<li>Milvus v2.6.0-rc1 is <strong>not compatible</strong> with v2.6.12. Direct upgrades from release candidates are not supported.</li>
...
<li>You <strong>must</strong> upgrade to v2.5.16 or later with <code translate="no">mixCoordinator</code> enabled before upgrading to v2.6.12.</li>
</ul>
<p><strong>Message Queue limitations</strong>: When upgrading to Milvus v2.6.12, you must maintain your current message queue choice. Switching between different message queue systems during the upgrade is not supported. Support for changing message queue systems will be available in future versions.</p>
...
kubectl get pods
<button class="copy-code-btn"></button></code></pre>
<h3 id="Step-3-Upgrade-to-v2612" class="common-anchor-header">Step 3: Upgrade to v2.6.12<button data-href="#Step-3-Upgrade-to-v2612" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Once v2.5.16 is running successfully with <code translate="no">mixCoordinator</code>, upgrade to v2.6.12:</p>
<pre><code translate="no" class="language-bash">helm upgrade my-release zilliztech/milvus \
  --<span class="hljs-built_in">set</span> image.all.tag=<span class="hljs-string">&quot;v2.6.12&quot;</span> \
