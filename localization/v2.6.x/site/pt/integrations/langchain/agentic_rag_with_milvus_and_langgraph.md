...
summary: >-
  Este guia demonstra como construir um sistema avançado de Geração Aumentada de
  Recuperação (RAG) utilizando LangGraph e Milvus. Ao contrário dos sistemas RAG
  tradicionais que se limitam a recuperar e gerar, os sistemas RAG agênticos
  podem tomar decisões inteligentes sobre quando recuperar informação, como
  tratar documentos irrelevantes e quando reescrever consultas para obter
  melhores resultados.
title: RAG agêntico com Milvus e LangGraph
---
...
<h1 id="Agentic-RAG-with-Milvus-and-LangGraph" class="common-anchor-header">RAG agêntico com Milvus e LangGraph<button data-href="#Agentic-RAG-with-Milvus-and-LangGraph" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h1><p>Este guia demonstra como construir um sistema avançado de Geração Aumentada de Recuperação (RAG) usando LangGraph e Milvus. Ao contrário dos sistemas RAG tradicionais que se limitam a recuperar e gerar, os sistemas RAG agênticos podem tomar decisões inteligentes sobre quando recuperar informação, como tratar documentos irrelevantes e quando reescrever consultas para obter melhores resultados.</p>
...
<li>Avaliar a relevância dos documentos recuperados</li>
<li>Reescrever perguntas quando os documentos recuperados não são relevantes</li>
<li>Gerar respostas de alta qualidade com base no contexto relevante</li>
</ul>
...
<h2 id="Prepare-the-data" class="common-anchor-header">Preparar os dados<button data-href="#Prepare-the-data" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Utilizamos o Langchain <a href="https://python.langchain.com/docs/integrations/document_loaders/web_base/">WebBaseLoader</a> para carregar documentos dos <a href="https://lilianweng.github.io/">posts do blogue de Lilian Weng</a> e dividi-los em partes utilizando o <a href="https://python.langchain.com/docs/how_to/recursive_text_splitter/">RecursiveCharacterTextSplitter</a>.</p>
...
<h2 id="Create-a-retriever-tool-with-Milvus" class="common-anchor-header">Criar uma ferramenta de recuperação com o Milvus<button data-href="#Create-a-retriever-tool-with-Milvus" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>Agora vamos criar um armazenamento de vectores utilizando o Milvus para indexar os nossos pedaços de documentos e criar uma ferramenta de recuperação que o nosso agente possa utilizar.</p>
...
<h2 id="Build-the-agentic-RAG-graph" class="common-anchor-header">Construir o gráfico RAG agêntico<button data-href="#Build-the-agentic-RAG-graph" class="anchor-icon" translate="no">
    ...
    </button></h2><h3 id="Define-the-graph-state" class="common-anchor-header">Definir o estado do grafo<button data-href="#Define-the-graph-state" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Utilizaremos o <code translate="no">MessagesState</code> do LangGraph, que mantém uma lista de mensagens na conversação.</p>
...
<button class="copy-code-btn"></button></code></pre>
<h3 id="Node-1-Generate-query-or-respond" class="common-anchor-header">Nó 1: Gerar consulta ou responder<button data-href="#Node-1-Generate-query-or-respond" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Este nó decide se deve utilizar a ferramenta retriever para procurar informações ou responder diretamente ao utilizador.</p>
...
<h3 id="Node-2-Grade-documents" class="common-anchor-header">Nó 2: Classificar documentos<button data-href="#Node-2-Grade-documents" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Este nó avalia se os documentos recuperados são relevantes para a pergunta do utilizador.</p>
...
<button class="copy-code-btn"></button></code></pre>
<h3 id="Node-3-Rewrite-question" class="common-anchor-header">Nó 3: Reescrever a pergunta<button data-href="#Node-3-Rewrite-question" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Se os documentos não forem relevantes, este nó reescreve a pergunta para melhorar os resultados da pesquisa.</p>
...
<button class="copy-code-btn"></button></code></pre>
<h3 id="Node-4-Generate-answer" class="common-anchor-header">Nó 4: Gerar resposta<button data-href="#Node-4-Generate-answer" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Este nó gera a resposta final com base nos documentos relevantes recuperados.</p>
<pre><code translate="no" class="language-python"><span class="hljs-keyword">def</span> <span class="hljs-title function_">generate</span>(<span class="hljs-params">state: MessagesState</span>):
    ...
    <span class="hljs-keyword">return</span> {<span class="hljs-string">&quot;messages&quot;</span>: [response]}
<button class="copy-code-btn"></button></code></pre>
<h3 id="Assemble-the-graph" class="common-anchor-header">Montar o grafo<button data-href="#Assemble-the-graph" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Agora, vamos ligar todos os nós para criar o nosso fluxo de trabalho RAG autêntico.</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>Vamos visualizar a estrutura do gráfico para entender o fluxo de trabalho:</p>
...
<h2 id="Run-the-agentic-RAG-system" class="common-anchor-header">Executar o sistema RAG autêntico<button data-href="#Run-the-agentic-RAG-system" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<h3 id="Test-2-Question-requiring-retrieval" class="common-anchor-header">Teste 2: Pergunta que requer recuperação<button data-href="#Test-2-Question-requiring-retrieval" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<h3 id="Test-3-Question-that-might-trigger-rewrite" class="common-anchor-header">Teste 3: Pergunta que pode acionar a reescrita<button data-href="#Test-3-Question-that-might-trigger-rewrite" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<h2 id="Summary" class="common-anchor-header">Resumo<button data-href="#Summary" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Neste tutorial, construímos um sistema RAG agêntico usando LangGraph e Milvus que pode decidir inteligentemente quando recuperar informação, avaliar a relevância do documento e reescrever consultas para obter melhores resultados. Esta abordagem oferece vantagens significativas em relação aos sistemas RAG tradicionais, incluindo uma melhor experiência do utilizador através de encaminhamento inteligente, respostas de maior qualidade com classificação de documentos e recuperação melhorada através da reescrita de consultas. Pode alargar ainda mais este sistema, acrescentando uma lógica de classificação mais sofisticada, implementando várias estratégias de recuperação ou incorporando ferramentas e fontes de dados adicionais.</p>
