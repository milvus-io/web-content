...
<h1 id="Upgrade-Milvus-Cluster-with-Helm-Chart" class="common-anchor-header">Atualizar o cluster Milvus com o Helm Chart<button data-href="#Upgrade-Milvus-Cluster-with-Helm-Chart" class="anchor-icon" translate="no">
    ...
    </button></h1><p>Este guia descreve como atualizar o seu cluster Milvus da versão 2.5.x para a versão 2.6.23 utilizando o Helm Chart.</p>
<h2 id="Before-you-start" class="common-anchor-header">Antes de começar<button data-href="#Before-you-start" class="anchor-icon" translate="no">
    ...
    </button></h2><h3 id="Whats-new-in-v2623" class="common-anchor-header">Novidades na v2.6.23<button data-href="#Whats-new-in-v2623" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<li>Cluster Milvus implementado através do Helm Chart</li>
</ul>
<p><strong>Requisitos de compatibilidade:</strong></p>
<ul>
<li>O Milvus v2.6.0-rc1 <strong>não</strong> é <strong>compatível</strong> com a v2.6.23. Não são suportadas atualizações diretas a partir de versões candidatas (release candidates).</li>
<li>Se estiver atualmente a utilizar a v2.6.0-rc1 e precisar de preservar os seus dados, consulte <a href="https://github.com/milvus-io/milvus/issues/43538#issuecomment-3112808997">este guia da comunidade</a> para obter ajuda na migração.</li>
<li><strong>Deve</strong> atualizar para a v2.5.16 ou posterior, com a funcionalidade « <code translate="no">mixCoordinator</code> » ativada, antes de atualizar para a v2.6.23.</li>
</ul>
<p><strong>Limitações da fila de mensagens</strong>: Ao atualizar para o Milvus v2.6.23, deve manter a sua escolha atual de fila de mensagens. A alternância entre diferentes sistemas de filas de mensagens durante a atualização não é suportada. O suporte para a mudança de sistemas de filas de mensagens estará disponível em versões futuras.</p>
...
<h2 id="Upgrade-process" class="common-anchor-header">Processo de atualização<button data-href="#Upgrade-process" class="anchor-icon" translate="no">
    ...
    </button></h2><h3 id="Step-1-Upgrade-Helm-Chart" class="common-anchor-header">Passo 1: Atualizar o gráfico Helm<button data-href="#Step-1-Upgrade-Helm-Chart" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Primeiro, atualize o seu gráfico Helm do Milvus para a versão 5.0.22:</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>Este guia parte do princípio de que está a instalar a versão mais recente. Se precisar de instalar uma versão específica, especifique o parâmetro <code translate="no">--version</code> em conformidade.</p>
<h3 id="Step-2-Upgrade-to-v2516-with-mixCoordinator" class="common-anchor-header">Passo 2: Atualizar para a v2.5.16 com o mixCoordinator<button data-href="#Step-2-Upgrade-to-v2516-with-mixCoordinator" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<button class="copy-code-btn"></button></code></pre>
<div class="alert-note">
<p>Se o seu cluster já utiliza <code translate="no">mixCoordinator</code>, basta atualizar a imagem:</p>
<pre><code translate="no" class="language-bash">helm upgrade my-release zilliztech/milvus \
  --<span class="hljs-built_in">set</span> image.all.tag=<span class="hljs-string">&quot;v2.5.16&quot;</span> \
  --reset-then-reuse-values \
  --version=4.2.58
...
<p>Aguarde até que a atualização esteja concluída:</p>
<pre><code translate="no" class="language-bash"><span class="hljs-comment"># Verify all pods are ready</span>
kubectl get pods
<button class="copy-code-btn"></button></code></pre>
<h3 id="Step-3-Upgrade-to-v2623" class="common-anchor-header">Passo 3: Atualize para a versão 2.6.23<button data-href="#Step-3-Upgrade-to-v2623" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Assim que a v2.5.16 estiver a funcionar corretamente com <code translate="no">mixCoordinator</code>, atualize para a v2.6.23:</p>
<pre><code translate="no" class="language-bash">helm upgrade my-release zilliztech/milvus \
