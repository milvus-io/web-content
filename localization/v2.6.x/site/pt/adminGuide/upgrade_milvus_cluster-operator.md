...
<h2 id="Before-you-start" class="common-anchor-header">Antes de começar<button data-href="#Before-you-start" class="anchor-icon" translate="no">
    ...
    </button></h2><p>A partir do Milvus 2.6.0, os antigos coordenadores separados (<code translate="no">dataCoord</code>, <code translate="no">queryCoord</code>, <code translate="no">indexCoord</code>) foram consolidados num único <code translate="no">mixCoord</code>. Antes de atualizar, certifique-se de que a sua especificação CRD utiliza <code translate="no">mixCoord</code> em vez de componentes de coordenadores individuais.</p>
<p>Se estiver a utilizar os coordenadores separados, modifique a sua especificação:</p>
...
<h2 id="Upgrade-your-Milvus-operator" class="common-anchor-header">Atualizar o seu operador Milvus<button data-href="#Upgrade-your-Milvus-operator" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>Execute o seguinte comando para atualizar a versão do seu Operador Milvus para v1.3.0-rc1-hotfix.</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>Depois de ter atualizado o seu operador Milvus para a versão mais recente, tem as seguintes opções:</p>
<ul>
<li>Para atualizar o Milvus a partir da versão 2.2.3, pode <a href="#Conduct-a-rolling-upgrade">efetuar uma atualização contínua</a>.</li>
<li>Para atualizar o Milvus a partir de uma versão secundária anterior à v2.2.3 para a 2.6.0-rc1, é aconselhável <a href="#Upgrade-Milvus-by-changing-its-image">atualizar o Milvus alterando a sua versão de imagem</a>.</li>
<li>Para atualizar o Milvus da v2.1.x para a 2.6.0-rc1, é necessário <a href="#Migrate-the-metadata">migrar os metadados</a> antes da atualização propriamente dita.</li>
</ul>
<blockquote>
<p><strong>Nota</strong>: É altamente recomendável atualizar uma versão secundária de cada vez e utilizar a última versão estável dessa versão secundária. Por exemplo, se estiver a atualizar da v2.4.x para a v2.6.x, deve primeiro atualizar para a v2.4.x mais recente, depois para a v2.5.x mais recente e, por fim, para a v2.6.x. Isto garante que está a utilizar a última versão estável de cada versão secundária, que é mais provável que seja compatível com os dados e configurações existentes.</p>
</blockquote>
