...
<p>A atualização do Milvus 2.5.x (ou versões anteriores) para o 2.6.0 envolve alterações arquitectónicas significativas que tornam esta atualização <strong>irreversível</strong>. Devido à introdução de novos componentes (como o Woodpecker e o Streaming Node) e à remoção de determinados componentes, <strong>não é possível reverter para uma versão anterior após a conclusão da atualização</strong>. Para obter detalhes sobre as alterações arquitetônicas introduzidas na versão 2.6.0, consulte <a href="/docs/pt/architecture_overview.md">Visão geral da arquitetura do Milvus</a>.</p>
</div>
...
<div style="display: none;">- [Conduzir uma atualização contínua](#conduct-a-rolling-upgrade) do Milvus v2.2.3 e versões posteriores para a v2.6.0.</div>
<ul>
<li><p><a href="#Upgrade-Milvus-using-Helm">Atualizar o Milvus utilizando o Helm</a> para uma atualização de uma versão menor antes da v2.2.3 para a v2.6.0.</p></li>
<li><p><a href="#Migrate-the-metadata">Migrar os metadados</a> antes da atualização do Milvus v2.1.x para a v2.6.0.</p></li>
</ul>
...
<p>Depois de se ter assegurado que todas as implementações na sua instância Milvus estão no seu estado normal. Você pode executar o seguinte comando para atualizar a instância do Milvus para a versão 2.6.0.</p>
...
<h4 id="2-Upgrade-Milvus-from-v21x-to-260" class="common-anchor-header">2. Atualizar o Milvus da v2.1.x para a 2.6.0</h4><p>Os comandos a seguir assumem que você atualizou o Milvus da v2.1.4 para a 2.6.0. Altere-os para as versões que atendam às suas necessidades.</p>
<ol>
