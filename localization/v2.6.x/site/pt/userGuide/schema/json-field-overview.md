...
<p>Neste exemplo, <code translate="no">metadata</code> é um único campo JSON que contém uma mistura de valores simples (por exemplo, <code translate="no">category</code>, <code translate="no">in_stock</code>), matrizes (<code translate="no">tags</code>) e objectos aninhados (<code translate="no">supplier</code>).</p>
