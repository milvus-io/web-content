...
title: Executar o Milvus com suporte a GPU utilizando o Docker Compose
---
<h1 id="Run-Milvus-with-GPU-Support-Using-Docker-Compose" class="common-anchor-header">Executar o Milvus com suporte a GPU utilizando o Docker Compose<button data-href="#Run-Milvus-with-GPU-Support-Using-Docker-Compose" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<h3 id="1-Download-and-configure-the-YAML-file" class="common-anchor-header">1. Descarregue e configure o ficheiro YAML<button data-href="#1-Download-and-configure-the-YAML-file" class="anchor-icon" translate="no">
    ...
    </button></h3><p>Descarregue <a href="https://github.com/milvus-io/milvus/releases/download/v2.6.24/milvus-standalone-docker-compose-gpu.yml"><code translate="no">milvus-standalone-docker-compose-gpu.yml</code></a> e guarde-o como docker-compose.yml manualmente ou com o comando seguinte.</p>
...
<ul>
<li>Para atribuir um dispositivo de GPU específico ao Milvus, localize o campo « <code translate="no">deploy.resources.reservations.devices[0].devices_ids</code> » na definição do serviço « <code translate="no">standalone</code> » e substitua o seu valor pelo ID da GPU pretendida. Pode utilizar a ferramenta « <code translate="no">nvidia-smi</code> », incluída nos controladores de vídeo da NVIDIA para GPU, para determinar o ID de um dispositivo de GPU. O Milvus suporta vários dispositivos de GPU.</li>
</ul>
...
<p>Para configurar o conjunto de memória, altere as definições « <code translate="no">initMemSize</code> » e « <code translate="no">maxMemSize</code> » no ficheiro « <code translate="no">milvus.yaml</code> » da seguinte forma.</p>
<ol>
...
<li><p>Consulte <a href="/docs/pt/v2.6.x/milvus-webui.md">a interface web do Milvus</a> para saber mais sobre a instância do Milvus.</p></li>
