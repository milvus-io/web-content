...
<p>Para obter detalhes, consulte a <a href="/docs/pt/architecture_overview.md">Visão geral da arquitetura</a>.</p>
  </div>
</li>
<li><p>Para implantar uma instância do Milvus no modo de cluster, execute o seguinte comando:</p>
<p>É possível usar <code translate="no">--set</code> para instalar o cluster do Milvus com configurações personalizadas. O seguinte comando define <code translate="no">streaming.enabled</code> para <code translate="no">true</code> para ativar o serviço de streaming e define <code translate="no">indexNode.enabled</code> para <code translate="no">false</code> para desativar o serviço de índice. Neste caso, o nó de streaming será responsável por todas as tarefas de processamento e indexação de dados.</p>
<pre><code translate="no" class="language-bash">helm install my-release milvus/milvus \
  --<span class="hljs-built_in">set</span> image.all.tag=v2.6.0-rc1 \
  --<span class="hljs-built_in">set</span> streaming.enabled=<span class="hljs-literal">true</span> \
...
<button class="copy-code-btn"></button></code></pre>
  <div class="alert note">
<p>A partir do Milvus 2.6.x, foram feitas as seguintes alterações de arquitetura no modo de cluster:</p>
<ul>
<li>O MQ padrão continua sendo <strong>o Pulsar</strong>.</li>
<li>O componente <strong>Streaming Node</strong> foi introduzido e ativado por padrão.</li>
...
<button class="copy-code-btn"></button></code></pre>
<p>Você também pode acessar a WebUI do Milvus em <code translate="no">http://127.0.0.1:9091/webui/</code> para saber mais sobre a instância do Milvus. Para obter detalhes, consulte <a href="/docs/pt/milvus-webui.md">Milvus WebUI</a>.</p>
...
<h2 id="Optional-Update-Milvus-configurations" class="common-anchor-header">(Opcional) Atualizar as configurações do Milvus<button data-href="#Optional-Update-Milvus-configurations" class="anchor-icon" translate="no">
    ...
    </button></h2><p>É possível atualizar as configurações do cluster do Milvus editando o arquivo <code translate="no">values.yaml</code> e aplicando-o novamente.</p>
<ol>
<li>Crie um arquivo <code translate="no">values.yaml</code> com as configurações desejadas.</li>
</ol>
<p>A seguir, presume-se que você deseja habilitar o <code translate="no">proxy.http</code>.</p>
...
<h2 id="Access-Milvus-WebUI" class="common-anchor-header">Acesso à WebUI do Milvus<button data-href="#Access-Milvus-WebUI" class="anchor-icon" translate="no">
    ...
    </button></h2><p>O Milvus é fornecido com uma ferramenta GUI integrada chamada Milvus WebUI, que pode ser acedida através do seu browser. A Milvus Web UI melhora a observabilidade do sistema com uma interface simples e intuitiva. Pode utilizar a Milvus Web UI para observar as estatísticas e métricas dos componentes e dependências do Milvus, verificar detalhes da base de dados e da coleção, e listar configurações detalhadas do Milvus. Para obter detalhes sobre a Milvus Web UI, consulte <a href="/docs/pt/milvus-webui.md">Milvus WebUI</a></p>
