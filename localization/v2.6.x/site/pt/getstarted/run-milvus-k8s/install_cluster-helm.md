...
<h1 id="Run-Milvus-in-Kubernetes-with-Helm" class="common-anchor-header">Executar o Milvus no Kubernetes com o Helm<button data-href="#Run-Milvus-in-Kubernetes-with-Helm" class="anchor-icon" translate="no">
    ...
    </button></h1><p>Esta página explica como iniciar uma instância do Milvus no Kubernetes utilizando <a href="https://github.com/zilliztech/milvus-helm">os gráficos Helm do Milvus</a>.</p>
...
<li><p><a href="/docs/pt/v2.6.x/prerequisite-helm.md#How-can-I-start-a-K8s-cluster-locally-for-test-purposes">Crie um cluster K8s</a>.</p></li>
...
<button class="copy-code-btn"></button></code></pre>
<p><strong>Nota</strong>: O modo autónomo utiliza o Woodpecker como fila de mensagens predefinida e ativa o componente Streaming Node. Para mais detalhes, consulte a <a href="/docs/pt/v2.6.x/architecture_overview.md">Visão Geral da Arquitetura</a> e <a href="/docs/pt/v2.6.x/use-woodpecker.md">Utilizar o Woodpecker</a>.</p>
</div>
<p><strong>Implantar o cluster do Milvus:</strong></p>
<p>O comando seguinte implementa um cluster do Milvus com definições otimizadas para a versão 2.6.23, utilizando o Woodpecker como fila de mensagens recomendada:</p>
<pre><code translate="no" class="language-bash">helm install my-release zilliztech/milvus \
  --<span class="hljs-built_in">set</span> image.all.tag=v2.6.23 \
  --<span class="hljs-built_in">set</span> pulsarv3.enabled=<span class="hljs-literal">false</span> \
  --<span class="hljs-built_in">set</span> woodpecker.enabled=<span class="hljs-literal">true</span> \
...
<li>Desativa o <strong>«Index Node»</strong> antigo (a funcionalidade é agora gerida pelo «Data Node»)</li>
<li>Desativa o Pulsar para utilizar o Woodpecker em seu lugar</li>
</ul>
<div class="alert note">
...
<li><strong>Novo componente</strong>: <strong>o «Streaming Node</strong> » é introduzido e ativado por predefinição</li>
...
<p>Para obter detalhes completos sobre a arquitetura, consulte a <a href="/docs/pt/v2.6.x/architecture_overview.md">Visão Geral da Arquitetura</a>.</p>
</div>
<p><strong>Opções alternativas de fila de mensagens:</strong></p>
<p>Se preferir utilizar <strong>o Pulsar</strong> (opção tradicional) em vez do Woodpecker:</p>
<pre><code translate="no" class="language-bash">helm install my-release zilliztech/milvus \
  --<span class="hljs-built_in">set</span> image.all.tag=v2.6.23 \
  --<span class="hljs-built_in">set</span> streaming.enabled=<span class="hljs-literal">true</span> \
  --<span class="hljs-built_in">set</span> indexNode.enabled=<span class="hljs-literal">false</span>
...
<li>Consulte <a href="https://milvus.io/docs/system_configuration.md">a Lista de Verificação de Configurações do Sistema Milvus</a> para opções de configuração avançadas</li>
</ul>
<div class="alert note">
<p><strong>Notas importantes:</strong></p>
<ul>
<li><strong>Nomeação da versão</strong>: utilize apenas letras, números e traços (não são permitidos pontos)</li>
<li><strong>Kubernetes v1.25+</strong>: Se encontrar problemas com o PodDisruptionBudget, utilize esta solução alternativa:
<pre><code translate="no" class="language-bash">helm install my-release zilliztech/milvus \
...
<button class="copy-code-btn"></button></code></pre>
<p><strong>Aguarde até que todos os pods apresentem o estado «Running».</strong> Com a configuração v2.6.23, deverá ver pods semelhantes a:</p>
...
<li><strong>Dependências</strong>: <code translate="no">etcd</code> (metadados), <code translate="no">minio</code> (armazenamento de objetos), <code translate="no">pulsar</code> (fila de mensagens)</li>
</ul>
<p>Também pode aceder à <strong>interface Web do Milvus</strong> em <code translate="no">http://127.0.0.1:9091/webui/</code> assim que o reencaminhamento de portas estiver configurado (consulte o passo seguinte). Para mais detalhes, consulte <a href="/docs/pt/v2.6.x/milvus-webui.md">a interface Web do Milvus</a>.</p>
...
<p>Para os itens de configuração aplicáveis, consulte <a href="/docs/pt/v2.6.x/system_configuration.md">«Configuração do sistema</a>».</p></li>
...
<h2 id="Access-Milvus-WebUI" class="common-anchor-header">Aceder à Milvus WebUI<button data-href="#Access-Milvus-WebUI" class="anchor-icon" translate="no">
    ...
    </button></h2><p>O Milvus inclui uma ferramenta GUI integrada chamada Milvus WebUI, à qual pode aceder através do seu navegador. A Milvus WebUI melhora a observabilidade do sistema com uma interface simples e intuitiva. Pode utilizar a Milvus WebUI para observar as estatísticas e métricas dos componentes e dependências do Milvus, verificar os detalhes da base de dados e da recolha de dados e listar as configurações detalhadas do Milvus. Para mais detalhes sobre a Milvus WebUI, consulte <a href="/docs/pt/v2.6.x/milvus-webui.md">Milvus WebUI</a></p>
...
<div class="alert note">
<ul>
<li>Para instalar uma instância do Milvus no modo autónomo, em que todos os componentes do Milvus estão contidos num único pod, deve executar <code translate="no">helm template my-release --set cluster.enabled=false --set etcd.replicaCount=1 --set minio.mode=standalone --set pulsarv3.enabled=false zilliztech/milvus &gt; milvus_manifest.yaml</code> em vez disso, para gerar modelos de gráficos para uma instância do Milvus no modo autónomo.</li>
...
<button class="copy-code-btn"></button></code></pre>
<p>Até ao momento, pode seguir os passos <a href="#2-Check-Milvus-cluster-status">2</a> e <a href="#3-Forward-a-local-port-to-Milvus">3</a> da instalação online para verificar o estado do cluster e redirecionar uma porta local para o Milvus.</p>
...
<ul>
<li><p>Consultar <a href="/docs/pt/v2.6.x/quickstart.md">o «Hello Milvus»</a> para ver o que o Milvus pode fazer.</p></li>
...
<li><a href="/docs/pt/v2.6.x/insert-update-delete.md">Inserir, atualizar ou excluir</a></li>
