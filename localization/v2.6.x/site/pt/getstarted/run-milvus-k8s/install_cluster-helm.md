...
<li><p><a href="/docs/pt/v2.6.x/prerequisite-helm.md#How-can-I-start-a-K8s-cluster-locally-for-test-purposes">Criar um cluster K8s</a>.</p></li>
...
<ul>
<li><p>Para implantar uma instância do Milvus em modo autônomo, execute o seguinte comando:</p>
...
<button class="copy-code-btn"></button></code></pre>
  <div class="alert note">
<p>A partir do Milvus 2.6.x, as seguintes alterações de arquitetura foram feitas no modo autônomo:</p>
<ul>
<li>A fila de mensagens (MQ) padrão é o <strong>Woodpecker</strong>.</li>
<li>O componente <strong>Streaming Node</strong> é introduzido e ativado por padrão.</li>
</ul>
<p>Para obter detalhes, consulte a <a href="/docs/pt/v2.6.x/architecture_overview.md">Visão geral da arquitetura</a>.</p>
  </div>
</li>
<li><p>Para implantar uma instância do Milvus em modo de cluster, execute o seguinte comando:</p>
<pre><code translate="no" class="language-bash">helm install my-release milvus/milvus \
  --<span class="hljs-built_in">set</span> image.all.tag=v2.6.0-rc1 \
  --<span class="hljs-built_in">set</span> streaming.enabled=<span class="hljs-literal">true</span> \
...
<button class="copy-code-btn"></button></code></pre>
  <div class="alert note">
<p>A partir do Milvus 2.6.x, as seguintes alterações na arquitetura foram feitas no modo de cluster:</p>
<ul>
<li>O MQ padrão ainda é o <strong>Pulsar</strong>.</li>
<li>O componente <strong>Streaming Node</strong> é introduzido e ativado por padrão.</li>
<li>O Nó <strong>de índice</strong> e o <strong>Nó de dados</strong> são mesclados em um único componente <strong>Nó de dados</strong>.</li>
</ul>
<p>Para obter detalhes, consulte a <a href="/docs/pt/v2.6.x/architecture_overview.md">Visão geral da arquitetura</a>.</p>
  </div>
</li>
</ul>
<p>No comando acima, <code translate="no">my-release</code> é o nome da versão e <code translate="no">milvus/milvus</code> é o repositório de gráficos instalado localmente. Para usar um nome diferente, substitua <code translate="no">my-release</code> pelo nome que achar mais adequado.</p>
<p>Os comandos acima implementam uma instância do Milvus com seus componentes e dependências usando configurações padrão. Para personalizar estas definições, recomendamos que utilize a <a href="https://milvus.io/tools/sizing">Milvus Sizing Tool</a> para ajustar as configurações com base no tamanho real dos seus dados e, em seguida, transfira o ficheiro YAML correspondente. Para saber mais sobre os parâmetros de configuração, consulte a <a href="https://milvus.io/docs/system_configuration.md">Lista de verificação das configurações do sistema Milvus</a>.</p>
<div class="alert note">
  <ul>
...
<button class="copy-code-btn"></button></code></pre>
<p>Você também pode acessar a WebUI do Milvus em <code translate="no">http://127.0.0.1:9091/webui/</code> para saber mais sobre a instância do Milvus. Para obter detalhes, consulte <a href="/docs/pt/v2.6.x/milvus-webui.md">Milvus WebUI</a>.</p>
...
<h2 id="Access-Milvus-WebUI" class="common-anchor-header">Acesso à WebUI do Milvus<button data-href="#Access-Milvus-WebUI" class="anchor-icon" translate="no">
    ...
    </button></h2><p>O Milvus vem com uma ferramenta GUI embutida chamada Milvus WebUI que pode ser acessada através do seu navegador. A Milvus Web UI melhora a observabilidade do sistema com uma interface simples e intuitiva. Pode utilizar a Milvus Web UI para observar as estatísticas e métricas dos componentes e dependências do Milvus, verificar detalhes da base de dados e da coleção, e listar configurações detalhadas do Milvus. Para obter detalhes sobre a Milvus Web UI, consulte <a href="/docs/pt/v2.6.x/milvus-webui.md">Milvus WebUI</a></p>
...
<ul>
<li><p>Verificar o <a href="/docs/pt/v2.6.x/quickstart.md">Hello Milvus</a> para ver o que o Milvus pode fazer.</p></li>
<li><p>Aprender as operações básicas do Milvus:</p>
<ul>
<li><a href="/docs/pt/v2.6.x/manage_databases.md">Gerenciar bancos de dados</a></li>
<li><a href="/docs/pt/v2.6.x/manage-collections.md">Gerenciar coleções</a></li>
...
<li><p><a href="/docs/pt/v2.6.x/upgrade_milvus_cluster-helm.md">Atualizar o Milvus usando o Helm Chart</a>.</p></li>
<li><p><a href="/docs/pt/v2.6.x/scaleout.md">Escalar seu cluster Milvus</a>.</p></li>
...
<li><p>Explore <a href="/docs/pt/v2.6.x/milvus_backup_overview.md">o Milvus Backup</a>, uma ferramenta de código aberto para backups de dados do Milvus.</p></li>
<li><p>Explore o <a href="/docs/pt/v2.6.x/birdwatcher_overview.md">Birdwatcher</a>, uma ferramenta de código aberto para depuração do Milvus e actualizações de configuração dinâmica.</p></li>
...
<li><p><a href="/docs/pt/v2.6.x/monitor.md">Monitore o Milvus com o Prometheus</a>.</p></li>
</ul>
