...
summary: >-
  Erfahren Sie, wie Sie Milvus als Standalone-Version mit einem vorgefertigten
...
<h2 id="Download-the-RPMDEB-Package" class="common-anchor-header">RPM-/DEB-Paket herunterladen<button data-href="#Download-the-RPMDEB-Package" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Sie können das RPM-/DEB-Paket entsprechend Ihrer Systemarchitektur von der <a href="https://github.com/milvus-io/milvus/releases/tag/v2.6.24">Seite „Milvus Releases“</a> herunterladen.</p>
<ul>
<li>Für x86_64/amd64 laden Sie das Paket <strong>„milvus_2.6.24-1_amd64.deb“</strong> oder <strong>„milvus_2.6.24-1_amd64.rpm“</strong> herunter.</li>
<li>Für ARM64 laden Sie das Paket <strong>„milvus_2.6.24-1_arm64.deb“</strong> oder <strong>„milvus_2.6.24-1_arm64.rpm“</strong> herunter.</li>
</ul>
<p>Der folgende Befehl geht davon aus, dass Sie Milvus Standalone auf einem x86_64/amd64-Rechner ausführen werden.</p>
<pre><code translate="no" class="language-shell">wget https://github.com/milvus-io/milvus/releases/download/v2.6.24/milvus_2.6.24-1_amd64.rpm -O milvus_2.6.24-1_amd64.rpm
...
<pre><code translate="no" class="language-shell">yum install -y ./milvus_2.6.24-1_amd64.rpm
rpm -qa| grep milvus
<button class="copy-code-btn"></button></code></pre>
<p>Bei DEB-basierten Systemen (wie Ubuntu und Debian) installieren Sie das Paket mit dem Befehl „ <code translate="no">apt</code> “.</p>
<pre><code translate="no" class="language-shell">apt install -y  ./milvus_2.6.24-1_amd64.deb
dpkg -l | grep milvus
<button class="copy-code-btn"></button></code></pre>
...
<h2 id="Optional-Update-Milvus-configurations" class="common-anchor-header">(Optional) Milvus-Konfigurationen aktualisieren<button data-href="#Optional-Update-Milvus-configurations" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Sie können die Milvus-Konfigurationen in der Datei <code translate="no">/etc/milvus/configs/milvus.yaml</code> ändern. Um beispielsweise die URL von <code translate="no">proxy.healthCheckTimeout</code> auf <code translate="no">1000</code> ms zu ändern, suchen Sie nach dem Parameter „target“ und passen Sie ihn entsprechend an. Informationen zu den entsprechenden Konfigurationselementen finden Sie unter <a href="/docs/de/v2.6.x/system_configuration.md">„Systemkonfiguration</a>“.</p>
...
<li><p>Entdecken Sie <a href="/docs/de/v2.6.x/birdwatcher_overview.md">„Birdwatcher“</a>, ein Open-Source-Tool zum Debuggen von Milvus und für dynamische Konfigurationsaktualisierungen.</p></li>
