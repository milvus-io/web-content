...
<h1 id="Upgrade-Milvus-Cluster-with-Milvus-Operator" class="common-anchor-header">Milvus Cluster mit Milvus Operator aufrüsten<button data-href="#Upgrade-Milvus-Cluster-with-Milvus-Operator" class="anchor-icon" translate="no">
    ...
    </button></h1><p>Diese Anleitung beschreibt, wie Sie Ihren Milvus-Cluster mit Milvus Operator aktualisieren können.</p>
<h2 id="Before-you-start" class="common-anchor-header">Bevor Sie beginnen<button data-href="#Before-you-start" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Ab Milvus 2.6.0 wurden die alten separaten Koordinatoren (<code translate="no">dataCoord</code>, <code translate="no">queryCoord</code>, <code translate="no">indexCoord</code>) zu einem einzigen <code translate="no">mixCoord</code> zusammengefasst. Vergewissern Sie sich vor dem Upgrade, dass Ihre CRD-Spezifikation <code translate="no">mixCoord</code> und nicht einzelne Koordinatorenkomponenten verwendet.</p>
<p>Wenn Sie die separaten Koordinatoren verwenden, ändern Sie Ihre Spezifikation:</p>
...
<h2 id="Upgrade-your-Milvus-operator" class="common-anchor-header">Aktualisieren Sie Ihren Milvus-Operator<button data-href="#Upgrade-your-Milvus-operator" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Führen Sie den folgenden Befehl aus, um die Version Ihres Milvus-Operators auf v1.3.0-rc1-hotfix zu aktualisieren.</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>Nachdem Sie Ihren Milvus-Operator auf die neueste Version aktualisiert haben, haben Sie die folgenden Möglichkeiten:</p>
<ul>
<li>Um Milvus von v2.2.3 zu aktualisieren, können Sie <a href="#Conduct-a-rolling-upgrade">ein Rolling Upgrade durchführen</a>.</li>
<li>Um Milvus von einem Minor-Release vor v2.2.3 auf 2.6.0-rc1 zu aktualisieren, sollten Sie <a href="#Upgrade-Milvus-by-changing-its-image">Milvus aktualisieren, indem Sie seine Image-Version ändern</a>.</li>
<li>Um Milvus von v2.1.x auf 2.6.0-rc1 zu aktualisieren, müssen Sie <a href="#Migrate-the-metadata">die Metadaten</a> vor dem eigentlichen Upgrade <a href="#Migrate-the-metadata">migrieren</a>.</li>
</ul>
<blockquote>
<p><strong>Hinweis</strong>: Es wird dringend empfohlen, jeweils eine Unterversion zu aktualisieren und die letzte stabile Version dieser Unterversion zu verwenden. Wenn Sie beispielsweise von v2.4.x auf v2.6.x aktualisieren, sollten Sie zuerst auf die neueste Version v2.4.x, dann auf die neueste Version v2.5.x und schließlich auf v2.6.x aktualisieren. Dadurch wird sichergestellt, dass Sie die neueste stabile Version jeder Unterversion verwenden, die mit größerer Wahrscheinlichkeit mit Ihren vorhandenen Daten und Konfigurationen kompatibel ist.</p>
</blockquote>
<h2 id="Conduct-a-rolling-upgrade" class="common-anchor-header">Führen Sie ein rollendes Upgrade durch<button data-href="#Conduct-a-rolling-upgrade" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
