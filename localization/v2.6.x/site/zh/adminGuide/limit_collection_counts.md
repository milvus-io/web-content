...
<p>要更改 Collections 限制，需要将三个参数一起修改：</p>
<table>
<thead>
<tr><th>参数</th><th>参数</th><th>默认值</th></tr>
</thead>
<tbody>
<tr><td><code translate="no">rootCoord.maxGeneralCapacity</code></td><td>当前实例可容纳的最大集合单元数（分片×分区）。</td><td><code translate="no">65536</code></td></tr>
<tr><td><code translate="no">quotaAndLimits.limits.maxCollectionNum</code></td><td>当前实例中所有数据库允许的最大集合数。</td><td><code translate="no">65536</code></td></tr>
<tr><td><code translate="no">quotaAndLimits.limits.maxCollectionNumPerDB</code></td><td>单个数据库中允许的最大集合数。</td><td><code translate="no">65536</code></td></tr>
</tbody>
</table>
<p>例如，将限制增加到 200,000 个 Collections：</p>
...
<p>只设置<code translate="no">maxGeneralCapacity</code> 而不同时调整<code translate="no">maxCollectionNum</code> 和<code translate="no">maxCollectionNumPerDB</code> 将不会生效。必须将所有三个参数设置为相同值或更高，才能提高 Collections 限制。</p>
</div>
<h2 id="Calculating-the-number-of-collections" class="common-anchor-header">计算 Collection 数量<button data-href="#Calculating-the-number-of-collections" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
