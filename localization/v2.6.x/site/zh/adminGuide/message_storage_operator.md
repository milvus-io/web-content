...
<h2 id="Before-you-begin" class="common-anchor-header">开始之前<button data-href="#Before-you-begin" class="anchor-icon" translate="no">
    ...
    </button></h2><p>下表展示了 Milvus Standalone 和集群模式下是否支持 RocksMQ、Pulsar、Kafka 和 Woodpecker。</p>
<table>
<thead>
...
<p>指定消息存储时还存在其他限制：</p>
<ul>
<li>每个 Milvus 实例仅支持一个消息存储。不过，对于为单个实例配置多个消息存储的情况，我们仍保持向后兼容性。优先级如下：
<ul>
...
<li><strong>升级限制</strong>：<strong>消息队列限制</strong>：升级至 Milvus v2.6.17 时，必须保留当前选择的消息队列。升级过程中不支持在不同的消息队列系统之间切换。未来版本将支持更改消息队列系统。</li>
</ul>
...
<h2 id="Configure-Woodpecker" class="common-anchor-header">配置 Woodpecker<button data-href="#Configure-Woodpecker" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Woodpecker 是一款专为对象存储设计的云原生预写日志（WAL）。它具备高吞吐量、低运维开销和无缝可扩展性。有关更多详细信息，请参阅《<a href="/docs/zh/v2.6.x/use-woodpecker.md">使用 Woodpecker</a>》。</p>
<h2 id="Configure-Pulsar" class="common-anchor-header">配置 Pulsar<button data-href="#Configure-Pulsar" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Pulsar 管理最近的变更日志、输出流日志并提供日志订阅。在 Milvus Standalone 和 Milvus 集群模式下均支持将 Pulsar 配置为消息存储。但若使用 Milvus Operator，则仅能将 Pulsar 配置为 Milvus 集群的消息存储。请在<code translate="no">spec.dependencies.pulsar</code> 下添加必填字段以配置 Pulsar。</p>
...
<ul>
<li><code translate="no">external</code>:<code translate="no">true</code> 的值表示 Milvus 使用外部 Pulsar 服务。</li>
<li><code translate="no">endpoints</code>: Pulsar 的端点。</li>
</ul>
<h4 id="Example" class="common-anchor-header">示例</h4><p>以下示例配置了外部 Pulsar 服务。</p>
...
<span class="hljs-attr">spec:</span> 
  ...
  <span class="hljs-attr">config:</span> {}
<button class="copy-code-btn"></button></code></pre>
<p><a href="https://artifacthub.io/packages/helm/bitnami/kafka">点击此处</a>查看配置内部 Kafka 服务的完整配置项。请根据需要，在<code translate="no">kafka.inCluster.values</code> 下添加配置项。</p>
