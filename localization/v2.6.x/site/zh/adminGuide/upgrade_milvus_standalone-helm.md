...
自 Milvus Helm 图表版本 4.2.21 起，我们引入了 pulsar-v3.x 图表作为依赖项。为确保向后兼容性，请将您的 Helm 升级至 v3.14 或更高版本，并且在使用 `<code translate="no">helm upgrade</code>` 时，请务必添加 `<code translate="no">--reset-then-reuse-values</code> ` 选项。
</div>
...
<h3 id="Step-3-Upgrade-to-v2617" class="common-anchor-header">步骤 3：升级至 v2.6.17<button data-href="#Step-3-Upgrade-to-v2617" class="anchor-icon" translate="no">
    ...
    </button></h3><p>待 v2.5.16 成功运行后，请升级至 v2.6.17：</p>
<pre><code translate="no" class="language-bash">helm upgrade my-release zilliztech/milvus \
