...
<h1 id="Upgrade-Milvus-Standalone-with-Helm-Chart" class="common-anchor-header">使用 Helm Chart 升级 Milvus 单机版<button data-href="#Upgrade-Milvus-Standalone-with-Helm-Chart" class="anchor-icon" translate="no">
    ...
    </button></h1><p>本指南介绍如何使用 Helm Chart 将 Milvus 单机版从 v2.5.x 升级到 v2.6.14。</p>
...
<p><strong>消息队列限制</strong>：升级到 Milvus v2.6.14 时，您必须保持当前的消息队列选择。不支持在升级过程中在不同的消息队列系统之间切换。未来版本将支持更改消息队列系统。</p>
...
kubectl get pods
<button class="copy-code-btn"></button></code></pre>
<h3 id="Step-3-Upgrade-to-v2614" class="common-anchor-header">步骤 3：升级到 v2.6.14<button data-href="#Step-3-Upgrade-to-v2614" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
