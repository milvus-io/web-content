...
<h1 id="Upgrade-Milvus-Cluster-with-Milvus-Operator" class="common-anchor-header">使用 Milvus Operator 升级 Milvus 群集<button data-href="#Upgrade-Milvus-Cluster-with-Milvus-Operator" class="anchor-icon" translate="no">
    ...
    </button></h1><p>本指南介绍如何使用 Milvus Operator 将 Milvus 群集从 v2.5.x 升级到 v2.6.15。</p>
<h2 id="Before-you-start" class="common-anchor-header">开始之前<button data-href="#Before-you-start" class="anchor-icon" translate="no">
    ...
    </button></h2><h3 id="Whats-new-in-v2615" class="common-anchor-header">v2.6.15 中的新功能<button data-href="#Whats-new-in-v2615" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>从 Milvus 2.5.x 升级到 2.6.15 涉及重大架构更改：</p>
<ul>
...
<ul>
<li>Milvus v2.6.0-rc1 与 v2.6.15<strong>不兼容</strong>。不支持从候选版本直接升级。</li>
...
<li>在升级到 v2.6.15 之前，您<strong>必须</strong>升级到 v2.5.16 或更高版本，并启用<code translate="no">mixCoord</code> 。</li>
</ul>
<p><strong>消息队列限制</strong>：升级到 Milvus v2.6.15 时，您必须保持当前的消息队列选择。不支持在升级过程中在不同的消息队列系统之间切换。未来版本将支持更换消息队列系统。</p>
...
kubectl get pods
<button class="copy-code-btn"></button></code></pre>
<h4 id="23-Upgrade-to-v2615" class="common-anchor-header">2.3 升级至 v2.6.15</h4><p>当<code translate="no">mixCoord</code> 成功运行 v2.5.16 后，升级到 v2.6.15：</p>
