...
<h1 id="Upgrade-Milvus-Cluster-with-Milvus-Operator" class="common-anchor-header">使用 Milvus Operator 升级 Milvus 群集<button data-href="#Upgrade-Milvus-Cluster-with-Milvus-Operator" class="anchor-icon" translate="no">
    ...
    </button></h1><p>本指南介绍如何使用 Milvus Operator 将 Milvus 群集从 v2.5.x 升级到 v2.6.17。</p>
<h2 id="Before-you-start" class="common-anchor-header">开始之前<button data-href="#Before-you-start" class="anchor-icon" translate="no">
    ...
    </button></h2><h3 id="Whats-new-in-v2617" class="common-anchor-header">v2.6.17 中的新功能<button data-href="#Whats-new-in-v2617" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>从 Milvus 2.5.x 升级到 2.6.17 涉及重大架构更改：</p>
<ul>
...
<p>此升级过程可确保向新架构的适当迁移。有关架构变化的更多信息，请参阅<a href="/docs/zh/v2.6.x/architecture_overview.md">Milvus 架构概述</a>。</p>
...
<li>在升级到 v2.6.17 之前，您<strong>必须</strong>升级到启用了<code translate="no">mixCoord</code> 的 v2.5.16 或更高版本。</li>
</ul>
<p><strong>消息队列限制</strong>：升级到 Milvus v2.6.17 时，您必须保持当前的消息队列选择。不支持在升级过程中在不同的消息队列系统之间切换。未来版本将支持更改消息队列系统。</p>
...
kubectl get pods
<button class="copy-code-btn"></button></code></pre>
<h4 id="23-Upgrade-to-v2617" class="common-anchor-header">2.3 升级至 v2.6.17</h4><p>v2.5.16 成功运行<code translate="no">mixCoord</code> 后，升级到 v2.6.17：</p>
