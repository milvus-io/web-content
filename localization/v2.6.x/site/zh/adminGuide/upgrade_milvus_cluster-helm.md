...
<h1 id="Upgrade-Milvus-Cluster-with-Helm-Chart" class="common-anchor-header">使用 Helm 图表升级 Milvus 集群<button data-href="#Upgrade-Milvus-Cluster-with-Helm-Chart" class="anchor-icon" translate="no">
    ...
    </button></h1><p>本指南介绍如何使用 Helm Chart 将 Milvus 集群从 v2.5.x 升级到 v2.6.8。</p>
<h2 id="Before-you-start" class="common-anchor-header">开始之前<button data-href="#Before-you-start" class="anchor-icon" translate="no">
    ...
    </button></h2><h3 id="Whats-new-in-v268" class="common-anchor-header">版本 2.6.8 的新功能<button data-href="#Whats-new-in-v268" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<li>在升级到 v2.6.8 之前，您<strong>必须</strong>升级到 v2.5.16 或更高版本，并启用<code translate="no">mixCoordinator</code> 。</li>
</ul>
<p><strong>消息队列限制</strong>：升级到 Milvus v2.6.8 时，您必须保持当前的消息队列选择。不支持在升级过程中在不同的消息队列系统之间切换。未来版本将支持更改消息队列系统。</p>
...
kubectl get pods
<button class="copy-code-btn"></button></code></pre>
<h3 id="Step-3-Upgrade-to-v268" class="common-anchor-header">步骤 3：升级到 v2.6.8<button data-href="#Step-3-Upgrade-to-v268" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>在<code translate="no">mixCoordinator</code> 上成功运行 v2.5.16 后，升级到 v2.6.8：</p>
<pre><code translate="no" class="language-bash">helm upgrade my-release zilliztech/milvus \
