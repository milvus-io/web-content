...
<h1 id="Upgrade-Milvus-Cluster-with-Helm-Chart" class="common-anchor-header">使用 Helm 图表升级 Milvus 集群<button data-href="#Upgrade-Milvus-Cluster-with-Helm-Chart" class="anchor-icon" translate="no">
    ...
    </button></h1><p>本指南介绍了如何使用 Helm Chart 将您的 Milvus 集群从 v2.5.x 升级至 v2.6.24。</p>
<h2 id="Before-you-start" class="common-anchor-header">开始之前<button data-href="#Before-you-start" class="anchor-icon" translate="no">
    ...
    </button></h3><p>从 Milvus 2.5.x 升级至 2.6.24 涉及重大的架构变更：</p>
<ul>
...
<ul>
<li>Milvus v2.6.0-rc1 与 v2.6.24<strong>不兼容</strong>。不支持从候选版本直接升级。</li>
...
<li>在升级至 v2.6.24 之前，您<strong>必须先</strong>升级至 v2.5.16 或更高版本，并启用<code translate="no">mixCoordinator</code> 功能。</li>
</ul>
<p><strong>消息队列限制</strong>：升级至 Milvus v2.6.24 时，您必须保留当前的消息队列选择。升级过程中不支持在不同的消息队列系统之间切换。未来版本将支持更改消息队列系统。</p>
<div class="alert note">
自 Milvus Helm 图表版本 4.2.21 起，我们引入了 pulsar-v3.x 图表作为依赖项。为确保向后兼容性，请将您的 Helm 升级至 v3.14 或更高版本，并在使用 `<code translate="no">helm upgrade</code>` 时务必添加 `<code translate="no">--reset-then-reuse-values</code> ` 选项。
</div>
...
<button class="copy-code-btn"></button></code></pre>
<p>本指南假设您正在安装最新版本。若需安装特定版本，请相应地指定<code translate="no">--version</code> 参数。</p>
...
kubectl get pods
<button class="copy-code-btn"></button></code></pre>
<h3 id="Step-3-Upgrade-to-v2624" class="common-anchor-header">步骤 3：升级至 v2.6.24<button data-href="#Step-3-Upgrade-to-v2624" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>当 v2.5.16 在<code translate="no">mixCoordinator</code> 上成功运行后，请升级至 v2.6.24：</p>
<pre><code translate="no" class="language-bash">helm upgrade my-release zilliztech/milvus \
