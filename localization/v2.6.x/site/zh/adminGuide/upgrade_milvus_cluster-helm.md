...
<h1 id="Upgrade-Milvus-Cluster-with-Helm-Chart" class="common-anchor-header">使用 Helm 图表升级 Milvus 集群<button data-href="#Upgrade-Milvus-Cluster-with-Helm-Chart" class="anchor-icon" translate="no">
    ...
    </button></h1><p>本指南介绍了如何使用 Helm 图表将您的 Milvus 集群从 v2.5.x 升级到 v2.6.23。</p>
<h2 id="Before-you-start" class="common-anchor-header">开始之前<button data-href="#Before-you-start" class="anchor-icon" translate="no">
    ...
    </button></h3><p>从 Milvus 2.5.x 升级到 2.6.23 涉及重大的架构变更：</p>
<ul>
...
<li><strong>组件移除</strong>：移除并整合了<code translate="no">indexNode</code> </li>
</ul>
...
<li>通过 Helm Chart 部署的 Milvus 集群</li>
</ul>
<p><strong>兼容性要求：</strong></p>
<ul>
<li>Milvus v2.6.0-rc1 与 v2.6.23<strong>不兼容</strong>。不支持直接从候选版本（RC）进行升级。</li>
<li>如果您当前正在运行 v2.6.0-rc1 且需要保留数据，请参考<a href="https://github.com/milvus-io/milvus/issues/43538#issuecomment-3112808997">此社区指南</a>以获取迁移帮助。</li>
<li>在升级至 v2.6.23 之前，您<strong>必须先</strong>升级至 v2.5.16 或更高版本，并启用 `<code translate="no">mixCoordinator</code> `。</li>
</ul>
<p><strong>消息队列限制</strong>：升级至 Milvus v2.6.23 时，您必须保留当前的消息队列选择。升级过程中不支持在不同的消息队列系统之间切换。未来版本将支持更改消息队列系统。</p>
<div class="alert note">
自 Milvus Helm 图表版本 4.2.21 起，我们引入了 pulsar-v3.x 图表作为依赖项。为确保向后兼容性，请将 Helm 升级至 v3.14 或更高版本，并且每次使用 `<code translate="no">helm upgrade</code>` 时，请务必添加 `<code translate="no">--reset-then-reuse-values</code> ` 选项。
</div>
<h2 id="Upgrade-process" class="common-anchor-header">升级流程<button data-href="#Upgrade-process" class="anchor-icon" translate="no">
    ...
    </button></h3><p>首先，将您的 Milvus Helm 图表升级至 5.0.22 版本：</p>
...
<p>要检查 Helm 图与 Milvus 版本的兼容性：</p>
<pre><code translate="no" class="language-bash">helm search repo zilliztech/milvus --versions
<button class="copy-code-btn"></button></code></pre>
<p>本指南假设您正在安装最新版本。如果您需要安装特定版本，请相应地指定<code translate="no">--version</code> 参数。</p>
<h3 id="Step-2-Upgrade-to-v2516-with-mixCoordinator" class="common-anchor-header">步骤 2：使用 mixCoordinator 升级至 v2.5.16<button data-href="#Step-2-Upgrade-to-v2516-with-mixCoordinator" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>检查您的集群当前是否使用独立的协调器：</p>
<pre><code translate="no" class="language-bash">kubectl get pods
<button class="copy-code-btn"></button></code></pre>
<p>若看到独立的协调器 Pod（<code translate="no">datacoord</code> 、<code translate="no">querycoord</code> 、<code translate="no">indexcoord</code> ），请升级至 v2.5.16 并启用<code translate="no">mixCoordinator</code> ：</p>
...
<button class="copy-code-btn"></button></code></pre>
<div class="alert-note">
<p>如果您的集群已使用<code translate="no">mixCoordinator</code> ，只需升级镜像：</p>
<pre><code translate="no" class="language-bash">helm upgrade my-release zilliztech/milvus \
  --<span class="hljs-built_in">set</span> image.all.tag=<span class="hljs-string">&quot;v2.5.16&quot;</span> \
  --reset-then-reuse-values \
  --version=4.2.58
...
kubectl get pods
<button class="copy-code-btn"></button></code></pre>
<h3 id="Step-3-Upgrade-to-v2623" class="common-anchor-header">步骤 3：升级到 v2.6.23<button data-href="#Step-3-Upgrade-to-v2623" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>当 v2.5.16 成功运行并启用<code translate="no">mixCoordinator</code> 后，请升级至 v2.6.23：</p>
<pre><code translate="no" class="language-bash">helm upgrade my-release zilliztech/milvus \
  ...
  --version=5.0.22
<button class="copy-code-btn"></button></code></pre>
<h2 id="Verify-the-upgrade" class="common-anchor-header">验证升级情况<button data-href="#Verify-the-upgrade" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
