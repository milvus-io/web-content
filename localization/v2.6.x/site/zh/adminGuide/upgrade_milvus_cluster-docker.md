...
---
<div class="tab-wrapper"><a href="/docs/zh/v2.6.x/upgrade_milvus_standalone-operator.md" class=''>Milvus</a><a href="/docs/zh/v2.6.x/upgrade_milvus_cluster-operator.md" class=''>OperatorMilvus</a><a href="/docs/zh/v2.6.x/configure_operator.md" class=''>OperatorMilvus</a><a href="/docs/zh/v2.6.x/configure-docker.md" class=''>OperatorHelmDocker</a><a href="/docs/zh/v2.6.x/upgrade_milvus_standalone-docker.md" class=''>ComposeHelmDocker</a><a href="/docs/zh/v2.6.x/upgrade_milvus_cluster-helm.md" class=''>ComposeHelm</a></div>
<h1 id="Upgrade-Milvus-Cluster-with-Docker-Compose" class="common-anchor-header">使用 Docker Compose 升级 Milvus 集群<button data-href="#Upgrade-Milvus-Cluster-with-Docker-Compose" class="anchor-icon" translate="no">
    ...
    </button></h1><p>本主题介绍如何使用 Docker Compose 升级您的 Milvus。</p>
<p>通常情况下，您<a href="#Upgrade-Milvus-by-changing-its-image">可以通过更换镜像来升级 Milvus</a>。但在从 v2.1.x 升级至 v2.6.20 之前，您需要<a href="#Migrate-the-metadata">先迁移元数据</a>。</p>
<div class="alert note">
<p><strong>消息队列限制</strong>：升级至 Milvus v2.6.20 时，必须保留当前的消息队列选择。升级过程中不支持在不同的消息队列系统之间切换。未来版本将支持更改消息队列系统。</p>
</div>
<h2 id="Upgrade-Milvus-by-changing-its-image" class="common-anchor-header">通过更换镜像升级 Milvus<button data-href="#Upgrade-Milvus-by-changing-its-image" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>通常情况下，您可以按照以下步骤升级 Milvus：</p>
<ol>
<li><p>在<code translate="no">docker-compose.yaml</code> 中修改 Milvus 镜像标签。</p>
<p>请注意，您需要修改代理节点、所有协调器以及所有工作节点的镜像标签。</p>
...
<button class="copy-code-btn"></button></code></pre></li>
<li><p>使用新的 Milvus 镜像重新启动 Milvus 组件。</p>
