...
<p>Rerankers 通常在第一阶段检索后使用，一般通过向量近似近邻（ANN）技术完成。虽然 ANN 搜索能高效地获取大量潜在的相关结果，但它们可能并不总是根据与查询的实际语义接近程度来确定结果的优先级。在这里，Rerankers 利用更深入的上下文分析来优化结果顺序，通常利用先进的机器学习模型，如 BERT 或其他基于 Transformer 的模型。通过这样做，Rerankers 可以显著提高呈现给用户的最终结果的准确性和相关性。</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>然后，创建一个<code translate="no">BGERerankFunction</code> 实例用于 Reranker：</p>
