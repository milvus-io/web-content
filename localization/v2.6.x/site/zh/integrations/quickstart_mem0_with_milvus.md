...
<h3 id="Configure-Mem0-with-Milvus" class="common-anchor-header">用 Milvus 配置 Mem0<button data-href="#Configure-Mem0-with-Milvus" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>在本例中，我们将使用 OpenAI 作为 LLM。您应将<a href="https://platform.openai.com/docs/quickstart">api 密钥</a> <code translate="no">OPENAI_API_KEY</code> 设置为环境变量。</p>
...
<h2 id="Managing-User-Memories-with-Mem0-and-Milvus" class="common-anchor-header">使用 Mem0 和 Milvus 管理用户记忆库<button data-href="#Managing-User-Memories-with-Mem0-and-Milvus" class="anchor-icon" translate="no">
    ...
    </button></h2><h3 id="Adding-a-Memory" class="common-anchor-header">添加记忆库<button data-href="#Adding-a-Memory" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p><code translate="no">add</code> 函数将非结构化文本作为内存存储在 Milvus 中，并将其与特定用户和可选元数据关联。</p>
...
<h3 id="Update-a-Memory" class="common-anchor-header">更新记忆<button data-href="#Update-a-Memory" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>我们可以使用<code translate="no">add</code> 函数的返回值检索内存 ID，这样就可以通过<code translate="no">update</code> 用新信息更新内存。</p>
...
<h3 id="Get-All-Memory-For-a-User" class="common-anchor-header">获取用户的所有内存<button data-href="#Get-All-Memory-For-a-User" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>我们可以使用<code translate="no">get_all</code> 函数查看所有插入的内存，或通过 Milvus 中的<code translate="no">user_id</code> 进行筛选。</p>
...
<h3 id="View-Memory-Update-History" class="common-anchor-header">查看记忆更新历史<button data-href="#View-Memory-Update-History" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>我们还可以通过<code translate="no">history</code> 函数指定我们感兴趣的内存_id，查看内存更新历史。</p>
...
<h3 id="Search-Memory" class="common-anchor-header">搜索内存<button data-href="#Search-Memory" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>我们可以使用<code translate="no">search</code> 函数查找与用户最相关的内存。</p>
...
<h3 id="Delete-Memory" class="common-anchor-header">删除内存<button data-href="#Delete-Memory" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>我们还可以通过提供相应的<code translate="no">memory_id</code> 来<code translate="no">delete</code> 内存。</p>
