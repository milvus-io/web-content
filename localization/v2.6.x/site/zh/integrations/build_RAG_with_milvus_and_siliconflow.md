...
<button class="copy-code-btn"></button></code></pre>
<h3 id="Prepare-the-Embedding-Model" class="common-anchor-header">准备嵌入模型<button data-href="#Prepare-the-Embedding-Model" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>我们初始化一个客户端来准备嵌入模型。SiliconFlow 启用了 OpenAI 风格的 API，您可以稍作调整后使用相同的 API 来调用嵌入模型和 LLM。</p>
...
<p>很好！我们利用 Milvus 和 SiliconFlow 成功构建了一个 RAG 管道。</p>
