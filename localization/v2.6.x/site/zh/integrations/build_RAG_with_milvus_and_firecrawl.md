...
summary: >-
  在本教程中，我们将向您展示如何使用 Milvus 和 Firecrawl 构建检索增强生成（RAG）管道。该管道集成了用于网络数据搜刮的
  Firecrawl、用于向量存储的 Milvus 和用于生成有洞察力的上下文感知响应的 OpenAI。
title: 利用 Milvus 和 Firecrawl 构建 RAG
---
<h1 id="Building-RAG-with-Milvus-and-Firecrawl" class="common-anchor-header">利用 Milvus 和 Firecrawl 构建 RAG<button data-href="#Building-RAG-with-Milvus-and-Firecrawl" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<h3 id="Setting-Up-API-Keys" class="common-anchor-header">设置 API 密钥<button data-href="#Setting-Up-API-Keys" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>要使用 Firecrawl 从指定 URL 抓取数据，需要获取<a href="https://www.firecrawl.dev/">FIRECRAWL_API_KEY</a>，并将其设置为环境变量。此外，在本例中我们将使用 OpenAI 作为 LLM。您也应将<a href="https://platform.openai.com/docs/quickstart">OPENAI_API_KEY</a>设置为环境变量。</p>
...
<h3 id="Prepare-the-LLM-and-Embedding-Model" class="common-anchor-header">准备 LLM 和 Embeddings 模型<button data-href="#Prepare-the-LLM-and-Embedding-Model" class="anchor-icon" translate="no">
    ...
    </button></h3><p>我们初始化 OpenAI 客户端以准备嵌入模型。</p>
...
<h2 id="Scrape-Data-Using-Firecrawl" class="common-anchor-header">使用 Firecrawl 抓取数据<button data-href="#Scrape-Data-Using-Firecrawl" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><h3 id="Initialize-the-Firecrawl-Application" class="common-anchor-header">初始化 Firecrawl 应用程序<button data-href="#Initialize-the-Firecrawl-Application" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>我们将使用<code translate="no">firecrawl</code> 库从指定的 URL 以 markdown 格式抓取数据。首先初始化 Firecrawl 应用程序：</p>
...
<button class="copy-code-btn"></button></code></pre>
<h3 id="Scrape-the-Target-Website" class="common-anchor-header">抓取目标网站<button data-href="#Scrape-the-Target-Website" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>从目标 URL 抓取内容。网站<a href="https://lilianweng.github.io/posts/2023-06-23-agent/">LLM-powered Autonomous Agents</a>深入探讨了使用大型语言模型（LLMs）构建的自主代理系统。我们将利用这些内容构建一个 RAG 系统。</p>
...
<button class="copy-code-btn"></button></code></pre>
<h3 id="Process-the-Scraped-Content" class="common-anchor-header">处理抓取的内容<button data-href="#Process-the-Scraped-Content" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>为了使抓取的内容便于管理，以便插入 Milvus，我们只需使用 "#"来分隔内容，这样就能大致分隔出抓取的标记文件的每个主要部分的内容。</p>
...
<h2 id="Build-RAG" class="common-anchor-header">构建 RAG<button data-href="#Build-RAG" class="anchor-icon" translate="no">
    ...
    </button></h3><p>让我们指定一个关于我们刚刚抓取的网站的查询问题。</p>
