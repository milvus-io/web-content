...
<h3 id="1-Download-and-configure-the-YAML-file" class="common-anchor-header">1. 下载并配置 YAML 文件<button data-href="#1-Download-and-configure-the-YAML-file" class="anchor-icon" translate="no">
    ...
    </button></h3><p>下载 <a href="https://github.com/milvus-io/milvus/releases/download/v2.6.23/milvus-standalone-docker-compose-gpu.yml"><code translate="no">milvus-standalone-docker-compose-gpu.yml</code></a> 并将其手动保存为 docker-compose.yml，或使用以下命令保存。</p>
...
<button class="copy-code-btn"></button></code></pre>
<div class="alert note">
<p>如果无法执行上述命令，请检查系统是否安装了 Docker Compose V1。如果是这种情况，建议您根据<a href="https://docs.docker.com/compose/">本页</a>的说明迁移至 Docker Compose V2。</p>
</div>
<p>Milvus 启动后，</p>
<ul>
...
<li><strong>milvus-minio</strong>容器使用默认认证凭据在本地提供<strong>9090</strong>和<strong>9091</strong>端口服务，并将数据映射到当前文件夹中的<strong>volumes/minio</strong>。</li>
