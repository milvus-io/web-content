...
<ul>
<li>若要将特定的 GPU 设备分配给 Milvus，请在<code translate="no">standalone</code> 服务的定义中找到 `<code translate="no">deploy.resources.reservations.devices[0].devices_ids</code> ` 字段，并将该字段的值替换为所需 GPU 的 ID。您可以使用 NVIDIA GPU 显示驱动程序中附带的<code translate="no">nvidia-smi</code> 工具来确定 GPU 设备的 ID。Milvus 支持多个 GPU 设备。</li>
</ul>
...
<ul>
<li><strong>milvus-etcd</strong>容器未向主机暴露任何端口，并将数据映射到当前文件夹下的<strong>volumes/etcd</strong>。</li>
...
<li><strong>Milvus Standalone</strong>容器在本地监听<strong>19530</strong>端口，使用默认设置，并将数据映射到当前目录下的<strong>volumes/milvus</strong>卷中。</li>
</ul></li>
</ul>
...
<button class="copy-code-btn"></button></code></pre>
<p>您还可以访问 Milvus WebUI（<code translate="no">http://127.0.0.1:9091/webui/</code> ）以进一步了解您的 Milvus 实例。有关详细信息，请参阅<a href="/docs/zh/v2.6.x/milvus-webui.md">Milvus WebUI</a>。</p>
...
<li><p>查看<a href="/docs/zh/v2.6.x/milvus-webui.md">Milvus WebUI</a>以进一步了解 Milvus 实例。</p></li>
