...
<h1 id="Run-Milvus-in-Kubernetes-with-Helm" class="common-anchor-header">使用 Helm 在 Kubernetes 中运行 Milvus<button data-href="#Run-Milvus-in-Kubernetes-with-Helm" class="anchor-icon" translate="no">
    ...
    </button></h1><p>本页面介绍了如何使用<a href="https://github.com/zilliztech/milvus-helm">Milvus Helm 图表在</a> Kubernetes 中启动 Milvus 实例。</p>
...
<p>以下命令将部署一个针对 v2.6.19 进行过优化设置的 Milvus 集群，并使用 Woodpecker 作为推荐的消息队列：</p>
<pre><code translate="no" class="language-bash">helm install my-release zilliztech/milvus \
  --<span class="hljs-built_in">set</span> image.all.tag=v2.6.19 \
  --<span class="hljs-built_in">set</span> pulsarv3.enabled=<span class="hljs-literal">false</span> \
  --<span class="hljs-built_in">set</span> woodpecker.enabled=<span class="hljs-literal">true</span> \
...
<button class="copy-code-btn"></button></code></pre>
<p><strong>等待所有 Pod 显示“Running”状态。</strong>采用 v2.6.19 配置时，您应看到类似以下的 Pod 状态：</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>此命令将您的本地端口<code translate="no">27017</code> 转发至 Milvus 端口<code translate="no">19530</code> 。您应看到：</p>
...
<li><p>了解<a href="/docs/zh/v2.6.x/birdwatcher_overview.md">Birdwatcher</a>——一款用于调试 Milvus 并动态更新配置的开源工具。</p></li>
