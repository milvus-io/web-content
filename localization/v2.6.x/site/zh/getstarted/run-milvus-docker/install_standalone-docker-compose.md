...
<h2 id="Install-Milvus" class="common-anchor-header">安装 Milvus<button data-href="#Install-Milvus" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Milvus 在其代码仓库中提供了一个 Docker Compose 配置文件。若要使用 Docker Compose 安装 Milvus，只需运行</p>
...
<ul>
<li><strong>架构增强</strong>：新增流式节点并优化了各项组件</li>
<li><strong>更新的依赖项</strong>：包含最新版本的 MinIO 和 etcd</li>
<li><strong>配置优化</strong>：优化了设置以提升性能</li>
</ul>
<p>请务必下载最新的 Docker Compose 配置文件，以确保与 v2.6.23 版本的功能兼容。</p>
<ul>
<li><p>若上述命令执行失败，请检查您的系统是否安装了 Docker Compose V1。如果是这种情况，建议您根据<a href="https://docs.docker.com/compose/">本页</a>说明迁移至 Docker Compose V2。</p></li>
...
<li><strong>milvus-minio</strong>容器在本地监听<strong>9090</strong>和<strong>9091</strong>端口，使用默认认证凭据，并将数据映射到当前文件夹中的<strong>volumes/minio</strong>目录。</li>
<li><strong>Milvus Standalone</strong>容器在本地以默认设置提供<strong>19530</strong>端口服务，并将数据映射到当前目录下的<strong>volumes/milvus</strong>目录中。</li>
</ul></li>
</ul>
...
<button class="copy-code-btn"></button></code></pre>
<p>您还可以访问<code translate="no">http://127.0.0.1:9091/webui/</code> 上的 Milvus WebUI，以进一步了解您的 Milvus 实例。有关详细信息，请参阅<a href="/docs/zh/v2.6.x/milvus-webui.md">Milvus WebUI</a>。</p>
<h2 id="Optional-Update-Milvus-configurations" class="common-anchor-header">（可选）更新 Milvus 配置<button data-href="#Optional-Update-Milvus-configurations" class="anchor-icon" translate="no">
    ...
    </button></h2><p>若要根据您的需求更新 Milvus 配置，需修改<code translate="no">milvus-standalone</code> 容器中的<code translate="no">/milvus/configs/user.yaml</code> 文件。</p>
<ol>
...
<li><p>添加额外配置以覆盖默认设置。
下文假设您需要覆盖默认的<code translate="no">proxy.healthCheckTimeout</code> 。有关适用的配置项，请<a href="/docs/zh/v2.6.x/system_configuration.md">参阅系统配置</a>。</p>
...
<li><p>探索<a href="https://github.com/zilliztech/attu">Attu</a>——一款用于直观管理 Milvus 的开源图形化界面 (GUI) 工具。</p></li>
