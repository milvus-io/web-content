...
<p><strong>版本 2.6.17 中的新功能：</strong></p>
<ul>
<li><strong>流节点</strong>：增强数据处理能力</li>
<li><strong>啄木鸟 MQ</strong>：改进了消息队列，减少了维护开销，详情请参见<a href="/docs/zh/v2.6.x/use-woodpecker.md">使用啄木鸟</a></li>
...
<p>你可以访问 Milvus WebUI，网址是<code translate="no">http://127.0.0.1:9091/webui/</code> ，了解有关 Milvus 实例的更多信息。有关详细信息，请参阅<a href="/docs/zh/v2.6.x/milvus-webui.md">Milvus WebUI</a>。</p>
<h2 id="Optional-Update-Milvus-configurations" class="common-anchor-header">(可选）更新 Milvus 配置<button data-href="#Optional-Update-Milvus-configurations" class="anchor-icon" translate="no">
    ...
    </button></h2><p>您可以修改当前文件夹下<strong>user.yaml</strong>文件中的 Milvus 配置。例如，要将<code translate="no">proxy.healthCheckTimeout</code> 更改为<code translate="no">1000</code> ms，可按如下方式修改文件：</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>有关适用的配置项目，请参阅<a href="/docs/zh/v2.6.x/system_configuration.md">系统配置</a>。</p>
<h2 id="Upgrade-Milvus" class="common-anchor-header">升级 Milvus<button data-href="#Upgrade-Milvus" class="anchor-icon" translate="no">
    ...
    </button></h2><p>您可以使用内置的升级命令升级到最新版本的 Milvus。它会自动下载最新配置和 Milvus 映像：</p>
...
<p>升级命令会自动</p>
<ul>
<li>下载带有更新配置的最新安装脚本</li>
<li>调用最新的 Milvus Docker 映像</li>
<li>使用新版本重启容器</li>
<li>保留现有数据和配置</li>
</ul>
<p>这是升级 Milvus 独立部署的推荐方法。</p>
</div>
<h2 id="Stop-and-delete-Milvus" class="common-anchor-header">停止和删除 Milvus<button data-href="#Stop-and-delete-Milvus" class="anchor-icon" translate="no">
    ...
    </button></h2><p>你可以按以下方式停止和删除该容器</p>
