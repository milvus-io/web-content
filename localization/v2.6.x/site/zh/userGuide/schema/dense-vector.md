...
summary: >-
  ...
  这种表示方式能够有效捕捉复杂的模式和关系，使数据在高维空间中更易于分析和处理。稠密向量通常具有固定的维数，范围从几十维到几百维甚至几千维不等，具体取决于具体的应用场景和要求。
---
<h1 id="Dense-Vector" class="common-anchor-header">稠密向量<button data-href="#Dense-Vector" class="anchor-icon" translate="no">
    ...
    </button></h1><p>稠密向量是机器学习和数据分析中广泛使用的数值数据表示形式。它们由包含实数的数组组成，其中大部分或所有元素均不为零。与稀疏向量相比，在相同维度下，稠密向量包含更多信息，因为每个维度都包含有意义的值。 这种表示方式能够有效捕捉复杂的模式和关系，使数据在高维空间中更易于分析和处理。稠密向量通常具有固定的维度数，范围从几十到几百甚至几千，具体取决于特定的应用和需求。</p>
...
<h2 id="Overview" class="common-anchor-header">概述<button data-href="#Overview" class="anchor-icon" translate="no">
    ...
    </button></h2><p>稠密向量通常以固定长度的浮点数数组形式表示，例如<code translate="no">[0.2, 0.7, 0.1, 0.8, 0.3, ..., 0.5]</code> 。这些向量的维度通常在数百到数千之间，例如128、256、768或1024。 每个维度都捕捉了对象的特定语义特征，从而通过相似度计算使其适用于各种场景。</p>
...
<p>除了稠密向量外，Milvus 还支持稀疏向量和二进制向量。稀疏向量适用于基于特定术语的精确匹配，例如关键词搜索和术语匹配；而二进制向量通常用于高效处理二值化数据，例如图像模式匹配和某些哈希应用。更多信息请参阅<a href="/docs/zh/v2.6.x/binary-vector.md">“二进制向量”</a>和<a href="/docs/zh/v2.6.x/sparse_vector.md">“稀疏向量”</a>。</p>
</div>
<h2 id="Use-dense-vectors" class="common-anchor-header">使用稠密向量<button data-href="#Use-dense-vectors" class="anchor-icon" translate="no">
    ...
    </button></h3><p>要在 Milvus 中使用稠密向量，需在创建 Collection 时首先定义一个用于存储稠密向量的向量字段。该过程包括：</p>
<ol>
...
<table>
   ...
   <tr>
     ...
     <td><p>存储各维度元素均为 8 位整数（int8）的向量，每个元素的取值范围为 –128 到 127。 INT8_VECTOR 专为量化深度学习模型（如 ResNet、EfficientNet）设计，可在最小精度损失的情况下缩减模型大小并加速推理。<br><strong>注意</strong>：此向量类型仅支持 HNSW 索引。</p></td>
   </tr>
</table>
...
<button class="copy-code-btn"></button></code></pre>
<p>在上例中，针对<code translate="no">dense_vector</code> 字段创建了一个名为<code translate="no">dense_vector_index</code> 的索引，并采用<code translate="no">AUTOINDEX</code> 索引类型。将<code translate="no">metric_type</code> 设置为<code translate="no">IP</code> ，表示将使用内积作为距离度量。</p>
<p>Milvus 提供了多种索引类型，以提供更好的向量搜索体验。AUTOINDEX 是一种特殊的索引类型，旨在降低向量搜索的学习门槛。您还可以从众多其他索引类型中进行选择。详情请参阅 xxx。</p>
<p>Milvus 支持其他度量类型。有关更多信息，请参阅《<a href="/docs/zh/v2.6.x/metric.md">度量类型</a>》。</p>
<h3 id="Create-collection" class="common-anchor-header">创建Collection<button data-href="#Create-collection" class="anchor-icon" translate="no">
    ...
    </button></h3><p>完成稠密向量和索引参数的配置后，即可创建包含稠密向量的Collection。下面的示例使用<code translate="no">create_collection</code> 方法创建了一个名为<code translate="no">my_collection</code> 的Collection。</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#javascript">   NodeJS</a>
...
<button class="copy-code-btn"></button></code></pre>
<p>有关相似度搜索参数的更多信息，请参阅《<a href="/docs/zh/v2.6.x/single-vector-search.md">基本人工神经网络搜索》</a>。</p>
