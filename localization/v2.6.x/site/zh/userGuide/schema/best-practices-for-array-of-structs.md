...
<p>Milvus 没有要求用户将原子结构事件转换为平面数据模型，而是引入了结构数组，数组中的每个结构都可以容纳标量和向量，从而保持了语义的完整性，并实现了强大的嵌套过滤和混合搜索功能。</p>
<h2 id="Why-Array-of-Structs" class="common-anchor-header">为什么选择结构数组<button data-href="#Why-Array-of-Structs" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<h3 id="Step-2-Initialize-the-schemas" class="common-anchor-header">步骤 2：初始化 Schema<button data-href="#Step-2-Initialize-the-schemas" class="anchor-icon" translate="no">
    ...
    </button></h3><p>首先，我们需要初始化标题 Struct 和 Collections 的 Schema 模式。</p>
<ul>
...
<li><p>初始化 Collections 的 Schema 模式。</p>
...
<h3 id="Step-3-Set-index-parameters" class="common-anchor-header">步骤 3：设置索引参数<button data-href="#Step-3-Set-index-parameters" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>所有向量字段都必须有索引。要索引元素 Struct 中的向量字段，需要使用<code translate="no">EMB_LIST_HNSW</code> 作为索引类型，并使用<code translate="no">MAX_SIM</code> 度量类型来衡量向量嵌入之间的相似性。</p>
...
<h3 id="Step-4-Create-a-collection" class="common-anchor-header">第 4 步：创建 Collections<button data-href="#Step-4-Create-a-collection" class="anchor-icon" translate="no">
    ...
    </button></h3><p>Schema 和索引准备就绪后，就可以按如下步骤创建目标 Collections：</p>
...
<p>您需要合并这些文件中每个视频片段的数据块并插入数据。以下是合并后的实体，供您参考。</p>
