...
<button class="copy-code-btn"></button></code></pre>
<div class="alert note">
<p>除了插入完整的数组外，在 Milvus v2.6.17 及更高版本中，<code translate="no">ARRAY</code> 字段还支持<code translate="no">upsert</code> API 上的<code translate="no">ARRAY_APPEND</code> 和<code translate="no">ARRAY_REMOVE</code> 部分更新操作符。这些操作符允许您向现有数组追加元素或移除匹配的元素，而无需先检索其当前值，从而避免了客户端的“读取-修改-写入”模式。 有关详细信息，请参阅《<a href="/docs/zh/v2.6.x/upsert-entities.md#Upsert-ARRAY-fields-with-partial-update-operators">使用部分更新操作符对 ARRAY 字段进行 Upsert</a>》。</p>
</div>
