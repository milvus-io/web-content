...
<h1 id="ARRAY-Operators" class="common-anchor-header">ARRAY 操作符<button data-href="#ARRAY-Operators" class="anchor-icon" translate="no">
    ...
    </button></h1><p>Milvus 提供了 ARRAY 运算符，用于过滤 ARRAY 字段以及部分更新 ARRAY 字段的值。</p>
...
<tbody>
<tr><td><a href="/docs/zh/v2.6.x/array-operators.md#ARRAYCONTAINS">ARRAY_CONTAINS(标识符, 表达式)</a></td><td>过滤表达式</td><td>检查特定元素是否存在于 ARRAY 字段中。</td></tr>
<tr><td><a href="/docs/zh/v2.6.x/array-operators.md#ARRAYCONTAINSALL">ARRAY_CONTAINS_ALL(标识符, 表达式)</a></td><td>筛选表达式</td><td>检查指定列表中的所有元素是否都存在于 ARRAY 字段中。</td></tr>
<tr><td><a href="/docs/zh/v2.6.x/array-operators.md#ARRAYCONTAINSANY">ARRAY_CONTAINS_ANY(标识符, 表达式)</a></td><td>筛选表达式</td><td>检查指定列表中的任何元素是否存在于 ARRAY 字段中。</td></tr>
<tr><td><a href="/docs/zh/v2.6.x/array-operators.md#ARRAYLENGTH">ARRAY_LENGTH(标识符)</a></td><td>筛选表达式</td><td>返回 ARRAY 字段中的元素个数，可与比较操作符结合用于筛选。</td></tr>
<tr><td><a href="/docs/zh/v2.6.x/array-operators.md#ARRAYAPPEND">ARRAY_APPEND</a></td><td><code translate="no">upsert</code> with<code translate="no">field_ops</code></td><td>将有效载荷元素追加到现有的 ARRAY 字段中。在 Milvus v2.6.17 及更高版本中可用。</td></tr>
<tr><td><a href="/docs/zh/v2.6.x/array-operators.md#ARRAYREMOVE">ARRAY_REMOVE</a></td><td><code translate="no">upsert</code> 与<code translate="no">field_ops</code></td><td>从现有 ARRAY 字段中移除所有与请求有效载荷中某个值匹配的元素。适用于 Milvus v2.6.17 及更高版本。</td></tr>
</tbody>
</table>
...
<h2 id="ARRAYAPPEND--Milvus-2617+" class="common-anchor-header">ARRAY_APPEND<span class="beta-tag" style="background-color:rgb(0, 179, 255);color:white" translate="no">Compatible with Milvus 2.6.17+</span><button data-href="#ARRAYAPPEND--Milvus-2617+" class="anchor-icon" translate="no">
    ...
    </button></h2><p><code translate="no">ARRAY_APPEND</code> 操作符在执行<code translate="no">upsert</code> 请求时，将有效负载元素追加到现有的 ARRAY 字段中。它不是一个过滤表达式。当您希望向数组添加值，而无需先查询当前数组值时，请使用它。</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>通过 `<code translate="no">field_ops</code> ` 将 `<code translate="no">ARRAY_APPEND</code> ` 附加到字段上，可为该字段启用部分更新语义。有关完整工作流、支持的元素类型和限制，请参阅《<a href="/docs/zh/v2.6.x/upsert-entities.md#Upsert-ARRAY-fields-with-partial-update-operators">使用部分更新操作符对 ARRAY 字段进行 Upsert</a>》。</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>通过<code translate="no">field_ops</code> 将<code translate="no">ARRAY_REMOVE</code> 附加到字段上，可为该字段启用部分更新语义。有关完整工作流、支持的元素类型和限制，请参阅《<a href="/docs/zh/v2.6.x/upsert-entities.md#Upsert-ARRAY-fields-with-partial-update-operators">使用部分更新操作符对ARRAY字段进行Upsert</a>》。</p>
