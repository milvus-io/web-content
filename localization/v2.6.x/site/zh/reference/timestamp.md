...
<h3 id="Guaranteetimestamp" class="common-anchor-header"><code translate="no">Guarantee_timestamp</code><button data-href="#Guaranteetimestamp" class="anchor-icon" translate="no">
    ...
    </button></h3><p><code translate="no">Guarantee_timestamp</code> 是一种时间戳类型，用于确保在进行向量相似性搜索或查询时，<code translate="no">Guarantee_timestamp</code> 之前的 DML 操作更新的所有数据都是可见的。例如，如果在下午 3 点插入一批数据，下午 5 点插入另一批数据，而在向量相似性搜索时，<code translate="no">Guarantee_timestamp</code> 的值被设置为下午 6 点。这意味着分别在下午 3 点和 5 点插入的两批数据都应参与搜索。</p>
