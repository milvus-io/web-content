...
<h1 id="Consistency" class="common-anchor-header">一致性<button data-href="#Consistency" class="anchor-icon" translate="no">
    ...
    </button></h1><p>本主题介绍 Milvus 中的四种一致性级别及其最适合的应用场景。本主题还将介绍 Milvus 中确保一致性的机制。</p>
...
<h3 id="Strong" class="common-anchor-header">强<button data-href="#Strong" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>强是最高、最严格的一致性级别。它确保用户可以读取最新版本的数据。</p>
<p>
...
<h3 id="Bounded-staleness" class="common-anchor-header">有界滞后<button data-href="#Bounded-staleness" class="anchor-icon" translate="no">
    ...
    </button></h3><p>有界僵化，顾名思义，允许数据在一定时间内不一致。不过，一般来说，在这段时间之外，数据始终是全局一致的。</p>
<p>
...
<h3 id="Session" class="common-anchor-header">会话<button data-href="#Session" class="anchor-icon" translate="no">
    ...
    </button></h3><p>会话确保所有数据写入都能在同一会话中立即被读取。换句话说，当你通过一个客户端写入数据时，新插入的数据会立即变成可搜索的数据。</p>
<p>
...
<h3 id="Eventually" class="common-anchor-header">最终<button data-href="#Eventually" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>读取和写入的顺序没有保证，在不再进行写操作的情况下，副本最终会收敛到相同的状态。在 "最终 "一致性下，副本会使用最新更新的值开始处理读取请求。最终一致性是四种一致性中最弱的一种。</p>
<p>
...
<li><p><strong>会话</strong>：客户端使用最新写操作的时间戳作为 GuaranteeTs，这样每个客户端至少可以检索到同一客户端插入的数据。</p></li>
