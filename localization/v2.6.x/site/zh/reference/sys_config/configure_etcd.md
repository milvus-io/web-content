...
summary: 了解如何为 Milvus 配置 etcd。
---
<h1 id="etcd-related-Configurations" class="common-anchor-header">etcd 相关配置<button data-href="#etcd-related-Configurations" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h1><p>用于存储 Milvus 元数据和服务发现的 etcd 相关配置。</p>
<h2 id="etcdendpoints" class="common-anchor-header"><code translate="no">etcd.endpoints</code><button data-href="#etcdendpoints" class="anchor-icon" translate="no">
  ...
  <tbody>
    <tr>
      <td>
        <li>用于访问 etcd 服务的端点。可以根据自己的 etcd 集群的端点更改此参数。</li>      
        <li>环境变量：ETCD_ENDPOINTS</li>      
        <li>启动 milvus 时，etcd 优先从环境变量 ETCD_ENDPOINTS 获取有效地址。</li>      </td>
      <td>本地主机：2379</td>
    </tr>
  </tbody>
</table>
  ...
  <tbody>
    <tr>
      <td>
        ...
        <li>要在多个 Milvus 实例之间共享一个 etcd 实例，可考虑在启动每个 Milvus 实例之前将其更改为不同的值。</li>      
        <li>如果已经存在 etcd 服务，为 Milvus 设置一个易于识别的根路径。</li>      
...
<h2 id="etcdmetaSubPath" class="common-anchor-header"><code translate="no">etcd.metaSubPath</code><button data-href="#etcdmetaSubPath" class="anchor-icon" translate="no">
  ...
  <tbody>
    <tr>
      <td>
        <li>Milvus 在 etcd 中存储元数据相关信息的关键字的子前缀。</li>      
...
<h2 id="etcdloglevel" class="common-anchor-header"><code translate="no">etcd.log.level</code><button data-href="#etcdloglevel" class="anchor-icon" translate="no">
  ...
  <tbody>
    <tr>
      <td>        仅支持 debug、info、warn、error、panic 或 fatal。默认为 "info"。      </td>
      <td>info</td>
    </tr>
  </tbody>
</table>
<h2 id="etcdlogpath" class="common-anchor-header"><code translate="no">etcd.log.path</code><button data-href="#etcdlogpath" class="anchor-icon" translate="no">
  ...
  <tbody>
    <tr>
      <td>
        <li>路径为</li>      
        <li>- "default "为 os.Stderr、</li>      
        <li>- "stderr "为 os.Stderr、</li>      
        <li>- "stdout "为 os.Stdout、</li>      
        <li>- 附加服务器日志的文件路径。</li>      
        <li>请在嵌入式 Milvus 中调整：/tmp/milvus/logs/etcd.log</li>      </td>
      <td>stdout</td>
    </tr>
  </tbody>
</table>
...
<h2 id="etcdssltlsMinVersion" class="common-anchor-header"><code translate="no">etcd.ssl.tlsMinVersion</code><button data-href="#etcdssltlsMinVersion" class="anchor-icon" translate="no">
  ...
  <tbody>
    <tr>
      <td>
        <li>TLS 最小版本</li>      
        <li>可选值：1.0, 1.1, 1.2, 1.3。</li>      
        <li>建议使用 1.2 及以上版本。</li>      </td>
      <td>1.3</td>
    </tr>
  </tbody>
</table>
...
<h2 id="etcduseembed" class="common-anchor-header"><code translate="no">etcd.use.embed</code><button data-href="#etcduseembed" class="anchor-icon" translate="no">
  ...
  <tbody>
    <tr>
      <td>        是否启用嵌入式 Etcd（进程内 EtcdServer）。      </td>
      <td>假</td>
    </tr>
  </tbody>
</table>
<h2 id="etcddatadir" class="common-anchor-header"><code translate="no">etcd.data.dir</code><button data-href="#etcddatadir" class="anchor-icon" translate="no">
  ...
  <tbody>
    <tr>
      <td>        仅限嵌入式 Etcd。请在嵌入式 Milvus 中调整：/tmp/milvus/etcdData/      </td>
      <td>default.etcd</td>
    </tr>
  </tbody>
</table>
