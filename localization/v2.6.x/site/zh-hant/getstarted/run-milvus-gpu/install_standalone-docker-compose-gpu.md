...
title: 使用 Docker Compose 執行具備 GPU 支援的 Milvus
---
<h1 id="Run-Milvus-with-GPU-Support-Using-Docker-Compose" class="common-anchor-header">使用 Docker Compose 執行具備 GPU 支援的 Milvus<button data-href="#Run-Milvus-with-GPU-Support-Using-Docker-Compose" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h1><p>本頁面說明如何使用 Docker Compose 啟動具備 GPU 支援的 Milvus 實例。</p>
...
<h3 id="1-Download-and-configure-the-YAML-file" class="common-anchor-header">1. 下載並配置 YAML 檔案<button data-href="#1-Download-and-configure-the-YAML-file" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>下載 <a href="https://github.com/milvus-io/milvus/releases/download/v2.6.24/milvus-standalone-docker-compose-gpu.yml"><code translate="no">milvus-standalone-docker-compose-gpu.yml</code></a> 並手動將其儲存為 docker-compose.yml，或使用以下指令執行。</p>
...
<p>您需要對 YAML 檔案中獨立服務的環境變數進行以下變更：</p>
<ul>
...
<h3 id="2-Start-Milvus" class="common-anchor-header">2. 啟動 Milvus<button data-href="#2-Start-Milvus" class="anchor-icon" translate="no">
    ...
    </button></h3><p>在存放 docker-compose.yml 的目錄中，執行以下命令以啟動 Milvus：</p>
...
<ul>
<li><strong>milvus-etcd</strong>容器不會向主機暴露任何連接埠，並將其資料映射至當前資料夾中的<strong>volumes/etcd</strong>。</li>
...
<li><strong>milvus-standalone</strong>容器以預設設定在本地端提供<strong>19530</strong>埠的服務，並將其資料映射至當前資料夾中的<strong>volumes/milvus</strong>目錄。</li>
</ul></li>
</ul>
...
<button class="copy-code-btn"></button></code></pre>
<p>您也可以透過<code translate="no">http://127.0.0.1:9091/webui/</code> 存取 Milvus WebUI，以進一步了解您的 Milvus 執行個體。詳細資訊請參閱<a href="/docs/zh-hant/v2.6.x/milvus-webui.md">Milvus WebUI</a>。</p>
<p>若您已在 `docker-compose.yml` 中為 Milvus 指派了多個 GPU 裝置，可指定哪些 GPU 裝置對 Milvus 可見或可供使用。</p>
...
<h2 id="Configure-memory-pool" class="common-anchor-header">設定記憶體池<button data-href="#Configure-memory-pool" class="anchor-icon" translate="no">
    ...
    </button></h2><p>在 Milvus 啟動並運行後，您可以透過修改 `<code translate="no">milvus.yaml</code> ` 檔案中的 `<code translate="no">initMemSize</code> ` 和 `<code translate="no">maxMemSize</code> ` 設定來自訂記憶體池。</p>
<div class="alert note">
<p><code translate="no">milvus.yaml</code> 檔案位於 Milvus 容器內的<code translate="no">/milvus/configs/</code> 目錄中。</p>
</div>
<p>若要設定記憶體池，請按照以下方式修改<code translate="no">milvus.yaml</code> 檔案中的<code translate="no">initMemSize</code> 和<code translate="no">maxMemSize</code> 設定。</p>
<ol>
<li><p>請使用以下指令，將 Milvus 容器中的<code translate="no">milvus.yaml</code> 檔案複製到您的本地端機器上。請將<code translate="no">&lt;milvus_container_id&gt;</code> 替換為您實際的 Milvus 容器 ID。</p>
...
<li><p>瀏覽<a href="/docs/zh-hant/v2.6.x/milvus-webui.md">Milvus WebUI</a>以進一步了解該 Milvus 執行個體。</p></li>
...
<li><p>探索<a href="/docs/zh-hant/v2.6.x/birdwatcher_overview.md">Birdwatcher，這</a>是一款用於除錯 Milvus 及動態配置更新的開源工具。</p></li>
<li><p>探索<a href="https://github.com/zilliztech/attu">Attu，這</a>是一款用於直覺式管理 Milvus 的開源圖形使用者介面工具。</p></li>
