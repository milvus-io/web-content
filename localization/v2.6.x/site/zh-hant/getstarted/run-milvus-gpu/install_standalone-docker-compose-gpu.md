...
<h1 id="Run-Milvus-with-GPU-Support-Using-Docker-Compose" class="common-anchor-header">使用 Docker Compose 執行支援 GPU 的 Milvus<button data-href="#Run-Milvus-with-GPU-Support-Using-Docker-Compose" class="anchor-icon" translate="no">
    ...
    </button></h1><p>本頁面說明如何使用 Docker Compose 啟動具有 GPU 支援的 Milvus 實例。</p>
...
<button class="copy-code-btn"></button></code></pre>
<div class="alert note">
<p>若無法執行上述指令，請確認您的系統是否已安裝 Docker Compose V1。若已安裝，建議您根據<a href="https://docs.docker.com/compose/">此頁面中的</a>說明，遷移至 Docker Compose V2。</p>
</div>
<p>啟動 Milvus 後，</p>
<ul>
...
<ol>
<li><p>請使用以下指令，將<code translate="no">milvus.yaml</code> 從 Milvus 容器複製到您的本地端機器。請將<code translate="no">&lt;milvus_container_id&gt;</code> 替換為您實際的 Milvus 容器 ID。</p>
