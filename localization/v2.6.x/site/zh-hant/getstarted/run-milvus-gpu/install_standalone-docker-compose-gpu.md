...
<h3 id="1-Download-and-configure-the-YAML-file" class="common-anchor-header">1. 下載並設定 YAML 檔案<button data-href="#1-Download-and-configure-the-YAML-file" class="anchor-icon" translate="no">
    ...
    </button></h3><p>下載 <a href="https://github.com/milvus-io/milvus/releases/download/v2.6.23/milvus-standalone-docker-compose-gpu.yml"><code translate="no">milvus-standalone-docker-compose-gpu.yml</code></a> 並手動將其儲存為 docker-compose.yml，或使用以下指令執行。</p>
...
<ul>
<li>若要將特定 GPU 裝置指派給 Milvus，請在<code translate="no">standalone</code> 服務的定義中找到 `<code translate="no">deploy.resources.reservations.devices[0].devices_ids</code> ` 欄位，並將其值替換為所需 GPU 的 ID。您可以使用 NVIDIA GPU 顯示驅動程式隨附的 `<code translate="no">nvidia-smi</code> ` 工具來確定 GPU 裝置的 ID。Milvus 支援多個 GPU 裝置。</li>
</ul>
...
<button class="copy-code-btn"></button></code></pre>
<div class="alert note">
<p>若無法執行上述指令，請檢查系統是否已安裝 Docker Compose V1。若已安裝，建議您根據<a href="https://docs.docker.com/compose/">此頁面</a>中的說明，遷移至 Docker Compose V2。</p>
</div>
<p>啟動 Milvus 後，</p>
<ul>
<li>名為<strong>milvus-standalone</strong>、<strong>milvus-minio</strong> 及<strong>milvus-etcd</strong>的容器已啟動。
<ul>
<li><strong>milvus-etcd</strong>容器不會向主機公開任何連接埠，並將其資料映射至當前資料夾中的<strong>volumes/etcd</strong>。</li>
<li><strong>milvus-minio</strong>容器使用預設的認證憑證在本地端提供<strong>9090</strong>和<strong>9091</strong>埠，並將其資料映射至當前資料夾中的<strong>volumes/minio</strong>。</li>
<li><strong>milvus-standalone</strong>容器使用預設設定在本地端提供<strong>19530</strong>埠，並將其資料映射至當前資料夾中的<strong>volumes/milvus</strong>目錄。</li>
</ul></li>
</ul>
...
<p>若要設定記憶體池，請依下述方式修改<code translate="no">milvus.yaml</code> 檔案中的<code translate="no">initMemSize</code> 和<code translate="no">maxMemSize</code> 設定。</p>
<ol>
<li><p>請使用以下命令，將 Milvus 容器中的<code translate="no">milvus.yaml</code> 複製到您的本地機器。請將<code translate="no">&lt;milvus_container_id&gt;</code> 替換為您實際的 Milvus 容器 ID。</p>
