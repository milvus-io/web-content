...
<h3 id="1-Download-and-configure-the-YAML-file" class="common-anchor-header">1. 下載並設定 YAML 檔案<button data-href="#1-Download-and-configure-the-YAML-file" class="anchor-icon" translate="no">
    ...
    </button></h3><p>下載 <a href="https://github.com/milvus-io/milvus/releases/download/v2.6.21/milvus-standalone-docker-compose-gpu.yml"><code translate="no">milvus-standalone-docker-compose-gpu.yml</code></a> 並手動將其儲存為 docker-compose.yml，或使用以下指令執行。</p>
...
<p>若您已在 `docker-compose.yml` 中為 Milvus 指派了多個 GPU 裝置，您可以指定哪個 GPU 裝置對 Milvus 可見或可供使用。</p>
...
<ol>
<li><p>請使用以下指令，將 Milvus 容器中的<code translate="no">milvus.yaml</code> 複製到您的本地機器。請將<code translate="no">&lt;milvus_container_id&gt;</code> 替換為您實際的 Milvus 容器 ID。</p>
