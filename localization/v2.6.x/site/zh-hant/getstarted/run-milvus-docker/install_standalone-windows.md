...
<li><p>執行下載的腳本，以 Docker 容器的形式啟動 Milvus。​</p>
...
<li><p><strong>milvus-standalone</strong>容器使用預設設定在本地端提供<strong>19530</strong>埠，並將其資料映射至當前資料夾中的<strong>volumes/milvus</strong>目錄。​</p></li>
</ul></li>
</ol>
...
<p>若虛擬化功能已停用，您可能需要檢查主機板韌體的 BIOS 設定。在 BIOS 設定中啟用虛擬化功能的方法因主機板廠商而異。以 ASUS<a href="https://www.asus.com/support/faq/1043786/">主機板為例，您可以參考這篇文章來了解</a>如何啟用<a href="https://www.asus.com/support/faq/1043786/">虛擬化功能</a>。​</p>
...
<p>請確保您是以系統管理員模式啟動 Docker Desktop。要執行此操作，請右鍵點擊<strong>Docker</strong>Desktop，然後選擇<strong>「以系統管理員身分執行</strong>」。​</p>
<p><span class="img-wrapper">
...
<h3 id="How-can-I-deal-with-the-volume-related-errors-prompted-during-Milvus-startup-that-reads-Read-config-failed​" class="common-anchor-header">如何處理 Milvus 啟動時出現的與卷相關的錯誤提示，內容為「<code translate="no">Read config failed</code> 」？​<button data-href="#How-can-I-deal-with-the-volume-related-errors-prompted-during-Milvus-startup-that-reads-Read-config-failed​" class="anchor-icon" translate="no">
   ...
   <span>Milvus 啟動時出現「讀取設定失敗」的錯誤提示</span>
  
 </span></p>
<p>若要在 Milvus 啟動時處理「讀取設定檔失敗（<code translate="no">docker exec</code> ）」的錯誤提示，您需要檢查掛載至 Milvus 容器中的卷是否正確。若卷已正確掛載至容器中，您可以使用 ` ` 指令進入容器，並如下所示列出 `<strong>/milvus/configs</strong>` 資料夾：​</p>
<p><span class="img-wrapper">
