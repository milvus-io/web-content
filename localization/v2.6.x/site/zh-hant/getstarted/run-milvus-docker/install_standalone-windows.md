...
<h2 id="Run-Milvus-in-Docker​" class="common-anchor-header">在 Docker 中執行 Milvus​<button data-href="#Run-Milvus-in-Docker​" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Milvus 提供安裝腳本，可將其安裝為 Docker 容器。在 Microsoft Windows 上安裝 Docker Desktop 後，您即可透過 PowerShell 或 Windows 命令提示字元（以<strong>系統管理員身分</strong>執行）以及 WSL 2 存取 Docker CLI。​</p>
...
<li><p>隨 Milvus 一起安裝的嵌入式 etcd 位於同一容器中，並在<strong>2379</strong> 埠提供服務。其配置檔案已映射至當前資料夾中的<strong>embedEtcd.yaml​</strong></p></li>
<li><p>Milvus 資料卷已映射至當前資料夾中的<strong>volumes/milvus</strong>目錄。​</p></li>
</ul>
...
<p>您可以使用以下命令來管理 Milvus 容器及其儲存的資料。​</p>
...
<h2 id="Run-Milvus-with-Docker-Compose​" class="common-anchor-header">使用 Docker Compose 執行 Milvus​<button data-href="#Run-Milvus-with-Docker-Compose​" class="anchor-icon" translate="no">
    ...
    </button></h2><p>在 Microsoft Windows 上安裝 Docker Desktop 後，您即可<strong>在管理員模式下</strong>透過 PowerShell 或 Windows 命令提示字元存取 Docker CLI。您可以在 PowerShell、Windows 命令提示字元或 WSL 2 中執行 Docker Compose 來啟動 Milvus​</p>
<h3 id="From-PowerShell-or-Windows-Command-Prompt​" class="common-anchor-header">透過 PowerShell 或 Windows 命令提示字元​<button data-href="#From-PowerShell-or-Windows-Command-Prompt​" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
...
<li><p>以管理員模式開啟 Docker Desktop，方法是右鍵點擊並選擇「<strong>以管理員身分執行</strong>」。​</p></li>
...
<li><p><strong>milvus-minio</strong>容器在本地端提供<strong>9090</strong>和<strong>9091</strong>埠，使用預設的認證憑證，並將其資料映射至當前資料夾中的<strong>volumes/minio</strong>目錄。​</p></li>
...
<h2 id="FAQs​" class="common-anchor-header">常見問題​<button data-href="#FAQs​" class="anchor-icon" translate="no">
    ...
    </button></h3><p>在 Windows 上安裝 Docker Desktop 後，若電腦設定不當，可能會遇到「<code translate="no">Docker Engine stopped</code> 」錯誤。此情況下，您可能需要進行以下檢查：​</p>
<ol>
<li><p>檢查虛擬化功能是否已啟用。​</p>
<p>您可透過「<strong>工作管理員</strong>」中的「<strong>效能</strong>」索引標籤，確認虛擬化功能是否已啟用。​</p>
...
<p>若虛擬化功能已停用，您可能需要檢查主機板韌體的 BIOS 設定。在 BIOS 設定中啟用虛擬化的方法因主機板廠商而異。以華碩（ASUS）<a href="https://www.asus.com/support/faq/1043786/">主機板為例，您可以參考這篇關於啟用虛擬化的文章</a>。​</p>
...
<p>請確保您已以系統管理員模式啟動 Docker Desktop。要執行此操作，請右鍵點擊<strong>Docker</strong>Desktop，並選擇<strong>「以系統管理員身分執行</strong>」。​</p>
<p><span class="img-wrapper">
...
<p>若要處理 Milvus 啟動時出現的「讀取設定檔失敗（<code translate="no">docker exec</code> ）」錯誤，您需要檢查掛載至 Milvus 容器中的卷是否正確。如果卷已正確掛載至容器中，您可以使用 指令進入容器，並如以下方式列出<strong>/milvus/configs</strong>資料夾：​</p>
<p><span class="img-wrapper">
...
<li><a href="/docs/zh-hant/v2.6.x/insert-update-delete.md">插入、Upsert 及刪除</a></li>
...
<li><p>探索<a href="/docs/zh-hant/v2.6.x/milvus_backup_overview.md">Milvus Backup</a>——一款用於 Milvus 資料備份的開源工具。</p></li>
