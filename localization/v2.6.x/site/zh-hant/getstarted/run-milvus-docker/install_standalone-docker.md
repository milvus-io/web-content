...
<h2 id="Install-Milvus-in-Docker" class="common-anchor-header">在 Docker 中安裝 Milvus<button data-href="#Install-Milvus-in-Docker" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Milvus 提供了一個安裝腳本，可用於將其安裝為 Docker 容器。該腳本可於<a href="https://raw.githubusercontent.com/milvus-io/milvus/master/scripts/standalone_embed.sh">Milvus 儲存庫中</a>取得。若要在 Docker 中安裝 Milvus，只需執行</p>
...
<li>嵌入式 etcd 已與 Milvus 一同安裝在同一容器中，並在<strong>2379</strong> 埠提供服務。其設定檔對應至當前資料夾中的<strong>embedEtcd.yaml</strong>。</li>
...
<p>您可以透過<code translate="no">http://127.0.0.1:9091/webui/</code> 存取 Milvus WebUI，以進一步了解您的 Milvus 執行個體。詳細資訊請參閱<a href="/docs/zh-hant/v2.6.x/milvus-webui.md">Milvus WebUI</a>。</p>
<h2 id="Optional-Update-Milvus-configurations" class="common-anchor-header">（可選）更新 Milvus 設定<button data-href="#Optional-Update-Milvus-configurations" class="anchor-icon" translate="no">
    ...
    </button></h2><p>您可修改當前資料夾中的<strong>user.yaml</strong>檔案來調整 Milvus 設定。例如，若要將<code translate="no">proxy.healthCheckTimeout</code> 變更為<code translate="no">1000</code> ms，請依下列方式修改該檔案：</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>有關適用的配置項目，請參閱《<a href="/docs/zh-hant/v2.6.x/system_configuration.md">系統配置》</a>。</p>
<h2 id="Upgrade-Milvus" class="common-anchor-header">升級 Milvus<button data-href="#Upgrade-Milvus" class="anchor-icon" translate="no">
    ...
    </button></h2><p>您可以使用內建的升級指令將 Milvus 升級至最新版本。此操作會自動下載最新的配置檔與 Milvus 映像檔：</p>
