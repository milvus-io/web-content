...
summary: 了解如何使用 Docker Compose 安裝 Milvus 獨立版。
...
<ul>
<li><p>若您無法執行上述指令，請檢查您的系統是否已安裝 Docker Compose V1。若確實如此，基於<a href="https://docs.docker.com/compose/">本頁面的</a>說明，建議您遷移至 Docker Compose V2。</p></li>
<li><p>若在拉取映像檔時遇到任何問題，請透過<a href="mailto:community@zilliz.com">community@zilliz.com</a>聯絡我們，並提供問題詳情，我們將為您提供必要的支援。</p></li>
</ul>
</div>
<p>啟動 Milvus 後，</p>
<ul>
<li>名為<strong>milvus-standalone</strong>、<strong>milvus-minio</strong> 及<strong>milvus-etcd</strong>的容器將處於運行狀態。
<ul>
<li><strong>milvus-etcd</strong>容器不會向主機公開任何埠，並將其資料映射至當前資料夾中的<strong>volumes/etcd</strong>。</li>
<li><strong>milvus-minio</strong>容器在本地端提供<strong>9090</strong>和<strong>9091</strong>埠口服務，並使用預設的驗證憑證，其資料會映射至當前資料夾中的<strong>volumes/minio</strong>目錄。</li>
<li><strong>milvus-standalone</strong>容器在本地端以預設設定提供<strong>19530</strong>埠，並將其資料映射至當前目錄中的<strong>volumes/milvus</strong>目錄。</li>
</ul></li>
</ul>
...
<li><p>新增額外設定以覆寫預設設定。
以下範例假設您需要覆寫預設的<code translate="no">proxy.healthCheckTimeout</code> 。有關適用的設定項目，請參閱「<a href="/docs/zh-hant/v2.6.x/system_configuration.md">系統設定」</a>。</p>
