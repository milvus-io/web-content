...
summary: 瞭解如何使用預先編譯的 RPM/DEB 套件安裝 Milvus 獨立版本。
...
<h2 id="Download-the-RPMDEB-Package" class="common-anchor-header">下載 RPM/DEB 套件<button data-href="#Download-the-RPMDEB-Package" class="anchor-icon" translate="no">
    ...
    </button></h2><p>您可以從<a href="https://github.com/milvus-io/milvus/releases/tag/v2.6.24">Milvus 發行版頁面</a>，根據您的系統架構下載 RPM/DEB 套件。</p>
<ul>
<li>若為 x86_64/amd64 架構，請下載<strong>milvus_2.6.24-1_amd64.deb</strong>或<strong>milvus_2.6.24-1_amd64.rpm</strong>套件。</li>
<li>若為 ARM64，請下載<strong>milvus_2.6.24-1_arm64.deb</strong>或<strong>milvus_2.6.24-1_arm64.rpm</strong>套件。</li>
</ul>
...
<h2 id="Start-Milvus-Standalone" class="common-anchor-header">啟動 Milvus 獨立模式<button data-href="#Start-Milvus-Standalone" class="anchor-icon" translate="no">
    ...
    </button></h2><p>安裝完成後，Milvus 會以 systemd 服務的形式安裝，並可透過以下命令啟動：</p>
<pre><code translate="no" class="language-shell">systemctl start milvus
<button class="copy-code-btn"></button></code></pre>
<p>您可以使用以下指令檢查 Milvus 服務的狀態：</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>您可以在<code translate="no">/usr/bin/milvus</code> 找到 Milvus 二進位檔、在<code translate="no">/lib/systemd/system/milvus.service</code> 找到 systemd 服務檔案，以及在<code translate="no">/usr/lib/milvus/</code> 找到相關依賴項。</p>
...
<h2 id="Stop-Milvus-Standalone" class="common-anchor-header">停止 Milvus 獨立執行模式<button data-href="#Stop-Milvus-Standalone" class="anchor-icon" translate="no">
    ...
    </button></h2><p>要停止 Milvus 獨立版，請使用以下指令：</p>
...
<li><p>探索<a href="/docs/zh-hant/v2.6.x/birdwatcher_overview.md">Birdwatcher</a>——一款用於 Milvus 除錯與動態配置更新的開源工具。</p></li>
