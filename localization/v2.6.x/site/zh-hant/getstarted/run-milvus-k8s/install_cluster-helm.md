...
<h2 id="Overview" class="common-anchor-header">概述<button data-href="#Overview" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Helm 使用一種稱為「charts」的封裝格式。charts 是一組檔案的集合，用於描述一組相關的 Kubernetes 資源。Milvus 提供了一組 charts，以協助您部署 Milvus 的依賴項和元件。</p>
...
<h2 id="Install-Milvus-Helm-Chart" class="common-anchor-header">安裝 Milvus Helm Chart<button data-href="#Install-Milvus-Helm-Chart" class="anchor-icon" translate="no">
    ...
    </button></h2><p>在安裝 Milvus Helm Charts 之前，您需要先新增 Milvus Helm 儲存庫。</p>
<pre><code translate="no" class="language-bash">helm repo add zilliztech https://zilliztech.github.io/milvus-helm/
<button class="copy-code-btn"></button></code></pre>
<div class="alert note">
<p>位於<code translate="no">https://github.com/milvus-io/milvus-helm</code> 的 Milvus Helm Charts 儲存庫已歸檔。我們現在使用位於<code translate="no">https://github.com/zilliztech/milvus-helm</code> 的新儲存庫。歸檔的儲存庫仍可提供 4.0.31 及更早版本的 Charts，但若需較新版本，請使用新儲存庫。</p>
</div>
...
<p>若您希望以獨立模式（單一節點）部署 Milvus 進行開發或測試，請使用以下指令：</p>
...
<p>以下命令將部署一個針對 v2.6.24 進行過最佳化設定的 Milvus 叢集，並使用 Woodpecker 作為推薦的訊息佇列：</p>
<pre><code translate="no" class="language-bash">helm install my-release zilliztech/milvus \
  --<span class="hljs-built_in">set</span> image.all.tag=v2.6.24 \
  --<span class="hljs-built_in">set</span> pulsarv3.enabled=<span class="hljs-literal">false</span> \
  --<span class="hljs-built_in">set</span> woodpecker.enabled=<span class="hljs-literal">true</span> \
...
<li>停用舊版<strong>索引節點</strong>（相關功能現已由資料節點處理）</li>
...
<li>請參閱《<a href="https://milvus.io/docs/system_configuration.md">Milvus 系統配置檢查清單</a>》以了解進階設定選項</li>
</ul>
<div class="alert note">
<p><strong>重要注意事項：</strong></p>
<ul>
<li><strong>版本命名</strong>：僅可使用字母、數字和連字號（不允許使用點號）</li>
<li><strong>Kubernetes v1.25 及以上版本</strong>：若遇到 PodDisruptionBudget 相關問題，請採用此解決方案：
<pre><code translate="no" class="language-bash">helm install my-release zilliztech/milvus \
...
<h3 id="2-Check-Milvus-cluster-status" class="common-anchor-header">2. 檢查 Milvus 叢集狀態<button data-href="#2-Check-Milvus-cluster-status" class="anchor-icon" translate="no">
    ...
    </button></h3><p>透過檢查 Pod 狀態來驗證部署是否成功：</p>
<pre><code translate="no" class="language-bash">kubectl get pods
<button class="copy-code-btn"></button></code></pre>
<p><strong>請等待所有 Pod 顯示「Running」狀態。</strong>採用 v2.6.24 配置時，您應會看到類似以下的 Pod 狀態：</p>
...
<h3 id="3-Connect-to-Milvus" class="common-anchor-header">3. 連線至 Milvus<button data-href="#3-Connect-to-Milvus" class="anchor-icon" translate="no">
    ...
    </button></h3><p>若要在 Kubernetes 外部連線至您的 Milvus 叢集，您需要設定端口轉發。</p>
...
<h2 id="Access-Milvus-WebUI" class="common-anchor-header">存取 Milvus WebUI<button data-href="#Access-Milvus-WebUI" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Milvus 內建名為 Milvus WebUI 的圖形化使用者介面工具，您可透過瀏覽器存取。Milvus WebUI 透過簡潔直觀的介面，提升系統的可觀察性。您可以使用 Milvus WebUI 觀察 Milvus 各元件及依賴項的統計資料與指標、檢查資料庫與資料收集的詳細資訊，並列出詳細的 Milvus 設定。 有關 Milvus WebUI 的詳細資訊，請參閱<a href="/docs/zh-hant/v2.6.x/milvus-webui.md">Milvus WebUI</a></p>
...
<button class="copy-code-btn"></button></code></pre>
<p>上述命令會為 Milvus 叢集渲染圖表範本，並將輸出結果儲存至名為<code translate="no">milvus_manifest.yaml</code> 的 manifests 檔案中。透過此 manifests 檔案，您可以安裝一個 Milvus 叢集，並將其組件與依賴項分別放置於不同的 Pod 中。</p>
<div class="alert note">
<ul>
<li>若要以獨立模式安裝 Milvus 實例（即所有 Milvus 組件均包含於單一 Pod 中），則應執行<code translate="no">helm template my-release --set cluster.enabled=false --set etcd.replicaCount=1 --set minio.mode=standalone --set pulsarv3.enabled=false zilliztech/milvus &gt; milvus_manifest.yaml</code> ，以渲染獨立模式下 Milvus 實例的圖表範本。</li>
<li>若要變更 Milvus 設定，請下載 <a href="https://raw.githubusercontent.com/milvus-io/milvus-helm/master/charts/milvus/values.yaml"><code translate="no">value.yaml</code></a> 範本，在其中填入您所需的設定，並使用<code translate="no">helm template -f values.yaml my-release zilliztech/milvus &gt; milvus_manifest.yaml</code> 據此渲染對應的 manifests 檔案。</li>
</ul>
</div>
<h3 id="2-Download-image-pulling-script" class="common-anchor-header">2. 下載映像拉取腳本<button data-href="#2-Download-image-pulling-script" class="anchor-icon" translate="no">
    ...
    </button></h3><p>映像拉取腳本是以 Python 開發的。您應將該腳本及其依賴項一併下載至<code translate="no">requirement.txt</code> 檔案中。</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>至此，您可以依照線上安裝的步驟<a href="#2-Check-Milvus-cluster-status">2</a>和<a href="#3-Forward-a-local-port-to-Milvus">3，</a>檢查叢集狀態並將本地埠口轉發至 Milvus。</p>
...
<ul>
<li><p>查看「<a href="/docs/zh-hant/v2.6.x/quickstart.md">Hello Milvus</a>」以了解 Milvus 的功能。</p></li>
...
<li><p>探索<a href="/docs/zh-hant/v2.6.x/milvus-webui.md">Milvus WebUI</a>，這是專為 Milvus 可觀測性與管理而設計的直觀網頁介面。</p></li>
...
<li><p>探索<a href="https://github.com/zilliztech/attu">Attu，</a>這是一款用於直覺式管理 Milvus 的開源 GUI 工具。</p></li>
