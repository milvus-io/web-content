...
<h2 id="Overview" class="common-anchor-header">概述<button data-href="#Overview" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Helm 使用一種稱為「charts」的封裝格式。charts 是一組檔案的集合，用於描述相關的 Kubernetes 資源集。Milvus 提供了一組 charts，以協助您部署 Milvus 的依賴項與組件。</p>
...
<p>以下指令將部署一個針對 v2.6.21 進行過最佳化設定的 Milvus 叢集，並使用 Woodpecker 作為推薦的訊息佇列：</p>
<pre><code translate="no" class="language-bash">helm install my-release zilliztech/milvus \
  --<span class="hljs-built_in">set</span> image.all.tag=v2.6.21 \
  --<span class="hljs-built_in">set</span> pulsarv3.enabled=<span class="hljs-literal">false</span> \
  --<span class="hljs-built_in">set</span> woodpecker.enabled=<span class="hljs-literal">true</span> \
...
<li><strong>整合元件</strong>：<strong>索引節點</strong>與<strong>資料節點</strong>已合併為單一的<strong>資料節點</strong></li>
</ul>
...
<button class="copy-code-btn"></button></code></pre>
<p><strong>請等待所有 Pod 顯示「Running」狀態。</strong>採用 v2.6.21 配置時，您應會看到類似以下的 Pod 狀態：</p>
...
<div class="alert note">
<ul>
<li>若要在獨立模式下安裝 Milvus 實例（即所有 Milvus 元件均包含於單一 Pod 中），您應改為執行<code translate="no">helm template my-release --set cluster.enabled=false --set etcd.replicaCount=1 --set minio.mode=standalone --set pulsarv3.enabled=false zilliztech/milvus &gt; milvus_manifest.yaml</code> ，以渲染獨立模式下 Milvus 實例的圖表範本。</li>
