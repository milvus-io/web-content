...
<ul>
<li><p><strong>集合建立後新增的欄位</strong>：如果您在建立資料集後才新增欄位，並且在插入時沒有指定值，Milvus 會自動以定義的預設值填入，如果沒有設定預設值，則填為 NULL。詳情請參閱<a href="/docs/zh-hant/v2.6.x/add-fields-to-an-existing-collection.md">新增欄位到現有的集合</a>。</p></li>
<li><p><strong>重複處理</strong>：標準的<code translate="no">insert</code> 操作不會檢查主鍵是否重複。使用現有的主索引鍵插入資料時，會建立具有相同索引鍵的新實體，導致資料重複和潛在的應用程式問題。若要更新現有的實體或避免重複，請使用 <strong><code translate="no">upsert</code></strong>操作。如需詳細資訊，請參閱<a href="/docs/zh-hant/v2.6.x/upsert-entities.md">Upsert Entities</a>。</p></li>
</ul>
</div>
