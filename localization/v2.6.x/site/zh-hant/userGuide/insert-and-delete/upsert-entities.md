...
<h2 id="Overview" class="common-anchor-header">概述<button data-href="#Overview" class="anchor-icon" translate="no">
    ...
    </button></h2><p>您可以使用 `<code translate="no">upsert</code> ` 來插入新實體或更新現有實體，具體取決於 `upsert` 請求中提供的主鍵是否存在於集合中。若未找到該主鍵，則執行插入操作；否則，將執行更新操作。</p>
...
<p>若目標集合在其主欄位上啟用了「<code translate="no">autoid</code> 」，Milvus 會在插入資料前，為請求載荷中攜帶的資料生成一個新的主鍵。</p>
...
<p>針對<code translate="no">ARRAY</code> 欄位，合併模式支援兩種運算子：<code translate="no">ARRAY_APPEND</code> 與<code translate="no">ARRAY_REMOVE</code> 。這些運算子可讓您將元素追加至現有的<code translate="no">ARRAY</code> 欄位，或從其中移除符合條件的元素，而無需事先查詢實體以取得其當前值。詳細資訊請參閱<a href="/docs/zh-hant/v2.6.x/upsert-entities.md#Upsert-ARRAY-fields-with-partial-update-operators">《使用部分更新運算子對 ARRAY 欄位執行 Upsert</a>》。</p>
...
<p>在合併模式下，<code translate="no">ARRAY</code> 欄位支援<code translate="no">ARRAY_APPEND</code> 和<code translate="no">ARRAY_REMOVE</code> 部分更新運算子。當您希望向現有的<code translate="no">ARRAY</code> 欄位新增元素，或從其中移除符合條件的元素，同時不替換整個陣列值時，請使用這些運算子。</p></li>
</ul>
...
<li><p>對於任何透過函式從其他欄位衍生而來的欄位，Milvus 會在更新或插入（upsert）過程中移除該衍生欄位，以便重新計算。</p></li>
</ul>
...
<h2 id="Upsert-ARRAY-fields-with-partial-update-operators--Milvus-v2617+" class="common-anchor-header">使用部分更新運算子對 ARRAY 欄位進行 Upsert<span class="beta-tag" style="background-color:rgb(0, 179, 255);color:white" translate="no">Compatible with Milvus v2.6.17+</span><button data-href="#Upsert-ARRAY-fields-with-partial-update-operators--Milvus-v2617+" class="anchor-icon" translate="no">
    ...
    </button></h2><p>在引入部分更新運算子之前，若要更新 `<code translate="no">ARRAY</code> ` 欄位的一部分，必須透過客戶端「讀取-修改-寫入」的流程：查詢現有陣列、在應用程式程式碼中進行變更，然後對整個陣列執行更新或插入操作。 透過部分更新運算子，您只需傳送要追加或移除的元素，這不僅能減少客戶端邏輯，還能避免在執行「Upsert」前進行額外的讀取操作。</p>
<p>假設主鍵為<code translate="no">1</code> 的實體已包含<code translate="no">tags = [&quot;new&quot;, &quot;trial&quot;]</code> 。若未使用部分更新運算子，要將<code translate="no">&quot;premium&quot;</code> 加入陣列中，則需對整個替換陣列執行 upsert 操作：</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#javascript">   NodeJS</a>
...
<button class="copy-code-btn"></button></code></pre>
<p>若使用<code translate="no">ARRAY_APPEND</code> ，則只需傳送要新增的元素即可：</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#javascript">   NodeJS</a>
...
<button class="copy-code-btn"></button></code></pre>
<div class="alert note">
<p>透過 `<code translate="no">field_ops</code> ` 將任一運算子附加至欄位，便會隱含地啟用部分更新語義。因此，您<strong>無需</strong>在傳遞 `<code translate="no">field_ops</code>` 時一併傳遞 `<code translate="no">partial_update=True</code> `。</p>
</div>
...
<li>有效載荷的值必須與目標<code translate="no">ARRAY</code> 欄位的<code translate="no">element_type</code> 相符。例如，若目標欄位為<code translate="no">ARRAY&lt;VARCHAR&gt;</code> ，則有效載荷必須包含字串值。</li>
<li><code translate="no">ARRAY_APPEND</code> 且<code translate="no">ARRAY_REMOVE</code> 支援<code translate="no">ARRAY</code> 欄位，其<code translate="no">element_type</code> 為<code translate="no">BOOL</code> 、<code translate="no">INT8</code> 、<code translate="no">INT16</code> 、<code translate="no">INT32</code> 、<code translate="no">INT64</code> 、<code translate="no">FLOAT</code> 、<code translate="no">DOUBLE</code> 或<code translate="no">VARCHAR</code> 。</li>
<li>執行<code translate="no">ARRAY_APPEND</code> 操作後，結果陣列的長度不得超過該欄位的<code translate="no">max_capacity</code> 。</li>
<li>對同一實體的並行更新插入（upsert）操作在不同請求之間並非原子性的。若兩個請求同時更新同一<code translate="no">ARRAY</code> 欄位，後寫入的資料可能會覆寫先前的資料。若需保留所有並行變更，請使用應用程式層級的協調機制。</li>
</ul>
<h3 id="Example" class="common-anchor-header">範例<button data-href="#Example" class="anchor-icon" translate="no">
    ...
    </button></h3><p>以下範例使用一個小型<code translate="no">users</code> 集合，其主鍵為<code translate="no">pk</code> ，包含一個類型為<code translate="no">ARRAY&lt;VARCHAR&gt;</code> 的<code translate="no">tags</code> 欄位，以及一個<code translate="no">embedding</code> 向量欄位。它首先插入兩個實體，其<code translate="no">tags</code> 欄位初始值為 ，接著使用<code translate="no">ARRAY_APPEND</code> 和<code translate="no">ARRAY_REMOVE</code> 來展示每個運算子如何變更儲存的陣列。</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#javascript">   NodeJS</a>
