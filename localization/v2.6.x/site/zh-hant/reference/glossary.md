...
<h2 id="Channel" class="common-anchor-header">頻道<button data-href="#Channel" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Milvus 使用兩種類型的通道，即<a href="#pchannel">PC</a>通道和<a href="#vchannel">V 通道</a>，作為其串流服務架構的一部分。每個 PChannel 對應於由<a href="/docs/zh-hant/woodpecker_architecture.md">Woodpecker</a> 管理的 WAL 流，而每個 VChannel 對應於集合中的一個分片。流服务管理这些通道，以确保数据一致性和故障恢复。</p>
...
<h2 id="MemoryBuffer" class="common-anchor-header">MemoryBuffer<button data-href="#MemoryBuffer" class="anchor-icon" translate="no">
    ...
    </button></h2><p>MemoryBuffer 是 Woodpecker 的一種輕量級部署模式，可暫時緩衝內存中的寫入內容，並定期將其刷新至雲端物件儲存。此模式最適合用於較小規模部署中的批次較多的工作負載，或將簡單性置於性能之上的生產環境。如需詳細資訊，請參閱<a href="/docs/zh-hant/woodpecker_architecture.md">Woodpecker Architecture</a>。</p>
...
<h2 id="PChannel" class="common-anchor-header">PC 通道<button data-href="#PChannel" class="anchor-icon" translate="no">
    ...
    </button></h2><p>PChannel 代表實體通道。每個 PChannel 對應於由 Woodpecker 管理的 WAL 流。預設情況下，當 Milvus 集群啟動時，會指派一組 PChannels 來儲存記錄資料插入、刪除和更新的日誌。如需詳細資訊，請參閱<a href="/docs/zh-hant/streaming_service.md">串流服務</a>。</p>
...
<h2 id="QuorumBuffer" class="common-anchor-header">QuorumBuffer<button data-href="#QuorumBuffer" class="anchor-icon" translate="no">
    ...
    </button></h2><p>QuorumBuffer 是 Woodpecker 的一種部署模式，專為對延遲敏感、高頻率讀/寫工作負載而設計，同時要求即時回應能力和強大的容錯能力。它的功能是高速寫入緩衝區，具有三個副本的 quorum 寫入，可確保強大的一致性和高可用性。如需詳細資訊，請參閱<a href="/docs/zh-hant/woodpecker_architecture.md">Woodpecker 架構</a>。</p>
...
<h2 id="Streaming-Service" class="common-anchor-header">串流服務<button data-href="#Streaming-Service" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Streaming Service 是 Milvus 內部串流系統模組的概念，以 Write-Ahead Log (WAL) 為核心，支援各種串流相關功能。這些功能包括串流資料的擷取/訂閱、群集狀態的故障復原、串流資料轉換為歷史資料，以及成長中的資料查詢。該服務由 Streaming Coordinator、Streaming Node Cluster 和 Streaming Client 元件組成。如需詳細資訊，請參閱<a href="/docs/zh-hant/streaming_service.md">串流服務</a>。</p>
...
<h2 id="WAL-Storage" class="common-anchor-header">WAL 儲存<button data-href="#WAL-Storage" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Write-Ahead Log (WAL) 儲存是分散式系統中資料耐久性與一致性的基礎。在提交任何變更之前，它會先記錄在日誌中，以確保在發生故障時，您可以準確地恢復到之前的位置。Milvus 使用 Woodpecker 作為 WAL 儲存系統，支援 MemoryBuffer 和 QuorumBuffer 兩種模式。如需詳細資訊，請參閱<a href="/docs/zh-hant/woodpecker_architecture.md">Woodpecker Architecture</a>。</p>
<h2 id="Woodpecker" class="common-anchor-header">啄木鳥<button data-href="#Woodpecker" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Woodpecker 是 Milvus 2.6 中的雲原生 WAL 系統，取代了 Kafka 和 Pulsar。透過零磁碟架構和兩種部署模式 (MemoryBuffer 和 QuorumBuffer)，它能在物件儲存上提供高吞吐量、低操作開銷和無縫擴充能力。如需詳細資訊，請參閱<a href="/docs/zh-hant/woodpecker_architecture.md">Woodpecker 架構</a>。</p>
