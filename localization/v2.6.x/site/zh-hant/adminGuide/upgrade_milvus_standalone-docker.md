...
<h1 id="Upgrade-Milvus-Standalone-with-Docker-Compose" class="common-anchor-header">使用 Docker Compose 升級 Milvus 獨立部署<button data-href="#Upgrade-Milvus-Standalone-with-Docker-Compose" class="anchor-icon" translate="no">
    ...
    </button></h1><p>本指南說明如何使用 Docker Compose 將您的 Milvus 獨立部署從 v2.5.x 升級至 v2.6.24。</p>
...
<p>基於安全性考量，Milvus 將於 v2.6.24 版本發布時，將 MinIO 升級至 RELEASE.2024-12-18T13-15-44Z 版本。</p>
</div>
...
docker compose up -d
<button class="copy-code-btn"></button></code></pre></li>
<li><p>驗證 v2.5.16 升級是否成功：</p>
<pre><code translate="no" class="language-bash">docker compose ps
<button class="copy-code-btn"></button></code></pre></li>
</ol>
<h3 id="Step-2-Upgrade-to-v2624" class="common-anchor-header">步驟 2：升級至 v2.6.24<button data-href="#Step-2-Upgrade-to-v2624" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>當 v2.5.16 成功運行後，請升級至 v2.6.24：</p>
<ol>
...
<h2 id="Verify-the-upgrade" class="common-anchor-header">驗證升級<button data-href="#Verify-the-upgrade" class="anchor-icon" translate="no">
    ...
    </button></h2><p>確認您的獨立部署環境是否已運行新版本：</p>
