...
<h2 id="Connect-to-Milvus-with-authentication" class="common-anchor-header">使用驗證連線到 Milvus<button data-href="#Connect-to-Milvus-with-authentication" class="anchor-icon" translate="no">
    ...
    </button></h2><p>啟用認證後，您需要使用使用者名稱和密碼連線到 Milvus。預設情況下，當 Milvus 啟動時，<code translate="no">root</code> 使用者會以密碼<code translate="no">Milvus</code> 建立。以下是如何使用預設<code translate="no">root</code> 使用者在啟用驗證後連線到 Milvus 的範例：</p>
