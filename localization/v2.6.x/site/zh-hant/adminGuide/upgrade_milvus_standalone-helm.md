...
<h1 id="Upgrade-Milvus-Standalone-with-Helm-Chart" class="common-anchor-header">使用 Helm Chart 升級 Milvus 獨立部署<button data-href="#Upgrade-Milvus-Standalone-with-Helm-Chart" class="anchor-icon" translate="no">
    ...
    </button></h1><p>本指南說明如何使用 Helm Chart，將您的 Milvus 獨立部署從 v2.5.x 升級至 v2.6.21。</p>
<h2 id="Before-you-start" class="common-anchor-header">開始之前<button data-href="#Before-you-start" class="anchor-icon" translate="no">
    ...
    </button></h2><h3 id="Whats-new-in-v2621" class="common-anchor-header">v2.6.21 的新功能<button data-href="#Whats-new-in-v2621" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>從 Milvus 2.5.x 升級至 2.6.21 涉及重大的架構變更：</p>
<ul>
...
<ul>
<li>Milvus v2.6.0-rc1 與 v2.6.21<strong>不相容</strong>。不支援直接從候選版本（release candidates）進行升級。</li>
...
<li>在升級至 v2.6.21 之前，您<strong>必須先</strong>升級至 v2.5.16 或更新版本。</li>
</ul>
<p><strong>訊息佇列限制</strong>：升級至 Milvus v2.6.21 時，您必須維持當前的訊息佇列選擇。升級過程中不支援在不同的訊息佇列系統之間切換。未來版本將支援變更訊息佇列系統。</p>
<div class="alert note">
自 Milvus Helm 圖表版本 4.2.21 起，我們已將 pulsar-v3.x 圖表納入依賴項。為確保向後相容性，請將您的 Helm 升級至 v3.14 或更高版本，並務必在每次使用 `<code translate="no">helm upgrade</code>` 時，加入 `<code translate="no">--reset-then-reuse-values</code> ` 選項。
</div>
...
kubectl get pods
<button class="copy-code-btn"></button></code></pre>
<h3 id="Step-3-Upgrade-to-v2621" class="common-anchor-header">步驟 3：升級至 v2.6.21<button data-href="#Step-3-Upgrade-to-v2621" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>當 v2.5.16 成功運行後，請升級至 v2.6.21：</p>
<pre><code translate="no" class="language-bash">helm upgrade my-release zilliztech/milvus \
