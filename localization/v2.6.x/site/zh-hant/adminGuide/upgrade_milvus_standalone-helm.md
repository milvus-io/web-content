...
<h1 id="Upgrade-Milvus-Standalone-with-Helm-Chart" class="common-anchor-header">使用 Helm Chart 升級 Milvus 獨立部署<button data-href="#Upgrade-Milvus-Standalone-with-Helm-Chart" class="anchor-icon" translate="no">
    ...
    </button></h1><p>本指南說明如何使用 Helm Chart，將您的 Milvus 獨立部署從 v2.5.x 升級至 v2.6.17。</p>
...
自 Milvus Helm 圖表版本 4.2.21 起，我們已將 pulsar-v3.x 圖表納入依賴項。為確保向後相容性，請將您的 Helm 升級至 v3.14 或更新版本，並請務必在每次使用 `<code translate="no">helm upgrade</code>` 時，加入 `<code translate="no">--reset-then-reuse-values</code> ` 選項。
</div>
<h2 id="Upgrade-process" class="common-anchor-header">升級流程<button data-href="#Upgrade-process" class="anchor-icon" translate="no">
    ...
    </button></h3><p>首先，將您的 Milvus Helm 圖表升級至 5.0.0 版本：</p>
...
kubectl get pods
<button class="copy-code-btn"></button></code></pre>
<h3 id="Step-3-Upgrade-to-v2617" class="common-anchor-header">步驟 3：升級至 v2.6.17<button data-href="#Step-3-Upgrade-to-v2617" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>當 v2.5.16 成功運行後，請升級至 v2.6.17：</p>
<pre><code translate="no" class="language-bash">helm upgrade my-release zilliztech/milvus \
