...
<li><p>若您曾透過 Helm 以獨立模式安裝 Milvus，且當時使用的訊息儲存庫並非 RocksMQ，現欲將其改回 RocksMQ，請先清空所有集合並停止 Milvus 服務，再使用以下 YAML 檔案執行 `<code translate="no">helm upgrade -f</code> ` 指令。</p></li>
</ul>
<pre><code translate="no" class="language-yaml"><span class="hljs-attr">extraConfigFiles:</span>
  <span class="hljs-attr">user.yaml:</span> <span class="hljs-string">|+
    rocksmq:
