...
<p>指定訊息儲存時還存在其他限制：</p>
<ul>
<li>每個 Milvus 實例僅支援一個訊息儲存。不過，我們仍向下相容於單一實例設定多個訊息儲存的情況。其優先順序如下：
<ul>
...
<li><strong>升級限制</strong>：<strong>訊息佇列限制</strong>：升級至 Milvus v2.6.17 時，必須維持當前的訊息佇列選項。升級過程中不支援在不同的訊息佇列系統之間切換。未來版本將支援變更訊息佇列系統。</li>
</ul>
<h2 id="Configure-RocksMQ" class="common-anchor-header">設定 RocksMQ<button data-href="#Configure-RocksMQ" class="anchor-icon" translate="no">
    ...
    </button></h2><p>RocksMQ 是 Milvus 獨立執行模式中的預設訊息儲存方案。</p>
<div class="alert note">
<p>目前，您只能透過 Milvus Operator 將 RocksMQ 設定為 Milvus 獨立模式的訊息儲存方案。</p>
</div>
...
<li><code translate="no">persistence.pvcDeletion</code>: 若設定為 true，當 Milvus 實例被刪除時，PVC 將一併刪除</li>
...
<h2 id="Configure-Woodpecker" class="common-anchor-header">設定 Woodpecker<button data-href="#Configure-Woodpecker" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Woodpecker 是一款專為物件儲存設計的雲原生預寫日誌（WAL）。它具備高吞吐量、低營運開銷及無縫擴展性。如需更多詳細資訊，請參閱《<a href="/docs/zh-hant/v2.6.x/use-woodpecker.md">使用 Woodpecker</a>》。</p>
<h2 id="Configure-Pulsar" class="common-anchor-header">配置 Pulsar<button data-href="#Configure-Pulsar" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Pulsar 負責管理近期變更的日誌、輸出串流日誌，並提供日誌訂閱功能。無論是 Milvus 獨立執行模式還是 Milvus 叢集模式，皆支援將 Pulsar 配置為訊息儲存。然而，若使用 Milvus Operator，您僅能將 Pulsar 配置為 Milvus 叢集的訊息儲存。請在 `<code translate="no">spec.dependencies.pulsar</code> ` 下新增必要欄位以配置 Pulsar。</p>
...
<span class="hljs-attr">spec:</span>
  ...
  <span class="hljs-attr">config:</span> {}            
<button class="copy-code-btn"></button></code></pre>
<div class="alert note">此範例指定了 Pulsar 各元件的複本數量、Pulsar BookKeeper 的運算資源，以及其他設定。</div>
<div class="alert note">請參閱<a href="https://artifacthub.io/packages/helm/apache/pulsar/2.7.8?modal=values">values.yaml</a> 檔案，以查找用於配置內部 Pulsar 服務的完整配置項目。如前例所示，請根據需要在 `<code translate="no">pulsar.inCluster.values</code> ` 下新增所需的配置項目。</div>
...
<h3 id="Internal-Kafka" class="common-anchor-header">內部 Kafka<button data-href="#Internal-Kafka" class="anchor-icon" translate="no">
    ...
    </button></h3><p><code translate="no">inCluster</code> 表示當 Milvus 叢集啟動時，叢集內會自動啟動一個 Kafka 服務。</p>
...
<span class="hljs-attr">spec:</span> 
  ...
  <span class="hljs-attr">config:</span> {}
<button class="copy-code-btn"></button></code></pre>
<p><a href="https://artifacthub.io/packages/helm/bitnami/kafka">請在此處</a>查看配置內部 Kafka 服務所需的完整配置項目。請根據需要，在 `<code translate="no">kafka.inCluster.values</code>` 下新增配置項目。</p>
