...
<li>Milvus 系統運行期間無法變更訊息儲存系統。</li>
<li>僅支援 Kafka 2.x 或 3.x 版本。</li>
<li><strong>升級限制</strong>：<strong>訊息佇列限制</strong>：升級至 Milvus v2.6.24 時，您必須維持當前的訊息佇列選擇。升級過程中不支援在不同的訊息佇列系統之間切換。未來版本將支援變更訊息佇列系統。</li>
</ul>
...
<h2 id="Configure-Pulsar" class="common-anchor-header">配置 Pulsar<button data-href="#Configure-Pulsar" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Pulsar 負責管理近期變更的日誌、輸出串流日誌，並提供日誌訂閱服務。無論是 Milvus 獨立執行模式還是 Milvus 叢集模式，均支援將 Pulsar 設定為訊息儲存。然而，若使用 Milvus Operator，您僅能將 Pulsar 設定為 Milvus 叢集的訊息儲存。請在「<code translate="no">spec.dependencies.pulsar</code> 」下新增所需欄位以設定 Pulsar。</p>
...
<h4 id="Example" class="common-anchor-header">範例</h4><p>以下範例示範如何配置外部 Pulsar 服務。</p>
...
<div class="alert note">請參閱<a href="https://artifacthub.io/packages/helm/apache/pulsar/2.7.8?modal=values">values.yaml</a> 檔案，查閱配置內部 Pulsar 服務所需的完整配置項目。如前例所示，請根據需要在<code translate="no">pulsar.inCluster.values</code> 下新增配置項目。</div>
<p>假設配置檔案名為<code translate="no">milvuscluster.yaml</code> ，請執行以下指令以套用該配置。</p>
...
<ul>
<li><code translate="no">external</code>:<code translate="no">true</code> 的值表示 Milvus 將使用外部 Kafka 服務。</li>
<li><code translate="no">brokerList</code>：要傳送訊息的代理伺服器清單。</li>
</ul>
<h4 id="Example" class="common-anchor-header">範例</h4><p>以下範例示範如何配置外部 Kafka 服務。</p>
<pre><code translate="no" class="language-yaml"><span class="hljs-attr">apiVersion:</span> <span class="hljs-string">milvus.io/v1alpha1</span>
<span class="hljs-attr">kind:</span> <span class="hljs-string">Milvus</span>
<span class="hljs-attr">metadata:</span>
  <span class="hljs-attr">name:</span> <span class="hljs-string">my-release</span>
...
<p>假設配置檔名稱為 `<code translate="no">milvuscluster.yaml</code>`，請執行以下命令以套用配置。</p>
