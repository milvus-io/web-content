...
<p>指定訊息儲存時還有其他限制：</p>
<ul>
<li>每個 Milvus 執行個體僅支援一個訊息儲存。不過，我們仍維持向下相容性，允許為單一執行個體設定多個訊息儲存。其優先順序如下：
<ul>
...
<span class="hljs-attr">spec:</span> 
  ...
  <span class="hljs-attr">config:</span> {}
<button class="copy-code-btn"></button></code></pre>
<p><a href="https://artifacthub.io/packages/helm/bitnami/kafka">請在此處</a>查閱配置內部 Kafka 服務所需的完整配置項目。請根據需要，在 `<code translate="no">kafka.inCluster.values</code>` 目錄下新增配置項目。</p>
