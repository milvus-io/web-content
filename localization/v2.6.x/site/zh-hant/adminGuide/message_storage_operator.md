...
summary: 瞭解如何使用 Milvus Operator 配置訊息儲存。
---
<h1 id="Configure-Message-Storage-with-Milvus-Operator" class="common-anchor-header">使用 Milvus Operator 設定訊息儲存<button data-href="#Configure-Message-Storage-with-Milvus-Operator" class="anchor-icon" translate="no">
    ...
    </button></h1><p>Milvus 採用 RocksMQ、Pulsar 或 Kafka 來管理近期變更的日誌、輸出串流日誌，並提供日誌訂閱服務。本主題將介紹當您使用 Milvus Operator 安裝 Milvus 時，如何設定訊息儲存的依賴項。如需更多詳細資訊，請參閱 Milvus Operator 儲存庫中的<a href="https://github.com/zilliztech/milvus-operator/blob/main/docs/administration/manage-dependencies/message-storage.md">《使用 Milvus Operator 配置訊息儲存</a>》。</p>
...
<p>指定訊息儲存時還存在其他限制：</p>
<ul>
<li>每個 Milvus 實例僅支援一個訊息儲存。不過，我們仍維持向下相容性，允許為單一實例設定多個訊息儲存。其優先順序如下：
<ul>
<li>獨立模式：RocksMQ（預設） &gt; Pulsar &gt; Kafka</li>
<li>叢集模式：Pulsar（預設）&gt; Kafka</li>
</ul></li>
<li>Milvus 系統運行期間無法變更訊息儲存機構。</li>
<li>僅支援 Kafka 2.x 或 3.x 版本。</li>
<li><strong>升級限制</strong>：<strong>訊息佇列限制</strong>：升級至 Milvus v2.6.23 時，您必須維持當前的訊息佇列選擇。升級過程中不支援在不同的訊息佇列系統之間切換。未來版本將支援變更訊息佇列系統。</li>
</ul>
<h2 id="Configure-RocksMQ" class="common-anchor-header">設定 RocksMQ<button data-href="#Configure-RocksMQ" class="anchor-icon" translate="no">
    ...
    </button></h2><p>RocksMQ 是 Milvus 獨立執行模式中的預設訊息儲存。</p>
<div class="alert note">
<p>目前，您只能透過 Milvus Operator 將 RocksMQ 設定為 Milvus 獨立執行模式的訊息儲存方案。</p>
</div>
...
<h2 id="Configure-Woodpecker" class="common-anchor-header">設定 Woodpecker<button data-href="#Configure-Woodpecker" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Woodpecker 是一款專為物件儲存設計的雲原生預寫日誌（WAL）。它具備高吞吐量、低運作開銷及無縫擴展性。如需更多詳細資訊，請參閱《<a href="/docs/zh-hant/v2.6.x/use-woodpecker.md">使用 Woodpecker</a>》。</p>
...
<ul>
<li><code translate="no">external</code>:  若設定為<code translate="no">true</code> ，表示 Milvus 使用外部 Pulsar 服務。</li>
...
<div class="alert note">請參閱<a href="https://artifacthub.io/packages/helm/apache/pulsar/2.7.8?modal=values">values.yaml</a> 檔案，以了解用於配置內部 Pulsar 服務的完整配置項目。如前例所示，請根據需要將配置項目新增至 `<code translate="no">pulsar.inCluster.values</code> ` 之下。</div>
<p>假設配置檔案名為<code translate="no">milvuscluster.yaml</code> ，請執行以下命令以套用該配置。</p>
...
<h2 id="Configure-Kafka" class="common-anchor-header">設定 Kafka<button data-href="#Configure-Kafka" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Pulsar 是 Milvus 叢集中的預設訊息儲存方案。若要使用 Kafka，請新增可選欄位 `<code translate="no">msgStreamType</code> ` 來配置 Kafka。</p>
...
<ul>
<li><code translate="no">external</code>: 設定為<code translate="no">true</code> 表示 Milvus 將使用外部 Kafka 服務。</li>
<li><code translate="no">brokerList</code>：要傳送訊息的經紀人清單。</li>
</ul>
<h4 id="Example" class="common-anchor-header">範例</h4><p>以下範例展示如何設定外部 Kafka 服務。</p>
<pre><code translate="no" class="language-yaml"><span class="hljs-attr">apiVersion:</span> <span class="hljs-string">milvus.io/v1alpha1</span>
<span class="hljs-attr">kind:</span> <span class="hljs-string">Milvus</span>
<span class="hljs-attr">metadata:</span>
  <span class="hljs-attr">name:</span> <span class="hljs-string">my-release</span>
...
<h4 id="Example" class="common-anchor-header">範例</h4><p>以下範例說明如何配置內部 Kafka 服務。</p>
<pre><code translate="no" class="language-yaml"><span class="hljs-attr">apiVersion:</span> <span class="hljs-string">milvus.io/v1alpha1</span>
<span class="hljs-attr">kind:</span> <span class="hljs-string">Milvus</span>
<span class="hljs-attr">metadata:</span>
  <span class="hljs-attr">name:</span> <span class="hljs-string">my-release</span>
...
<ul>
<li><a href="/docs/zh-hant/v2.6.x/object_storage_operator.md">使用 Milvus Operator 配置物件儲存</a></li>
<li><a href="/docs/zh-hant/v2.6.x/meta_storage_operator.md">使用 Milvus Operator 配置元資料儲存</a></li>
</ul>
