...
<h1 id="Upgrade-Milvus-Cluster-with-Helm-Chart" class="common-anchor-header">使用 Helm Chart 升級 Milvus 集群<button data-href="#Upgrade-Milvus-Cluster-with-Helm-Chart" class="anchor-icon" translate="no">
    ...
    </button></h1><p>本指南描述如何使用 Helm Chart 將您的 Milvus 叢集從 v2.5.x 升級到 v2.6.10。</p>
<h2 id="Before-you-start" class="common-anchor-header">在您開始之前<button data-href="#Before-you-start" class="anchor-icon" translate="no">
    ...
    </button></h3><p>從 Milvus 2.5.x 升級到 2.6.10 涉及重大的架構改變：</p>
<ul>
...
<li>在升級至 v2.6.10 之前，您<strong>必須先升級</strong>至 v2.5.16 或更新版本，並啟用<code translate="no">mixCoordinator</code> 。</li>
</ul>
...
kubectl get pods
<button class="copy-code-btn"></button></code></pre>
<h3 id="Step-3-Upgrade-to-v2610" class="common-anchor-header">步驟 3：升級至 v2.6.10<button data-href="#Step-3-Upgrade-to-v2610" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>一旦 v2.5.16 與<code translate="no">mixCoordinator</code> 成功執行，請升級至 v2.6.10：</p>
<pre><code translate="no" class="language-bash">helm upgrade my-release zilliztech/milvus \
