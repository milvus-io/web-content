...
summary: >-
  ...
  使用可插拔的適配器系統，而非與任何特定資料庫緊密耦合，適配器就像是定義嵌入式如何格式化、索引和儲存於目標向量儲存的封裝。
...
<h3 id="Clone-the-Repository-and-Load-Adapter" class="common-anchor-header">複製儲存庫並載入適配器<button data-href="#Clone-the-Repository-and-Load-Adapter" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>接下來，我們要克隆<a href="https://github.com/StarlightSearch/EmbedAnything">EmbedAnything</a>repo，並將<code translate="no">examples/adapters</code> 目錄加入 Python 路徑。這是我們儲存自訂 Milvus 適配器實作的地方，它允許 EmbedAnything 與 Milvus 通訊以插入向量。</p>
...
<h2 id="Build-RAG" class="common-anchor-header">建立 RAG<button data-href="#Build-RAG" class="anchor-icon" translate="no">
    ...
    </button></h3><p>在嵌入任何檔案之前，我們需要準備兩個與 Milvus 互動的元件：</p>
<ol>
...
<li><code translate="no">MilvusClient</code> - 這是來自<code translate="no">pymilvus</code> 的官方用戶端，可<strong>完整存取</strong>Milvus 功能，包括向量搜尋、過濾和集合管理。</li>
</ol>
<p>為了避免混淆：</p>
<ul>
...
<h3 id="Initialize-Embedding-Model-and-Embed-PDF-Document" class="common-anchor-header">初始化嵌入模型和嵌入 PDF 文件<button data-href="#Initialize-Embedding-Model-and-Embed-PDF-Document" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<h3 id="Retrieve-and-Generate-Response" class="common-anchor-header">擷取並產生回應<button data-href="#Retrieve-and-Generate-Response" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>同樣地，EmbedAnything 的<code translate="no">MilvusVectorAdapter</code> 目前只是一個輕量級的抽象，僅用於向量攝取和索引。它<strong>不支援搜尋或</strong>檢索查詢。因此，為了搜尋相關文件以建立我們的 RAG 管道，我們必須直接使用<code translate="no">MilvusClient</code> 實例 (<code translate="no">milvus_client</code>) 來查詢我們的 Milvus 向量儲存庫。</p>
