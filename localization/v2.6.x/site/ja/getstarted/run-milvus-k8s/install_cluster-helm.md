...
<p>次に、以下の手順でリポジトリからMilvusチャートを取得します。</p>
...
<p>次のコマンドは、推奨されるメッセージキューとしてWoodpeckerを使用し、v2.6.17向けに最適化された設定でMilvusクラスタをデプロイします：</p>
<pre><code translate="no" class="language-bash">helm install my-release zilliztech/milvus \
  --<span class="hljs-built_in">set</span> image.all.tag=v2.6.17 \
  --<span class="hljs-built_in">set</span> pulsarv3.enabled=<span class="hljs-literal">false</span> \
  --<span class="hljs-built_in">set</span> woodpecker.enabled=<span class="hljs-literal">true</span> \
...
<li>従来の<strong>Index Node</strong>を無効化します（機能はData Nodeによって処理されるようになりました）</li>
...
<button class="copy-code-btn"></button></code></pre>
<p><strong>すべてのポッドのステータスが「Running」になるまで待ちます。</strong>v2.6.17 の設定では、次のようなポッドが表示されるはずです：</p>
...
<h3 id="4-Load-images" class="common-anchor-header">4. 画像の読み込み<button data-href="#4-Load-images" class="anchor-icon" translate="no">
    ...
    </button></h3><p>これで、ネットワーク制限のある環境内のホストに、次のように画像を読み込むことができます：</p>
