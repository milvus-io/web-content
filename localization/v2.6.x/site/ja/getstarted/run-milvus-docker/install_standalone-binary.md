...
<h1 id="Install-Milvus-Standalone-with-RPMDEB-Package" class="common-anchor-header">RPM/DEB パッケージを使用した Milvus スタンドアロンのインストール<button data-href="#Install-Milvus-Standalone-with-RPMDEB-Package" class="anchor-icon" translate="no">
    ...
    </button></h1><p>このページでは、あらかじめビルド済みのRPM/DEBパッケージを使用してMilvusスタンドアロンをインストールする方法について説明します。</p>
...
<li>libstdc++ 8.5.0 以降のバージョンがすでにインストールされていること。</li>
...
<h2 id="Download-the-RPMDEB-Package" class="common-anchor-header">RPM/DEB パッケージのダウンロード<button data-href="#Download-the-RPMDEB-Package" class="anchor-icon" translate="no">
    ...
    </button></h2><p><a href="https://github.com/milvus-io/milvus/releases/tag/v2.6.24">Milvus のリリースページ</a>から、お使いのシステムアーキテクチャに応じた RPM/DEB パッケージをダウンロードできます。</p>
<ul>
<li>x86_64/amd64 の場合は、<strong>milvus_2.6.24-1_amd64.deb</strong>または<strong>milvus_2.6.24-1_amd64.rpm</strong>パッケージをダウンロードしてください。</li>
<li>ARM64 の場合は、<strong>milvus_2.6.24-1_arm64.deb</strong>または<strong>milvus_2.6.24-1_arm64.rpm</strong>パッケージをダウンロードしてください。</li>
</ul>
<p>以下のコマンドは、x86_64/amd64マシン上でMilvus Standaloneを実行することを前提としています。</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>Milvus が正常に実行されている場合、次のような出力が表示されます:</p>
...
<h2 id="Optional-Update-Milvus-configurations" class="common-anchor-header">(オプション) Milvusの設定を更新する<button data-href="#Optional-Update-Milvus-configurations" class="anchor-icon" translate="no">
    ...
    </button></h2><p><code translate="no">/etc/milvus/configs/milvus.yaml</code> ファイルで Milvus の設定を変更できます。たとえば、<code translate="no">proxy.healthCheckTimeout</code> を<code translate="no">1000</code> ms に変更するには、target パラメータを検索し、必要に応じて変更します。適用可能な設定項目については、「<a href="/docs/ja/v2.6.x/system_configuration.md">システム設定</a>」を参照してください。</p>
...
<h2 id="Whats-next" class="common-anchor-header">次の手順<button data-href="#Whats-next" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Milvus Standalone のインストールが完了したら、次の操作が可能です:</p>
<ul>
...
<li><p>Milvusを直感的に管理できるオープンソースのGUIツール「<a href="https://github.com/zilliztech/attu">Attu</a>」についてご覧ください。</p></li>
