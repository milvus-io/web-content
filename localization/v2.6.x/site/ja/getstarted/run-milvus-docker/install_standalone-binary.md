...
<h2 id="Download-the-RPMDEB-Package" class="common-anchor-header">RPM/DEB パッケージのダウンロード<button data-href="#Download-the-RPMDEB-Package" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Milvusリリースページ](https://github.com/milvus-io/milvus/releases/tag/v{{ var.milvus_release_version }})から、システムアーキテクチャに応じたRPM/DEBパッケージをダウンロードすることができます。</p>
<ul>
<li>x86_64/amd64 の場合は<strong>{{ var.milvus_deb_amd64 }}</strong>または<strong>{{ var.milvus_rpm_amd64 }}</strong>パッケージをダウンロードしてください。</li>
<li>ARM64 の場合は<strong>{{ var.milvus_deb_arm64 }}</strong>または<strong>{{ var.milvus_rpm_arm64 }}</strong>パッケージをダウンロードしてください。</li>
</ul>
