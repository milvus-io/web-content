...
<h1 id="Upgrade-Milvus-Cluster-with-Milvus-Operator" class="common-anchor-header">Milvus OperatorによるMilvusクラスタのアップグレード<button data-href="#Upgrade-Milvus-Cluster-with-Milvus-Operator" class="anchor-icon" translate="no">
    ...
    </button></h1><p>このガイドでは、Milvus Operatorを使用してMilvusクラスタをv2.5.xからv2.6.11にアップグレードする方法について説明します。</p>
...
<li>v2.6.11にアップグレードする前に、<code translate="no">mixCoord</code> を有効にして、v2.5.16以降にアップグレードする<strong>必要があります</strong>。</li>
</ul>
<p><strong>メッセージキューの制限</strong>：Milvusをv2.6.11にアップグレードする場合、現在選択しているメッセージキューを維持する必要があります。アップグレード中に異なるメッセージキューシステムを切り替えることはサポートされていません。メッセージキューシステムの変更は、将来のバージョンでサポートされる予定です。</p>
...
kubectl get pods
<button class="copy-code-btn"></button></code></pre>
<h4 id="23-Upgrade-to-v2611" class="common-anchor-header">2.3 v2.6.11へのアップグレード</h4><p>v2.5.16が<code translate="no">mixCoord</code> 、正常に動作するようになったら、v2.6.11にアップグレードします：</p>
