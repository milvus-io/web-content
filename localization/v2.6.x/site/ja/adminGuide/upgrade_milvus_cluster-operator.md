...
<h1 id="Upgrade-Milvus-Cluster-with-Milvus-Operator" class="common-anchor-header">Milvus Operator を使用した Milvus クラスタのアップグレード<button data-href="#Upgrade-Milvus-Cluster-with-Milvus-Operator" class="anchor-icon" translate="no">
    ...
    </button></h1><p>このガイドでは、Milvus Operator を使用して Milvus クラスタを v2.5.x から v2.6.24 にアップグレードする方法について説明します。</p>
<h2 id="Before-you-start" class="common-anchor-header">開始する前に<button data-href="#Before-you-start" class="anchor-icon" translate="no">
    ...
    </button></h3><p>Milvus 2.5.x から 2.6.24 へのアップグレードには、アーキテクチャ上の大幅な変更が含まれます:</p>
<ul>
...
<p>このアップグレードプロセスにより、新しいアーキテクチャへの適切な移行が保証されます。アーキテクチャの変更に関する詳細については、「<a href="/docs/ja/v2.6.x/architecture_overview.md">Milvus アーキテクチャの概要</a>」を参照してください。</p>
...
<ul>
<li>Milvus Operatorを介してMilvusがデプロイされたKubernetesクラスタ</li>
...
<li>v2.6.24 にアップグレードする前に、<code translate="no">mixCoord</code> を有効にした状態で v2.5.16 以降にアップグレード<strong>する必要があります</strong>。</li>
</ul>
<p><strong>メッセージキューの制限事項</strong>：Milvus v2.6.24 へのアップグレード時には、現在のメッセージキューの設定を維持する必要があります。アップグレード中に異なるメッセージキューシステムへ切り替えることはサポートされていません。メッセージキューシステムの変更機能は、将来のバージョンで提供される予定です。</p>
...
<h3 id="Step-2-Upgrade-your-Milvus-cluster" class="common-anchor-header">ステップ 2: Milvus クラスタのアップグレード<button data-href="#Step-2-Upgrade-your-Milvus-cluster" class="anchor-icon" translate="no">
    ...
    </button></h3><h4 id="21-Check-current-coordinator-configuration" class="common-anchor-header">2.1 現在のコーディネーターの設定を確認する</h4><p>クラスタがすでに<code translate="no">mixCoord</code> を使用しているか確認してください:</p>
...
<p><code translate="no">mixCoord</code> を有効にし、v2.5.16にアップグレードするために、設定ファイル `<code translate="no">milvusupgrade.yaml</code> ` を作成します:</p>
...
kubectl get pods
<button class="copy-code-btn"></button></code></pre>
<h4 id="23-Upgrade-to-v2624" class="common-anchor-header">2.3 v2.6.24 へのアップグレード</h4><p>v2.5.16が<code translate="no">mixCoord</code> を有効にした状態で正常に動作したら、v2.6.24にアップグレードします:</p>
...
<h2 id="Verify-the-upgrade" class="common-anchor-header">アップグレードの確認<button data-href="#Verify-the-upgrade" class="anchor-icon" translate="no">
    ...
    </button></h2><p>クラスタが新しいバージョンで実行されていることを確認します：</p>
<pre><code translate="no" class="language-bash"><span class="hljs-comment"># Check pod status</span>
kubectl get pods
<button class="copy-code-btn"></button></code></pre>
<p>さらにサポートが必要な場合は、<a href="https://milvus.io/docs">Milvus のドキュメント</a>または<a href="https://github.com/milvus-io/milvus/discussions">コミュニティフォーラム</a>を参照してください。</p>
