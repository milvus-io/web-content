...
<li><strong>アップグレードに関する制限事項</strong>：<strong>メッセージキューに関する制限事項</strong>：Milvus v2.6.19 へのアップグレード時には、現在のメッセージキューの選択を維持する必要があります。アップグレード中に異なるメッセージキューシステムへの切り替えはサポートされていません。メッセージキューシステムの変更機能は、将来のバージョンで提供される予定です。</li>
</ul>
...
<li><code translate="no">persistentVolumeClaim.spec</code>: 標準的な Kubernetes PVC 仕様</li>
...
<ul>
<li><code translate="no">external</code>:<code translate="no">true</code> の値が設定されている場合、Milvus は外部の Kafka サービスを使用することを示します。</li>
