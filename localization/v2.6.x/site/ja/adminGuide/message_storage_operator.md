...
title: Milvus Operator を使用したメッセージストレージの設定
related_key: 'minio, s3, storage, etcd, pulsar'
summary: Milvus Operator を使用してメッセージストレージを設定する方法について学びましょう。
---
<h1 id="Configure-Message-Storage-with-Milvus-Operator" class="common-anchor-header">Milvus Operator を使用したメッセージストレージの設定<button data-href="#Configure-Message-Storage-with-Milvus-Operator" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<div class="alert note">詳細については、「<a href="https://milvus.io/docs/v2.2.x/install_cluster-milvusoperator.md">Milvus Operator のデプロイ</a>」を参照してください。 </div>
...
<h2 id="Before-you-begin" class="common-anchor-header">開始する前に<button data-href="#Before-you-begin" class="anchor-icon" translate="no">
    ...
    </button></h2><p>以下の表は、Milvus のスタンドアロンモードおよびクラスタモードにおいて、RocksMQ、Pulsar、Kafka、Woodpecker がサポートされているかどうかを示しています。</p>
<table>
<thead>
...
<ul>
<li>1つのMilvusインスタンスにつき、1つのメッセージストレージのみがサポートされています。ただし、1つのインスタンスに複数のメッセージストレージを設定していた場合についても、後方互換性は維持されています。優先順位は以下の通りです：
<ul>
<li>スタンドアロンモード： RocksMQ（デフォルト） &gt; Pulsar &gt; Kafka</li>
<li>クラスタモード：Pulsar（デフォルト） &gt; Kafka</li>
</ul></li>
<li>Milvusシステムの稼働中は、メッセージストレージを変更することはできません。</li>
<li>Kafka 2.x または 3.x バージョンのみがサポートされています。</li>
<li><strong>アップグレードに関する制限</strong>：<strong>メッセージキューの制限</strong>：Milvus v2.6.24 へのアップグレード時には、現在のメッセージキューの選択を維持する必要があります。アップグレード中に異なるメッセージキューシステムへの切り替えはサポートされていません。メッセージキューシステムの変更機能は、将来のバージョンで提供される予定です。</li>
</ul>
<h2 id="Configure-RocksMQ" class="common-anchor-header">RocksMQ の設定<button data-href="#Configure-RocksMQ" class="anchor-icon" translate="no">
    ...
    </button></h2><p>RocksMQ は、Milvus スタンドアロンのデフォルトのメッセージストレージです。</p>
<div class="alert note">
<p>現在、Milvus Operator を使用する場合にのみ、Milvus スタンドアロンのメッセージストレージとして RocksMQ を設定できます。</p>
</div>
<h4 id="Example" class="common-anchor-header">例</h4><p>以下の例は、RocksMQ サービスを設定するものです。</p>
...
<li><code translate="no">persistentVolumeClaim.spec</code>: 標準の Kubernetes PVC 仕様</li>
...
<h2 id="Configure-Woodpecker" class="common-anchor-header">Woodpecker の設定<button data-href="#Configure-Woodpecker" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>Woodpecker は、オブジェクトストレージ向けに設計されたクラウドネイティブな Write-Ahead Log (WAL) です。高いスループット、低い運用オーバーヘッド、シームレスなスケーラビリティを提供します。詳細については、「<a href="/docs/ja/v2.6.x/use-woodpecker.md">Woodpecker の使用</a>」を参照してください。</p>
<h2 id="Configure-Pulsar" class="common-anchor-header">Pulsar の設定<button data-href="#Configure-Pulsar" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Pulsar は、最近の変更履歴を管理し、ストリームログを出力し、ログのサブスクリプションを提供します。メッセージストレージとしての Pulsar の設定は、Milvus スタンドアロンおよび Milvus クラスターの両方でサポートされています。ただし、Milvus Operator を使用する場合、Pulsar をメッセージストレージとして設定できるのは Milvus クラスターのみです。Pulsar を設定するには、<code translate="no">spec.dependencies.pulsar</code> の下に必要なフィールドを追加してください。</p>
...
<ul>
<li><code translate="no">external</code>: 「<code translate="no">true</code> 」という値は、Milvusが外部Pulsarサービスを使用することを示します。</li>
...
<h3 id="Internal-Pulsar" class="common-anchor-header">内部Pulsar<button data-href="#Internal-Pulsar" class="anchor-icon" translate="no">
    ...
    </button></h3><p><code translate="no">inCluster</code> は、Milvus クラスタの起動時に、Pulsar サービスがクラスタ内で自動的に起動することを示します。</p>
<h4 id="Example" class="common-anchor-header">例</h4><p>次の例は、内部 Pulsar サービスを設定するものです。</p>
<pre><code translate="no" class="language-YAML"><span class="hljs-attr">apiVersion:</span> <span class="hljs-string">milvus.io/v1alpha1</span>
<span class="hljs-attr">kind:</span> <span class="hljs-string">Milvus</span>
<span class="hljs-attr">metadata:</span>
  <span class="hljs-attr">name:</span> <span class="hljs-string">my-release</span>
...
<div class="alert note">内部 Pulsar サービスを設定するための完全な設定項目については、<a href="https://artifacthub.io/packages/helm/apache/pulsar/2.7.8?modal=values">values.yaml</a> を参照してください。前の例に示すように、必要に応じて<code translate="no">pulsar.inCluster.values</code> の下に設定項目を追加してください。</div>
...
<h2 id="Configure-Kafka" class="common-anchor-header">Kafka の設定<button data-href="#Configure-Kafka" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Pulsar は、Milvus クラスタのデフォルトのメッセージストレージです。Kafka を使用する場合は、オプションのフィールド<code translate="no">msgStreamType</code> を追加して Kafka を設定してください。</p>
...
<h3 id="External-Kafka" class="common-anchor-header">外部Kafka<button data-href="#External-Kafka" class="anchor-icon" translate="no">
    ...
    </button></h3><p><code translate="no">external</code> は、外部のKafkaサービスを使用することを示します。</p>
<p>外部Kafkaサービスの設定に使用されるフィールドは次のとおりです。</p>
<ul>
<li><code translate="no">external</code>:<code translate="no">true</code> の値が設定されている場合、Milvus は外部 Kafka サービスを使用することを示します。</li>
<li><code translate="no">brokerList</code>: メッセージを送信するブローカーのリスト。</li>
</ul>
<h4 id="Example" class="common-anchor-header">例</h4><p>以下の例は、外部Kafkaサービスを設定するものです。</p>
<pre><code translate="no" class="language-yaml"><span class="hljs-attr">apiVersion:</span> <span class="hljs-string">milvus.io/v1alpha1</span>
<span class="hljs-attr">kind:</span> <span class="hljs-string">Milvus</span>
<span class="hljs-attr">metadata:</span>
  <span class="hljs-attr">name:</span> <span class="hljs-string">my-release</span>
...
<button class="copy-code-btn"></button></code></pre>
<blockquote>
<p>SASL 設定は、operator v0.8.5 以降のバージョンでサポートされています。</p>
</blockquote>
<h3 id="Internal-Kafka" class="common-anchor-header">内部 Kafka<button data-href="#Internal-Kafka" class="anchor-icon" translate="no">
    ...
    </button></h3><p><code translate="no">inCluster</code> は、Milvus クラスタの起動時に、そのクラスタ内で Kafka サービスが自動的に起動することを示します。</p>
<h4 id="Example" class="common-anchor-header">例</h4><p>以下の例は、内部 Kafka サービスを設定するものです。</p>
<pre><code translate="no" class="language-yaml"><span class="hljs-attr">apiVersion:</span> <span class="hljs-string">milvus.io/v1alpha1</span>
<span class="hljs-attr">kind:</span> <span class="hljs-string">Milvus</span>
<span class="hljs-attr">metadata:</span>
  <span class="hljs-attr">name:</span> <span class="hljs-string">my-release</span>
...
<span class="hljs-attr">spec:</span> 
  ...
  <span class="hljs-attr">config:</span> {}
<button class="copy-code-btn"></button></code></pre>
<p>内部Kafkaサービスを設定するための完全な設定項目については、<a href="https://artifacthub.io/packages/helm/bitnami/kafka">こちら</a>をご覧ください。必要に応じて、<code translate="no">kafka.inCluster.values</code> の下に設定項目を追加してください。</p>
<p>設定ファイルの名前を<code translate="no">milvuscluster.yaml</code> とする場合、以下のコマンドを実行して設定を適用します。</p>
...
<ul>
<li><a href="/docs/ja/v2.6.x/object_storage_operator.md">Milvus Operator を使用したオブジェクトストレージの設定</a></li>
<li><a href="/docs/ja/v2.6.x/meta_storage_operator.md">Milvus Operator を使用したメタストレージの設定</a></li>
</ul>
