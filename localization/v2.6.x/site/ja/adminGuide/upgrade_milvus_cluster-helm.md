...
<h1 id="Upgrade-Milvus-Cluster-with-Helm-Chart" class="common-anchor-header">Helm ChartによるMilvusクラスタのアップグレード<button data-href="#Upgrade-Milvus-Cluster-with-Helm-Chart" class="anchor-icon" translate="no">
    ...
    </button></h1><p>このガイドでは、Helm Chartを使用してMilvusクラスタをv2.5.xからv2.6.6にアップグレードする方法について説明します。</p>
...
<li>v2.6.6にアップグレードする前に、<code translate="no">mixCoordinator</code> を有効にして、v2.5.16以降にアップグレードする<strong>必要があります</strong>。</li>
</ul>
<p><strong>メッセージキューの制限</strong>：Milvusをv2.6.6にアップグレードする場合、現在選択しているメッセージキューを維持する必要があります。アップグレード中に異なるメッセージキューシステムを切り替えることはサポートされていません。メッセージキューシステムの変更は将来のバージョンでサポートされる予定です。</p>
...
kubectl get pods
<button class="copy-code-btn"></button></code></pre>
<h3 id="Step-3-Upgrade-to-v266" class="common-anchor-header">ステップ3: v2.6.6へのアップグレード<button data-href="#Step-3-Upgrade-to-v266" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>v2.5.16が<code translate="no">mixCoordinator</code> 、正常に動作するようになったら、v2.6.6にアップグレードします：</p>
<pre><code translate="no" class="language-bash">helm upgrade my-release zilliztech/milvus \
