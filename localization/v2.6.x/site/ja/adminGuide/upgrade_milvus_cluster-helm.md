...
<h1 id="Upgrade-Milvus-Cluster-with-Helm-Chart" class="common-anchor-header">Helm ChartによるMilvusクラスタのアップグレード<button data-href="#Upgrade-Milvus-Cluster-with-Helm-Chart" class="anchor-icon" translate="no">
    ...
    </button></h1><p>このガイドでは、Helm Chartを使用してMilvusクラスタをv2.5.xからv2.6.2にアップグレードする方法について説明します。</p>
...
<li>v2.6.2にアップグレードする前に、<code translate="no">mixCoordinator</code> を有効にして v2.5.16 以降にアップグレードする<strong>必要があります</strong>。</li>
</ul>
<div class="alert note">
Milvus Helmチャートバージョン4.2.21以降、依存関係としてpulsar-v3.xチャートを導入しました。後方互換性のため、Helmをv3.14またはそれ以降のバージョンにアップグレードし、<code translate="no">helm upgrade</code> を使用する際は必ず<code translate="no">--reset-then-reuse-values</code> オプションを追加してください。</div>
...
kubectl get pods
<button class="copy-code-btn"></button></code></pre>
<h3 id="Step-3-Upgrade-to-v262" class="common-anchor-header">ステップ3: v2.6.2へのアップグレード<button data-href="#Step-3-Upgrade-to-v262" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>v2.5.16が<code translate="no">mixCoordinator</code> 、正常に動作するようになったら、v2.6.2にアップグレードします：</p>
<pre><code translate="no" class="language-bash">helm upgrade my-release zilliztech/milvus \
