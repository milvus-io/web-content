...
<h1 id="Upgrade-Milvus-Standalone-with-Docker-Compose" class="common-anchor-header">Docker Compose を使用した Milvus スタンドアロンのアップグレード<button data-href="#Upgrade-Milvus-Standalone-with-Docker-Compose" class="anchor-icon" translate="no">
    ...
    </button></h1><p>このガイドでは、Docker Compose を使用して Milvus スタンドアロン環境を v2.5.x から v2.6.24 にアップグレードする方法について説明します。</p>
...
<p><strong>メッセージキューの制限事項</strong>:Milvus v2.6.24 へアップグレードする際は、現在のメッセージキューの選択を維持する必要があります。アップグレード中に異なるメッセージキューシステムへ切り替えることはサポートされていません。メッセージキューシステムの変更に関するサポートは、将来のバージョンで提供される予定です。</p>
<div class="alter note">
<p>セキュリティ上の理由により、Milvusはv2.6.24のリリースに伴い、MinIOをRELEASE.2024-12-18T13-15-44Zにアップグレードします。</p>
</div>
...
<ol>
<li><p>既存の<code translate="no">docker-compose.yaml</code> ファイルを編集し、Milvusイメージのタグをv2.5.16に更新してください:</p>
...
<h3 id="Step-2-Upgrade-to-v2624" class="common-anchor-header">ステップ 2: v2.6.24 へのアップグレード<button data-href="#Step-2-Upgrade-to-v2624" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>v2.5.16が正常に動作していることを確認したら、v2.6.24にアップグレードします:</p>
<ol>
...
<h2 id="Verify-the-upgrade" class="common-anchor-header">アップグレードの確認<button data-href="#Verify-the-upgrade" class="anchor-icon" translate="no">
    ...
    </button></h2><p>スタンドアロン展開が新しいバージョンで実行されていることを確認してください:</p>
...
<ul>
<li><a href="/docs/ja/v2.6.x/scaleout.md">Milvusクラスターのスケーリング</a></li>
</ul></li>
<li>クラウド上でクラスターをデプロイする準備ができている場合は：
<ul>
