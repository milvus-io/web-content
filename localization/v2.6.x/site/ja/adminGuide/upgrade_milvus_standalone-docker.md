...
summary: Docker Compose を使用して Milvus スタンドアロンをアップグレードする方法をご覧ください。
...
<h1 id="Upgrade-Milvus-Standalone-with-Docker-Compose" class="common-anchor-header">Docker Compose を使用した Milvus スタンドアロンのアップグレード<button data-href="#Upgrade-Milvus-Standalone-with-Docker-Compose" class="anchor-icon" translate="no">
    ...
    </button></h1><p>このガイドでは、Docker Compose を使用して Milvus スタンドアロン環境を v2.5.x から v2.6.17 にアップグレードする方法について説明します。</p>
...
<p>セキュリティ上の理由により、Milvus v2.6.17のリリースに伴い、MinIOがRELEASE.2024-12-18T13-15-44Zにアップグレードされます。</p>
</div>
...
<ol>
<li><p>既存の<code translate="no">docker-compose.yaml</code> ファイルを編集し、Milvusイメージタグをv2.5.16に更新します:</p>
...
<h3 id="Step-2-Upgrade-to-v2617" class="common-anchor-header">ステップ 2: v2.6.17 へのアップグレード<button data-href="#Step-2-Upgrade-to-v2617" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>v2.5.16が正常に動作していることを確認したら、v2.6.17にアップグレードします:</p>
<ol>
