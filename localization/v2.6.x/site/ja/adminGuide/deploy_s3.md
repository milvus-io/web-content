...
<h3 id="2-Refine-docker-composeyaml" class="common-anchor-header">2.docker-compose.yaml を洗練させる。</h3><p>また、milvusサービスの環境変数<code translate="no">MINIO_ADDRESS</code> を<code translate="no">docker-compose.yaml</code> で削除します。デフォルトでは、milvusは外部のS3ではなくローカルのminioを使用する。</p>
