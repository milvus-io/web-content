...
<p>様々な検索ベクトル・フィールドの能力を示すために、サンプルのクエリを使用して3つの検索リクエスト（<code translate="no">AnnSearchRequest</code> ）を作成します。この際、事前に計算された密なベクトルも使用する。検索リクエストは以下のベクトルフィールドを対象とする：</p>
<ul>
