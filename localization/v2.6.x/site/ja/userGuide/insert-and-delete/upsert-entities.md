...
<h1 id="Upsert-Entities" class="common-anchor-header">アップサート・エンティティ<button data-href="#Upsert-Entities" class="anchor-icon" translate="no">
    ...
    </button></h1><p><code translate="no">upsert</code> 操作は、コレクション内のエンティティを挿入または更新する便利な方法を提供します。主キーが既に存在する場合は、対応するエンティティが更新され、そうでない場合は新しいエンティティが挿入されます。このため、<code translate="no">upsert</code> は、エンティティが既に存在するかどうか不明な場合や、重複エントリの作成を避ける必要がある場合のデータ管理に推奨される方法です。</p>
