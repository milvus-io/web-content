...
<div class="alert note">
<ul>
<li><p><strong>コレクション作成後に追加されたフィールド</strong>：コレクション作成後に新しいフィールドをコレクションに追加し、挿入時に値を指定しなかった場合、Milvusは自動的に定義されたデフォルト値またはデフォルトが設定されていない場合はNULLを入力します。詳細については、<a href="/docs/ja/add-fields-to-an-existing-collection.md">既存のコレクションへのフィールドの追加を</a>参照してください。</p></li>
