...
<h2 id="What-is-a-JSON-field" class="common-anchor-header">JSONフィールドとは何ですか？<button data-href="#What-is-a-JSON-field" class="anchor-icon" translate="no">
    ...
    </button></h2><p>JSONフィールドとは、Milvusでスキーマ定義されたフィールドで、構造化されたKey-Valueデータを格納します。値には文字列、数値、ブーリアン、配列、または深くネストされたオブジェクトを含めることができます。</p>
...
<p>この例では</p>
<ul>
<li><p><code translate="no">metadata</code> はスキーマで定義されたJSONフィールドです。</p></li>
<li><p>フラットな値（例：<code translate="no">category</code> 、<code translate="no">in_stock</code> ）、配列（<code translate="no">tags</code> ）、ネストされたオブジェクト（<code translate="no">supplier</code> ）を格納することができます。</p></li>
</ul>
<h2 id="Define-a-JSON-field-in-the-schema" class="common-anchor-header">スキーマでJSONフィールドを定義する<button data-href="#Define-a-JSON-field-in-the-schema" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<li><p><code translate="no">vector</code> フィールド（各コレクションに必須）</p></li>
<li><p>フラット値、配列、ネストされたオブジェクトのような構造化データを格納できる<code translate="no">JSON</code> 型の<code translate="no">metadata</code> フィールド。</p></li>
</ul>
...
<h2 id="Insert-entities-with-JSON-data" class="common-anchor-header">JSON データを持つエンティティの挿入<button data-href="#Insert-entities-with-JSON-data" class="anchor-icon" translate="no">
    ...
    </button></h2><p>コレクションが作成されたら、<code translate="no">metadata</code> JSONフィールドに構造化JSONオブジェクトを含むエンティティを挿入する。</p>
<div class="multipleCode">
   <a href="#python">Python</a> <a href="#java">Java</a> <a href="#javascript">NodeJS</a> <a href="#go">Go</a> <a href="#bash">cURL</a></div>
...
<pre><code translate="no" class="language-bash"><span class="hljs-comment"># restful</span>
<button class="copy-code-btn"></button></code></pre>
<p>検索やクエリでこれらの式を使用するには、以下を確認してください：</p>
<ul>
...
<h3 id="Are-there-any-naming-conventions-for-JSON-field-keys" class="common-anchor-header">JSONフィールド・キーの命名規則はありますか？</h3><p>クエリとインデックス作成の互換性を確保するためです：</p>
<ul>
...
<p>インデックスが<code translate="no">json_cast_type=&quot;double&quot;</code> で作成された場合、数値フィルタ条件（例:<code translate="no">&gt;</code>,<code translate="no">&lt;</code>,<code translate="no">== 42</code> ）のみがインデックスを利用します。数値以外の条件は、ブルートフォーススキャンを強制します。</p></li>
<li><p><strong>文字列インデックス</strong>：</p>
<p>文字列インデクシング:<code translate="no">json_cast_type=&quot;varchar&quot;</code> を使用するインデックスの場合、文字列フィルタ条件のみがインデックスの恩恵を受ける。</p></li>
<li><p><strong>ブールインデックス</strong>：</p>
<p>ブール・インデクシングは、文字列インデクシングと似た振る舞いをし、条件が厳密に真か偽にマッチするときのみインデックスを使用します。</p></li>
</ul>
<h3 id="How-do-term-expressions-work-with-JSON-field-indexing" class="common-anchor-header">JSONフィールドインデックスで条件式はどのように機能しますか？</h3><p><code translate="no">json[&quot;field&quot;] IN [value1, value2, …]</code> のような条件式を使用して、エンティティをフィルタリングできます。</p>
<ul>
<li><p>インデックスが適用されるのは、対象となる値がスカラーである場合のみです。</p></li>
<li><p><code translate="no">json[&quot;field&quot;]</code> が配列の場合、クエリはインデックスを使用せず、総当たり検索にフォールバックします。</p></li>
</ul>
<h3 id="What-about-numeric-precision-when-indexing-JSON-fields" class="common-anchor-header">JSONフィールドにインデックスを付ける際の数値精度についてはどうですか？</h3><p>Milvusはインデックスされたすべての数値をdoubleとして保存します。</p>
<p>数値が<strong>2^53を</strong>超える場合、精度を失う可能性があります。この精度の低下により、フィルタクエリが範囲外の値に正確にマッチしなくなる可能性があります。</p>
<h3 id="How-does-Milvus-handle-data-integrity-for-JSON-field-indexing" class="common-anchor-header">MilvusはJSONフィールドインデックスのデータ整合性をどのように扱っていますか？</h3><p>Milvusは不整合なデータ型を自動的に変換したり正規化したりはしません。</p>
<p>例えば、<code translate="no">&quot;price&quot;: &quot;99.99&quot;</code> を文字列として格納する行と、<code translate="no">&quot;price&quot;: 99.99</code> を数値として格納する行があり、インデックスがdoubleとして定義されている場合、数値を持つ行のみがインデックス化されます。</p>
<p>矛盾が発生すると、インデックス作成時に該当する行はスキップされます。</p>
<h3 id="What-happens-if-type-casting-fails-when-indexing-a-JSON-field" class="common-anchor-header">JSONフィールドのインデックス作成時に型キャストに失敗した場合はどうなりますか?</h3><p>値が指定された<code translate="no">json_cast_type</code> にキャストできない場合（たとえば、<code translate="no">double</code> を期待しているときに数値以外の文字列が指定された場合など）、その値は無言でスキップされ、<strong>インデックスに含まれません</strong>。その結果、キャストに失敗したエンティティは、インデックスに依存する<strong>フィルタ結果から除外さ</strong>れます。</p>
<p>予期しないクエリの動作を回避するには、インデックス化された JSON パス下のすべての値が一貫して型付けされていることを確認します。</p>
<h3 id="Can-I-create-multiple-indexes-on-the-same-JSON-path-with-different-cast-types" class="common-anchor-header">同じ JSON パス上に、異なるキャスト型で複数のインデックスを作成できますか?</h3><p>いいえ。各 JSON パスがサポートする<strong>インデックスは 1 つだけです</strong>。データに一致する 1 つの<code translate="no">json_cast_type</code> を選択する必要があります。異なるキャスト・タイプで同じパスに複数のインデックスを作成することはサポートされていません。</p>
<h3 id="What-if-values-at-a-JSON-path-have-inconsistent-types" class="common-anchor-header">JSON パスの値に一貫性のない型がある場合はどうなりますか?</h3><p>エンティティ間で型が一貫していないと、<strong>部分的なインデックスが作成さ</strong>れる可能性があります。たとえば、<code translate="no">metadata[&quot;price&quot;]</code> が数値 (<code translate="no">99.99</code>) と文字列 (<code translate="no">&quot;99.99&quot;</code>) の両方として格納され、インデックスが<code translate="no">json_cast_type=&quot;double&quot;</code> で定義されている場合、数値のみがインデックス化されます。文字列形式のエントリはスキップされ、フィルタ結果には表示されません。</p>
