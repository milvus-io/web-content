...
title: 配列フィールド
summary: ARRAYフィールドには、同じデータ型の要素が順序付けられて格納されます。以下に、ARRAYフィールドによるデータの格納例を示します。
---
<h1 id="Array-Field" class="common-anchor-header">配列フィールド<button data-href="#Array-Field" class="anchor-icon" translate="no">
    ...
    </button></h1><p>ARRAYフィールドは、同じデータ型の要素からなる順序付き集合を格納します。以下に、ARRAYフィールドによるデータの格納例を示します。</p>
...
<li><p><strong>デフォルト値</strong>：ARRAYフィールドはデフォルト値をサポートしていません。ただし、<code translate="no">nullable</code> 属性を<code translate="no">True</code> に設定することで、NULL値を許可することができます。詳細については、「<a href="/docs/ja/v2.6.x/nullable-and-default.md">Nullable &amp; Default</a>」を参照してください。</p></li>
<li><p><code translate="no">element_type</code> データ型：<strong></strong>ARRAYフィールド内のすべての要素は、<code translate="no">element_type</code> パラメータで定義された同一のデータ型でなければなりません。 が<code translate="no">VARCHAR</code> に設定されている場合、配列要素の<code translate="no">max_length</code> も指定する必要があります。<code translate="no">element_type</code> は、<code translate="no">JSON</code> を除く、Milvusがサポートするすべてのスカラーデータ型を受け入れます。</p></li>
<li><p><strong>配列の容量</strong>：ARRAYフィールドの要素数は、<code translate="no">max_capacity</code> で指定されているように、配列の作成時に定義された最大容量以下でなければなりません。この値<strong>は</strong> <strong>1から4096までの</strong>整数である必要があります。</p></li>
<li><p><strong>文字列の取り扱い</strong>：配列フィールド内の文字列値は、セマンティックなエスケープや変換を行わずに、そのままの形で格納されます。例えば、<code translate="no">'a&quot;b'</code> 、<code translate="no">&quot;a'b&quot;</code> 、<code translate="no">'a\'b'</code> 、および<code translate="no">&quot;a\&quot;b&quot;</code> は入力されたまま格納されますが、<code translate="no">'a'b'</code> および<code translate="no">&quot;a&quot;b&quot;</code> は無効な値とみなされます。</p></li>
</ul>
<h2 id="Add-ARRAY-field" class="common-anchor-header">ARRAYフィールドの追加<button data-href="#Add-ARRAY-field" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>Milvus で ARRAY フィールドを使用するには、コレクションスキーマの作成時に該当するフィールド型を定義します。この手順には以下が含まれます：</p>
<ol>
<li><p><code translate="no">datatype</code> を、サポートされている配列データ型である<code translate="no">ARRAY</code> に設定します。</p></li>
<li><p><code translate="no">element_type</code> パラメータを使用して、配列内の要素のデータ型を指定します。同じ配列内のすべての要素は、同じデータ型でなければなりません。</p></li>
<li><p><code translate="no">max_capacity</code> パラメータを使用して、配列の最大容量（つまり、配列が格納できる要素の最大数）を定義します。</p></li>
</ol>
<p>ARRAYフィールドを含むコレクションスキーマを定義する方法は次のとおりです：</p>
<div class="alert note">
<p>スキーマ定義時に `<code translate="no">enable_dynamic_fields=True</code> ` を設定すると、Milvus では事前に定義されていないスカラーフィールドを挿入できるようになります。ただし、これによりクエリや管理の複雑さが増し、パフォーマンスに影響を与える可能性があります。詳細については、<a href="/docs/ja/v2.6.x/enable-dynamic-field.md">「動的フィールド」</a>を参照してください。</p>
</div>
...
<p>次の例では、ベクトルフィールド `<code translate="no">embedding</code> ` および ARRAY フィールド `<code translate="no">tags</code>` に対して、いずれも `<code translate="no">AUTOINDEX</code> ` インデックス型を使用してインデックスを作成しています。この型を使用すると、Milvus はデータ型に基づいて最適なインデックスを自動的に選択します。また、フィールドごとにインデックス型やパラメータをカスタマイズすることも可能です。詳細については、<a href="/docs/ja/v2.6.x/index-explained.md">「インデックスの解説</a>」を参照してください。</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#go">   Go</a>
...
<h2 id="Create-collection" class="common-anchor-header">コレクションの作成<button data-href="#Create-collection" class="anchor-icon" translate="no">
    ...
    </button></h2><p>スキーマとインデックスを定義したら、ARRAYフィールドを含むコレクションを作成します。</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#go">   Go</a>
...
<h2 id="Insert-data" class="common-anchor-header">データの挿入<button data-href="#Insert-data" class="anchor-icon" translate="no">
    ...
    </button></h2><p>コレクションを作成したら、ARRAYフィールドを含むデータを挿入できます。</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#go">   Go</a>
...
<button class="copy-code-btn"></button></code></pre>
<div class="alert note">
<p>完全な配列の挿入に加え、<code translate="no">ARRAY</code> フィールドは、Milvus v2.6.17以降の<code translate="no">upsert</code> APIにおける<code translate="no">ARRAY_APPEND</code> および<code translate="no">ARRAY_REMOVE</code> の部分更新演算子もサポートしています。これらを使用すると、現在の値を事前に取得することなく、既存の配列に要素を追加したり、一致する要素を削除したりできます。これにより、クライアント側の「読み取り-変更-書き込み」パターンを回避できます。 詳細については、「<a href="/docs/ja/v2.6.x/upsert-entities.md#Upsert-ARRAY-fields-with-partial-update-operators">部分更新演算子を使用したARRAYフィールドのアップサート</a>」を参照してください。</p>
</div>
...
<p><code translate="no">tags</code> が null ではないエンティティを取得するには：</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#go">   Go</a>
...
<button class="copy-code-btn"></button></code></pre>
<p><code translate="no">ratings</code> の最初の要素の値が 4 より大きいエンティティを取得するには：</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#go">   Go</a>
...
<h2 id="Vector-search-with-filter-expressions" class="common-anchor-header">フィルタ式を使用したベクトル検索<button data-href="#Vector-search-with-filter-expressions" class="anchor-icon" translate="no">
    ...
    </button></h2><p>基本的なスカラーフィールドによるフィルタリングに加え、ベクトル類似度検索とスカラーフィールドのフィルタを組み合わせることができます。たとえば、次のコードは、ベクトル検索にスカラーフィールドのフィルタを追加する方法を示しています:</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#go">   Go</a>
...
<button class="copy-code-btn"></button></code></pre>
<p>さらに、Milvusは<code translate="no">ARRAY_CONTAINS</code> 、<code translate="no">ARRAY_CONTAINS_ALL</code> 、<code translate="no">ARRAY_CONTAINS_ANY</code> 、<code translate="no">ARRAY_LENGTH</code> といった高度な配列フィルタ演算子をサポートしており、クエリ機能をさらに強化できます。詳細については、「<a href="/docs/ja/v2.6.x/array-operators.md">ARRAY演算子</a>」を参照してください。</p>
