...
<p>Vertex AIは、様々なユースケースに対応した埋め込みモデルをサポートしています：</p>
<ul>
<li><p>text-embedding-005 (最新のテキスト埋め込みモデル)</p></li>
<li><p>text-multilingual-embedding-002 (最新の多言語テキスト埋め込みモデル)</p></li>
</ul>
<p>詳細は、<a href="https://cloud.google.com/vertex-ai/generative-ai/docs/model-reference/text-embeddings">Vertex AIテキスト埋め込みモデルリファレンスを</a>ご参照ください。</p>
<h2 id="Vertex-AI-deployment" class="common-anchor-header">Vertex AI の導入<button data-href="#Vertex-AI-deployment" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>Vertex AI機能をMilvusに導入する前に、MilvusインスタンスがGoogle Cloudサービスのアカウント情報を使用するように設定する必要があります。Milvusは主に2つのデプロイアプローチをサポートしています：</p>
<h3 id="Standard-deployment-Docker-Compose" class="common-anchor-header">標準デプロイ（Docker Compose）</h3><p>docker-compose.yamlファイルで、クレデンシャルファイルをマウントし、<code translate="no">MILVUSAI_GOOGLE_APPLICATION_CREDENTIALS</code> 環境変数を設定する必要があります。</p>
...
<h3 id="Milvus-Helm-Chart-deployment-Kubernetes" class="common-anchor-header">Milvus Helm Chart デプロイメント (Kubernetes)</h3><p>Kubernetes環境では、クレデンシャルファイルを保存するためにKubernetes Secretを使用することを推奨します：</p>
<ol>
<li><p><strong>シークレットの作成</strong></p>
...
<li><p><strong>values.yamlの設定</strong></p>
<p>standaloneまたはproxy/dataNodeセクションの下に以下を追加します：</p>
...
<h2 id="Configuration-in-Milvus" class="common-anchor-header">Milvusでの設定<button data-href="#Configuration-in-Milvus" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Vertex AIクレデンシャルをデプロイした後は、エンベッディング機能を設定する必要があります。MilvusはVertex AI用の認証情報を設定するための複数の方法をサポートしており、以下の優先順位で適用されます：</p>
<ul>
<li><p><strong>Milvus設定ファイル(milvus.yaml)</strong>- 最も優先順位が高い。</p></li>
<li><p><strong>環境変数</strong>- 最も優先順位が低い</p></li>
</ul>
<p><strong>Milvus 設定ファイル (milvus.yaml)</strong></p>
<p>永続的なクラスタ全体の設定の場合、クレデンシャルjsonデータをbase64形式でエンコードし、milvus.yamlファイルで定義することができます。<code translate="no">cat credentials.json|jq .|base64</code>replace<code translate="no">credentials.json</code> to your credential file path.</p>
...
<p>環境変数は、Docker ComposeやKubernetesのデプロイでコンテナ環境を設定するときによく使われる、別の設定方法を提供します。</p>
...
<h2 id="Use-embedding-function" class="common-anchor-header">埋め込み機能を使用する<button data-href="#Use-embedding-function" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Vertex AI が設定されたら、以下の手順に従って埋め込み関数を定義し、使用します。</p>
