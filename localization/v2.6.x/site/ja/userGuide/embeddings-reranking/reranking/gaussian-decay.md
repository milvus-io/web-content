...
<h3 id="Create-a-decay-ranker" class="common-anchor-header">ディケイランカーの作成<button data-href="#Create-a-decay-ranker" class="anchor-icon" translate="no">
    ...
    </button></h3><p>コレクションに数値フィールド（この例では、ユーザーからのメートルで<code translate="no">distance</code> ）を設定したら、ガウス減衰ランカーを作成します：</p>
...
<button class="copy-code-btn"></button></code></pre>
<h3 id="Apply-to-standard-vector-search" class="common-anchor-header">標準的なベクトル検索に適用<button data-href="#Apply-to-standard-vector-search" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
