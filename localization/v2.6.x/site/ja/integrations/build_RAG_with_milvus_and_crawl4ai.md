...
<h3 id="Setting-Up-OpenAI-API-Key" class="common-anchor-header">OpenAI APIキーの設定<button data-href="#Setting-Up-OpenAI-API-Key" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>この例では、OpenAIをLLMとして使用します。<a href="https://platform.openai.com/docs/quickstart">OPENAI_API_KEYを</a>環境変数として用意してください。</p>
...
<h3 id="Process-the-Crawled-Content" class="common-anchor-header">クロールされたコンテンツを処理する<button data-href="#Process-the-Crawled-Content" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>クロールされたコンテンツをMilvusに挿入するために管理しやすくするために、単純に "# " を使ってコンテンツを区切ります。これは、クロールされたマークダウンファイルの各主要部分のコンテンツを大まかに区切ることができます。</p>
...
<h2 id="Build-RAG" class="common-anchor-header">RAGの構築<button data-href="#Build-RAG" class="anchor-icon" translate="no">
    ...
    </button></h3><p>先ほどクロールしたウェブサイトに関するクエリの質問を指定してみよう。</p>
