...
summary: このガイドでは、LangExtractとMilvusを使用して、インテリジェントな文書処理・検索システムを構築する方法を説明します。
title: LangExtract + Milvusインテグレーション
---
<h1 id="LangExtract-+-Milvus-Integration" class="common-anchor-header">LangExtract + Milvusインテグレーション<button data-href="#LangExtract-+-Milvus-Integration" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<button class="copy-code-btn"></button></code></pre>
<h2 id="Define-the-LangExtract-+-Milvus-pipeline" class="common-anchor-header">LangExtract + Milvusパイプラインの定義<button data-href="#Define-the-LangExtract-+-Milvus-pipeline" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>構造化情報抽出にLangExtract、ベクトルストアにMilvusを使用するパイプラインを定義します。</p>
...
<button class="copy-code-btn"></button></code></pre>
<h2 id="Configuration-and-Setup" class="common-anchor-header">設定とセットアップ<button data-href="#Configuration-and-Setup" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>統合のためのグローバルパラメータを設定しましょう。Geminiの埋め込みモデルを使用して、ドキュメントのベクトル表現を生成します。</p>
...
<button class="copy-code-btn"></button></code></pre>
<h2 id="Initialize-Milvus-Client" class="common-anchor-header">Milvusクライアントの初期化<button data-href="#Initialize-Milvus-Client" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>それでは、Milvusクライアントを初期化しましょう。ここでは簡単のため、ローカルのデータベースファイルを使用しますが、Milvusサーバをフルに展開することも可能です。</p>
...
<h2 id="Sample-Data-Preparation" class="common-anchor-header">サンプルデータの準備<button data-href="#Sample-Data-Preparation" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>このデモでは、サンプルドキュメントとして映画の説明を使用します。これは、非構造化テキストからジャンル、キャラクター、テーマのような構造化情報を抽出するLangExtractの能力を紹介します。</p>
...
<h2 id="Setting-Up-the-Milvus-Collection" class="common-anchor-header">Milvusコレクションのセットアップ<button data-href="#Setting-Up-the-Milvus-Collection" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>抽出したデータを保存する前に、適切なスキーマを持つMilvusコレクションを作成する必要があります。このコレクションには、元のドキュメントテキスト、ベクトル埋め込み、抽出されたメタデータフィールドが格納される。</p>
...
<h2 id="Defining-the-Extraction-Schema" class="common-anchor-header">抽出スキーマの定義<button data-href="#Defining-the-Extraction-Schema" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>LangExtractはプロンプトと例を使用して、LLMが構造化情報を抽出するのをガイドします。映画の説明のための抽出スキーマを定義して、抽出する情報とそれをどのように分類するかを指定しましょう。</p>
...
<h2 id="Providing-Examples-for-Better-Extraction" class="common-anchor-header">より良い抽出のための例の提供<button data-href="#Providing-Examples-for-Better-Extraction" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>抽出の品質と一貫性を向上させるために、LangExtractにいくつかの例を提供します。こ れ ら の例は、 予期 さ れ る 形式を実証 し 、 私達の抽出要件をモデルが理解す る のを助け ます。</p>
...
<button class="copy-code-btn"></button></code></pre>
<h2 id="Processing-and-Vectorizing-the-Results" class="common-anchor-header">結果の処理とベクトル化<button data-href="#Processing-and-Vectorizing-the-Results" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>次に、抽出結果を処理して、各文書のベクトル埋め込みを生成する必要があります。また、Milvusで簡単に検索できるように、抽出された属性を別々のフィールドに平坦化します。</p>
...
<h2 id="Inserting-Data-into-Milvus" class="common-anchor-header">Milvusへのデータ挿入<button data-href="#Inserting-Data-into-Milvus" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>処理したデータの準備ができたので、Milvusコレクションに挿入してみましょう。これにより、セマンティック検索と正確なメタデータフィルタリングの両方が可能になる。</p>
...
<h2 id="Demonstrating-Metadata-Filtering" class="common-anchor-header">メタデータフィルタリングのデモ<button data-href="#Demonstrating-Metadata-Filtering" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>LangExtractとMilvusを組み合わせることの重要な利点の一つは、抽出されたメタデータに基づいて正確なフィルタリングを実行できることです。いくつかのフィルタ式検索で実演してみましょう。</p>
...
<h2 id="Combining-Semantic-Search-with-Metadata-Filtering" class="common-anchor-header">セマンティック検索とメタデータフィルタリングの組み合わせ<button data-href="#Combining-Semantic-Search-with-Metadata-Filtering" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>この統合の真の威力は、セマンティックベクトル検索と正確なメタデータフィルタリングを組み合わせることにある。これにより、抽出された属性に基づいた特定の制約を適用しながら、意味的に類似したコンテンツを見つけることができる。</p>
