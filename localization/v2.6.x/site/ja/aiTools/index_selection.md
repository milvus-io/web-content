...
title: プロンプトMilvusインデックス選択
summary: AIコーディングアシスタントがMilvusインデックスを選択・設定するためのルール。
---
<h1 id="Index-Selection" class="common-anchor-header">インデックス選択<button data-href="#Index-Selection" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h1><p>AUTOINDEX、HNSW、DiskANN、IVF、スパースインデックスを含むMilvusインデックスの選択とチューニングのための決定ガイドと設定ルール。これらのルールを自動的に適用するには、以下のプロンプトをAIツールにコピーしてください。全てのプロンプトの概要については、<a href="/docs/ja/milvus_for_agents.md">AIプロンプトを</a>参照してください。</p>
