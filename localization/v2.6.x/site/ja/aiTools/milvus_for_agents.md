...
<div class="start_card_container">
  <a href="/docs/ja/integrations_overview.md" style="text-decoration: none; color: inherit;">
    <p class="link-btn" style="font-size: 1rem; white-space: nowrap;">エージェントフレームワーク</p>
    <p style="font-size: 0.875rem; font-weight: 400; color: #555;">LangChain、LlamaIndex、OpenAI Agents、その他のエージェントオーケストレーションフレームワークとの統合。</p>
  </a>
</div>
</div>
<h2 id="AI-prompts" class="common-anchor-header">AIプロンプト<button data-href="#AI-prompts" class="anchor-icon" translate="no">
    ...
    </button></h2><p>AIコーディングアシスタントがMilvusの正しいコードを書くのを助けるプロンプトが用意されています。各プロンプトには、最も一般的なミスを防ぐルールとパターンがエンコードされています。</p>
...
<div class="start_card_container">
  <a href="/docs/ja/search_patterns.md" style="text-decoration: none; color: inherit;">
    ...
    <p style="font-size: 0.875rem; font-weight: 400; color: #555;">クリティカルな制約ルールによるANN検索、ハイブリッド検索、全文検索。</p>
  </a>
</div>
<div class="start_card_container">
...
<tr><td>プロダクション</td><td><a href="https://cloud.zilliz.com/signup">Zillizクラウド</a></td><td>フルマネージド、サーバーレスのMilvus。管理するインフラはなく、エージェントは接続して操作するだけです。</td></tr>
...
<p>エージェントのワークロードについては、<strong>Zilliz Cloudを</strong>本番環境で使用することを推奨します。エージェントは通常インフラを管理しないため、サーバーレス・デプロイメントでは運用のオーバーヘッドがなくなり、自動スケーリングが可能になります。</p>
</div>
<h2 id="Quick-connection-examples" class="common-anchor-header">クイック接続例<button data-href="#Quick-connection-examples" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>エージェントコードからMilvusに接続します：</p>
...
<li><a href="/docs/ja/quickstart.md">クイックスタート</a>- 最初のMilvus検索を数分で実行します。</li>
<li><a href="/docs/ja/integrations_overview.md">エージェントフレームワークの統合</a>- MilvusとLangChain、LlamaIndex、OpenAIエージェントなどを接続します。</li>
</ul>
