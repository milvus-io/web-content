...
<p>밀집 벡터는 주로 시맨틱 검색이나 추천 시스템과 같이 데이터의 의미론을 이해해야 하는 상황에서 사용됩니다. 시맨틱 검색에서 밀집 벡터는 쿼리와 문서 간의 근본적인 연관성을 포착하여 검색 결과의 관련성을 높이는 데 도움이 됩니다. 추천 시스템에서는 사용자와 항목 간의 유사성을 식별하여 더욱 개인화된 제안을 제공하는 데 기여합니다.</p>
<h2 id="Overview" class="common-anchor-header">개요<button data-href="#Overview" class="anchor-icon" translate="no">
    ...
    </button></h2><p>밀도 높은 벡터는 일반적으로 <code translate="no">[0.2, 0.7, 0.1, 0.8, 0.3, ..., 0.5]</code> 와 같이 고정 길이의 부동 소수점 숫자 배열로 표현됩니다. 이러한 벡터의 차원은 보통 128, 256, 768, 1024와 같이 수백에서 수천까지 다양합니다. 각 차원은 객체의 특정 의미적 특징을 포착하므로, 유사도 계산을 통해 다양한 시나리오에 적용할 수 있습니다.</p>
...
<li><p><strong>의미적 관계:</strong> 점들 간의 거리는 개념 간의 의미적 유사성을 반영합니다. 서로 가까운 점일수록 개념 간에 의미적 관련성이 더 높음을 나타냅니다.</p></li>
...
<p>밀집 벡터 외에도 Milvus는 스파스 벡터와 이진 벡터를 지원합니다. 스파스 벡터는 키워드 검색 및 용어 일치와 같이 특정 용어를 기반으로 한 정밀한 일치에 적합하며, 이진 벡터는 이미지 패턴 일치 및 특정 해싱 응용 프로그램과 같이 이진화된 데이터를 효율적으로 처리하는 데 일반적으로 사용됩니다. 자세한 내용은 <a href="/docs/ko/v2.6.x/binary-vector.md">이진 벡터</a> 및 <a href="/docs/ko/v2.6.x/sparse_vector.md">스파스 벡터</a> 섹션을 참조하십시오.</p>
</div>
...
<ol>
<li><p><code translate="no">datatype</code> 를 지원되는 빽빽한 벡터 데이터 유형으로 설정합니다. 지원되는 빽빽한 벡터 데이터 유형에 대해서는 ‘데이터 유형’을 참조하십시오.</p></li>
...
<table>
   ...
   <tr>
     ...
     <td><p>32비트 부동 소수점 숫자를 저장하며, 과학 계산 및 머신 러닝에서 실수를 표현하는 데 일반적으로 사용됩니다. 유사한 벡터를 구별하는 등 높은 정밀도가 필요한 시나리오에 이상적입니다.</p></td>
   </tr>
   <tr>
     <td><p><code translate="no">FLOAT16_VECTOR</code></p></td>
     <td><p>16비트 반정밀도 부동소수점 숫자를 저장하며, 딥 러닝 및 GPU 계산에 사용됩니다. 추천 시스템의 저정밀도 리콜 단계와 같이 정밀도가 그다지 중요하지 않은 상황에서 저장 공간을 절약해 줍니다.</p></td>
   </tr>
   <tr>
     <td><p><code translate="no">BFLOAT16_VECTOR</code></p></td>
     <td><p>16비트 Brain 부동소수점(bfloat16) 수를 저장하며, Float32와 동일한 지수 범위를 제공하지만 정밀도는 낮습니다. 대규모 이미지 검색과 같이 대량의 벡터를 빠르게 처리해야 하는 시나리오에 적합합니다.</p></td>
   </tr>
   <tr>
...
<p>Milvus는 더 나은 벡터 검색 경험을 위해 다양한 인덱스 유형을 제공합니다. AUTOINDEX는 벡터 검색의 학습 곡선을 완화하도록 설계된 특수한 인덱스 유형입니다. 선택할 수 있는 인덱스 유형은 매우 다양합니다. 자세한 내용은 xxx를 참조하십시오.</p>
...
<h3 id="Insert-data" class="common-anchor-header">데이터 삽입<button data-href="#Insert-data" class="anchor-icon" translate="no">
    ...
    </button></h3><p>컬렉션을 생성한 후, ` <code translate="no">insert</code> ` 메서드를 사용하여 밀집 벡터가 포함된 데이터를 추가합니다. 삽입하는 밀집 벡터의 차원이 밀집 벡터 필드를 추가할 때 정의한 ` <code translate="no">dim</code> ` 값과 일치하는지 확인하십시오.</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#javascript">   NodeJS</a>
...
<h3 id="Perform-similarity-search" class="common-anchor-header">유사도 검색 수행<button data-href="#Perform-similarity-search" class="anchor-icon" translate="no">
    ...
    </button></h3><p>밀집 벡터를 기반으로 한 시맨틱 검색은 Milvus의 핵심 기능 중 하나로, 벡터 간의 거리를 바탕으로 쿼리 벡터와 가장 유사한 데이터를 빠르게 찾을 수 있게 해줍니다. 유사도 검색을 수행하려면 쿼리 벡터와 검색 매개변수를 준비한 다음, <code translate="no">search</code> 메서드를 호출하십시오.</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#javascript">   NodeJS</a>
