...
<h3 id="Step-1-Define-a-GEOMETRY-field" class="common-anchor-header">1단계: 기하학 필드 정의하기<button data-href="#Step-1-Define-a-GEOMETRY-field" class="anchor-icon" translate="no">
    ...
    </button></h3><p><code translate="no">GEOMETRY</code> 필드를 사용하려면 컬렉션을 만들 때 컬렉션 스키마에 명시적으로 정의합니다. 다음 예제에서는 <code translate="no">DataType.GEOMETRY</code> 유형의 <code translate="no">geo</code> 필드가 있는 컬렉션을 만드는 방법을 보여 줍니다.</p>
