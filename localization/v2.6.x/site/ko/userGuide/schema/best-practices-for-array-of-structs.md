...
<h2 id="Why-Array-of-Structs" class="common-anchor-header">왜 구조체 배열인가?<button data-href="#Why-Array-of-Structs" class="anchor-icon" translate="no">
    ...
    </button></h2><p>자율 주행부터 멀티모달 검색에 이르기까지 최신 AI 애플리케이션은 점점 더 중첩된 이기종 데이터에 의존하고 있습니다. 기존의 평면 데이터 모델로는<strong>'주석이 달린 청크가 많은 하나의 문서</strong>'나<strong>'여러 개의 운전 장면이 관찰되는 하나의 주행 장면</strong>'과 같은 복잡한 관계를 표현하는 데 어려움을 겪습니다. 바로 이 부분에서 Milvus의 구조체 배열 데이터 유형이 빛을 발합니다.</p>
...
<h3 id="Step-3-Set-index-parameters" class="common-anchor-header">3단계: 인덱스 매개변수 설정<button data-href="#Step-3-Set-index-parameters" class="anchor-icon" translate="no">
    ...
    </button></h3><p>모든 벡터 필드는 색인화되어야 합니다. Struct 요소의 벡터 필드를 색인하려면 색인 유형으로 <code translate="no">EMB_LIST_HNSW</code> 및 <code translate="no">MAX_SIM</code> 메트릭 유형을 사용하여 벡터 임베딩 간의 유사성을 측정해야 합니다.</p>
