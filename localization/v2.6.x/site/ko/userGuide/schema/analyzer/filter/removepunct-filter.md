...
<h1 id="Remove-Punct" class="common-anchor-header">구두점 제거<span class="beta-tag" style="background-color:rgb(0, 179, 255);color:white" translate="no">Compatible with Milvus 2.5.11+</span><button data-href="#Remove-Punct" class="anchor-icon" translate="no">
    ...
    </button></h1><p><code translate="no">removepunct</code> 필터는 <code translate="no">jieba</code>, <code translate="no">lindera</code>, <code translate="no">icu</code>과 같은 일부 토큰라이저가 일반적으로 유지하는 구두점, 공백, 줄 바꿈 등을 제거합니다. 쉼표, 마침표 및 기타 문장 부호가 없는 의미 있는 텍스트 토큰만 포함된 깔끔한 토큰 스트림을 원할 때 이 필터를 사용하세요.</p>
...
<button class="copy-code-btn"></button></code></pre>
<p><code translate="no">removepunct</code> 필터는 토큰 생성기에 의해 생성된 조건에서 작동하므로 토큰 생성기와 함께 사용해야 합니다.</p>
...
<pre><code translate="no" class="language-bash"><span class="hljs-comment"># restful</span>
<button class="copy-code-btn"></button></code></pre>
<h3 id="Expected-output" class="common-anchor-header">예상 출력</h3><pre><code translate="no" class="language-plaintext">[&#x27;Привет&#x27;, &#x27;Как&#x27;, &#x27;дела&#x27;]
<button class="copy-code-btn"></button></code></pre>
