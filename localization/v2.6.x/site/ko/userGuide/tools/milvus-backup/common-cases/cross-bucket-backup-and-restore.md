...
<p>두 개의 Milvus 인스턴스( <code translate="no">milvus_A</code> 및 <code translate="no">milvus_B</code>)가 있으며, 둘 다 기본 MinIO 스토리지 엔진을 오브젝트 스토리지에 사용한다고 가정해 보겠습니다. 이 인스턴스는 동일한 오브젝트 스토리지 내에서 서로 다른 버킷 bucket_A와 <code translate="no">bucket_B</code> 를 사용합니다. 이 예제에서는 다음 작업을 완료하는 것이 목표입니다:</p>
<ol>
