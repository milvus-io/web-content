...
<h1 id="IVFPQ" class="common-anchor-header">IVF_PQ<button data-href="#IVFPQ" class="anchor-icon" translate="no">
    ...
    </button></h1><p><strong>IVF_PQ</strong> 인덱스는 고차원 공간에서 대략적인 최접근 이웃 검색을 위한 <strong>양자화 기반</strong> 인덱싱 알고리즘입니다. 일부 그래프 기반 방법만큼 빠르지는 않지만 <strong>IVF_PQ는</strong> 훨씬 적은 메모리를 필요로 하므로 대용량 데이터 세트에 실용적인 선택입니다.</p>
