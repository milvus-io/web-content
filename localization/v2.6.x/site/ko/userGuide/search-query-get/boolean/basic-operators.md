...
<h3 id="Example-1-Using-IN-to-Match-Multiple-Values" class="common-anchor-header">예 1: <code translate="no">IN</code> 을 사용하여 여러 값 일치시키기<button data-href="#Example-1-Using-IN-to-Match-Multiple-Values" class="anchor-icon" translate="no">
    ...
    </button></h3><p><code translate="no">color</code> 가 "빨간색", "녹색" 또는 "파란색"인 모든 엔티티를 찾고자 하는 경우:</p>
...
<p>질리즈 클라우드는 필터 조건의 일치값 목록에 길이 제한을 두지 않습니다. 하지만 지나치게 긴 목록은 쿼리 성능에 큰 영향을 줄 수 있습니다. 필터 조건에 일치 값 목록이 길거나 요소가 많은 복잡한 표현식이 포함된 경우 <a href="/docs/ko/filtering-templating.md">필터 템플릿을</a> 사용하여 쿼리 성능을 개선하는 것이 좋습니다.</p>
