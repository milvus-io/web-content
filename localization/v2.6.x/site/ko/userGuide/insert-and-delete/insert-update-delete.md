...
<ul>
<li><p><strong>컬렉션 생성 후 추가된 필드</strong>: 컬렉션 생성 후 컬렉션에 새 필드를 추가하고 삽입 중에 값을 지정하지 않으면 Milvus는 정의된 기본값으로 필드를 자동으로 채우거나 기본값이 설정되지 않은 경우 NULL로 채웁니다. 자세한 내용은 <a href="/docs/ko/v2.6.x/add-fields-to-an-existing-collection.md">기존 컬렉션에 필드 추가하기를</a> 참조하세요.</p></li>
<li><p><strong>중복 처리</strong>: 표준 <code translate="no">insert</code> 작업은 중복된 기본 키를 확인하지 않습니다. 기존 기본 키를 사용하여 데이터를 삽입하면 동일한 키를 가진 새 엔티티가 생성되어 데이터 중복 및 잠재적인 애플리케이션 문제가 발생할 수 있습니다. 기존 엔티티를 업데이트하거나 중복을 방지하려면 대신 <strong><code translate="no">upsert</code></strong> 작업을 대신 사용하세요. 자세한 내용은 <a href="/docs/ko/v2.6.x/upsert-entities.md">엔티티 삽입을</a> 참조하세요.</p></li>
</ul>
</div>
