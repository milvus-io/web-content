...
<p>개발자와 기업은 Lepton<a href="https://www.lepton.ai/">AI를</a> 통해 몇 분 안에 프로덕션에 사용할 수 있는 규모로 AI 애플리케이션을 효율적으로 실행할 수 있습니다. Lepton AI를 사용하면 Python 네이티브 방식으로 모델을 구축하고, 로컬에서 모델을 디버그 및 테스트하고, 단일 명령으로 클라우드에 배포하고, 간단하고 유연한 API로 모든 애플리케이션에서 모델을 소비할 수 있습니다. 대규모 언어 모델(LLM) 및 확산 모델을 비롯한 다양한 AI 모델을 대규모 인프라 설정 없이도 배포할 수 있는 포괄적인 환경을 제공합니다.</p>
<p>이 튜토리얼에서는 Milvus 및 Lepton AI를 사용하여 RAG(검색 증강 생성) 파이프라인을 구축하는 방법을 보여드립니다.</p>
<h2 id="Preparation" class="common-anchor-header">준비 사항<button data-href="#Preparation" class="anchor-icon" translate="no">
    ...
    </button></h2><h3 id="Dependencies-and-Environment" class="common-anchor-header">종속성 및 환경</h3><pre><code translate="no" class="language-shell"><span class="hljs-meta prompt_">$ </span><span class="language-bash">pip install --upgrade pymilvus[model] openai requests tqdm</span>
...
<p>Lepton은 OpenAI 스타일 API를 지원합니다. 공식 웹사이트에 로그인하여 환경 변수로 <code translate="no">LEPTONAI_TOKEN</code> <a href="https://www.lepton.ai/docs">API 키를</a> 준비할 수 있습니다.</p>
...
<button class="copy-code-btn"></button></code></pre>
<h3 id="Prepare-the-LLM-and-Embedding-Model" class="common-anchor-header">LLM 및 임베딩 모델 준비</h3><p>Lepton은 OpenAI 스타일의 API를 지원하며, 약간의 조정을 통해 동일한 API를 사용하여 LLM을 호출할 수 있습니다.</p>
...
<p>훌륭합니다! Milvus와 Lepton AI로 RAG 파이프라인을 성공적으로 구축했습니다.</p>
