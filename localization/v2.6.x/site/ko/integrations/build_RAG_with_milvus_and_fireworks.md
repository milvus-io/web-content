...
summary: 이 튜토리얼에서는 Milvus와 Fireworks AI로 RAG(검색 증강 생성) 파이프라인을 구축하는 방법을 보여드리겠습니다.
title: Milvus 및 Fireworks AI로 RAG 구축하기
---
<h1 id="Build-RAG-with-Milvus-and-Fireworks-AI" class="common-anchor-header">Milvus 및 Fireworks AI로 RAG 구축하기<button data-href="#Build-RAG-with-Milvus-and-Fireworks-AI" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<h2 id="Preparation" class="common-anchor-header">준비 사항<button data-href="#Preparation" class="anchor-icon" translate="no">
    ...
    </button></h2><h3 id="Dependencies-and-Environment" class="common-anchor-header">종속성 및 환경</h3><pre><code translate="no" class="language-shell"><span class="hljs-meta prompt_">$ </span><span class="language-bash">pip install --upgrade pymilvus openai requests tqdm</span>
...
<p>Fireworks AI는 OpenAI 스타일 API를 지원합니다. 공식 웹사이트에 로그인하여 환경 변수로 <code translate="no">FIREWORKS_API_KEY</code> <a href="https://docs.fireworks.ai/getting-started/introduction">API 키를</a> 준비할 수 있습니다.</p>
...
<button class="copy-code-btn"></button></code></pre>
<h3 id="Prepare-the-LLM-and-Embedding-Model" class="common-anchor-header">LLM 및 임베딩 모델 준비</h3><p>클라이언트를 초기화하여 LLM과 임베딩 모델을 준비합니다. Fireworks AI는 OpenAI 스타일의 API를 지원하며, 약간의 조정을 통해 동일한 API를 사용하여 임베딩 모델과 LLM을 호출할 수 있습니다.</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>클라이언트를 사용하여 텍스트 임베딩을 생성하는 함수를 정의합니다. <code translate="no">nomic-ai/nomic-embed-text-v1.5</code> 모델을 예로 사용합니다.</p>
...
&quot;&quot;&quot;</span>
<button class="copy-code-btn"></button></code></pre>
<p>Fireworks에서 제공하는 <code translate="no">llama-v3p1-405b-instruct</code> 모델을 사용하여 프롬프트에 따라 응답을 생성합니다.</p>
...
<p>훌륭합니다! Milvus와 Fireworks AI로 RAG 파이프라인을 성공적으로 구축했습니다.</p>
