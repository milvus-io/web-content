...
<p>Milvus는 완전히 분리된 스토리지 및 컴퓨팅 계층을 갖춘 공유 스토리지 아키텍처를 채택하여 컴퓨팅 노드의 수평적 확장이 가능합니다. 우드페커를 제로 디스크 쓰기 전 로그(WAL) 계층으로 구현함으로써 Milvus는 운영 오버헤드를 줄이면서 더욱 탄력적이고 클라우드 네이티브가 되었습니다. 데이터 플레인과 제어 플레인 분리 원칙에 따라 Milvus는 액세스 레이어, 코디네이터 서비스, 워커 노드, 스토리지의 <a href="/docs/ko/four_layers.md">네 가지 레이어로</a> 구성되며, 각 <a href="/docs/ko/four_layers.md">레이어는</a> 다른 <a href="/docs/ko/four_layers.md">레이어에</a> 영향을 주지 않고 확장하거나 복구할 수 있습니다.</p>
<p>
