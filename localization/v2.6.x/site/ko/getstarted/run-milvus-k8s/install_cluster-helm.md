...
<p>다음 명령어는 권장 메시지 큐로 Woodpecker를 사용하여 v2.6.22에 최적화된 설정으로 Milvus 클러스터를 배포합니다:</p>
<pre><code translate="no" class="language-bash">helm install my-release zilliztech/milvus \
  --<span class="hljs-built_in">set</span> image.all.tag=v2.6.22 \
  --<span class="hljs-built_in">set</span> pulsarv3.enabled=<span class="hljs-literal">false</span> \
  --<span class="hljs-built_in">set</span> woodpecker.enabled=<span class="hljs-literal">true</span> \
...
<ul>
<li><a href="https://milvus.io/tools/sizing">Milvus 사이징 도구를</a> 사용하여 데이터 크기에 따라 설정을 최적화하십시오</li>
...
<button class="copy-code-btn"></button></code></pre>
<p><strong>모든 파드가 “Running” 상태를 표시할 때까지 기다리십시오.</strong> v2.6.22 구성의 경우 다음과 유사한 파드 상태를 확인할 수 있습니다:</p>
