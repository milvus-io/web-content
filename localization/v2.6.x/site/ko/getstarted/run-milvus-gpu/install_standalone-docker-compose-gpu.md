...
title: Docker Compose를 사용하여 GPU를 지원하는 Milvus 실행하기
---
<h1 id="Run-Milvus-with-GPU-Support-Using-Docker-Compose" class="common-anchor-header">Docker Compose를 사용하여 GPU를 지원하는 Milvus 실행하기<button data-href="#Run-Milvus-with-GPU-Support-Using-Docker-Compose" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h1><p>이 페이지에서는 Docker Compose를 사용하여 GPU를 지원하는 Milvus 인스턴스를 시작하는 방법을 설명합니다.</p>
...
<h2 id="Install-Milvus" class="common-anchor-header">Milvus 설치<button data-href="#Install-Milvus" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Docker Compose를 사용하여 GPU를 지원하는 Milvus를 설치하려면 다음 단계를 따르십시오.</p>
<h3 id="1-Download-and-configure-the-YAML-file" class="common-anchor-header">1. YAML 파일을 다운로드하고 구성하십시오.<button data-href="#1-Download-and-configure-the-YAML-file" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>다운로드 <a href="https://github.com/milvus-io/milvus/releases/download/v2.6.24/milvus-standalone-docker-compose-gpu.yml"><code translate="no">milvus-standalone-docker-compose-gpu.yml</code></a> 파일을 다운로드하여 수동으로 또는 다음 명령어를 사용하여 docker-compose.yml로 저장하십시오.</p>
<pre><code translate="no" class="language-shell"><span class="hljs-meta prompt_">$ </span><span class="language-bash">wget https://github.com/milvus-io/milvus/releases/download/v2.6.24/milvus-standalone-docker-compose-gpu.yml -O docker-compose.yml</span>
<button class="copy-code-btn"></button></code></pre>
<p>YAML 파일 내의 독립형 서비스 환경 변수를 다음과 같이 약간 수정해야 합니다:</p>
<ul>
<li>Milvus에 특정 GPU 장치를 할당하려면, ` <code translate="no">standalone</code> ` 서비스 정의에서 ` <code translate="no">deploy.resources.reservations.devices[0].devices_ids</code> ` 필드를 찾아 해당 값을 원하는 GPU의 ID로 변경하십시오. NVIDIA GPU 디스플레이 드라이버에 포함된 ` <code translate="no">nvidia-smi</code> ` 도구를 사용하여 GPU 장치의 ID를 확인할 수 있습니다. Milvus는 여러 GPU 장치를 지원합니다.</li>
</ul>
<p>Milvus에 단일 GPU 장치 할당하기:</p>
...
<span class="hljs-string">...</span>
<button class="copy-code-btn"></button></code></pre>
<h3 id="2-Start-Milvus" class="common-anchor-header">2. Milvus 시작<button data-href="#2-Start-Milvus" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>docker-compose.yml 파일이 있는 디렉터리에서 다음 명령을 실행하여 Milvus를 시작합니다:</p>
...
<button class="copy-code-btn"></button></code></pre>
<div class="alert note">
<p>위의 명령어를 실행하는 데 실패한 경우, 시스템에 Docker Compose V1이 설치되어 있는지 확인하십시오. 설치되어 있는 경우, <a href="https://docs.docker.com/compose/">이 페이지의</a> 참고 사항을 고려하여 Docker Compose V2로 마이그레이션하는 것이 좋습니다.</p>
</div>
<p>Milvus가 시작된 후,</p>
<ul>
...
<li><strong>milvus-minio</strong> 컨테이너는 기본 인증 자격 증명을 사용하여 로컬에서 <strong>9090번</strong> 및 <strong>9091번</strong> 포트를 제공하며, 데이터를 현재 폴더의 <strong>volumes/minio에</strong> 매핑합니다.</li>
...
<p>docker-compose.yml에서 Milvus에 여러 GPU 장치를 할당했다면, 어떤 GPU 장치를 노출하거나 사용 가능하게 할지 지정할 수 있습니다.</p>
<p>Milvus에서 GPU 장치 <code translate="no">0</code> 를 표시하도록 설정:</p>
...
<h2 id="Configure-memory-pool" class="common-anchor-header">메모리 풀 구성<button data-href="#Configure-memory-pool" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Milvus가 실행된 후, <code translate="no">milvus.yaml</code> 파일에서 <code translate="no">initMemSize</code> 및 <code translate="no">maxMemSize</code> 설정을 수정하여 메모리 풀을 사용자 정의할 수 있습니다.</p>
<div class="alert note">
<p><code translate="no">milvus.yaml</code> 파일은 Milvus 컨테이너 내부의 <code translate="no">/milvus/configs/</code> 디렉터리에 있습니다.</p>
</div>
...
<ol>
<li><p>다음 명령어를 사용하여 Milvus 컨테이너의 <code translate="no">milvus.yaml</code> 파일을 로컬 컴퓨터로 복사하십시오. <code translate="no">&lt;milvus_container_id&gt;</code> 부분은 실제 Milvus 컨테이너 ID로 대체하십시오.</p>
...
<button class="copy-code-btn"></button></code></pre></li>
<li><p>필요에 따라 <code translate="no">initMemSize</code> 및 <code translate="no">maxMemSize</code> 설정을 편집하고 변경 사항을 저장하십시오:</p>
<pre><code translate="no" class="language-yaml"><span class="hljs-string">...</span>
...
<button class="copy-code-btn"></button></code></pre>
<ul>
<li><code translate="no">initMemSize</code>: 메모리 풀의 초기 크기입니다. 기본값은 1024입니다.</li>
<li><code translate="no">maxMemSize</code>: 메모리 풀의 최대 크기입니다. 기본값은 2048입니다.</li>
</ul></li>
<li><p>다음 명령어를 사용하여 수정된 ` <code translate="no">milvus.yaml</code> ` 파일을 Milvus 컨테이너로 다시 복사하십시오. ` <code translate="no">&lt;milvus_container_id&gt;</code> `을 실제 Milvus 컨테이너 ID로 대체하십시오.</p>
<pre><code translate="no" class="language-shell">docker cp milvus.yaml &lt;milvus_container_id&gt;:/milvus/configs/milvus.yaml
<button class="copy-code-btn"></button></code></pre></li>
<li><p>변경 사항을 적용하려면 Milvus 컨테이너를 다시 시작하십시오:</p>
...
<li><p><a href="/docs/ko/v2.6.x/milvus-webui.md">Milvus WebUI를</a> 확인하여 Milvus 인스턴스에 대해 자세히 알아보세요.</p></li>
...
<li><p>클라우드에 Milvus 클러스터를 배포하세요:</p>
<ul>
