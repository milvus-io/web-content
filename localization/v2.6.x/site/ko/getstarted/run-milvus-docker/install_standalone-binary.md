...
<h2 id="Download-the-RPMDEB-Package" class="common-anchor-header">RPM/DEB 패키지 다운로드<button data-href="#Download-the-RPMDEB-Package" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Milvus 릴리스 페이지](https://github.com/milvus-io/milvus/releases/tag/v{{ var.milvus_release_version }})에서 시스템 아키텍처에 따라 RPM/DEB 패키지를 다운로드할 수 있습니다.</p>
<ul>
<li>x86_64/amd64의 경우 <strong>{{ var.milvus_deb_amd64 }}</strong> 또는 <strong>{{ var.milvus_rpm_amd64 }}</strong> 패키지를 다운로드하세요.</li>
<li>ARM64의 경우 <strong>{{ var.milvus_deb_arm64 }}</strong> 또는 <strong>{{ var.milvus_rpm_arm64 }}</strong> 패키지를 다운로드하세요.</li>
</ul>
