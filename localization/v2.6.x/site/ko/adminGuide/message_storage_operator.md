...
title: Milvus Operator를 사용하여 메시지 저장소 구성하기
related_key: 'minio, s3, storage, etcd, pulsar'
summary: Milvus Operator를 사용하여 메시지 저장소를 구성하는 방법을 알아보세요.
---
<h1 id="Configure-Message-Storage-with-Milvus-Operator" class="common-anchor-header">Milvus Operator를 사용하여 메시지 저장소 구성하기<button data-href="#Configure-Message-Storage-with-Milvus-Operator" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<p>메시지 스토리지 지정과 관련하여 다음과 같은 다른 제한 사항도 있습니다:</p>
<ul>
<li>Milvus 인스턴스당 하나의 메시지 저장소만 지원됩니다. 하지만 하나의 인스턴스에 여러 메시지 저장소가 설정된 경우와도 여전히 하위 호환성이 유지됩니다. 우선순위는 다음과 같습니다:
<ul>
...
<li><strong>업그레이드 제한 사항</strong>: <strong>메시지 큐 제한 사항</strong>: Milvus v2.6.17로 업그레이드할 때는 현재 사용 중인 메시지 큐를 유지해야 합니다. 업그레이드 과정에서 다른 메시지 큐 시스템으로 전환하는 것은 지원되지 않습니다. 메시지 큐 시스템 변경 기능은 향후 버전에서 제공될 예정입니다.</li>
</ul>
...
<h4 id="Example" class="common-anchor-header">예</h4><p>다음 예제는 RocksMQ 서비스를 구성하는 방법입니다.</p>
...
<li><code translate="no">persistence.enabled</code>: RocksMQ 데이터에 대한 영구 저장소를 활성화합니다</li>
...
<h2 id="Configure-Pulsar" class="common-anchor-header">Pulsar 구성<button data-href="#Configure-Pulsar" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Pulsar는 최근 변경 사항에 대한 로그를 관리하고, 스트림 로그를 출력하며, 로그 구독 기능을 제공합니다. 메시지 저장소로 Pulsar를 구성하는 기능은 Milvus 독립 실행형 및 Milvus 클러스터 모두에서 지원됩니다. 그러나 Milvus Operator를 사용하는 경우, Milvus 클러스터의 메시지 저장소로만 Pulsar를 구성할 수 있습니다. Pulsar를 구성하려면 ‘ <code translate="no">spec.dependencies.pulsar</code> ’ 아래에 필수 필드를 추가하십시오.</p>
...
<div class="alert note"><a href="https://artifacthub.io/packages/helm/apache/pulsar/2.7.8?modal=values">values.</a>yaml에서 내부 Pulsar 서비스를 구성하는 전체 구성 항목을 확인하십시오. 앞의 예시에서 보듯이 <code translate="no">pulsar.inCluster.values</code> 아래에 필요에 따라 구성 항목을 추가하십시오.</div>
...
<p><code translate="no">kafka</code> <code translate="no">external</code> 및 를 지원합니다. <code translate="no">inCluster</code></p>
...
<ul>
<li><a href="/docs/ko/v2.6.x/object_storage_operator.md">Milvus Operator를 사용하여 오브젝트 스토리지 구성하기</a></li>
