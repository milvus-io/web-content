...
<li><strong>업그레이드 제한</strong>: <strong>메시지 큐 제한</strong>: Milvus v2.6.14로 업그레이드할 때는 현재 선택한 메시지 큐를 유지해야 합니다. 업그레이드 중에 다른 메시지 큐 시스템 간에 전환하는 것은 지원되지 않습니다. 메시지 큐 시스템 변경에 대한 지원은 향후 버전에서 제공될 예정입니다.</li>
</ul>
