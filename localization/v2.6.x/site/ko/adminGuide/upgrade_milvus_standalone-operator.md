...
<h1 id="Upgrade-Milvus-Standalone-with-Milvus-Operator" class="common-anchor-header">Milvus Operator를 사용하여 Milvus 독립형 버전 업그레이드<button data-href="#Upgrade-Milvus-Standalone-with-Milvus-Operator" class="anchor-icon" translate="no">
    ...
    </button></h1><p>이 가이드에서는 Milvus Operator를 사용하여 Milvus 독립형 배포 환경을 v2.5.x에서 v2.6.23으로 업그레이드하는 방법을 설명합니다.</p>
<h2 id="Before-you-start" class="common-anchor-header">시작하기 전에<button data-href="#Before-you-start" class="anchor-icon" translate="no">
    ...
    </button></h3><p>Milvus 2.5.x에서 2.6.23으로의 업그레이드에는 다음과 같은 주요 아키텍처 변경 사항이 포함됩니다:</p>
<ul>
...
<li>v2.6.23으로 업그레이드하기 전에 <strong>반드시</strong> v2.5.16 이상으로 업그레이드해야 <strong>합니다</strong>.</li>
</ul>
...
<p>스탠드얼론 배포 환경이 이미 v2.5.16 이상을 실행 중이라면 이 단계를 건너뛰세요.</p>
</div>
...
kubectl get pods
<button class="copy-code-btn"></button></code></pre>
<h4 id="22-Upgrade-to-v2623" class="common-anchor-header">2.2 v2.6.23으로 업그레이드</h4><p>v2.5.16이 정상적으로 실행되면 v2.6.23으로 업그레이드하십시오:</p>
