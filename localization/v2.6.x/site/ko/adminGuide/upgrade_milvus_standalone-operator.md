...
<h1 id="Upgrade-Milvus-Standalone-with-Milvus-Operator" class="common-anchor-header">Milvus Operator로 Milvus 스탠드얼론 업그레이드하기<button data-href="#Upgrade-Milvus-Standalone-with-Milvus-Operator" class="anchor-icon" translate="no">
    ...
    </button></h1><p>이 가이드는 Milvus Operator를 사용하여 Milvus 독립 실행형 배포를 v2.5.x에서 v2.6.18로 업그레이드하는 방법을 설명합니다.</p>
<h2 id="Before-you-start" class="common-anchor-header">시작하기 전에<button data-href="#Before-you-start" class="anchor-icon" translate="no">
    ...
    </button></h2><h3 id="Whats-new-in-v2618" class="common-anchor-header">v2.6.18의 새로운 기능<button data-href="#Whats-new-in-v2618" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Milvus 2.5.x에서 2.6.18로 업그레이드하려면 아키텍처가 크게 변경됩니다:</p>
<ul>
...
<ul>
<li>Milvus v2.6.0-rc1은 v2.6.18과 <strong>호환되지 않습니다</strong>. 릴리스 후보에서 직접 업그레이드는 지원되지 않는다.</li>
...
<li>v2.6.18로 업그레이드하기 전에 v2.5.16 이상으로 <strong>업그레이드해야</strong> 합니다.</li>
</ul>
<p><strong>메시지 큐 제한</strong>: Milvus v2.6.18로 업그레이드할 때 현재 선택한 메시지 큐를 유지해야 합니다. 업그레이드 중에 다른 메시지 큐 시스템 간에 전환하는 것은 지원되지 않습니다. 메시지 큐 시스템 변경에 대한 지원은 향후 버전에서 제공될 예정입니다.</p>
...
kubectl get pods
<button class="copy-code-btn"></button></code></pre>
<h4 id="22-Upgrade-to-v2618" class="common-anchor-header">2.2 v2.6.18로 업그레이드하기</h4><p>v2.5.16이 성공적으로 실행되면 v2.6.18로 업그레이드하세요:</p>
