...
<h2 id="Configure-Pulsar-with-Helm" class="common-anchor-header">Helm을 사용하여 Pulsar 구성<button data-href="#Configure-Pulsar-with-Helm" class="anchor-icon" translate="no">
    ...
    </button></h2><p>K8s상의 Milvus 클러스터의 경우, Milvus를 시작하는 동일한 명령어를 통해 Pulsar를 구성할 수 있습니다. 또는 Milvus를 시작하기 전에 <a href="https://github.com/milvus-io/milvus-helm">milvus-helm</a> 저장소의 /charts/milvus 경로에 있는 ` <code translate="no">values.yml</code> ` 파일을 사용하여 Pulsar를 구성할 수도 있습니다.</p>
