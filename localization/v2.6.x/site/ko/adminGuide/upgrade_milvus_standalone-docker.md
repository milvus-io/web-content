...
<h1 id="Upgrade-Milvus-Standalone-with-Docker-Compose" class="common-anchor-header">Docker Compose를 사용하여 Milvus 독립형 버전 업그레이드하기<button data-href="#Upgrade-Milvus-Standalone-with-Docker-Compose" class="anchor-icon" translate="no">
    ...
    </button></h1><p>이 가이드에서는 Docker Compose를 사용하여 Milvus 독립형 배포 환경을 v2.5.x에서 v2.6.20으로 업그레이드하는 방법을 설명합니다.</p>
...
<p>보안상의 이유로, Milvus는 v2.6.20 릴리스와 함께 MinIO를 RELEASE.2024-12-18T13-15-44Z로 업그레이드합니다.</p>
</div>
...
<h3 id="Step-2-Upgrade-to-v2620" class="common-anchor-header">2단계: v2.6.20으로 업그레이드<button data-href="#Step-2-Upgrade-to-v2620" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<ol>
<li><p>기존 <code translate="no">docker-compose.yaml</code> 파일을 편집하고 Milvus 및 MinIO 이미지 태그를 모두 v2.6.20으로 업데이트하십시오:</p>
<pre><code translate="no" class="language-yaml"><span class="hljs-string">...</span>
