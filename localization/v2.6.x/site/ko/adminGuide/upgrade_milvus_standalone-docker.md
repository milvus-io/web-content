...
<h1 id="Upgrade-Milvus-Standalone-with-Docker-Compose" class="common-anchor-header">Docker Compose로 Milvus 스탠드얼론 업그레이드하기<button data-href="#Upgrade-Milvus-Standalone-with-Docker-Compose" class="anchor-icon" translate="no">
    ...
    </button></h1><p>이 가이드는 Docker Compose를 사용하여 Milvus 독립 실행형 배포를 v2.5.x에서 v2.6.9로 업그레이드하는 방법을 설명합니다.</p>
...
<ul>
<li>Milvus v2.6.0-rc1은 v2.6.9와 <strong>호환되지 않습니다</strong>. 릴리스 후보에서 직접 업그레이드는 지원되지 않습니다.</li>
...
<p>보안 문제로 인해 Milvus는 v2.6.9 릴리스와 함께 MinIO를 RELEASE.2024-12-18T13-15-44Z로 업그레이드합니다.</p>
</div>
...
<h3 id="Step-2-Upgrade-to-v269" class="common-anchor-header">2단계: v2.6.9로 업그레이드하기<button data-href="#Step-2-Upgrade-to-v269" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>v2.5.16이 성공적으로 실행되면 v2.6.9로 업그레이드합니다:</p>
<ol>
