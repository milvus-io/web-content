...
<h1 id="Upgrade-Milvus-Cluster-with-Helm-Chart" class="common-anchor-header">헬름 차트로 밀버스 클러스터 업그레이드하기<button data-href="#Upgrade-Milvus-Cluster-with-Helm-Chart" class="anchor-icon" translate="no">
    ...
    </button></h1><p>이 가이드는 헬름 차트를 사용하여 Milvus 클러스터를 v2.5.x에서 v2.6.13으로 업그레이드하는 방법을 설명한다.</p>
...
<ul>
<li>밀버스 v2.6.0-rc1은 v2.6.13과 <strong>호환되지 않는다</strong>. 릴리스 후보에서 직접 업그레이드하는 것은 지원되지 않는다.</li>
...
<li>v2.6.13으로 업그레이드하기 전에 <code translate="no">mixCoordinator</code> 을 활성화하여 v2.5.16 이상으로 <strong>업그레이드해야</strong> 합니다.</li>
</ul>
...
kubectl get pods
<button class="copy-code-btn"></button></code></pre>
<h3 id="Step-3-Upgrade-to-v2613" class="common-anchor-header">3단계: v2.6.13으로 업그레이드하기<button data-href="#Step-3-Upgrade-to-v2613" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>v2.5.16이 <code translate="no">mixCoordinator</code> 에서 성공적으로 실행되면 v2.6.13으로 업그레이드합니다:</p>
<pre><code translate="no" class="language-bash">helm upgrade my-release zilliztech/milvus \
