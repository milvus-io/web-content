...
<p>이 페이지는 더 이상 사용되지 않습니다. Milvus와 함께 CoPali를 사용하는 최신 예는 <a href="/docs/ko/search-with-embedding-lists.md">목록 임베딩이 있는 세사치를</a> 참조하세요.</p>
</div>
<p>
...
<p>MaxSim 함수는 토큰 임베딩을 확인하여 쿼리와 문서(검색 대상)를 비교합니다. 쿼리의 각 단어에 대해 문서에서 가장 유사한 단어를 선택하고(코사인 유사도 또는 제곱 L2 거리 사용) 쿼리의 모든 단어에 대해 이러한 최대 유사도를 합산합니다.</p>
