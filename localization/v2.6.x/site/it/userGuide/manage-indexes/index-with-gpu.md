...
<h1 id="Index-with-GPU" class="common-anchor-header">Indice con GPU<button data-href="#Index-with-GPU" class="anchor-icon" translate="no">
    ...
    </button></h1><p>Questa guida illustra i passaggi per la creazione di un indice con supporto GPU in Milvus, che può migliorare significativamente le prestazioni di ricerca in scenari ad alta produttività e ad alto richiamo. Per informazioni dettagliate sui tipi di indici GPU supportati da Milvus, consultare <a href="/docs/it/gpu_index.md">Indice GPU</a>.</p>
<div class="alert warning">
<p>Questa pagina è stata deprecata. Per l'ultima implementazione, consultare <a href="/docs/it/gpu-index-overview.md">Panoramica sugli indici GPU.</a></p>
</div>
<h2 id="Configure-Milvus-settings-for-GPU-memory-control" class="common-anchor-header">Configurare le impostazioni di Milvus per il controllo della memoria della GPU<button data-href="#Configure-Milvus-settings-for-GPU-memory-control" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Milvus usa un pool di memoria grafica globale per allocare la memoria della GPU.</p>
...
<h3 id="Prepare-index-parameters" class="common-anchor-header">Preparare i parametri dell'indice<button data-href="#Prepare-index-parameters" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Quando si impostano i parametri dell'indice GPU, definire <strong>index_type</strong>, <strong>metric_type</strong> e <strong>params</strong>:</p>
<ul>
...
<h3 id="Build-index" class="common-anchor-header">Costruire l'indice<button data-href="#Build-index" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Dopo aver configurato i parametri dell'indice in <strong>index_params</strong>, chiamare il metodo <a href="https://milvus.io/api-reference/pymilvus/v2.4.x/ORM/Collection/create_index.md"><code translate="no">create_index()</code></a> per costruire l'indice.</p>
...
<h3 id="Prepare-search-parameters" class="common-anchor-header">Preparare i parametri di ricerca<button data-href="#Prepare-search-parameters" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Di seguito sono riportati esempi di configurazioni per diversi tipi di indice:</p>
<ul>
<li><p>Indice<strong>GPU_BRUTE_FORCE</strong> </p>
...
<h3 id="Conduct-a-search" class="common-anchor-header">Eseguire una ricerca<button data-href="#Conduct-a-search" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Utilizzare il metodo <a href="https://milvus.io/api-reference/pymilvus/v2.4.x/ORM/Collection/search.md"><code translate="no">search()</code></a> per eseguire una ricerca di similarità vettoriale sull'indice GPU.</p>
...
<li><p>Per <strong>GPU_IVF_PQ</strong> e <strong>GPU_CAGRA</strong>, il valore massimo per <strong>limit</strong> è 1024.</p></li>
...
<li><p>Il caricamento della protezione OOM per gli indici GPU non è completamente supportato, una quantità eccessiva di dati potrebbe causare l'arresto anomalo del QueryNode.</p></li>
