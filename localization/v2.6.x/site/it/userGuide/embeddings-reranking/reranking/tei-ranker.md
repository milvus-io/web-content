---
id: tei-ranker.md
title: Classificatore TEICompatible with Milvus 2.6.x
summary: >-
  Il TEI Ranker sfrutta il servizio Text Embedding Inference (TEI) di Hugging
  Face per migliorare la rilevanza delle ricerche attraverso una
  riclassificazione semantica. Rappresenta un approccio avanzato all'ordinamento
  dei risultati di ricerca che va oltre la tradizionale similarità vettoriale.
beta: Milvus 2.6.x
---
<h1 id="TEI-Ranker" class="common-anchor-header">Classificatore TEI<span class="beta-tag" style="background-color:rgb(0, 179, 255);color:white" translate="no">Compatible with Milvus 2.6.x</span><button data-href="#TEI-Ranker" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h1><p>Il TEI Ranker sfrutta il servizio <a href="https://huggingface.co/docs/text-embeddings-inference/index">Text Embedding Inference (TEI)</a> di Hugging Face per migliorare la rilevanza delle ricerche attraverso una riclassificazione semantica. Rappresenta un approccio avanzato all'ordinamento dei risultati di ricerca che va oltre la tradizionale similarità vettoriale.</p>
<h2 id="Prerequisites" class="common-anchor-header">Prerequisiti<button data-href="#Prerequisites" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Prima di implementare TEI Ranker in Milvus, assicurarsi di avere:</p>
<ul>
<li><p>Una raccolta Milvus con un campo <code translate="no">VARCHAR</code> contenente il testo da rerankizzare.</p></li>
<li><p>Un servizio TEI funzionante con capacità di reranking. Per istruzioni dettagliate sulla configurazione di un servizio TEI, consultare la <a href="https://huggingface.co/docs/text-embeddings-inference/en/quick_tour">documentazione ufficiale TEI</a>.</p></li>
</ul>
<h2 id="Create-a-TEI-ranker-function" class="common-anchor-header">Creare una funzione TEI Ranker<button data-href="#Create-a-TEI-ranker-function" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>Per utilizzare TEI Ranker nella vostra applicazione Milvus, create un oggetto Function che specifichi come deve funzionare il reranking. Questa funzione sarà passata alle operazioni di ricerca di Milvus per migliorare la classificazione dei risultati.</p>
...
<button class="copy-code-btn"></button></code></pre>
<h3 id="TEI-ranker-specific-parameters" class="common-anchor-header">Parametri specifici del ranker TEI<button data-href="#TEI-ranker-specific-parameters" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>I seguenti parametri sono specifici del ranker TEI:</p>
<table>
   <tr>
     <th><p>Parametro</p></th>
     <th><p>Richiesto?</p></th>
   ...
   <tr>
     ...
     <td><p>Sì</p></td>
     <td><p>L'URL del servizio TEI.</p></td>
     <td><p><code translate="no">"http://localhost:8080"</code></p></td>
   </tr>
   <tr>
     <td><p><code translate="no">max_client_batch_size</code></p></td>
     <td><p>No</p></td>
     <td><p>Poiché i servizi di modello non possono elaborare tutti i dati in una sola volta, si imposta la dimensione del batch per accedere al servizio di modello in più richieste.</p></td>
     <td><p><code translate="no">32</code> (predefinito)</p></td>
   </tr>
   <tr>
     <td><p><code translate="no">truncate</code></p></td>
     <td><p>No</p></td>
     <td><p>Se troncare gli input che superano la lunghezza massima della sequenza. Se <code translate="no">False</code>, gli input lunghi generano errori.</p></td>
   ...
   <tr>
     ...
     <td><p>No</p></td>
     <td><p>Direzione da cui troncare quando l'input è troppo lungo:</p><ul><li><p><code translate="no">"Right"</code> (predefinito):  I token vengono rimossi dalla fine della sequenza finché non viene soddisfatta la dimensione massima supportata.</p></li><li><p><code translate="no">"Left"</code>: I token vengono rimossi dall'inizio della sequenza.</p></li></ul></td>
...
<h2 id="Apply-to-hybrid-search" class="common-anchor-header">Applicare alla ricerca ibrida<button data-href="#Apply-to-hybrid-search" class="anchor-icon" translate="no">
    ...
    </button></h2><p>TEI Ranker può essere utilizzato anche con la ricerca ibrida per combinare metodi di reperimento densi e radi:</p>
<div class="multipleCode">
   <a href="#python">Python</a> <a href="#java">Java</a> <a href="#javascript">NodeJS</a> <a href="#go">Go</a> <a href="#bash">cURL</a></div>
