...
<h2 id="Create-a-Cohere-ranker-function" class="common-anchor-header">Creare una funzione ranker di Cohere<button data-href="#Create-a-Cohere-ranker-function" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Per utilizzare Cohere Ranker nella vostra applicazione Milvus, create un oggetto Function che specifichi come deve funzionare il reranking. Questa funzione verrà passata alle operazioni di ricerca di Milvus per migliorare la classificazione dei risultati.</p>
...
<table>
   ...
   <tr>
     ...
     <td><p>No</p></td>
     <td><p>Credenziale di autenticazione per l'accesso ai servizi API di Cohere. Se non è specificato, il sistema cercherà la variabile d'ambiente <code translate="no">COHERE_API_KEY</code>.</p></td>
