...
<h1 id="JSON-Indexing" class="common-anchor-header">Indicizzazione JSON<button data-href="#JSON-Indexing" class="anchor-icon" translate="no">
    ...
    </button></h1><p>I campi JSON rappresentano un modo flessibile per memorizzare metadati strutturati in Milvus. Senza indicizzazione, le interrogazioni sui campi JSON richiedono la scansione dell'intera collezione, che diventa lenta con la crescita del set di dati. L'indicizzazione JSON consente di effettuare ricerche rapide creando indici all'interno dei dati JSON.</p>
...
<table>
   ...
   <tr>
     ...
     <td><p>Il percorso della chiave che si vuole indicizzare all'interno dell'oggetto JSON.</p></td>
...
<h3 id="Can-I-create-multiple-indexes-on-the-same-JSON-key" class="common-anchor-header">È possibile creare più indici sulla stessa chiave JSON?<button data-href="#Can-I-create-multiple-indexes-on-the-same-JSON-key" class="anchor-icon" translate="no">
    ...
    </button></h3><p>No, ogni chiave JSON supporta un solo indice. È necessario scegliere un singolo <code translate="no">json_cast_type</code> che corrisponda ai propri dati. Tuttavia, è possibile creare un indice sull'intero oggetto JSON e un indice su una chiave nidificata all'interno di tale oggetto.</p>
