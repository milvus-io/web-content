...
<p>Invece di chiedere all'utente di convertire gli eventi strutturali atomici in modelli di dati piatti, Milvus introduce l'array di strutture, dove ogni struttura dell'array può contenere scalari e vettori, preservando l'integrità semantica e consentendo un robusto filtraggio annidato e una ricerca ibrida.</p>
<h2 id="Why-Array-of-Structs" class="common-anchor-header">Perché gli array di strutture<button data-href="#Why-Array-of-Structs" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Le moderne applicazioni di intelligenza artificiale, dalla guida autonoma al recupero multimodale, si basano sempre più su dati eterogenei e annidati. I tradizionali modelli di dati piatti faticano a rappresentare relazioni complesse come<strong>"un documento con molti pezzi annotati</strong>" o<strong>"una scena di guida con più manovre osservate</strong>". È qui che il tipo di dati Array of Structs di Milvus si fa notare.</p>
...
<li><p><code translate="no">front_cars</code> è un corpo JSON che contiene tutte le auto in testa identificate nel frame corrente.</p></li>
</ul>
<h3 id="Step-2-Initialize-the-schemas" class="common-anchor-header">Passo 2: inizializzare gli schemi<button data-href="#Step-2-Initialize-the-schemas" class="anchor-icon" translate="no">
    ...
    </button></h3><p>Per iniziare, è necessario inizializzare lo schema per una caption Struct e per la collezione.</p>
<ul>
...
<li><p>Inizializzare lo schema della collezione</p>
...
<h3 id="Step-3-Set-index-parameters" class="common-anchor-header">Passo 3: Impostare i parametri dell'indice<button data-href="#Step-3-Set-index-parameters" class="anchor-icon" translate="no">
    ...
    </button></h3><p>Tutti i campi vettoriali devono essere indicizzati. Per indicizzare i campi vettoriali in una struttura di elementi, è necessario utilizzare <code translate="no">EMB_LIST_HNSW</code> come tipo di indice e il tipo di metrica <code translate="no">MAX_SIM</code> per misurare le somiglianze tra le incorporazioni vettoriali.</p>
...
<p>È necessario unire i dati per ciascun video clip da questi file e inserire i dati. Di seguito è riportata un'entità unita come riferimento.</p>
