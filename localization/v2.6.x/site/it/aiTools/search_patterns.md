...
title: 'Prompt: Schemi di ricerca Milvus'
summary: >-
  Regole per gli assistenti di codifica AI per implementare la ricerca, la
  ricerca ibrida e la ricerca full-text in Milvus.
---
<h1 id="Search-Patterns" class="common-anchor-header">Modelli di ricerca<button data-href="#Search-Patterns" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h1><p>Regole per implementare la ricerca per similarità, la ricerca ibrida, la ricerca filtrata e la ricerca full-text in Milvus, compresi i vincoli di AnnSearchRequest e l'uso del ranker. Copiare il prompt completo qui sotto nel proprio strumento di IA per applicare queste regole automaticamente. Per una panoramica di tutti i prompt, vedere i <a href="/docs/it/milvus_for_agents.md">prompt dell'IA</a>.</p>
