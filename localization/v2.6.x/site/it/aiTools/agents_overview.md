...
<h1 id="AGENTSmd--Milvus" class="common-anchor-header">AGENTS.md - Milvus<button data-href="#AGENTSmd--Milvus" class="anchor-icon" translate="no">
    ...
    </button></h1><p>Milvus è un database vettoriale open-source per la ricerca di similarità, la ricerca ibrida e il RAG. Si interagisce con esso attraverso l'interfaccia <code translate="no">MilvusClient</code> del PyMilvus SDK. Copiate il prompt completo qui sotto nel vostro strumento di IA per applicare queste regole automaticamente. Per informazioni dettagliate sui prompt specifici per ogni attività, vedere i <a href="/docs/it/milvus_for_agents.md">prompt dell'IA</a>.</p>
<h2 id="How-to-use-this-prompt" class="common-anchor-header">Come usare questo prompt<button data-href="#How-to-use-this-prompt" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
