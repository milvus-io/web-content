...
<div class="admonition note">
<p><b>note</b></p>
<p>Le dimensioni del vettore devono essere di tipo Python <code translate="no">int</code> o <code translate="no">numpy.integer</code> e i valori devono essere di tipo Python <code translate="no">float</code> o <code translate="no">numpy.floating</code>.</p>
</div>
<p>Per generare embeddings, si può anche usare il pacchetto <code translate="no">model</code> integrato nella libreria PyMilvus, che offre una serie di funzioni di embedding. Per maggiori dettagli, consultare <a href="/docs/it/embeddings.md">Embeddings</a>.</p>
<h2 id="Create-a-collection-with-a-sparse-vector-field" class="common-anchor-header">Creare un insieme con un campo vettoriale sparso<button data-href="#Create-a-collection-with-a-sparse-vector-field" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<h2 id="Perform-scalar-queries" class="common-anchor-header">Eseguire interrogazioni scalari<button data-href="#Perform-scalar-queries" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Oltre alla ricerca RNA, Milvus supporta anche query scalari su vettori sparsi. Queste query consentono di recuperare i documenti in base a un valore scalare associato al vettore sparse. Per ulteriori informazioni sui parametri, consultare <a href="https://milvus.io/api-reference/pymilvus/v2.4.x/MilvusClient/Vector/query.md">query()</a>.</p>
...
<p>La parte della dimensione deve essere un numero intero a 32 bit senza segno e la parte del valore può essere un numero a virgola mobile a 32 bit non negativo.</p></li>
