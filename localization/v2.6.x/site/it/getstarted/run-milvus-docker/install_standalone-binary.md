...
<h2 id="Download-the-RPMDEB-Package" class="common-anchor-header">Scaricare il pacchetto RPM/DEB<button data-href="#Download-the-RPMDEB-Package" class="anchor-icon" translate="no">
    ...
    </button></h2><p>È possibile scaricare il pacchetto RPM/DEB in base all'architettura del sistema dalla <a href="https://github.com/milvus-io/milvus/releases/tag/v2.6.17">pagina Milvus Releases</a>.</p>
<ul>
...
<p>Il seguente comando presuppone l'esecuzione di Milvus Standalone su una macchina x86_64/amd64.</p>
...
<h2 id="Optional-Update-Milvus-configurations" class="common-anchor-header">(Opzionale) Aggiornare le configurazioni di Milvus<button data-href="#Optional-Update-Milvus-configurations" class="anchor-icon" translate="no">
    ...
    </button></h2><p>È possibile modificare le configurazioni di Milvus nel file <code translate="no">/etc/milvus/configs/milvus.yaml</code>. Ad esempio, per cambiare i ms da <code translate="no">proxy.healthCheckTimeout</code> a <code translate="no">1000</code>, si può cercare il parametro di destinazione e modificarlo di conseguenza. Per le voci di configurazione applicabili, fare riferimento a <a href="/docs/it/v2.6.x/system_configuration.md">Configurazione del sistema</a>.</p>
