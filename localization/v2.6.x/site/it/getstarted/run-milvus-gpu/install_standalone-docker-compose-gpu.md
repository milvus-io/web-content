...
title: Eseguire Milvus con supporto GPU tramite Docker Compose
---
<h1 id="Run-Milvus-with-GPU-Support-Using-Docker-Compose" class="common-anchor-header">Eseguire Milvus con supporto GPU tramite Docker Compose<button data-href="#Run-Milvus-with-GPU-Support-Using-Docker-Compose" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<p>Se si riscontrano problemi durante il download dell'immagine, contattarci all'indirizzo <a href="mailto:community@zilliz.com">community@zilliz.com</a> fornendo i dettagli del problema e forniremo il supporto necessario.</p>
</div>
...
<h3 id="1-Download-and-configure-the-YAML-file" class="common-anchor-header">1. Scaricare e configurare il file YAML<button data-href="#1-Download-and-configure-the-YAML-file" class="anchor-icon" translate="no">
    ...
    </button></h3><p>Scarica <a href="https://github.com/milvus-io/milvus/releases/download/v2.6.24/milvus-standalone-docker-compose-gpu.yml"><code translate="no">milvus-standalone-docker-compose-gpu.yml</code></a> e salvarlo come docker-compose.yml manualmente o con il seguente comando.</p>
...
<button class="copy-code-btn"></button></code></pre>
<div class="alert note">
<p>Se non si riesce a eseguire il comando sopra indicato, verificare che sul sistema sia installato Docker Compose V1. In tal caso, si consiglia di migrare a Docker Compose V2, come indicato nelle note riportate <a href="https://docs.docker.com/compose/">in questa pagina</a>.</p>
</div>
...
<li>Il contenitore <strong>milvus-standalone</strong> serve localmente le porte <strong>19530</strong> con le impostazioni predefinite e mappa i propri dati nella directory <strong>volumes/milvus</strong> nella cartella corrente.</li>
</ul></li>
</ul>
...
<p>Se nel file docker-compose.yml sono state assegnate più GPU a Milvus, è possibile specificare quale GPU sia visibile o disponibile per l’uso.</p>
...
<li><p>Utilizza il seguente comando per copiare il file <code translate="no">milvus.yaml</code> modificato nel container Milvus. Sostituisci <code translate="no">&lt;milvus_container_id&gt;</code> con l’ID effettivo del tuo container Milvus.</p>
