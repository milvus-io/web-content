...
<p>Il seguente comando distribuisce un cluster Milvus con impostazioni ottimizzate per la versione 2.6.9, utilizzando Woodpecker come coda di messaggi consigliata:</p>
<pre><code translate="no" class="language-bash">helm install my-release zilliztech/milvus \
  --<span class="hljs-built_in">set</span> image.all.tag=v2.6.9 \
  --<span class="hljs-built_in">set</span> pulsarv3.enabled=<span class="hljs-literal">false</span> \
  --<span class="hljs-built_in">set</span> woodpecker.enabled=<span class="hljs-literal">true</span> \
...
<button class="copy-code-btn"></button></code></pre>
<p><strong>Attendere che tutti i pod mostrino lo stato "Running".</strong> Con la configurazione v2.6.9, si dovrebbero vedere pod simili a:</p>
