---
id: quickstart.md
summary: ابدأ مع ميلفوس
title: البداية سريعة
---
<h1 id="Quickstart-with-Milvus-Lite" class="common-anchor-header">بداية سريعة مع ميلفوس لايت<button data-href="#Quickstart-with-Milvus-Lite" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<h2 id="Install-Milvus" class="common-anchor-header">تثبيت ميلفوس<button data-href="#Install-Milvus" class="anchor-icon" translate="no">
    ...
    </button></h2><p>في هذا الدليل نستخدم Milvus Lite، وهي مكتبة بايثون مضمنة في <code translate="no">pymilvus</code> والتي يمكن تضمينها في تطبيق العميل. يدعم Milvus أيضًا النشر على <a href="/docs/ar/install_standalone-docker.md">Docker</a> و <a href="/docs/ar/install_cluster-milvusoperator.md">Kubernetes</a> لحالات استخدام الإنتاج.</p>
<p>قبل البدء، تأكد من توفر Python 3.8+ في البيئة المحلية. قم بتثبيت <code translate="no">pymilvus</code> الذي يحتوي على مكتبة عميل بايثون وميلفوس لايت:</p>
...
<h2 id="Set-Up-Vector-Database" class="common-anchor-header">إعداد قاعدة بيانات المتجهات<button data-href="#Set-Up-Vector-Database" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<button class="copy-code-btn"></button></code></pre>
<p>في الإعداد أعلاه,</p>
<ul>
<li>يستخدم حقلا المفتاح الأساسي والمتجه اسميهما الافتراضيين ("معرف" و"متجه").</li>
<li>يتم تعيين نوع القياس (تعريف مسافة المتجه) إلى قيمته الافتراضية<a href="/docs/ar/metric.md#Cosine-Similarity">(COSINE</a>).</li>
<li>يقبل حقل المفتاح الأساسي الأعداد الصحيحة ولا يزيد تلقائيًا (أي لا يستخدم <a href="/docs/ar/schema.md">ميزة المعرف التلقائي</a>) بدلاً من ذلك، يمكنك تحديد مخطط المجموعة رسميًا باتباع هذه <a href="https://milvus.io/api-reference/pymilvus/v2.4.x/MilvusClient/Collections/create_schema.md">التعليمات</a>.</li>
</ul>
<h2 id="Prepare-Data" class="common-anchor-header">إعداد البيانات<button data-href="#Prepare-Data" class="anchor-icon" translate="no">
    ...
    </button></h2><p>في هذا الدليل، نستخدم المتجهات لإجراء بحث دلالي على النص. نحتاج إلى إنشاء متجهات للنص عن طريق تنزيل نماذج التضمين. يمكن القيام بذلك بسهولة باستخدام الدوال المساعدة من مكتبة <code translate="no">pymilvus[model]</code>.</p>
<h2 id="Represent-text-with-vectors" class="common-anchor-header">تمثيل النص باستخدام المتجهات<button data-href="#Represent-text-with-vectors" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>أولاً، قم بتثبيت مكتبة النماذج. تتضمن هذه الحزمة أدوات التعلم الآلي الأساسية مثل PyTorch. قد يستغرق تنزيل الحزمة بعض الوقت إذا لم يسبق لبيئتك المحلية تثبيت PyTorch.</p>
<pre><code translate="no" class="language-python">$ pip install <span class="hljs-string">&quot;pymilvus[model]&quot;</span>
<button class="copy-code-btn"></button></code></pre>
<p>توليد تضمينات المتجهات مع النموذج الافتراضي. يتوقع ميلفوس أن يتم إدراج البيانات منظمة كقائمة من القواميس، حيث يمثل كل قاموس سجل بيانات، ويطلق عليه اسم كيان.</p>
...
<button class="copy-code-btn"></button></code></pre>
<h2 id="Alternatively-Use-fake-representation-with-random-vectors" class="common-anchor-header">[بديل] استخدام تمثيل وهمي مع متجهات عشوائية<button data-href="#Alternatively-Use-fake-representation-with-random-vectors" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>إذا لم تتمكن من تنزيل النموذج بسبب مشاكل في الشبكة، كحل بديل، يمكنك استخدام متجهات عشوائية لتمثيل النص مع الاستمرار في إنهاء المثال. فقط لاحظ أن نتيجة البحث لن تعكس التشابه الدلالي لأن المتجهات مزيفة.</p>
...
<h2 id="Insert-Data" class="common-anchor-header">إدراج البيانات<button data-href="#Insert-Data" class="anchor-icon" translate="no">
    ...
    </button></h2><p>دعنا ندرج البيانات في المجموعة:</p>
...
<h2 id="Semantic-Search" class="common-anchor-header">البحث الدلالي<button data-href="#Semantic-Search" class="anchor-icon" translate="no">
    ...
    </button></h2><p>يمكننا الآن إجراء عمليات بحث دلالية من خلال تمثيل نص استعلام البحث كمتجه، وإجراء بحث تشابه المتجهات على ميلفوس.</p>
<h3 id="Vector-search" class="common-anchor-header">البحث المتجه<button data-href="#Vector-search" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<button class="copy-code-btn"></button></code></pre>
<p>ويكون الناتج عبارة عن قائمة من النتائج، كل منها مخصص لاستعلام بحث متجه. يحتوي كل استعلام على قائمة بالنتائج، حيث تحتوي كل نتيجة على المفتاح الأساسي للكيان، والمسافة إلى متجه الاستعلام، وتفاصيل الكيان مع <code translate="no">output_fields</code>.</p>
<h2 id="Vector-Search-with-Metadata-Filtering" class="common-anchor-header">البحث المتجه مع تصفية البيانات الوصفية<button data-href="#Vector-Search-with-Metadata-Filtering" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>يمكنك أيضًا إجراء بحث متجه مع الأخذ في الاعتبار قيم البيانات الوصفية (تسمى حقول "قياسية" في ميلفوس، حيث تشير البيانات القياسية إلى بيانات غير متجهة). يتم ذلك باستخدام تعبير مرشح يحدد معايير معينة. لنرى كيفية البحث والتصفية باستخدام الحقل <code translate="no">subject</code> في المثال التالي.</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>بشكل افتراضي، لا تتم فهرسة الحقول العددية. إذا كنت بحاجة إلى إجراء بحث مفهرس للبيانات الوصفية في مجموعة بيانات كبيرة، يمكنك التفكير في استخدام مخطط ثابت وتشغيل <a href="/docs/ar/scalar_index.md">الفهرس</a> أيضًا لتحسين أداء البحث.</p>
<p>بالإضافة إلى البحث المتجه، يمكنك أيضًا إجراء أنواع أخرى من عمليات البحث:</p>
<h3 id="Query" class="common-anchor-header">الاستعلام<button data-href="#Query" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>الاستعلام() هو عملية تسترجع جميع الكيانات المطابقة لمعيار ما، مثل <a href="/docs/ar/boolean.md">تعبير مرشح</a> أو مطابقة بعض المعرفات.</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>استرداد الكيانات مباشرةً حسب المفتاح الأساسي:</p>
<pre><code translate="no" class="language-python">res = client.query(
    collection_name=<span class="hljs-string">&quot;demo_collection&quot;</span>,
...
<h2 id="Delete-Entities" class="common-anchor-header">حذف الكيانات<button data-href="#Delete-Entities" class="anchor-icon" translate="no">
    ...
    </button></h2><p>إذا كنت ترغب في حذف البيانات، يمكنك حذف الكيانات التي تحدد المفتاح الأساسي أو حذف جميع الكيانات المطابقة لتعبير مرشح معين.</p>
...
<button class="copy-code-btn"></button></code></pre>
<h2 id="Load-Existing-Data" class="common-anchor-header">تحميل البيانات الموجودة<button data-href="#Load-Existing-Data" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
client = MilvusClient(<span class="hljs-string">&quot;milvus_demo.db&quot;</span>)
<button class="copy-code-btn"></button></code></pre>
<h2 id="Drop-the-collection" class="common-anchor-header">حذف المجموعة<button data-href="#Drop-the-collection" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>إذا كنت ترغب في حذف جميع البيانات الموجودة في مجموعة، يمكنك إسقاط المجموعة باستخدام</p>
...
<h2 id="Learn-More" class="common-anchor-header">معرفة المزيد<button data-href="#Learn-More" class="anchor-icon" translate="no">
    ...
    </button></h2><p>يعتبر Milvus Lite رائعًا لبدء استخدام برنامج بايثون محلي. إذا كانت لديك بيانات على نطاق واسع أو ترغب في استخدام Milvus في الإنتاج، يمكنك التعرف على نشر Milvus على <a href="/docs/ar/install_standalone-docker.md">Docker</a> و <a href="/docs/ar/install_cluster-milvusoperator.md">Kubernetes</a>. تشترك جميع أنماط النشر في ميلفوس في نفس واجهة برمجة التطبيقات، لذلك لا تحتاج شيفرة جانب العميل إلى تغيير الكثير في حال الانتقال إلى وضع نشر آخر. ما عليك سوى تحديد <a href="https://milvus.io/api-reference/pymilvus/v2.4.x/MilvusClient/Client/MilvusClient.md">URI والرمز المميز</a> لخادم Milvus المنشور في أي مكان:</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>يوفر Milvus واجهة برمجة تطبيقات REST و gRPC، مع مكتبات العميل بلغات مثل <a href="/docs/ar/install-pymilvus.md">Python</a> و <a href="/docs/ar/install-java.md">Java</a> و <a href="/docs/ar/install-go.md">Go</a> و C# و <a href="/docs/ar/install-node.md">Node.js.</a></p>
