...
<h2 id="Overview" class="common-anchor-header">نظرة عامة<button data-href="#Overview" class="anchor-icon" translate="no">
    ...
    </button></h2><p>يستخدم Helm تنسيق حزم يُسمى "charts". و"chart" عبارة عن مجموعة من الملفات التي تصف مجموعة مترابطة من موارد Kubernetes. يوفر Milvus مجموعة من "charts" لمساعدتك في نشر تبعيات ومكونات Milvus.</p>
...
<div class="alert note">
<p>تم أرشفة مستودع مخططات Helm لـ Milvus الموجود على <code translate="no">https://github.com/milvus-io/milvus-helm</code>. نستخدم الآن المستودع الجديد الموجود على <code translate="no">https://github.com/zilliztech/milvus-helm</code>. لا يزال المستودع المؤرشف متاحًا للمخططات حتى الإصدار 4.0.31، ولكن استخدم المستودع الجديد للإصدارات الأحدث.</p>
</div>
...
<p>ينشر الأمر التالي مجموعة Milvus بإعدادات مُحسَّنة للإصدار v2.6.23، باستخدام Woodpecker كقائمة انتظار الرسائل الموصى بها:</p>
<pre><code translate="no" class="language-bash">helm install my-release zilliztech/milvus \
  --<span class="hljs-built_in">set</span> image.all.tag=v2.6.23 \
  --<span class="hljs-built_in">set</span> pulsarv3.enabled=<span class="hljs-literal">false</span> \
  --<span class="hljs-built_in">set</span> woodpecker.enabled=<span class="hljs-literal">true</span> \
...
<ul>
<li>يستخدم <strong>Woodpecker</strong> كقائمة انتظار للرسائل (موصى به لتقليل الصيانة)</li>
...
<li><strong>مكون جديد</strong>: تم تقديم " <strong>عقدة البث" (Streaming Node</strong> ) وتم تفعيلها بشكل افتراضي</li>
...
<h3 id="2-Check-Milvus-cluster-status" class="common-anchor-header">2. تحقق من حالة مجموعة Milvus<button data-href="#2-Check-Milvus-cluster-status" class="anchor-icon" translate="no">
    ...
    </button></h3><p>تأكد من نجاح عملية النشر عن طريق التحقق من حالة البود:</p>
<pre><code translate="no" class="language-bash">kubectl get pods
<button class="copy-code-btn"></button></code></pre>
<p><strong>انتظر حتى تظهر حالة «قيد التشغيل» لجميع البودات.</strong> مع التكوين v2.6.23، من المفترض أن ترى بودات مشابهة لما يلي:</p>
...
<h3 id="3-Connect-to-Milvus" class="common-anchor-header">3. الاتصال بـ Milvus<button data-href="#3-Connect-to-Milvus" class="anchor-icon" translate="no">
    ...
    </button></h3><p>للاتصال بمجموعة Milvus الخاصة بك من خارج Kubernetes، تحتاج إلى إعداد إعادة توجيه المنفذ.</p>
...
<p><strong>اترك هذا المحطة الطرفية مفتوحة</strong> أثناء استخدام Milvus. يمكنك الآن الاتصال بـ Milvus باستخدام أي حزمة SDK لـ Milvus على <code translate="no">localhost:27017</code>.</p>
...
<div class="alert note">
<ul>
<li>لتثبيت مثيل Milvus في الوضع المستقل حيث يتم تضمين جميع مكونات Milvus داخل بود واحد، يجب عليك تشغيل الأمر <code translate="no">helm template my-release --set cluster.enabled=false --set etcd.replicaCount=1 --set minio.mode=standalone --set pulsarv3.enabled=false zilliztech/milvus &gt; milvus_manifest.yaml</code> بدلاً من ذلك لعرض قوالب المخططات لمثيل Milvus في الوضع المستقل.</li>
...
<button class="copy-code-btn"></button></code></pre>
<p>حتى الآن، يمكنك اتباع الخطوتين <a href="#2-Check-Milvus-cluster-status">2</a> و <a href="#3-Forward-a-local-port-to-Milvus">3</a> من التثبيت عبر الإنترنت للتحقق من حالة المجموعة وتوجيه منفذ محلي إلى Milvus.</p>
