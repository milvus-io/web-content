...
<h3 id="Get-Load-Status" class="common-anchor-header">الحصول على حالة التحميل<button data-href="#Get-Load-Status" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<button class="copy-code-btn"></button></code></pre>
<p>لاحظ أنه يمكن استخدام الحقول المدرجة في <code translate="no">load_fields</code> فقط كشروط تصفية وحقول إخراج في عمليات البحث والاستعلامات. يجب عليك دائمًا تضمين المفتاح الأساسي في القائمة. لن تكون أسماء الحقول المستبعدة من التحميل متاحة للتصفية أو الإخراج.</p>
<p>يمكنك استخدام <code translate="no">skip_load_dynamic_field=True</code> لتخطي تحميل الحقل الديناميكي. يعامل Milvus الحقل الديناميكي كحقل واحد، لذلك سيتم تضمين جميع المفاتيح في الحقل الديناميكي أو استبعادها معًا.</p>
