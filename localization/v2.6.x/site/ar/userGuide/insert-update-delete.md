...
summary: >-
  يرشدك هذا الدليل إلى عمليات معالجة البيانات داخل المجموعة، بما في ذلك الإدراج،
  والإضافة، والحذف.
title: الإدراج والإضافة والحذف
---
<h1 id="Insert-Upsert--Delete" class="common-anchor-header">الإدراج والإضافة والحذف<button data-href="#Insert-Upsert--Delete" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h1><p>يرشدك هذا الدليل إلى عمليات معالجة البيانات داخل المجموعة، بما في ذلك الإدراج والإضافة والحذف.</p>
...
<h2 id="Preparations" class="common-anchor-header">الإعدادات<button data-href="#Preparations" class="anchor-icon" translate="no">
    ...
    </button></h2><p>يقوم مقتطف التعليمات البرمجية أدناه بإعادة استخدام التعليمات البرمجية الحالية لإنشاء اتصال بمجموعة Milvus وإعداد مجموعة بسرعة.</p>
<div class="language-python">
<p>للتحضيرات، استخدم <a href="https://milvus.io/api-reference/pymilvus/v2.4.x/MilvusClient/Client/MilvusClient.md"><code translate="no">MilvusClient</code></a> للاتصال بخادم Milvus و <a href="https://milvus.io/api-reference/pymilvus/v2.4.x/MilvusClient/Collections/create_collection.md"><code translate="no">create_collection()</code></a> لإنشاء مجموعة في وضع الإعداد السريع.</p>
</div>
<div class="language-java">
<p>للتحضيرات، استخدم <a href="https://milvus.io/api-reference/java/v2.4.x/v2/Client/MilvusClientV2.md"><code translate="no">MilvusClientV2</code></a> للاتصال بخادم Milvus و <a href="https://milvus.io/api-reference/java/v2.4.x/v2/Collections/createCollection.md"><code translate="no">createCollection()</code></a> لإنشاء مجموعة في وضع الإعداد السريع.</p>
</div>
<div class="language-javascript">
<p>للتحضيرات، استخدم <a href="https://milvus.io/api-reference/node/v2.4.x/Client/MilvusClient.md"><code translate="no">MilvusClient</code></a> للاتصال بخادم Milvus و <a href="https://milvus.io/api-reference/node/v2.4.x/Collections/createCollection.md"><code translate="no">createCollection()</code></a> لإنشاء مجموعة في وضع الإعداد السريع.</p>
</div>
...
<p><b>ملاحظات</b></p>
<p>تحتوي المجموعة التي تم إنشاؤها في الكود أعلاه على حقلين فقط: <code translate="no">id</code> (كمفتاح أساسي) و <code translate="no">vector</code> (كحقل متجه)، مع تمكين إعدادات <code translate="no">auto_id</code> و <code translate="no">enable_dynamic_field</code> افتراضيًا. عند إدراج البيانات,</p>
<ul>
<li><p>لا تحتاج إلى تضمين <strong>المعرف</strong> في البيانات المراد إدراجها، لأن الحقل الأساسي يزداد تلقائيًا عند إدراج البيانات.</p></li>
<li><p>سيتم حفظ الحقول غير المعرّفة على شكل أزواج مفاتيح-قيمة في حقل JSON محجوز باسم <strong>$meta</strong>.</p></li>
</ul>
</div>
<h2 id="Insert-entities" class="common-anchor-header">إدراج الكيانات<button data-href="#Insert-entities" class="anchor-icon" translate="no">
    ...
    </button></h2><p>لإدراج كيانات، تحتاج إلى تنظيم البيانات في قائمة من القواميس، حيث يمثل كل قاموس كيانًا. يحتوي كل قاموس على المفاتيح المطابقة لكل من الحقول المحددة مسبقًا والحقول الديناميكية في المجموعة المستهدفة.</p>
<div class="language-python">
<p>لإدراج كيانات في مجموعة، استخدم طريقة <a href="https://milvus.io/api-reference/pymilvus/v2.4.x/MilvusClient/Vector/insert.md"><code translate="no">insert()</code></a> الطريقة.</p>
</div>
<div class="language-java">
<p>لإدراج كيانات في مجموعة، استخدم الطريقة <a href="https://milvus.io/api-reference/java/v2.4.x/v2/Vector/insert.md"><code translate="no">insert()</code></a> الطريقة.</p>
</div>
<div class="language-javascript">
<p>لإدراج كيانات في مجموعة، استخدم الطريقة <a href="https://milvus.io/api-reference/node/v2.4.x/Vector/insert.md"><code translate="no">insert()</code></a> الطريقة.</p>
</div>
<div class="multipleCode">
...
<span class="hljs-comment">// </span>
<button class="copy-code-btn"></button></code></pre>
<h3 id="Insert-into-partitions" class="common-anchor-header">الإدراج في الأقسام<button data-href="#Insert-into-partitions" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>لإدراج بيانات في قسم معين، يمكنك تحديد اسم القسم في طلب الإدراج كما يلي:</p>
<div class="multipleCode">
   <a href="#python">بايثون </a> <a href="#java">جافا جافا</a> <a href="#javascript">Node.js</a></div>
...
<h2 id="Upsert-entities" class="common-anchor-header">إدراج الكيانات<button data-href="#Upsert-entities" class="anchor-icon" translate="no">
    ...
    </button></h2><p>إدراج البيانات هو مزيج من عمليات التحديث والإدراج. في Milvus، تقوم عملية upsert بتنفيذ إجراء على مستوى البيانات إما لإدراج كيان أو تحديثه بناءً على ما إذا كان مفتاحه الأساسي موجودًا بالفعل في مجموعة. على وجه التحديد:</p>
<ul>
<li><p>إذا كان المفتاح الأساسي للكيان موجودًا بالفعل في المجموعة، فسيتم الكتابة فوق الكيان الحالي.</p></li>
<li><p>إذا كان المفتاح الأساسي غير موجود في المجموعة، فسيتم إدراج كيان جديد.</p></li>
</ul>
<div class="alert note">
<ul>
<li>لن تقوم عمليات Upsert بتحديث المفاتيح الأساسية.</li>
<li>إذا كنت تخطط لاستخدام عملية <code translate="no">upsert</code> بدلاً من <code translate="no">insert</code> لاستيعاب البيانات على نطاق واسع (على سبيل المثال ملايين المتجهات)، فكن على علم بأن هذا يمكن أن يؤدي إلى استهلاك ذاكرة عالية على عقد بيانات Milvus.</li>
</ul>
</div>
<div class="language-python">
<p>لإعادة إدراج الكيانات، استخدم طريقة <a href="https://milvus.io/api-reference/pymilvus/v2.4.x/MilvusClient/Vector/upsert.md"><code translate="no">upsert()</code></a> الطريقة.</p>
</div>
<div class="language-java">
<p>لإعادة إدراج الكيانات، استخدم الطريقة <a href="https://milvus.io/api-reference/java/v2.4.x/v2/Vector/insert.md"><code translate="no">upsert()</code></a> الطريقة.</p>
</div>
<div class="language-javascript">
<p>لإعادة إدراج الكيانات، استخدم الطريقة <a href="https://milvus.io/api-reference/node/v2.4.x/Vector/upsert.md"><code translate="no">upsert()</code></a> الطريقة.</p>
</div>
<div class="multipleCode">
...
<span class="hljs-comment">// </span>
<button class="copy-code-btn"></button></code></pre>
<h3 id="Upsert-data-in-partitions" class="common-anchor-header">رفع البيانات في الأقسام<button data-href="#Upsert-data-in-partitions" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>لإعادة إدراج البيانات في قسم معين، يمكنك تحديد اسم القسم في طلب الإدراج كما يلي:</p>
<div class="multipleCode">
...
<p>إذا لم تعد هناك حاجة إلى كيان ما، يمكنك حذفه من المجموعة باستخدام <a href="https://milvus.io/api-reference/pymilvus/v2.4.x/MilvusClient/Vector/delete.md"><code translate="no">delete()</code></a>.</p>
</div>
<div class="language-java">
<p>إذا لم تعد هناك حاجة لكيان ما، يمكنك حذفه من المجموعة باستخدام . <a href="https://milvus.io/api-reference/java/v2.4.x/v2/Vector/delete.md"><code translate="no">delete()</code></a>.</p>
</div>
<div class="language-javascript">
<p>إذا لم تعد هناك حاجة لكيان ما، يمكنك حذفه من المجموعة باستخدام . <a href="https://milvus.io/api-reference/node/v2.4.x/Vector/delete.md"><code translate="no">delete()</code></a>.</p>
</div>
<p>يوفر لك Milvus طريقتين لتحديد الكيانات المراد حذفها.</p>
<ul>
<li><p><strong>حذف الكيانات عن طريق التصفية.</strong></p>
   <div class='alert note'>
<p>عند استخدام تعبيرات التصفية لحذف الكيانات، تأكد من تحميل المجموعة. وإلا سيعيد Milvus خطأً.</p>
   </div>
...
<li><p><strong>حذف الكيانات حسب المعرفات.</strong></p>
<p>توضح المقتطفات التالية كيفية حذف الكيانات حسب المعرفات من قسم معين. يعمل أيضًا إذا تركت اسم القسم غير محدد.</p>
<p><div class="multipleCode">
<a href="#python">بايثون </a><a href="#java">جافا</a><a href="#javascript">Node.js</a></div></p>
