...
summary: >-
  ...
  الآلي وتحليل البيانات. وهي تتكون من مصفوفات تحتوي على أعداد حقيقية، حيث تكون
  ...
  يجعل تحليل البيانات ومعالجتها أسهل في الفضاءات عالية الأبعاد. وعادةً ما يكون
...
<ul>
<li><p><strong>التمثيل متعدد الأبعاد:</strong> تمثل كل نقطة كائنًا مفاهيميًا (مثل <strong>Milvus</strong>، <strong>وقاعدة بيانات المتجهات</strong>، <strong>ونظام الاسترجاع</strong>، وما إلى ذلك)، حيث يتم تحديد موضعها من خلال قيم أبعادها.</p></li>
...
<li><p><strong>تأثير التكتل:</strong> تُوضع المفاهيم ذات الصلة (مثل <strong>Milvus</strong> <strong>وقاعدة بيانات المتجهات</strong> <strong>ونظام الاسترجاع</strong>) بالقرب من بعضها البعض في الفضاء، لتشكل مجموعة دلالية.</p></li>
</ul>
...
<p>إلى جانب المتجهات الكثيفة، يدعم Milvus أيضًا المتجهات المتفرقة والمتجهات الثنائية. تعد المتجهات المتفرقة مناسبة للمطابقات الدقيقة استنادًا إلى مصطلحات محددة، مثل البحث عن الكلمات المفتاحية ومطابقة المصطلحات، بينما تُستخدم المتجهات الثنائية عادةً للتعامل بكفاءة مع البيانات الثنائية، مثل مطابقة أنماط الصور وبعض تطبيقات التجزئة. لمزيد من المعلومات، راجع <a href="/docs/ar/v2.6.x/binary-vector.md">«المتجهات الثنائية»</a> <a href="/docs/ar/v2.6.x/sparse_vector.md">و«المتجهات المتفرقة</a>».</p>
</div>
<h2 id="Use-dense-vectors" class="common-anchor-header">استخدام المتجهات الكثيفة<button data-href="#Use-dense-vectors" class="anchor-icon" translate="no">
    ...
    </button></h3><p>لاستخدام المتجهات الكثيفة في Milvus، قم أولًا بتعريف حقل متجه لتخزين المتجهات الكثيفة عند إنشاء مجموعة. تتضمن هذه العملية ما يلي:</p>
<ol>
<li><p>تعيين المعلمة " <code translate="no">datatype</code> " إلى نوع بيانات متجه كثيف مدعوم. للاطلاع على أنواع بيانات المتجهات الكثيفة المدعومة، راجع "أنواع البيانات".</p></li>
<li><p>تحديد أبعاد المتجه الكثيف باستخدام المعلمة <code translate="no">dim</code>.</p></li>
</ol>
<p>في المثال أدناه، نضيف حقل متجهًا باسم <code translate="no">dense_vector</code> لتخزين المتجهات الكثيفة. نوع بيانات الحقل هو <code translate="no">FLOAT_VECTOR</code> ، بأبعاد <code translate="no">4</code>.</p>
...
<table>
   ...
   <tr>
     ...
     <td><p>يخزن أرقامًا عائمة بنصف دقة 16 بت، تُستخدم للتعلم العميق وحسابات وحدة معالجة الرسومات (GPU). يوفر مساحة التخزين في الحالات التي تكون فيها الدقة أقل أهمية، مثل مرحلة الاسترجاع منخفضة الدقة في أنظمة التوصيات.</p></td>
   </tr>
   <tr>
...
<h3 id="Set-index-params-for-vector-field" class="common-anchor-header">تعيين معلمات الفهرس لحقل المتجهات<button data-href="#Set-index-params-for-vector-field" class="anchor-icon" translate="no">
    ...
    </button></h3><p>لتسريع عمليات البحث الدلالي، يجب إنشاء فهرس لحقل المتجهات. يمكن أن تؤدي الفهرسة إلى تحسين كفاءة استرجاع البيانات المتجهة واسعة النطاق بشكل ملحوظ.</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#javascript">   NodeJS</a>
...
<p>يوفر Milvus أنواعًا متنوعة من الفهارس لتجربة بحث أفضل عن المتجهات. AUTOINDEX هو نوع فهرس خاص مصمم لتسهيل التعلم في مجال البحث عن المتجهات. هناك العديد من أنواع الفهارس المتاحة لتختار منها. لمزيد من التفاصيل، راجع xxx.</p>
<p>يدعم Milvus أنواعًا أخرى من المقاييس. لمزيد من المعلومات، راجع <a href="/docs/ar/v2.6.x/metric.md">أنواع المقاييس</a>.</p>
...
<h3 id="Insert-data" class="common-anchor-header">إدراج البيانات<button data-href="#Insert-data" class="anchor-icon" translate="no">
    ...
    </button></h3><p>بعد إنشاء المجموعة، استخدم طريقة ` <code translate="no">insert</code> ` لإضافة بيانات تحتوي على متجهات كثيفة. تأكد من أن أبعاد المتجهات الكثيفة التي يتم إدراجها تتطابق مع قيمة ` <code translate="no">dim</code> ` المحددة عند إضافة حقل المتجه الكثيف.</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#javascript">   NodeJS</a>
...
<h3 id="Perform-similarity-search" class="common-anchor-header">إجراء بحث التشابه<button data-href="#Perform-similarity-search" class="anchor-icon" translate="no">
    ...
    </button></h3><p>يعد البحث الدلالي القائم على المتجهات الكثيفة إحدى الميزات الأساسية لـ Milvus، حيث يتيح لك العثور بسرعة على البيانات الأكثر تشابهًا مع متجه الاستعلام استنادًا إلى المسافة بين المتجهات. لإجراء بحث عن التشابه، قم بإعداد متجه الاستعلام ومعلمات البحث، ثم استدعِ الطريقة <code translate="no">search</code>.</p>
<div class="multipleCode">
   <a href="#python">Python</a>
 <a href="#java">   Java</a>
 <a href="#javascript">   NodeJS</a>
...
<button class="copy-code-btn"></button></code></pre>
<p>لمزيد من المعلومات حول معلمات البحث عن التشابه، راجع <a href="/docs/ar/v2.6.x/single-vector-search.md">البحث الأساسي</a> باستخدام الشبكات العصبية <a href="/docs/ar/v2.6.x/single-vector-search.md">الاص</a>ط <a href="/docs/ar/v2.6.x/single-vector-search.md">ناعية (ANN)</a>.</p>
