...
<h1 id="JSON-Shredding" class="common-anchor-header">تمزيق JSON<span class="beta-tag" style="background-color:rgb(0, 179, 255);color:white" translate="no">Compatible with Milvus 2.6.2+</span><button data-href="#JSON-Shredding" class="anchor-icon" translate="no">
    ...
    </button></h1><p>يعمل تمزيق JSON على تسريع استعلامات JSON عن طريق تحويل التخزين التقليدي القائم على الصفوف إلى تخزين عمودي محسّن. مع الحفاظ على مرونة JSON لنمذجة البيانات، يقوم Milvus بتحسين الأعمدة من وراء الكواليس مما يحسن بشكل كبير من كفاءة الوصول والاستعلام.</p>
