...
summary: >-
  تجمع عملية Upsert بين إجراءات تحديث وإدراج البيانات. يحدد ميلفوس ما إذا كان
  سيتم إجراء عملية تحديث أو إدراج عن طريق التحقق من وجود المفتاح الأساسي. سيقدم
  هذا القسم كيفية إدراج كيان والسلوكيات المحددة لعملية Upsert في سيناريوهات
  مختلفة.
---
<h1 id="Upsert-Entities" class="common-anchor-header">إدراج الكيانات<button data-href="#Upsert-Entities" class="anchor-icon" translate="no">
    ...
    </button></h1><p>توفر العملية <code translate="no">upsert</code> طريقة ملائمة لإدراج أو تحديث الكيانات في مجموعة. وهي تتعامل بذكاء مع البيانات عن طريق التحقق من وجود مفتاح أساسي: إذا كان المفتاح موجودًا بالفعل، يتم تحديث الكيان المقابل؛ وإلا يتم إدراج كيان جديد. هذا يجعل <code translate="no">upsert</code> الطريقة الموصى بها لإدارة البيانات عندما لا تكون متأكدًا مما إذا كان الكيان موجودًا بالفعل أو عندما تحتاج إلى تجنب إنشاء إدخالات مكررة.</p>
<div class="alert note">
<p>إذا قمت بإضافة حقول جديدة ديناميكيًا بعد إنشاء المجموعة، ولم تحدد قيمًا لهذه الحقول عند إدراج الكيانات، فإن Milvus يقوم تلقائيًا بتعبئتها إما بقيمها الافتراضية المحددة أو بقيمها الافتراضية NULL إذا لم يتم تعيين القيم الافتراضية. لمزيد من التفاصيل، راجع <a href="/docs/ar/add-fields-to-an-existing-collection.md">إضافة حقول إلى مجموعة موجودة</a>.</p>
</div>
<h2 id="Overview" class="common-anchor-header">نظرة عامة<button data-href="#Overview" class="anchor-icon" translate="no">
    ...
    </button></h2><p>عندما تحتاج إلى تحديث كيان في مجموعة أو عندما لا تكون متأكدًا مما إذا كنت تريد التحديث أو الإدراج، يمكنك محاولة استخدام عملية Upsert. عند استخدام هذه العملية، من الضروري التأكد من أن الكيان المضمن في طلب Upsert يحتوي على المفتاح الأساسي؛ وإلا سيحدث خطأ. عند تلقي طلب Upsert، سيقوم ميلفوس بتنفيذ العملية التالية:</p>
<ol>
<li><p>التحقق مما إذا كان الحقل الأساسي للمجموعة يحتوي على معرف تلقائي ممكّن.</p>
<ol>
<li><p>إذا كان كذلك، سيقوم Milvus باستبدال المفتاح الأساسي في الكيان بمفتاح أساسي تم إنشاؤه تلقائيًا وإدراج البيانات.</p></li>
<li><p>إذا لم يكن كذلك، سيستخدم Milvus المفتاح الأساسي الذي يحمله الكيان لإدراج البيانات.</p></li>
</ol></li>
<li><p>تنفيذ عملية حذف استنادًا إلى قيمة المفتاح الأساسي للكيان المضمن في طلب Upsert.</p></li>
</ol>
<p>
   ...
   </span> <span class="img-wrapper"> <span>سير عمل Upsert Entities</span> </span></p>
<h2 id="Upsert-Entity-in-a-Collection" class="common-anchor-header">رفع الكيانات في مجموعة<button data-href="#Upsert-Entity-in-a-Collection" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>في هذا القسم، ستقوم في هذا القسم بإدراج الكيانات في مجموعة تم إنشاؤها بطريقة الإعداد السريع. تحتوي المجموعة التي تم إنشاؤها بهذه الطريقة على حقلين فقط، وهما <strong>المعرف</strong> <strong>والمتجه</strong>. بالإضافة إلى ذلك، تحتوي هذه المجموعة على حقل ديناميكي ممكّن، لذا فإن الكيانات في شيفرة المثال تتضمن حقلاً يسمى <strong>اللون</strong> غير معرّف في المخطط.</p>
