...
title: استخدام ميلفوس كمخزن متجهات
---
<h1 id="Use-Milvus-as-a-LangChain-Vector-Store" class="common-anchor-header">استخدام ميلفوس كمخزن متجهات لانغ تشين<button data-href="#Use-Milvus-as-a-LangChain-Vector-Store" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<h2 id="Setup" class="common-anchor-header">الإعداد<button data-href="#Setup" class="anchor-icon" translate="no">
    ...
    </button></h2><p>ستحتاج إلى تثبيت <code translate="no">langchain-milvus</code> والتبعيات الضرورية الأخرى.</p>
...
<button class="copy-code-btn"></button></code></pre>
<h3 id="Compartmentalize-the-data-with-Milvus-Collections" class="common-anchor-header">تجزئة البيانات باستخدام مجموعات ميلفوس<button data-href="#Compartmentalize-the-data-with-Milvus-Collections" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>يمكنك تخزين مستندات مختلفة غير ذات صلة في مجموعات مختلفة داخل نفس مثيل ميلفوس للحفاظ على السياق</p>
...
<h3 id="Add-items-to-vector-store" class="common-anchor-header">إضافة عناصر إلى مخزن المتجهات<button data-href="#Add-items-to-vector-store" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>يمكننا إضافة عناصر إلى مخزن المتجهات باستخدام الدالة <code translate="no">add_documents</code>.</p>
...
<h3 id="Delete-items-from-vector-store" class="common-anchor-header">حذف عناصر من مخزن المتجهات<button data-href="#Delete-items-from-vector-store" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<h3 id="Query-directly" class="common-anchor-header">الاستعلام مباشرة<button data-href="#Query-directly" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><h4 id="Similarity-search" class="common-anchor-header">البحث عن التشابه</h4><p>يمكن إجراء بحث تشابه بسيط مع تصفية البيانات الوصفية على النحو التالي:</p>
...
<p>للاطلاع على قائمة كاملة بجميع خيارات البحث المتاحة عند استخدام مخزن المتجهات <code translate="no">Milvus</code> ، يمكنك زيارة <a href="https://python.langchain.com/api_reference/milvus/vectorstores/langchain_milvus.vectorstores.milvus.Milvus.html">مرجع واجهة برمجة التطبيقات</a>.</p>
<h3 id="Query-by-turning-into-retriever" class="common-anchor-header">الاستعلام عن طريق التحويل إلى مسترجع<button data-href="#Query-by-turning-into-retriever" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>يمكنك أيضًا تحويل مخزن المتجهات إلى مسترجع لسهولة الاستخدام في سلاسل البيانات الخاصة بك.</p>
...
<h2 id="Usage-for-Retrieval-Augmented-Generation" class="common-anchor-header">الاستخدام لتوليد المسترجع المعزز للاسترجاع<button data-href="#Usage-for-Retrieval-Augmented-Generation" class="anchor-icon" translate="no">
    ...
    </button></h2><p>للحصول على إرشادات حول كيفية استخدام مخزن المتجهات هذا للتوليد المعزز للاسترجاع (RAG)، راجع <a href="https://milvus.io/docs/integrate_with_langchain.md">دليل RAG</a> هذا.</p>
<h3 id="Per-User-Retrieval" class="common-anchor-header">الاسترجاع لكل مستخدم<button data-href="#Per-User-Retrieval" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>عند إنشاء تطبيق استرجاع، غالبًا ما يتعين عليك إنشاؤه مع وضع عدة مستخدمين في الاعتبار. هذا يعني أنك قد تقوم بتخزين البيانات ليس لمستخدم واحد فقط، ولكن للعديد من المستخدمين المختلفين، ويجب ألا يتمكنوا من رؤية بيانات بعضهم البعض.</p>
...
<h2 id="API-reference" class="common-anchor-header">مرجع واجهة برمجة التطبيقات<button data-href="#API-reference" class="anchor-icon" translate="no">
    ...
    </button></h2><p>للحصول على وثائق مفصلة، توجه إلى مرجع واجهة برمجة التطبيقات: https://reference.langchain.com/python/integrations/langchain_milvus/</p>
