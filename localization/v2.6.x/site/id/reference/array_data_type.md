...
title: Menggunakan Bidang Array
---
<h1 id="Use-Array-Fields" class="common-anchor-header">Menggunakan Bidang Array<button data-href="#Use-Array-Fields" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h1><p>Panduan ini menjelaskan cara menggunakan bidang larik, seperti menyisipkan nilai larik, membuat indeks pada bidang vektor dan larik, serta mencari dan melakukan kueri dalam bidang larik dengan operator dasar dan lanjutan.</p>
<h2 id="Prerequisites" class="common-anchor-header">Prasyarat<button data-href="#Prerequisites" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Pastikan Anda memiliki hal-hal berikut ini:</p>
<ul>
<li>Milvus telah terinstal dan berjalan. Untuk informasi tentang cara menginstal Milvus, lihat Menginstal <a href="/docs/id/install-overview.md">Milvus</a>.</li>
<li>Salah satu SDK Milvus terinstal di lingkungan Anda. Untuk informasi lebih lanjut, lihat Menginstal <a href="/docs/id/install-pymilvus.md">SDK</a>.</li>
</ul>
<h2 id="Prepare-data-with-an-array-field" class="common-anchor-header">Mempersiapkan data dengan bidang array<button data-href="#Prepare-data-with-an-array-field" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>Milvus mendukung larik sebagai salah satu tipe data field. Larik dalam koleksi Milvus harus selalu memiliki elemen dengan tipe data yang sama, dan tipe data untuk elemen larik dapat berupa salah satu tipe data yang didukung di Milvus. Untuk daftar tipe data yang didukung, lihat <a href="https://milvus.io/docs/schema.md#Supported-data-types">Tipe data yang didukung</a>.</p>
<p>Cuplikan kode berikut ini menghasilkan kumpulan data acak yang berisi bidang larik bernama <code translate="no">color_coord</code>, dengan semua elemen tipe data interger.</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>Cuplikan kode ini menyiapkan daftar warna acak dan menghasilkan kumpulan data yang berisi 1.000 entitas. Setiap entitas memiliki ID, vektor lima angka floating-point, warna, tag warna, dan bidang larik <code translate="no">color_coord</code> yang berisi antara 3 hingga 5 nilai integer. Data sampel dicetak untuk memverifikasi strukturnya.</p>
<p>Struktur keluaran:</p>
...
<button class="copy-code-btn"></button></code></pre>
<h2 id="Set-up-MilvusClient" class="common-anchor-header">Menyiapkan MilvusClient<button data-href="#Set-up-MilvusClient" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>Untuk berinteraksi dengan Milvus, siapkan klien Milvus dengan menentukan alamat server.</p>
<div class="multipleCode">
   <a href="#python">Python </a> <a href="#java">Java</a> <a href="#javascript">Node.js</a></div>
...
<button class="copy-code-btn"></button></code></pre>
<h2 id="Create-a-collection-with-an-array-field" class="common-anchor-header">Membuat koleksi dengan bidang array<button data-href="#Create-a-collection-with-an-array-field" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><h3 id="Define-collection-schema" class="common-anchor-header">Menentukan skema koleksi<button data-href="#Define-collection-schema" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Skema mendefinisikan struktur koleksi, termasuk bidang dan tipe datanya. Contoh di bawah ini mendefinisikan skema koleksi yang cocok dengan contoh data yang dibuat di <a href="#prepare-data-with-an-array-field">bagian sebelumnya</a>.</p>
<p>Untuk mengonfigurasi bidang array dalam koleksi:</p>
<div class="language-python">
<ol>
<li>Tetapkan <code translate="no">datatype</code>: Konfigurasikan sebagai <code translate="no">DataType.ARRAY</code>.</li>
<li>Tentukan <code translate="no">element_type</code>: Pilih tipe data untuk elemen dalam larik. Elemen dalam bidang larik harus memiliki tipe data yang sama. Dalam contoh ini, <code translate="no">element_type</code> diatur ke <code translate="no">DataType.INT64</code>.</li>
<li>Tentukan <code translate="no">max_capacity</code>: Tetapkan parameter ini untuk menentukan jumlah maksimum elemen yang dapat ditampung oleh bidang array.</li>
</ol>
</div>
<div class="language-java">
<ol>
<li>Tentukan <code translate="no">dataType</code>: Konfigurasikan sebagai <code translate="no">DataType.Array</code>.</li>
<li>Tentukan <code translate="no">elementType</code>: Pilih tipe data untuk elemen dalam larik. Elemen dalam bidang larik harus memiliki tipe data yang sama. Dalam contoh ini, <code translate="no">elementType</code> diatur ke <code translate="no">DataType.Int64</code>.</li>
<li>Tentukan <code translate="no">maxCapacity</code>: Tetapkan parameter ini untuk menentukan jumlah maksimum elemen yang dapat ditampung oleh bidang array.</li>
</ol>
</div>
<div class="language-javascript">
<ol>
<li>Tentukan <code translate="no">data_type</code>: Konfigurasikan sebagai <code translate="no">DataType.Array</code>.</li>
<li>Tentukan <code translate="no">element_type</code>: Pilih tipe data untuk elemen dalam larik. Elemen dalam bidang larik harus memiliki tipe data yang sama. Dalam contoh ini, <code translate="no">element_type</code> diatur ke <code translate="no">DataType.Int64</code>.</li>
<li>Tentukan <code translate="no">max_capacity</code>: Tetapkan parameter ini untuk menentukan jumlah maksimum elemen yang dapat ditampung oleh bidang array.</li>
</ol>
</div>
<p>Contoh kode di bawah ini mendefinisikan skema koleksi dengan bidang array <code translate="no">color_coord</code>, dengan maksimum 5 elemen dan setiap elemen bertipe data integer.</p>
<div class="multipleCode">
   <a href="#python">Python </a> <a href="#java">Java</a> <a href="#javascript">Node.js</a></div>
...
<button class="copy-code-btn"></button></code></pre>
<div class="language-python">
<p>Untuk informasi lebih lanjut mengenai metode dan parameter, lihat <a href="https://milvus.io/api-reference/pymilvus/v2.4.x/MilvusClient/Collections/create_schema.md">create_schema</a> dan <a href="https://milvus.io/api-reference/pymilvus/v2.4.x/MilvusClient/CollectionSchema/add_field.md">add_field</a>.</p>
</div>
<div class="language-java">
<p>Untuk informasi lebih lanjut mengenai metode dan parameter, lihat <a href="https://milvus.io/api-reference/java/v2.4.x/v2/Collections/createSchema.md">createSchema</a> dan <a href="https://milvus.io/api-reference/java/v2.4.x/v2/CollectionSchema/addField.md">addField</a>.</p>
</div>
<div class="language-javascript">
<p>Untuk informasi lebih lanjut mengenai metode dan parameter, lihat <a href="https://milvus.io/api-reference/node/v2.4.x/Collections/createCollection.md">createCollection</a>.</p>
</div>
<h3 id="Create-the-collection" class="common-anchor-header">Membuat koleksi<button data-href="#Create-the-collection" class="anchor-icon" translate="no">
    ...
    </button></h3><p>Kemudian, buat koleksi menggunakan skema yang telah didefinisikan.</p>
<div class="multipleCode">
   <a href="#python">Python </a> <a href="#java">Java</a> <a href="#javascript">Node.js</a></div>
...
<button class="copy-code-btn"></button></code></pre>
<div class="language-python">
<p>Untuk informasi lebih lanjut mengenai metode dan parameter, lihat <a href="https://milvus.io/api-reference/pymilvus/v2.4.x/MilvusClient/Collections/create_collection.md">create_collection</a> dan <a href="https://milvus.io/api-reference/pymilvus/v2.4.x/MilvusClient/Collections/list_collections.md">list_collections</a>.</p>
</div>
<div class="language-java">
<p>Untuk informasi lebih lanjut mengenai metode dan parameter, lihat <a href="https://milvus.io/api-reference/java/v2.4.x/v2/Collections/createCollection.md">createCollection</a>.</p>
</div>
<div class="language-javascript">
<p>Untuk informasi lebih lanjut mengenai metode dan parameter, lihat <a href="https://milvus.io/api-reference/node/v2.4.x/Collections/createCollection.md">createCollection</a> dan <a href="https://milvus.io/api-reference/node/v2.4.x/Collections/listCollections.md">listCollections</a>.</p>
</div>
<h2 id="Create-indexes" class="common-anchor-header">Membuat indeks<button data-href="#Create-indexes" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>Indeks meningkatkan kinerja operasi pencarian dan kueri. Di Milvus, Anda dapat membuat indeks pada bidang vektor dan bidang skalar. Pada contoh ini, kita akan membuat indeks <code translate="no">IVF_FLAT</code> pada bidang vektor <code translate="no">vector</code> dan indeks <code translate="no">INVERTED</code> pada bidang larik <code translate="no">color_coord</code>. Untuk informasi lebih lanjut tentang indeks, lihat <a href="https://milvus.io/docs/index-vector-fields.md?tab=floating">Bidang Vektor Indeks</a> dan <a href="https://milvus.io/docs/index-scalar-fields.md">Bidang Skalar Indeks</a>.</p>
<h3 id="Index-vector-field" class="common-anchor-header">Bidang vektor indeks<button data-href="#Index-vector-field" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Membuat indeks pada bidang vektor dapat meningkatkan kinerja pencarian kemiripan vektor, yang diperlukan untuk setiap operasi pencarian.</p>
<p>Contoh di bawah ini membuat indeks bertipe <code translate="no">IVF_FLAT</code> pada bidang vektor <code translate="no">vector</code>.</p>
<div class="multipleCode">
   <a href="#python">Python </a> <a href="#java">Java</a> <a href="#javascript">Node.js</a></div>
...
<button class="copy-code-btn"></button></code></pre>
<div class="language-python">
<p>Untuk informasi lebih lanjut mengenai metode dan parameter, lihat <a href="https://milvus.io/api-reference/pymilvus/v2.4.x/MilvusClient/Management/prepare_index_params.md">prepare_index_params</a>, <a href="https://milvus.io/api-reference/pymilvus/v2.4.x/MilvusClient/Management/create_index.md">create_index</a>, dan <a href="https://milvus.io/api-reference/pymilvus/v2.4.x/MilvusClient/Management/describe_index.md">describe_index</a>.</p>
</div>
<div class="language-java">
<p>Untuk informasi lebih lanjut mengenai metode dan parameter, lihat <a href="https://milvus.io/api-reference/java/v2.4.x/v2/Management/IndexParam.md">IndexParam</a> dan <a href="https://milvus.io/api-reference/java/v2.4.x/v2/Management/createIndex.md">createIndex</a>.</p>
</div>
<div class="language-javascript">
<p>Untuk informasi lebih lanjut mengenai metode dan parameter, lihat <a href="https://milvus.io/api-reference/node/v2.4.x/Management/createIndex.md">createIndex</a>, dan <a href="https://milvus.io/api-reference/node/v2.4.x/Management/describeIndex.md">describeIndex</a>.</p>
</div>
<h3 id="Index-array-field" class="common-anchor-header">Bidang larik indeks<button data-href="#Index-array-field" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Membuat indeks pada bidang skalar dapat meningkatkan performa pengambilan kueri pada bidang tersebut, yang bersifat opsional namun disarankan untuk kumpulan data yang besar.</p>
<p>Dalam contoh ini, kita akan membuat indeks terbalik pada bidang larik <code translate="no">color_coord</code>. Hal ini akan memungkinkan kita untuk mempercepat pemfilteran berdasarkan bidang ini. Indeks terbalik menunjukkan kinerja keseluruhan yang sangat baik, secara signifikan mengungguli pemfilteran brute force menggunakan data mentah ketika data tidak sering diambil, dan mempertahankan kinerja yang sebanding dengan operasi pengambilan yang sering. Untuk informasi lebih lanjut tentang indeks terbalik, lihat <a href="/docs/id/scalar_index.md#Inverted-indexing">Indeks Skalar</a>.</p>
<div class="multipleCode">
   <a href="#python">Python </a> <a href="#java">Java</a> <a href="#javascript">Node.js</a></div>
<pre><code translate="no" class="language-python">index_params = client.prepare_index_params()
...
<h2 id="Insert-data" class="common-anchor-header">Memasukkan data<button data-href="#Insert-data" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Setelah koleksi dan indeks dibuat, kita dapat menyisipkan data ke dalam koleksi. Langkah ini menyisipkan 1.000 entitas ke dalam <code translate="no">test_collection</code>.</p>
<div class="multipleCode">
   <a href="#python">Python </a> <a href="#java">Java</a> <a href="#javascript">Node.js</a></div>
...
<button class="copy-code-btn"></button></code></pre>
<h2 id="Load-the-collection" class="common-anchor-header">Memuat koleksi<button data-href="#Load-the-collection" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>Setelah memasukkan data, kita perlu memuat koleksi agar tersedia untuk operasi pencarian dan kueri.</p>
<div class="multipleCode">
   <a href="#python">Python </a> <a href="#java">Java</a> <a href="#javascript">Node.js</a></div>
...
<button class="copy-code-btn"></button></code></pre>
<h2 id="Basic-scalar-filtering" class="common-anchor-header">Pemfilteran skalar dasar<button data-href="#Basic-scalar-filtering" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>Setelah semua data Anda ditambahkan, Anda dapat melakukan pencarian dan kueri menggunakan elemen dalam bidang larik dengan cara yang sama seperti yang Anda lakukan dengan bidang skalar standar.</p>
<div class="language-python">
<p>Untuk informasi lebih lanjut tentang parameter, lihat <a href="https://milvus.io/api-reference/pymilvus/v2.4.x/MilvusClient/Vector/search.md"><code translate="no">search()</code></a> dalam referensi SDK.</p>
</div>
<div class="language-java">
...
<span class="hljs-comment">// ]</span>
<button class="copy-code-btn"></button></code></pre>
<h2 id="Advanced-filtering" class="common-anchor-header">Pemfilteran tingkat lanjut<button data-href="#Advanced-filtering" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>Seperti apa yang kita miliki di bidang JSON, Milvus juga menyediakan operator pemfilteran tingkat lanjut untuk array, yaitu <code translate="no">ARRAY_CONTAINS</code>, <code translate="no">ARRAY_CONTAINS_ALL</code>, <code translate="no">ARRAY_CONTAINS_ANY</code>, dan <code translate="no">ARRAY_LENGTH</code>. Untuk informasi lebih lanjut mengenai operator, lihat <a href="#reference-on-array-filters">Referensi mengenai filter larik</a>.</p>
<ul>
<li><p>Memfilter semua entitas yang memiliki <code translate="no">10</code> dalam nilai <code translate="no">color_coord</code>.</p>
...
<li><p>Memfilter semua entitas yang memiliki <code translate="no">7</code> dan <code translate="no">8</code> dalam nilai <code translate="no">color_coord</code>.</p>
<p><div class="multipleCode">
<a href="#python">Python </a><a href="#java">Java</a><a href="#javascript">Node.js</a></div></p>
...
<span class="hljs-comment">// </span>
<button class="copy-code-btn"></button></code></pre></li>
<li><p>Memfilter semua entitas yang memiliki nilai 7, 8, atau 9 pada nilai <code translate="no">color_coord</code>.</p>
<p><div class="multipleCode">
<a href="#python">Python </a><a href="#java">Java</a><a href="#javascript">Node.js</a></div></p>
<pre><code translate="no" class="language-python">res = client.query(
    collection_name=<span class="hljs-string">&quot;test_collection&quot;</span>,
...
<span class="hljs-comment">// </span>
<button class="copy-code-btn"></button></code></pre></li>
<li><p>Menyaring entitas yang memiliki tepat empat elemen.</p>
<p><div class="multipleCode">
<a href="#python">Python </a><a href="#java">Java</a><a href="#javascript">Node.js</a></div></p>
<pre><code translate="no" class="language-python">res = client.query(
    collection_name=<span class="hljs-string">&quot;test_collection&quot;</span>,
...
<li><p>Elemen-elemen dalam bidang ARRAY harus memiliki tipe data yang sama, yang ditentukan oleh <code translate="no">element_type</code>. Semua tipe data valid yang tersedia untuk bidang skalar di Milvus dapat digunakan sebagai <code translate="no">element_type</code>. Untuk daftar tipe data yang didukung, lihat <a href="https://milvus.io/docs/schema.md#Supported-data-types">Tipe data yang didukung</a>.</p></li>
<li><p>Jumlah elemen dalam bidang ARRAY harus kurang dari atau sama dengan kapasitas maksimum bidang array, yang ditentukan oleh <code translate="no">max_capacity</code>.</p></li>
</ul>
<h2 id="Reference-on-array-filters" class="common-anchor-header">Referensi tentang filter array<button data-href="#Reference-on-array-filters" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>Saat bekerja dengan bidang array, Anda dapat mengapit nilai string dengan tanda petik ganda ("") atau tanda petik tunggal (''). Penting untuk dicatat bahwa Milvus menyimpan nilai string di dalam field array sebagaimana adanya tanpa melakukan semantic escape atau konversi. Sebagai contoh, <strong>'a "b',</strong> <strong>"a'b",</strong> <strong>'a'b',</strong> dan <strong>"a "b"</strong> akan disimpan apa adanya, sedangkan <strong>'a'b'</strong> dan <strong>"a "b"</strong> akan diperlakukan sebagai nilai yang tidak valid.</p>
<p>Asumsikan bahwa dua bidang larik <code translate="no">int_array</code> dan <code translate="no">var_array</code> telah didefinisikan. Tabel berikut ini menjelaskan ekspresi boolean yang didukung yang dapat Anda gunakan di <code translate="no">expr</code> saat mencari dengan bidang array.</p>
<table>
<thead>
<tr><th>Operator</th><th>Contoh</th><th>Keterangan</th></tr>
</thead>
<tbody>
<tr><td>&lt;</td><td><code translate="no">‘int_array[0] < 3’</code></td><td>Ekspresi ini bernilai benar jika nilai <code translate="no">int_array[0]</code> kurang dari 3.</td></tr>
<tr><td>&gt;</td><td><code translate="no">‘int_array[0] > 5’</code></td><td>Ekspresi ini bernilai benar jika nilai <code translate="no">int_array[0]</code> lebih besar dari 5.</td></tr>
<tr><td>==</td><td><code translate="no">‘int_array[0] == 0’</code></td><td>Ekspresi ini bernilai benar jika nilai <code translate="no">int_array[0]</code> sama dengan 0.</td></tr>
<tr><td>!=</td><td><code translate="no">‘var_array[0] != "a"’</code></td><td>Ekspresi ini bernilai benar jika nilai <code translate="no">var_array[0]</code> tidak sama dengan <code translate="no">“a”</code>.</td></tr>
<tr><td>&lt;=</td><td><code translate="no">‘int_array[0] <= 3’</code></td><td>Ekspresi ini bernilai benar jika nilai <code translate="no">int_array[0]</code> lebih kecil dari atau sama dengan 3.</td></tr>
<tr><td>&gt;=</td><td><code translate="no">‘int_array[0] >= 10’</code></td><td>Ekspresi ini bernilai benar jika nilai <code translate="no">int_array[0]</code> lebih besar dari atau sama dengan 10.</td></tr>
<tr><td>in</td><td><code translate="no">'var_array[0] in ["str1", “str2”]'</code></td><td>Ekspresi ini bernilai benar jika nilai <code translate="no">var_array[0]</code> adalah <code translate="no">“str1”</code> atau <code translate="no">“str2”</code>.</td></tr>
<tr><td>tidak di</td><td><code translate="no">'int_array[0] not in [1, 2, 3]'</code></td><td>Ekspresi ini bernilai benar jika nilai <code translate="no">int_array[0]</code> bukan 1, 2, atau 3.</td></tr>
<tr><td>+, -, *, /, %, **</td><td><code translate="no">‘int_array[0] + 100 > 200’</code></td><td>Ekspresi ini bernilai benar jika nilai <code translate="no">int_array[0] + 100</code> lebih besar dari 200.</td></tr>
<tr><td>suka (LIKE)</td><td><code translate="no">‘var_array[0] like "prefix%"’</code></td><td>Ekspresi ini bernilai benar jika nilai <code translate="no">var_array[0]</code> diawali dengan <code translate="no">“prefix”</code>.</td></tr>
<tr><td>dan (&amp;&amp;)</td><td><code translate="no">‘var_array[0] like “prefix%” && int_array[0] <= 100’</code></td><td>Ekspresi ini bernilai benar jika nilai <code translate="no">var_array[0]</code> diawali dengan <code translate="no">“prefix”</code>, dan nilai <code translate="no">int_array[0]</code> lebih kecil dari atau sama dengan 100.</td></tr>
<tr><td>atau (||)</td><td><code translate="no">‘var_array[0] like “prefix%” || int_array[0] <= 100’</code></td><td>Ekspresi ini bernilai benar jika nilai <code translate="no">var_array[0]</code> diawali dengan <code translate="no">“prefix”</code>, atau nilai <code translate="no">int_array[0]</code> lebih kecil dari atau sama dengan 100.</td></tr>
<tr><td>array_berisi (ARRAY_BERISI)</td><td><code translate="no">'array_contains(int_array, 100)'</code></td><td>Ekspresi ini bernilai benar jika <code translate="no">int_array</code> berisi elemen <code translate="no">100</code>.</td></tr>
<tr><td>array_contains_all (ARRAY_CONTAINS_ALL)</td><td><code translate="no">'array_contains_all(int_array, [1, 2, 3])'</code></td><td>Ekspresi ini bernilai benar jika <code translate="no">int_array</code> berisi semua elemen <code translate="no">1</code>, <code translate="no">2</code>, dan <code translate="no">3</code>.</td></tr>
<tr><td>array_berisi_apa saja (ARRAY_BERISI_APA SAJA)</td><td><code translate="no">'array_contains_any(var_array, ["a", "b", “c”])'</code></td><td>Ekspresi ini bernilai benar jika <code translate="no">var_array</code> berisi elemen apa pun dari <code translate="no">“a”</code>, <code translate="no">“b”</code>, dan <code translate="no">“c”</code>.</td></tr>
<tr><td>panjang_larik (array_length)</td><td><code translate="no">‘array_length(int_array) == 10’</code></td><td>Ekspresi ini bernilai benar jika <code translate="no">int_array</code> berisi tepat 10 elemen.</td></tr>
</tbody>
</table>
