...
<h1 id="Upgrade-Milvus-Cluster-with-Helm-Chart" class="common-anchor-header">Upgrade Milvus Cluster dengan Helm Chart<button data-href="#Upgrade-Milvus-Cluster-with-Helm-Chart" class="anchor-icon" translate="no">
    ...
    </button></h1><p>Panduan ini menjelaskan cara meng-upgrade cluster Milvus Anda dari v2.5.x ke v2.6.6 dengan menggunakan Helm Chart.</p>
<h2 id="Before-you-start" class="common-anchor-header">Sebelum Anda memulai<button data-href="#Before-you-start" class="anchor-icon" translate="no">
    ...
    </button></h3><p>Peningkatan dari Milvus 2.5.x ke 2.6.6 melibatkan perubahan arsitektur yang signifikan:</p>
<ul>
...
<li>Anda <strong>harus</strong> meng-upgrade ke v2.5.16 atau yang lebih baru dengan <code translate="no">mixCoordinator</code> diaktifkan sebelum meng-upgrade ke v2.6.6.</li>
</ul>
<p><strong>Keterbatasan Antrian Pesan</strong>: Ketika mengupgrade ke Milvus v2.6.6, anda harus mempertahankan pilihan antrian pesan anda saat ini. Beralih di antara sistem antrian pesan yang berbeda selama upgrade tidak didukung. Dukungan untuk mengubah sistem antrian pesan akan tersedia di versi mendatang.</p>
...
kubectl get pods
<button class="copy-code-btn"></button></code></pre>
<h3 id="Step-3-Upgrade-to-v266" class="common-anchor-header">Langkah 3: Tingkatkan ke v2.6.6<button data-href="#Step-3-Upgrade-to-v266" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h3><p>Setelah v2.5.16 berjalan dengan sukses dengan <code translate="no">mixCoordinator</code>, tingkatkan ke v2.6.6:</p>
<pre><code translate="no" class="language-bash">helm upgrade my-release zilliztech/milvus \
