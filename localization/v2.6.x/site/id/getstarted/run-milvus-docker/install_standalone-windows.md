...
related_key: Docker
summary: >-
  Pelajari cara menginstal Milvus standalone menggunakan Docker Desktop untuk
  Windows.
...
<h1 id="Run-Milvus-in-Docker-Windows" class="common-anchor-header">Menjalankan Milvus di Docker (Windows)<button data-href="#Run-Milvus-in-Docker-Windows" class="anchor-icon" translate="no">
    ...
    </button></h1><p>Halaman ini menjelaskan cara menjalankan Milvus di Windows menggunakan Docker Desktop untuk Windows.​</p>
<h2 id="Prerequisites​" class="common-anchor-header">Persyaratan Awal​<button data-href="#Prerequisites​" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<li><p>Instal Python 3.8 atau versi yang lebih baru.</p></li>
</ul>
<h2 id="Run-Milvus-in-Docker​" class="common-anchor-header">Menjalankan Milvus di Docker​<button data-href="#Run-Milvus-in-Docker​" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Milvus menyediakan skrip instalasi untuk menginstalnya sebagai kontainer Docker. Setelah Anda menginstal Docker Desktop di Microsoft Windows, Anda dapat mengakses Docker CLI dari PowerShell atau Command Prompt Windows dalam mode <strong>administrator</strong> serta dari WSL 2. ​</p>
...
<li><p>Jalankan skrip yang telah diunduh untuk memulai Milvus sebagai kontainer Docker.​</p>
...
<li><p>Embed etcd diinstal bersama Milvus di kontainer yang sama dan berjalan di port <strong>2379</strong>. Berkas konfigurasinya dipetakan ke ` <strong>embedEtcd.yaml</strong> ` di folder saat ini.​</p></li>
...
<h3 id="From-WSL-2​" class="common-anchor-header">Dari WSL 2​<button data-href="#From-WSL-2​" class="anchor-icon" translate="no">
    ...
    </button></h3><p>Jika Anda lebih suka menjalankan Milvus menggunakan perintah Linux dan skrip shell di Windows, pastikan Anda sudah menginstal WSL 2. Untuk detail tentang cara menginstal WSL 2, Anda dapat merujuk ke <a href="https://learn.microsoft.com/en-us/windows/wsl/install#install-wsl-command">artikel Microsoft</a> ini.​</p>
<ol>
...
<h2 id="Run-Milvus-with-Docker-Compose​" class="common-anchor-header">Jalankan Milvus dengan Docker Compose​<button data-href="#Run-Milvus-with-Docker-Compose​" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<p>Anda juga dapat menjalankan perintah Docker Compose versi Linux jika Anda telah menginstal WSL 2.​</p>
<h3 id="From-WSL-2​" class="common-anchor-header">Dari WSL 2​<button data-href="#From-WSL-2​" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
    ...
    </button></h3><p>Prosedurnya mirip dengan menggunakan Docker Compose untuk menginstal Milvus di sistem Linux.</p>
<ol>
<li><p>Jalankan WSL 2.</p>
<pre><code translate="no" class="language-powershell">C:\&gt;wsl --install​
Ubuntu already installed.​
...
<h2 id="FAQs​" class="common-anchor-header">Pertanyaan Umum​<button data-href="#FAQs​" class="anchor-icon" translate="no">
    ...
    </button></h3><p>Setelah Anda menginstal Docker Desktop di Windows, Anda mungkin mengalami kesalahan " <code translate="no">Docker Engine stopped</code> " jika komputer Anda tidak dikonfigurasi dengan benar. Dalam hal ini, Anda mungkin perlu melakukan pemeriksaan berikut:​</p>
<ol>
<li><p>Periksa apakah virtualisasi telah diaktifkan.​</p>
<p>Anda dapat memeriksa apakah virtualisasi telah diaktifkan dengan melihat tab " <strong>Performance</strong> " di <strong>Task Manager</strong>.​</p>
<p><span class="img-wrapper">
   ...
   <span>Virtualisasi di Task Manager</span>
  
 </span></p>
<p>Jika virtualisasi dinonaktifkan, Anda mungkin perlu memeriksa pengaturan BIOS pada firmware motherboard Anda. Cara mengaktifkan virtualisasi di pengaturan BIOS bervariasi tergantung pada produsen motherboard. Untuk motherboard ASUS, misalnya, Anda dapat merujuk ke <a href="https://www.asus.com/support/faq/1043786/">artikel ini</a> tentang cara mengaktifkan virtualisasi.​</p>
<p>Kemudian, Anda perlu me-restart komputer dan mengaktifkan Hyper-V. Untuk detailnya, lihat <a href="https://learn.microsoft.com/en-us/virtualization/hyper-v-on-windows/quick-start/enable-hyper-v#enable-the-hyper-v-role-through-settings">artikel Microsoft</a> ini.​</p></li>
<li><p>Periksa apakah Layanan Docker Desktop telah dijalankan.</p>
<p>Anda dapat menjalankan perintah berikut untuk memulai Layanan Docker Desktop.</p>
...
<p>Anda perlu pergi ke direktori instalasi Docker Desktop dan menjalankan perintah ` <code translate="no">.\DockerCli.exe -SwitchDaemon</code> ` untuk menjalankan Docker Daemon.​</p>
...
<p><span class="img-wrapper">
   ...
   <span>Jalankan Docker Desktop sebagai Administrator</span>
  
 </span></p></li>
</ol>
<h3 id="How-can-I-deal-with-WSL-related-issues-while-deploying-Milvus​" class="common-anchor-header">Bagaimana cara mengatasi masalah terkait WSL saat mengimplementasikan Milvus?​<button data-href="#How-can-I-deal-with-WSL-related-issues-while-deploying-Milvus​" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<ol>
<li><p>Pastikan opsi “Gunakan mesin berbasis WSL 2” telah dicentang di <strong>Pengaturan</strong> &gt; <strong>Umum</strong>. ​</p>
<p><span class="img-wrapper">
...
<p><span class="img-wrapper">
   ...
   <span>Memilih distribusi WSL 2 di Pengaturan Docker Desktop</span>
  
 </span></p></li>
</ol>
<h3 id="How-can-I-deal-with-the-volume-related-errors-prompted-during-Milvus-startup-that-reads-Read-config-failed​" class="common-anchor-header">Bagaimana cara mengatasi kesalahan terkait volume yang muncul saat Milvus dimulai dengan pesan “ <code translate="no">Read config failed</code> ”?​<button data-href="#How-can-I-deal-with-the-volume-related-errors-prompted-during-Milvus-startup-that-reads-Read-config-failed​" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<p>Untuk mengatasi kesalahan yang muncul saat Milvus dimulai dengan pesan “Read config failed,” Anda perlu memeriksa apakah volume yang dipasang ke dalam kontainer Milvus sudah benar. Jika volume sudah dipasang dengan benar ke dalam kontainer, Anda dapat menggunakan perintah ` <code translate="no">docker exec</code> ` untuk masuk ke dalam kontainer dan menampilkan daftar folder <strong>`/milvus/configs</strong> ` sebagai berikut:​</p>
<p><span class="img-wrapper">
   ...
   <span>Menampilkan daftar berkas konfigurasi Milvus</span>
