---
id: abs.md
title: ワークロードアイデンティティによるBlobストレージアクセスの構成
related_key: 'blob storage, workload identity, iam'
summary: Workload IdentityでBlob Storageを構成する方法を学ぶ。
---
<h1 id="Configure-Blob-Storage-Access-by-Workload-Identity" class="common-anchor-header">ワークロードアイデンティティによるBlobストレージアクセスの構成<button data-href="#Configure-Blob-Storage-Access-by-Workload-Identity" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h1><p>このトピックでは、helmを使用してMilvusをインストールする際に、Workload Identityを使用してAzure Blob Storageアクセスを構成する方法を紹介します。 詳細は、<a href="https://azure.github.io/azure-workload-identity/docs/introduction.html">Workload Identityを</a>参照してください。</p>
<h2 id="Configure-applications-to-use-Workload-Identity" class="common-anchor-header">Workload Identityを使用するアプリケーションの設定<button data-href="#Configure-applications-to-use-Workload-Identity" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<ul>
<li>OIDC IssuerとWorkload IdentityでAKSクラスタを更新する。</li>
</ul>
...
<button class="copy-code-btn"></button></code></pre>
<ul>
<li>ユーザー割り当てのマネージドIDを作成し、ロールを割り当てます。</li>
</ul>
...
<button class="copy-code-btn"></button></code></pre>
<ul>
<li>アイデンティティとサービスアカウント発行者およびサブジェクトの間でフェデレートされたIDクレデンシャルを確立する。</li>
</ul>
