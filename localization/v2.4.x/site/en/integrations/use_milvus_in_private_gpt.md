...
<h2 id="Install-Available-Modules" class="common-anchor-header">Install Available Modules<button data-href="#Install-Available-Modules" class="anchor-icon" translate="no">
    ...
    </button></h2><p>PrivateGPT allows customization of the setup. We need to specify some modules settings. In this tutorial, we will use the following modules:</p>
<ul>
...
<button class="copy-code-btn"></button></code></pre>
<p>Install the models to be used, the default <code translate="no">settings-ollama.yaml</code> is configured to user llama3.1 8b LLM (~4GB) and nomic-embed-text Embeddings (~275MB)</p>
...
<tbody>
<tr><td>uri</td><td>Default is set to “local_data/private_gpt/milvus/milvus_local.db” as a local file; you can also set up a more performant Milvus server on docker or k8s e.g.http://localhost:19530, as your uri; To use Zilliz Cloud, adjust the uri and token to Endpoint and Api key in Zilliz Cloud.</td></tr>
