...
<h1 id="minio-related-Configurations" class="common-anchor-header">MinIO 相关配置<button data-href="#minio-related-Configurations" class="anchor-icon" translate="no">
    ...
    </button></h1><p>MinIO/S3/GCS 或任何其他服务的相关配置都支持 S3 API，它负责 Milvus 的数据持久性。</p>
  ...
  <tbody>
    <tr>
      <td>
        ...
        <li>minio.address 和 minio.port 将共同生成对 MinIO 或 S3 服务的有效访问。</li>      
  ...
  <tbody>
    <tr>
      <td>
        ...
        <li>要在多个 Milvus 实例之间共享 MinIO 实例，请考虑在启动每个 Milvus 实例前将此值更改为不同的值。有关详细信息，请参阅操作常见问题解答。</li>      
  ...
  <tbody>
    <tr>
      <td>
        ...
        <li>要在多个 Milvus 实例之间共享 MinIO 实例，请考虑在启动每个 Milvus 实例之前将其更改为不同的值。有关详细信息，请参阅操作常见问题解答。</li>      
  ...
  <tbody>
    <tr>
      <td>
        ...
        <li>如果其他云提供商使用虚拟主机式桶，则可使用 "aliyun"。</li>      
