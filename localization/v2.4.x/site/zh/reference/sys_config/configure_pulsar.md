  ...
  <tbody>
    <tr>
      <td>
        ...
        <li>默认情况下，Pulsar 在单条信息中最多可传输 5 MB 的数据。当插入数据的大小大于此值时，代理会将数据分割成多条信息，以确保能正确传输。</li>      
        <li>如果 Pulsar 中的相应参数保持不变，增加该配置会导致 Milvus 故障，而减少该配置则没有任何好处。</li>      </td>
      <td>5242880</td>
    </tr>
  </tbody>
</table>
  ...
  <tbody>
    <tr>
      <td>
        ...
        <li>要在多个 Milvus 实例之间共享一个 Pulsar 实例，可以在启动 Milvus 实例之前，将其更改为 Pulsar 租户，而不是每个 Milvus 实例的默认租户。不过，如果不想使用 Pulsar 多租户，建议将 msgChannel.chanNamePrefix.cluster 更改为不同的值。</li>      </td>
      <td>公共</td>
    </tr>
  </tbody>
</table>
