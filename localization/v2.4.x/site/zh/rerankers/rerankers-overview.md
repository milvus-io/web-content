...
order: 1
summary: PyMilvus 模型库集成了 rerank 函数，用于优化初始搜索返回结果的顺序。
title: 重新排名概述
---
...
<p>Reranker 通常在第一阶段检索后使用，一般通过向量近似近邻（ANN）技术完成。虽然 ANN 检索能高效地获取大量潜在相关结果，但它们可能并不总是根据与查询的实际语义相关性来确定结果的优先级。在这种情况下，reerankers 通常利用先进的机器学习模型（如 BERT 或其他基于 Transformer 的模型），通过更深入的上下文分析来优化结果顺序。通过这样做，rerankers 可以显著提高最终呈现给用户的结果的准确性和相关性。</p>
<p>PyMilvus 模型库集成了 rerank 函数，用于优化初始搜索返回结果的顺序。从 Milvus 获取最近嵌入后，您可以利用这些重排工具来完善搜索结果，从而提高搜索结果的精确度。</p>
<table>
<thead>
<tr><th>重新排名功能</th><th>应用程序接口或开源</th></tr>
</thead>
<tbody>
...
<tr><td><a href="https://milvus.io/api-reference/pymilvus/v2.4.x/Rerankers/VoyageRerankFunction/VoyageRerankFunction.md">航海</a></td><td>应用程序接口</td></tr>
...
<ul>
<li><p>在使用开源 reranker 之前，请确保下载并安装所有必需的依赖项和模型。</p></li>
<li><p>对于基于 API 的重排器，请从提供商处获取 API 密钥，并将其设置在相应的环境变量或参数中。</p></li>
</ul>
</div>
<h2 id="Example-1-Use-BGE-rerank-function-to-rerank-documents-according-to-a-query" class="common-anchor-header">示例 1：使用 BGE rerank 函数根据查询对文档进行重排<button data-href="#Example-1-Use-BGE-rerank-function-to-rerank-documents-according-to-a-query" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>在本例中，我们将演示如何使用<a href="/docs/zh/rerankers-bge.md">BGE reranker</a>根据特定查询对搜索结果进行重新排序。</p>
<p>要使用带有<a href="https://github.com/milvus-io/milvus-model">PyMilvus 模型库</a>的重排器，首先要安装 PyMilvus 模型库以及包含所有必要重排实用程序的模型子包：</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>然后，创建一个<code translate="no">BGERerankFunction</code> 实例用于重排：</p>
...
<button class="copy-code-btn"></button></code></pre>
<h2 id="Example-2-Use-a-reranker-to-enhance-relevance-of-search-results" class="common-anchor-header">例 2：使用排序器提高搜索结果的相关性<button data-href="#Example-2-Use-a-reranker-to-enhance-relevance-of-search-results" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>在本指南中，我们将探讨如何利用 PyMilvus 中的<code translate="no">search()</code> 方法进行相似性搜索，以及如何使用重排序器增强搜索结果的相关性。我们将使用以下数据集进行演示：</p>
...
<li><code translate="no">doc_vector</code>:代表文档的向量嵌入。有关生成嵌入的指导，请参阅<a href="/docs/zh/embeddings.md">嵌入</a>。</li>
<li><code translate="no">doc_text</code>:文档的文本内容。</li>
</ul>
<h3 id="Preparations" class="common-anchor-header">准备工作</h3><p>在启动相似性搜索之前，您需要与 Milvus 建立连接，创建一个数据集，并准备和插入数据到该数据集中。以下代码片段说明了这些初步步骤。</p>
...
<button class="copy-code-btn"></button></code></pre>
<h3 id="Use-a-reranker-to-enhance-search-results" class="common-anchor-header">使用检索器增强搜索结果</h3><p>然后，通过重新排序步骤提高搜索结果的相关性。在本例中，我们使用 PyMilvus 内置的<code translate="no">CrossEncoderRerankFunction</code> 对结果进行重排，以提高准确性。</p>
