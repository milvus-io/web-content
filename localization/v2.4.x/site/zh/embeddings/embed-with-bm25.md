...
<button class="copy-code-btn"></button></code></pre>
<p>BM25 算法在处理文本时，首先使用内置分析器将文本分解成词块，如图所示的<strong>"artifici"、</strong> <strong>"intellig</strong> <strong>"</strong>和<strong>"academ</strong> <strong>"</strong>等英语词块<strong>。</strong>然后，它收集这些标记的统计数据，评估它们在文档中的出现频率和分布情况。BM25 的核心是根据每个标记的重要性计算其相关性得分，较罕见的标记得分较高。这一简洁的过程可以根据与查询的相关性对文档进行有效的排序。</p>
...
bm25_ef.fit(corpus)
<button class="copy-code-btn"></button></code></pre>
<p>然后，使用<strong>encode_documents()</strong>为文档创建嵌入：</p>
...
Sparse dim: 21 (1, 21)
<button class="copy-code-btn"></button></code></pre>
<p>使用<strong>encode_queries()</strong>方法为查询创建嵌入式内容：</p>
...
<p>使用<strong>BM25EmbeddingFunction</strong> 时，请注意<strong>encoding_queries()</strong>和<strong>encoding_documents(</strong> <strong>)</strong>操作在数学上不能互换。因此，没有可用的<strong>bm25_ef(text)</strong>。</p>
