---
id: cli_overview.md
summary: Milvus 命令行界面（CLI）是一种命令行工具，支持数据库连接、数据操作以及数据导入和导出。
title: Milvus 命令行界面
---
<h1 id="Milvus-Command-Line-Interface" class="common-anchor-header">Milvus 命令行界面<button data-href="#Milvus-Command-Line-Interface" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h1><p>Milvus 命令行界面（CLI）是一种命令行工具，支持数据库连接、数据操作以及数据导入和导出。它基于<a href="https://github.com/milvus-io/pymilvus">Milvus Python SDK</a>，允许使用交互式命令行提示通过终端执行命令。</p>
<h2 id="Recommended-version" class="common-anchor-header">推荐版本<button data-href="#Recommended-version" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>在下表中，你可以根据使用的 Milvus 版本找到 PyMilvus 和 Milvus_CLI 的推荐版本。</p>
<table>
<thead>
...
<div class="alert note">Milvus 2.0.0-RC7 及更高版本与 2.0.0-RC6 及更早版本不向后兼容，原因是存储格式有所更改。</div>
<h2 id="Current-version" class="common-anchor-header">当前版本<button data-href="#Current-version" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>Milvus_CLI 的当前版本是 1.0.0。要查找已安装的版本并确定是否需要更新，请运行<code translate="no">milvus_cli --version</code> 。</p>
