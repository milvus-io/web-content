...
<h1 id="Birdwatcher" class="common-anchor-header">Birdwatcher<button data-href="#Birdwatcher" class="anchor-icon" translate="no">
    ...
    </button></h1><p>Milvus 是一种无状态向量数据库，它将读写分离，并让 etcd 扮演单一状态源的角色。所有协调人员都必须先从 etcd 中查询状态，然后才能对其进行任何更改。一旦用户需要检查或清理状态，他们就需要一个与 etcd 通信的工具。这就是 Birdwatcher 的用武之地。</p>
<p>Birdwatcher 是 Milvus 的调试工具。使用它连接到 etcd，你就可以检查 Milvus 系统的状态或即时配置。</p>
