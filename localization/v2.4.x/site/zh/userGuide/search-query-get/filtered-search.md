...
summary: >-
  ANN 搜索能找到与指定向量嵌入最相似的向量嵌入。但是，搜索结果不一定总是正确的。您可以在搜索请求中包含过滤条件，这样 Zilliz Cloud 就会在进行
  ANN 搜索前进行元数据过滤，将搜索范围从整个 Collections 缩小到只搜索符合指定过滤条件的实体。
---
<h1 id="Filtered-Search​" class="common-anchor-header">过滤搜索<button data-href="#Filtered-Search​" class="anchor-icon" translate="no">
    ...
    </button></h1><p>ANN 搜索能找到与指定向量嵌入最相似的向量嵌入。但是，搜索结果不一定总是正确的。您可以在搜索请求中包含过滤条件，以便 Zilliz Cloud 在进行 ANN 搜索前进行元数据过滤，将搜索范围从整个 Collections 缩小到仅与指定过滤条件相匹配的实体。</p>
<h2 id="Overview" class="common-anchor-header">概述<button data-href="#Overview" class="anchor-icon" translate="no">
    ...
    </button></h2><p>如果 Collections 包含向量嵌入及其元数据，您可以在 ANN 搜索前过滤元数据，以提高搜索结果的相关性。Zilliz Cloud 收到带有过滤条件的搜索请求后，会将搜索范围限制在符合指定过滤条件的实体内。</p>
<p>
...
<p>如上图所示，搜索请求携带<code translate="no">chunk like % red %</code> 作为过滤条件，表明 Zilliz Cloud 应在<code translate="no">chunk</code> 字段中包含<code translate="no">red</code> 的所有实体内进行 ANN 搜索。具体来说，Zilliz Cloud 会执行以下操作。</p>
<ul>
...
<h2 id="Examples" class="common-anchor-header">示例<button data-href="#Examples" class="anchor-icon" translate="no">
    ...
    </button></h2><p>本节演示如何进行过滤搜索。本节中的代码片段假定您的 Collections 中已包含以下实体。每个实体都有四个字段，即<strong>id</strong>、<strong>向量</strong>、<strong>颜色</strong>和<strong>喜欢</strong>。</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>有关元数据过滤中可使用的操作符的更多信息，请参阅<a href="/docs/zh/boolean.md">元数据过滤</a>。</p>
