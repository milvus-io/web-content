...
title: 标准标记符号
summary: Milvus 中的 "标准 "标记符根据空格和标点符号分割文本，因此适用于大多数语言。
---
<h1 id="Standard​" class="common-anchor-header">标准<button data-href="#Standard​" class="anchor-icon" translate="no">
    ...
    </button></h1><p>Milvus 中的<code translate="no">standard</code> 标记器根据空格和标点符号分割文本，因此适用于大多数语言。</p>
<h2 id="Configuration​" class="common-anchor-header">配置<button data-href="#Configuration​" class="anchor-icon" translate="no">
    ...
    </button></h2><p>要配置使用<code translate="no">standard</code> 标记符的分析器，请在<code translate="no">analyzer_params</code> 中将<code translate="no">tokenizer</code> 设置为<code translate="no">standard</code> 。</p>
