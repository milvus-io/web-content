...
<button class="copy-code-btn"></button></code></pre>
<h2 id="Create-a-collection" class="common-anchor-header">创建数据集<button data-href="#Create-a-collection" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>本节涉及 Milvus 和为本用例设置数据库。在 Milvus 中，我们需要建立一个数据集并为其建立索引。</p>
...
<h2 id="Insert-data" class="common-anchor-header">插入数据<button data-href="#Insert-data" class="anchor-icon" translate="no">
    ...
    </button></h2><p>建立数据集后，我们需要开始插入数据。这分为三个步骤</p>
<ul>
<li>读取数据、</li>
<li>嵌入原始问题，以及</li>
<li>将数据插入我们刚刚在 Milvus 上创建的集合。</li>
</ul>
<p>在本例中，数据包括原始问题、原始问题的嵌入以及原始问题的答案。</p>
...
<h2 id="Ask-questions" class="common-anchor-header">提问<button data-href="#Ask-questions" class="anchor-icon" translate="no">
    ...
    </button></h2><p>将所有数据插入 Milvus 数据集中后，我们就可以向系统提问了，方法是使用 Cohere 嵌入我们的问题短语，然后在数据集中进行搜索。</p>
