...
<h1 id="Movie-Search-Using-Milvus-and-SentenceTransformers" class="common-anchor-header">使用 Milvus 和 SentenceTransformers 进行电影搜索<button data-href="#Movie-Search-Using-Milvus-and-SentenceTransformers" class="anchor-icon" translate="no">
    ...
    </button></h1><p>在本示例中，我们将使用 Milvus 和 SentenceTransformers 库进行维基百科文章搜索。我们要搜索的数据集是<a href="https://huggingface.co/datasets/vishnupriyavr/wiki-movie-plots-with-summaries">维基百科电影情节与摘要</a>，托管在 HuggingFace 上。</p>
<p>让我们开始吧！</p>
...
<h2 id="Downloading-and-Opening-the-Dataset" class="common-anchor-header">下载和打开数据集<button data-href="#Downloading-and-Opening-the-Dataset" class="anchor-icon" translate="no">
    ...
    </button></h2><p>只需一行，<code translate="no">datasets</code> 就能让我们下载并打开数据集。库将在本地缓存数据集，并在下次运行时使用该副本。每一行都包含一部电影的详细信息，该电影附有一篇维基百科文章。我们只使用<code translate="no">Title</code> 和<code translate="no">PlotSummary</code> 列。</p>
...
<button class="copy-code-btn"></button></code></pre>
<ol start="3">
<li>定义向量搜索索引算法。Milvus Lite 实现了暴力搜索和 HNSW，而 Milvus Standalone 和 Milvus Distributed 则实现了多种方法。对于这种规模的数据，简单的暴力搜索就足够了。</li>
</ol>
...
<button class="copy-code-btn"></button></code></pre>
<p>完成这些步骤后，我们就可以将数据插入 Collections 并执行搜索了。任何添加的数据都将自动编入索引，并立即可供搜索。如果数据非常新，搜索可能会慢一些，因为蛮力搜索将用于仍在索引过程中的数据。</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>我们循环浏览数据行，嵌入情节摘要字段，并将实体插入向量数据库。一般来说，你应该在成批数据项上执行这一步骤，以最大限度地提高嵌入模型的 CPU 或 GPU 吞吐量，我们在这里就是这样做的。</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>为了安全起见，我们会刷新数据写入队列，并检查数据库中是否存在预期数量的元素。</p>
...
<h2 id="Performing-the-Search" class="common-anchor-header">执行搜索<button data-href="#Performing-the-Search" class="anchor-icon" translate="no">
    ...
    </button></h2><p>将所有数据插入 Milvus 后，我们就可以开始执行搜索了。在本例中，我们将根据情节搜索电影。由于我们进行的是批量搜索，因此搜索时间将在电影搜索中共享。(你能猜到基于电影搜索的预期结果是什么吗？）</p>
