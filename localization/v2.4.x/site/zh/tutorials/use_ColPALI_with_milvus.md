...
<p>现代检索模型通常使用单一嵌入来表示文本或图像。而 ColBERT 是一种神经模型，它利用每个数据实例的嵌入列表，并采用 "MaxSim "操作符来计算两个文本之间的相似度。除了文本数据，图、表和图表也包含丰富的信息，而这些信息在基于文本的信息检索中往往被忽略。</p>
<p>$$ S_{q,d} := \sum_{i \in |E_q|}\max_{j \in |E_d|}E_{q_i}\cdot E_{d_j}^T $$ MaxSim 函数通过查看标记嵌入来比较查询和文档（你正在搜索的内容）。对于查询中的每个单词，它都会从文档中选出最相似的单词（使用余弦相似度或平方 L2 距离），并将查询中所有单词的最大相似度相加。</p>
...
<p>在本笔记本中，我们将这种多向量表示法称为 "ColBERT 嵌入"，以示通用。不过，实际使用的模型是<strong>ColPALI 模型</strong>。我们将演示如何使用 Milvus 进行多向量检索。在此基础上，我们将介绍如何使用 ColPALI 根据给定查询检索网页。</p>
...
<li>如果使用<a href="https://zilliz.com/cloud">Zilliz Cloud</a>（Milvus 的全托管云服务），请调整<code translate="no">uri</code> 和<code translate="no">token</code> ，它们与 Zilliz Cloud 中的<a href="https://docs.zilliz.com/docs/on-zilliz-cloud-console#cluster-details">公共端点和 API 密钥</a>相对应。</li>
</ul>
</div>
