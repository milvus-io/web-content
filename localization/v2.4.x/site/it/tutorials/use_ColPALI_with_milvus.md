...
<p>$$ S_{q,d} := \sum_{i \in |E_q|} \max_{j \\code(01)\code(01)\} in |E_d|} E_{q_i} \cdot E_{d_j}^T $$ La funzione MaxSim confronta una query con un documento (ciò che si sta cercando) osservando le loro incorporazioni di token. Per ogni parola della query, sceglie la parola più simile dal documento (usando la somiglianza coseno o la distanza L2 al quadrato) e somma queste somiglianze massime tra tutte le parole della query.</p>
