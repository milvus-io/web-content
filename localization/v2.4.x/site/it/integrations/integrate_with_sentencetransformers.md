...
<h1 id="Movie-Search-Using-Milvus-and-SentenceTransformers" class="common-anchor-header">Ricerca di film con Milvus e SentenceTransformers<button data-href="#Movie-Search-Using-Milvus-and-SentenceTransformers" class="anchor-icon" translate="no">
    ...
    </button></h1><p>In questo esempio, esamineremo una ricerca di articoli di Wikipedia utilizzando Milvus e la libreria SentenceTransformers. L'insieme di dati che stiamo cercando è <a href="https://huggingface.co/datasets/vishnupriyavr/wiki-movie-plots-with-summaries">Wikipedia Movie Plots with Summaries</a>, ospitato su HuggingFace.</p>
<p>Iniziamo!</p>
<h2 id="Required-Libraries" class="common-anchor-header">Librerie necessarie<button data-href="#Required-Libraries" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Per questo esempio, utilizzeremo <code translate="no">pymilvus</code> per connetterci a Milvus, <code translate="no">sentence-transformers</code> per generare embeddings vettoriali e <code translate="no">datasets</code> per scaricare il dataset di esempio.</p>
...
<button class="copy-code-btn"></button></code></pre>
<ol start="3">
<li>Definire l'algoritmo di indicizzazione della ricerca vettoriale. Milvus Lite implementa la ricerca a forza bruta e HNSW, mentre Milvus Standalone e Milvus Distributed implementano un'ampia gamma di metodi. Per questa scala di dati, la ricerca ingenua a forza bruta è sufficiente.</li>
</ol>
...
<button class="copy-code-btn"></button></code></pre>
<p>Si esegue un ciclo sulle righe dei dati, si incorpora il campo di riepilogo della trama e si inseriscono le entità nel database dei vettori. In generale, si dovrebbe eseguire questo passaggio su gruppi di dati per massimizzare il rendimento della CPU o della GPU per il modello di embedding, come facciamo qui.</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>Per sicurezza, si scarica la coda di scrittura dei dati e si controlla che il numero previsto di elementi sia presente nel database.</p>
...
<p>Questa operazione è relativamente lunga perché l'incorporazione richiede tempo. Questa fase richiede circa 2 minuti utilizzando la CPU su un MacBook Pro 2023 e sarà molto più veloce con le GPU dedicate. Fate una pausa e godetevi una tazza di caffè!</p>
</div>
<h2 id="Performing-the-Search" class="common-anchor-header">Esecuzione della ricerca<button data-href="#Performing-the-Search" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Dopo aver inserito tutti i dati in Milvus, possiamo iniziare a eseguire le nostre ricerche. In questo esempio, cercheremo i film in base alla trama. Poiché stiamo eseguendo una ricerca in batch, il tempo di ricerca è condiviso tra le ricerche dei film. (Riuscite a indovinare il risultato desiderato in base alla ricerca dei film?).</p>
