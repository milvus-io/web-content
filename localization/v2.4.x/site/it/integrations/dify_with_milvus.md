...
<p>Per il file <code translate="no">MILVUS_TOKEN</code>, se non è stato impostato un token per il server Milvus, è possibile impostarlo su una stringa vuota come <code translate="no">MILVUS_TOKEN=</code>, altrimenti è necessario impostarlo sul proprio token Milvus. Per ulteriori informazioni su come impostare il token in Milvus, potete consultare la <a href="https://milvus.io/docs/authenticate.md?tab=docker#Update-user-password">pagina Autentica</a>.</p>
