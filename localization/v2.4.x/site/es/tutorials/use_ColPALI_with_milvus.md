...
<p>Los modelos de recuperación modernos suelen utilizar una única incrustación para representar texto o imágenes. ColBERT, sin embargo, es un modelo neuronal que utiliza una lista de incrustaciones para cada instancia de datos y emplea una operación "MaxSim" para calcular la similitud entre dos textos. Además de los datos textuales, las figuras, las tablas y los diagramas también contienen abundante información, que a menudo no se tiene en cuenta en la recuperación de información basada en texto.</p>
<p>$$ S_{q,d} := \sum_{i \in |E_q|} \max_j |en |E_d|} E_{q_i} \cdot E_{d_j}^T $$ La función MaxSim compara una consulta con un documento (lo que se está buscando) mirando sus incrustaciones de token. Para cada palabra de la consulta, elige la palabra más similar del documento (utilizando la similitud coseno o la distancia L2 al cuadrado) y suma estas similitudes máximas entre todas las palabras de la consulta.</p>
...
<p>Por último, recuperamos el nombre de la página original. Con ColPALI, podemos recuperar documentos multimodales sin necesidad de complejas técnicas de procesamiento para extraer texto e imágenes de los documentos. Al aprovechar grandes modelos de visión, se puede analizar más información, como tablas y figuras, sin una pérdida significativa de información.</p>
