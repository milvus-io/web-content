...
<h2 id="Configuration​" class="common-anchor-header">Configuración<button data-href="#Configuration​" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Para aplicar el analizador <code translate="no">standard</code> a un campo, basta con configurar <code translate="no">type</code> en <code translate="no">standard</code> en <code translate="no">analyzer_params</code>, e incluir los parámetros opcionales que sean necesarios.</p>
<pre><code translate="no" class="language-python">analyzer_params = {​
