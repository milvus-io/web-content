---
id: s3.md
title: Configurar Acceso S3 por Rol IAM
related_key: 's3, storage, iam'
summary: Aprenda a configurar s3 con IAM Role.
---
<h1 id="Configure-S3-Access-by-IAM-Role" class="common-anchor-header">Configurar Acceso S3 por Rol IAM<button data-href="#Configure-S3-Access-by-IAM-Role" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h1><p>Este tema presenta cómo configurar el acceso s3 por Rol IAM cuando instala Milvus con helm. Para más detalles, refiérase a <a href="https://docs.aws.amazon.com/eks/latest/userguide/associate-service-account-role.html">Roles IAM</a>.</p>
...
<li><p>Por favor habilite OIDC cuando cree un cluster EKS usando eksctl. Para más detalles, refiérase a <a href="https://docs.aws.amazon.com/eks/latest/userguide/enable-iam-roles-for-service-accounts.html">IAM OIDC</a>.</p></li>
<li><p>Esta guía asume que ya ha <a href="/docs/es/v2.4.x/eks.md">implementado un clúster Milvus en AWS con Kubernetes</a>.</p></li>
</ul>
<h2 id="Associate-an-IAM-role-with-a-Kubernetes-service-account" class="common-anchor-header">Asociar un rol de IAM con una cuenta de servicio de Kubernetes<button data-href="#Associate-an-IAM-role-with-a-Kubernetes-service-account" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<li><p>Cree un bucket AWS S3.</p>
<p>Lea <a href="https://docs.aws.amazon.com/AmazonS3/latest/userguide/bucketnamingrules.html">Bucket Naming Rules</a> y observe las reglas de nomenclatura al asignar un nombre a su bucket de AWS S3.</p>
...
<li><p>Cree una política de IAM para leer y escribir objetos en el bucket creado anteriormente. <strong>Sustituya el nombre del bucket por el suyo propio.</strong></p>
...
<span class="hljs-comment"># }    </span>
<button class="copy-code-btn"></button></code></pre></li>
<li><p>Cree un rol IAM y asócielo con una cuenta de servicio Kubernetes. Sustituya <code translate="no">your-account-id</code> por el ID de su cuenta.</p></li>
</ul>
...
<h2 id="Verify-the-role-and-service-account-setup" class="common-anchor-header">Verifique la configuración de la función y la cuenta de servicio<button data-href="#Verify-the-role-and-service-account-setup" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>Consulte <a href="https://docs.aws.amazon.com/eks/latest/userguide/associate-service-account-role.html">Roles de</a> IAM.</p>
<ul>
<li>Confirme que la política de confianza del rol IAM está configurada correctamente.</li>
</ul>
...
<button class="copy-code-btn"></button></code></pre>
<ul>
<li>Confirme que la política que adjuntó a su rol en un paso anterior está adjunta al rol.</li>
</ul>
...
<button class="copy-code-btn"></button></code></pre>
<ul>
<li>Vea la versión predeterminada de la política.</li>
</ul>
...
<button class="copy-code-btn"></button></code></pre>
<ul>
<li>Vea el contenido de la política para asegurarse de que incluye todos los permisos que necesita su Pod. Si es necesario, sustituya 1 en el siguiente comando por la versión que se devuelve en la salida anterior.</li>
</ul>
...
<button class="copy-code-btn"></button></code></pre>
<ul>
<li>Confirme que la cuenta de servicio Kubernetes está anotada con el rol.</li>
</ul>
...
<ul>
<li>Añada el repositorio de Milvus Helm Chart.</li>
</ul>
<pre><code translate="no" class="language-bash">helm repo <span class="hljs-keyword">add</span> milvus https:<span class="hljs-comment">//zilliztech.github.io/milvus-helm/</span>
helm repo update
<button class="copy-code-btn"></button></code></pre>
<ul>
<li>Prepare el archivo de configuración de Milvus <code translate="no">milvus.yaml</code>, y sustituya <code translate="no">&lt;bucket-name&gt;</code> por el nombre del cubo creado anteriormente.</li>
</ul>
...
<button class="copy-code-btn"></button></code></pre>
<ul>
<li>Instale Milvus.</li>
</ul>
<pre><code translate="no" class="language-bash">helm upgrade --install milvus-demo milvus/milvus -n milvus -f milvus.yaml
<button class="copy-code-btn"></button></code></pre>
<h2 id="Verify-the-installation" class="common-anchor-header">Verifique la instalación<button data-href="#Verify-the-installation" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>Consulte <a href="https://milvus.io/docs/eks.md#Verify-the-installation">Verificar la instalación</a>.</p>
