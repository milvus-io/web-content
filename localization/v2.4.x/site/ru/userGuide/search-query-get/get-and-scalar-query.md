...
<h2 id="Reference-on-scalar-filters" class="common-anchor-header">Справочник по скалярным фильтрам<button data-href="#Reference-on-scalar-filters" class="anchor-icon" translate="no">
    ...
    </button></h2><h3 id="Basic-Operators" class="common-anchor-header">Основные операторы</h3><p><strong>Булево выражение</strong> - это всегда <strong>строка, состоящая из имен полей, объединенных операторами</strong>. В этом разделе вы узнаете больше об основных операторах.</p>
<table>
<thead>
...
<tr><td><strong>&lt;, &gt;</strong></td><td>Меньше, больше</td></tr>
...
<tr><td><strong>подобно</strong></td><td>Сравнивает значение с аналогичными значениями с помощью операторов подстановки.<br/> Например, like &quot;prefix%&quot; сопоставляет строки, начинающиеся с &quot;prefix&quot;.</td></tr>
<tr><td><strong>in</strong></td><td>Проверяет, совпадает ли выражение с любым значением в списке значений.</td></tr>
</tbody>
</table>
<h3 id="Advanced-operators" class="common-anchor-header">Дополнительные операторы</h3><ul>
<li><p><code translate="no">count(*)</code></p>
<p>Подсчитывает точное количество сущностей в коллекции. Используйте это поле в качестве поля вывода для получения точного количества сущностей в коллекции или разделе.</p>
