...
<p>Milvus는 데이터 분리에서 파티션을 재사용하여 컬렉션에 생성할 수 있는 파티션 수의 제한을 극복할 수 있도록 파티션 키를 도입했습니다. 컬렉션을 만들 때 스칼라 필드를 파티션 키로 사용할 수 있습니다. 컬렉션이 준비되면 Milvus는 컬렉션 내에 지정된 수의 파티션을 생성하며, 각 파티션은 파티션 키의 값 범위에 해당합니다. 삽입된 엔티티를 받으면 Milvus는 파티션 키 값에 따라 엔티티를 다른 파티션에 저장합니다.</p>
<p>
