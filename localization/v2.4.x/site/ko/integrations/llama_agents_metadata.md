...
title: '미스트랄 AI, 밀버스 및 라마 에이전트가 포함된 멀티 에이전트 시스템'
---
...
<h2 id="Get-your-API-Key-for-Mistral" class="common-anchor-header">Mistral용 API 키 받기<button data-href="#Get-your-API-Key-for-Mistral" class="anchor-icon" translate="no">
    ...
    </button></h2><p><a href="https://console.mistral.ai/api-keys/">미스트랄 클라우드 콘솔에서 미스트랄</a> API 키를 받을 수 있습니다.</p>
...
<p>아래에 정의된 <code translate="no">QueryEngineTool</code> 은 위에 정의된 것보다 더 일반적입니다. 위의 경우에는 회사별(Uber 및 Lyft) 도구가 있었지만 여기서는 더 일반적입니다. 메타데이터 필터링을 추가하면 특정 문서에서 데이터를 가져오는 것만 필터링할 수 있습니다.</p>
...
<h2 id="Function-Calling" class="common-anchor-header">함수 호출<button data-href="#Function-Calling" class="anchor-icon" translate="no">
    ...
    </button></h2><p>미스트랄 네모와 라지는 기본 함수 호출을 지원합니다. llm의 <code translate="no">predict_and_call</code> 함수를 통해 라마 인덱스 도구와 원활하게 통합됩니다. 이를 통해 사용자는 어떤 도구든 첨부할 수 있으며, 호출할 도구가 있는 경우 LLM이 이를 결정하도록 할 수 있습니다.</p>
