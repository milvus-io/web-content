...
<h2 id="Overview​" class="common-anchor-header">Vue d'ensemble<button data-href="#Overview​" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Lorsque vous devez mettre à jour une entité dans une collection ou que vous n'êtes pas sûr de devoir effectuer une mise à jour ou une insertion, vous pouvez essayer d'utiliser l'opération Upsert. Lors de l'utilisation de cette opération, il est essentiel de s'assurer que l'entité incluse dans la requête Upsert contient la clé primaire ; dans le cas contraire, une erreur se produira. Lorsqu'il reçoit une requête Upsert, Milvus exécute le processus suivant.</p>
<ol>
<li><p>Vérifier si le champ primaire de la collection a l'option AutoId activée.</p>
<p>a.  Si c'est le cas, Milvus remplace la clé primaire de l'entité par une clé primaire générée automatiquement et insère les données.</p>
<p>b. Si ce n'est pas le cas, Milvus utilise la clé primaire de l'entité pour insérer les données.</p></li>
<li><p>Effectuer une opération de suppression basée sur la valeur de la clé primaire de l'entité incluse dans la requête Upsert.</p></li>
</ol>
<p>
   ...
   </span> <span class="img-wrapper"> <span>Entités Upsert</span> </span></p>
<h2 id="Upsert-Entity-in-a-Collection​" class="common-anchor-header">Insertion d'une entité dans une collection<button data-href="#Upsert-Entity-in-a-Collection​" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>Dans cette section, vous allez insérer des entités dans une collection créée de <a href="/docs/fr/create-collection-instantly.md#Quick-Setup">manière rapide</a>. Une collection créée de cette manière n'a que deux champs, nommés <strong>id</strong> et <strong>vector</strong>. En outre, cette collection a activé le champ dynamique, de sorte que les entités dans le code d'exemple incluent un champ appelé <strong>color</strong> qui n'est pas défini dans le schéma.</p>
