...
<p>La fonction <strong>Partition Key</strong> est une optimisation de la recherche basée sur les partitions et permet à Zilliz Cloud de distribuer des entités dans différentes partitions en fonction des valeurs d'un champ scalaire spécifique. Cette fonctionnalité permet de mettre en œuvre une multi-location orientée partition et d'améliorer les performances de recherche.</p>
<p>Cette fonctionnalité ne sera pas abordée dans cette page. Pour en savoir plus, reportez-vous à la section <a href="/docs/fr/use-partition-key.md">Utiliser la clé de partition</a>.</p>
</div>
<h2 id="List-Partitions​" class="common-anchor-header">Lister les partitions<button data-href="#List-Partitions​" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Lors de la création d'une collection, Zilliz Cloud crée également une partition nommée <strong>_default</strong> dans la collection. Vous pouvez lister les partitions d'une collection comme suit.</p>
