...
summary: >-
  ...
  paradigma prático de RAG, os documentos são divididos em vários fragmentos e é
  utilizada uma base de dados vetorial para procurar a consulta, recuperando os
  fragmentos mais relevantes. Em seguida, um LLM responde à consulta usando
  esses pedaços recuperados. No entanto, este processo de fragmentação pode
  resultar na perda de informação contextual, dificultando a determinação da
  relevância pelo recuperador.
...
<h2 id="Preparation" class="common-anchor-header">Preparação<button data-href="#Preparation" class="anchor-icon" translate="no">
    ...
    </button></h2><h3 id="Install-Dependencies" class="common-anchor-header">Instalar dependências</h3><pre><code translate="no" class="language-shell">$ pip install <span class="hljs-string">&quot;pymilvus[model]&quot;</span>
$ pip install tqdm
