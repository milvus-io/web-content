...
<p>Para o <code translate="no">MILVUS_TOKEN</code>, se não tiver definido um token para o seu servidor Milvus, pode defini-lo como uma cadeia vazia, como <code translate="no">MILVUS_TOKEN=</code>, caso contrário, deve defini-lo como o seu token Milvus. Para mais informações sobre como definir o token no Milvus, pode consultar a <a href="https://milvus.io/docs/authenticate.md?tab=docker#Update-user-password">página de autenticação</a>.</p>
