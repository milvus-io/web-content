...
<h2 id="Use-within-a-chain" class="common-anchor-header">Utilização numa cadeia<button data-href="#Use-within-a-chain" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Inicializar o ChatOpenAI e definir um modelo de prompt</p>
...
<h2 id="API-reference" class="common-anchor-header">Referência da API<button data-href="#API-reference" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Para obter documentação pormenorizada de todas as caraterísticas e configurações de <code translate="no">MilvusCollectionHybridSearchRetriever</code>, consulte a <a href="https://api.python.langchain.com/en/latest/retrievers/langchain_milvus.retrievers.milvus_hybrid_search.MilvusCollectionHybridSearchRetriever.html">referência da API</a>.</p>
