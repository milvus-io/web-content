...
<h2 id="Set-Collection-TTL​" class="common-anchor-header">Definir TTL da coleção<button data-href="#Set-Collection-TTL​" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Se uma coleção precisar de ser abandonada durante um período específico, considere definir o seu Time-To-Live (TTL) em segundos. Assim que o TTL se esgotar, o Zilliz Cloud elimina as entidades da coleção e elimina a coleção. A exclusão é assíncrona, indicando que as pesquisas e consultas ainda são possíveis antes que a exclusão seja concluída.</p>
