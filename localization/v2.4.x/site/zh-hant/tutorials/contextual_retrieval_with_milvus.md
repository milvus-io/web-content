...
<h2 id="Experiment-II-Hybrid-Retrieval" class="common-anchor-header">實驗二：混合式擷取<button data-href="#Experiment-II-Hybrid-Retrieval" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<h2 id="Experiment-III-Contextual-Retrieval" class="common-anchor-header">實驗三：內容檢索<button data-href="#Experiment-III-Contextual-Retrieval" class="anchor-icon" translate="no">
    ...
    </button></h2><p>混合式檢索顯示出了改進，但是應用上下文檢索方法可以進一步增強結果。為了達到這個目的，我們會使用 Anthropic 的語言模型，為每個 chunk 預先加入整個文件的上下文。</p>
