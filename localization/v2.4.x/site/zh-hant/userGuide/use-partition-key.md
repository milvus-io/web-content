...
title: 使用分割區金鑰
---
<h1 id="Use-Partition-Key" class="common-anchor-header">使用分割區金鑰<button data-href="#Use-Partition-Key" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h1><p>本指南將教您如何使用分割區金鑰，以加速從您的收藏擷取資料。</p>
<h2 id="Overview" class="common-anchor-header">概述<button data-href="#Overview" class="anchor-icon" translate="no">
    ...
    </button></h2><p>您可以設定資料集中的特定欄位為分區鍵值，這樣 Milvus 就會根據此欄位中各自的分區值，將傳入的實體分佈到不同的分區中。這可將具有相同關鍵值的實體分組在一個分區中，避免在根據關鍵字段進行篩選時掃描不相關的分區，從而加快搜尋效能。與傳統的篩選方法相比，分區鍵可大幅提升查詢效能。</p>
<p>您可以使用分割區金鑰實現多重租用。有關多重租用的詳細資訊，請閱讀<a href="https://milvus.io/docs/multi_tenancy.md">多重租用</a>。</p>
<h2 id="Enable-partition-key" class="common-anchor-header">啟用分割區金鑰<button data-href="#Enable-partition-key" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>若要設定欄位為分割區金鑰，請在建立集合模式時指定<code translate="no">partition_key_field</code> 。</p>
<p>在下面的範例程式碼中，<code translate="no">num_partitions</code> 決定將建立的分割區數量。預設值為<code translate="no">64</code> 。建議您保留預設值。</p>
<div class="language-python">
<p>有關參數的詳細資訊，請參閱 <a href="https://milvus.io/api-reference/pymilvus/v2.4.x/MilvusClient/Client/MilvusClient.md"><code translate="no">MilvusClient</code></a>, <a href="https://milvus.io/api-reference/pymilvus/v2.4.x/MilvusClient/Collections/create_schema.md"><code translate="no">create_schema()</code></a>和 <a href="https://milvus.io/api-reference/pymilvus/v2.4.x/MilvusClient/CollectionSchema/add_field.md"><code translate="no">add_field()</code></a>SDK 參考資料。</p>
</div>
<div class="language-java">
<p>有關參數的詳細資訊，請參閱 <a href="https://milvus.io/api-reference/java/v2.4.x/v2/Client/MilvusClientV2.md"><code translate="no">MilvusClientV2</code></a>, <a href="https://milvus.io/api-reference/java/v2.4.x/v2/Collections/createSchema.md"><code translate="no">createSchema()</code></a>和 <a href="https://milvus.io/api-reference/java/v2.4.x/v2/CollectionSchema/addField.md"><code translate="no">addField()</code></a>在 SDK 參考資料中。</p>
</div>
<div class="language-javascript">
<p>有關參數的詳細資訊，請參閱 SDK 參考資料中的 <a href="https://milvus.io/api-reference/node/v2.4.x/Client/MilvusClient.md"><code translate="no">MilvusClient</code></a>和 <a href="https://milvus.io/api-reference/node/v2.4.x/Collections/createCollection.md"><code translate="no">createCollection()</code></a>SDK 參考資料中。</p>
</div>
...
<h2 id="Insert-data" class="common-anchor-header">插入資料<button data-href="#Insert-data" class="anchor-icon" translate="no">
    ...
    </button></h2><p>集合準備就緒後，開始插入資料，如下所示：</p>
<h3 id="Prepare-data" class="common-anchor-header">準備資料</h3><div class="multipleCode">
   <a href="#python">Python </a> <a href="#java">Java</a> <a href="#javascript">Node.js</a></div>
...
<button class="copy-code-btn"></button></code></pre>
<h3 id="Insert-data" class="common-anchor-header">插入資料</h3><div class="language-python">
...
<span class="hljs-comment">// </span>
<button class="copy-code-btn"></button></code></pre>
<h2 id="Use-partition-key" class="common-anchor-header">使用分割鍵<button data-href="#Use-partition-key" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>索引和載入集合以及插入資料之後，就可以使用分割鍵進行相似性搜尋。</p>
<div class="language-python">
...
<div class="language-javascript">
<p>有關參數的詳細資訊，請參閱 SDK 參考資料中的 <a href="https://milvus.io/api-reference/node/v2.4.x/Vector/search.md"><code translate="no">search()</code></a>在 SDK 參考資料中。</p>
</div>
...
<h2 id="Typical-use-cases" class="common-anchor-header">典型用例<button data-href="#Typical-use-cases" class="anchor-icon" translate="no">
    ...
    </button></h2><p>您可以利用分割區金鑰功能來達到更好的搜尋效能，並啟用多租戶功能。這可透過指定特定於租戶的值作為每個實體的分割區金鑰欄位來實現。在搜尋或查詢集合時，您可以在布林表達式中包含分割區金鑰欄位，藉由特定於租戶的值來篩選實體。此方法可確保租戶的資料隔離，並避免掃描不必要的分割區。</p>
