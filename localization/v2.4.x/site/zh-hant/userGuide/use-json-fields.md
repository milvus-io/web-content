...
<ul>
<li><p>過濾所有以<code translate="no">[&quot;blue&quot;, &quot;brown&quot;, &quot;grey&quot;]</code> 作為參考顏色集的實體。</p>
...
<span class="hljs-comment">// </span>
<button class="copy-code-btn"></button></code></pre></li>
<li><p>篩選坐標包含<code translate="no">4</code> 或<code translate="no">5</code> 的實體。</p>
<p><div class="multipleCode">
<a href="#python">Python </a><a href="#java">Java</a><a href="#javascript">Node.js</a></div></p>
<pre><code translate="no" class="language-python">res = client.query(
    collection_name=<span class="hljs-string">&quot;test_collection&quot;</span>,
...
<h3 id="Basic-Operators-in-JSON-Fields" class="common-anchor-header">JSON 欄位中的基本運算符號</h3><p>下表假設名為<code translate="no">json_key</code> 的 JSON 欄位的值有一個名為<code translate="no">A</code> 的 key 。在使用 JSON 欄位鍵建構布林表達式時，請使用它作為參考。</p>
<table>
<thead>
...
<tr><td><strong>不</strong></td><td><code translate="no">'not json_field[&quot;A&quot;] == 1'</code></td><td>如果<br/> -<code translate="no">json_field</code> 沒有名為<code translate="no">A</code> 的鍵，此表達式的值為真。<br/> -<code translate="no">json_field[&quot;A&quot;]</code> 不等於<code translate="no">1</code>.<br/></td></tr>
...
<h3 id="Advanced-Operators" class="common-anchor-header">進階運算符</h3><p>以下運算符號是 JSON 欄位特有的：</p>
<ul>
...
<p>此操作符可過濾其識別碼包含任何 JSON 表達式成員的實體。</p>
<p>範例：<code translate="no">{&quot;x&quot;: [1,2,3,4,5,7,8]}</code></p>
