...
<p>При выполнении гибридного поиска с помощью <code translate="no">MilvusHybridRetriever</code> мы можем опционально задать параметры topK и reranker. Он будет автоматически обрабатывать векторные вложения и встроенные функции и, наконец, использовать реранкер для уточнения результатов. Основные детали реализации процесса поиска скрыты от пользователя.</p>
