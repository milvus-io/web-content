...
summary: >-
  В этом руководстве показано, как использовать Ragas для оценки конвейера
  Retrieval-Augmented Generation (RAG), построенного на базе Milvus.
title: Оценка с помощью Ragas
---
<h1 id="Evaluation-with-Ragas" class="common-anchor-header">Оценка с помощью Ragas<button data-href="#Evaluation-with-Ragas" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<p>Это руководство демонстрирует, как использовать Ragas для оценки конвейера Retrieval-Augmented Generation (RAG), построенного на базе <a href="https://milvus.io/">Milvus</a>.</p>
...
<p><a href="https://docs.ragas.io/en/latest/index.html#">Ragas</a> - это фреймворк, который помогает оценивать конвейеры RAG. Существуют инструменты и фреймворки, которые помогают создавать такие конвейеры, но оценить их и определить количественную производительность может быть непросто. Именно здесь на помощь приходит Ragas (RAG Assessment).</p>
...
<table border="1" class="dataframe">
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>Язык программирования, используемый для написания Kno...</td>
      <td>Язык программирования, используемый для написания Knowher...</td>
    </tr>
    <tr>
      <th>2</th>
      <td>Что необходимо убедиться перед запуском кода, покрывающег...</td>
      <td>[Покрытие кода\n\nПеред отправкой вашего pull ...</td>
      <td>Прежде чем выполнять покрытие кода, вы должны убедиться, что...</td>
      <td>Перед выполнением покрытия кода необходимо сделать ...</td>
    </tr>
  </tbody>
</table>
</div>
<h2 id="Evaluation-with-Ragas" class="common-anchor-header">Оценка с помощью Ragas<button data-href="#Evaluation-with-Ragas" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>Мы используем Ragas для оценки производительности наших результатов работы конвейера RAG.</p>
<p>Ragas предоставляет набор метрик, которые легко использовать. Мы взяли <code translate="no">Answer relevancy</code>, <code translate="no">Faithfulness</code>, <code translate="no">Context recall</code> и <code translate="no">Context precision</code> в качестве метрик для оценки нашего конвейера RAG. Более подробную информацию о метриках можно найти в разделе <a href="https://docs.ragas.io/en/latest/concepts/metrics/index.html">Метрики Ragas</a>.</p>
