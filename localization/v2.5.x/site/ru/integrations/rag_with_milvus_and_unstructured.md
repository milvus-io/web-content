...
<h2 id="Preparation" class="common-anchor-header">Подготовка<button data-href="#Preparation" class="anchor-icon" translate="no">
    ...
    </button></h2><h3 id="Dependencies-and-Environment" class="common-anchor-header">Зависимости и среда</h3><pre><code translate="no" class="language-python">$ pip install -qU <span class="hljs-string">&quot;unstructured-ingest[pdf]&quot;</span> unstructured pymilvus openai
...
<p>Переменные окружения <code translate="no">UNSTRUCTURED_API_KEY</code> и <code translate="no">UNSTRUCTURED_URL</code> можно получить <a href="https://docs.unstructured.io/api-reference/api-services/saas-api-development-guide">здесь</a>.</p>
...
<h2 id="Load-data-from-Unstructured" class="common-anchor-header">Загрузка данных из Unstructured<button data-href="#Load-data-from-Unstructured" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Unstructured предоставляет гибкий и мощный конвейер для обработки различных типов файлов, включая PDF, HTML и т. д. Мы воспользуемся функцией ingest для разделения PDF-файлов в локальной директории. А затем загрузим данные в Milvus.</p>
