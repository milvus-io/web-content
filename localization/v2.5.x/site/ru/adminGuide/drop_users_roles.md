...
summary: >-
  Для обеспечения безопасности данных рекомендуется удалять пользователей и
  роли, которые больше не используются. В этом руководстве описано, как удалять
  пользователей и роли.
title: Удаление пользователей и ролей
---
<h1 id="Drop-Users--Roles​" class="common-anchor-header">Удаление пользователей и ролей<button data-href="#Drop-Users--Roles​" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h1><p>Для обеспечения безопасности данных рекомендуется удалять пользователей и роли, которые больше не используются. В этом руководстве описано, как удалять пользователей и роли.</p>
<h2 id="Drop-a-user​" class="common-anchor-header">Удаление пользователя<button data-href="#Drop-a-user​" class="anchor-icon" translate="no">
    ...
    </button></h2><p>В следующем примере показано, как удалить пользователя <code translate="no">user_1</code>. </p>
<div class="alert note">
<p>Пользователь <code translate="no">root</code> не может быть удален.</p>
</div>
...
<button class="copy-code-btn"></button></code></pre>
<p>После того как пользователь будет удален, вы можете вывести список всех существующих пользователей, чтобы проверить, успешно ли прошла операция удаления. </p>
<div class="multipleCode">
 <a href="#python">Python </a> <a href="#java">Java</a> <a href="#javascript">Node.js</a> <a href="#shell">cURL</a></div>
...
<button class="copy-code-btn"></button></code></pre>
<p>Ниже приведен пример вывода. В списке нет ни одного <code translate="no">user_1</code>. Операция удаления прошла успешно.</p>
<pre><code translate="no">[<span class="hljs-string">&#x27;root&#x27;</span>]​

<button class="copy-code-btn"></button></code></pre>
<h2 id="Drop-a-role​" class="common-anchor-header">Сброс роли<button data-href="#Drop-a-role​" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>В следующем примере показано, как сбросить роль <code translate="no">role_a</code>.</p>
<div class="alert note">
<p>Встроенная роль <code translate="no">admin</code> не может быть сброшена.</p>
</div>
<div class="multipleCode">
 <a href="#python">Python </a> <a href="#java">Java</a> <a href="#javascript">Node.js</a> <a href="#shell">cURL</a></div>
<pre><code translate="no" class="language-python"><span class="hljs-keyword">from</span> pymilvus <span class="hljs-keyword">import</span> <span class="hljs-title class_">MilvusClient</span>​
...
<button class="copy-code-btn"></button></code></pre>
<p>После того как роль сброшена, вы можете перечислить все существующие роли, чтобы проверить, успешно ли прошла операция сброса. </p>
<div class="multipleCode">
 <a href="#python">Python </a> <a href="#java">Java</a> <a href="#javascript">Node.js</a> <a href="#shell">cURL</a></div>
<pre><code translate="no" class="language-python"><span class="hljs-keyword">from</span> pymilvus <span class="hljs-keyword">import</span> <span class="hljs-title class_">MilvusClient</span>​
​
...
<button class="copy-code-btn"></button></code></pre>
<p>Ниже приведен пример вывода. В списке нет ни одной роли <code translate="no">role_a</code>. Операция сброса прошла успешно.</p>
