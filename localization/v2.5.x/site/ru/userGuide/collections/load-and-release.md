...
<p>Частичная загрузка коллекций в настоящее время находится в бета-версии и не рекомендуется для использования в производстве.</p>
</div>
...
<button class="copy-code-btn"></button></code></pre>
<p>Если вы решили загрузить конкретные поля, стоит отметить, что только поля, включенные в <code translate="no">load_fields</code>, могут быть использованы в качестве фильтров и выходных полей в поиске и запросах. Вы всегда должны включать имена первичного поля и хотя бы одного векторного поля в <code translate="no">load_fields</code>.</p>
