...
title: Включить динамическое поле
summary: >-
...
<p>Динамическое поле - это зарезервированное поле с именем <code translate="no">$meta</code>, которое имеет тип JavaScript Object Notation (JSON). Любые поля сущностей, не определенные в схеме, будут храниться в этом зарезервированном поле JSON в виде пар ключ-значение.</p>
...
<ol>
<li><strong>Выберите ключ динамического поля</strong>, которое вы хотите проиндексировать. Например, <code translate="no">&quot;color&quot;</code> в примере выше.</li>
<li><strong>Определите тип приведения</strong> для значений, найденных по этому ключу. Milvus разберет динамическое поле, извлечет значения по указанному ключу и приведет их к тому типу, который вы зададите.<ul>
<li>Поддерживаются следующие значения <code translate="no">json_cast_type</code>: <code translate="no">bool</code> (или <code translate="no">BOOL</code>), <code translate="no">double</code> (или <code translate="no">DOUBLE</code>) и <code translate="no">varchar</code> (или <code translate="no">VARCHAR</code>).</li>
<li>Если парсинг или приведение не удается (например, попытка разобрать строку как double), такие строки будут пропущены в индексе.</li>
</ul></li>
<li><strong>Укажите путь JSON</strong> к этому ключу как <code translate="no">json_path</code>. Поскольку динамическое поле хранится в формате JSON, вы можете указать что-то вроде <code translate="no">&quot;color&quot;</code>, или, если у вас есть вложенные структуры, вы можете указать более глубокие пути (например, <code translate="no">my_json[&quot;field&quot;][&quot;subfield&quot;]</code>).</li>
<li><strong>Создайте индекс INVERTED</strong>. В настоящее время для индексирования путей JSON поддерживается только тип <code translate="no">INVERTED</code>.</li>
</ol>
<p>Подробнее о параметрах и правилах см. в разделе <a href="/docs/ru/use-json-fields.md">Индексирование поля JSON</a>.</p>
