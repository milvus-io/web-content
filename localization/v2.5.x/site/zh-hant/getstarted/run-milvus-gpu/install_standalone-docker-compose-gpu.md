...
<ul>
<li>若要指定特定的 GPU 裝置給 Milvus，請在<code translate="no">standalone</code> 服務的定義中找到<code translate="no">deploy.resources.reservations.devices[0].devices_ids</code> 欄位，並將其值換成所需 GPU 的 ID。您可以使用 NVIDIA GPU 顯示驅動程式隨附的<code translate="no">nvidia-smi</code> 工具來確定 GPU 裝置的 ID。Milvus 支援多個 GPU 裝置。</li>
</ul>
