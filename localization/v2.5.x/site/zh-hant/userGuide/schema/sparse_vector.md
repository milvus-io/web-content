...
<h2 id="Set-Index-Parameters" class="common-anchor-header">設定索引參數<button data-href="#Set-Index-Parameters" class="anchor-icon" translate="no">
    ...
    </button></h2><p>為稀疏向量建立索引的過程與<a href="/docs/zh-hant/v2.5.x/dense-vector.md">稠密向量</a>相似，但指定的索引類型 (<code translate="no">index_type</code>)、距離度量 (<code translate="no">metric_type</code>) 和索引參數 (<code translate="no">params</code>) 有所不同。</p>
<div class="multipleCode">
   <a href="#python">Python</a> <a href="#java">Java</a> <a href="#javascript">NodeJS</a> <a href="#go">Go</a> <a href="#bash">cURL</a></div>
...
<button class="copy-code-btn"></button></code></pre>
<p>本範例使用<code translate="no">SPARSE_INVERTED_INDEX</code> 索引類型，並以<code translate="no">IP</code> 作為度量。如需詳細資訊，請參閱下列資源：</p>
<ul>
