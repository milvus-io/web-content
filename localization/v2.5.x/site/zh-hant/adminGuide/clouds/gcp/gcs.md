---
id: gcs.md
title: 依據工作負載身分設定 GCS 存取
related_key: 'gcs, storage, workload identity, iam'
summary: 瞭解如何使用 Workload Identity 設定 gcs。
---
<h1 id="Configure-GCS-Access-by-Workload-Identity" class="common-anchor-header">依據工作負載身分設定 GCS 存取<button data-href="#Configure-GCS-Access-by-Workload-Identity" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h1><p>本主題介紹當您使用 helm 安裝 Milvus 時，如何透過 Workload Identity 設定 gcs 存取。 詳細資訊請參閱<a href="https://cloud.google.com/kubernetes-engine/docs/how-to/workload-identity">Workload Identity</a>。</p>
<h2 id="Before-you-start" class="common-anchor-header">開始之前<button data-href="#Before-you-start" class="anchor-icon" translate="no">
    ...
    </button></h2><p>請使用 Google Cloud CLI 或 Google Cloud 主控台在群集和節點池上啟用 Workload Identity。必須先在群集層級啟用 Workload Identity，才能在節點池上啟用 Workload Identity。</p>
<h2 id="Configure-applications-to-use-Workload-Identity" class="common-anchor-header">設定應用程式以使用 Workload Identity<button data-href="#Configure-applications-to-use-Workload-Identity" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<button class="copy-code-btn"></button></code></pre>
<ul>
<li>透過在兩個服務帳戶之間新增 IAM 策略綁定，允許 Kubernetes 服務帳戶冒充 IAM 服務帳戶。此绑定允许 Kubernetes 服务帐户充当 IAM 服务帐户。</li>
</ul>
...
<button class="copy-code-btn"></button></code></pre>
<ul>
<li>使用 IAM 服務帳戶的電子郵件地址註解 Kubernetes 服務帳戶。</li>
</ul>
...
<button class="copy-code-btn"></button></code></pre>
<h2 id="Verify-the-Workload-Identity-setup" class="common-anchor-header">驗證 Workload Identity 設定<button data-href="#Verify-the-Workload-Identity-setup" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>請參閱<a href="https://cloud.google.com/kubernetes-engine/docs/how-to/workload-identity">Workload Identity</a>。在 Pod 內執行下列指令：</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>如果結果是<code translate="no">milvus-gcs-access-sa@milvus-testing-nonprod.iam.gserviceaccount.com</code> ，就OK了。</p>
