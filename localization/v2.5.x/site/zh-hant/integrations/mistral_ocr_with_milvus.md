...
summary: 本教學示範如何使用 Milvus 和 Mistral OCR 建立一個文件理解系統。
title: 使用 Mistral OCR 和 Milvus 理解文件
---
...
<h1 id="Document-Understanding-with-Mistral-OCR-and-Milvus" class="common-anchor-header">使用 Mistral OCR 和 Milvus 理解文件<button data-href="#Document-Understanding-with-Mistral-OCR-and-Milvus" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h1><p>本教學示範如何使用以下工具建立一個文件理解系統：</p>
<h2 id="Mistral-OCR" class="common-anchor-header">Mistral OCR<button data-href="#Mistral-OCR" class="anchor-icon" translate="no">
    ...
    </button></h2><p>一個功能強大的光學字元識別服務，可以</p>
<ul>
<li>處理 PDF、圖像和其他文件格式</li>
<li>保留文件結構和格式</li>
<li>處理多頁文件</li>
<li>識別表格、列表和其他複雜元素</li>
</ul>
...
<li>將文字轉換為數值表示：</li>
<li>將文字轉換為 1024 維向量</li>
<li>捕捉概念之間的語意關係</li>
<li>可根據意義進行相似性比對</li>
<li>提供語意搜尋的基礎</li>
</ul>
<h2 id="Milvus-Vector-Database" class="common-anchor-header">Milvus 向量資料庫<button data-href="#Milvus-Vector-Database" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>向量相似性搜尋的專門資料庫：</p>
<ul>
<li>開放原始碼</li>
<li>執行有效率的向量搜尋</li>
<li>可擴充至大型文件集</li>
<li>支援混合搜尋 (向量相似性 + 元資料篩選)</li>
<li>針對 AI 應用程式最佳化</li>
</ul>
<p>本教學結束時，您將擁有一個可以</p>
<ol>
<li>透過 URL 處理文件（PDF/圖片</li>
<li>使用 OCR 擷取文字</li>
<li>在 Milvus 中儲存文字和向量嵌入</li>
<li>在您的文件集中執行語意搜尋</li>
</ol>
<hr>
<h2 id="Setup-and-Dependencies" class="common-anchor-header">設定與相依性<button data-href="#Setup-and-Dependencies" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<ol>
<li>Mistral API 金鑰 (請至 https://console.mistral.ai/ 取得)</li>
<li>透過<a href="https://milvus.io/docs/install_standalone-docker.md">Docker</a>或<a href="https://zilliz.com">Zilliz Cloud</a>在本機執行 Milvus</li>
</ol>
<p>讓我們來設定環境</p>
...
<h2 id="Setting-Up-Milvus-Collection" class="common-anchor-header">設定 Milvus 套件<button data-href="#Setting-Up-Milvus-Collection" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>現在，讓我們建立一個 Milvus 套件來儲存我們的文件資料。這個集合將有以下欄位：</p>
<ul>
<li><code translate="no">id</code>:主鍵 (自動產生)</li>
<li><code translate="no">url</code>:文件的來源 URL</li>
<li><code translate="no">page_num</code>:文件的頁碼</li>
<li><code translate="no">content</code>:擷取的文字內容</li>
<li><code translate="no">embedding</code>:內容的向量表示 (1024 維度)</li>
</ul>
...
<h2 id="Core-Functionality" class="common-anchor-header">核心功能<button data-href="#Core-Functionality" class="anchor-icon" translate="no">
    ...
    </button></h2><p>讓我們來實現文件理解系統的核心功能：</p>
...
<span class="hljs-keyword">def</span> <span class="hljs-title function_">process_document</span>(<span class="hljs-params">url</span>):
    ...
    <span class="hljs-keyword">return</span> ocr_result
<button class="copy-code-btn"></button></code></pre>
<h2 id="Search-Functionality" class="common-anchor-header">搜尋功能<button data-href="#Search-Functionality" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>現在，讓我們來實作搜尋功能，以擷取相關的文件內容：</p>
...
<button class="copy-code-btn"></button></code></pre>
<h2 id="Demo-Processing-Documents" class="common-anchor-header">示範：處理文件<button data-href="#Demo-Processing-Documents" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>讓我們來處理一些範例文件。您可以用自己的文件取代這些 URL。</p>
...
<h2 id="Demo-Searching-Documents" class="common-anchor-header">示範：搜尋文件<button data-href="#Demo-Searching-Documents" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>現在我們已經處理了一些文件，讓我們來搜尋它們：</p>
...
<h2 id="Conclusion" class="common-anchor-header">結論<button data-href="#Conclusion" class="anchor-icon" translate="no">
    ...
    </button></h2><p>在本教程中，我們使用 Mistral OCR 和 Milvus 建立了一個完整的文件理解系統。這個系統可以</p>
<ol>
<li>從 URL 處理文件</li>
<li>使用 Mistral 的 OCR 功能擷取文字</li>
<li>為內容產生向量嵌入</li>
<li>在 Milvus 中儲存文字和向量</li>
<li>在所有處理過的文件中執行語意搜尋</li>
</ol>
<p>此方法可實現強大的文件理解能力，超越簡單的關鍵字比對，讓使用者可根據意義而非精確的文字比對來尋找資訊。</p>
