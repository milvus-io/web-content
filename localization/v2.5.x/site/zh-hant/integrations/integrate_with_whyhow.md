...
<button class="copy-code-btn"></button></code></pre>
<h3 id="Add-schema" class="common-anchor-header">添加模式</h3><p>在插入任何資料到 Milvus Lite 資料庫之前，我們需要先定義資料欄位，在這裡稱為 schema。透過建立物件<code translate="no">CollectionSchema</code> 和新增資料欄位<code translate="no">add_field()</code> ，我們可以控制資料類型和它們的特性。在插入任何資料到 Milvus 之前，這個步驟是必須的。</p>
