...
<p>리소스 그룹의 구성이 변경되면 Milvus는 새 구성에 따라 현재 쿼리 노드 리소스를 최대한 조정하여 모든 리소스 그룹이 결국 다음 조건을 충족하도록 합니다:</p>
...
<p>이 페이지의 모든 코드 샘플은 PyMilvus 2.5.16 버전입니다. 실행하기 전에 PyMilvus 설치를 업그레이드하세요.</p>
</div>
<ol>
...
<h2 id="A-good-practice-to-manage-cluster-scaling" class="common-anchor-header">클러스터 확장을 관리하는 좋은 방법<button data-href="#A-good-practice-to-manage-cluster-scaling" class="anchor-icon" translate="no">
    ...
    </button></h2><p>현재 Milvus는 클라우드 네이티브 환경에서 독립적으로 스케일 인/아웃이 불가능합니다. 그러나 컨테이너 오케스트레이션과 함께 <strong>선언적 리소스 그룹 API를</strong> 사용하면 Milvus는 손쉽게 쿼리 노드에 대한 리소스 격리 및 관리를 수행할 수 있습니다. 다음은 클라우드 환경에서 쿼리 노드를 관리하는 좋은 예시입니다:</p>
<ol>
