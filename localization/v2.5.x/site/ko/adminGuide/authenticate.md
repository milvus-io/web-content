...
<h2 id="Connect-to-Milvus-with-authentication" class="common-anchor-header">인증을 사용하여 Milvus에 연결하기<button data-href="#Connect-to-Milvus-with-authentication" class="anchor-icon" translate="no">
    ...
    </button></h2><p>인증을 활성화한 후 사용자 이름과 비밀번호를 사용하여 Milvus에 연결해야 합니다. 기본적으로 Milvus가 시작될 때 <code translate="no">root</code> 사용자는 <code translate="no">Milvus</code> 비밀번호로 생성됩니다. 다음은 기본 <code translate="no">root</code> 사용자를 사용하여 인증을 활성화한 상태에서 Milvus에 연결하는 방법의 예시입니다:</p>
