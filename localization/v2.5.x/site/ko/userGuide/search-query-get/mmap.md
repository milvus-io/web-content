...
<h1 id="Use-mmap" class="common-anchor-header">mmap 사용<button data-href="#Use-mmap" class="anchor-icon" translate="no">
    ...
    </button></h1><p>메모리 매핑(Mmap)을 사용하면 디스크의 대용량 파일에 직접 메모리 액세스를 할 수 있으므로 Milvus는 인덱스와 데이터를 메모리와 하드 드라이브 모두에 저장할 수 있습니다. 이 접근 방식은 액세스 빈도에 따라 데이터 배치 정책을 최적화하여 검색 성능에 큰 영향을 주지 않으면서 컬렉션의 저장 용량을 확장하는 데 도움이 됩니다. 이 페이지에서는 Milvus가 mmap을 사용하여 빠르고 효율적인 데이터 저장 및 검색을 가능하게 하는 방법을 설명합니다.</p>
...
<p>이러한 문제를 해결하기 위해 Milvus는 컬렉션에서 핫 데이터와 콜드 데이터의 로딩 균형을 맞추기 위해 mmap을 도입했습니다. 위의 오른쪽 그림에서 볼 수 있듯이, 특정 필드에 있는 원시 데이터를 메모리에 완전히 로드하는 대신 메모리 맵핑하도록 Milvus를 구성할 수 있습니다. 이렇게 하면 메모리 문제에 대한 걱정 없이 필드에 직접 메모리 액세스를 확보하고 컬렉션의 용량을 확장할 수 있습니다.</p>
...
<p>위의 설정을 밀버스 클러스터에 적용하려면 <a href="/docs/ko/v2.5.x/configure-helm.md#Configure-Milvus-via-configuration-file">헬름으로 밀버스 구성하기</a> 및 <a href="/docs/ko/v2.5.x/configure_operator.md">밀버스 오퍼레이터로 밀버스 구성하기의</a> 단계를 따르세요.</p>
...
<h3 id="Index-specific-mmap-settings" class="common-anchor-header">인덱스별 메모리 맵 설정<button data-href="#Index-specific-mmap-settings" class="anchor-icon" translate="no">
    ...
    </button></h3><p>인덱스별 mmap을 구성하려면 인덱스를 추가할 때 인덱스 매개변수에 <code translate="no">mmap.enabled</code> 속성을 포함해야 합니다. 속성을 <code translate="no">true</code> 으로 설정하여 이 특정 인덱스에서 mmap을 활성화할 수 있습니다.</p>
