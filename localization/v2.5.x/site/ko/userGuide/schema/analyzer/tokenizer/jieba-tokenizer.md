...
<h2 id="Configuration" class="common-anchor-header">구성<button data-href="#Configuration" class="anchor-icon" translate="no">
    ...
    </button></h2><p><code translate="no">jieba</code> 토큰화기를 사용하여 분석기를 구성하려면 <code translate="no">analyzer_params</code> 에서 <code translate="no">tokenizer</code> 을 <code translate="no">jieba</code> 로 설정하세요.</p>
...
<h2 id="Example-output" class="common-anchor-header">예제 출력<button data-href="#Example-output" class="anchor-icon" translate="no">
    ...
    </button></h2><p>다음은 <code translate="no">jieba</code> 토큰화기가 텍스트를 처리하는 방법의 예시입니다:</p>
