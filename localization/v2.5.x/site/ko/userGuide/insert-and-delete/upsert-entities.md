...
<h1 id="Upsert-Entities​" class="common-anchor-header">엔티티 삽입<button data-href="#Upsert-Entities​" class="anchor-icon" translate="no">
    ...
    </button></h1><p>업서트 작업은 데이터 업데이트와 삽입 작업을 결합합니다. Milvus는 기본 키가 존재하는지 확인하여 업데이트 작업을 수행할지 삽입 작업을 수행할지 결정합니다. 이 섹션에서는 엔티티를 업서트하는 방법과 다양한 시나리오에서 업서트 작업의 구체적인 동작을 소개합니다.</p>
<h2 id="Overview​" class="common-anchor-header">개요<button data-href="#Overview​" class="anchor-icon" translate="no">
    ...
    </button></h2><p>컬렉션의 엔티티를 업데이트해야 하거나 업데이트할지 삽입할지 확실하지 않은 경우 Upsert 작업을 사용해 볼 수 있습니다. 이 작업을 사용할 때는 Upsert 요청에 포함된 엔티티에 기본 키가 포함되어 있는지 확인해야 하며, 그렇지 않으면 오류가 발생합니다. Upsert 요청을 받으면 Milvus는 다음 프로세스를 실행합니다.</p>
<ol>
<li><p>컬렉션의 기본 필드에 AutoId가 활성화되어 있는지 확인합니다.</p>
<p>a.  활성화된 경우, Milvus는 엔티티의 기본 키를 자동으로 생성된 기본 키로 대체하고 데이터를 삽입합니다.</p>
<p>b. 그렇지 않은 경우, Milvus는 엔티티가 가지고 있는 기본 키를 사용하여 데이터를 삽입합니다.</p></li>
...
<p>
   ...
   </span> <span class="img-wrapper"> <span>엔티티 삽입</span> </span></p>
<h2 id="Upsert-Entity-in-a-Collection​" class="common-anchor-header">컬렉션의 엔티티 삽입<button data-href="#Upsert-Entity-in-a-Collection​" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
