...
<p><a href="https://milvus.io/docs">Milvus</a>是一个开源向量数据库，用于支持 Embeddings 相似性搜索和人工智能应用。Milvus 使非结构化数据搜索更易于访问，无论部署环境如何，都能提供一致的用户体验。</p>
</blockquote>
<p>这将帮助你开始使用 Milvus 混合搜索<a href="/docs/concepts/#retrievers">检索器</a>，它结合了密集向量搜索和稀疏向量搜索的优势。有关所有<code translate="no">MilvusCollectionHybridSearchRetriever</code> 功能和配置的详细文档，请访问<a href="https://api.python.langchain.com/en/latest/retrievers/langchain_milvus.retrievers.milvus_hybrid_search.MilvusCollectionHybridSearchRetriever.html">API 参考</a>。</p>
...
<button class="copy-code-btn"></button></code></pre>
<h3 id="Prepare-OpenAI-API-Key" class="common-anchor-header">准备 OpenAI API 密钥</h3><p>请参考 OpenAI<a href="https://platform.openai.com/account/api-keys">文档</a>获取 OpenAI API 密钥，并将其设置为环境变量。</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>我们将使用<a href="https://platform.openai.com/docs/guides/embeddings">OpenAI Embedding</a>生成稠密向量，使用<a href="https://en.wikipedia.org/wiki/Okapi_BM25">BM25 算法</a>生成稀疏向量。</p>
<p>初始化密集嵌入函数并获取维度</p>
...
<h3 id="Create-Milvus-Collection-and-load-data" class="common-anchor-header">创建 Milvus Collections 并加载数据</h3><p>初始化连接 URI 并建立连接</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>使用定义的 Schema 创建 Collections</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>将实体插入 Collections 并加载 Collections</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>在这个 Retriever 的输入参数中，我们使用密集嵌入和稀疏嵌入对这个 Collections 的两个字段进行混合搜索，并使用 WeightedRanker 进行重排。最后，将返回 3 个排名前 K 的文档。</p>
