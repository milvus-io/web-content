...
id: boolean.md
summary: >-
  过滤表达式可用于在搜索或查询过程中过滤特定的标量字段，以获得精确匹配的结果。本指南将通过一个示例数据集介绍如何在 Zilliz
  中使用过滤表达式。出于演示目的，本指南仅提供查询操作符的示例。
title: 元数据过滤
---
...
<p>To use the <code translate="no">TEXT_MATCH</code> filter expression, you must enable text matching for the target <code translate="no">VARCHAR</code> field when creating the collection. For details, refer to <a href="/docs/zh/v2.5.x/keyword-match.md">Text Match</a>.​</p>
<p></div></p></li>
</ul>
