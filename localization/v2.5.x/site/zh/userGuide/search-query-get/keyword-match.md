...
<p>
   ...
   </span> <span class="img-wrapper"> <span>文本匹配</span> </span></p>
...
<button class="copy-code-btn"></button></code></pre>
<h3 id="Optional-Configure-an-analyzer​" class="common-anchor-header">可选：配置分析器</h3><p>文本匹配的性能和准确性取决于所选的分析器。不同的分析器适用于不同的语言和文本结构，因此选择正确的分析器会对特定用例的搜索结果产生重大影响。</p>
