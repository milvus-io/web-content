...
<button class="copy-code-btn"></button></code></pre>
<p>上述代码片段中的<code translate="no">dim</code> 参数表示向量字段中要保存的向量嵌入的维度。<code translate="no">FLOAT_VECTOR</code> 值表示向量字段持有 32 位浮点数列表，通常用于表示反比例。除此之外，Zilliz Cloud 还支持以下类型的向量嵌入：</p>
<ul>
