...
<h2 id="Standard-Filtering" class="common-anchor-header">Filtraggio standard<button data-href="#Standard-Filtering" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<p>Come mostrato nel diagramma precedente, la richiesta di ricerca riporta <code translate="no">chunk like % red %</code> come condizione di filtraggio, indicando che Milvus deve condurre la ricerca RNA all'interno di tutte le entità che hanno la parola <code translate="no">red</code> nel campo <code translate="no">chunk</code>. In particolare, Milvus esegue le seguenti operazioni.</p>
<ul>
...
<p>Come illustrato nel diagramma precedente, una ricerca con filtraggio iterativo esegue la ricerca vettoriale in iterazioni. Ogni entità restituita dall'iteratore viene sottoposta a filtraggio scalare e questo processo continua fino a quando non vengono raggiunti i risultati topK specificati.</p>
...
<h2 id="Examples" class="common-anchor-header">Esempi<button data-href="#Examples" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Questa sezione mostra come condurre una ricerca filtrata. I frammenti di codice in questa sezione presuppongono che nella collezione siano già presenti le seguenti entità. Ogni entità ha quattro campi: <strong>id</strong>, <strong>vector</strong>, <strong>color</strong> e <strong>likes</strong>.</p>
...
<h3 id="Search-with-Standard-Filtering" class="common-anchor-header">Ricerca con filtro standard</h3><p>I seguenti frammenti di codice dimostrano una ricerca con filtraggio standard; la richiesta nel frammento di codice seguente contiene una condizione di filtraggio e diversi campi di output.</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>La condizione di filtraggio contenuta nella richiesta di ricerca legge <code translate="no">color like &quot;red%&quot; and likes &gt; 50</code>. Utilizza l'operatore and per includere due condizioni: la prima chiede le entità che hanno un valore che inizia con <code translate="no">red</code> nel campo <code translate="no">color</code> e l'altra chiede le entità che hanno un valore maggiore di <code translate="no">50</code> nel campo <code translate="no">likes</code>. Ci sono solo due entità che soddisfano questi requisiti. Con il top-K impostato su <code translate="no">3</code>, Milvus calcolerà la distanza tra queste due entità e il vettore della query e le restituirà come risultati della ricerca.</p>
<pre><code translate="no" class="language-JSON">[​
    {​
...
<button class="copy-code-btn"></button></code></pre>
<p>Per ulteriori informazioni sugli operatori che si possono usare nel filtraggio dei metadati, consultare <a href="/docs/it/boolean.md">Filtraggio dei metadati</a>.</p>
<h3 id="Search-with-iteraive-filtering" class="common-anchor-header">Ricerca con filtro iterativo</h3><p>Per effettuare una ricerca filtrata con il filtraggio iterativo, si può procedere come segue:</p>
<div class="multipleCode">
   <a href="#python">Python </a> <a href="#java">Java</a> <a href="#javascript">Node.js</a> <a href="#curl">cURL</a></div>
<pre><code translate="no" class="language-python"><span class="hljs-keyword">from</span> pymilvus <span class="hljs-keyword">import</span> MilvusClient​
​
