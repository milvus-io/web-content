...
<h2 id="Enable-dynamic-field​" class="common-anchor-header">Abilitare il campo dinamico<button data-href="#Enable-dynamic-field​" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Le raccolte create con il metodo descritto in <a href="/docs/it/create-collection-instantly.md">Creare immediatamente una raccolta</a> hanno il campo dinamico abilitato per impostazione predefinita. È anche possibile abilitare il campo dinamico manualmente quando si crea una collezione con impostazioni personalizzate.</p>
...
<button class="copy-code-btn"></button></code></pre>
<h3 id="Index-a-scalar-field-in-the-dynamic-field" class="common-anchor-header">Indicizzare un campo scalare nel campo dinamico</h3><p>Quando si attiva un campo dinamico, qualsiasi campo scalare non definito viene memorizzato come coppia chiave-valore in formato JSON. Milvus supporta la creazione di un indice su tali campi scalari indefiniti, costruendo un indice di percorso JSON. Ecco come funziona:</p>
<ol>
<li><strong>Scegliere la chiave del campo dinamico</strong> che si vuole indicizzare. Ad esempio, <code translate="no">&quot;color&quot;</code> nell'esempio precedente.</li>
<li><strong>Decidere un tipo di cast</strong> per i valori trovati in quella chiave. Milvus analizzerà il campo dinamico, estrarrà i valori sotto la chiave specificata e li trasformerà nel tipo configurato.<ul>
<li>I valori <code translate="no">json_cast_type</code> supportati sono <code translate="no">bool</code> (o <code translate="no">BOOL</code>), <code translate="no">double</code> (o <code translate="no">DOUBLE</code>) e <code translate="no">varchar</code> (o <code translate="no">VARCHAR</code>).</li>
<li>Se il parsing o il casting fallisce (ad esempio, se si cerca di analizzare una stringa come double), queste righe saranno saltate nell'indice.</li>
</ul></li>
<li><strong>Specificare il percorso JSON</strong> della chiave come <code translate="no">json_path</code>. Poiché il campo dinamico è memorizzato come JSON, si può specificare qualcosa come <code translate="no">&quot;color&quot;</code>, oppure, se si hanno strutture annidate, si possono specificare percorsi più profondi (per esempio <code translate="no">my_json[&quot;field&quot;][&quot;subfield&quot;]</code>).</li>
<li><strong>Creare un indice INVERTED</strong>. Attualmente, solo il tipo <code translate="no">INVERTED</code> è supportato per l'indicizzazione dei percorsi JSON.</li>
</ol>
<p>Per i dettagli sui parametri e le considerazioni, fare riferimento a <a href="/docs/it/use-json-fields.md">Indicizzare un campo JSON</a>.</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>Nell'espressione di filtro utilizzata nell'esempio di codice precedente, <code translate="no">color like &quot;red%&quot; and likes &gt; 50</code>, le condizioni specificano che il valore del campo <code translate="no">color</code> deve iniziare con <strong>"rosso".</strong> Nei dati di esempio, solo due entità soddisfano questa condizione. Pertanto, quando <code translate="no">limit</code> (topK) è impostato su <code translate="no">3</code> o meno, entrambe le entità saranno restituite.</p>
<pre><code translate="no" class="language-JSON">[​
    {​
