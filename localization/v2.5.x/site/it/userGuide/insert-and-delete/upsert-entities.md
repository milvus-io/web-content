...
<h1 id="Upsert-Entities​" class="common-anchor-header">Entità Upsert<button data-href="#Upsert-Entities​" class="anchor-icon" translate="no">
    ...
    </button></h1><p>L'operazione Upsert combina le azioni di aggiornamento e inserimento dei dati. Milvus determina se eseguire un'operazione di aggiornamento o di inserimento controllando se la chiave primaria esiste. Questa sezione illustra come eseguire l'Upsert di un'entità e i comportamenti specifici dell'operazione Upsert in diversi scenari.</p>
<h2 id="Overview​" class="common-anchor-header">Panoramica<button data-href="#Overview​" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Quando è necessario aggiornare un'entità in una collezione o non si è sicuri se aggiornare o inserire, si può provare a utilizzare l'operazione Upsert. Quando si utilizza questa operazione, è essenziale assicurarsi che l'entità inclusa nella richiesta di Upsert contenga la chiave primaria; in caso contrario, si verificherà un errore. Quando riceve una richiesta di Upsert, Milvus esegue il seguente processo.</p>
<ol>
<li><p>Controllare se il campo primario della collezione ha l'AutoId abilitato.</p>
<p>a.  In caso affermativo, Milvus sostituisce la chiave primaria dell'entità con una chiave primaria generata automaticamente e inserisce i dati.</p>
<p>b. In caso contrario, Milvus utilizzerà la chiave primaria dell'entità per inserire i dati.</p></li>
<li><p>Eseguire un'operazione di cancellazione in base al valore della chiave primaria dell'Entità inclusa nella richiesta di Upsert.</p></li>
</ol>
<p>
   ...
   </span> <span class="img-wrapper"> <span>Entità Upsert</span> </span></p>
<h2 id="Upsert-Entity-in-a-Collection​" class="common-anchor-header">Upsert di entità in una collezione<button data-href="#Upsert-Entity-in-a-Collection​" class="anchor-icon" translate="no">
    ...
    </button></h2><p>In questa sezione, si inseriscono le Entità in una Raccolta creata <a href="/docs/it/create-collection-instantly.md#Quick-Setup">in modo rapido</a>. Una raccolta creata in questo modo ha solo due campi, denominati <strong>id</strong> e <strong>vector</strong>. Inoltre, questa Collection ha il campo dinamico abilitato, quindi le Entità nel codice di esempio includono un campo chiamato <strong>color</strong>, non definito nello Schema.</p>
