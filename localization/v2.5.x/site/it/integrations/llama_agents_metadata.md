...
<ul>
<li><p>1️⃣ Memorizzare i dati in Milvus: imparare a memorizzare i dati in Milvus, un efficiente database vettoriale progettato per ricerche di somiglianza ad alta velocità e applicazioni AI.</p></li>
...
<p>Con Milvus Lite, è possibile iniziare a costruire un'applicazione di IA con ricerca di similarità vettoriale in pochi minuti! Milvus Lite può essere eseguito nei seguenti ambienti:</p>
<ul>
...
<h2 id="Function-Calling" class="common-anchor-header">Chiamata di funzione<button data-href="#Function-Calling" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Mistral Nemo e Large supportano le chiamate di funzione native. Esiste una perfetta integrazione con gli strumenti di LlamaIndex, attraverso la funzione <code translate="no">predict_and_call</code> dell'llm. Ciò consente all'utente di allegare qualsiasi strumento e lasciare che sia l'LLM a decidere quali strumenti chiamare (se ce ne sono).</p>
...
<ol>
<li>Nel primo caso, l'Agente non dovrebbe essere in grado di trovare nulla che corrisponda alla richiesta dell'utente, poiché si tratta di Uber e noi filtriamo solo i documenti relativi a Lyft.</li>
