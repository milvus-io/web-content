...
summary: Milvus用パルサーの設定方法をご紹介します。
---
<h1 id="pulsar-related-Configurations" class="common-anchor-header">pulsar関連コンフィギュレーション<button data-href="#pulsar-related-Configurations" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h1><p>最近の突然変異操作のMilvusログを管理し、ストリーミング・ログを出力し、ログ・パブリッシュ・サブスクライブ・サービスを提供するために使用されるpulsarの関連設定。</p>
<h2 id="pulsaraddress" class="common-anchor-header"><code translate="no">pulsar.address</code><button data-href="#pulsaraddress" class="anchor-icon" translate="no">
  ...
  <tbody>
    <tr>
      <td>
        <li>PulsarサービスのIPアドレス。</li>      
        <li>環境変数PULSAR_ADDRESS</li>      
        <li>pulsar.addressとpulsar.portの組み合わせが、パルサーへの有効なアクセスを生成します。</li>      
        <li>Milvus起動時、Pulsarは環境変数PULSAR_ADDRESSから有効なIPアドレスを優先的に取得します。</li>      
        <li>デフォルト値は、PulsarがMilvusと同じネットワーク上で動作している場合に適用されます。</li>      </td>
      <td>ローカルホスト</td>
    </tr>
  </tbody>
</table>
...
<h2 id="pulsarwebport" class="common-anchor-header"><code translate="no">pulsar.webport</code><button data-href="#pulsarwebport" class="anchor-icon" translate="no">
  ...
  <tbody>
    <tr>
      <td>        Pulsarサービスのウェブ・ポート。プロキシなしで直接接続する場合は8080を使用してください。      </td>
      <td>80</td>
    </tr>
  </tbody>
</table>
<h2 id="pulsarmaxMessageSize" class="common-anchor-header"><code translate="no">pulsar.maxMessageSize</code><button data-href="#pulsarmaxMessageSize" class="anchor-icon" translate="no">
  ...
  <tbody>
    <tr>
      <td>
        <li>Pulsarにおける各メッセージの最大サイズ。単位：バイト。</li>      
        <li>デフォルトでは、Pulsarは1つのメッセージで最大5MBのデータを送信できます。挿入されるデータのサイズがこの値を超えると、プロキシはデータを複数のメッセージに断片化し、正しく送信できるようにします。</li>      
        <li>Pulsarの対応するパラメータに変更がない場合、この設定を増やすとmilvusは失敗し、減らすと何のメリットもありません。</li>      </td>
      <td>5242880</td>
    </tr>
  </tbody>
</table>
<h2 id="pulsartenant" class="common-anchor-header"><code translate="no">pulsar.tenant</code><button data-href="#pulsartenant" class="anchor-icon" translate="no">
  ...
  <tbody>
    <tr>
      <td>
        <li>Pulsarは、テナントに適切な容量を割り当てた上で、特定のテナント向けにプロビジョニングすることができます。</li>      
        <li>複数のMilvusインスタンス間でPulsarインスタンスを共有するには、Milvusインスタンスを起動する前に、これを各Milvusインスタンスのデフォルト・テナントではなくPulsarテナントに変更することができます。ただし、Pulsarのマルチ・テナントを望まない場合は、msgChannel.chanNamePrefix.clusterを別の値に変更することをお勧めします。</li>      </td>
      <td>パブリック</td>
    </tr>
  </tbody>
</table>
<h2 id="pulsarnamespace" class="common-anchor-header"><code translate="no">pulsar.namespace</code><button data-href="#pulsarnamespace" class="anchor-icon" translate="no">
  ...
  <tbody>
    <tr>
      <td>        Pulsar名前空間は、テナント内の管理単位命名法です。      </td>
      <td>デフォルト</td>
    </tr>
  </tbody>
</table>
<h2 id="pulsarrequestTimeout" class="common-anchor-header"><code translate="no">pulsar.requestTimeout</code><button data-href="#pulsarrequestTimeout" class="anchor-icon" translate="no">
  ...
  <tbody>
    <tr>
      <td>        pulsarクライアント・グローバル・リクエスト・タイムアウト（秒      </td>
      <td>60</td>
    </tr>
  </tbody>
</table>
<h2 id="pulsarenableClientMetrics" class="common-anchor-header"><code translate="no">pulsar.enableClientMetrics</code><button data-href="#pulsarenableClientMetrics" class="anchor-icon" translate="no">
  ...
  <tbody>
    <tr>
      <td>        pulsarクライアント・メトリクスをmilvusメトリクス・パスへ登録するかどうか。      </td>
      <td>false</td>
    </tr>
  </tbody>
</table>
