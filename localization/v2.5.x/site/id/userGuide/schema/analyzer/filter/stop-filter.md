...
<h1 id="Stop" class="common-anchor-header">Berhenti<button data-href="#Stop" class="anchor-icon" translate="no">
    ...
    </button></h1><p>Filter <code translate="no">stop</code> menghapus kata henti yang ditentukan dari teks yang diberi tanda, sehingga membantu menghilangkan kata-kata yang umum dan kurang bermakna. Anda dapat mengonfigurasi daftar kata henti menggunakan parameter <code translate="no">stop_words</code>.</p>
...
<table>
   ...
   <tr>
     ...
     <td><p>Daftar kata yang akan dihapus dari tokenisasi. Secara default, daftar <code translate="no">_english_</code> yang sudah ditentukan sebelumnya, yang berisi kata-kata penghenti dalam bahasa Inggris, digunakan. Rincian <code translate="no">_english_</code> dapat ditemukan <a href="https://github.com/milvus-io/milvus/blob/master/internal/core/thirdparty/tantivy/tantivy-binding/src/stop_words.rs">di sini</a>.</p></td>
   </tr>
</table>
<p>Filter <code translate="no">stop</code> beroperasi pada istilah yang dihasilkan oleh tokenizer, sehingga harus digunakan bersama dengan tokenizer. Untuk daftar tokenizer yang tersedia di Milvus, lihat <a href="/docs/id/tokenizers">Referensi Tokenizer</a>.</p>
