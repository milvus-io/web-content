...
<h1 id="JSON-Field​" class="common-anchor-header">Bidang JSON<button data-href="#JSON-Field​" class="anchor-icon" translate="no">
    ...
    </button></h1><p>Bidang <a href="https://en.wikipedia.org/wiki/JSON">JSON</a> adalah bidang skalar yang menyimpan informasi tambahan bersama dengan penyematan vektor, dalam pasangan kunci-nilai. Berikut adalah contoh bagaimana data disimpan dalam format JSON:</p>
...
<li><strong>Ukuran Bidang</strong>: Bidang JSON dibatasi hingga 65.536 byte.</li>
<li><strong>Kamus Bersarang</strong>: Setiap kamus bersarang dalam nilai bidang JSON diperlakukan sebagai string biasa untuk penyimpanan.</li>
<li><strong>Nilai Default</strong>: Bidang JSON tidak mendukung nilai default. Namun, Anda dapat menyetel atribut <code translate="no">nullable</code> ke <code translate="no">True</code> untuk mengizinkan nilai null. Untuk detailnya, lihat <a href="/docs/id/nullable-and-default.md">Nullable &amp; Default</a>.</li>
<li><strong>Pencocokan Jenis</strong>: Jika nilai kunci bidang JSON adalah bilangan bulat atau float, nilai tersebut hanya dapat dibandingkan (melalui filter ekspresi) dengan kunci numerik lain dengan jenis yang sama.</li>
<li><strong>Penamaan</strong>: Ketika menamai kunci JSON, disarankan untuk hanya menggunakan huruf, angka, dan garis bawah. Menggunakan karakter lain dapat menyebabkan masalah ketika melakukan penyaringan atau pencarian.</li>
<li><strong>Penanganan String</strong>: Milvus menyimpan nilai string dalam bidang JSON seperti yang dimasukkan, tanpa konversi semantik. Sebagai contoh<ul>
<li><code translate="no">'a&quot;b'</code>, <code translate="no">&quot;a'b&quot;</code>, <code translate="no">'a\\'b'</code>, dan <code translate="no">&quot;a\\&quot;b&quot;</code> disimpan persis seperti apa adanya.</li>
<li><code translate="no">'a'b'</code> dan <code translate="no">&quot;a&quot;b&quot;</code> dianggap tidak valid.</li>
</ul></li>
<li><strong>Pengindeksan JSON</strong>: Saat mengindeks bidang JSON, Anda dapat menentukan satu atau beberapa jalur di bidang JSON untuk mempercepat pemfilteran. Setiap jalur tambahan akan meningkatkan biaya pengindeksan, jadi rencanakan strategi pengindeksan Anda dengan hati-hati. Untuk pertimbangan lebih lanjut tentang pengindeksan bidang JSON, lihat <a href="#considerations-on-json-indexing">Pertimbangan pengindeksan JSON</a>.</li>
</ul>
<h2 id="Add-JSON-field​" class="common-anchor-header">Menambahkan bidang JSON<button data-href="#Add-JSON-field​" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Untuk menambahkan bidang JSON <code translate="no">metadata</code> ke skema koleksi Anda, gunakan <code translate="no">DataType.JSON</code>. Contoh di bawah ini mendefinisikan metadata bidang JSON yang mengizinkan nilai nol:</p>
...
<ul>
<li>Tetapkan <code translate="no">enable_dynamic_fields=True</code> jika Anda perlu menyisipkan bidang tambahan yang tidak terdefinisi di masa mendatang.</li>
<li>Gunakan <code translate="no">nullable=True</code> untuk mengizinkan objek JSON yang hilang atau null.</li>
</ul>
</div>
<h2 id="Set-index-params" class="common-anchor-header">Mengatur parameter indeks<button data-href="#Set-index-params" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Pengindeksan membantu Milvus dengan cepat memfilter atau mencari di seluruh volume data yang besar. Di Milvus, pengindeksan adalah:</p>
<ul>
<li>Wajib untuk bidang vektor (untuk menjalankan pencarian kemiripan secara efisien).</li>
<li>Opsional untuk bidang JSON (untuk mempercepat penyaringan skalar pada jalur JSON tertentu).</li>
</ul>
<h3 id="Index-a-JSON-field" class="common-anchor-header">Mengindeks bidang JSON</h3><p>Secara default, bidang JSON tidak diindeks, sehingga kueri filter apa pun (misalnya, <code translate="no">metadata[&quot;price&quot;] &lt; 100</code>) harus memindai semua baris. Jika Anda ingin mempercepat kueri pada jalur tertentu di dalam bidang <code translate="no">metadata</code>, Anda dapat membuat indeks terbalik pada setiap jalur yang Anda pedulikan. Dalam contoh ini, kita akan membuat dua indeks pada jalur yang berbeda di dalam bidang JSON <code translate="no">metadata</code>:</p>
<div class="multipleCode">
   <a href="#python">Python </a> <a href="#java">Java</a> <a href="#javascript">Node.js</a> <a href="#go">Go</a> <a href="#curl">cURL</a></div>
...
<table>
<thead>
<tr><th><strong>Parameter</strong></th><th><strong>Deskripsi</strong></th><th><strong>Contoh Nilai</strong></th></tr>
</thead>
<tbody>
<tr><td><code translate="no">field_name</code></td><td>Nama bidang JSON dalam skema Anda.</td><td><code translate="no">&quot;metadata&quot;</code></td></tr>
<tr><td><code translate="no">index_type</code></td><td>Jenis indeks yang akan dibuat; saat ini hanya <code translate="no">INVERTED</code> yang didukung untuk pengindeksan jalur JSON.</td><td><code translate="no">&quot;INVERTED&quot;</code></td></tr>
<tr><td><code translate="no">index_name</code></td><td>(Opsional) Nama indeks khusus. Tentukan nama yang berbeda jika Anda membuat beberapa indeks pada bidang JSON yang sama.</td><td><code translate="no">&quot;json_index_1&quot;</code></td></tr>
<tr><td><code translate="no">params.json_path</code></td><td>Menentukan jalur JSON mana yang akan diindeks. Anda dapat menargetkan kunci bersarang, posisi larik, atau keduanya (misalnya, <code translate="no">metadata[&quot;product_info&quot;][&quot;category&quot;]</code> atau <code translate="no">metadata[&quot;tags&quot;][0]</code>). Jika jalur hilang atau elemen larik tidak ada untuk baris tertentu, baris tersebut akan dilewati begitu saja saat pengindeksan, dan tidak ada kesalahan yang dilemparkan.</td><td><code translate="no">&quot;metadata[\&quot;product_info\&quot;][\&quot;category\&quot;]&quot;</code></td></tr>
<tr><td><code translate="no">params.json_cast_type</code></td><td>Tipe data yang akan digunakan Milvus untuk meng-cast nilai JSON yang diekstrak ketika membangun indeks. Nilai yang valid: <ul><li> <code translate="no">&quot;bool&quot;</code> atau <code translate="no">&quot;BOOL&quot;</code></li><li><code translate="no">&quot;double&quot;</code> atau <code translate="no">&quot;DOUBLE&quot;</code></li><li> <code translate="no">&quot;varchar&quot;</code> atau <code translate="no">&quot;VARCHAR&quot;</code></li></ul><br><strong>Catatan</strong>: Untuk nilai bilangan bulat, Milvus secara internal menggunakan double untuk indeks. Bilangan bulat besar di atas 2^53 akan kehilangan presisi. Jika cast gagal (karena ketidakcocokan tipe), tidak ada kesalahan yang dilemparkan, dan nilai baris tersebut tidak diindeks.</td><td><code translate="no">&quot;varchar&quot;</code></td></tr>
</tbody>
</table>
<h4 id="Considerations-on-JSON-indexing" class="common-anchor-header">Pertimbangan tentang pengindeksan JSON</h4><ul>
<li><strong>Logika pemfilteran</strong>:<ul>
<li>Jika Anda <strong>membuat indeks tipe ganda</strong> (<code translate="no">json_cast_type=&quot;double&quot;</code>), hanya kondisi filter tipe numerik yang dapat menggunakan indeks tersebut. Jika filter membandingkan indeks ganda dengan kondisi non-numerik, Milvus akan kembali ke pencarian brute force.</li>
<li>Jika Anda <strong>membuat indeks tipe varchar</strong> (<code translate="no">json_cast_type=&quot;varchar&quot;</code>), hanya kondisi filter tipe string yang dapat menggunakan indeks tersebut. Jika tidak, Milvus akan kembali ke pencarian brute force.</li>
<li>Pengindeksan<strong>boolean</strong> berperilaku serupa dengan tipe varchar.</li>
</ul></li>
<li><strong>Ekspresi istilah</strong>:<ul>
...
<li><strong>Ketepatan numerik</strong>:<ul>
...
<li><strong>Integritas data</strong>:<ul>
...
<h3 id="Index-a-vector-field" class="common-anchor-header">Mengindeks bidang vektor</h3><p>Contoh berikut ini membuat indeks pada penyematan bidang vektor, menggunakan tipe indeks AUTOINDEX. Dengan tipe ini, Milvus secara otomatis memilih indeks yang paling sesuai berdasarkan tipe datanya. Anda juga dapat menyesuaikan tipe indeks dan parameter untuk setiap field.</p>
<div class="multipleCode">
   <a href="#python">Python </a> <a href="#java">Java</a> <a href="#javascript">Node.js</a> <a href="#go">Go</a> <a href="#curl">cURL</a></div>
...
<button class="copy-code-btn"></button></code></pre>
<h3 id="Query-with-filter-expressions" class="common-anchor-header">Kueri dengan ekspresi filter</h3><p>Setelah menyisipkan entitas, gunakan metode kueri untuk mengambil entitas yang cocok dengan ekspresi filter yang ditentukan.</p>
<div class="alert note">
<p>Untuk bidang JSON yang mengizinkan nilai null, bidang tersebut akan diperlakukan sebagai null jika seluruh objek JSON tidak ada atau disetel ke None. Untuk informasi lebih lanjut, lihat Bidang JSON dengan Nilai Nol.</p>
</div>
<p>Untuk mengambil entitas yang metadatanya tidak bernilai null:</p>
<div class="multipleCode">
   <a href="#python">Python </a> <a href="#java">Java</a> <a href="#javascript">Node.js</a> <a href="#go">Go</a> <a href="#curl">cURL</a></div>
...
<button class="copy-code-btn"></button></code></pre>
<p>Untuk mengambil entitas di mana <code translate="no">metadata[&quot;product_info&quot;][&quot;category&quot;]</code> adalah <code translate="no">&quot;electronics&quot;</code>:</p>
<div class="multipleCode">
   <a href="#python">Python </a> <a href="#java">Java</a> <a href="#javascript">Node.js</a> <a href="#go">Go</a> <a href="#curl">cURL</a></div>
...
<button class="copy-code-btn"></button></code></pre>
<h3 id="Vector-search-with-JSON-filtering​" class="common-anchor-header">Pencarian vektor dengan pemfilteran JSON</h3><p>Selain pemfilteran bidang skalar dasar, Anda dapat menggabungkan pencarian kemiripan vektor dengan pemfilteran bidang skalar. Sebagai contoh, kode berikut ini menunjukkan cara menambahkan penyaring bidang skalar ke pencarian vektor:</p>
<div class="multipleCode">
   <a href="#python">Python </a> <a href="#java">Java</a> <a href="#javascript">Node.js</a> <a href="#go">Go</a> <a href="#curl">cURL</a></div>
