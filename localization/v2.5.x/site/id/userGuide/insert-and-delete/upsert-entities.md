...
<h1 id="Upsert-Entities​" class="common-anchor-header">Entitas Upsert<button data-href="#Upsert-Entities​" class="anchor-icon" translate="no">
    ...
    </button></h1><p>Operasi Upsert menggabungkan tindakan memperbarui dan menyisipkan data. Milvus menentukan apakah akan melakukan operasi update atau insert dengan memeriksa apakah kunci utama ada. Bagian ini akan memperkenalkan bagaimana cara melakukan Upsert sebuah Entitas dan perilaku spesifik dari operasi Upsert dalam berbagai skenario.</p>
<h2 id="Overview​" class="common-anchor-header">Gambaran Umum<button data-href="#Overview​" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Ketika Anda perlu memperbarui Entitas dalam Koleksi atau tidak yakin apakah akan memperbarui atau menyisipkan, Anda dapat mencoba menggunakan operasi Upsert. Ketika menggunakan operasi ini, sangat penting untuk memastikan bahwa Entitas yang disertakan dalam permintaan Upsert berisi kunci utama; jika tidak, maka akan terjadi kesalahan. Setelah menerima permintaan Upsert, Milvus akan menjalankan proses berikut.</p>
<ol>
<li><p>Periksa apakah bidang utama Koleksi telah diaktifkan AutoId.</p>
<p>a.  Jika ya, Milvus akan mengganti kunci utama di Entitas dengan kunci utama yang dibuat secara otomatis dan menyisipkan data.</p>
<p>b. Jika tidak, Milvus akan menggunakan primary key yang dimiliki oleh Entity untuk memasukkan data.</p></li>
<li><p>Lakukan operasi hapus berdasarkan nilai primary key dari Entitas yang disertakan di dalam permintaan Upsert.</p></li>
</ol>
<p>
   ...
   </span> <span class="img-wrapper"> <span>Menyisipkan Entitas</span> </span></p>
<h2 id="Upsert-Entity-in-a-Collection​" class="common-anchor-header">Menyisipkan Entitas ke dalam Koleksi<button data-href="#Upsert-Entity-in-a-Collection​" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>Pada bagian ini, Anda akan meng-upsert Entitas ke dalam Koleksi yang dibuat <a href="/docs/id/create-collection-instantly.md#Quick-Setup">dengan cara penyiapan cepat</a>. Koleksi yang dibuat dengan cara ini hanya memiliki dua field, yaitu <strong>id</strong> dan <strong>vektor</strong>. Sebagai tambahan, Koleksi ini memiliki field dinamis yang diaktifkan, sehingga Entitas dalam kode contoh menyertakan field bernama <strong>color</strong> yang tidak didefinisikan dalam Skema.</p>
...
<button class="copy-code-btn"></button></code></pre>
<h2 id="Upsert-Entities-in-a-Partition​" class="common-anchor-header">Menyisipkan Entitas ke dalam Partisi<button data-href="#Upsert-Entities-in-a-Partition​" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>Anda juga dapat menyisipkan entitas ke dalam partisi tertentu. Cuplikan kode berikut mengasumsikan bahwa Anda memiliki partisi bernama <strong>PartitionA</strong> dalam koleksi Anda.</p>
<div class="multipleCode">
 <a href="#python">Python </a> <a href="#java">Java</a> <a href="#javascript">Node.js</a> <a href="#go">Go</a> <a href="#curl">cURL</a></div>
