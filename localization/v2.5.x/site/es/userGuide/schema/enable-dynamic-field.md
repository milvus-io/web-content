...
<h2 id="Enable-dynamic-field​" class="common-anchor-header">Activar el campo dinámico<button data-href="#Enable-dynamic-field​" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Las colecciones creadas mediante el método descrito en <a href="/docs/es/create-collection-instantly.md">Crear colección instantáneamente</a> tienen el campo dinámico activado por defecto. También puede habilitar el campo dinámico manualmente al crear una colección con una configuración personalizada.</p>
...
<h3 id="Insert-data​" class="common-anchor-header">Insertar datos</h3><p>El siguiente código muestra cómo insertar este conjunto de datos en la colección.</p>
<div class="multipleCode">
 <a href="#python">Python </a> <a href="#java">Java</a> <a href="#javascript">Node.js</a> <a href="#go">Go</a> <a href="#curl">cURL</a></div>
...
<ol>
<li><strong>Elija la clave del campo dinámico</strong> que desea indexar. Por ejemplo, <code translate="no">&quot;color&quot;</code> en el ejemplo anterior.</li>
<li>Decida <strong>un tipo de reparto</strong> para los valores encontrados en esa clave. Milvus analizará el campo dinámico, extraerá los valores bajo la clave especificada y los convertirá al tipo que usted configure.<ul>
<li>Los valores de <code translate="no">json_cast_type</code> admitidos son <code translate="no">bool</code> (o <code translate="no">BOOL</code>), <code translate="no">double</code> (o <code translate="no">DOUBLE</code>) y <code translate="no">varchar</code> (o <code translate="no">VARCHAR</code>).</li>
<li>Si el análisis sintáctico o la conversión fallan (por ejemplo, al intentar analizar una cadena como double), esas filas se omitirán en el índice.</li>
</ul></li>
<li><strong>Especifique la ruta JSON</strong> a esa clave como <code translate="no">json_path</code>. Dado que el campo dinámico se almacena como JSON, puedes especificar algo como <code translate="no">&quot;color&quot;</code>, o si tienes estructuras anidadas, puedes especificar rutas más profundas (por ejemplo, <code translate="no">my_json[&quot;field&quot;][&quot;subfield&quot;]</code>).</li>
<li>Cree<strong>un índice INVERTIDO</strong>. Actualmente, sólo se admite el tipo <code translate="no">INVERTED</code> para la indexación de rutas JSON.</li>
</ol>
<p>Para más detalles sobre parámetros y consideraciones, consulte <a href="/docs/es/use-json-fields.md">Indexar un campo JSON</a>.</p>
