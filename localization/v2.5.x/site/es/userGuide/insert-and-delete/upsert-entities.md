...
title: Entidades Upsert
summary: >-
  La operación Upsert combina las acciones de actualizar e insertar datos.
  Milvus determina si realizar una operación de actualización o de inserción
  comprobando si la clave primaria existe. Esta sección introducirá cómo Upsert
  una entidad y los comportamientos específicos de la operación Upsert en
  diferentes escenarios.
---
<h1 id="Upsert-Entities" class="common-anchor-header">Entidades Upsert<button data-href="#Upsert-Entities" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h1><p>La operación Upsert combina las acciones de actualizar e insertar datos. Milvus determina si realizar una operación de actualización o de inserción comprobando si la clave primaria existe. Esta sección introducirá cómo Upsert una entidad y los comportamientos específicos de la operación Upsert en diferentes escenarios.</p>
<h2 id="Overview" class="common-anchor-header">Visión General<button data-href="#Overview" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Cuando necesita actualizar una Entidad en una Colección o no está seguro si actualizar o insertar, puede intentar utilizar la operación Upsert. Al utilizar esta operación, es esencial asegurarse de que la Entidad incluida en la solicitud Upsert contiene la clave primaria; de lo contrario, se producirá un error. Al recibir una solicitud Upsert, Milvus ejecutará el siguiente proceso:</p>
<ol>
<li><p>Comprobar si el campo primario de la Colección tiene AutoId habilitado.</p>
<ol>
<li><p>Si lo está, Milvus sustituirá la clave primaria de la Entidad por una clave primaria generada automáticamente e insertará los datos.</p></li>
<li><p>Si no es así, Milvus utilizará la clave primaria que lleva la Entidad para insertar los datos.</p></li>
</ol></li>
<li><p>Realizar una operación de borrado basada en el valor de la clave primaria de la Entidad incluida en la solicitud de Upsert.</p></li>
</ol>
<p>
   ...
   </span> <span class="img-wrapper"> <span>Flujo de Trabajo de Entidades Upsert</span> </span></p>
<h2 id="Upsert-Entity-in-a-Collection" class="common-anchor-header">Upsert Entidad en una Colección<button data-href="#Upsert-Entity-in-a-Collection" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>En esta sección, usted insertará Entidades en una Colección creada de <a href="/docs/es/create-collection-instantly.md#Quick-Setup">manera rápida</a>. Una Colección creada de esta manera tiene sólo dos campos, llamados <strong>id</strong> y <strong>vector</strong>. Además, esta Colección tiene el campo dinámico habilitado, por lo que las Entidades en el código de ejemplo incluyen un campo llamado <strong>color</strong> que no está definido en el Esquema.</p>
...
<span class="hljs-comment"># }</span>
<button class="copy-code-btn"></button></code></pre>
<h2 id="Upsert-Entities-in-a-Partition" class="common-anchor-header">Insertar Entidades en una Partición<button data-href="#Upsert-Entities-in-a-Partition" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
