...
target:
  ...
  milvus2x:
    ...
    password: xxxx
<button class="copy-code-btn"></button></code></pre>
<p>A tabela a seguir descreve os parâmetros no arquivo de configuração de exemplo. Para obter uma lista completa das configurações, consulte <a href="https://github.com/zilliztech/milvus-migration/blob/main/README_1X.md#migrationyaml-reference">Migração do Milvus: Milvus1.x para Milvus 2.x</a>.</p>
<ul>
...
<tr><td><code translate="no">dumper.worker.workMode</code></td><td>O modo operacional da tarefa de migração. Definido para <code translate="no">milvus1x</code> aquando da migração do Milvus 1.x.</td></tr>
