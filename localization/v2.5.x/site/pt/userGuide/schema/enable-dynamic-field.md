...
<span class="hljs-comment"># }</span>
<button class="copy-code-btn"></button></code></pre>
<h3 id="Index-a-scalar-field-in-the-dynamic-field" class="common-anchor-header">Indexar um campo escalar no campo dinâmico</h3><p>Quando ativa um campo dinâmico, quaisquer campos escalares indefinidos são armazenados como pares de valores chave no formato JSON. Milvus suporta a criação de um índice em tal campo escalar indefinido, efetivamente construindo um índice de caminho JSON. Veja como isso funciona:</p>
<ol>
<li><p><strong>Escolha a chave do campo dinâmico</strong> que deseja indexar. Por exemplo, <code translate="no">&quot;color&quot;</code> no exemplo acima.</p></li>
<li><p><strong>Decidir sobre um tipo de elenco</strong> para os valores encontrados nessa chave. O Milvus analisará o campo dinâmico, extrairá os valores sob a chave especificada e converte-os para o tipo que configurar.</p>
<ul>
...
<p>Para obter detalhes sobre parâmetros e considerações, consulte <a href="/docs/pt/v2.5.x/use-json-fields.md#Index-a-JSON-field">Indexar um campo JSON</a>.</p>
