...
<button class="copy-code-btn"></button></code></pre>
<h3 id="Index-a-scalar-field-in-the-dynamic-field" class="common-anchor-header">Indexar um campo escalar no campo dinâmico</h3><p>Quando ativa um campo dinâmico, quaisquer campos escalares indefinidos são armazenados como pares chave-valor no formato JSON. Milvus suporta a criação de um índice em tal campo escalar indefinido, efetivamente construindo um índice de caminho JSON. Veja como isso funciona:</p>
<ol>
<li><strong>Escolha a chave do campo dinâmico</strong> que deseja indexar. Por exemplo, <code translate="no">&quot;color&quot;</code> no exemplo acima.</li>
<li><strong>Decidir sobre um tipo de elenco</strong> para os valores encontrados nessa chave. O Milvus analisará o campo dinâmico, extrairá os valores sob a chave especificada e converte-os para o tipo que configurar.<ul>
<li>Os valores suportados em <code translate="no">json_cast_type</code> são <code translate="no">bool</code> (ou <code translate="no">BOOL</code>), <code translate="no">double</code> (ou <code translate="no">DOUBLE</code>) e <code translate="no">varchar</code> (ou <code translate="no">VARCHAR</code>).</li>
<li>Se a análise ou a conversão falhar (por exemplo, ao tentar analisar uma cadeia de caracteres como double), essas linhas serão ignoradas no índice.</li>
</ul></li>
<li><strong>Especifique o caminho JSON</strong> para essa chave como <code translate="no">json_path</code>. Como o campo dinâmico é armazenado como JSON, pode especificar algo como <code translate="no">&quot;color&quot;</code>, ou se tiver estruturas aninhadas, pode especificar caminhos mais profundos (por exemplo, <code translate="no">my_json[&quot;field&quot;][&quot;subfield&quot;]</code>).</li>
<li><strong>Crie um índice INVERTED</strong>. Atualmente, apenas o tipo <code translate="no">INVERTED</code> é suportado para a indexação do caminho JSON.</li>
</ol>
<p>Para obter detalhes sobre parâmetros e considerações, consulte <a href="/docs/pt/use-json-fields.md">Indexar um campo JSON</a>.</p>
<p>Abaixo está um exemplo de como criar um índice no campo <code translate="no">&quot;color&quot;</code>:</p>
<div class="multipleCode">
 <a href="#python">Python </a> <a href="#java">Java</a> <a href="#javascript">Node.js</a> <a href="#go">Go</a> <a href="#curl">cURL</a></div>
