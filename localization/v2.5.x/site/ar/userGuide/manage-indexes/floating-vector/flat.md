...
<h2 id="Build-index" class="common-anchor-header">بناء الفهرس<button data-href="#Build-index" class="anchor-icon" translate="no">
    ...
    </button></h2><p>لإنشاء فهرس <code translate="no">FLAT</code> على حقل متجه في ميلفوس، استخدم الطريقة <code translate="no">add_index()</code> ، مع تحديد المعلمات <code translate="no">index_type</code> و <code translate="no">metric_type</code> للفهرس.</p>
