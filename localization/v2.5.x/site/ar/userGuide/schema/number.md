...
title: حقل رقمي
summary: >-
  الحقل العددي هو حقل رقمي يخزن القيم الرقمية. يمكن أن تكون هذه القيم أعدادًا
  صحيحة (أعداد صحيحة) أو أرقامًا عشرية (أرقام ذات فاصلة عائمة). تُستخدم عادةً
  لتمثيل الكميات أو القياسات أو أي بيانات تحتاج إلى معالجة رياضية.
---

<h1 id="Number-Field" class="common-anchor-header">حقل رقمي<button data-href="#Number-Field" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h1><p>حقل الأرقام هو حقل رقمي يخزن القيم الرقمية. يمكن أن تكون هذه القيم أعدادًا صحيحة<strong>(أعداد صحيحة</strong>) أو أرقامًا عشرية<strong>(أرقام ذات فاصلة عائمة</strong>). تُستخدم عادةً لتمثيل الكميات أو القياسات أو أي بيانات تحتاج إلى معالجة رياضية.</p>
<p>يصف الجدول أدناه أنواع بيانات حقول الأرقام المتوفرة في ملفوس.</p>
<table>
   <tr>
     <th><p>نوع الحقل</p></th>
     <th><p>الوصف</p></th>
   ...
   <tr>
     ...
     <td><p>نوع منطقي لتخزين <code translate="no">true</code> أو <code translate="no">false</code> ، مناسب لوصف الحالات الثنائية.</p></td>
   </tr>
   <tr>
     <td><p><code translate="no">INT8</code></p></td>
     <td><p>عدد صحيح 8 بت، مناسب لتخزين بيانات الأعداد الصحيحة صغيرة المدى.</p></td>
   </tr>
   <tr>
     <td><p><code translate="no">INT16</code></p></td>
     <td><p>عدد صحيح 16 بت، مناسب لبيانات الأعداد الصحيحة متوسطة المدى.</p></td>
   </tr>
   <tr>
     <td><p><code translate="no">INT32</code></p></td>
     <td><p>عدد صحيح 32 بت، مثالي لتخزين البيانات الصحيحة العامة مثل كميات المنتجات أو معرّفات المستخدمين.</p></td>
   </tr>
   <tr>
     <td><p><code translate="no">INT64</code></p></td>
     <td><p>عدد صحيح 64 بت، مناسب لتخزين البيانات ذات النطاق الكبير مثل الطوابع الزمنية أو المعرفات.</p></td>
   </tr>
   <tr>
     <td><p><code translate="no">FLOAT</code></p></td>
     <td><p>رقم عائم 32 بت، للبيانات التي تتطلب دقة عامة، مثل التصنيفات أو درجة الحرارة.</p></td>
   </tr>
   <tr>
     <td><p><code translate="no">DOUBLE</code></p></td>
     <td><p>رقم ذو فاصلة عائمة مزدوجة الدقة 64 بت، للبيانات عالية الدقة مثل المعلومات المالية أو الحسابات العلمية.</p></td>
   </tr>
</table>
<p>لتعريف حقل رقم، ما عليك سوى تعيين <code translate="no">datatype</code> إلى أحد أنواع البيانات الرقمية المتاحة. على سبيل المثال، <code translate="no">DataType.INT64</code> لحقل عدد صحيح أو <code translate="no">DataType.FLOAT</code> لحقل ذي فاصلة عائمة.</p>
<div class="alert note">
<p>يدعم ميلفوس القيم الفارغة والقيم الافتراضية لحقول الأرقام. لتمكين هذه الميزات، قم بتعيين <code translate="no">nullable</code> إلى <code translate="no">True</code> و <code translate="no">default_value</code> إلى قيمة رقمية. للحصول على التفاصيل، ارجع إلى <a href="/docs/ar/v2.5.x/nullable-and-default.md">Nullable &amp; Default</a>.</p>
</div>
<h2 id="Add-number-field" class="common-anchor-header">إضافة حقل رقمي<button data-href="#Add-number-field" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>لتخزين البيانات الرقمية، قم بتعريف حقل رقمي في مخطط المجموعة. فيما يلي مثال لمخطط مجموعة يحتوي على حقلي أرقام:</p>
<ul>
<li><p><code translate="no">age</code>: يخزن بيانات الأعداد الصحيحة، ويسمح بالقيم الفارغة، وله قيمة افتراضية <code translate="no">18</code>.</p></li>
<li><p><code translate="no">price</code>: يخزن البيانات العائمة، ويسمح بالقيم الفارغة، ولكن ليس له قيمة افتراضية.</p></li>
</ul>
<div class="alert note">
<p>إذا قمت بتعيين <code translate="no">enable_dynamic_fields=True</code> عند تعريف المخطط، يسمح لك Milvus بإدراج الحقول العددية التي لم يتم تعريفها مسبقًا. ومع ذلك، قد يؤدي ذلك إلى زيادة تعقيد الاستعلامات والإدارة، مما قد يؤثر على الأداء. لمزيد من المعلومات، راجع <a href="/docs/ar/v2.5.x/enable-dynamic-field.md">الحقل الديناميكي</a>.</p>
</div>
...
<p>ينشئ المثال التالي فهارس على الحقل المتجه <code translate="no">embedding</code> والحقل القياسي <code translate="no">age</code> ، وكلاهما يستخدم نوع الفهرس <code translate="no">AUTOINDEX</code>. باستخدام هذا النوع، يختار Milvus تلقائيًا الفهرس الأنسب بناءً على نوع البيانات. يمكنك أيضًا تخصيص نوع الفهرس والبارامترات لكل حقل. لمزيد من التفاصيل، راجع <a href="/docs/ar/v2.5.x/index-explained.md">شرح الفهرس</a>.</p>
<div class="multipleCode">
   <a href="#python">بايثون</a> <a href="#java">جافا جافا</a> <a href="#javascript">NodeJS</a> <a href="#go">الذهاب</a> <a href="#bash">cURL</a></div>
...
<h2 id="Create-collection" class="common-anchor-header">إنشاء مجموعة<button data-href="#Create-collection" class="anchor-icon" translate="no">
    ...
    </button></h2><p>بمجرد تعريف المخطط والفهارس، قم بإنشاء مجموعة تتضمن حقول الأرقام.</p>
<div class="multipleCode">
...
<p>لاسترداد الكيانات التي يكون فيها <code translate="no">age</code> أكبر من 30:</p>
<div class="multipleCode">
   <a href="#python">بايثون</a> <a href="#java">جافا جافا</a> <a href="#javascript">NodeJS</a> <a href="#go">Go</a> <a href="#bash">cURL</a></div>
...
<button class="copy-code-btn"></button></code></pre>
<p>لاسترداد الكيانات التي يكون فيها <code translate="no">price</code> لاغية:</p>
<div class="multipleCode">
...
<button class="copy-code-btn"></button></code></pre>
<p>لاسترداد الكيانات التي يكون فيها <code translate="no">age</code> له القيمة <code translate="no">18</code> ، استخدم التعبير التالي أدناه. نظرًا لأن القيمة الافتراضية لـ <code translate="no">age</code> هي <code translate="no">18</code> ، يجب أن تتضمن النتيجة المتوقعة كيانات تم تعيين <code translate="no">age</code> صراحةً إلى <code translate="no">18</code> أو مع تعيين <code translate="no">age</code> إلى لا شيء.</p>
<div class="multipleCode">
   <a href="#python">بايثون</a> <a href="#java">جافا جافا</a> <a href="#javascript">NodeJS</a> <a href="#go">Go</a> <a href="#bash">cURL</a></div>
...
<h2 id="Vector-search-with-filter-expressions" class="common-anchor-header">البحث المتجه مع تعبيرات التصفية<button data-href="#Vector-search-with-filter-expressions" class="anchor-icon" translate="no">
    ...
    </button></h2><p>بالإضافة إلى التصفية الأساسية لحقل الأرقام، يمكنك دمج عمليات البحث عن تشابه المتجهات مع مرشحات حقول الأرقام. على سبيل المثال، توضح الشيفرة التالية كيفية إضافة فلتر حقل رقمي إلى بحث متجه:</p>
<div class="multipleCode">
   <a href="#python">بايثون</a> <a href="#java">جافا جافا</a> <a href="#javascript">NodeJS</a> <a href="#go">Go</a> <a href="#bash">cURL</a></div>
...
<button class="copy-code-btn"></button></code></pre>
<p>في هذا المثال، نحدد أولًا متجه استعلام ونضيف شرط تصفية <code translate="no">25 &lt;= age &lt;= 35</code> أثناء البحث. هذا يضمن ألا تكون نتائج البحث مشابهة لمتجه الاستعلام فحسب، بل أن تكون مطابقة للنطاق العمري المحدد. لمزيد من المعلومات، راجع <a href="/docs/ar/v2.5.x/filtering">التصفية</a>.</p>
