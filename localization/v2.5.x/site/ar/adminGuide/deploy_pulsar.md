...
<h2 id="Configure-Pulsar-with-Docker-Compose" class="common-anchor-header">تكوين بولسار باستخدام Docker Compose<button data-href="#Configure-Pulsar-with-Docker-Compose" class="anchor-icon" translate="no">
    ...
    </button></h2><h3 id="1-Configure-Pulsar" class="common-anchor-header">1. تكوين بولسار<button data-href="#1-Configure-Pulsar" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<h2 id="Configure-Kafka-with-Helm" class="common-anchor-header">تكوين كافكا مع هيلم<button data-href="#Configure-Kafka-with-Helm" class="anchor-icon" translate="no">
    ...
    </button></h2><p>بالنسبة لمجموعات Milvus على K8s، يمكنك تكوين Kafka في نفس الأمر الذي يبدأ تشغيل Milvus. بدلاً من ذلك، يمكنك تهيئة كافكا باستخدام الملف <code translate="no">values.yml</code> على المسار /charts/milvus في مستودع <a href="https://github.com/milvus-io/milvus-helm">milvus-helm</a> قبل بدء تشغيل Milvus.</p>
...
<p>لا ينصح بتغيير مخزن الرسائل. إذا كنت تريد القيام بذلك، قم بإيقاف جميع عمليات DDL، ثم قم باستدعاء FlushAll API لمسح جميع المجموعات، وأخيرًا أوقف Milvus في النهاية قبل أن تقوم بتغيير مخزن الرسائل فعليًا.</p>
</div>
<blockquote>
...
<h2 id="Configure-NATS-with-Helm" class="common-anchor-header">تكوين NATS مع Helm<button data-href="#Configure-NATS-with-Helm" class="anchor-icon" translate="no">
    ...
    </button></h2><p>NATS هو مخزن رسائل تجريبي بديل لـ RocksMQ. للحصول على خطوات مفصلة حول كيفية تكوين ميلفوس مع Helm، راجع <a href="/docs/ar/v2.5.x/configure-helm.md">تكوين ميلفوس مع مخططات Helm</a>. للحصول على تفاصيل حول عناصر التكوين المتعلقة بـ RocksMQ، راجع <a href="/docs/ar/v2.5.x/configure_natsmq.md">التكوينات المتعلقة بـ NATS</a>.</p>
<ul>
