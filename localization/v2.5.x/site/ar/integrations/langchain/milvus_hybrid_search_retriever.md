...
<p><a href="https://milvus.io/docs">Milvus</a> عبارة عن قاعدة بيانات متجهة مفتوحة المصدر مصممة لتشغيل تطبيقات البحث عن التشابه المدمج والذكاء الاصطناعي. يجعل Milvus البحث عن البيانات غير المهيكلة أكثر سهولة، ويوفر تجربة مستخدم متسقة بغض النظر عن بيئة النشر.</p>
</blockquote>
<p>سيساعدك هذا على بدء استخدام <a href="/docs/concepts/#retrievers">مسترجع</a> البحث الهجين Milvus Hybrid Search، الذي يجمع بين نقاط القوة في كل من البحث المتجه الكثيف والمتفرق. للحصول على وثائق مفصلة لجميع ميزات وتكوينات <code translate="no">MilvusCollectionHybridSearchRetriever</code> توجه إلى <a href="https://api.python.langchain.com/en/latest/retrievers/langchain_milvus.retrievers.milvus_hybrid_search.MilvusCollectionHybridSearchRetriever.html">مرجع واجهة برمجة التطبيقات</a>.</p>
<p>راجع أيضًا <a href="https://milvus.io/docs/multi-vector-search.md">مستندات</a> البحث متعدد المتجهات في ميلفوس.</p>
<h3 id="Integration-details" class="common-anchor-header">تفاصيل التكامل</h3><table>
<thead>
<tr><th style="text-align:left">المسترد</th><th style="text-align:left">الاستضافة الذاتية</th><th style="text-align:center">العرض السحابي</th><th style="text-align:center">الحزمة</th></tr>
</thead>
<tbody>
...
<h2 id="Setup" class="common-anchor-header">الإعداد<button data-href="#Setup" class="anchor-icon" translate="no">
    ...
    </button></h2><p>إذا كنت ترغب في الحصول على التتبع الآلي من الاستعلامات الفردية، يمكنك أيضًا تعيين مفتاح واجهة برمجة تطبيقات <a href="https://docs.smith.langchain.com/">لانجسميث</a> من خلال عدم وضع علامة أدناه:</p>
...
<h3 id="Installation" class="common-anchor-header">التثبيت</h3><p>يعيش هذا المسترد في الحزمة <code translate="no">langchain-milvus</code>. يتطلب هذا الدليل التبعيات التالية:</p>
...
<button class="copy-code-btn"></button></code></pre>
<h3 id="Start-the-Milvus-service" class="common-anchor-header">بدء تشغيل خدمة ميلفوس</h3><p>يرجى الرجوع إلى <a href="https://milvus.io/docs/install_standalone-docker.md">وثائق Milvus</a> لبدء تشغيل خدمة Milvus.</p>
<p>بعد بدء تشغيل خدمة Milvus، تحتاج إلى تحديد URI اتصال Milvus الخاص بك.</p>
...
<button class="copy-code-btn"></button></code></pre>
<h3 id="Prepare-OpenAI-API-Key" class="common-anchor-header">إعداد مفتاح OpenAI API Key</h3><p>يرجى الرجوع إلى <a href="https://platform.openai.com/account/api-keys">وثائق OpenAI</a> للحصول على مفتاح OpenAI API الخاص بك، وتعيينه كمتغير بيئة.</p>
...
<button class="copy-code-btn"></button></code></pre>
<h3 id="Prepare-dense-and-sparse-embedding-functions" class="common-anchor-header">إعداد دوال التضمين الكثيفة والمتناثرة</h3><p>دعنا نتخيل 10 أوصاف وهمية للروايات. في الإنتاج الفعلي، قد تكون كمية كبيرة من البيانات النصية.</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>سوف نستخدم دالة التضمين <a href="https://platform.openai.com/docs/guides/embeddings">OpenAI Embedding</a> لتوليد متجهات كثيفة، <a href="https://en.wikipedia.org/wiki/Okapi_BM25">وخوارزمية BM25</a> لتوليد متجهات متفرقة.</p>
<p>تهيئة دالة التضمين الكثيف والحصول على البُعد</p>
...
<p>تهيئة دالة التضمين المتناثر.</p>
<p>لاحظ أن ناتج التضمين المتناثر هو مجموعة من المتجهات المتناثرة، والتي تمثل فهرس ووزن الكلمات الرئيسية للنص المُدخَل.</p>
...
<h3 id="Create-Milvus-Collection-and-load-data" class="common-anchor-header">إنشاء مجموعة ميلفوس وتحميل البيانات</h3><p>تهيئة URI للاتصال وإنشاء اتصال</p>
<pre><code translate="no" class="language-python">connections.connect(uri=CONNECTION_URI)
<button class="copy-code-btn"></button></code></pre>
<p>تحديد أسماء الحقول وأنواع بياناتها</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>إنشاء مجموعة مع المخطط المحدد</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>تحديد فهرس للمتجهات الكثيفة والمتناثرة</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>إدراج الكيانات في المجموعة وتحميل المجموعة</p>
...
collection.load()
<button class="copy-code-btn"></button></code></pre>
<h2 id="Instantiation" class="common-anchor-header">التثبيت<button data-href="#Instantiation" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>يمكننا الآن إنشاء المسترجع الخاص بنا، وتحديد معلمات البحث للحقول المتفرقة والكثيفة:</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>في معلمات الإدخال لهذا المسترد، نستخدم تضمينًا كثيفًا وتضمينًا متناثرًا لإجراء بحث هجين على حقلي هذه المجموعة، ونستخدم أداة الترجيح لإعادة الترتيب. وأخيرًا، سيتم إرجاع 3 مستندات من أعلى K.</p>
...
<h2 id="Use-within-a-chain" class="common-anchor-header">الاستخدام ضمن سلسلة<button data-href="#Use-within-a-chain" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>تهيئة ChatOpenAI وتعريف قالب مطالبة</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>تحديد دالة لتنسيق المستندات</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>تحديد سلسلة باستخدام المسترد والمكونات الأخرى</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>إجراء استعلام باستخدام السلسلة المحددة</p>
...
<h2 id="API-reference" class="common-anchor-header">مرجع واجهة برمجة التطبيقات<button data-href="#API-reference" class="anchor-icon" translate="no">
    ...
    </button></h2><p>للحصول على وثائق مفصلة لجميع ميزات وتكوينات <code translate="no">MilvusCollectionHybridSearchRetriever</code> توجه إلى <a href="https://api.python.langchain.com/en/latest/retrievers/langchain_milvus.retrievers.milvus_hybrid_search.MilvusCollectionHybridSearchRetriever.html">مرجع واجهة برمجة التطبيقات</a>.</p>
