...
<ul>
<li><p>Unterstützte <code translate="no">json_cast_type</code> Werte sind <code translate="no">bool</code> (oder <code translate="no">BOOL</code>), <code translate="no">double</code> (oder <code translate="no">DOUBLE</code>), und <code translate="no">varchar</code> (oder <code translate="no">VARCHAR</code>).</p></li>
<li><p>Wenn das Parsing oder Casting fehlschlägt (z. B. wenn versucht wird, eine Zeichenkette als double zu parsen), werden diese Zeilen im Index übersprungen.</p></li>
</ul></li>
<li><p><strong>Geben Sie den JSON-Pfad</strong> zu diesem Schlüssel als <code translate="no">json_path</code><strong>an</strong>. Da das dynamische Feld als JSON gespeichert wird, können Sie etwas wie <code translate="no">&quot;color&quot;</code> angeben, oder wenn Sie verschachtelte Strukturen haben, können Sie tiefere Pfade angeben (z. B. <code translate="no">my_json[&quot;field&quot;][&quot;subfield&quot;]</code>).</p></li>
<li><p><strong>Erstellen Sie einen INVERTED-Index</strong>. Derzeit wird nur der Typ <code translate="no">INVERTED</code> für die JSON-Pfadindizierung unterstützt.</p></li>
</ol>
<p>Einzelheiten zu Parametern und Überlegungen finden Sie unter <a href="/docs/de/v2.5.x/use-json-fields.md#Index-a-JSON-field">Indizieren eines JSON-Feldes</a>.</p>
<p>Nachfolgend finden Sie ein Beispiel für die Erstellung eines Indexes für das Feld <code translate="no">&quot;color&quot;</code>:</p>
<div class="multipleCode">
