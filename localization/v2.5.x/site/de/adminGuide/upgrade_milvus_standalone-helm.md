...
<div style="display: none;">- [Führen Sie ein rollendes Upgrade durch](#conduct-a-rolling-upgrade) von Milvus v2.2.3 und späteren Versionen auf v2.5.18.</div>
<ul>
<li><p><a href="#Upgrade-Milvus-using-Helm">Führen Sie ein Upgrade von Milvus mit Helm</a> für ein Upgrade von einer Nebenversion vor v2.2.3 auf v2.5.18 durch.</p></li>
<li><p><a href="#Migrate-the-metadata">Migrieren Sie die Metadaten</a> vor dem Upgrade von Milvus v2.1.x auf v2.5.18.</p></li>
</ul>
...
<p>Sobald Sie sichergestellt haben, dass sich alle Einsätze in Ihrer Milvus-Instanz in ihrem normalen Status befinden, können Sie den folgenden Befehl ausführen. Sie können den folgenden Befehl ausführen, um die Milvus-Instanz auf 2.5.18 zu aktualisieren.</p>
...
<h4 id="2-Upgrade-Milvus-from-v21x-to-2518" class="common-anchor-header">2. Upgrade von Milvus von v2.1.x auf 2.5.18</h4><p>Die folgenden Befehle gehen davon aus, dass Sie Milvus von v2.1.4 auf 2.5.18 aktualisieren. Ändern Sie sie auf die Versionen, die Ihren Anforderungen entsprechen.</p>
<ol>
<li><p>Geben Sie den Namen der Milvus-Instanz, die Quell-Milvus-Version und die Ziel-Milvus-Version an.</p>
<pre><code translate="no">./migrate.sh -i my-release -s 2.1.4 -t 2.5.18
...
<pre><code translate="no">./migrate.sh -i my-release -n milvus -s 2.1.4 -t 2.5.18
<button class="copy-code-btn"></button></code></pre></li>
...
<pre><code translate="no">./migrate<span class="hljs-selector-class">.sh</span> -<span class="hljs-selector-tag">i</span> my-release -n milvus -s <span class="hljs-number">2.1</span>.<span class="hljs-number">4</span> -t <span class="hljs-number">2.5</span>.<span class="hljs-number">18</span> -<span class="hljs-attribute">r</span> by-dev
<button class="copy-code-btn"></button></code></pre></li>
...
<pre><code translate="no">./migrate.sh -i my-release -n milvus -s 2.1.4 -t 2.5.18 -r by-dev -w milvusdb/milvus:v2.5.18
<button class="copy-code-btn"></button></code></pre></li>
...
<pre><code translate="no">./migrate.sh -i my-release -n milvus -s 2.1.4 -t 2.5.18 -w milvusdb/milvus:v2.5.18 -d <span class="hljs-literal">true</span>
<button class="copy-code-btn"></button></code></pre></li>
<li><p>Machen Sie einen Rollback und migrieren Sie erneut, wenn die Migration fehlschlägt.</p>
<pre><code translate="no">./migrate.sh -i my-release -n milvus -s 2.1.4 -t 2.5.18 -r by-dev -o rollback -w milvusdb/milvus:v2.1.1
./migrate.sh -i my-release -n milvus -s 2.1.4 -t 2.5.18 -r by-dev -o migrate -w milvusdb/milvus:v2.5.18
<button class="copy-code-btn"></button></code></pre></li>
</ol>
