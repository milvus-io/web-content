...
`ANNOY` 索引类型基于树结构构建，依靠随机投影（random projection）的方式选择超平面并对空间进行分割。因为数据可以保存到文件中并以 mmap 的方式映射到内存，多个进程可以同时共享数据。

相比其他索引类型，`ANNOY` 在多进程场景下具有最好的性能。然而，由于数据是映射到内存中的，内存占用量也较高。
