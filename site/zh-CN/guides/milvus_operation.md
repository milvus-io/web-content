...
1. 创建搜索参数。搜索参数是一个 JSON 字符串，在 Python SDK 中以字典来表示。
   ...
   | `ANNOY`                              | `search_k`: 影响搜索性能。值越大，搜索结果越精确，但搜索时间越长。</br>这个参数取值范围我们不作限制。但你需要明白的是，这个参数正常取值范围应该为[topk, nb]，topk为需要返回的最相似的向量数，nb为当前索引中向量总数。但是当search_k参数取值范围为(-∞,0]的非法值时，我们会给一个默认值：max(n_trees * topk, nb * 0.05)。当给的参数范围是(0, topk)时，返回的结果会不足topk条。 | `{"search_k": 100}`    | (-∞, ∞) |
