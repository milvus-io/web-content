...
1. Prepare search parameters.
   ...
   > Note: For different index types, search parameters also differ. You **must** assign values to all search parameters.
      ...
      | `ANNOY`                 |  `search_k`: Affects the search performance. A larger value will give more accurate results, but will take longer time to return.</br>We don't limit the value range of this parameter. What you need to understand is that the normal value range is [topk, nb], topk means the size of result set and nb means the total number of vectors in the index. When we get an invalid value of this parameter in range (-∞, 0], we will replace it with a default value: max(topk * n_trees, nb * 0.05). If this parameter ranges in (0, topk), it will work normally, but the size of result set will less than topk.   | `{"search_k": 100}`  | (-∞, ∞) |
