...
- **Note**: `.github/workflows/publish-ref-docs.yml` periodically regenerates
  `API_Reference/` from Feishu and opens a PR. If merged, it overwrites manual
  edits to `API_Reference/` — confirm the workflow's enabled state before doing
  long-lived manual SDK doc work.
