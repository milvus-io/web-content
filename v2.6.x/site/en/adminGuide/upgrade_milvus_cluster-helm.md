...
This guide describes how to upgrade your Milvus cluster from v2.5.x to v2.6.8 using Helm Chart.
...
- You **must** upgrade to v2.5.16 or later with `mixCoordinator` enabled before upgrading to v2.6.8.
...
### Step 3: Upgrade to v2.6.8

Once v2.5.16 is running successfully with `mixCoordinator`, upgrade to v2.6.8:

```bash
helm upgrade my-release zilliztech/milvus \
  --set image.all.tag="v2.6.8" \
