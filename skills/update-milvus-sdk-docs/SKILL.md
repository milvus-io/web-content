...
5. **Resolving "latest"** — when the user says "latest" (no explicit version),
   ...
   git -C sdk-tmp/sdks/<sdk-repo> tag --sort=-v:refname | head -1
   ```
...
- **New SDKs are out of scope**: this skill only **updates** existing doc trees
  under `API_Reference/`. It does NOT scaffold a brand-new SDK tree. If the user
  asks to update an SDK with no `API_Reference/<sdk>/` directory (e.g. rust),
  decline and explain that creating the initial tree is a separate task.
