...
For Arabic text, use the built-in [`arabic`](arabic-analyzer.md) analyzer in most cases. The built-in analyzer includes Arabic stemming together with Arabic normalization, decimal digit normalization, and Arabic stop-word removal. Use the `stemmer` filter directly only when you need to build a custom analyzer pipeline.
...
```cpp
// Sample text to analyze
std::string sample_text = "running runs looked ran runner";
