...
fn generate_dense_vector(dim: usize) -> Vec<f32> {
...
```rust
let query_text = "white headphones, quiet and comfortable";
let query_dense_vector = generate_dense_vector(768);
let query_multimodal_vector = generate_dense_vector(512);

// text semantic search (dense)
let request_1 = SubSearchRequest::builder()
    .vector_field("text_dense")
    .vectors(SearchVectors::Float(vec![query_dense_vector.clone()]))
    .extra_params(HashMap::from([("nprobe".into(), "10".into())]))
    .limit(2)
    .build()?;

// full-text search (sparse)
let request_2 = SubSearchRequest::builder()
...
// text-to-image search (multimodal)
let request_3 = SubSearchRequest::builder()
    .vector_field("image_dense")
    .vectors(SearchVectors::Float(vec![query_multimodal_vector.clone()]))
    .extra_params(HashMap::from([("nprobe".into(), "10".into())]))
    .limit(2)
    .build()?;

let reqs = vec![request_1, request_2, request_3];
```
...
```rust
let ranker = RRFRerank::new().k(100);
```
...
```rust
let tz_req_1 = SubSearchRequest::builder()
    .vector_field("text_dense")
    .vectors(SearchVectors::Float(vec![query_dense_vector.clone()]))
    // highlight-next-line
    .timezone("America/Havana")
...
let tz_req_2 = SubSearchRequest::builder()
    .vector_field("text_sparse")
    .vectors(SearchVectors::EmbeddedText(vec![query_text.into()]))
    // highlight-next-line
    .timezone("America/Havana")
...
let tz_req_3 = SubSearchRequest::builder()
    .vector_field("image_dense")
    .vectors(SearchVectors::Float(vec![query_multimodal_vector.clone()]))
    // highlight-next-line
    .timezone("America/Havana")
...
let tz_response = client
    .hybrid_search(
        HybridSearchRequest::builder()
            .collection_name("my_collection")
            .sub_requests(vec![tz_req_1, tz_req_2, tz_req_3])
            .rerank(ranker.clone())
            .limit(2)
            .build()?,
    )
