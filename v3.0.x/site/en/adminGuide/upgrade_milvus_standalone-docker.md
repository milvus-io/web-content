...
This guide describes how to upgrade a Milvus 2.6.x standalone deployment to v3.0.2 with Docker Compose.

<div class="alert note">

This procedure has been validated with the official Milvus 2.6.20 standalone Docker Compose configuration. The upgrade retained etcd, MinIO, Woodpecker, and the existing data directories, and changed only the Milvus image to `milvusdb/milvus:v3.0.2`.
...
Verify that the `standalone` service is healthy, its image is `milvusdb/milvus:v3.0.2`, and the existing collections remain queryable and searchable. Complete these checks before you enable any v3.0.2-specific feature.
