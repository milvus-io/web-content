...
**Default deployment (v3.0.2):** `docker compose up -d` starts three containers — `milvus-etcd` (metadata), `milvus-minio` (object storage), and `milvus-standalone`. The message queue is **Woodpecker (embedded, with MinIO / object storage as its WAL backend)**, so no separate message-queue container is required.
...
Always download the latest Docker Compose configuration to ensure compatibility with v3.0.2 features.
