...
| Woodpecker | Bundled with Milvus (service mode: `v0.1.34`+) | Default message queue. For distributed deployments, Woodpecker can run as a dedicated **service**; pin its version with `--set woodpecker.image.tag`. Service mode is supported from Woodpecker `v0.1.34` onward. |
