...
`segment_row_limit` 的值决定数据段文件能高能存储的 Entity 的行数。你可以在创建集合时设置 `segment_row_limit`。 `segment_row_limit` 的默认值为 512 &times; 1024 行，最高可设为 4 &times; 1024 &times; 1024 行。 该参数越大，将文件合并到该值设定的大小所需的次数就越多，影响插入操作的吞吐量。该参数越小，则产生的数据段越多，查询性能可能会变差。
