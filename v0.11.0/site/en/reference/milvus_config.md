...
| `preload_collection`  | A comma-separated list of collection names to load when Milvus server starts up. <ul><li>`'*'` means preload all existing tables.For example: `preload_collection=*`.</li><li>To load specific collections, list the collection names that need to be loaded and separate adjacent collections with a comma. For example: `preload_collection=collection1,collection2`.</li></ul>  | StringList | N/A   |
