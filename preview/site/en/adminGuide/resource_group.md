...
In Milvus, you can use a resource group to physically isolate certain query nodes from others. This guide walks you through how to create and manage custom resource groups as well as move nodes between them.
...
3. Describe a resource group.
    ...
    #        <capacity:0>,            // int, num_node which has been transfer to this rg
...
5. Load collections and partitions to a resource group.
    ...
    # Or, you can ask Milvus to load the collection to the desired resource group.
    resource_groups = ['rg']
...
6. Move replicas between resource groups.
...
7. Drop a resource group.
    ...
    You can drop a resource group at any time only if there are no query nodes in the resource group. In this guide, resource group `rg` now has one query node. You need to move it to another resource group before you can drop this resource group.
