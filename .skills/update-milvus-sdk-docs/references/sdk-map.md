...
- **Page mapping**:
  ...
  - **C++ groups related types in ONE page**: when a header declares a primary
    type plus related classes (e.g. `Function.h` has `Function` +
    `RRFRerank`/`WeightedRerank`/`BoostRerank`/`DecayRerank`/`ModelRerank`;
    `FunctionChain.h` has `FunctionChain` + `FunctionChainStage`/`ColumnRef`/
    `ExprArg`/`Expr`/`Op`), document them all in **one `<Type>.md` page with a
    full `##` section per related type** — NOT separate pages and NOT a brief
    one-line list. `Collections/Function.md` is the precedent.
...
- **Extraction commands**:
  ...
  # IMPORTANT: also include the OTHER base headers the class inherits from
  # (RequestBase.h, CollectionRequestBase.h, SearchRequestBase.h,
  # IndexRequestBase.h, ...), not just the three shown here — omit one and
  # methods from it will falsely look "missing" on the page.
...
- **Type page layout**: related-type clusters go in a subdirectory named after
  the primary type (`Vector/FunctionScore/FunctionScore.md`,
  `Vector/Highlighter/Highlighter.md`, `Collections/Function/`); single types are
  flat (`Vector/DataType.md`). New type clusters (e.g. `FunctionChain` +
  `FunctionChainArg/Expr/Op/Stage`) must follow this subdirectory convention.
- **Legacy enum values**: `DataType.String` (value 20) exists in the SDK but is
  legacy/internal — do **not** document it; readers should use `DataType.VarChar`.
- **Tags**: plain `vX.Y.Z` (line `v3.0.x`; resolve the latest tag dynamically
  rather than hard-coding a patch number here).
...
- **Tags**: Go SDK releases are tagged `client/vX.Y.Z` inside the milvus repo
  ...
  git -C sdk-tmp/sdks/milvus show client/v2.6.5:client/milvusclient/collection.go
  ```

  When resolving "latest", prefer **stable** tags over pre-releases (beta/rc);
  if the newest tag for a line is pre-release, ask the user before documenting it.
  Also verify any `client/vX.Y.Z` link in `About.md` actually resolves — links to
  nonexistent tags (e.g. a stable `client/v3.0.0` that was never cut) are a
...
- **Public API surface**: `Milvus.Client/` (namespace `Milvus.Client`).
  A **newer `Milvus.Client.V2/` implementation** (in-repo) is being prepared as
  the basis for the next csharp versions (2.6.x / 3.x) — future csharp docs
  should be written from `Milvus.Client.V2/` (`MilvusClientV2.*.cs`), not the
  legacy `Milvus.Client/` V1 API.
...
- **SDK repo**: `milvus-sdk-rust` (cloned into `sdk-tmp/sdks/milvus-sdk-rust/`)
- **Online URL**: `https://milvus.io/api-reference/rust/<version-line>/` (route `rust`; no doc tree yet — bootstrap)
- **Public API surface**: crate `milvus-sdk-rust`, `src/v2/` (`ClientV2` at
  `src/v2/client.rs`; types/requests under `src/v2/`); examples under `examples/v2/`.
- **Doc tree**: none yet — created via the **New SDK bootstrap mode** using
  `milvus-sdk-cpp` or `milvus-sdk-java` as the structure template.
- **Tags**: plain `vX.Y.Z` (e.g. `v3.0.1`, `v3.0.2`, `v2.6.1`).
...
- **Version pins**: `About.md` compatibility table + `cargo add milvus-sdk-rust@<ver>`.
- **Extraction commands**:
